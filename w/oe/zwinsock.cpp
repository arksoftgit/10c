                    ...
                    zVIEW    lpView )
{
   ...
   zstrcpy( szServerAddress, pchAddress );
   ...
   while ( TRUE )
   {
      ...
      if ( lEnd - lStart > 5 * zTICKS_PER_SECOND )
      {
         ...
         zsprintf( sz, "%s took %lf seconds.", szServerAddress,
      ...
      zsprintf( szMsg, "Couldn't resolve network address '%s'.  Make sure the server "
   ...
   while ( TRUE )
   {
      ...
      zsprintf( szMsg, "Connection to server '%s' failed.  Make sure the server "
   ...
   zstrcpy( lpConn->pchPeerName, szServerAddress );
...
typedef zSOCKTASK zFAR *LPSOCKTASK;
...
// LPTASK             lpTask = (LPTASK) zGETPTR( pSockTask->hTask );
   ...
   if ( DoSync == 0 )
   {
      ...
      SysReadZeidonIni( -1, "[zWinSock]", "SyncServerAccess", szBuf );
      DoSync = atoi( szBuf );
      if ( DoSync == 0 )
   ...
   else
   {
      ...
      catch ( ... )
      {
         ...
         UfFormatDateTime( szDateTime, szSysDateTime, "YYYY-MM-DD HH:MI:SS" );
...
// SOCKET       NewSocket;
   ...
   if ( g_nListenPort == -1 )
   {
      SysReadZeidonIni( -1, "[zWinSock]", "ListenPort", szPort );
                    ...
                    zPCHAR  pchHostAddress )
{
   ...
   if ( ppvConnPtr && cHost == 'R' )
   {
      ...
      if ( lpConn->pchPeerName == 0 )
      {
         ...
         zstrcpy( lpConn->pchPeerName, pHost->h_name );
      }

      zstrcpy( pchHostAddress, lpConn->pchPeerName );
   }
   else
   {
...
zNetStart( zPCHAR pchNetworkName, zPPVOID  ppHandle )
{
   ...
   *ppHandle = 0;
   ...
   if ( (status = WSAStartup( 0x0101, &WSAData )) != 0 )
   {
      ...
      sprintf( szTemp, "Error loading starting WinSock: %d", status );
...
// TraceLineX( "(zwinsock) Task ID             = ", (zLONG) SysGetTaskID( ) );
   ...
   *ppHandle = (zPCHAR) hMemHandle;
   ...
   zstrcpy( lpSockets->szNetworkName, pchNetworkName );
...
GetDirectoryList( zPVIEW pvList, zVIEW vAppList )
{
   ...
   GetStringFromAttribute( szFileName, vAppList, "APPLICATION", "APP_ADOBIN" );
   ...
   if ( hFind != INVALID_HANDLE_VALUE )
   {
      do
      {
         if ( ( FileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) == 0 )
         {
            ...
            zsprintf( szTimestamp, "%d%02d%02d%02d%02d%02d%d",
...
CompareFiles( zPVIEW pvList )
{
   ...
   GetStringFromAttribute( szFileName, vAppList, "APPLICATION", "APP_NAME" );
   if ( lMaxTotalFileSize == 0 )
   {
      zCHAR szTemp[ 100 ];

      SysReadZeidonIni( -1, szFileName, "MaxTotalFileSize", szTemp );
      if ( *szTemp )
   ...
   zstrcat( szFileName, "_ExecutableSourceList" );
   ...
   while ( nServerRC >= zCURSOR_SET )
   {
      ...
      {
         GetStringFromAttribute( szServerFile, vServerList, "File", "FileName" );
         GetStringFromAttribute( szClientFile, vClientList, "File", "FileName" );
      ...
      {
         // File mod dates match so we don't have to do anything.  Go on to
         // next files.
         nClientRC = SetCursorNextEntity( vClientList, "File", 0 );
      ...
      GetStringFromAttribute( &szFileName[ iNameIdx ],
                              vClientList, "File", "FileName" );
      SetAttributeFromString( vClientList, "File", "Updated", "Y" );
      SetAttributeFromAttribute( vClientList, "File", "ModDate",
                                 vServerList, "File", "ModDate" );
...
PerformApplicationUpdate( zVIEW lpView )
{
         ...
         nRC = SetCursorNextEntity( vAppList, "APPLICATION", 0 ) )
   {
      ...
      // then skip it.
      SysReadZeidonIni( -1, pchAppName, "SourceServer", szSourceServer );
      ...
      do
      {
         ...
         GetStringFromAttribute( szFileName, vAppList, "APPLICATION", "APP_ADOBIN" );
         SysAppendcDirSep( szFileName );
         iFileNameIdx = zstrlen( szFileName );
               ...
               nRC = SetCursorNextEntity( vList, "File", 0 ) )
         {
            ...
            GetStringFromAttribute( &szFileName[ iFileNameIdx ],
                                    vList, "File", "FileName" );
            TraceLineS( "FileName: ", szFileName );
            WriteBlobToFile( vList, "File", "Data", szFileName );

            GetStringFromAttribute( szFileDateTime, vList, "File", "ModDate" );
