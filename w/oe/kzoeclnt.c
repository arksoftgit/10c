...
fnTrace( zSHORT nThreadNumber, char *pchMsg )
{
   ...
   zsprintf( szMsg, "%d: %s", nThreadNumber, pchMsg );
...
fnThreadProc2( zPVOID p )
{
   ...
   // TRACE( nThreadNumber, "Mutex locked" );
      ...
      zsprintf( szMsg, "Thread test #%d, rc = %d", nCount++, nRC );
...
TimerProc( HWND hWnd, UINT uMsg, UINT idEvent, DWORD dwTime )
{
   ...
   zsprintf( szMsg, "Done with test %ld", g_lTestCount++ );
   TRACE( 0, szMsg );
              ...
              LPSTR lpCmdLine )     // Command line
{
   ...
   ShowWindow( hMainWnd, SW_SHOW );  // Show the window
   {
      ...
      zstrcpy( psz, "laskjdflkasjdflkjasdlfkjasdlkfj" );
      SysFreeMemory( h );
   }
