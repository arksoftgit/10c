...
1999.09.24  DGC  10a
   Fixed bug from previous change--we now drop the mutex if the close is
   aborted.
   ...
   switch ( uMsg )
   {
      ...
      // TraceLine( "WM_TIMER !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! %d  Wnd: 0x%08x", uIdTimer, hWnd );
         ...
         // TraceLine( "App idle for count %d", lCount );
            ...
            SysGetDateTimeDifference( &lDiffSeconds,
                                      g_szTimeStamp,
                                      szDateTime, zDT_SECOND );
            if ( g_uKillTimerId == 0 && lDiffSeconds > g_lTimeOut )
            {
               ...
               g_uKillTimerId = SetTimer( hWnd, g_nIdKillEvt,
                                          60000, 0 );  // 60000 ==> one minute
         ...
         int nCmdShow )
{
   ...
   if ( (hMutex2 = OpenMutex( MUTEX_ALL_ACCESS,
                              FALSE, "KZOENGWA - INIT 2!!!" )) != 0 )
   {
      ...
      SysMessageBox( 0, "Zeidon Object Engine",
                     "Only one instance of KZOENGWA can exist at once.", 1 );
   ...
   SysReadZeidonIni( -1, "[ObjectEngine]", "TimeOut", szWork );
   if ( szWork[ 0 ] && (g_lTimeOut = zatol( szWork )) != 0 )
   {
      SysReadZeidonIni( -1, "[ObjectEngine]", "KillZeidon", szWork );
      if ( szWork[ 0 ] )
         SysGetDateTime( g_szTimeStamp );
      else
      {
         MessageBox( 0, "TimeOut requires KillZeidon specification",
                     "Object Services", MB_OK );
         g_lTimeOut = 0;
      }
   }
...
WriteTraceToLog( LPTASK lpTask )
{
   ...
   for ( k = 0; k < g_nTraceWindowCnt; k++ )
   {
      ...
      wItemCnt = (WORD) SendMessage( g_hwndTraceWindow[ idx ],
                                     LB_GETCOUNT, 0, 0L );
      for ( wItem = 0; wItem < wItemCnt; wItem++ )
      {
         SendMessage( g_hwndTraceWindow[ idx ], LB_GETTEXT, wItem,
                      (LONG)(LPSTR) szTraceText );
...
//       TraceLineI( "OE WM_COMMAND wCommand: ", wCommand );
         ...
         {
            DialogBox( hInst, MAKEINTRESOURCE( IDD_LOGTRACE ),
                       hWnd, fpLogTrace );
         }
         else
         if ( wCommand == IDM_SHOWDLL )
         {
            DialogBox( hInst, MAKEINTRESOURCE( IDD_SHOWDLL ),
                       hWnd, fpShowDLL );
         }
         else
         if ( wCommand == IDM_LINELIMIT )
         {
            DialogBox( hInst, MAKEINTRESOURCE( IDD_LINELIMIT ),
                       hWnd, fpLineLimit );
         ...
         if ( wCommand == IDM_FINDSTRING ||
              ...
              wCommand == IDM_FINDPREV )
         {
            if ( wCommand == IDM_FINDSTRING )
            {
               if ( DialogBox( hInst, MAKEINTRESOURCE( IDD_FINDSTRING ),
                               hWnd, fpFindString ) != IDOK )
               {
                  break;
               }
            }
            ...
            else
            {
               ...
               zstrcpy( szFindString, g_szFindString );
               ...
               if ( nFound == 0 )
               {
                  zstrcpy( szText, "Search string not found - " );
                  zstrcat( szText, g_szFindString );
                  MessageBox( hWnd, szText, "Zeidon Object Engine",
         ...
         if ( wCommand == IDM_COPYSTRING || wCommand == IDM_CUTSTRING )
         {
            ...
            else
            {
               ...
               SfAllocTaskMemory( g_vSystemTaskView,
                                  (zCOREMEM) &pchMem, lTextLth + 1 );
               SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                            LB_GETTEXT, wCurSel, (LPARAM) pchMem );
               pchMem[ sizeof( g_szCopyBuffer ) - 1 ] = 0;
               strcpy( g_szCopyBuffer, pchMem );
            ...
            {
               SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                            LB_DELETESTRING, wCurSel, 0L );
               SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                            LB_SETCURSEL, wCurSel, 0L );
            }
         }
         else
         ...
         {
            WORD wCurSel =
               (WORD) SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                                   LB_GETCURSEL, 0, 0 );
            SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                         LB_INSERTSTRING,
                         wCurSel + 1, (LPARAM) g_szCopyBuffer );
            SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
         ...
         if ( wCommand == IDM_INSERTMARKER )
         {
            ...
            SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                         LB_INSERTSTRING,
                         wCurSel + 1, (LPARAM) szMark );
            SendMessage( g_hwndTraceWindow[ g_nTraceWindowDisplayed ],
                         LB_SETCURSEL, wCurSel + 1, 0L );
         }
         else
         if ( wCommand == IDM_PRINTPREVIEW    ||
              wCommand == IDM_PREVIEW_ZOOMOUT ||
              wCommand == IDM_PREVIEW_ZOOM    ||
              wCommand == IDM_PREVIEW_ZOOMIN )
         {
            HMENU hMenu = GetMenu( hWnd );

            CheckMenuItem( hMenu, IDM_PRINTPREVIEW,
                           MF_BYCOMMAND | MF_UNCHECKED );
            CheckMenuItem( hMenu, IDM_PREVIEW_ZOOMOUT,
                           MF_BYCOMMAND | MF_UNCHECKED );
            CheckMenuItem( hMenu, IDM_PREVIEW_ZOOM,
                           MF_BYCOMMAND | MF_UNCHECKED );
            CheckMenuItem( hMenu, IDM_PREVIEW_ZOOMIN,
                           MF_BYCOMMAND | MF_UNCHECKED );
            switch ( wCommand )
            {
            ...
            {
               g_chPrintDialog = 0;
               CheckMenuItem( hMenu, IDM_PRINTDIALOG,
                              MF_BYCOMMAND | MF_UNCHECKED );
            }
         }
         else
         if ( wCommand == IDM_PRINTDIALOG )
         {
            ...
            CheckMenuItem( hMenu, IDM_PRINTDIALOG,
                           MF_BYCOMMAND |
                             (g_chPrintDialog ? MF_CHECKED : MF_UNCHECKED) );
         ...
         if ( wCommand == IDM_SHOWTASKID )
         {
            ...
            if ( AnchorBlock->bShowTaskID )
            {
               ...
               CheckMenuItem( hMenu, IDM_SHOWTASKID,
                              MF_BYCOMMAND | MF_UNCHECKED );
            }
            else
            {
         ...
         if ( wCommand == IDM_SHOWINTL )
         {
            ...
            if ( lpIntl )
            {
               ...
               TraceLineS( "(wa) No international information ",
                           "is available" );
            break;
         }
         else
         ...
         {
            DialogBox( hInst, MAKEINTRESOURCE( IDD_SETTRACE ),
                       hWnd, fpSetTrace );
         }
         else
         if ( wCommand == IDM_SETOETRACE )   // Set OE Warning trace
         {
            ...
            CheckMenuItem( GetMenu( hWnd ), IDM_SETOETRACE,
                           MF_BYCOMMAND |
                           (g_chTraceOE_Warning == 0 ? MF_UNCHECKED :
                                                       MF_CHECKED) );
         }
         else
         if ( wCommand == IDM_SETZDRTRACE )   // Set ZDr Action trace
         {
            ...
            CheckMenuItem( GetMenu( hWnd ), IDM_SETZDRTRACE,
                           MF_BYCOMMAND |
                           (g_chTraceZDrActions == 0 ? MF_UNCHECKED :
                                                       MF_CHECKED) );
         }
         else
         if ( wCommand == IDM_SETWEBTRACE )   // Set ZDr Action trace
         {
            ...
            CheckMenuItem( GetMenu( hWnd ), IDM_SETWEBTRACE,
                           MF_BYCOMMAND |
                           (g_chTraceWebActions == 0 ? MF_UNCHECKED :
                                                       MF_CHECKED) );
         }
         ...
         if ( wCommand == IDM_SETMEMORYTRACE )   // Set Memory trace
         {
            ...
            CheckMenuItem( GetMenu( hWnd ), IDM_SETMEMORYTRACE,
                           MF_BYCOMMAND |
                           (g_chTraceMemory == 0 ? MF_UNCHECKED :
                                                   MF_CHECKED) );
         }
         else
         ...
         if ( wCommand == IDM_SCROLLTRACE )   // Scroll trace
         {
            ...
            CheckMenuItem( GetMenu( hWnd ), IDM_SCROLLTRACE,
                           MF_BYCOMMAND |
                              (g_chScrollTrace ? MF_CHECKED : MF_UNCHECKED) );
         }
         else
         ...
         if ( wCommand == IDM_LISTTASKS )   // Display task information
         {
            ...
            // zstrcpy( szMessage, "System Memory Available:" );
            // zltoa( lFree, szMessage + 200 );
            // zstrcat( szMessage, szMessage + 201 );
            // zstrcat( szMessage, "K" );
            // SysMessageList( szMessage );
               ...
               zstrcpy( szMessage, "Anchor Block  Alloc: " );
               ulUsed = ulAlloc - ulFree;
               zltoa( ulAlloc, szMessage + 200 );
               zstrcat( szMessage, szMessage + 200 );
               zstrcat( szMessage, "   Used: " );
               zltoa( ulUsed, szMessage + 200 );
               zstrcat( szMessage, szMessage + 200 );
               zstrcat( szMessage, "   Free: " );
               zltoa( ulFree, szMessage + 200 );
               zstrcat( szMessage, szMessage + 200 );
               SysMessageList( szMessage );
#endif
            ...
            // fnStartBrowseOfTaskList( -1, FALSE );  should do this, but can't
               ...
               // LPOPENFILE   lpOpenFile;
                  ...
                  zstrcat( szMessage, ") - Status:" );

                  // Make sure tasks window is still active.
                  nError = 0;
                  if ( lpTask->bShutdown )
                  {
                     zstrcat( szMessage, "SHUTDOWN" );
                  }
                  else
               // if ( IsWindow( (HWND) lpTask->hMainWnd ) )
                  {
                     zstrcat( szMessage, "OK" );
                  }
#if 0
                  else
                  {
...
#endif
                     {
                        zstrcat( szMessage, "INVALID WINDOW HANDLE!" );
                     }
                     else
                     {
                        zstrcat( szMessage, "DELETED" );
                        nError = 1;
                     }
                  }
#endif
                  ...
                  SysMessageList( szMessage );
                  zstrcpy( szMessage, "      Title: '" );
                  if ( lpTask->szTaskTitle[ 0 ] )
                     zstrcat( szMessage, lpTask->szTaskTitle );

                  zstrcat( szMessage, "'" );

                  if ( lpTask->szUserId[ 0 ] )
                  {
                     zstrcat( szMessage, "   UserId: " );
                     zstrcat( szMessage, lpTask->szUserId );
                  }
                  ...
                  else
                  {
                     ...
                     while ( lpSubtask )
                     {
                        ...
                        {
                           zstrcat( szMessage,
                                    zGETPTR( lpSubtaskView->hViewName ) );
                           zstrcat( szMessage, "'" );
                        }
                        else
                           zstrcat( szMessage, "(unnamed)'" );

                        zstrcat( szMessage, ", Application:'" );
                        zstrcat( szMessage, zGETPTR( lpTempApp->hName ) );
                        zstrcat( szMessage, "'" );
                        SysMessageList( szMessage );
                        ...
                        while( lpSubtaskView )
                        {
                           ...
                           if ( lpSubtaskView->hSubtaskView == 0 )
                           {
                              ...
                              {
                                 LPVIEWCSR lpViewCsr =
                                          zGETPTR( lpSubtaskView->hViewCsr );
                                 LPVIEWOI  lpViewOI  =
                                          zGETPTR( lpViewCsr->hViewOI );
                                 LPVIEWOD  lpViewOD  =
                                          zGETPTR( lpViewOI->hViewOD );

                                 zstrcat( szMessage, ", Object:'" );
                                 zstrcat( szMessage,
                                          lpViewOD->szName );
                                 zstrcat( szMessage, "'" );
                              }
                     ...
                     zstrcpy( szMessage, "      Memory Alloc:" );
                     lUsed = lAlloc - lFree;
                     zltoa( lAlloc, szMessage + 200 );
                     zstrcat( szMessage, szMessage + 200 );
                     zstrcat( szMessage, ",  Used:" );
                     zltoa( lUsed, szMessage + 200 );
                     zstrcat( szMessage, szMessage + 200 );
                     zstrcat( szMessage, ",  Free:" );
                     zltoa( lFree, szMessage + 200 );
                     zstrcat( szMessage, szMessage + 200 );
                        ...
                        lpTaskDomain = zGETPTR( lpTaskDomain->hNextTaskDomainForTask ) )
                  {
                     ...
                     if ( lpDomain )
                     {
                        ...
                        zstrcpy( szMessage, "         Domain Name : " );
                        zstrcat( szMessage, lpDomain->szName );
                        SysMessageList( szMessage );
                        if ( *lpDomain->szDomainOper )
                        {
                           zstrcpy( szMessage, "         Domain Oper : " );
                           zstrcat( szMessage, lpDomain->szDomainOper );
                           SysMessageList( szMessage );
                        }

                        zltoa( lpDomain->lApDmTok, sz );
                        zstrcpy( szMessage, "         Domain Token: " );
                        zstrcat( szMessage, sz );
                        SysMessageList( szMessage );
                        zltoa( (zLONG) lpDomain, sz );
                        zstrcpy( szMessage, "         Domain Ptr  : " );
                        zstrcat( szMessage, sz );
                        SysMessageList( szMessage );
                        SysMessageList( " " );
                     }
         ...
         if ( wCommand == IDM_LISTVOBJS )   // Display objects
         {
            if ( AnchorBlock )
            {
               ...
               while ( lpApp )
               {
                  zstrcpy( szMessage, "Application:'" );
                  if ( *lpApp->szName )
                     zstrcat( szMessage, lpApp->szName );
                  ...
                  while ( lpViewOD )
                  {
                     zstrcpy( szMessage, "   View Object:'" );
                     if ( *lpViewOD->szName )
                        zstrcat( szMessage, lpViewOD->szName );
         ...
         if ( wCommand == IDM_IDLE )
         {
            if ( g_lTimeOut && g_uIdleTimerId == 0 )
            {
               ...
               g_uIdleTimerId = SetTimer( hWnd, g_nIdIdleEvt,
                                          30000, 0 );  // 30000 ==> 30 seconds
            }
         ...
         {
            zstrcpy( (zPCHAR) lParam, g_szFindString );
         ...
         if ( wCommand >= IDM_TRACEWINDOW01 && wCommand <= IDM_TRACEWINDOW20 )
         {
            ...
            nNewTraceWindow = (g_nTraceWindowHead + g_nTraceWindowCnt - idx)
                                                          % g_nTraceWindowCnt;

            // If user selected the window already displayed then don't do
            // anything.
            if ( nNewTraceWindow == g_nTraceWindowDisplayed )
               break;

            // Find the difference between the displayed window and the head.
            iDiff = ( g_nTraceWindowDisplayed + g_nTraceWindowCnt -
                                    g_nTraceWindowHead ) % g_nTraceWindowCnt;

            // Determine the old trace window index.
            nOldTraceWindow = ( g_nTraceWindowCnt - iDiff )
                                                          % g_nTraceWindowCnt;

            // Turn off the check mark...we will no longer be displaying the
            // current window.
            CheckMenuItem( GetMenu( hWnd ),
                           IDM_TRACEWINDOW01 + nOldTraceWindow,
                           MF_BYCOMMAND | MF_UNCHECKED );
            ...
            CheckMenuItem( GetMenu( hWnd ), wCommand,
                           MF_BYCOMMAND | MF_CHECKED );
      ...
      case WM_CLOSE:
         ...
         if ( AnchorBlock )       // Anchor block OK?
         {
            ...
            // SysReadZeidonIni( -1, szlWorkstation, "LocalDir", szTempName );
               ...
               zstrcat( szIniName, "\\zeidon.ini" );
               GetPrivateProfileString( "Workstation", "LocalDir", "",
                                        szTempName, sizeof( szTempName ), szIniName );
               SysConvertEnvironmentString( szFileName, szTempName );
               if ( szFileName[ 0 ] )
               {
                  ...
                  zstrcpy( szFileName, szIniName );

               WritePrivateProfileString( "ObjectEngine", // section name
                                          "WindowPos",    // key name
                                          szBuffer,       // string to add
                                          szFileName );   // ini filename
               zltoa( g_uTraceLineLimit, szBuffer );
               WritePrivateProfileString( "ObjectEngine", "LineLimit",
                                          szBuffer, szFileName );
               WritePrivateProfileString( "ObjectEngine", "FindString",
                                          g_szFindString, szFileName );
               szBuffer[ 1 ] = 0;
               szBuffer[ 0 ] = g_chScrollTrace;
               WritePrivateProfileString( "ObjectEngine", "ScrollTrace",
                                          szBuffer, szFileName );
               szBuffer[ 0 ] = g_chPrintPreview;
               WritePrivateProfileString( "ObjectEngine", "PrintPreview",
                                          szBuffer, szFileName );
               szBuffer[ 0 ] = g_chPrintDialog;
               WritePrivateProfileString( "ObjectEngine", "PrintDialog",
                                          szBuffer, szFileName );
               szBuffer[ 0 ] = g_chTraceOE_Warning;
               WritePrivateProfileString( "ObjectEngine", "TraceOE_Warning",
                                          szBuffer, szFileName );
               szBuffer[ 0 ] = g_chTraceZDrActions;
               WritePrivateProfileString( "ObjectEngine", "TraceAction",
                                          szBuffer, szFileName );
               szBuffer[ 0 ] = g_chTraceWebActions;
               WritePrivateProfileString( "ObjectEngine", "TraceWeb",
                                          szBuffer, szFileName );
               ...
               WritePrivateProfileString( "ObjectEngine", "ShowTaskID",
                                          szBuffer, szFileName );
            }
...
#define zZEIDON_POSTED_CLOSE                (zZEIDON_DRIVER_MSG + 2)
               ...
               if ( nResponse == IDYES )
               {
                  ...
                  while ( TRUE )
                  {
                     ...
                     if ( zGETHNDL( lpTask ) != AnchorBlock->hMainTask &&
                          IsWindow( (HWND) lpTask->hMainWnd ) &&
                          SendMessage( (HWND) lpTask->hMainWnd,
                                       zZEIDON_COMMUNICATE_OE, 0, -1L ) == -1 )
                     {
                        nResponse = IDYES;
                     // TraceLineI( "OE IDYES for Task: ", lpTask->lTaskID );

                        PostMessage( (HWND) lpTask->hMainWnd,
                                     zZEIDON_POSTED_CLOSE, 0, 0L );
                     }
                  // else
            ...
            // If an attempt was made to close applications, then
            // see if all applications have left.
            lpTask = zGETPTR( AnchorBlock->hFirstTask );
            if ( nResponse == IDYES )
            {
...
// Stay up!    PostMessage( hWnd, WM_CLOSE, 0, 0L );
      ...
      case WM_WININICHANGE:
         ...
         if ( lParam == 0L || zstrcmpi( szlINTL, (zPCHAR) lParam ) == 0 )
         {
            ...
            {
               TraceLineS( "(wa) changing international info - ",
                           (zPCHAR) lParam );
      ...
      case WM_WRITELOG:
         {
            ...
            sprintf( szFileName, "%04d%02d%02d%02d%02d%02d.log",
            ...
            {
               if ( (g_hLogFile = fnSysOpenFile( lpTask, szFileName,
                                                 COREFILE_CREATE )) != -1 )
               {
                  char szTraceText[ 512 ];

                  // Send message that log file opened.
                  zstrcpy( szTraceText, "(ng) Log file Opened '" );
                  zstrcat( szTraceText, szFileName );
                  zstrcat( szTraceText, "'" );
...
// FARPROC      lpFarProc[ 2 ];
   ...
   // First, create the listbox that will receive TraceLine messages from
   // core.
   g_hWndLB = CreateWindow(
      "ListBox",                      // see RegisterClass( ) call
      "Trace Window",                 // text for window title bar
      WS_CHILD | WS_VSCROLL | WS_HSCROLL |
         WS_CAPTION | LBS_NOINTEGRALHEIGHT,
   ...
   g_nTraceWindowCnt = GetPrivateProfileInt( "ObjectEngine", "TraceWindows",
                                             2, szFileName );
   ...
   for ( i = 0; i < g_nTraceWindowCnt; i++ )
   {
      ...
      SendMessage( g_hwndTraceWindow[ i ], WM_SETFONT,
      ...
      SendMessage( g_hwndTraceWindow[ i ], LB_SETHORIZONTALEXTENT,
                   txtm.tmMaxCharWidth * 256, 0L );
   }
   ...
   CheckMenuItem( hMenu, IDM_TRACEWINDOW01,
                  MF_BYCOMMAND | MF_CHECKED );

   // Make proc instances for message procedures
   fpAbout = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) AboutDlgProc, hInstance );
   fpLogon = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) LogonDlgProc, hInstance );
   fpSetTrace = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) SetTraceDlgProc, hInstance );
   fpLogTrace = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) LogTraceDlgProc, hInstance );
   fpTraceList = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) TraceListWndProc, hInstance );
   fpLineLimit = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) LimitDlgProc, hInstance );
   fpFindString = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) FindStringDlgProc, hInstance );
   fpShowDLL = (zFARPROC_CALLBACK)
      MakeProcInstance( (FARPROC) ShowDLLDlgProc, hInstance );

   // Change the WndProc for the list window so we can trap LB_ADDSTRING
   // messages and write them to the trace log (if open)
   fpDfltListWndProc = (zFARPROC_CALLBACK)
      SetWindowLong( g_hWndLB, GWL_WNDPROC, (DWORD) TraceListWndProc );
...
#ifdef __BORLANDC__
   // start address, stack size, argument, security, initflag, thread address
   // unsigned long _RTLENTRY _EXPFUNC
   // _beginthreadNT( void (_USERENTRY *__start)(void *),
   ...
   if ( szFileName[ 0 ] )
   {
      ...
      GetPrivateProfileString( "ObjectEngine", "LineLimit", "",
      ...
      GetPrivateProfileString( "ObjectEngine", "FindString", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      zstrcpy( g_szFindString, szBuffer );
      GetPrivateProfileString( "ObjectEngine", "ScrollTrace", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
         g_chScrollTrace = szBuffer[ 0 ];

      GetPrivateProfileString( "ObjectEngine", "PrintPreview", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
      {
      ...
      GetPrivateProfileString( "ObjectEngine", "PrintDialog", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
         g_chPrintDialog = szBuffer[ 0 ];

      GetPrivateProfileString( "ObjectEngine", "TraceOE_Warning", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
         g_chTraceOE_Warning = szBuffer[ 0 ];

      GetPrivateProfileString( "ObjectEngine", "TraceAction", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
         g_chTraceZDrActions = szBuffer[ 0 ];

      GetPrivateProfileString( "ObjectEngine", "TraceWeb", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
         g_chTraceWebActions = szBuffer[ 0 ];

      GetPrivateProfileString( "ObjectEngine", "ShowTaskID", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      if ( szBuffer[ 0 ] )
         AnchorBlock->bShowTaskID = 1;
      ...
      GetPrivateProfileString( "ObjectEngine", "ShowOpenFile", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      lpTraceFlags->bOpenFile = (toupper( szBuffer[ 0 ] ) == 'Y') ? TRUE : FALSE;

      GetPrivateProfileString( "ObjectEngine", "ShowOI_Times", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      lpTraceFlags->bOI_Times = (toupper( szBuffer[ 0 ] ) == 'Y') ? TRUE : FALSE;

      GetPrivateProfileString( "ObjectEngine", "ShowTaskMsgs", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
      lpTraceFlags->bTaskMsgs = (toupper( szBuffer[ 0 ] ) == 'Y') ? TRUE : FALSE;
   }
   else
   {
      SysReadZeidonIni( -1, "[ObjectEngine]", "LineLimit", szBuffer );
      if ( szBuffer[ 0 ] )
      {
         g_uTraceLineLimit = atoi( szBuffer );
         if ( g_uTraceLineLimit == 0 )
      ...
      SysReadZeidonIni( -1, "[ObjectEngine]", "FindString", szBuffer );
      zstrcpy( g_szFindString, szBuffer );
      SysReadZeidonIni( -1, "[ObjectEngine]", "ScrollTrace", szBuffer );
      if ( szBuffer[ 0 ] )
         g_chScrollTrace = szBuffer[ 0 ];

      SysReadZeidonIni( -1, "[ObjectEngine]", "PrintPreview", szBuffer );
      if ( szBuffer[ 0 ] )
      {
         g_chPrintPreview = szBuffer[ 0 ];
         if ( g_chPrintPreview == 'Y' )
      ...
      SysReadZeidonIni( -1, "[ObjectEngine]", "PrintDialog", szBuffer );
      if ( szBuffer[ 0 ] )
         g_chPrintDialog = szBuffer[ 0 ];

      SysReadZeidonIni( -1, "[ObjectEngine]", "TraceOE_Warning", szBuffer );
      if ( szBuffer[ 0 ] )
         g_chTraceOE_Warning = szBuffer[ 0 ];

      SysReadZeidonIni( -1, "[ObjectEngine]", "TraceAction", szBuffer );
      if ( szBuffer[ 0 ] )
         g_chTraceZDrActions = szBuffer[ 0 ];

      SysReadZeidonIni( -1, "[ObjectEngine]", "TraceWeb", szBuffer );
      if ( szBuffer[ 0 ] )
         g_chTraceWebActions = szBuffer[ 0 ];

      SysReadZeidonIni( -1, "[ObjectEngine]", "ShowTaskID", szBuffer );
      if ( szBuffer[ 0 ] )
         AnchorBlock->bShowTaskID = 1;

      lpTraceFlags = GetTracingFlags( );

      SysReadZeidonIni( -1, "[ObjectEngine]", "ShowOpenFile", szBuffer );
      lpTraceFlags->bOpenFile = (toupper( szBuffer[ 0 ] ) == 'Y') ? TRUE : FALSE;

      SysReadZeidonIni( -1, "[ObjectEngine]", "ShowOI_Times", szBuffer );
      lpTraceFlags->bOI_Times = (toupper( szBuffer[ 0 ] ) == 'Y') ? TRUE : FALSE;

      SysReadZeidonIni( -1, "[ObjectEngine]", "ShowTaskMsgs", szBuffer );
      lpTraceFlags->bTaskMsgs = (toupper( szBuffer[ 0 ] ) == 'Y') ? TRUE : FALSE;
   }

   // Retrieve WindowStartup parameter and show window in hidden,
   // systray or normal mode.
   SysReadZeidonIni( -1, "[ObjectEngine]", "WindowStartup", szBuffer );
   if ( zstricmp( szBuffer, "Hidden" ) == 0 )
   {
      ShowWindow( g_hWndMainFrame, SW_HIDE );
   }
   else
   if ( zstricmp( szBuffer, "SysTray" ) == 0 )
   {
   ...
   SysReadZeidonIni( -1, "ObjectEngine", "DropEngineIfNoTasks", szBuffer );
   if ( ztoupper( *szBuffer ) == 'Y' )
      AnchorBlock->bDropEngineIfNoTasks = TRUE;

   CheckMenuItem( hMenu, IDM_SCROLLTRACE,
                  MF_BYCOMMAND |
                     (g_chScrollTrace ? MF_CHECKED : MF_UNCHECKED) );
   CheckMenuItem( hMenu, IDM_PRINTPREVIEW,
                  MF_BYCOMMAND |
                     (g_chPrintPreview ? MF_UNCHECKED : MF_CHECKED) );
   CheckMenuItem( hMenu, IDM_PREVIEW_ZOOMOUT,
                  MF_BYCOMMAND |
                     (g_chPrintPreview == '0' ? MF_CHECKED : MF_UNCHECKED) );
   CheckMenuItem( hMenu, IDM_PREVIEW_ZOOM,
                  MF_BYCOMMAND |
                     (g_chPrintPreview == '1' ? MF_CHECKED : MF_UNCHECKED) );
   CheckMenuItem( hMenu, IDM_PREVIEW_ZOOMIN,
                  MF_BYCOMMAND |
                     (g_chPrintPreview == '2' ? MF_CHECKED : MF_UNCHECKED) );
   CheckMenuItem( hMenu, IDM_PRINTDIALOG,
                  MF_BYCOMMAND |
                     (g_chPrintDialog ? MF_CHECKED : MF_UNCHECKED) );
   CheckMenuItem( hMenu, IDM_SHOWTASKID,
                  MF_BYCOMMAND |
                    (AnchorBlock->bShowTaskID ? MF_CHECKED : MF_UNCHECKED) );
   CheckMenuItem( hMenu, IDM_SETOETRACE,
                  MF_BYCOMMAND |
                     (g_chTraceOE_Warning == 0 ||
                        g_chTraceOE_Warning == 'n' ||
                        g_chTraceOE_Warning == 'N' ? MF_UNCHECKED :
                                                     MF_CHECKED) );
   CheckMenuItem( hMenu, IDM_SETZDRTRACE,
                  MF_BYCOMMAND |
                     (g_chTraceZDrActions == 0 ||
                        g_chTraceZDrActions == 'n' ||
                        g_chTraceZDrActions == 'N' ? MF_UNCHECKED :
                                                     MF_CHECKED) );
   CheckMenuItem( hMenu, IDM_SETWEBTRACE,
                  MF_BYCOMMAND |
                     (g_chTraceWebActions == 0 ||
                        g_chTraceWebActions == 'n' ||
                        g_chTraceWebActions == 'N' ? MF_UNCHECKED :
                                                     MF_CHECKED) );
   ...
   // Try and retrieve the workstation userid;
   {
      ...
      SysReadZeidonIni( -1, szlWorkstation, szlPassword, szIniPassword );
      szLanUser[ 0 ] = 0;
      if ( (hNetApi = GetModuleHandle( "NETAPI" )) != 0 )
      {
         if ( (lpfnNetWkstaGetInfo =
                      GetProcAddress( hNetApi, "NetWkstaGetInfo" )) != 0 )
         {
            uRC = (*lpfnNetWkstaGetInfo)( (zPCHAR) NULL, (zUSHORT) 10,
                                          ...
                                          (zPUSHORT) &uReturnLth );
            if ( uRC == 0 )
               zstrcpy( szLanUser, Buffer[ 1 ] );
         }
      }

      if ( szLanUser[ 0 ] || szIniUser[ 0 ] )
      {
         SysReadZeidonIni( -1, szlWorkstation, "Encrypted", szEncrypted );

         if ( szIniUser[ 0 ] == 0 )
            zstrcpy( szIniUser, szLanUser );
      ...
      else
      {
         ...
         DialogBox( hInst, MAKEINTRESOURCE( IDD_LOGON ),
                    g_hWndMainFrame, fpLogon );
      }
   }
#endif
   ...
   GetViewByName( &vDbhWork, zDBHANDLER_WORK_VIEW_NAME,
                  g_vSystemTaskView, zLEVEL_SYSTEM );
   if ( vDbhWork == 0 )
   {
      SfActivateSysEmptyOI( &vDbhWork, "KZDBHWOB",
                            g_vSystemTaskView, zSINGLE );
      SfLockView( vDbhWork );
      CreateEntity( vDbhWork, "ROOT", zPOS_AFTER );
      SetNameForView( vDbhWork, zDBHANDLER_WORK_VIEW_NAME,
                      g_vSystemTaskView, zLEVEL_SYSTEM );
   }

   // Just a way to test if we are opening the right INI file.
   SysReadZeidonIni( -1, "[Debug]", "StartMessage", szBuffer );
   if ( szBuffer[ 0 ] )
              ...
              WPARAM wParam, LPARAM lParam )
{
   ...
   switch ( uMsg )
   {
      case WM_COMMAND:
         if ( wParam == IDC_OK )
         {
            GetDlgItemText( hWndDlg, IDC_USERID,
                            szUserID, sizeof( szUserID ) );
            GetDlgItemText( hWndDlg, IDC_PASSWORD,
            ...
            if ( szUserID[ 0 ] )
            {
               ...
               if ( SendDlgItemMessage( hWndDlg, IDC_SAVEUSERID,
                                        BM_GETCHECK, 0, 0L ) )
               {
      ...
      case WM_INITDIALOG:
         SysGetUserID( 0, szUserID, szPassword );
                   ...
                   WPARAM wParam, LPARAM lParam )
{
   switch ( uMsg )
   {
      case WM_COMMAND:
         if ( wParam == IDC_OK )
         {
            GetDlgItemText( hWndDlg, IDC_FINDSTRING,
                            g_szFindString, sizeof( g_szFindString ) );
      ...
      case WM_INITDIALOG:
         SendDlgItemMessage( hWndDlg, IDC_FINDSTRING, EM_LIMITTEXT,
                             sizeof( g_szFindString ), 0L );
      ...
      case WM_COMMAND:
         if ( wParam == IDC_OK )
         {
            ...
            if ( iTraceLevel >= 0 )
            {
               ...
               TraceLineI( "(kzoengwa) DBH Trace Level for all tasks set to ",
                           (zLONG) iTraceLevel );
            }
              ...
              WPARAM wParam, LPARAM lParam )
{
   switch ( uMsg )
   {
      case WM_COMMAND:
         ...
         {
            BOOL nTransFlag;
            g_uTraceLineLimit = GetDlgItemInt( hWndDlg, IDC_LINELIMIT,
                                               &nTransFlag, 0 );
            EndDialog( hWndDlg, TRUE );
         }
         else
      ...
      case WM_INITDIALOG:
         ...
         SetDlgItemInt( hWndDlg, IDC_LINEUSAGE,
                        (WORD) SendMessage( g_hWndLB,
                                            LB_GETCOUNT, 0, 0 ), 0 );
         return( TRUE );
   ...
   switch ( uMsg )
   {
      case WM_COMMAND:
#if 0
         {
            ...
            zstrcpy( szWorkString, "DLLDlg WM_COMMAND  Message " );

            zltoa( (zLONG) (unsigned) uMsg,
                   (szWorkString + zstrlen( szWorkString )) );
            zstrcat( szWorkString, "(" );
            _ltoa( (zLONG) (unsigned) uMsg,
                   (szWorkString + zstrlen( szWorkString )), 16 );

            zstrcat( szWorkString, ") wParam " );
            zltoa( (zLONG) wParam, (szWorkString + zstrlen( szWorkString )) );
            zstrcat( szWorkString, "(" );
            _ltoa( (zLONG) wParam,
                   (szWorkString + zstrlen( szWorkString )), 16 );

            zstrcat( szWorkString, ") lParam " );
            zltoa( lParam, (szWorkString + zstrlen( szWorkString )) );
            zstrcat( szWorkString, "(" );
            _ltoa( lParam, (szWorkString + zstrlen( szWorkString )), 16 );
            zstrcat( szWorkString, ")" );
      ...
      case WM_INITDIALOG:
      {
         ...
         while ( bOkay )
         {
            if ( ge.wType == GT_MODULE )
            {
               ...
               for ( i = 0; i < nCCount; i++ )
                  ModuleName[ i ] = *lpcModuleName++;

               ModuleName[ i ] = 0;

               zstrcpy( szLBItem, ModuleName );
               i = zstrlen( szLBItem );
               while ( i < 9 )
                  szLBItem[ i++ ] = ' ';
               ...
               zltoa( *lpwLoadCount, szLBItem + zstrlen( szLBItem ) );
                  ...
                  WPARAM wParam, LPARAM lParam )
{
   switch ( uMsg )
   {
      ...
      case LB_ADDSTRING:              // Trace going to list box
      {
         ...
         nItemCount = (WORD) SendMessage( g_hwndTraceWindow[ g_nTraceWindowHead ],
         ...
         if ( nItemCount >= g_uTraceLineLimit )
         {
            ...
            SendMessage( g_hwndTraceWindow[ g_nTraceWindowHead ],
                         LB_RESETCONTENT, 0, 0 );
            ...
            else
            {
               ...
               // Find the difference between the displayed window and the head.
               iDiff = (g_nTraceWindowDisplayed + g_nTraceWindowCnt -
                                               nOldHead) % g_nTraceWindowCnt;

               // Determine the old trace window index.
               wCmd = (g_nTraceWindowCnt - iDiff) % g_nTraceWindowCnt +
                                                           IDM_TRACEWINDOW01;

               // Turn off the check mark...we will no longer be displaying the
               // current window.
               CheckMenuItem( GetMenu( g_hWndMainFrame ), wCmd,
                              MF_BYCOMMAND | MF_UNCHECKED );
   ...
   switch ( uMsg )
   {
      case WM_COMMAND:
         if ( wParam == IDC_OPEN ) // open pushbutton
         {
            ...
            if ( (g_hLogFile = fnSysOpenFile( lpTask, szLogFileName,
                                              COREFILE_CREATE )) == -1 )
            {
               MessageBox( GetActiveWindow( ),
            ...
            else
            {
               ...
               // Send message that log file opened.
               zstrcpy( szTraceText, "(ng) Log file Opened '" );
               zstrcat( szTraceText, szLogFileName );
               zstrcat( szTraceText, "'" );
               SysMessageList( szTraceText );
      ...
      case WM_INITDIALOG:
         if ( g_hLogFile != -1 )
         {
            ...
            SendDlgItemMessage( hWndDlg, IDC_CLOSE,
                                BM_SETSTYLE,
         ...
         else
         {
            ...
            SendDlgItemMessage( hWndDlg, IDC_OPEN,
                                BM_SETSTYLE,
                                (WORD) BS_DEFPUSHBUTTON, 1L );
            if ( szLogFileName[ 0 ] == 0 )
            {
               SysReadZeidonIni( -1, szlWorkstation, "LogFile", szLogFileName );
               if ( szLogFileName[ 0 ] == 0 )
               {
                  if ( AnchorBlock->szZeidonLoc[ 0 ] )
                     zstrcpy( szLogFileName, AnchorBlock->szZeidonLoc );
                  else
                     zstrcpy( szLogFileName, "\\" );

                  zstrcat( szLogFileName, "ZEIDON00.LOG" );
               }
            }

            SetWindowText( GetDlgItem( hWndDlg, IDC_LOGFILE ),
                           szLogFileName );
         }
...
ReadWindowPlacement( LPTASK lpTask, LPWINDOWPLACEMENT pwp )
{
   ...
   if ( szFileName[ 0 ] )
   {
      ...
      SysReadZeidonIni( -1, "[ObjectEngine]", "WindowPos", szBuffer );
