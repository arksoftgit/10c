   ...
   else
   {
      ...
      for ( int i = 1; i < argc && nRC == 0; i++ )
      {
         ...
         if ( i < argc )
         {
            ...
            {
               i++;
               if( ::stricmp( argv[i], "XOD" ) == 0 )
                  nGenType = XOD;
               else
               if( ::stricmp( argv[i], "XWD" ) == 0 )
                  nGenType = XWD;
               else
               if( ::stricmp( argv[i], "XLP" ) == 0 )
                  nGenType = XLP;
               else
               if( ::stricmp( argv[i], "XPE" ) == 0 )
                  nGenType = XPE;
               else
               if( ::stricmp( argv[i], "XDM" ) == 0 )
                  nGenType = XDM;
               else
               if( ::stricmp( argv[i], "DTE" ) == 0 )
                  nGenType = DTE;
               else
               if( ::stricmp( argv[i], "XRP" ) == 0 )
                  nGenType = XRP;
               else
                             ...
                             zCPCHAR pszApplicationName )
{
   ...
   if ( nRC == 0 )
   {
      ...
      zVIEW vMSGOBJ;
      if ( GetViewByName( &vMSGOBJ, "__MSGQ",
              *pvApp, zLEVEL_TASK ) != zLEVEL_TASK )
      {
         if ( ActivateEmptyObjectInstance( &vMSGOBJ, "KZMSGQOO", *pvApp,
                                           zMULTIPLE ) >= 0 )
         {
...
int InvokeSystemCommand ( zCPCHAR pszOrder, CFile *fLog )
{
   ...
   if (nRC == -1)
   {
      ...
      cerr << "Error runnig command" << szMsg << endl;
...
int ProcessDTE( zVIEW vSubtask, CFile *fLog )
{
   ...
   */
   ...
   {
      zstrcpy( szMsg, "Error generating dte: Zeidon DTE could not be loaded. Aborting Build!" );
      cerr << szMsg << endl;
   ...
   while (nRC >= zCURSOR_SET)
   {
      GetStringFromAttribute (szDataSource, vDTE, "TE_DBMS_Source", "Name");

      zstrcpy (szMsg, "Deleting/Initializing Tables from Datasource: ");
      zstrcat (szMsg, szDataSource);
      ...
      {
         zstrcpy( szMsg, "Can't add Table Records or Relationships for Remote Servers" );
      ...
      else
      {
         ...
         if ( vEMD )
         {
            ...
            */
            ...
            if ( nRC < 0 )
            {
               zstrcpy( szMsg, "Unable to save Physical Environment. Aborting Build");
...
int ProcessXLP( zVIEW vSubtask, CFile *fLog )
{
   ...
   LPLR_Activated = GetViewByName( &LPLR_View, szLPLR_Name, vZeidonCM,
                                                      zLEVEL_SUBTASK );
   GetStringFromAttribute( szLPLR_FileSpec, vTZCMWKSO, "LPLR", "ExecDir" );
   ...
   if ( LPLR_Activated < 1 ) // LPLR currently not activated
   {
      ...
      else
      {
         ...
         SetAttributeFromAttribute( vTZCMWKSO, "LPLR", "ZKey",
                                    LPLR_View, "LPLR", "ZKey" );
         SetMatchingAttributesByName( LPLR_View, "LPLR", vTZCMWKSO, "LPLR",
                                      zSET_NOTNULL );
   ...
   else  // LPLR is currently active
   {
      ...
      {
         GetIntegerFromAttribute( &TaskUseCount,
                                    LPLR_View, "LPLR", "TaskUseCount" );
         if ( TaskUseCount > 1 )
         {
   ...
   nRC = zgSortEntityWithinParent( zASCENDING, LPLR_View,
                                   "W_MetaType", "Type", "" );
   if ( CommitOI_ToFile( LPLR_View, szLPLR_FileSpec,
                         zSINGLE ) == 0 )
   {
                ...
                zCPCHAR cRemReadOnly)
{
   ...
   if ( *cRemReadOnly == 'Y')
   {
      ...
      */
      ...
      pszPath = getenv ("KZV");
      strcat (szTargetPath, pszPath);
      strcpy (szPPEPath, szTargetPath);
      strcat (szTargetPath, "\\savexpe");
      strcat (szPPEPath, "\\a\\bin\\sys");
...
int ProcessXRP( zVIEW vSubtask, CFile *fLog,
                         zCPCHAR cRemReadOnly)
{
   zVIEW vLPLR;
   zVIEW vReport;
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szReportName,
                              vLPLR, "W_MetaDef", "Name" );
...
KillObjectEngineProcess( )
{
   ...
   */
   ...
   for (i=0; i<numTasks; i++) {
       ...
       if (MatchPattern( tlist[i].WindowTitle, (UCHAR *)"*KILL*" )) {
           continue;
       }

       tname[0] = 0;
       strcpy( tname, (LPCSTR)tlist[i].ProcessName );
       p = strchr( tname, '.' );
       if (p) {
           p[0] = '\0';
       }
       if (MatchPattern( (UCHAR*)tname, (UCHAR*)pname )) {
            tlist[i].flags = TRUE;
       } else if (MatchPattern( tlist[i].ProcessName, (UCHAR*)pname )) {
           tlist[i].flags = TRUE;
       } else if (MatchPattern( tlist[i].WindowTitle, (UCHAR*)pname )) {
           tlist[i].flags = TRUE;
       }
   }
