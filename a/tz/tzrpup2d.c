...
// TraceLineS( "In OpenReportFile", "============== " );
   ...
   GetStringFromAttribute( szSironName, vNewReport, "Report", "SironName" );
...
TZRPUPDD_SortWindows( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSelected, vReport, "Page", "Tag" );
...
TZRPUPDD_AskForSave( zVIEW vSubtask )
{
        ...
        ObjectInstanceUpdatedFromFile( vReport ) == 1 )
   {
      ...
      {
         nSaveAs = TRUE;
         GetStringFromAttribute( szReportName, vReport, "Report", "Tag" );
         zstrcpy( szMessageText, "Report is not checked out, but Report '" );
         zstrcat( szMessageText, szReportName );
...
fnTZRPUPDD_CreateGroupSet( zVIEW vSubtask, zVIEW vNewReport )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      GetStringFromAttribute( szType, vNewReport, "GroupSet", "Type" );
      if ( szType[ 0 ] )
      {
...
TZRPUPDD_NewPage( zVIEW vSubtask )
{
                                                            ...
                                                            zCURSOR_NULL )
   {
      ...
      for ( nIdx = 1; nIdx < 1000; nIdx++ )
      {
         ...
         if ( SetCursorFirstEntityByString( vReport, "Page", "Tag",
                                            szTag, 0 ) < zCURSOR_SET )
         {
            break;
         }
   ...
   {
      GetStringFromAttribute( szTag, vReport, "Report", "Tag" );
   }
   ...
   fnCreateReportMetaEntity( vSubtask, vReport, "PH",
                             zGROUPSET_REPORT, zPOS_FIRST );
   fnCreateReportMetaEntity( vSubtask, vReport, "ga",
                             zGROUP_REPORT, zPOS_AFTER );

   fnCreateReportMetaEntity( vSubtask, vReport, "PF",
                             zGROUPSET_REPORT, zPOS_LAST );
   fnCreateReportMetaEntity( vSubtask, vReport, "ga",
                             zGROUP_REPORT, zPOS_AFTER );
...
TZRPUPDD_NewReportFile( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileName, vPainter,
                           "Palette", "NewFile" );
   if ( szFileName[ 0 ] == 0 )
   {
   ...
   zstrcpy( szReportName, szFileName );
...
TZRPUPDD_DeletePage( zVIEW vSubtask )
{
   ...
   zstrcpy( szMessage, "OK to delete Page '" );
   GetAddrForAttribute( &szTag, vReport, "Page", "Tag" );
   zstrcat( szMessage, szTag );
   zstrcat( szMessage, "'?" );

   if ( MessagePrompt( vSubtask, "RP00202",
                       "Report Maintenance",
                       szMessage,
   ...
   if ( fnPainterCall( zMSG_DELETECURRENTPAINTERWINDOW, vSubtask, 0, 0 ) == 0 )
   {
...
TZRPUPDD_EnablePainterWindows( zVIEW vSubtask )
{
   ...
   return( (zSHORT) fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, "Y" ) );
}

zOPER_EXPORT zSHORT OPERATION
SetToolBox( zVIEW vSubtask )
{
   zLONG lState = GetOptionState( vSubtask, "ToolBox",
                                  zOPTION_STATUS_CHECKED );
   lState = !lState;
   SetOptionState( vSubtask, "ToolBox", zOPTION_STATUS_CHECKED, lState );
// 1998.07.21 RG
// fnPainterCall( zMSG_SETTOOLBOX, vSubtask, 0, (zPCHAR) lState );
...
UPD_GROUP_SetPrintOptions( zVIEW vSubtask )
{
   ...
   SetCtrlState( vSubtask, "RepeatHeader",
                 zCONTROL_STATUS_ENABLED, bRepeatHeader );
   SetCtrlState( vSubtask, "PageBreak",
                 zCONTROL_STATUS_ENABLED, bPageBreak );
   SetCtrlState( vSubtask, "ForcePageBreak",
                 zCONTROL_STATUS_ENABLED, bForcePageBreak );
...
UPD_GROUP_OK( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szType, vReport, "Group", "Type" );

   // Group Type  is required
   if ( zstrcmp( szType, "" ) == 0 )
   {
      zstrcpy( szMsg, "Group Type is required." );
   ...
   if ( zstrcmp( szType, "gh" ) == 0 || zstrcmp( szType, "gf" ) == 0 )
   {
      ...
      if ( nRC >= zCURSOR_SET )
      {
         GetStringFromAttributeByContext( szContext, vReport, "Group", "Type",
                                          "ReportGroupType", 32 );
         zstrcpy( szMsg, szContext );
         zstrcat( szMsg, " already exists in the GroupSet." );
         MessageSend( vSubtask, "ZO00137", "Report Maintenance",
                      szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
         SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
         SetFocusToCtrl( vSubtask, "GroupType" );
   ...
   GetStringFromAttribute( szOverlayTag, vReport, "Group", "OverlayTag" );
   if ( szOverlayTag[ 0 ] )
   {
      zVIEW  vReportTemp;
      zSHORT nRC;
      ...
      nRC = SetCursorFirstEntityByString( vReportTemp, "Group", "Tag",
                                          szOverlayTag, 0 );
...
UPD_GROUPSET_CheckValues( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szType, vReport, "GroupSet", "Type" );

   // GroupSet Type  is required
   if ( zstrcmp( szType, "" ) == 0 )
   {
      zstrcpy( szMsg, "GroupSet Type is required." );
      MessageSend( vSubtask, "ZO00137", "Report Maintenance",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
   ...
   if ( zstrcmp( szType, "PH" ) == 0 || zstrcmp( szType, "PF" ) == 0 )
   {
      ...
      if ( nRC >= zCURSOR_SET )
      {
         ...
         zstrcpy( szMsg, szContext );
         zstrcat( szMsg, " already exists in the Page." );
         MessageSend( vSubtask, "ZO00137", "Report Maintenance",
                      szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
         SetFocusToCtrl( vSubtask, "GroupSetType" );
         DropView( vReportC );
      ...
      if ( SetCursorFirstEntityByString( vReportC, "Group", "Type", "gh",
                                         "" ) >= zCURSOR_SET ||
           SetCursorFirstEntityByString( vReportC, "Group", "Type", "gf",
                                         "" ) >= zCURSOR_SET )
      {
         GetStringFromAttributeByContext( szContext, vReport, "GroupSet", "Type",
                                          "ReportGroupSetType", 32 );
         zstrcpy( szMsg, szContext );
         zstrcat( szMsg, " GroupSet cannot include \na Group of Type " );
         GetStringFromAttributeByContext( szContext, vReportC, "Group", "Type",
                                          "ReportGroupType", 32 );
         zstrcat( szMsg, szContext );
         zstrcat( szMsg, "." );
         MessageSend( vSubtask, "ZO00137", "Report Maintenance",
                      szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
         SetFocusToCtrl( vSubtask, "GroupSetType" );
   ...
   if ( zstrcmp( szType, "E" ) == 0 )
   {
      ...
      nDupCount = 0;
      GetStringFromAttribute( szTag, vReport, "GroupSet", "Tag" );
      CreateViewFromViewForTask( &vReportC, vReport, 0 );
      nRC = SetCursorFirstEntityByString( vReportC, "GroupSet", "Tag",
                                          szTag, "" );
      while ( nRC > zCURSOR_UNCHANGED )
      {

         if ( CompareAttributeToString( vReportC, "GroupSet",
                                        "Type", szType ) == 0 )
         {
            nDupCount++;
         }

         nRC = SetCursorNextEntityByString( vReportC, "GroupSet", "Tag",
                                            szTag, "" );
      }
                            ...
                            zPCHAR szNewName )
{
   ...
   {
      zstrcpy( szMsg, "Report Name is required." );
      MessageSend( vSubtask, "ZO00137", "Report Maintenance",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
   ...
   {
      zstrcpy( szMsg, "Report already exists. Replace existing Report?" );
      ...
      if ( CompareAttributeToString( vPAGEL, "Report",
                                     "Tag", szOutName ) == 0 )
      {
         SetAttributeFromString( vPAGEL, "Report", "Tag", szOutName );
         SetAttributeFromAttribute( vPAGEL, "Report", "Desc",
...
TZRPUP2D_SaveAsReport( zVIEW vSubtask )
{
   ...
   // check Save as Name
   if ( fnTZRPUP2D_SaveAsCheckName( vSubtask, vCM_List, vPAGEL,
                                   szOutName, szNewName ) < 1 )
   {
      return( -1 );
   }
   ...
   fnPainterCall( zMSG_RESTORE_PLACEMENT, vSubtask, 0, (zPCHAR) vProfileXFER );
