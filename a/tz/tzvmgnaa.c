...
zPCHAR szMessageListGenerate[ ] =
{
   ...
   "Application's Global Operation List could not be accessed from"
       "Configuration Management, GenerateTerminating",
             ...
             zLONG   lType )
{
   ...
   nRC = GetStringFromAttribute( szCFileSpec, vTaskLPLR, "LPLR", "PgmSrcDir" );
   SysConvertEnvironmentString( szPgmSrcDir, szCFileSpec );
   ...
   // the File.
   zstrcpy( szCFileSpec, szPgmSrcDir );
   ...
   if ( zstrcmp( szTempCFileSpec, "TZDGSRCO" ) == 0 )
      zstrcpy( szSourceFileEntity, "DomainGroup" );
   else
   {
      if ( zstrcmp( szTempCFileSpec, "TZOGSRCO" ) == 0 )
         zstrcpy( szSourceFileEntity, "GlobalOperationGroup" );
      else
         zstrcpy( szSourceFileEntity, "SourceFile" );
   }

   if ( g_szGenLang[ 0 ] == 'J' )
   {
      /*
      zstrcat( szCFileSpec, "JavaProject\\src\\" );
      nLth += 16;
      GetStringFromAttribute( szCFileSpec, vTaskLPLR, "LPLR", "JavaPackageName" );
      GetStringFromAttribute( szCFileSpec + nLth, vTaskLPLR, "LPLR", "JavaPackageName" );
      nLth = SysAppendcDirSep( szCFileSpec );
      */
      ...
      zstrcpy( szSystemIniApplName, "[App." );
      zstrcat( szSystemIniApplName, szLPLR_Name );
      zstrcat( szSystemIniApplName, "]" );
      SysReadZeidonIni( -1, szSystemIniApplName, "JavaGenDir", szMsg );
      if ( szMsg[ 0 ] == 0 )
      {
         zstrcpy( szCFileSpec, szPgmSrcDir );
         nLth = SysAppendcDirSep( szCFileSpec );
         zstrcpy( szCFileSpec + nLth, "JavaProject\\src\\com\\quinsoft\\" );
         nLth += 29;
         zstrcpy( szCFileSpec + nLth, szLPLR_Name );
      }
      else
      {
         SysConvertEnvironmentString( szCFileSpec, szMsg );
      }

      nLth = SysAppendcDirSep( szCFileSpec );
   }

   GetStringFromAttribute( szCFileSpec + nLth, g_vSourceMeta, szSourceFileEntity, "Name" );

   // Need a C/Java file and a tmp file.
   if ( g_szGenLang[ 0 ] == 'J' )
   {
      if ( lType == zREFER_LOD_META ) // 2007
         zstrcat( szCFileSpec, "_Object" );
      else
      if ( lType == zREFER_DIALOG_META ) // 2011
         zstrcat( szCFileSpec, "_Dialog" );
      else
      if ( lType == zREFER_GOPGRP_META ) // 2014
         zstrcat( szCFileSpec, "_Operation" );
      else
      {
      ...
      zstrcpy( szTempCFileSpec, szCFileSpec );
      zstrcat( szCFileSpec, ".java" );
   }
   else
   {
      zstrcpy( szTempCFileSpec, szCFileSpec );
      zstrcat( szCFileSpec, ".c" );
   }

   zstrcat( szTempCFileSpec, ".tmp" );

   //-------------------------------------- Step Four -------------------------------------------//
   // Build the XPG name / Load the File.

   // Get the ExecDir off of the LPLR Entity.  This will be the directory where the xpg is now or
   // will be saved to.
   nRC = GetStringFromAttribute( szMsg, vTaskLPLR, "LPLR", "ExecDir" );
   SysConvertEnvironmentString( szXPGFileName, szMsg );

   // Be sure the dir name is terminated with the delimiter.
   nLth = SysAppendcDirSep( szXPGFileName );

   // Build the information needed to load the XPG File.  Append the xpg name onto the target.
   // This is the name of the ExecComposite.
   GetStringFromAttribute( szXPGFileName + nLth, g_vSourceMeta, szSourceFileEntity, "Name" );

   // Concat the Extension onto the directory.
   if ( g_szGenLang[ 0 ] == 'J' )
      zstrcat( szXPGFileName, ".XPJ" );
   else
      zstrcat( szXPGFileName, ".XPG" );

   //-------------------------------------- Step Five -------------------------------------------//
   // Activate the XPG object.  We'll first drop the current one if it is active.  It should be
   ...
   nRC = ActivateOI_FromFile( &g_lpPIView, "TZVSPOOO", lpVSSubtask,
                              szXPGFileName, zSINGLE | zNOI_OKAY );
   if ( nRC )
   {
      zstrcpy( szMsg, szMessageListGenerate[ 1 ] );
      zstrcat( szMsg, " file: " );
      zstrcat( szMsg, szXPGFileName );
   ...
   // Get XPG name.
   GetStringFromAttribute( szXPGName, g_lpPIView, "VML_XSource", "Name" );
   ...
   nRC = ActivateOI_FromFile( &g_lpZOListView, "TZOPGRPO", lpVSSubtask,
                              szXGODirectory, zSINGLE | zNOI_OKAY );
   ...
   if ( g_szGenLang[ 0 ] == 'J' )
   {
      GenerateJavaSource( vSubtask, pchFileName, szXPGName,
                          lfCFileHandle, lfTempCFileHandle, lType );
      nRC = PostGenerateJava( vSubtask, szCFileSpec, szTempCFileSpec,
                              lfCFileHandle, lfTempCFileHandle );
   }
   else
   {
      GenerateC_Source( vSubtask, pchFileName, szXPGName,
                        lfCFileHandle, lfTempCFileHandle );
      nRC = PostGenerateC( vSubtask, szCFileSpec, szTempCFileSpec,
                           lfCFileHandle, lfTempCFileHandle );
   }
