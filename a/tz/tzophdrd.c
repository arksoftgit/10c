...
zwTZOPHDRD_MainRtnFromSubWnd( zVIEW vSubtask )
{
   ...
   nRC = GetViewByName( &vLPL, "TaskLPLR", vSubtask, zLEVEL_TASK );
   zstrcpy( szTitle, "[Untitled]" );
   if ( nRC > 0 )
   {
      GetStringFromAttribute( szLPL_Name, vLPL, "LPLR", "Name" );
      zstrcat( szLPL_Name, " - Header File Definition" );
      if ( GetViewByName( &vHEADERMETA, "HEADERMETA", vSubtask, zLEVEL_TASK ) > 0 )
         GetStringFromAttribute( szTitle, vHEADERMETA, "HeaderFile", "Name" );
...
zCHAR szMsg[ 100 ];
   ...
   {
      GetStringFromAttribute( &szMsg[ 50 ], vHEADERLIST,
                              "W_MetaDef", "Name" );
      zstrcpy( szMsg, "OK to delete header " );
...
fnCommitMetaOI( zVIEW vSubtask, zVIEW vWindow )
{
   ...
   if ( nRC == zLEVEL_TASK )
   {
      ...
      {
         nRC =
            GetStringFromAttribute( szName, vHEADERMETA, "HeaderFile", "Name" );

         zstrcpy( szMessage, "Commit failed for Header File " );
...
zwTZOPHDRD_SelectHeaderFile( zVIEW vSubtask )
{
   ...
   nRC = GetStringFromAttribute( szSrcType, vSOURCEMETA,
...
zwTZOPHDRD_HeaderFileDelete( zVIEW vSubtask )
{
   ...
   if ( nRC != zLEVEL_TASK )
   {
      ...
      if ( nRC != 1 )
      {
         ...
         zstrcpy( szMessage, "Header File " );
         zstrcat( szMessage, szName );
         zstrcat( szMessage, " cannot be deleted." );
   ...
   nRC = GetStringFromAttribute( szName, vHEADERMETA, "HeaderFile", "Name" );

   zstrcpy( szMessage, "Delete Header File " );
   zstrcat( szMessage, szName );
   zstrcat( szMessage, " from LPLR ?" );
...
zwTZOPHDRD_HDR_Set_Caption( zVIEW    vSubtask )
{
   ...
   GetStringFromAttribute( szWrk, vProfileXFER, "OP", "Title" );
...
zwTZOPHDRD_HDR_AddNew( zVIEW    vSubtask )
{
   ...
   GetViewByName( &vProfileXFER, "ProfileXFER", vSubtask, zLEVEL_ANY );
   nZRetCode = GetStringFromAttribute( szTempString_0,
...
//     ( szHdr_TgtEntity used as temp string to save stack space )
   GetStringFromAttribute( szHdr_TgtEntity,
...
// get the passed ViewName and EntityName of the target of the include
   GetStringFromAttribute( szViewName,
                             vProfileXFER, "OP", "PassedViewName" );
...
// Prompt to confirm Delete
   zstrcpy( szWork, "Delete Header File\n\n" );
   GetStringFromAttribute( &szWork[ zstrlen( szWork) ],
                           vHDR_Upd, "HeaderFile", "Name" );
...
zwTZOPHDRD_HDR_CreateByCopy( zVIEW    vSubtask )
{
   ...
   GetStringFromAttribute( szHdrName,
                           vProfileXFER, "OP", "HeaderFileName" );
...
// Prompt to confirm Delete
   zstrcpy( szWork, "Delete Defined Item\n\n" );
   GetStringFromAttribute( &szWork[ zstrlen( szWork) ],
                           vHDR_Upd, "DefinedItem", "ExternalValue" );
   if ( MessagePrompt( vSubtask, "OP00113",
                       "Domain Management",
                       szWork,
                       zBEEP, zBUTTONS_YESNO,
                       zRESPONSE_NO,      0 )  == zRESPONSE_NO )
