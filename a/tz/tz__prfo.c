                            ...
                            zSHORT nCurrentOrReload )
{
   ...
   if ( SfCreateSubtask( &vTmpSubtask, vSubtask,
                         "Zeidon_Tools" ) == zCALL_ERROR )
   {
      return( zCALL_ERROR );
   }
   ...
   if ( nRScope != zLEVEL_APPLICATION )
   {
      ...
      if ( SysGetEnvVar( szPRF_FileName, "ZEIDON", sizeof( szPRF_FileName ) - 1 ) == 0 )
      {
         if ( szPRF_FileName[ zstrlen( szPRF_FileName ) ] != '\\' )
            zstrcat( szPRF_FileName, "\\" );

         zstrcat( szPRF_FileName, "TZTOOLS.PRF" );
      }
      else
         return( -2 );
      ...
      if ( szToolID[ 0 ] && vLPLR )
      {
         GetStringFromAttribute( szLPLR, vLPLR, "LPLR", "Name" );
