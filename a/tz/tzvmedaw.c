...
zwTZVSEDAW_OpenSourceFile( zVIEW vSubtask )
{
   ...
   GetViewByName( &vSource, "TZVSSROO", vSubtask, zLEVEL_SUBTASK );
   GetStringFromAttribute( szFileName, vSource, "VMLMODULE", "NAME" );

   zstrcat( szFileName, ".VML" );

   GetStringFromAttribute( szDirectory, vSource, "VMLMODULE", "DIRECTORY" );

   //   GetApplDirectoryFromView( szDirectory, vSource,
   //                             zAPPL_DIR_OBJECT, 128 );
   //
   zstrcpy( szQualifiedName, szDirectory );
   zstrcat( szQualifiedName, szFileName );
