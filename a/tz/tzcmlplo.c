                         ...
                         zVIEW   vTZCMLPLO )
{
   ...
   zstrcpy( szFileName, szL_ExecDir );
   zstrcat( szFileName, "ZEIDON.XDM" );

   // Loop through each of the meta defs (DomainGroups).  For each one,
   // activate the portable file containing the OI and process each
   // Domain.
   nRC = SetCursorFirstEntity( vTZCMLPLO, "W_MetaDef", 0 );
   vMeta = 0;      // Indicate no current meta.
   while ( nRC == zCURSOR_SET )
   {
      // Construct the name of the portable file containing the domain
      // group.
      zstrcpy( szFileName, szL_MetaSrcDir );
      GetStringFromAttribute( &szFileName[ zstrlen( szFileName ) ],
                               vTZCMLPLO, "W_MetaDef", "Name" );
      zstrcat( szFileName, SRC_CMOD[ zSOURCE_DOMAINGRP_META ].szOD_EXT );
      ...
      // Now loop through each of the domains in the domain group and
      // add them to the .XDM,
      nRC = SetCursorFirstEntity( vMeta, "Domain", 0 );
      while ( nRC == zCURSOR_SET )
      {
         // Request building the Domain in the XDM, without writing
         // the XDM to file.
                         ...
                         zVIEW   vTZCMCPRO )
{
                       ...
                       zRESPONSE_YES,      0 )  == zRESPONSE_YES )
   {
      ...
      GetStringFromAttribute( szL_ExecDir, vTZCMLPLO, "LPLR", "ExecDir" );
      ofnTZCMWKSO_AppendSlash( szL_ExecDir );
      GetStringFromAttribute( szL_MetaSrcDir, vTZCMLPLO, "LPLR", "MetaSrcDir" );
                                       ...
                                       zVIEW vTZCMCPRO )
{
   ...
   {
      zstrcpy( szMsg, "An other Technical Environment \nalready exists in " );
      zstrcat( szMsg, "your Project.\n\nRefresh Processing aborted!" );
      MessageSend( vTZCMLPLO, "CM00702", "Configuration Management",
                   szMsg,
                   zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      return( -1 );
   ...
   {
      zstrcpy( szMsg, "An other Presentation Environment \nalready exists in " );
      zstrcat( szMsg, "your Project.\n\nRefresh Processing aborted!" );
      MessageSend( vTZCMLPLO, "CM00702", "Configuration Management",
                   szMsg,
                   zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
                                  ...
                                  zBOOL   bDelete )
{
   ...
   if ( bDelete )
      nRC = CheckDuplicateName( vTZCMLPLO, vTZCMCPRO, szTypeCPLR, szTypeLPLR,
                                szName );
   else
      nRC = CheckDuplicateNameForRefresh( vTZCMLPLO, vTZCMCPRO, szTypeCPLR,
                                          szTypeLPLR, szName, ulZKeyLPLR );
   if ( nRC < 0 )
   {
      ...
      if ( nRC == -2 )
      {
         ...
         zstrcat( szMsg, " could not be refreshed,\nbecause a component '" );
         zstrcat( szMsg, szName );
         zstrcat( szMsg, "' of type " );
                                ...
                                zLONG   Type )
{
   ...
   if ( SetCursorFirstEntityByInteger( *vTZCMLPLO, "W_MetaType",
                              "Type", Type + 2000, 0 ) != zCURSOR_SET )
   {
      CreateEntity( *vTZCMLPLO, "W_MetaType", zPOS_AFTER );
      SetAttributeFromInteger( *vTZCMLPLO, "W_MetaType", "Type",
                                                   Type + 2000 );
   }
                             ...
                             zSHORT  nPos )
{
   ...
   CreateEntity( *vTZCMRPTO, "Z_MetaDef", zPOS_AFTER );
   SetAttributeFromAttribute( *vTZCMRPTO, "Z_MetaDef", "Name",
                               vTZCMLPLO, "W_MetaDef", "Name" );
   SetAttributeFromAttribute( *vTZCMRPTO, "Z_MetaDef", "ZKey",
                               vTZCMLPLO, "W_MetaDef", "CPLR_ZKey" );
   SetAttributeFromAttribute( *vTZCMRPTO, "Z_MetaDef", "Desc",
                               vTZCMLPLO, "W_MetaDef", "Desc" );
   SetAttributeFromAttribute( *vTZCMRPTO, "Z_MetaDef", "LastUpdateDate",
                               vTZCMLPLO, "W_MetaDef", "LastUpdateDate" );
   SetAttributeFromAttribute( *vTZCMRPTO, "Z_MetaDef",  "Type",
                               vTZCMLPLO, "W_MetaType", "Type" );

   GetVariableFromAttribute( &szMsg[ zstrlen( szMsg ) ], 0,
            zTYPE_STRING, 125, vTZCMLPLO, "W_MetaType", "Type", "CM_Type", 0 );
   zstrcat( szMsg, " " );
   GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ], vTZCMLPLO, "W_MetaDef",
                           "Name" );
   MB_SetMessage( vSubtask, 1, szMsg );
                                         ...
                                         zLONG  Type )
{
   ...
   for ( nRC = SetCursorFirstEntityByString( vTemp, "W_MetaDef",
                                             "GroupName", szName, 0 );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByString( vTemp, "W_MetaDef",
                                            "GroupName", szName, 0 ))
   {
                                    ...
                                    zPULONG pulZKeyLPLR )
{
   ...
   if ( lType == zSOURCE_ERD_META || lType == zSOURCE_DTE_META ||
        lType == zSOURCE_PENV_META )
   {
      ...
      if ( nRC < 0 )
      {
         GetVariableFromAttribute( szTypeLPLR, 0, zTYPE_STRING, 125,
                                   vTZCMRPTO_List, "Z_MetaDef",
                                   "Type", "CM_Type", 0 );

         if ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaType", "Type",
                                             lType + 2000, "" ) >= zCURSOR_SET )
            GetIntegerFromAttribute( (zPULONG) pulZKeyLPLR, vTZCMLPLO,
                                     "W_MetaDef", "CPLR_ZKey" );
         else
            return( 0 );

         zstrcpy( szMsg, "In your Project exists an other " );
         zstrcat( szMsg, szTypeLPLR );
         zstrcat( szMsg, "." );
         zstrcat( szMsg, "\n\nDo you want to overwrite?" );
         nRC = MessagePrompt( vTZCMLPLO, "CM00702", "Configuration Management",
                         ...
                         zPCHAR  szPrevLPLR_LastRefreshDate )
{
   ...
   if ( CompareAttributeToString( *vTZCMLPLO, "W_MetaDef", "RepositoryCPLR_ZKey",
                                  "" ) == 0 )
   {
      SetAttributeFromAttribute( *vTZCMLPLO, "W_MetaDef", "RepositoryCPLR_ZKey",
                                  vTZCMCPRO, "CPLR",      "ZKey" );
   ...
   GetStringFromAttribute( szC_LastUpdateDate, vTZCMCPRO, "Z_MetaDef",
                           "LastUpdateDate" );
   GetStringFromAttribute( szC_LastCheckInDate, vTZCMCPRO, "Z_MetaDef",
                           "LastCheckInDate" );
   GetStringFromAttribute( szL_LastUpdateDate, *vTZCMLPLO, "W_MetaDef",
                           "LastUpdateDate" );
   ...
   {
      zwTZCMLPLO_WriteReportView( vSubtask, vTZCMCPRO, vTZCMRPTO, szMsg,
                                  szName, nPos );
      SetAttributeFromInteger( *vTZCMRPTO, "CPLR", "ComponentRefreshed", 1 );
                                 ...
                                 zBOOL   bRefreshLPLR )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      GetIntegerFromAttribute(( zPLONG ) &ulZKey, vTZCMCPRO,
                                    "ReusableCPLR", "ZKey" );
      nRC = oTZCMCPRO_ActivateCPLR( &vTZCMCPRO_Reusable, ( zLONG ) ulZKey,
                                    vTZCMLPLO );
      if ( nRC < 0 )
      {
      ...
      SetNameForView( vTZCMCPRO_Reusable, "***TZCMCPRO_Reusable",
                        ...
                        zSHORT Selected )
{
   ...
   GetStringFromAttribute( szTemp, vTZCM_ChkInList, "LPLR", "MetaSrcDir" );
   SysConvertEnvironmentString( szL_MetaSrcDir, szTemp );
   ofnTZCMWKSO_AppendSlash( szL_MetaSrcDir );
   GetStringFromAttribute( szTemp, vTZCM_ChkInList, "LPLR", "ExecDir" );
   SysConvertEnvironmentString( szL_ExecDir, szTemp );
   ofnTZCMWKSO_AppendSlash( szL_ExecDir );
   GetStringFromAttribute( szTemp, vTZCM_ChkInList, "LPLR", "PgmSrcDir" );
   SysConvertEnvironmentString( szL_PgmSrcDir, szTemp );
...
//////////////////////////////////////////////////////////////////////////////////
   CreateViewFromViewForTask( &vTemp, vTZCMLPLO, 0 );
   nRC = SetCursorFirstEntityByInteger( vTemp, "W_MetaDef",
                                        "UpdateInd", 3, "LPLR" );
   DropView( vTemp );
   if ( nRC >= zCURSOR_SET )
   {
                           ...
                           zRESPONSE_YES,      0 )  == zRESPONSE_YES )
      {
         ...
         nRC = oTZCMLPLO_DeleteMetas( vSubtask, vLPLR_Deletes,
                                      vTZCMCPRO, vTZCM_ChkInList );
         if ( nRC )
            return( -1 );
...
//////////////////////////////////////////////////////////////////////////////////
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      {
         if ( SetCursorFirstEntityByInteger( vTZCMCPRO, "User_1", "ZKey",
                                             (zLONG) ulUserZKey, 0 ) != zCURSOR_SET )
         {
            zstrcpy( szTemp, "The Meta " );
            GetStringFromAttribute( &szTemp[ zstrlen( szTemp ) ],
                                    vTZCM_ChkInList, "W_MetaDef", "Name" );
            zstrcat( szTemp, ", Type " );
...
// entities.
                                       ...
                                       0 ) == zCURSOR_SET )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         {
            zstrcpy( szTemp, "The following Attribute has no Domain:\n\n" );
            zstrcat( szTemp, "Entity:    " );
            GetStringFromAttribute( szName, vERD, "ER_Entity", "Name" );
            zstrcat( szTemp, szName );
            zstrcat( szTemp, "\nAttribute: " );
            GetStringFromAttribute( szName, vERD, "ER_Attribute", "Name" );
            zstrcat( szTemp, szName );
...
//////////////////////////////////////////////////////////////////////////////////
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      while ( nRC == zCURSOR_SET )
      {
         ...
         zstrcpy( szTemp, "Processing " );
         lCurrent++;
         zltoa( lCurrent, &szTemp[ 110 ] );
         zstrcat( szTemp, &szTemp[ 110 ] );
         zstrcat( szTemp, " of " );
         zltoa( lCount, &szTemp[ 110 ] );
         zstrcat( szTemp, &szTemp[ 110 ] );
                           ...
                           "ZKey", ulZKey, 0 ) < zCURSOR_SET )
         {
            ...
            GetStringFromAttribute( szName, vTZCM_ChkInList,
                                    "W_MetaDef", "Name" );
            CreateEntity( vTZCMCPRO, "Z_MetaDef", zPOS_AFTER );
            SetAttributeFromAttribute( vTZCMCPRO, "Z_MetaDef", "ZKey",
                                 vTZCM_ChkInList, "W_MetaDef", "CPLR_ZKey" );
            SetAttributeFromAttribute( vTZCMCPRO, "Z_MetaDef", "Type",
                                 vTZCM_ChkInList, "W_MetaType", "Type" );
            ...
            GetIntegerFromAttribute( &T_Type,
                                     vTZCM_ChkInList, "W_MetaType", "Type" );
         ...
         if ( SetCursorFirstEntityByInteger( vTemp, "W_MetaDef",
                              "CPLR_ZKey", ulZKey, 0 ) == zCURSOR_SET )
         {
            zstrcpy( szTemp, "Activating " );
            GetVariableFromAttribute( &szTemp[ zstrlen( szTemp ) ], 0,
                                      zTYPE_STRING, 125, vTZCM_ChkInList,
                                      "W_MetaType", "Type", "CM_Type", 0 );
            zstrcat( szTemp, " " );
         ...
         // TraceLineX( "CheckInMetas Locked Mutex: ActMeta  for Task: ",
         //             (zLONG) vSubtask->hTask );
            if ( lType == zSOURCE_ERD_META )
            {
               nRC = fnActivateMetaOI( vSubtask, &vMeta, vTemp,
                                       zREFER_ERD_META,
                                       zLEVEL_APPLICATION | zSINGLE );
               if ( nRC > 0 )
                  DropObjectInstance( vMeta );
            }

            nRC = fnActivateMetaOI( vSubtask, &vMeta, vTemp, (zSHORT) lType,
                                    zLEVEL_APPLICATION | zSINGLE );
         // TraceLineX( "CheckInMetas Unlocking Mutex: ActMeta  for Task: ",
         //             (zLONG) vSubtask->hTask );
            ...
            if ( nRC > 0 )
            {
               ...
               SetAttributeFromAttribute( vTZCMCPRO, "Z_MetaDef", "Desc",
                                    vTZCM_ChkInList, "W_MetaDef", "Desc" );
               SetAttributeFromAttribute( vTZCMCPRO, "Z_MetaDef", "LastUpdateDate",
                                 vTZCM_ChkInList, "W_MetaDef", "LastUpdateDate" );
               SetAttributeFromString( vTZCMCPRO, "Z_MetaDef", "LastCheckInDate",
                                       szTimestamp );

               zstrcpy( szTemp, "Committing" );
               szTemp[ 10 ] = ' ';
               nOffset = zstrlen( szTemp );

               if ( CheckExistenceOfEntity( vMeta, "Z_MetaDef" )
                                                          != zCURSOR_SET )
                  IncludeSubobjectFromSubobject( vMeta, "Z_MetaDef",
                                                 vTZCMCPRO, "Z_MetaDef",
                                                 zPOS_AFTER );
               else
                  RelinkInstanceToInstance( vMeta, "Z_MetaDef",
                                            vTZCMCPRO, "Z_MetaDef" );

               // We must commit the meta to file, so that the shadow file
               //  has the Z_MetaDef included.
               zstrcpy( szFileSpec, szL_MetaSrcDir );
               if ( lType == zSOURCE_VOR_META )
               {
                  GetIntegerFromAttribute( &lMetaOI_ZKey, vMeta,
                                           SRC_CMOD[ lType ].szOD_ROOT, "ZKey" );
                  zltoxa( lMetaOI_ZKey, szMetaOI_File );
                  zstrcat( szFileSpec, szMetaOI_File );
               }
               else
               {
                  nLength = zstrlen( szFileSpec );
               ...
               zstrcat( szFileSpec, SRC_CMOD[ lType ].szOD_EXT );
               CommitOI_ToFile( vMeta, szFileSpec, zSINGLE | zINCREMENTAL );

               zstrcpy( &szTemp[ nOffset ], " to Repository" );
               MB_SetMessage( vDialog, 1, szTemp );

               // now, commit the AuditTrail Object...
               if ( GetViewByName( &vTZCMAUDO, "TZCMAUDO",
         ...
         zstrcpy( szTemp, szL_MetaSrcDir );
         zstrcat( szTemp, szName );
         zstrcat( szTemp, SRC_CMOD[ lType ].szOD_EXT );
...
// Copy dependent files to the CPLR.
         ...
         while ( nRC == zCURSOR_SET )
         {
               ...
               ( zstrcmp( szC_LastCheckInDate, szTimestamp ) == 0 ) )
            {
               ...
               GetIntegerFromAttribute( &ulZKeyCPRO, vTZCMCPRO,  "Z_MetaDef", "ZKey" );
               ...
               zstrcpy( &szTemp[ nOffset ], " meta file " );
               GetStringFromAttribute( &szTemp[ zstrlen( szTemp ) ], vTZCMCPRO,
                                       "Z_MetaDef", "Name" );
...
/* This Code deletes the metas updated as dependents from the shadow
   files, i.e. LODs, to force an activation on the next Refresh */
               if ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaDef",
                    "CPLR_ZKey", ulZKeyCPRO, "LPLR" ) >= zCURSOR_SET )
               {
                  if ( CompareAttributeToInteger( vTZCMLPLO, "W_MetaDef",
                                 "UpdateInd", 99 ) == 0 )
                  {
                     ...
                     SetAttributeFromString( vTZCMCPRO, "Z_MetaDef",
...
/* Copy Executables */
               ...
               {
                  zstrcpy( &szTemp[ nOffset ], " exec file " );
                  GetStringFromAttribute( &szTemp[ zstrlen( szTemp ) ],
         ...
         //BL, 1999.12.23
         nRC = CommitMultipleObjectInstances( CheckIn_ViewCluster,
                                              &lErrorIndex );
         if ( nRC < 0 )
         {
            // the checkin failed.
            ...
            if ( lTypeForRepositoryCPLR_ZKey )
            {
               ...
               SetAttributeFromString(
                  vTZCMLPLO, "W_MetaDef", "RepositoryCPLR_ZKey", "" );
            }
   ...
   if ( nXOD_CopyFlag == 1 )
   {
      ...
      CreateViewFromViewForTask( &vTemp2, vTZCMLPLO, 0 );
      nRC = SetCursorFirstEntityByInteger( vTemp2, "W_MetaType", "Type",
                                           zREFER_LOD_META, 0 );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         zstrcpy( szTemp, "Processing " );
         lCurrentXOD++;
         zltoa( lCurrentXOD, &szTemp[ 110 ] );
         zstrcat( szTemp, &szTemp[ 110 ] );
         zstrcat( szTemp, " of " );
         zltoa( lXodCount, &szTemp[ 110 ] );
         zstrcat( szTemp, &szTemp[ 110 ] );
         MB_SetMessage( vDialog, 2, szTemp );

         GetStringFromAttribute( szName, vTemp2, "W_MetaDef", "Name" );

         zstrcpy( szTemp, "Copying " );
         zstrcat( szTemp, szName );
         zstrcat( szTemp, ".XOD to CPLR" );
         MB_SetMessage( vDialog, 1, szTemp );
...
fnPostRefreshGlobalOp( zVIEW vMetaOI, zVIEW vTaskLPLR )
{
         ...
         nRC = SetCursorNextEntity( vTempTaskLPLR, "W_MetaDef", 0 ) )
   {
      ...
      if ( CompareAttributeToAttribute( vMetaOI,       "GlobalOperationGroup", "Name",
           ...
           SetCursorFirstEntityByAttr( vMetaOI,       "Operation", "ZKey",
                                       vTempTaskLPLR, "W_MetaDef", "CPLR_ZKey",
                                       0 ) < zCURSOR_SET )
      {
   ...
   //
   // Each of the operations in the op group need to be added to the
   // task LPLR.
   //
                            ...
                            zLONG  Type )
{
   ...
   zstrcat( szFileName, SRC_CMOD[ Type ].szOD_EXT );

   zstrcpy( szExtension, SRC_CMOD[ Type ].szOD_EXT + 1 );

   // Tell the operator what is happening.
   GetStringFromAttribute( szNameType, vTZCMCPRO, "Z_MetaDef", "Name" );
   zstrcat( szNameType, ", Type " );
   GetVariableFromAttribute( &szNameType[ zstrlen( szNameType ) ], 0,
                             zTYPE_STRING, 125, vTZCMCPRO,
                             "Z_MetaDef", "Type", "CM_Type", 0 );
   zstrcat( szNameType, "," );
   zstrcpy( szMsg, "Refreshing " );
   zstrcat( szMsg, szNameType );
   zstrcat( szMsg, " to Project" );
   ...
   if ( nRC < 0 )
   {
      zstrcpy( szMsg, "Error activating Meta " );
      zstrcat( szMsg, szNameType );
      zstrcat( szMsg, "\nfrom Repository!" );
                          ...
                          zLONG  lViewCluster )
{
   ...
   GetStringFromAttribute( szName, vHdrMeta,
                           SRC_CMOD[ zSOURCE_HDR_META ].szOD_ROOT, "Name" );
   GetStringFromAttribute( szExtension, vHdrMeta,
                           SRC_CMOD[ zSOURCE_HDR_META ].szOD_ROOT, "Extension" );
   GetStringFromAttribute( szTemp, vTZCMLPLO, "LPLR", "PgmSrcDir" );
   if ( szTemp[ 0 ] == 0 )
      GetStringFromAttribute( szTemp, vTZCMLPLO, "LPLR", "MetaSrcDir" );

   SysConvertEnvironmentString( szL_FileName, szTemp );
   ofnTZCMWKSO_AppendSlash( szL_FileName );
   zstrcat( szL_FileName, szName );
   if ( szExtension[ 1 ] != '.' );
   {
      zstrcat( szL_FileName, "." );
   }

   zstrcat( szL_FileName, szExtension );
                             ...
                             zLONG  lViewCluster)
{
...

   {
      ...
      szL_FileName[ nL_Lth ] = 0;

      GetStringFromAttribute( szName, vMeta, pszSourceFileEntityName, "Name" );
      GetStringFromAttribute( szExtension, vMeta, pszSourceFileEntityName, "Extension" );

      GetStringFromAttribute( szSubdir, vMeta, pszSourceFileEntityName, "Subdirectory" );
      if ( *szSubdir != 0 )
      {
         zstrcat( szL_FileName, szSubdir );
         ofnTZCMWKSO_AppendSlash( szL_FileName );
      }
                               ...
                               zLONG   ViewCluster )
{
   ...
   zstrcpy( szL_FileName, szL_MetaSrcDir );
   zstrcat( szL_FileName, szName );
   zstrcat( szL_FileName, ".REP" );
   zstrcpy( szExtension, "REP" );
   ...
   zstrcpy( szL_FileName, szL_ExecDir );
   zstrcat( szL_FileName, szName );
   zstrcat( szL_FileName, ".XSQ" );
   zstrcpy( szExtension, "XSQ" );
                        ...
                        zLONG   ViewCluster )
{
   ...
   {
      zstrcat( szL_FileName, ".XOD" );
      zstrcpy( szExtension, "XOD" );
   }
   else
   if ( Type == zSOURCE_DIALOG_META )
   {
      ...
      SysReadZeidonIni( -1, "[Workstation]", "GenLang", szGenLang ); // borrow szExecDir for a bit
      if ( szGenLang[ 0 ] == 'j' )
         szGenLang[ 0 ] = 'J';
      ...
      if ( szGenLang[ 0 ] == 'J' )
      {
         zstrcat( szL_FileName, ".XPJ" );
         zstrcpy( szExtension, "XPJ" );
      }
      else
      {
         zstrcat( szL_FileName, ".XPG" );
         zstrcpy( szExtension, "XPG" );
      }
      ...
      zstrcpy( szL_FileName, szL_ExecDir );
      zstrcat( szL_FileName, szName );
      zstrcat( szL_FileName, ".XWD" );
      zstrcpy( szExtension, "XWD" );
   }
   else
   if ( Type == zSOURCE_REPORT_META )
   {
      ...
      ofnTZCMLPLO_SironReport_Files( vSubtask, vTZCMCPRO, szL_MetaSrcDir,
                                     szL_ExecDir, szName,
                                     Direction, ViewCluster );

      zstrcat( szL_FileName, ".XRP" );
      zstrcpy( szExtension, "XRP" );
   }
   else
   if ( Type == zSOURCE_XSLT_META )
   {
      zstrcat( szL_FileName, ".XSL" );
      zstrcpy( szExtension, "XSL" );
   }
   else
   if ( Type == zSOURCE_PENV_META )
   {
      zstrcat( szL_FileName, ".XPE" );
      zstrcpy( szExtension, "XPE" );
   }
   else
   if ( Type == zSOURCE_ERD_META )
   {
      return( 0 );
//      zstrcat( szL_FileName, ".XMD" );
//      zstrcpy( szExtension, "XMD" );
   }
                       ...
                       zVIEW vTZCM_ChkInList )
{
   ...
   GetStringFromAttribute( szL_MetaSrcDir, vTZCMLPLO, "LPLR", "MetaSrcDir" );
   ofnTZCMWKSO_AppendSlash( szL_MetaSrcDir );
   GetStringFromAttribute( szC_MetaSrcDir, vTZCMCPRO, "CPLR", "MetaSrcDir" );
   ofnTZCMWKSO_AppendSlash( szC_MetaSrcDir );
   GetStringFromAttribute( szL_ExecDir, vTZCMLPLO, "LPLR", "ExecDir" );
   ofnTZCMWKSO_AppendSlash( szL_ExecDir );
   GetStringFromAttribute( szC_ExecDir, vTZCMCPRO, "CPLR", "ExecDir" );
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      GetIntegerFromAttribute( &ulZKey, vLPLR_Deletes, "W_MetaDef", "CPLR_ZKey" );
      GetStringFromAttribute( szName, vLPLR_Deletes, "W_MetaDef", "Name" );
      zstrcpy( szMsg, "Deleting " );
      ...
      if ( Type == zSOURCE_LOD_META )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
            GetIntegerFromAttribute( &ulZKeyVOR,
                                     vLPLR_VOR, "W_MetaDef", "CPLR_ZKey" );
            zltoxa( ulZKeyVOR, szTempName );
            zstrcpy( szFileName, szL_MetaSrcDir );
            zstrcat( szFileName, szTempName );
            zstrcat( szFileName, ".PVR" );
                                      ...
                                      zSINGLE | zIGNORE_ATTRIB_ERRORS ) == 0 )
            {
               if ( CompareAttributeToString( vVOR, "LOD", "Name", szName ) == 0 )
               {
                  ...
                  hFile = (HFILE) SysOpenFile( vSubtask, szFileName,
                                               COREFILE_DELETE );
      ...
      if ( GetViewByName( &vTZCMAUDO, "TZCMAUDO", vSubtask, zLEVEL_TASK ) >= 0 )
      {
         ...
         //IncludeSubobjectFromSubobject( vTZCMAUDO, "Z_MetaDef",
         //                               vLPLR_Deletes, "Z_MetaDef",
         //                               zPOS_AFTER );
         SetAttributeFromString( vTZCMAUDO, "AuditTrailMeta", "MetaDefName",
                                 szName );
         SetAttributeFromInteger( vTZCMAUDO, "AuditTrailMeta", "MetaDefType",
                                  Type );
      ...
      nRC = SetCursorFirstEntityByInteger( vCPLR_Deletes, "Z_MetaDef",
                                           "ZKey", ulZKey, "" );
      if ( nRC >= zCURSOR_SET )
         DeleteEntity( vCPLR_Deletes, "Z_MetaDef", zREPOS_NONE );

      nRC = SetCursorFirstEntityByInteger( vTZCMLPLO_Deletes, "W_MetaDef",
                                           "CPLR_ZKey", ulZKey,
                                           "LPLR" );
      ...
      // shadow file.
      nRC = oTZCMCPRO_DeleteServerFile( vTZCMCPRO,
                                        szName,
                          ...
                          zLONG  lZKey )
{
   ...
   nRC = ActivateObjectInstance( vFile, "TZCMFILO",
                                 vTZCMCPRO, vKZDBHQUA,
                                 zSINGLE );
                                  ...
                                  zPCHAR szExtension )
{
   ...
   {
      if ( CompareAttributeToString( vTZCMCPRO, "File", "Extension",
                                     szExtension ) == 0 )
         nFound = 1;
      else
   ...
   if ( nFound == 1 )
   {
      ...
      if ( nRC >= 0 )
      {
         ...
         {
            zstrcpy( szMsg, "File " );
            zstrcat( szMsg, szL_FileName );
            zstrcat( szMsg, " could not be opened for creation." );
         ...
         while ( nRC >= zCURSOR_SET )
         {
            ...
            GetBlobFromAttribute( szBuffer, &nLth,
                                  vFile, "Buffer", "Value" );
                                ...
                                zLONG  ViewCluster )
{
   ...
   else
   {
      ...
      if ( nRC < 0 )
      {
         zstrcpy( szMsg, "Error on committing File." );
         zstrcat( szMsg, szName );
         zstrcat( szMsg, "." );
         zstrcat( szMsg, szExtension );
         zstrcat( szMsg, " to Server." );
                         ...
                         zSHORT ERD_Refreshed )
{
   ...
   ofnTZCMWKSO_AppendSlash( szFileName );
   zstrcat( szFileName, "ZREFRESH.RPT" );
