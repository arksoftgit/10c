...
// Set up the XGO File Name and the View Name
   ...
   {
      zstrcpy( szAppViewName, "xoOPERSIGS." );
      nRC = GetViewByName( &vT, "TaskLPLR", vSubtask, zLEVEL_TASK );
      nRC = GetStringFromAttribute( szXGO_FileName, vT, "LPLR", "ExecDir" );
      if ( szXGO_FileName[ zstrlen( szXGO_FileName ) - 1 ] != '\\' )
          zstrcat( szXGO_FileName, "\\" );

      zstrcat( szXGO_FileName, "opersigs.xgo" );
   }
   else
   {
      zstrcpy( szAppViewName, "xoTZOPSIGK." );
      ...
      zstrcat( szXGO_FileName, "tzopsigk.xgo" );
   }
...
// See if XGO View is at the Application level
   ...
   if ( vTask )
   {
      GetStringFromAttribute( &szAppViewName[ 11 ], vTask, "LPLR", "Name" );
...
// Set up the XGO File Name
   ...
   GetStringFromAttribute( szXGO_FileName, vT, "LPLR", "ExecDir" );
   ...
   {
      zstrcpy( szMsg, "The Executable version of the Operation is about to "
                      "stored in the executable directory of your currently "
                      "executing Application.\n\n\t" );
      zstrcat( szMsg, szApplExecDir );
