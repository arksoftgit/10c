...
wTZOPRUSD_SetCtrlState( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szOType, vSOURCE, "Operation", "Type" );
...
wTZOPRUSD_VerifyParms( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szOperType, vSOURCE_Copy, "Operation", "Type" );
                          ...
                          zPCHAR szDesc )
{
   ...
   if ( nRetCode == zCURSOR_SET )
   {
      ...
      nRetCode =
         GetStringFromAttribute( szCurrentType,
   ...
   else
   {
      ...
      if ( ComponentIsCheckedOut( vSubtask, vT, zSOURCE_DIALOG_META ) == 1 )
      {
         if ( MiGetUpdateForView( vT ) && ObjectInstanceUpdatedFromFile( vT ) )
         {
            ...
            {
               SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, "", "" );
               fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, "Y" );
...
wTZOPRUSD_InvokeEditor( zVIEW vSubtask )
{
   ...
   GetViewByName( &vSOURCE, "TZOPRUSO", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szName, vSOURCE, "OperationList", "Name" );
...
wTZOPRUSD_ParameterDelete( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szMsg, "OK to delete the parameter " );
      GetStringFromAttribute( &szMsg[ 27 ], vSOURCEMETA,
                              "Parameter", "ShortDesc" );
      zstrcat( szMsg, "?" );
...
wTZOPRUSD_SaveOperation( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szReturnType,
   ...
   if ( szReturnType[ 0 ] == 'V' )
   {
      ...
      GetStringFromAttribute( szOperationType,
...
fnBuildOperationList( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vMetaView, "TZZOLODO", vSubtask, zLEVEL_TASK ) < 0 )
      zstrcpy( szRootName, "Dialog" );
   else
      zstrcpy( szRootName, "LOD" );
   ...
   if ( CheckExistenceOfEntity( vMetaView, "Operation" ) >= zCURSOR_SET )
      GetStringFromAttribute( szOperationName, vMetaView, "Operation", "Name" );
   ...
   if ( nRC == zRESPONSE_YES )
   {
            ...
            nRC = SetCursorNextSelectedEntity( vMeta, "SourceFile", "" ) )
      {
         ...
         if ( CheckExistenceOfEntity( vMeta, "Operation" ) >= zCURSOR_SET )
         {
            ...
            GetStringFromAttribute( szName, vMeta, "SourceFile", "Name" );
            zstrcat( szMsg, szName );
            zstrcat( szMsg, "' cannot be deleted if it has Operations." );
            MessageSend( vSubtask, "OP00802",
                         "Operations",
                         szMsg,
...
wTZOPRUSD_OperActionsCreate( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szActionname, vOrigMeta, "OperationList", "Name");
...
wTZOPRUSD_DeleteSelectedOperat( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szRootName, "Dialog" );
      zstrcpy( szEntityName, "WD" );
      lSourceType = zSOURCE_DIALOG_META;
   }
   else
   {
      zstrcpy( szRootName, "LOD" );
      zstrcpy( szEntityName, "ZO" );
                           ...
                           zPCHAR szMessageText )
{
   ...
   GetStringFromAttribute( szNewName, vTZOPRUSO, szEntityName, AttributeName );
...
wTZOPRUSD_CopyOperation( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szName + 1, vMetaView, "Operation", "Name" );
   szName[ 0 ] = '_';
   szName[ 33 ] = 0;
...
wTZOPRUSD_DeleteCurrentOperation( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szName, vSOURCE, "Operation", "Name" );
   zstrcpy( szMsg, "OK to delete Operation '" );
   zstrcat( szMsg, szName );
   zstrcat( szMsg, "'?" );
   ...
   // The following validation is executed for Dialog metas only.
   if ( wTZOPRUSD_CheckActionForDeleteOp( vSubtask, vDialog, lSourceType,
                                          szName ) == 0 )
      return( -1 );
                                  ...
                                  zPCHAR szName )
{
   ...
   if ( lCallByDialog == zSOURCE_DIALOG_META )
   {
                                         ...
                                         szName, "Dialog" ) >= zCURSOR_SET )
      {
         ...
         zstrcat( szMsg, szName );
         zstrcat( szMsg, "' cannot be deleted if it is used by an Action." );
         TraceLineS( szMsg, "" );
...
wTZOPRUSD_DeleteParameter( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szShortDesc, vSOURCE, "Parameter", "ShortDesc" );
                          ...
                          zVIEW vTZOPRUSO_Move )
{
   ...
   // TZOPRUSO to the end of the SourceFile for view, TZOPRUSO2, from
   // which the new souce entity was selected.
   GetStringFromAttribute( szOperationName, vTZOPRUSO_Move, "Operation", "Name" );
   zstrcpy( szMsg, "Move Operation: " );
   ...
   if ( GetCtrlState( vSubtask, "cbCopyCode", zCONTROL_STATUS_CHECKED ) == 1 )
   {
      zstrcpy( szMsg, "Move Operation Code: " );
      zstrcat( szMsg, szOperationName );
      MB_SetMessage( vSubtask, 0, szMsg );
