                       ...
                       zBOOL        bValidationOnly )
{
   ...
   switch ( lpDomainData->cType )
   {
      case zTYPE_STRING:
         ...
         {
            zstrncpy( sz, psz, sizeof( sz ) - 1 );
            sz[ sizeof( sz ) - 1 ] = 0;
            nRC = fnStrToDec( sz, lpDomainData->zView,
                              lpDomainData->lpViewEntity,
                              lpDomainData->lpViewAttribute,
                              lpDomain, lpDomainData->pszContextName,
                              &d, bValidationOnly );
            if ( nRC == zCALL_ERROR )
               return( nRC );
         }
      ...
      case zTYPE_DECIMAL:
         {
            ...
            SysConvertDecimalToString( (zPDECIMAL) lpDomainData->lpData,
                                       szDecimal, nPrecision );
        ...
        lpContext->hEditString )
   {
      ...
      zstrcpy( szEditString, (zPCHAR) SysGetPointerFromHandle( lpContext->hEditString ) );
      ...
      for ( ; ; ) // loop until colon is found
      {
         ...
         if ( sz[ 0 ] == '>' || sz[ 0 ] == '<' || sz[ 0 ] == '=' )
         {
            ...
            if ( bCompare == 0 )
            {
               ...
               *(sz + 1) = 0;
               ...
               SendDomainError( lpDomainData->zView, lpDomain, 8, 23,
                                0, sz, lpDomainData->lpViewAttribute->szName );
               return( zCALL_ERROR );
            }
         }
         ...
         if ( szEditString[ i ] == ';' )
         {
            ...
            for ( i = nStart;
                  szEditString[ i ] != ';' && szEditString[ i ] != 0;
                  i++ )
            {
               ;
            }
         ...
         else
         {
            if ( lpDomainData->pszContextName && *lpDomainData->pszContextName )
            {
                             ...
                             szlDecimalIntegerRounded ) == 0 )
               {
                  ...
                  SysAssignDecimalFromDecimal( &dTemp,
   ...
   switch ( lpDomainData->cType )
   {
      ...
      case zTYPE_DECIMAL:
         ...
         if ( SysCompareDecimalToNull( (zPDECIMAL) lpDomainData->lpData ) == 0 ||
              SysCompareDecimalToDecimal( (zPDECIMAL) lpDomainData->lpData,
                                          &dWork ) == 0 )
         {
            return( 0 );
         }
    ...
    case zTYPE_INTEGER:
      ...
      if ( lpDomainData->pszContextName && *lpDomainData->pszContextName )
      {
         ...
         if ( zstrcmp( lpDomainData->pszContextName,
                       szlDecimalIntegerRounded ) == 0 )
         {
            zstrcpy( sz, lpDomainData->lpViewEntity->szName );
            zstrcat( sz, ", " );
            zstrcat( sz, lpDomainData->lpViewAttribute->szName );
      ...
      case zTYPE_DECIMAL:
         ...
         if ( lpDomainData->pszContextName && *lpDomainData->pszContextName )
         {
                          ...
                          szlDecimalIntegerRounded ) == 0 )
            {
               ...
               if ( SysCompareDecimalToDouble( (zPDECIMAL) lpDomainData->lpData,
                                               0.0 ) < 0 )
                  SysConvertDoubleToDecimal( -0.5, &d );
               else
                  SysConvertDoubleToDecimal( 0.5, &d );

               SysAddDecimalToDecimal( (zPDECIMAL) lpDomainData->lpData,
                          ...
                          szlDecimalFraction ) == 0 )
            {
               ...
               SysConvertLongToDecimal( l1, &d );
               SysSubtractDecimalFromDecimal( &d,
                                              (zPDECIMAL) lpDomainData->lpData,
                                              &d );
            }
            else
               SysAssignDecimalFromDecimal( &d,
                                            (zPDECIMAL) lpDomainData->lpData );
         }
         else
            SysAssignDecimalFromDecimal( &d,
...
fnGetCompValDecimalAttribute( LPDOMAINDATA lpDomainData )
{
   ...
   switch ( lpDomainData->cType )
   {
      ...
      default:
         ...
         // "TZDME001 - Invalid Input Data Type"
         SendDomainError( lpDomainData->zView, (LPDOMAIN)
                          SysGetPointerFromHandle( lpDomainData->lpViewAttribute->hDomain ),
                        ...
                        zPLONG      lplPrecision )
{
   ...
   if ( zstrncmp( pszName, "DecimalPoint", 12 ) == 0 )
   {
      ...
      zstrcpy( szReturnThou, "," );
      zstrcpy( szReturnDecPt, "." );
            ...
            zBOOL          bValidationOnly )
{
   ...
   if ( lpContext )
   {
      ...
      if ( fnDecIntContext( lpContext ) )
      {
         ...
         zstrcat( sz2, ", " );
         zstrcat( sz2, lpViewAttribute->szName );
      ...
      if ( nRC )
      {
         ...
         // We continue with the US default...
         SendDomainError( zView, lpDomain, 16, 19, 0,
                           lpViewEntity->szName,
   ...
   // Locate the decimal point in the string and then the number of digits
   ...
   // If there is at least one digit after the decimal point, subtract out
   // the decimal point.
   if ( l >= k + 1 )
   {
   ...
   // The number of precision places after the decimal point must be <=
   // the precision for the Context/Domain.
