...
oTZZOXODO_SaveXOD( zVIEW vSubtask, zVIEW vTZZOLODO )
{
   ...
   zCHAR  szXMD_DateTime[ 18 ];
   zCHAR  szEMD_DateTime[ 18 ];
   zCHAR  szDTE_DateTime[ 18 ];
   ...
   nRC = ActivateEmptyObjectInstance( &vTZZOXODO, "TZZOXODO", vTZZOLODO,
   ...
   if ( GetViewByName( &vTZEREMDO_REF, "TZEREMDO_REF", vSubtask, zLEVEL_TASK ) < 1 )
   {
      ...
      {
         ActivateMetaOI( vSubtask, &vTZEREMDO_REF, vT, zREFER_ERD_META,
   ...
   if ( CheckExistenceOfEntity( vTZZOLODO, "POD" ) >= zCURSOR_SET )
   {
      ...
      if ( CompareAttributeToInteger( vTZZOLODO, "POD",
                                      "MULTI_ROOT_LIMIT", 0 ) != 0 )
      {
         SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "MR_LIMIT",
                                    vTZZOLODO, "POD", "MULTI_ROOT_LIMIT" );
      }

      GetViewByName( &vTZTENVRO_REF, "TE_DB_Environ", vSubtask, zLEVEL_TASK );
      if ( vTZTENVRO_REF == 0 )
      {
         GetIntegerFromAttribute( &lTE_ZKey, vTZZOLODO,
                                  "TE_DB_Environ", "ZKey" );
         ActivateMetaOI_ByZKey( vSubtask, &vTZTENVRO_REF, 0, zREFER_DTE_META,
                                zSINGLE | zLEVEL_APPLICATION,
                                lTE_ZKey, zCURRENT_OI );
         if ( !vTZTENVRO_REF )
         {
      ...
      // DGC 12/9/96  Check to see if TE Source ZKey has been set in the LOD.
      // If it has, then set the entity in the TE view to match.  This will
      // cause us to save the current LOD with the same TE as the last time
      // the LOD was saved.
      GetIntegerFromAttribute( &lZKey, vTZZOLODO, "POD", "TE_SourceZKey" );
      if ( lZKey )
      {
         if ( SetCursorFirstEntityByInteger( vTZTENVRO_REF, "TE_DBMS_Source",
   ...
   if ( vTaskLPLR )
   {
      ...
      if ( hXMD_File >= 0 )
      {
         SysGetFileDateTime( hXMD_File, szXMD_DateTime );
      ...
      if ( vTZEREMDO_REF )
      {
         GetStringFromAttribute( szEMD_DateTime, vTZEREMDO_REF,
                                 "EntpER_Model", "LastSyncDate" );
         if ( szEMD_DateTime[ 0 ] == 0 )
         {
            RetrieveViewForMetaList( vSubtask, &vT, zREFER_ERD_META );
            GetStringFromAttribute( szEMD_DateTime,
                                    vT, "W_MetaDef", "LastUpdateDate" );
            DropView( vT );
         }
      }
      else
      ...
      if ( lTE_ZKey > 0 )
      {
         ...
         SetCursorFirstEntityByInteger( vT, "W_MetaDef",
                                        "CPLR_ZKey", lTE_ZKey, "" );
         GetStringFromAttribute( szDTE_DateTime,
                                 vT, "W_MetaDef", "LastUpdateDate" );
         DropView( vT );
      }

      if ( (zstrcmp( szXMD_DateTime, szEMD_DateTime ) < 0  ||
            zstrcmp( szXMD_DateTime, szDTE_DateTime ) < 0) && vTZTENVRO_REF )
      {
         // Use a work view so as not to alter position of DBMS_Source.
         CreateViewFromViewForTask( &vTZTENVR1, vTZTENVRO_REF, 0 );
         oTZEREMXO_BuildExecutableModel( vSubtask, 0,
                                         vTZEREMDO_REF, vTZTENVR1 );
         DropView( vTZTENVR1 );
      }
   ...
   GetStringFromAttribute( szLOD_Name, vTZZOLODO, "LOD", "Name" );

   SetAttributeFromString( vTZZOXODO, "OBJECT", "NAME", szLOD_Name );
   SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "ZKey",
                              vTZZOLODO, "LOD", "ZKey" );
                                                           ...
                                                           >= zCURSOR_SET )
   {
      ...
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "OCACTE",
                                 vTZZOLODO, "LOD", "ActivateEmptyConstraint" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "OCCOM",
                                 vTZZOLODO, "LOD", "CommitConstraint" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "OCDROP",
                                 vTZZOLODO, "LOD", "DropOIConstraint" );
   }

   SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "NETCOMPRESS",
                              vTZZOLODO, "LOD", "NetCompressedFlag" );
   nRC = CompareAttributeToString( vTZZOLODO, "LOD", "DLL_Name", "" );
   if ( nRC == 0 )
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "OPER_LIBNM",
                                 vTZZOLODO, "LOD", "Name" );
   else
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "OPER_LIBNM",
                                 vTZZOLODO, "LOD", "DLL_Name" );
   ...
   if ( vTZTENVRO_REF )
   {
      ...
      ofnTZZOXODO_BldXOD( vSubtask, vTZZOXODO_Root, vTZZOXODO, vTZZOLOD1,
                          vTZEREMDO_REF, vTZTENVRO_REF );

      SetMatchingAttributesByName( vTZZOXODO, "OBJECT",
                                   vTZZOXOD1, "OBJECT",
                                   zSET_KEYS | zSET_NULL );
      ...
      if ( vTZTEDBLO == 0 )
      {
         ActivateOI_FromFile( &vTZTEDBLO, "tztedblo", vTZZOLODO, "tztedblo.xdl",
                              zMULTIPLE );
         SetNameForView( vTZTEDBLO, "TZTEDBLO", vSubtask, zLEVEL_TASK );
      }

      SetCursorFirstEntityByAttr( vTZTEDBLO, "TE_DBMS_Source", "DBMS",
                                  vTZTENVRO_REF, "TE_DBMS_Source", "DBMS", 0 );

      if ( CompareAttributeToString( vTZTEDBLO, "TE_DBMS_Source",
                                     "DBS_PostXOD_BuildHook", "Y" ) == 0 )
      {
         zPCHAR    pszExecutable;
         LPLIBRARY hLibrary;
         zSHORT    (POPERATION pfnHook) ( zVIEW, zVIEW, zVIEW, zVIEW, zVIEW );

         GetAddrForAttribute( &pszExecutable, vTZTEDBLO, "TE_DBMS_Source",
                              "GenerateExecutable" );
   ...
   {
      CreateViewFromViewForTask( &vTZZOXODO_Root, vTZZOXODO, 0 );
      ofnTZZOXODO_BldXOD( vSubtask, vTZZOXODO_Root,
                          vTZZOXODO, vTZZOLODO,
                          vTZEREMDO_REF, vTZTENVRO_REF );
      DropView( vTZZOXODO_Root );
   }

   // Tell core to reload the XOD.  First get a subtask view to the
   // application.  If SourceLPLR and TargetLPLR are defined, then we are doing
   // a migrate and we don't want to drop the view object because we get
   // errors.
   if ( GetViewByName( &vT, "TargetLPLR", vSubtask, zLEVEL_TASK ) < 0 &&
        GetViewByName( &vT, "SourceLPLR", vSubtask, zLEVEL_TASK ) <= 0 )
   {
      GetStringFromAttribute( szAppName, vTaskLPLR, "LPLR", "Name" );
...
/*
   ...
   user to sort entities using the values of child entities if the child
   entity has a 1-1 relationship.
...
fnResetSequenceAttributes( zVIEW vXOD )
{
   ...
   // We'll start looking after the first "ENTITY" because we don't care
   // about the root.
   lPos = 3;
         ...
         nRC = SetCursorAbsolutePosition( szEntityName, vXOD, lPos++ ) )
   {
      ...
      GetStringFromAttribute( szEntityName, vXOD, "ENTITY", "NAME" );
      ...
      // If the CardMax for the child entity is > 1 then we can't do anything
      // with the sequence numbers so forget it.
      ...
      SetAttributeFromAttribute( vT, "ENTITY", "NAME",
                                 vXOD, "CHILDENTITY", "NAME" );
   ...
   // Now we'll back up through the entities and look for sequence fields
   // that are also keys.
         ...
         nRC = SetCursorPrevEntity( vT, "ENTITY", 0 ) )
   {
            ...
            nRC = SetCursorNextEntity( vXOD, "ATTRIB", 0 ) )
      {
         ...
         // Find the *target* attribute token for the key.  This should be
         // the FK of the parent.  First find the source RelField.
         GetIntegerFromAttribute( &lTok, vXOD, "ATTRIB", "XVAATT_TOK" );
         if ( SetCursorFirstEntityByInteger( vXOD, "RELFIELD", "SRCXVAATT_TOK",
                                             lTok, "ENTITY" ) < zCURSOR_SET )
         {
         ...
         // No try to find the attribute that matches lTok.  This should be the
         // FK in the parent.  We'll do this by looking in each parent for a match.
         SetViewFromView( vXOD2, vXOD );
         do
         {
            ResetViewFromSubobject( vXOD2 );
         } while ( SetCursorFirstEntityByInteger( vXOD2, "ATTRIB", "XVAATT_TOK",
                                                  lTok, 0 ) < zCURSOR_SET );

         // Set the sequencing number for the FK and turn off sequencing in
         // the original attribute.
         SetAttributeFromInteger( vXOD2, "ATTRIB", "SEQUENCING", lSeqNum );
         SetAttributeFromAttribute( vXOD2, "ATTRIB", "SEQ_AD",
                                    vXOD,  "ATTRIB", "SEQ_AD" );
...
/*
   Makes sure that a recursive entity and it's (recursive) parent both have
   the same work attributes.
*/
void
fnCopyRecursiveWorkAttributes( zVIEW vChild )
{
   ...
   } while ( CompareAttributeToAttribute( vChild, "ENTITY", "ERENT_TOK",
                                          vParent, "ENTITY", "ERENT_TOK" ) != 0 );
         ...
         nRC = SetCursorNextEntity( vParent, "ATTRIB", 0 ) )
   {
      ...
      if ( SetCursorFirstEntityByAttr( vChild, "ATTRIB", "NAME",
                                       vParent, "ATTRIB", "NAME", 0 ) < zCURSOR_SET )
      {
         // Attr doesn't exist so create it.
         CreateEntity( vChild, "ATTRIB", zPOS_LAST );
      }

      // Now copy all the attrib info from parent to child.  This might not
      // be necessary because the info *should* be the same anyway.  Doing
      // this enforces it.
         ...
         nRC = SetCursorNextEntity( vChild, "ATTRIB", 0 ) )
   {
      ...
      // See if we can find the attr by NAME in the child.
      if ( SetCursorFirstEntityByAttr( vParent, "ATTRIB", "NAME",
                                       vChild, "ATTRIB", "NAME",
                                       0 ) >= zCURSOR_SET )
      {
      ...
      SetMatchingAttributesByName( vParent, "ATTRIB", vChild, "ATTRIB",
                                   zSET_ALL );
   }
                    ...
                    zVIEW vTZEREMDO_REF, zVIEW vTZTENVRO_REF )
{
   ...
   // nDerivedFlag tells me, whether the current Entity or any of its parents
   // is derived.
   zSHORT nDerivedFlag;

   if ( vTZTENVRO_REF )
   {
      GetAddrForAttribute( &pszParadigm, vTZTENVRO_REF,
                           "TE_DBMS_Source", "Paradigm" );
      GetAddrForAttribute( &pszNetwork, vTZTENVRO_REF,
                           "TE_DBMS_Source", "Network" );
   }
   else
   {
   ...
   if ( *pszNetwork )
   {
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "DFT_DBNAME",
                                 vTZTENVRO_REF, "TE_DBMS_Source", "Name" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "NETWORK",
                                 vTZTENVRO_REF, "TE_DBMS_Source", "Network" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "LOCK",
                                 vTZZOLODO, "LOD",    "DataLocking" );
   }
   else
   if ( *pszParadigm )
   {
      // Stuff that's common to all physical paradigms.  Note that for
      // network stuff none of this stuff is set.
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "DFT_DBNAME",
                                 vTZTENVRO_REF, "TE_DBMS_Source", "Name" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "DATABASE",
                                 vTZTENVRO_REF, "TE_DBMS_Source", "Name" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "HANDLER",
                                 vTZTENVRO_REF, "TE_DBMS_Source",
                                 "Executable" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "GKHANDLER",
                                 vTZTENVRO_REF, "TE_DBMS_Source",
                                 "GenKeyExecutable" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "SironDB_Type",
                                 vTZTENVRO_REF, "TE_DBMS_Source",
                                 "SironDB_Type" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "CACHENUM",
                                 vTZZOLODO, "LOD",    "CacheNumberPerEntity" );
      SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "LOCK",
                                 vTZZOLODO, "LOD",    "DataLocking" );
      ...
      switch ( *pszParadigm )
      {
         case 'N':
            ...
            SetAttributeFromAttribute( vTZZOXODO, "OBJECT", "OFBUFSZ",
                                       vTZTENVRO_REF, "TE_DBMS_Source",
                                       "OverflowAttrLth" );
            break;
   ...
   SysReadZeidonIni( -1, "[Debug]", "DEBUGCHG", szDEBUGCHG );  // currently Java Debug only
   ...
   // If the LOD was built with TE (vTZTENVRO_REF view exists) and the
   // LOD_Entity is for entity in data model (ER_EntityRec exists), then
   // position on the corresponding table in TE using the ER_Entity.
   if ( vTZTENVRO_REF &&
        CheckExistenceOfEntity( vTZZOLOD1, "ER_EntityRec" ) >= zCURSOR_SET &&
        CompareAttributeToString( vTZZOLOD1, "ER_EntityRec", "Purpose",
                                  "W" ) != 0 )
   {
      GetIntegerFromAttribute( &lEntityZKey, vTZZOLOD1, "ER_EntityRec", "ZKey" );
      nRC = SetCursorFirstEntityByInteger( vTZTENVRO_REF, "ER_Entity",
                                           "ZKey", lEntityZKey,
                                           "TE_DBMS_Source" );
      if ( nRC < zCURSOR_SET )
      {
         zstrcpy( szMsg, "Unable to Find Entity in TE Data Source.\n\n"
                  "Entity Name = " );
         GetAddrForAttribute( &szMsgName, vTZZOLOD1, "ER_EntityRec", "Name" );
   ...
   if ( *pszParadigm &&
        CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Work", "Y" ) != 0 &&
        CompareAttributeToString( vTZZOLOD1, "ER_EntityRec",
                                  "Purpose", "W" ) != 0 )
   {
   ...
   // End of DonC change
      ...
      {
         if ( SetCursorFirstEntityByAttr( vTZTENVRO_REF, "ER_Attribute", "ZKey",
                                          vTZZOLOD1, "ER_AttributeRec", "ZKey",
                                          "TE_TablRec" ) < zCURSOR_SET )
         {
            if ( SetCursorFirstEntityByAttr( vTZTENVRO_REF, "ER_Attribute", "Name",
                                             vTZZOLOD1, "ER_AttributeRec", "Name",
                                             "TE_TablRec" ) < zCURSOR_SET )
            {
               zstrcpy( szMsg, "Unable to Find Attribute in TE.\n\nAttr Name = " );
               GetAddrForAttribute( &szMsgName, vTZZOLOD1,
   ...
   // Process any Duplicate Entities to make sure they have all necessary
   // ER attributes.
   ofnTZZOXODO_ProcDupEntity( vSubtask, vTZZOXODO, vTZEREMDO_REF, vTZTENVRO_REF,
                              vTZZOLOD1, &lAttrCount );

   // Complete any ATTRIB entries that have a DB_OPER specification.
   ofnTZZOXODO_BldDB_DataFields( vTZZOXODO, vTZZOLOD1 );

   // We'll assume that we have all the ER attributes in the entity.  If we
   // don't we will turn off the flag later.
   ...
   if ( CheckExistenceOfEntity( vTZZOLOD1, "ER_EntityRec" ) >= zCURSOR_SET &&
        SetCursorFirstEntityByEntityCsr( vTZEREMDO_REF, "ER_Entity",
                                         vTZZOLOD1, "ER_EntityRec", 0 ) >= zCURSOR_SET )
   {
      // Loop through all ER attributes.  If it's not part of the LOD, then
            ...
            nRC = SetCursorNextEntity( vTZEREMDO_REF, "ER_Attribute", 0 ) )
      {
         ...
         if ( SetCursorFirstEntityByEntityCsr( vTZZOLOD1, "ER_AttributeRec",
                                           vTZEREMDO_REF, "ER_Attribute",
                                           "LOD_EntityParent" ) >= zCURSOR_SET )
         {
            continue;
         }

         // Skip if it's a work attribute.
         if ( CompareAttributeToString( vTZEREMDO_REF, "ER_Attribute",
                                        "Work", "Y" ) == 0 )
         {
            continue;
         }
         ...
         CreateEntity( vTZZOXODO, "ATTRIB", zPOS_AFTER );
         lAttrCount++;
         SetAttributeFromInteger( vTZZOXODO, "ATTRIB", "XVAATT_TOK",
                                  lAttrCount );
      ...
      //    continue;
         ...
         if ( *pszParadigm &&
              (CompareAttributeToString( vTZEREMDO_REF, "ER_Attribute",
                                         "ParticipatesInKey", "Y" ) == 0 ||
               (CheckExistenceOfEntity( vTZZOLOD1,
                                        "LOD_AttributeRec" ) >= zCURSOR_SET &&
                CompareAttributeToString( vTZZOLOD1, "LOD_AttributeRec",
                                          "Work", "Y" ) != 0  &&
                CompareAttributeToString( vTZZOLOD1, "ER_AttributeRec",
                                          "Work", "Y" ) != 0  &&
                CompareAttributeToString( vTZZOLOD1, "ER_EntityRec", "Purpose",
                                          "W" ) != 0)) )
         {
            GetIntegerFromAttribute( &lAttribZKey, vTZEREMDO_REF,
                                     "ER_Attribute", "ZKey" );
            nRC = SetCursorFirstEntityByInteger( vTZTENVRO_REF, "ER_Attribute",
                                                "ZKey", lAttribZKey,
                                                "TE_TablRec" );
            if ( nRC < zCURSOR_SET )
            {
               zstrcpy( szMsg, "Unable to Find Attribute in TE."
                        "\n\nAttr Name = " );
               GetAddrForAttribute( &szMsgName, vTZTENVRO_REF,
                                    "ER_Attribute", "Name" );
               zstrcat( szMsg, szMsgName );
               zstrcat( szMsg, "\nfor entity: " );
               GetAddrForAttribute( &szMsgName, vTZZOLOD1,
                                    "ER_EntityRec", "Name" );
               zstrcat( szMsg, szMsgName );
               zstrcat( szMsg, "\n\n Please re-synchronize the TE "
                               "entities with ER Model" );
   ...
   if ( *pszParadigm )
   {
      // Loop for all data fields.  If there is no ER_Attribute record defined
      // under the field, then it's either a GenKey field or an AutoSeq field.
      // Add either accordingly.
      for ( nRC = SetCursorFirstEntityByString( vTZTENVRO_REF,
                                                "TE_FieldDataRel",
                                                "DataOrRelfieldOrSet", "D", 0 );
            nRC >= zCURSOR_SET;
            nRC = SetCursorNextEntityByString( vTZTENVRO_REF,
                                               "TE_FieldDataRel",
                                               "DataOrRelfieldOrSet", "D", 0 ) )
      {
         if ( CheckExistenceOfEntity( vTZTENVRO_REF,
...
//            if ( CheckExistenceOfEntity( vTZZOXODO, "DATARECORD" ) >= zCURSOR_SET )
      ...
      {
         for ( nRC = SetCursorFirstEntityByString( vTZTENVRO_REF, "TE_FieldDataRel",
                                                   "DataOrRelfieldOrSet", "R", 0 );
               nRC >= zCURSOR_SET;
               nRC = SetCursorNextEntityByString( vTZTENVRO_REF, "TE_FieldDataRel",
                                                  "DataOrRelfieldOrSet", "R", "" ) )
         {
            CreateEntity( vTZZOXODO, "ATTRIB", zPOS_LAST );
            lAttrCount++;
            SetAttributeFromInteger( vTZZOXODO, "ATTRIB", "XVAATT_TOK",
   ...
   // Loop through the ATTRIB entities to compute the total length and
   // put that length in the ENTITY.
   lEntityLth = 0;
   ...
   if ( nRC >= zCURSOR_SET )
   {
      ...
      ofnTZZOXODO_BldXODChildEnt( vSubtask, vTZZOXODO_Root, vTZZOXODO, vTZZOLOD1,
                                  vTZEREMDO_REF, vTZTENVRO_REF,
                                  pszParadigm, pszNetwork, &lAttrCount,
                                  nDerivedFlag, szDEBUGCHG );

      // For relational DB's we need to change the sequencing attributes so
      // that we sequence FK's instead of keys.
      if ( *pszParadigm == 'R' )
   ...
   // Create prototypes for all the operations that are global in scope and
   // merge them into the external header file.
                            ...
                            zCPCHAR cpcDEBUGCHG )
{
         ...
         nRC  = SetCursorNextEntity( vTZZOLOD1, "LOD_EntityChild", 0 ) )
   {
      ...
      // If the entity is not a work entity then there should be a matching
      // entity in the TE.  Set cursor to that entity.
      if ( *pszParadigm &&
           CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                     "Work", "Y" ) != 0 &&
           CompareAttributeToString( vTZZOLOD1, "ER_EntityRec",
     ...
     DropView( vTZZOLODR );
      ...
      // Process any Duplicate Entities to make sure they have all necessary
      // ER attributes.
      ofnTZZOXODO_ProcDupEntity( vSubtask, vTZZOXODO, vTZEREMDO_REF,
                                 vTZTENVRO_REF, vTZZOLOD1, plAttrCount );
      ...
      {
         SetCursorFirstEntityByEntityCsr( vTZEREMDO_REF, "ER_Entity",
                                          vTZZOLOD1, "ER_EntityRec", 0 );
         for ( nRC = SetCursorFirstEntity( vTZEREMDO_REF, "ER_Attribute", 0 );
               nRC >= zCURSOR_SET;
               nRC = SetCursorNextEntity( vTZEREMDO_REF, "ER_Attribute", 0 ) )
         {
            nRC = SetCursorFirstEntityByEntityCsr( vTZZOLOD1, "ER_AttributeRec",
                                                   vTZEREMDO_REF, "ER_Attribute",
                                                   "LOD_EntityParent" );
            if ( nRC >= zCURSOR_SET )
               continue;
                                         ...
                                         "Purpose", "W" ) != 0 )
            {
               GetIntegerFromAttribute( &lAttribZKey, vTZEREMDO_REF,
                                       "ER_Attribute", "ZKey" );
               nRC = SetCursorFirstEntityByInteger( vTZTENVRO_REF,
                                                   "ER_Attribute",
                                                   "ZKey", lAttribZKey,
                                                   "TE_TablRec" );
               if ( nRC < zCURSOR_SET )
               {
                  zstrcpy( szMsg, "Unable to Find Attribute in TE."
                                  "\n\nAttr Name = " );
                  GetAddrForAttribute( &szMsgName, vTZEREMDO_REF, "ER_Attribute", "Name" );
                  zstrcat( szMsg, szMsgName );
                  zstrcat( szMsg, "\nfor entity: " );
                  GetAddrForAttribute( &szMsgName, vTZZOLOD1, "ER_EntityRec", "Name" );
                  zstrcat( szMsg, szMsgName );
         ...
         if ( *pszParadigm &&
              CompareAttributeToString( vTZZOLOD1, "ER_EntityRec", "Purpose",
                                        "W" ) != 0 )
         {
            // Loop for all data fields.  If there is no ER_Attribute record
            // defined under the field, then it's either a GenKey field or
            // an AutoSeq field. Add either accordingly.
            for ( nRC = SetCursorFirstEntityByString( vTZTENVRO_REF, "TE_FieldDataRel",
                                                      "DataOrRelfieldOrSet", "D", 0 );
                  nRC >= zCURSOR_SET;
                  nRC = SetCursorNextEntityByString( vTZTENVRO_REF, "TE_FieldDataRel",
            ...
            if ( CompareAttributeToString( vTZZOLOD1, "ER_EntityRec",
                                           "Purpose", "W" ) != 0 &&
                 CompareAttributeToString( vTZZOLODO_Parent, "LOD_EntityParent",
                                           "Work", "Y" ) != 0 &&
                 CompareAttributeToString( vTZZOLODO_Parent, "ER_EntityRec",
                                           "Purpose", "W" ) != 0 )

            {
               CreateEntity( vTZZOXODO, "RELRECORD", zPOS_AFTER );
               ofnTZZOXODO_BldXODRelRec( vSubtask, vTZZOXODO, vTZTENVRO_REF, vTZZOLOD1,
                                         vTZTENVRO_Parent, pszParadigm, plAttrCount );
            }
      ...
      // Check to make sure that recursive entities have work attributes that
      // match their recursive parents.
      // NOTE!
      // NOTE! This change does not keep XVAATT_TOKs in order.  This shouldn't
      // NOTE! be a problem because the tokens are only used for persistent
      // NOTE! attributes but someday we should probably change it so that
      // NOTE! the tokens are "correct".
      // NOTE!
      ...
      if ( CheckExistenceOfEntity( vTZZOLOD1,
                                   "LOD_EntityChild" ) >= zCURSOR_SET )
      {
         ofnTZZOXODO_BldXODChildEnt( vSubtask, vTZZOXODO_Root, vTZZOXODO,
                                     vTZZOLOD1, vTZEREMDO_REF,
                                     vTZTENVRO_REF, pszParadigm,
                                     pszNetwork, plAttrCount,
                                     nDerivedFlag, cpcDEBUGCHG );
      }
      ...
      // 1. If all of the "not hidden" attributes under the XOD Entity are
      //    keys.  If this is true, then all of those key attributes should
      //    be in the parent as foreign keys.

      // 2. If the entity being processed is a leaf node (has no child)
      //    OR
      //    If the entity has only one child and that
      //    relationship to its child is in the Identifier for the entity
      //    being processed.
      ...
      if ( *pszParadigm == 'R' && nDerivedFlag == 0 )
      {
         ...
         if ( CheckExistenceOfEntity( vTZZOLOD1, "ER_EntityRec" ) >= zCURSOR_SET )
         {
            SetCursorFirstEntityByAttr( vWorkERD, "ER_Entity", "ZKey",
                                        vTZZOLOD1, "ER_EntityRec", "ZKey", 0 );
         }
         ...
         if ( nAutoLoadFlag == 1 )
         {
            // The first condition above was successful. We will now check
            // the second. We will actually make three separate tests.
            ...
            else
            {
               ...
               {
                  // There is a single child. We must now check to see if the
                  // relationship to the child is included in the Identifier
                  // for the entity being processed.

                  // First position on child relationship in the ERD.
                  SetViewToSubobject( vTZZOLOD1, "LOD_EntityChild" );
                  SetCursorFirstEntityByAttr( vWorkERD,  "ER_RelLink",    "ZKey",
                                              vTZZOLOD1, "ER_RelLinkRec", "ZKey", 0 );
                  ResetViewFromSubobject( vTZZOLOD1 );

                  // 3. Try to locate relationship in Identifier.
                  nRC = SetCursorFirstEntityByAttr( vWorkERD, "ER_RelLinkIdentifier", "ZKey",
                                                    vWorkERD, "ER_RelLink_Other",     "ZKey",
                                                    "ER_EntIdentifier" );
                  if ( nRC >= zCURSOR_SET )
                  {
                     // The relationship is part of the Identifer, set AUTOLOAD
                     // flag.
                     nAutoLoadFlag = 1;
                  }
                  else
                  {
                     // The relationship is NOT part of the Identifer, don't
         ...
         // Now set the AUTOLOADFROMPARENT flag based on results of the logic
         // above.
         if ( nAutoLoadFlag == 1 )
         {
            SetAttributeFromString( vTZZOXODO, "ENTITY",
                                    "AUTOLOADFROMPARENT", "Y" );
         }
      }

      // nDerivedFlag tells me, whether the currend Entity or any of its parents
      // is derived.
      //if ( nDerivedFlag &&
...
ofnTZZOXODO_BldXODEntity( zVIEW vSubtask, zVIEW vTZZOXODO, zVIEW vTZZOLOD1 )
{
   ...
   SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "NAME",
                              vTZZOLOD1, "LOD_EntityParent", "Name" );
   SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "LOCK",
                              vTZZOLOD1, "LOD_EntityParent", "DataLocking" );
   SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ACT_LIMIT",
                              vTZZOLOD1, "LOD_EntityParent", "MultipleChildLimit" );
   SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "XML_NAME",
                              vTZZOLOD1, "LOD_EntityParent", "XML_Name" );
   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "AutoCreate", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "AUTOCREATE",
                                 vTZZOLOD1, "LOD_EntityParent", "AutoCreate" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Create", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "CREATE",
                                 vTZZOLOD1, "LOD_EntityParent", "Create" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Delete", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "DELETE",
                                 vTZZOLOD1, "LOD_EntityParent", "Delete" );
   }
   ...
   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Exclude", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "EXCLUDE",
                                 vTZZOLOD1, "LOD_EntityParent", "Exclude" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Include", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "INCLUDE",
                                 vTZZOLOD1, "LOD_EntityParent", "Include" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "InclSrc", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "INCLSRC",
                                 vTZZOLOD1, "LOD_EntityParent", "InclSrc" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Hidden", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "HIDDEN",
                                 vTZZOLOD1, "LOD_EntityParent", "Hidden" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "Recursive", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "RECURSIVE",
                                 vTZZOLOD1, "LOD_EntityParent", "Recursive" );
   }
   ...
   if ( (CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
        ...
        (CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                   "SeqType", "A" ) == 0) )
   {
      GetStringFromAttribute( szInd, vTZZOLOD1, "LOD_EntityParent", "SeqType" );
      SetAttributeFromString( vTZZOXODO, "ENTITY", "SEQTYPE", szInd );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "DupEntityInstance", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "DUPENTIN",
                                 vTZZOLOD1, "LOD_EntityParent", "DupEntityInstance" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "DupRelationshipInstance", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "DUPRELIN",
                                 vTZZOLOD1, "LOD_EntityParent",
                                 "DupRelationshipInstance" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                  "LazyLoadFlag", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "LAZYLOAD",
                                 vTZZOLOD1, "LOD_EntityParent", "LazyLoadFlag" );
   }
   ...
   if ( (CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
        ...
        (CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                   "ParentDeleteBehave", "R" ) == 0) )
   {
      GetStringFromAttribute( szInd, vTZZOLOD1,
                              "LOD_EntityParent", "ParentDeleteBehave" );
   ...
   if (( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
     ...
     ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                 "Update", "Y" ) == 0 ))
      nRC = SetAttributeFromString( vTZZOXODO, "ENTITY", "UPDATE",
                                    "Y" );
   if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent", "Work", "Y" ) == 0 )
   {
      nRC = SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ERENT_TOK",
...
//                               vTZZOLOD1, "LOD_EntityConstraintOperRec", "Name" );
      ...
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ECDEL",
                      vTZZOLOD1, "LOD_EntityParent", "DeleteConstraint" );
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ECINC",
                      vTZZOLOD1, "LOD_EntityParent", "IncludeConstraint" );
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ECEXC",
                      vTZZOLOD1, "LOD_EntityParent", "ExcludeConstraint" );
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ECACC",
                      vTZZOLOD1, "LOD_EntityParent", "AcceptConstraint" );
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ECCAN",
                      vTZZOLOD1, "LOD_EntityParent", "CancelConstraint" );
   }

   // Don't build rest on Root.
   nRC = GetIntegerFromAttribute( &lIndentLvl, vTZZOLOD1,
                                  "LOD_EntityParent", "IndentLvl" );
   if ( lIndentLvl == 0 )
      return( 0 );
   ...
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ERREL_TOK",
                                 vTZZOLOD1, "LOD_EntityParent", "ZKey" );
   ...
   {
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "ERREL_TOK",
                                 vTZZOLOD1, "ER_RelTypeRec", "ZKey" );
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "CARDMIN",
                                 vTZZOLOD1, "ER_RelLinkRec", "CardMin" );
      SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "HANG_FK",
                                 vTZZOLOD1, "ER_RelLinkRec", "HangingForeignKeyEntity" );

      // CardMax is 999999 in XOD if it is "M" in LOD.
      if ( CompareAttributeToString( vTZZOLOD1, "ER_RelLinkRec", "CardMax",
                                     "M" ) == 0 )
         SetAttributeFromInteger( vTZZOXODO, "ENTITY", "CARDMAX", 999999 );
      else
         SetAttributeFromAttribute( vTZZOXODO, "ENTITY", "CARDMAX",
                                    vTZZOLOD1, "ER_RelLinkRec", "CardMax" );

      if ( CompareAttributeToString( vTZZOLOD1, "LOD_EntityParent",
                                     "RelLinkIndicator", "L" ) == 0 )
      {
                          ...
                          zCPCHAR cpcDEBUGCHG )
{
   ...
   GetStringFromAttribute( szEntityName, vTZZOXODO, "ENTITY", "NAME" );

   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "NAME",
                              vTZZOLOD1, "ER_AttributeRec", "Name" );
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "XML_SIMPLE_NAME",
                              vTZZOLOD1, "ER_AttributeRec", "XML_SimpleName" );
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "XML_NAME_EXTERNAL",
                              vTZZOLOD1, "ER_AttributeRec", "XML_NameExternal" );

   GetStringFromAttribute( szAttribName, vTZZOXODO, "ATTRIB", "NAME" );
   if ( cpcDEBUGCHG && cpcDEBUGCHG[ 0 ] && zstrcmp( szAttribName, cpcDEBUGCHG ) == 0 )
      SetAttributeFromString( vTZZOXODO, "ATTRIB", "DEBUGCHG", "Y" );


   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "ERATT_TOK",
                              vTZZOLOD1, "ER_AttributeRec", "ZKey" );
   ...
   if ( CompareAttributeToString( vTZZOLOD1, "ER_AttributeRec",
                                  "NotNull", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "REQUIRED",
                                 vTZZOLOD1, "ER_AttributeRec", "NotNull" );
   }

   if ( CompareAttributeToString( vTZZOLOD1, "ER_AttributeRec",
                                  "CaseSens", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "CASESENS",
                                 vTZZOLOD1, "ER_AttributeRec", "CaseSens" );
   }

   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DB_OPER",
                              vTZZOLOD1, "LOD_AttributeRec", "DB_DerivedValue" );
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DOMAIN",
                              vTZZOLOD1, "DomainRec", "Name" );
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "APDM_TOK",
                              vTZZOLOD1, "DomainRec", "ZKey" );

   // If Domain.DataType is "Date" or "Time", set ATTRIB.TYPE to "T".
   GetStringFromAttribute( szDataType, vTZZOLOD1, "DomainRec", "DataType" );
   if ( *szDataType == 'D' || *szDataType == 'I' )
      SetAttributeFromString( vTZZOXODO, "ATTRIB", "TYPE", "T" );
   else
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "TYPE",
                                 vTZZOLOD1, "DomainRec", "DataType" );

   GetAddrForAttribute( &pchType, vTZZOXODO, "ATTRIB", "TYPE" );
   switch ( *pchType )
   {
      case zTYPE_STRING:
         if ( CompareAttributeToInteger( vTZZOLOD1, "ER_AttributeRec",
                                         "Lth", 0 ) == 0 ||
              CompareAttributeToString( vTZZOLOD1, "ER_AttributeRec",
                                        "Lth", "" ) == 0 )
         {
            SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "LTH",
                                       vTZZOLOD1, "DomainRec", "MaxStringLth" );
         }
         else
         {
            SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "LTH",
                                       vTZZOLOD1, "ER_AttributeRec", "Lth" );
         }
      ...
      case zTYPE_DECIMAL:
         ...
         SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DECIMAL",
                                    vTZZOLOD1, "DomainRec", "DecimalFormat" );
         break;
   ...
   // We need to make sure the GENKEY attribute is set here in case the
   // TE source is network. The GENKEY attribute used to be set in
   // BldXODDataField.
   // First use the ER_Entity position in the LOD to position on the correct
   // entity in the ERD. Then use the ER_Attribute position in the LOD to
   // position on the correct ER_AttributeIdentifier in the ERD. Then check
   // the corresponding ER_EntIdentifier (the Identifier entity) to see if
   // it is for a generated key.
   // but not for work LOD's (ER_EntityRec does not exist!)
   if (CheckExistenceOfEntity( vTZZOLOD1,"ER_EntityRec") >= zCURSOR_SET)
   {
     SetCursorFirstEntityByAttr( vTZEREMDO_REF, "ER_Entity",    "ZKey",
                                 vTZZOLOD1,     "ER_EntityRec", "ZKey", 0 );
     nRC = SetCursorFirstEntityByAttr( vTZEREMDO_REF, "ER_AttributeIdentifier", "ZKey",
                                       vTZZOLOD1,     "ER_AttributeRec",        "ZKey", "ER_Entity" );
     if ( nRC >= zCURSOR_SET &&
          CompareAttributeToString( vTZEREMDO_REF, "ER_EntIdentifier",
   ...
   if ( CompareAttributeToString( vTZZOLOD1, "LOD_AttributeRec",
                                  "Hidden", "Y" ) == 0 )
   {
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "HIDDEN",
                                 vTZZOLOD1, "LOD_AttributeRec", "Hidden" );
   }

   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "INIT",
                              vTZZOLOD1, "LOD_AttributeRec", "Init" );

   // Set SEQUENCING information.
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "SEQUENCING",
                              vTZZOLOD1, "LOD_AttributeRec", "SeqPriority" );
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "SEQ_AD",
                              vTZZOLOD1, "LOD_AttributeRec", "SeqOrder" );
                                  ...
                                  "DerivedExpression", "" ) != 0 )
   {
      ...
      // If the XPG is NULL then we don't need to use the XPG, just the
      // original simple "interpreter".
      if ( CompareAttributeToString( vTZZOLOD1, "ER_AttributeRec",
                                     "DerivedExpressionXPG", "" ) == 0 )
      {
         SetAttributeFromString( vTZZOXODO, "ATTRIB", "DERIVEDF", "DfExprVML" );
         SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DERIVEDE",
                                    vTZZOLOD1, "ER_AttributeRec",
                                    "DerivedExpression" );
      }
      else
      {
         SetAttributeFromString( vTZZOXODO, "ATTRIB", "DERIVEDF", "*xpg*" );
         SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DERIVEDOPERXPG",
                                    vTZZOLOD1, "ER_AttributeRec",
                                    "DerivedExpressionXPG" );
      }
   }
                                ...
                                zVIEW vTZEREMDO_REF, zCPCHAR cpcDEBUGCHG )
{
   ...
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "NAME",
                              vTZEREMDO_REF, "ER_Attribute", "Name" );

   GetStringFromAttribute( szAttribName, vTZZOXODO, "ATTRIB", "NAME" );

   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "ERATT_TOK",
                              vTZEREMDO_REF, "ER_Attribute", "ZKey" );
   if ( CompareAttributeToString( vTZEREMDO_REF, "ER_Attribute",
                                  "ParticipatesInKey", "Y" ) == 0 )
   {
      SetAttributeFromString( vTZZOXODO, "ATTRIB", "KEY", "Y" );
   ...
   {
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "REQUIRED",
                                 vTZEREMDO_REF, "ER_Attribute", "NotNull" );
   }
   ...
   {
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "CASESENS",
                                 vTZEREMDO_REF, "ER_Attribute", "CaseSens" );
   }

   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DOMAIN",
                              vTZEREMDO_REF, "Domain", "Name" );
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "APDM_TOK",
                              vTZEREMDO_REF, "Domain", "ZKey" );

   // If Domain.DataType is "Date" or "Time", set ATTRIB.TYPE to "T".
   GetStringFromAttribute( szDataType,
                           vTZEREMDO_REF, "Domain", "DataType" );
   ...
   {
      case zTYPE_STRING:
         if ( CompareAttributeToInteger( vTZEREMDO_REF, "ER_Attribute",
                                         "Lth", 0 ) == 0 ||
              CompareAttributeToString( vTZEREMDO_REF, "ER_Attribute",
                                        "Lth", "" ) == 0 )
         {
            SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "LTH",
                                       vTZEREMDO_REF, "Domain",
                                       "MaxStringLth" );
         }
         else
         {
            SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "LTH",
                                       vTZEREMDO_REF, "ER_Attribute", "Lth" );
         }
      ...
      case zTYPE_DECIMAL:
         SetAttributeFromInteger( vTZZOXODO, "ATTRIB", "LTH", 8 );
         SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DECIMAL",
                                    vTZEREMDO_REF, "Domain", "DecimalFormat" );
         break;
...
// DGC 12/11/96   Removed the following stuff because these types are invalid.
   ...
   if ( CompareAttributeToString( vTZEREMDO_REF, "ER_Attribute", "Work",
                                  "Y" ) != 0 )
   {
      SetAttributeFromString( vTZZOXODO, "ATTRIB", "PERSIST", "Y" );
   }
                           ...
                           zVIEW vTZTENVRO_REF, zVIEW vTZZOLODO )
{
   ...
   // RECNAME is the name of the record or SQL table.  If SQL_TableOwner is
   // specified, then prefix RECNAME with the owner name followed by a '.'.
   // First we have to check if the Entityname includes the tableowner.
   GetStringFromAttribute( szRecName, vTZTENVRO_REF, "TE_TablRec", "Name" );

   // If we find a '.' the entityname includes the ownername and we may
   // not check the SQL_TableOwner and the Defaultownwer
   pchDot = strchr( szRecName, '.' );
   if ( pchDot == 0 )
   {
      GetStringFromAttribute( szRecName,
                              vTZTENVRO_REF, "TE_TablRec", "SQL_TableOwner" );
      if ( szRecName[ 0 ] == 0 )
      {
         GetStringFromAttribute( szRecName,
                                 vTZTENVRO_REF, "TE_DBMS_Source", "DefaultOwner" );
      }
   ...
   SetAttributeFromAttribute( vTZZOXODO, "DATARECORD", "RECID",
                              vTZTENVRO_REF, "TE_TablRec", "DBMS_ID" );

   // Set to Primary for Temporary Reasons.
   SetAttributeFromString( vTZZOXODO, "DATARECORD", "TYPE", "P" );

   // If there is a POD_Entity, we must forward the DBH specific
   // information to the XOD.
                             ...
                             zVIEW vTZTENVRO_REF )
{
   ...
   nRC = SetCursorFirstEntityByEntityCsr( vTZTENVR1, "TE_FieldDataRelKey",
                                          vTZTENVR1, "TE_FieldDataRel", 0 );
   if ( nRC >= zCURSOR_SET )
   {
      SetAttributeFromString( vTZZOXODO, "ATTRIB", "KEY", "Y" );
#if 0
      if ( CompareAttributeToString( vTZTENVR1, "TE_TablRecKey",
                                     "Category", "1" ) == 0 ||
           CompareAttributeToString( vTZTENVR1, "TE_TablRecKey",
                                     "Category", "2" ) == 0 )
      {
         if ( CompareAttributeToString( vTZTENVR1, "ER_EntIdentifier",
                                        "SystemMaintained", "Y" ) == 0 )
            SetAttributeFromString( vTZZOXODO, "ATTRIB", "GENKEY", "Y" );
      }
#endif
   ...
   SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "XVAATT_TOK",
                              vTZZOXODO, "ATTRIB", "XVAATT_TOK" );
   SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDID",
                              vTZTENVRO_REF, "TE_FieldDataRel", "DBMS_ID" );
   SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDOFFSET",
                              vTZTENVRO_REF, "TE_FieldDataRel", "Offset" );
   SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDTYPE",
                              vTZTENVRO_REF, "TE_FieldDataRel", "DataType" );
   SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDLTH",
                              vTZTENVRO_REF, "TE_FieldDataRel", "Length" );
   SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDNAME",
                              vTZTENVRO_REF, "TE_FieldDataRel", "Name" );
...
ofnTZZOXODO_BldDB_DataFields( zVIEW vTZZOXODO, zVIEW vTZZOLOD1 )
{
   ...
   // We will now loop through the ATTRIB entities and complete any
   // entries that have a DB_OPER specification.  For each of these
   // we must add a DATAFIELD entry and tie it back to the original
   // ATTRIB entry with the DB_OPER specification.  We will do this
   // by locating the ATTRIB entry for the DB_LOD_AttributeRec entity and
   // its corresponding DATAFIELD entry.  Then we will duplicate that
   // entry, and change its XVAATT_TOK value to point to the original
   // ATTRIB entry.
   ...
   while ( nRC >= zCURSOR_SET )
   {
      if ( CompareAttributeToString( vTZZOXODO, "ATTRIB", "DB_OPER", "" ) != 0 )
      {
         ...
         SetCursorFirstEntityByInteger( vTZZOLOD2, "ER_AttributeRec", "ZKey",
                                        lAttribZKey, "LOD_EntityParent" );
         // Then position on ATTRIB entity for corresponding DB_ER_AttributeRec.
         GetIntegerFromAttribute( &lAttribZKey,
                                  vTZZOLOD2, "DB_ER_AttributeRec", "ZKey" );
         SetCursorFirstEntityByInteger( vTZZOXOD2, "ATTRIB", "ERATT_TOK",
                                        lAttribZKey, "" );
         GetIntegerFromAttribute( &lXVAATT_TOK,
                                  vTZZOXOD2, "ATTRIB", "XVAATT_TOK" );
         SetCursorFirstEntityByInteger( vTZZOXOD2, "DATAFIELD", "XVAATT_TOK",
                                        lXVAATT_TOK, "" );
         ...
         {
            CreateEntity( vTZZOXODO, "DATAFIELD", zPOS_AFTER );
            SetMatchingAttributesByName( vTZZOXODO, "DATAFIELD",
                                       vTZZOXOD2, "DATAFIELD", zSET_NULL );
            SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "XVAATT_TOK",
                                       vTZZOXODO, "ATTRIB",    "XVAATT_TOK" );
         }
                         ...
                         zVIEW vTZZOLOD1 )
{
   ...
   GetStringFromAttribute( szEntName, vTZTENVRO_REF, "TE_TablRec", "Name" );
   GetStringFromAttribute( szAttName, vTZTENVRO_REF, "TE_FieldDataRel", "Name" );
   ...
   {
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "REQUIRED",
                                 vTZTENVRO_REF, "TE_FieldDataRel", "SQL_NULLS" );
   }

   // We must set the ATTRIB.TYPE value from the Domain of the attribute which
   // is the original source of the foreign key. Because this could be
   // recursive up several levels in the LOD, we will follow the chain of
   // TE_FieldDataRelPK entries until we find a TE_FieldDataRel entity of
   // type "data". We will then get the Domain.DataType value for that entry.
   CreateViewFromViewForTask( &vTZTENVR1, vTZTENVRO_REF, 0 );
   nRC = SetCursorFirstEntityByAttr( vTZTENVR1,     "TE_FieldDataRel",    "ZKey",
                                     vTZTENVRO_REF, "TE_FieldDataRel_PK", "ZKey",
                                     "TE_DBMS_Source" );
   while ( nRC >= zCURSOR_SET &&
           CompareAttributeToString( vTZTENVR1, "TE_FieldDataRel",
                                     "DataOrRelfieldOrSet", "D" ) != 0 )
   {
      CreateViewFromViewForTask( &vTZTENVR2, vTZTENVR1, 0 );
      nRC = SetCursorFirstEntityByAttr( vTZTENVR1, "TE_FieldDataRel",    "ZKey",
                                        vTZTENVR2, "TE_FieldDataRel_PK", "ZKey",
                                        "TE_DBMS_Source" );
      DropView( vTZTENVR2 );
   }
   ...
   {
      // If Domain.DataType is "Date" or "Time", set ATTRIB.TYPE to "T".
      GetStringFromAttribute( szDataType,vTZTENVR1, "Domain", "DataType" );
      if ( szDataType[ 0 ] == 'D' || szDataType[ 0 ] == 'I' )
         SetAttributeFromString( vTZZOXODO, "ATTRIB", "TYPE", "T" );
      else
         SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "TYPE",
                                    vTZTENVR1, "Domain", "DataType" );

      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "DOMAIN",
                                 vTZTENVR1, "Domain", "Name" );
      SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "APDM_TOK",
                                 vTZTENVR1, "Domain", "ZKey" );
      if ( CompareAttributeToString( vTZTENVR1, "ER_Attribute", "CaseSens", "Y" ) == 0 )
      {
         SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "CASESENS",
                                    vTZTENVR1, "ER_Attribute", "CaseSens" );
      }
   }
   else
   {
      // If there is no ER_Attribute entity, we will treat this entry
      // as a System Maintained key.
                          ...
                          zPLONG plAttrCount )
{
   ...
   // Position on the TE_FieldDataRel entity that is the first foreign key for
   // the RelLink.  We must be careful not to stop at an AutoSeq field by
   // mistake.
   for ( nRC = SetCursorFirstEntityByInteger( vTZTENVR1, "ER_RelLink",
                                              "ZKey", lRelLinkZKey,
                                              "TE_DBMS_Source" );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByInteger( vTZTENVR1, "ER_RelLink",
                                             "ZKey", lRelLinkZKey,
                                             "TE_DBMS_Source" ) )
   {
   ...
   if ( nRC >= zCURSOR_SET )
   {
      if ( CompareAttributeToString( vTZTENVR1, "TE_TablRec", "DataOrRel",
                                     "D" ) == 0 )
      {
         SetAttributeFromString( vTZZOXODO, "RELRECORD", "OWNER_MEMB", "O" );
         if ( *pszParadigm == 'R' )
         {
            ofnTZZOXODO_BldRelFieldFK( vTZZOXODO, vTZTENVR1,
                                       vTZZOXOD1, vTZTENVR1_Alt );
         }
         else
         {
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "SETID1",
                                       vTZTENVR1, "TE_FieldDataRel", "DBMS_ID" );
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "SETNAME1",
      ...
      else
      {
         ...
         // specified, then prefix RECNAME with the owner name followed by a '.'.
         GetStringFromAttribute( szRecName,
                                 vTZTENVR1, "TE_TablRec", "SQL_TableOwner" );
         if ( *szRecName == 0 )
         {
            GetStringFromAttribute( szRecName,
                                    vTZTENVR1, "TE_DBMS_Source", "DefaultOwner" );
         }
         if ( *szRecName != 0 )
         {
            zstrcat( szRecName, "." );
            GetStringFromAttribute( &szRecName[ zstrlen( szRecName ) ],
                                    vTZTENVR1, "TE_TablRec", "Name" );
         }
         else
            GetStringFromAttribute( szRecName,
                                    vTZTENVR1, "TE_TablRec", "Name" );
         ...
         if ( *pszParadigm == 'R' )
         {
            ofnTZZOXODO_BldRelFieldsForMM( vSubtask, vTZZOXODO, vTZTENVR1,
                                           vTZZOXOD1, vTZTENVR1_Alt,
                                           vTZZOLOD1, plAttrCount );
         }
         else
         {
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "RECID",
                                       vTZTENVR1, "TE_TablRec", "DBMS_ID" );
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "SETID1",
                                       vTZTENVR1, "TE_FieldDataRel", "DBMS_ID" );
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "SETNAME1",
                                       vTZTENVR1, "TE_FieldDataRel", "Name" );
         ...
         if ( SetCursorNextEntity( vTZTENVR1, "TE_FieldDataRel",
                                   0 ) <  zCURSOR_SET )
         {
         ...
         if ( *pszParadigm == 'N' )
         {
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "SETID2",
                                       vTZTENVR1, "TE_FieldDataRel", "DBMS_ID" );
            SetAttributeFromAttribute( vTZZOXODO, "RELRECORD", "SETNAME2",
                                       vTZTENVR1, "TE_FieldDataRel", "Name" );
         }
      }
   }
   else
   {
      ...
      nRC = SetCursorFirstEntityByInteger( vTZTENVR1, "ER_RelType_O",
                                          "ZKey", lRelTypeZKey,
                                          "TE_DBMS_Source" );
      if ( nRC < zCURSOR_SET )
      {
      ...
      if ( nRC >= zCURSOR_SET )
      {
         SetAttributeFromString( vTZZOXODO, "RELRECORD", "OWNER_MEMB",
                                        "M" );
         if ( *pszParadigm == 'R' )
         {
            ofnTZZOXODO_BldRelFieldFK( vTZZOXODO, vTZTENVR1,
      ...
      {
         GetStringFromAttribute( szSironDB_Type, vTZTENVR1, "TE_DBMS_Source", "SironDB_Type");
         ...
         {
            // If the target entity for the relationship is a SironGroup, without
            // a Katalog (meaning it is a subordinate Siron Group), then it is valid
            // to get here.
            // We know that it is a subordinate Siron Group if there is a relationship
            // in the ERD and the entity has no key.
            GetStringFromAttribute( szEntityName,
                                    vTZZOLOD1, "ER_EntityRec", "Name" );

            //  Get access to ER Object
            nRC = GetViewByName( &vERD, "TZEREMDO_REF", vSubtask, zLEVEL_TASK );

            CreateViewFromViewForTask( &vERDWork, vERD, 0 );
            nRC = SetCursorFirstEntityByAttr( vERDWork, "ER_Entity", "Name",
                                              vTZZOLOD1, "ER_EntityRec",
                                              "Name", 0 );
            nRelError = 0;
            if ( nRC >= zCURSOR_SET )
            {
               ...
               if ( nRC == zCURSOR_SET )
               {
                   ...
                   while ( nRC >= zCURSOR_SET )
                   {
                       nRC = CompareAttributeToString(vERDWork, "ER_Attribute",
                                                      "ParticipatesInKey", "Y");
                       if ( nRC == 0 )
                       {
         ...
         if ( nRelError > 0 )
         {
            ...
            //------------------------------------------------------------------------
            zstrcpy( szRelName, "Relationship: '" );
            GetStringFromAttribute( szSrcEntity, vTZZOXOD1, "ENTITY", "NAME" );

            zstrcat( szRelName, szSrcEntity );
            zstrcat( szRelName, " (" );

            GetStringFromAttribute( szSrcRelLink, vTZZOLOD1, "ER_RelLinkRec", "Name" );

            zstrcat( szRelName, szSrcRelLink );
            zstrcat( szRelName, ") " );
            GetStringFromAttribute( szTgtEntity, vTZZOLOD1, "LOD_EntityParent", "Name" );
            zstrcat( szRelName, szTgtEntity );
            zstrcat( szRelName, "'" );
            //------------------------------------------------------------------------

            zstrcpy( szMsg, "Unable to Find Relationship in TE.\n\n" );
            zstrcat( szMsg, szRelName );
            zstrcat( szMsg, "\n\nA relationship may have been deleted and recreated with " );
            zstrcat( szMsg, "a new name or your ER may be out of sync with your TE. " );
            zstrcat( szMsg, "\nIf you recreated a relationship with a new name, you must delete" );
            zstrcat( szMsg, " and recreate the subobject in the LOD." );
            zstrcat( szMsg, "\nIf your ER is out of sync with the TE, you must delete and recreate your " );
            zstrcat( szMsg, "tables and relationships in the TE." );
            MessageSend( vSubtask, "ZO00305", "Operations",
                         szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
         }
      }
                           ...
                           zVIEW vTZZOXOD1, zVIEW vTZTENVR1_Alt )
{
   ...
   // Position on the TE_FieldDataRel entity that is the first foreign key for
   // the RelLink.  We must be careful not to stop at an AutoSeq field by mistake.
   for ( nRC = SetCursorFirstEntityByInteger( vTZTENVRO_REF, "ER_RelLink",
                                              "ZKey", lRelLinkZKey,
                                              "TE_DBMS_Source" );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByInteger( vTZTENVRO_REF, "ER_RelLink",
                                             "ZKey", lRelLinkZKey,
                                             "TE_DBMS_Source" ) )
   {
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      if ( CompareAttributeToString( vTZZOXODO, "RELRECORD", "OWNER_MEMB",
                                      "O" ) == 0 )
      {
         ...
         nRC = SetCursorFirstEntityByInteger( vTZTENVR1, "ER_Entity", "ZKey",
                                              lEntityZKey, "TE_DBMS_Source" );
         if ( nRC < zCURSOR_SET )
         {
            ...
            if ( nRC < zCURSOR_SET )
            {
               zstrcpy( szMsg, "Unable to Find Entity for Bld FK in TE."
                               "\n\nEntity Name = " );
               GetAddrForAttribute( &szMsgName, vTZZOXOD1, "ENTITY", "NAME" );
               zstrcat( szMsg, szMsgName );
               zstrcat( szMsg, "\n\n Please re-synchronize the TE entities "
                               "and relationships with ER Model" );
         ...
         // Position on parent Entity primary key which matches self's foreign key
         //  *HH* take care:
         //  if it is a "switched" foreign key, then the TE_FieldDataRel_PK might
         //  be the Base Key, which ist NOT in the parent.
         nRC = LocateTE_Field_PK( vTZTENVR1, vTZTENVRO_REF );
         if ( nRC < zCURSOR_SET ||
              CompareAttributeToAttribute( vTZZOXOD1, "ENTITY", "ERENT_TOK",
                                           vTZTENVR1, "ER_Entity", "ZKey" ) != 0 )
         {
            // The foreign key on self matches a foreign key on parent and
            // this is typically the case in relationship based identifiers where
            // the foreign key on parent is part of it's key.
            SetCursorFirstEntityByEntityCsr( vTZTENVR1, "TE_FieldDataRel_PK",
                                             vTZTENVRO_REF, "TE_FieldDataRel_PK",
                                             "TE_TablRec" );
            GetIntegerFromAttribute( &lAttrZKey, vTZTENVR1, "TE_FieldDataRel", "ZKey" );

            // Position on parent Entity attribute which is a foreign key which
            // participates in the key for parent.
            SetCursorFirstEntityByInteger( vTZZOXOD1, "ATTRIB", "ERATT_TOK",
                                           lAttrZKey, "" );
         }
         else
         {
            // The foreign key on self matches a primary key on parent we will
            // now determine if the primary key on parent is visible or manufactured.
            if ( CheckExistenceOfEntity( vTZTENVR1, "ER_Attribute" ) >= zCURSOR_SET )
               GetIntegerFromAttribute( &lAttrZKey, vTZTENVR1, "ER_Attribute", "ZKey" );
            else
               GetIntegerFromAttribute( &lAttrZKey, vTZTENVR1, "TE_FieldDataRel", "ZKey" );

            // Position on parent Entity attribute which participates in the key
            // for parent.
            SetCursorFirstEntityByInteger( vTZZOXOD1, "ATTRIB",
                                           "ERATT_TOK", lAttrZKey, "" );
         }

         // Position on the ATTRIB which implements this foreign key on self.
         // If the FK is an ER attribute, which is only possible in "wrong"
         // SAP data models, then we go by the ZKey of the ER attribute.
         if ( CheckExistenceOfEntity( vTZTENVRO_REF, "ER_Attribute" ) >= zCURSOR_SET )
            GetIntegerFromAttribute( &lFK_ZKey,
                                     vTZTENVRO_REF, "ER_Attribute", "ZKey" );
         else
            GetIntegerFromAttribute( &lFK_ZKey,
                                     vTZTENVRO_REF, "TE_FieldDataRel", "ZKey" );
         ...
         if ( CheckExistenceOfEntity( vTZZOXOD1, "ATTRIB" ) >= zCURSOR_SET )
            SetAttributeFromAttribute( vTZZOXODO, "RELFIELD", "SRCXVAATT_TOK",
                                       vTZZOXOD1, "ATTRIB", "XVAATT_TOK" );

         if ( CheckExistenceOfEntity( vTZZOXODO, "ATTRIB" ) >= zCURSOR_SET )
            SetAttributeFromAttribute( vTZZOXODO, "RELFIELD", "RELXVAATT_TOK",
                                       vTZZOXODO, "ATTRIB", "XVAATT_TOK" );
      }
      else // The relationship being implemented is implemented on parent
      {
         ...
         if ( nRC < zCURSOR_SET )
         {
            ...
            if ( nRC < zCURSOR_SET )
            {
               zstrcpy( szMsg, "Unable to Find Parent Entity for Bld FK in TE.\n\nEntity Name = " );
               GetAddrForAttribute( &szMsgName, vTZZOXOD1, "ENTITY", "NAME" );
               zstrcat( szMsg, szMsgName );
               zstrcat( szMsg, "\n\n Please re-synchronize the TE "
                               "entities and relationships with ER Model" );
         ...
         //  We then have to find the key part in TZTENVR1, which is going to the same
         //  Base Key.
         nRC = LocateTE_Field_PK( vTZTENVR1, vTZTENVRO_REF );
         if ( nRC < zCURSOR_SET ||
              CompareAttributeToAttribute( vTZZOXODO, "ENTITY", "ERENT_TOK",
                                           vTZTENVR1, "ER_Entity",
                                           "ZKey" ) != 0 )
         {
            SetCursorFirstEntityByEntityCsr( vTZTENVR1,     "TE_FieldDataRel_PK",
                                             ...
                                             "TE_TablRec" );
            GetIntegerFromAttribute( &lAttrZKey, vTZTENVR1, "TE_FieldDataRel", "ZKey" );
            SetCursorFirstEntityByInteger( vTZZOXODO, "ATTRIB", "ERATT_TOK",
                                           lAttrZKey, "" );
         }
         else
         {
            if ( CheckExistenceOfEntity( vTZTENVR1, "ER_Attribute" ) >= zCURSOR_SET )
            {
               GetIntegerFromAttribute( &lAttrZKey,
                                        vTZTENVR1, "ER_Attribute", "ZKey" );
            }
            else
            {
               GetIntegerFromAttribute( &lAttrZKey,
                                        vTZTENVR1, "TE_FieldDataRel", "ZKey" );
            }
      ...
      nRC = SetCursorNextEntityByInteger( vTZTENVRO_REF, "ER_RelLink", "ZKey",
                                          lRelLinkZKey, "TE_TablRec" );
   }
                               ...
                               zVIEW vTZZOLOD1, zPLONG plAttrCount )
{
   ...
   nRC = SetCursorFirstEntityByInteger( vTZTENVRO_REF, "ER_RelType_O",
                                        "ZKey", lRelTypeZKey, "TE_TablRec" );
   while ( nRC >= zCURSOR_SET )
   {
      if ( CheckExistenceOfEntity( vTZTENVRO_REF, "TE_FieldDataRel_PK" ) < zCURSOR_SET )
      {
      ...
      else
      {
         ...
         if ( CompareAttributeToAttribute( vTZTENVRO_REF, "ER_EntityRelLink", "ZKey",
                                           vTZZOXODO, "ENTITY", "ERENT_TOK" ) == 0 &&
              CompareAttributeToAttribute( vTZTENVRO_REF, "ER_RelLink", "ZKey",
                                           vTZZOLOD1, "ER_RelLinkRec", "ZKey" ) != 0 )
         {
            ...
            nRC = SetCursorFirstEntityByInteger( vTZZOXODO, "ATTRIB",
                                                 "ERATT_TOK", lAttrZKey, "" );
            if ( nRC >= zCURSOR_SET )
            {
               SetAttributeFromAttribute( vTZZOXODO, "RELFIELD", "SRCXVAATT_TOK",
         ...
         else
         {
                                              ...
                                              vTZZOXOD1, "ENTITY", "ERENT_TOK" ) == 0 )
            {
               ...
               nRC = SetCursorFirstEntityByInteger( vTZZOXOD1, "ATTRIB",
                                                   "ERATT_TOK", lAttrZKey, "" );
               if ( nRC >= zCURSOR_SET )
               {
                  nRC = SetAttributeFromAttribute( vTZZOXODO, "RELFIELD", "SRCXVAATT_TOK",
                                                   vTZZOXOD1, "ATTRIB", "XVAATT_TOK" );
               }
            }
         }

         SetAttributeFromAttribute( vTZZOXODO, "RELFIELD", "FLDNAME",
                                    vTZTENVRO_REF, "TE_FieldDataRel", "Name" );
      }

      nRC = SetCursorNextEntityByInteger( vTZTENVRO_REF, "ER_RelType_O",
                                          "ZKey", lRelTypeZKey, "TE_TablRec" );
   }
                            ...
                            zVIEW vTZTENVR1_Alt )
{
   ...
   if ( nRC < zCURSOR_SET )
   {
      ...
      if ( nRC < zCURSOR_SET )
      {
         zstrcpy( szMsg, "Unable to Find Entity for Base Field in TE."
                         "\n\nEntity Name = " );
         GetAddrForAttribute( &szMsgName, vTZZOXODO, "ENTITY", "NAME" );
         zstrcat( szMsg, szMsgName );
         zstrcat( szMsg, "\n\n Please re-synchronize the TE entities "
                         "with ER Model" );
   ...
   nRC = SetCursorFirstEntityByEntityCsr( vTZTENVR1, "TE_FieldDataRel",
                                          vTZTENVRO_REF, "TE_FieldDataRel_PK",
                                          "TE_TablRec" );
   if ( nRC < zCURSOR_SET )
   {
      nRC = SetCursorFirstEntityByEntityCsr( vTZTENVR1, "TE_FieldDataRel_PK",
                                             vTZTENVRO_REF, "TE_FieldDataRel_PK",
                                             "TE_TablRec" );
   }
   ...
   if ( nRC < zCURSOR_SET )
   {
      zstrcpy( szMsg, "Unable to Find Base Field in TE.\n\nEntity Name = " );
      GetAddrForAttribute( &szMsgName, vTZZOXODO, "ENTITY", "NAME" );
      zstrcat( szMsg, szMsgName );
      zstrcat( szMsg, "\n\n Please re-synchronize the TE "
                      "entities with ER Model" );
                        ...
                        zVIEW vTZTENVRO_REF, zVIEW vTZZOLOD1 )
{
   ...
   // Retrieve the Integer domain and set up corresponding values in
   // ATTRIB entity.
   ...
   SetAttributeFromAttribute( vTZZOXODO, "ATTRIB", "TYPE",
                              vDomain,   "Domain", "DataType" );
   DropMetaOI( vSubtask, vDomain );

   //
   // Create and build the DATAFIELD data for AutoSeq field if there
   // is a DATARECORD entry.
   //
   if ( CheckExistenceOfEntity( vTZZOXODO, "DATARECORD" ) >= zCURSOR_SET )
   {
      CreateEntity( vTZZOXODO, "DATAFIELD", zPOS_LAST );
      SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDLTH",
                                 vTZZOXODO, "ATTRIB",    "LTH" );

      SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDNAME",
                                 vTZZOXODO, "ATTRIB",    "NAME" );

      SetAttributeFromAttribute( vTZZOXODO, "DATAFIELD", "FLDTYPE",
                                 vTZZOXODO, "ATTRIB",    "TYPE" );
                           ...
                           zPLONG plAttrCount )
{
   ...
   // If the LOD_Entity specifies "DupEntityInstance", then make sure that
   // all Attributes for any other LOD_Entity for the same ER_Entity are in
   // the XOD.
         ...
         CheckExistenceOfEntity( vTZZOLOD1, "ER_EntityRec" ) >= zCURSOR_SET )
   {
      ...
      // Loop through all LOD_Entities for the same ER Entity that also
      // specify DupEntityInstance and make sure all of their attributes are
      // included under this XOD ENTITY.
      for ( nRC = SetCursorFirstEntityByEntityCsr( vLOD2,     "ER_Entity",
                                                   vTZZOLOD1, "ER_EntityRec", "LOD" );
            nRC >= zCURSOR_SET;
            nRC = SetCursorNextEntityByEntityCsr( vLOD2,     "ER_Entity",
                                                  vTZZOLOD1, "ER_EntityRec", "LOD" ) )
      {
         // Skip the original entity.
         if ( CompareAttributeToAttribute( vLOD2,     "LOD_Entity",       "ZKey",
                                           vTZZOLOD1, "LOD_EntityParent", "ZKey" )
                                           == 0 )
            continue;

         // Skip if it is not a DupEntityInstance.
         if ( CompareAttributeToString( vLOD2, "LOD_Entity",
                                        "DupEntityInstance", "Y" ) == 0 )
         {
            continue;
         }

         // Loop through all the attributes under this LOD entity and make sure
         // all ER attributes exist under the original LOD entity.
               ...
               nRC = SetCursorNextEntity( vLOD2, "LOD_Attribute", 0 ) )
         {
            ...
            nRC = SetCursorFirstEntityByEntityCsr( vTZZOLOD1,
                                                   "ER_AttributeRec",
                                                   vLOD2,
                                                   "ER_Attribute",
                                                   "LOD_EntityParent" );
            ...
            // Skip if it's a work attribute.
            if ( CompareAttributeToString( vLOD2, "ER_Attribute", "Work",
                                             "Y" ) != 0 &&
                  CompareAttributeToString( vLOD2, "LOD_Attribute", "Work",
                                             "Y" ) != 0 &&
                  CompareAttributeToString( vLOD2, "ER_Entity", "Purpose",
                                             "W" ) != 0 )
               continue;
            ...
            *plAttrCount++;
            ...
            if ( SetCursorFirstEntityByAttr( vTZTENVRO_REF, "ER_Attribute", "ZKey",
                                             vLOD2,         "ER_Attribute", "ZKey",
                                             "TE_TablRec" ) < zCURSOR_SET )
            {
               zstrcpy( szMsg, "Unable to Find Attribute in TE.\n\nAttr Name = " );
               GetAddrForAttribute( &szMsgName,
                                    vLOD2, "ER_Attribute", "Name" );
               zstrcat( szMsg, szMsgName );
               zstrcat( szMsg, "\nfor entity: " );
               GetAddrForAttribute( &szMsgName,
                                    vLOD2, "ER_Entity", "Name" );
               zstrcat( szMsg, szMsgName );
               zstrcat( szMsg, "\n\n Please re-synchronize the TE entities with ER Model" );
               MessageSend( vSubtask, "ZO00306", "Operations",
                           szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
...
// For a TE_Field in the current entity (vTZTENVRO_REF), find the
// matching key field in the parent (vTZTENVR1).
...
LocateTE_Field_PK( zVIEW vTZTENVR1, zVIEW vTZTENVRO_REF )
{
   ...
   // We assume, that the _PK from vTZTENVRO_REF is pointing to the Base Key.
   // This can't be true, if the _PK is not a ER attribute.
   if ( CompareAttributeToString( vTZTENVRO_REF, "TE_FieldDataRel_PK",
                                  "DataOrRelfieldOrSet", "D" ) != 0 )
   {
   ...
   GetIntegerFromAttribute( &lZKeyBase, vTZTENVRO_REF,
                            "TE_FieldDataRel_PK", "ZKey" );
   CreateViewFromViewForTask( &vTZTENVR2, vTZTENVR1, 0 );
   SetCursorFirstEntity( vTZTENVR2, "TE_TablRecKey", "" ); // not necessary

   // The candidate must be a foreign key, DataOrRelfieldOrSet = "R".
   for ( nRC = SetCursorFirstEntityByString( vTZTENVR2, "TE_FieldDataRelKey",
                                             "DataOrRelfieldOrSet", "R", 0 );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByString( vTZTENVR2, "TE_FieldDataRelKey",
                                            "DataOrRelfieldOrSet", "R", 0 ) )
   {
      SetCursorFirstEntityByEntityCsr( vTZTENVR2, "TE_FieldDataRel",
                                       vTZTENVR2, "TE_FieldDataRelKey", 0 );
   ...
   if ( nRC >= zCURSOR_SET )
   {
      ...
      SetCursorFirstEntityByEntityCsr( vTZTENVR1, "TE_FieldDataRel",
                                       vTZTENVR2, "TE_FieldDataRelKey", 0 );
   }
...
GetBaseKey( zVIEW vTZTENVRO )
{
   ...
   if ( CompareAttributeToString( vTZTENVRO, "TE_FieldDataRel_PK",
                                  "DataOrRelfieldOrSet", "D" ) == 0 )
   {
   ...
   nRC = SetCursorFirstEntityByAttr( vTZTENVR3, "TE_FieldDataRel",    "ZKey",
                                     vTZTENVRO, "TE_FieldDataRel_PK", "ZKey",
                                     "TE_DBMS_Source" );
   while ( nRC >= zCURSOR_SET &&
          CompareAttributeToString( vTZTENVR3, "TE_FieldDataRel_PK",
                                    "DataOrRelfieldOrSet", "D" ) != 0 )
   {
      CreateViewFromViewForTask( &vTZTENVR4, vTZTENVR3, 0 );
      nRC = SetCursorFirstEntityByAttr( vTZTENVR3, "TE_FieldDataRel",    "ZKey",
                                        vTZTENVR4, "TE_FieldDataRel_PK", "ZKey",
                                        "TE_DBMS_Source" );
      DropView( vTZTENVR4 );
   }
