                                ...
                                zPCHAR   szScopingEntityTE )
{
   ...
   pfnGenerateName = SysGetProc( hLibrary, "GenerateName" );
   if ( !pfnGenerateName )
   {
   ...
   pfnSetDataType = SysGetProc( hLibrary, "SetDataType" );
   if ( !pfnSetDataType )
   {
         ...
         RESULT = SetCursorNextEntity( vEMD, "ER_Entity", 0 ) )
   {
      ...
      RESULT = SetCursorFirstEntityByAttr( vDTE3, "ER_Entity", "Name",
                                           vEMD, "ER_Entity", "Name",
                                           szScopingEntityTE );
      if ( RESULT < zCURSOR_SET )
      {
         // If RESULT = zCURSOR_UNCHANGED, it is possible, that
         // the entity is already there. Check this here.
         if (CheckExistenceOfEntity( vDTE3, "ER_Entity") == zCURSOR_SET)
            RESULT = CompareAttributeToAttribute(vEMD, "ER_Entity", "Name",
                                                 vDTE3, "ER_Entity", "Name");
         else
            RESULT = 1;
         ...
         if ( RESULT != 0 )
         {
            ...
            // Entity name.
            GetStringFromAttribute( szName, vEMD, "ER_Entity", "TE_TableName" );
            if ( *szName == 0 )
            {
            ...
            SetAttributeFromAttribute( vDTE, "TE_TablRec", "SQL_TableOwner",
                                       vEMD, "ER_Entity",  "TE_TableOwner" );
            SetAttributeFromString( vDTE, "TE_TablRec", "DataOrRel", "D" );

            // If the table isn't found in the DBMS_Source we have to process
            // identifiers.
      ...
      if (bExists)
      {
         ...
         {
            // Identifier processing should not run, if only table attributes are
            // added.
      ...
      // This will be done only if the ER_Entity exists under the current
      // TE_DBMS_Source entity.
      RESULT = SetCursorFirstEntityByEntityCsr( vDTE, "ER_Entity",
                                                vEMD, "ER_Entity",
                                                "TE_DBMS_Source" );
      if ( RESULT >= zCURSOR_SET )
      {
         ...
         {
            RESULT = SetCursorFirstEntityByEntityCsr( vDTE, "ER_Attribute",
                                                      vEMD, "ER_Attribute",
                                                      "TE_TablRec" );
            ...
            // Attribute name.
            GetStringFromAttribute( szName, vEMD, "ER_Attribute", "TE_ColumnName" );
            if ( *szName == 0 )
            {
            ...
            {
               // If the column name is embedded in quotation marks, delete
               // them for DB2, otherwise we get runtime errors
               if ( *szName == '\"' )
            ...
            SetAttributeFromAttribute( vDTE, "TE_FieldDataRel", "Desc",
                                       vEMD, "ER_Attribute", "Desc" );
            IncludeSubobjectFromSubobject( vDTE, "ER_Attribute",
                                          vEMD, "ER_Attribute", zPOS_AFTER );
            SetAttributeFromString( vDTE, "TE_FieldDataRel",
                                    "DataOrRelfieldOrSet", "D" );
         ...
         if ( !bModifyExistingTables )
         {
            ...
            {
               // Delete the Identifier entry if it currently exists.  Then
               // re-add it.
               RESULT = SetCursorFirstEntityByEntityCsr( vDTE, "ER_EntIdentifier",
                                                         vEMD, "ER_EntIdentifier",
                                                         "TE_TablRec" );
               if ( RESULT >= zCURSOR_SET )
               ...
               SetAttributeFromAttribute( vDTE, "TE_TablRecKey",    "IndexName",
               ...
               // Set description.
               zstrcpy( szTemp, "Implements " );
               GetStringFromAttribute( &szTemp[ zstrlen( szTemp ) ],
                                       vEMD, "ER_EntIdentifier", "Desc" );
               SetAttributeFromString( vDTE, "TE_TablRecKey", "Desc", szTemp );

               SetAttributeFromString( vDTE, "TE_TablRecKey", "Category", "9" );
               IncludeSubobjectFromSubobject( vDTE, "ER_EntIdentifier",
                                             vEMD, "ER_EntIdentifier", zPOS_AFTER );

               if ( CheckExistenceOfEntity( vEMD, "ER_FactType" ) >= zCURSOR_SET )
               {
                  if ( SetCursorFirstEntity( vEMD, "ER_RelLinkIdentifier",
                                             "ER_EntIdentifier" ) < zCURSOR_SET )
                  {
               ...
               {
                  if ( CompareAttributeToString( vEMD, "ER_EntIdentifier",
                                                "SystemMaintained", "Y" ) == 0 )
                  {
                     // We have a System Maintained Key with no FactType.
                     ofnTZTENVRO_BldPrimaryKeyToken( vSubtask, vDTE,
                                                     vEMD, pfnGenerateName );
                  }
                  else
                  {
                     zstrcpy( szTemp, "Entity does not have a valid Key. " );
                     GetStringFromAttribute( szName, vEMD, "ER_Entity", "Name" );
...
// evaluation.
//            zgSortEntityWithinParent( zASCENDING, vDTE, "TE_TablRecKey",
//                                    "Category", "" );
         }
      }
   }
   ...
   if ( !bModifyExistingTables )
   {
      // We are looping here through ll the TablRecKey entries that have not
      ...
      for ( RESULT = SetCursorFirstEntityByString( vDTE, "TE_TablRecKey",
                                                   "Category", "9",
                                                   "TE_DBMS_Source" );
            RESULT > zCURSOR_UNCHANGED;
            RESULT = SetCursorNextEntityByString( vDTE, "TE_TablRecKey",
                                                  "Category", "9",
                                                  "TE_DBMS_Source" ) )
      {
         SetCursorFirstEntityByEntityCsr( vEMD, "ER_EntIdentifier",
                                          vDTE, "ER_EntIdentifier",
                                          "EntpER_Model" );
...
//  ***ADD ???? from ERD.
   ...
   SetAttributeFromString( vDTE, "TE_FieldDataRel", "Desc",
                           "Auto build key" );
   SetAttributeFromString( vDTE, "TE_FieldDataRel", "DataOrRelfieldOrSet",
                           "D" );
   ...
   IncludeSubobjectFromSubobject( vDTE, "TE_FieldDataRelKey",
                                  vDTE, "TE_FieldDataRel", zPOS_FIRST );
                               ...
                               zVIEW    vEMD )
{
   ...
   {
      if ( CheckExistenceOfEntity( vEMD,
                                   "ER_AttributeIdentifier" ) >= zCURSOR_SET )
      {
         SetCursorFirstEntityByEntityCsr( vDTE, "ER_Attribute",
                                          vEMD, "ER_AttributeIdentifier",
                                          "TE_TablRec" );
         SetAttributeFromString( vDTE, "TE_FieldDataRel", "Key", "Y" );
         IncludeSubobjectFromSubobject( vDTE, "TE_FieldDataRelKey",
                                        vDTE, "TE_FieldDataRel", zPOS_LAST );

         if ( CompareAttributeToString( vDTE, "TE_FieldDataRel",
                                        "SQL_NULLS", "Y" ) != 0 &&
              GetRelativeEntityNumber( vEMD, "ER_EntIdentifier", "", 0 ) == 0 )
         {
            zstrcpy( sz, "Attribute '" );
            GetAddrForAttribute( &szName, vEMD, "ER_AttributeIdentifier",
                                 "Name" );
            zstrcat( sz, szName );
            zstrcat( sz, "' in Identifier '" );
            GetAddrForAttribute( &szName, vEMD, "ER_EntIdentifier", "Name" );
            zstrcat( sz, szName );
            zstrcat( sz, "' is specified as NOT required. " );
            zstrcat( sz, "Physical key will be created as required. " );
            zstrcat( sz, "Please correct ER Data Model." );
                                 ...
                                 zVIEW    vEMD )
{
   ...
   {
      // Process the condition where this part of the Identifier is simply
      // an Attribute.
      if ( CheckExistenceOfEntity( vEMD,
                                   "ER_AttributeIdentifier" ) >= zCURSOR_SET )
      {
      ...
      // Relationship.  What we are doing here is simply skipping that
      // entry.  (DonC)
      if ( CheckExistenceOfEntity( vEMD, "ER_Entity_Other_Identifier" )
                                                              < zCURSOR_SET )
         continue;
      ...
      if ( SetCursorFirstEntityByEntityCsr( vDTE2, "ER_Entity",
                                            vEMD, "ER_Entity_Other_Identifier",
                                            "TE_DBMS_Source" ) < zCURSOR_SET )
      {
         zstrcpy( sz, "Cannot locate Entity in TE in same DS for Rel Key." );
         GetAddrForAttribute( &szName, vEMD, "ER_Entity_Other_Identifier", "Name" );
         oTZTENVRO_WriteErrorMsgToList( vDTE, szName, sz );
#if 0
         for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE2, "ER_Entity",
                                           vEMD, "ER_Entity_Other_Identifier",
                                           "TE_DB_Environ" );
               RESULT > zCURSOR_UNCHANGED;
               RESULT = SetCursorNextEntityByEntityCsr( vDTE2, "ER_Entity",
                                           vEMD, "ER_Entity_Other_Identifier",
                                           "TE_DB_Environ" ) )
         {
            if ( CompareAttributeToAttribute( vDTE, "TE_DBMS_Source", "DBMS",
                                              vDTE2, "TE_DBMS_Source",
                                              "DBMS" ) == 0 )
            {
      ...
      {
         zstrcpy( sz, "ER Relationship has Entity without an Identifier." );
         GetAddrForAttribute( &szName, vEMD, "ER_Entity_Other_Identifier", "Name" );
         oTZTENVRO_WriteErrorMsgToList( vDTE, szName, sz );
         continue;
      }
      ...
      for ( RESULT = SetCursorFirstEntity( vDTE2, "TE_TablRecKey",
                                           "TE_TablRec" );
            RESULT > zCURSOR_UNCHANGED;
            RESULT = SetCursorNextEntity( vDTE2, "TE_TablRecKey",
                                          "TE_TablRec" ) )
      {
         zPCHAR psz;
      ...
      if ( RESULT < zCURSOR_SET )
      {
         ...
         // been processed.
         SetCursorFirstEntityByEntityCsr( vEMD2, "ER_EntIdentifier",
                                          vDTE2, "ER_EntIdentifier",
                                          "EntpER_Model" );
      ...
      // parent TempRelKey entries, which are the relationship parts of the
      // key.
            ...
            RESULT = SetCursorNextEntity( vDTE2, "TE_FieldDataRelKey", 0 ) )
      {
         ...
         SetAttributeFromAttribute( vDTE,  "TempRelKey",                 "EntityZKey",
                                    vEMD,  "ER_Entity_Other_Identifier", "ZKey" );
         SetAttributeFromAttribute( vDTE,  "TempRelKey",         "KeyName",
                                    vDTE2, "TE_FieldDataRelKey", "Name" );
         SetAttributeFromAttribute( vDTE,  "TempRelKey",           "RelZKey",
                                    vEMD,  "ER_RelLinkIdentifier", "ZKey" );
      }
                            ...
                            zVIEW vDTE_Save )
{
   ...
   *nRelImplemented = zREL_IMPLD_NOT;
   ...
   // see if the relationship is between two entities implemented in
   // the current Data Source or with same DBMSs.
   nEntsInDS = 0;
   ...
   {
      bSrcInDS = FALSE;
      for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE, "ER_Entity",
                                    vEMD, "ER_Entity_2", "TE_DB_Environ" );
            RESULT > zCURSOR_UNCHANGED;
            RESULT = SetCursorNextEntityByEntityCsr( vDTE, "ER_Entity",
                                    vEMD, "ER_Entity_2", "TE_DB_Environ" ) )
      {
         if ( CompareAttributeToAttribute( vDTE, "TE_DBMS_Source", "DBMS",
                     vDTE_Save, "TE_DBMS_Source", "DBMS" ) == 0 )
         {
            break;   // found Entity implemented in same DBMS
         }
      }
   ...
   else
   {
      bTgtInDS = FALSE;
      for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE2, "ER_Entity",
                                    vEMD2, "ER_Entity_2", "TE_DB_Environ" );
            RESULT > zCURSOR_UNCHANGED;
            RESULT = SetCursorNextEntityByEntityCsr( vDTE2, "ER_Entity",
                                    vEMD2, "ER_Entity_2", "TE_DB_Environ" ) )
      {
         if ( CompareAttributeToAttribute( vDTE2, "TE_DBMS_Source", "DBMS",
                     vDTE_Save, "TE_DBMS_Source", "DBMS" ) == 0 )
         {
            break;   // found Entity implemented in same DBMS
         }
   ...
   // see if the relationship is currently implemented in
   ...
   if ( SetCursorFirstEntityByEntityCsr( vDTE, "ER_RelType_O",
                                         vEMD, "ER_RelType",
                                         "TE_TablRec" ) > zCURSOR_UNCHANGED )
   {
   ...
   else
   {
      ...
      if ( SetCursorFirstEntityByEntityCsr( vDTE2, "ER_RelType_O",
                                            vEMD, "ER_RelType",
                                            "TE_TablRec" ) > zCURSOR_UNCHANGED )
      {
         bRelImplemented = TRUE;
      ...
      {
         // Let's try to find as a TablRec in the current DS.  If so, it's
         // already been processed and we'll skip it.
         SetViewFromView( vDTE_RelTR, vDTE_Save );
         if ( SetCursorFirstEntityByEntityCsr( vDTE_RelTR, "ER_RelType",
                                           vEMD, "ER_RelType",
                                          "TE_DBMS_Source" ) >= zCURSOR_SET )
         {
//            bRelImplemented = TRUE;
...
//            *nRelImplemented = zREL_IMPLD_AsTablRec;
         ...
         else
         {
            ...
            for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE_RelTR,
                          "ER_RelType", vEMD, "ER_RelType", "TE_DB_Environ" );
                  RESULT > zCURSOR_UNCHANGED;
                  RESULT = SetCursorNextEntityByEntityCsr( vDTE_RelTR,
                          "ER_RelType", vEMD, "ER_RelType", "TE_DB_Environ" ) )
            {
               if ( CompareAttributeToAttribute( vDTE_RelTR, "TE_DBMS_Source",
                                                 "DBMS",
                                                 vDTE_Save, "TE_DBMS_Source",
                                                 "DBMS" ) == 0 )
               {
   ...
   // if the Rel is implemented and either Entity is 'work', then
   ...
   if ( bRelImplemented )
   {
                                     ...
                                     "W" ) == 0 )
      {
         ...
         {
            RESULT = SetCursorFirstEntityByEntityCsr( vDTE, "ER_RelType_O",
                                 vEMD, "ER_RelType", "TE_DBMS_Source" );
            while ( RESULT > zCURSOR_UNCHANGED )
            {
               // was NOT a MM rel so delete all FieldDataRec's used to
               // implement
               DeleteEntity( vDTE, "TE_FieldDataRel", zREPOS_NONE );
               RESULT = SetCursorNextEntityByEntityCsr( vDTE, "ER_RelType_O",
                                                        vEMD, "ER_RelType",
                                                        "TE_TablRec" );
            }
         }
   ...
   if ( bSrcIsMany && bTgtIsMany )
   {
      ...
      {
         zstrcpy( sz, "Relationship: '" );

         SetCursorFirstEntityByEntityCsr( vEMD, "ER_Entity", vEMD, "ER_Entity_2", "" );
         GetStringFromAttribute( szSrcEntity, vEMD, "ER_Entity", "Name" );
         zstrcpy( szRelDesc, szSrcEntity );
         zstrcat( szRelDesc, " (" );
         GetStringFromAttribute( szSrcRelLink, vEMD, "ER_RelLink_2", "Name" );
         zstrcat( szRelDesc, szSrcRelLink );
         SetCursorFirstEntityByEntityCsr( vEMD2, "ER_Entity", vEMD2, "ER_Entity_2", "" );
         GetStringFromAttribute( szTgtEntity, vEMD2, "ER_Entity_2", "Name" );
         zstrcat( szRelDesc, ") " );
         zstrcat( szRelDesc, szTgtEntity );

         zstrcat( sz, szRelDesc );
         ...
         if (  MessagePrompt( vDTE_RelTR,  "TE00105",
                              "Physical Data Model",
                              sz,
         ...
         {
            zstrcpy( sz, "Delete current implementation before re-implementing?" );
            if (  MessagePrompt( vDTE_RelTR,  "TE00111",
                                 "Physical Data Model",
                                 sz,
                                 zBEEP,        zBUTTONS_YESNO,
                                 zRESPONSE_NO, 0 )           == zRESPONSE_NO )
   ...
   else
   {
      ...
      if ( CompareAttributeToInteger( vEMD2, "ER_RelLink_2", "CardMin",
                                       0 ) != 0 )
         bSrcIsReq = TRUE;
      else
         bSrcIsReq = FALSE;

      if ( CompareAttributeToInteger( vEMD, "ER_RelLink_2", "CardMin",
                                       0 ) != 0 )
         bTgtIsReq = TRUE;
      else
         bTgtIsReq = FALSE;
   ...
   {
      // choose the one that is currently used for implementation
      //( which means bypass if it is in a different DS ) or simply
   ...
   // let's see if the 1-to-1 relationship is used in an identifier for
   // either side
   if ( SetCursorFirstEntityByEntityCsr( vEMD, "ER_RelLinkIdentifier",
                                         vEMD, "ER_RelLink_2",
                                         "ER_Entity" ) >= zCURSOR_SET ||
        SetCursorFirstEntityByEntityCsr( vEMD, "ER_RelLinkIdentifier",
                                         vEMD2, "ER_RelLink_2",
                                         "ER_Entity" ) >= zCURSOR_SET )
      bSrcIsInID = TRUE;
   else
   ...
   if ( SetCursorFirstEntityByEntityCsr( vEMD2, "ER_RelLinkIdentifier",
                                         vEMD, "ER_RelLink_2",
                                         "ER_Entity" ) >= zCURSOR_SET ||
        SetCursorFirstEntityByEntityCsr( vEMD2, "ER_RelLinkIdentifier",
                                         vEMD2, "ER_RelLink_2",
                                         "ER_Entity" ) >= zCURSOR_SET )
      bTgtIsInID = TRUE;
   else
                             ...
                             PFNGENERATENAME pfnGenerateName )
{
   ...
   SetAttributeFromString(  vDTE1, "TE_FieldDataRel", "Name",     szTemp );
   ...
   SetAttributeFromString(  vDTE1, "TE_FieldDataRel", "DataOrRelfieldOrSet",
                                                                  "D" );

   // Include the RelLink under the AutoSeq field.
   IncludeSubobjectFromSubobject( vDTE1, "ER_RelLink",
                                ...
                                zVIEW    vEntpModel )
{
   ...
   GetStringFromAttribute( szName,
                           vDTE_Save, "TE_DBMS_Source", "GenerateExecutable" );
   ...
   // Loop through all RelLinks and create an AutoSeq field for
   // each RelLink that has AutoSeq = "Y".  The field created will be
   // somewhat similar to a foreign key in that it will not have an
   // associated ER_Attribute and it will have an ER_RelLink included beneath
   // it.
         ...
         RESULT = SetCursorNextEntity( vEMD, "ER_RelLink_2", "EntpER_Model" ) )
   {
...

      {
         ...
         // Only create AutoSeq field here if this is not many-to-many rel,
         // which means one of the rels must have CardMax of 1.
         if ( CompareAttributeToInteger( vEMD,  "ER_RelLink_2", "CardMax", 1 ) == 0 ||
              CompareAttributeToInteger( vEMD2, "ER_RelLink_2", "CardMax", 1 ) == 0 )
         {
            ...
            nRC = SetCursorFirstEntityByEntityCsr( vDTE1, "ER_Entity",
                               vEMD2, "ER_Entity_2", "TE_DBMS_Source" );
            if ( nRC < zCURSOR_SET )
               continue;

            // Go to build the AutoSeq field.
            oTZTENVRO_BuildAutoSeqField( vSubtask, vDTE1, vEMD,
                                         pfnGenerateName );
         }
      }
   }

   // Continue multiple loops through the ERD RelTypes until all
   // relationships have been processed for foreign keys.
   ...
   while ( bRelProcsIncomplete )
   {
            ...
            RESULT = SetCursorNextEntity( vEMD, "ER_RelType", 0 ) )
      {
         ...
         nImpl = ofnTZTENVRO_ImplementRelAs( &nRelImplemented,
                                             vEMD,       vEMD2,
                                             vDTE1,      vDTE2,
                                             vDTE_RelTR, vDTE_Save );

         if ( nImpl == zIMPL_REL_BYPASS )
            continue;  // process next rel...

         // Only process the RelType entry if the foreign keys in the parent
         // have already been processed.

         // Implement the rel in the Data Source based on "ownership".
         if ( nImpl == zIMPL_REL_AsFkInSrc &&
              CheckExistenceOfEntity( vDTE2, "TempRelKey" ) < zCURSOR_SET )

         {
            ofnTZTENVRO_ImplementSQL_RelLink( vSubtask, vDTE1, vEMD,
                                              vDTE2, vEMD2,
                                              pfnGenerateName );
         }
         else
         ...
         {
              ofnTZTENVRO_ImplementSQL_RelLink( vSubtask, vDTE2, vEMD2,
                                                vDTE1, vEMD,
                                                pfnGenerateName );
         }
         else
         if ( nImpl == zIMPL_REL_AsTablRec )
         {
            ...
            ofnTZTENVRO_CreateRelTablRec( vSubtask, vDTE1, vEMD,
                                          vDTE2, vEMD2, vDTE_RelTR,
                                          pfnGenerateName );
            ofnTZTENVRO_ImplementSQL_RelLink( vSubtask, vDTE_RelTR,
                                              vEMD2, vDTE1, vEMD,
                                              pfnGenerateName );
            ofnTZTENVRO_ImplementSQL_RelLink( vSubtask, vDTE_RelTR, vEMD,
                                              vDTE2, vEMD2,
                                              pfnGenerateName );
            // build the Keys by looping thru all attributes of the just created enitiy
            // (many-to-many --> all attributes should be keys
                  ...
                  RESULT = SetCursorNextEntity( vDTE_RelTR, "TE_FieldDataRel", 0 ) )
            {
               ...
               IncludeSubobjectFromSubobject( vDTE_RelTR, "TE_FieldDataRelKey",
                                               vDTE_RelTR, "TE_FieldDataRel", zPOS_AFTER );
            }
...
IssueError( vEMD, 0, 0, "stop" );
#endif
            // If either side of the relationship uses Auto Sequencing, create
            // an AutoSeq field.
            if ( CompareAttributeToString( vEMD, "ER_RelLink_2", "AutoSeq", "Y" ) == 0 )
            {
               // Go to build the AutoSeq field for the first side of the relationship.
               oTZTENVRO_BuildAutoSeqField( vSubtask, vDTE_RelTR, vEMD,
                                            pfnGenerateName );
            }
            ...
            {
               // If we have executed the outer loop 10 times, we will assume
               // that we have some kind of a relationship loop, that is a
               // ring of entities where they each have a relationship to
               // one of the other entities as a part of the key.
               zstrcpy( sz, "Relationship is part of an Identifier loop. " );
               zstrcat( sz, "From Entity = " );
               GetStringFromAttribute( szName, vEMD,  "ER_Entity_2",  "Name" );
               zstrcat( sz, szName );
               zstrcat( sz, ",  Relationship = " );
               GetStringFromAttribute( szName, vEMD,  "ER_RelLink_2", "Name" );
               zstrcat( sz, szName );
               zstrcat( sz, ",  To Entity = " );
               GetStringFromAttribute( szName, vEMD2, "ER_Entity_2",  "Name" );
               zstrcat( sz, szName );
               GetStringFromAttribute( szName, vEMD,  "ER_Entity_2",  "Name" );
         ...
         RESULT = SetCursorNextEntity( vEMD, "ER_EntIdentifier", "EntpER_Model" ) )
   {
      ...
      {
         nCount = 1;
         SetCursorFirstEntityByAttr( vDTE1, "ER_Entity", "ZKey",
                                     vEMD,  "ER_Entity", "ZKey", "TE_DBMS_Source" );
               ...
               RESULT = SetCursorNextEntity( vEMD, "ER_FactType", 0 ) )
         {
            ...
            // Positioning logic depends on whether the ER_FactType if for a REL
            // or an ATR.
            if ( CheckExistenceOfEntity( vEMD, "ER_RelLinkIdentifier" ) >= zCURSOR_SET )
            {
               ...
               RESULT = SetCursorFirstEntityByAttr( vDTE1, "ER_RelLink",           "ZKey",
                                                    vEMD,  "ER_RelLinkIdentifier", "ZKey",
                                                    "TE_TablRec" );
               while ( RESULT >= zCURSOR_SET )
               {
                  if ( SetCursorFirstEntityByAttr( vDTE1, "TE_FieldDataRelKey", "ZKey",
                                                   vDTE1, "TE_FieldDataRel",    "ZKey",
                                                   0 ) >= zCURSOR_SET )
                  {
                     SetAttributeFromInteger( vDTE1, "TE_FieldDataRelKey", "TempOrderNo",
                                              nCount );
                  }
                  nCount++;
                  RESULT = SetCursorNextEntityByAttr( vDTE1, "ER_RelLink",           "ZKey",
                                                      vEMD,  "ER_RelLinkIdentifier", "ZKey",
                                                      "TE_TablRec" );
               }
            }
            ...
            else
            {
               ...
               // To position on it, first position on the correct TE_FieldDataRel
               // entity.
               SetCursorFirstEntityByAttr( vDTE1, "ER_Attribute",           "ZKey",
                                           vEMD,  "ER_AttributeIdentifier", "ZKey",
                                           "TE_TablRec" );
               if ( CheckExistenceOfEntity( vDTE1, "TE_FieldDataRelKey" ) >= zCURSOR_SET &&
                   CheckExistenceOfEntity( vDTE1, "TE_FieldDataRel" ) >= zCURSOR_SET )
               {
                  SetCursorFirstEntityByAttr( vDTE1, "TE_FieldDataRelKey", "ZKey",
                                              vDTE1, "TE_FieldDataRel",    "ZKey", 0 );
                  SetAttributeFromInteger( vDTE1, "TE_FieldDataRelKey", "TempOrderNo",
                                           nCount );
               }
            }
                                ...
                                zVIEW    vEntpModel )
{
   ...
   // This routine builds relationships for Network databases.  It once was
   // also used for SQL databases but handling relationships for SQL was
   // rewritten as a separate routine.
         ...
         RESULT = SetCursorNextEntity( vEMD, "ER_RelType", 0 ) )
   {
      ...
      // At this point we have determined that we will implement the Rel in
      // the current Data Source, so we should delete it if it is currently
      // implemented in any Data Source.
      if ( nRelImplemented == zREL_IMPLD_AsTablRec )
         DeleteEntity( vDTE_RelTR, "TE_TablRec", zREPOS_PREV );
      else
      if ( nRelImplemented == zREL_IMPLD_AsFkInSrc )
      {
         for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE, "ER_RelType_O",
                                                         vEMD, "ER_RelType",
                                                         "TE_DBMS_Source" );
               RESULT > zCURSOR_UNCHANGED;
               RESULT = SetCursorNextEntityByEntityCsr( vDTE, "ER_RelType_O",
      ...
      {
         RESULT = SetCursorFirstEntityByEntityCsr( vDTE2, "ER_RelType_O",
                              vEMD2, "ER_RelType", "TE_DBMS_Source" );
         while ( RESULT > zCURSOR_UNCHANGED )
         {  // was NOT a MM rel so delete all FieldDataRec's used to implement
            DeleteEntity( vDTE2, "TE_FieldDataRel", zREPOS_NONE );
            RESULT = SetCursorNextEntityByEntityCsr( vDTE2, "ER_RelType_O",
                                 vEMD2, "ER_RelType", "TE_TablRec" );
         }
      }

      // implement the rel in the Data Source based on "ownership".
      if ( nImpl == zIMPL_REL_AsFkInSrc )
         ofnTZTENVRO_ImplementNET_RelLink( vSubtask, vDTE, vEMD, vDTE2, vEMD2,
                                           pfnGenerateName );
      else
      if ( nImpl == zIMPL_REL_AsFkInTgt )
         ofnTZTENVRO_ImplementNET_RelLink( vSubtask, vDTE2, vEMD2, vDTE, vEMD,
                                           pfnGenerateName );
      else
      if ( nImpl == zIMPL_REL_AsTablRec )
      {
         // this is generally for a many-to-many
         SetViewFromView( vDTE_RelTR, vDTE_Save );  // reset to current DS
         ofnTZTENVRO_CreateRelTablRec( vSubtask, vDTE, vEMD,
                                       vDTE2, vEMD2, vDTE_RelTR,
                                       pfnGenerateName );
         ofnTZTENVRO_ImplementNET_RelLink( vSubtask, vDTE_RelTR,
                                           vEMD2, vDTE, vEMD,
                                           pfnGenerateName );
         ofnTZTENVRO_ImplementNET_RelLink( vSubtask, vDTE_RelTR,
                                           vEMD, vDTE2, vEMD2,
...
// try to handle any non-set keys ...
         ...
         RESULT = SetCursorNextEntity( vDTE, "TE_FieldDataRelKey", "TE_DBMS_Source" ) )
   {
      ...
      if ( SetCursorFirstEntityByEntityCsr( vDTE2, "TE_FieldDataRel",
                                            ...
                                            0 ) < zCURSOR_SET )
      {
         ...
         // because it's not one of the attributes listed in
         // TE_FieldDataRel.
         //
         ...
         for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE2,
                                                         "TE_FieldDataRel",
                                                         vDTE,
                                                         "TE_FieldDataRelKey",
                                                         0 );
               RESULT < zCURSOR_SET;
               RESULT = SetCursorFirstEntityByEntityCsr( vDTE2,
                                                         "TE_FieldDataRel",
                                                         vDTE,
                                                         "TE_FieldDataRelKey",
                                                         0 ) )
         {
            ...
            if ( CheckExistenceOfEntity( vDTE, "TE_FieldDataRelKey" ) ==
                 zCURSOR_NULL )
               break;
         }
      }
   ...
   for ( RESULT = SetCursorFirstEntity( vEMD2, "ER_EntIdentifier",
                                        "EntpER_Model" );
         RESULT >= zCURSOR_SET;
         RESULT = SetCursorNextEntity( vEMD2, "ER_EntIdentifier",
                                       "EntpER_Model" ) )
   {
      for ( RESULT = SetCursorFirstEntity( vEMD2, "ER_RelLinkIdentifier",
                                           "ER_EntIdentifier" );
            RESULT > zCURSOR_UNCHANGED;
            RESULT = SetCursorNextEntity( vEMD2, "ER_RelLinkIdentifier",
                                          "ER_EntIdentifier" ) )
      {
         for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE2, "ER_RelLink",
                                                         vEMD2,
                                                         "ER_RelLinkIdentifier",
                                                         "TE_DBMS_Source" );
               RESULT > zCURSOR_UNCHANGED;
               RESULT = SetCursorNextEntityByEntityCsr( vDTE2, "ER_RelLink",
                                                        vEMD2,
                                                        "ER_RelLinkIdentifier",
                                                        "TE_DBMS_Source" ) )
         {
            if ( SetCursorFirstEntityByEntityCsr( vDTE2, "TE_FieldDataRelKey",
                     vDTE2, "TE_FieldDataRel", "TE_TablRec" ) < zCURSOR_SET )
            {
               SetAttributeFromString( vDTE2, "TE_FieldDataRel", "Key", "Y" );
               IncludeSubobjectFromSubobject( vDTE2, "TE_FieldDataRelKey",
                                             vDTE2, "TE_FieldDataRel",
                                             zPOS_AFTER );
            }
                                 ...
                                 zVIEW    vEntpModel )
{
   ...
   if ( CompareAttributeToString( vDTE_Save, "TE_DBMS_Source", "Paradigm",
                                  "R" ) == 0 )
                                  ...
                                  PFNGENERATENAME pfnGenerateName )
{
   ...
   {
      if ( CompareAttributeToString( vDTE_p, "TE_TablRecKey",
                                     "Category", "9" ) < 0 )
         break;
   }
   ...
   if ( RESULT < zCURSOR_SET )
   {
      zstrcpy( sz, "Parent Entity does not have a valid Key. " );
      if ( CheckExistenceOfEntity( vDTE_p, "ER_Entity" ) >= zCURSOR_SET )
      {
         GetStringFromAttribute( szDesc, vDTE_p, "ER_Entity", "Name" );
      }
      else
      {
         zstrcat( sz, " TE_TablRec: Name = " );
         GetStringFromAttribute( szDesc, vDTE_p, "TE_TablRec", "Name" );
         zstrcat( sz, szDesc );
      }
   ...
   // Return if this relationship has already been processed.
   // We know this if there is a FieldDataRel with a subordinate ER_RelLink
   // that matches the vEMD_p.ER_RelLink_2 entity AND the FieldDataRel also
   // has a FieldDataRelFK entity under it.  An AutoSeq entry could have the
   // same ER_RelLink but no FieldDataRelFK.
   for ( RESULT = SetCursorFirstEntityByEntityCsr( vDTE_c, "ER_RelLink",
                                                   vEMD_p, "ER_RelLink_2", "TE_TablRec" );
         RESULT >= zCURSOR_SET;
         RESULT = SetCursorNextEntityByEntityCsr( vDTE_c, "ER_RelLink",
                                                  vEMD_p, "ER_RelLink_2", "TE_TablRec" ) )
   {
   ...
   SetOI_FromBlob( &vDBH_Data, szDBH_DataObjectName, vDTE_c, vDTE_p,
                   "TE_DBMS_Source", "DBH_Data", zNOI_OKAY );

   // Prior to this code, the TempRelKey entities for DTE_p have been
   // converted to TE_FieldDataRelKey entries, which point to the corresponding
   // foreign key in DTE_p.
   // This code must
         ...
         RESULT = SetCursorNextEntity( vDTE_p, "TE_FieldDataRelKey", 0 ) )
   {
      ...
      if ( CompareAttributeToString( vDTE_p, "TE_FieldDataRelKey",
                                     "DataOrRelfieldOrSet", "R" ) == 0 )
      {
         GetStringFromAttribute( szTemp2, vDTE_c, "TE_FieldDataRel", "Name" );
         zstrcpy( szTemp, szTemp2 );
      }
      else
      {
         ...
         SetCursorFirstEntityByAttr( vDTE_t, "TE_FieldDataRel",    "ZKey",
                                     vDTE_p, "TE_FieldDataRelKey", "ZKey", 0 );
         GetStringFromAttribute( szNoPrefix,  vEMD_p, "ER_RelLink_2",
                                 "TE_NoForeignKeyPrefix" );
         GetStringFromAttribute( szPrefix,  vEMD_p, "ER_RelLink_2",
                                 "TE_ForeignKeyPrefix" );
         // get Foreign Key Column Name
         if ( CheckExistenceOfEntity( vDTE_t, "ER_Attribute" ) >= zCURSOR_SET )
         {
            GetStringFromAttribute( szColumnName, vDTE_t, "ER_Attribute",
                                    "TE_ForeignKeyName" );
            if ( *szColumnName == 0 )
            {
               GetStringFromAttribute( szColumnName, vDTE_t, "ER_Attribute",
            ...
            GetStringFromAttribute( szColumnName, vDTE_c, "TE_FieldDataRel", "Name" );

         // create Default Prefix + Column Name
         if ( zstrcmp( szNoPrefix, "Y" ) != 0 && *szPrefix == 0 )
         {
            zstrcpy( szTemp, "FK_" );
            zstrcat( szTemp, szColumnName );
         ...
         else
         {
            ...
            if ( zstrcmp( szNoPrefix, "Y" ) == 0 || *szPrefix == 0 )
               zstrcpy( szTemp, szColumnName );
            else
            {
               zstrcpy( szTemp, szPrefix );
               zstrcat( szTemp, szColumnName );
            }
         }
         DropView( vDTE_t );
      ...
      // If we have a DBH-specific object and if the object is the ODBC
      // object then we'll determine whether or not we should add table
      // names from there.
      if ( vDBH_Data && zstrcmpi( szDBH_DataObjectName, "TZDBHODO" ) == 0 )
      {
         if ( CompareAttributeToString( vDBH_Data, "ODBC",
                                        "AddTableNameToFK_Override", "Y" ) == 0 )
         {
            GetStringFromAttribute( szName, vDTE_p, "TE_TablRec", "Name" );
            zstrcat( szTemp, "_" );
            zstrcat( szTemp, szName );
         }
      }
      else
      ...
      // DGC 4/2/97  Blank out attribute Name so we don't get any frivolous
      // Duplicate names.
      SetAttributeFromString( vDTE_c, "TE_FieldDataRel", "Name", " " );

      ofnTZTENVRO_GenerateUniqueName( vSubtask, vDTE_c, szTemp, 'A',
                                      "TE_FieldDataRel",
                                      "Name", 0, pfnGenerateName );

      SetAttributeFromString( vDTE_c, "TE_FieldDataRel", "DataOrRelfieldOrSet",
                              "R" );
      SetAttributeFromString( vDTE_c, "TE_FieldDataRel", "Key", "" );

      if ( CheckExistenceOfEntity( vDTE_c, "ER_Entity" ) >= zCURSOR_SET )
      {
         if ( CompareAttributeToAttribute( vEMD_p, "ER_Entity_2", "ZKey",
                                           vDTE_c, "ER_Entity", "ZKey" ) == 0 )
         {
            if ( CompareAttributeToString( vEMD_p, "ER_RelLink_2", "CardMin",
                                           "0" ) == 0 )
               SetAttributeFromString( vDTE_c, "TE_FieldDataRel",
                                       "SQL_NULLS", "" );
            else
         ...
         {
            if ( CompareAttributeToString( vEMD, "ER_RelLink_2", "CardMin",
                                           "0" ) == 0 )
               SetAttributeFromString( vDTE_c, "TE_FieldDataRel", "SQL_NULLS",
                                       "" );
            else
      ...
      GetStringFromAttribute( szDesc, vDTE_p, "TE_TablRec", "Name" );
      zstrcat( szDesc, "(" );
      GetStringFromAttribute( szTemp, vEMD_p, "ER_RelLink_2", "Name" );
      zstrcat( szDesc, szTemp );
      zstrcat( szDesc, " [" );
      GetStringFromAttribute( szTemp, vEMD_p, "ER_RelLink_2", "CardMin" );
      zstrcat( szDesc, szTemp );
      zstrcat( szDesc, ":" );
      GetStringFromAttribute( szTemp, vEMD_p, "ER_RelLink_2", "CardMax" );
      zstrcat( szDesc, szTemp );
      zstrcat( szDesc, "] ) " );
      ...
      IncludeSubobjectFromSubobject( vDTE_c, "ER_RelLink",
      ...
      if ( CheckExistenceOfEntity( vDTE_c, "TE_TablRecKey" ) < zCURSOR_SET )
      {
         ...
         ofnTZTENVRO_GenerateUniqueName( vSubtask, vDTE_c, szName, 'A',
                                         "TE_TablRecKey",
                                         "Name", 0, pfnGenerateName );

         zstrcpy( szTemp, "Implements " );
         GetStringFromAttribute( szTemp2, vDTE_c, "TE_TablRec", "Name" );
         zstrcat( szTemp, szTemp2 );
      ...
      // column on the parent that is the source of the value for the foreign
      // key.
      IncludeSubobjectFromSubobject( vDTE_p, "TE_FieldDataRelAsFK",
                                     vDTE_c, "TE_FieldDataRel", zPOS_AFTER );
      ...
      // We will identify the TempRelKey entity by both Entity ZKey and
      // RelLink ZKey.
      GetIntegerFromAttribute( &lEntityZKey,
                               vEMD_p, "ER_Entity_2", "ZKey" );
      GetIntegerFromAttribute( &lRelZKey,
                               vEMD_p, "ER_RelLink_2", "ZKey" );
      for ( RESULT = SetCursorFirstEntity( vDTE_c, "TempRelKey", 0 );
            RESULT > zCURSOR_UNCHANGED;
            RESULT = SetCursorNextEntity( vDTE_c, "TempRelKey", 0 ) )
      {
         if ( CompareAttributeToInteger( vDTE_c, "TempRelKey", "EntityZKey",
                                         lEntityZKey ) == 0 &&
              CompareAttributeToInteger( vDTE_c, "TempRelKey", "RelZKey",
                                         lRelZKey ) == 0 )
         {
            // Add the foreign key just created as part of the identifier for
            // the child entity.
            SetCursorLastEntity( vDTE_c, "TE_FieldDataRelKey", 0 );

            SetAttributeFromString( vDTE_c, "TE_FieldDataRel", "Key", "Y" );
            IncludeSubobjectFromSubobject( vDTE_c, "TE_FieldDataRelKey",
                                           vDTE_c, "TE_FieldDataRel", zPOS_AFTER );
                                  ...
                                  PFNGENERATENAME pfnGenerateName )
{
   ...
   GetStringFromAttribute( szOwner, vDTE_p, "TE_TablRec", "Name" );
   GetStringFromAttribute( szMember, vDTE, "TE_TablRec", "Name" );
   GetStringFromAttribute( szSet, vEMD_p, "ER_RelLink_2", "Name" );
   zsprintf( szDesc, "%s_%s_%s", szOwner, szSet, szMember );

   // Generate a unique name for sets.  We need to make sure that the set name
   // is unique across the entire DBMS.
   ofnTZTENVRO_GenerateUniqueName( vSubtask, vDTE, szDesc,
                                   'S', "TE_FieldDataRel",
                                   "Name", "TE_DBMS_Source", pfnGenerateName );
   ...
   zstrcat( szDesc, szTemp );
   zstrcat( szDesc, " [" );
   GetStringFromAttribute( szTemp, vEMD, "ER_RelLink_2", "CardMax" );
   zstrcat( szDesc, szTemp );
   zstrcat( szDesc, ":" );
   GetStringFromAttribute( szTemp, vEMD_p, "ER_RelLink_2", "CardMax" );
   zstrcat( szDesc, szTemp );
   zstrcat( szDesc, "] ) " );
   GetStringFromAttribute( szTemp, vDTE, "TE_TablRec", "Name" );
   zstrcat( szDesc, szTemp );
                              ...
                              PFNGENERATENAME pfnGenerateName )
{
   ...
   GetStringFromAttribute( szDesc, vDTE2, "TE_TablRec", "Name" );
   zstrcat( szDesc, "(" );
   GetStringFromAttribute( szTemp, vEMD2, "ER_RelLink_2", "Name" );
   zstrcat( szDesc, szTemp );
   zstrcat( szDesc, " [" );
   GetStringFromAttribute( szTemp, vEMD1, "ER_RelLink_2", "CardMax" );
   zstrcat( szDesc, szTemp );
   zstrcat( szDesc, ":" );
   GetStringFromAttribute( szTemp, vEMD2, "ER_RelLink_2", "CardMax" );
   zstrcat( szDesc, szTemp );
   zstrcat( szDesc, "] ) " );
   GetStringFromAttribute( szTemp, vDTE1, "TE_TablRec", "Name" );
   zstrcat( szDesc, szTemp );
   ...
   GetStringFromAttribute( szDesc, vEMD2, "ER_RelType", "TE_TableName" );
   if ( *szDesc == 0 )
   {
      zstrcpy( szDesc, "Mm_" );
      GetStringFromAttribute( szTemp, vDTE2, "TE_TablRec", "Name" );
      /* Change entity-name to all upper-case. It is not longer changes while
         initializing Tables */
      SysTranslateString( szTemp, 'U' );
      zstrcat( szDesc, szTemp );
      zstrcat( szDesc, "_" );
   ...
   ofnTZTENVRO_GenerateUniqueName( vSubtask, vDTE_new, szDesc,
                                   'E', "TE_TablRec",
                                   "Name", 0, pfnGenerateName );
                                ...
                                PFNGENERATENAME pfnGenerateName )
{
   ...
   if (*szSironDB_Type != 'F')
   {
      ...
      RESULT = SetCursorFirstEntityByString( vDTE2, pszEntity, pszAttribute,
                                             pszName, pszScopingEntity );
      while ( RESULT >= zCURSOR_SET )
      {
         (*pfnGenerateName) ( vDTE, pszName, 'X' );
...
oTZTENVRO_GetUpdViewForDTE_P( zVIEW vSubtask, zPVIEW pvTZTENVRO )
{
   ...
   *pvTZTENVRO = 0;
   ...
   else
   {
      ...
      if ( vLPL )
         GetStringFromAttribute( sz, vLPL, "LPLR", "Name" );
...
// Get LPL Name
   ...
   {  // search all TE's to find one that is the Pysical Model
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ActivateMetaOI( vSubtask, &vDTE, vCM_List, zREFER_DTE_META,
                         zSINGLE | zACTIVATE_ROOTONLY );
         if ( vDTE &&
              CompareAttributeToString( vDTE, "TE_DB_Environ",
                                        "Type", "P" ) == 0 )
                             ...
                             zVIEW  vDTE_Copy1 )
{
   ...
   // As the Target FK is for more than 1 relationship, we want the
   //  TE_FieldDataRel_PK to be the Base Key (having an ER attribute).
   fnReferenceBaseKey( vDTE_Copy1 );
   if ( SetCursorFirstEntityByAttr( vDTE_Copy1, "ER_RelLink", "ZKey",
                                    vDTE, "ER_RelLink", "ZKey",
                                    "" ) < zCURSOR_SET )
   {
      IncludeSubobjectFromSubobject( vDTE_Copy1, "ER_RelLink",
                                     vDTE, "ER_RelLink", zPOS_AFTER );
   }
...
// vDTE is pointing to the TE_FieldDataRel FK to be switched off (old).
// vDTE_Copy1 is pointing to the TE_FieldDataRel FK(new), which must
...
fnUpdateKeys( zVIEW  vDTE, zVIEW  vDTE_Copy1 )
{
   ...
   // If the original foreign key was a part of the identifier, replace the
   // original TE_FieldDataRelKey entry with the new one and include all of
   // the foreign keys referencing it to the new entry.
   if ( SetCursorFirstEntityByEntityCsr( vDTE, "TE_FieldDataRelKey",
                                         vDTE, "TE_FieldDataRel", "TE_TablRec" )
                                         >= zCURSOR_SET )
   {
      // If the new foreign key is not already a part of the Identifier,
      // create it.
      if ( SetCursorFirstEntityByEntityCsr( vDTE_Copy2, "TE_FieldDataRelKey",
                                            vDTE_Copy1, "TE_FieldDataRel",
                                            "TE_TablRec" ) < zCURSOR_SET )
      {
         IncludeSubobjectFromSubobject( vDTE_Copy2, "TE_FieldDataRelKey",
                                        vDTE_Copy1, "TE_FieldDataRel", zPOS_BEFORE );
      }
      ...
      {
         IncludeSubobjectFromSubobject( vDTE_Copy2, "TE_FieldDataRelAsFK",
                                         vDTE,  "TE_FieldDataRelAsFK", zPOS_AFTER );
...
// vDTE is pointing to a Foreign Key (TE_FileDataRel) which should
// have the Base Key (ER attribute) as TE_FileDataRel_PK.
// If necessary, we have to exclude the FK under the "old" key
// and include it under the Base Key
static void
fnReferenceBaseKey( zVIEW vDTE )
{
   zVIEW vDTE2;
   ...
   nRC = SetCursorFirstEntityByInteger( vDTE2, "TE_FieldDataRelAsFK", "ZKey",
                                        lCurrentZKey, "TE_DBMS_Source" );
   while ( nRC >= zCURSOR_SET )
   {
      ExcludeEntity( vDTE2, "TE_FieldDataRelAsFK", zREPOS_NONE );
      nRC = SetCursorNextEntityByInteger( vDTE2, "TE_FieldDataRelAsFK", "ZKey",
                                        lCurrentZKey, "TE_DBMS_Source" );
   }
   // include the current FK under the Base Key
   nRC = SetCursorFirstEntityByInteger( vDTE2, "TE_FieldDataRelKey", "ZKey",
                                        lBaseZKey, "TE_DBMS_Source" );
   // is it possible that the FK is already here?
   //  probably not, but to be sure ...
   nRC = SetCursorFirstEntityByEntityCsr( vDTE2, "TE_FieldDataRelAsFK",
                                          vDTE, "TE_FieldDataRel", 0 );
   if ( nRC < zCURSOR_SET )
   {
      // not there, include it
      nRC = SetCursorLastEntity( vDTE2, "TE_FieldDataRelAsFK", 0 );
      IncludeSubobjectFromSubobject( vDTE2, "TE_FieldDataRelAsFK",
...
fnFK_SearchBaseKey( zVIEW  vDTE_BaseKey, zLONG  lZKey )
{
   ...
   nRC = SetCursorFirstEntityByInteger( vDTE_BaseKey, "TE_FieldDataRel", "ZKey",
                                        lZKey, "TE_DBMS_Source" );

   while ( nRC >= zCURSOR_SET &&
           CompareAttributeToString( vDTE_BaseKey, "TE_FieldDataRel",
