                                           ...
                                           vTZCMCPRO, "CPLR", "LastCheckInDate" ) == 0 )
         {
            ...
            if ( SetCursorFirstEntity( vTZCMLPLO, "W_MetaDef", "" ) >= zCURSOR_SET )
            {
               if ( CompareAttributeToInteger( vTZCMLPLO, "W_MetaDef",
                                               "UpdateInd", 0 ) == 0 )
               {
                  GetIntegerFromAttribute( &ulZKey, vTZCMLPLO, "W_MetaDef", "CPLR_ZKey" );
                  ExcludeEntity( vTZCMLPLO, "W_MetaDef", zREPOS_NONE );
                  CreateViewFromViewForTask( &vTZCMCPRO2, vTZCMCPRO, 0 );
                  SetCursorFirstEntityByInteger( vTZCMCPRO2, "Z_MetaDef",
                                                 "ZKey", ulZKey, "" );
                               ...
                               zSHORT           sMsg )
{
   ...
   GetStringFromAttribute( szWorkString, vMeta, "CPL", "Name" );
   zstrblank( szWorkString, 20, 1 );
   zstrcat( szWorkString, " - " );
   GetStringFromAttribute( szCPLR_Name, vMeta, "CPLR", "Name" );
   zstrcat( szWorkString, szCPLR_Name );
                    ...
                    zSHORT EnqueueType)
{
   ...
   while ( nRC >= zCURSOR_SET && TestRC != 0)
   {
      ...
      {
         TestRC = CompareAttributeToInteger( *vTZCMQCPO_List, "CPLR", "ZKey",
                                             lCPLR_ZKey );
      }
      else
      if ( CheckExistenceOfEntity( *vTZCMQCPO_List, "CPLR_Enqueue" )
                                   >= zCURSOR_SET )
      {
   ...
   if ( TestRC == 0 )
   {
      ...
      if ( iSame == 0 )
      {
         ...
         GetStringFromAttribute( UserName, vTZCMWIPO, "ROOT", "UserName" );
         strcpy( szMsg, "The CPLR '" );
         strcat( szMsg, pszCPLR_Name );
         strcat( szMsg, "' is at this time enqueued \n\n by\t" );
         GetStringFromAttribute( szUserName_Queue,
                                 *vTZCMQCPO_List, "CPLR_Enqueue", "UserName" );
         strcat( szMsg, szUserName_Queue );
         GetStringFromAttribute( szUserDesc_Queue,
                                 *vTZCMQCPO_List, "CPLR_Enqueue", "UserDesc" );
         if ( szUserDesc_Queue[ 0 ] != 0 )
         {
            strcat( szMsg, " (" );

            strcat( szMsg, szUserDesc_Queue );
            strcat( szMsg, ")" );
         }
         strcat( szMsg, "\n on\t" );
         GetStringFromAttributeByContext(szDate_Queue,
                                         *vTZCMQCPO_List, "CPLR_Enqueue", "TimeStamp",
                                         "DD.MM.YYYY HH:MM:SS", 19 );
         zstrcat( szMsg, szDate_Queue );

         GetIntegerFromAttribute( ( zPLONG ) &EnqueueType_Queue,
                                  *vTZCMQCPO_List, "CPLR_Enqueue", "EnqueueType" );
         strcat( szMsg, "\n for\t" );

         switch( EnqueueType_Queue )
         {
            case zCPLR_ENQUEUE_FOR_REFRESH:   strcat( szMsg, "Refresh." );
                                              break;
            case zCPLR_ENQUEUE_FOR_CHECK_IN:  strcat( szMsg, "CheckIn." );
                                              break;
            case zCPLR_ENQUEUE_FOR_CHECK_OUT: strcat( szMsg, "CheckOut." );
                                              break;
            case zCPLR_ENQUEUE_BY_SYSADM:     strcat( szMsg, "Maintenance." );
                                              break;
            default:strcat( szMsg, "unknown EnqueueType (!)" );
         ...
         // the SystemAdmin has the authority to cancel the Enqueue !!!
         {
            ...
            strcat( szMsg, "you have the authority to cancel this user action." );
   ...
   else
   {
      ...
      strcpy (szMsg, "OK, no enqueue conflict for CPLR: " );
      strcat( szMsg, pszCPLR_Name );
                        ...
                        zSHORT EnqueueType )
{
   ...
   SetAttributeFromString( *vTZCMQCPO, "CPLR_Enqueue", "TimeStamp",
                           szTimeStamp );
   SetAttributeFromInteger( *vTZCMQCPO, "CPLR_Enqueue", "EnqueueType",
                            EnqueueType );

   IncludeSubobjectFromSubobject( *vTZCMQCPO, "CPLR",
   ...
   if ( nRC >= zCURSOR_SET )
   {
      ...
      IncludeSubobjectFromSubobject( *vTZCMQCPO, "RepositoryClient",
                                     vTZCMWKSO, "RepositoryClient", zPOS_AFTER );
      SetAttributeFromAttribute( *vTZCMQCPO, "CPLR_Enqueue", "UserName",
                                 vTZCMWKSO, "User", "Name" );
      SetAttributeFromAttribute( *vTZCMQCPO, "CPLR_Enqueue", "UserDesc",
                                 vTZCMWKSO, "User", "Desc" );
   }
   else
   {
                        ...
                        zSHORT EnqueueType )
{
   ...
   GetStringFromAttribute( szMsg, vTZCMCPRO, "CPLR", "Name" );
   zstrcat( szMsg, ", CPLR Enqueue: " );
   ...
   if ( EnqueueType > 0 )
   {
      ...
      nRC = oTZCMCPRO_WriteEnqueue( vTZCMCPRO, vTZCMWIPO,
                                    &vTZCMQCPO, EnqueueType );

      // load all Enqueues for the testing of the ReusableCPLRs
      ActivateObjectInstance( &vTZCMQCPO_List,"TZCMQCPO", vSubtask,
                              0, zMULTIPLE );
      if ( nRC == zDUPLICATE_ROOT )
      {
         ...
         GetStringFromAttribute(  szCPLR_Name, vTZCMCPRO, "CPLR", "Name" );
         ...
         {
            // now the Enqueue-Object ist removed thru oTZCMCPRO_TestCPLR
            // and should be enqueued (2nd try):
            nRC = oTZCMCPRO_WriteEnqueue( vTZCMCPRO, vTZCMWIPO,
                                          &vTZCMQCPO, EnqueueType );
            if (nRC < 0 )
            {
      ...
      {
         GetIntegerFromAttribute( &lCPLR_ZKey,
                                  vTZCMCPRO, "ReusableCPLR", "ZKey" );
         GetStringFromAttribute( szCPLR_Name,
                                  vTZCMCPRO, "ReusableCPLR", "Name" );
   ...
   {
      // DEqueue the CPLR:
      if ( GetViewByName( &vTZCMQCPO, "TZCMQCPO",
                          vSubtask, zLEVEL_TASK ) >= zCURSOR_SET )
      {
                                 ...
                                 zPCHAR szExtension)
{
   ...
   if ( lFile_ViewCluster == 0 )
   {
      ...
      SetAttributeFromInteger( vTZCMFILE, "CPLR", "File_ViewCluster",
                               lFile_ViewCluster );
   }
   ...
   {
      if ( CompareAttributeToString( vTempFile, "File", "Extension",
                                     szExtension ) == 0 )
         bFound = TRUE;
      else
   ...
   GetIntegerFromAttribute( &ulZKey, vTempFile, "File", "ZKey" );
   nRC = oTZCMLPLO_ActivateFileOI( &vFile, vTempFile, ulZKey );

   if ( nRC >= 0 )
   {
      zsprintf( szViewName, "TZCMFILO_%p", vFile );
   ...
   {
      zsprintf( szMsg, "File '%s.%s' not found.", szName, szExtension );
                                       ...
                                       zVIEW vSubtask )
{
   ...
   switch ( lType )
   {
      case zSOURCE_PENV_META:
         ...
         GetStringFromAttribute( szName, vMetaOI, "PresEnvDef", "Name" );
         nRC = oTZCMCPRO_DeleteFileWithCluster( vSubtask, vTZCMFILE,
                                                szName, "XPE" );
      ...
      case zSOURCE_GOPGRP_META:
         ...
         GetStringFromAttribute( szExtension,
                                 vMetaOI, "GlobalOperationGroup", "Extension" );
         nRC = oTZCMCPRO_DeleteFileWithCluster( vSubtask, vTZCMFILE,
                                                szName, szExtension );
      ...
      case zSOURCE_DIALOG_META:
         ...
         {
            GetStringFromAttribute( szName,
                                    vMetaOI, "SourceFile", "Name" );
            GetStringFromAttribute( szExtension,
                                    vMetaOI, "SourceFile", "Extension" );
            nRC = oTZCMCPRO_DeleteFileWithCluster( vSubtask, vTZCMFILE,
                                                   szName, szExtension );
         }
                                                   ...
                                                   zSOURCE_VOR_META, 0 ) )
         {
            ...
            if ( nRC >= 0 )
            {
               if ( CheckExistenceOfEntity( vMetaOI, "LOD" ) >= zCURSOR_SET &&
                    CompareAttributeToString( vMetaOI, "LOD", "Name",
                                              szName ) == 0 )
               {
                  DeleteEntity( vTempCPLR, "Z_MetaDef", zREPOS_NONE );
                  oTZCMCPRO_DeleteFileWithCluster( vSubtask, vTZCMFILE,
                                                   szZKey, "PVR" );
               }
               DropView( vMetaOI );
            }
      ...
      case zSOURCE_ERD_META:
         ...
         nRC = SetCursorFirstEntityByInteger( vTempCPLR, "Z_MetaDef", "Type",
                                              zSOURCE_DTE_META, 0 );
         if ( nRC >= zCURSOR_SET )
         {
            oTZCMCPRO_DeleteMetaWithCluster( vTempCPLR, vTZCMFILE,
                                             vKZDBHQUA, vSubtask );
         }
                                 ...
                                 zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute(( zPLONG ) &ulZKey,
                             vTempCPLR, "Z_MetaDef", "ZKey" );
   GetStringFromAttribute( szName, vTempCPLR, "Z_MetaDef", "Name" );

   if ( ( lType >= 0 ) && ( lType <=  zSOURCE_MAX_META ) )
   {
      // Issue a Message, which component is being deleted.
      zstrcpy( szMsg, "Processing delete of " );

      switch ( lType )
      {
         case zSOURCE_GOPGRP_META:
            zstrcat( szMsg, "GlobalOperationGroup" );
            break;

         case zSOURCE_DIALOG_META:
            zstrcat( szMsg, "Dialog" );
            break;

         case zSOURCE_VOR_META:
            zstrcat( szMsg, "Registered View" );
            break;
      ...
      if ( nRC < 0 )
      {
         zstrcpy( szMsg, "Activate failed on load of " );
         zstrcat( szMsg, szName );
         zstrcat( szMsg, ", OD type " );
         zstrcat( szMsg, SRC_CMOD[ lType ].szOD );
      ...
      else
      {
         ...
         oTZCMCPRO_DeleteDependentsWithCluster( vTempCPLR, vTZCMFILE, vMetaOI,
                                                vKZDBHQUA, vSubtask );
         DropView( vMetaOI );

         // delete the Meta File
         oTZCMCPRO_DeleteFileWithCluster( vSubtask, vTZCMFILE, szName,
                                          SRC_CMOD[ lType ].szOD_EXT + 1 );
      }
   }
                      ...
                      zVIEW vSubtask )
{
   ...
   if ( ( lType >= 0 ) && ( lType <=  zSOURCE_MAX_META ) )
   {
      ...
      {
         // Delete any subordinate metas.
         oTZCMCPRO_DeleteDependents( vTempCPLR,
                                     vMetaOI,
                                     vKZDBHQUA,
                                     vSubtask );
         ...
         nRC = oTZCMCPRO_DeleteServerFile( vTempCPLR,
                                           szName,
                                           SRC_CMOD[ lType ].szOD_EXT + 1 );
         DropView( vMetaOI );
      }
                            ...
                            zVIEW vSubtask )
{
   ...
   switch ( lType )
   {
      ...
      case zSOURCE_LOD_META:
                                                    ...
                                                    zSOURCE_VOR_META, 0 ) )
         {
            ...
            if ( nRC >= 0 )
            {
               ...
               {
                  DeleteEntity( vTempCPLR, "Z_MetaDef", zREPOS_NONE );
                  oTZCMCPRO_DeleteServerFile( vTempCPLR,
                                              szZKey,
                                              "PVR" );
               }
               DropView( vMetaOI );
      ...
      case zSOURCE_ERD_META:
         ...
         if ( nRC >= zCURSOR_SET )
         {
            oTZCMCPRO_DeleteMeta( vTempCPLR,
                                  vKZDBHQUA,
                                  vSubtask );
         }
                            ...
                            zPCHAR szExtension )
{
   ...
   {
      if ( CompareAttributeToString( vTempCPLR, "File", "Extension",
                                     szExtension ) == 0 )
         nFound = 1;
      else
                         ...
                         zBOOL  bDeleteFile )   // delete file from temp dir
{
   ...
   nRC = oTZCMLPLO_CopyFileFromServerFile( vSubtask, szTempName, vTZCMCPRO,
                                           szName, szExtension );
   if ( nRC < 0 )
   {
      *pvMeta = 0;
   ...
   nRC =
    ActivateOI_FromFile( pvMeta,
                         SRC_CMOD[ lType ].szOD, // View OD name
                         vTZCMCPRO, szTempName,
                               ...
                               zSHORT        nGetOrSetFlag )
{
   ...
   if ( nGetOrSetFlag == zDERIVED_GET )
   {
      ...
      GetIntegerFromAttribute( &ulZKey, vTZCMRCRO, "CPLR", "ZKey" );
      SetCursorFirstEntityByInteger( vTempCPL, "CPLR", "ZKey",
                                    (zLONG) ulZKey, "Installation" );
      GetStringFromAttribute( szCPLR_CPL_Name, vTempCPL, "CPLR", "Name" );
      zstrcat( szCPLR_CPL_Name, "   CPL: " );
      GetStringFromAttribute( &szCPLR_CPL_Name[ zstrlen( szCPLR_CPL_Name ) ],
                              vTempCPL, "CPL", "Name" );

      StoreValueInRecord( vTZCMRCRO, lpViewEntity, lpViewAttrib,
                          szCPLR_CPL_Name, 0 );
