...
fnAllowBinaryDataInSourceMetaFiles( zVIEW lpTaskView )
{
   ...
   szBuffer[ 0 ] = 0;
   SysReadZeidonIni( -1, "[Workstation]", "LocalDir", szFileName );
   if ( szFileName[ 0 ] )
   {
      SysAppendcDirSep( szFileName );
      zstrcat( szFileName, "ZeidonRT.ini" );
      GetPrivateProfileString( "ZeidonTools", "AllowBinaryDataInSourceMetaFiles", "",
                               szBuffer, sizeof( szBuffer ), szFileName );
   }

   if ( szBuffer[ 0 ] == 0 )
   {
      SysReadZeidonIni( -1, "[ZeidonTools]",
                        "AllowBinaryDataInSourceMetaFiles", szBuffer );
   }
...
/////////////////////////////////////////////////////////////////////////////
zSHORT
fnGetDirectorySpec( zVIEW vSubtask, zPCHAR pchDirectorySpec, zLONG lType, zBOOL bConvertEnvironment )
{
   zVIEW vTaskLPLR;
                ...
                zPCHAR pchLPLR_NameIn )
{
   ...
   while ( (lpTask = SfGetNextTask( lpTask )) != 0 )
   {
      ...
      if ( zstrcmpi( szDialogName, "TZCMLPLD" ) == 0 &&
           lpTask != SfGetCurrentTask( vSubtask ) )
      {
   ...
   if ( vZeidonCM == 0 )
   {
      ...
      {
         SetNameForView( vZeidonCM, "ZeidonCM",
   ...
   if ( vActiveMetas == 0 )
   {
      ...
      SetNameForView( vActiveMetas, "OpenCM_Metas",
                      vZeidonCM, zLEVEL_SUBTASK );
   }

   fnGetTaskOI_ListName( vSubtask, szLPLR_Name );
   ...
   {
      if ( oTZCMWKSO_ActivateWorkStation( &WKS_View, vZeidonCM,
                zSINGLE | zLEVEL_APPLICATION | zIGNORE_ATTRIB_ERRORS ) != 0 )
      {
         zstrcpy( szErrMsg, "Unable to open the RepositoryClient File - " );
         GetStringFromAttribute( &szErrMsg[ zstrlen( szErrMsg ) ],
                                 WKS_View, "LPLR", "Name" );
         MessageSend( vSubtask, "CM00402", "Configuration Management",
                      szErrMsg,
                      zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
         zstrcpy( szErrMsg, "If you are a new user use the LPLR Dialog" );
         zstrcat( szErrMsg,
   ...
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKey, WKS_View,
                               "RepositoryClient", "DefaultLPLR_ZKey" );
      if ( SetCursorFirstEntityByInteger( WKS_View, "LPLR",
                                          "ZKey", (zLONG) ulZKey,
                                          "" ) != zCURSOR_SET )
      {
         if ( SetCursorFirstEntity( WKS_View, "LPLR", "" ) == zCURSOR_SET )
         {
            SetAttributeFromAttribute( WKS_View, "RepositoryClient",
                                       "DefaultLPLR_ZKey", WKS_View,
                                       "LPLR", "ZKey" );
            oTZCMWKSO_CommitWorkstation( WKS_View );
         }
         else
         {
            //zstrcpy( szErrMsg, "Unable to ActivateDefaultLPL because" );
            //zstrcat( szErrMsg, " no LPL Releases defined for Client." );
            ...
            zstrcpy( szErrMsg, "Unable to activate a default project because" );
            zstrcat( szErrMsg, " no projects exist." );
   ...
   GetStringFromAttribute( szLPLR_Name, WKS_View, "LPLR", "Name" );
   GetStringFromAttribute( szErrMsg, WKS_View, "LPLR", "ExecDir" );  // borrow szErrMsg for a sec...
   SysConvertEnvironmentString( szLPLR_Spec, szErrMsg );
   ofnTZCMWKSO_AppendSlash( szLPLR_Spec );
   zstrncpy( szLPLR_FileName, szLPLR_Name, 9 );
   TruncateName8( szLPLR_FileName );
   zstrcat( szLPLR_Spec, szLPLR_FileName );
   zstrcat( szLPLR_Spec, ".XLP" );
   ...
   if ( vLPLR == 0 )  // view isn't there
   {
      ...
      else
      {
         ...
         if ( SetCursorFirstEntityByInteger( vLPLR, "W_MetaType", "Type",
                                             zSOURCE_REPORT_META,
                                             0 ) < zCURSOR_SET )
         {
            ...
            SetAttributeFromInteger( vLPLR, "W_MetaType", "Type",
                                     zSOURCE_REPORT_META );
         }
      ...
      {
         zstrcpy( szErrMsg, "The default Project File " );
         zstrcat( szErrMsg, szLPLR_Spec );
      ...
      if ( CheckExistenceOfEntity( vActiveMetas, "LPLR" ) != zCURSOR_SET )
      {
         ...
         SetAttributeFromAttribute( vActiveMetas, "LPLR", "Name",
                                    WKS_View, "LPLR", "Name" );
      }
      else
      {
         if ( SetCursorFirstEntityByString( vActiveMetas, "LPLR", "Name",
                                            szLPLR_Name, 0 ) != zCURSOR_SET )
         {
            CreateEntity( vActiveMetas, "LPLR", zPOS_AFTER );
            SetAttributeFromAttribute( vActiveMetas, "LPLR", "Name",
                                       WKS_View, "LPLR", "Name" );
...
TerminateLPLR( zVIEW vSubtask )
{
   ...
   GetViewByName( &vActiveMetas, "OpenCM_Metas", vZeidonCM, zLEVEL_SUBTASK );
   GetStringFromAttribute( szViewName, vLPLR, "LPLR", "Name" );
   if ( SetCursorFirstEntityByString( vActiveMetas, "LPLR", "Name",
                                      szViewName, "" ) < zCURSOR_SET )
   {
      zCHAR  szMsg[ 512 ];
   ...
   nRC = SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaDef",
                                        "TaskID", lTaskID, "LPLR" );
   while ( (nRC == zCURSOR_SET) || (nRC == zCURSOR_SET_NEWPARENT) )
   {
      GetStringFromAttribute( szViewName, vActiveMetas,
      ...
      if ( GetViewByName( &WorkView, szViewName,
      ...
      nRC = SetCursorNextEntityByInteger( vActiveMetas, "W_MetaDef",
                                          "TaskID", lTaskID, "LPLR" );
   }

   fnGetTaskOI_ListName( vSubtask, szViewName );
   GetViewByName( &vTaskMetas, szViewName, vZeidonCM, zLEVEL_SUBTASK );
   if ( vTaskMetas )
   {
      nRC = SfGetFirstNamedView( &vTaskOI, szViewName,
                                 vTaskMetas, zLEVEL_SUBTASK );
      while( nRC > 0 )
      {
         DropView( vTaskOI );
         nRC = SfGetFirstNamedView( &vTaskOI, szViewName,
   ...
   // DropObjectInstance( vLPLR );
      SetAttributeFromInteger( vLPLR, "LPLR",
                               "TaskUseCount", lTaskUseCnt );
...
*/
   ...
   {
      nRC = SetAttributeFromInteger( vLPLR, "LPLR",
                                     "TaskUseCount", lTaskUseCnt );
   }
...
RetrieveViewForMetaList( zVIEW vSubtask,
                         zPVIEW lpListView, zSHORT nType )
{
   zSHORT nEntityType;
   zVIEW  vLPLR;
   zVIEW  vZeidonCM;
   ...
   if ( vLPLR )  // View is there
   {
      ...
      fnGetTaskOI_ListName( vSubtask, szName ); // __CM.800013b8
      ...
      {
         if ( SetCursorFirstEntityByInteger( *lpListView, "W_MetaType",
                                             "Type", nType, "" ) != zCURSOR_SET )
         {
            nRC = CreateEntity( *lpListView, "W_MetaType", zPOS_AFTER );
            SetAttributeFromInteger( *lpListView, "W_MetaType",
                                     "Type", nType );
         }

         SetViewToSubobject( *lpListView, "W_MetaType" );
         szName[ 0 ] = 'l';
         zltoa( *((zPLONG) lpListView), &szName[ 1 ] );
         SetNameForView( *lpListView, szName, vTaskMetas,
                         zLEVEL_SUBTASK | zNAME_AUTODROP );
         return( 1 );
      }
   }
   else
...
// zVIEW  vApplication;
   ...
   if ( nType == zSOURCE_DOMAIN_META || nType == zREFER_DOMAIN_META )
   {
      ...
      GetIntegerFromAttribute( (zPLONG) &ulOrigMetaOI_ZKey, vWkListView,
                               "W_MetaDef", "CPLR_ZKey" );
      ...
      SetCursorFirstEntityByInteger( vWkListView, "W_MetaType", "Type",
                                     zREFER_DOMAINGRP_META, "" );

      // Then position on the correct Domain within the group type.
   // SetCursorFirstEntityByAttr( vWkListView, "W_MetaDef", "Name",
   //                             vListView, "W_MetaDef", "GroupName", "" );
      GetStringFromAttribute( szGroupName, vListView, "W_MetaDef", "GroupName" );
   ...
   fnGetTaskOI_ListName( vSubtask, szMetaOI_Name );
   GetViewByName( &vTaskMetas, szMetaOI_Name, vZeidonCM, zLEVEL_SUBTASK );

   // Get the view to the ActiveMeta OI.
   GetViewByName( &vActiveMetas, "OpenCM_Metas", vZeidonCM, zLEVEL_SUBTASK );

   SetCursorFirstEntityByAttr( vActiveMetas, "LPLR", "Name",
                               vTaskLPLR, "LPLR", "Name", "" );
   bReactivate = TRUE;

   // Check to see if a W_MetaType exists for Type = nActiveType.  If not,
   // then create one.
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaType", "Type",
                                       nActiveType, "" ) != zCURSOR_SET )
   {
   ...
   if ( nRC == zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szCM_ViewName, vActiveMetas, "W_MetaDef", "CM_ViewName" );
      GetIntegerFromAttribute( &lTaskID, vActiveMetas, "W_MetaDef", "TaskID" );
      if ( nEntityType == CM_REFER_TYPE )
      {
         zstrcat( szCM_ViewName, ".r" );
      ...
      else
      {
         ...
         zstrcat( szCM_ViewName, ".u" );
         if ( GetViewByName( &CM_View, szCM_ViewName, vZeidonCM, zLEVEL_SUBTASK ) < 1 )
         {
            // It's in vActiveMetas but it's not loaded for UPDATE.

            // Change last zCHAR in view name from "u" to "r" and try to get
            // THAT view.
   ...
   {
      IncludeSubobjectFromSubobject( vActiveMetas, "W_MetaDef",
                                     vWkListView, "W_MetaDef", zPOS_AFTER );
   ...
   if ( bReactivate == FALSE ) // REFER is already activated
   {
      ...
      szCM_ViewName[ 0 ] = 'r';
      zltoa( *((zPLONG) pvMOI), &szCM_ViewName[ 1 ] );
      SetNameForView( *pvMOI, szCM_ViewName, vTaskMetas,
                      zLEVEL_SUBTASK | zNAME_AUTODROP );
   ...
   else
   {
      ...
      if ( nEntityType == CM_ACTIVE_TYPE )
         zstrcpy( szMetaOI_Def, SRC_CMOD[ nType ].szOD );
      else
         zstrcpy( szMetaOI_Def, REFER_CMOD[ nType - 2000 ].szOD );
      ...
      switch ( nActiveType )
      {
         ...
         default:
            GetStringFromAttribute( szMetaOI_Name, vWkListView, "W_MetaDef", "Name" );
      ...
      if ( nEntityType == CM_ACTIVE_TYPE )
         ...
         zstrcat( szFileSpec, REFER_CMOD[ nType - 2000 ].szOD_EXT );
   ...
   // SfDropSubtask( vApplication, 0 );
      if ( nRC )
      {
         ...
         zsprintf( szMsg, "Error activating OI from file: %s", szFileSpec );
   ...
   if ( nOrigType == zSOURCE_LOD_META || nOrigType == zREFER_LOD_META )
   {
            ...
            nRC = SetCursorNextEntity( vTempLOD, "LOD_Entity", 0 ) )
      {
         ...
         GetStringFromAttribute( szIndentName, vTempLOD, "LOD_Entity", "IndentName" );
         if ( (lLevel > 0 && szIndentName[ 0 ] != ' ') || (szIndentName[ 0 ] == 0) )
         {
            ...
            GetStringFromAttribute( szName, vTempLOD, "LOD_Entity", "Name" );
   ...
   szSyncDate[ 0 ] = 0;
   cfPostActivate( vSubtask, nType, szSyncDate, vZeidonCM,
                   *pvMOI, vWkListView, nPhaseCtl, 0 );

   // DGC 8/17/95
   // I don't think the following command is necessary, so I'm going to take
   // it out.
// ResetView( *pvMOI );

   // If we are activating a Domaim, we must position on the correct Domain
   // within the Domain Group.
   if ( nOrigType == zSOURCE_DOMAIN_META || nOrigType == zREFER_DOMAIN_META )
   {
      if ( SetCursorFirstEntityByInteger( *pvMOI, "Domain", "ZKey",
                                          ulOrigMetaOI_ZKey, "" ) != zCURSOR_SET )
   ...
   // Name the view based upon the ZKey and update/refer type for
...
// TraceLineX( "ActivateMetaOI Locked Mutex: ActMeta  for Task: ",
//             (zLONG) vSubtask->hTask );
   nRC = fnActivateMetaOI( vSubtask, pvMOI, vListView, nType, 0 );
// TraceLineX( "ActivateMetaOI Unlocking Mutex: ActMeta  for Task: ",
//             (zLONG) vSubtask->hTask );
...
//             (zLONG) vSubtask->hTask );
   ...
   if ( vListView )
   {
      ...
      if ( nType < 2000 )
      {
         nRC = SetCursorFirstEntityByInteger( vCM_List, "W_MetaType",
                                              "Type", nType + 2000, "" );
      }
      else
      {
   ...
   if ( nRC != zCURSOR_SET )
   {
     ...
     // the Named View (PVR).
      ...
      //TraceLine( "(tzcmoprs) ActivateMetaOI_ByName - missing Name: %s  in View: _CM_List  Entity: W_MetaDef",
      //            pchName );
...
//             (zLONG) vSubtask->hTask );
   ...
   else
   {
      if ( nType < 2000 )
      {
         nRC = RetrieveViewForMetaList( vSubtask, &vCM_List,
                                        (zSHORT) (nType + 2000) );
      }
      else
      {
         nRC = RetrieveViewForMetaList( vSubtask,
                                        &vCM_List, nType );
      }
   }
                         ...
                         zLONG   lListHandle )
{
   ...
   if ( nNameOrZKey == 0 )
   {
      ...
      if ( pViewItem == 0 )
      {
         ActivateMetaOI_ByName( vSubtask, pvMOI, vListView, nType,
                                lControl, pchName, nCurrentOrReload );
   ...
   else
   {
      ...
      {
         ActivateMetaOI_ByZKey( vSubtask, pvMOI, vListView, nType,
                                lControl, lZKey, nCurrentOrReload );
...
fnRemoveCreateFlag( zVIEW  pvMOI )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      if ( GetIncrementalUpdateFlags( vMOI, szEntityName,
                                      zSET_INCR_CREATED ) == 1 )
      {
         if ( GetIncrementalUpdateFlags( vMOI, szEntityName,
                                         zSET_INCR_INCLUDED ) == 1 )
         {
            // zSET_INCR_PERSISTENT clears the "created" flag.
            SetIncrementalUpdateFlags( vMOI, szEntityName,
...
// zCHAR  szTempFileName[ zMAX_FILENAME_LTH + 1 ];
   ...
   GetStringFromAttribute( szMetaOI_Name, vMOI,
                           SRC_CMOD[ nType ].szOD_ROOT,
                           SRC_CMOD[ nType ].szOD_NAME );

   // If the view is read-only, then we can't commit it.
   nRC = MiGetUpdateForView( vMOI );
   if ( (nEntityType == CM_REFER_TYPE) || (nRC == 0) )
   {
      zstrcpy( szMsg, "The meta passed to CommitMetaOI, " );
      zstrcat( szMsg, szMetaOI_Name );
      zstrcat( szMsg, "is not an Active Meta. See Trace" );
   ...
   GetIntegerFromAttribute( &lMetaOI_ZKey,
                            vMOI, SRC_CMOD[ nType ].szOD_ROOT, "ZKey" );

   // Position on the correct W_MetaType. If it doesn't exist, create one.
   if ( SetCursorFirstEntityByInteger( vLPLR, "W_MetaType",
                                       "Type", nType, "" ) != zCURSOR_SET )
   {
   ...
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaType",
                                       "Type", nType, "" ) != zCURSOR_SET )
   {
      CreateEntity( vActiveMetas, "W_MetaType", zPOS_AFTER );
   ...
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaDef",
                                       ...
                                       "" ) < zCURSOR_SET )
   {
      nRC = fnCheckForDuplicateName( vMOI, vLPLR, szMetaOI_Name,
                                     lMetaOI_ZKey, 1 ); // Adding New Meta
   }
   else
   {
        ...
        nType == zSOURCE_GOPGRP_META )
   {
      ...
      SetCursorFirstEntityByInteger( vTempLPLR, "W_MetaType", "Type",
                                     SubType + 2000, 0 );
      nRC = SetCursorFirstEntity( vMOI, szSubEntityName, 0 );
      while ( nRC >= zCURSOR_SET )
      {
         GetStringFromAttribute( szSubOI_Name,
                                 vMOI, szSubEntityName, "Name" );
         GetIntegerFromAttribute( &lSubOI_ZKey,
                                  vMOI, szSubEntityName, "ZKey" );
         nRC = fnCheckForDuplicateName( vMOI, vTempLPLR, szSubOI_Name,
                                        lSubOI_ZKey, 0 );
         if ( nRC == -1 )
            return( -1 );
            ...
            nRC = SetCursorNextEntity( vMOI, szSubEntityName, 0 ) )
      {
         ...
         SetCursorFirstEntityByInteger( vTempLPLR, "W_MetaType", "Type",
                                        SubType, 0 );
         nRC = SetCursorFirstEntityByInteger( vTempLPLR,
                                              "W_MetaDef", "CPLR_ZKey",
                                              lSubOI_ZKey, 0 );
         if ( nRC >= zCURSOR_SET )
            SetAttributeFromString( vTempLPLR, "W_MetaDef", "Name",
                                    szSubOI_Name );

         // zREFER type
         SetCursorFirstEntityByInteger( vTempLPLR, "W_MetaType", "Type",
                                        SubType + 2000, 0 );
         nRC = SetCursorFirstEntityByInteger( vTempLPLR, "W_MetaDef", "CPLR_ZKey",
                                              lSubOI_ZKey, 0 );
         if ( nRC >= zCURSOR_SET )
            SetAttributeFromString( vTempLPLR, "W_MetaDef", "Name",
                                    szSubOI_Name );
...
#if 0
// PETTIT  I think the MetaDef should always exist after the change to
//         ActivateEmptyMetaOIis made

      nRC = IncludeSubobjectFromSubobject( vActiveMetas, "W_MetaDef",
                                           vLPLR, "W_MetaDef",
                                           zPOS_AFTER );
      GetStringFromAttribute( szCM_ViewName, vActiveMetas,
                              "W_MetaDef", "CM_ViewName" );
      zstrcat( szCM_ViewName, ".u" );
   ...
   else
   {
                                      ...
                                      "TaskID", lTaskID ) != 0 )
      {
         ...
         else
         {
            ...
            {
               zstrcpy( szMsg, "The View to the Meta passed to CommitMetaOI"
         ...
         while ( nRC > 0 )
         {
            if ( zstrncmp( szNamedView, "__CM.", 5 ) == 0 )
            {
               ...
               while ( nRC > 0 )
               {
                  ...
                  nRC = SfGetNextNamedView( &vWork2, szTaskView, vWork1,
                                            zLEVEL_SUBTASK );
               }
            }
   ...
   if ( GetViewByName( &WKS_View, "TZCMWKSO",
                       vZeidonCM, zLEVEL_SUBTASK ) <= 0 )
   {
      return( -1 );
   }

   nRC = fnGetDirectorySpec( vMOI, szFileSpec, nType, TRUE );
   ...
   switch ( nType )
   {
      ...
      default:
         if ( bNewMeta == FALSE &&
              CompareAttributeToString( vActiveMetas, "W_MetaDef",
                                        "Name", szMetaOI_Name ) != 0 )
         {
            // FileName change
            nRC = zstrlen( szFileSpec );
            ...
            {
               GetStringFromAttribute( szFileSpec,
                                       vLPLR, "LPLR", "ExecDir" );
               ...
               if ( nType == zSOURCE_LOD_META )
                  zstrcat( szFileSpec, ".XOD" );
               else
                  zstrcat( szFileSpec, ".XWD" );

               SysOpenFile( vLPLR, szFileSpec, COREFILE_DELETE );
            }

            nRC = fnGetDirectorySpec( vLPLR, szFileSpec, nType, TRUE );
         }
   ...
   zstrcat( szFileSpec, SRC_CMOD[ nType ].szOD_EXT );
   GetVariableFromAttribute( szDesc, 0, zTYPE_STRING, 255, vMOI,
                             SRC_CMOD[ nType ].szOD_ROOT, "Desc", "", 0 );
   ...
   if ( nType == zSOURCE_DIALOG_META || nType == zSOURCE_REPORT_META || nType == zSOURCE_XSLT_META )
   {
      ...
      GetStringFromAttribute( szFileSpec, vLPLR, "LPLR", "ExecDir" );
      ofnTZCMWKSO_AppendSlash( szFileSpec );
      zstrcat( szFileSpec, szMetaOI_File );
...
//    MOI_ExecView = ConvertDialog( vDialogSubtask, vMOI, "" );
      if ( nType == zSOURCE_DIALOG_META )
      {
         ...
         SysReadZeidonIni( -1, "[Workstation]",
                           "ExecutableRemote", szRemotePath );
         if ( szRemotePath[ 0 ] )
         {
            SysReadZeidonIni( -1, "[Workstation]",
                              "ExecutableRemotePath", szRemotePath );
            if ( szRemotePath[ 0 ] )
            {
               ofnTZCMWKSO_AppendSlash( szRemotePath );
               zstrcat( szRemotePath, szMetaOI_File );
               zstrcat( szRemotePath, ".XRA" );
            }
            else
            {
               zstrcpy( szRemotePath, szFileSpec );
            ...
            if ( nRC )
            {
               zstrcpy( szMsg, "CommitMetaOI failed trying to save " );
               zstrcat( szMsg, szRemotePath );
      ...
      fnGetDirectorySpec( vMOI, szFileSpec, nType, TRUE );
      zstrcat( szFileSpec, szMetaOI_File );
      zstrcat( szFileSpec, SRC_CMOD[ nType ].szOD_EXT );
   }
           ...
           nType != zSOURCE_XSLT_META && nType != zSOURCE_UIS_META )
      {
         ...
         if ( nType == zSOURCE_DOMAINGRP_META )
         {
            ...
            while ( nRC > 0 )
            {
               ...
               if ( zstrcmp( szObjectName, "TZEREMDO" ) == 0 )
               {
   ...
   switch ( nType )
   {
      case zSOURCE_ERD_META:
                                             ...
                                             zSOURCE_LOD_META, 0 ) >= zCURSOR_SET )
         {
            ...
            {
               GetStringFromAttribute( szNamedView, vActiveMetas,
                                       "W_MetaDef", "CM_ViewName" );
               zstrcat( szNamedView, ".r" );
               if ( GetViewByName( &CM_View, szNamedView, vZeidonCM,
                                   zLEVEL_SUBTASK ) > 0 )
               {
                  DropObjectInstance( CM_View );
               }
      ...
      case zSOURCE_LOD_META:
         ...
         {
            zstrcpy( szMsg, "(fnCommitMetaOI) Unable to Access XOD.  XOD must be opened." );
         ...
         {
            zstrcpy( szMsg, "Commit of XOD failed." );
      ...
      case zSOURCE_PENV_META:
         ...
         GetStringFromAttribute( szFileSpec, vLPLR, "LPLR", "ExecDir" );
         ofnTZCMWKSO_AppendSlash( szFileSpec );
         zstrcat( szFileSpec, "ZEIDON.XPE" );
         if ( CommitOI_ToFile( MOI_ExecView, szFileSpec, zSINGLE ) != 0 )
         {
            zstrcpy( szMsg, "Commit of Executable Presentation Environment -\n" );
            zstrcpy( szMsg, szFileSpec );
            zstrcpy( szMsg, " failed." );
      ...
      case zSOURCE_DOMAINGRP_META:
      {
         ...
         if ( GetIntegerFromAttribute( &lStatus,
                                       vLPLR, "W_MetaDef", "Status" ) < 0 )
         {
            lStatus = -1;
         }

         // Find the item Meta type (eg Domain). If it doesn't exist, create it.
         if ( SetCursorFirstEntityByInteger( vWorkSource,
                                             "W_MetaType",
                                             "Type",
                                             lSourceItem,
                                             "" )           != zCURSOR_SET )
         {
            ...
            SetAttributeFromInteger( vWorkSource, "W_MetaType", "Type",
                                     lSourceItem );
         }

         // Same for REFER
         if ( SetCursorFirstEntityByInteger( vWorkRefer,
                                             "W_MetaType",
                                             "Type",
                                             lReferItem, "" ) != zCURSOR_SET )
         {
            CreateEntity( vWorkRefer, "W_MetaType", zPOS_LAST );
            SetAttributeFromInteger( vWorkRefer, "W_MetaType", "Type",
                                     lReferItem );
         }
         ...
         for ( nRC = SetCursorFirstEntityByAttr( vWorkRefer,
                                                 "W_MetaDef", "GroupName",
                                                 vMOI, lpchGroupName,
                                                 "Name", 0 );
               nRC >= zCURSOR_SET;
               nRC = SetCursorNextEntityByAttr( vWorkRefer,
                                                "W_MetaDef", "GroupName",
                                                vMOI, lpchGroupName,
                                                "Name", 0 ) )
         {
               ...
               nRC = SetCursorNextEntity( vMOI, lpchItemName, "" ) )
         {
                                             ...
                                             "ZKey", "" ) != zCURSOR_SET )
            {
               ...
               if ( lStatus >= 0 )
                  SetAttributeFromInteger( vWorkRefer, "W_MetaDef", "Status",
                                           lStatus );

               // Create entity in source meta type
               CreateEntity( vWorkSource, "W_MetaDef", zREPOS_LAST );
               SetAttributeFromAttribute( vWorkSource, "W_MetaDef", "Name",
                                          vMOI, lpchItemName, "Name" );
               SetAttributeFromAttribute( vWorkSource, "W_MetaDef", "CPLR_ZKey",
                                          vMOI, lpchItemName, "ZKey" );
               SetAttributeFromAttribute( vWorkSource, "W_MetaDef", "GroupName",
                                          vMOI, lpchGroupName, "Name" );

               // If lStatus is not -1, then we need to set status in W_MetaDef.
               if ( lStatus >= 0 )
               {
                  SetAttributeFromInteger( vWorkSource, "W_MetaDef",
                                           "Status", lStatus );
               }
            }
              ...
              zSHORT nType )
{
   ...
   else
   {
      ...
      do
      {
         nRC = fnCommitMetaOI( vSubtask, vMOI, nType, vActiveMetas,
                               vZeidonCM, vLPLR );
         if ( nRC == 0 )
         {
            nRC = MessagePrompt( vMOI, "CM00437",
                                 "Configuration Management",
                                  szMsg,
                                  zBEEP, zBUTTONS_YESNO,
                                  zRESPONSE_YES, 0 );
...
// zCHAR szTempFileName[ 2 * zMAX_FILESPEC_LTH + 1 ]; // includes szFileSpec
   ...
   {
      zstrcpy( szMsg, "Meta passed to CommitDependentOI - " );
      zstrcat( szMsg, szMetaOI_Name );
      zstrcat( szMsg, " Not found in LPLR!" );
...
///// All this just to get an updatable Meta  //////////////////////////
   ...
   fnGetTaskOI_ListName( vSubtask, szMetaOI_Name );
   nRC = GetViewByName( &vTaskMetas, szMetaOI_Name,
                        vZeidonCM, zLEVEL_SUBTASK );
   */
   /* Get the view to the ActiveMeta OI */
   nRC = GetViewByName( &vActiveMetas, "OpenCM_Metas",
                        vZeidonCM, zLEVEL_SUBTASK );
   /* I need the name of the TaskLPLR to get to the root LPLR
      in the ActiveMeta OI */
   nRC = GetStringFromAttribute( szMsg, vTaskLPLR, "LPLR", "Name" );
   ...
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaType", "Type",
                                       nType - 2000, "" ) != zCURSOR_SET )
   {
      zstrcpy( szMsg, "Meta Type not found in ActiveMeta List.\nType is " );
      zltoa( nType - 2000, &szMsg[ zstrlen( szMsg ) ] );
   ...
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaDef",
                                       "CPLR_ZKey", lMetaOI_ZKey, "" )
                                != zCURSOR_SET )   // Not in the Active List
   {
      zstrcpy( szMsg, "Meta not found in ActiveMeta List.\nType is " );
      zstrcat( szMsg, szMetaOI_Name );
   ...
   GetStringFromAttribute( szMsg, vActiveMetas, "W_MetaDef", "CM_ViewName" );
   zstrcat( szMsg, ".r" );
   ...
   if ( nRC )
   {
      zstrcpy( szMsg, "CommitDependentOI failed trying to save " );
      zstrcat( szMsg, szFileSpec );
   ...
   if ( nType == zREFER_LOD_META )
   {
      ...
      if ( GetViewByName( &MOI_ExecView, "TZZOXODO", vSubtask, zLEVEL_TASK ) < 1 )
      {
         zstrcpy( szMsg, "(CommitDependentOI) Unable to Access XOD.  XOD must be opened." );
...
/* PETTIT Probably don't need to commit WKS because MaxZKey shouldn't have
          been updated.
                     ...
                     zLONG  lControl )
{
   ...
   if ( nEntityType == CM_ACTIVE_TYPE )
   {
      zstrcpy( szWorkName, SRC_CMOD[ nType ].szOD );
      zstrcpy( szOD_RootName, SRC_CMOD[ nType ].szOD_ROOT );
      lControl = zLEVEL_APPLICATION;
//    lControl = zLEVEL_TASK;
   }
   else
   {
      zstrcpy( szWorkName, REFER_CMOD[ nType - 2000 ].szOD );
      zstrcpy( szOD_RootName, REFER_CMOD[ nType - 2000 ].szOD_ROOT );
      lControl = zLEVEL_APPLICATION;
   }
#if 0
   ...
   // Get the view to the vTaskMetas subtask.
   fnGetTaskOI_ListName( vSubtask, szWorkName );
   GetViewByName( &vTaskMetas, szWorkName, vZeidonCM, zLEVEL_SUBTASK );

   // Get the view to the ActiveMeta OI.
   GetViewByName( &vActiveMetas, "OpenCM_Metas", vZeidonCM, zLEVEL_SUBTASK );
   nRC = GetStringFromAttribute( szWorkName, vTaskLPLR, "LPLR", "Name" );
   nRC = SetCursorFirstEntityByString( vActiveMetas, "LPLR", "Name",
                                                     szWorkName, "" );
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaType", "Type",
                                       nType, "" ) != zCURSOR_SET )
...
*/
   ...
   zltoa( *((zPLONG) pvMOI), &szWorkName[ 1 ] );
   SetNameForView( *pvMOI, szWorkName, vTaskMetas,
                   zLEVEL_SUBTASK | zNAME_AUTODROP );
   return( 1 );
}
...
DropMetaOI( zVIEW vSubtask, zVIEW vMOI )
{
   ...
   szViewName[ 0 ] = 'u';
   if ( GetViewByName( &RU_View, szViewName, vTaskMetas,
                       zLEVEL_SUBTASK ) < 0 )  // not an Update View
   {
      return( -1 );
   }
   ...
   // in the ActiveMeta OI.
   GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szLPLR_Name, vTaskLPLR, "LPLR", "Name" );
   SetCursorFirstEntityByString( vActiveMetas, "LPLR",
                                 "Name", szLPLR_Name, "" );
   if ( fnCheckCM_ForActiveMetaByTask( vSubtask, lMOI_InstanceID ) == 1 )
   {
      ...
      GetStringFromAttribute( szViewName, vActiveMetas,
                              "W_MetaDef", "CM_ViewName" );
      zstrcat( szViewName, ".r" );
      if ( GetViewByName( &RU_View, szViewName,
                          vZeidonCM, zLEVEL_SUBTASK ) < 0 )
      {
         ExcludeEntity( vActiveMetas, "W_MetaDef", zREPOS_NONE );
      }
              ...
              zSHORT nType )
{
   ...
   GetStringFromAttribute( szMetaOI_Name, vListView, "W_MetaDef", "Name" );
   nRC = SetCursorFirstEntityByInteger( vLPLR, "W_MetaDef",
                                        "CPLR_ZKey", lMetaOI_ZKey, "LPLR" );
   if ( nRC < zCURSOR_SET )
   {
      zstrcpy( szErrMsg, "Component to be Deleted not found in the LPLR." );
   ...
   {
      zstrcpy( szErrMsg, "Component to be Deleted not active in the LPLR." );
   ...
   if ( nRC >= zCURSOR_SET )
   {
      ...
      if ( lTaskID )
      {
         GetStringFromAttribute( szCM_ViewName, vActiveList,
                                 "W_MetaDef", "CM_ViewName" );
         zstrcat( szCM_ViewName, ".u" );
         if ( GetViewByName( &CM_View, szCM_ViewName,
                             vZeidonCM, zLEVEL_SUBTASK ) > 0 )
   ...
   nRC = fnGetDirectorySpec( vListView, szFileSpec, nType, TRUE );
   if ( nWorkType == zSOURCE_VOR_META )
   {
      zltoxa( lMetaOI_ZKey, szMetaOI_Name );
   }
   zstrcat( szFileSpec, szMetaOI_Name );
   zstrcat( szFileSpec, SRC_CMOD[ nWorkType ].szOD_EXT );
   ...
   else
   {
      ...
      {
         zstrcpy( szErrMsg, "File " );
         zstrcat( szErrMsg, szFileSpec );
      ...
      if ( nWorkType == zSOURCE_DIALOG_META || nWorkType == zSOURCE_LOD_META )
      {
         ...
         {
            zstrcpy( szErrMsg, "Corresponding Executable File " );
            zstrcat( szErrMsg, szFileSpec );
            zstrcat( szErrMsg, " Not found." );
           ...
           nWorkType == zSOURCE_XSLT_META || nWorkType == zREFER_XSLT_META )
      {
         ...
         ofnTZCMWKSO_AppendSlash( szFileSpec );
         zstrcat( szFileSpec, szMetaOI_Name );
         zstrcpy( szReportFile, szFileSpec );     // Save the name without extension
         ...
         if ( nWorkType == zSOURCE_REPORT_META || nWorkType == zREFER_REPORT_META )
         {
             ...
             zstrcpy( szFileSpec, szReportFile );
             zstrcat( szFileSpec, ".XSQ" );
             ...
             GetStringFromAttribute( szFileSpec, vLPLR, "LPLR", "PgmSrcDir" );
             ofnTZCMWKSO_AppendSlash( szFileSpec );
             zstrcat( szFileSpec, szMetaOI_Name );
             zstrcat( szFileSpec, ".REP" );
             SysOpenFile( vLPLR, szFileSpec,
                     COREFILE_DELETE ); // We don't test the Returncode of
                                             // SysOpenFile, because we get an error
                                             // if the report is a zeidon report.
   ...
   if ( CheckExistenceOfEntity( vMeta, "Z_MetaDef" ) >= zCURSOR_SET )
   {
      zstrcpy( szEntityName, SRC_CMOD[ nWorkType ].szOD_ROOT );
...
CreatePE_ExecVersion( zVIEW vSubtask, zVIEW vPresEnv )
{
 ...
 //zLONG  lFontCount;
   ...
   GetStringFromAttribute( szWorkString, vSubtask, "LPLR", "ExecDir" );
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      GetStringFromAttribute( szWorkString, vPresEnv,
                              "ControlDef", "RT_OperationName" );
      GetStringFromAttribute( szGUID, vPresEnv,
                              "ControlDef", "GUID" );
...
/* PETTIT  Add this code when we have a CPL to work with!!!!!!  */
        ...
        lWkZKey > 999999 )
   {
      ...
      if ( vTZCMREPO == 0 )
      {
         if ( ActivateObjectInstance( &vTZCMREPO, "TZCMREPO",
                                      lpView, 0, zSINGLE ) != 0 )
         {
      ...
      if ( vTZCMREPO )
      {
         GetIntegerFromAttribute( &lWKS_Id, vTZCMREPO,
      ...
      if ( vTZCMREPO )
      {
         SetAttributeFromInteger( vTZCMREPO, "Installation",
                                  "NextWKS_Id", lWKS_Id );
...
//./ END + 4
zOPER_EXPORT zSHORT OPERATION
CreateFileNameFromZKey( zPCHAR szFileName,
                        zVIEW lpView,
                        zPCHAR szEntity )
{
   zULONG  ulZKey;
   ...
   nRC = GetIntegerFromAttribute( (zPLONG) &ulZKey, lpView,
                                  szEntity, "ZKey" );
   zltoxa( ulZKey, szFileName );
   return( 0 );
   ...
   {
      lWKS_Id = lWKS_Id + 64;
      szFileName[ 0 ] = ( zCHAR )lWKS_Id;
      if ( lNumZKey < 10 )
      {
         zstrcpy( &szFileName[ 1 ], "000000" );
         zltoa( lNumZKey, &szFileName[ 7 ] );
      }
      else
      if ( lNumZKey < 100 )
      {
         zstrcpy( &szFileName[ 1 ], "00000" );
         zltoa( lNumZKey, &szFileName[ 6 ] );
      }
      else
      if ( lNumZKey < 1000 )
      {
         zstrcpy( &szFileName[ 1 ], "0000" );
         zltoa( lNumZKey, &szFileName[ 5 ] );
      }
      else
      if ( lNumZKey < 10000 )
      {
         zstrcpy( &szFileName[ 1 ], "000" );
         zltoa( lNumZKey, &szFileName[ 4 ] );
      }
      else
      if ( lNumZKey < 100000 )
      {
         zstrcpy( &szFileName[ 1 ], "00" );
         zltoa( lNumZKey, &szFileName[ 3 ] );
      }
      else
      if ( lNumZKey < 1000000 )
      {
         zstrcpy( &szFileName[ 1 ], "0" );
         zltoa( lNumZKey, &szFileName[ 2 ] );
      }
      else
         zltoa( lNumZKey, &szFileName[ 1 ] );
   return( 0 );
   }
   else
      return( -1 );
                    ...
                    zSHORT Assign )
{
   ...
   if ( nEntityType == CM_ACTIVE_TYPE )
   {
      zstrcpy( szEntityName, SRC_CMOD[ nType ].szOD_ROOT );
      if ( DefineHierarchicalCursor( OI_View,
                                     SRC_CMOD[ nType ].szOD_ROOT ) != 0 )
      {
         return( -1 );
      }
   }
   ...
   if ( nEntityType == CM_REFER_TYPE )
   {
      zstrcpy( szEntityName, REFER_CMOD[ nType - 10000 ].szOD_ROOT );
      if ( DefineHierarchicalCursor( OI_View,
                                REFER_CMOD[ nType - 2000 ].szOD_ROOT ) != 0 )
      {
         return( -1 );
      }
   }
   ...
   do
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKey, OI_View, szEntityName,
                               "ZKey" );
      if ( (ulZKey > 999999999) || (ulZKey <= 0) || (Assign == 1) )
      {
         if ( SetZKeyToNextZKey( vSubtask, OI_View, szEntityName ) < 0 )
         {
            TraceLineS( "Configuration Management",
                        "Assign of ZKey Failed!!!" );
            zstrcpy( szErrMsg, "Entity is: " );
            zstrcpy( szErrMsg, szEntityName );
            TraceLineS( "Configuration Management", szErrMsg );
            DisplayEntityInstance( OI_View, szEntityName );
            zstrcpy( szErrMsg, "Root Entity is: " );
            zstrcpy( szErrMsg, SRC_CMOD[ nType ].szOD_ROOT );
            TraceLineS( "Configuration Management", szErrMsg );
            return( -1 );
         }
      }
                   ...
                   zPCHAR szEntityName )
{
   ...
   if ( ulMaxZKey == 0 )
   {
      GetIntegerFromAttribute( &lWKS_Id, WKS_View,
                               "RepositoryClient", "WKS_Id" );
      ulMaxZKey = lWKS_Id * 1000000;
   }
                  ...
                  zPCHAR szPrefix )
{
   if ( nType <= zSOURCE_MAX_META )
   {
      zstrcpy( szPrefix, SRC_CMOD[ nType ].szOD_PREFIX );
      return( 0 );
   }

   zstrcpy( szPrefix, REFER_CMOD[ nType - 2000 ].szOD_PREFIX );
   return( 2 );
}
...
/* PETTIT  Add this code when we have a CPL to work with!!!!!!  */
   ...
   ulMaxZKey++;
   if ( SetAttributeFromInteger( vTaskLPLR, szEntityName,
                                 "CPLR_ZKey", ulMaxZKey ) != 0 )
   {
...
BuildLPLR_MetaTypes( zVIEW vSubtask, zVIEW vLPLR, zSHORT BuildMetas )
{
   ...
   for ( nType = 2000; nType <= zREFER_MAX_META; nType++ )
   {
      ...
      if ( BuildMetas == 1 )
      {
         ...
         GetStringFromAttribute( szTempSpec, vLPLR, "LPLR", "MetaSrcDir" );
         SysConvertEnvironmentString( szDirectorySpec, szTempSpec );
         ofnTZCMWKSO_AppendSlash( szDirectorySpec );
         zstrcat( szDirectorySpec, "*" );
         zstrcat( szDirectorySpec, SRC_CMOD[ nType - 2000 ].szOD_EXT );
   ...
   for ( nType = 0; nType <= zSOURCE_MAX_META; nType++ )
   {
                                          ...
                                          nType + 2000, "" ) == zCURSOR_SET )
      {
         ...
         while ( nRC == zCURSOR_SET )
         {
            GetIntegerFromAttribute( (zPLONG) &ulZKey, IncludeView,
                                     "W_MetaDef", "CPLR_ZKey" );
            if ( SetCursorFirstEntityByInteger( vLPLR, "W_MetaDef",
               ...
               zPCHAR pchMessageText )
{
   ...
   // SR 2000.10.23:
   // allocate 2*zMAX_FILESPEC_LTH + zLONG_MESSAGE_LTH + 1  space because
   // szMessage includes szLPLR_Phat and szSystem_Phat and
   ...
   if ( SetCursorFirstEntity( vMetaView, "W_MetaDef", "" ) < zCURSOR_SET )
   {
      ...
      GetViewByName( &vZeidonCM, "ZeidonCM", vSubtask, zLEVEL_APPLICATION );
      nRC = GetViewByName( &vSystemPPE, NAME_OF_CACHED_PPE,
                           vZeidonCM, zLEVEL_SUBTASK );
      ...
      zstrcpy( szFileName, szSystem_Phat );
      zstrcat( szFileName, "ZEIDON.PPE" );
      ...
      nRC = ActivateOI_FromFile( &vSystemPPE, "TZPESRCO",
                                 vMetaView, szFileName,
                                 zIGNORE_ERRORS | zLEVEL_APPLICATION );
      if ( nRC < 0 )
      {
         if ( GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK ) > 0 )
         {
            GetStringFromAttribute( szLPLR_Phat,
                                    vTaskLPLR, "LPLR", "MetaSrcDir" );
         }
         else
            zstrcpy( szSystem_Phat, "" );

         zstrcpy( szMessage, "Presentation Environment was not found in directories\n" );
         zstrcat( szMessage, szLPLR_Phat );
         zstrcat( szMessage, "  and  " );
         zstrcat( szMessage, szSystem_Phat );
         zstrcat( szMessage, ".\n\n" );
         zstrcat( szMessage, pchMessageText );
      ...
      CreateViewFromViewForTask( pvPPEView, vSystemPPE, vSubtask );
      SetNameForView( vSystemPPE, NAME_OF_CACHED_PPE,
                      vZeidonCM, zLEVEL_SUBTASK );
                       ...
                       zLONG  lType )
{
   ...
   if ( lType == zSOURCE_DIALOG_META || lType == zSOURCE_REPORT_META || lType == zSOURCE_XSLT_META )
      zstrcpy( szAttributename, "Tag" );
   else
      zstrcpy( szAttributename, "Name" );
   ...
   if ( SetCursorFirstEntityByAttr( vActiveMetas, "W_MetaDef", "Name",
        ...
        CompareAttributeToInteger( vActiveMetas, "W_MetaDef",
                                   "Status", 1 ) != 0 )
   {
      nRC = 0;
   }
                          ...
                          zPCHAR  szName )        // Name for duplicate Component
{
        ...
        CheckExistenceOfEntity( vTZCMLPLO_Copy, "W_MetaDef" ) >= zCURSOR_SET )
   {
      ...
      if ( lTypeCPRO == lComponentType )
      {
         ...
         GetIntegerFromAttribute( (zPLONG) &ulZKeyLPLR, vTZCMLPLO_Copy,
                                   "W_MetaDef", "CPLR_ZKey" );
         GetIntegerFromAttribute( (zPLONG) &ulZKeyCPRO, vTZCMCPRO_Copy,
                                   "Z_MetaDef", "ZKey" );

         if ( ulZKeyLPLR != ulZKeyCPRO )
         {
            // duplicate component
            GetStringFromAttribute( szName, vTZCMCPRO_Copy, "Z_MetaDef", "Name" );
            nRC = 1;
            return( -1 );
         }
      }
                       ...
                       zPCHAR  szName )     // Return Name for duplicate Component
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      nRC = CheckDuplicateName( vTZCMLPLO_Copy,  vTZCMCPRO, szTypeCPLR,
                                szTypeLPLR, szName );
      if ( nRC < 0 )
      {
         DropView( vTZCMLPLO_Copy );
                    ...
                    zPCHAR  szName )         // Return Name for duplicate Component
{
   ...
   GetStringFromAttribute( szName, vTZCMLPLO, "W_MetaDef", "Name" );
   //exists a component with this name, then check the Key
   if ( SetCursorFirstEntityByString( vTZCMCPRO_Copy, "Z_MetaDef", "Name",
                                      szName, "" ) >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKeyLPLR, vTZCMLPLO,
                               "W_MetaDef", "CPLR_ZKey" );
      do
      {
         GetIntegerFromAttribute( (zPLONG) &ulZKeyCPRO, vTZCMCPRO_Copy,
         ...
         if ( ulZKeyLPLR != ulZKeyCPRO )
         {
            GetIntegerFromAttribute( &lTypeCPRO, vTZCMCPRO_Copy,
                                     "Z_MetaDef", "Type" );
            GetIntegerFromAttribute( &lTypeLPLR, vTZCMLPLO,
                                     "W_MetaType", "Type" );
            if ( lTypeLPLR >= 2000 )
            ...
            if ( nRC < 0 )
            {
               GetVariableFromAttribute( szTypeCPLR, 0, zTYPE_STRING, 125,
                                         vTZCMCPRO_Copy, "Z_MetaDef",
                                         "Type", "CM_Type", 0 );
               GetVariableFromAttribute( szTypeLPLR, 0, zTYPE_STRING, 125,
                                         vTZCMLPLO, "W_MetaType",
                                         "Type", "CM_Type", 0 );
               DropView( vTZCMCPRO_Copy );
               return( nRC );
            }
      ...
      while ( SetCursorNextEntityByString( vTZCMCPRO_Copy, "Z_MetaDef",
                                  "Name", szName, "" ) >= zCURSOR_SET );
                              ...
                              zPULONG  pulZKeyLPLR )// ReturnZKey for duplicate Comp
{
   ...
   *pulZKeyLPLR = 0;
   ...
   if ( SetCursorFirstEntityByString( vTZCMLPLO_Copy, "W_MetaDef", "Name",
                                      szName, "LPLR" ) >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKeyCPRO, vTZCMCPRO,
                               "Z_MetaDef", "ZKey" );
      do
      {
         GetIntegerFromAttribute( (zPLONG) pulZKeyLPLR, vTZCMLPLO_Copy,
                                 "W_MetaDef", "CPLR_ZKey" );

         // If it is another component ... check the type.
         if ( *pulZKeyLPLR != ulZKeyCPRO )
         {
            GetIntegerFromAttribute( &lTypeCPRO, vTZCMCPRO,
                                     "Z_MetaDef", "Type" );
            GetIntegerFromAttribute( &lTypeLPLR, vTZCMLPLO_Copy,
                                     "W_MetaType", "Type" );
            if ( lTypeLPLR >= 2000 )
               lTypeLPLR = lTypeLPLR - 2000;
            if ( lTypeCPRO >= 2000 )
      ...
      while ( SetCursorNextEntityByString( vTZCMLPLO_Copy, "W_MetaDef",
                                  "Name", szName, "LPLR" ) >= zCURSOR_SET );
...
//BL, 2000.02.25  set Project name, Tool name, component name and check out
//                state in title
//
//                RETURNS:    < 0   - error
//                            = 0   - no error
                            ...
                            zLONG   lType )
{
   ...
   GetStringFromAttribute ( szLPL_Name, vTaskLPLR, "LPLR", "Name" );

   if ( GetViewByName( &vToolView, szLodname, vSubtask, zLEVEL_TASK ) >= 0 )
   {
      GetStringFromAttribute( szComponentname, vToolView,
      ...
      if ( ComponentIsCheckedOut( vSubtask, vToolView, lType ) )
         zstrcpy( szCheckedOut, "" );
      else
         zstrcpy( szCheckedOut, "   < not checked out >" );
   }
   else
   {
      zstrcpy( szComponentname, "[Untitled]" );
      zstrcpy( szCheckedOut, "" );
   }

   zstrcpy( szTitle, szToolname );
   zstrcat( szTitle, " - " );
   zstrcat( szTitle, szComponentname );

   zstrcat( szTitle, szCheckedOut );
               ...
               zVIEW vCM_List )
{
   ...
   else
   {
      if ( lType == zREFER_DOMAIN_META || lType == zREFER_GO_META )
         zstrcpy( szAttributeName, "GroupName" );
      else
         zstrcpy( szAttributeName, "Description" );
...
SortComponents( zPVIEW vCM_List )
{
   ...
   switch( lSortOrder )
   {
      case 0:        // no sort order
         zstrcpy( szOrderKeys, "LastUpdateDate D Name A" );
         break;

      case 1:        // sort by name
         zstrcpy( szOrderKeys, "Name A LastUpdateDate D" );
         break;
      ...
      case 3:        // sort by Group or Description
         if ( lType == zSOURCE_DOMAIN_META || lType == zSOURCE_GO_META ||
            ...
            zstrcpy( szOrderKeys, "GroupName A Name A" );
         else
            zstrcpy( szOrderKeys, "Desc A Name A" );
         break;
   }
      ...
      case 1 :
         ...
         else
         {
            ...
            else
            {
               ...
               // the new one.
               GetStringFromAttribute( szAktLPLRName, vTarget, "LPLR", "Name" );
               zSearchAndReplace(pchSourceAktDir, 255, pchSourceLPLRName, szAktLPLRName);
               nZRetCode = SetAttributeFromString( vTarget, pchEntityName, pchAttributeName,
                                                   pchSourceAktDir );
            }
         }
