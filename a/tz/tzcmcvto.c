...
fnCloneActions( zVIEW vAct, zVIEW vActSrc, zVIEW vSubtask, zCPCHAR cpcDLL_Name, zCHAR ch )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      if ( CheckExistenceOfEntity( vActSrc, "ActOpt" ) == 0 )
      {
         ...
         GetStringFromAttribute( szTag + nOffset, vActSrc, "Action", "Tag" );
         ...
         {
            GetStringFromAttribute( szTag + nOffset, vActSrc, "ActOper", "Name" );
...
// zCHAR    szViewName[ 33 ];
   ...
   GetStringFromAttribute( szViewName, vOrigWO, "OptMap", "X_ViewName" );
   ...
   if ( nRC == zCURSOR_SET )
   {
      ...
      {
         GetStringFromAttribute( szTag + nOffset, vOrigWO, "OptAct", "Tag" );
         szTag[ 32 ] = 0;
...
// while ( nRC > zCURSOR_UNCHANGED )
   ...
   if ( bDflt == FALSE )
   {
      GetStringFromAttribute( szTag, vOrigW, "DfltMenu", "Tag" );
...
// zCHAR  szAppName[ zZEIDON_NAME_LTH + 1 ];
   ...
   SysReadZeidonIni( -1, "[Workstation]", "ReusableMainSideWindow", szReusableMainSideWindow );
   ...
   GetStringFromAttribute( szRemoteWndTag, vSrcDialog, "Dialog", "Tag" );
   nRemoteDlgTagLth = zstrlen( szRemoteWndTag );
   ...
   SetResourceFileIDs( vSubtask, vSrcDialog, 0, 0, szBuffer, vSrcDialog );
   ...
   //                           sizeof( szAppName ) );
      zsprintf( szAppSection, "[App.%s]", pch );

      SysReadZeidonIni( -1, szAppSection,
                        "ExecutableRemoteImageStrip", szTempString );
      if ( szTempString[ 0 ] == 0 )
      {
      ...
      SysReadZeidonIni( -1, szAppSection,
                        "ExecutableRemoteImage", szTempString );
      if ( szTempString[ 0 ] == 0 )
      {
         SysReadZeidonIni( -1, "[Workstation]",
                           "ExecutableRemoteImage", szTempString );
      ...
      SysReadZeidonIni( -1, szAppSection, "DLTextEditUnits", szTempString );
      if ( szTempString[ 0 ] )
      {
         g_dDLTextEditUnits = atof( szTempString );
         TraceLine( "Using DLTextEditUnits %g for application: %s",
                    g_dDLTextEditUnits, pch );
      }
   }
   ...
   {
      SetAttributeFromAttribute( vTgtDialog, "Dlg", "DLL",
                                 vSrcDialog, "Dialog", "DLL_Name" );
   }
// else
// {
...
// }
   ...
   {
      GetStringFromAttribute( szDfltWnd, vSrcDialog, "DfltWnd", "Tag" );
...
// nRC = CheckExistenceOfEntity( vSrcDialog, "Window" );
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szRemoteWndTag + nRemoteDlgTagLth,
                              vSrcDialog, "Window", "Tag" );
#ifdef DEBUG_ALL
      TraceLineS( "(tzcmcvto) Processing Window ====> ",
                  szRemoteWndTag + nRemoteDlgTagLth );
#endif
      ...
      if ( CheckExistenceOfEntity( vSrcDialog, "DfltMenu" ) == zCURSOR_SET )
      {
         ...
         GetStringFromAttribute( szDfltMenu, vSrcDialog, "DfltMenu", "Tag" );
//       TraceLineS( "(tzcmcvto) Processing DfltMenu ", szDfltMenu );
      ...
      //xSetAttributeFromString( vTgtRemote, "Wnd", "DfltMenu", szDfltMenu );
         if ( szReusableMainSideWindow[ 0 ] == 'Y' &&
              CheckExistenceOfEntity( vSrcDialog, "ReusableSideWindow" ) == 0 )
         {
            ...
            if ( lZKey == lZKeyTmp )
            {
               ...
               // fnCloneMenuAndOptions( vAct, vActSrc, vSubtask, TRUE, 0 );
                  ...
                  {
                     // Get the name of the default menu and store it for later use.
                     GetStringFromAttribute( szDfltMenu, vAct, "DfltMenu", "Tag" );
                     SetAttributeFromString( vTgtDialog, "Wnd", "DfltMenu", szDfltMenu );
                  }
            ...
            else
            {
               ...
               if ( SetCursorFirstEntityByInteger( vActSrc, "Window", "ZKey", lZKeyTmp, "" ) == 0 )
               {
                  ...
                  GetStringFromAttribute( szDLL_Name, vActSrc, "Dialog", "DLL_Name" );
      ...
      if ( szReusableMainSideWindow[ 0 ] == 'Y' &&
           CheckExistenceOfEntity( vSrcDialog, "ReusableMainWindow" ) == 0 )
      {
         zBOOL bDefaultMenu;
   ...
   // nRC = CheckExistenceOfEntity( vSrcDialog, "Action" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         SetMatchingAttributesByName( vTgtDialog, "Act",
                                      vSrcDialog, "Action", zSET_ALL );
         if ( CheckExistenceOfEntity( vSrcDialog, "ActOper" ) == 0 )
         {
            SetAttributeFromAttribute( vTgtDialog, "Act", "DO",
                                       vSrcDialog, "ActOper", "Name" );
         }

         SetAttributeFromAttribute( vTgtDialog, "Act", "DlgN",
                                    vSrcDialog, "Action", "DialogName" );
         SetAttributeFromAttribute( vTgtDialog, "Act", "WndN",
                                    vSrcDialog, "Action", "WindowName" );
         SetAttributeFromAttribute( vTgtDialog, "Act", "Popup",
                                    vSrcDialog, "Action", "PopupMenuName" );
         GetIntegerFromAttribute( (zPLONG) &ul, vSrcDialog,
         ...
         GetIntegerFromAttribute( (zPLONG) &ulActSB, vSrcDialog,
                                  "Action", "SB_SubAction" );
         ...
         SetAttributeFromAttribute( vTgtDialog, "Act", "Prompt",
                                    vSrcDialog, "Action",
                                    "ActionTypeAssociatedText" );
         if ( CheckExistenceOfEntity( vSrcDialog, "ActMap" ) == 0 )
         {
            if ( CheckExistenceOfEntity( vSrcDialog, "ActMapView" ) == 0 )
               SetAttributeFromAttribute( vTgtDialog, "Act", "VN",
                                          vSrcDialog, "ActMapView", "Name" );
//x         else
...
//x            TraceLineS( "(tzcmcvto) ActMap entity without ActMapLOD_Entity entity", "" );
            ...
            {
               if ( CheckExistenceOfEntity( vSrcDialog, "ActMapLOD_Entity" ) == 0 )
                  SetAttributeFromAttribute( vTgtDialog, "Act", "Scope",
                                             vSrcDialog, "ActMapLOD_Entity",
                                             "Name" );
//x            else
//x               TraceLineS( "(tzcmcvto) ActMap entity without ActMapLOD_Entity entity", "" );
   ...
   //xSetAttributeFromInteger( vTgtRemote, "Wnd", "ActCnt", g_nActCnt );
      SetMatchingAttributesByName( vTgtDialog, "Wnd",
                                   vSrcDialog, "Window", zSET_ALL );
      SetMatchingAttributesByName( vTgtRemote, "Wnd",
                                   vSrcDialog, "Window", zSET_ALL );
      ...
      SetAttributeFromAttribute( vTgtDialog, "Wnd", "Caption",
                                 vSrcDialog, "Window", "Caption" );
      ...
      if ( nRC >= zCURSOR_SET )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
            ...
            SetAttributeFromAttribute( vTgtDialog, "NLS_Text", "LI",
                                       vSrcDialog, "CaptionNLS_Text",
                                       "LanguageIndex" );
            SetAttributeFromAttribute( vTgtDialog, "NLS_Text", "Text",
                                       vSrcDialog, "CaptionNLS_Text", "Text" );
            nRC = SetCursorNextEntity( vSrcDialog, "CaptionNLS_Text", "" );
         }
      }

      // Get the name of the default button and store it for later use.
      GetStringFromAttribute( szDfltBtn, vSrcDialog, "Window", "DfltButton" );
      SetAttributeFromString( vTgtDialog, "Wnd", "DfltBtn", szDfltBtn );
   //xSetAttributeFromString( vTgtRemote, "Wnd", "DfltBtn", szDfltBtn );

      SetAttributeFromAttribute( vTgtDialog, "Wnd", "ResID",
                                 vSrcDialog, "Window", "ResourceID" );
      ...
      if ( SetCursorFirstEntityByString( vPE, "WindowStyle", "Tag",
                                         pch, 0 ) == zCURSOR_SET )
      {
         SetAttributeFromAttribute( vTgtDialog, "Wnd", "Style",
                                    vPE, "WindowStyle", "Style" );
         SetAttributeFromAttribute( vTgtRemote, "Wnd", "Style",
                                    vPE, "WindowStyle", "Style" );
         SetAttributeFromAttribute( vTgtDialog, "Wnd", "StyleEx",
                                    vPE, "WindowStyle", "StyleEx" );
      }
      else
      {
   ...
   // nRC = CheckExistenceOfEntity( vSrcDialog, "Hotkey" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         SetMatchingAttributesByName( vTgtDialog, "Hotkey",
                                      vSrcDialog, "Hotkey", zSET_ALL );
         if ( CheckExistenceOfEntity( vSrcDialog, "HotAct" ) == 0 )
         {
            ulLth = sizeof( zACCEL_TABLE );
            g_lpAT->chType = zACCEL_ACTION;
            GetBlobFromAttribute( &(g_lpAT->ac), &ulLth,
                                  vSrcDialog, "Hotkey", "KeyCombo" );
            SetEntityCursor( vAct, "Action", 0, zPOS_FIRST | zQUAL_ENTITYCSR,
                             vSrcDialog, "HotAct", 0, 0, 0, 0 );
            g_lpAT->nID = (zSHORT) GetRelativeEntityNumber( vAct, "Action",
                                                            0, 0L );
            ...
            if ( SetCursorFirstEntityByString( g_vTgtDialogOrig, "Act", "Tag",
                                               pch, 0 ) == zCURSOR_SET )
            {
               IncludeSubobjectFromSubobject( vTgtDialog, "HotAct",
                                              g_vTgtDialogOrig, "Act",
                                              zPOS_AFTER );
               CreateEntity( vTgtRemote, "Hotkey", zPOS_AFTER );
               SetMatchingAttributesByName( vTgtRemote, "Hotkey",
                                            vSrcDialog, "Hotkey", zSET_ALL );
               SetAttributeFromString( vTgtRemote, "Hotkey",
                                       "ActionTag", pch );
               GetIntegerFromAttribute( (zPLONG) &ul, g_vTgtDialogOrig,
                                        "Act", "SyncIdx" );
               SetAttributeFromInteger( vTgtRemote, "Hotkey",
                                        "SyncIdx", ul + 1 );
               SetAttributeFromInteger( vTgtRemote, "Hotkey",
                                        "ID", g_lpAT->nID );
               ...
               GetIntegerFromAttribute( (zPLONG) &ul, vSrcDialog,
                                        "Hotkey", "VKey" );
               if ( ul == VK_ESCAPE && g_lpAT->ac.fCtrl == 0 &&
                    g_lpAT->ac.fAlt == 0 && g_lpAT->ac.fShift == 0 )
               {
                  bESC = TRUE;
               }
            }
      ...
      DropView( vAct );
      if ( g_usHotkeyLth )
         SetAttributeFromBlob( vTgtDialog, "Wnd", "Hotkey",
                               g_lpHotkey, g_usHotkeyLth );
   ...
   // nRC = CheckExistenceOfEntity( vSrcDialog, "WndEvent" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         SetMatchingAttributesByName( vTgtDialog, "WndEvent",
                                      vSrcDialog, "WndEvent", zSET_ALL );
         ...
         if ( CheckExistenceOfEntity( vSrcDialog, "WndAct" ) >= zCURSOR_SET )
         {
                                               ...
                                               pch, 0 ) == zCURSOR_SET )
            {
               GetIntegerFromAttribute( (zPLONG) &ul, vSrcDialog,
                                        "WndEvent", "Type" );
               IncludeSubobjectFromSubobject( vTgtDialog, "WndAct",
                                              g_vTgtDialogOrig, "Act", zPOS_AFTER );

               // Some events must not be transferred to the LightDriver.
               if ( ul != zWND_EVENT_PRE_BUILD  &&
                    ul != zWND_EVENT_POST_BUILD  &&
                    ul != zWND_EVENT_ON_RETURN )  // REFRESH?
               {
                  CreateEntity( vTgtRemote, "WndEvent", zPOS_AFTER );
                  SetAttributeFromString( vTgtRemote, "WndEvent",
                                          "ActionTag", pch );
                  if ( ul == zWND_EVENT_CLOSE )
                     bSystemClose = TRUE;

                  SetAttributeFromInteger( vTgtRemote, "WndEvent",
                                           "Tag", ul );
                  GetIntegerFromAttribute( (zPLONG) &ul, g_vTgtDialogOrig,
                                           "Act", "SyncIdx" );
                  SetAttributeFromInteger( vTgtRemote, "WndEvent",
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( *pch == 'Y' || *pch == 'y' )
         {
            GetIntegerFromAttribute( &ul, vPE, "EventDef", "Type" );
            if ( SetCursorFirstEntityByInteger( vSrcDialog, "WndEvent",
                                                "Type", ul,
                                                0 ) != zCURSOR_SET )
            {
               CreateEntity( vTgtRemote, "WndEvent", zPOS_AFTER );
               ...
               zstrcpy( szTempString, szRemoteWndTag + nRemoteDlgTagLth );
               k = zstrlen( szTempString );
               szTempString[ k++ ] = ';';
               zltoa( ul, szTempString + k );
               SetAttributeFromString( vTgtRemote, "WndEvent",
                                       "ActionTag", szTempString );
            }
         }
      ...
      SetAttributeFromBlob( vTgtDialog, "Wnd",
                            "OperCtrl", &ulWndOper, sizeof( zLONG ) );
...
//       TraceLineS( "(tzcmcvto) Processing Menu ", pch );
         ...
         SetMatchingAttributesByName( vTgtDialog, "Menu",
                                      vSrcDialog, "Menu", zSET_ALL );
         CreateEntity( vTgtRemote, "Menubar", zPOS_AFTER );
         SetMatchingAttributesByName( vTgtRemote, "Menubar",
                                      vSrcDialog, "Menu", zSET_ALL );
         ...
         SetAttributeFromInteger( vTgtDialog, "Menu",
                                  "OptCnt", g_nOptCnt );
   ...
   //    TraceLineS( "", "" );
      ...
      if ( CheckExistenceOfEntity( vSrcDialog, "Control" ) > zCURSOR_UNCHANGED )
      {
         lExtentX = 0;
         lExtentY = 0;
         GetIntegerFromAttribute( &lFlags, vSrcDialog,
                                  "Window", "GeneralFlag" );
         ...
         {
            SetAttributeFromInteger( vTgtWndRemote, "Wnd",
                                     "ScrollX", lExtentX );
            SetAttributeFromInteger( vTgtWndRemote, "Wnd",
                                     "ScrollY", lExtentY );
         }
   ...
   //xSetAttributeFromInteger( vTgtRemote, "Wnd", "CtrlCnt", g_nCtrlCnt );
      ...
      {
         SetAttributeFromInteger( vTgtDialog, "Wnd", "REL_Y",
                                  zMAKELONG( g_nCtrlRelTop, g_nCtrlRelBottom ) );
...
//       }
      ...
      {
         SetAttributeFromInteger( vTgtDialog, "Wnd", "REL_X",
                                  zMAKELONG( g_nCtrlRelLeft, g_nCtrlRelRight ) );
   ...
   *pvXRA = vTgtRemote;
   ...
   if ( hLibrary )
   {
                                             ...
                                             "CheckDuplicateTags" )) )
      {
         ...
         {
            zsprintf( szTempString,
                      "(tzcmcvto) Duplicate control tags found in Dialog: %s",
                      pch );
            TraceLineS( szTempString, "" );
...
// nRC = CheckExistenceOfEntity( vSrcDialog, "Option" );
   ...
   {
      g_nOptCnt++;
      GetStringFromAttribute( szTempString, vSrcDialog, "Option", "Tag" );
//    TraceLineS( "(tzcmcvto) Processing Option ", szTempString );
      CreateEntity( vTgtDialog, "Opt", zPOS_AFTER );
      SetMatchingAttributesByName( vTgtDialog, "Opt",
                                   vSrcDialog, "Option", zSET_ALL );
      ...
      SetMatchingAttributesByName( vTgtRemote, "Opt",
                                   vSrcDialog, "Option", zSET_ALL );
   ...
   // SetAttributeFromString( vTgtRemote, "Opt", "Tag", szTempString );
      ...
      // with the subordinate NLS_Text entries.
      SetAttributeFromAttribute( vTgtDialog, "Opt", "Text",
                                 vSrcDialog, "Option", "Text" );
      ...
      if ( g_chDIL_Active )
      {
                                            ...
                                            szTempString, 0 ) != zCURSOR_SET )
         {
            ...
            SetAttributeFromString( vTgtWndDialog, "DIL", "Msg",
                                    szTempString );
         ...
         nDIL_Msg = (zSHORT) GetRelativeEntityNumber( vTgtWndDialog, "DIL",
                                                      0, 0L );
      ...
      if ( ul )
      {
         ...
         nRC = (zSHORT) ul - 1;
         GetStringFromAttribute( szTempString + 1, vSrcDialog,
                                 "Option", "Text" );
         zstrncpy( szTempString, szTempString + 1, nRC );
         ...
         if ( pch[ 0 ] )
         {
            ...
            else
            {
               MessageSend( vSubtask, "CM00807", "Configuration Management",
                            ...
                            zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
               TraceLineS( "(tzcmcvto) Option cannot locate action "
                              "for include ", pch );
            }
         }
      }
      ...
      if ( CheckExistenceOfEntity( vSrcDialog, "OptMap" ) > zCURSOR_UNCHANGED )
      {
         ...
         if ( CheckExistenceOfEntity( vSrcDialog, "OptMapContext" ) == 0 )
         {
            SetAttributeFromAttribute( vTgtDialog, "OptMap", "Context",
                                       vSrcDialog, "OptMapContext",
                                       "Name" );
         }

         SetAttributeFromAttribute( vTgtDialog, "Opt", "ChkOn",
                                    vSrcDialog, "Option",
                                    "CheckMarkValueOn" );
         SetAttributeFromAttribute( vTgtDialog, "Opt", "ChkOff",
                                    vSrcDialog, "Option",
                                    "CheckMarkValueOff" );
      }
...
// GetStringFromAttribute( szTag, vCtrl, "Control", "Tag" );
// TraceLineS( "= Invoked fnCountRelCtrls  for: ", szTag );
   nRC = SetCursorFirstEntity( vCtrl, "Control", 0 );
   while ( nRC >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( &ul, vCtrl, "Control", "Subtype" );
...
#endif
// nRC = CheckExistenceOfEntity( vSrcDialog, "Control" );
   ...
   // }
      ...
      SetMatchingAttributesByName( vTgtDialog, "Ctrl",
                                   vSrcDialog, "Control", zSET_ALL );
      SetMatchingAttributesByName( vTgtRemote, "Ctrl",
                                   vSrcDialog, "Control", zSET_ALL );

      GetIntegerFromAttribute( (zPLONG) &lCtrlType, vSrcDialog,
                               "ControlDef", "Key" );
   ...
   // GetStringFromAttribute( szCtrlTag, vSrcDialog, "Control", "Tag" );
   //xfnRemoveAmpersand( ControlDef.Tag );
   //xSetAttributeFromString( vTgtRemote, "Ctrl", "Tag", ControlDef.Tag );

      SetAttributeFromInteger( vTgtDialog, "Ctrl", "StyleX",
                               ControlDef.ExtendedStyle );
      SetAttributeFromInteger( vTgtRemote, "Ctrl", "StyleX",
                               ControlDef.ExtendedStyle );
      ...
      if ( SetCursorFirstEntityByInteger( vPE, "ControlDef",
                                          "Key", lCtrlType, 0 ) == 0 )
      {
         GetStringFromAttribute( szTempString, vPE,
                                 "ControlDef", "Tag" );
         ...
         SetAttributeFromString( vTgtDialog, "Ctrl",
                                 "ScriptDLL", szTempString );
      }
      else
      {
         GetStringFromAttribute( szTempString, vSrcDialog,
      ...
      if ( nRC >= zCURSOR_SET )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
            ...
            SetAttributeFromAttribute( vTgtWndDialog, "NLS_Text", "LI",
                                       ...
                                       "LanguageIndex" );
            SetAttributeFromAttribute( vTgtWndDialog, "NLS_Text", "Text",
                                       vSrcDialog, "ControlNLS_Text", "Text" );
      ...
      //      (ul & zCONTROL_RELATIVE_POS_Y) )
         {
            ...
            fnCountRelCtrls( vCtrl, &rect, ul,
                             &nRelSizeCntX, &nRelSizeTotalX,
                             &nRelSizeCntY, &nRelSizeTotalY );
         ...
         //    TraceLineS( "", "" );
            ...
            SetAttributeFromInteger( vTgtDialog, "Ctrl", "RELCNT_X",
                                     zMAKELONG( nRelSizeCntX, nRelSizeTotalX ) );
            SetAttributeFromInteger( vTgtDialog, "Ctrl", "RELCNT_Y",
                                     zMAKELONG( nRelSizeCntY, nRelSizeTotalY ) );
...
//       }
         ...
         if ( ul & (zCONTROL_RELATIVE_SIZE_X | zCONTROL_RELATIVE_POS_X) )
         {
            ...
            if ( g_nCtrlRelRight <
                        ControlDef.PSDLG_X + lOffsetX + ControlDef.SZDLG_X )
            {
               g_nCtrlRelRight = (zSHORT)
                       (ControlDef.PSDLG_X + lOffsetX + ControlDef.SZDLG_X);
            }
         }
         ...
         if ( ul & (zCONTROL_RELATIVE_SIZE_Y | zCONTROL_RELATIVE_POS_Y) )
         {
            ...
            if ( g_nCtrlRelBottom <
                        ControlDef.PSDLG_Y + lOffsetY + ControlDef.SZDLG_Y )
            {
               g_nCtrlRelBottom = (zSHORT)
                       (ControlDef.PSDLG_Y + lOffsetY + ControlDef.SZDLG_Y);
            }
         }
...
//       }
      ...
      if ( lFlags & 0x00000001 )
      {
         if ( ControlDef.PSDLG_X + lOffsetX +
                                   ControlDef.SZDLG_X > *plExtentX )
         {
            *plExtentX = ControlDef.PSDLG_X + lOffsetX + ControlDef.SZDLG_X;
         }

         if ( ControlDef.PSDLG_Y + lOffsetY +
                                   ControlDef.SZDLG_Y > *plExtentY )
         {
      ...
      if ( g_chDIL_Active )
      {
         ...
         else
         {
            ...
            {
               GetAddrForAttribute( &pchAttr, vSrcDialog,
                                    "ControlDef", "Tag" );
               if ( zstrcmp( pchAttr, "Radio" ) == 0 ||
                    zstrcmp( pchAttr, "Push" ) == 0 )
               {
                  //Case 3 above, Text.
                  GetStringFromAttribute( szTempString, vSrcDialog,
                                          "Control", "Text" );
         ...
         //x                        szTempString );
            ...
            nRC = SetCursorFirstEntity( vSrcDialog,
                                        "ControlNLS_DIL_Text", "" );
            while ( nRC >= zCURSOR_SET )
            {
               CreateEntity( vTgtWndDialog, "NLS_DIL_Text", zPOS_AFTER );
      ...
      GetStringFromAttribute( szTempString + k, vSrcDialog,
                              "Control", "DD_SourceKey" );
      if ( szTempString[ k ] )
      {
         zSHORT j;
...
//       szTempString[ 0 ] = 'N';            // no drag & drop
   ...
   // nRC = CheckExistenceOfEntity( vSrcDialog, "Event" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         SetMatchingAttributesByName( vTgtDialog, "Event",
                                      vSrcDialog, "Event", zSET_ALL );
         ...
         //    TraceLineS( "", "" );
                                                ...
                                                0 ) == zCURSOR_SET )
            {
               ...
               {
#ifdef DEBUG_ALL
                  GetAddrForAttribute( &pch, vSrcDialog,
                                       "EventAct", "Tag" );
                  GetAddrForAttribute( &pchAttr, vSrcWndDialog,
                                       "Action", "Tag" );
                  zsprintf( szTempString, "Event Tag mismatch: %s <====> %s",
                            pch, pchAttr );
               ...
               //                                    0 ) == zCURSOR_SET )
               // {
               //    GetAddrForAttribute( &pch, vSrcWndDialog,
               //                         "Action", "Tag" );
               //    SetAttributeFromString( g_vTgtDialogOrig, "Act",
               //                            "Tag", pch );
               // }
               //
                  ExcludeEntity( vSrcDialog, "EventAct", zREPOS_NONE );
                  IncludeSubobjectFromSubobject( vSrcDialog, "EventAct",
                                                 vSrcWndDialog, "Action",
                                                 zPOS_AFTER );
               }
            }
...
//          TraceLineS( "(tzcmcvto) Processing EventAct ", pch );
                                               ...
                                               pch, 0 ) == zCURSOR_SET )
            {
               ...
               SetAttributeFromString( vTgtRemote, "Event",
                                       "ActionTag", pch );
               SetAttributeFromAttribute( vTgtRemote, "Event", "Tag",
                                          vSrcDialog, "Event", "Type" );
               GetIntegerFromAttribute( (zPLONG) &ul, g_vTgtDialogOrig,
                                        "Act", "SyncIdx" );
               SetAttributeFromInteger( vTgtRemote, "Event",
                                        "SyncIdx", ul + 1 );

               // Load up a table with all the events for this control.
               GetIntegerFromAttribute( (zPLONG) (g_lpEventAct + uLth),
                                        vTgtDialog, "Event", "Type" );
               uLth += sizeof( zLONG );
               GetIntegerFromAttribute( (zPLONG) (g_lpEventAct + uLth),
                                        vTgtDialog, "EventAct", "SyncIdx" );
            ...
            // GetStringFromAttribute( g_lpEventAct + uLth, vSrcDialog,
            //                         "EventAct", "Tag" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( *pch == 'Y' || *pch == 'A' )
         {
            GetIntegerFromAttribute( &ul, vPE, "CtrlEventDef", "Type" );
...
//       TraceLineS( "(tzcmcvto) Processing EventAct ", pch );
   ...
   // nRC = CheckExistenceOfEntity( vSrcDialog, "CtrlMap" );
      ...
      GetStringFromAttribute( szTempString + 100, vPE,
      ...
      if ( szTempString[ 100 ] )  // there is a conversion operation
      {
         ...
         if ( hLibrary )
         {
            ...
            {
               // ERROR !!!
               zstrcpy( szTempString,
                        "(tzcmcvto) Cannot access Ctrl convert operation: " );
               zstrcat( szTempString, szTempString + 100 );
               MessageSend( vSubtask, "CM00813", "Configuration Management",
                            szTempString + 11,
                            zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
               zstrcat( szTempString, " for token " );
         ...
         {
            // ERROR !!!
            zstrcpy( szTempString + 100, szTempString );
            zstrcpy( szTempString,
...
//    TraceLineS( "(tzcmcvto) Processing Page ", pch );
      ...
      // TraceLineS( "(tzcmcvto) Processing GroupSet ", pch );
         ...
         // TraceLineS( "(tzcmcvto) Processing Group ", pch );
            ...
            GetStringFromAttribute( szAllowPageBreak, vTgtReport, "Group",
                                    "AllowPageBreak" );
            ...
            // TraceLineS( "(tzcmcvto) Processing Control ", pch );
               ...
               if ( CheckExistenceOfEntity( vTgtReport, "CtrlCtrl" ) == 0 )
               {
                  ...
                  while ( nRC >= zCURSOR_SET )
                  {
                     ...
                     {
                        GetAddrForAttribute( &pch, vTgtReport,
                                             "CtrlCtrl", "Tag" );
            ...
            SetAttributeFromInteger( vTgtReport, "Group",
                                     "Extent", lExtent );
            SetAttributeFromInteger( vTgtReport, "Group",
            ...
            while ( bChange )
            {
               ...
               while ( nRC >= zCURSOR_SET )
               {
                  GetIntegerFromAttribute( &lRangeTop, vTgtReport,
                                           "Control", "RangeTop" );
                  GetIntegerFromAttribute( &lRangeBottom, vTgtReport,
                                           "Control", "RangeBottom" );
                  GetIntegerFromAttribute( &lPosY, vTgtReport,
                                           "Control", "PSDLG_Y" );
                  GetIntegerFromAttribute( &lSizeY, vTgtReport,
                  ...
                  while ( nRC >= zCURSOR_SET )
                  {
                     ...
                     if ( lPos == 0 )
                     {
                        ...
                        SetAttributeFromInteger( vTmpReport, "Control",
                                                 "RangeBottom", lSize );
                        if ( lPos == 0 )
                           lPos = -1;
            ...
            while ( nRC >= zCURSOR_SET )
            {
               GetStringFromAttribute( szTag, vTgtReport, "Control", "Tag" );
               GetStringFromAttribute( szPosY, vTgtReport, "Control", "PSDLG_Y" );
               GetStringFromAttribute( szSizeY, vTgtReport, "Control", "SZDLG_Y" );
               GetStringFromAttribute( szTop, vTgtReport, "Control", "RangeTop" );
               GetStringFromAttribute( szBottom, vTgtReport, "Control", "RangeBottom" );
               GetIntegerFromAttribute( &lPosY, vTgtReport, "Control", "PSDLG_Y" );
               GetIntegerFromAttribute( &lSizeY, vTgtReport, "Control", "SZDLG_Y" );
               lSizeY += lPosY;
               lSizeY--;

               zsprintf( szMsg, "Top %s; Bot %s; Pos %s; Siz %s; End %ld; for %s",
            ...
            while ( nRC >= zCURSOR_SET )
            {
               GetIntegerFromAttribute( &lRangeTop, vTmpReport,
                                        "Control", "RangeTop" );
               GetIntegerFromAttribute( &lRangeBottom, vTmpReport,
                                        "Control", "RangeBottom" );
               if ( lRangeTop == -1 )
               {
                  lRangeTop = 0;
...
//#define DEBUG_RANGE
   ...
   {
      // ERROR out of here ...
      TraceLineS( "(tzcmcvto) Cannot activate executable XSLT - ",
                  "TZRPSRCO" );
      MessageSend( vSubtask, "CM00802", "Configuration Management",
...
//    TraceLineS( "(tzcmcvto) Processing Panel ", pch );
      ...
      // TraceLineS( "(tzcmcvto) Processing Control ", pch );
         ...
         GetIntegerFromAttribute( &lPos, vTgtXSLT,
                                  "Control", "PSDLG_X" );
         GetIntegerFromAttribute( &lPosY, vTgtXSLT,
                                  "Control", "PSDLG_Y" );
         GetIntegerFromAttribute( &lSizeY, vTgtXSLT,
                                  "Control", "SZDLG_Y" );
         if ( lSizeY + lPosY > lExtent )
            lExtent = lSizeY + lPosY;
         ...
         if ( CheckExistenceOfEntity( vTgtXSLT, "CtrlCtrl" ) == 0 )
         {
            ...
            while ( nRC >= zCURSOR_SET )
            {
               ...
               SetViewToSubobject( vSubCtrl, "CtrlCtrl" );
               GetIntegerFromAttribute( &lOffsetX, vTgtXSLT,
                                        "CtrlCtrl", "PSDLG_X" );
               GetIntegerFromAttribute( &lOffsetY, vTgtXSLT,
                                        "CtrlCtrl", "PSDLG_Y" );
               SetAttributeFromInteger( vTgtXSLT, "CtrlCtrl",
                                        "PSDLG_X", lPos + lOffsetX );
               SetAttributeFromInteger( vTgtXSLT, "CtrlCtrl",
                                        "PSDLG_Y", lPosY + lOffsetY );
               if ( MoveSubobject( vTgtXSLT, "Control",
                                   vSubCtrl, "Control",
                                   zPOS_AFTER, zREPOS_NEXT ) == zCALL_ERROR )
               {
                  GetAddrForAttribute( &pch, vTgtXSLT,
                                       "CtrlCtrl", "Tag" );
                  MessageBox( 0, "MoveSubobject failed", pch, MB_OK );
                  break;
               }
      ...
      SetAttributeFromInteger( vTgtXSLT, "Group",
                               "Extent", lExtent );
      SetAttributeFromInteger( vTgtXSLT, "Group",
                               "MinimumExtent", lExtent );
      ...
      while ( bChange )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
            GetIntegerFromAttribute( &lRangeTop, vTgtXSLT,
                                     "Control", "RangeTop" );
            GetIntegerFromAttribute( &lRangeBottom, vTgtXSLT,
                                     "Control", "RangeBottom" );
            GetIntegerFromAttribute( &lPosY, vTgtXSLT,
                                     "Control", "PSDLG_Y" );
            GetIntegerFromAttribute( &lSizeY, vTgtXSLT,
            ...
            if ( lRangeTop == 0 )
            {
               ...
               SetAttributeFromInteger( vTgtXSLT, "Control",
                                        "RangeTop", lRangeTop );
            }
            ...
            while ( nRC >= zCURSOR_SET )
            {
               GetIntegerFromAttribute( &lPos, vTmpXSLT,
                                        "Control", "RangeTop" );
               GetIntegerFromAttribute( &lSize, vTmpXSLT,
                                        "Control", "RangeBottom" );
               if ( lPos == 0 )
               {
                  GetIntegerFromAttribute( &lPos, vTmpXSLT,
                  ...
                  SetAttributeFromInteger( vTmpXSLT, "Control",
                                           "RangeBottom", lSize );
                  if ( lPos == 0 )
                     lPos = -1;
      ...
      while ( nRC >= zCURSOR_SET )
      {
         GetStringFromAttribute( szTag, vTgtXSLT, "Control", "Tag" );
         GetStringFromAttribute( szPosY, vTgtXSLT, "Control", "PSDLG_Y" );
         GetStringFromAttribute( szSizeY, vTgtXSLT, "Control", "SZDLG_Y" );
         GetStringFromAttribute( szTop, vTgtXSLT, "Control", "RangeTop" );
         GetStringFromAttribute( szBottom, vTgtXSLT, "Control", "RangeBottom" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         GetIntegerFromAttribute( &lRangeTop, vTmpXSLT,
                                  "Control", "RangeTop" );
         GetIntegerFromAttribute( &lRangeBottom, vTmpXSLT,
                                  "Control", "RangeBottom" );
         if ( lRangeTop == -1 )
         {
            lRangeTop = 0;
      ...
      // Sort target controls by position.
      OrderEntityForView( vTgtXSLT, "Control",
                          "RangeBottom A PSDLG_X A PSDLG_Y A" );
