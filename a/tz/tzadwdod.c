...
oTZWDLGSO_AutoDsgnIntoExistWnd( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szObjectName, vAutoDesWk,
   ...
   GetStringFromAttribute( szWindowName, vNewDialog, "Window", "Tag" );
   fnPainterCall( zMSG_DELETECURRENTPAINTERWINDOW, vWindow, 0, 0 );
   lRC = ofnTZWDLGSO_AutoDesignWindow( vSubtask, vUserSpec,
                                       vNewDialog, vAutoDesWk );
   fnPainterCall( zMSG_UPDATEWINDOWBYNAME, vWindow, 0, szWindowName );
   DropView( vNewDialog );
...
oTZWDLGSO_AutoDsgnWndWOStyle( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( cFullSize, vUserSpec, "WndDesign", "FullWnd" );

   lRC = SetCursorFirstEntityByString( vNewDialog, "Control",
                                       "Tag", "DIL1", 0 );
...
oTZWDLGSO_AutoDsgnSubordinates( zVIEW vSubtask )
{
   ...
   nRC = ofnTZWDLGSO_AutoDsgnWndAndSubs( vSubtask, vUserSpec,
                                          vNewDialog, vAutoDesWk );
...
oTZWDLGSO_AutoDesignDialog( zVIEW vSubtask )
{
   ...
   fnPainterCall( zMSG_DELETEALLPAINTERWINDOWS, vWindow, 0, 0 );
   ...
   SetAttributeFromString( vAutoDesWk, "AutoDesignWork",
                           "SaveRoutineFlagForRootEntity", "S" );
   SetAttributeFromString( vAutoDesWk, "AutoDesignWork",
                           "DeleteRoutineFlagForRootEntity", "D" );
   SetAttributeFromString( vAutoDesWk, "AutoDesignWork",
                           "CancelRoutineFlagForRootEntity", "C" );
   SetAttributeFromString( vAutoDesWk, "AutoDesignWork",
                           "AutoDesignNewWnd", "Y" );
   SetAttributeFromInteger( vAutoDesWk, "AutoDesignWork",
                            "AutoDesignSubords", 1 ); /* TRUE */
   ...
   SetAttributeFromInteger( vAutoDesWk, "AutoDesignWork",
                           "FirstWindow", 1 ); /* TRUE */
   GetStringFromAttribute( cDialogStyle, vAutoDesWk,
                           "AutoDesignWork", "WDOD_Style" );
   ...
   GetStringFromAttribute( szFileSpec, vT1, "LPLR", "PgmSrcDir" );
   SysConvertEnvironmentString( szFileName, szFileSpec );

   zstrcat( szFileName, "\\" );
   zstrcat( szFileName, szDialogName );
   ...
   if ( pfnUserTermination != 0 )
   {
      ...
      // If the Termination routine added VML code, then concatenate it to
      // the existing code.
      if ( szTemplate[ 0 ] != 0 )
      {
         ...
         *lpMemory = 0;  // Initialize to null.
         ...
         // Concatenate the new VML, add the EOF zCHAR and write the
         // string back.
         zstrcat( lpMemory, szTemplate );
   ...
   if ( cDialogStyle[ 0 ] == 'G' )
   {
      GetStringFromAttribute( szWindowName, vUserSpec, "WndDesign", "WndName" );
      if ( szWindowName[ 0 ] == 0 )
         GetStringFromAttribute( szWindowName, vUserSpec, "LOD_Entity", "Name" );
   }
   else
   {
   ...
   if ( hSourceFlag < 0 )
   {
      GetStringFromAttribute( szFileName, vWindow, "SourceFile", "Name" );
...
oTZWDLGSO_GetObjectEntities( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szObjectName, vVOR, "LOD", "Name" );
...
oTZWDLGSO_GetStyleEntities( zVIEW  vSubtask )
{
   ...
   nRC = GetStringFromAttribute( szStyleName, vWorkObj,
                                        "Palette", "UIS_Name" );
...
oTZWDLGSO_AutoPaintWindow( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileName, vAutoDesWk,
                           "AutoDesignWork", "ObjectName" );
...
// MessageBox( 0, "Text1", "Caption1", MB_OK );
   ...
   fnPainterCall( zMSG_REPAINTZEIDONWINDOW, vWindow, 0, 0 );
...
//   zVIEW    vVOR;
   ...
   GetStringFromAttribute( szObjectName, vWindow, "LOD", "Name" );
...
oTZWDLGSO_GetWindowActions( zVIEW vSubtask )
{
   ...
   GetViewByName( &vUserSpec, "User_Spec", vSubtask, zLEVEL_ANY );
   GetViewByName( &vAutoDesWk, "TZADWWKO", vSubtask, zLEVEL_ANY );
   GetStringFromAttribute( szEntityName, vAutoDesWk,
                            "AutoDesignWork", "ObjectEntityName" );
...
oTZWDLGSO_LoadAutoDesignObject( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szVOR, vAutoDesWk, "AutoDesignWork", "ObjectName" );
   ...
   GetStringFromAttribute( szLOD, vVOR, "LOD", "Name" );
...
zwTZADWIAW_GrayMenuItems( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( cActionType, vUserSpec,
                           "WndDesign", "ActionType" );

   if ( cActionType[ 0 ] == 'O' )
   {
      SetCtrlState( vSubtask, "MenuTitle",
                    zCONTROL_STATUS_ENABLED, TRUE );
      SetCtrlState( vSubtask, "MenuTitleText",
                    zCONTROL_STATUS_ENABLED, TRUE );
   }
   else
   {
