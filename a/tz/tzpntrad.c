...
// TraceLineS( "In OpenDialogFile", "============== " );
   ...
   fnPainterCall( zMSG_DELETEALLPAINTERWINDOWS, vSubtask, 0, 0 );
...
//    TraceLineX( "TZPNTRAD Prebuild SysPainterHotkey: ", vKZWDLGXO );
   ...
   {
      GetStringFromAttribute( szLPLR_Name, vProfileXFER, "WD", "StartupLPLR_Name" );
...
zwTZPNTRAD_PostBuild( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szDialogName, vKZWDLGXO, "Dlg", "Tag" );
      GetStringFromAttribute( szWindowName, vKZWDLGXO, "Wnd", "Tag" );
                                         ...
                                         szDialogName, "" ) == zCURSOR_SET )
      {
         ...
         {
            fnPainterCall( zMSG_UPDATEWINDOWBYNAME, vSubtask, 0, szWindowName );
   ...
   fnPainterCall( zMSG_INITPAINTER, vSubtask, 0, (zPCHAR) 0 );
   ...
   if ( GetViewByName( &vProfileXFER, "ProfileXFER", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      fnPainterCall( zMSG_SETSCROLLBARS, vSubtask, 0, (zPCHAR) nState );
      nState = (lFlags & 0x00000002) ? 1 : 0;
      SetOptionState( vSubtask, "Grid", zOPTION_STATUS_CHECKED, nState );
      fnPainterCall( zMSG_SETGRID, vSubtask, 0, (zPCHAR) nState );
...
ExitDialog( zVIEW vSubtask )
{
   ...
   // We are terminating.
   fnPainterCall( zMSG_TERMPAINTER, vSubtask, 0, (zPCHAR) 1 );
...
//    TraceLineX( "TZPNTRAD ExitDialog SysPainterHotkey: ", vKZWDLGXO );
   ...
   if ( nRC )
   {
      ...
      else
      {
         ...
         {
            // Cancel termination.
            fnPainterCall( zMSG_TERMPAINTER, vSubtask, 0, 0 );
            return( 0 );
         }
   ...
   fnPainterCall( zMSG_SAVE_PLACEMENT, vSubtask, 0, (zPCHAR) vProfileXFER );
   ...
   if ( vKZWDLGXO == 0 || vIdlePainter )
     fnPainterCall( zMSG_DELETETOOLBAR, vSubtask, 0, 0 );
   else
...
AskForSave( zVIEW vSubtask )
{
   ...
   if ( MiGetUpdateForView( vDialog ) && ObjectInstanceUpdatedFromFile( vDialog ) )
   {
      ...
      if ( !ComponentIsCheckedOut( vSubtask, vDialog, zSOURCE_DIALOG_META ) )
      {
         ...
         zstrcpy( szMessageText, "Dialog is not checked out, but Dialog '" );
         zstrcat( szMessageText, szDialogName );
         zstrcat( szMessageText, "' has been modified. \n\nWould you like to " );
         zstrcat( szMessageText, "save it with differend name?" );
...
{
   fnPainterCall( zMSG_DELETECURRENTPAINTERWINDOW, vSubtask, 0, 0 );
   return( 0 );
}
...
fnRecurseChangeDlgTags( zVIEW vDialog )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szTag, vDialog, "Control", "Tag" );
      if ( szTag[ 0 ] == 0   ||
          (szTag[ 0 ] == 'T' &&
           szTag[ 1 ] == 'e' &&
           szTag[ 2 ] == 'x' &&
           szTag[ 3 ] == 't') )
      {
         GetStringFromAttribute( szText, vDialog, "Control", "Text" );
         if ( szText[ 0 ] )
         {
            szText[ 32 ] = 0;
            zstrcpy( szTag, szText );
         }
         else
         {
            szText[ 0 ] = 0;
            ...
            {
               GetStringFromAttribute( szText, vDialog,
         ...
         if ( szText[ 0 ] )
         {
            ...
            GetStringFromAttribute( szTag, vDialog, "ControlDef", "Tag" );
            nCnt = fnGetTagPrefix( szTag );
            if ( nCnt )
               szText[ 32 - nCnt ] = 0;

            zstrcpy( szTag + nCnt, szText );
                             ...
                             zVIEW vNewDialogL )
{
   ...
   if ( GetViewByName( &vWindowList, "TZWND_LIST_Subtask", vSubtask, zLEVEL_TASK ) > 0 )
   {
           ...
           SetCursorNextEntity( vNewDialogL, "Window", 0 ) != zCURSOR_SET )
      {
         ...
         fnPainterCall( zMSG_CREATEZEIDONWINDOW, vSubtask, 0, 0 );
      }
      else
      {
   ...
   fnPainterCall( zMSG_REFRESHWINDOWLIST, vSubtask, 0, 0 );
#endif
...
zwTZPNTRAD_GenerateXRA( zVIEW vSubtask )
{
   ...
   if ( szRemotePath[ 0 ] == 0 )
      GetStringFromAttribute( szRemotePath, vTaskLPLR, "LPLR", "ExecDir" );
         ...
         nRC = SetCursorNextEntity( vMetaList, "W_MetaDef", "" ) )
   {
      ...
      zstrcpy( szMsg, "Generate XRA for Dialog: '" );
      ...
      {
         zstrcpy( szMsg, "Could not load Dialog '" );
         zstrcat( szMsg, szDialogName );
         zstrcat( szMsg, "'." );
      ...
      {
         zstrcpy( szFileSpec, szRemotePath );
         zstrcat( szFileSpec, szDialogName );
         zstrcat( szFileSpec, ".XRA" );
         ...
         if ( vXRA )
         {
            ...
            {
               zstrcpy( szMsg, "Unable to save XRA '" );
...
UpdateWindowFromList( zVIEW vSubtask )
{
   ...
   // for the window
   fnPainterCall( zMSG_GETACTIVEWINDOWNAME, vSubtask, 0, szTag );
   if ( szTag[ 0 ] == 0 )
      return( -1 );

   nRC = (zSHORT) fnPainterCall( zMSG_UPDATEWINDOWBYNAME, vSubtask, 0, szTag );
   return( 0 );
}
...
NewWindow( zVIEW vSubtask )
{
   ...
   if ( (nCursor = CheckExistenceOfEntity( vDialog, "Window" )) !=
                                                            zCURSOR_NULL )
   {
      zstrcpy( szTag, "Window" );
      for ( nIdx = 1; nIdx < 1000; nIdx++ )
      {
         zltoa( (zLONG) nIdx, szTag + 6 );
   ...
   {
      GetStringFromAttribute( szTag, vDialog, "Dialog", "Tag" );
   }
...
DlgCloneWindow( zVIEW vSubtask )
{
   ...
   return( (zSHORT) fnPainterCall( zMSG_UPDATEWINDOWBYNAME, vSubtask, 0, szTag ) );
}
...
CleanupActions( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vDialog, "TZOPENDLG", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      zstrcpy( szTempString, "Action unreferenced Dialog.Window.Action: " );
      nLth1 = zstrlen( szTempString );
      GetStringFromAttribute( szTempString + nLth1,
                              vDialog, "Dialog", "Tag" );
      nLth1 += zstrlen( szTempString + nLth1 );
      szTempString[ nLth1++ ] = '.';
...
CleanupOperations( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vDialog, "TZOPENDLG", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      zstrcpy( szTempString, "Operation unreferenced in Dialog - " );
      nLth = zstrlen( szTempString );
      CreateViewFromViewForTask( &vDialog, vDialog, 0 );
      nRC = SetCursorFirstEntity( vDialog, "Operation", "Dialog" );
      while ( nRC >= zCURSOR_SET )
      {
         if ( SetCursorFirstEntityByEntityCsr( vDialog, "ActOper",
                                                vDialog, "Operation",
                                                "Dialog" ) >= zCURSOR_SET )
         {
         ...
         GetStringFromAttribute( szTempString + nLth,
                                 vDialog, "Window", "Tag" );
         nLth2 = zstrlen( szTempString + nLth );
         zstrcpy( szTempString + nLth + nLth2, " - " );
         nLth2 += 3;
         GetStringFromAttribute( szTempString + nLth + nLth2,
                                 vDialog, "Operation", "Name" );
         zstrcat( szTempString + nLth + nLth2, "\n Delete this operation?" );
                            ...
                            zPCHAR  szDialogName )
{
   ...
   GetStringFromAttribute( szDialogName, vPainter, "Palette", "NewFile" );
   if ( szDialogName[ 0 ] == 0 )
   {
...
DeleteWindow( zVIEW vSubtask )
{
   ...
   zstrcpy( szMessage, "OK to delete Window '" );
   zstrcat( szMessage, szTag );
   zstrcat( szMessage, "'?" );
   ...
   if ( fnPainterCall( zMSG_DELETECURRENTPAINTERWINDOW,
                       vSubtask, 0, 0 ) == 0 )
   {
...
EnablePainterWindows( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZPESRCO, "TZPESRCO", vSubtask, zLEVEL_TASK ) <= 0 )
   {
      ...
      fnPainterCall( zMSG_REPLACETOOLBARBUTTONS, vSubtask, 0, 0 );
...
//    SetWindowState( vWindowList, zWINDOW_STATUS_ENABLED, 1 );
   ...
   nRC = (zSHORT) fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, "Y" );
   return( nRC );
}
...
DisablePainterWindows( zVIEW vSubtask )
{
   return( (zSHORT) fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, 0 ) );
}
...
ShowTabbing( zVIEW vSubtask )
{
   return( (zSHORT) fnPainterCall( zMSG_SHOWTABBING, vSubtask, 0, 0 ) );
}
...
RemoveTabbing( zVIEW vSubtask )
{
   ...
   {
      return( (zSHORT) fnPainterCall( zMSG_REMOVETABBING, vSubtask, 0, 0 ) );
   }
...
SortWindows( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSelected, vDialog, "Window", "Tag" );
...
SortActionsForWindow( zVIEW vSubtask )
{
   ...
   GetViewByName( &vDialog, "TZWINDOW", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szSelected, vDialog, "Action", "Tag" );
                         ...
                         zSHORT nLevel, zSHORT nID )
{
   ...
   if ( SetCursorFirstEntity( vDialog, "Option", 0 ) == zCURSOR_SET )
   {
      do
      {
         ...
         GetStringFromAttribute( szText, vDialog, "Option", "Text" );
         zstrcat( szDisplayText, szText );
...
LIST_MENU_Prebuild( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTempString, vDialog, "Window", "Tag");
...
LIST_MENU_OK( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vDialog, "TZWINDOW", vSubtask, zLEVEL_TASK ) > 0 )
   {
            ...
            nRC = SetCursorNextEntity( vDialog, "Menu", 0 ) )
      {
         ...
         {
            zstrcpy( szMsg, "Menu item, " );
            GetStringFromAttribute( szMenuName, vDialog, "Menu", "Tag" );
            zstrcat( szMsg, szMenuName );
            zstrcat( szMsg, ", must have at least one Option" );
                       ...
                       zVIEW    vSubtask )
{
   ...
   if ( lTempInteger_1 == zCURSOR_SET )
   {
      ...
      {
         GetStringFromAttribute( szTempString_0, OrigWO, "OptAct", "Tag" );
...
UPD_MENU_OptionDelete( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szMenuItem, vOptions, "Option", "Text" );
   zstrcpy( szMsg, "OK to delete Menu Item '" );
   zstrcat( szMsg, szMenuItem );
   zstrcat( szMsg, "'" );
   // check SubItems
   if ( CheckExistenceOfEntity( vOptions, "OptOpt" ) >= zCURSOR_SET )
      zstrcat( szMsg, " with all Subitems?" );
   else
      zstrcat( szMsg, "?" );
...
UPD_MENU_OK( zVIEW vSubtask )
{
   ...
   CreateViewFromViewForTask( &vDialogCopy, vDialog, 0 );
   GetStringFromAttribute( szTag, vDialog, "Menu", "Tag" );
   ...
   fnPainterCall( zMSG_REPAINTACTIONBAR, vSubtask, 0, 0 );
...
// if ( CheckExistenceOfEntity( vDialog, "Control" ) != 0 )
      fnPainterCall( zMSG_ACTIVATEPAINTERWINDOW, vSubtask, 0, 0 );
// fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, 0 );
...
SwitchLPLR( zVIEW vSubtask )
{
   ...
   // Send in flag to force reload of task view for LPLR.
   fnPainterCall( zMSG_DELETEALLPAINTERWINDOWS, vSubtask, 0, (zPCHAR) 1 );
...
SystemMinimize( zVIEW vSubtask )
{
   ...
   fnPainterCall( zMSG_SYSTEMMINIMIZE, vSubtask, 0, 0 );
   if ( GetViewByName( &vDialog, "TZOPENDLG", vSubtask, zLEVEL_TASK ) < 1 )
   {
            ...
            if ( GetViewByName( &vDialog, "TZOPENDLG", vSubtask, zLEVEL_TASK ) > 0 )
            {
                                ...
                                vKZWDLGXO, "Dlg", "Tag" ) == 0 )
               {
                  ...
                  DropNameForView( vSubtask, "__SysIdlePainter",
                                   0, zLEVEL_SYSTEM );
                  GetStringFromAttribute( szWorkString,
                                          vKZWDLGXO, "Wnd", "Tag" );
                  GetViewByName( &vWindowList, "TZWINDOWL", vSubtask, zLEVEL_TASK );
                  if ( SetCursorFirstEntityByString( vWindowList,
                       "Window", "Tag", szWorkString, 0 ) >= zCURSOR_SET )
                  {
                     fnPainterCall( zMSG_UPDATEWINDOWBYNAME, vSubtask,
                                    0, szWorkString );
//                   UpdateWindowFromList( vSubtask );
                  }
   ...
   if ( fnPainterCall( zMSG_PAINTERINITIALIZED, vSubtask, 0, 0 ) == 0 )
   {
//    TraceLineS( "(tzpntrad) - SystemActivate - ", "Early return" );
   ...
   fnPainterCall( zMSG_PUTTOOLBOXONTOP, vSubtask, 0, 0 );
#endif
...
fnWndUpdate( zVIEW vSubtask, zBOOL bStayOnWindow )
{
   ...
   GetStringFromAttribute( szNewTag, vDialog, "Window", "Tag" );
   if ( szNewTag[ 0 ] == 0 ||
   ...
   fnPainterCall( zMSG_UPDATEACTIVEWINDOWNAME, vSubtask, 0, szNewTag );
...
WND_UPD_OperationEdit( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( sTag, vWork, "EventAct", "Tag" );
      SetCursorFirstEntityByString( vDialogW, "Action", "Tag", sTag, "" );

      if ( CheckExistenceOfEntity( vDialogW, "ActOper" ) >= zCURSOR_SET )
      {
         GetStringFromAttribute( sTag, vDialogW, "ActOper", "Name" );
         SetCursorFirstEntityByString( vDialogW, "Operation", "Name",
                                       sTag, "Dialog" );
...
OPT_UPD_OK( zVIEW vSubtask )
{
   ...
   GetViewByName( &vDialogO, "TZWINOPT", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szTag, vDialogO, "Option", "NLS_Text" );

   // Make sure that the option tag is unique
   GetStringFromAttribute( szTag, vDialogO, "Option", "Tag" );
...
OPT_UPD_OperationEdit( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( sTag, vDialogC, "EventAct", "Tag" );
      SetCursorFirstEntityByString( vDialogW, "Action", "Tag", sTag, "" );
...
zwTZPNTRAD_Control_OK( zVIEW vSubtask )
{
   ...
   fnPainterCall( zMSG_REPLACETOOLBARBUTTONS, vTZPNMWIN, 0, 0 );
...
zwTZPNTRAD_InitModalSubwindow( zVIEW vSubtask )
{
   ...
   if ( vWindow )
   {
      ...
      fnPainterCall( zMSG_UPDATEZEIDONWINDOW, vSubtask, 0, 0 );
...
SetScrollbars( zVIEW vSubtask )
{
   ...
   fnPainterCall( zMSG_SETSCROLLBARS, vSubtask, 0, (zPCHAR) lState );
   return( 0 );
}
...
SetGrid( zVIEW vSubtask )
{
   ...
   fnPainterCall( zMSG_SETGRID, vSubtask, 0, (zPCHAR) lState );
   return( 0 );
}
...
InitMenu( zVIEW vSubtask )
{
   ...
   if ( fnPainterCall( zMSG_UNDO, vSubtask, 0, szLabel ) == 0 )
   {
   ...
   if ( fnPainterCall( zMSG_REDO, vSubtask, 0, szLabel ) == 0 )
   {
      SetOptionState( vSubtask, "Redo", zOPTION_STATUS_TEXT,
                      (zULONG) szLabel );
   ...
   if ( fnPainterCall( zMSG_GETTABBING, vSubtask, 0, 0 ) == 0 )
   {
...
fnRecurseGroupCtrls( zVIEW vDialog )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      if ( lKey == zGROUPBOX_CONTROL )
      {
         ...
         GetStructFromEntityAttrs( (zPCHAR) GroupBoxDef.Tag,
                                   vDialog, "Control" );
#ifdef DEBUG_LODSTRUCT
         if ( GroupBoxDef.CheckByte != 0xFE )
            MessageBox( 0, "GroupBoxDef Structure Length Error",
         ...
         while ( nRC >= zCURSOR_SET )
         {
            ...
            GetStructFromEntityAttrs( (zPCHAR) ControlDef.Tag,
                                      vCtrlDialog, "Control" );
#ifdef DEBUG_LODSTRUCT
            if ( ControlDef.CheckByte != 0xFE )
               MessageBox( 0, "ControlDef Structure Length Error",
                           "GetStructFromEntityAttrs", MB_OK );
#endif

            if ( GroupBoxDef.PSDLG_X < ControlDef.PSDLG_X &&
                 GroupBoxDef.PSDLG_Y < ControlDef.PSDLG_Y &&
                 GroupBoxDef.PSDLG_X + GroupBoxDef.SZDLG_X >
                   ControlDef.PSDLG_X + ControlDef.SZDLG_X &&
                 GroupBoxDef.PSDLG_Y + GroupBoxDef.SZDLG_Y >
                   ControlDef.PSDLG_Y + ControlDef.SZDLG_Y )
            {
               ControlDef.PSDLG_X -= GroupBoxDef.PSDLG_X;
               ControlDef.PSDLG_Y -= GroupBoxDef.PSDLG_Y;
               SetAttributeFromInteger( vCtrlDialog, "Control",
                                        "PSDLG_X", ControlDef.PSDLG_X );
               SetAttributeFromInteger( vCtrlDialog, "Control",
                                        "PSDLG_Y", ControlDef.PSDLG_Y );
               nRC = MoveSubobject( vDialog, "CtrlCtrl",
                                    vCtrlDialog, "Control",
                                    zPOS_LAST, zREPOS_NEXT );
            }
            else
                         ...
                         zLONG  lSubtype )
{
   ...
   if ( fnPainterCall( zMSG_ISREPORT, vSubtask, 0, 0 ) == 0 )
   {
...
fnShowCtrlList( zVIEW vSubtask )
{
        ...
        zstrcmp( szEntityName, "Control" ) == 0 )
   {
      ...
      fnPainterCall( zMSG_BUILDTZPNEVWO, vSubtask, 0, 0 );
...
CtrlListUpdateAction( zVIEW vSubtask )
{
   ...
   if ( (CheckExistenceOfEntity( vWork, "EventAct" ) == zCURSOR_SET) &&
        (CompareAttributeToString( vWork, "EventAct", "Tag", "" ) != 0) )
   {
      GetStringFromAttribute( szTag, vWork, "EventAct", "Tag" );
...
ChgCtrlPostbuild( zVIEW vSubtask )
{
                                ...
                                0, 0, 0 ) >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szTag, vPE, "ControlDef", "Tag" );
      GetStringFromAttribute( szAcceptsAllParents, vPE,
                              "ControlDef", "AcceptsAllParents" );
      GetStringFromAttribute( szAcceptsAllChildren, vPE,
                              "ControlDef", "AcceptsAllChildren" );
      GetStringFromAttribute( szRequiresParent, vPE,
      ...
      while ( nRC == zCURSOR_SET )
      {
         ...
         if ( lCurrKey != lKey )
         {
            GetStringFromAttribute( szCurrTag, vPE, "ControlDef", "Tag" );
            GetStringFromAttribute( szCurrAcceptsAllParents, vPE,
                                    "ControlDef", "AcceptsAllParents" );
            GetStringFromAttribute( szCurrAcceptsAllChildren, vPE,
                                    "ControlDef", "AcceptsAllChildren" );
            GetStringFromAttribute( szCurrRequiresParent, vPE,
                                    "ControlDef", "RequiresParent" );
...
ChgCtrlChangeType( zVIEW vSubtask )
{
                         ...
                         0, 0, 0 ) >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szNewTag, vPE, "ControlDef", "Tag" );
      SetSelectSetForView( vPE, nPrevSelSet );
...
// TraceLineS( "CtrlListDetail", "" );
   ...
   fnPainterCall( zMSG_SETCURRCTRL, vSubtask, 0, pchTag );
   if ( GetViewByName( &vPE, "TZPESRCO", vSubtask, zLEVEL_TASK ) <= 0 )
   {
...
CtrlListDeleteCtrl( zVIEW vSubtask )
{
   ...
   if ( vControl )
   {
      GetStringFromAttribute( szCtrlTag, vControl, "Control", "Tag" );
      zstrcpy( szMsg, "OK to delete Control '" );
      zstrcat( szMsg, szCtrlTag );
      zstrcat( szMsg, "'?" );
      if ( MessagePrompt( vSubtask, "PE002",
                          "Dialog Maintenance",
...
Undo( zVIEW vSubtask )
{
   fnPainterCall( zMSG_UNDO, vSubtask, 0, 0 );
   return( 0 );
}
...
Redo( zVIEW vSubtask )
{
   fnPainterCall( zMSG_REDO, vSubtask, 0, 0 );
   return( 0 );
}
...
{
   fnPainterCall( zMSG_CLIPBOARD_COPY, vSubtask, 0, 0 );
   return( 0 );
}
...
{
   fnPainterCall( zMSG_CLIPBOARD_PASTE, vSubtask, 0, 0 );
   return( 0 );
}
...
{
   fnPainterCall( zMSG_CLIPBOARD_COPY_MENU, vSubtask, 0, 0 );
   return( 0 );
}
...
{
   fnPainterCall( zMSG_CLIPBOARD_PASTE_MENU, vSubtask, 0, 0 );
   return( 0 );
}
                 ...
                 zVIEW     vSubtask )
{
                                      ...
                                      pchViewName, 0 ) < 0 )
   {
      ...
      else
      {
         ...
         if ( nRC < 0 )
         {
            zstrcpy( szMsg, "Window has mapping with no VOR: " );
            zstrcat( szMsg, pchViewName );
         ...
         GetStringFromAttribute( szName, vSrc, "LOD", "Name" );
               ...
               zVIEW     vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTag, vSrc, "ActMapView", "Name" );
      ...
      RetrieveViewForMetaList( vSubtask, &vLOD_List, zREFER_LOD_META );
      GetStringFromAttribute( szTag, vTgt, "LOD", "Name" );
      nRC = SetCursorFirstEntityByString( vLOD_List, "W_MetaDef",
                                          "Name", szTag, 0 );
      ...
      {
         GetStringFromAttribute( szTag, vSrc, "ActMapLOD_Entity", "Name" );
               ...
               zVIEW     vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vSrc, "Action", "Tag" );
   ...
   {
      GetStringFromAttribute( szTag, vSrc, "ActOper", "Name" );
      ...
      if ( nRC < zCURSOR_SET )
      {
         ...
         if ( nRC < zCURSOR_SET )
         {
            ...
            if ( nRC >= zCURSOR_SET )
            {
               ...
               if ( lLth >= 8 )
               {
                  zstrcpy( szSourceName, szSourceName );
               }

               zstrcat( szSourceName, szLanguageType );
            }
                        ...
                        zVIEW  vLOD )
{
   ...
   GetStringFromAttribute( szTag, vSrcC, "CtrlMapER_Attribute", "Name" );
   ...
   if ( nRC >= 0 )
   {
      ...
      if ( CheckExistenceOfEntity( vSrcC, "CtrlMapContext" ) >= 0 )
      {
         ...
         GetStringFromAttribute( szTag, vSrcLPLR, "W_MetaDef", "Name" );
         ...
         if ( nRC >= 0 )
         {
            ...
            GetStringFromAttribute( szTag, vSrcC, "CtrlMapContext", "Name" );
            ...
            else
            {
               ...
               zstrcpy( szMsg, "Context doesn't exist within Domain: " );
               zstrcat( szMsg, szTag );
               zstrcat( szMsg, ", " );
         ...
         else
         {
            GetVariableFromAttribute( szTag, 0, 'S', 33, vSrcLPLR,
                                      "W_MetaDef", "Name", "", 0 );
            zstrcpy( szMsg, "Domain doesn't exist: " );
            zstrcat( szMsg, szTag );
   ...
   else
   {
      ...
      zstrcpy( szMsg, "ER_Attribute doesn't exist: " );
      zstrcat( szMsg, szTag );
                ...
                zVIEW     vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTag, vSrcC, "CtrlMapView", "Name" );
      if ( fnPositionOnVOR( vTgt, vSrc, vSrcLPLR, szTag, vSubtask ) >= 0 )
      {
                ...
                zVIEW     vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vSrcC, "Control", "Tag" );
   zstrcpy( szMsg, szTag );  // hold onto original tag
   ...
   else
   {
      ...
      else
      {
         ...
         zstrcpy( szMsg, "ControlDef doesn't exist: " );
         zstrcat( szMsg, szTag );
   ...
   if ( bReport == FALSE )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         GetStringFromAttribute( szTag, vSrcC,
            "CtrlColorDef", "Tag" );
         ...
         if ( nRC >= 0 )
         {
            ...
            GetStringFromAttribute( szTag, vSrcC, "CtrlColor", "Tag" );
         ...
         else
         {
            GetVariableFromAttribute( szTag, 0, 'S', 33, vSrcC,
                                      "CtrlColorDef", "Tag", "", 0 );
            zstrcpy( szMsg, "Color Definition doesn't exist: " );
            zstrcat( szMsg, szTag );
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         GetStringFromAttribute( szTag, vSrcC, "CtrlFontDef", "Tag" );
         ...
         if ( nRC >= 0 )
         {
            ...
            GetStringFromAttribute( szTag, vSrcC, "CtrlFont", "Tag" );
         ...
         else
         {
            ...
            zstrcpy( szMsg, "Font Definition doesn't exist: " );
            zstrcat( szMsg, szTag );
   ...
   if ( bReport == FALSE )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         if ( CompareAttributeToString( vSrcC, "EventAct", "Tag", "" ) != 0 )
         {
            ...
            GetStringFromAttribute( szTag, vSrcC, "EventAct", "Tag" );
            nRC = SetCursorFirstEntityByString( vTgt, "Action", "Tag", szTag, 0 );
            if ( nRC >= 0 )
            {
...
zwTZPNTRAD_InitMenuItems( zVIEW vSubtask )
{
   ...
   // Generate XRA's
   SysReadZeidonIni( -1, "[Workstation]",
                     "ExecutableRemote", szExecutableRemote );
...
zwTZPNTRAD_DeleteAllMenus( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vDialog, "Window", "Tag" );

   zstrcpy( szMsg, "OK to delete all Menus from Window '" );
   zstrcat( szMsg, szTag );
   zstrcat( szMsg, "'?" );
   nRC = MessagePrompt( vSubtask, "PE002",
                        "Dialog Maintenance",
...
{
   fnPainterCall( zMSG_ALIGN_LEFT, vSubtask, 0, 0 );
...
{
   fnPainterCall( zMSG_ALIGN_RIGHT, vSubtask, 0, 0 );
...
{
   fnPainterCall( zMSG_ALIGN_TOP, vSubtask, 0, 0 );
...
{
   fnPainterCall( zMSG_ALIGN_BOTTOM, vSubtask, 0, 0 );
...
{
   fnPainterCall( zMSG_SIZE_HEIGHT, vSubtask, 0, 0 );
...
{
   fnPainterCall( zMSG_SIZE_WIDTH, vSubtask, 0, 0 );
   return( 0 );
...
{
   fnPainterCall( zMSG_SIZE_WIDTH_HEIGHT, vSubtask, 0, 0 );
   return( 0 );
...
{
   fnPainterCall( zMSG_EQUAL_SPACE_VERTICAL, vSubtask, 0, 0 );
   return( 0 );
...
{
   fnPainterCall( zMSG_EQUAL_SPACE_HORIZONTAL, vSubtask, 0, 0 );
   return( 0 );
...
{
   fnPainterCall( zMSG_ABUT_VERTICAL, vSubtask, 0, 0 );
   return( 0 );
...
{
   fnPainterCall( zMSG_ABUT_HORIZONTAL, vSubtask, 0, 0 );
   return( 0 );
...
{
   fnPainterCall( zMSG_DELETE_SELECTED, vSubtask, 0, 0 );
   return( 0 );
                                  ...
                                  zSHORT  nIndex )
{
   ...
   do
   {
      ...
      nRC = -1;
      if ( vDialogData )
         nRC = SetCursorFirstEntityByString( vDialogData, "SourceFile",
                                             "Name", szNewName, "" );
                              ...
                              zPCHAR   szOutName )
{
   ...
   if ( zstrcmp( szOutName, "" ) == 0 )
   {
      zstrcpy( szMsg, "Dialog Name is required." );
      MessageSend( vSubtask, "ZO00137", "Dialog Maintenance",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
      SetFocusToCtrl( vSubtask, "edDialogName" );
   ...
   {
      zstrcpy( szMsg, "DLL Name is required." );
      MessageSend( vSubtask, "ZO00137", "Dialog Maintenance",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
                              ...
                              zVIEW    vSaveAs )
{
   ...
   for ( nRC = SetCursorFirstEntity( vSaveAs, "SourceFile", "" );
         ...
         nRC = SetCursorNextEntity( vSaveAs, "SourceFile", "" ) )
   {
      zwfnTZPNTRAD_SaveAsGetFileName( vTaskLPLR, vSaveAs, "SaveAsName",
                                      szNewFileName );
         ...
         nRC = SetCursorNextEntity( vSaveAs, "SourceFile", "" ) )
   {
      ...
      {
         zwfnTZPNTRAD_SaveAsGetFileName( vTaskLPLR, vSaveAs, "SourceName",
                                         szSourceFileName );
         zwfnTZPNTRAD_SaveAsGetFileName( vTaskLPLR, vSaveAs, "SaveAsName",
                                         szNewFileName );
...
zwTZPNTRAD_SaveAsDialog( zVIEW vSubtask )
{
   ...
   if ( SetCursorFirstEntityByString( vTZDIALOGS, "W_MetaDef", "Name",
                                      szOutName, 0 ) > zCURSOR_UNCHANGED )
   {
      zstrcpy( szMsg, "Dialog already exists. Replace existing Dialog?" );
      nRC = MessagePrompt( vSubtask, "ZO00138", "Dialog Maintenance",
                           szMsg, zBEEP, zBUTTONS_YESNO,
                   ...
                   zPCHAR  szDialogName)
{
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      GetStringFromAttribute( szViewObjRefName, vOldDialog, "ViewObjRef", "Name" );
   ...
   // Copy Windows
   GetStringFromAttribute( szOldDialogName, vOldDialog, "Dialog", "Tag" );
   ...
   {
      GetStringFromAttribute( szDfltWnd, vOldDialog, "DfltWnd", "Tag" );
   ...
   if ( bReport == FALSE )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         if ( CompareAttributeToString( vSrcO, "EventAct", "Tag", "" ) != 0 )
         {
            ...
            GetStringFromAttribute( szTag, vSrcO, "EventAct", "Tag" );
            nRC = SetCursorFirstEntityByString( vTgt, "Action", "Tag", szTag, 0 );
            if ( nRC >= 0 )
            {
           ...
           zVIEW  vPE )
{
   ...
   // ensure that the tag is unique.
   GetStringFromAttribute( szTag, vSrcO, "Menu", "Tag" );
   zstrcpy( szMsg, szTag );  // hold onto original tag
                   ...
                   zLONG lData )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      GetStringFromAttribute( szCtrlTag, vDialog, "Control", "Tag" );
      ...
      if ( lCheckFlags & (zCHECK_REPORT_EVENTS | zCHECK_CHANGE_WAB) )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
                                  ...
                                  0, 0, 0, 0 ) >= zCURSOR_SET )
            {
               ...
               GetStringFromAttribute( szDlgTag, vAct, "Action", "DialogName" );
               GetStringFromAttribute( szWndTag, vAct, "Action", "WindowName" );
         ...
         // zsprintf( szMsg, "Dlg.Wnd.Ctrl: %s.%s.%s  Action: %s  WAB: %s (%s.%s) Operation: %s",
      ...
      else
      {
              ...
              (lKey == zTAB_CONTROL) )
         {
            ...
            while ( nRC >= zCURSOR_SET )
            {
               ...
               GetStringFromAttribute( szCtrlTag, vDialog, "CtrlCtrl", "Tag" );
               ...
               while ( nRC >= zCURSOR_SET )
               {
                                        ...
                                        0, 0, 0, 0 ) >= zCURSOR_SET )
                  {
                     if ( CompareAttributeToString( vAct, "Action",
                                                    "NoMap", "Y" ) != 0 )
                     {
                        GetStringFromAttribute( szTag, vAct, "Action", "Tag" );
      ...
      // }
         ...
         if ( lKey == zLISTBOX_CONTROL )
         {
            ...
            if ( lSubtype & (zLISTBOX_COLUMNHEADERS | zLISTBOX_SORTEDHEADERS) )
            {
               if ( (lCheckFlags & zCHECK_EVENTS) &&
                    SetCursorFirstEntityByInteger( vDialog, "Event", "Type",
                                                   12, "" ) < zCURSOR_SET )
               {
                  if ( SetCursorFirstEntityByInteger( vDialog, "Event", "Type",
                                                      13, "" ) >= zCURSOR_SET ||
         ...
         if ( lCheckFlags & zCHECK_CTRL_TYPE )
         {
            ...
            if ( lSubtype )
            {
               ...
               GetAddrForAttribute( &pch, vDialog, "Control", "Tag" );
               zsprintf( szMsg, "(%s) Type for ctrl: \"%s\" is: ",
                         cpcWndTag, pch );
              ...
              (lKey == zCOMBOBOX_CONTROL) )
         {
            ...
            if ( lSubtype & zCOMBOBOX_NO_NULL )
            {
               ...
               GetAddrForAttribute( &pch, vDialog, "Control", "Tag" );
               zsprintf( szMsg, "(%s) ComboBox No Null for ctrl: \"%s\"",
                         cpcWndTag, pch );
               TraceLineS( szMsg, "" );
            }
         }
                            ...
                            zCPCHAR cpcDlgTag, zCPCHAR cpcWndTag )
{
                                                ...
                                                zCHECK_INCREMENT_POS_SIZEY) )
   {
      ...
      {
         lPosX += lIncrX * lPosX / 100;
         nClientWidth += lIncrX * nClientWidth / 100;
         nClientSizeX += lIncrX * nClientSizeX / 100;
      }
      ...
      {
         lPosY += lIncrY * lPosY / 100;
         nClientHeight += lIncrY * nClientHeight / 100;
         nClientSizeY += lIncrY * nClientSizeY / 100;
      }
   }
   else
   {
      if ( (lCheckFlags & zCHECK_DECREMENT_POS_SIZEX) ==
                                                zCHECK_DECREMENT_POS_SIZEX )
      {
         lPosX -= lIncrX * lPosX / 100;
         nClientWidth -= lIncrX * nClientWidth / 100;
   ...
   if ( nClientWidth <= 0 || nClientHeight <= 0 )
   {
      ...
      GetStringFromAttribute( szWindowTag, vDialog, "Window", "Tag" );
      zstrcpy(szMsg, "Window size became equal to zero; Window: ");
      zstrcat(szMsg, szWindowTag);
      zstrcat(szMsg, "\nDo you wish to abort the resize?");

      if ( MessagePrompt( vDialog, "PE002",
                          "Dialog Maintenance",
                          szMsg,
                          zBEEP, zBUTTONS_YESNO,
                          zRESPONSE_NO,  0 ) != zRESPONSE_YES )
...
// }
                    ...
                    "Dialog Migration", MB_YESNO ) == IDYES )
   {
      ...
      zsprintf( szMsg, "Are you sure you want to change mapping for Dialog: %s", g_szDlgTag );
                    ...
                    "Dialog Migration", MB_YESNO ) == IDYES )
   {
      ...
      zsprintf( szMsg, "Are you sure you want to add a GroupBox after specified GroupBox: %s\n"
   ...
   while ( nRC == zCURSOR_SET )
   {
                           ...
                           zSINGLE | zLEVEL_TASK ) == 1 )
      {
              ...
              (zCHECK_INCREMENT_POS_SIZE | zCHECK_DECREMENT_POS_SIZE) )
         {
            if ( !ComponentIsCheckedOut( vSubtask, vDialog, zSOURCE_DIALOG_META ) )
            {
               zstrcpy( szMsg, "Unable to save Dialog '" );
               zstrcat( szMsg, pchDlgTag );
               zstrcat( szMsg, "' because it is not checked out" );
               MessageSend( vSubtask, "PN00202", "Dialog Maintenance",
                            szMsg,
                          zMSGQ_OBJECT_CONSTRAINT_WARNING, zBEEP );
            }
         ...
         if ( (lCheckFlags & zCHECK_CHANGE_MAPPING) && zstrcmp( g_szDlgTag, pchDlgTag ) == 0 )
         {
            ...
            {
               zsprintf( szMsg, "Checking Dialog: %s for Migration", pchDlgTag );
               nRC = SetCursorFirstEntity( vDialog, "Window", 0 );
               while ( nRC == 0 )
               {
                  zVIEW  vOldViewName;
                  ...
                  zstrcpy( szOldViewName, "mSubLC" );
                  zstrcpy( szNewViewName, "mSPLDef" );
               ...
               while ( nRC == 0 )
               {
                  ...
                  while ( nRC == 0 )
                  {
                     ...
                     if ( zstrcmp( g_szGroupBox, pchCtrlTag ) == 0 )
                     {
                        ...
                        zstrcpy( szCtrlTag, pchWndTag );
                        zstrcat( szCtrlTag, "Section" );
         ...
         if ( bSaveAll )
         {
            ...
            {
               zsprintf( szMsg, "Saving Dialog: %s", pchDlgTag );
               MB_SetMessage( vSubtask, 1, szMsg );
         ...
         else
         {
            ...
            else
            {
               ...
               zsprintf( szMsg, "Checking Dialog: %s for synchronization", pchDlgTag );
               TraceLineS( szMsg, "" );
               MB_SetMessage( vSubtask, 1, szMsg );
               ...
               zstrcpy( szFileSpec2 + nLth, pchDlgTag );
               zstrcat( szFileSpec2 + nLth, ".xwd" );
...
ProportionallyResizeWndIncrement( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vDialog, "TZWINDOW", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      if ( lIncr )
      {
         GetAddrForAttribute( &pchDlgTag, vDialog, "Dialog", "Tag" );
         GetAddrForAttribute( &pchWndTag, vDialog, "Window", "Tag" );
         fnPainterCall( zMSG_UPDATEWINDOWBYNAME, vSubtask, 0, pchWndTag );
...
ProportionallyResizeDlgIncrement( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vDialog, "TZWINDOW", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      if ( lIncr )
      {
         ...
         fnPainterCall( zMSG_REPAINTALLZEIDONWINDOWS, vSubtask, 0, 0 );
         SetWindowActionBehavior( vSubtask, zWAB_StayOnWindowWithRefresh,
                                  0, 0 );
      }
   }
...
// GetViewByName( &vDialog, "TZWINDOW", vSubtask, zLEVEL_TASK );
   ...
   if ( vDialog )
   {
      ...
      zsprintf( szMsg, "Reset Control Pos/Size/Class for ALL Windows in Dialog: %s?", pchDlg );
...
ChangeSelectedViewMapping( zVIEW vSubtask )
{
   ...
   if ( vDialog && vTZZOVEAO )
   {
      ...
      GetStringFromAttribute( szVEA, vTZZOVEAO, "ViewObjRef", "Name" );
...
SetSelectedEntityMapping( zVIEW vSubtask )
{
   ...
   if ( vDialog && vTZZOVEAO )
   {
      ...
      nLth = zstrlen( szVEA );
      szVEA[ nLth++ ] = '.';
      GetStringFromAttribute( szVEA + nLth, vTZZOVEAO, "LOD_Entity", "Name" );
...
SetSelectedAttributeMapping( zVIEW vSubtask )
{
   ...
   if ( vDialog && vTZZOVEAO )
   {
      ...
      nLth = zstrlen( szVEA );
      szVEA[ nLth++ ] = '.';
      GetStringFromAttribute( szVEA + nLth, vTZZOVEAO, "ER_Attribute", "Name" );
      GetIntegerFromAttribute( &lZKey, vTZZOVEAO, "LOD", "ZKey" );
      RetrieveViewForMetaList( vSubtask,
                               &vCM_List, zREFER_LOD_META );
      SetCursorFirstEntityByInteger( vCM_List, "W_MetaDef",
...
fnChangeMappingViewChildren( zVIEW vSubtask, zBOOL bAtLevel )
{
   ...
   GetViewByName( &vDialog, "TZWINDOW", vSubtask, zLEVEL_TASK );
   GetViewByName( &vTZWINDOWL, "TZWINDOWL", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szCtrlTag, vTZWINDOWL, "Control", "Tag" );
...
GenerateAllJSP( zVIEW vSubtask )
{
   ...
   zsprintf( szMsg, "Generate JSP for ALL Windows in Dialog: %s?", pchDlg );
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      zsprintf( szMsg, "Generating JSP: %s.%s", pchDlg, pchWnd );
      MB_SetMessage( vSubtask, 1, szMsg );
      SetViewFromView( vValidate, vTZWINDOW );
      ValidateCtrlAndActionTags( vSubtask, vValidate );
      oTZWDLGSO_GenerateJSP( vTZWINDOW, vSubtask );
...
GenerateAllJSPJava( zVIEW vSubtask )
{
   ...
   // Prompt operator to ensure ALL windows are to be generated.
   GetAddrForAttribute( &pchDlg, vTZWINDOWL, "Dialog", "Tag" );
   zsprintf( szMsg, "Generate Java JSP for ALL Windows in Dialog: %s?", pchDlg );
   if ( g_bSkipPrompt == FALSE )
   {
...
FindCtrl( zVIEW vSubtask )
{
   ...
   if ( szText[ 0 ] )
   {
      ...
      if ( GetCtrlState( vSubtask, "FindByTag", zCONTROL_STATUS_CHECKED ) )
         zstrcpy( szAttrib, "Tag" );

      if ( GetCtrlState( vSubtask, "FindByText", zCONTROL_STATUS_CHECKED ) )
      {
         nLth = zstrlen( szAttrib );
         if ( nLth > 0 )
            szAttrib[ nLth++ ] = ',';

         zstrcpy( szAttrib + nLth, "Text" );
      }
...
fnSetTagType( zVIEW vFlow )
{
   ...
   GetStringFromAttribute( szText, vFlow, "Widget", "Tag" );
   GetStringFromAttribute( szType, vFlow, "Widget", "Type" );
   if ( szType[ 0 ] )
   {
      zstrcat( szText, " - " );
      zstrcat( szText, szType );
   }
...
BuildDialogFlow( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vDialog, "TZOPENDLG", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      GetStringFromAttribute( szText, vFlow, "TZFLOW", "Tag" );
      GetStringFromAttribute( szWork, vFlow, "TZFLOW", "Name" );
      if ( szWork[ 0 ] )
      {
         nLth = zstrlen( szText );
         szText[ nLth++ ] = ' ';
      ...
      while ( nRC == zCURSOR_SET )
      {
         ...
         GetStringFromAttribute( szText, vFlow, "Window", "Tag" );
         nLth = zstrlen( szText );
         szText[ nLth++ ] = ' ';
         szText[ nLth++ ] = '-';
         szText[ nLth++ ] = ' ';
         GetStringFromAttribute( szText + nLth, vFlow, "Window", "Caption" );
         ...
         while ( nRC == zCURSOR_SET )
         {
            ...
            GetStringFromAttribute( szTag, vFlow, "Action", "Tag" );
            GetStringFromAttribute( szOper, vFlow, "Action", "Operation" );
            GetStringFromAttribute( szDlgTag, vDialog, "Action", "DialogName" );
            GetStringFromAttribute( szWndTag, vDialog, "Action", "WindowName" );
            if ( szWndTag[ 0 ] )
            {
               zsprintf( szText, "Tag: %s   Oper: %s   WAB: %s   ====> %s.%s",
                         szTag, szOper, GetActionString( lActType ), szDlgTag, szWndTag );
            }
            else
            {
               zsprintf( szText, "Tag: %s   Oper: %s   WAB: %s",
            ...
            if ( CheckExistenceOfEntity( vDialog, "ActEvent" ) == 0 )
            {
               ...
               while ( nRC == zCURSOR_SET )
               {
                  if ( CheckExistenceOfEntity( vDialog, "ActCtrl" ) == 0 )
                  {
                     ...
                     GetStringFromAttribute( szText, vDialog, "ActCtrl", "Tag" );
                     nLth = zstrlen( szText );
                     szText[ nLth++ ] = ' ';
                     szText[ nLth++ ] = '-';
                     szText[ nLth++ ] = ' ';
                     GetStringFromAttribute( szText + nLth, vDialog, "ActCtrl", "Text" );
