...
InsertSetCursorWithWhere( zVIEW vSubtask )  // java???
{
   ...
   if ( g_lSetFNPL == qFIRST || g_lSetFNPL == qLAST || g_lSetFNPL == qNEXT )
   {
      ...
      if ( g_sWhereRight[ lUseFirst ].lElementClass == qATTRIBUTEQUALIFIER )
      {
         ...
         switch ( g_sWhereRight[ lUseFirst ].lDataType )
         {
            case qTDECIMAL:
               ...
               zstrcat( szOperationName, "DecimalFromAttribute" );
               break;

            case qTINTEGER:
               case qTSHORT:
               g_sWhereRight[ lUseFirst ].lDataClass = qNUMERIC;
               zstrcat( szOperationName, "IntegerFromAttribute" );
               break;

            default:
               g_sWhereRight[ lUseFirst ].lDataClass = qTEXT;
               zstrcat( szOperationName, "StringFromAttribute" );
               break;
         }
      ...
      if ( g_szGenLang[ 0 ] == 'J' )
      {
         ...
         if ( g_lSetFNPL == qNEXT )
         {
            ...
            zstrcpy( szOperationName, "setLast" );
#if 0
         switch ( g_sWhereRight[ lUseFirst ].lDataClass )
         {
            case qNUMERIC:
               if ( g_sWhereRight[ lUseFirst ].lDataType == qTDECIMAL )
                  zstrcat( szOperationName, "ByDecimal" );
               else
                  zstrcat( szOperationName, "ByInteger" );
               break;

            case qTEXT:
               zstrcat( szOperationName, "ByString" );
               break;
	  ...
	  */
         ...
         if ( g_lSetFNPL == qNEXT )
            ...
            zstrcpy( szOperationName, "SetCursorLastEntity" );
   ...
   for ( lLoop = 1; lLoop < g_lWhereIndex; lLoop++ )
   {
      if ( lLoop != lUseFirst )
      {
         ...
         zstrcpy( szOperationName, "CompareAttributeTo" );

         if ( g_sWhereRight[ lLoop ].lElementClass == qATTRIBUTEQUALIFIER )
         {
            zstrcat( szOperationName, "Attribute" );
         }
         else
         {
   ...
   if ( g_lWhereIndex > 2 )
   {
      ...
      if ( g_szGenLang[ 0 ] == 'J' )
      {
         ...
         if ( g_lSetFNPL == qFIRST || g_lSetFNPL == qNEXT )
            ...
            zstrcpy( szOperationName, "setPrevContinue" );
      }
      else
      {
	  */
         ...
         if ( g_lSetFNPL == qFIRST || g_lSetFNPL == qNEXT )
            ...
            zstrcpy( szOperationName, "SetCursorPrevEntity" );
...
// else
   ...
   //    while ( RESULT > zCURSOR_UNCHANGED &&
      ...
      for ( lLoop = 1; lLoop < g_lWhereIndex; lLoop++ )
      {
         ...
         // change the operator to the opposite since we are doing the "other" comparison.
         zstrcpy( g_szOperationName, "CompareAttributeTo" );

         if ( g_sWhereRight[ lLoop ].lElementClass == qATTRIBUTEQUALIFIER )
         {
            zstrcat( g_szOperationName, "Attribute" );
         }
         else
         {
            ...
            switch( g_sWhereRight[ lLoop ].lDataType )
            {
               ...
               case qTSTRING:
               case qTCHARACTER:
                  zstrcat( g_szOperationName, "String" );
                  break;

               // if the data type was Integer
               case qTINTEGER:
               case qTSHORT:
                  zstrcat( g_szOperationName, "Integer" );
                  break;

               // if the data type was Decimal
               case qTDECIMAL:
                  zstrcat( g_szOperationName, "Decimal" );
                  break;
