...
TZActiveX *GetCurrentActiveX( zVIEW vSubtask )
{
   ...
   fnPainterCall( zMSG_GETCURRCTRLWND, vSubtask, vSubtask, (zPCHAR) &pAtx );
...
zwTZAXCFGD_ReturnFromSubwindow( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vDialogC, "EventAct" ) == zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szEventTag, vTZPNEVWO, pszEvent, "Tag" );
...
zwTZAXCFGD_OK( zVIEW vSubtask )
{
   ...
   GetViewByName( &vControl, pszTZCONTROL, vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szTag, vControl, pszControl, pszTag );
   GetStringFromAttribute( szText, vControl, pszControl, pszText );
...
SetupGridColumnFromControlView( zVIEW vSubtask, Column &Col, ZColumns& cols )
{
   ...
   if ( nRC >= zCURSOR_UNCHANGED )
   {
      ...
      if ( CheckExistenceOfEntity( vControl, pszCtrlMapView ) == zCURSOR_SET )
      {
         ...
         GetStringFromAttribute( szView, vControl, pszCtrlMapView, pszName );
         GetStringFromAttribute( szEntity, vControl, pszCtrlMapRelatedEntity, pszName );
         GetStringFromAttribute( szAttribute, vControl, pszCtrlMapER_Attribute, pszName );
         if ( CheckExistenceOfEntity( vControl, pszCtrlMapContext ) == zCURSOR_SET )
         {
            GetStringFromAttribute( szContext, vControl, pszCtrlMapContext, pszName );
         }
      }
      ...
      GetStringFromAttribute( szHelp, vControl, pszControl, pszText );
   ...
   while ( nRC > zCURSOR_UNCHANGED && !bExitLoop )
   {
      ...
      switch( k )
      {
         ...
         case 2:
            ...
            {
               // Get the view, entity, attribute name  and context to create edit and listbox mapping
               GetStringFromAttribute( szViewName, vControl, pszCtrlMapView, pszName );
               GetStringFromAttribute( szEntityName, vControl, pszCtrlMapRelatedEntity, pszName );
               GetStringFromAttribute( szAttributeName, vControl, pszCtrlMapER_Attribute, pszName );
               if ( CheckExistenceOfEntity( vControl, pszCtrlMapContext ) == zCURSOR_SET )
               {
                  GetStringFromAttribute( szContextName, vControl, pszCtrlMapContext, pszName );
               }
         ...
         case 4:
            ...
            {
               GetStringFromAttribute( szEntityName, vControl, pszCtrlMapLOD_Entity, pszName );
               if ( k == 3 )
...
SetupGridRadioGroup( zVIEW vSubtask, Column &Col, ZColumns& cols, ZColRadioGrp* pRadioGrpProps )
{
   ...
   if ( CheckExistenceOfEntity( vControl, pszCtrlCtrl ) >= zCURSOR_SET )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         GetStringFromAttribute( szHelp, vSubControl, pszControl, pszText );
...
zwTZAXCFGD_DeleteRadioButton( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vRadioWork, pszRadio, pszTag );
