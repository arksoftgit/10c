...
static zPCHAR RemoveTabs (zPCHAR szWork);
static zVOID
             ...
             zCPCHAR cpcGenLang )
{
   ...
   GetStringFromAttribute( szSrcDirectory2, vLPLR, "LPLR", "PgmSrcDir" );
   SysConvertEnvironmentString( szSrcDirectory, szSrcDirectory2 );
   SysAppendcDirSep( szSrcDirectory );
   zstrcpy( szSrcDirectory2, szSrcDirectory );

   // DM - Is this doing what it is supposed to?  Save the operation name.
   zstrcpy( szOperationName, szInOperationName );

   // Fill out directory for error file.
   zstrcpy( szErrorFile, szSrcDirectory );

   // Stick in the file name.  DM - I am assuming the file name was properly passed in!!
   zstrcat( szSrcDirectory, szSourceFileName );
   zstrcat( szErrorFile, szSourceFileName );

   // Tack on extensions.
   zstrcat( szSrcDirectory, ".VML" );
   zstrcat( szErrorFile, ".ERR" );
   ...
   zstrcat( szXGODirectory, "TZOPSIGK.XGO" );
   ...
   // Get the ExecDir off of the LPLR Entity.  This will be the directory where the xpg is
   // now or will be saved to.
   GetStringFromAttribute( szFullXPGPathName, vViewObject, "LPLR", "ExecDir" );

   // Be sure the dir name is terminated with the delimiter.
   SysAppendcDirSep( szFullXPGPathName );

   // For Global Operations and Domain Operations, a work object (type LOD) for the source
   // meta was created and is to be used here instead of the vInSourceMeta passed to the Parser.
   ...
   if ( lMetaType == zSOURCE_DOMAINGRP_META || lMetaType == zREFER_DOMAINGRP_META )
   {
      ...
      zstrcpy( g_szSourceMetaObjectName, "TZDGSRCO" );
   }
   else
   if ( lMetaType == zSOURCE_GOPGRP_META || lMetaType == zREFER_GOPGRP_META )
   {
      ...
      zstrcpy( g_szSourceMetaObjectName, "TZOGSRCO" );
   }
   else
   if ( lMetaType == zSOURCE_SRC_META ||
        lMetaType == zREFER_SRC_META )
   {
      ...
      zCHAR szOpType[ 2 ] = { 0 };
      GetStringFromAttribute( szOpType, g_vSourceMeta, "Operation", "Type" );
      if ( szOpType[ 0 ] == 'G' )
      {
   ...
   zstrcpy( szXPGFileName, szSourceFileName );
   zstrcat( szFullXPGPathName, szXPGFileName );

   // Concat the Extension onto the directory.
   if ( g_szGenLang[ 0 ] == 'J' )
      zstrcat( szFullXPGPathName, ".XPJ" );
   else
      zstrcat( szFullXPGPathName, ".XPG" );
...
#define PARSER_VERSION 1
   ...
   zstrcat( szSrcDirectory2, "z__temp.vml" );
   ...
   if ( lOrigFile >= 0 )
   {
      ...
      {
         // Copy the input line to the output line, converting German umlaut characters
         // to |x and eliminating other characters with hex value > 80.
   ...
   //------------------------------------- Step Seven -------------------------------------------//
   // Set up the read buffer to begin reading.  Be sure the Source file can be checked out as
   // SOURCE not REFER.
   ...
   // Verify that the core header file was included at some point during the parse. It is needed
   // for all source to minimally define some data types even if it was never referenced in the
   // code.  Never being referenced is rare but could possibly happen.
   ...
   {
   //------------------------------------- Step Eight -------------------------------------------//
   // Finally we actually begin parsing.  Begin parsing based on what flags are set.  Switch
   // based on how much code we are going to deal with.
...
// }
   ...
   {
      zCHAR szIni[ 16 ] = { 0 };
      SysReadZeidonIni( -1, "[Zeidon]", "PrintXPG", szIni );
...
// zVIEW  lpTaskView = GetDefaultViewForActiveTask( );
   ...
   // If this could not be done, then give an error stating this.  Notice we do not use the global
   // error object since this is what we are trying to load.
   if ( nRC == -1 )
   {
...
SaveTextLine( void )
{
        ...
        (CheckExistenceOfEntity( g_lpPIView, "OperationSource" ) == zCURSOR_SET) )
   {
      ...
      SetAttributeFromString( g_lpPIView, "OperationText", "Text", RemoveTabs( szWork ) );
   }
   else
   {
      // Save line.
      ...
      SetAttributeFromString( g_lpPIView, "PreParseText", "Text", RemoveTabs( szWork ) );
   }
                ...
                zPCHAR szOperationName )
{
   ...
   if ( lRC == 0 )
   {
      ...
      GetStringFromAttribute( szSrcDirectory, vProfileView, "VML", "Dir" );
      ...
      lLth = 0;
      GetStringFromAttribute( szSourceFileName, vProfileView, "VML", "SourceFileName" );
      GetStringFromAttribute( szXPGFileName, vProfileView, "VML", "XPGFileName" );
      ...
      GetStringFromAttribute( szOperationName, vProfileView, "OP", "PassedName1" );
      return( 0 );
   }
   else
      return( -1 );
...
IncludeCoreHeader( zVIEW lpView, zPCHAR szHeaderName )
{
   ...
   if ( lRC > zCURSOR_UNCHANGED )
   {
      ...
      if ( lRC < zCURSOR_SET )
      {
         ...
         IncludeSubobjectFromSubobject( lpView, "ExternalHeaderFile",
                                        g_lpZOListView, "HeaderFile", zPOS_AFTER );
      }
   }
...
RemoveTabs( zPCHAR pchWork )
{
   ...
   {
      // The workstring is not large enough, so we replace each tab in qqin by a blank and
      // return qqin without using the workstring.
      pch = qqin;
      while ( (pch = zstrchr( pch,'\t' )) != 0 )
      {
         *pch = ' ';
...
fnPrintOperation( zVIEW vXPG )
{
   ...
   GetStringFromAttribute( szOperation, vXPG, "Operation", "Name" );
   GetIntegerFromAttribute( &lZKey, vXPG, "Operation", "ZKey" );
   // Description is skipped
   GetStringFromAttribute( szType, vXPG, "Operation", "Type" );
   // ReturnDesc is skipped
   GetStringFromAttribute( szReturnDataType,
                           vXPG, "Operation", "ReturnDataType" );
   // OrdinalValue is skipped
   // CurrentDebugFlag is skipped
   // C_GeneratedOperationName is skipped

   sprintf( szLine, "Operation=%s ZKey=%ld Type=%s ReturnType=%s",
...
fnPrintVariable( zVIEW vXPG )
{
   ...
   GetStringFromAttribute( szName, vXPG, "Variable", "Name" );
   ...
   GetStringFromAttribute( szPFlag, vXPG, "Variable", "PFlag" );
   GetStringFromAttribute( szUFlag, vXPG, "Variable", "UFlag" );

   sprintf( szLine, "   Variable=%s ID=%ld Parameter=%ld VMLType=%ld DataType=%ld"
                    " NElements=%ld LineNumber=%ld PFlag=%s UFlag=%s",
                    szName, lID, lParameterFlag, lVMLType, lDataType,
                    lNElements, lLineNumber, szPFlag, szUFlag );
   TraceLineS( szLine, "" );
}
...
fnPrintStatement( zVIEW vXPG, zLONG lLevel )
{
   ...
   {
      case qEXPRESSION:
         zstrcpy( szClass, "qEXPRESSION" );
         break;
      case qWHILE:
         zstrcpy( szClass, "qWHILE" );
         break;
      case qEND:
         zstrcpy( szClass, "qEND" );
         break;
      case qRETURN:
      case qRETURNEND:
         zstrcpy( szClass, "qRETURN" );
         break;
      case qIF:
         zstrcpy( szClass, "qIF" );
         break;
      case qFOR:
         zstrcpy( szClass, "qFOR" );
         break;
   }

   sprintf( szLine, "*STATEMENT* LineNumber=%ld Class=%s(%ld)", lLineNumber, szClass, lClass );
   TraceLineS( "", "" );
   ...
   if ( uTextLth )
   {
      ...
      sprintf( szLine, "%s ExpressionText=", szIndent );
...
fnPrintExpression( sQExprStruct *pWorkExpr, zLONG lLevel )
{
   ...
   {
      case qVARIABLE:
         zstrcpy( szElementClass, "qVARIABLE" );
         break;
      case qDEFINE:
         zstrcpy( szElementClass, "qDEFINE" );
         break;
      case qCONSTANT:
         zstrcpy( szElementClass, "qCONSTANT" );
         break;
      case qATTRIBUTEQUALIFIER:
         zstrcpy( szElementClass, "qATTRIBUTEQUALIFIER" );
         break;
      case qOPERATIONCALL:
         zstrcpy( szElementClass, "qOPERATIONCALL" );
         break;
      case qOPERATOR:
   ...
   {
      case qTEXT:
         zstrcpy( szDataClass, "qTEXT" );
         break;
      case qNUMERIC:
         zstrcpy( szDataClass, "qNUMERIC" );
         break;
   }
   ...
   {
      case qTINTEGER:
         zstrcpy( szDataType, "qTINTEGER" );
         break;
      case qTDECIMAL:
         zstrcpy( szDataType, "qTDECIMAL" );
         break;
      case qTSHORT:
         zstrcpy( szDataType, "qTSHORT" );
         break;
      case qTSTRING:
         zstrcpy( szDataType, "qTSTRING" );
         break;
      case qTCHARACTER:
         zstrcpy( szDataType, "qTCHARACTER" );
         break;
      case qTUNKNOWN:
         zstrcpy( szDataType, "qTUNKNOWN" );
         break;
   }

   sprintf( szLine, "%s DataClass=%s(%ld) DataType=%s(%ld) "
                    "Offset=%ld Level=%ld Flags=%ld",
            szElementClass, szDataClass, pWorkExpr->lDataClass,
            szDataType, pWorkExpr->lDataType,
...
fnPrintOperator( sQExprStruct *pWorkExpr, zCPCHAR szIndent )
{
   ...
   {
      case qLPAREN:
         zstrcpy( szOffset, "qLPAREN" );
         break;
      case qRPAREN:
         zstrcpy( szOffset, "qRPAREN" );
         break;
      case qOPENOPER:
         zstrcpy( szOffset, "qOPENOPER" );
         break;
      case qCLOSEOPER:
         zstrcpy( szOffset, "qCLOSEOPER" );
         break;
      case qCOMMA:
         zstrcpy( szOffset, "qCOMMA" );
         break;
      case qPERIOD:
         zstrcpy( szOffset, "qPERIOD" );
         break;
      case qEQUAL:
         zstrcpy( szOffset, "qEQUAL" );
         break;
      case qMINUS:
         zstrcpy( szOffset, "qMINUS" );
         break;
      case qADDITION:
         zstrcpy( szOffset, "qADDITION" );
         break;
      case qMULTIPLICATION:
         zstrcpy( szOffset, "qMULTIPLICATION" );
         break;
      case qDIVISION:
         zstrcpy( szOffset, "qDIVISION" );
         break;
      case qCONDEQUAL:
         zstrcpy( szOffset, "qCONDEQUAL" );
         break;
      case qLESSTHANEQUAL:
         zstrcpy( szOffset, "qLESSTHANEQUAL" );
         break;
      case qLESSTHAN:
         zstrcpy( szOffset, "qLESSTHAN" );
         break;
      case qGREATERTHANEQUAL:
         zstrcpy( szOffset, "qGREATERTHANEQUAL" );
         break;
      case qGREATERTHAN:
         zstrcpy( szOffset, "qGREATERTHAN" );
         break;
      case qNOTEQUAL:
         zstrcpy( szOffset, "qNOTEQUAL" );
         break;
      case qNOT:
         zstrcpy( szOffset, "qNOT" );
         break;
      case qAND:
         zstrcpy( szOffset, "qAND" );
         break;
      case qOR:
         zstrcpy( szOffset, "qOR" );
         break;
   }

   sprintf( szLine, "qOPERATOR Offset=%s(%ld)", szOffset, pWorkExpr->lOffset );
   TraceLineS( szIndent, szLine );
}
...
fnDeleteGenerated( zVIEW vSourceMeta )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szOperName, vSearchView, "Operation", "Name" );
