...
zwTZEREMDD_InitTool2( zVIEW vSubtask )
{
   ...
   // Always have an ER view available for mapping if this is not the first
   // time that the LPLR is loaded.
   ...
   // Model startup - show window as full size or comressed based on the way
   //                 it was on last exit.
   ...
   //                    window was active (at last exit), then the window
   //                    is shown.
...
zwTZEREMDD_InitMenuForEdit_RS( zVIEW  vSubtask, zVIEW  vTZEREMDO, zSHORT nERD_IsCheckedOut )
{
   ...
   if ( GetSelectStateOfEntity( vTZEREMDO, "ER_RelLink" ) == 1  || GetSelectStateOfEntity( vTZEREMDO, "ER_RelType" ) == 1 )
   {
      ...
      if ( nERD_IsCheckedOut )
      {
         ...
         if ( SetCursorFirstEntityByAttr( vTZEREMDO1, "ER_RelType_1", "ZKey", vTZEREMDO1, "ER_RelType", "ZKey", "EntpER_Model" ) >= zCURSOR_SET && ( CompareAttributeToInteger( vTZEREMDO1, "ER_Entity", "SAP_Entity", 1 ) == 0 || CompareAttributeToInteger( vTZEREMDO1, "ER_Entity_Other", "SAP_Entity", 1 ) == 0 ) )
         {
            SetOptionState( vSubtask, "DeleteSelectedRelationship", zOPTION_STATUS_ENABLED, 0 );
         }
         else
...
zwTZEREMDD_MainRtnFromSubWnd( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      ...
      // Re-Establish the EMD view.  If a view cannot be found, there is
      // a serious error. Show the LPLR window
...
zwTZEREMDD_CreateNewModel( zVIEW vSubtask )
{
   ...
   if ( nRC < 0 )
   {
      zstrcpy( szWorkMessage, "Unable to Activate empty Data Model." );
      MessageSend( vSubtask, "ER00101", "ER Data Model Maintenance", szWorkMessage, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      return( 0 );
   }

   // Build Root Instance
   nRC = CreateMetaEntity( vSubtask, vTZEREMDO, "EntpER_Model", zPOS_AFTER );
   nRC = GetViewByName( &vT, "TaskLPLR", vSubtask, zLEVEL_TASK );
   nRC = SetAttributeFromAttribute( vTZEREMDO, "EntpER_Model", "Name", vT, "LPLR", "Name" );
   nRC = SetAttributeFromAttribute( vTZEREMDO, "EntpER_Model", "Desc", vT, "LPLR", "Desc" );
   nRC = SetNameForView( vTZEREMDO, "newTZEREMDO", vSubtask, zLEVEL_TASK );

   // Add a single entity to mode.  The model must always have one entity.
   zwfnTZEREMDD_AddEntity( vSubtask, vTZEREMDO );

   nRC = SetAttributeFromAttribute( vTZEREMDO, "ER_Entity", "Name", vT, "LPLR", "Name" );
...
zwTZEREMDD_CancelNewModel( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access new Data Model.  Model must be opened to Cancel New." );
   ...
   nRC = DropMetaOI( vSubtask, vTZEREMDO );
...
zwfnTZEREMDD_CheckRelLinkIdentifier( zVIEW vSubtask, zVIEW vERD2 )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      {
         zstrcpy( szWorkMessage, "Identifier '" );
         GetStringFromAttribute( szName, vERD2, "ER_EntIdentifier", "Name" );
         zstrcat( szWorkMessage, szName );
         zstrcat( szWorkMessage, "' does not have an Attribute or Relationship assigned. " );
...
zwfnTZEREMDD_CheckDomains( zVIEW vSubtask, zVIEW vERD2 )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      {
         GetStringFromAttribute( szName, vERD2, "ER_Attribute", "Name" );
         zstrcpy( szWorkMessage, "Attribute '" );
         zstrcat( szWorkMessage, szName );
         zstrcat( szWorkMessage, "' does not have Domain assigned." );
...
zwfnTZEREMDD_CheckRelationships( zVIEW vSubtask, zVIEW vERD2 )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
            if ( (CompareAttributeToString( vERD3, "ER_Entity", "Name", pchEntityName1 ) == 0) && (CompareAttributeToString( vERD3, "ER_RelLink", "Name", pchRelName1 ) == 0) && (CompareAttributeToString( vERD3, "ER_RelLink_Other", "Name", pchRelName2 ) == 0) && (CompareAttributeToString( vERD3, "ER_Entity_Other", "Name", pchEntityName2) == 0) )
            {
               zstrcpy( szWorkMessage, "Duplicate Relationship: '" );
               zstrcat( szWorkMessage, pchEntityName1 );
               zstrcat( szWorkMessage, " " );
               zstrcat( szWorkMessage, pchRelName1 );
               zstrcat( szWorkMessage, " " );
               zstrcat( szWorkMessage, pchEntityName2 );
               zstrcat( szWorkMessage, "', '" );
               zstrcat( szWorkMessage, pchEntityName2 );
               zstrcat( szWorkMessage, " " );
               zstrcat( szWorkMessage, pchRelName2 );
               zstrcat( szWorkMessage, " " );
               zstrcat( szWorkMessage, pchEntityName1 );
               zstrcat( szWorkMessage, "'." );
               zwTZEREMDD_WriteErrorMsg( vSubtask, vERD2, szWorkMessage, 1 );
...
zwfnTZEREMDD_CommitERD( zVIEW vSubtask, zVIEW vTZEREMDO )
{
   ...
   {
      zstrcpy( szWorkMessage, "Unable to save Data Model." );
   ...
   else
   {
      ...
      if ( SetCursorFirstEntity( vDTE, "W_MetaDef", "" ) < zCURSOR_SET )
      {
         ...
         if ( !vDTE )
         {
            zstrcpy( szWorkMessage, "Unable to Activate empty Physical Model." );
...
zwTZEREMDD_SaveModel( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szWorkMessage, "Data Model is empty and cannot be Saved." );
...
zwTZEREMDD_DeleteModel( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Data Model List.  Model list must be opened to Delete." );
      MessageSend( vSubtask, "ER00110", "ER Data Model Maintenance", szWorkMessage, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      return( 0 );
   }

   // Prompt to confirm Delete
   GetStringFromAttribute( sz, vTZERMFLO, "W_MetaDef", "Name" );
   zstrcpy( szWorkMessage, "Delete Data Model\n\n" );
   zstrcat( szWorkMessage, sz );
   nRC = MessagePrompt( vSubtask,                 "ER10011", "ER Data Model Maintenance", szWorkMessage, zBEEP,                    zBUTTONS_YESNO, zRESPONSE_NO,             0 );
...
zwTZEREMDD_SaveAsModel( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Data Model.  Model must be opened to Save As." );
...
zwfnTZEREMDD_RefreshEntitySelect( zVIEW  vSubtask, zSHORT nDeselectRelationship )
{
   ...
   if ( nDeselectRelationship == 1 && CheckExistenceOfEntity( vTZEREMDO, "ER_RelType" ) >= zCURSOR_SET  && GetSelectStateOfEntity( vTZEREMDO, "ER_RelType" ) == 1 )
   {
   ...
   if ( GetViewByName( &vTZERSASO, "TZERSASO", vSubtask, zLEVEL_TASK ) > 0 && vWindow )
   {
      ...
      if ( nDeselectRelationship == 1 && CheckExistenceOfEntity( vTZERSASO, "SA_RelType" ) >= zCURSOR_SET  && GetSelectStateOfEntity( vTZERSASO, "SA_RelType" ) == 1 )
      {
         SetSelectStateOfEntity( vTZERSASO, "SA_RelType", 0 );
      }

      // First, deselect all selected Entities
      for ( nRC = SetCursorFirstSelectedEntity( vTZERSASO, "SA_Entity", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZERSASO, "SA_Entity", "" ) )
      {
      ...
      for ( nRC = SetCursorFirstSelectedEntity( vTZEREMDO, "ER_Entity", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZEREMDO, "ER_Entity", "" ) )
      {
...
// PURPOSE:    This Entry refreshes the Entity List in the
//             ER Dialog.
//
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
zwTZEREMDD_RefreshEntWnd( zVIEW vSubtask )
{
   ...
   if ( GetSubtaskForWindowName( vSubtask, &vW, "ENTVIEW" ) >= 0 )
   {
      ...
      {
         GetStringFromAttribute( szEntityName, vTZEREMDO, "ER_Entity", "Name" );
...
// PURPOSE:    This Entry refreshes the Identifier List in the
//             ER Dialog.
//
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
...
// PURPOSE:    This Entry refreshes the Relationship List in the
//             ER Dialog.
//
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
...
// PURPOSE:    This Entry refreshes the Subject Area List in the
//             ER Dialog.
//
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
zwTZEREMDD_RefreshSAWnd( zVIEW vSubtask )
{
   ...
   if ( GetSubtaskForWindowName( vSubtask, &vW, "SA_Diagram" ) >= 0 )
   {
      ...
      for ( nRC = SetCursorFirstSelectedEntity( vTZEREMDO, "ER_Entity", 0 ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZEREMDO, "ER_Entity", 0 ) )
      {
...
zwTZEREMDD_PostBuildENT( zVIEW vSubtask )
{
   ...
   // Disable Next and Previous buttons differently depending whether we
   // came from the ERD window or the subject area window.
   ...
   GetStringFromAttribute( szEntityPurpose, vTZEREMDO, "ER_Entity", "Purpose" );
...
zwTZEREMDD_ValidateEntity( zVIEW vSubtask )
{
...

   {
      ...
      {
       zstrcpy( szMsg, "Duplicate TE Table Name in Data Model. Entity/TE Table: ");
       zstrcat( szMsg, szEntityName );
         zstrcat( szMsg, NEW_LINE );
       zstrcat( szMsg, "Do you want to keep the duplicate names?" );
...
zwTZEREMDD_EntityNext( zVIEW vSubtask )
{
   ...
   // Process differently when coming directly from ER window versus coming
   // from Subject Area window.
   if ( nSA_RC >= zCURSOR_SET && CheckExistenceOfEntity( vTZERSASO, "SA_Entity" ) >= zCURSOR_SET )
   {
...
zwTZEREMDD_CopyEntity( zVIEW vSubtask )
{
   ...
   zstrcpy( szEntityName, "new_" );
   GetStringFromAttribute( szEntityName + 4, vTZEREMDO, "ER_Entity", "Name" );
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         {
            nRC = CreateMetaEntity( vSubtask, vTZEREMD1, "ER_FactType", zPOS_LAST );
            SetMatchingAttributesByName( vTZEREMD1, "ER_FactType", vTZEREMDO, "ER_FactType", zSET_NULL | zSET_NOTNULL );
            GetStringFromAttribute( szName, vTZEREMDO, "ER_AttributeIdentifier", "Name" );
            nRC = SetCursorFirstEntityByString( vTZEREMD1, "ER_Attribute", "Name", szName, 0 );
            nRC = IncludeSubobjectFromSubobject( vTZEREMD1, "ER_AttributeIdentifier", vTZEREMD1, "ER_Attribute", zPOS_AFTER );
         }
...
zwTZEREMDD_DeleteEntity( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szMsg, "Delete Bypassed. Data Model must have at least one Entity." );
...
zwTZEREMDD_DeleteSelectedEnts( zVIEW vSubtask )
{
   ...
   // Loop thru TZEREMDO1 and delete all selected entities, Set TZEREMDO to
   // the proper entity to highlite.
   GetSubtaskForWindowName( vSubtask, &vW, "TZEREMDD" );
   CreateViewFromViewForTask( &vTZEREMDO1, vTZEREMDO, 0 );
...
zwTZEREMDD_DeleteAttrib( zVIEW vSubtask )
{
   ...
   if ( nRC == zRESPONSE_YES )
   {
      for ( nRC = SetCursorFirstSelectedEntity( vTZEREMDO, "ER_Attribute", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZEREMDO, "ER_Attribute", "" ) )
      {
         // DC 2000.09.26
...
zwTZEREMDD_DiagToRLLMaint( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZEREMDO, "TZEREMDO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      if ( CompareAttributeToAttribute( vTZEREMDO, "ER_Entity", "ER_DiagramPosX", vTZEREMDO, "ER_Entity_Other", "ER_DiagramPosX" ) > 0 || (CompareAttributeToAttribute( vTZEREMDO, "ER_Entity", "ER_DiagramPosX", vTZEREMDO, "ER_Entity_Other", "ER_DiagramPosX" ) == 0 && CompareAttributeToAttribute( vTZEREMDO, "ER_Entity", "ER_DiagramPosX", vTZEREMDO, "ER_Entity_Other", "ER_DiagramPosX" ) > 0) )
      {
...
zwTZEREMDD_IsRelToSapEntity( zVIEW vSubtask, zVIEW vTZEREMDO )
{
   ...
   if ( CompareAttributeToInteger( vTZEREMDO, "ER_Entity", "SAP_Entity", 1 ) != 0 )
   {
      GetCtrlText( vSubtask, "EntOther", szEntity_Other, 33 );
      CreateViewFromViewForTask( &vERD, vTZEREMDO, 0 );
...
zwTZEREMDD_RelLinkOK( zVIEW vSubtask )
{
   ...
   // RelLink entry.
   if ( CompareAttributeToString( vProfileXfer, "TempRelLink", "RelLink1HangingForeignKeyEntity", "Y" ) == 0 && CompareAttributeToString( vProfileXfer, "TempRelLink", "RelLink1CardMax", "1" ) != 0 )
   {
   ...
   if ( CompareAttributeToString( vProfileXfer, "TempRelLink", "RelLink2HangingForeignKeyEntity", "Y" ) == 0 && CompareAttributeToString( vProfileXfer, "TempRelLink", "RelLink2CardMax", "1" ) != 0 )
   {
      MessageSend( vSubtask, "ER00191", "ER Data Model Maintenance", "Max cardinality must be 1 when 'Hanging FK Entity' is specified.", zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
...
zwTZEREMDD_IsRelLinkToSapEntity( zVIEW  vSubtask, zVIEW  vTZRELTGT, zVIEW  vTZEREMDO )
{
   ...
   if ( GetSubtaskForWindowName( vSubtask, &vWindow, "RLLMAINT" ) < 0 && ( CompareAttributeToInteger( vTZRELTGT, "ER_Entity", "SAP_Entity", 1 ) == 0 || CompareAttributeToInteger( vTZEREMDO, "ER_Entity", "SAP_Entity", 1 ) == 0 ) )
   {
      SysMessageBox( vSubtask, "ER Data Model Maintenance", "It is not allowed to create a Relationship to a SAP Entity.", 1 );
...
zwTZEREMDD_AddRelLinkToSA( zVIEW  vSubtask, zVIEW  vTZEREMDO, zVIEW  vTZERSASO )
{
   ...
   if ( SetCursorFirstEntityByInteger( vTZERSASO, "ER_RelType", "ZKey", lZKey1, "SubjectArea" ) < zCURSOR_SET )
   {
      ...
      if ( SetCursorFirstEntityByInteger( vSA2, "ER_Entity", "ZKey", lZKey1, "SubjectArea" ) >= zCURSOR_SET && SetCursorFirstEntityByInteger( vSA3, "ER_Entity", "ZKey", lZKey2, "SubjectArea" ) >= zCURSOR_SET )
      {
...
zwTZEREMDD_DeleteRelLink( zVIEW vSubtask )
{
   ...
   if ( vSubtask )
      zstrcpy( szEntityName, "ER_RelLink" );
   else
   {
      zstrcpy( szEntityName, "ER_RelType" );
      nRelType = 1;
   }
   ...
   if ( nRC == zRESPONSE_YES )
   {
      for ( nRC = SetCursorFirstSelectedEntity( vTZEREMDO, szEntityName, "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZEREMDO, szEntityName, "" ) )
...
zwTZEREMDD_DeleteAllRelLinks( zVIEW vSubtask )
{
   ...
   if ( nRC == zRESPONSE_YES )
   {
      ...
      for ( nRC = SetCursorFirstEntity( vTZEREMDO, "ER_RelLink", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vTZEREMDO, "ER_RelLink", "" ) )
         oTZEREMDO_DeleteRelationship( vTZEREMDO, zREPOS_NONE, 0 );
   }
...
zwfnTZEREMDD_RedirectRelationship( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Data Model.  Model must be opened." );
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Entity List for changing Target of Relationship." );
   ...
   if ( GetSelectStateOfEntityForSet( vTZERENLO, "ER_Entity", 1 ) == 0 || CompareAttributeToAttribute( vTZERENLO, "ER_Entity", "ZKey", vTZEREMDO, "ER_Entity_Other", "ZKey" ) == 0 )
   {
      return( 0 );
   }

   // Capture Relationship settings prior to Delete
   GetStringFromAttribute( szRelNameFwd, vTZEREMDO, "ER_RelLink", "Name" );
   GetStringFromAttribute( szMinCardFwd, vTZEREMDO, "ER_RelLink", "CardMin" );
   GetStringFromAttribute( szMaxCardFwd, vTZEREMDO, "ER_RelLink", "CardMax" );
   GetStringFromAttribute( szRelNameInv, vTZEREMDO, "ER_RelLink_Other", "Name" );
   GetStringFromAttribute( szMinCardInv, vTZEREMDO, "ER_RelLink_Other", "CardMin" );
   GetStringFromAttribute( szMaxCardInv, vTZEREMDO, "ER_RelLink_Other", "CardMax" );
   GetIntegerFromAttribute( &lZKey, vTZEREMDO, "ER_Entity", "ZKey" );
...
zwTZEREMDD_SaveAllLODs( zVIEW vSubtask )
{
   ...
   if ( MessagePrompt( vSubtask, "ER10031", "ER Data Model Maintenance", "Data Model and All LODs will be saved. Continue?", zBEEP, zBUTTONS_YESNO, zRESPONSE_NO, 0 )          == zRESPONSE_YES )
   {
      nRC = zwTZEREMDD_SaveModel( vSubtask );
      if ( nRC == 0 )
...
zwTZEREMDD_IncludeDomain( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access ER Object." );
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Domain List." );
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Domain to be included." );
...
zwTZEREMDD_SaveSA( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szWorkMessage, "Unable to save empty Subject Area." );
...
zwTZEREMDD_SA_AskForSave( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szName, vTZERSASO, "SubjectArea", "Name" );
      zstrcpy( szMsg, "Subject Area '" );
      zstrcat( szMsg, szName );
      zstrcat( szMsg, "' has been modified, save it with Data Model?" );
...
zwTZEREMDD_OpenSA( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Subject Area.  SA must be opened." );
   ...
   {
      zstrcpy( szWorkMessage, "Unable to Open Subject Area." );
...
zwTZEREMDD_CreateNewSA( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szWorkMessage, "Subject Area Name is required." );
   ...
   {
      zstrcpy( szWorkMessage, "Subject Area already exists." );
...
zwTZEREMDD_AddOneEntityToSA( zVIEW vSubtask )
{
   ...
   // Now select our currently selected SA Entity (new)
   // nRC = SetCursorFirstEntityByEntityCsr( vTZEREMDO, "ER_Entity",
   //                                        vTZERSASO, "ER_Entity",
   //                                        "" );
...
zwTZEREMDD_ExcludeOneEntityFrSA( zVIEW vSubtask )
{
   ...
   // Prompt to confirm Delete
   for ( nRC = SetCursorFirstSelectedEntity( vTZERSASO, "SA_Entity", 0 ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZERSASO, "SA_Entity", 0 ) )
   {
...
zwTZEREMDD_DfltAddIdViaAttr( zVIEW vSubtask )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      zstrcpy( szWorkMessage, "ID_" );
      GetStringFromAttribute( &szWorkMessage[ 3 ], vTZEREMDO, "ER_Entity", "Name" );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         {  // check to see if default exists
            ...
            GetStringFromAttribute( &szWorkMessage[ 64 ], vTZEREMDO, "ER_Attribute", "Name" );
...
zwTZEREMDD_AddIdViaRel( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szWorkMessage, "Relationship being added has a Max Cardinality\n" );
      zstrcat( szWorkMessage, "on the inverse side that is not equal to 1.\n\n" );
      zstrcat( szWorkMessage, "Select another Relationship." );
...
zwTZEREMDD_DeleteIdentifier( zVIEW vSubtask )
{
   ...
   if ( nRC == zRESPONSE_YES )
   {
      for ( nRC = SetCursorFirstSelectedEntity( vTZEREMDO, "ER_EntIdentifier", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZEREMDO, "ER_EntIdentifier", "" ) )
      {
...
zwTZEREMDD_PostBuildATTR( zVIEW vSubtask )
{
   ...
   else
   {
      ...
      else
      {
         ...
         // if SAP Entity and not SAP Attribute, Attribute is a Work
         // Attribute, disable Work CheckBox
         if ( CompareAttributeToInteger( vTZEREMDO, "ER_Entity", "SAP_Entity", 1 ) == 0 )
            nEnableWork = 0;
      }
   }
...
zwTZEREMDD_PostBuildID( zVIEW vSubtask )
{
   ...
   // If Data Model not checked out or Entity is a SAP Entity,
...
zwTZEREMDD_PostBuildRLL( zVIEW vSubtask )
{
   ...
   for ( nRC = SetCursorFirstSelectedEntity( vTZEREMDO, "ER_RelLink", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vTZEREMDO, "ER_RelLink", "" ) )
   {
...
// PURPOSE:   Activate selected ER Model and empty SA if ER is being
//            switched from SA Dialog.
// TRIGGERED BY:
//    WINDOW: TZEROPMD
//    ACTION: BUTTON
//    NAME: Open
...
zwTZEREMDD_OpenModel( zVIEW vSubtask )
{
   ...
   if ( nRC > 0 )
   {
      ...
      if ( nRC )
      {
         ...
         if ( nRC < 0 )
         {
            zstrcpy( szWorkMessage, "Unable to Activate empty Subject Area." );
...
zwTZEREMDD_PreBuildSAVIEW( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Subject Area.  Subject Area must be opened." );
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access ER Dialog." );
...
zwTZEREMDD_PostBuildSAVIEW( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSAName, vTZERSASO, "SubjectArea", "Name" );
...
zwTZEREMDD_ADW_OK( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szDirectory, vADW_Work, "ADW_Work", "Directory" );
   GetStringFromAttribute( szPrefix, vADW_Work, "ADW_Work", "ExportPrefix" );
   ...
   if ( SetCursorFirstEntityByString( vTaskLPLR, "W_MetaDef", "Name", "Text", 0 ) < zCURSOR_SET  || SetCursorFirstEntityByString( vTaskLPLR, "W_MetaDef", "Name", "Integer", 0 ) < zCURSOR_SET )
   {
   ...
   // Look to see if a work file has been saved using the Directory and
   // Prefix entered.  If so, activate it and copy over the InfoTypeDef entries.
   // Import may want to use the InfoType information saved in the work object.
   zstrcpy( szFileName, szDirectory );
   zstrcat( szFileName, "\\" );
   zstrcat( szFileName, szPrefix );
   zstrcat( szFileName, ".ADW" );
   ...
   {
      nRC = ActivateEmptyMetaOI( vSubtask, &vERM, zSOURCE_ERD_META, zSINGLE | zLEVEL_APPLICATION );
      // Build Root Instance
      nRC = CreateMetaEntity( vSubtask, vERM, "EntpER_Model", zPOS_AFTER );
      nRC = SetAttributeFromAttribute( vERM, "EntpER_Model", "Name", vTaskLPLR, "LPLR", "Name" );
      nRC = SetAttributeFromAttribute( vERM, "EntpER_Model", "Desc", vTaskLPLR, "LPLR", "Desc" );
      nRC = SetNameForView( vERM, "TZEREMDO", vSubtask, zLEVEL_TASK );
      // Set window Title with check out state
...
zwTZEREMDD_ADW_InfoTypeInit( zVIEW vSubtask )
{
   ...
   // Look to see if a work file has been saved using the Directory and
   // Prefix entered.  If so, Activate it and proceed.  If not, initialize
   // the current work object with InfoTypes from the Export files.
   zstrcpy( szFileName, lpszDirectory );
   zstrcat( szFileName, "\\" );
   zstrcat( szFileName, lpszPrefix );
   zstrcat( szFileName, ".ADW" );
   hFile = SysOpenFile( vSubtask, szFileName, COREFILE_READ );
   if ( hFile >= 0 )
   {
   ...
   {
      // Execute the routine to build the work object and then loop through
      // through all the InfoType entries (OI.Type 10094), creating InfoType
      // entities in the work object.
      ...
      // Save the work area as a file in the same directory as the ADW export
      // files.
...
zwTZEREMDD_ADW_InfoTypeOK( zVIEW vSubtask )
{
   ...
   // Look to see if a work file has been saved using the Directory and
   // Prefix entered.  If so, Activate it and proceed.  If not, return with
   // an error.
   zstrcpy( szFileName, lpszDirectory );
   zstrcat( szFileName, "\\" );
   zstrcat( szFileName, lpszPrefix );
...
{
   // This routine generates a Genkey LOD.  It is called from the Dialog
   // operation below, CreateGenkeyEntityLOD.
...
{
   // This routine generates an Entity for generated keys, along with
   // the associated LOD for generated keys.
...
{
   // This routine generates a Locking LOD.  It is called from the Dialog
   // operation below, CreateLockingEntityLOD.
   ...
   for ( nRC = SetCursorFirstEntity( vERD, "ER_Attribute", 0 ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vERD, "ER_Attribute", 0 ) )
   {
      CreateMetaEntity( vSubtask, vLOD, "LOD_AttributeRec", zPOS_AFTER );
      IncludeSubobjectFromSubobject( vLOD, "ER_AttributeRec", vERD, "ER_Attribute", zPOS_AFTER );
   }
...
{
   // This routine generates an Entity for pessimistic locking, along with
   // the associated LOD for pessimistic locking.
   ...
   // LOD name.  We make the length of the attribute 35 instead of the usual
   // 32 because we need the ability to prepend a few characters.
   CreateMetaEntity( vSubtask, vERD, "ER_Attribute", zPOS_AFTER );
...
{
   // This routine generates a Dynamic Domain LOD, "DOMAINT".  It is called
   // from the Dialog operation below, CreateDynamicDomain.
   ...
   // We will use a new LOD view to keep position at top as we step down
   // one subobject level.
...
CreateDynamicDomain( zVIEW vSubtask )
{
   ...
   // Go to also create the LOD.  Pass vERD2, since it is positioned on the
   // Domain entity.
...
zwTZEREMDD_DomainAttributeList( zVIEW vSubtask )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szDomainName, vERD, "Domain", "Name" );
...
{
   // This routine refreshes the listbox of Attributes underneath the
   // Domain selected in the other litbox.
...
{
   // This routine creates a hierarchical diagram for a Domain and the
   // Attributes which use it.
   // It also brings up the Graph window if it isn't already up, or
   // refreshes it if it is up.
   ...
   // Loop through all the ER_Attributes underneath the Domain at which the
   // cursor is positioned and create HD_ChildNode entries.
...
{
   // This routine names the view of the Domain/Attribute Graph window
   // so that the parent window can refresh it.
...
zwTZEREMDD_SetDIL_Message( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szChildEntityName,  vERD, "ER_Entity",       "Name" );
      GetStringFromAttribute( szParentEntityName, vERD, "ER_Entity_Other", "Name" );
   }
   else
      if ( zstrcmp( szReturnCtrlTag, "AutoSeq_Other" ) == 0 )

      {
         GetStringFromAttribute( szParentEntityName, vERD, "ER_Entity",       "Name" );
         GetStringFromAttribute( szChildEntityName,  vERD, "ER_Entity_Other", "Name" );
      }

   if ( zstrcmp( szReturnCtrlTag, "AutoSeq" ) == 0 || zstrcmp( szReturnCtrlTag, "AutoSeq_Other" ) == 0 )
   {
      strcpy (szWorkMessage, "Maintain the last order (positions) of '" );
      strcat (szWorkMessage, szParentEntityName );
      strcat (szWorkMessage, "' under '" );
      strcat (szWorkMessage, szChildEntityName );
      strcat (szWorkMessage, "'." );
...
zwTZEREMDD_PrintSelectedDomains( zVIEW vSubtask )
{
   ...
   for ( nRC = SetCursorFirstSelectedEntity( vDomainList, "Domain", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vDomainList, "Domain", "" ) )
   {
...
{
   // This operation copies all the selected attributes (actually includes
   // them) to the derived relationship ER_AttributeCopy.
   ...
   for ( nRC = SetCursorFirstEntity( vERD, "ER_AttributeCopy", 0 ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vERD, "ER_AttributeCopy", 0 ) )
   {
   ...
   for ( nRC = SetCursorFirstSelectedEntity( vERD, "ER_Attribute", 0 ); nRC >= zCURSOR_SET; nRC = SetCursorNextSelectedEntity( vERD, "ER_Attribute", 0 ) )
   {
...
zwTZEREMDD_PasteAttributes( zVIEW vSubtask )
{
   ...
   // Loop through all ER_AttributeCopy entities and copy them to
   // ER_Attribute entities, include the correct Domain.
   for ( nRC = SetCursorFirstEntity( vERD, "ER_AttributeCopy", 0 ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vERD, "ER_AttributeCopy", 0 ) )
   {
      CreateMetaEntity( vSubtask, vERD, "ER_Attribute", zPOS_AFTER );
...
zwTZEREMDD_SA_SetNewName( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSAName, vTZERSFLO, "W_MetaDef", "Name" );
   ...
   {
      GetStringFromAttribute( szDescription, vTZERSFLO, "W_MetaDef", "Desc" );
...
zwTZEREMDD_SA_LoadListForSaveAs( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szDescription, vTZERSASO, "SubjectArea", "Desc" );
   SetCtrlText( vSubtask, "edDesc", szDescription );
...
zwTZEREMDD_SA_SaveAs( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szNewName, vTZERSASO, "SubjectArea", "SaveAsName" );
   ...
   {
      zstrcpy( szWorkMessage, "Subject Area already exists. Replace existing Subject Area?" );
...
zwTZEREMDD_ADW_Set_Dir_Prefix( zVIEW vSubtask, zPCHAR szFileName, zLONG  Suffix )
{
   ...
   if ( Backslash )
   {
      zstrncpy( szDirectory, szFileName, lIndex );
      szDirectory[ lIndex ] = 0;
      zstrncpy( szPrefix, szFileName + 1 + lIndex, lStringLen - 4 - 1 - Suffix - lIndex );
      szPrefix[ lStringLen - 4 - 1 - Suffix - lIndex ] = 0;
      zstrncpy( szExtention, szFileName + lStringLen - 3, 3 );
      szExtention[ 3 ] = 0;
   }
...
zwTZEREMDD_ADW_Exp_OK( zVIEW vSubtask )
{
   ...
   GetViewByName( &vTZEREMDO, "TZEREMDO", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szDirectory, vTZERADWE, "ADW_Work_Export", "Directory" );
   if ( zstrcmp( szDirectory, "" ) == 0 )
   {
...
zwTZEREMDD_AcceptSubobject( zVIEW  vSubtask, zPCHAR szEntityName, zPCHAR szMessageText, zPCHAR szTE_Attribute, zPCHAR szTE_MessText )
{
   ...
   GetStringFromAttribute( szName, vTZEREMDO, szEntityName, "Name" );
   if ( CheckDuplicateEntityByString( vTZEREMDO, szEntityName, "Name", szName, 0 ) < 0 )
   {
      zstrcpy( szMsg, "Duplicate " );
      zstrcat( szMsg, szMessageText );
      zstrcat( szMsg, " Name for this Entity." );
   ...
   GetStringFromAttribute( szName, vTZEREMDO, szEntityName, szTE_Attribute );
   ...
   else
   {
      ...
      {
         zstrcpy( szMsg, "Duplicate " );
         zstrcat( szMsg, szTE_MessText );
         zstrcat( szMsg, " for this Entity." );
         MessageSend( vSubtask, "ER00180", "ER Data Model Maintenance", szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
...
zwTZEREMDD_SetCountError( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vTZERDERR, "Import" ) >= zCURSOR_SET )
   {
      ...
      lCountError   = 0;
      lCountWarning = 0;
      for ( nRC = SetCursorFirstEntity( vTZERDERR, "ErrorMsg", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vTZERDERR, "ErrorMsg", "" ) )
      {
...
zwTZEREMDD_ErrorListSaveToFile( zVIEW vSubtask )
{
   ...
   if ( OperatorPromptForFile( vSubtask, szFileName, sizeof( szFileName ), "Text Files (*.txt)|*.txt|", "txt", ulZFlags ) == 1 )
   {
      ...
      zstrcpy( szMsg, "Zeidon:  Data Model Error List from " );
      ...
      for ( nRC = SetCursorFirstEntity( vTZERDERR, "ErrorMsg", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vTZERDERR, "ErrorMsg", "" ) )
      {
         GetStringFromAttribute( szTemp, vTZERDERR, "ErrorMsg", "EntityName" );
         if ( zstrcmp( szTemp, "" ) != 0 )
         {
         ...
         zstrcat( szMsg, "\t" );
         GetStringFromAttribute( szTemp, vTZERDERR, "ErrorMsg", "ErrorText" );
         zstrcat( szMsg, szTemp );
         SysWriteLine( vSubtask, FileHandle, szMsg );
      }
...
EntityPurposeChange( zVIEW vSubtask )
{
   ...
   // only enable dimension hierarchy button for dimensions
   GetStringFromAttribute( szEntityPurpose, vERD, "ER_Entity", "Purpose" );
   if ( *szEntityPurpose == '2' )
      SetCtrlState( vSubtask, "DimHierBtn",  zCONTROL_STATUS_ENABLED, 1 );
   else
      SetCtrlState( vSubtask, "DimHierBtn",  zCONTROL_STATUS_ENABLED, 0 );
...
DimLevelsOutlinePopup( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( sz, vWork, szEntityName, "Name" );
   TraceLineS( "DimLevelsOutlinePopup Name=", sz );
   GetStringFromAttribute( sz, vWork, szEntityName, "ZKey" );
   TraceLineS( "DimLevelsOutlinePopup ZKey=", sz );
   GetStringFromAttribute( sz, vWork, szEntityName, "Desc" );
...
zwTZEREMDD_CompareERDs( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSourceLPLR_Name, vTaskLPLR, "LPLR", "wMergeSourceLPLR_Name" );
   ...
   else
   {
      ...
      GetStringFromAttribute( szERD_Name, vTaskLPLR, "W_MetaDef", "Name" );
   }
...
zwTZEREMDD_SelectCompEntries( zVIEW vSubtask )
{
   ...
   GetViewByName( &vTargetERD, "TZEREMDO", vSubtask, zLEVEL_TASK );
   for ( nRC = SetCursorFirstEntity( vTargetERD, "DisplayCompareResult", "" ); nRC >= zCURSOR_SET; nRC = SetCursorNextEntity( vTargetERD, "DisplayCompareResult", "" ) )
   {
