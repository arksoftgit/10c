...
Copy( zVIEW  vSubtask )
{
   ...
   lRC =
      GetStringFromAttribute( szSourceMetaDirectory,
   ...
   lRC =
      GetStringFromAttribute( szTargetMetaDirectory,
                              vTZVSINIT, "ROOT", "TargetOfSourceCopy" );
   ...
   lRC =
      GetStringFromAttribute( szSourceExeDirectory,
   ...
   lRC =
      GetStringFromAttribute( szTargetExeDirectory,
                              vTZVSINIT, "ROOT", "TargetOfExeCopy" );
   ...
   if ( CompareAttributeToInteger( vTZVSINIT, "ROOT", "IncludeDomains", 1 ) == 0 )
   {
      ...
      while ( k < NBR_DOMAINS )
      {
         zstrcpy( &szSourceMetaDirectory[ lSourceMetaLen ],
                  szDomainFiles[ k ] );
         zstrcpy( &szTargetMetaDirectory[ lTargetMetaLen ],
                  szDomainFiles[ k ] );

         SourceFileHandle =
            SysOpenFile( vSubtask, szSourceMetaDirectory,COREFILE_READ );
         ...
         lBytesRead = SysReadFile( vSubtask, SourceFileHandle,
                                   lpReadBuffer, BUFFER_SIZE );
         lRC = SysWriteFile( vSubtask, TargetFileHandle, lpReadBuffer,
                                  (zUSHORT) lBytesRead );
      ...
      zstrcpy( &szSourceExeDirectory[ lSourceExeLen ], "DOMAINS.IDM" );
      zstrcpy( &szTargetExeDirectory[ lTargetExeLen ], "ZEIDON.XDM" );
      ...
      lRC = SysWriteFile( vSubtask, TargetFileHandle,
   ...
   {
      zstrcpy( &szSourceMetaDirectory[ lSourceMetaLen ], "ZEIDON.PPE" );
      zstrcpy( &szTargetMetaDirectory[ lTargetMetaLen ], "ZEIDON.PPE" );
      ...
      zstrcpy( &szSourceExeDirectory[ lSourceExeLen ], "ZEIDON.XPE" );
      zstrcpy( &szTargetExeDirectory[ lTargetExeLen ], "ZEIDON.XPE" );
