                 ...
                 zVIEW    NewWC )
{
                                  ...
                                  "X_EntityName", "" ) != 0 )
   {
      ...
      nZRetCode = GetStringFromAttribute( szTempString_0, NewW,
                                          "LOD", "Name" );
      ...
      nZRetCode = GetStringFromAttribute( szTempString_0, OrigWC,
               ...
               zVIEW    vSubtask )
{
   ...
   nZRetCode = CreateMetaEntity( vSubtask, NewWC, "CtrlMap", zPOS_AFTER );
   nZRetCode = SetMatchingAttributesByName( NewWC, "CtrlMap", OrigWC, "CtrlMap", zSET_NULL );
   if ( CompareAttributeToString( OrigWC, "CtrlMap", "X_ViewName", "" ) != 0 )
   {
      nZRetCode = GetStringFromAttribute( pchViewName, OrigWC, "CtrlMap", "X_ViewName" );
   }
   if ( ZeidonStringCompare( pchViewName, 1, 0, "", 1, 0, 33 ) )
   {
      if ( CompareAttributeToString( OrigWC, "CtrlMap", "X_EntityName", "" ) != 0 )
      {
         ...
         nZRetCode = IncludeSubobjectFromSubobject( NewWC, "CtrlMapView", NewW, "ViewObjRef", zPOS_AFTER );
         nZRetCode = IncludeMapping( vSubtask, OrigW, OrigWC, NewW, NewWC );
      }
      else
      {
         nZRetCode = SetAttributeFromString( NewWC, "CtrlMap", "X_ViewName", "" );
         nZRetCode = SetAttributeFromString( NewWC, "CtrlMap", "X_EntityName", "" );
         nZRetCode = SetAttributeFromString( NewWC, "CtrlMap", "X_AttributeName", "" );
         nZRetCode = SetAttributeFromString( NewWC, "CtrlMap", "X_Context", "" );
      }
   }
   else
   {
               ...
               zVIEW    vSubtask )
{
   ...
   nZRetCode = GetStringFromAttribute( szViewName, OrigWC, "CtrlMap", "X_ViewName" );
   RESULT = SetCursorFirstEntity( OrigWC, "CtrlMap", "" );
   while ( RESULT > zCURSOR_UNCHANGED )
   {
      nZRetCode = CloneCtrlMapO( OrigW, OrigWC, NewW, NewWC, szViewName, vSubtask );
   ...
   while ( RESULT > zCURSOR_UNCHANGED )
   {
      if ( CompareAttributeToString( OrigWC, "EventAct", "Tag", "" ) != 0 )
      {
         ...
         nZRetCode = GetStringFromAttribute( szTempString_0, OrigWC, "EventAct", "Tag" );
                        ...
                        zVIEW    NewW )
{
   ...
   while ( RESULT > zCURSOR_UNCHANGED )
   {
      ...
      if ( RESULT != zCURSOR_SET && CompareAttributeToString( OrigW, "WndAct", "Tag", "" ) != 0 )
      {
         ...
         nZRetCode = GetStringFromAttribute( szTempString_0, OrigW, "WndAct", "Tag" );
         RESULT = SetCursorFirstEntityByString( NewW, "Action", "Tag", szTempString_0, "" );
         if ( RESULT >= 0 )
         {
               ...
               zVIEW  vSubtask )
{
   ...
   if ( RESULT != 0 )
   {
      ...
      else
      {
         ...
         if ( RESULT >= 0 )
         {
            nZRetCode = GetStringFromAttribute( szTempString_0, OrigW, "LOD", "Name" );
                     ...
                     zVIEW  vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTemp, OrigW, "DfltMenu", "Tag" );
             ...
             zVIEW    vSubtask )
{
   ...
   GetStringFromAttribute( szViewName, OrigWO, "OptMap", "X_ViewName" );
   RESULT = SetCursorFirstEntity( OrigWO, "OptMap", "" );
   while ( RESULT > zCURSOR_UNCHANGED )
   {
      CloneOptMap( OrigW, OrigWO, NewW, NewWO, szViewName, vSubtask );
   ...
   if ( lTemp == zCURSOR_SET )
   {
      ...
      {
         GetStringFromAttribute( szTemp, OrigWO, "OptAct", "Tag" );
         RESULT = SetCursorFirstEntityByString( NewW, "Action", "Tag",
                                                szTemp, "" );
         if ( RESULT >= 0 )
         {
            IncludeSubobjectFromSubobject( NewWO, "OptAct", NewW,
                                           "Action", zPOS_AFTER );
         }
      }
   }
             ...
             zVIEW    vSubtask )
{
   ...
   nZRetCode = SetMatchingAttributesByName( NewWO, "OptMap",
                                            OrigWO, "OptMap", zSET_NULL );
   if ( CompareAttributeToString( OrigWO, "OptMap", "X_ViewName", "" ) != 0 )
   {
      nZRetCode = GetStringFromAttribute( pchViewName, OrigWO,
                                          "OptMap", "X_ViewName" );
   }
   if ( ZeidonStringCompare( pchViewName, 1, 0, "", 1, 0, 33 ) )
   {
      if ( CompareAttributeToString( OrigWO, "OptMap",
                                     "X_AttributeName", "" ) != 0 )
      {
         nZRetCode = PositionOnVOR( OrigW, NewW, pchViewName, vSubtask );
         ...
         if ( CompareAttributeToString( OrigWO, "OptMap", "X_AttributeName", "" ) != 0 )
         {
            ...
            GetStringFromAttribute( szTempString_0, NewW, "LOD", "Name" );
            SetCursorFirstEntityByString( LOD_List, "W_MetaDef",
                                          "Name", szTempString_0, "" );
            ActivateMetaOI( vSubtask, &vLOD, LOD_List, zREFER_LOD_META, zSINGLE );
            GetStringFromAttribute( szTempString_0, OrigWO,
                                    "OptMap", "X_EntityName" );
            RESULT = SetCursorFirstEntityByString( vLOD, "LOD_Entity",
                                                   "Name", szTempString_0, "" );
            if ( RESULT >= 0 )
            {
               nZRetCode = GetStringFromAttribute( szTempString_0, OrigWO,
                                                   "OptMap", "X_AttributeName" );
               RESULT = SetCursorFirstEntityByString( vLOD, "ER_Attribute",
                                                      "Name", szTempString_0, "LOD_Entity" );
               if ( RESULT >= 0 )
               {
                  nZRetCode = IncludeSubobjectFromSubobject( NewWO,
                    "OptMapLOD_Attribute", vLOD, "LOD_Attribute", zPOS_AFTER );
               }
            }
