...
zwTZPEUPDD_DeletePresEnv( zVIEW vSubtask )
{
   ...
   if ( nRC == zRESPONSE_YES )
   {
      ...
      {
         zstrcpy( szERR_Msg, "Corresponding Executable File " );
         zstrcat( szERR_Msg, szFileSpec );
         zstrcat( szERR_Msg, " not found." );
...
// TraceLineS( "!!! IN ", "CreateNewFont" );
   ...
   GetStringFromAttribute( szNewName, vWorkObj, "ROOT", "NewName" );
                          ...
                          zPCHAR   szMessageChildEnitity1 )
{
   ...
   while ( nRC == zCURSOR_SET && !bDeleteError )
   {
      ...
      while ( nRC == zCURSOR_SET && !bDeleteError )
      {
         if ( SetCursorFirstEntityByString(   vPresEnvCopy, szChildName,
                                             "Tag", szTag, "" ) == zCURSOR_SET )
         {
            bDeleteError = TRUE;
            zstrcpy( szMessageParentEntity, "Window Style" );
            GetStringFromAttribute( szParentTag, vPresEnvCopy,
                                    "WindowStyle", "Tag" );
            GetStringFromAttribute( szDefTag, vPresEnvCopy,
                                    szEntityName, "Tag" );
         }
   ...
   if ( !bDeleteError )
   {
      ...
      while ( nRC == zCURSOR_SET && !bDeleteError )
      {
         ...
         while ( nRC == zCURSOR_SET && !bDeleteError )
         {
            ...
            {
               bDeleteError = TRUE;
               zstrcpy( szMessageParentEntity, "Control Definition" );
               GetStringFromAttribute( szParentTag, vPresEnvCopy,
                                       "ControlDef", "Tag" );
               zstrcpy( szMessageChildEnitity, szMessageChildEnitity1 );
               GetStringFromAttribute( szDefTag, vPresEnvCopy,
                                       szEntityName1, "Tag" );
            }
   ...
   if ( bDeleteError )
   {
      zstrcpy( szMessageText, "The " );
      zstrcat( szMessageText, szMessageEntity );
      zstrcat( szMessageText, " cannot be removed, because it " );
      zstrcat( szMessageText, "is referenced by \n" );
      zstrcat( szMessageText, szMessageParentEntity );
      zstrcat( szMessageText, " '" );
      zstrcat( szMessageText, szParentTag );
      zstrcat( szMessageText, "', " );
      zstrcat( szMessageText, szMessageChildEnitity );
      zstrcat( szMessageText, " ' " );
      zstrcat( szMessageText, szDefTag );
      zstrcat( szMessageText, "'." );
...
zwTZPEUPDD_DeleteFont( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vPresEnv, "Font" ) == zCURSOR_SET )
   {
                                      ...
                                      "LastFont", lId + 1 ) == 0 )
      {
         ...
         // delete this Font
         GetStringFromAttribute( szTag, vPresEnv, "Font", "Tag" );
...
zwTZPEUPDD_DeleteColor( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vPresEnv, "Color" ) == zCURSOR_SET )
   {
      ...
      // delete this Color
      GetStringFromAttribute( szTag, vPresEnv, "Color", "Tag" );
...
zwTZPEUPDD_CtrlDefOK( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( &lId, vPresEnv, "ControlDef", "Id" );
   GetStringFromAttribute( szScript, vPresEnv,
                           "ControlDef", "ActiveX_Script" );
   GetStringFromAttribute( szScriptDLL, vPresEnv,
                           "ControlDef", "ActiveX_ScriptDLL" );
   GetStringFromAttribute( szGUID, vPresEnv, "ControlDef", "GUID" );
...
zwTZPEUPDD_SetActiveX_GUID( zVIEW vSubtask )
{
   ...
   GetViewByName( &vPresEnv, "TZPESRCV", vSubtask, zLEVEL_ANY );
   GetStringFromAttribute( szGUID, vTZPEAXWO, "ActiveX", "ClassId" );
...
zwTZPEUPDD_MapDefOK( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szMapDefTag, vPresEnv, "CtrlMapDef", "Tag" );
   ...
   GetStringFromAttribute( szMapDefType, vPresEnv, "CtrlMapDef", "Type" );
...
zwTZPEUPDD_GenerateCodePostBuild( zVIEW vSubtask )
{
   ...
   GetViewByName( &vPresEnv, "TZPESRCV", vSubtask, zLEVEL_ANY );
   GetStringFromAttribute( szTag, vPresEnv, "ControlDef", "Tag" );
...
zwTZPEUPDD_GenerateCode( zVIEW vSubtask )
{
   ...
   GetViewByName( &vTZPEAXWO, "TZPEAXWO", vSubtask, zLEVEL_ANY );
   GetViewByName( &vPresEnv, "TZPESRCV", vSubtask, zLEVEL_ANY );
   GetStringFromAttribute( szTypeLib, vTZPEAXWO, "ActiveX", "TypeLib" );
   GetStringFromAttribute( szScriptOper, vPresEnv,
                           "ControlDef", "ActiveX_Script" );
                            ...
                            zPCHAR szMessageText )
{
   ...
   GetStringFromAttribute( szNewName, vPresEnv, szEntityName, "Tag" );
   zGetStringWithoutBlank( szNewName, szNewName );
