        ...
        LPDOMAINDATA lpDomainData )
{
         ...
         if ( lpDomainData->lpViewAttribute->cType == zTYPE_STRING )
         {
            ...
            switch ( lpDomainData->cType )
            {
               ...
               default:
                  ...
                  zstrcat( szWorkString, lpDomainData->lpViewEntity->szName );
   ...
   //                           (zPCHAR) lpAttrAddr );
   //                zstrncpy( (zPCHAR) lpDomainData->lpData,
   //                          (zPCHAR) lpAttrAddr,
   //                          lpDomainData->uMaxStringLength );
   //                *((zPCHAR) lpDomainData->lpData +
   //                               lpDomainData->uMaxStringLength - 1 ) = 0;

                     *((zPCHAR) lpDomainData->lpData) = 0;
                     if ( nID == eDM_EXPAND_ENVIRONMENT )
                     {
                        szWorkString[ 0 ] = 0;
                        SysConvertEnvironmentString( szWorkString,
                                                     (zPCHAR) lpAttrAddr );
               ...
               case zTYPE_DECIMAL:
                  ...
                  if ( (zPCHAR) lpAttrAddr == 0 ) //null string
                  {
                     SysConvertLongToDecimal( 0,
                                              (zPDECIMAL) lpDomainData->lpData );
                     nRC = -1;   // indicate attribute is null
                  }
                  else
                  {
                     SysConvertStringToDecimal( (zPCHAR) lpAttrAddr,
                                                (zPDECIMAL) lpDomainData->lpData );
      ...
      case zDME_SET_ATTRIBUTE:
         if ( lpDomainData->lpViewAttribute->cType == zTYPE_INTEGER )
         {
            ...
            switch ( lpDomainData->cType )
            {
               case zTYPE_STRING:
                  ...
                  else
                  {
                     ...
                     {
                        sz[ 0 ] = '+';
                        zstrcpy( sz + 1, (zPCHAR) lpDomainData->lpData );
                     }
                 ...
                 SysGetPointerFromHandle( lpContext->hEditString ) )
            {
               ...
               zstrcpy( szEditString, lpEditString );
                 ...
                 LPDOMAINDATA lpDomainData )
{
   ...
   {
      zstrcat( szSavedTableName, lpDomainData->pszContextName );
      lpContextName = lpDomainData->pszContextName;
   }
   else
   {
      GetDefaultContext( &lpContext, lpDomain );
      zstrcat( szSavedTableName, lpContext->szName );
      lpContextName = lpContext->szName;
   }

   nRC = GetViewByName( &vDomainTable, szSavedTableName,
                        lpDomainData->zView, zLEVEL_TASK );
   if ( nRC < 0 )
   {
      zCHAR szMutexName[ 500 ];

      // Make sure we are the only ones accessing the application OI.
      zsprintf( szMutexName, "DomainMutex_%s", szSavedTableName );
      ...
      if ( nRC < 0 )
      {
         fnDyamicT_fnLocalBuildQual_0( lpDomainData->zView,
                                       &vTempViewVar_0, lpContextName );
         nRC = ActivateObjectInstance( &vDomainTable, "DOMAINT",
                                       lpDomainData->zView, vTempViewVar_0,
                                       zSINGLE | zLEVEL_APPLICATION );
         if ( nRC >= 0 )
         {
            SetNameForView( vDomainTable, szSavedTableName, 0,
                            zLEVEL_APPLICATION );
         }
         else
         {
            ...
            zstrcpy( szMsg, "Validation table, " );
            zstrcat( szMsg, szSavedTableName );
            zstrcat( szMsg, ", cannot be loaded." );
      ...
      SetNameForView( vDomainTable, szSavedTableName,
                      lpDomainData->zView, zLEVEL_TASK );
   ...
   switch( lEntryType )
   {
      ...
      case zDME_SET_ATTRIBUTE:
         ...
         if ( lpDomainData->pszContextName && *lpDomainData->pszContextName )
         {
            ...
            // A Context name was passed, so use data value passed in as an
            // external value.
         ...
         {
            // No Context name was passed, treat the value passed as an
            // internal value.
            ...
            case zTYPE_STRING:
               ...
               case zTYPE_STRING:
                  ...
                  SendDomainError( lpDomainData->zView, lpDomain, nSeverity, 25, 0,
                                   lpDomainData->lpViewEntity->szName,
                                   lpDomainData->lpViewAttribute->szName );
                  TraceLineS( "TZDME025 attempting to set value: ",
                              (zPCHAR) lpDomainData->lpData );
                  return( zCALL_ERROR );
               ...
               case zTYPE_DECIMAL:
                  ...
                  if ( nRC >= zCURSOR_SET )
                  {
                     if ( lpDomainData->lpViewAttribute->cType == zTYPE_DECIMAL )
                     {
                        ...
                        SysAssignDecimalFromDecimal( (zPDECIMAL) lpInternalValue,
                                                     &dTempValue );
                     }
                     else
            ...
            case zTYPE_DECIMAL:
               if ( lpDomainData->lpViewAttribute->cType != zTYPE_INTEGER &&
                  lpDomainData->lpViewAttribute->cType != zTYPE_DECIMAL )
               {
                  if ( lEntryType == zDME_VALIDATE_LPDATA )
                     nSeverity = -1;
                  else
      ...
      case zDME_GET_VARIABLE:
         ...
         if ( nRC >= zCURSOR_SET )
         {
            nRC = GetStringFromAttribute( lpDomainData->lpData,
                                          vDomainTable,
                                          "DomainValue",
                                          "ExternalDescription" );
         }
      ...
      case zDME_GET_COMPARE_VALUE:
         if ( lpDomain->cType == zTYPE_STRING )
         {
            ...
            GetStringFromAttribute( lpDomainData->lpData,
                                    vDomainTable,
                                    "DomainValue",
                                    "InternalStringValue" );
      ...
      case zDME_COMPARE_ATTRIBUTE:
         ...
         if ( lpDomain->cType == zTYPE_STRING )
         {
            ...
            nRC = zstrcmp( (zPCHAR) lpDomainData->lpData,
      ...
      case zDME_GET_FIRST_TBL_ENT_FOR_ATTR:
         ...
         if ( nRC >= zCURSOR_SET )
         {
            nRC = GetStringFromAttribute( lpDomainData->lpData, vDomainTable, "DomainValue",
                                          "ExternalDescription" );
            nRC = 0;
         }
         else
...

{
   ...
   switch( lEntryType )
   {
      ...
      case zDME_GET_FIRST_TBL_ENT_FOR_ATTR:
         ...
         if ( nRC >= zCURSOR_SET )
         {
            nRC = GetStringFromAttribute( lpDomainData->lpData, vDomainTable, lpDomainData->pszContextName,
                                          szExternalAttributeName );
            nRC = 0;
         }
         else
