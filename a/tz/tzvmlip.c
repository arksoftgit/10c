...
qbldelta( zLONG  lPatternLth,       // pattern length
          zCPCHAR szPattern,        // pattern to locate
...
zSysTranslateString( zPCHAR szString,
                     zCPCHAR szUpperLower )
{
   SysTranslateString( szString, szUpperLower[ 0 ] );
   return;
}
              ...
              zULONG ulMaxLth )
{
   ...
   if ( nRC == 1 )
   {
      ...
      memcpy( szBuffer, pchReturnLine, nLth + 1 );
   }
                 ...
                 zULONG ulLth )
{
   ...
   if ( nRC == 1 )
   {
      ...
      memcpy( szBuffer, pchReturnLine, ulLth );
   }
                       ...
                       zCPCHAR cpcDelimiters )
{
   ...
   zPCHAR  pchPos;
   zULONG  ulLth, ulOffset, ulOrigLth, ulSearchLth, ulReplaceLth, ulDiffLth;
   zSHORT  nReplaceCnt = 0;
   zCHAR   szDelimiters[ 256 ];

   strcpy( szDelimiters, cpcDelimiters );
   ...
   strcpy( pchTargetCopy, pchTarget );
   pchPos = pchTargetCopy;

   do
   {
      pchPos = strtok( pchPos, szDelimiters );
      if ( pchPos == 0 )   // Search string not found
      {
         break;
      }
      ...
      {
         // Found a match!
         ulOffset = (pchPos - pchTargetCopy) + ulSearchLth;
         if ( strlen( pchTarget + ulOffset ) + ulLth + ulReplaceLth < ulTargetLth )
         {
            nReplaceCnt++;
            strcpy( pchPos, cpcReplace );
            strcpy( pchPos + ulReplaceLth, pchTarget + ulOffset );
            strcpy( pchTarget, pchTargetCopy );
            ulOrigLth = strlen( pchTarget );
         }
         else
         {
                     ...
                     zCPCHAR  szCommand )
{
   ...
   lRc = system( szCommand );

   return lRc;
} // zStartSystemCommand

// Function: zReadLine
// liest eine Zeile eines flat Files und wandelt OEM to CHAR; wird mit
// Zeidon-Operation "SysReadLine" nicht gehändelt
zOPER_EXPORT zLONG OPERATION
           ...
           zLONG    ulFileHandle )
{
   ...
   l = SysReadLine( lpTaskView, &pcData, ulFileHandle );
   if ( l < 1 || pcData == NULL )
      return( l ); // Sorry, there was an error

   if ( (zLONG) strlen( pcData ) >= ulMaxStringLen )
...
{
   zstrcpy( pchTargetString, szSourceString );
...
zGetDayOfYearFromDate( zCPCHAR cpcDateOrDateTime )
{
   ...
   lLeapYear = zYearIsLeapYear( (zSHORT) lYearPart );
   lDayCnt = 0;
   lMonthCnt = 1;

   while ( lMonthCnt < lMonthPart )
   {
      if ( lMonthCnt == 1 || lMonthCnt == 3 || lMonthCnt == 5 ||
           lMonthCnt == 7 || lMonthCnt == 8 || lMonthCnt == 10 ||
           lMonthCnt == 12 )
      {
         lDayCnt = lDayCnt + 31;
      }
                        ...
                        zUSHORT  uDayOfYear )
{
   ...
   lLeapYear = zYearIsLeapYear( uYear );
   ...
   return( 0 );
} // zGetDateFromDayOfYear0

// Function: zStringIsDate
// prueft, ob ein String ein gueltiges Datum
// ist
zOPER_EXPORT zSHORT OPERATION
zStringIsDate( zCPCHAR szSourceString )
{
   ...
   lRC = zGetDayOfYearFromDate( szSourceString );
                   ...
                   zPCHAR  pchTargetDate )
{
   ...
   ZeidonStringCopy( szYearPart, 1, 0, szSourceDate, 1, 4, 5 );
   ZeidonStringCopy( szMonthPart, 1, 0, szSourceDate, 5, 2, 3 );
   ZeidonStringCopy( szDayPart, 1, 0, szSourceDate, 7, 2, 3 );
   lYearPart = zStringToInteger( szYearPart );

   k = zGetDayOfYearFromDate( szSourceDate );
   ...
   if ( k == 1 )
   {
      ...
      ZeidonStringCopy( pchTargetDate, 1, 0, szYearPart, 1, 0, 33 );
      ZeidonStringConcat( pchTargetDate, 1, 0, szMonthPart, 1, 0, 33 );
      ZeidonStringConcat( pchTargetDate, 1, 0, szDayPart, 1, 0, 33 );
      ZeidonStringConcat( pchTargetDate, 1, 0, "000000000", 1, 0, 33 );
      lLeapYear = zYearIsLeapYear( (zSHORT) lYearPart );
   ...
   lRC = zGetDateFromDayOfYear( pchTargetDate,
                                (zUSHORT) lYearPart, (zUSHORT) k );
                   ...
                   zPCHAR  pchTargetDate )
{
   ...
   k = k + 1;
   lRC = zGetDateFromDayOfYear( pchTargetDate, (zUSHORT) lYearPart,
                                (zUSHORT) k );
                       ...
                       zPCHAR   pchMonthName )
{
   ...
   //Test, ob gueltiges Datum uebergeben wurde
   bOk = zStringIsDate( szSourceDate );
   if ( bOk != 1 )
      return( -1 );
   ...
   if ( ZeidonStringCompare( szMonthPart, 1, 0, "01", 1, 0, 3 ) == 0 )
      ZeidonStringCopy( pchMonthName, 1, 0, "Januar", 1, 0, 33 );
   else
      if ( ZeidonStringCompare( szMonthPart, 1, 0, "02", 1, 0, 3 ) == 0 )
         ZeidonStringCopy( pchMonthName, 1, 0, "Februar", 1, 0, 33 );
      else
         if ( ZeidonStringCompare( szMonthPart, 1, 0, "03", 1, 0, 3 ) == 0 )
            ZeidonStringCopy( pchMonthName, 1, 0, "März", 1, 0, 33 );
         else
            if ( ZeidonStringCompare( szMonthPart, 1, 0, "04", 1, 0, 3 ) == 0 )
               ZeidonStringCopy( pchMonthName, 1, 0, "April", 1, 0, 33 );
            else
               if ( ZeidonStringCompare( szMonthPart, 1, 0, "05", 1, 0, 3 ) == 0 )
                  ZeidonStringCopy( pchMonthName, 1, 0, "Mai", 1, 0, 33 );
               else
                  if ( ZeidonStringCompare( szMonthPart, 1, 0, "06", 1, 0, 3 ) == 0 )
                     ZeidonStringCopy( pchMonthName, 1, 0, "Juni", 1, 0, 33 );
                  else
                     if ( ZeidonStringCompare( szMonthPart, 1, 0, "07", 1, 0, 3 ) == 0 )
                        ZeidonStringCopy( pchMonthName, 1, 0, "Juli", 1, 0, 33 );
                     else
                        if ( ZeidonStringCompare( szMonthPart, 1, 0, "08", 1, 0, 3 ) == 0 )
                           ZeidonStringCopy( pchMonthName, 1, 0, "August", 1, 0, 33 );
                        else
                           if ( ZeidonStringCompare( szMonthPart, 1, 0, "09", 1, 0, 3 ) == 0 )
                              ZeidonStringCopy( pchMonthName, 1, 0, "September", 1, 0, 33 );
                           else
                              if ( ZeidonStringCompare( szMonthPart, 1, 0, "10", 1, 0, 3 ) == 0 )
                                 ZeidonStringCopy( pchMonthName, 1, 0, "Oktober", 1, 0, 33 );
                              else
                                 if ( ZeidonStringCompare( szMonthPart, 1, 0, "11", 1, 0, 3 ) == 0 )
                                    ZeidonStringCopy( pchMonthName, 1, 0, "November", 1, 0, 33 );
                                 else
                                    if ( ZeidonStringCompare( szMonthPart, 1, 0, "12", 1, 0, 3 ) == 0 )
                                       ZeidonStringCopy( pchMonthName, 1, 0, "Dezember", 1, 0, 33 );
                                    else
                                       bOk = -1;
                  ...
                  zLONG    ulValue )
{
   ...
   *pchTargetString = 0;
   ...
   if ( ulExpectedLength > ulTargetStringLength )
   {
      nRectValue = -1;
   }
   else
   {
      ...
      zltoa( ulValue, szBuffer );
      zstrcpy( pchTargetString, szBuffer );
   }
                        ...
                        zPCHAR   pchTargetString )
{
   ...
   i = 0;
   while ( szSourceString[ i ] == ' ' )
      i++; // skip over leading blanks

   if ( szSourceString[ i ] == 0 )
   {
      pchTargetString[ 0 ] = 0; // target is empty
      return( 0 );
   }

   j = zstrlen( szSourceString ) -1;
   while ( szSourceString[ j ] == ' ' )
      j--; // skip over trailing blanks

   memcpy( pchTargetString, szSourceString + i, j - i + 1 );
   pchTargetString[ j - i + 1 ] = 0;
   return( j - i + 1 );
...
// liefert die Länge eines Strings ohne vorangestellte und ohne nachfolgenden
// Blanks
zOPER_EXPORT zLONG OPERATION
...
static zLONG
qfind( zLONG  lPatternLth,         // pattern length
       zCPCHAR szPattern1,         // pattern to locate
       zLONG lBufferLth,          // length of buffer to be searched
       zPCHAR Buffer,            // buffer to be searched for pattern
       zPLONG lplDelta1Table,    // Boyer-Moore Delta1 Table
       zPLONG lplDelta2Table )   // Boyer-Moore Delta2 Table
                           ...
                           zCPCHAR szAttributeName )
{
   ...
   // by the string szTextValue. Return a 0 if szTextValue was found in the
   ...
   lPatternLth = zstrlen( szTextValue );
   if ( lPatternLth == 0 )
   {
      return( -1 );
   }
   ...
   GetAddrForAttribute( &lpBuffer, vView, szEntityName, szAttributeName );
                            ...
                            zPCHAR pchEntityName )
{
   ...
   zstrcpy( pchEntityName, lpViewEntity->szName );
   return( 0 );
...
{
   LPVIEWENTITY lpViewEntity;
   zCHAR EntityNameSave[ 33 ];
   LPVIEWOD lpViewOD;

   zstrcpy( EntityNameSave, pchEntityName );

   if ( (zstrcmp( pchEntityName, "" ) == 0) || (zstrcmp( AnsiUpper(EntityNameSave), "ROOT" ) == 0 ) )
                                 ...
                                 zPCHAR   pchAttributeName )
{
   ...
   lpViewOD = (LPVIEWENTITY) MiGetViewEntityForView( vViewToInstance, szEntityName );
   if ( !lpViewOD )
      return( -16 );
   ...
   if ( lpViewAttrib )
      zstrcpy( pchAttributeName, lpViewAttrib->szName );
   else
      return( -1 );
                                ...
                                zPCHAR  pchAttributeName )
{
   ...
   if ( zstrcmp( pchAttributeName, "" ) == 0 )
      return(zGetFirstAttributeNameForEntity( vViewToInstance, szEntityName, pchAttributeName ));
                    ...
                    zCPCHAR cpcEntityNameToSearch )
{
   ...
   while ( nRC >= zCURSOR_SET &&
         ZeidonStringCompare( cpcEntityNameToSearch, 1, 0, szEntityName, 1, 0, 33 )
         != 0 )
   {
                       ...
                       zCPCHAR cpcAttributeNameToSearch )
{
   ...
   while ( nRC >= zCURSOR_SET &&
      ZeidonStringCompare( cpcAttributeNameToSearch, 1, 0, szAttributeName, 1, 0, 33 ) != 0 )
   {
                  ...
                  zSHORT  nPosition )
{
   ...
   nLthSearchString = zstrlen( cpcSearchString );
   nLthSubString = zstrlen( cpcSubString );
   ...
   if ( *cpcDirection == 'f' || *cpcDirection == 'F' )
   {
      ...
      {
          if ( zstrncmpi( cpcSearchString + nLoc,
                          cpcSubString, nLthSubString ) == 0 )
         {
            return( nLoc );
         }
      }
                   ...
                   zCPCHAR cpcRepString ) // replace string
{
   ...
   nRepLth = zstrlen( cpcRepString );
   if ( nRepLth > nModRepLth )
   {
      // result will be longer, create space by moving the tail
      //  towards string end.
      zSHORT nSpaceNeeded = nRepLth - nModRepLth;
      pc = pchModString + zstrlen( pchModString ); // points to end 0
      while ( pc >= pchModString + nEndPos )
      {
               ...
               zPCHAR  pchReturnString )
{
  ...
  h = FindFirstFile( cpcFileString,
                     lpFindFileData );

  if ( lpFindFileData )
  {
     strcpy( pchReturnString, lpFindFileData->cFileName );
  }
              ...
              zPCHAR pchReturnString )
{
  ...
  bRC = FindNextFile( (HANDLE)lHandle, lpFindFileData );
            ...
            zPCHAR pchResultString, zUSHORT uResultStringLth )
{
   ...
   // Use zstrcpy for the first 'replication'.
   zstrcpy( pchResultString, cpcString );

   // Append the remaining 'replications' using zstrcat.
   for ( k = 1; k < uRepeats; k++ )
   {
      zstrcat( pchResultString, cpcString );
   }
...
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
zRTrim( zCPCHAR String, zPCHAR ResultString, zUSHORT ResultStringLth )
{
   zLONG i = 0, len;
   zCPCHAR pString = String;
   ...
   // calculate new string's length and, if the outstring is large enough,
   // copy it.
   len = (String-pString)+1;
   if (ResultStringLth<len+1) return( -1 );
   strncpy( ResultString, pString, len );
   ResultString[len] = 0;
...
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
zLTrim( zCPCHAR String, zPCHAR ResultString, zUSHORT ResultStringLth )
{
   zLONG i = 0;
   ...
   // calculate new string's length and, if the outstring is large enough,
   // copy it.
   if ( ResultStringLth < (strlen( String ) + 1) )
      return( -1 );
...
// last changed:   15.January 2002
// description:
//                 puts the 'Numleft' leftmost characters of 'String' in 'ResultString'.
...
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
zLeft( zCPCHAR String, zUSHORT Numleft,
       zPCHAR ResultString, zUSHORT ResultStringLth )
{
   // check if 'Numleft' is within range
   if ( Numleft > strlen( String ) )
      Numleft = strlen( String );

   // check if ResultString is large enough
   if ( ResultStringLth < (Numleft + 1) )
      return( -1 );

   // copy and append terminating null
   strncpy( ResultString, String, Numleft );
...
/////////////////////////////////////////////////////////////////////////////
zOPER_EXPORT zSHORT OPERATION
zRight( zCPCHAR String, zUSHORT Numright,
        zPCHAR ResultString, zUSHORT ResultStringLth )
{
   zUSHORT uLth = strlen( String );
...
IsNumeric( zCPCHAR String )
{
   ...
   ulStringLth = strlen( String );
   if ( ulStringLth == 0 )
      return( zCALL_ERROR );

   if ( strcspn( String, "1234567890" ) == ulStringLth )
      return( -2 );
...
zSoundex( zCPCHAR String, zPCHAR SoundexString )
{
   ...
   {
      if ( isalpha( String[ i ] ) == 0 || String[ i ] == 'ß' )
      {
         return( -1 );
      }
   }
   ...
   for ( i = 0; i < strlen( String ) && szCount < 4; i++ )
   {
      ch = ztoupper( String[ i ] );
      ...
      if ( szCount == 0 )
      {
         SoundexString[ szCount ] = ztoupper( String[ i ] );
         szCount++;
         lastch = ch;
         continue;
      }
                     ...
                     zSHORT nResultStringMaxLth )
{
   ...
   if ( pch )
   {
      ...
      *pch = 0;
      nLth = zstrlen( pchSource );
      if ( nLth <= nResultStringMaxLth )
      {
         zstrcpy( pchResultString, pchSource );
         zstrcpy( pchSource, pch + 1 );
         return( nLth );
      }
      else
         nRC = -2;
                     ...
                     zPCHAR pchDfltQuoteChar )
{
   ...
   if ( nLth == 0 )
   {
      if ( pchDfltQuoteChar && pchDfltQuoteChar[ 0 ] )
      {
         ...
         {
            zstrcpy( pchTarget, pchSkipString );
            nLth = zstrlen( pchTarget );
         }
      ...
      else
      {
         ...
         zstrcpy( pchTarget, pchAppendString );
         nLth = zstrlen( pchTarget );
         return( nLth );
      }
   }
   else
   {
      ...
      if ( pchSkipString && pchSkipString[ 0 ] )
      {
         zSHORT nSkipLth = zstrlen( pchSkipString );
         ...
         if ( pchDfltQuoteChar && pchDfltQuoteChar[ 0 ] )
         {
            ...
            {
               // It's already quoted.
               zstrcpy( szTemp, pchSkipString );
               zstrcat( szTemp, pchTarget );
            ...
            else
            {
               // Needs to be quoted.
               zstrcpy( szTemp, pchSkipString );
               zstrcat( szTemp, pchDfltQuoteChar );
               zstrcat( szTemp, pchTarget );
               zstrcpy( pchTarget, szTemp );
               nLth = zstrlen( pchTarget );
               pchTarget[ nLth++ ] = ch;
   ...
   if ( nLth > 0 && pchTarget[ nLth - 1 ] == ch )
      nLth--;
   else
      ch = 0;

   zstrcpy( pchTarget + nLth, pchAppendString );
   nLth = zstrlen( pchTarget );
   if ( ch )
   {
      pchTarget[ nLth++ ] = ch;
   ...
   {
      zstrcpy( pchTarget, pchSkipString );
      zstrcat( pchTarget, pchDfltQuoteChar );
      zstrcat( pchTarget, pchAppendString );
      nLth = zstrlen( pchTarget );
      pchTarget[ nLth++ ] = pchDfltQuoteChar[ 0 ];
      pchTarget[ nLth ] = 0;
   }
