...
StepOnce( zVIEW vSubtask, zPVOID hWRKS, zPLONG plRetVal )
{
   ...
   switch( lClass )
   {
      ...
      case qELSE:
         ...
         GetStringFromAttribute( szIfConditionResult, g_vStatementView,
                                 "Statement", "IfConditionResult" );
...
// else
   {
      ...
      if ( nRC != zCURSOR_SET )
      {
         ...
         zstrcpy( szMsg, szOperName );
         zstrcat( szMsg, " could not be located in the VML code, re-parse" );
