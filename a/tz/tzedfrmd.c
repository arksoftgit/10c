...
FindBeginOfOperation( zVIEW vSubtask, LPCSTR szOperationName, ZDrTBEdt *oEditor )
{
   ...
   GetStringFromAttribute( szExtension, vEdWrk, szlBuffer, szlLanguageType );
                                ...
                                zPCHAR   pchFileName )
{
                                  ...
                                  pchFileName, szSourceFileName, FALSE ) >= 0 )
   {
            ...
            nRC = SetCursorNextEntity( vDeleteOp, "Operation", "" ) )
      {
         ...
         lOpStartIndex  = -1;
         lOpEndIndex    = -1;
         GetStringFromAttribute( szOperName, vDeleteOp, "Operation", "Name" );
...
TZEDFRMD_DropOperationFromSource( zVIEW vSubtask )
{
   ...
   if ( InitEditorForMoveAndDelete( oEditor, vSubtask, &vDeleteOp, &vSource,
                                    &vEdWrk, &vProfileXFER, szFileName ) < 0 )
   {
      return( -1 );
   }
...
OperTemplate( ZDrTBEdt *oEditor, zVIEW vSource, zVIEW vSubtask )
{
         ...
         nRC = SetCursorNextEntity( vSource, "Parameter", "" ) )
   {
      ...
      GetStringFromAttribute( szParam + nLth, vSource, "Parameter", "ShortDesc" );
      ...
      switch ( *szDataType )
      {
         case 'V':
                   ...
                   *szType == 'C' || *szType == 'O' ) )
            {
               ...
               zstrcat( szLine, " BASED ON LOD " );
               zstrcat( szLine, szLOD );
               if ( bFileC )
                  zstrcat( szLine, "  */" );
            }
         ...
         case 'N':
            zstrcat( szLine, "SHORT ");
            zstrcat( szLine, szParam );
            break;

         case 'M':
            zstrcat( szLine, "DECIMAL ");
            zstrcat( szLine, szParam );
            break;
         ...
         case 'L':
            bFileC ? zstrcat( szLine, "LONG " )
                   : zstrcat( szLine, "INTEGER " );
            zstrcat( szLine, szParam );
            break;

         case 'O':
            bFileC ? zstrcat( szLine, "VOID " )
                   : zstrcat( szLine, "VOID " );
            zstrcat( szLine, szParam );
            break;

         case 'I':
            bFileC ? zstrcpy( szLine, "LPDOMAINDATA " )
                   : zstrcpy( szLine, "STRING ( 32 ) " );
            zstrcat( szLine, szParam );
            break;
         ...
         default:
            if ( bFileC )
            {
               if ( !bPointer && GetVMLConstChar( ) )
                  zstrcat( szLine, "CPCHAR " );
               else
                  zstrcat( szLine, "PCHAR " );
            }
            else
               zstrcat( szLine, "STRING ( 32 ) " );
   ...
   if ( bFileC )
   {
      ...
      if ( *szReturnDataType == 'O' ||
         *szReturnDataType == 0 ) // empty data type handled as void
         zstrcat( szLine, "\treturn;\r\n" );
      else
         zstrcat( szLine, "\treturn( 0 );\r\n" );

      zstrcat( szLine, "} // ");
      zstrcat( szLine, szOperName );
      zstrcat( szLine, "\r\n" );
   }
   else
   {
...

{
   ...
   UfFormatDateTime( szDateFormatted, szDateRaw, "DD-MM-YYYY" );
                    ...
                    zPCHAR szMsgText )
{
   ...
   if ( zstrcmp( pszInvokingTool, "GO" ) == 0 )
      zstrcpy( szComponentType, "Global Operation Group" );
   else
   if ( zstrcmp( pszInvokingTool, "DM" ) == 0 )
      zstrcpy( szComponentType, "Domain Group" );
   else
   if ( zstrcmp( pszInvokingTool, "ZO" ) == 0 )
      zstrcpy( szComponentType, "LOD" );
   else
      zstrcpy( szComponentType, "Dialog" );

   zstrcpy( szMsg, "Cannot create " );
   zstrcat( szMsg, szMsgText );
   zstrcat( szMsg, ", because " );
   zstrcat( szMsg, szComponentType );
                         ...
                         zPLONG plMetaType )
{
   ...
   if ( zstrcmp( pszInvokingTool, "GO" ) == 0 )
   {
      ...
      *plMetaType = zSOURCE_GOPGRP_META;
      GetStringFromAttribute( szMetaName, vSource, "GlobalOperationGroup", "Name" );
   }
   else
   // Domains
   if ( zstrcmp( pszInvokingTool, "DM" ) == 0 )
   {
      ...
      *plMetaType = zSOURCE_DOMAINGRP_META;
      GetStringFromAttribute( szMetaName, vSource, "DomainGroup", "Name" );
   }
   else
   // LOD's
   if ( zstrcmp( pszInvokingTool, "ZO" ) == 0 )
   {
      ...
      *plMetaType = zSOURCE_LOD_META;
      GetStringFromAttribute( szMetaName, vSource, "LOD", "Name" );
   }
   else
        ...
        zstrcmp( pszInvokingTool, "PN" ) == 0 )
   {
      ...
      *plMetaType = zSOURCE_DIALOG_META;
      GetStringFromAttribute( szMetaName, vSource, "Dialog", "Tag" );
   }
                ...
                zPBOOL  pbFileExists )
{
   ...
   //
   if ( zstrcmp( pszInvokingTool, "GO" ) == 0 )
      zstrcpy( szSourceFileEntityName, "GlobalOperationGroup" );
   else
   if ( zstrcmp( pszInvokingTool, "DM" ) == 0 )
      zstrcpy( szSourceFileEntityName, "DomainGroup" );
   else
      zstrcpy( szSourceFileEntityName, szlSourceFile );

   // Get the source dir from the lplr view.
   GetStringFromAttribute( pchFileName, vTaskLPLR, szlLPLR, szlPgmSrcDir );
   ...
   if ( GetStringFromAttribute( &pchFileName[ zstrlen( pchFileName ) ],
                                vSource, szSourceFileEntityName,
                                szlSubdirectory ) >= 0 )
   {
      SysAppendcDirSep( pchFileName );
   }
   ...
   zstrcpy( szFileName, pchFileName );
   SysConvertEnvironmentString( pchFileName, szFileName );
...
InitSession( zVIEW  vSubtask )
{
   ...
   // Don't know if this is the best way to do this, but it's better than an
   // access violation.
   if ( oEditor == 0 )
      return( -1 );
   ...
   if ( vSource )
   {
      ...
      if ( CreateFileName( vTaskLPLR, vSource, szFileName, pszInvokingTool,
   ...
   else
   {
      ...
      GetStringFromAttribute( szFileName, vEdWrk, szlEditor, "OpenFileName" );
   ...
   SetAttributeFromInteger( vEdWrk, szlBuffer, szlSourceViewID,
                            (zLONG) vSource );
   ...
   GotoCurrentOperation( vSubtask, vSource, oEditor, szOperSrch,
                         szMetaName, bCFile );
   return( 0 );
                     ...
                     zSHORT   nAskForParse )
{
   ...
   else
   {
                                     ...
                                     szlVML_File ) == 0 )
      {
         ...
         GetStringFromAttribute( pchFileName,  vEdWrk, szlBuffer, szlFileName );
                              ...
                              zSHORT nAskForParse )
{
   ...
   if ( g_bIsFileNew == FALSE)
   {
      ...
      GetStringFromAttribute( szFileName, vEdWrk, szlBuffer, szlFileName );
   }
   else
   {
      // This is a good name for a new source.
      zstrcpy( szFileName, "Noname.vml" );
   }

   zsprintf( szMsg, "File '%s' has changed.  Do you want to save it?",
             szFileName );
   ...
   {
      if ( fnTZEDFRMD_SaveFile( vSubtask, oEditor, szFileName, nAskForParse ) < 0 )
      {
         SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
         return( -1 );
      }
...
AEQ_GetViews( zVIEW vSubtask )
{
   ...
   if ( vEdWrk == 0 )
   {
      zstrcpy( sz, "TZEDWRKO not Available at GetViews." );
   ...
   if ( CompareAttributeToString( vEdWrk, szlBuffer,
                                  szlLanguageType, szlVML_File ) != 0 )
   {
      zstrcpy( sz, "'Get Views' only works on VML Files." );
      OperatorPrompt( 0, "ED - Error", sz, 0, zBUTTONS_OK, 0, 0 );
      return( zCALL_ERROR );
   }
   ...
   do
   {
      ...
      {
         zstrcpy( sz, "Couldn't find beginning of operation" );
   ...
   for ( nViewType = 0; nViewType < 2; nViewType++ )
   {
                                       ...
                                       SRCH_FORWARD | SRCH_REGEXPR ) )
      {
         ...
         if ( nViewType == 1 )
         {
            ...
            if ( nRC < 0 )
            {
               zstrcpy( sz, "Can't find Registered View with name " );
               zstrcat( sz, szObjectName );
               OperatorPrompt( 0, szlZeidonEditor, sz, 0, zBUTTONS_OK, 0, 0 );
               continue;
            }

            // Get Object name from VOR.
            GetStringFromAttribute( szObjectName, vVor, "LOD", szlName );
                  ...
                  zBOOL  bInsertWithQuote )
{
   ...
   if ( vEdWrk == 0 )
   {
      zstrcpy( sz, "TZEDWRKO not Available at PasteQualifier." );
      OperatorPrompt( 0, "ED - Error", sz, 0, zBUTTONS_OK, 0, 0 );
      return( zCALL_ERROR );
   }
   ...
   if ( CompareAttributeToString( vEdWrk, szlBuffer,
                                  szlLanguageType, szlVML_File ) != 0 )
   {
      zstrcpy( sz, "Paste only works for VML Files." );
      OperatorPrompt( 0, szlZeidonEditor, sz, 0, zBUTTONS_OK, 0, 0 );
      return( zCALL_ERROR );
   }
...
OpIns_RefreshSelectButton( zVIEW vSubtask )
{
   ...
   mGetWorkView( &vEdWrk, vSubtask );
   GetStringFromAttribute( szCurrentType, vEdWrk, "OperListType", "Type" );
...
OpIns_InsertOperation( zVIEW vSubtask )
{
   ...
   mGetWorkView( &vEdWrk, vSubtask );
   GetStringFromAttribute( szType, vEdWrk, szlOperListType, szlType );
   switch ( *szType )
   {
      case 'Z':
      {
      ...
      case 'G':
      {
         ...
         GetStringFromAttribute( szOpName, vEdWrk, szlOper, szlName );
      ...
      case 'O':
      {
         ...
         GetStringFromAttribute( szObjectName, vEdWrk, szlOper, szlName );
      ...
      case 'X':
      {
         ...
         GetStringFromAttribute( szBuffer, vEdWrk, szlOper, szlName );
...
#define SEL_SELECT_SET     2
   ...
   GetStringFromAttribute( szCurrentType, vEdWrk, szlBuffer,
                           "OpIns_CurrentType" );
   ...
   GetStringFromAttribute( szKeyword, vEdWrk, szlBuffer, "OpIns_Keyword" );
   if ( *szKeyword &&
                                  ...
                                  szlBuffer, "OpIns_UseKeyword", "Y" ) == 0 )
   {
      ...
      GetStringFromAttribute( szIgnoreCase, vEdWrk, szlBuffer,
...
VML_DisplayList( zVIEW vSubtask )
{
   ...
   zsprintf( szMsg, "Listing %ld of %ld", lSelectedCount, lEntityCount );
                  ...
                  ZDrTBEdt   *oEditor )
{
   ...
   {
      GetStringFromAttribute( szInsertString, vProfileXFER,
...
// DropView( vTempEdWrk );
   ...
   GetParentWindow( &vParentSubtask, vSubtask );
   SetWindowActionBehavior( vParentSubtask,
                            zWAB_StartModelessSubwindow |
...
SaveFile( zVIEW vSubtask )
{
   ...
   if ( lChanged && !oEditor->IsReadOnly( ) )
   {
      ...
      if ( fnTZEDFRMD_SaveFile( vSubtask, oEditor, szFileName, 0 ) < 0 )
      {
         SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
         return( -1 );
      }
...
FileSaveAs( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szExtension, vEdWrk, szlBuffer, szlLanguageType);
      zstrcpy( szFileName, "NONAME." );
      if (zstrcmp(szExtension, szlC_File) != 0)
         zstrcpy(szExtension, szlVML_FileExtension);
      zstrcat( szFileName, szExtension );
   }
   ...
   {
      g_bIsFileNew = FALSE;
      zsprintf( szMsg, "File saved as '%s'.", szFileName);
...
SaveAndParse( zVIEW vSubtask, zCPCHAR cpcGenLang )
{
   ...
   if ( vEdWrk == 0 )
   {
      zstrcpy( sz, "TZEDWRKO not Available at _zfParseFile." );
      MessagePrompt( vSubtask, "ED0001", szlZeidonEditor, sz, 1,
                        0, 0, zICON_EXCLAMATION );
      return( 0 );
   }
   ...
   if ( CompareAttributeToString( vEdWrk, szlBuffer, szlLanguageType,
                                  szlVML_File ) != 0 )
   {
      zstrcpy( sz, "Parse only valid with VML Files." );
   ...
   *psz = 0;
   ...
   if ( nRC < 0 )
   {
      ...
      {
         zstrcpy( sz, "Parse completed with Errors.\n\n"
   ...
   else
   {
      ...
      sprintf( sz, "Parse completed successfully for: '%s.VML'", szFileName );
      MB_SetMessage( vSubtask, MAIN_DIL, sz );
      MessagePrompt( vSubtask, "ED0003", szlZeidonEditor,
...
Generate( zVIEW vSubtask, zCPCHAR cpcGenLang )
{
   ...
   if ( vEdWrk == 0 )
   {
      zstrcpy( sz, "TZEDWRKO not Available at _zfGenerateCFile." );
   ...
   *psz = 0;
   ...
   {
      zstrcpy( sz, "Generate completed successfully." );
      MessagePrompt( vSubtask, "ED0003", szlZeidonEditor,
                     sz, 0, 0, 0, zICON_INFORMATION );
      nRC = 0;
   }
...
OpIns_StartOperationHelp( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szOperName, vEdWrk, "Oper", "Name" );
   zstrcpy( szOperTag, "fn " );
   zstrcat( szOperTag, szOperName );
                 ...
                 zLONG    lPosition )
{
   ...
   GetStringFromAttribute( szOperation, vDeleteOp, "Operation", "Name" );

   SetCursorNextEntity( vDeleteOp, "SourceFile", "" );
   if ( SetCursorFirstEntity( vDeleteOp, "Operation", "" ) < zCURSOR_SET )
      return( 0 );

   GetStringFromAttribute( szReplaceText, vDeleteOp, "Operation", "Name" );
                            ...
                            zPCHAR   pchFileName )
{
   ...
   // registry problem. Don't know if this is the best way to do that
   // but it's better than an access-violation
   if ( oEditor == 0 )
      return( -1 );
   ...
   GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szInvokingTool, *pvProfileXFER, szlED, "InvokingTool" );
                          ...
                          zBOOL    bCreateFile )
{
   ...
   zstrcpy( szSourceFileName, pchFileName );

   // Get the subdir from the source file entity.  If GetString... returns < 0
   // then the subdir attribute is null, so nothing was retrieved.  If subdir
   // is set, then we need to add a '\'.
   if ( GetStringFromAttribute( &szSourceFileName[ zstrlen( szSourceFileName ) ],
   ...
   if ( bCreateFileInfo &&
        CompareAttributeToString( vDeleteOp, "SourceFile", "LanguageType",
                                  szlC_File ) == 0 )
   {
      GetStringFromAttribute( szName, vDeleteOp, "SourceFile", "Name" );
...
SetUpdateFlagInMeta( zVIEW vSubtask )
{
   ...
   if ( SetCursorFirstEntityByInteger( vActiveMetas, "W_MetaDef", "CPLR_ZKey",
        ...
        CompareAttributeToInteger( vActiveMetas, "W_MetaDef", "Status",
                                   1 ) == 0 )
   {
      GetStringFromAttribute( szUpdateInd, vActiveMetas, "W_MetaDef", "UpdateInd" );
