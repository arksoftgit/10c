...
oTZOPSIGO_RemoveOperFromXGO( zVIEW  vOperation )
{
   ...
   GetStringFromAttribute( szOperationName, vOperation, "Operation", "Name" );
...
// copy complete structure of the Operation Object to the XGO Group.
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      if ( vTmpHdr != 0 )
      {
         ...
         {
            zstrcpy( sz, "Do you wish to save Header '" );
            GetStringFromAttribute( &sz[ zstrlen( sz ) ],
                                    vTmpHdr, "HeaderFile", "Name" );
            zstrcat( sz, "'?" );
            nRC = MessagePrompt( vOperation, "OP01101",
                                 "Operations",
                                 sz,
...
//         1 - unique
   ...
   nRC = GetStringFromAttribute( szHdrFileName, vHdrFile,
                                 "HeaderFile", "Name" );
...
oTZOPSIGO_RemoveHdrFromXGO( zVIEW vHdrFile )
{
   ...
   GetStringFromAttribute( szHdrFileName, vHdrFile, "HeaderFile", "Name" );
                            ...
                            zSHORT       nMsg )
{
   ...
   if ( nRC > 0 )
   {
      ...
      if ( SetCursorFirstEntityByInteger( vTaskLPLR, "W_MetaDef",
                                          ...
                                          "LPLR" ) >= zCURSOR_SET )
      {
         GetStringFromAttribute( szSRC_Name, vTaskLPLR,
                                 "W_MetaDef", "Name" );
      }
   }
