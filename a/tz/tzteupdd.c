...
zwTZTEUPDD_SwitchLPLR( zVIEW vSubtask )
{
   ...
   if ( nRC > 0 )
   {
      ...
      {
         zstrcpy( szWorkMessage, "Do you wish to save current Physical Model?" );
...
zwTZTEUPDD_SaveModelDTE( zVIEW  vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szWorkMessage, "Unable to Access Physical Model. "
...
zwTZTEUPDD_RebuildTablesRels( zVIEW vSubtask )
{
   ...
   if ( CountEntitiesForView( vDTE, "TE_DBMS_Source" ) > 1 )
   {
      ...
      zstrcpy( sz, "Do you want to initialize Data Source '" );
      zstrcat( sz, szDS );
      zstrcat( sz, "' with all Entities in the ER Model that:" );
...
// zSHORT    (POPERATION pfn)( zVIEW, zVIEW, zPCHAR, zVIEW );
   ...
   GetStringFromAttribute( szFileName, vDTE, "TE_DBMS_Source", "GenerateExecutable" );
...
zwTZTEUPDD_DeleteAllTablRecs( zVIEW    vSubtask )
{
   ...
   zstrcpy( szWorkMessage, "OK to delete all Tables/Records "
...
zwTZTEUPDD_SyncPreBuild( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szMaxCardinality,
                               ...
                               zVIEW vEMD )
{
   ...
   GetStringFromAttribute( szTargetEntityName, vEMD, "ER_Entity_2", "Name" );
   ...
   GetStringFromAttribute( szRelationshipName, vEMD2, "ER_RelLink_2", "Name" );
   GetStringFromAttribute( szRelationshipText, vEMD2, "ER_Entity_2", "Name" );
   zstrcat( szRelationshipText, " - " );
   zstrcat( szRelationshipText, szRelationshipName );
   zstrcat( szRelationshipText, " - " );
   zstrcat( szRelationshipText, szTargetEntityName );
   ...
   while( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szFieldType, vDTE2, "TE_FieldDataRel", "DataOrRelfieldOrSet" );
      if ( szFieldType[ 0 ] == 'R' )
...
zwTZTEUPDD_RebuildLODsOnLPLR( zVIEW vSubtask )
{
         ...
         nRC = SetCursorNextSelectedEntity( vLOD_List, "W_MetaDef", "" ) )
   {
      ...
      else
      {
       ...
       GetStringFromAttribute( szFileName, vLPLR, "LPLR", "MetaSrcDir" );
         ofnTZCMWKSO_AppendSlash( szFileName );
         zstrcat( szFileName, szLOD_Name );
...
zwTZTEUPDD_BuildXMD_OnServer( zVIEW vSubtask )
{
   ...
   nRC = ActivateOI_FromFile( &vXMD, "TZDMXGPO", vSubtask, szFileName,
                              zSINGLE );
...
zwTZTEUPDD_DeleteSelectedEntity( zVIEW vSubtask )
{
   ...
   // No Siron catalogs than any other database
   zstrcpy( szWorkMessage, "OK to delete selected Tables/Records "
            "from the Data Source" );
   nRC = MessagePrompt( vSubtask, "TE00414", "Physical Data Model",
                        szWorkMessage,
                        zBEEP, zBUTTONS_YESNO,
         ...
         nRC = SetCursorNextSelectedEntity( vDTE, "TE_TablRec", "" ) )
   {
      ...
      {
          RESULT = SetCursorFirstEntityByInteger (vDTE_Del, "ER_EntityRelLink",
                                                  "ZKey", lEntityKey, "");
...
zwTZTEUPDD_ReconstructSelectedEntity( zVIEW vSubtask, zVIEW vDTE, zVIEW vEMD )
{
    ...
    {
       if ( CheckExistenceOfEntity( vDTE_Work,
                                    "TE_FieldDataRel_PK" ) >= zCURSOR_SET )
       {
          // If the attribute is a foreign key, reconstruct the attribute
          // in the ER-Model.  Set the ER-Attribute and the Domain.
          nRC = zwTZTEUPDD_SetERAttributeWithDomain( vSubtask,
                                                     vDTE_Work, vEMD );
          if ( nRC < 0 )
          {
             DropView( vDTE_Work );
          ...
          while ( nRC >= zCURSOR_SET )
          {
              ...
              nRC = SetCursorFirstEntityByString( vEMD, "ER_FactType",
                                                  "Type", "REL", 0 );
          }

          // If the attribute is a key attribute create a new instance of
          // ER_FactType where ER_FactType = "ATR".
   ...
   while (nRC >= zCURSOR_SET)
   {
      ...
      if (CompareAttributeToString (vDTE_Work, "TE_FieldDataRel", "Key", "Y") == 0)
      {
         ...
         if (CheckExistenceOfEntity (vDTE_Work, "TE_FieldDataRelFK_1") >= zCURSOR_SET)
         {
            ...
            GetStringFromAttribute (szCatalogName, vDTE_Work, "TE_TablRec", "Name");
            GetStringFromAttribute (szAttributeName, vDTE_Work, "TE_FieldDataRel", "Name");
               ...
               if ( nRC && nRC != zCALL_ERROR )
               {
                  ...
                  while ( nRC >= zCURSOR_SET )
                  {
                     if ( CheckExistenceOfEntity(vDTE2, "TE_FieldDataRel_PK") >= zCURSOR_SET &&
                          ...
                          CompareAttributeToString( vDTE2, "ER_EntityRelLink", "Name",
                                                    szCatalogName) == 0 )
                     {
                        // If the attribute is a foreign key compare its name with the original
                        // Set the ER-Attribute and the Domain
                        nRC = CompareAttributeToString( vDTE2, "TE_FieldDataRel_PK", "Name",
                                                        szAttributeName );
                        if ( nRC == 0 )
                        {
                           ...
                           // and the ER_Entity_Other_Identifier.Name == the catalog which should be
                           // deleted
                           nRC = SetCursorFirstEntityByString (vEMD, "ER_FactType", "Type", "REL", 0);
                           while (nRC >= zCURSOR_SET)
                           {
                               nRC = CompareAttributeToString( vEMD, "ER_Entity_Other_Identifier",
                                                               "Name", szCatalogName);
                               if ( nRC == 0 )
                               {
                                  DeleteEntity( vEMD, "ER_FactType", zREPOS_NONE );
                               ...
                               nRC = SetCursorNextEntityByString( vEMD, "ER_FactType",
                                                                  "Type", "REL", 0 );
                           }
                           // If the attribute is a key attribute create a new instance of
                           // ER_FactType where ER_FactType = "ATR"
                           ...
                           IncludeSubobjectFromSubobject (vDTE2, "ER_Attribute", vEMD, "ER_Attribute",
                                                          zPOS_AFTER);
                                     ...
                                     zVIEW vDTE_Work, zVIEW vEMD )
{
   ...
   // First position the ER-Model on the actual catalog. We find the catalogname
   // in TE in Entity "TE_TableRec"
   nRC = SetCursorFirstEntityByAttr( vEMD, "ER_Entity", "Name",
                                     vDTE_Work, "ER_Entity", "Name", 0 );
   ...
   SetAttributeFromAttribute  (vEMD, "ER_Attribute", "Name",
                               vDTE_Work, "TE_FieldDataRel", "Name");
   SetAttributeFromAttribute  (vEMD, "ER_Attribute", "Lth",
                               vDTE_Work, "TE_FieldDataRel", "Length");
   SetAttributeFromAttribute  (vEMD, "ER_Attribute", "NotNull",
                               vDTE_Work, "TE_FieldDataRel", "SQL_NULLS");
   SetAttributeFromAttribute  (vEMD, "ER_Attribute", "ParticipatesInKey",
                               vDTE_Work, "TE_FieldDataRel", "Key");

   // Include the Domains
   memset (szDomainName, 0, 256);
   GetStringFromAttribute (szDataType, vDTE_Work, "TE_FieldDataRel", "DataType");
   switch (*szDataType)
   {
      case 'S': strcpy (szDomainName, "Text");
                break;
      case 'L': strcpy (szDomainName, "Integer");
                break;
      case 'M': strcpy (szDomainName, "Decimal");
                break;
      default:  break;
   }
   ...
   IncludeSubobjectFromSubobject( vEMD, "Domain",
...
zwTZTEUPDD_SetFactType( zVIEW vSubtask, zVIEW vDTE_Work, zVIEW vEMD)
{
   ...
   GetStringFromAttribute (szKeyKen, vDTE_Work, "TE_FieldDataRel", "Key");
   if ( *szKeyKen == 'Y' )
   {
      ...
      IncludeSubobjectFromSubobject( vEMD, "ER_AttributeIdentifier",
...
zwTZTEUPDD_PreBuildDelete( zVIEW vSubtask )
{
   ...
   switch (lDelAllTables)
   {
      case 0: /* Only delete selected table */
           zstrcpy( szWorkMessage, "If you Delete the selected Entity "
                    "from the Data Source, ERD and TE will be changed. "
                    "Do you want to continue ?");
           break;

      case 1: /* Delete all Tables */
           zstrcpy( szWorkMessage, "If you Delete all Tables/Records "
                    "from the Data Source, ERD and TE will be changed. "
                    "Do you want to continue ?");
           break;

      case 2: /* Delete the DataSource */
           zstrcpy (szWorkMessage, "If you Delete the DataSource, ERD and TE will "
...
zwTZTEUPDD_DeleteVsamTables( zVIEW  vSubtask )
{
   ...
   GetIntegerFromAttribute( &lDelAllTables, vDelWork,
                            "TZSIDEWK", "wkAllTables" );
   ...
   switch ( lDelAllTables )
   {
      case 0: /* Delete only the selected tables */
         ...
         while ( nZRetCode >= zCURSOR_SET )
         {
            ...
            nRC = zwTZTEUPDD_ReconstructEntitiesAndForeignKeys( vSubtask,
                                                                vDTE, vEMD );
            if ( nRC )
               return( zCALL_ERROR );

            // Now the changes in the ERD are done. Delete the actual Table from
            // the DataSource.
            nZRetCode = DeleteEntity( vDTE, "TE_TablRec", zREPOS_NONE );
            nZRetCode = SetCursorNextSelectedEntity( vDTE, "TE_TablRec", "" );
         }

         zstrcpy( szWindowName, "TableRecord" );
         zstrcpy( szEntityName, "TE_TablRec" );
         zstrcpy( szWorkMessage, "TE and ERD have been changed. Before using function "
      ...
      case 1: /* Delete all tables */
           ...
           if (nRC != 0)
           {
              zstrcpy( szWorkMessage, "ER Attributes cannot be recontructed from Foreign Keys");
              nRC = MessagePrompt( vSubtask, "TE00414", "Physical Data Model",
      ...
      case 2: /* Delete DataSource */
           ...
           zstrcpy( szWindowName, "DataSrc" );
           zstrcpy( szEntityName, "TE_DBMS_Source" );
           zstrcpy( szWorkMessage, "TE and ERD have been changed. Before using any function "
...
zwTZTEUPDD_DeleteDataSource( zVIEW vSubtask )
{
   ...
   // No Siron catalogs than any other database
   zwTZTEUPDD_DeleteSubobject( vSubtask, zENAME_DTE_DATASOURCE,
                               "TE_DBMS_Source", nCloseWindow );
...
zwTZTEUPDD_DisableButton( zVIEW vSubtask )
{
   ...
   if ( (CheckExistenceOfEntity( vTE_Work,
                                 "CandidateSourceKey" ) == zCURSOR_SET) &&
        (CheckExistenceOfEntity( vTE_Work, "TargetKey" ) == zCURSOR_SET) )
   {
      nEnable = 1;
   }

   SetCtrlState( vSubtask, "pbIncludeSourceKey",
...
zwTZTEUPDD_RefreshTableDetail( zVIEW vSubtask )
{
        ...
        GetViewByName( &vDTE_Old, "TE_DB_Environ_old", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      if ( CompareAttributeToAttribute(   vDTE, "TE_TablRec", "ZKey",
                                          vDTE_Old, "TE_TablRec", "ZKey" ) == 0 )
      {
         return( 0 );
      }

      GetIntegerFromAttribute( &lnewZKey, vDTE, "TE_TablRec", "ZKey" );
      if ( SetCursorFirstEntityByEntityCsr( vDTE, "TE_TablRec", vDTE_Old,
                                            "TE_TablRec", "" ) >= zCURSOR_SET )
      {
         ...
         SetSelectStateOfEntity( vDTE, "TE_TablRec", 0 );
         if ( zwTZTEUPDD_AcceptSubobject( vSubtask, "TE_TablRec",
                                          "Table / Record", 1 ) < 0 )
         {
            SetSelectStateOfEntity( vDTE, "TE_TablRec", 1 );
         ...
         SetAttributeFromString( vProfileXFER, "TE", "Name", "" );
         SetCursorFirstEntityByInteger( vDTE, "TE_TablRec", "ZKey",
                                        lnewZKey, "TE_DB_Environ" );
...
zwTZTEUPDD_DeleteSelectedColumn( zVIEW vSubtask )
{
   ...
   zstrcpy( szWorkMessage, "OK to delete selected Columns/Fields" );
   nRC = MessagePrompt( vSubtask, "TE00414", "Physical Data Model",
                        szWorkMessage,
                        zBEEP, zBUTTONS_YESNO,
                        zRESPONSE_NO, zICON_QUESTION );
...
zwTZTEUPDD_DeleteSelectedKeys( zVIEW vSubtask )
{
   ...
   zstrcpy( szWorkMessage, "OK to delete selected Keys" );
   nRC = MessagePrompt( vSubtask, "TE00414", "Physical Data Model",
                        szWorkMessage,
                        zBEEP, zBUTTONS_YESNO,
                        zRESPONSE_NO, zICON_QUESTION );
                            ...
                            zSHORT nIsCheckedOut )
{
   ...
   if ( nIsCheckedOut == 1 )
   {
      ...
      if ( GetViewByName( &vTE_Work, "TZTEWRKO", vSubtask, zLEVEL_TASK ) >= 0 )
      {
         GetStringFromAttribute( szER_UpdateFlag, vTE_Work, "TE_Work", "ER_UpdateFlag" );
...
zwTZTEUPDD_KeepPhysCharField( zVIEW vSubtask )
{
         ...
         nRC = SetCursorNextSelectedEntity( vDTE, "TE_FieldDataRel", 0 ) )
   {
      SetAttributeFromString( vDTE, "TE_FieldDataRel",
                              "KeepPhysicalCharacteristicsFlag", szPhysChar );
   }
...
zwTZTEUPDD_KeepPhysCharTable( zVIEW vSubtask )
{
   ...
   {
      SetAttributeFromString( vDTE, "TE_TablRec",
                              "KeepPhysicalCharacteristicsFlag", szPhysChar );
   }
                                              ...
                                              zVIEW vDTE, zVIEW vEMD )
{
   ...
   zSHORT nRC = zwTZTEUPDD_ReconstructSelectedEntity( vSubtask,
                                                      vDTE, vEMD );
   if ( nRC )
   {
      zstrcpy( szWorkMessage, "ER Attributes cannot be recontructed from Foreign Keys");
                           ...
                           zPCHAR szEntityName )
{
   ...
   if ( nCloseWindow == 1 &&
        CheckExistenceOfEntity( vDTE, szEntityName ) == zCURSOR_NULL )
   {
      SetWindowActionBehavior( vWindow, zWAB_ReturnToParentWithRefresh |
                               zWAB_ProcessImmediateAction, 0, 0 );
   }
   else
   {
                            ...
                            zSHORT bCheck )
{
   ...
   GetStringFromAttribute( szNewName, vDTE, szEntityName, "Name" );
   zGetStringWithoutBlank( szNewName, szNewName );

   if ( zstrcmp( szNewName, "") == 0 )
   {
      zstrcpy( szMessage, szMessageText );
      zstrcat( szMessage, " Name is required.");
      if ( CompareAttributeToString( vProfileXFER, "TE", "Name", "Error" ) != 0 )
      {
   ...
   if ( bCheck == 1 )
   {
      ...
      nRC = SetCursorFirstEntityByString( vDTECopy, szEntityName,
                                          "Name", szNewName, "" );
      nRC = SetCursorNextEntityByString( vDTECopy, szEntityName,
                                         "Name", szNewName, "" );
      DropView( vDTECopy );
      ...
      {
         zstrcpy( szMessage, "A " );
         zstrcat( szMessage, szMessageText );
         zstrcat( szMessage, " already exists by this name!" );
         if ( CompareAttributeToString( vProfileXFER, "TE", "Name", "Error" ) != 0 )
         {
...
zwTZTEUPDD_CloseTablRecordWindow( zVIEW vSubtask )
{
   ...
   {
      MapWindow( vWindow );
      if ( zwTZTEUPDD_AcceptSubobject( vWindow, "TE_TablRec",
                                       "Table / Record", 1 ) < 0 )
      {
         SetFocusToCtrl( vWindow, "edName" );
         RefreshWindow( vWindow );
...
zwTZTEUPDD_DataSrcSingleClick( zVIEW vSubtask )
{
        ...
        GetViewByName( &vDTE, "TE_DB_Environ", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      SetCursorFirstEntityByEntityCsr(   vDTE, "TE_DBMS_Source", vDTE_Old,
                                       "TE_DBMS_Source", "TE_DB_Environ" );
      SetCursorFirstEntityByEntityCsr(   vDTE, "TE_TablRec", vDTE_Old,
                                       "TE_TablRec", "" );
                                 ...
                                 zPCHAR szWindowName )
{
   ...
   SetCursorFirstEntityByAttr( vDTE, szEntityName, "ZKey",
                               vDTECopy, szEntityName, "ZKey", "" );
   DropView( vDTECopy );
...
zwTZTEUPDD_KeepPhysCharAllColumn( zVIEW vSubtask )
{
   ...
   {
      SetAttributeFromString( vDTE, "TE_TablRec",
                              "KeepPhysCharsFlagForAllColumns", szPhysChar );
   ...
   SetCursorFirstEntityByAttr( vDTE, "TE_TablRec", "ZKey",
                               vDTECopy, "TE_TablRec", "ZKey", "" );
   SetCursorFirstEntityByAttr( vDTE, "TE_FieldDataRel", "ZKey",
                               vDTECopy, "TE_FieldDataRel", "ZKey", "" );
...
zwTZTEUPDD_KeepPhysCharAll( zVIEW vSubtask )
{
   ...
   {
      SetAttributeFromString( vDTECopy, "TE_TablRec",
                              "KeepPhysicalCharacteristicsFlag", szPhysChar );
      SetAttributeFromString( vDTECopy, "TE_TablRec",
                              "KeepPhysCharsFlagForAllColumns", szPhysChar );
      ...
      {
         SetAttributeFromString( vDTECopy, "TE_FieldDataRel",
                                 "KeepPhysicalCharacteristicsFlag", szPhysChar );
      }
   }
