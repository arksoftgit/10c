                   ...
                   zBOOL nSaveAs )
{
   ...
   GetStringFromAttribute( szMetaName, vOperGrp, "GlobalOperationGroup", "Name" );
...
zwfnPromptForSave( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vOperGrp, "TZOGSRCO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      if ( MiGetUpdateForView( vOperGrp ) == 1  &&
           ObjectInstanceUpdatedFromFile( vOperGrp ) )
      {
         // if Operation Group not checked out, then ask for save as
         if ( !ComponentIsCheckedOut( vSubtask, vOperGrp, zSOURCE_GOPGRP_META ) )
         {
            nSaveAs = TRUE;
            GetStringFromAttribute(   szOperationName, vOperGrp,
                                    "Operation", "Name" );
            zstrcpy( szMessageText, "Operation Group is not checked out," );
            zstrcat( szMessageText, " but Operation '" );
            zstrcat( szMessageText, szOperationName );
            zstrcat( szMessageText, "' has been modified. \n\nWould you like" );
            zstrcat( szMessageText, " to save it with differend name?" );
...
zwfnDropActiveOperation( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSrcZKey, vSOURCEMETA, "SourceFile", "ZKey" );
...
zwTZOPUPDD_OpenOperation( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szOperName, vCM_List, "W_MetaDef", "Name" );
   ...
   else
   {
      ...
      GetStringFromAttribute( szOperGroupName, vCM_List, "W_MetaDef", "GroupName" );
...
zwTZOPUPDD_CreateOperation( zVIEW vSubtask )
{
   ...
   nZRetCode = GetStringFromAttribute( szName, vProfileXFER, "OP", "Name" );
   if ( szName[ 0 ] == 0 )
   {
   ...
   else
   {
      ...
      GetStringFromAttribute( szProfileGroupName, vProfileXFER, "OP", "GroupName" );
                              ...
                              zSHORT nDeleteOperationCode )
{
   ...
   nRC = ActivateMetaOI( vSubtask, &vOperGrp, vCM_List, zSOURCE_GO_META,
                         zSINGLE | zLEVEL_APPLICATION );
   GetStringFromAttribute( szGroupName, vCM_ListOrig, "W_MetaDef", "GroupName" );
   DropView( vCM_List );
   vCM_List = 0;
...
zwTZOPUPDD_NewOperationGroup( zVIEW vSubtask )
{
   ...
   GetViewByName( &vProfileXFER, "ProfileXFER", vSubtask, zLEVEL_ANY );
   GetViewByName( &vCM_List, "CMLIST", vSubtask, zLEVEL_ANY );
   GetStringFromAttribute( szOperName, vProfileXFER, "OP", "Name" );
                           ...
                           zVIEW  vCM_ListGroup )
{
   ...
   GetStringFromAttribute( szExecName, vOperGrp, "GlobalOperationGroup",
...
zwTZOPUPDD_SetExtension( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szLanguageType, vOperGrp, "GlobalOperationGroup",
                           "LanguageType" );
...
zwTZOPUPDD_SaveOperationGroup( zVIEW vSubtask )
{
   ...
   GetViewByName( &vProfileXFER, "ProfileXFER", vSubtask, zLEVEL_ANY );
   GetViewByName( &vCM_ListGroup, "CM_ListGroup", vSubtask, zLEVEL_ANY );
   GetStringFromAttribute( szOperGrpName, vOperGrp, "GlobalOperationGroup", "Name" );
...
zwTZOPUPDD_LoadOperation( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute ( szNewOperation, vTZOGSRCO, "Operation", "Name" );
...
zwTZOPUPDD_MoveOperationPostBuil( zVIEW vSubtask )
{
   ...
   {
      GetViewByName( &vCM_List, "CMLIST", vSubtask, zLEVEL_TASK );
      GetStringFromAttribute( szOperationName, vCM_List, "W_MetaDef", "Name" );
      GetStringFromAttribute( szOperationGroup, vCM_List, "W_MetaDef", "GroupName" );
   ...
   {
      GetViewByName( &vTZOGSRCO, "TZOGSRCO", vSubtask, zLEVEL_TASK );
      GetStringFromAttribute( szOperationName, vTZOGSRCO, "Operation", "Name" );
      GetStringFromAttribute( szOperationGroup, vTZOGSRCO,
                               ...
                               zSHORT nCreateNewGroup )
{
   ...
   if ( GetCtrlState( vWindow, "cbCopyCode", zCONTROL_STATUS_CHECKED ) == 1 )
   {
      ...
      {
         zstrcpy( szMsg, "Language Type of Operation Group do not match." );
...
zwTZOPUPDD_MoveOperToNewGroup( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szOperationName, vSource, "Operation", "Name" );
...
zwTZOPUPDD_SetGroupName( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szGroupName, vMoveOperation, "SourceFile", "Name" );
   GetStringFromAttributeByContext( szType, vMoveOperation, "SourceFile",
                                    "LanguageType", "LanguageType", 4 );
   SetCtrlText( vSubtask, "txtOperationGroup", szGroupName );
   SetCtrlText( vSubtask, "txtType", szType );
...
zwTZOPUPDD_SaveAsInNewGroup( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szNewName, vProfileXFER, "OP", "Name" );
   ...
   {
      GetStringFromAttribute( szGroupName, vCM_List_Copy, "W_MetaDef", "GroupName" );
      zstrcpy( szMsg, "Cannot create a new Operation Group for an "
                      "existing Operation." );
                              ...
                              zPCHAR szOutName )
{
   ...
   if ( SetCursorFirstEntityByString( vCM_List_Copy, "W_MetaDef", "Name",
        ...
        CompareAttributeToString( vCM_List_Copy, "W_MetaDef", "GroupName",
                                  szGroupName ) != 0 )
   {
      zstrcpy( szMsg, "Cannot save Operation, because already\n" );
      zstrcat( szMsg, "exists in Operation Group '" );
                            ...
                            zPCHAR szMessageText )
{
   ...
   GetStringFromAttribute( szNewName, vTZOGSRCO, szEntityName, szAttributeName );
...
zwTZOPUPDD_DeleteParameter( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szShortDesc, vTZOGSRCO, "Parameter", "ShortDesc" );
   zstrcpy( szMsg, "OK to delete Parameter '" );
   zstrcat( szMsg, szShortDesc );
   zstrcat( szMsg, "'?" );
