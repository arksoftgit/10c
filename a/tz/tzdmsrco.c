...
oTZDMSRCO_UniqueDomainName( zVIEW vDomain, zVIEW vCM_List )
{
   ...
   GetStringFromAttribute( szDomainName, vDomain, "Domain", "Name" );
...
ofnTZDMSRCO_CopyDomToXDM( zVIEW vSubtask, zVIEW vDomain, zVIEW vXDM )
{
   ...
   // internal type used.
   GetStringFromAttribute( szDataType, vDomain, "Domain", "DataType" );
                             ...
                             zSHORT       nMsg )
{
   ...
   if ( nRC >= 0 )
   {
      nRC = GetStringFromAttribute( szDomainDataType, vDomain, "Domain", "DataType" );
                         ...
                         zSHORT       nMsg )
{
   ...
   if ( nRC >= 0 )
   {
      GetStringFromAttribute( szDomainType, vDomain, "Domain", "DomainType" );
                             ...
                             zSHORT       nMsg )
{
   ...
   zstrcpy( szEntityName, lpViewEntity->szName );
   ...
   if ( nRC >= 0 )
   {
      GetStringFromAttribute( szContextName, vDomain, "Context", "Name" );
      StoreValueInRecord( vMeta, lpViewEntity, lpViewAttrib,
                          szContextName, 0 );
   }
   else
                          ...
                          zSHORT           nMsg )
{
   ...
   if ( CheckExistenceOfEntity( vMeta, "Operation" ) >= zCURSOR_SET )
      GetStringFromAttribute( szOperName, vMeta, "Operation", "Name" );
...
oTZDMSRCO_CheckDomainType( zVIEW vMeta  /*  BASED ON LOD TZDGSRCO  */ )
{
   ...
   while( nRC >= zCURSOR_SET )
   {
      ...
      {
         GetStringFromAttribute( szTemp, vTZDGSRCO, "Context", "EditString" );
         zGetStringWithoutBlank( szTemp, szTemp );

         if ( zstrcmp( szTemp, "") == 0 )
         {
            zstrcpy( szMsg, "Domain '" );
            GetStringFromAttribute( szTemp, vTZDGSRCO, "Domain", "Name" );
            zstrcat( szMsg, szTemp );
            zstrcat( szMsg, "' is from Type 'Format' \nand the Context '");
            GetStringFromAttribute( szTemp, vTZDGSRCO, "Context", "Name" );
            zstrcat( szMsg, szTemp );
