                      ...
                      zPLONG plExprTextIdx )
{
   ...
   while ( lExprEntry < *plExprCodeIdx )
   {
      ...
      if ( sWorkExpr.lElementClass == qCONSTANT ||
           sWorkExpr.lElementClass == qATTRIBUTEQUALIFIER ||
           (sWorkExpr.lElementClass == qOPERATIONCALL && sWorkExpr.lOffset > 0) )
      {
         SetAttributeFromString( vReturnExpr, "ExprNode", "Text",
                                 (zPCHAR) pchExprText + sWorkExpr.lOffset );
      }
      else
      {
         SetAttributeFromString( vReturnExpr, "ExprNode", "Text", "" );
   ...
   if ( CheckExistenceOfEntity( vSourceView, "ExprNode" ) == zCURSOR_SET )
   {
      do
      {
              ...
              sWorkExpr.lElementClass == qATTRIBUTEQUALIFIER )
         {
            ...
            GetStringFromAttribute( sWorkExpr.szText, vSourceView, "ExprNode", "Text" );  // was szText ... dks 2009.12.28

            zstrcpy( (zPCHAR) pchExprText + sWorkExpr.lOffset, sWorkExpr.szText );
            lExprText += zstrlen( sWorkExpr.szText ) + 1;  // was szText ... dks 2009.12.28
         }
         else
         if ( sWorkExpr.lElementClass == qOPERATIONCALL &&
              sWorkExpr.lOffset <= 1024 ) // a zkey would be larger
         {
            sWorkExpr.lOffset = lExprText;
            GetStringFromAttribute( sWorkExpr.szText, vSourceView, "ExprNode", "Text" );  // was szText ... dks 2009.12.28
         ...
         // Make sure that the size of the expression is not too big for the
         // pchExprCode or pchExprText work areas.  check resr length >
         // twice the size of the increment of pchExprCode during each loop
         // pass.  80 is a guess of max size of the next expression text.
         if ( lExprEntry > EXPRCODE_MAXSTRING - (2 * lExprIncrement) ||
              lExprText  > EXPRTEXT_MAXSTRING - 80 )
         {
                       ...
                       zCPCHAR cpcGenLang )
{
   ...
   nRC = (zSHORT)(*lf)( (zVIEW) vSubtask, (zVIEW) vSourceMeta, (zLONG) lIlParseAction,
                        (zLONG) lIlParseLimit, (zLONG) lIlParseSource,
