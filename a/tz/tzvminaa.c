             ...
             zPCHAR szMetaType )
{
   ...
   if ( nRC != zLEVEL_APPLICATION )
   {
      ...
      GetStringFromAttribute( szExecDir, CM_List, "LPLR", "ExecDir" );
   ...
   {
      zstrcpy( szSourceFileMeta1, "DialogSourceFile" );
      zstrcpy( szSourceFileMeta2, "Dialog" );
      zstrcpy( szOperationMeta, "DialogOperation" );
      zstrcpy( szScopeMeta, "Dialogs" );
   }
   else
   if ( zstrcmp( szMetaType, "LOD" ) == 0 )
   {
      zstrcpy( szSourceFileMeta1, "LODSourceFile" );
      zstrcpy( szSourceFileMeta2, "LOD" );
      zstrcpy( szOperationMeta, "LOD_Operation" );
      zstrcpy( szScopeMeta, "LODs" );
   }
   else
   {
      zstrcpy( szSourceFileMeta1, "GlobalSourceFile" );
      zstrcpy( szSourceFileMeta2, "GlobalOperation" );
      zstrcpy( szOperationMeta, "Operation" );
      zstrcpy( szScopeMeta, "GlobalOperations" );
   }
   ...
   nRC = SetCursorFirstEntityByString( vXChecker, szOperationMeta,
                                       "Name", szOperationName, "" );

   if ( nRC >= zCURSOR_SET )
   {
      nRC = SetAttributeFromInteger( vXChecker,
   ...
   {
      nRC = SetAttributeFromInteger( vXChecker, "APPLICATION",
                                          "CurrentMode", 1 );
      return( zXC_OPERATION_STAGED );
   }
}
...

{
   ...
   if ( nRC != zLEVEL_APPLICATION )
   {
      nRC = GetViewByName( &CM_List, "TZCMWKSO",
                                lpInterpSubtask, zLEVEL_APPLICATION );

      nRC = GetStringFromAttribute( szExecDir, CM_List,
                                         "LPLR", "ExecDir" );
   ...
   nRC = SetCursorFirstEntityByInteger( vXChecker, "DialogOperation",
                                             "ZKey", lZKey, "APPLICATION" );
#if 0
   if ( nRC != zCURSOR_SET )
      return( -1 );
#else
   ...
   if ( nRC < zCURSOR_SET )
      nRC = SetCursorFirstEntityByInteger( vXChecker, "LOD_Operation",
                                                "ZKey", lZKey, "APPLICATION" );
   if ( nRC < zCURSOR_SET )
      nRC = SetCursorFirstEntityByInteger( vXChecker, "Operation",
                                                "ZKey", lZKey, "APPLICATION" );
   ...
   {
      nRC = SetAttributeFromInteger( vXChecker, "APPLICATION",
                                          "CurrentMode", zXC_SETUP_DEBUGGER );
      return( zXC_SETUP_DEBUGGER );
   }
   else
   {
...
Exec_StepOnce( zVIEW vSubtask )
{
   ...
   //   1.  The next statement to execute is within an operation
   //       currently in debug mode.
   //   2.  The return code from the last statement executed indicates
   //       debugging is to start for that operation.
            ...
            szDebugFlag[ 0 ] != 'Y' )
   {
      ...
      if ( lStepRC )
      {
         ...
         GetStringFromAttribute( szDebugFlag, vS_View,
                                 "Operation", "CurrentDebugFlag" );
      }
   }

   if ( lStepRC == zXC_SETUP_DEBUGGER )
   {
      SetAttributeFromString( vS_View, "Operation",
                              "CurrentDebugFlag", "Y" );
   }
...
// zVIEW  vViewObject;
                       ...
                       zLEVEL_TASK ) != zLEVEL_TASK )
   {
      ...
      zstrcat( szExecDir, "TZOPSIGK.XGO" );
         ...
         zCPCHAR cpcGenLang )
{
   ...
   // get the exec directory
   GetStringFromAttribute( szExecDir, vViewObject, "LPLR", "ExecDir" );

   // be sure it was terminated with a '\'
   nLth = zstrlen( szExecDir );
   if ( szExecDir[ nLth ] != '\\' )
   {
      szExecDir[ nLth ] = '\\';
      szExecDir[ nLth + 1 ] = 0;
   ...
   zstrcat( szExecDir, szSrcDLLName );
   if ( cpcGenLang[ 0 ] == 'J' )
      zstrcat( szExecDir, ".XPJ" );
   else
      zstrcat( szExecDir, ".XPG" );
   ...
   if ( nRC != 0 ) //  failed
   {
      zstrcpy( szMessage, "Source VML :" );
      zstrcat( szMessage, szSrcDLLName );
      zstrcat( szMessage, " must be parsed prior to execution" );
