                         ...
                         zVIEW  vDomainGrp )
{
   ...
   {
      nSaveAs = TRUE;
      GetStringFromAttribute(   szDomainName, vDomainGrp, "Domain", "Name" );
      zstrcpy( szMessageText, "Domain Group is not checked out, but Domain '" );
      zstrcat( szMessageText, szDomainName );
...
zwTZDMUPDD_CreateNewDomain( zVIEW    vSubtask )
{
   ...
   nZRetCode = GetStringFromAttribute( szTempString,
                              ...
                              zSHORT nDeleteOperationCode )
{
   ...
   GetStringFromAttribute ( szOperationName, vTZDGSRCO, "SelectedOperation", "Name" );
   ...
   if ( nPromptForDelete == 1 )
   {
      zstrcpy( szMsg, "Delete assigned Domain Operation '" );
      zstrcat( szMsg, szOperationName );
      zstrcat( szMsg, "' from Source File?" );
                         ...
                         zVIEW vTZDGSRCO )
{
   ...
   if ( SetCursorFirstEntityByEntityCsr( vTZDGSRCO, "Operation",
                                         ...
                                         "" ) >= zCURSOR_SET )
   {
      GetStringFromAttribute ( szOperationName, vTZDGSRCO,
...
zwTZDMUPDD_AddTableEntry( zVIEW    vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vDomainGrp, "TableEntry" ) >= zCURSOR_SET )
   {
      ...
      if ( CompareAttributeToString( vDomainGrp, "Domain", "DataType",
                                    "L" ) == 0 )
      {
         GetStringFromAttribute( szInternalValue,
...
zwTZDMUPDD_TABLEENT_BUTTON_Next( zVIEW    vSubtask )
{
   ...
   if ( CompareAttributeToString( vDomainGrp, "Domain", "DataType",
                                  "L" ) == 0 )
   {
      GetStringFromAttribute( szInternalValue, vDomainGrp,
                                ...
                                zVIEW  vDomain )
{
  ...
  {
      zstrcpy( szMsg, "DLL Name does not exists or wrong Language\n" );
      zstrcat( szMsg, "Type selected.\n\n" );
      zstrcat( szMsg, "Please switch to Domain Group Properties\n" );
      zstrcat( szMsg, "and correct the problem." );
      MessageSend( vSubtask, "DM00115", "Domain Maintenance",
...
zwTZDMUPDD_OpenDomainPreBuild( zVIEW vSubtask )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szGroupName,
...
zwTZDMUPDD_LoadDomain( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szNewDomain, vDomainGrp, "Domain", "Name" );
...
zwTZDMUPDD_DeleteOperation( zVIEW vSubtask )
{
   ...
   // Prompt for Delete
   zstrcpy( szMsg, "OK to Delete Domain Operation '" );
   GetStringFromAttribute( szName, vTZDGSRCO, "Operation", "Name" );
   ...
   if ( nRC == zRESPONSE_YES )
   {
      ...
      if ( nRC >= zCURSOR_SET )
      {
         zstrcpy( szMsg, "Domain Operation '" );
         zstrcat( szMsg, szName );
         zstrcat( szMsg, "' cannot be deleted \nif it is used by an Domain." );
...
zwTZDMUPDD_MoveDomainPostBuild( zVIEW vSubtask )
{
   ...
   {
      GetViewByName( &vCM_List, "CM_List", vSubtask, zLEVEL_TASK );
      GetStringFromAttribute( szDomainName, vCM_List, "W_MetaDef", "Name" );
      GetStringFromAttribute( szDomainGroup, vCM_List, "W_MetaDef", "GroupName" );
   ...
   {
      GetViewByName( &vTZDGSRCO, "TZDGSRCO", vSubtask, zLEVEL_TASK );
      GetStringFromAttribute( szDomainName, vTZDGSRCO, "Domain", "Name" );
      GetStringFromAttribute( szDomainGroup, vTZDGSRCO, "DomainGroup", "Name" );
...
zwTZDMUPDD_SetSaveAsName( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szGroupName, vCM_List, "W_MetaDef", "GroupName" );
   GetStringFromAttributeByContext( szType, vMoveDomain, "SourceFile",
                                    "LanguageType", "LanguageType", 4 );
...
zwTZDMUPDD_SetGroupName( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szGroupName, vMoveDomain, "SourceFile", "Name" );
   GetStringFromAttributeByContext( szType, vMoveDomain, "SourceFile",
                                    "LanguageType", "LanguageType", 4 );
                              ...
                              zPCHAR szOutName )
{
   ...
   GetCtrlText( vWindow, "txtDomainGroup", szGroupName, 9 );
   if ( zstrcmp( szGroupName, "" ) == 0 )
   {
      zstrcpy( szMsg, "Target Group Name is required." );
   ...
   GetStringFromAttribute( szNewName, vProfileXFER, "DM", "DomainName" );
   UfCompressName( szNewName, szOutName, 32, "", "", "", "", 0 );
   if ( zstrcmp( szOutName, "" ) == 0 )
   {
      zstrcpy( szMsg, "Domain Name is required." );
      MessageSend( vSubtask, "ZO00137", "Domain Maintenance",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
   ...
   if ( SetCursorFirstEntityByString( vCM_List_Copy, "W_MetaDef", "Name",
        ...
        CompareAttributeToString( vCM_List_Copy, "W_MetaDef", "GroupName",
                                  szGroupName ) != 0 )
   {
      zstrcpy( szMsg, "Cannot save Domain, because already exists\n" );
      zstrcat( szMsg, "in Domain Group '" );
      GetStringFromAttribute( szNewName, vCM_List_Copy, "W_MetaDef", "GroupName" );
                              ...
                              zSHORT  nMoveFlag )
{
   ...
   if ( nRC >= zCURSOR_SET )
   {
      zstrcat( szMsg, "is assigned to more than one Domains.\n\n" );
      zstrcat( szMsg, "Replace existing Operation? ");
   }
   else
   {
      zstrcat( szMsg, "already exists.\n\n" );
      zstrcat( szMsg, "Replace existing Operation? ");
   }
                                ...
                                zPCHAR szOperationName )
{
   ...
   if ( nRC >= zCURSOR_SET )
   {
      zstrcpy( szMsg, "Operation Code cannot be moved, because\n" );
      zstrcat( szMsg, "Operation is related to an other Domain.\n\n" );
      zstrcat( szMsg, "Operation Code will be copied." );
                            ...
                            zSHORT nMoveFlag )
{
   ...
   GetStringFromAttribute( szOperationName, vSource, "SelectedOperation", "Name" );
                               ...
                               zSHORT nCheckIdenticalGroup )
{
        ...
        GetCtrlState( vWindow, "cbCopyCode", zCONTROL_STATUS_CHECKED ) == 1 )
   {
      ...
      if ( zstrcmp( szControlText, szLanguageType ) != 0 )
      {
         ...
         zstrcpy( szMsg, "Language Type of Domain Group do not match.\n"
...
zwTZDMUPDD_MoveDomainToNewGroup( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( strDomainName, vSource, "Domain", "Name" );
   ...
   else
   {
      ...
      SetAttributeFromAttribute( vTarget, "Domain", "ZKey",
                                 vSource, "Domain", "ZKey" );
   }
   ...
   // Create Context And Operation
   nCopyOrMoveFlag = zwTZDMUPDD_CreateContextAndOper( vSubtask, vSource, vTarget,
                                                      zMOVE_OPERATION );
   if ( nCopyOrMoveFlag < 0 )
      return( -1 );
   ...
   SetCursorFirstEntityByString( vTarget, "Domain", "Name", strDomainName, "" );
   SetCursorFirstEntityByString( vDomainList, "W_MetaDef", "Name", strDomainName, "" );
   SetAttributeFromString( vProfileXFER, "DM", "NewDomainGroupFlag", "" );
...
zwTZDMUPDD_AcceptDomainGroup( zVIEW vSubtask )
{
   ...
   GetViewByName( &vCM_ListGroup, "CM_ListGroup", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szDomainGroup, vTZDGSRCO_DETAIL, "DomainGroup", "Name" );
...
zwTZDMUPDD_SaveAsInNewGroup( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szGroupName, vCM_List, "W_MetaDef", "GroupName" );
      zstrcpy( szMsg, "Cannot create a new Domain Group for an existing Domain.\n" );
      zstrcat( szMsg, "Domain exists in Doamin Group '");
      zstrcat( szMsg, szGroupName );
      zstrcat( szMsg, "'.");
      MessageSend( vSubtask, "DM00103", "Domain Maintenance",
...
zwTZDMUPDD_SaveAsRtnFromSubwnd( zVIEW vSubtask )
{
   ...
   GetCtrlText( vSubtask, "txtDomainGroup", szGroupName, 9 );
   zwTZDMUPDD_CreateWorkLOD( vSubtask );
                            ...
                            zPCHAR szMessageText )
{
   ...
   GetStringFromAttribute( szNewName, vTZDGSRCO, szEntityName, szAttributeName );
