...
zwTZHLPCXD_ActivateHlpMapFile( zVIEW vSubtask )
{
   ...
   if ( nRC < 0 )
   {
      zstrcpy( pBuffer, "Unable to get current system directory" );
      zstrcat( pBuffer, szFileName );
      zwTZHLPCXD_ErrMsg( vSubtask, "zwTZHLPCXD_ActivateHlpMapFile",
                         "SysGetLocalDirectory", pBuffer,
                         zMSGQ_SYSTEM_ERROR, nRC );
     return( nRC );
   }
...
zwTZHLPCXD_ImportHlpMapInfo( zVIEW vSubtask )
{
   ...
   else
   {
      ...
      if ( nRC == zCURSOR_SET )
      {
         zstrcpy( pBuffer, "Manual already exists. Please delete manual " );
         zstrcat( pBuffer, pHelpMapInfo->pszHelpFileName );
         zstrcat( pBuffer, " first, before you recreate it." );
         zwTZHLPCXD_ErrMsg( vSubtask, "zwTZHLPCXD_ImportHlpMapInfo",
                            "SetCursorFirstEntityByString", pBuffer,
                            zMSGQ_OBJECT_CONSTRAINT_ERROR, nRC );
         return( nRC );
      }
   }
   ...
   while ( pHelpFcts )
   {
      ...
      if (nRC < 0 )
      {
         zstrcpy(pBuffer, "Unable to set Attribute 'Tag'");
      }
      else
      {
         nRC = SetAttributeFromString( vTZHLPCXO, "Context", "Number",
                                       pHelpFcts->pszNumber );
         if ( nRC < 0 )
         {
            zstrcpy( pBuffer, "Unable to set attribute Number" );
         }
         else
         {
            ...
            if ( nRC < 0 )
            {
               zstrcpy( pBuffer, "Unable to set attribute FileName" );
            }
         }
      }
      ...
      {
         zwTZHLPCXD_ErrMsg( vSubtask, "zwTZHLPCXD_ImportHlpMapInfo",
                            "SetAttributeFromString", pBuffer,
                            zMSGQ_REQUIRED_DATA_ITEM_ERROR, nRC );
         nRC = zwTZHLPCXD_FreeHelpMapInfo( &pHelpMapInfo );
         return( nRC );
      }
   ...
   {
      zstrcpy(pBuffer, "File tzhlpcxd.phl has to be changed by hand! ");
      zstrcat(pBuffer, "Topic Names for Progref are different in Zeidon and Robohelp");
      nRC = MessageSend( vSubtask, "HL00001",
                         "I M P O R T A N T             I N F O R M A T I O N",
                         pBuffer, zMSGQ_OBJECT_CONSTRAINT_INFORMATION, FALSE );
   }
...
zwTZHLPCXD_ReadRobhelpFile( zVIEW vSubtask, zHelpMapInfo **pHelpInfo )
{
   ...
   // Get the name of the robohelp report file (= import file)
   // from zeidon object.
   nRC = GetStringFromAttribute( szSourceFileName, vTZHLPCXO, "Root",
                                 "ImportFile" );
   if ( nRC < 0 )
   {
      zwTZHLPCXD_ErrMsg( vSubtask, "zwTZHLPCXD_ReadRobhelpFile",
   ...
   if ( hFile == -1 )
   {
      zstrcpy( msg, "Cannot find Robohelp file: " );
      zstrcat( msg, szSourceFileName);
      zwTZHLPCXD_ErrMsg( vSubtask, "zwTZHLPCXD_ReadRobhelpFile",
                         "SysOpenFile", msg, zMSGQ_REQUIRED_DATA_ITEM_ERROR,
                         nRC );
      nRC = zCALL_ERROR;
      pHelpInfo = 0;
      return( nRC );
   ...
   // Create structure to take the help mapping information and
   // initialize it.
...
zwTZHLPCXD_ExitDialog( zVIEW vSubtask )
{
   ...
   if ( nRC == 1 )
   {
      ...
      nRC = MessagePrompt( vSubtask, "HL00011", "Help Context Mapping",
                           pBuffer, 0, zBUTTONS_YESNOCANCEL,
                   ...
                   zPCHAR cpcMsgText, zLONG lMsgType, zSHORT returnCode )
{
   ...
   if ( vSubtask == 0 )
   {
      zstrcpy( errMsg, "zwTZHLPCXD_ErrMsg | " );
      zstrcat( errMsg, "zMSGQ_OBJECT_CONSTRAINT_ERROR | " );
      zstrcat( errMsg, "Parameter Checking: " );
      zstrcat( errMsg, "Parameter vSubtask is null. |" );
      TraceLineI( errMsg, nRC );
   }

   zstrcpy( errMsg, cpcErrObj );
   zstrcat( errMsg, "\nReturn code = " );
   zltoa( returnCode, rcStr );
   zstrcat( errMsg, rcStr );
   zstrcat( errMsg, "\n" );
   zstrcat( errMsg, cpcMsgText );

   // Open Error Message Box and view error message.
   nRC = MessageSend( vSubtask, "HL00001", cpcFct, errMsg, lMsgType, FALSE );
   if ( nRC < 0 )
   {
      zstrcpy( errMsg, "HL00001 | zwTZHLPCXD_ErrMsg | " );
      zstrcat( errMsg, "zMSGQ_OBJECT_CONSTRAINT_ERROR | " );
      zstrcat( errMsg, "MessageSend: " );
      zstrcat( errMsg, "Error processing error message." );
      TraceLineI( errMsg, nRC );
   }
   ...
   {
      zstrcat( errMsg, cpcFct );
      zstrcat( errMsg, " | " );
      zltoa( lMsgType, rcStr );
      zstrcat( errMsg, rcStr );
      zstrcat( errMsg, " | " );
      zstrcat( errMsg, errMsg );
      zstrcat( errMsg, ": " );
      TraceLineI( errMsg, returnCode );
   }
