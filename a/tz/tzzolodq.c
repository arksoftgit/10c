...
zwTZZOLODD_InitializeDialog( zVIEW vSubtask )
{
   ...
   if ( nRC >= 0 )
   {
      GetStringFromAttribute( szLOD_Name, wXferO, "QueryValues", "QueryObjectName" );
      if ( szLOD_Name[ 0 ] != 0 )
      {
         GetViewByName( &vXLP, "TaskLPLR", vSubtask, zLEVEL_TASK );
         GetStringFromAttribute( szFileName, vXLP, "LPLR", "MetaSrcDir" );
...
//  Refresh and Set Caption for Attribute View Window
   ...
   if ( vTZZOLODO )
   {
      GetStringFromAttribute( szCaption, vTZZOLODO, "LOD_Entity", "Name" );
      zstrcat( szCaption, " Attributes" );
...
zwTZZOLODD_LoadLODList( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szLodName, vTZLODS, "W_MetaDef", "Name" );
      if ( *szLodName == 'q' )
      {
...
zwTZZOLODD_OpenLOD( zVIEW vSubtask )
{
   ...
   if ( nRC < 1 )
   {
      zstrcpy( szMsg, "OpenLOD unable to access LOD file list.  Open terminated." );
...
zwTZZOLODD_OkNewLOD( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szNewName, vProfileXFER, "TZ", "Name" );
   if ( *szNewName != 'q' )
   {
...
zwTZZOLODD_SaveLOD( zVIEW vSubtask )
{
   ...
   SysReadZeidonIni( -1, "[Query]", "DBMS", szQueryDBMS_Type);
   ...
   if ( *szQueryDBMS_Type == 'N')
   {
      ...
      SysReadZeidonIni( -1, "[Query]", "ServerExec", szFileName);
      if ( *szFileName == 0 )
      {
         zstrcpy( szMsg, "The ServerExec entry in the ZEIDON.INI must exist for Query DBMS 'N'." );
      ...
      GetStringFromAttribute( szOrigDBMS_Name,
                                 ...
                                 zVIEW vLOD_ViewRoot )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szCurrentEntityName,
      ...
      while ( nRC >= zCURSOR_SET )
      {
         GetStringFromAttribute( szCurrentAttributeName,
         ...
         else
         {
            ...
            SetAttributeFromAttribute( vLOD_ViewRoot,   "QueryFrameAttribute", "AttributeName",
                                       vLOD_ViewRecurs, "ER_AttributeRec", "Name" );
            GetStringFromAttribute( szPrompt, vLOD_ViewRecurs, "LOD_EntityParent", "Name" );
            GetStringFromAttribute( szTitle, vLOD_ViewRecurs, "ER_AttributeRec", "Name" );
            zstrcat( szPrompt, szTitle );
...
zwTZZOLODD_QueryFrameSetUp( zVIEW vSubtask )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szEntityName, vLOD_ViewRoot, "QueryFrameAttribute", "EntityName" );
      GetStringFromAttribute( szAttributeName, vLOD_ViewRoot, "QueryFrameAttribute", "AttributeName" );
