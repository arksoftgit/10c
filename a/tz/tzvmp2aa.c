...
fnFindDefine( zPCHAR pchName, zLONG lType )
{
   ...
   zsprintf( pchName, "[%4d]:%s", g_qDefines[ k ].lType, g_qDefines[ k ].pchName );
}
...
TraceStatementExpression( zCPCHAR cpcMsg, zCPCHAR cpcExpression, zLONG lExprIdx )
{
   ...
   TraceLine( "TraceStatementExpression: %s   Lth: %d", cpcMsg, lExprIdx );
...
#define ADDEXPRESSIONENTRYTOSTRING( p1, p2, p3, p4, p5, p6 ) \
        AddExpressionEntryToString( szLocal2Code,\
           &lLocal2Code, szLocal2Text, &lLocal2Text,\
           p1, p2, p3, p4, p5, p6 )

#define ADDCOMMAEXPRESSIONTOSTRING\
        AddExpressionEntryToString( szLocal2Code,\
           &lLocal2Code, szLocal2Text, &lLocal2Text,\
           qOPERATOR, qNUMERIC, qTINTEGER, "", qCOMMA, 0 )
...
//    is called to build the Operation meta entity in the xpg before
//    we actually get to the point of sucessfully parsing an operation.
//    this is done to allow some data to be stored before the full operation
//    data is known. Primarily, the text associated with the actual
//    vml source code.
//
//
                   ...
                   zLONG lLineNumber )
{
   ...
   // If the parse limit is greater than sig then go back. operation
   // meta entities are only created when the signatures are being parsed.
...
//
// DESCRIPTION: AddOperationEntityToPI adds an operation to the parsed object as it
//    is being declared.  It first determines if the Operation has already
//    been defined.  If so it generates an error message.  If not then it
//    creates it.  AddOperationEntityToPI also nulls out the necessary globals that
//    deal specifically with an operation. It is passed three parms: the
//    name of the operation, the type of operation it is to be, and the
//    line number on which it was defined.  Other information is amended
//    to it later.
//
//    AddOperationEntityToPI uses two global views.  The PI (parsed instance) view
//    which it will modify, and the GS (global static) view which it will
//    use for searches.
//
zOPER_EXPORT zLONG OPERATION
                        ...
                        zSHORT nShow )             // show in Operation List
{
   ...
   // Try to position on the operation entity by name.  If we can, then we have an error
   // condition of redefining the operation.
   ...
   if ( lRC >= zCURSOR_SET )
   {
      if ( g_lParseLimit == zPARSE_LIMIT_SIGNATURES )
      {
         ...
         zstrcpy( g_szFatalErrorMsg, "Found operations with duplicate name: " );
         zstrcat( g_szFatalErrorMsg, pchOperationName );
   ...
   if ( nShow == 0 && lLth > 7 &&
        memcmp( pchOperationName, "fnLocal", 7 ) == 0 &&
        isdigit( pchOperationName[ lLth - 1 ] ) )
   {
      nGenerated = 1;
   }
   ...
   if ( lRC >= zCURSOR_SET )
   {
      ...
      // Unfortunately old PWD's sometimes have no return data type for dialog operations, so if we
      // have a dialog operation, correct the problem by forcing the return type to be short (N).
      GetStringFromAttribute( szType, g_vSourceMeta, "Operation", "Type" );
      if ( szType[ 0 ] == 'D' )
      {
   ...
   else
   {
      if ( nGenerated )
      {
         // It is a "generated operation" (e.g. fnLocalBuildQual_12). Do NOT create a meta
         // entity (using CreateMetaEntity) since we want to assign a special ZKey.
         ...
         // The first part (for the function name) is for future use when
         // we could have something different from "BuildQual".
...
ResetOperation( zPCHAR pchOperationName )
{
   ...
   // If an operation entity does exist, try to position on it.  If we can, then this
   // is an error situation of redefining the operation.
                       ...
                       zLONG  lLineNumber )      // line number on which it was defined
{
   ...
   // First we want to see if any variable exists in this instance.  First,
   // however, we need to position the static global view to this operation
   ...
   {
      // Position on the last "Variable" entity, get and increment its ID to be set as the ID
      // for the next "Variable" entity created.
   ...
   // We are all set to add the variable at this point.  We can assume that no positioning is
   // necessary since queries are made to the GS (global static) view and updates are made to the
   // PI (parsed instance) view.  Therefore, we can just add the entity at the current position.
   ...
   // Set the VMLType attribute.  If VMLType is a date, set it as a string.  Otherwise set the
   // VMLType value as specified.
   if ( lVMLType == qTDATE )
                        ...
                        zLONG  lPosition )       // insert before or after the last
{
   ...
   // First we need to add the statement to the operation.  We do this
   // depending on what the class of the operation is.
   ...
   {
      // If this is an of or otherwise, we need to add a $qBREAK
      // to the previous one if it exists.
...
//         nSetOFToSub = 1;
//         // reset back to the "OF" level since we must be on a subordinate
//         // sub_statement entity
//         ResetViewFromSubobject( g_vStatementView );
//         // add a break;
//      }
//      else
//         nSetOFToSub = 0;
   ...
   // If g_lAddNextStatementBefore == 1, create the statement before the one that is
   // there regardless of the lPosition parm.
   ...
   MoveViewToStringExpr( vSubtask, vSourceView, szLocal2Code,
                         &lLocal2Code, szLocal2Text, &lLocal2Text );
...
//         nSetOFToSub = 0;
   ...
   // If g_lAddNextStatementBefore equals 1, create the statement before the one that
   // is there regardless of the szPosition parm.
   if ( g_lAddNextStatementBefore > 0 )
   {
      // Create the statement entity.
      CreateEntity( g_vStatementView, "Statement", zPOS_BEFORE );
...
// }
   ...
   switch ( lEntryClass )
   {
      ...
      case qCONSTANT:  // qTINTEGER 1040   qTVIEW 1045
         ...
         if ( (lEntryDataType == qTSTRING    ||
               lEntryDataType == qTCHARACTER ||
               lEntryDataType == -qTSTRING   ||
               lEntryDataType == -qTCHARACTER) )
         {
                   ...
                   zLONG  lOperator )
{
   ...
   {
#ifdef DEBUG_PARSEGEN
      fnFindDefine( szOperator, g_nOperStack[ g_nStackPtr ].lOffset );
...
#ifdef DEBUG_PARSEGEN
      fnFindDefine( szOperator, lOperator );
   ...
   if ( lOperator == qLPAREN   ||
        lOperator == qLBRACE   ||
        lOperator == qOPENOPER )
   {
      InsertExprNodeValuesToEntity( vTargetView, qOPERATOR, 0, qTINTEGER, lOperator,
                                    g_lPriorityFudgeFactor, "", 0, zPOS_AFTER );
   ...
   if ( lOperator == qRBRACE ||
        lOperator == qCLOSEOPER ||
        lOperator == qCOMMA ||
        lOperator == qPERIOD ||
        lOperator == qCOLON ||
        lOperator == qOR ||
   ...
   // If the priority of the operator being sent in is greater than the priority of the
   // operator on the top of the stack, then push the "operator sent in" onto the stack.
   ...
   {
      // If the priority of the operator being sent in is less than or equal to
      // the priority or the top operator, pop the stack onto the expression
      // being built and push the incoming operator onto the stack.
      // While there are entries in the stack, and the priority of what's there
...
{
   if ( lOper == qCONDEQUAL         ||
        lOper == qLESSTHANEQUAL     ||
        lOper == qLESSTHAN          ||
        lOper == qGREATERTHANEQUAL  ||
        lOper == qGREATERTHAN       ||
        lOper == qNOTEQUAL          ||
        lOper == qNOT )
   {
                          ...
                          zPCHAR pchStringLth )
{
   ...
   // The first part of this code is very close to CreateTargetZeidonStrOp,
   // except we always use the convert operation and the max length parm is
   // set to the max string size, if there is no indexing on the target string.
   // This part creates the first part of the operation as follows:
   //    "ZeidonStringConvertFromNumber( szTargetString, lIndex, lMaxLth,"

   // The operation is always ZeidonStringConvertFromNumber.
   zstrcpy( szOperationName, "ZeidonStringConvertFromNumber" );
...
// }
   ...
   // This section is somewhat like CreateSourceZeidonStrOp, except that
   // routine adds a string variable or constant, whereas the following
   // code adds the numeric variable or constant.  If the variable or constant
   // is integer, it adds the integer value in the integer position, adds
   // a 0 in the decimal position and a NumberTypeFlag of 'I'.  If the
   // variable or constant is decimal, it adds a 0 to the integer position,
   // the decimal variable or constant to the decimal position and a
   // NumberTypeFlag of 'D'.
   // The result is that this section adds code to the previous section of
   // the form:
...
//
// DESCRIPTION: AddEntityOperWithScope creates a call to an operation
//    with the result of the operation being put in RESULT
//
// VML example:
      ...
      for ( lLoop = 1; lLoop < g_lWhereIndex; lLoop++ )
      {
         ...
         else
         {
            ...
            if ( k == 3 )  // full attribute qualification
            {
               ...
               zstrcpy( szCursorVEA, szViewName );
               zstrcat( szCursorVEA, ".cursor( \"" );
               zstrcat( szCursorVEA, szEntityName );
               zstrcat( szCursorVEA, "\" ).getStringFromAttribute( \"" );
               zstrcat( szCursorVEA, szAttributeName );
               zstrcat( szCursorVEA, "\" )" );  // close 6 open
