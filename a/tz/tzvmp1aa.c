...
ActionQ( zVIEW vSubtask, zLONG lActionRequested ) // number of the action requested
{
   ...
   switch( lActionRequested )
   {
      ...
      // This is the case where the name of the operation is known as well as the operation type,
      // but not the parm list.  What needs to be done is to update the operation name in the
      // program object and initialize values as necessary.
      case aQADDOPERATIONBYNAME:
         // Call to the add the operation.  If this was called to only position on this operation,
         // then simply do a set cursor first entity by operation name.
      ...
      // Set a flag to indicate that the locals about to be declared are in a
      // parameter list. If they are, then we need to update them here.  If they
      // are not in the parameter list, then we need to add them later.
      ...
      case aQMARKLASTVARASPARM:
         ...
         // If the Operation already exists in the source meta, then we want to check that the
         // parameter definition is in sync with the parameters defined in the source meta.
         ...
         if ( g_lAddToSrc == 0 )
         {
            ...
            if ( g_lLastTypeDeclared == qTINTEGER ||
                 ...
                 g_lLastTypeDeclared == qTDECIMAL )
            {
               ...
               szUFlag[ 0 ] = 0;
               GetStringFromAttribute( szUFlag, g_vSourceMeta, "Parameter", "UFlag" );
               SetAttributeFromString( g_lpPIView, "Variable", "UFlag", szUFlag );
            }

            GetStringFromAttribute( szWorkPFlag, g_vSourceMeta, "Parameter", "PFlag" ); // pointer flag
         }

         // If the parameter was a string, it is always a pointer ... however, the "pointer" flag
         // for a string indicates, that it is "return" (writable or mutable in javaspeak).
         if ( g_lLastTypeDeclared == qTSTRING ||
         ...
         if ( g_lAddToSrc )
         {
            ...
            // If we are adding the operation to the source file, we do not know whether the parameter
            // is return (pointer) or not.  Therefore, the PointerFlag is cleared.  If a parameter is
            // designed to be "update", it must be manually changed.
            SetAttributeFromString( g_vSourceMeta, "Parameter", "PFlag", "" );
            SetAttributeFromAttribute( g_vSourceMeta, "Parameter", "ShortDesc",
                                       g_lpPIView, "Variable", "Name" );
         }
         else
         {
      ...
      // This is the action called to declare a local variable that is either in the
      // declaration list or is in the parameter list.
      case aQDECLARELOCALVARIABLE:
         // If we are creating the operation signature where this declaration is not part
         // of a parameter list, then break because this will be declared in the body on
         // the next pass.
         //   g_lFlag == 1 implies variable is in a parmlist
         if ( g_lOperationFlag == 1 && g_lFlag != 1 ) // create
            break;

         // If we are just positioning and we are in a parameter list, then position on
         // the correct parm.
         //   g_lFlag == 1 implies variable is in a parmlist
         ...
         // If this is a local declaration within the body of the operation, then do the actual
         // declaration here.
      ...
      case  aQADDVIEWVARIABLE:
         ...
         // zstrcpy( g_szViewName, sQToken.szToken );
      ...
      // When a view is declared, the def of the view is loaded as an instance of the
      // object object.  The view to this object is then stored as the value of the view
      // variable for faster retrieval during the parse.
      case  aQLOADDEFASINSTANCE:
         ...
         // Check that we have access to the LOD via the view name.  If this is the first time
         // the LOD will be loaded, then do an activate from the meta for the OI.
         ...
         if ( lRC == zCURSOR_SET )
         {
            ...
            if ( lpTempView == 0 )
            {
               ...
               if ( lRC == 1 )
               {
                  // Set the value of the view variable to be the view pointer so we can
                  // get access later without having to know the def name (since it will
                  // be overwritten as parsing continues).
      ...
      case  aQLOADDEFFROMVOR:
         ...
         // If this is the first time the VOR will be loaded, then
         ...
         if ( lRC >= zCURSOR_SET )
         {
            ...
            if ( lRC >= 0 )
            {
               lRC = SetCursorFirstEntityByAttr( g_vLODList, "W_MetaDef", "Name",
                                                 lpTempView, "LOD", "Name", "" );
      ...
      case aQINSERTIFSTMT:
         ...
         // Loop through all statements and check for any arithmetic operators
         // (Offset = 3002, 3003, 3004 or 3005) are used.
         ...
         while ( lRC >= zCURSOR_SET )
         {
            ...
            if ( lOperType == 3002 || lOperType == 3003 ||
                 lOperType == 3004 || lOperType == 3005 )
            {
      ...
      case aQINSERTOPERATIONENDSTMT:
         ...
         {
            // We need to see if a return statement exists.  If there is a return anywhere here,
            // then we should be fine, otherwise we will need to generate one.
            ...
            else
            {
               ...
               GetStringFromAttribute( szReturnType, g_vStatementView, "Operation", "ReturnDataType" );
               SetCursorLastEntity( g_vStatementView, "Statement", "" );
               if ( szReturnType[ 0 ] == 0 || szReturnType[ 0 ] == 'O' )
               {
                  // The return type is void ... add a simple dummy return.  The -1 as the
                  // third parameter signifies that this is the last return.
                  AddReturnStatement( vSubtask, qCONSTANT, -1, NULL, zPOS_BEFORE );
               }
               else
               {
                  // The return type is not void ... add a dummy return( 0 ).  The -1 as the
                  // third parameter signifies that this is the last return.
         ...
         {
            // If we finished the end of the operation we may need to update the
            // beginning line numbers of the opers that follow.
            if ( g_lOperationFlag == 1 )  // create
               g_lOperationFlag++;
      ...
      case aQINSERTENDSTMT:
         ...
         // Skip creating END statement if it already exists.  Also, if a FORIF statement exists,
         // delete it, add an END statement and return an additional subobject level.
      ...
      case aQINSERTWHILESTMT:
         ...
         // bug XC284
         // prepare to check, whether OptimizeExpression generates
         //  another statement and a temp variable.
         //   For checking the statements, we must use g_vStatementView,
         //   which handles the recursive stuff. If there was an,
         ...
         {
            AddEntityOperWithNoScope( vSubtask, "CheckExistenceOfEntity", g_lViewID, g_szEntityName,
                                      g_lEntityID, 0, "", g_lScopeViewID );
         ...
         if ( lStatement2 > lStatement && lVariable2 > lVariable )
         {
            ...
            //     { Body of while }
            // In that case, the setting of tempvariable must be copied into the body of
            // the while loop.
            ...
            // Position on the first statement generated new and copy statements to the end
            // of the loop body backward.
            for ( l = lStatement2; l > lStatement; l-- )
            {
               ...
               // copy the statement
               SetMatchingAttributesByName( g_vStatementView, "Statement",
                                            vTempView, "Statement", zSET_ALL );
...
//       OptimizeExpression( vSubtask, vTargetView, g_vGlobalView );
      ...
      case aQSAVENAMEVIEWNAME:
         zstrcpy( g_szViewName, sQToken.szLastToken );
         g_szEntityName[ 0 ] = 0;
         break;

      case aQSAVENAMEVIEWCONSTANT:
         zstrcpy( g_szScopeViewName, sQToken.szToken );
         g_lScopeViewID = -1;
         break;
      ...
      case aQADDNAMEVIEW:
         ...
         if ( g_szEntityName[ 0 ] == 0 )
            zstrcpy( g_szEntityName, "zLEVEL_TASK" );

         BuildNameView( vSubtask, (zSHORT) lIDNumber, g_szScopeViewName,
                        (zSHORT) g_lScopeViewID, -1, g_szEntityName ); //level
      ...
      case aQADDNAMEVIEWWITHVIEW:
         ...
         BuildNameView( vSubtask, (zSHORT) lIDNumber, g_szScopeViewName,
                        (zSHORT) g_lScopeViewID, (zSHORT) lTempID, g_szEntityName ); //level
      ...
      case aQADDGETVIEW:
         GetVariableID( &lIDNumber, g_szViewName );
         BuildGetView( vSubtask, (zSHORT) g_lGetViewID, g_szViewName,
                       (zSHORT) g_lViewID, -1, "zLEVEL_TASK" );
         g_szViewName[ 0 ] = 0;
         g_lLeftDataType = qTUNKNOWN;
         g_lLeftDataClass = qTUNKNOWN;
         break;
      ...
      case aQADDGETVIEWATLEVEL:
         ...
         {
            case qSUBTASK:
               zstrcpy( g_szEntityName, "zLEVEL_SUBTASK" );
               break;
            case qTASK:
               zstrcpy( g_szEntityName, "zLEVEL_TASK" );
               break;
            case qAPPLICATION:
               zstrcpy( g_szEntityName, "zLEVEL_APPLICATION" );
               break;
            case qSYSTEM:
               zstrcpy( g_szEntityName, "zLEVEL_SYSTEM" );
               break;
            case qANY:
            default:
               zstrcpy( g_szEntityName, "zLEVEL_ANY" );
               break;
         }
         break;
      ...
      case aQADDGETVIEWATLEVELU:
         GetVariableID( &lIDNumber, g_szViewName );
         BuildGetView( vSubtask, (zSHORT) g_lGetViewID, g_szViewName,
                       (zSHORT) g_lViewID, -1, g_szEntityName );
         g_szViewName[ 0 ] = 0;
         g_szEntityName[ 0 ] = 0;
         g_lLeftDataType = qTUNKNOWN;
         g_lLeftDataClass = qTUNKNOWN;
      ...
      case aQADDGETVIEWATLEVELFROMLEVEL:
         ...
         BuildGetView( vSubtask, (zSHORT) g_lGetViewID, g_szViewName,
                       (zSHORT) g_lViewID, (zSHORT) lTempID, g_szEntityName );
         g_szViewName[ 0 ] = 0;
         g_szEntityName[ 0 ] = 0;
         g_lLeftDataType = qTUNKNOWN;
         g_lLeftDataClass = qTUNKNOWN;
      ...
      case aQADDINCLUDEBEFORE:
         ...
         AddEntityOperWithEntity( vSubtask, "IncludeSubobjectFromSubobject", g_lViewID,
                                  g_szEntityName, g_lEntityID, g_szScopeViewName, g_lScopeViewID,
      ...
      case aQADDEXCLUDE:
         ...
         AddEntityOperWithNoScope( vSubtask, "ExcludeEntity", g_lViewID, g_szEntityName, g_lEntityID,
                                   qDEFINE, "zREPOS_AFTER", lResultID );
         g_szViewName[ 0 ] = 0;
         g_szEntityName[ 0 ] = 0;
         break;
      ...
      case aQADDEXCLUDEBEFORE:
         ...
         lResultID = AddResult( vSubtask );
         AddEntityOperWithNoScope( vSubtask, "ExcludeEntity", g_lViewID, g_szEntityName, g_lEntityID,
                                   qDEFINE, "zREPOS_BEFORE", lResultID );
         g_szViewName[ 0 ] = 0;
         g_szEntityName[ 0 ] = 0;
         break;
      ...
      case aQADDCREATESTMTAFTER:
         // break the entity qualifier up into its two component parts
         // the View and the Entity Name
         SplitQualifier( sQToken.szLastToken,  // token to be split
                         g_szViewName,         // string to store view in
                         g_szEntityName,       // string to store Entity
                         g_szAttributeName );  // String to store attribute
         VerifyQualifier( vSubtask, qENTITYQUALIFIER, g_szViewName, &g_lViewID,
                          g_szEntityName, &g_lEntityID, g_szAttributeName, &g_lAttributeID );
         lResultID = AddResult( vSubtask );
         AddEntityOperWithNoScope( vSubtask, "CreateEntity", g_lViewID, g_szEntityName, g_lEntityID,
                                   qTINTEGER, "zPOS_AFTER", /*"3",*/ lResultID );
         break;
      ...
      case aQADDCREATESTMTBEFORE:
         ...
         lResultID = AddResult( vSubtask );
         AddEntityOperWithNoScope( vSubtask, "CreateEntity", g_lViewID, g_szEntityName, g_lEntityID,
                                   qTINTEGER, "zPOS_BEFORE", lResultID );
         break;
      ...
      case aQADDCREATESTMTFIRST:
         ...
         lResultID = AddResult( vSubtask );
         AddEntityOperWithNoScope( vSubtask, "CreateEntity", g_lViewID, g_szEntityName, g_lEntityID,
                                   qTINTEGER, "zPOS_FIRST", lResultID );
         break;
      ...
      case aQADDDELETESTMT:
         ...
         VerifyQualifier( vSubtask, qENTITYQUALIFIER, g_szViewName, &g_lViewID,
                          g_szEntityName, &g_lEntityID, g_szAttributeName, &g_lAttributeID );
         lResultID = AddResult( vSubtask );
         AddEntityOperWithNoScope( vSubtask, "DeleteEntity", g_lViewID,
                                   g_szEntityName, g_lEntityID, /*0,*/
                                   qTINTEGER, /*"",*/ "zPOS_AFTER", lResultID );
         break;
      ...
      case aQADDDELETESTMTFIRST:
         ...
         AddEntityOperWithNoScope( vSubtask, "DeleteEntity", g_lViewID,
                                   g_szEntityName, g_lEntityID, /*0,*/
                                   qTINTEGER, /*"",*/ "zREPOS_FIRST", lResultID );
         break;
      ...
      case aQADDDELETESTMTLAST:
         ...
         AddEntityOperWithNoScope( vSubtask, "DeleteEntity", g_lViewID,
                                   g_szEntityName, g_lEntityID, /*0,*/
                                   qTINTEGER, /*"",*/ "zREPOS_LAST", lResultID );
         break;
      ...
      case aQADDDELETESTMTPREVIOUS:
         ...
         AddEntityOperWithNoScope( vSubtask, "DeleteEntity", g_lViewID,
                                   g_szEntityName, g_lEntityID, /*0,*/
                                   qTINTEGER, /*"",*/ "zREPOS_PREV", lResultID );
         break;
      ...
      case aQADDDELETESTMTNONE:
         ...
         AddEntityOperWithNoScope( vSubtask, "DeleteEntity", g_lViewID,
                                   g_szEntityName, g_lEntityID, /*0,*/
                                   qTINTEGER, /*"",*/ "zREPOS_NONE", lResultID );
         break;
      ...
      case aQINSERTOPERATIONCALL:
         ...
         switch ( lRC )
         {
            ...
            case 1:
               sprintf( szErrorMsg, " Parameter Number %ld", lError );
         ...
         if ( lRC == 0 )
            AddStatementEntityToPI( vSubtask, qEXPRESSION, sQToken.lLastLineNo,
      ...
      case aQADDASSIGNMENTOPERATOR:
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR,
                                  0, qTINTEGER, "", sQToken.lToken, 0 );  // actual operator
         break;
      ...
      case aQINSERTEXPRESSIONSTMT:
      {
         ...
         // First we want to Dump the stack to be sure all of the operators have been placed in
         // the expression.
         AddOperatorToView( vSubtask, g_vGlobalView, -1 );

         // Create another expression entity off of the global g_lpExprView, this is so that we
         // have an expression to optimize into.
         CreateEntity( g_lpExprView, "Expression", zPOS_AFTER );

         // Get a view to the new expression called target view so that we have a view to the
         ...
         // Count the number of Error entities in the error object. If there are more than zero,
         // then errors have occurred in the current parse.  If there are errors, then there is
         // no need to build the current expression and put it into the XPG.
         //
         // NOTE:  This is probably slow.  When making changing for performance, maybe this
         ...
         if ( lCountErrors == 0 )
         {
            ...
            // If the optimize was ok and an expression node exists for view target, then add
            // the expression statement using target as the expression for this statement.
            if ( lRC == 0 &&
         ...
         // Now we need to be sure the expression is all cleaned up and set up for the new
         // expression that could be built based on the upcoming parse.
      ...
      // Add the name of an operation to an expression.  At this point we have the name of an
      // operation that needs to be verified.  Operations need to be looked up in a particular
      // order. First we look for a LOCALOPERATION (i.e. an operation that has been defined
      // within the same source member that we are parsing).  If it is not a LOCALOPERATION,
      // then in order we look for OBJECT, GLOBAL, and ZEIDON operations.  Get Operation
      // Class does all of this for us.
      case aQADDOPERATIONNAMETOEXPR:
      // if ( zstrcmp( g_szOperationName, "SetCursorNextEntityHierarchical" ) == 0 )
      //    TraceLineS( "", "" );

         zstrcpy( g_szOperationName, sQToken.szLastToken );
      ...
      case aQADDOCEATOEXPRASTARGET:
         ...
         // Verify that the parts given are valid within the view given.  Note that if all of
         // the "parts" are literal, then we can determine the data type that we need to deal
         // with.  It will be passed back as a side effect through AttributeID.
         lDataClass = VerifyQualifier( vSubtask, qATTRIBUTEQUALIFIER, g_szViewName, &g_lViewID,
                                       g_szEntityName, &g_lEntityID, g_szAttributeName, &g_lAttributeID );

         // If all the parts are literal, then AttributeID was set to the data type of the
         // attribute.
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qATTRIBUTEQUALIFIER, g_lLeftDataClass,
                                  g_lLeftDataType, sQToken.szToken, 0, 0 );
         break;

      case aQADDNOTOPERATORTOEXPR:
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                  qTINTEGER, "", sQToken.lToken, 0 );  // actual operator
      ...
      case aQADDVARIABLEASTARGET:
         ...
         // If we don't get one then that's an error.  We cannot have a define for a target
         // of an expression.
         if ( lIDNumber < 0 )
         {
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qVARIABLE, g_lLeftDataClass,
                                  g_lLeftDataType, "", lIDNumber, 0 );
         break;

      // Add the left element of an expression (like: x = ...) which must be a string.
      // Otherwise, we would not have the "offset". If we do then the problem is that
      // we have a bad data type for this variable.
      case aQADDVARIABLEOFFSETASTARGET:
         ...
         // Do a consistency check against string or text.  If the parm type is void, we
         // won't do the check.
         if ( g_lLeftDataType != qTVOID )
         {
            lRC = CompareDataTypesForConsistency( g_lLeftDataType, g_lLeftDataClass,
                                                  qTSTRING, qTEXT );
      ...
      case aQADDCONDOPERATORTOEXPR:
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                  qTINTEGER, "", sQToken.lLastToken, 0 );  // actual operator
      ...
      case aQADDCONDEQUALTOEXPR:
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                  qTINTEGER, "", qCONDEQUAL, 0 );  // actual operator
      ...
      case aQADDRBRACETOEXPR:
         // Get the mini-expression that is the string and offset and copy it into a temp
         // string replacing it with a temp id.
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                  qTINTEGER, "", sQToken.lToken, 0 );
      ...
      case aQADDRETURNSTATEMENT: // dks ... RETURN
         ...
         if ( lRC == -1 )
         {
            ...
            // Add a default return value according to function type.
      ...
      case aQSETUPWHERE:
      {
         ...
         zstrcpy( g_sWhereLeft[ 0 ].szText, g_szViewName );
         zstrcat( g_sWhereLeft[ 0 ].szText, "." );
         zstrcat( g_sWhereLeft[ 0 ].szText, g_szEntityName );
      ...
      // We are using g_sWhereLeft.lLevel/g_sWhereRight.lLevel to store the number of
      // opening/closing parens.  Thus we get an effect, that unnecessary parens,
      // like (( V.E.A = 123 )) are optimized away.
      ...
      case aQADDWHEREVARIABLE:
         ...
         g_sWhereRight[ g_lWhereIndex ].lDataType =
                         GetVariableDataType( g_sWhereRight[ g_lWhereIndex ].lOffset );

         g_sWhereRight[ g_lWhereIndex ].lElementClass = qVARIABLE;

         g_sWhereRight[ g_lWhereIndex ].lDataClass =
                      GetClassBasedOnType( g_sWhereRight[ g_lWhereIndex ].lDataType );
         break;
      ...
      case aQADDWHERECONSTANT:
         ...
         zstrcpy( g_sWhereRight[ g_lWhereIndex ].szText, sQToken.szToken );
         break;
      ...
      case aQADDWHERELEFTAQ:
         ...
         // Check that the ViewName of the AQ is the same as the ViewName of the Activate
         // ViewName, which was stored in first entry of g_sWhereLeft.
         // This is only valid for Activate operations, so also check if it is an Activate
         // and then turn off the indicator.
         if ( g_bActivate )
         {
            g_bActivate = FALSE;
         ...
         lRC = VerifyQualifier( vSubtask, qATTRIBUTEQUALIFIER, g_szViewName, &g_lViewID,
                                g_szEntityName, &g_lEntityID, g_szAttributeName, &g_lAttributeID );
         if ( lRC == qENTITYANDATTRIBUTEARECONSTANT )
         {
            // If a valid attribute, the type was stored in AttributeID as a side effect
            // for performance
         ...
         zstrcpy( g_sWhereLeft[ g_lWhereIndex ].szText, sQToken.szLastToken );
         break;
      ...
      case aQADDWHERELEFTEQCOND:
         ...
         g_sWhereLeft[ g_lWhereIndex ].lOffset = 0;
         zstrcpy( g_sWhereLeft[ g_lWhereIndex ].szText, sQToken.szToken );
         break;
      ...
      case aQADDWHERERIGHTEQ:
         ...
         lRC = VerifyQualifier( vSubtask, qENTITYQUALIFIER, g_szViewName, &g_lViewID,
                                g_szEntityName, &g_lEntityID, g_szAttributeName, &g_lAttributeID );
      ...
      // A comma was found. This is a comma to separate parameters in an operation call.  It adds
      // the operator, then increments the global parm index pointer to the next type to be sure
      ...
      case aQADDCOMMATOEXPR:
         ...
         if ( g_lParmListIndex > 0 )
         {
            ...
            // If Zero, then we do not know what type as the operation that is being called
            // has yet to be defined so we have to assume that the type given is unknown.
            else
            if ( g_lParmList[ g_lParmListIndex ] == 0 )
               g_lLeftDataType = qTUNKNOWN;
         }
      ...
      case aQADDVARIABLEOFFSETTOEXPR:
         ...
         // If there was no variable of this name then it must be an error define values
         // cannot be indexed, therefore, this cannot be a define.
         if ( lIDNumber < 0 )
         {
            // Try to get a define id for it anyway to give a better error message.
            // If an attempt was made to index it, this is not allowed.
         ...
         // Compare the data types for consistency.  If the parm type is void, we won't do
         // the check.
         if ( g_lLeftDataType != qTVOID )
         {
            lRC = CompareDataTypesForConsistency( g_lLeftDataType, g_lLeftDataClass,
                                                  lVarDataType, lDataClass );
            if ( lRC == qNOMATCHONTYPE )
            {
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qVARIABLE, 0,
                                  lVarDataType, "", lIDNumber, 0 );
         break;
      ...
      case aQADDDEFINETOEXPR:
         ...
         if ( g_lParmListIndex > 0 )
         {
            // If the entry at index is negative, then we have a pointer to some data type,
            // but a define cannot be negative ... so give an error.
            if ( g_lParmList[ g_lParmListIndex ] < 0 )
            {
               SyntaxError( vSubtask, eQDEFINEPUTWHEREVARIABLEMUSTBE, sQToken.szToken );
               break;
            }
            // else, if we are building an unknown paramter list,
            // put this entry in and assume it is correct.
            else
            if ( g_lParmList[ g_lParmListIndex ] == 0 )
            {
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qDEFINE, lDataClass,
                                  lVarDataType, sQToken.szToken, lDefineZKey, 0 );
         break;
      ...
      case aQADDVARIABLETOEXPR:
         ...
         if ( lIDNumber < 0 )
         {
            ...
            if ( g_lParmListIndex > 0 )
            {
               ...
               // Otherwise, if we are building an unknown paramter list, put this entry in
         ...
         else  // it was a variable after all
         {
            ...
            {
               // If the entry at index is negative, then we have a pointer to some data type.
               // Set the id to negative to encode this information.
               if ( g_lParmList[ g_lParmListIndex ] < 0 )
               {
               ...
               // If the Parameter List is expecting a BYTE (character), then we do not alter the type.  If however,
               // the parameter wants a string, we need to change the type to string here.  5/20/93
            ...
            {
               lRC = CompareTargetTypeForConsistency( g_lTargetDataType, g_lLeftDataType, g_lLeftDataClass,
                                                      lVarDataType, lDataClass );
            ...
            if ( lUnaryOperator == qMINUS )
            {
               ...
               if ( lVarDataType == qTINTEGER || lVarDataType == qTDECIMAL )
                  ...
                  SyntaxError( vSubtask, eQINVALIDUNARYOPERATOR,
      ...
      case aQADDAQTOEXPR:
         ...
         {
            // If a valid attribute, the type was stored in AttributeID as a side effect for
            // performance.
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qATTRIBUTEQUALIFIER,
                                  lDataClass, lType, sQToken.szToken, 0, 0 );

//       // The following Compare and SytaxError code was commented out
//       // by DonC on 11/10/94, because consistency should be left up
...
//         }
      ...
      case aQADDCONSTANTTOEXPR:
         ...
         if ( g_lParmListIndex > 0 )
         {
            // If the parameter is to be returned, it is an error, since
            // we are dealing here with a constant.  A parameter is
            // to be returned if its value is negative.
            // The only exception is for a string constant (-1042).  This
            // shouldn't really be, but at present all our Zeidon operation
            // definitions use the returned flag.  Later only when the
            // operations are corrected, this exception can be removed.
            //
            // HH, 10.11.98
            // I went trough all the Zeidon Operations and checked
            // the string parameters, so that a string parameter would
            // only have "Return" flagged, if it *IS* a return parameter.
            // As a consequence, the following code is taken out,
 ...
 //              && g_lParmList[ g_lParmListIndex ] != -1042 )
            ...
            if ( g_lParmList[ g_lParmListIndex ] == qTSHORT &&
                 sQToken.lToken == qTINTEGER )
            {
               sQToken.lToken = qTSHORT;
            }

            // If it expects a VIEW and we received an INTEGER, then change it to a VIEW.
            if ( g_lParmList[ g_lParmListIndex ] == qTVIEW &&
                 sQToken.lToken == qTINTEGER )
            {
         ...
         // Don't allow character types, meaning all single characters must be defined as strings
         // with double quotes.
         ...
         // Compare the data types for consistency.  If the parm type is void, we won't do the
         // check.  Also if the constant is null (""), we won't do the check.
         if ( g_lLeftDataType != qTVOID &&
              sQToken.szToken[ 0 ] != '"' && sQToken.szToken[ 1 ] != '"' )
         {
            lRC = CompareTargetTypeForConsistency( g_lTargetDataType, g_lLeftDataType, g_lLeftDataClass,
                                                   sQToken.lToken, lDataClass );
         ...
         {
            zstrcpy( sQToken.szToken, "null" );  // we cannot compare a view to 0
            AddExprNodeEntityToView( vSubtask, g_vGlobalView, qCONSTANT, 0, sQToken.lToken,
                                     sQToken.szToken, 0, g_lParmListExt[ g_lParmListIndex ] );
         }
         else
         {
            AddExprNodeEntityToView( vSubtask, g_vGlobalView, qCONSTANT, 0, sQToken.lToken,
      ...
      case aQADDCOLONTOEXPR:
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                  qTINTEGER, "", sQToken.lToken, 0 ); // COLON
         break;

      // This is to add a Left paren to an Operation Call.
      // Add the name of the operation to an expression.
      // At this point we have the name of an operation that needs to be verified.
      // Operations need to be looked up in a particular order. First we look for
      // a LOCALOPERATION. That is, an operation that has been defined within the
      // same source member that we are parsing. If it is not a LOCALOPERATION,
      // then in order we look for OBJECT GLOBAL ZEIDON. Get Operation Class does
      // all of this.  Once we are here, we need to check the first parameter to
      // see what the look up order is.
      ...
      case aQADDOPENOPERTOEXPR:
      {
         ...
         // Default the search order to one since this is what will be used in
         // any error situation.
         lSearchOrder = 1;

         // Determine the lookup order. The first parm in is szToken.  If it is not
         // a variable, then use search order 1.
         if ( sQToken.lToken == qVARIABLE )
         {
            zLONG lID;

            // Get the variable id.  If the variable does not exist, then don't bother reporting
            // it since the next phase of inserting the variable will handle this.
            ...
            if ( lRC != -1 )
            {
               ...
               if ( lRC == qTVIEW )
               {
                  // OK, we have a view.  See if it is a known view.  If the value parameter
                  // contains a view ptr, then this is a known view.
      ...
      //    TraceLineS( "", "" );
         ...
         GetOperationInformation( vSubtask,
                                  ...
                                  &lType, szType,     // type of oper
                                  &vSource,           // view to assoc. src
                                  g_szOperationName,  // name (sending in)
                                  lSearchOrder,       // (from above)
                                  (zLONG) lpTempView ); // LODview if needed

         // If the operation could not be located, then we have an error since all operations
         // should be known at this point.
         if ( g_lOperationClass < 0 )
         {
            SyntaxError( vSubtask, eQUNKNOWNOPERATION, g_szOperationName );

            // Set the flag that indicates an unknown operation has been encountered so that
            // the aQINSERTOPERATIONCALL function will not try to create the operation.
            g_lOperationUnknownFlag = 1;

            // add it anyway to avoid errors
            // AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATIONCALL, 0,
            //                          0, g_szOperationName, 0, 0 );
            break;
         }
         ...
         lReturnType = GetOperationParmListFromSource( g_lParmList, g_lParmListExt, szReturnVarName, vSource );
      ...
      //    lType = lReturnType;                              // parms implemented!  dks 2010.07.14
         ...
         // DGC 10/23/95
         // Added check for Global Operation.  If the op is a global op, then create an
         // ExternalOperation entity.
         // This is for creating make files later on.  As of yet the parser and generator
         ...
         if ( g_lOperationClass == qOBJECTOPERATION )  // 4003
         {
            ...
            if ( lRC != zCURSOR_SET )
            {
               ...
               // Set the MetaDLL_Name to the LOD.DLL_Name if that attribute is not null, or
               // to LOD.Name if the DLL_Name attribute is null.
         ...
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATIONCALL, g_lOperationClass,
                                  lType, sQToken.szLastToken, g_lOperationZKey, 0 );
         ...
         if ( g_lLeftDataType != qTVOID && lType != qTVOID )  // qTVOID:1101 ... dks 2010.07.01  added lType check
         {
            ...
            // Allow return type string to be cast to an integer variable so that a pointer
            // would be assigned to an integer.
            if ( lRC == qNOMATCHONTYPE &&
      ...
      case aQADDCLOSEOPERTOEXPR:
         ...
         if ( g_lOperationUnknownFlag == 0 )
         {
            ...
            if ( g_lParmList[ 0 ] > 1 &&
                 g_lParmListIndex != g_lParmList[ 0 ] - 1 )
            {
      ...
      case aQVERIFYACTIVATEOBJECT:
         ...
         // Save g_lAddNextStatementBefore, because it will be altered during Activate processing.
         // It is restored in InsertActivateIntoCurrentOperation.
      ...
      case aQSAVEACTIVATESINGLE:
         zstrcpy( g_szLoadControl, "zSINGLE" );
         break;

      case aQSAVEACTIVATEMULTIPLE:
         zstrcpy( g_szLoadControl, "zMULTIPLE" );
         break;

      case aQSAVEACTIVATEROOTONLY:
         zstrcpy( g_szLoadControl, "zACTIVATE_ROOTONLY" );
         break;

      case aQSAVEACTIVATEROOTONLYMULTIPLE:
         zstrcpy( g_szLoadControl, "zACTIVATE_ROOTONLY_MULTIPLE" );
         break;

      case aQSAVEACTIVATECONTINUE:
         zstrcpy( g_szLoadControl, "zACTIVATE_MULTIPLE_CONTINUE" );
         break;

      case aQSAVEACTIVATEROOTONLYCONTINUE:
         zstrcpy( g_szLoadControl, "zACTIVATE_ROOTONLY_CONTINUE" );
         break;

      case aQSAVEACTIVATESINGLEFORUPDATE:
         zstrcpy( g_szLoadControl, "zSINGLE_FOR_UPDATE" );
         break;
      ...
      case aQSETRESTRICTION:
      {
         ...
         zstrcpy( g_sWhereLeft[ 0 ].szText, sQToken.szToken );
      ...
      case aQADDCHECKEXIST:
         ...
         AddEntityOperWithNoScope( vSubtask, "CheckExistenceOfEntity", g_lViewID,
                                   g_szEntityName, g_lEntityID, 0, "", g_lScopeViewID );
         // Get the data type.
         ...
         // Add the entry.
         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qVARIABLE, lDataClass,
                                  lVarDataType, "", g_lScopeViewID, 0 );
         if ( g_lWhereOp[ g_lWhereIndex ] == qEXISTS )
            AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                     qTINTEGER, "", qCONDEQUAL, 0 );
         else
            AddExprNodeEntityToView( vSubtask, g_vGlobalView, qOPERATOR, 0,
                                     qTINTEGER, "", qNOTEQUAL, 0 );

         AddExprNodeEntityToView( vSubtask, g_vGlobalView, qCONSTANT, qNUMERIC,
                                  qTINTEGER, "0", 0, 0 );
         //g_lWhereIndex = 1;
         break;
      ...
      case aQSETVARIABLETEXT:
         ...
         zstrncpy( szGlbVariable, sQToken.szToken, sizeof( szGlbVariable ) );
         break;
...
ForceExprIntoParens( zVIEW vExpression )
{
   ...
   lRC = SetEntityCursor( vExpression,  "ExprNode", "Level",
                          zPOS_NEXT | zQUAL_INTEGER | zQUAL_LESS_THAN,
                          &lMaxLevel, "", "", 0, "", 0 );

   // If we did not find one, this means that the expression is enclosed in
   // parens and we can just get out now as it is in acceptable form
   if ( lRC < zCURSOR_SET )
      return( 0 );

   // If we found one, this means that the expression is NOT enclosed in parens and it needs to be.
   ...
   // Now, loop throught the rest of the expression and increment the priorities by 10 so that they
   // are set as they would be if the expr was coded with ( ) in the first place.
...
ParameterCheck( zVIEW g_lpExprView, zPLONG plErrorPara )
{
   ...
   // Sometimes we get an empty view with no expressions.  In that case, return 0.  Don't
   // question why.
   if ( nRC < zCURSOR_SET )
      return( 0 );
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      // A type of VIEW in the parameter definition or as the parameter matches only type
      // VIEW or INTEGER.
      ...
      if ( (lDataType == qTVIEW) &&
           (lParmListType != qTVIEW) && (lParmListType != qTINTEGER) )
      ...
      if ( (lParmListType == qTINTEGER || lParmListType == qTSHORT || lParmListType == qTDECIMAL) &&
           lDataType == qTSTRING )
      {
         *plErrorPara = lParm;
         return( 1 );
      ...
      // If the target type is string and the source type is short or decimal, we fire an error message.
      // We allow source type integer variable (NOT constant), because sometimes it is used as an address
      // in VML (ugly but real).
      // Integer Constant with value 0 is ok.
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( lElementClass == qOPERATOR )
         {
            ...
            // We found an Operator, not Comma or CloseParen, so the parameter is an expression.
            // Then the parameter definiton must not be of pointer type.
         ...
         {
            // When we get here, we have found anything different from qOPERATOR, so we got an
            // expression.  Check for parameter definition NOT being of type pointer.
            if ( (g_lParmListExt[ lParm ] & POINTER_BIT) != 0 )
            {
               *plErrorPara = lParm;
               return( 1 );
...
CheckParaWithSource( void )
{
   ...
   ConvertVMLTypeToZeidonType( cZeidonType, g_lLastTypeDeclared );
   cMetaType[ 0 ] = 0;
   GetStringFromAttribute( cMetaType, g_vSourceMeta, "Parameter", "DataType" );
...
fnAddDefaultReturn( zVIEW vExpression )
{
   ...
   GetStringFromAttribute( szReturnType, g_lpPIView, "Operation", "ReturnDataType" );
   if ( szReturnType[ 0 ] == 0 || szReturnType[ 0 ] == 'O' )
...
OperationNameValid( zPCHAR szToken )
{
   ...
   if ( zstrcmp( g_szSourceMetaObjectName, "TZZOLODO" ) == 0 )
   {
      ...
      GetStringFromAttribute( szLodName, g_lpPIView, "VML_XSource", "MetaName" );
...
UnaryOperatorConstant( zVIEW vSubtask )
{
   ...
   // If we got an unary operator preceding the constant, then the constant must be of
   ...
   // Just add the MINUS to the token.  Admittedly not a beautiful solution, but otherwise we
   // would have to rewrite too much.
   // To handle the unary operator as qUNARYMINUS as with a variable would not be sufficient,
   // because unary minus as a function parameter would not work any more.
   // Note, that the old (erroneous) grammer handled unary operators in the lex part.
   zstrcpy( szSave, sQToken.szToken );
   zstrcpy( sQToken.szToken, "-" );
   zstrncat( sQToken.szToken, szSave, sizeof( sQToken.szToken ) - 1 );
   return( 0 );
}
...
ForNoWhere( zVIEW vSubtask )
{
   ...
   else // is qNOEACH || qCONSTANT || qVARIABLE
   {
      ...
      AddEntityOperWithNoScope( vSubtask, "CheckExistenceOfEntity", g_lViewID,
                                g_szEntityName, g_lEntityID, 0, "", lResultID );
   }
...
GenerateSetCursorWithScope( zVIEW vSubtask, zLONG lScope )  // java???
{
   ...
   if ( g_szGenLang[ 0 ] == 'J' )
   {
      // In the case of a straight "SET CURSOR NEXT" or "SET CURSOR PREV" with no parms,
      // we need to do a setNext rather than a setNextContinue because we may have done
      // a qualified SET CURSOR FIRST/LAST and then want to continue on through the list
   ...
   */
   ...
   if ( g_szScopeEntityName[ 0 ] == '\1' )
      ...
      AddEntityOperWithScope( vSubtask, pchFunction, g_lViewID, g_szEntityName,
                              g_lEntityID, 0, "", 0,
