...
ShowTrans_ExecuteTrans( zVIEW vDBH_Subtask )
{
   ...
   *szArgControl = 0;
   ...
   else
   {
      ...
      zstrcat( szFileName, lpViewOD->szOperLibname );
   ...
   if ( nRC < 0 )
   {
      zstrcpy( szArgControl, "CallTran error" );
      goto EndOfFunction;
   }
   ...
   nRC = 0;
   sprintf( szArgControl, "OK/RC = %ld", lRC );
                      ...
                      zVIEW  vT )
{
         ...
         lpViewEntity = (LPVIEWENTITY) SysGetPointerFromHandle( lpViewEntity->hNextHier ) )
   {
      ...
      szEntityName[ nRC ] = 0;
      zstrcat( szEntityName, lpViewEntity->szName );
...
void TestDec( zVIEW v, zPCHAR psz1, zPCHAR psz2 )
{
   ...
   zsprintf( szMsg, "Result of comparing Int1 = '%s' and Dec2 = '%s' = %d",
             psz1, psz2, nRC );
...
void TestInt( zVIEW v, zPCHAR psz1, zPCHAR psz2 )
{
   ...
   zsprintf( szMsg, "Result of comparing Dec1 = '%s' and Int2 = '%s' = %d",
             psz1, psz2, nRC );
   TraceLineS( "DGC ", szMsg );
...
#if 0
   //GetStringFromAttribute( sz, v, "SalesOrder", "DonTest" );
   //TraceLineS( "DonTest =====> ", sz );
   GetStringFromAttribute( sz, v, "SalesOrder", "DGTest" );
