...
static void fnMyHelp(int nHelpType)
{
  ...
  zstrcpy( szMsg, "TZSTARTO - back ground process to start global ops\n");
  zstrcat( szMsg, "Copyright (C) Ton Beller GmbH, Quinsoft Inc. 1997.\n\n");

  if (nHelpType == LINE_HELP)
  {
    zstrcat( szMsg, "\n");
    zstrcat( szMsg, "Call Syntax:\n");
    zstrcat( szMsg, "  TZSTARTO LPLR DLL Operation\n");
    zstrcat( szMsg, "\n");
  }
  else
  {
    ...
    zstrcat( szMsg, "Call Syntax:\n");
    zstrcat( szMsg, "  TZSTARTO LPLR DLL Operation\n");
    zstrcat( szMsg, "    LPLR - name of the Zeidon application (LPLR)\n");
    zstrcat( szMsg, "    DLL - name of the DLL with the Global Op\n");
    zstrcat( szMsg, "    Operation - name of the Global Op to run\n");
    zstrcat( szMsg, "\n");
  }
...
fnProcessCommandParm(int argc, zCHAR **argv, T_GLOBAL_OP *pGO)
{
  ...
  zstrncpy( pGO->szApp, argv[ 0 ], 32 );
  zstrncpy( pGO->szDLL, argv[ 1 ], 255 );
  zstrncpy( pGO->szOp,  argv[ 2 ], 32 );
