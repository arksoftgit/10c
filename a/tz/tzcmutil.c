              ...
              zPCHAR pchDirectorySpec, int nEntityType )
{
   ...
   if ( sh != INVALID_HANDLE_VALUE )
   {
      do
      {
         TraceLineS( "fnBuildMetas ", fd.cFileName );
         zstrcpy( szMetaName, fd.cFileName );
         zstrcpy( szFullName, pchDirectorySpec );
         szFullName[ zstrlen( szFullName ) - 5 ] = 0;
         zstrcat( szFullName, szMetaName );
         hFile = (HFILE) SysOpenFile( vLPLR, szFullName, COREFILE_READ );
         SysGetFileDateTime( hFile, szTimestamp );
      ...
      // AnsiUpper( szMetaName );
         ...
         if ( nRC != zCURSOR_SET ) // New Meta
         {
            ...
            {
               GetStringFromAttribute( szMetaName, vOI_View,
...
fnGetAndSetZKey( zVIEW vSubtask, zVIEW vLPLR, zPCHAR szFileName )
{
   ...
   if ( ulZKey == 0 )
   {
      zstrcpy( szERR_Msg, "Root entity, " );
      zstrcat( szERR_Msg, SRC_CMOD[ lType -2000 ].szOD_ROOT );
      zstrcat( szERR_Msg, ", ZKey is 0 in File: " );
      zstrcat( szERR_Msg, szFileName );
      TraceLineS( "Zeidon Configuration Management", szERR_Msg );
      return;
   }
   ...
   if ( lType == zREFER_VOR_META )
   {
      ...
      GetStringFromAttribute( szTaskLPLR_Name, vTaskLPLR, "LPLR", "Name" );
      GetStringFromAttribute( szLPLR_Name, vLPLR, "LPLR", "Name" );
   ...
   if ( lType == zREFER_DOMAINGRP_META || lType == zREFER_GOPGRP_META )
   {
      ...
      SetCursorFirstEntityByInteger( vTarget, "W_MetaType", "Type",
                                     (zLONG) nTargetType, "" );

      // For each of the sub-metas (eg "Domains") in the meta we just
                                  ...
                                  "" ) )
      {
         ...
         SetAttributeFromAttribute( vTarget,
                                    "W_MetaDef", "Name",
                                    vOI_View,
                                    REFER_CMOD[ nTargetType - 2000 ].szOD_ROOT,
   ...
   SetAttributeFromAttribute( vLPLR, "W_MetaDef", "Desc", vOI_View,
                              SRC_CMOD[ lType - 2000 ].szOD_ROOT, "Desc" );
   DropObjectInstance( vOI_View );
...
/////////////////////////////////////////////////////////////////////////////
void
fnGetTaskOI_ListName( zVIEW vSubtask, zPCHAR pchTaskOI_ListName )
{
   zCHAR szTaskID[ 18 ];
   zLONG lTaskID;
   ...
   zstrcpy( pchTaskOI_ListName, "__CM." );
   zstrcpy( pchTaskOI_ListName + 5, szTaskID );
}
...
// lTaskID = SysGetTaskFromView( vSubtask );
   ...
   while ( (nRC == zCURSOR_SET) || (nRC == zCURSOR_SET_NEWPARENT) )
   {
      ...
      if ( GetViewByName( &vWork, szViewName,
                         ...
                         zSHORT nNewInd )
{
   ...
   {
      // Adding New Meta
      if ( SetCursorFirstEntityByString( vLPLR, "W_MetaDef", "Name",
   ...
   else
   {
                                         ...
                                         szMetaOI_Name, "" ) == zCURSOR_SET )
      {
         ...
         } while ( SetCursorNextEntityByString( vLPLR, "W_MetaDef",
                                                "Name", szMetaOI_Name,
                                                "" ) == zCURSOR_SET );
      }
   }
   ...
   if ( nDupFound == 0 )
   {
           ...
           lType != zSOURCE_GO_META )
      {
         ...
         SetCursorFirstEntityByInteger( vTempLPLR, "W_MetaType",
                                        "Type", lType, "" );
         if ( SetCursorFirstEntityByString( vTempLPLR, "W_MetaDef", "Name",
                                          szMetaOI_Name, "" ) == zCURSOR_SET )
         {
            do
            {
               if ( CompareAttributeToInteger( vTempLPLR, "W_MetaDef",
                                               "CPLR_ZKey",
                                               lMetaOI_ZKey ) != 0 )
               {   // Duplicately Named Meta exists
                  GetVariableFromAttribute( &szNewName, 0,
            ...
            } while ( SetCursorNextEntityByString( vTempLPLR, "W_MetaDef",
                                                   "Name", szMetaOI_Name,
                                                   "" ) == zCURSOR_SET );
         }
   ...
   if ( nDupFound == 1 )
   {
      zstrcpy( szMsg, "A component of type " );
      zstrcat( szMsg, szNewName );
      zstrcat( szMsg, " already exists\n" );
      zstrcat( szMsg, "with the name of the component you are trying to add," );
      zstrcat( szMsg, "\n       " );
...
fnCleanupActiveMetas( zVIEW vActiveMetas, zVIEW vZeidonCM )
{
   ...
   {
      GetIntegerFromAttribute( &lTaskID, vActiveLookup,
                               "W_MetaDef", "TaskID" );
      if ( lTaskID ) // any non-zero task id is valid (Win9x has negative IDs)
      {
         ...
         if ( nRC != 1 )
         {
            GetStringFromAttribute( szViewName, vActiveLookup,
                                    "W_MetaDef", "CM_ViewName" );
            zstrcat( szViewName, ".u" );
            if ( GetViewByName( &vMeta, szViewName, vZeidonCM,
                                zLEVEL_SUBTASK ) > 0 )
            {
