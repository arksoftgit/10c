...
UpdateControlEvents( zVIEW vSubtask )
{
   ...
   do
   {
      ...
      GetStringFromAttribute( szAction, vEvents, "EventAct", "Tag" );
      if ( szAction[ 0 ] &&
...
UPD_RadioUpdateSubControl( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vWork, "Radio", "Tag" );
...
UPD_RadioGroupOK( zVIEW vSubtask )
{
   ...
   do
   {
      ...
      if ( lKey == zRADIOBUTTON_CONTROL )
      {
         ...
         GetStringFromAttribute( szBlob, vWork, "Radio", "DefaultIndicator" );
         GetStringFromAttribute( szBlob + 1, vWork, "Radio", "Value" );
...
PostbuildSpreadSheet( zVIEW vSubtask )
{
   ...
   if ( lSplitRow )
   {
      ...
      zltoa( lSplitRowAtCol, szNumber );
      SetCtrlText( vSubtask, "SplitRowAtCol", szNumber );
      zltoa( lIndent2ndLine, szNumber );
                ...
                zVIEW    vDialogW )
{
        ...
        CheckExistenceOfEntity( vDialogW, "ActEvent" ) == zCURSOR_SET )
   {
      ...
      if ( CheckExistenceOfEntity( vDialogW, "ActWndEvent" ) == zCURSOR_SET )
      {
         GetStringFromAttribute( szTag, vDialogW, "Window", "Tag" );
         zsprintf( szMsg,
   ...
   if ( CheckExistenceOfEntity( vDialogW, "ActOpt" ) == zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szTag, vDialogW, "ActOpt", "Tag" );
      zsprintf( szMsg,
...
// CreateTemporalMetaEntity( vSubtask, vDialog, "Control", zPOS_AFTER );
   ...
   zstrcpy( sWork2, "ListCtl" );
   zstrcat( sWork2, sWork1 );
...
// if ( zstrcmp( szWork, "Left" ) == 0 )
   ...
   CB_GetSelectedString( vSubtask, "Vertical", szWork );
...
TextTestIt( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vControl, "EventAct" ) == zCURSOR_SET )
      GetStringFromAttribute( szTag, vControl, "EventAct", "Tag" );
   else
      zstrcpy( szTag, "No Action Specified" );
...
// GetStringFromAttribute( szTag, vControl, "Control", "Tag" );
// TraceLineS( "TextAddAction TZCONTROL: ", szTag );
// GetStringFromAttribute( szTag, vWork, "Control", "Tag" );
...
UPD_EDIT_OK( zVIEW vSubtask )
{
   ...
   if ( szBlob[ 3 * sizeof( zLONG ) ] )
   {
      ...
      zstrcpy( szBlob + (3 * sizeof( zLONG )) + lFormatLth + 1, szWork );
...
UPD_BITMAPBTN_OK( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( BMP.szDLLName, vWork, "Bitmap", "DLL_Name" );

   // if there is input for UP
   GetStringFromAttribute( szName, vWork, "Bitmap", "BitmapUP" );
   ...
   if ( szName[ 0 ] )
   {
     ...
     if ( pchBMPName[ 0 ] == 0 )
     {
       ...
       zstrcpy( BMP.szBMPUpOrIconName, szName );
     }
     else
     {
       // if ID given, take it
       zstrcpy( BMP.szBMPUpOrIconName, ++pchBMPName );
   ...
   GetStringFromAttribute( szName, vWork, "Bitmap", "BitmapDN" );
   if ( szName[ 0 ] )
   {
     // has the line an ID separated by ';'
     pchBMPName = szName;
     ...
     if ( pchBMPName[ 0 ] == 0 )
     {
       ...
       zstrcpy( BMP.szBMPDownName, szName );
     }
     else
     {
       // if ID given, take it
       zstrcpy( BMP.szBMPDownName, ++pchBMPName );
   ...
   else
   {
     ...
     if ( pchBMPNameFirst != NULL )
       zstrcpy( BMP.szBMPDownName, pchBMPNameFirst );
   }
   ...
   GetStringFromAttribute( szName, vWork, "Bitmap", "BitmapFO" );
   if ( szName[ 0 ] )
   {
     // has the line an ID separated by ';'
     pchBMPName = szName;
     ...
     if ( pchBMPName[ 0 ] == 0 )
     {
       ...
       zstrcpy( BMP.szBMPFocusName, szName );
     }
     else
     {
       // if ID given, take it
       zstrcpy( BMP.szBMPFocusName, ++pchBMPName );
   ...
   else
   {
     ...
     // if we had a name for the previous BMP, use it here
     if ( pchBMPNameFirst != NULL )
       zstrcpy( BMP.szBMPFocusName, pchBMPNameFirst );
   }

   // if there is input for Disabled
   GetStringFromAttribute( szName, vWork, "Bitmap", "BitmapDA" );
   if ( szName[ 0 ] )
   {
     // has the line an ID separated by ';'
     pchBMPName = szName;
     ...
     if ( pchBMPName[ 0 ] == 0 )
     {
       ...
       zstrcpy( BMP.szBMPDisabledName, szName );
     }
     else
     {
       // if ID given, take it
       zstrcpy( BMP.szBMPDisabledName, ++pchBMPName );
   ...
   else
   {
     ...
     // if we had a name for the previous BMP, use it here
     if ( pchBMPNameFirst != NULL )
       zstrcpy( BMP.szBMPDisabledName, pchBMPNameFirst );
   }
...
*/
      ...
      GetStringFromAttribute( szTempString, vWindow, "Window", "Tag" );
...
LIST_ACT_Cancel( zVIEW vSubtask )
{
   ...
   GetViewByName( &vDialogW, "TZWINDOW", vSubtask, zLEVEL_TASK );
   GetViewByName( &vWork, "TZPNCTWO", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szParentFlag, vWork, "TZPNCTWO",
...
LIST_ACT_DeleteAction( zVIEW vSubtask )
{
   ...
   if ( SetCursorFirstSelectedEntity( vDialogW, "Action", "" ) == zCURSOR_SET )
   {
      ...
      zstrcpy( szMsg, "OK to delete selected Actions" );
...
MapDisableAction( zVIEW vSubtask, zVIEW vDialog, zBOOL bFromOI )
{
   ...
   else
   {
      ...
      CB_GetSelectedString( vSubtask, "DisableAction", szDisableAction );
...
UPD_ACT_CheckAction( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vDialogW, "Action", "Tag" );
   if ( szTag[ 0 ] == 0 )
   {
...
UPD_ACT_ActionInclude( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szParentFlag,
...
//    SetAttributeFromString( vWork, "ComboBox", "SortByListValue", "N" );
   ...
   GetStringFromAttribute( szStringSubtype, vWork, "ComboBox", "Subtype" );
...
UPD_CHECK_OK( zVIEW vSubtask )
{
   ...
   CB_GetSelectedString( vSubtask, "Style", szStyle );
   ...
   GetStringFromAttribute( szCtrlBOI, vWork, "CheckBox", "CheckOn" );
   nLth = 0;
   while ( szCtrlBOI[ nLth ] )
      nLth++;

   nLth++;
   GetStringFromAttribute( &szCtrlBOI[ nLth ], vWork,
                           "CheckBox", "CheckOff" );
   while ( szCtrlBOI[ nLth ] )
      nLth++;
...
// TraceLineI( "BEFORE Common OK ", lSubtype );
   ...
   } //endif ( fnPainterCall( zMSG_ISREPORT, vSubtask, 0, 0 ) == 0 )
...
ControlOperationEdit( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTag, vDialogC, "EventAct", "Tag" );
      SetCursorFirstEntityByString( vDialogW, "Action", "Tag", szTag, "" );
...
//x   GetStringFromAttribute( szTempString, vVOR, "LOD", "Name" );
      GetStringFromAttribute( szTempString, vControl,
                              "CtrlMapView", "Name" );
      RetrieveViewForMetaList( vSubtask, &vLOD_List, zREFER_LOD_META );
//    zstrcpy( szTempString, "TZZOLODO" );   use if modifying tzpnolad.pwd
...
//
      ...
      /* get the scoping entity from the control's CtrlBOI and set it into
...
UPD_OUTLINER_ObjectOK( zVIEW vSubtask )
{
   ...
   RetrieveViewForMetaList( vSubtask, &vLOD_List, zREFER_LOD_META );
   GetStringFromAttribute( szTempString, vVOR, "LOD", "Name" );
   SetCursorFirstEntityByString( vLOD_List, "W_MetaDef",
                                 "Name", szTempString, "" );
      ...
      /* we need to keep the order and the TOP_VIEW information
         ...
         structure as follows:
            ...
            -------   ---------------------------------------------------
             ...
             8-39     Context
      */
      GetStringFromAttribute( szTempString, vOL, "ER_AttributeRec", "Desc" );
#ifdef DEBUG_TAG
      ...
      /* we need to keep the order and the TOP_VIEW information
         ...
         structure as follows:
            ...
            -------   ---------------------------------------------------
             ...
             8-39     Context
      */
      GetStringFromAttribute( szTempString, vAttr, "Attrib", "Tag" );
      if ( szTempString[ 0 ] == 'Y' )
      {
         if ( nTagCnt )
         {
      ...
      GetStringFromAttribute( szTempString + 8, vAttr, "Attrib", "Context" );
...
UPD_OUTLINER_SelectOK( zVIEW vSubtask )
{
   ...
   GetViewByName( &vOL, "TZOUTLINER", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szEntityName, vWork, "Outliner", "EntityName" );
...
UPD_OUTLINER_OK( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szTempString, vWork, "OutlinerGrp", "DualMode" );
   if ( szTempString[ 0 ] )
      lSubtype |= OLS_DUAL;

   GetStringFromAttribute( szTempString, vWork, "OutlinerGrp", "MultiSelect" );
   if ( szTempString[ 0 ] )
      lSubtype |= OLS_MULTISEL;

   GetStringFromAttribute( szTempString, vWork, "OutlinerGrp", "AllChildren" );
   if ( szTempString[ 0 ] )
      lSubtype |= OLS_OPENALLCHILDREN;

   GetStringFromAttribute( szTempString, vWork, "OutlinerGrp", "DisplayText" );
   if ( szTempString[ 0 ] == 0 )
      lSubtype |= OLS_NOSHOW_TOPNODE;

   GetStringFromAttribute( szTempString, vWork, "OutlinerGrp", "RootIndicator" );
   if ( szTempString[ 0 ] == 0 )
   ...
   GetStringFromAttribute( szTempString, vWork, "OutlinerGrp", "Operation" );
   if ( szTempString[ 0 ] != 0 )
   {
      ulLth = zstrlen( szTempString ) + 1;
      GetStringFromAttribute( szTempString + ulLth, vWork,
                              "OutlinerGrp", "DLL_Name" );
                      ...
                      zVIEW vControl )
{
   ...
   if ( (nRC = SetCursorFirstEntity( vOL, "LOD_EntityParent",
                                     0 ) ) == zCURSOR_SET )
   {
      nPrevIdx = 0;
      ...
      while ( nRC == zCURSOR_SET )
      {
         ...
         if ( SetCursorFirstEntity( vTempOL, "LOD_AttributeRec", 0 ) >= 0 &&
             (bSelected = GetSelectStateOfEntityForSet( vOL,
                                                         "LOD_EntityParent",
                                                         1 )) == TRUE )
         {
            ...
            lSubtype = 0;
            // if this entity is recursive, then set the control's
            // Subtype and Text attributes.
            ...
            SetAttributeFromInteger( vControl, "Control",
                                     "Subtype", lSubtype );
            ...
            nRC = SetCursorFirstEntity( vTempOL, "ER_AttributeRec",
                                        "LOD_EntityParent" );
            while ( nRC >= 0 )
            {
               GetStringFromAttribute( szTempString, vTempOL,
                                       "ER_AttributeRec", "Desc" );
               if ( GetSelectStateOfEntityForSet( vTempOL,
                                                  "ER_AttributeRec", 1 ) )
               {
                  szTempString[ 2 ] = 'Y';
               }
               ...
               if ( szTempString[ 0 ] == 'Y' || szTempString[ 2 ] == 'Y' )
               {
                  ...
                  TraceLineS( "Including(TempLOD) Selected ER_AttributeRec Name = ",
                              pch );
#endif
               }

               nRC = SetCursorNextEntity( vTempOL, "ER_AttributeRec",
                                          "LOD_EntityParent" );
            }

            // Loop backwards through the control to see what the
            // scoping entity is ... set that in the CtrlBOI.
            ...
            if ( CheckExistenceOfEntity( vTempCtrl, "ControlDef" ) == 0 )
            {
               ...
               GetStringFromAttribute( szWorkString, vOL,
                                       "LOD_EntityParent", "Name" );
               /* if szTempString = LOD_EntityParent.Name then this
                  entity is the root entity and we do not want a
                  scoping entity */
            ...
            {
               GetStringFromAttribute( szTempString, vTempCtrl,
                                       "CtrlMapRelatedEntity", "Name" );
            }

            SetAttributeFromBlob( vControl, "Control",
                                  "CtrlBOI", szTempString,
                                  (zUSHORT) zstrlen( szTempString ) );
         }
...
UPD_BMPBTN_BrowseUP( zVIEW vSubtask )
{
   ...
   if ( UPD_BmpBtnBrowseFileName( vSubtask, szFileName,
                                  sizeof( szFileName ) ) == 1 )
   {
...
{
   // Build a list of Window Events, Control Events and Menu Options that
   // use an Action.
  ...
  if ( !ComponentIsCheckedOut( vSubtask, vDialog, zSOURCE_DIALOG_META ) )
   ...
   GetStringFromAttribute( szActionTag, vDialog, "Action", "Tag" );
...
CommonSelectedDetailPostbuild( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( &lSubtype, vControl, "Control", "Subtype" );
   fnPainterCall( zMSG_GET_COMMON_DETAIL_FLAGS, vSubtask,
                  0, (zPCHAR) &ulCommonFlags );
...
// TraceLineI( "BEFORE Common OK ", lSubtype );
   ...
   fnPainterCall( zMSG_UPDATE_COMMON_DETAIL, vSubtask,
                  (zVIEW) lSubtypeX, (zPCHAR) lSubtype );
   fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, (zPCHAR) 1 );
   return( 0 );
}
...
{
   fnPainterCall( zMSG_UPDATE_COMMON_DETAIL, vSubtask,
                  (zVIEW) 0xffffffff, (zPCHAR) 0xffffffff );
   fnPainterCall( zMSG_ENABLEPAINTERWINDOWS, vSubtask, 0, (zPCHAR) 1 );
   return( 0 );
}
...
ComboBoxItemPostBuild( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szText, vControl, "Control", "Text" );
