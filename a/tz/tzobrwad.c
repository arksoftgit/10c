...
zwTZOBRWAD_CheckShowDesc( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szFlag, vProfile, "OBR", "ShowDesc" );
      if ( szFlag[ 0 ] == 'Y' )
...
// SysMutexLock( vSubtask, "ZEIDON MUTEX -- ANCHOR BLOCK", 0, 0 );
         ...
         lpTask = SfGetNextTask( lpTask ) )
   {
      ...
      // Set a default task title, for background processes,
      ...
      if ( bTitle == FALSE )
      {
         ...
         if ( lpTask->szTaskTitle[ 0 ] )
            zstrcpy( szTaskTitle, lpTask->szTaskTitle );
         else
         {
            zsprintf( szTaskTitle, "<Task 0x%08x, %s>",
                      SysGetHandleFromPointer( lpTask ),
...
// GetViewByName( &vProfile, "ProfileXFER", vSubtask, zLEVEL_TASK );
   ...
   GetIntegerFromAttribute( (zPLONG) &vObject, vBrowser,
                            szlName, szlID );
   ...
   if ( lpViewOI && lpViewOI->hRootEntityInstance )
   {
      ...
      while ( lpWorkViewEntityCsr )
      {
         if ( lpWorkViewEntityCsr->hEntityInstance == UNSET_CSR )
         {
            LPVIEWENTITY lpViewEntity =
                  (LPVIEWENTITY) SysGetPointerFromHandle( lpWorkViewEntityCsr->hViewEntity );

            CheckExistenceOfEntity( vObject, lpViewEntity->szName );
         }

         lpWorkViewEntityCsr =
                    (LPVIEWENTITYCSR) SysGetPointerFromHandle( lpWorkViewEntityCsr->hNextHier );
      }
      ...
      lpRootViewEntityCsr = (LPVIEWENTITYCSR)
                                    SysGetPointerFromHandle( lpViewCsr->hRootViewEntityCsr );
      lpFirstSelectedInstance = (LPSELECTEDINSTANCE)
                                SysGetPointerFromHandle( lpViewCsr->hFirstSelectedInstance );
      ...
      lpEntityInstance = (LPENTITYINSTANCE)
                                    SysGetPointerFromHandle( lpViewOI->hRootEntityInstance );
      lEntities   = 0;
      lAttributes = 0;
      szInd[ 4 ]  = 0;
      nRC = 0;
      while ( nRC == 0 && lpEntityInstance )
      {
              ...
              pchShowHidden[ 0 ] == 'Y' )
         {
            ...
            {
               lpLinkedEntityInstance = (LPENTITYINSTANCE)
                                    SysGetPointerFromHandle( lpEntityInstance->hNextLinked );

               while ( lpLinkedEntityInstance != lpEntityInstance &&
                       lpLinkedEntityInstance->u.nInd.bHidden )
               {
                  lpLinkedEntityInstance = (LPENTITYINSTANCE)
                              SysGetPointerFromHandle( lpLinkedEntityInstance->hNextLinked );
               }
            ...
            if ( lpFirstSelectedInstance )
            {
               for ( lpSelectedInstance = lpFirstSelectedInstance;
                     lpSelectedInstance;
                     lpSelectedInstance = (LPSELECTEDINSTANCE)
                        SysGetPointerFromHandle( lpSelectedInstance->hNextSelectedInstance ) )
               {
                  if ( SysGetPointerFromHandle( lpSelectedInstance->xEntityInstance ) ==
                                                           lpEntityInstance )
                  {
                     szInd[ 1 ] = 's';
                     break;
            ...
            while ( lpWorkViewEntityCsr )
            {
               if ( SysGetPointerFromHandle( lpWorkViewEntityCsr->hEntityInstance ) ==
                                                           lpEntityInstance )
               {
                  szInd[ 3 ] = '>';
                  break;
               ...
               lpWorkViewEntityCsr = (LPVIEWENTITYCSR)
                                   SysGetPointerFromHandle( lpWorkViewEntityCsr->hNextHier );
            }
            ...
            szAttrib[ nIdx ] = 0;
            zstrcat( szAttrib, lpViewEntity->szName );
                                   ...
                                   SysGetPointerFromHandle( lpViewAttrib->hNextOD_Attrib ) )
            {
               ...
               {
                  lpViewEntityCsr = (LPVIEWENTITYCSR)
                                   SysGetPointerFromHandle( lpViewCsr->hFirstOD_EntityCsr );
...
BuildObjectInstanceForView( zVIEW vSubtask )
{
   ...
   GetViewByName( &vBrowser, szlTZOBRWOO, vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szID, vBrowser,
...
CommitOI( zVIEW vSubtask )
{
   ...
   if ( pchFileName[ 0 ] )
   {
      ...
      GetStringFromAttribute( szFileType, vBrowser, szlRoot, "FileType" );
      ...
      {
         strcpy( szMsg, "ObjectInstance successfully written to File: " );
         strcat( szMsg, pchFileName );
...
// GetViewByName( &vProfile, "ProfileXFER", vInstance, zLEVEL_TASK );
   ...
   if ( vObject->hViewCsr )
   {
      ...
      if ( lpViewEntityCsr )
      {
                 ...
                 (lpViewAttrib->bHidden == FALSE || pchShowHidden[ 0 ] == 'Y') )
            {
               ...
               else
               {
                  GetAttributeLength( &uLth, vWorkObject,
                                      lpViewEntity->szName,
                                      lpViewAttrib->szName );
                  zstrcpy( szAttrib, "(Blob, lth=" );
                  zltoa( uLth, szAttrib + 50 );
                  zstrcat( szAttrib, szAttrib + 50 );
                  zstrcat( szAttrib, ")" );
               }
               SetAttributeFromString( vInstance, szlAttrib, szlValue,
                                       szAttrib );
               if ( (lpViewAttrib->szDerivedOper[ 0 ] == 0 || lShowDerived) &&
                    (nAttrSet == 0 ||
                    (nAttrSet == 1 && lpViewAttrib->cType == 'S')) )
         ...
         zsprintf( szAttrib, "0x%8lx", (zLONG) lpEntityInstance );
         SetAttributeFromString( vInstance, szlEntity, szlInstancePointer,
                                 szAttrib );

         if ( lpEntityInstance->u.nInd.bCreated )
            SetAttributeFromInteger( vInstance, szlEntity,
                                     szlCreatedFlag, 1 );
         else
            SetAttributeFromInteger( vInstance, szlEntity,
                                     szlCreatedFlag, 0 );
...
// GetViewByName( &vProfile, "ProfileXFER", vSubtask, zLEVEL_TASK );
   ...
   if ( lpViewAttrib->cType != 'B' )
   {
      ...
      while ( lpDispAttrib )
      {
         ...
         lpDispAttrib = (LPVIEWATTRIB)
                                    SysGetPointerFromHandle( lpDispAttrib->hNextOD_Attrib );
      }
...
ViewOI_Prebuild( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szInstanceID, vBrowser,
                           szlName, szlObjectInstanceID );
...
SetTraceLevel( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &lpTask, vTZOBRWOO,
                            szlTask, szlPointer );
   GetIntegerFromAttribute( &lTraceLevel,
                            vTZOBRWOO, "Root", "TraceLevel" );
...
GetTraceLevel( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &lpTask, vTZOBRWOO,
                            szlTask, szlPointer );
   SetAttributeFromInteger( vTZOBRWOO, "Root", "TraceLevel",
                            (zLONG) lpTask->nDBHandlerTraceLevel );
   return( 0 );
}
...
fnDeleteExcludeEntityInstance( zVIEW vSubtask, zBOOL bDelete )
{
   ...
   if ( lpTwinInstance == 0 )
   {
      lpTwinInstance = (LPENTITYINSTANCE)
                                      SysGetPointerFromHandle( lpEntityInstance->hPrevTwin );
      while ( lpTwinInstance && lpTwinInstance->u.nInd.bHidden )
         lpTwinInstance = (LPENTITYINSTANCE)
                                        SysGetPointerFromHandle( lpTwinInstance->hPrevTwin );
   }

   zstrcpy( szMessage, "OK to continue with " );
   if ( bDelete )
      zstrcat( szMessage, "Delete " );
   else
      zstrcat( szMessage, "Exclude " );

   lpViewEntity = (LPVIEWENTITY) SysGetPointerFromHandle( lpEntityInstance->hViewEntity );
   zstrcat( szMessage, "Entity '" );
   zstrcat( szMessage, lpViewEntity->szName );
   zstrcat( szMessage, "'" );
...
PrebuildCreateEntity( zVIEW vSubtask )
{
   ...
   {
      while ( DeleteEntity( vInstance, szlWkEntity, zREPOS_NEXT ) ==
                                                               zCURSOR_SET );
   }

   CreateEntity( vInstance, szlWkEntity, zPOS_AFTER );
   SetAttributeFromString( vInstance, szlWkEntity, szlName,
                           lpViewEntity->szName );
...
CreateEntityInstance( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &vID, vInstance, szlName, szlID );
   vID = (zVIEW) SysGetPointerFromHandle( vID );
   GetStringFromAttribute( szEntityName, vInstance, szlWkEntity, szlName );
   CreateViewFromViewForTask( &vWork, vID, vSubtask );
   // Find the entity and the attrib in the view object
   GetIntegerFromAttribute( (zPLONG) &lpEntityInstance,
                            vInstance, szlEntity, szlPointer );
   ...
   if ( nRC != zCALL_ERROR )
   {
      ...
      SetNameForView( vInstance, szlTZOBOIOO,
...
UpdateIndicators( zVIEW vSubtask )
{
   ...
   zsprintf( szPtr, "0x%8lx", (zLONG) lpEntityInstance );
   SetAttributeFromString( vInstance, szlEntity, szlInstancePointer,
                           szPtr );
   ...
   if ( lpViewEntityCsr->nLevel != lpEntityInstance->nLevel )
   {
      while ( lpViewEntityCsr->nLevel < lpEntityInstance->nLevel )
      {
         ...
         while ( lpWorkInstance1->nLevel != lpViewEntityCsr->nLevel )
            lpWorkInstance1 = (LPENTITYINSTANCE)
                                         SysGetPointerFromHandle( lpWorkInstance1->hParent );

         while ( lpWorkInstance2 == 0 )
         {
            lpWorkViewEntity = (LPVIEWENTITY)
                                     SysGetPointerFromHandle( lpWorkInstance2->hViewEntity );
         ...
         lpViewCsr = (LPVIEWCSR) SysGetPointerFromHandle( lpView->hViewCsr );
         lpRecursiveCsr = (LPVIEWENTITYCSR)
                                   SysGetPointerFromHandle( lpViewCsr->hFirstOD_EntityCsr );
         lpWorkViewEntity = (LPVIEWENTITY)
                                     SysGetPointerFromHandle( lpWorkInstance2->hViewEntity );
         ...
         {
            lpWorkCsr->hEntityInstance = (LPENTITYINSTANCE)
                                  SysGetHandleFromPointer( lpWorkInstance1 );
...
fnSetInstanceOwnerForName( LPVIEWNAME lpViewName, zVIEW vBrowser )
{
   ...
   if ( lpView->hViewCsr )
   {
      ...
      else
      {
         ...
         if ( lpViewCsr )
            SetAttributeFromString( vBrowser, szlName,
                                    szlInstanceOwner, szlApplication );
         else
            SetAttributeFromString( vBrowser, szlName,
                                    szlInstanceOwner, szlTask );
      }
                              ...
                              zCPCHAR cpcSpecifyColor )
{
   ...
   while ( lpViewEntity )
   {
           ...
           SysGetPointerFromHandle( lpViewEntityCsr->hViewEntity ) != lpViewEntity )
      {
         ...
         SetAttributeFromString( vTZZOHDGO, szlHD_Node, szlText,
                                 "(out of view)" );
      ...
      else
      {
         ...
         if ( lpViewEntityCsr->hEntityInstance == NULL_CSR )
         {
            ...
            if ( lpWkCsr )
               lpWkEntityInstance = (LPENTITYINSTANCE)
                                         SysGetPointerFromHandle( lpWkCsr->hEntityInstance );

            if ( lpWkCsr && lpWkEntityInstance->u.nInd.bHidden )
            {
               SetAttributeFromString( vTZZOHDGO, szlHD_Node,
                                       szlText, "(undefined)" );
            }
            else
            {
               SetAttributeFromString( vTZZOHDGO, szlHD_Node,
                                       szlText, "(null)" );
               SetAttributeFromInteger( vTZZOHDGO, szlHD_Node,
                                        szlColor, 1 );
            }
         }
         else
         {
            ...
            lpEntityInstance = (LPENTITYINSTANCE)
                                 SysGetPointerFromHandle( lpViewEntityCsr->hEntityInstance );
            lPrevTwins = 0;
            if ( lpEntityInstance )
               lpEntityInstance = (LPENTITYINSTANCE)
                                      SysGetPointerFromHandle( lpEntityInstance->hPrevTwin );
            ...
            lpEntityInstance = (LPENTITYINSTANCE)
                                 SysGetPointerFromHandle( lpViewEntityCsr->hEntityInstance );
            lpEntityInstance = (LPENTITYINSTANCE)
                                 SysGetPointerFromHandle( lpEntityInstance->hNextTwin );
            while ( lpEntityInstance )
            {
               if ( lpEntityInstance->u.nInd.bHidden == FALSE ||
                    pchShowHidden[ 0 ] == 'Y' )
            ...
            {
               zltoa( lPrevTwins + 1, szMsg );
               zstrcat( szMsg, " of " );
               zltoa( lPrevTwins + 1 + lNextTwins, szMsg + 20 );
               zstrcat( szMsg, szMsg + 20 );
               ...
               if ( cpcSpecifyColor && vFlatCompare &&
                    SetCursorFirstEntityByInteger( vFlatCompare, szlHD_Node,
                                                   szlZKey, lAbsolutePos,
                                                   0 ) >= zCURSOR_SET )
               {
                  zLONG lColor;

                  GetIntegerFromAttribute( &lColor, vFlatCompare,
                                           szlHD_Node, szlColor );
                  SetAttributeFromInteger( vTZZOHDGO, szlHD_Node,
                                           szlColor, lColor );
               // SetAttributeFromAttribute( vTZZOHDGO, szlHD_Node, szlColor,
               //                            vFlatCompare, szlHD_Node, szlColor );
               }
               else
               ...
               if ( lpDispAttrib )
               {
                  // if the display attribute is too long, we can get a
                  // a message that we don't want (text too long for the
                  // HD work object).  So instead of using
                  // SetAttributeFromAttribute, use GetVariable and
                  // SetVariable.
...
GetWKS_FileName( zPCHAR pchZeidonWKS )
{
   if ( SysGetEnvVar( pchZeidonWKS, "ZEIDON",  zMAX_FILENAME_LTH + 1 ) == 0 )
   {
      ...
      zstrcat( pchZeidonWKS, "TZCMWKS8.POR" );
      return( 0 );
   }
   return( -1 );
}
...
zwTZOBRWAD_OpenDescDisp( zVIEW vBrowser, zVIEW lpView )
{
   ...
   {
      SetCursorFirstEntityByAttr( lpView, "Application", "Name",
                                 lpView, "Name", "ApplicationName", "" );
      GetStringFromAttribute( szAppName, lpView, "Application", "Name" );
   ...
   else
   {
      ...
      GetStringFromAttribute( szAppName, vTZCMLPLO, "LPLR", "Name" );
   }

   GetStringFromAttribute( szObject, lpView, "Name", "ObjectType" );
   ...
   if ( zstrcmp( szAppName, "Zeidon_Tools" ) == 0 ||
        zstrcmp( szAppName, "Zeidon System" ) == 0 )
   {
      ...
      while ( nRC >= zCURSOR_SET &&
              nRC_cmp == 0 )
      {
         GetStringFromAttribute( szAppName2, vTZCMWKSO, "LPLR", "Name" );
      ...
      nRC_cmp = SetCursorFirstEntityByString( vTZCMWKSO, "LPLR", "Name",
                                              szAppName, 0 );

   // nRC_cmp >= 0 --> found a match.
   if ( nRC_cmp < zCURSOR_SET )
   {
      strcpy( szMsg, "Descriptions of OI cannot be shown because " );
      strcat( szMsg, "Local workstation does not have application source (Appl: " );
      strcat( szMsg, szAppName );
      strcat( szMsg, ")." );
      MessageSend( vBrowser, "CM00107", "Object Browser",
   ...
   GetStringFromAttribute( szFileName, vTZCMWKSO, "LPLR", "MetaSrcDir");

   DropView( vTZCMWKSO );
   AppendSlash( szFileName );
   strcat( szFileName, szObject );
   strcat( szFileName, ".LOD" );

   if ( SysOpenFile( vBrowser, szFileName, COREFILE_EXIST ) == -1 )
   {
      strcpy( szMsg, "The LOD-File with Descriptions does not exist: " );
      strcat( szMsg, szFileName );
      MessageSend( vBrowser, "CM00107", "Object Browser",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_WARNING, zBEEP );
      return( -1 );
   }

   nRC = ActivateOI_FromFile( &vTZZOLODO_Desc, "TZZOLODO", vBrowser,
                              szFileName, zLEVEL_TASK | zIGNORE_ERRORS );

   if ( nRC == 0 )
   {
      SetNameForView( vTZZOLODO_Desc, "TZZOLODO_Desc", vBrowser, zLEVEL_TASK );

      SetWindowActionBehavior( vBrowser, zWAB_StartModelessSubwindow,
                               "TZOBRWAD", "DescDisp" );
   }
...
ToggleShowDescFromOI( zVIEW vSubtask )
{
   ...
   else
   {
      ...
      if ( GetViewByName( &vTZOBRWAD_Desc, "TZOBRWAD_Desc", vSubtask, zLEVEL_TASK ) >= 0 )
         SetWindowActionBehavior( vTZOBRWAD_Desc,
                                  zWAB_ReturnToParent | zWAB_ProcessImmediateAction,
                                  "", "" );
...
// CreateViewFromViewForTask( 0, 0, 0 );  // start view debug
   ...
   GetViewByName( &vTZOBRWOO, szlTZOBRWOO, vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szInstanceID, vTZOBRWOO,
                           szlName, szlObjectInstanceID );
   SetNameForView( vSubtask, szInstanceID, vSubtask, zLEVEL_TASK );
...
fnSetModeEntities( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szMode, vTZOBOIOO, szlName, szlMode );
      if ( szMode[ 0 ] == 'E' )
         return( 0 );
   }
   else
...
zwTZOBRWAD_RefreshDescDisp( zVIEW vSubtask )
{
   ...
   SetCursorFirstEntityByString( vTZZOLODO_Desc, "LOD_Entity", "Name",
                                 pchGraphEntity, "LOD" );
...
BuildAttrsForGraph( zVIEW vSubtask, zCPCHAR cpcTag )
{
   ...
   if ( zstrchr( cpcTag, '1' ) )
      zstrcpy( szTag, "CompareHD1" );
   else
   if ( zstrchr( cpcTag, '2' ) )
      zstrcpy( szTag, "CompareHD2" );
   else
   {
      zstrcpy( szTag, cpcTag );
      bOI_Hier = TRUE;
   }
   ...
   while ( lpViewEntityCsr )
   {
      ...
      if ( szMode[ 0 ] == 'E' )
      {
         CreateEntity( vTZOBOIOO, szlAttrib, zPOS_AFTER );
         zstrcpy( szAttribute, "                              " );
         zstrcpy( &szAttribute[ (2 * (lpViewEntity->nLevel - 1)) ],
                  lpViewEntity->szName );
   ...
   if ( CheckExistenceOfEntity( vTZZOHDGV, pchEntityName ) > zCURSOR_NULL )
   {
      ...
      if ( szMode[ 0 ] == 'M' )
      {
         ...
         while ( lpSelectedInstance &&
                 ((LPENTITYINSTANCE)
                   SysGetPointerFromHandle( lpSelectedInstance->xEntityInstance ) !=
                                                          lpEntityInstance ||
         ...
         CreateEntity( vTZOBOIOO, szlAttrib, zPOS_AFTER );
         zsprintf( szAttribute, "Instance Ptr 0x%8lx  Key %d",
                   lpEntityInstance, lpEntityInstance->ulKey );
         SetAttributeFromString( vTZOBOIOO, szlAttrib, szlName,
                                 szAttribute );
         ...
         if ( lpSelectedInstance && lpSelectedInstance->nSelectSet )
         {
            ...
            for ( nWk3 = 1; nWk3 < 17; nWk3++ )
            {
               if ( lpSelectedInstance->nSelectSet & nWk2 )
               {
                  ...
                  zltoa( (zLONG) nWk3, &szAttribute[ 250 ] );
                  zstrcat( szAttribute, &szAttribute[ 250 ] );
                  nWk1++;
               }
         ...
         if ( lpEntityInstance->hPrevVsn )
         {
            ...
            if ( lpTempEI->u.nInd.bPrevVsnRoot == FALSE )
            {
               ...
               lpViewEntity = (LPVIEWENTITY)
                                      SysGetPointerFromHandle( lpLinkInstance->hViewEntity );

               while ( lpTempEI->u.nInd.bPrevVsnRoot == FALSE )
               {
                  lpLinkInstance = (LPENTITYINSTANCE)
                                          SysGetPointerFromHandle( lpLinkInstance->hParent );
                  lpViewEntity = (LPVIEWENTITY)
                                      SysGetPointerFromHandle( lpLinkInstance->hViewEntity );
                  lpTempEI = (LPENTITYINSTANCE)
                                         SysGetPointerFromHandle( lpLinkInstance->hPrevVsn );
               }

               lpViewEntity = (LPVIEWENTITY)
                                      SysGetPointerFromHandle( lpLinkInstance->hViewEntity );
               zstrcpy( szAttribute, "Versioned by " );
               zstrcat( szAttribute, lpViewEntity->szName );
               SetAttributeFromString( vTZOBOIOO, szlAttrib,
                                       szlName, szAttribute );
            }
            else
            {
         ...
         if ( lpEntityInstance->u.nInd.bPrevVersion )
         {
            ...
            while ( lpLinkInstance && lpLinkInstance != lpEntityInstance )
            {
               ...
               lpLinkInstance = (LPENTITYINSTANCE)
                                      SysGetPointerFromHandle( lpLinkInstance->hNextLinked );
            }
         ...
         while ( lpLinkInstance && lpLinkInstance != lpEntityInstance )
         {
            ...
            if ( lpLinkInstance->u.nInd.bHidden == FALSE ||
                 pchShowHidden[ 0 ] == 'Y' )
            {
               LPVIEWENTITY lpLinkViewEntity = (LPVIEWENTITY)
                           SysGetPointerFromHandle( lpLinkInstance->hViewEntity );

               if ( lpLinkInstance->hViewOI == lpEntityInstance->hViewOI )
               {
                  zstrcpy( szAttribute, "Internal Link to " );
                  zstrcat( szAttribute, lpLinkViewEntity->szName );
               }
               else
               {
                  LPVIEWOI lpViewOI = (LPVIEWOI)
                                           SysGetPointerFromHandle( lpLinkInstance->hViewOI );
                  LPVIEWOD lpViewOD = (LPVIEWOD) SysGetPointerFromHandle( lpViewOI->hViewOD );

                  zstrcpy( szAttribute, "External Link to " );
                  zstrcat( szAttribute, lpViewOD->szName );
                  zstrcat( szAttribute, "." );
                  zstrcat( szAttribute, lpLinkViewEntity->szName );
               }

               bDup = 0;
               if ( SetCursorFirstEntityByString( vWork, szlAttrib, szlName,
                                                  szAttribute, 0 ) == 0 )
               {
                  bDup = TRUE;
               }
         ...
         {
            CreateEntity( vTZOBOIOO, szlAttrib, zPOS_AFTER );
            zstrcpy( szAttribute, "Number of links is " );
            zltoa( lLinks, &szAttribute[ 250 ] );
            zstrcat( szAttribute, &szAttribute[ 250 ] );
            SetAttributeFromString( vTZOBOIOO, szlAttrib, szlName,
                                    szAttribute );
         }
      ...
      {
         lpViewEntity= (LPVIEWENTITY)
                   SysGetPointerFromHandle( lpViewEntityCsr->hViewEntity );
         lpViewAttrib = (LPVIEWATTRIB)
                   SysGetPointerFromHandle( lpViewEntity->hFirstOD_Attrib );
         if ( szMode[ 0 ] == 'A' )
         {
                    ...
                    SysGetPointerFromHandle( lpViewAttrib->hNextOD_Attrib ) )
            {
               ...
               if ( lpViewAttrib->cType == 'B' )
               {
                  if ( lpViewAttrib->bHidden )
                     ...
                     GetAttributeLength( &uLth, vTZZOHDGV, pchEntityName,
                                         lpViewAttrib->szName );

                  if ( uLth > 60 || uLth == 0 )
                  {
                     zstrcpy( szAttribute, "(Blob, lth=" );
                     zltoa( uLth, &szAttribute[ 50 ] );
                     zstrcat( szAttribute, &szAttribute[ 50 ] );
                     zstrcat( szAttribute, ")" );
                  }
                  else
                  {
               ...
               {
                  GetStringFromRecord( vTZZOHDGV, lpViewEntity, lpViewAttrib,
                                       szAttribute, 298 );
               ...
               {
                  szHiddenAttribute[ 0 ] = '*';
                  zstrcpy( szHiddenAttribute + 1, lpViewAttrib->szName );
               }
               else
                  zstrcpy( szHiddenAttribute, lpViewAttrib->szName );
         ...
         else
         {
            ...
            lpWkEntityCsr = (LPVIEWENTITYCSR)
                    SysGetPointerFromHandle( lpViewCsr->hRootViewEntityCsr );
            while ( (LPVIEWENTITY) SysGetPointerFromHandle( lpWkEntityCsr->hViewEntity ) !=
                                                               lpViewEntity )
            {
               lpWkEntityCsr = (LPVIEWENTITYCSR)
                                         SysGetPointerFromHandle( lpWkEntityCsr->hNextHier );
            }
            ...
            do
            {
               ...
               SetAttributeFromString( vTZOBOIOO, szlAttrib, szlValue,
                                       szAttribute );
               if ( lpWkEntityCsr->hEntityInstance ==
                                           lpViewEntityCsr->hEntityInstance )
               {
                  SetSelectStateOfEntity( vTZOBOIOO, szlAttrib, 1 );
               }
            }  while ( SetCursorNextEntity( vWork, lpViewEntity->szName,
                                            0 ) >= zCURSOR_SET );
            DropView( vWork );
         }
      }
...
zwTZOBRWAD_BuildGraphForView( zVIEW vSubtask )
{
   ...
   SetAttributeFromInteger( vTZZOHDGO, szlHD_Diagram, szlZKey,
                            (zLONG) vTZZOHDGV );
...
// GetViewByName( &vTZZOHDGV, szlTZZOHDGV, vSubtask, zLEVEL_TASK );  // debugging only
   ...
   while ( lpViewEntity )
   {
      ...
      SetAttributeFromString( vTZZOHDGO, szlHD_Node, szlDIL,
                              lpViewEntity->szName );
      SetAttributeFromInteger( vTZZOHDGO, szlHD_Node, szlZKey,
                               (zLONG) lpViewEntity->nHierNbr );
      lpViewEntity = (LPVIEWENTITY) SysGetPointerFromHandle( lpViewEntity->hNextHier );
   }
...
zwTZOBRWAD_CursorChange( zVIEW vSubtask, zSHORT nPos, zCPCHAR cpcTag )
{
   ...
   switch ( nPos )
   {
      ...
      case zENTITY_DELETEALL:
         ...
         if ( nRC )
         {
            while ( SetCursorFirstEntity( vTZZOHDGV,
                                          pchEntityName, 0 ) >= zCURSOR_SET &&
                    DeleteEntity( vTZZOHDGV, pchEntityName,
                                  zREPOS_NONE ) >= zCURSOR_UNDEFINED )
            {
               ; // Nothing needs to be done here.
            }
         }
      ...
      case zENTITY_DROP:
         nRC = CheckExistenceOfEntity( vTZZOHDGV,
                                       pchEntityName ) == zCURSOR_SET &&
               DropEntity( vTZZOHDGV, pchEntityName,
                           zREPOS_AFTER ) >= zCURSOR_NULL;
         break;
      ...
      case zENTITY_EXCLUDE:
         nRC = CheckExistenceOfEntity( vTZZOHDGV,
                                       pchEntityName ) == zCURSOR_SET &&
               ExcludeEntity( vTZZOHDGV, pchEntityName,
                              zREPOS_AFTER ) >= zCURSOR_NULL;
         break;
      ...
      case zENTITY_TEMPORAL:
         nRC = CheckExistenceOfEntity( vTZZOHDGV,
                                       pchEntityName ) == zCURSOR_SET &&
               CreateTemporalSubobjectVersion( vTZZOHDGV,
                                               pchEntityName ) > zCALL_ERROR;
         break;
...
zwTZOBRWAD_ExpandNode( zVIEW vSubtask )
{
   ...
   {
      if ( CompareAttributeToString( vTZZOHDGO, szlHD_Node, szlHidden,
                                     "Y" ) == 0 )
      {
      ...
      else
      {
         ...
         SetAttributeFromString( vTZZOHDGO, szlHD_Node, szlHidden, "N" );
         if ( SetCursorFirstEntity( vTZZOHDGO, szlHD_ChildNode, 0 ) >=
                                                         zCURSOR_SET )
         {
            do
            {
               SetAttributeFromString( vTZZOHDGO, szlHD_ChildNode, szlCollapsed,
                                       "Y" );
               SetAttributeFromString( vTZZOHDGO, szlHD_ChildNode, szlHidden,
                                       "N" );
            } while ( SetCursorNextEntity( vTZZOHDGO, szlHD_ChildNode, 0 ) >=
                                                         zCURSOR_SET );
         }
      }
   }
...
zwTZOBRWAD_DisplayLB_Click( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZOBOIOO, szlTZOBOIOO, vSubtask, zLEVEL_SUBTASK ) > 0 )
   {
      ...
      if ( szMode[ 0 ] == 'E' )
      {
         ...
         GetAddrForAttribute( &pchEntityName, vTZOBOIOO,
                              szlAttrib, szlValue );
         if ( CompareAttributeToString( vTZZOHDGO, szlHD_Node, szlDIL,
                                        pchEntityName ) != 0 )
         {
   ...
   GetStringFromAttribute( szAttribute, vTZOBOIOO, "Attrib", "Name" );
   zwTZOBRWAD_RefreshDescDisp( vSubtask );
...
zwTZOBRWAD_ChangeAttribute( zVIEW vSubtask )
{
        ...
        GetViewByName( &vTZZOHDGV, szlTZZOHDGV, vSubtask, zLEVEL_SUBTASK ) > 0 )
   {
      GetStringFromAttribute( szMode, vTZOBOIOO, szlName, szlMode );
      if ( szMode[ 0 ] == 'A' &&
           SetCursorFirstSelectedEntity( vTZOBOIOO,
                                         szlAttrib, 0 ) >= zCURSOR_SET )
      {
         GetAddrForAttribute( &pchEntityName, vTZZOHDGO,
                              szlHD_Node, szlDIL );
         GetCtrlText( vSubtask, "OI_LockEntity", szLockEntity, 33 );
         if ( szLockEntity[ 0 ] )
            pchEntityName = szLockEntity;

         GetStringFromAttribute( pchAttribName, vTZOBOIOO,
                                 szlAttrib, szlName );
         // Now that we have the entity and the attribute, make sure
         // the attribute is not a blob
         lpViewOD     = (LPVIEWOD) SysGetPointerFromHandle( vTZZOHDGV->hViewOD );
         lpViewEntity = (LPVIEWENTITY) SysGetPointerFromHandle( lpViewOD->hFirstOD_Entity );
         ...
         lpViewAttrib = (LPVIEWATTRIB) SysGetPointerFromHandle( lpViewEntity->hFirstOD_Attrib );
         while ( zstrcmp( lpViewAttrib->szName, pchAttribName ) )
            lpViewAttrib = (LPVIEWATTRIB) SysGetPointerFromHandle( lpViewAttrib->hNextOD_Attrib );

         if ( lpViewAttrib->cType != 'B' &&
              CheckExistenceOfEntity( vTZZOHDGV,
                                      pchEntityName ) == zCURSOR_SET )
         {
            if ( SetAttributeFromAttribute( vTZZOHDGV,
                                            pchEntityName, pchAttribName,
                                            vTZOBOIOO, szlEntity,
                                            szlUpdateAttr ) >= 0 )
            {
               if ( CompareAttributeToAttribute(
                                     vTZZOHDGO, szlHD_Node, szlText,
                                     vTZOBOIOO, szlAttrib, szlValue ) == 0 )
               {
                  zwTZOBRWAD_BuildTextForGraph( vSubtask );
                  RefreshCtrl( vSubtask, szlOI_Hier );
               }
               ...
               zwTZOBRWAD_BuildAttrsForGraph( vSubtask );
               GetViewByName( &vTZOBOIOO, szlTZOBOIOO, vSubtask, zLEVEL_SUBTASK );
               if ( SetCursorFirstEntityByString( vTZOBOIOO, szlAttrib,
                                                  szlName, pchAttribName,
                                                  0 ) >= zCURSOR_SET )
               {
                  SetSelectStateOfEntity( vTZOBOIOO, szlAttrib, 1 );
                  RefreshCtrl( vSubtask, "OI_ListBox1" );
...
zwTZOBRWAD_OIDisplayExit( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZZOHDGO, szlTZZOHDGO,
                       vSubtask, zLEVEL_SUBTASK ) > 0 )
   {
      DropObjectInstance( vTZZOHDGO );
   }
...
// LPTASK hTask;
   ...
   GetStringFromAttribute( szAppName, vKZAPPLOO,
   ...
   // lpTask = (LPTASK) hTask; // SysGetPointerFromHandle( hTask );
      ...
      if ( pchFileName[ 0 ] )
      {
         nRC = ActivateOI_FromFile( &vActivate, szObject, vAppSubtask,
                                    pchFileName, lOptions );
      }
      else
      {
         nRC = ActivateEmptyObjectInstance( &vActivate, szObject,
                                            vAppSubtask, lOptions );
      }
...
zwTZOBRWAD_ShowSelectSet( zVIEW vSubtask )
{
                       ...
                       vSubtask, zLEVEL_SUBTASK ) > 0 )
   {
      ...
      zstrcpy( szMsg, "Select set is currently " );
      zltoa( (zLONG) nSet, &szMsg[ 90 ] );
      zstrcat( szMsg, &szMsg[ 90 ] );
      OperatorPrompt( vSubtask, "Select Set Display", szMsg,
                      0, 0, 0, zICON_INFORMATION );
   }
...
zwTZOBRWAD_NextSelectSet( zVIEW vSubtask )
{
                       ...
                       vSubtask, zLEVEL_SUBTASK ) > 0 )
   {
      ...
      SetSelectSetForView( vTZZOHDGV, nSet );
      zstrcpy( szMsg, "Select set is now " );
      zltoa( (zLONG) nSet, &szMsg[ 90 ] );
      zstrcat( szMsg, &szMsg[ 90 ] );
      OperatorPrompt( vSubtask, "Select Set Display", szMsg,
                      0, 0, 0, zICON_INFORMATION );
                           ...
                           zBOOL bShare )
{
   ...
   if ( lpViewCsr && lpViewOI->hRootEntityInstance )
   {
      ...
      while ( lpWorkViewEntityCsr )
      {
         ...
         {
            lpViewEntity = (LPVIEWENTITY)
                                 SysGetPointerFromHandle( lpWorkViewEntityCsr->hViewEntity );
            CheckExistenceOfEntity( vObject, lpViewEntity->szName );
         }
      ...
      lpEntityInstance = (LPENTITYINSTANCE)
                                SysGetPointerFromHandle( lpWorkViewOI->hRootEntityInstance );
      lEntities   = 0;
      lAttributes = 0;
      ...
      while ( nRC == 0 && lpEntityInstance )
      {
              ...
              pchShowHidden[ 0 ] == 'Y' )
         {
            ...
            {
               if ( (LPENTITYINSTANCE)
                    SysGetPointerFromHandle( lpWorkViewEntityCsr->hEntityInstance ) ==
                                                           lpEntityInstance )
               {
                  szInd[ 3 ] = '>';
                  break;
            ...
            lpViewEntity = (LPVIEWENTITY)
                                    SysGetPointerFromHandle( lpEntityInstance->hViewEntity );
            zstrcat( szAttrib, lpViewEntity->szName );
            SetAttributeFromString( vTZOLOBWO, szlEntity, szlName,
                                    lpViewEntity->szName );
            SetAttributeFromInteger( vTZOLOBWO, szlEntity, szlLevel,
                                     (zLONG) lpEntityInstance->nLevel );
            ...
            while ( nRC == 0 && lpViewAttrib )
            {
                    ...
                    pchShowHidden[ 0 ] == 'Y' )
               {
                  ...
                  {
                     lpViewEntityCsr =
                              (LPVIEWENTITYCSR) SysGetPointerFromHandle( lpWorkViewCsr->hFirstOD_EntityCsr );
                     while ( lpViewEntityCsr->hViewEntity !=
                                             lpEntityInstance->hViewEntity )
                     {
                        lpViewEntityCsr = (LPVIEWENTITYCSR) SysGetPointerFromHandle( lpViewEntityCsr->hNextHier );
                     ...
                     SetAttributeFromString( vTZOLOBWO, szlEntity,
                                             szlFirstAttr, szAttrib );
                     if ( lpViewAttrib->cType == 'S' )
                        nAttrSet = 2;
                     else
...
BuildOutlinerOI_ForView( zVIEW vSubtask )
{
   ...
   /* first, get the handle to the browser object */
   GetViewByName( &vBrowser, szlTZOBRWOO, vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szID, vBrowser, szlName, szlObjectInstanceID );
   if ( GetViewByName( &vInstance, szID, vSubtask, zLEVEL_TASK ) > 0 )
   {
      ActivateWindow( vInstance );
      return( 0 );
...
zwTZOBRWAD_BuildOutlinerAttrs( zVIEW vSubtask )
{
   ...
   /* retrieve the pointer to the real entity from the outliner display
      work object */
   GetIntegerFromAttribute( (zPLONG) &lpEntityInstance,
...
//    SetCtrlState( vSubtask, "OI_LockEntity", zCONTROL_STATUS_VISIBLE, 0 );
      ...
      while ( lpViewAttrib )
      {
         if ( lpViewAttrib->bHidden == FALSE )
         {
            if ( lpViewAttrib->cType == 'B' )
            {
               ...
               GetAttributeLength( &uLth, vObject, lpViewEntity->szName,
                                   lpViewAttrib->szName );
               if ( uLth > 60 || uLth == 0 )
               {
                  zstrcpy( szAttribute, "(Blob, lth=" );
               ...
               {
                  GetAddrForAttribute( &pchBlob, vObject,
                                       lpViewEntity->szName,
                                       lpViewAttrib->szName );
                  nWk1 = 2;
                  zstrcpy( szAttribute, "x'" );
   ...
   if ( szMode[ 0 ] == 'E' || szMode[ 0 ] == 'M' )
   {
      SetCtrlState( vSubtask, "OI_ListAttributes",
                    zCONTROL_STATUS_VISIBLE, 0 );
   }
   else
   {
   ...
   if ( CheckExistenceOfEntity( vTZOLOBWO, pchEntityName ) > zCURSOR_NULL )
   {
      ...
      if ( szMode[ 0 ] == 'M' )
      {
         ...
         if ( lpEntityInstance->hPrevVsn )
         {
            ...
            if ( lpPrevVsn->u.nInd.bPrevVsnRoot == FALSE )
            {
               while ( lpPrevVsn->u.nInd.bPrevVsnRoot == FALSE )
               {
                  lpLinkInstance = (LPENTITYINSTANCE)
                          SysGetPointerFromHandle( lpLinkInstance->hParent );
                  lpPrevVsn = (LPENTITYINSTANCE)
                          SysGetPointerFromHandle( lpLinkInstance->hPrevVsn );
               }

               lpLinkViewEntity = (LPVIEWENTITY)
                                      SysGetPointerFromHandle( lpLinkInstance->hViewEntity );
               zstrcpy( szAttribute, "Versioned by " );
               zstrcat( szAttribute, lpLinkViewEntity->szName );
               SetAttributeFromString( vTZOBOIOO, szlAttrib, szlName,
         ...
         while ( lpLinkInstance && lpLinkInstance != lpEntityInstance )
         {
            ...
            {
               LPVIEWENTITY lpLinkViewEntity =
                       (LPVIEWENTITY) SysGetPointerFromHandle( lpLinkInstance->hViewEntity );
               ...
               {
                  LPVIEWOI lpLinkOI = (LPVIEWOI)
                                          SysGetPointerFromHandle( lpLinkInstance->hViewOI );
                  LPVIEWOD lpLinkOD = (LPVIEWOD)
                                                SysGetPointerFromHandle( lpLinkOI->hViewOD );
                  LPVIEWENTITY lpLinkEntity = (LPVIEWENTITY)
                                      SysGetPointerFromHandle( lpLinkInstance->hViewEntity );

                  zstrcpy( szAttribute, "External Link to " );
                  zstrcat( szAttribute, lpLinkOD->szName );
                  zstrcat( szAttribute, "." );
                  zstrcat( szAttribute, lpLinkViewEntity->szName );
               }
            ...
            while ( lpViewAttrib )
            {
               if ( lpViewAttrib->bHidden == FALSE )
               {
                  if ( lpViewAttrib->cType == 'B' )
                  {
                     ...
                     GetAttributeLength( &uLth, vTZOLOBWO, pchEntityName,
                                         lpViewAttrib->szName );
                     if ( uLth > 60 || uLth == 0 )
                     {
                        zstrcpy( szAttribute, "(Blob, lth=" );
                     ...
                     {
                        GetAddrForAttribute( &pchBlob, vTZOLOBWO,
                                             pchEntityName,
                                             lpViewAttrib->szName );
                        nWk1 = 2;
                        zstrcpy( szAttribute, "x'" );
...
BuildRelatedOI_ForView( zVIEW vSubtask, zVIEW vCompare )
{
   ...
   ActivateEmptyObjectInstance( &vHD_Compare, szlTZZOHDGO,
                                vSubtask, zSINGLE );
   CreateEntity( vHD_Compare, szlHD_Diagram, zPOS_AFTER );
   SetAttributeFromInteger( vHD_Compare, szlHD_Diagram, szlZKey,
                            (zLONG) vCompare );
   ...
   if ( lpViewOI && lpViewOI->hRootEntityInstance )
   {
      ...
      zLONG  lEntityCnt = 0;
      zSHORT nRC = 0;
      LPENTITYINSTANCE lpEntityInstance = (LPENTITYINSTANCE)
                                    SysGetPointerFromHandle( lpViewOI->hRootEntityInstance );
      ...
      while ( nRC == 0 && lpEntityInstance )
      {
              ...
              pchShowHidden[ 0 ] == 'Y' )
         {
            ...
            SetAttributeFromInteger( vHD_Compare, szlHD_Node,
                                     szlZKey, lEntityCnt );
         }
...
zwTZOBRWAD_FlagView( zVIEW vSubtask )
{
   ...
   // First, get the handle to the browser object
   GetViewByName( &vBrowser, szlTZOBRWOO, vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( sz, vBrowser, szlName, szlID );
...
BuildCompareViews( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &vCompare2, vCompare2,
                            szlHD_Diagram, szlZKey );
   ...
   ActivateEmptyObjectInstance( &vHD_Compare1, szlTZZOHDGO,
                                vSubtask, zSINGLE );
   ActivateEmptyObjectInstance( &vHD_Compare2, szlTZZOHDGO,
                                vSubtask, zSINGLE );
   ...
   SetAttributeFromInteger( vHD_Compare1, szlHD_Diagram, szlZKey,
                            (zLONG) vCompare1 );
   SetAttributeFromInteger( vHD_Compare2, szlHD_Diagram, szlZKey,
                            (zLONG) vCompare2 );
   ...
   while ( lpViewEntity )
   {
      ...
      SetAttributeFromString( vHD_Compare1, szlHD_Node, szlDIL,
                              lpViewEntity->szName );
      SetAttributeFromInteger( vHD_Compare1, szlHD_Node, szlZKey,
                               (zLONG) lpViewEntity->nHierNbr );
      lpViewEntity = (LPVIEWENTITY) SysGetPointerFromHandle( lpViewEntity->hNextHier );
   }
   ...
   while ( lpViewEntity )
   {
      ...
      SetAttributeFromString( vHD_Compare2, szlHD_Node, szlDIL,
                              lpViewEntity->szName );
      SetAttributeFromInteger( vHD_Compare2, szlHD_Node, szlZKey,
                               (zLONG) lpViewEntity->nHierNbr );
      lpViewEntity = (LPVIEWENTITY) SysGetPointerFromHandle( lpViewEntity->hNextHier );
   }
...
zwTZOBRWAD_CompareNext( zVIEW vSubtask )
{
   ...
   if ( zstrcmp( szTag, "Next" ) == 0 || zstrcmp( szTag, "Next1" ) == 0 )
   {
      ...
      GetIntegerFromAttribute( (zPLONG) &vCompare, vCompare,
                               szlHD_Diagram, szlZKey );
      SetNameForView( vCompare, szlTZZOHDGV, vSubtask, zLEVEL_SUBTASK );
...
MiGetRootEntityForView( zVIEW v, zPCHAR pchRootEntity )
{
   ...
   zstrcpy( szParentEntity, lpViewEntity->szName );
   while ( szParentEntity[ 0 ] )
   {
      zstrcpy( pchRootEntity, szParentEntity );
                   ...
                   zVIEW vHD_Compare, zCPCHAR cpcHD_CtrlTag )
{
   ...
   GetStringFromAttribute( szEntityName, vFlatCompare, szlHD_Node, szlDIL );
   ...
   while ( (nRC = SetCursorNextEntityHierarchical( &uReturnLevel,
                                                   szHD_EntityName,
                                                   vHD_Compare )) >= zCURSOR_SET )
   {
      if ( nRC == zCURSOR_SET_RECURSIVECHILD )
         SetViewToSubobject( vHD_Compare, szlHD_ChildNode );

      GetStringFromAttribute( szTempEntityName, vHD_Compare,
                              szlHD_Node, szlDIL );
      if ( zstrcmp( szTempEntityName, szEntityName ) == 0 )
      {
         GetAbsolutePositionForEntity( &lAbsolutePos, vHD_Compare,
                                       szlHD_Node );
         break;
      }
   }
...
CompareFirstDifference( zVIEW vSubtask )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( &rgbCurrColor, vFlatCompare1,
                               szlHD_Node, szlColor );
      if ( rgbCurrColor && rgbCurrColor != rgbColor )
      {
         GetIntegerFromAttribute( &lAbsolutePos1, vFlatCompare1,
                                  szlHD_Node, szlZKey );
         GetIntegerFromAttribute( &lMatch1, vFlatCompare1,
                                  szlHD_Node, "Match" );
         break;
      }
   ...
   while ( nRC >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( &rgbCurrColor, vFlatCompare2,
                               szlHD_Node, szlColor );
      if ( rgbCurrColor && rgbCurrColor != rgbColor )
      {
         GetIntegerFromAttribute( &lAbsolutePos2, vFlatCompare2,
                                  szlHD_Node, szlZKey );
         GetIntegerFromAttribute( &lMatch2, vFlatCompare2,
                                  szlHD_Node, "Match" );
         break;
      }
   ...
   else
   {
      ...
      CompareSelectNode( vSubtask, vCompare1, vFlatCompare1,
                         vHD_Compare1, "CompareHD1" );
      ...
      CompareSelectNode( vSubtask, vCompare2, vFlatCompare2,
                         vHD_Compare2, "CompareHD2" );
   }
...
CompareNextDifference( zVIEW vSubtask )
{
   ...
   if ( zstrchr( szTag, '1' ) )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( rgbCurrColor && rgbCurrColor != rgbColor )
         {
            GetIntegerFromAttribute( &lAbsolutePos, vFlatCompare1,
                                     szlHD_Node, szlZKey );
            GetIntegerFromAttribute( &lMatch, vFlatCompare1,
                                     szlHD_Node, "Match" );
            GetViewByName( &vHD_Compare1, "__HD_Compare1", vSubtask, zLEVEL_TASK );
            CompareSelectNode( vSubtask, vCompare1, vFlatCompare1,
                               vHD_Compare1, "CompareHD1" );
            ...
            // SetCursorAbsolutePosition( szEntity, vFlatCompare2, lMatch );
               SetCursorFirstEntityByInteger( vFlatCompare2, szlHD_Node,
                                              szlZKey, lMatch, 0 );
               GetViewByName( &vHD_Compare2, "__HD_Compare2", vSubtask, zLEVEL_TASK );
               CompareSelectNode( vSubtask, vCompare2, vFlatCompare2,
                                  vHD_Compare2, "CompareHD2" );
   ...
   else
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( rgbCurrColor && rgbCurrColor != rgbColor )
         {
            GetIntegerFromAttribute( &lAbsolutePos, vFlatCompare2,
                                     szlHD_Node, szlZKey );
            GetIntegerFromAttribute( &lMatch, vFlatCompare2,
                                     szlHD_Node, "Match" );
            GetViewByName( &vHD_Compare2, "__HD_Compare2", vSubtask, zLEVEL_TASK );
            CompareSelectNode( vSubtask, vCompare2, vFlatCompare2,
                               vHD_Compare2, "CompareHD2" );
            ...
            // SetCursorAbsolutePosition( szEntity, vFlatCompare1, lMatch );
               SetCursorFirstEntityByInteger( vFlatCompare1, szlHD_Node,
                                              szlZKey, lMatch, 0 );
               GetViewByName( &vHD_Compare1, "__HD_Compare1", vSubtask, zLEVEL_TASK );
               CompareSelectNode( vSubtask, vCompare1, vFlatCompare1,
                                  vHD_Compare1, "CompareHD1" );
...
{
   zSHORT nLth = strlen( cpcName );
   m_pchName = new char[ nLth + 1 ];
   strcpy( m_pchName, cpcName );
   m_pNext = 0;
}
...
CompareObjects( zVIEW vSubtask )
{
   ...
   if ( nRC == 0 )
   {
      ...
      zsprintf( szMsg, "Object Instances: %s and %s are identical",
   ...
   else
   {
      ...
      zsprintf( szMsg, "Object Instances: %s and %s are different",
                inf[ 0 ].cpcViewName, inf[ 1 ].cpcViewName );
      TraceLineS( szMsg, "" );
   }
          ...
          zLONG lMinimal, zPARTITION *part, short bHeuristic )
{
   ...
   for ( lCost = 1;; lCost++ )
   {
      ...
      for ( d = fmax; d >= fmin; d -= 2 )
      {
         ...
         y = x - d;
         while ( x < xlim && y < ylim &&
                 fnCompareEntities( viewvec[ 0 ].vOI, &xv[ x ],
                                    viewvec[ 1 ].vOI, &yv[ y ] ) == 0 )
         {
            x++;
            y++;
         }
      ...
      for ( d = bmax; d >= bmin; d -= 2 )
      {
         ...
         y = x - d;
         while ( x > xoff && y > yoff &&
                 fnCompareEntities( viewvec[ 0 ].vOI, &xv[ x - 1 ],
                                    viewvec[ 1 ].vOI, &yv[ y - 1 ] ) == 0 )
         {
            x--;
            y--;
         }
...
FindHD_Entity( zVIEW vHD_Compare, zLONG lAbsolutePos )
{
   ...
   {
      zSHORT nRC = SetCursorFirstEntityByInteger( vHD_Compare, szlHD_Node,
                                                  szlZKey, lAbsolutePos, 0 );
      if ( nRC >= zCURSOR_SET )
         return( nRC );
      ...
      {
         SetViewToSubobject( vHD_Compare, szlHD_ChildNode );
         if ( (nRC = FindHD_Entity( vHD_Compare,
                                    lAbsolutePos )) >= zCURSOR_SET )
         {
            return( nRC );
         }
                   ...
                   zLONG   lMatch )
{
   ...
   {
      nRC = SetCursorFirstEntityByInteger( vFlatCompare, szlHD_Node,
                                           szlZKey, lAbsolutePos, 0 );
      if ( nRC >= zCURSOR_SET )
      {
         SetAttributeFromString( vFlatCompare, szlHD_Node,
                                 szlDIL, cpcEntityName );
         SetAttributeFromInteger( vFlatCompare, szlHD_Node,
                                  szlColor, rgbColor );
         SetAttributeFromInteger( vFlatCompare, szlHD_Node,
                                  "Match", lMatch );
      }
      else
         TraceLineI( "SetHD_EntityColor did not find AbsolutePos: ",
                     lAbsolutePos );
   }
...
//    TraceLineS( "Separator ", "<" );
   ...
   else
   {
      ...
      if ( f1 )
      {
         ...
         SetCursorAbsolutePosition( pEntity1->pchEntity, f1->vOI,
                                    pEntity1->lAbsolutePos );
      ...
      if ( f2 )
      {
         ...
         SetCursorAbsolutePosition( pEntity2->pchEntity, f2->vOI,
                                    pEntity2->lAbsolutePos );
   ...
   if ( f1 )
   {
      ...
      SetHD_EntityColor( vSubtask, "__FlatCompare1", pEntity1->pchEntity,
                         ...
                         (bMatch && pEntity2) ? pEntity2->lAbsolutePos :
                                                MAXLONG );
   // SetCursorAbsolutePosition( pEntity1->pchEntity, f1->vOI,
   //                            pEntity1->lAbsolutePos );
   // DisplayEntityInstance( f1->vOI, pEntity1->pchEntity );
...
// }
   ...
   if ( f2 )
   {
      ...
      SetHD_EntityColor( vSubtask, "__FlatCompare2", pEntity2->pchEntity,
                         ...
                         (bMatch && pEntity1) ? pEntity1->lAbsolutePos :
                                                MAXLONG );
   // SetCursorAbsolutePosition( pEntity2->pchEntity, f2->vOI,
   //                            pEntity2->lAbsolutePos );
   // DisplayEntityInstance( f2->vOI, pEntity2->pchEntity );
...
DiffViews( zVIEW vSubtask, zVIEW_DATA viewvec[ ] )
{
   ...
   viewvec[ 1 ].changed_flag = viewvec[ 0 ].changed_flag +
                               viewvec[ 0 ].lEntityCnt + 2;
   ...
   DrAllocTaskMemory( (zCOREMEM) &g_fdiag,
                      lDiagCnt * (2 * sizeof( zLONG )), 3 );
   ...
   else
   {
      ...
      // match.
      g_lNext0 = g_lNext1 = 0;
      PrintSdiffCommonLines( vSubtask, g_data[ 0 ].lEntityCnt,
                             g_data[ 1 ].lEntityCnt );
   }
   ...
   {
      szChange[ 0 ] =
               viewvec[ 1 ].changed_flag[ k + viewvec[ 0 ].lEntityCnt + 2 ];
      sprintf( szMsg, "%d - %s", k + 1, szChange );
      TraceLineS( szMsg, "" );
   }
#endif
...
ReadViews( zVIEW vSubtask, zVIEW_DATA viewvec[ ] )
{
   ...
   for ( k = 0; k < 2; k++ )
   {
      ...
      lEntityCnt = 0;
      zstrcpy( szEntityName, szRootEntityName );
      ...
      } while ( SetCursorNextEntityHierarchical( &uReturnLevel, szEntityName,
                                                 vHierOI ) >= zCURSOR_SET );
      ...
      do
      {
         ...
         if ( pCurr == 0 )
         {
            ...
            GetViewByName( &vSystemCompare, "__SystemCompare",
         ...
         GetAbsolutePositionForEntity( &lAbsolutePos,
                                       vHierOI, szEntityName );
