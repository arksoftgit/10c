                               ...
                               zVIEW    vEntpModel )
{
   ...
   nZRetCode = GetIntegerFromAttribute( &lEntityZKey, vSubjectArea,
                                        "ER_Entity", "ZKey" );
   ...
   if ( ( nRC > zCURSOR_UNCHANGED ) )
   {
      ...
      while ( RESULT > zCURSOR_UNCHANGED )
      {
         nZRetCode = GetIntegerFromAttribute( &lRelLinkZKey, vEntpModel_Wrk,
                                              "ER_RelLink", "ZKey" );
         nRC = SetCursorFirstEntityByInteger( vSubjectArea, "ER_RelLink",
                                              "ZKey", lRelLinkZKey,
                                              "ER_Entity" );
         nRC = IncludeSubobjectFromSubobject( vSubjectArea, "ER_RelLink_Other",
                                              vEntpModel_Wrk, "ER_RelLink_Other",
                                              zPOS_AFTER );
   ...
   {
      nZRetCode = GetStringFromAttribute( szEntityName, vSubjectArea,
                                          "ER_Entity", "Name" );
      nRC = SetCursorFirstEntityByString( vEntpModel_Wrk, "ER_Entity",
                                          "Name", szEntityName, "" );
      if ( ( nRC > zCURSOR_UNCHANGED ) )
      {
         nRC = ExcludeEntity( vSubjectArea, "ER_Entity", zREPOS_NONE );
                               ...
                               zVIEW vER )
{
   ...
   if ( SetCursorFirstEntityByInteger( vSA, "ER_Entity", "ZKey",
                                       lZKey, "SubjectArea" ) >= zCURSOR_SET )
   {
      return( 1 );
   }

   CreateMetaEntity( vSubtask, vSA, "SA_Entity", zPOS_LAST );
   IncludeSubobjectFromSubobject( vSA, "ER_Entity", vER, "ER_Entity",
                                  zPOS_FIRST );

   CreateViewFromViewForTask( &vER2, vER, 0 );
   CreateViewFromViewForTask( &vSA2, vSA, 0 );
   CreateViewFromViewForTask( &vSA3, vSA, 0 );
   if ( SetCursorFirstEntityByInteger( vER, "ER_Entity_2", "ZKey", lZKey,
                                       "EntpER_Model" ) >= zCURSOR_SET )
   {
      ...
      } while ( SetCursorNextEntityByInteger( vER, "ER_Entity_2", "ZKey",
                                   lZKey, "EntpER_Model" ) >= zCURSOR_SET );
   }
   DropView( vER2 );
   DropView( vSA2 );
   DropView( vSA3 );
...
//   Now see if the ER_Entity is allready in the Subject Area
   nRC = GetIntegerFromAttribute( &lEntityZKey,
                                  vTZEREMDO_Wrk, "ER_Entity", "ZKey" );
   nRC = SetCursorFirstEntityByInteger( vTZERSASO_Wrk, "ER_Entity",
                                        "ZKey", lEntityZKey,
                                        "SubjectArea" );
   if ( nRC > zCURSOR_UNCHANGED )
   {
//    copy positioning from current SA_Entity and then Delete it
      SetMatchingAttributesByName( vTZERSASO, "SA_Entity",
                        vTZERSASO_Wrk, "SA_Entity", zSET_NULL );
...
//    by including ER_Entity from the ER_Model.
   nRC = IncludeSubobjectFromSubobject( vTZERSASO, "ER_Entity",
                                        vTZEREMDO_Wrk, "ER_Entity",
                                        zPOS_AFTER );

//    Create ER_RelLink_Other and ER_Entity_Other in the Subject Area
//    by including ER_RelLink_Other from the ER_Model.  Position is obtained
//    first on the ER_RelType_1 in the SA from the linked ER_RelType_1 from
//    the ER Model.
   for ( nRC = SetCursorFirstEntity( vTZERSASO, "ER_RelType_1", "ER_Entity" );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntity( vTZERSASO, "ER_RelType_1", "ER_Entity" ) )
   {
      SetCursorFirstEntityByEntityCsr( vTZEREMDO_Wrk, "ER_RelType_1",
                                       vTZERSASO, "ER_RelType_1",
                                       "ER_Entity" );
      IncludeSubobjectFromSubobject( vTZERSASO, "ER_RelLink_Other",
                                     vTZEREMDO_Wrk, "ER_RelLink_Other",
                                     zPOS_AFTER );
...
oTZERSASO_RemoveOneSA_Entity( zVIEW vTZERSASO )
{
   ...
   zstrcpy( szMessage, "OK to remove Entity from SA ?\n\n" );
...
//            down the derived path.
   ...
   for ( nRC = SetCursorFirstEntityByEntityCsr( vTZERSASO_Wk,
                                                "ER_Entity_Other",
                                                vTZERSASO, "ER_Entity",
                                                "SubjectArea" );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByEntityCsr( vTZERSASO_Wk,
                                               "ER_Entity_Other",
                                               vTZERSASO, "ER_Entity",
                                               "SubjectArea" ) )
   {
...
//            line associated with the entity.
   for ( nRC = SetCursorFirstEntityByEntityCsr( vTZERSASO, "ER_Entity_2",
                                                vTZERSASO, "ER_Entity",
                                                "SubjectArea" );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByEntityCsr( vTZERSASO, "ER_Entity_2",
                                               vTZERSASO, "ER_Entity",
                                               "SubjectArea" ) )
   {
      ...
      if ( vSAVIEW )
      {
         if ( lpfn == 0 )   // only get func addr once
         {
            lpfn =(zFARPROC_RemoveFunc)
                     GetOperationDynamicCallAddress( vSAVIEW,
...
// Step 3, Delete the SA Entity after telling the diagram to remove it
   ...
   {
      lpfn =(zFARPROC_RemoveFunc)
             GetOperationDynamicCallAddress( vSAVIEW,(LPLPLIBRARY) &hLibrary,
...
//    because both SA_Entities are gone.
         ...
         nRC = SetCursorNextEntity( vTZERSASO, "ER_RelType_1", "ER_Entity" ) )
   {
      ...
      nRC = SetCursorFirstEntityByInteger( vTZERSASO, "ER_RelLink_2",
                                           "ZKey", lZKey, "SubjectArea" );
      if ( nRC >= zCURSOR_SET )
      {
         if ( vSAVIEW )
         {
         ...
         {
            SetAttributeFromInteger( vTZERSASO, "SA_RelType",
                             ...
                             zVIEW vTZERSASO )
{
   ...
   OperatorPrompt( 0, "Subject Area",
                   "oTZERSASO_CompleteRelCreate - This code is supposed "
                   "to be disabled and never called again. However, a call "
                   "has taken place as a result of your last action! "
                   "Do you promise to run and tell Gig (or Don) NOW?",
   ...
   nRC = SetCursorFirstEntityByInteger( vTZERSASO, "ER_RelType",
                                        "ZKey", lZKey, "SubjectArea" );
   if ( nRC < zCURSOR_SET )
   {
      if ( MiGetUpdateForView( vTZERSASO ) == 0 )
      {
         TraceLineS( "SA_Complete(1): =========== ER_Entity: ",
                     szName );  // debug only
         TraceLineI( "SA_Complete(2): ReadOnly View. ---- SA_RelType "
                     "not created ZKey=", lZKey );
         return( 0 );
      }
      TraceLineS( "SA_Complete(1): =========== ER_Entity: ",
                  szName );  // debug only
      ...
      IncludeSubobjectFromSubobject( vTZERSASO, "ER_RelType",
                                     vTZERSASO, "ER_RelType_1", zPOS_AFTER );
   }
   ...
   if ( nRC < zCURSOR_SET )
   {
      IncludeSubobjectFromSubobject( vTZERSASO, "ER_RelLink_2",
                                     vTZERSASO, "ER_RelLink", zPOS_AFTER );
      SetAttributeFromInteger( vTZERSASO, "SA_RelType",
                               "ER_DiagramBendPosX", 0 );
      SetAttributeFromInteger( vTZERSASO, "SA_RelType",
                               "ER_DiagramBendPosY", 0 );
