...
#define ADDEXPRESSIONENTRYTOSTRING( p1, p2, p3, p4, p5, p6 ) \
        AddExpressionEntryToString( szLocal9Code,\
           &lLocal9Code, szLocal9Text, &lLocal9Text,\
           p1, p2, p3, p4, p5, p6 )
...
SetupActivate( zVIEW vSubtask, zLONG lIDNumber )
{
   ...
   GenerateLocalOperationName( szLocalName, "BuildQual" );
   ...
   if ( lRC < 0 )
   {
      zstrcpy( g_szFatalErrorMsg, "found operation with reserved name: " );
      zstrcat( g_szFatalErrorMsg, szLocalName );
                            ...
                            zPCHAR pchIncInName )
{
   ...
   zstrcpy( pchReturnName, "fnLocal" );
   zstrcat( pchReturnName, pchIncInName );
   zstrcat( pchReturnName, "_" );
   lLth = zstrlen( pchReturnName );
   zltoa( lMaxNbr, pchReturnName + lLth );
                                    ...
                                    zPCHAR pchLoadControl )
{
   ...
   if ( lpTempView == 0 )
   {
      ...
      GetStringFromAttribute( szViewName, g_lpPIView, "Variable", "Name" );
      SyntaxError( vSubtask, eQNOVIEWDEFGIVEN, szViewName );
      return( -1 );
   }

   GetStringFromAttribute( szLODName, lpTempView, "LOD", "Name" );
                          ...
                          zLONG lQualID )
{
   ...
   GetStringFromAttribute( szLocalName, vLocalView,  "Operation", "Name" );
                            ...
                            &lVal, "", "", 0, "", 0 ) >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szLocalName, vLocalView, "Variable", "Name" );
                         ...
                         zLONG  lLoadQual )
{
   ...
   GetStringFromAttribute( szLocalVal, g_lpSGView, "Variable", "PFlag" );
                      ...
                      zLONG  lRootOrRestrict )
{
   ...
   if ( lRootOrRestrict == 0 ) // root
   {
      ...
      zstrcpy( szRootName, szEntityName );
      fnBuildCreateEntity( vSubtask, "EntitySpec" );
      fnBuildSetAttributeConstant( vSubtask, "EntitySpec", "EntityName", szRootName );
   }
   ...
   for ( ; lLoop < g_lWhereIndex; lLoop++ )
   {
      ...
      switch( g_sWhereRight[ lLoop ].lElementClass )
      {
         ...
         case qATTRIBUTEQUALIFIER:
            ...
            {
               zstrcpy( g_szOperationName, "GetDecimalFromAttribute" );
               lDataType = qTDECIMAL;
            }
            else
            {
               ...
               {
                  zstrcpy( g_szOperationName, "GetIntegerFromAttribute" );
                  lDataType = qTINTEGER;
               }
               else
               {
                  zstrcpy( g_szOperationName, "GetStringFromAttribute" );
                  lDataType = qTSTRING;
               }
            }
      ...
      GetStringForSQLOperator( szOperString, g_lWhereOp[ lLoop ] );
      ...
      {
         GetStringForSQLOperator( szOperString, g_lWhereJoin[ lLoop ] );
         fnBuildCreateEntity( vSubtask, "QualAttrib" );
         fnBuildSetAttributeConstant( vSubtask, "QualAttrib", "Oper", szOperString );
      }
                             ...
                             zLONG  lVarDataType )
{
   ...
   GetStringFromAttribute( szOperationName, vCallingView, "Variable", "Name" );
   PositionAtNewOperation( );
...
// lRCID = AddRetCode( vSubtask );       // add nZRetCode to the list of variables to be declared
   ...
   GetVariableID( &lQualID, "vQualObject" );
   zstrcpy( szOperationName, "SetAttributeFrom" );

   switch( lVarDataType )
   {
      case qTSTRING:
      case qTCHARACTER :
         zstrcat( szOperationName, "String" );
         break;

      case qTINTEGER:
      case qTSHORT   :
         zstrcat( szOperationName, "Integer" );
         break;

      case qTDECIMAL:
         zstrcat( szOperationName, "Decimal" );
         break;
...
zOPER_EXPORT zLONG OPERATION
GetRootEntityName( zPCHAR pchReturnEntityName )
{
   zLONG  lRC;
   zVIEW  lpViewOfView;;
   ...
   if ( lRC >= zCURSOR_SET )
      lRC = GetStringFromAttribute( pchReturnEntityName, lpViewOfView, "LOD_EntityParent", "Name" );
                         ...
                         zLONG lOperator )
{
   switch( lOperator )
   {
      ...
      case qOR:
         zstrcpy( pchOperString, "OR" );
         break;

      case qAND:
         zstrcpy( pchOperString, "AND" );
         break;
      ...
      case qLIKE:
         zstrcpy( pchOperString, "LIKE" );
         break;

      case qEXISTS:
         zstrcpy( pchOperString, "EXISTS" );
         break;

      case qNOTEXISTS:
         zstrcpy( pchOperString, "NOT EXISTS" );
         break;

      case qIS:
         zstrcpy( pchOperString, "IS" );
         break;

      case qNULL:
         zstrcpy( pchOperString, "IS NULL" );
         break;

      case qNOTNULL:
         zstrcpy( pchOperString, "IS NOT NULL" );
         break;
              ...
              zPCHAR pchQualLevel )
{
   ...
   if ( lQualView == -1 )
   {
      ...
      GetStringFromAttribute( szReturnFlag, g_lpSGView, "Variable", "PFlag" );
