               ...
               zPCHAR pchDefineValue )
{
   ...
   case qZEIDONOPERATION:
      ...
      if ( lRC == zCURSOR_SET )
      {
         lRC = SetCursorFirstEntityByString( vLookupView, "DefinedItem", "ExternalValue",
                                             pchDefineValue, "" );
         if ( lRC == zCURSOR_SET )
         {
            lRC = SetCursorFirstEntityByString( g_lpSGView, "DefinedItem",
                                                "ExternalValue", pchDefineValue, "" );
            if ( lRC < zCURSOR_SET )
               lRC = IncludeSubobjectFromSubobject( g_lpSGView, "DefinedItem",
                                                    vLookupView, "DefinedItem", zPOS_AFTER );
      ...
      while ( lRC >= 0 )
      {
         ...
         lRC = SetCursorFirstEntityByString( vHdr, "DefinedItem",
                                             "ExternalValue", pchDefineValue, "" );
         // see if the define is there
         if ( lRC == zCURSOR_SET )
         {
            // if it is include both the header and the define
            lRC = SetCursorFirstEntityByInteger( g_lpSGView, "ExternalHeaderFile",
                                                 "ZKey", lRefZKey, "" );
            if ( lRC != zCURSOR_SET )
            {
               lRC = SetCursorLastEntity( g_lpSGView, "ExternalHeaderFile", "" );
               lRC = IncludeSubobjectFromSubobject( g_lpSGView, "ExternalHeaderFile",
                                                    vHdr, "HeaderFile", zPOS_AFTER );
            }

            lRC = SetCursorFirstEntityByString( g_lpSGView, "DefinedItem",
                                                "ExternalValue", pchDefineValue, "" );
            if ( lRC != zCURSOR_SET )
               lRC = IncludeSubobjectFromSubobject( g_lpSGView, "DefinedItem", vHdr,
                                                    "DefinedItem", zPOS_AFTER );
            DropView( vHdrList );
            DropMetaOI( vSubtask, vHdr );
            //////////////////////////
               lRC = GetIntegerFromAttribute( &lDefineZKey, g_lpSGView, "DefinedItem", "ZKey" );
               lRC = GetStringFromAttribute( cZeidonType, g_lpSGView, "DefinedItem", "DataType" );
      ...
      if ( lRC >= zCURSOR_SET )
      {
         ...
         if ( lRC < zCURSOR_SET )
         {
            lRC = SetCursorLastEntity( g_lpSGView, "ExternalHeaderFile", "" );
            lRC = IncludeSubobjectFromSubobject( g_lpSGView, "ExternalHeaderFile",
                                                 vLookupView, "HeaderFile", zPOS_AFTER );
         }
         ...
         if ( lRC < zCURSOR_SET )
         {
            ...
            lRC = IncludeSubobjectFromSubobject( g_lpSGView, "DefinedItem", vLookupView,
                                                 "DefinedItem", zPOS_AFTER );
         }
   ...
   if ( lRC >= zCURSOR_SET &&
        CheckExistenceOfEntity( g_lpSGView, "DefinedItem" ) >= zCURSOR_SET )
   {
      lRC = GetIntegerFromAttribute( &lDefineZKey, g_lpSGView, "DefinedItem", "ZKey" );
      lRC = GetStringFromAttribute( cZeidonType, g_lpSGView, "DefinedItem", "DataType" );
      *plDataType = ConvertZeidonTypeToVMLType( cZeidonType[ 0 ] );
   ...
   if ( SetCursorFirstEntityByString( vLookupView, "Operation", "Name",
                                      pchDefineValue, 0 ) >= zCURSOR_SET )
   {
      ...
      if ( CheckExistenceOfEntity( vLookupView, "HeaderFile_Oper" ) >= zCURSOR_SET )
      {
         ...
         if ( SetCursorFirstEntityByAttr( g_lpSGView, "ExternalHeaderFile", "ZKey",
                                          vLookupView, "HeaderFile_Oper", "ZKey", 0 ) < zCURSOR_SET )
         {
            // No, then add it.
            SetCursorLastEntity( g_lpSGView, "ExternalHeaderFile", "" );
            IncludeSubobjectFromSubobject( g_lpSGView, "ExternalHeaderFile", vLookupView,
                                           "HeaderFile_Oper", zPOS_AFTER );
         }
      }
   }
...
zOPER_EXPORT zLONG OPERATION
GetVariableName( zLONG lID, zPCHAR pchVariableName )
{
   zLONG lRC;                         // temp return code
   ...
   // get the Name
   lRC = GetStringFromAttribute( pchVariableName, g_lpSGView, "Variable", "Name" );
   return( lID );
}

zOPER_EXPORT zLONG OPERATION
GetVariableLength( zPCHAR szLen, zLONG lID )
{
   ...
   {
      //lRC =
      //   GetStringFromAttribute( szLen, g_lpSGView, "Variable", "NElements" );
...
GetVariableDataClass( zLONG lVariableID )   // id of the variable looked for
{
   ...
   // since an entity exists, check to see if this same name was
   // declared for this operation.
   if ( SetCursorFirstEntityByInteger( g_lpSGView, "Variable", "ID",
                                        lVariableID, "" ) < zCURSOR_SET )
   {
      // variable is not there
      return( -1 );
   }
   ...
   if ( zstrcmp( szObjectName, "TZOGSRCO") != 0 )
   {
                                         ...
                                         pchOperationName, szParentEntityName ) >= zCURSOR_SET )
      {
         *plOperationClass = qLOCALOPERATION;
         GetOperationData( vSubtask, plOperationZKey, plOperationType, pchType, *pvSource, pchOperationName );
         return( 0 );
      }
...
// }
   ...
   GetStringFromAttribute( pchType, vGenericView, "Operation", "ReturnDataType" );
   *plOperationType = ConvertZeidonTypeToVMLType( pchType[ 0 ] );
   GetStringFromAttribute( pchType, vGenericView, "Operation", "Type" );
...
GetOperationParmListPM( zVIEW vSubtask,
                        zLONG zFAR sParmList[],
                        zLONG zFAR sParmListExt[],
                        zLONG lZKey,
                        zLONG lOperationClass )
{
   zLONG lVal;
   ...
   if ( lRC > zCURSOR_UNCHANGED )    // found
   {
      ...
      while ( lRC > zCURSOR_UNCHANGED )
      {
         GetStringFromAttribute( cZeidonType, vLookupView, "Parameter", "DataType" );

         lParmType = ConvertZeidonTypeToVMLType( cZeidonType[ 0 ] );

         sParmListExt[ lCheckParms ] |= remember_type( lParmType );

         GetStringFromAttribute( cPtrFlag, vLookupView, "Parameter", "PFlag" );
         if ( cPtrFlag[ 0 ] == 'Y' )
         {
            if ( lParmType > 0 )
               lParmType *= -1;
         ...
         // Determine if this parameter is unsigned.
         // This info is stored in UNSIGNED_BIT of the extended parm list.
         if ( parameter_is_unsigned( vLookupView ) )
            sParmListExt[ lCheckParms ] |= UNSIGNED_BIT;
...
zOPER_EXPORT zLONG OPERATION
GetOperationParmListFromSource( zLONG zFAR sParmList[],
                                zLONG zFAR sParmListExt[],
                                zPCHAR pchParmName,
                                zVIEW  vSource )
{
            ...
            if ( lParmType != qTVIEW )  // qTVIEW:1045
            {
               if ( lReturnType == 0 )
               {
                  if ( lParmType != qTSTRING || bStringRejected == FALSE )
                  {
                     ...
                     GetStringFromAttribute( pchParmName, vLookupView, "Parameter", "ShortDesc" );
                  }
               }
               else
        ...
        lQualiferType == qATTRIBUTEQUALIFIER )
   {
      ...
      else
      {
         ...
         *lpEntityID = 0;
         ...
         if ( lRC >= zCURSOR_SET )
            GetStringFromAttribute( szRealEntityName, lpViewOfView, "LOD_Entity", "Name" );
      ...
      if ( lQualiferType == qATTRIBUTEQUALIFIER )
      {
         ...
         else
         {
            ...
            // the correct one.
            lRC = SetCursorFirstEntityByString( lpViewOfView, "ER_Attribute", "Name",
                                                pchAttributeName, "LOD_Entity" );
            // Check for case sensitive match
            if ( lRC >= zCURSOR_SET )
               GetStringFromAttribute( szRealAttrName, lpViewOfView, "ER_Attribute", "Name" );

            if ( lRC < zCURSOR_SET ||
                 zstrcmp( pchAttributeName, szRealAttrName ) != 0 )
            {
               // even if it does not have to be a variable it can be
               // See if the variable is defined
               lRC = GetVariableID( lpAttributeID, pchAttributeName );
            ...
            else
            {
               ...
               lRC = GetStringFromAttribute( cType, lpViewOfView, "Domain", "DerivedDataType" );
                        ...
                        zPCHAR pchAttributeName )
{
   ...
   GetVariableName( lViewID, g_szViewName );
   ...
   if ( lRC >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szDataType, lpViewOfView, "Domain", "DerivedDataType" );
      if ( szDataType[ 0 ] == 'L' )
      {
         return( 9 );
      }
                         ...
                         zPLONG plNewDataType )
{
   ...
   switch ( lDataType )
   {
      ...
      case qTSTRING:
         zstrcpy( szHold, "szTempString_" );
         lDataLength += 1;
         break;

      case qTCHARACTER:
         zstrcpy( szHold, "cTempCharact_" );
         lDataLength = 2;
         break;

      case qTINTEGER:
      case qTSHORT:
         zstrcpy( szHold, "lTempInteger_" );
         break;

      case qTDECIMAL:
         zstrcpy( szHold, "dTempDecimal_" );
         break;

      case qTVIEW:
         zstrcpy( szHold, "vTempViewVar_" );
         break;

      default:
         zstrcpy( szHold, "kTempVariable" );
         break;
   }
...
parameter_is_unsigned( zVIEW vLookupView )
{
   ...
   GetStringFromAttribute( cUnsignedFlag, vLookupView, "Parameter", "UFlag" );
