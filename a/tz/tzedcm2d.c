...
void
fnGetFileDateTime( zVIEW lpTaskView, zPCHAR pchFile, zPCHAR pchTimeStamp )
{
   zLONG hFile;
   ...
   SysGetFileDateTime( hFile, pchTimeStamp );
...
fnParseGenTarget( zVIEW vSubtask, zVIEW vTaskLPLR, zCPCHAR cpcGenLang )
{
   ...
   GetStringFromAttribute( szTargetFile, vTaskLPLR, "LPLR", "ExecDir" );
   SysConvertEnvironmentString( szXPG, szTargetFile );
   nExecDirLth = SysAppendcDirSep( szXPG );
   GetStringFromAttribute( szTargetFile, vTaskLPLR, "LPLR", "PgmSrcDir" );
   SysConvertEnvironmentString( szSourceFile, szTargetFile );
   nSourceDirLth = SysAppendcDirSep( szSourceFile );
   zstrcpy( szTargetFile, szSourceFile );
   nTargetDirLth = nSourceDirLth;

   if ( cpcGenLang[ 0 ] == 'J' )
   {
      zstrcpy( szTargetFile + nTargetDirLth, "JavaProject\\src\\" );
      nTargetDirLth += 16;
      GetStringFromAttribute( szTargetFile + nTargetDirLth, vTaskLPLR, "LPLR", "JavaPackageName" );
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "Meta", 0 ) )
   {
      ...
      GetStringFromAttribute( szMetaName, vTaskLPLR, "Meta", "Name" );  // TZRPSRCO
      ...
      switch ( lType )
      {
         ...
         case zSOURCE_GOPGRP_META: // Global Op
            ...
            GetStringFromAttribute( szMetaName2, vMeta, "GlobalOperationGroup", "Name" );
         ...
         case zSOURCE_LOD_META: // LOD
            ...
            GetStringFromAttribute( szMetaName2, vMeta, "LOD", "Name" );
            if ( zstrcmp( szMetaName, szMetaName2 ) != 0 )
               nMetaRC = 0; // no, not the right one
         ...
         case zSOURCE_DIALOG_META: //Dialog
            ...
            // Determine if the open meta is the one to be parsed.
            GetStringFromAttribute( szMetaName2, vMeta, "Dialog", "Tag" );
            if ( zstrcmp( szMetaName, szMetaName2 ) != 0 )
               nMetaRC = 0;  // no, not the right one
      ...
      {
         zstrcpy( szMsg , "Internal error - ActivateMeta failed for " );
            ...
            nRC = SetCursorNextEntity( vMetaCopy, pchSourceFileEntity, 0 ) )
      {
         ...
         zstrcat( szSourceFile, pchPtr );
         SysAppendcDirSep( szSourceFile );
         GetAddrForAttribute( &pchPtr, vMetaCopy, pchSourceFileEntity, "Name" );
         zstrcat( szSourceFile, pchPtr );
         zstrcat( szTargetFile, pchPtr );
         zstrcat( szSourceFile, ".VML" );
         zstrcat( szXPG, pchPtr );
         if ( cpcGenLang[ 0 ] == 'J' )
            zstrcat( szXPG, ".XPJ" );
         else
            zstrcat( szXPG, ".XPG" );

         zstrcpy( szSourceBase, pchPtr );

         zsprintf( szMsg, "Checking %s for parse ... ", szSourceFile );
         MB_SetMessage( vSubtask, 0, szMsg );

         fnGetFileDateTime( vSubtask, szSourceFile, szSourceDateTime );
         fnGetFileDateTime( vSubtask, szXPG, szXPG_DateTime );

         // Check to see if we need to parse the file.
         if ( szXPG_DateTime[ 0 ] == 0 || bAlwaysParse ||
              zstrcmpi( szSourceDateTime, szXPG_DateTime ) > 0 )
         {
            zsprintf( szMsg, "Parsing %s...", szSourceFile );
            MB_SetMessage( vSubtask, 0, szMsg );
            ...
            zsprintf( szMsg, "Completed parse of %s...", szSourceFile );
            MB_SetMessage( vSubtask, 0, szMsg );
         }
         ...
         if ( cpcGenLang[ 0 ] == 'J' )
         {
            if ( lType == zREFER_LOD_META ) // 2007
               zstrcpy( szTargetFile + zstrlen( szTargetFile ), "_Object.java" );
            else
            if ( lType == zREFER_DIALOG_META ) // 2011
               zstrcpy( szTargetFile + zstrlen( szTargetFile ), "_Dialog.java" );
            else
            if ( lType == zREFER_GOPGRP_META ) // 2014
               zstrcpy( szTargetFile + zstrlen( szTargetFile ), "_Operation.java" );
            else
            {
            ...
            zstrcpy( szTargetFile + zstrlen( szTargetFile ), ".c" );

         // Now retrieve the date/time of the .c/.java file and the .XPG.  We retrieve the
         // date/time of the .XPG again because it might have been changed by a parse.
         fnGetFileDateTime( vSubtask, szTargetFile, szSourceDateTime );
         fnGetFileDateTime( vSubtask, szXPG, szXPG_DateTime );

         // Check to see if we need to generate the file.
         if ( szTargetFile[ 0 ] == 0 || bAlwaysGen || bParsed ||
              zstrcmpi( szXPG_DateTime, szSourceDateTime ) > 0 )
         {
            zCHAR szMsg[ 200 ];

            zsprintf( szMsg, "Generating %s...", szTargetFile );
            TraceLineS( szMsg, "" );
            MB_SetMessage( vSubtask, 0, szMsg );
            ...
            if ( Generate3GL( vSubtask, vSubtask, vMetaCopy, szSourceBase, cpcGenLang, lType ) < 0 )
            {
               ...
               SysReadZeidonIni( -1, "[Workstation]", "ContinueParseGen", szContinueParseGen );
               zsprintf( szMsg, "Error generating %s.", szTargetFile );
               if ( szContinueParseGen[ 0 ] != 'Y' )
               {
            ...
            zsprintf( szMsg, "Completed generation of %s ...", szTargetFile );
            TraceLineS( szMsg, "" );
            MB_SetMessage( vSubtask, 0, szMsg );

            // The gen function creates a named view _GenFromXPG, which we want to get rid of
            // here because they accrue in core.
            ...
            zCPCHAR szCommand )
{
   ...
   zsprintf( szMsg, "Compiling %s...", pchTargetExecutableName );
   MB_SetMessage( vSubtask, 0, szMsg );
   ...
   switch ( nCompilerType )
   {
      case zCOMP_TYPE_MSVC15:
      {
         ...
         // Windows NT 3.51 show us always Windows version 3.10, but there is a different format
         // in the PIF, which we use to detect NT.
         GetWindowsDirectory( szDefaultPIF, 300 );
         zstrcat( szDefaultPIF, "\\_DEFAULT.PIF");
         ...
         // Compiling under NT starts a shell (COMMAND.COM) without a PIF we need no PIF,
         // because under NT the COMMAND.COM is a real windows application.
         if ( nVersion > 311 || cPIF[ 1 ] == '\xC9' )
         {
            // Build new Command "COMMAND.COM /C Path\\ZMAKE.BAT"
            if ( SysGetOS_Info( 0 ) == zOS_WINDOWSNT )
               zstrcpy( szCommandNT, "CMD.EXE ");
            else
               zstrcpy( szCommandNT, "COMMAND.COM ");

            if ( CompareAttributeToString( vTaskLPLR, "Compiler",
                                           "CloseWindowAfterCompile", "Y" ) == 0 )
            {
               zstrcat( szCommandNT, "/C ");
            }
            else
               zstrcat( szCommandNT, "/K ");

            zstrcat( szCommandNT, szCommand );
            szCommandNT[  strlen( szCommandNT ) - 12  ] = 0; // remove PIF file
            zstrcat( szCommandNT, "ZMAKE.BAT" );
         ...
         else
         {
            ...
            if ( hInst <= 32 )
            {
               ...
               {
                  case 0:
                     zsprintf( szMsg, "Error running compile.  System out of memory" );
                     break;

                  case 2:
                     zsprintf( szMsg, "Error running compile.  Command shell not found." );
                     break;

                  case 3:
                     zsprintf( szMsg, "Error running compile.  Path was not found.  "
                               "Check the specification of the Compiler directory." );
                     break;

                  case 5:
                     zsprintf( szMsg, "Error running compile.  Sharing or network-protection error." );
                     break;

                  case 8:
                     zsprintf( szMsg, "Error running compile.  Insufficient memory.  "
                               "Close apps and try again. " );
                     break;

                  default:
                     zsprintf( szMsg, "Error running compile.  Reason code = %d", hInst );
                     break;
               }
      ...
      default:
         // Compiling for OS/2 under WinOS2 runs with PIF for standard mode
         // but WinOS2 could be still in enhanced mode???

         // we set WinOS2=Y in ZEIDON.INI, if we are under WinOS2
         // otherwise, we just create a make file but don't compile
         SysReadZeidonIni( -1, "[Workstation]", "WinOS2", szWinOS2 );
         ...
         {
            zsprintf( szMsg, "Makefile for %s has been generated.", pchTargetExecutableName );
            MB_SetMessage( vSubtask, 0, szMsg );
            return( 0 );
         }
   ...
   zstrcpy( szMsg, "Compiling Zeidon Target..." );
   ...
   zsprintf( szMsg, "Make of %s completed", pchTargetExecutableName );
   MB_SetMessage( vSubtask, 0, szMsg );
   return( 0 );
...
//    MiGetSetViewTaskAppFlags( vTaskLPLR, 1, 0, 1 ); // reset KeepAlive to OFF
   ...
   nRC = CreateMakefileForAllTargets( vSubtask, vTaskLPLR, 0,
                                      bCompileAllTargets, szCommand, cpcGenLang );
   if ( nRC == 0 )
   {
...
MakeAllTargets( zVIEW vSubtask, zVIEW vTaskLPLR )
{
   ...
   bCompileAllTargets = (lMakeOption == 6);
   if ( !CreateMakefileForAllTargets( vSubtask, vTempTaskLPLR, 0, bCompileAllTargets, szCommand ) )
   {
      MB_SetMessage( vSubtask, 0, "Build of Makefile failed" );
      DropView( vTempTaskLPLR );
      return( zCALL_ERROR );
                       ...
                       zBOOL  bOverwrite )
{
   ...
   SysConvertEnvironmentString( szBaseDir, szFileName );
   ...
   SysConvertEnvironmentString( szMakefileDir, pchMakefileDir );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szMakefileDir, szBaseDir,
                                       zMAX_FILENAME_LTH - 2, "Makefile Directory") == -1 )
   {
      return( 0 );
   }
   ...
   zstrcpy( szFileName, pchMakefileDir );
   SysAppendcDirSep( szFileName );
   zstrcat( szFileName, "ZENV.HNM" );
   ...
   SysGetDateTime( szDT );
   UfFormatDateTime( szDTFormatted, szDT, "YYYY.MM.DD HH:MI:SS" );
   zsprintf( pchLine, "#  Generated at %s", szDTFormatted);
   SWL( vTaskLPLR, pchLine );
   SWL( vTaskLPLR, "#" );

   zsprintf( pchLine, "APPNAME    = %s", pchAppName );
   SWL( vTaskLPLR, pchLine );

   zsprintf( pchLine, "MODULE_DIR = %s", "" );   // source sub-dir.
   SWL( vTaskLPLR, pchLine );
   ...
   // Build relative path if necessary; Base directory is Component Source Dir
   zstrcpy( szExecDir, pch );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szExecDir, szBaseDir, zMAX_FILENAME_LTH - 2,
   ...
   SysConvertEnvironmentString( szFileName, pchPgmSrcDir );
   zsprintf( pchLine, "SRC_DIR    = %s$(MODULE_DIR)", szFileName );
   SWL( vTaskLPLR, pchLine );
   ...
   // Build relative path if necessary; Base directory is Component Source Dir.
   zstrcpy( szObj, pchObj );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szObj, szBaseDir,
                                       zMAX_FILENAME_LTH - 2, "Obj/Lib directory" ) == -1 )
   {
      return( 0 );
   }
   ...
   zstrcpy( szFileName, pchMakefileDir );
   SysAppendcDirSep( szFileName );
   zstrcat( szFileName, "ZCOMPILE.HNM" );
   hFile = SysOpenFile( vTaskLPLR, szFileName, COREFILE_WRITE );
   if ( hFile == -1 )
      return( FALSE );

   SWL( vTaskLPLR, "#" );
   zsprintf( pchLine, "#  zCompile.HNM" );
   SWL( vTaskLPLR, pchLine );
   SWL( vTaskLPLR, "#  Make Include File (MSVC 5.0)." );
   SysGetDateTime( szDT );
   UfFormatDateTime( szDTFormatted, szDT, "YYYY.MM.DD HH:MI:SS" );
   ...
   zsprintf( pchLine, "   @set PATH=%s;%s;%s", pchExecDir, pch, pch2 );
   SWL( vTaskLPLR, pchLine );

   // Write Environment-Variable LIB in Makefile
   zstrcpy( pchLine, "   @set LIB=" );
   for ( nRC = SetCursorFirstEntity( vTaskLPLR, "Lib", 0 );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntity( vTaskLPLR, "Lib", 0 ) )
   {
      GetStringFromAttribute( pchLine + zstrlen( pchLine ), vTaskLPLR, "Lib", "LibDir" );
      zstrcat( pchLine, ";" );
   }
   ...
   zstrcpy( pchLine, "   @set INCLUDE=" );
   for ( nRC = SetCursorFirstEntity( vTaskLPLR, "Include", 0 );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntity( vTaskLPLR, "Include", 0 ) )
   {
      GetStringFromAttribute( pchLine + zstrlen( pchLine ), vTaskLPLR, "Include", "IncludeDir" );
      zstrcat( pchLine, ";" );
   }

   zstrcat( pchLine, pchPgmSrcDir );
   SWL( vTaskLPLR, pchLine );
   SWL( vTaskLPLR, "" );
                                      ...
                                      "Name", "<ALL>", 0 ) >= zCURSOR_SET )
   {
      ...
      zstrcpy( pchLine, "BASELINKDLLS = $(BASELINKDLLS) " );
      ...
      {
         GetStringFromAttribute( szFileName, vTaskLPLR, "ExternalLibFile", "Name" );
      ...
      zstrcpy( pchLine, "LINKOBJS = $(LINKOBJS) " );
                       ...
                       zBOOL bOverwrite )
{
   ...
   if ( nRC < zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ], vTaskLPLR, "LPLR", "CurrentCompilerSpec" );
      zstrcat( szMsg, "'" );
...
CreatePIF_File( zVIEW vTaskLPLR, zCHAR cOverwrite )
{
   ...
   // Base directory for relative paths.
   GetStringFromAttribute (szBaseDir, vTaskLPLR, "LPLR", "MetaSrcDir");
   ...
   zstrcpy( szFileName, pchEnvDir );
   SysAppendcDirSep( szFileName );

   // we have different PIFs for Windows and OS/2 compiling
   if ( nCompilerType == zCOMP_TYPE_MSVC15 )
     zstrcat( szFileName, "ZCOMPILE.PIF" );
   else
     zstrcat( szFileName, "ZCOMPOS2.PIF" );
   ...
   if ( cOverwrite == 'N' || cOverwrite == 'A' )
   {
      if ( SysOpenFile( vTaskLPLR, szFileName, COREFILE_EXIST ) != -1 )
      {
         ...
         zsprintf( szMsg, "Overwrite %s?", szFileName );
   ...
   if ( nCompilerType == zCOMP_TYPE_MSVC15 )
   {
     ...
     zstrcpy( sPIF_Info.szWindowTitle, "Compiling Zeidon Target..." );
     zmemcpy( sPIF_Info.szDummy1, "\x02\x80\0", 3 );
     zstrcpy( sPIF_Info.szCommand, "COMMAND.COM" );
     zstrcpy( sPIF_Info.szDirectory, pchMakefileDir );
     ...
     if ( CompareAttributeToString( vTaskLPLR, "Compiler", "CloseWindowAfterCompile", "Y" ) == 0 )
        zstrcpy( sPIF_Info.szParameters, "/e:2048 /c " );
     else
        zstrcpy( sPIF_Info.szParameters, "/e:2048 /k " );

     zstrcat( sPIF_Info.szParameters, pchEnvDir );
     SysAppendcDirSep( sPIF_Info.szParameters );
     zstrcat( sPIF_Info.szParameters, "ZMAKE.BAT" );
   ...
   else
   {
     ...
     zstrcpy( sPIF_Info.szWindowTitle, "Compiling Zeidon Target..." );
     zstrcpy( sPIF_Info.szCommand, "ZMAKEOS2.BAT" );
     zstrncpy( sPIF_Info.szDirectory, pchMakefileDir, 64 );
     sPIF_Info.szDirectory[63] = 0;
     zstrcpy( sPIF_Info.szParameters, "" );
     ...
     hFile = SysOpenFile( vTaskLPLR, szFileName, COREFILE_WRITE );
     SysWriteFile( vTaskLPLR, hFile,
                   (zPCHAR) &sPIF_Info, sizeof( struct PIF_Struct286 ) );
     SysCloseFile( vTaskLPLR, hFile, 0 );
...
CloseCompilerSpecification( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szCompilerSpec, vCompilerSpec, "LPLR", "CurrentCompilerSpec" );
   if ( SetCursorFirstEntityByString( vCompilerSpec, "Compiler", "Name",
                                      szCompilerSpec, 0 ) != zCURSOR_SET )
   {
      zCHAR szMsg[ 200 ];

      if ( zstrcmp( szCompilerSpec, "" ) == 0 )
      {
         zstrcpy( szMsg, "Current Compiler Specification is required." );
         zstrcat( szMsg, "\n\nDo you want to set Current Compiler Specification?" );
      }
      else
      {
         zsprintf( szMsg, "Current Compiler Specification '%s' is invalid.", szCompilerSpec );
         zstrcat( szMsg, "\n\nDo you want to set Current Compiler Specification?" );
      }
                            ...
                            zPCHAR pchControlName)
{
   ...
   {
      zsprintf( szMsg, "%s '%s' has invalid extension.", pchControlName, pchName, szExtension );
   ...
   // do not check libs, cause it is too much
   // efford to chech the LIB variable
...
fnVerifyExternalTargetDir( zVIEW vSubtask, zVIEW vTaskLPLR )
{
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "ExternalTarget", 0 ) )
   {
      ...
      for ( nRC = SetCursorFirstEntity( vTaskLPLR, "ExternalObjFile", 0 );
            ...
            nRC = SetCursorNextEntity( vTaskLPLR, "ExternalObjFile", 0 ) )
      {
         GetStringFromAttribute( szFileName, vTaskLPLR, "ExternalObjFile", "Name" );
...
fnVerifyIncludeLibTargetDir( zVIEW vSubtask, zVIEW vTaskLPLR )
{
   ...
   for ( nRC = SetCursorFirstEntity( vTaskLPLR, "Include", 0 );
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "Include", 0 ) )
   {
      GetStringFromAttribute( szFileName, vTaskLPLR, "Include", "IncludeDir" );
   ...
   for ( nRC = SetCursorFirstEntity( vTaskLPLR, "Lib", 0 );
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "Lib", 0 ) )
   {
      GetStringFromAttribute( szFileName, vTaskLPLR, "Lib", "LibDir" );
...
fnExternalLibObjToLibTarget (zVIEW vTask2, zPCHAR pchEntity)
{
        ...
        nRC = SetCursorNextEntity (vTask2, pchEntity, 0) )
   {
      ...
      GetStringFromAttribute( szFullFileName, vTask2, pchEntity, "Name" );
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "ExternalTarget", 0 ) )
   {
            ...
            nRC = SetCursorNextEntity( vTaskLPLR, "ExternalLibFile", 0 ) )
      {
         ...
         if ( nRC >= 0 )
         {
            ...
            zsprintf( szMsg, "Duplicate lib \"%s\" in target \"%s\" and in \"<ALL>\"",
                      pchName, pchTarget );
            MessageSend( vSubtask, "ED00999", "Zeidon Compiler",
            ...
            nRC = SetCursorNextEntity( vTaskLPLR, "ExternalObjFile", 0 ) )
      {
         ...
         {
            // Show a warning, then delete the item for our target
            zsprintf( szMsg, "Duplicate object \"%s\" in target \"%s\" and in \"<ALL>\"",
                      pchName, pchTarget );
            MessageSend( vSubtask, "ED00999", "Zeidon Compiler",
                         szMsg, zMSGQ_OBJECT_CONSTRAINT_WARNING, zBEEP );
...
fnCheckCompilerSpec( zVIEW vSubtask, zVIEW vTaskLPLR )
{
   ...
   nRC = SetCursorFirstEntityByString( vCopy, "Compiler", "Name", szName, "" );
...
VerifyCompilerSpecOptions( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileName, vTaskLPLR, "Compiler", "TargetExecutableDir" );
   if ( IntGenFullQualPathFromRelPath( vSubtask, szFileName, szBaseDir, zMAX_FILENAME_LTH - 2,
                                       "Target Executable Directory" ) == -1 )
   {
      goto EndOfFunction;
   }
   ...
   GetStringFromAttribute( szFileName, vTaskLPLR, "Compiler", "TargetObjectDir" );
   if ( IntGenFullQualPathFromRelPath( vSubtask, szFileName, szBaseDir, zMAX_FILENAME_LTH - 2,
                                       "OBJ Directory") == -1 )
   {
      goto EndOfFunction;
   ...
   GetStringFromAttribute( szFileName, vTaskLPLR, "Compiler", "MakefileDir" );
   if ( IntGenFullQualPathFromRelPath( vSubtask, szFileName, szBaseDir, zMAX_FILENAME_LTH - 2,
   ...
   GetStringFromAttribute( szFileName, vTaskLPLR, "Compiler", "EnvironmentDir" );
   if ( IntGenFullQualPathFromRelPath( vSubtask, szFileName, szBaseDir, zMAX_FILENAME_LTH - 2,
   ...
   // Set the external lib/obj directories into the lib directories for
   // LIB definition
...
ExternalTargetLibFile_Browse( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      {
         GetStringFromAttribute( szMetaSrcDir, vTaskLPLR, "LPLR", "MetaSrcDir" );
         _chdir( szMetaSrcDir );
      }

      // do a loop for input of filename until the filename is valid or
      ...
      do
      {
         nRC = OperatorPromptForFile( vSubtask, szFileName, sizeof( szFileName ),
                ...
                !fnVerifyExternalTargetFile( vSubtask, szFileName, ".LIB",
   ...
   if ( GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      do
      {
         nRC = OperatorPromptForFile( vSubtask, szFileName, sizeof( szFileName ),
                ...
                !fnVerifyExternalTargetFile( vSubtask, szFileName, ".OBJ",
...
PostbuildTargetList( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szCurrentCompilerSpec, vTaskLPLR, "LPLR",
                           "CurrentCompilerSpec" );
   if ( CheckExistenceOfEntity( vTaskLPLR, "Compiler" ) >= zCURSOR_SET &&
...
DeleteCompilerSpec( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szCompilerName, vTaskLPLR, "Compiler", "Name" );

   zstrcpy( szMsg, "OK to delete Compiler Specification '" );
   zstrcat( szMsg, szCompilerName );
   zstrcat( szMsg, "'?" );
