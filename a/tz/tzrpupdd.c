...
AutoDesignReport( zVIEW     vSubtask )
{
   ...
   fnPainterCall( zMSG_REPAINTZEIDONWINDOW, vSubtask, 0, "" );
   return( 0 );
}
...
fnRecurseChangeRptTags( zVIEW vReport )
{
   ...
   {
      GetStringFromAttribute( szTag, vReport, "Control", "Tag" );
      if ( szTag[ 0 ] == 'R' &&
           szTag[ 1 ] == 'p' &&
           szTag[ 2 ] == 't' &&
           szTag[ 3 ] == '_' )
      {
         GetStringFromAttribute( szText, vReport, "Control", "Text" );
         if ( szText[ 0 ] )
         {
            szText[ 32 ] = 0;
         ...
         else
         {
            ...
            {
               GetStringFromAttribute( szText, vReport,
...
// TraceLineI( "Text control blob lth = ", ulLth );
   ...
   // if ( ld.chMultiLine == 0 )
      ...
      zltoa( ld.lBorderColor, szWork );
      SetCtrlText( vSubtask, "BorderColor", szWork );
      zltoa( ld.lEscapement, szWork );
...
// if ( zstrcmp( szWork, "Left" ) == 0 )
   ...
   GetCtrlProperty( vSubtask, "__ColorText",
                    ...
                    (zPULONG) &ld.lTextColor, 0 );
   GetCtrlProperty( vSubtask, "__ColorBackground",
                    zCONTROL_PROPERTY_INTEGER_DATA,
                    (zPULONG) &ld.lTextBkColor, 0 );
   ulLth = LF_FACESIZE;
...
// TraceLine( "UPD_TEXT_OK Text Color: 0x%08x", ld.lTextColor );
   ...
   if ( ulCommon )
   {
      ...
      do
      {
         fnPainterCall( zMSG_GET_NEXT_SELECTED_CTRL,
                        vSubtask, vTemp, (zPCHAR) &nPos );
         if ( nPos >= 0 )
         {
            ...
            while ( nRC >= zCURSOR_SET )
            {
               ...
               if ( *pch == 'Y' )
               {
                  ...
                  if ( *pchUpdate )
                  {
                     ...
                     {
                        zstrncpy( szEntity, pchUpdate + 1, pch - pchUpdate );
                     ...
                     {
                        zstrcpy( szEntity, "Control" );
                     }

                     if ( szEntity[ 0 ] )
                     {
                        GetAddrForAttribute( &pchTag, vTZPESRCO,
                                             "PropertyDef", "Tag" );
                        if ( *pchTag == '*' )
                        {
                           SetMatchingAttributesByName( vTemp, szEntity,
                                                        vControl, szEntity,
                                                        zSET_ALL );
                        }
                        else
...
// TraceLineI( "Shape control blob lth = ", ulLth );
        ...
        ulLth == sizeof( szBlob ) - (2 * sizeof( zLONG )) )
   {
      ...
      zltoa( lPenWidth, szBlob );
      SetCtrlText( vSubtask, "PenWidth", szBlob );
      zltoa( lPenType, szBlob );
      switch ( lPenType )
      {
         case 1:
...
ShapeOK( zVIEW vSubtask )
{
   ...
   GetCtrlProperty( vSubtask, "__ColorLine",
                    zCONTROL_PROPERTY_INTEGER_DATA, &ulPenColor, 0 );
   GetCtrlText( vSubtask, "RoundValue", szBlob, sizeof( szBlob ) );
   lRoundValue = atol( szBlob );
   GetCtrlProperty( vSubtask, "__ColorShape",
                    zCONTROL_PROPERTY_INTEGER_DATA, &ulShapeColor, 0 );

   if ( bRoundRect || bEllipse || bDiamond || bHorizontalLine ||
        bVerticalLine || bDiagonalDescend || bDiagonalAscend ||
        lPenWidth != 1 || lPenType || ulPenColor || lRoundValue ||
        ulShapeColor || lFlags )
   {
      zPLONG lpLong;
...
RefreshPropertyAttribute( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szProperty, vControl, "CtrlProperty", "Name" );
      GetStringFromAttribute( szPropertyValue, vControl, "CtrlProperty", "Value" );
...
AddBlockAttribute( zVIEW vSubtask )
{
   ...
   CB_GetSelectedString( vSubtask, "BlockProperty", szProperty );
...
UPD_BmpCtlOK( zVIEW vSubtask )
{
   ...
   if ( szName[ 0 ] )
   {
      ...
      else
      {
         ...
         zstrcpy( BMP.szBMPUpOrIconName, ++pszBMPName );
...
fnGetCM_ListAndLOD( zVIEW vSubtask, zPVIEW pvCM_List, zPVIEW pvLOD,
                    zVIEW vTZZOVEAO, zPCHAR pchVEA, zBOOL bAttr )
{
   zLONG  lZKey;
   zLONG  lZKeyA;
   zSHORT nLth;

   GetStringFromAttribute( pchVEA, vTZZOVEAO, "ViewObjRef", "Name" );
   nLth = zstrlen( pchVEA );
   pchVEA[ nLth++ ] = '.';
   GetStringFromAttribute( pchVEA + nLth, vTZZOVEAO, "LOD_Entity", "Name" );
   if ( bAttr )
   {
      nLth = zstrlen( pchVEA );
      pchVEA[ nLth++ ] = '.';
      GetStringFromAttribute( pchVEA + nLth, vTZZOVEAO, "ER_Attribute", "Name" );
   }
   ...
   SetCursorFirstEntityByInteger( *pvCM_List, "W_MetaDef",
                                  "CPLR_ZKey", lZKey, "" );
   ActivateMetaOI( vSubtask, pvLOD,
                   *pvCM_List, zREFER_LOD_META, 0 );

   GetIntegerFromAttribute( &lZKey, vTZZOVEAO, "LOD_Entity", "ZKey" );
   if ( bAttr )
      GetIntegerFromAttribute( &lZKeyA, vTZZOVEAO, "LOD_Attribute", "ZKey" );

   if ( SetCursorFirstEntityByInteger( *pvLOD, "LOD_Entity",
                                       "ZKey", lZKey, "" ) == zCURSOR_SET &&
        (bAttr == FALSE ||
         SetCursorFirstEntityByInteger( *pvLOD, "LOD_Attribute",
...
SetSelectedEntityMapping( zVIEW vSubtask )
{
   ...
   if ( vReport && vTZZOVEAO )
   {
      ...
      if ( fnGetCM_ListAndLOD( vSubtask, &vCM_List, &vLOD,
                               vTZZOVEAO, szVEA, FALSE ) )
      {
         lRC = CallPainterForSelectedControls( vSubtask, "tzrpupdd",
                                               "PainterSelectedCallback",
...
CtrlListChangeMappingViewGS( zVIEW vSubtask )
{
   ...
   GetViewByName( &vReport, "vReport", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szGroupSetName, vPAGECTRL, "GroupSet", "Tag" );
...
CreateEntityGroupSet( zVIEW vSubtask, zBOOL bHorizontal )
{
   ...
   if ( vReport && vTZZOVEAO && vPE &&
        SetCursorFirstEntityByString( vPE, "ControlDef", "Tag",
                                      "Rpt_Text", "" ) == 0 )
   {
      zVIEW vCM_List;
      zVIEW vLOD;
                               ...
                               vTZZOVEAO, szVEA, FALSE ) )
      {
         ...
         zSHORT nLth = zstrlen( szVEA );

         szVEA[ nLth++ ] = '.';

         GetStringFromAttribute( szViewName, vTZZOVEAO, "ViewObjRef", "Name" );
         GetStringFromAttribute( szEntityName, vTZZOVEAO, "LOD_Entity", "Name" );
         ...
         zstrcpy( szEntityTemp, szEntityName );
                                               ...
                                               szEntityTemp, 0 ) != zCURSOR_SET )
         {
            ...
            GetStringFromAttribute( szEntityTemp, vTemp, "LOD_Entity", "Name" );
         }
         ...
         zstrcpy( szEntityTemp + lCnt, "Header" );
         ...
         zstrcpy( szEntityTemp + lCnt, "Detail" );
         CreateMetaEntity( vSubtask, vReport, "Group", zPOS_AFTER );
         ...
         while ( nRC == zCURSOR_SET )
         {
            GetStringFromAttribute( szAttribName, vTemp, "ER_Attribute", "Name" );
            zstrcpy( szVEA + nLth, szAttribName );
            TraceLineS( "CreateEntityGroupSet creating control for attribute: ",
                        szVEA );
            ...
            zstrcat( szAttribName, ":" );  // to differentiate tags
            if ( bHorizontal == FALSE )
               SetAttributeFromString( vReportHeader, "Control", "Text", szAttribName );
         ...
         zstrcpy( szEntityTemp + lCnt, "Footer" );
         CreateMetaEntity( vSubtask, vReport, "Group", zPOS_AFTER );
         SetAttributeFromString( vReport, "Group", "Tag", szEntityTemp );
                              ...
                              zPCHAR szEntityName )
{
   ...
   zstrcpy( szTitle, "  " );
   zstrcat( szTitle, szEntityName );
   zstrcat( szTitle, " Detail" );
...
//                                        vPE, "ConvertableFrom", "" );
// fnPainterCall( zMSG_CHANGESELECTEDCONTROLS, vSubtask, 0, 0 );
...
CtrlListDeleteCtrl( zVIEW vSubtask )
{
   ...
   if ( vControl )
   {
      GetStringFromAttribute( szControlTag, vControl, "Control", "Tag" );
      zstrcpy( szMsg, "OK to delete Control '" );
      zstrcat( szMsg, szControlTag );
      zstrcat( szMsg, "'?" );
...
// zstrcpy( szUnknown, "__Unknown" );
   ...
   while ( (nPos = GetNextCtrlAtPoint( vSubtask, nPos, szCurrentCtrl, 1 )) >= 0 )
   {
      for ( k = 0; k < mElementCnt( cpcOptionTags ); k++ )
      {
         ...
         zstrcpy( szOptionTag, szCurrentCtrl );
         zstrcat( szOptionTag, "." );
         zstrcat( szOptionTag, cpcOptionTags[ k ] );
         zstrcpy( szActionTag, "Update" );
         zstrcat( szActionTag, cpcOptionTags[ k ] );
         zstrcat( szActionTag, "InPlace" );
         AddOptionToPopupMenu( vSubtask,
                               cpcOptionTags[ k ], 0, szOptionTag,
...
CheckMigrateCtrls( zVIEW vReport, zVIEW vPE, zLONG lCheckFlags )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      if ( lCheckFlags & zCHECK_POSITION_ONLY )
      {
         ...
         while ( nRC == zCURSOR_SET )
         {
            if ( lKey != (zLONG) GetEntityKey( vWork, "Control" ) )
            {
               ...
               {
                  // We have found containment of "vWork" control by
                  // "vReport" control.  Move the work control under
                  // the report control.
      ...
      else
      {
         ...
         if ( CompareAttributeToInteger( vReport, "ControlDef",
                                         "Key", lKey ) != 0 )
         {
            SetCursorFirstEntityByInteger( vPE, "ControlDef",
                                           "Key", lKey, 0 );
            ExcludeEntity( vReport, "ControlDef", zREPOS_NONE );
            IncludeSubobjectFromSubobject( vReport, "ControlDef",
                                           vPE, "ControlDef", zPOS_BEFORE );
...
MigrateReports( zVIEW vSubtask )
{
   ...
   {
      k++;
      if ( ActivateMetaOI( vSubtask, &vReport, vReportList,
                           zSOURCE_REPORT_META,
                           zSINGLE | zLEVEL_TASK ) == 1 )
      {
         ...
         {
            zsprintf( szMsg, "Checking Report: %s for Migration", pch );
            TraceLineS( szMsg, "" );
         ...
         if ( bSaveAll )
         {
            zsprintf( szMsg, "Saving Report: %s", pch );
            MB_SetMessage( vSubtask, 1, szMsg );
         ...
         else
         {
            ...
            zsprintf( szMsg, "Checking Report: %s for synchronization", pch );
            TraceLineS( szMsg, "" );
            MB_SetMessage( vSubtask, 1, szMsg );
      ...
      {
         TraceLineI( "MigrateReports unexpected failure to open report in list ",
                     k );
      }
