...
PainterCall( zSHORT, zVIEW, zPVOID, zVIEW, zPCHAR );
              ...
              zCPCHAR szContextName )
{
   ...
   {
      if ( cpcAttributeName )
         zstrcpy( szAttribute, cpcAttributeName );
   }
   else
   {
      GetVariableFromAttribute( szAttribute, 0, zTYPE_STRING, 34,
   ...
   if ( nObject )
      LoadString( g_hInstanceDLL, nObject, szObject, 34 );
   else
   if ( cpcEntityName )
      zstrcpy( szObject, cpcEntityName );
   else
      szObject[ 0 ] = 0;

   LoadString( g_hInstanceDLL, nTool, szTool, 100 );
   LoadString( g_hInstanceDLL, nStdMsg, szMsg, 128 );
   zsprintf( szMsgOut, szMsg, szObject, szAttribute );
   switch ( nStdMsg )
   {
      case zTOOLMSG_SAVE:
...
#if 0
         nRC = GetStringFromAttribute( szVal1,
                        vSort1, pchEntityName, pchAttribName );
         nRC = GetStringFromAttribute( szVal2,
                        vSort2, pchEntityName, pchAttribName );
#endif
         lCompares++;
// Compare Attr 1
         nRC = CompareAttributeToAttribute(
                        vSort1, pchEntityName, pchAttribName,
                        vSort2, pchEntityName, pchAttribName );
...
// Compare Attr 2
         ...
         {
            nRC2 = CompareAttributeToAttribute(
                           vSort1, pchEntityName, pchAttribName2,
...
// Compare Attr 3
         ...
         {
            nRC3 = CompareAttributeToAttribute(
                           vSort1, pchEntityName, pchAttribName3,
...
// Compare Attr 4
         ...
         {
            nRC4 = CompareAttributeToAttribute(
                           vSort1, pchEntityName, pchAttribName4,
...
// See if swap required
         ...
         {
            nRC = MoveSubobject( vSort2, pchEntityName,
                                 vSort1, pchEntityName,
...
// zsprintf( sz, "%4d Entities, %4d Moves, %6ld Compares",
//          nEntities, nMoves, nCompares );
   zstrcpy( sz, pchEntityName );
   zstrcat( sz, " -- " );
   zltoa( nEntities, &sz[ zstrlen( sz ) ] );
   zstrcat( sz, " Entities,  " );
   zltoa( nMoves, &sz[ zstrlen( sz ) ] );
   zstrcat( sz, " Moves,  " );
   zltoa( lCompares, &sz[ zstrlen( sz ) ] );
   zstrcat( sz, " Compares" );
                            ...
                            zSHORT bAscDsc4 )
{
   ...
   if ( SetCursorFirstEntity( vSort1, pchPEntityName, 0 ) > zCURSOR_UNCHANGED )
   {
      ...
      {
         nEntities++;
         if ( SetCursorNextEntity( vSort2, pchPEntityName, 0 )
                                                   < zCURSOR_SET )
         {
            if ( nSwap == 0 )
            {
         ...
         lCompares++;
// Compare Attr 1
         nRC = CompareAttributeToAttribute( vSort1, pchEntityName,
                                            pchAttribName,
                                            vSort2, pchEntityName,
                                            pchAttribName );
         if ( bAscDsc == zDESCENDING )
            nRC = nRC * -1;
...
// See if swap required
         if ( nRC > 0 ||
              ...
              ( nRC == 0 && nRC2 == 0 && nRC3 == 0 && nRC > 0 ) )
         {
            nRC = MoveSubobject( vSort2, pchPEntityName,
                                 vSort1, pchPEntityName,
                                 zPOS_AFTER, zREPOS_NONE );
            if ( nRC == zCALL_ERROR )
               return( nRC );
            nMoves++;
...
// zsprintf( sz, "%4d Entities, %4d Moves, %6ld Compares",
//          nEntities, nMoves, lCompares );
   zstrcpy( sz, pchPEntityName );
   zstrcat( sz, "||" );
   zstrcat( sz, pchEntityName );
   zstrcat( sz, " -- " );
   zltoa( nEntities, &sz[ zstrlen( sz ) ] );
   zstrcat( sz, " Entities,  " );
   zltoa( nMoves, &sz[ zstrlen( sz ) ] );
                          ...
                          HFILE hFile )
{
   ...
   SysGetFileDateTime( hFile, szWorkString );
   pchTimestamp[ 0 ]  = szWorkString[ 4 ];   // month
   pchTimestamp[ 1 ]  = szWorkString[ 5 ];
   pchTimestamp[ 2 ]  = '/';
   pchTimestamp[ 3 ]  = szWorkString[ 6 ];   // day
             ...
             zPCHAR szParm4 )
{
   ...
   for ( i = 0; szLine[ i ] == ' '; i++ ); // Find first nonblank

   // szParm1
   j = 0;
   while ( szLine[ i ] != 0 && szLine[ i ] != ' ' )
      szParm1[j++] = szLine[i++];   // Copy each nonblank character
   szParm1[j] = 0;
   if ( szLine[ i ] == 0 )
      return( 0 );

   // szParm2
   if ( szParm2 == 0 )    // Exit if no parm.
      return( 0 );
   while ( szLine[ i ] == ' ' )
      i++;
   j = 0;
   while ( szLine[ i ] != 0 && szLine[ i ] != ' ' )
      szParm2[j++] = szLine[i++];   // Copy each nonblank character
   szParm2[j] = 0;
   if ( szLine[ i ] == 0 )
      return( 0 );

   // szParm3
   if ( szParm3 == 0 )    // Exit if no parm.
      return( 0 );
   while ( szLine[ i ] == ' ' )
      i++;
   j = 0;
   while ( szLine[ i ] != 0 && szLine[ i ] != ' ' )
      szParm3[j++] = szLine[i++];   // Copy each nonblank character
   szParm3[j] = 0;
   if ( szLine[ i ] == 0 )
      return( 0 );

   // szParm4
   if ( szParm4 == 0 )    // Exit if no parm.
      return( 0 );
   while ( szLine[ i ] == ' ' )
      i++;
   j = 0;
   while ( szLine[ i ] != 0 && szLine[ i ] != ' ' )
      szParm4[j++] = szLine[i++];   // Copy each nonblank character
   szParm4[j] = 0;
                  ...
                  zLONG  lInvokingCommand )
{
   ...
   GetStringFromAttribute( szFileName, vTaskLPLR, "LPLR", "PgmSrcDir" );
   ...
   GetStringFromAttribute( &szFileName[ zstrlen( szFileName ) ],
                           vMeta, lpchSourceFileEntityName, "Name" );
   zstrcat( szFileName, "." );
   GetStringFromAttribute( &szFileName[ zstrlen( szFileName ) ],
                           vMeta, lpchSourceFileEntityName, "Extension" );
   ...
   if ( vEdWrk )
   {
      ...
      if ( SetCursorFirstEntityByString( vEdWrk, "Buffer", "FileName",
                                         szFileName, 0 ) >= zCURSOR_SET )
      {
         zVIEW vEditorSubtask;

         GetIntegerFromAttribute( (zPLONG) &vEditorSubtask, vEdWrk,
                                  "Buffer", "vSubtask" );
   ...
   SetAttributeFromString( vProfileXFER, "ED", "InvokingTool",
                           pchInvokingTool );
   ...
   SysReadZeidonIni( -1, "[Workstation]", "Editor", szTemp );
               ...
               zPCHAR pchParm )
{
        ...
        GetViewByName( &vTZPNTROO, "TZPNTROO", vSubtask, zLEVEL_TASK ) == zLEVEL_TASK )
   {
      ...
      GetBlobFromAttribute( &pTZCToolbar, &ulBlobMax, vTZPNTROO,
                            "Palette", "Pointer" );
   }
   else
   {
        ...
        GetViewByName( &vKZCALLBK, "KZCALLBK", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      GetBlobFromAttribute( &fpPainterCall, &ulBlobMax, vKZCALLBK,
                            "Root", "Painter" );
      return( (*fpPainterCall)( nMsg, vTZPNTROO, pTZCToolbar,
                                vDialog, pchParm ) );
   }
   else
   {
#ifdef __WIN32__
      typedef zSHORT (POPERATION PFN_PAINTERCALL) ( zSHORT, zVIEW, zPVOID,
                                                    zVIEW, zPCHAR );

      static PFN_PAINTERCALL pfnPainterCall = 0;

      // Load PainterCall().  Note that we'll just let Core close the library
      ...
      if ( pfnPainterCall )
      {
         return( (*pfnPainterCall)( nMsg, vTZPNTROO, pTZCToolbar, vDialog, pchParm ) );
      }
      else
         return( 0 );
#else
      return( PainterCall( nMsg, vTZPNTROO, pTZCToolbar, vDialog, pchParm ) );
#endif
   }
}
                   ...
                   zBOOL  bPointer )
{
   ...
   switch ( *pchDataTypeCode )
   {
      ...
      case 'S':
         ...
         {
            zstrcpy( &lpchReturnType[ nReturnLth ], "CPCHAR" );
            nReturnLth += 6;
         }
         else
         {
            zstrcpy( &lpchReturnType[ nReturnLth ], "PCHAR" );
            nReturnLth += 5;
         }
         break;

      case 'Y':
         zstrcpy( &lpchReturnType[ nReturnLth ], "CHAR" );
         nReturnLth += 4;
         break;

      case 'L':
         zstrcpy( &lpchReturnType[ nReturnLth ], "LONG" );
         nReturnLth += 4;
         break;

      case zTYPE_DECIMAL:
         zstrcpy( &lpchReturnType[ nReturnLth ], "DECIMAL" );
         nReturnLth += 7;
         break;

      case 'V':
         zstrcpy( &lpchReturnType[ nReturnLth ], "VIEW" );
         nReturnLth += 4;
         break;
      ...
      case 'O':
      case 0:
         zstrcpy( &lpchReturnType[ nReturnLth ], "VOID" );
         nReturnLth += 4;
         break;
      ...
      case 'N':
      default:
         zstrcpy( &lpchReturnType[ nReturnLth ], "SHORT" );
         nReturnLth += 5;
         break;
                         ...
                         zCHAR  cOperType )
{
   ...
   zstrcpy( pchEndPtr, "zOPER_EXPORT " );
   pchEndPtr += 13;

   // Set the correct return type string.
   GetAddrForAttribute( &pchDataTypeCode,
                        vOperation, "Operation", "ReturnDataType" );
   ...
   pchEndPtr = zstrcpyp( pchEndPtr, " OPERATION\n" );
   ...
   nParmStartColumn = zstrlen( pchOperationName ) + 2;
   ...
   if ( cOperType != 'G' )
   {
      ...
      GetStringFromAttribute( szLanguageType,
                              vOperation, pchSourceFileEntity, "LanguageType" );
      if ( szLanguageType[ 0 ] == 'V' )
      {
         zPCHAR pchSourceFileName;

         pchEndPtr = zstrcpyp( pchEndPtr, "o" );

         GetAddrForAttribute( &pchSourceFileName,
                              vOperation, "LOD", "Name" );
         pchEndPtr = zstrcpyp( pchEndPtr, pchSourceFileName );
         pchEndPtr = zstrcpyp( pchEndPtr, "_" );

         // The operation name just got longer, so add the extra chars to
         // nParmStartColumn (don't forget about "o" and "_").
         nParmStartColumn += zstrlen( pchSourceFileName ) + 2;
      }
   }

   pchEndPtr = zstrcpyp( pchEndPtr, pchOperationName );
   pchEndPtr = zstrcpyp( pchEndPtr, "( " );
         ...
         nRC = SetCursorNextEntity( vOperation, "Parameter", 0 ), nCount++ )
   {
      ...
      if ( bFirstParm == FALSE )
      {
         ...
         // This is not the first parm in the parm list, so add newline and
         // spaces to indent the parm.
         pchEndPtr = zstrcpyp( pchEndPtr, ",\n" );
      ...
      GetAddrForAttribute( &pchDataTypeCode,
                           vOperation, "Parameter", "DataType" );
      GetAddrForAttribute( &pchPointerFlag,
                           vOperation, "Parameter", "PFlag" );
      GetAddrForAttribute( &pchUnsignedFlag,
                           vOperation, "Parameter", "UFlag" );
      pchEndPtr += DetermineDataType( vOperation, pchEndPtr,
                                      pchDataTypeCode,
      ...
      // Get short desc.
      GetStringFromAttribute( szShortDesc,
                              vOperation, "Parameter", "ShortDesc" );
   ...
   if ( bFirstParm )
   {
      pchEndPtr = zstrcpyp( pchEndPtr, " void" );
   }
                       ...
                       zVIEW  vTaskLPLR )
{
   ...
   GetStringFromAttribute( szNew, vTaskLPLR, "LPLR", "PgmSrcDir" );
   SysConvertEnvironmentString( szOld, szNew );
   zstrcat( szOld, "\\ZEIDONOP" );     // add back slash and name
   ...
   if ( lfOldFile >= 0 )
   {
      ...
      zstrcpy( szStartLine, "//start " );
      zstrcat( szStartLine, pchMetaName );
      zstrcat( szStartLine, " " );
      nStartLineLth = zstrlen( szStartLine );
   ...
   hPrototype = SysAllocMemory( (zCOREMEM) &pchPrototypeBuffer, 1000,
                                0, zCOREMEM_ALLOC, 0 );
         ...
         nRC = SetCursorNextEntity( vMeta, "Operation", pchRootName ) )
   {
      ...
      if ( bFirstPrototype )
      {
         ...
         UfFormatDateTime( szFormat, szDateTime, "YYYY.MM.DD HH:MI:SS" );
         zstrcpy( szStartLine, "//start " );
         zstrcat( szStartLine, pchMetaName );
         zstrcat( szStartLine, " " );
         zstrcat( szStartLine, szFormat );
      ...
      // GlobalOperationGroup for global operations, but SourceFile for
      // LOD operations.
      if ( pchRootName[ 0 ] == 'L' )
         zstrcpy( szName, "SourceFile" );
      else
         zstrcpy( szName, "GlobalOperationGroup" );

      // Build the operation prototype.  We need to tack on the ending
      // semi-colon.
      nLth = BuildOperationPrototype( vMeta, szName, pchMetaName,
                                      pchPrototypeBuffer, *pchOperType );
                   ...
                   zPCHAR pchLine )
{
  ...
  zsprintf( pchLine, "DESCRIPTION   '%s'", pchTargetName );
                   ...
                   zPCHAR pchLine )
{
  ...
  // 19.03.97 GT
  zsprintf( pchLine, "LIBRARY       %s   INITINSTANCE TERMINSTANCE", pchTargetName );
  SysWriteLine( lpTaskView, hFile, pchLine );
  zsprintf( pchLine, "DESCRIPTION   '%s - Generated by Zeidon'", pchTargetName );
  SysWriteLine( lpTaskView, hFile, pchLine );
  SysWriteLine( lpTaskView, hFile, "" );
                    ...
                    zPCHAR        pchLine )
{
   ...
   if ( hDFB != -1 )
   {
      ...
      if ( hDEF != -1 )
      {
         ...
         nRC = SetCursorFirstEntityByInteger( vTaskLPLR, "W_MetaType",
                                              "Type", zREFER_GOPGRP_META, 0 );
         ...
         if ( nRC >= zCURSOR_SET )
         {
                  ...
                  nRC = SetCursorNextEntity( vTaskLPLR, "W_MetaDef", 0 ) )
            {
               ...
               // Get the group name
               nRCM = GetAddrForAttribute( &pchGroup, vTaskLPLR,
                                           "W_MetaDef", "Name" );

               // look, if we have this global op group in our Target DLL
               for ( nRCM = SetCursorFirstEntityByInteger( vTaskLPLR, "Meta",
                                                           "MetaType",
                                                           zREFER_GOPGRP_META,
                                                           0 );
                     nRCM >= zCURSOR_SET;
                     nRCM = SetCursorNextEntityByInteger( vTaskLPLR, "Meta",
                                                          "MetaType",
                                                          zREFER_GOPGRP_META,
                                                          0 ) )
               {
                  if ( SetCursorFirstEntityByString( vTaskLPLR, "Meta",
                                                     "Name", pchGroup,
                                                     0 ) >= zCURSOR_SET )
                  {
                     nGroupInTarget = TRUE;
               ...
               if ( !nGroupInTarget )
               {
                  ...
                  GetAddrForAttribute( &pchOpDLL, vMeta,
                                       "GlobalOperationGroup",
                                       "DomainAndGlobalOpGroupName" );

                  zstrcpy( szDLLUpper, pchOpDLL);
                        ...
                        nRCG = SetCursorNextEntity( vMeta, "Operation", 0 ) )
                  {
                    ...
                    if (*pchOpType != 'L')
                    {
                      ...
                      zsprintf( pchLine, "   %-32s=%s.%s",
                                szOpNameUpper, szDLLUpper, pchOpName );
...
// zCHAR  szMetaFileName[ zMAX_FILENAME_LTH ];
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "Meta", 0 ) )
   {
      ...
      {
         // Activate the meta.
         GetStringFromAttribute( szMetaName, vTaskLPLR, "Meta", "Name" );
         ...
         {
            zsprintf( pchLine,
                      "Could not open dialog meta for update: '%s'",
                      szMetaName );
         ...
         nRC = SetResourceFileIDs( vSubtask, vTaskLPLR, hFile, pcd, pchLine, vDialog );
         if ( nRC > nChangeLevel )
            nChangeLevel = nRC;
            ...
            zPCHAR        pchLine )
{
   ...
   if ( hFile == -1 )
   {
      ...
      // write header
      zsprintf( pchLine, "/* %s.rc : Defines the resources for the target "
               "executable %s */", pchTargetName, pchTargetName );
      SysWriteLine( vTaskLPLR, hFile, pchLine );

      // Write resources.
      nRC = fnWriteZeidonResourcesToRC( hFile, vSubtask, vTaskLPLR,
                                        pcd, pchLine );
   ...
   else
   {
      ...
      for ( ; ; )
      {
         ...
         if ( pchRC[ 0 ] == '/' &&
              zstrncmp( pchRC, ZEIDON_RC_START,
                        zstrlen( ZEIDON_RC_START ) ) == 0 )
         {
            ...
            for ( ; ; )
            {
               ...
               if ( pchRC[ 0 ] == '/' &&
                    zstrncmp( pchRC, ZEIDON_RC_END,
                              zstrlen( ZEIDON_RC_END ) ) == 0 )
               {
                  break;
               }
            }
         ...
         {
            // We haven't hit ZEIDON section yet ... copy line from
      ...
      nRC = fnWriteZeidonResourcesToRC( hOut, vSubtask, vTaskLPLR, pcd, pchLine );
      ...
      if ( nRC )
      {
         ...
         zstrcpy( pchLine, szFileNameTemp );
         zstrcpy( &pchLine[ zstrlen( pchLine ) + 1 ], szFileName );
...
// zstrcpy( szObjectDir, cdir.pszTargetObjectDir );
   SysConvertEnvironmentString( szObjectDir, cdir.pszTargetObjectDir );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szObjectDir,
                                       cdir.pszMetaSrcDir,
                                       zMAX_FILENAME_LTH - 2,
                                       "OBJ Directory") == -1 )
   {
   ...
   if ( !VerifyDir( vTaskLPLR, cdir.pszTargetObjectDir,
                    "OBJ Directory", TRUE ) )
   {
      goto EndOfFunction;
   }
...
// zstrcpy( szMakefileDir, cdir.pszMakefileDir );
   SysConvertEnvironmentString( szMakefileDir, cdir.pszMakefileDir );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szMakefileDir,
                                       cdir.pszMetaSrcDir,
                                       zMAX_FILENAME_LTH - 2,
                                       "Makefile Directory") == -1 )
   {
      goto EndOfFunction;
   }
...
// zstrcpy( szEnvironmentDir, cdir.pszEnvironmentDir );
   SysConvertEnvironmentString( szEnvironmentDir, cdir.pszEnvironmentDir );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szEnvironmentDir, cdir.pszMetaSrcDir, zMAX_FILENAME_LTH - 2,
...
// zstrcpy( szTargetDir, cdir.pszTargetExecutableDir );
   SysConvertEnvironmentString( szTargetDir, cdir.pszTargetExecutableDir );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szTargetDir,
                                       cdir.pszMetaSrcDir,
                                       zMAX_FILENAME_LTH - 2,
                                       "Target Executable Directory" ) == -1 )
   {
      goto EndOfFunction;
   }

   cdir.pszTargetExecutableDir = szTargetDir;
   if ( !VerifyDir( vTaskLPLR, cdir.pszTargetExecutableDir,
                    "Target Executable Directory", TRUE ) )
   {
      goto EndOfFunction;
   }
   ...
   if ( *cdir.szZeidonExecutableDir == 0 )
      zstrcpy( cdir.szZeidonExecutableDir, cdir.pszCompilerDir );
   ...
   hLine = SysAllocMemory( (zCOREMEM) &pchLine, zLINESIZE, 0,
                           zCOREMEM_ALLOC, 0 );
   if ( pchLine == 0 )
   {
      nReturnCode = 0;
   ...
   switch ( nCompilerType )
   {
      case zCOMP_TYPE_MSVC15:
         nRC = fnCreateRC( pchTargetName, vSubtask, vTaskLPLR, &cdir, pchLine );
         if ( nRC < 0 )
         {
            nReturnCode = 0;
            goto EndOfFunction;
   ...
   if ( hFile == -1 )
   {
      zsprintf( pchLine, "Could not create file\n  '%s'\nCheck Compiler "
   ...
   SWL( vTaskLPLR, "#" );
   zsprintf( pchLine, "#  %s.mak", pchTargetName );
   SWL( vTaskLPLR, pchLine );
   SWL( vTaskLPLR, "#  Makefile for Zeidon Source File" );
   SysGetDateTime( szFileName );
   UfFormatDateTime( szFileName2, szFileName, "YYYY.MM.DD HH:MI:SS" );
   zsprintf( pchLine, "#  Generated at %s", szFileName2 );
   SWL( vTaskLPLR, pchLine );
   SWL( vTaskLPLR, "#" );
   ...
   zsprintf( pchLine, "!include \"%s\\zenv.hnm\"", cdir.pszMakefileDir );
   SWL( vTaskLPLR, pchLine );
   SWL( vTaskLPLR, " " );

   zsprintf( pchLine, "MODULE = %s", pchTargetName );
   SWL( vTaskLPLR, pchLine );
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "Meta", 0 ) )
   {
      ...
      {
         case zREFER_GOPGRP_META:
            zstrcat( szFileName, ".POG" );
            pchOD_Name = "TZOGSRCO";
            pchSourceFileEntityName = "GlobalOperationGroup";
            break;

         case zREFER_DIALOG_META:
            zstrcat( szFileName, ".PWD" );
            pchOD_Name = "TZWDLGSO";
            pchSourceFileEntityName = "SourceFile";
            break;
            ...
            nRC = SetCursorNextEntity( vMeta, pchSourceFileEntityName, 0 ) )
      {
         ...
         zstrcpy( szObjString, " $(OBJ_DIR)\\" );
         zstrcat( szObjString, pch );
         ...
         if ( SetCursorFirstEntity( vMeta, "Operation", NULL ) != zCURSOR_SET )
         {
            ...
            if ( hCFile == -1 )
            {
               ...
               {
                  // write comment
                  zsprintf( szMsg, "/* %s : Dummy C file */", szFileName2 );
         ...
         if ( CompareAttributeToString( vMeta, pchSourceFileEntityName,
                                        "LanguageType", "V" ) == 0 )
         {
            zVIEW  vXPG;
            zPCHAR pchXPG_Name;
            ...
            if ( cpcGenLang[ 0 ] == 'J' )
               zstrcat( szXPG_FileName, ".XPJ" );
            else
               zstrcat( szXPG_FileName, ".XPG" );

            ActivateOI_FromFile( &vXPG, "TZVSPOOO", vTaskLPLR,
                                 szXPG_FileName, zSINGLE );
                  ...
                  nRC = SetCursorNextEntity( vXPG, "ExternalOperation", 0 ) )
            {
               ...
               // add it to ZeidonLib.
               if ( CompareAttributeToAttribute( vXPG, "ExternalOperation",
                                                 "MetaDLL_Name",
                                                 vTaskLPLR, "TargetExecutable",
                                                 "Name" ) == 0 )
               {
                  // Skip this one.
                  continue;
               ...
               if ( SetCursorFirstEntityByAttr( vTaskLPLR,
                                                "ZeidonLib", "Name",
                                                vXPG, "ExternalOperation",
                                                "MetaDLL_Name",
                                                0 ) < zCURSOR_SET )
               {
                  CreateEntity( vTaskLPLR, "ZeidonLib", zPOS_LAST );
                  SetAttributeFromAttribute( vTaskLPLR, "ZeidonLib", "Name",
                                             vXPG, "ExternalOperation",
                                             "MetaDLL_Name" );
               }
   ...
   // If there is an ExternalTarget entity with name matching the target
   // name, then the user has possibly specified some external modules that
   // need to be linked in with the current target.
   if ( SetCursorFirstEntityByString( vTaskLPLR, "ExternalTarget", "Name",
                                      pchTargetName, 0 ) >= zCURSOR_SET )
   {
      SWL( vTaskLPLR, "# External libraries/objects for this target" );
      zstrcpy( pchLine, "LINKDLLS = " );
            ...
            nRC = SetCursorNextEntity( vTaskLPLR, "ExternalObjFile", 0 ) )
      {
         ...
         if ( bPath == FALSE )
            zstrcat( pchLine, "$(OBJ_DIR)\\" );

         zstrcat( pchLine, pchObj );
         zstrcat( pchLine, ".OBJ" );
         zstrcat( pchLine, " " );
      }
   ...
   *pchLine = 0;
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "ZeidonLib", 0 ) )
   {
      ...
      if ( *pchLine == 0 )
         zsprintf( pchLine, "LINKDLLS = $(LINKDLLS) %s.lib", pchLib );
      else
      {
         zstrcat( pchLine, " " );
         zstrcat( pchLine, pchLib );
         zstrcat( pchLine, ".lib" );
      }
   ...
   zsprintf( pchLine, "!include \"%s\\zcompile.hnm\"", cdir.pszMakefileDir );
   SWL( vTaskLPLR, pchLine );
                         ...
                         zCPCHAR cpcGenLang )
{
   ...
   GetStringFromAttribute( szLocalDir, vTaskLPLR, "LPLR", "MetaSrcDir" );
   SysConvertEnvironmentString( szBaseDir, szLocalDir );
   ...
   zstrcpy( szLocalDir, pchEnvironmentDir );
   SysConvertEnvironmentString( szEnvDir, szLocalDir );
   if (IntGenFullQualPathFromRelPath( vTaskLPLR, szEnvDir, szBaseDir,
                                      zMAX_FILENAME_LTH - 2,
                                      "Resource Directory") == -1 )
   {
      return( FALSE );
   ...
   // Build relative Path; base is Component Source directory.
   zstrcpy (szMakefileDir, pchMakefileDir);
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szMakefileDir, szBaseDir,
                                       zMAX_FILENAME_LTH - 2,
                                       "Makefile Directory") == -1 )
   {
      return( FALSE );
   ...
   else
   {
      ...
      SysWriteLine( vTaskLPLR, hFile, "/*                             */" );
      zsprintf( szLine, "'@SET BEGINLIBPATH=%s;'", pchCompilerDir);
      SysWriteLine( vTaskLPLR, hFile, szLine );
      zsprintf( szLine, "'@SET PATH=%s;%%PATH%%'", pchCompilerDir);
      SysWriteLine( vTaskLPLR, hFile, szLine );
      if (pchMakefileDir[ 1 ] == ':')
      {
        zsprintf( szLine, "'@%c:'", pchMakefileDir[ 0 ] );
        SysWriteLine( vTaskLPLR, hFile, szLine );
        zsprintf( szLine, "'@CD %s'", &pchMakefileDir[ 2 ] );
      }
      else
      {
        zsprintf( szLine, "'@CD %s'", pchMakefileDir );
      }
   ...
   {
      if (nCompilerType == zCOMP_TYPE_MSVC15)
         zsprintf( pchReturnCommand, "%s\\ZCOMPILE.PIF", pchEnvironmentDir );
      else
         zsprintf( pchReturnCommand, "%s\\ZCOMPOS2.PIF", pchEnvironmentDir );
   }
...
//BL     DropView( vTaskLPLR );
   ...
   zstrcpy( szLocalDir, pchEnvironmentDir );
   SysConvertEnvironmentString( szEnvironmentDir, szLocalDir );
   if ( IntGenFullQualPathFromRelPath( vTaskLPLR, szEnvironmentDir, szBaseDir,
                                       zMAX_FILENAME_LTH - 2,
                                       "Resource Directory") == -1 )
   {
      return( 0 );
   ...
   else
   {
            ...
            nRC = SetCursorNextSelectedEntity( vTaskLPLR, "TargetExecutable", 0 ) )
      {
         ...
         SetSelectStateOfEntity( vTaskLPLR, "TargetExecutable", 0 );  // Turn off select
         GetAddrForAttribute( &pchTargetName,
                              vTaskLPLR, "TargetExecutable", "Name" );
                        ...
                        zLONG  lLength )
{
   ...
   zstrcpy( pchStringValue, szWorkString );
#if 0
...
StartZeidonHelp( zVIEW vSubtask, zPCHAR pchHelpTag )
{
   ...
   {
      zstrcpy( szFileName, "SYSTEM HELP" );
...
// if ( GetViewByName( &vHelpMap, "TZHLPCXO", vSubtask, zLEVEL_TASK ) < 0 )
   if ( GetViewByName( &vHelpMap, "TZHLPCXO", vSubtask, zLEVEL_TASK ) < 0 )
   {
      ...
      zstrcat( szFileName, "TZHLPCXP.PHL");
   ...
   GetStringFromAttribute( szContextID, vHelpMap, "Context", "Number" );

   // build up file name for help file
   GetStringFromAttribute( szFile, vHelpMap, "Context", "FileName" );
   SysGetLocalDirectory( szFileName );
   zstrcat( szFileName, szFile );
      ...
      case zDME_SET_ATTRIBUTE:
         if ( lpDomainData->lpViewAttribute->cType == zTYPE_STRING )
         {
            ...
            switch ( lpDomainData->cType )
            {
               case zTYPE_STRING:
                  // string to string
                  zstrcpy( sz, (zPCHAR) lpDomainData->lpData );
         ...
         if ( lpDomainData->lpViewAttribute->cType == zTYPE_STRING )
         {
            ...
            switch ( lpDomainData->cType )
            {
               case zTYPE_STRING:
                  ...
                  {
                     // string to string
                     zstrcpy( (zPCHAR) lpDomainData->lpData,
                              (zPCHAR) lpAttrAddr );
                     nRC = 0;
                  }
      ...
      case zDME_GET_COMPARE_VALUE:
         ...
         switch ( lpDomainData->cType )
         {
            ...
            case zTYPE_INTEGER:
               ...
               zstrcpy( (zPCHAR) lpDomainData->lpData, szWorkString );
         ...
         if ( lpDomainData->lpViewAttribute->cType == zTYPE_STRING )
         {
            ...
            switch ( lpDomainData->cType )
            {
               case zTYPE_STRING:
                  ...
                  {
                     // string to string
                     zstrcpy( (zPCHAR) lpDomainData->lpData, (zPCHAR) lpAttrAddr );
                     nRC = 0;
                  }
...
#define CACHE_SIZE      10000
   ...
   zstrcpy( szTempFileName, pchFileName );
   ...
   nOperNameLth = zstrlen( pchOperName );
   lLineCount   = 0;
   nLineIdx     = 0;
   nBufferIdx   = 0;
   ...
   while ( SysReadLine( vSubtask, &pchLine, fIn ) == 1 )
   {
      ...
      // Check to see if there is enough room in the buffer to store the new
      // line.
      nLineLth = zstrlen( pchLine ) + 1;
      if ( pchBufferEnd + nLineLth > pchBuffer + CACHE_SIZE )
      {
         // The new line can't fit at the end of the buffer so put it at the
   ...
   if ( !bOperationFound )
   {
      ...
      zsprintf( pchBuffer, "Couldn't find the operation '%s' in the source "
   ...
   else
   {
      ...
      zstrcpy( pchBuffer, szTempFileName );
      zstrcpy( &pchBuffer[ zstrlen( pchBuffer ) + 1 ], pchFileName );
                              ...
                              zPCHAR  lpLOD_Name )
{
         ...
         nRC = SetCursorNextEntity( vMeta, "ER_RelLinkRec", 0 ) )
   {
      ...
      if ( nRC < zCURSOR_SET )
      {
         ...
         GetStringFromAttribute( szRelLinkName, vMeta2,
                                 "ER_RelLinkRec", "Name" );
         ResetViewFromSubobject( vMeta2 );
         GetStringFromAttribute( szTargetEntityName, vMeta2,
                                 "ER_EntityRec", "Name" );
         DropView( vMeta2 );
      ...
      if ( nRC < zCURSOR_SET )
      {
         ...
         zstrcpy( szMsg, "A relationship in LOD, " );
         zstrcat( szMsg, lpLOD_Name );
         zstrcat( szMsg, ", has been altered in the ERD.\nSave the LOD " );
         zstrcat( szMsg, "and rerun the report to make sure it is accurate." );
                            ...
                            zPCHAR pchSuffix )
{
   ...
   if ( hFile >= 0 )
   {
                                ...
                                zSINGLE | zIGNORE_ERRORS | zLEVEL_SYSTEM ) < 0 )
      {
         ...
         zstrcpy( szMsg, "Error in reading Meta: " );
         zstrcat( szMsg, pchMetaName );
                   ...
                   zVIEW  vERD )
{
   ...
   if ( zstrcmp( pchSuffix, "PVR" ) == 0 )
   {
      SetCursorFirstEntityByInteger( vTaskLPLR, "W_MetaType", "Type",
                                     zREFER_VOR_META, 0 );
      zstrcpy( szType, "TZWDVORO" );
   }
   else
   {
      SetCursorFirstEntityByInteger( vTaskLPLR, "W_MetaType", "Type",
                                     zREFER_DIALOG_META, 0 );
      zstrcpy( szType, "TZWDLGSO" );
   }
         ...
         nRC = SetCursorNextEntity( vTaskLPLR, "W_MetaDef", 0 ) )
   {
      ...
      {
         GetStringFromAttribute( szMetaName,
...
GetVMLConstChar( )
{
   ...
   nVMLConstChar = 0;
   szTemp[ 0 ] = 0;
   SysReadZeidonIni( -1, "[Zeidon]", "VMLConstChar", szTemp );
                          ...
                          zPCHAR lpC_VariableName )
{
   ...
   if ( pCtrlBOI )
   {
      if ( zstrcmp( lpC_VariableName, "BMP.szBMPUpOrIconName" ) == 0 )
      {
         ...
         zstrcpy( lpReturnString, (zPCHAR)BMP.szBMPUpOrIconName );
      }
   }
   ...
   if ( zstrcmp( lpC_VariableName, "zCONTROL_BORDEROFF" ) == 0 )
   {
      ...
      if ( (lSubtype & zCONTROL_BORDEROFF ) == 0 )
         zstrcpy( lpReturnString, "Y");
      else
         zstrcpy( lpReturnString, "");
   }
...
{
   zSHORT nLth = zstrlen( cpcOperationName );
   zSHORT nPos;
   zSHORT k = 0;
...
fnTestForKeyWord( zCPCHAR cpcLine, zBOOL *pbMultiLineComment )
{
   ...
   nLth = zstrlen( cpcLine );
   if ( nLth >= sizeof( szLine ) )
   {
      TraceLineI( "CopyOperationCode Line greater than buffer size (4096) ... using malloc: ", nLth );
      pchLine = SysMalloc( nLth );
   }
   else
      pchLine = szLine;

   // strtok modifies cpcLine ... so we copy it to a modifiable buffer!
   zstrcpy( pchLine, cpcLine );

   // Establish string and get the first token.
   pchToken = strtok( pchLine, chSeparators );

   while ( pchToken ) // while there are tokens in szLine
   {
      nLth = zstrlen( pchToken );
      ...
      // Get next token.
      pchToken = strtok( 0, chSeparators );
   }
                   ...
                   zPCHAR  pchOperationName )
{
   ...
   zstrcat( szTgtFile, pchTgtFileName );
   zstrcat( szSrcFile, pchSrcFileName );
   ...
   if ( nRC > 0 )
   {
      ...
      else
      {
         ...
         if ( nRC > 0 )
         {
            ...
            zstrcpy( szTmpFile, szTgtFile );
            zstrcat( szTmpFile, ".tmp" );
                      ...
                      zLONG  lMaxLength )
{
   ...
   lpReturnString[ k ] = 0;
   zstrcpy( lpOriginalString, lpReturnString );
   return( 0 );
}