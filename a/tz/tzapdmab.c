...
// Month table
static const
char zFAR *pchMon[ 12 ] = { "January", "February", "March", "April",
...
fnSpecialContext( zPCHAR pchContext, zCHAR  cType )
{
   ...
   for ( k = 0; k < nNbrContextEntries; k++ )
   {
      ...
      {
         // if cType is null, we only care if the Context name is in
         // the special table...
         if ( cType == 0 )
            return( k );
         ...
         LPDOMAINDATA lpDomainData )
{
   if ( lpDomainData && lpDomainData->pszContextName &&
        (zstrcmp( lpDomainData->pszContextName, "REVMMDDYY" ) == 0 ||
         zstrcmp( lpDomainData->pszContextName, "ESLMMDDYY" ) == 0) )
   {
      zLONG  lRC;
      ...
      if ( lRC >= 0 )
      {
         ...
         zstrcpy( szDate + 3, (zPCHAR) (lpDomainData->lpData) );
         zstrcpy( (zPCHAR) (lpDomainData->lpData), szDate );
      }
            ...
            LPDOMAINDATA lpDomainData )
{
   ...
   if ( lpDomainData->lpViewAttribute->cType != zTYPE_DATETIME &&
        lEntryType != zDME_VALIDATE_LPDATA &&
        lpDomainData->cType != zTYPE_STRING )
   {
                        ...
                        zBOOL        bValidationOnly )
{
   ...
   switch ( lpDomainData->cType )
   {
      case zTYPE_STRING:
         ...
         {
            nRC = GetStringFromRecord( lpDomainData->zView,
            ...
            switch ( nRC )
            {
               ...
               case -1:
                  ...
                  bWasNull = 1;
                  zstrcpy( szDT1, "00000000000000000" );
                  break;
            ...
            if ( GetContext( &lpContext, lpDomain,
                             (zCPCHAR) lpDomainData->pszContextName ) )
            {
               if ( (nContextIndex = fnSpecialContext( lpContext->szName,
                                                       0 ) ) >= 0 )
               {
                  switch ( nContextIndex )
                  {
                     case eDAYOFMONTH:
                        ...
                        else
                        {
                           ...
                           SendDomainError( lpDomainData->zView, lpDomain, nSeverity, 16,
                                            0, sz, lpContext->szName );
                           return( zCALL_ERROR );
                        }
                     ...
                     case eYY:
                        // Eliminated support of 2-digit years and forced
                        // them to be treated as 4-digit years.
#if 0
                        if ( isdigit( *sz ) &&
                             isdigit( *(sz + 1) ) )
                        {
                     ...
                     case eYEAR:
                        ...
                        // "TZDME015 - Context only used for arithmetic
                        //  operations "
                        SendDomainError( lpDomainData->zView, lpDomain,
                                         nSeverity, 15, 0,
                                         lpContext->szName, 0 );
                     ...
                     default:
                        ...
                        nRC = UfEditFormatDateTime( sz, pszEditString );
                        ...
                        if ( nRC )
                        {
                           ...
                           else
                           {
                              ...
                              // "TZDME009 - Context edit string is
                              //  invalid for Domain "
                              SendDomainError( lpDomainData->zView, lpDomain,
                                               nSeverity, 9, 0,
                                               lpContext->szName,
               ...
               else
               {
                  ...
                  if ( nRC )
                  {
                     ...
                     else
                     {
                        ...
                        SendDomainError( lpDomainData->zView, lpDomain,
                                         nSeverity, 9, 0,
                                         lpContext->szName, pszEditString );
                     }
            ...
            else
            {
               ...
               SendDomainError( lpDomainData->zView, lpDomain,
                                nSeverity, 8, 0,
                                (zCPCHAR) lpDomainData->pszContextName, 0 );
               return( zCALL_ERROR );
            }

#if 0
//* This code was eliminated 1999/04/22 by DC because DM_Date should
always have Time portion set to zeros and DM_Time should always have Date
portion set to "19000101".
            // If domain is Date or Time, retrieve the portion that will
            // not be changing and place into string
            if ( nDomain != eDM_DateTime )
            {
               switch ( nDomain )
               {
                  case eDM_Date:
                     // When working with the Date domain leave the
                     // time portion as is.
                     zmemcpy( psz + 8, szDT1 + 8, 10 );
                     break;

                  case eDM_Time:
                     // When working with the Time domain leave the
                     // date portion as is.
                     zmemcpy( psz, szDT1, 8 );
                     break;
               }
         ...
         // Added 1999/04/22 by DC so that Dates would have time portion set
         // to zeros and Times would have date portion set to "19000101".
         if ( nDomain != eDM_DateTime )
         {
            switch ( nDomain )
            {
               case eDM_Date:
                  // When working with the Date domain, force the time portion
                  // to zeros.
                  zmemcpy( psz + 8, "000000000", 10 );
                  break;

               case eDM_Time:
                  // When working with the Time domain, force the date portion
                  // to "19000101".
         ...
         if ( nRC )
         {
            ...
            zstrcat( sz, lpDomainData->lpViewEntity->szName );
            // "TZDME010 - DateTime input string invalid "
            SendDomainError( lpDomainData->zView, lpDomain, nSeverity, 10, 0,
                             sz, lpDomainData->lpViewAttribute->szName );
         }
         ...
         if ( nRC == 0 && bValidationOnly == FALSE )
         {
            ...
            if ( nRC )
            {
               ...
               SendDomainError( lpDomainData->zView, lpDomain, nSeverity, 11, 0,
                                sz, lpDomainData->lpViewAttribute->szName );
            }
         }
      ...
      case zTYPE_INTEGER:
         if ( lpDomainData->pszContextName == 0 || *lpDomainData->pszContextName == 0 )
         {
            ...
            SendDomainError( lpDomainData->zView, lpDomain, nSeverity, 12, 0,
                             lpDomainData->lpViewEntity->szName,
                             lpDomainData->lpViewAttribute->szName );
            return( zCALL_ERROR );
         }

         nRC = GetStringFromRecord( lpDomainData->zView, lpDomainData->lpViewEntity, lpDomainData->lpViewAttribute,
                                    szDT1, sizeof( szDT1 ) );
         switch ( nRC )
         {
            case 0:
         ...
         {
            if ( (nContextIndex =
                             fnSpecialContext( lpContext->szName,
                                               lpDomainData->cType ) ) >= 0 )
            {
               switch ( nContextIndex )
               {
                  case eDAYOFMONTH:
                     zltoa( *((zPLONG) lpDomainData->lpData) + 100, sz );
                     *(szDT1 + 6) = *(sz + 1);
                     *(szDT1 + 7) = *(sz + 2);
                     break;
                  ...
                  case eYY:
                     ...
                     // them to be treated as 4-digit years.
#if 0
                     zltoa( *((zPLONG) lpDomainData->lpData) + 1900, sz );
                     zmemcpy( szDT1, sz, 4 );
                     break;
#endif
                  case eYYYY:
                     zltoa( *((zPLONG) lpDomainData->lpData) + 10000, sz );
                  ...
                  {
                     zCHAR sz2[ 18 ];
                     zltoa( *((zPLONG) lpDomainData->lpData) + 1000, sz );
                     zmemcpy( sz2, szDT1, 4 ); // copy yyyy
                     zstrcpy( sz2 + 4, sz + 1 ); // copy jjj
               ...
               if ( nRC == 0 && bValidationOnly == FALSE )
               {
                  ...
                  if ( nRC )
                  {
                     ...
                     zstrcat( sz, lpDomainData->lpViewEntity->szName );
                     // "TZDME011 - Error storing value in record "
                     SendDomainError( lpDomainData->zView, lpDomain, 16, 11, 0,
                                      sz, lpDomainData->lpViewAttribute->szName );
                  }
               }
            }
            else
            {
               ...
               sz[ k + 2 ] = 0;
               // "TZDME013 - Context/cType Combination is invalid "
               SendDomainError( lpDomainData->zView,
                                lpDomain, nSeverity, 13, 0, sz, 0 );
               return( zCALL_ERROR );
            }
         }
         else
         {
            ...
            // "TZDME008 - Could not find context for Domain "
            SendDomainError( lpDomainData->zView, lpDomain, nSeverity, 8, 0,
                             (zCPCHAR) lpDomainData->pszContextName, 0 );
            return( zCALL_ERROR );
         }
  ...
  //* Eliminated 27.01.1998 by DonC.
      Date to Date operations are OK without considering ContextName.
         if ( lpDomainData->pszContextName && *lpDomainData->pszContextName &&
              zstrcmp( lpDomainData->pszContextName, "DateTime" ) != 0 )
         {
            ...
            *(sz + k + 1) = 0;
            ...
            zstrcat( sz, lpDomainData->lpViewEntity->szName );
            // "TZDME013 - Context/cType Combination is invalid "
            SendDomainError( lpDomainData->zView, lpDomain, 8, 13, 0,
                             sz, lpDomainData->lpViewAttribute->szName );
            return( zCALL_ERROR );
         }
#endif
                        ...
                        LPDOMAINDATA lpDomainData )
{
   ...
   // When retrieving the pic value for an attribute, we cannot assume
   // that there is an entity instance present.
   if ( lpDomainData->cType != zTYPE_PIC )
   {
      nRC = GetStringFromRecord( lpDomainData->zView,
                                 lpDomainData->lpViewEntity, lpDomainData->lpViewAttribute,
                                 szDT1, sizeof( szDT1 ) );
      switch ( nRC )
      {
         case 0:
   ...
   switch ( lpDomainData->cType )
   {
         ...
         if ( lpDomainData->pszContextName && *lpDomainData->pszContextName )
         {
                             ...
                             (zCPCHAR) lpDomainData->pszContextName ) )
            {
                                        ...
                                        lpDomainData->cType ) ) >= 0 )
               {
                  ...
                  else
                  {
                     ...
                     else
                     {
                        ...
                        SendDomainError( lpDomainData->zView, lpDomain, 8, 18,
                                         0, lpContext->szName, 0 );
                  ...
                  if ( nContextIndex == eWEEKOFYEAR )
                     zltoa( UfGetWeekOfYear( szDT1 ), sz2 );
                  else
                  if ( nContextIndex == eFREEFORM )
                  {
                     CheckEditString( pszEditString, lpDomainData, lpContext );
                     if ( zstrcmp( "Date", lpDomain->szName) == 0 )
                        nRC = FormatFreeFormDate( sz2, szDT1, pszEditString );
                     else
                        nRC = FormatFreeFormTime( sz2, szDT1, pszEditString );
                  }
                  else
                     nRC = UfFormatDateTime( sz2, szDT1, pszEditString );

                  if ( nRC == zCALL_ERROR )
                  {
                     // "TZDME009 - Context edit string is invalid for Domain "
                     SendDomainError( lpDomainData->zView, lpDomain, 8,
                                      9, 0, lpContext->szName,
                                      pszEditString );
                     return( nRC );
                  }
               }
               else
               {
                  ...
                  nRC = UfFormatDateTime( sz2, szDT1,
                                          SysGetPointerFromHandle( lpContext->hEditString ) );
                  if ( nRC == zCALL_ERROR )
                  {
                     // "TZDME009 - Context edit string is invalid for Domain "
               ...
               // Copy value to target string.  Since strncpy doesn't automatically
               // add the null-terminator we have to do it ourselves.
               zstrncpy( (zPCHAR) lpDomainData->lpData, sz2,
            ...
            else
            {
               // "TZDME008 - Could not find context for Domain "
               SendDomainError( lpDomainData->zView, lpDomain,
                                8, 8, 0, (zCPCHAR) lpDomainData->pszContextName, 0 );
               return( zCALL_ERROR );
            }
         }
         else
         {
            // no context, just copy our DateTime string as is.
            // Since strncpy doesn't automatically
            // add the null-terminator we have to do it ourselves.
            zstrncpy( (zPCHAR) lpDomainData->lpData, szDT1,
                      lpDomainData->uMaxStringLength );
            ((zPCHAR) lpDomainData->lpData)[ lpDomainData->uMaxStringLength ] = 0;
         }
      ...
      case zTYPE_PIC:
         ...
         if ( GetContext( &lpContext, lpDomain, lpDomainData->pszContextName ) )
         {
            ...
            if ( lpEditString && *lpEditString )
            {
               // Copy value to target string.  Since strncpy doesn't automatically
               // add the null-terminator we have to do it ourselves.
               zstrncpy( (zPCHAR) lpDomainData->lpData, lpEditString,
                         lpDomainData->uMaxStringLength );
               ((zPCHAR) lpDomainData->lpData )[ lpDomainData->uMaxStringLength ] = 0;
            ...
            else
            {
                                              ...
                                              lpDomainData->cType ) ) >= 0 )
               {
                  ...
                  // Copy value to target string.  Since strncpy doesn't automatically
                  // add the null-terminator we have to do it ourselves.
                  zstrncpy( (zPCHAR) lpDomainData->lpData, szTemp,
                            lpDomainData->uMaxStringLength );
                  ((zPCHAR) lpDomainData->lpData)
                                      [ lpDomainData->uMaxStringLength ] = 0;
               }
               else
                  *((zPCHAR) lpDomainData->lpData) = 0;
            }
      ...
      case zTYPE_INTEGER:
         ...
         {
            // "TZDME012 - Context Required when lpDomainData->cType is zTYPE_INTEGER "
            SendDomainError( lpDomainData->zView, lpDomain, 8, 12, 0,
                             lpDomainData->lpViewEntity->szName,
                             lpDomainData->lpViewAttribute->szName );
            return( zCALL_ERROR );
         }
      ...
      default:
         ...
         sz1[ 1 ] = 0;
         zstrcat( sz1, ", " );
         zstrcat( sz1, lpDomainData->lpViewEntity->szName );
         // "TZDME001 - Invalid Input Data Type"
         SendDomainError( lpDomainData->zView, lpDomain, 8, 1,
                          0, sz1, lpDomainData->lpViewAttribute->szName );
         return( zCALL_ERROR );
   }
                            ...
                            LPDOMAINDATA lpDomainData )
{
   ...
   lpDomain = SysGetPointerFromHandle( lpDomainData->lpViewAttribute->hDomain );
   nRC = GetStringFromRecord( lpDomainData->zView, lpDomainData->lpViewEntity,
                              lpDomainData->lpViewAttribute,
                              szDT1, sizeof( szDT1 ) );
   ...
   switch ( lpDomainData->cType )
   {
      case zTYPE_STRING:
         ...
         if ( lpDomainData->lpData )
         {
            ...
            // Case 2: Attribute value is null but compare value is not.
            //         Return -1.
            if ( bWasNull && *((zPCHAR) lpDomainData->lpData) != 0 )
               return( -1 );

            // Case 3: Compare value is null but attribute value is not.
            //         Return +1.
            if ( !bWasNull && *((zPCHAR) lpDomainData->lpData) == 0 )
               return( 1 );

            // if no context specified, assumption is that the input
            // string looks like a DateTime data type that has been
            ...
            if ( lpDomainData->pszContextName == 0 || *lpDomainData->pszContextName == 0 )
            {
               switch ( nDomain )
               {
                  ...
                  case eDM_Time:
                     ...
                     {
                        // no 'time' in the date time input string
                        // indicate that attr > string value
                        nRC = 1;
                        break;
                     }
  ...
  //* Eliminated 27.01.1998 by DonC.
      Date to Date operations are OK without considering ContextName.
         if ( lpDomainData->pszContextName && *lpDomainData->pszContextName )
         {
            zstrcpy( sz1, lpDomainData->pszContextName );
            zstrcat( sz1, "/" );
            k = zstrlen( sz1 );
            ...
            *(sz1 + k + 1) = 0;
            ...
            // "TZDME013 - Context/cType Combination is invalid "
            SendDomainError( lpDomainData->zView, lpDomain, 8, 13, 0,
                             sz1, lpDomainData->lpViewAttribute->szName );
            return( zCALL_ERROR );
         }
#endif
                                        ...
                                        lpDomainData->cType ) ) >= 0 )
               {
                  ...
                  switch ( nContextIndex )
                  {
                     ...
                     case eDAYOFMONTH:
                        *(szDT1 + 8) = 0;
                        l = atol( szDT1 + 6 );
                        nRC = ( l == *((zPLONG) lpDomainData->lpData) ? 0 :
                                ( l > *((zPLONG) lpDomainData->lpData) ? 1 : -1 ) );
                        break;
                           ...
                           LPDOMAINDATA lpDomainData )
{
   ...
   if ( lpDomainData->cType == zTYPE_DATETIME &&
        (lpDomainData->pszContextName == 0 || *lpDomainData->pszContextName == 0) )
      nRC = 0;
   else
      nRC = -2;
                          ...
                          LPDOMAINDATA lpDomainData )
{
   ...
   switch ( nRC )
   {
      ...
      case -1:
         // attribute value was null
         zstrcpy( szDT2, "00000000000000000" );
         break;
...
static zSHORT NormalizeFreeFormDate( zPCHAR sz )
{
   ...
   if ( (pc = strpbrk( szTemp, "./-" )) != NULL )
   {
      ...
      // if l1 or l3 is bigger than 1000, it is clear
   ...
   sprintf( sz, "%04ld", l3 ); // year
   sprintf( sz + 4, "%02ld", l2 ); // month
   sprintf( sz + 6, "%02ld", l1 ); // day
...
NormalizeFreeFormTime( zPCHAR sz )
{
   ...
   zstrcpy( szTemp, sz + strspn( sz, " " ) ); // copy without leading blanks
   ...
   sprintf( sz + 8, "%02ld", l1 ); // hour
   sprintf( sz + 10, "%02ld", l2 ); // minute
   sprintf( sz + 12, "%02ld", l3 ); // second
   sprintf( sz + 14, "%03ld", l4 ); // millisecond
...
LongYearFromShort( zLONG lYear )
{
   ...
   // time window is 70 years back and 29 years in advance
                    ...
                    zCPCHAR pchEditString )
{
   ...
   {
      case 0:  // dd.mm.yyyy
         zstrncpy( pchTarget, pchZeidon + 6, 2 ); // dd
         pchTarget[ 2 ] = 0;
         zstrncat( pchTarget, &cDateSeparator, 1 );
         zstrncat( pchTarget, pchZeidon + 4, 2 ); // mm
         zstrncat( pchTarget, &cDateSeparator, 1 );
         zstrncat( pchTarget, pchZeidon, 4 ); // yyyy
         break;

      case 1:  // yyyy.mm.dd
         zstrncpy( pchTarget, pchZeidon, 4 ); // yyyy
         pchTarget[ 4 ] = 0;
         zstrncat( pchTarget, &cDateSeparator, 1 );
         zstrncat( pchTarget, pchZeidon + 4, 2 ); // mm
         zstrncat( pchTarget, &cDateSeparator, 1 );
         zstrncat( pchTarget, pchZeidon + 6, 2 ); // dd
         break;

      case 2:  // mm.dd.yyyy (US)
         zstrncpy( pchTarget, pchZeidon + 4, 2 ); // mm
         pchTarget[ 2] = 0;
         zstrncat( pchTarget, &cDateSeparator, 1 );
         zstrncat( pchTarget, pchZeidon + 6, 2 ); // dd
         zstrncat( pchTarget, &cDateSeparator, 1 );
                    ...
                    zCPCHAR pchEditString )
{
   ...
   zstrncpy( pchTarget, pchZeidon +8, 2 ); // hh
   pchTarget[ 2 ] = 0;
   if ( kTimeAM )
   {
      ...
      if ( k > 12 )
      {
         k = k - 12;
         iPM = 1;
         sprintf( pchTarget, "%02d", k );
      }
   }

   zstrncat( pchTarget, cTimeSeparator, 1 );
   zstrncat( pchTarget, pchZeidon + 10, 2 ); // mm
   ...
   if ( *pc )
   {
      zstrncat( pchTarget, cTimeSeparator +1, 1 );
      zstrncat( pchTarget, pchZeidon + 12, 2 ); // ss
   }
   ...
   if ( *pc )
   {
      zstrncat( pchTarget, cTimeSeparator + 2, 1 );
      zstrncat( pchTarget, pchZeidon + 14, 3 ); // sss
   }

   if ( iPM )
      zstrcat( pchTarget, " PM");
   else
   if ( kTimeAM )
      zstrcat( pchTarget, " AM");
...
//                      pchEditString, sizeof( szBuffer ) );
      ...
      if ( cDateSeparator == 0 )
      {
         ...
         // "TZDME009 - Context edit string is
         //  invalid for Domain "
         SendDomainError( lpDomainData->zView, lpDomain, 8, 9, 0,
                          lpContext->szName,
                          pchEditString );
...
//                      pchEditString, sizeof( pchEditString ) );
         return;
      }
#endif
