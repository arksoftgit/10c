...
zwTZWDVORD_SetVOR_Caption( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZWINDOW, "PAGEL", vSubtask, zLEVEL_ANY ) > 0 )
      GetStringFromAttribute( szWrk, vTZWINDOW, "Report", "Tag" );
   else
   if ( GetViewByName( &vTZWINDOW, "TZWINDOWL", vSubtask, zLEVEL_ANY ) > 0 )
      GetStringFromAttribute( szWrk, vTZWINDOW, "Dialog", "Tag" );
                                ...
                                zVIEW vLPLR_VOR_Meta )
{
   ...
   GetStringFromAttribute( szViewName, vApplVOR, "ViewObjRef", "Name" );
   if ( szViewName[ 0 ] == 0 )
   {
...
zwTZWDVORD_UpdateVOR_Delete( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szViewName, vApplVOR, "ViewObjRef", "Name" );
      zstrcpy( szMsg, "OK to delete Registered View '" );
                                  ...
                                  zVIEW vTZWINDOWL )
{
   ...
   GetStringFromAttribute( szViewAndLodName, vTZWINDOWL, "ViewObjRef", "Name" );
   zstrcat( szViewAndLodName, "  ( Object Name: " );
   GetStringFromAttribute( szLodName, vTZWINDOWL, "LOD", "Name" );
   zstrcat( szViewAndLodName, szLodName );
   zstrcat( szViewAndLodName, " )" );
                               ...
                               zPCHAR szParentTag )
{
   ...
   if ( SetCursorFirstEntity( vDialog, "Control", "" ) >= zCURSOR_SET )
   {
      do
      {
         ...
         GetStringFromAttribute( szControlTag, vDialog, "Control", "Tag" );
                               ...
                               zSHORT nIsReport )
{
   ...
   {
      zstrcpy( szEntityName, "Window" );
      zstrcpy( szGoupName, "Window" );
      zstrcpy( szScope, "" );
   }
   else  // Zeidon Reports
   {
      zstrcpy( szEntityName, "Group" );
      zstrcpy( szGoupName, "GroupSet" );
      zstrcpy( szScope, "Report" );
   }
   ...
   {
      GetStringFromAttribute( szWindowName, vDialog, szGoupName, "Tag" );

      // if Zeidon Report, set Group Tag additional
      if ( nIsReport == 1 )
      {
         GetStringFromAttribute( szTemp, vDialog, szEntityName, "Tag" );
         zstrcat( szWindowName, " - " );
         zstrcat( szWindowName, szTemp );
      }
...
zwTZWDVORD_Mapp_SortLodStructure( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vMapTreeCtrl, "MapTreeCtrl", vSubtask, zLEVEL_TASK ) >= 0 )
   {
      ...
      SetAttributeFromInteger( vMapTreeCtrl, "RegView", "SortFlag",
                               zSortByLodStructure );
      RefreshCtrl( vSubtask, "ViewTree" );
   }
...
zwTZWDVORD_LoadControlDetailWnd( zVIEW vSubtask, zPCHAR szEntityName )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZPNMAPP,
                            szEntityName, "ZKey" );
                              ...
                              zVIEW vTZPNMAPP_new, zVIEW vTZPNMAPP_old )
{
   if ( CheckExistenceOfEntity( vControl, "CtrlMapView" ) >= zCURSOR_SET &&
        ...
        SetCursorFirstEntityByAttr( vTZPNMAPP_new, "RegView", "ZKey",
                                    ...
                                    "" ) >= zCURSOR_SET )
   {
      if ( SetCursorFirstEntity( vControl, "CtrlMapLOD_Entity",
                                 "" ) < zCURSOR_SET )
      {
         CreateEntity( vTZPNMAPP_new, "RegViewMapping", zPOS_AFTER );
         SetMatchingAttributesByName( vTZPNMAPP_new, "RegViewMapping",
                                      vTZPNMAPP_old, "RegViewMapping",
                                      zSET_ALL );
      }
      else
      if ( CompareAttributeToString( vControl, "CtrlMapLOD_Entity", "Name",
                                     "" ) != 0 &&
                                   ...
                                   zVIEW vTZPNMAPP_new, zVIEW vTZPNMAPP_old )
{
                                     ...
                                     "" ) >= zCURSOR_SET )
   {
      ...
      SetMatchingAttributesByName( vTZPNMAPP_new, "RegAttributeMapping",
                                   vTZPNMAPP_old, "RegAttributeMapping",
                                   zSET_ALL );
   }
...
zwTZWDVORD_ReturnFromSubwindow( zVIEW vSubtask )
{
   ...
   zstrcpy( szMappingEntity, szTreeEntity );
   zstrcat( szMappingEntity, "Mapping" );
   ...
   {
      fnTZWDVORD_ChangeViewMapping( vSubtask, vControl,
