...
void OPERATION
SysConvertEnvironmentString( zPCHAR pchTarget, zPCHAR pchSource );
...
zwTZCMLPLD_CleanCM_Views( zVIEW vSubtask )
{
   ...
   // Try to retrieve the CM view that is named in the "Zeidon Core Services"
   // task.
                            ...
                            zPCHAR szWKS_Desc )
{
   ...
   ActivateEmptyObjectInstance( &vTZCMWKSO, "TZCMWKSO", vSubtask,
   ...
   SetAttributeFromInteger( vTZCMWKSO, "RepositoryClient",
                            "MaxZKey", ++ulMaxZKey );
   SetAttributeFromString( vTZCMWKSO, "RepositoryClient",
                           "Desc", szWKS_Desc );
...
zwfnTZCMLPLD_InitDialog( zVIEW vSubtask )
{
   ...
   while ( (lpTask = SfGetNextTask( lpTask )) != 0 )
   {
      ...
      MessageSend( vSubtask, "CM00299", "Configuration Management",
                   ...
                   "Manager.",
                   zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      // Task isn't loaded. Therefore we have to use  zWAB_ReturnToParent
      // instead of zWAB_ExitDialogTask
...
zwfnTZCMLPLD_EditNewWorkstation( zVIEW vSubtask, zPVIEW vTZCMREPO, zPCHAR szWKS_Desc )
{
   ...
   zstrcpy( szMsg, "A Workstation was not found.\n" );
   zstrcat( szMsg, "Enter Description to build one." );
   szWKS_Desc[ 0 ] = 0;
   ...
   if ( nRC < 0 )
   {
      zstrcpy( szMsg, "The Repository could not be accessed." );
      zstrcat( szMsg, "\nIf you want to create a work station without a "
                      "Repository, respond OK."
                      "\nIf you want to abort this process to install a"
   ...
   while ( SetCursorFirstEntityByString( *vTZCMREPO, "RepositoryClient", "Desc",
                                         szWKS_Desc, 0 ) >= zCURSOR_SET )
   {
      zstrcpy( szMsg, "The Workstation Description was not unique.\n" );
      zstrcat( szMsg, "Please enter a new Description." );
      szWKS_Desc[ 0 ] = 0;
      nRC = MessagePromptForInput( vSubtask,   "CM00201",
                                 "Configuration Management",
                                 szMsg, zBEEP, szWKS_Desc, 60 );
...
zwTZCMLPLD_XferToAddUser( zVIEW vSubtask )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      if ( vTZCMWKSO > 0 )
      {
         if ( SetCursorFirstEntityByInteger( vTZCMWKSW, "User", "ZKey",
                                        (zLONG) ulZKey, "" ) != zCURSOR_SET )
         {
            CreateEntity( vTZCMREPW, "User", zPOS_AFTER );
            SetMatchingAttributesByName( vTZCMREPW, "User",
                                         vTZCMREPO, "User", zSET_ALL );
         }
      }
      else
      {
...
zwfnTZCMLPLD_SetNewWorkstationValue( zVIEW vSubtask, zVIEW vTZCMREPO, zPCHAR szWKS_Desc )
{
   ...
   SetMatchingAttributesByName( vTZCMWKSO, "RepositoryClient",
                                vTZCMREPO, "RepositoryClient", zSET_ALL );
   ...
   if ( ActivateEmptyObjectInstance( &vTZCMWIPO, "TZCMWIPO", vSubtask,
                                     zSINGLE ) >= 0 )
   {
...
zwTZCMLPLD_InitDialogoLD( zVIEW vSubtask )
{
   ...
   if ( oTZCMWKSO_ActivateWorkStation( &vTZCMWKSO, vSubtask,
           zSINGLE | zNOI_OKAY | zIGNORE_ATTRIB_ERRORS ) != 0 )
   {
   ...
   if ( SetCursorFirstEntityByAttr( vTZCMWKSO, "LPLR", "ZKey",
                                    vTZCMWKSO, "RepositoryClient", "DefaultLPLR_ZKey",
                                    "" ) != zCURSOR_SET )
   {
...
zwTZCMLPLD_SignOnPreBuild( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCMWIPO, "TZCMWIPO", vSubtask, zLEVEL_TASK ) < 0 )
   {
      ActivateEmptyObjectInstance( &vTZCMWIPO, "TZCMWIPO",
                                   vSubtask, zSINGLE );
      CreateEntity( vTZCMWIPO, "ROOT", zPOS_AFTER );
      SetNameForView( vTZCMWIPO,"TZCMWIPO", vSubtask, zLEVEL_TASK );
      ...
      if ( GetViewByName( &vTZCMWKSO, "TZCMWKSO", vSubtask, zLEVEL_TASK ) >= 0 )
      {
         ...
         if ( SetCursorNextEntity( vTZCMWKSO, "User",
                                   "RepositoryClient" ) < zCURSOR_SET )
         {
            // there is just one User assigned to this workstation
            SetAttributeFromAttribute( vTZCMWIPO, "ROOT", "UserName",
                                       vTZCMWKSO, "User", "Name" );
            SetAttributeFromAttribute( vTZCMWIPO, "ROOT", "UserDesc",
                                       vTZCMWKSO, "User", "Desc" );
         }
      }
   }
...
zwTZCMLPLD_SignOnPostBuild( zVIEW vSubtask )
{
   ...
   // if the Username isn't NULL, then position the Cursor on the
   // password-control
...
zwTZCMLPLD_XferToUpdateUser( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szUserName, vTZCMWIPO, "ROOT", "UserName" );
   ...
   if ( SetCursorFirstEntityByInteger( vTZCMREPO, "User", "ZKey",
                                       (zLONG) ulZKey, "" ) != zCURSOR_SET )
   {
...
zwTZCMLPLD_AddUser( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szName, vTZCMREPW, "User", "Name" );
   ...
   nRC = IncludeSubobjectFromSubobject( vTemp, "User",
                                        vTZCMREPO, "User", zPOS_AFTER );

   if ( SetCursorFirstEntityByAttr( vTZCMREPO, "UserRepositoryClient",
                                    "ZKey", vTZCMREPO, "RepositoryClient",
                                    "ZKey", "" ) >= zCURSOR_SET )
   {
      RelinkInstanceToInstance( vTZCMREPO, "UserRepositoryClient",
                                vTZCMREPO, "RepositoryClient" );
   }
                                 ...
                                 zVIEW vTZCMWIPO, zVIEW vTZCMWKSO )
{
   ...
   if ( CompareAttributeToAttribute( vTZCMWIPO, "ROOT", "Password",
                                     vTZCMWIPO, "ROOT", "PasswordValidation" ) != 0 )
   {
      MessageSend( vSubtask, "CM00211", "Configuration Management",
...
zwfnTZCMLPLD_CheckPasswordZKey( zVIEW vSubtask, zULONG ulZKey, zVIEW vTZCMREPO )
{
   ...
   if ( ulZKey < 1 )
   {
      zstrcpy( szMsg, "This User ID is missing a Zeidon Key.\n" );
      zstrcat( szMsg, "To obtain a valid User ID:\n" );
      zstrcat( szMsg, "   1. Cancel from Update User\n" );
      zstrcat( szMsg, "   2. Delete this User from this Workstation\n" );
      zstrcat( szMsg, "   3. Add a new User" );
   ...
   if ( SetCursorFirstEntityByInteger( vTZCMREPO, "User", "ZKey",
                                       (zLONG) ulZKey, "" ) != zCURSOR_SET )
   {
      zstrcpy( szMsg, "This User ID was not found on the Repository.\n" );
      zstrcat( szMsg, "To obtain a valid User ID:\n" );
      zstrcat( szMsg, "   1. Cancel from Update User\n" );
      zstrcat( szMsg, "   2. Delete this User from this Workstation\n" );
      zstrcat( szMsg, "   3. Add a new User" );
...
zwTZCMLPLD_UpdateUser( zVIEW vSubtask )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMWKSO, "User", "ZKey" );
   GetStringFromAttribute( szPasswordNew, vTZCMWIPO, "ROOT", "Password" );
   GetStringFromAttribute( szPasswordValidation,
                           vTZCMWIPO, "ROOT", "PasswordValidation" );
   GetStringFromAttribute( szName, vTZCMWKSO, "User", "Name" );
   GetStringFromAttribute( szPassword, vTZCMWKSO, "User", "Password" );
   ...
   // 2. The new Password is the same as the old.
   // 3. The new Password and PasswordValidation values do not match.
   if ( zwfnTZCMLPLD_CheckValidPassword( vSubtask, szPasswordNew,
                                         vTZCMWIPO, vTZCMWKSO ) < 0 )
   {
      return( -1 );
   }
...
zwTZCMLPLD_CancelFromNewLPLR( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "CorrespondingCPLR" ) >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szName, vTZCMLPLO, "CorrespondingCPLR", "Name" );
                         ...
                         zPCHAR szZeidonDirectory )
{
   ...
   nZRetCode = SysGetEnvVar( szZeidonDirectory, "ZEIDON",  zMAX_FILENAME_LTH + 1 );
   if ( nZRetCode == -1 )
   {
   ...
   if ( szZeidonDirectory[ nLth ] != '\\' )
   {
      szZeidonDirectory[ nLth ] = '\\';
      szZeidonDirectory[ nLth + 1 ] = 0;
   }

   zstrcat( szZeidonDirectory, "ZEIDON.APP" );

   nZRetCode =
      SfActivateSysOI_FromFile( pvKZAPPLOO, "KZAPPLOO", vSubtask,
                                szZeidonDirectory, zSINGLE );
                       ...
                       zPCHAR szPathName )
{
   ...
   GetStringFromAttribute( szPathName, vKZAPPLOO, "ZEIDON", "ZEIDON_LOC" );
   if ( *szPathName == 0 )
   {
      // There is no path in APP_LOCAL for the actual LPLR
      // We take the path from Zeidon_Tools next
      CreateViewFromViewForTask( &vKZAPPLTemp, vKZAPPLOO, 0 );
      SetCursorFirstEntityByString( vKZAPPLTemp, "APPLICATION",
                                    "APP_NAME", "Zeidon_Tools", "");

      GetStringFromAttribute( szPathName, vKZAPPLTemp,
      ...
      if ( *szPathName == 0 )
      {
         ...
         strcpy( szPathName, "%TEMP%" );
         SetAttributeFromString( vKZAPPLTemp, "APPLICATION",
                                 "APP_LOCAL", szPathName );
      }

      SetAttributeFromString( vKZAPPLOO, "APPLICATION",
                              "APP_LOCAL", szPathName);
...
zwTZCMLPLD_NewLPLR_Init( zVIEW vSubtask )
{
   ...
   if ( zwTZCMLPLD_GetZeidonApp( vSubtask, &vKZAPPLOO, szZeidonDirectory ) < 0 )
      return( -1 );

   GetStringFromAttribute( szName, vTZCMLPLO, "LPLR", "Name" );
   if ( SetCursorFirstEntityByString( vKZAPPLOO, "APPLICATION", "APP_NAME",
                                      szName, "" ) < zCURSOR_SET )
   {
      nRC = CreateEntity( vKZAPPLOO, "APPLICATION", zPOS_LAST );
   }

   SetAttributeFromAttribute( vKZAPPLOO, "APPLICATION", "APP_NAME",
                              vTZCMLPLO, "LPLR", "Name" );
   SetAttributeFromAttribute( vKZAPPLOO, "APPLICATION", "APP_DLL",
                              vTZCMLPLO, "LPLR", "ExecDir" );
   SetAttributeFromAttribute( vKZAPPLOO, "APPLICATION", "APP_ADOBIN",
                              vTZCMLPLO, "LPLR", "ExecDir" );
   SetAttributeFromAttribute( vKZAPPLOO, "APPLICATION", "APP_SHARED",
                              vKZAPPLOO, "ZEIDON", "ZEIDON_SHR" );
   // get WorkDirectory
   GetCtrlText( vSubtask, "edWorkDirectory", szWorkDirectory, zMAX_FILESPEC_LTH );
   if ( zstrcmp( szWorkDirectory, "" ) != 0 )
   {
      SetAttributeFromString( vKZAPPLOO, "APPLICATION", "APP_LOCAL",
                              szWorkDirectory );
   }
   ...
   GetStringFromAttribute( szToFileName, vTZCMLPLO, "LPLR", "ExecDir" );
   zstrcat( szToFileName, "\\ZEIDON.HKY" );
   zgGetZeidonToolsDir( vSubtask, szFromFileName, zAPPL_DIR_OBJECT );
   zstrcat( szFromFileName, "ZEIDON.HKY" );
...
zwTZCMLPLD_SetLPLR_Type( zVIEW vSubtask )
{
   ...
   // triggered this operation, modify the CPLR seclection combo box as
   // follows:
   ...
   GetStringFromAttribute( szLPLR_Type, vLPLR, "LPLR", "LPLR_Type" );
                          ...
                          zPCHAR pchReturnName )
{
   ...
   GetStringFromAttribute( szFileName, vTZCMLPLO, "LPLR", "ExecDir" );
   ...
   hWndList = CreateWindow( "ListBox", NULL, WS_CHILD | LBS_SORT,
                            1, 1, 1, 1, hWnd, (HMENU) 101,
                            hInst, NULL );
                             ...
                             zVIEW vTZCMWKSO )
{
   ...
   GetStringFromAttribute( szFileName, vTZCMLPLO1, "LPLR", "ExecDir" );
   ofnTZCMWKSO_AppendSlash( szFileName );
   GetStringFromAttribute( szLPLR_Name, vTZCMLPLO1, "LPLR", "Name" );
   ...
   if ( hFile < 0 )
   {
      ...
      if ( nRC == zRESPONSE_OK )
      {
         ...
         GetStringFromAttribute( szFileName, vTZCMLPLO1, "LPLR", "ExecDir" );
         ofnTZCMWKSO_AppendSlash( szFileName );
         zstrcat( szFileName, szReturnName );
      }
      else
      {
         return( -1 );
   ...
   nRC = ActivateOI_FromFile( vTZCMLPLO, "TZCMLPLO", vSubtask, szFileName,
...
zwfnTZCMLPLD_CheckXLP_ForRepo( zVIEW vSubtask, zVIEW vTZCMLPLO, zVIEW vTZCMWKSO )
{
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "CorrespondingCPLR" ) >= zCURSOR_SET )
   {
      zstrcpy( szERR_Msg, "This Project is a Repository Project.\n" );
      zstrcat( szERR_Msg, "Do you really want to create an External Project?" );
...
zwfnTZCMLPLD_DeleteAllOldXPL( zVIEW vSubtask, zVIEW vTZCMLPLO1 )
{
   ...
   // Always delete all old XLP files
   GetStringFromAttribute( szFileName1, vTZCMLPLO1, "LPLR", "ExecDir" );
   ofnTZCMWKSO_AppendSlash( szFileName1 );
   zstrcpy( szERR_Msg, "attrib " );
   zstrcat( szERR_Msg, szFileName1 );
...
zwfnTZCMLPLD_ChangeUpdateStatus( zVIEW vSubtask, zVIEW vTZCMLPLO )
{
   ...
   zstrcpy( szMsg, "Do you want the status of all metas in the Project changed " );
                        ...
                        zRESPONSE_NO,  0 ) == zRESPONSE_YES )
   {
      ...
      {
         nREFER = lSOURCE + 2000;
         nRC1 = SetCursorFirstEntityByInteger( vTempR, "W_MetaType", "Type",
                                               nREFER, "" );
         nRC2 = SetCursorFirstEntityByInteger( vTempS, "W_MetaType", "Type",
                                               lSOURCE, "" );
         ...
         if ( nRC1 >= zCURSOR_SET && nRC2 >= zCURSOR_SET )
         {
            ...
            {
               if ( SetCursorFirstEntityByAttr( vTempS, "W_MetaDef", "Name",
                                                vTempR, "W_MetaDef", "Name", 0
                                                            < zCURSOR_SET ) )
               {
                  // The meta doesn't exist as a source so add it.
                  IncludeSubobjectFromSubobject( vTempS, "W_MetaDef",
                                                 vTempR, "W_MetaDef", zPOS_AFTER );
               }
            }
         }
      }
...
zwfnTZCMLPLD_SetNewReportPath( zVIEW vSubtask, zVIEW vTZCMLPLO )
{
   ...
   GetStringFromAttribute( szReportPathName, vTemp, "LPLR", "MetaSrcDir" );
...
zwfnTZCMLPLD_SetNewDirPath( zVIEW vSubtask, zVIEW vTZCMLPLO1, zVIEW vTZCMLPLO )
{
   ...
   SetAttributeFromAttribute( vTZCMLPLO,  "LPLR", "ExecDir",
                              vTZCMLPLO1, "LPLR", "ExecDir" );
   SetAttributeFromAttribute( vTZCMLPLO,  "LPLR", "PgmSrcDir",
                              vTZCMLPLO1, "LPLR", "PgmSrcDir" );
   SetAttributeFromAttribute( vTZCMLPLO,  "LPLR", "MetaSrcDir",
                              vTZCMLPLO1, "LPLR", "MetaSrcDir" );
   SetAttributeFromAttribute( vTZCMLPLO,  "LPLR", "Name",
                              vTZCMLPLO1, "LPLR", "Name" );

   // Also set the TempDesc attribute for the update window.
   SetAttributeFromAttribute( vTZCMLPLO,  "LPLR", "TempDesc",
                              vTZCMLPLO,  "LPLR", "Desc" );
...
zwfnTZCMLPLD_SetCompSpec( zVIEW vTZCMLPLO, zPCHAR pszOldCompSrcDir, zPCHAR pszOldLPLRName)
{
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "Compiler") >= 0)
   {
      ...
      GetStringFromAttribute( szAktDir, vTZCMLPLO, "Compiler", "TargetExecutableDir" );
      sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName,
                              szAktDir, "Compiler", "TargetExecutableDir",
                              "Target Executable Dir", "Create External LPLR - Compiler Spec",
                              254);

      GetStringFromAttribute( szAktDir, vTZCMLPLO, "Compiler", "TargetObjectDir" );
      sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName,
                              szAktDir, "Compiler", "TargetObjectDir", "Target Object Dir",
                              "Create External LPLR - Compiler Spec", 254);

      GetStringFromAttribute( szAktDir, vTZCMLPLO, "Compiler", "MakefileDir" );
      sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName,
                              szAktDir, "Compiler", "MakefileDir", "Makefile Dir",
                              "Create External LPLR - Compiler Spec", 254);

      GetStringFromAttribute( szAktDir, vTZCMLPLO, "Compiler", "EnvironmentDir" );
      sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName,
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "Include") >= 0)
   {
      ...
      {
         GetStringFromAttribute (szAktDir, vTZCMLPLO, "Include", "IncludeDir");
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "Lib") >= 0)
   {
      ...
      while ( RESULT > zCURSOR_UNCHANGED )
      {
         GetStringFromAttribute (szAktDir, vTZCMLPLO, "Lib", "LibDir");
         sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName, szAktDir,
                                 "Lib", "LibDir", "Lib Dir",
                                 "Create External LPLR - Compiler Spec", 254 );
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "ExternalTarget") >= 0)
   {
      ...
      while ( RESULT > zCURSOR_UNCHANGED )
      {
         if ( CheckExistenceOfEntity( vTZCMLPLO, "ExternalObjFile") >= 0)
         {
            ...
            while ( RESULT > zCURSOR_UNCHANGED )
            {
               GetStringFromAttribute (szAktDir, vTZCMLPLO, "ExternalObjFile", "Name");
               sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName, szAktDir,
         ...
         if ( CheckExistenceOfEntity( vTZCMLPLO, "ExternalLibFile") >= 0)
         {
            ...
            while ( RESULT > zCURSOR_UNCHANGED )
            {
               GetStringFromAttribute( szAktDir, vTZCMLPLO, "ExternalLibFile", "Name");
               sRC = SetCompSpecPaths( vTZCMLPLO, pszOldCompSrcDir, pszOldLPLRName, szAktDir,
...
zwTZCMLPLD_ExternalLPLR( zVIEW vSubtask )
{
   ...
   // Determine the file name for the existing LPLR (ie., the XLP), which is:
   // DirectoryName + "/" + FileName + ".XLP"
   if ( zwfnTZCMLPLD_ActivateOldXLP( vSubtask, &vTZCMLPLO, vTZCMLPLO1,
                                     vTZCMWKSO ) < 0 )
   {
      return( -1 );
   }
   ...
   GetStringFromAttribute( szOldCompSrcDir, vTZCMLPLO, "LPLR", "MetaSrcDir" );
   GetStringFromAttribute( szOldLPLRName, vTZCMLPLO, "LPLR", "Name" );
   ...
   SetAttributeFromAttribute( vTZCMLPLO, "LPLR", "ZKey",
                              vTZCMLPLO1, "LPLR", "ZKey" );
   ...
   if ( CheckExistenceOfEntity( vTZCMWKSO, "LPLR" ) == zCURSOR_SET &&
        CompareAttributeToString( vTZCMWKSO, "LPLR", "Name", "" ) == 0 )
   {
      DeleteEntity( vTZCMWKSO, "LPLR", zREPOS_NONE );
   }

   CreateEntity( vTZCMWKSO, "LPLR", zPOS_AFTER );
   SetMatchingAttributesByName( vTZCMWKSO, "LPLR",
                                vTZCMLPLO, "LPLR", zSET_ALL );
                  ...
                  zSHORT nMaxPathLth )
{
   ...
   zstrcpy( szERR_Msg, "Do you want to create " );
   zstrcat( szERR_Msg, pchDirectoryType );
   zstrcat( szERR_Msg, " Directory:\n  " );
   zstrcat( szERR_Msg, pchPath );
   if ( MessagePrompt( vSubtask, "CM00297",
                       "Configuration Management",
                       szERR_Msg,
                                ...
                                zPCHAR szLPLR_Name )
{
   ...
   GetStringFromAttribute( szPgmSrcDir,  vTZCMLPLO, "LPLR", "PgmSrcDir" );
   GetStringFromAttribute( szExecDir,    vTZCMLPLO, "LPLR", "ExecDir" );
   GetStringFromAttribute( szMetaSrcDir, vTZCMLPLO, "LPLR", "MetaSrcDir" );

   nRC = SetCursorFirstEntity( vTZCMWKSO2, "LPLR", 0 );
   while ( nRC >= zCURSOR_SET )
   {
      GetStringFromAttribute( szPgmSrcDir2,  vTZCMWKSO2, "LPLR", "PgmSrcDir" );
      GetStringFromAttribute( szExecDir2,    vTZCMWKSO2, "LPLR", "ExecDir" );
      GetStringFromAttribute( szMetaSrcDir2, vTZCMWKSO2, "LPLR", "MetaSrcDir" );
      ...
      {
         zstrcpy( szMsg, "Duplicate Directory on other Project '" );
         GetStringFromAttribute( szLPLR_Name, vTZCMWKSO2, "LPLR", "Name" );
...
zwfnTZCMLPLD_CheckValidDir( zVIEW vSubtask, zVIEW vTZCMLPLO )
{
   ...
   GetStringFromAttribute( szToFileName, vTZCMLPLO, "LPLR", "ExecDir" );
   nValid = IsValidDirOrFile( vSubtask, szToFileName, "Executable", TRUE,
                              TRUE, sizeof( szToFileName ) );
   if ( nValid == 1 )
   {
      // reset the attribute to the full path
      SetAttributeFromString( vTZCMLPLO, "LPLR", "ExecDir", szToFileName );
      GetStringFromAttribute( szToFileName, vTZCMLPLO,
                              "LPLR", "MetaSrcDir" );
      nValid = IsValidDirOrFile( vSubtask, szToFileName, "Component Source",
                                 TRUE, TRUE, sizeof( szToFileName ) );
      if ( nValid == 1 )
      {
         SetAttributeFromString( vTZCMLPLO, "LPLR", "MetaSrcDir", szToFileName );
         GetStringFromAttribute( szToFileName, vTZCMLPLO, "LPLR", "PgmSrcDir" );
         nValid = IsValidDirOrFile( vSubtask, szToFileName, "VML / C Source",
                                    TRUE, TRUE, sizeof( szToFileName ) );
         if ( nValid == 1 )
         {
            ...
            SysConvertEnvironmentString( szWorkEnv, szToFileName );

            if ( zstrcmp( szWorkEnv, "" ) == 0 )
               zstrcpy( szWorkEnv, szToFileName );

            nValid = IsValidDirOrFile( vSubtask, szWorkEnv, "Temporary Work",
                                       TRUE, TRUE, sizeof( szWorkEnv ) );
            if ( nValid == 1 )
            {
   ...
   if ( nValid != 1 )
   {
      if ( nValid != zRESPONSE_NO )
      {
         ...
         zstrcpy( szERR_Msg, "Could not create directory:\n  " );
         zstrcat( szERR_Msg, szToFileName );
                              ...
                              zPCHAR szLPLR_Name )
{
   ...
   // directory names (PgmSrcDir, ExecDir, and MetaSrcDir) from the new
   // LPLR to all the names in existing LPLRs.
   if ( zwfnTZCMLPLD_CheckDuplicateDir( vSubtask, vTZCMWKSO, vTZCMLPLO,
                                        szLPLR_Name ) < 0 )
   {
      return( -1 );
   }
...
zwTZCMLPLD_CreateNewLPLR( zVIEW vSubtask )
{
   ...
   if ( zwfnTZCMLPLD_CheckProjectDir( vSubtask, vTZCMWKSO, vTZCMLPLO,
                                      szLPLR_Name ) < 0 )
   {
      return( -1 );
   }
                           ...
                           zPCHAR szName )
{
   ...
   zltoa( ulZKey, szZKeyValue );
   SetAttributeFromString( vKZDBHQUA, "QualAttrib", "Value", szZKeyValue );

   nRC = ActivateObjectInstance( vTZCMLPLR, "TZCMLPLR", vSubtask,
   ...
   if ( nRC < 0 )
   {
      zstrcpy( szERR_Msg, "Unable to activate CPLR '" );
      zstrcat( szERR_Msg, szName );
      zstrcat( szERR_Msg, "' from database! " );
...
zwTZCMLPLD_IncludeCPLR( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO, "CorrespondingCPLR" ) >= zCURSOR_SET )
   {
      ...
      if ( zwfnTZCMLPLD_ActivateLPLR( vSubtask, &vTZCMLPLR, lC_ZKey, szName ) >= 0 )
      {
         GetIntegerFromAttribute( &lZKey, vTZCMLPLO, "LPLR", "ZKey" );
         SetCursorFirstEntityByInteger( vTZCMLPLR, "LPLR", "ZKey", lZKey, "" );
         RelinkInstanceToInstance( vTZCMLPLO, "CorrespondingCPLR",
                                   vTZCMLPLR, "CPLR" );
         RelinkInstanceToInstance( vTZCMLPLO, "LPLR",
                                   vTZCMLPLR, "LPLR" );
   ...
   GetIntegerFromAttribute( &lZKey, vTZCMCRWO, "CPLR", "ZKey" );
   GetStringFromAttribute( szName, vTZCMCRWO, "CPLR", "Name" );
   ...
   IncludeSubobjectFromSubobject( vTZCMLPLO, "CorrespondingCPLR",
                                  vTZCMLPLR, "CPLR", zPOS_AFTER );
   SetAttributeFromAttribute( vTZCMLPLO, "CorrespondingCPLR",
                              "DerivedCPL_Description",
                              vTZCMLPLR, "CPL", "Desc" );
...
zwTZCMLPLD_DeleteLPLR( zVIEW vSubtask )
{
   ...
   if ( nRC > 0 )
   {
      ...
      zstrcat( szERR_Msg, szName );
      zstrcat( szERR_Msg, " is currently in use.\n" );
      zstrcat( szERR_Msg, "Unable to delete!" );
   ...
   if ( ActivateOI_FromFile( &vTZCMLPLO, "TZCMLPLO", vSubtask, szFileName,
                             zSINGLE | zNOI_OKAY | zIGNORE_ERRORS ) < 0 )
   {
      DeleteEntity( vTZCMWKSO, "LPLR", zREPOS_NONE );
   }
   else
   {
           ...
           CheckExistenceOfEntity( vTZCMLPLO, "CorrespondingCPLR" ) >= zCURSOR_SET )
      {
         ...
         if ( nRC >= 0 )
         {
            ...
            if ( SetCursorFirstEntityByInteger( vTZCMCPRO, "LPLR_CheckOut", "ZKey",
                                                lLPLR_ZKey, "CPLR" ) >= 0 )
            {
               DropObjectInstance( vTZCMCPRO );
               zstrcpy( szERR_Msg, "Your Project has components currently checked out.\n" );
               zstrcat( szERR_Msg, "You are not allowed to delete the Project." );
         ...
         else
         {
            ...
            // to delete the LPLR anyway. (This might be valid if the repository
            // has been bombed.)
            vTZCMCPRO = 0;   // Set view to zero since the entry could not be accessed properly.
            zstrcpy( szERR_Msg, "The Repository cannot be activated.\n" );
            zstrcat( szERR_Msg, "Do you want to continue with the delete?" );
            if ( MessagePrompt( vSubtask,
                              "CM00227",
                              "Configuration Management",
                              szERR_Msg,
                              zBEEP,         zBUTTONS_YESNO,
      ...
      if ( SetCursorFirstEntity( vTZCMLPLO, "W_MetaDef",
                                 "LPLR" ) >= zCURSOR_SET )
      {
         zstrcpy( szERR_Msg, "Project " );
         zstrcat( szERR_Msg, szName );
         zstrcat( szERR_Msg, " contains Meta definitions.\n" );
         zstrcat( szERR_Msg,
            "Meta files in this Project directories will NOT be deleted.\n" );
         zstrcat( szERR_Msg, "Continue with Delete?" );
      }
      else
      {
         zstrcpy( szERR_Msg, "Continue with delete of Project " );
         zstrcat( szERR_Msg, szName );
         zstrcat( szERR_Msg, "?" );
      }

      if ( MessagePrompt( vSubtask,                 "CM00227",
                          "Configuration Management",
                          szERR_Msg,
                          zBEEP,         zBUTTONS_YESNO,
                          zRESPONSE_NO,  0 ) == zRESPONSE_NO )
      ...
      SendMessage( hWndList, LB_DIR,
                   0x0000, (LONG)(LPSTR) szSearchFileName );
      nCount = ( int )SendMessage( hWndList, LB_GETCOUNT, 0, 0L );
      if ( nCount > 1 )
   ...
   if ( vTZCMLPLO != 0 )
   {
      ...
      {
         GetIntegerFromAttribute( &lZKey, vTZCMLPLO, "LPLR", "ZKey" );
         SetCursorFirstEntityByInteger( vTZCMCPRO, "LPLR",
                                          "ZKey", lZKey,
                                          "" );
...
zwTZCMLPLD_LoadLPLR( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szMsg, "Project File " );
      zstrcat( szMsg, szFileName );
      zstrcat( szMsg, " not found!" );
                                ...
                                zVIEW vTZCMLPLO )
{
   ...
   szAppDirectory[ 0 ] = 0;
   if ( zwTZCMLPLD_GetZeidonApp( vSubtask, &vKZAPPLOO, szAppDirectory ) >= 0 )
   {
      if ( SetCursorFirstEntityByAttr( vKZAPPLOO, "APPLICATION", "APP_NAME",
                                       vTZCMLPLO, "LPLR", "Name",
                                       "" ) >= zCURSOR_SET )
      {
         GetCtrlText( vSubtask, "edWorkDirectory", szNewWorkDirectory, zMAX_FILESPEC_LTH );
         if ( CompareAttributeToString( vKZAPPLOO, "APPLICATION", "APP_LOCAL",
                                        szNewWorkDirectory ) != 0 )
         {
            return( 1 );
         }
      }
...
{
   // Save the LPLR Description (which is in attribute TempDesc) in the
   // LPLR object and also in the TZCMWKSO object.
   ...
   SetAttributeFromAttribute( vTZCMLPLO, "LPLR", "Desc",
                              vTZCMLPLO, "LPLR", "TempDesc" );
   SetAttributeFromAttribute( vTZCMWKSO, "LPLR", "Desc",
                              vTZCMLPLO, "LPLR", "TempDesc" );
   ...
   SysConvertEnvironmentString( szWorkEnv, szWorkDirectory );
   nValid = IsValidDirOrFile( vSubtask, szWorkEnv, "Temporary Work",
                              TRUE, TRUE, sizeof( szWorkEnv ) );
   if ( nValid == 1 )
   {
   ...
   else
   {
      if ( nValid != zRESPONSE_NO )
      {
         ...
         zstrcpy( szERR_Msg, "Could not create Temporary Work Directory:\n  " );
         zstrcat( szERR_Msg, szWorkDirectory );
         MessageSend( vSubtask, "CM00298", "Configuration Management",
                      szERR_Msg,
                      zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      }
...
zwTZCMLPLD_SaveLPLR( zVIEW vSubtask )
{
   ...
   SetCursorFirstEntityByAttr( vTZCMWKSO, "LPLR", "ZKey",
                               vTZCMLPLO, "LPLR", "ZKey", 0 );
   ...
   if ( ( vTZCMWKSO > 0 ) && ( vTZCMLPLO > 0 ) )
   {
      ...
      // by name.
      nRC = IncludeSubobjectFromSubobject( vTZCMWKSO, "LPLR",
                                           vTZCMLPLO, "LPLR", zPOS_AFTER );
      OrderEntityForView( vTZCMWKSO, "LPLR", "Name A" );
...
zwTZCMLPLD_CheckForSave( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCMLPLO, "TZCMLPLO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      // save it.
      nRC = CompareAttributeToAttribute( vTZCMLPLO, "LPLR", "Desc",
                                         vTZCMLPLO, "LPLR", "TempDesc" );

      if ( nRC || zwTZCMLPLD_UpdateWorkDirectory( vSubtask, vTZCMLPLO ) == 1 )
      {
         if ( MessagePrompt( vSubtask, "CM00282",
                             "Configuration Management",
...
zwTZCMLPLD_BuildCPLR_List( zVIEW vSubtask )
{
   ...
   nRC = ActivateObjectInstance( &vTZCMCRWO, "TZCMCRWO", vSubtask,
                                  vKZDBHQUA, zSINGLE | zLEVEL_APPLICATION );
   ...
   if ( nRC >= 0 )
   {
      ...
      {
         zstrcpy( szMsg, "User " );
         GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ], vTZCMWKSO,
                                 "User", "Name" );
         zstrcat( szMsg, " is not authorized\nfor access to any CPLR's." );
   ...
   // Indicate that the LPLR cannot be of type "Repository", for the
   // DisableRepositoryOK function.
...
zwTZCMLPLD_DisableRepositoryOK( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szLPLR_Type, vTZCMWKSO, "LPLR", "LPLR_Type" );
   if ( szLPLR_Type[ 0 ] == '2' )
...
zwTZCMLPLD_BuildCI_List( zVIEW vSubtask )
{
   ...
   if ( ActivateEmptyObjectInstance( &vTZCM_ChkInList, "TZCMLPLO", vSubtask,
                                      zSINGLE ) >= 0 )
   {
      CreateEntity( vTZCM_ChkInList, "LPLR", zPOS_AFTER );
      SetMatchingAttributesByName( vTZCM_ChkInList, "LPLR",
                                   vTZCMLPLO, "LPLR", zSET_ALL );
   ...
   if ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaType",
                             "Type", zSOURCE_SA_META, "" ) == zCURSOR_SET )
   {
      if (( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaDef",
                             "UpdateInd", 2, "" ) == zCURSOR_SET ) ||
          ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaDef",
                             "UpdateInd", 3, "" ) == zCURSOR_SET ))
         lFlag = 1;
   }

   // Is the TE updated, then set all Subject Areas and ERD in CheckIn List
   if ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaType",
                          "Type", zSOURCE_DTE_META, "" ) == zCURSOR_SET )
   {
      if (( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaDef",
                             "UpdateInd", 2, "" ) == zCURSOR_SET ) ||
          ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaDef",
   ...
   if ( SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaType",
                          "Type", zSOURCE_ERD_META, "" ) == zCURSOR_SET )
   {
      if ( (SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaDef",
                                           "UpdateInd", 2,
                                           "" ) == zCURSOR_SET) ||
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      // We want to temporarily eliminate UI_Specs from the list.  To do this
      // and make it easy to remove later, we will just set the nRC to indicate
      // no W_MetaDefs if the Type is a UI_Spec.
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( lStatus == 1 )
         {
            if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType",
                                          "Type", lType, "" ) != zCURSOR_SET )
            {
               CreateEntity( vTZCM_ChkInList, "W_MetaType", zPOS_AFTER );
               SetAttributeFromInteger( vTZCM_ChkInList, "W_MetaType", "Type",
                                                                        lType );
            }
            GetIntegerFromAttribute( &lUpdateInd,
                                     vTZCMLPLO, "W_MetaDef", "UpdateInd" );
                 ...
                 (nChkInERD_Flag == 1 && lType == zSOURCE_ERD_META ))
            {
               ...
               SetMatchingAttributesByName( vTZCM_ChkInList, "W_MetaDef",
                                          vTZCMLPLO, "W_MetaDef", zSET_ALL );
   ...
   if ( SetCursorFirstEntity( vTZCM_ChkInList,
                              "W_MetaDef", "LPLR" ) < zCURSOR_SET )
   {
...
zwTZCMLPLD_FillAuditTrail( zVIEW vSubtask )
{
   ...
   IncludeSubobjectFromSubobject( vTZCMAUDO, "CPLR",
                                  vTZCMCPRO, "CPLR", zPOS_AFTER );

   if ( CheckExistenceOfEntity( vTZCMWKSO, "RepositoryClient" ) >= zCURSOR_SET )
      IncludeSubobjectFromSubobject( vTZCMAUDO, "RepositoryClient",
                                    vTZCMWKSO, "RepositoryClient", zPOS_AFTER );

   if ( CheckExistenceOfEntity( vTZCMAUDO, "RepositoryClient" ) >= zCURSOR_SET )
      SetAttributeFromAttribute( vTZCMAUDO, "AuditTrail", "WKS_Id",
                                 vTZCMAUDO, "RepositoryClient", "WKS_Id" );

   if ( CheckExistenceOfEntity( vTZCMAUDO, "User" ) >= zCURSOR_SET )
   {
      SetAttributeFromAttribute( vTZCMAUDO, "AuditTrail", "UserName",
                                 vTZCMAUDO, "User", "Name" );
      SetAttributeFromAttribute( vTZCMAUDO, "AuditTrail", "UserDesc",
                                 vTZCMAUDO, "User", "Desc" );
   }
   ...
   nRC = ActivateObjectInstance( &vTZCMREVO, "TZCMREVO",
                                 vSubtask, vKZDBHQUA,
                                 zSINGLE | zLEVEL_APPLICATION | zACTIVATE_ROOTONLY );
   DropObjectInstance( vKZDBHQUA );

   if ( nRC >= 0 )
      IncludeSubobjectFromSubobject( vTZCMAUDO, "RevEvent",
...
zwTZCMLPLD_CancelCheckIn( zVIEW vSubtask )
{
   ...
   if ( CompareAttributeToString( vTZCMAUDO, "AuditTrail",
                                  "Desc", "" ) != 0 ||
        CompareAttributeToString( vTZCMAUDO, "AuditTrail",
                                  "ShortDesc", "" ) != 0 )
   {
                                 ...
                                 zVIEW vTZCMCPRO )
{
   ...
   {
      zstrcpy( szMsg, "A component '" );
      zstrcat( szMsg, szName );
      zstrcat( szMsg, "' of type Data Model \nalready exists." );
                                  ...
                                  zSOURCE_DTE_META, szName ) < 0 )
   {
      zstrcpy( szMsg, "A component '" );
      zstrcat( szMsg, szName );
      zstrcat( szMsg, "' of type Technical Environment \nalready exists." );
      zstrcat( szMsg, "\n\nCheck In Processing aborted!" );
      MessageSend( vSubtask, "CM00702", "Configuration Management",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      return( -1 );
                              ...
                              zVIEW  vTZCMCPRO )
{
   ...
   if ( SetCursorFirstEntityByInteger( vTZCML_ChkInList1, "W_MetaType",
                                       "Type", (zSHORT) zSOURCE_PENV_META,
                                       "" ) == zCURSOR_SET &&
        CheckExistenceOfEntity( vTZCML_ChkInList1,
                                "W_MetaDef" ) >= zCURSOR_SET &&
        SetCursorFirstEntityByInteger( vTZCMCPRO, "Z_MetaDef", "Type",
                                       (zSHORT) zSOURCE_PENV_META,
                                       "" ) != zCURSOR_SET )
   {
      ...
      SetAttributeFromString( vKZDBHQUA, "QualAttrib", "Oper", "=" );
      nRC = ActivateObjectInstance( &vTZCMCPRO_PPE, "TZCMCPRO", vSubtask,
                                    vKZDBHQUA, zSINGLE );
      ...
      if ( nRC >= 0 )
      {
         GetStringFromAttribute( szCPLRName, vTZCMCPRO_PPE, "CPLR", "Name" );
         zstrcpy( szMsg, "A PPE already exists in the CPLR \n" );
         zstrcat( szMsg, szCPLRName );
         zstrcat( szMsg, ".\nCheck In Processing aborted!" );
         MessageSend( vSubtask, "CM00702", "Configuration Management",
                      szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
                               ...
                               zPCHAR szName )
{
   ...
   GetStringFromAttribute( szName, vChkInList, "W_MetaDef", "Name" );

   //exists a component with this name, then check the Key
   if ( SetCursorFirstEntityByString( vChkInList_Copy, "W_MetaDef", "Name",
                                      szName, "LPLR" ) >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKeyLPLR1, vChkInList,
                               "W_MetaDef", "CPLR_ZKey" );
      do
      {
         GetIntegerFromAttribute( (zPLONG) &ulZKeyLPLR2, vChkInList_Copy,
                                  "W_MetaDef", "CPLR_ZKey" );
         // is a other component, check the type
         if ( ulZKeyLPLR1 != ulZKeyLPLR2 )
         {
            GetIntegerFromAttribute( &lTypeLPLR1, vChkInList,
                                     "W_MetaType", "Type" );
            GetIntegerFromAttribute( &lTypeLPLR2, vChkInList_Copy,
                                     "W_MetaType", "Type" );
            if ( lTypeLPLR1 >= 2000 )
            ...
            if (( lTypeLPLR2 == zSOURCE_DIALOG_META &&
                  lTypeLPLR1 == zSOURCE_LOD_META )  ||
                ( lTypeLPLR2 == zSOURCE_LOD_META &&
                  lTypeLPLR1 == zSOURCE_DIALOG_META ) )
               nRC = -2;

            if ( nRC < 0 )
            {
               GetVariableFromAttribute( szTypeLPLR1, 0, zTYPE_STRING, 125,
                                         vChkInList, "W_MetaType",
                                         "Type", "CM_Type", 0 );
               GetVariableFromAttribute( szTypeLPLR2, 0, zTYPE_STRING, 125,
                                         vChkInList_Copy, "W_MetaType",
                                         "Type", "CM_Type", 0 );
      ...
      while ( SetCursorNextEntityByString( vChkInList_Copy, "W_MetaDef", "Name",
                                      szName, "LPLR" ) >= zCURSOR_SET );
                               ...
                               zVIEW vTZCMCPRO )
{
   ...
   nRC = CheckAllDuplicateName( vTZCML_ChkInList, vTZCMCPRO,
                                szTypeCPLR, szTypeLPLR, szName );
   ...
   if ( nRC >= 0 )
   {
      ...
      {
         nRC = zwfnTZCMLPLD_CheckCheckInList( vChkInList, szTypeCPLR,
                                              szTypeLPLR, szName );
         if ( nRC < 0 )
            break;
   ...
   if ( nRC < 0 )
   {
      if ( nRC == -2 )
      {
         zstrcpy( szMsg, "You are trying to add a component '" );
         zstrcat( szMsg, szName );
         zstrcat( szMsg, "' of type " );
         zstrcat( szMsg, szTypeLPLR );
         zstrcat( szMsg, ",\nbut a component '" );
         zstrcat( szMsg, szName );
         zstrcat( szMsg, "' of type " );
         zstrcat( szMsg, szTypeCPLR );
         zstrcat( szMsg, " already exists in CPLR." );
      }
      else
      {
         zstrcpy( szMsg, "You are trying to add a component '" );
   ...
   if ( zwTZCMLPLD_CheckDuplicateERD_TE( vSubtask, vTZCML_ChkInList,
                                         vTZCMCPRO ) < 0 )
   {
      return( -1 );
   }
...
zwTZCMLPLD_CheckInReus( zVIEW  vSubtask )
{
   ...
   GetViewByName( &vTZCMWKSO, "TZCMWKSO", vSubtask, zLEVEL_TASK );
   GetViewByName( &vTZCMLPLO, "TZCMLPLO", vSubtask, zLEVEL_TASK );
   GetIntegerFromAttribute( (zPLONG)&ulZKey, vTZCMWKSO, "RepositoryClient",
                                                       "DefaultLPLR_ZKey" );
   if ( CompareAttributeToInteger( vTZCMLPLO, "LPLR", "ZKey",
                               (zLONG) ulZKey ) != 0 )
   {
      zwfnTZCMLPLD_SwitchLPLR( vSubtask, vTZCMLPLO );
      DropObjectInstance( vTZCMLPLO );
   ...
   else
   {
      ...
      zstrcat( szMsg, "' not found on Installation! " );
   ...
   if ( zwTZCMLPLD_CheckDuplicateName( vSubtask, vTZCML_ChkInList,
                                       vTZCMCPRO ) != 0 )
   {
      SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
      return( -1 );
   ...
   while ( nRC > 0 )
   {
      ...
      if ( iDropInd == 1 )
         nRC = SfGetFirstNamedView( &vMeta, szTaskView, vZeidonCM,
                                    zLEVEL_SUBTASK );
      else
         nRC = SfGetNextNamedView( &vMeta, szTaskView, vZeidonCM,
                                   zLEVEL_SUBTASK );
   }
         ...
         zQUAL_SCOPE_OI | zPOS_FIRST, 0, 0, 0, 0, 0, 0 ) >= zCURSOR_SET )
   {
      ...
      zstrcpy( szMsg, "Metas have been refreshed. You may now choose" );
      zstrcat( szMsg, "\nto continue the Check In process( OK ), or " );
      zstrcat( szMsg, "\nto Cancel the Check In process( Cancel ) and" );
      zstrcat( szMsg, "\nreview the Refresh Report." );
      zstrcat( szMsg, "\n\n         Continue check in?" );
      nRC = MessagePrompt( vSubtask,                 "CM00237",
                           "Configuration Management",
                           szMsg,
                           zBEEP,         zBUTTONS_OKCANCEL,
                           zRESPONSE_OK,  0 );
      ...
      // Test whether the CPLR ist not enqueued and enqueue it.
      if (oTZCMCPRO_CPLR_Enqueue( vTZCMCPRO, vSubtask,
                                  zCPLR_ENQUEUE_FOR_CHECK_IN ) < 0 )
      {
         return( -1 );
      }
   }
   ...
   SetAttributeFromAttribute( vTZCMLPLO, "LPLR", "LastRefreshDate",
                              vTZCMCPRO, "CPLR", "LastCheckInDate" );
   nRC = CommitObjectInstance( vTZCMCPRO );
...
zwTZCMLPLD_CheckInMeta( zVIEW vSubtask )
{
   ...
   {
      if ( SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef",
                                         "LPLR" ) < zCURSOR_SET )
      {
         zstrcpy( szERR_Msg, "Please select at least one Component for Check In" );
         MessageSend( vSubtask, "CM00241", "Configuration Management",
   ...
   while ( ( nRC >= zCURSOR_SET ) && ( !DependentNeeded ) )
   {
      ...
      SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaDef",
                                     "CPLR_ZKey", lZKey,
                                     "LPLR" );
   ...
   if ( DependentNeeded )
   {
      ...
      zstrcpy( szERR_Msg, "Meta type(s) subordinate to the selected\n" );
      zstrcat( szERR_Msg, "Metas are checked out and must be checked\n" );
      zstrcat( szERR_Msg, "in with this meta!" );
      MessageSend( vSubtask, "CM00242", "Configuration Management",
                   szERR_Msg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
   }
   else
                            ...
                            zLONG  lType )
{
   ...
   nRC = SetCursorFirstEntityByInteger( vTZCM_ChkInList2, "W_MetaType",
                                        "Type", lType, 0 );
   if ( nRC < zCURSOR_SET )
   {
   ...
   nRC = SetCursorFirstEntityByAttr( vTZCM_ChkInList2, "W_MetaDef", "CPLR_ZKey",
                                     vTZCM_ChkInList,  "W_MetaDef", "CPLR_ZKey", 0 );
   if ( nRC < zCURSOR_SET )
   {
      CreateEntity( vTZCM_ChkInList2, "W_MetaDef", zPOS_AFTER );
      SetMatchingAttributesByName( vTZCM_ChkInList2, "W_MetaDef",
                                   vTZCM_ChkInList,  "W_MetaDef", zSET_ALL );
                              ...
                              zLONG  lType )
{
   ...
   if ( SetCursorFirstEntityByInteger( vListLOD, "W_MetaType", "Type",
   ...
   if ( SetCursorFirstEntityByInteger( vListVOR, "W_MetaType", "Type",
                                       zSOURCE_VOR_META, 0 ) < zCURSOR_SET )
   {
      vListVOR = 0;
   }
         ...
         nRC = SetCursorNextSelectedEntity( vListDialog, "W_MetaDef", 0 ) )
   {
      ...
      if ( ActivateMetaOI( vSubtask, &vDialog, vListDialog,
                           (zSHORT) lType, zLEVEL_TASK ) < 1 )
      {
      ...
      else
      {
               ...
               nRC = SetCursorNextEntity( vDialog, "ViewObjRef", 0 ) )
         {
            if ( vListVOR > 0 )
            {
               nRC = SetCursorFirstEntityByAttr( vListVOR, "W_MetaDef",  "CPLR_ZKey",
                                                 vDialog,  "ViewObjRef", "ZKey", 0 );
               if ( nRC >= zCURSOR_SET )
               {
                  // First copy over the entry from List to List2.
                  zwfnTZCMLPLD_CopyMetaEntry( vListVOR,
                                              vTZCM_ChkInList2,
                                              zSOURCE_VOR_META );
                  ...
                  if ( GetSelectStateOfEntityForSet( vListVOR, "W_MetaDef", 1 ) == 0 )
                  {
                     ...
                     GetStringFromAttribute( szMsg, vListVOR, "W_MetaDef", "Name" );
                     zstrcat( szMsg, " is a dependent Named View for Dialog, " );
                     GetStringFromAttribute( szDialogName,
                                             vDialog, "Dialog", "Tag" );
                     zstrcat( szMsg, szDialogName );
                     zstrcat( szMsg, ", and must be checked in with that Dialog." );
            ...
            if ( vListLOD > 0 )
            {
               nRC = SetCursorFirstEntityByAttr( vListLOD, "W_MetaDef", "CPLR_ZKey",
                                                 vDialog,  "LOD",       "ZKey", 0 );
               if ( nRC >= zCURSOR_SET )
               {
                  // First copy over the entry from List to List2.
                  nRC = zwfnTZCMLPLD_CopyMetaEntry( vListLOD,
                                                    vTZCM_ChkInList2,
                                                    zSOURCE_LOD_META );

                  // Then give an error message if that entry did not already
                  // exist (it had not yet been processed) and it is not
                  // selected.
                  if ( nRC == 0 &&
                       GetSelectStateOfEntityForSet( vListLOD, "W_MetaDef", 1 ) == 0 )
                  {
                     // The entry wasn't selected.
                     GetStringFromAttribute( szMsg, vListLOD, "W_MetaDef", "Name" );
                     zstrcat( szMsg, " is a dependent LOD for Dialog, " );
                     GetStringFromAttribute( szDialogName,
                                             vDialog, "Dialog", "Tag" );
                     zstrcat( szMsg, szDialogName );
                     zstrcat( szMsg, ", and must be checked in with that Dialog." );
...
zwfnTZCMLPLD_CheckInEdits( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCM_ChkInList, "TZCM_ChkInList", vSubtask, zLEVEL_TASK ) < 0 )
      return( -1 );
   if ( ActivateEmptyObjectInstance( &vTZCM_ChkInList2, "TZCMLPLO", vSubtask,
                                      zSINGLE ) >= 0 )
   {
      CreateEntity( vTZCM_ChkInList2, "LPLR", zPOS_AFTER );
      SetMatchingAttributesByName( vTZCM_ChkInList2, "LPLR",
                                   vTZCM_ChkInList, "LPLR", zSET_ALL );
      SetNameForView( vTZCM_ChkInList2, "TZCM_ChkInList2", vSubtask, zLEVEL_TASK );
   }

   /* Headers */
   /* Copy all Active Headers */
   zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                  zSOURCE_HDR_META );

   /* Global Operation Groups */
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList,
                                       "W_MetaType",
                                       "Type",
                                       zSOURCE_GOPGRP_META,
                                       "" )                 == zCURSOR_SET )
   {
      if ( SetCursorFirstSelectedEntity( vTZCM_ChkInList,
                                         "W_MetaDef", "" ) == zCURSOR_SET )
      {
         GO_Selected = 1;
         ...
         if ( HDRsCopied == 0 )
         {
            zwfnTZCMLPLD_GetAllActiveMetas( vTZCM_ChkInList,
                                            vTZCM_ChkInList2,
                                            zSOURCE_HDR_META );
            HDRsCopied = 1;
         }
                                       ...
                                       "" )                 == zCURSOR_SET )
   {
      ...
      {
         zwfnTZCMLPLD_GetAllActiveMetas( vTZCM_ChkInList,
                                         vTZCM_ChkInList2, zSOURCE_HDR_META );
         HDRsCopied = 1;
         zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                        zSOURCE_DOMAINGRP_META );
         DomainSelected = 1;
   ...
   /* ERD */
   if ( ((SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType",
                                         "Type",(zLONG) zSOURCE_ERD_META,
                                         "" ) == zCURSOR_SET) &&
         (SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef",
                                        "" ) == zCURSOR_SET)) ||
        ((SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType",
                                         "Type",(zLONG) zSOURCE_SA_META,
                                         "" ) == zCURSOR_SET) &&
         (SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef",
                                        "" ) == zCURSOR_SET)) ||
        ((SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType",
                                         "Type",(zLONG) zSOURCE_DTE_META,
                                         "" ) == zCURSOR_SET) &&
         (SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef",
                                        "" ) == zCURSOR_SET )) )
   {
      ...
      if ( CheckExistenceOfEntity( vTZCM_ChkInList, "W_MetaDef" ) >= zCURSOR_SET )
      {
         ...
         CreateEntity( vTZCM_ChkInList2, "W_MetaType", zPOS_AFTER );
         SetAttributeFromInteger( vTZCM_ChkInList2, "W_MetaType",
                                  "Type",(zLONG) zSOURCE_ERD_META );
         ...
         ER_Copied = 1;
         /* Copy all Active SAs */
         zwfnTZCMLPLD_GetAllActiveMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
   ...
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
                             (zLONG ) zSOURCE_LOD_META, "" ) == zCURSOR_SET )
   {
      nRC = SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef", "" );
      if ( nRC == zCURSOR_SET )
      {
         ...
         if ( ER_Copied == 0 )
         {
            ...
            nRC = SetCursorFirstEntityByInteger( vTemp, "W_MetaType",
                                                 "Type",
         ...
         zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                        (zLONG ) zSOURCE_LOD_META );
      }
   }

   /* VOR */
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
                             (zLONG ) zSOURCE_VOR_META, "" ) == zCURSOR_SET )
   {
      if ( SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef",
                                         "" ) == zCURSOR_SET )
      {
         ...
         if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType",
                   "Type",(zLONG ) zSOURCE_LOD_META, "" ) == zCURSOR_SET )
         {
            if ( SetCursorFirstEntity( vTZCM_ChkInList,
                                       "W_MetaDef", "" ) == zCURSOR_SET )
            {
               ...
               if ( ER_Copied == 0 )
               {
                  zwfnTZCMLPLD_BuildER_List( vTZCM_ChkInList,
                                             vTZCM_ChkInList2 );
                  ER_Copied = 1;
               }
               zwfnTZCMLPLD_GetAllActiveMetas( vTZCM_ChkInList,
                                     vTZCM_ChkInList2, zSOURCE_LOD_META );
               LODsCopied = 1;
            }
         }
         zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                          (zLONG) zSOURCE_VOR_META );
      }
   }

   /* PENVs */
   zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                    (zLONG ) zSOURCE_PENV_META );

   /* DIALOGS */
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
               (zLONG ) zSOURCE_DIALOG_META, "" ) == zCURSOR_SET )
   {
      nRC = SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef", "" );
      if ( nRC == zCURSOR_SET )
      {
         ...
         SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
                                        zSOURCE_DIALOG_META, 0 );
         zwfnTZCMLPLD_CheckDialogDeps( vSubtask, vTZCM_ChkInList,
                                       vTZCM_ChkInList2,
                                       zSOURCE_DIALOG_META );
         ...
         zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                        (zLONG ) zSOURCE_DIALOG_META );
      }
   }

   /* REPORTS */
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
                                       (zLONG) zSOURCE_REPORT_META, "" ) == zCURSOR_SET )
   {
      nRC = SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef", "" );
      if ( nRC == zCURSOR_SET )
      {
         ...
         zwfnTZCMLPLD_GetSelectedMetas( vTZCM_ChkInList, vTZCM_ChkInList2,
                                        (zLONG ) zSOURCE_REPORT_META );
      }
   }

   /* XSLT */
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
               (zLONG) zSOURCE_XSLT_META, "" ) == zCURSOR_SET )
   {
      nRC = SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef", "" );
      if ( nRC == zCURSOR_SET )
      {
                               ...
                               zVIEW vTZCM_ChkInList2, zLONG lType )
{
   ...
   SetCursorFirstEntityByInteger( vTZCMLPLO2, "W_MetaType", "Type",
                                  zSOURCE_SRC_META, "" );
   SetViewFromView( vChkInListCmp, vTZCM_ChkInList );
   RetrieveViewForMetaList( vSubtask, &vMetaList, (zSHORT) lType );
   SetCursorFirstEntityByInteger( vChkInListCmp, "W_MetaType",
                                  "Type", lType, "" );
   nRC = SetCursorFirstSelectedEntity( vTZCM_ChkInList, "W_MetaDef", "" );
   while ( nRC == zCURSOR_SET )
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCM_ChkInList,
                               "W_MetaDef", "CPLR_ZKey" );
      SetCursorFirstEntityByInteger( vMetaList, "W_MetaDef",
                                     "CPLR_ZKey", ulZKey, "" );
      if ( ActivateMetaOI( vSubtask, &vMeta, vMetaList,
                           (zSHORT) lType, zLEVEL_TASK ) < 1 )
      {
//PETTIT add error code here later
      ...
      if ( SetCursorFirstEntity( vMeta, "SourceFile",
                        SRC_CMOD[ Type ].szOD_ROOT ) == zCURSOR_SET )
      {
         GetIntegerFromAttribute( (zPLONG) &ulZKey, vMeta, "SourceFile",
                                    "ZKey" );
         SetCursorFirstEntityByInteger( vTZCMLPLO2, "W_MetaDef",
                                          "CPLR_ZKey", ulZKey, "" );
         ...
         while ( nRC == zCURSOR_SET )
         {
            ...
            {  // Not Selected
               GetIntegerFromAttribute( (zPLONG) &ulZKey, vChkInListCmp,
                                       "W_MetaDef", "CPLR_ZKey" );
               SetCursorFirstEntityByInteger( vMetaList, "W_MetaDef",
                                             "CPLR_ZKey", ulZKey, "" );
               if ( ActivateMetaOI( vSubtask, &vMetaCmp, vMetaList,
                                    (zSHORT) Type, zLEVEL_TASK ) < 1 )
               {
   //PETTIT add error code here later
                  break;
               }
               if ( SetCursorFirstEntity( vMetaCmp, "SourceFile",
                           SRC_CMOD[ Type ].szOD_ROOT ) == zCURSOR_SET )
               {
                  if ( CompareAttributeToAttribute( vMeta, "SourceFile",
                              "ZKey", vMetaCmp, "SourceFile", "ZKey" ) == 0 )
                  {  // Mark it in ChkInList2
                     if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList2,
                           "W_MetaType", "Type", Type, "" ) != zCURSOR_SET )
                     {
                        CreateEntity( vTZCM_ChkInList2, "W_MetaType",
                                      zPOS_AFTER );
                        SetAttributeFromInteger( vTZCM_ChkInList2,
                                     "W_MetaType", "Type", Type );
                     }
                     CreateEntity( vTZCM_ChkInList2, "W_MetaDef", zPOS_AFTER );
                     SetMatchingAttributesByName( vTZCM_ChkInList2,
                        "W_MetaDef", vChkInListCmp, "W_MetaDef", zSET_ALL );
                  }
               }
...
zwfnTZCMLPLD_BuildER_List( zVIEW vTZCM_ChkInList, zVIEW vTZCM_ChkInList2 )
{
   /* Copy Active ER */
   if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList, "W_MetaType", "Type",
                             (zLONG ) zSOURCE_ERD_META, "" ) == zCURSOR_SET )
   {
      if ( SetCursorFirstEntityByInteger( vTZCM_ChkInList2, "W_MetaType",
                                          "Type",(zLONG ) zSOURCE_ERD_META,
                                          "" ) != zCURSOR_SET )
      {
         CreateEntity( vTZCM_ChkInList2, "W_MetaType", zPOS_AFTER );
         SetAttributeFromInteger( vTZCM_ChkInList2, "W_MetaType", "Type",
                                  (zLONG ) zSOURCE_ERD_META );
         CreateEntity( vTZCM_ChkInList2, "W_MetaDef", zPOS_AFTER );
         SetMatchingAttributesByName( vTZCM_ChkInList2, "W_MetaDef",
                                      vTZCM_ChkInList, "W_MetaDef", zSET_ALL );
                            ...
                            zPVIEW vTZCMCPRO, zVIEW vTZCMCOML )
{
   ...
   GetStringFromAttribute( szName, vTZCMCOML, "CPLR", "Name" );
   GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMCOML, "CPLR", "ZKey" );

   if ( oTZCMCPRO_ActivateCPLR( vTZCMCPRO, (zLONG) ulZKey, vSubtask ) < 0 )
   {
      zstrcpy( szERR_Msg, "CPLR '" );
      zstrcat( szERR_Msg, szName );
      zstrcat( szERR_Msg, "' not found on Installation!" );
...
zwTZCMLPLD_BuildCO_List( zVIEW vSubtask )
{
   ...
   if ( CompareAttributeToInteger( vTZCMWIPO, "ROOT", "ListDomain", 1 ) != 0 )
      zwfnTZCMLPLD_DeleteChkOutType( vSubtask, vTZCMCOML,
                                     (zLONG) zSOURCE_DOMAINGRP_META );
...
zwfnTZCMLPLD_DeleteChkOutType( zVIEW vSubtask, zVIEW vTZCMCOML, zLONG Type )
{
   ...
   nRC = SetCursorFirstEntityByInteger( vTZCMCOML, "Z_MetaDef", "Type",
                                        Type, "" );
   if ( nRC != zCURSOR_SET )
      return( -1 );
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      nRC = SetCursorNextEntityByInteger( vTZCMCOML, "Z_MetaDef",
                                          "Type", Type, "" );
   }
...
zwfnTZCMLPLD_InitCheckOutAbort( zVIEW vSubtask, zVIEW vTZCMCOML, zPCHAR szUserEntity )
{
   ...
   {
      zstrcpy( szMsg, "Project component '" );
      GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ],
                              vTZCMCOML, "Z_MetaDef", "Name" );
      zstrcat( szMsg, "' is currently being checked out.\nAborting Check Out." );
   ...
   else
   {
      ...
      GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ],
                              vTZCMCOML, "Z_MetaDef", "Name" );
      zstrcat( szMsg, "' is checked out by '" );
      GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ],
                              vTZCMCOML, "User_1", "Name" );
      zstrcat( szMsg, "'. \nAborting Check Out!" );
...
zwTZCMLPLD_CheckOutSnglClk( zVIEW vSubtask )
{
   ...
   {
      if ( CheckExistenceOfEntity( vTZCMCOML_Copy, "UserMetaStatus" )
                                   >= zCURSOR_SET )
      {
         lEnableCheckOut = 0;
         break;
...
zwTZCMLPLD_fnCheckOutSourceAbort( zVIEW vSubtask, zVIEW vTZCMCOML, zVIEW vTZCMCPRO )
{
   ...
   {
      zstrcpy( szMsg, "Commit of CPLR, " );
      GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ],
                              vTZCMCPRO, "CPLR", "Name" );
      zstrcat( szMsg, " failed! " );
...
zwfnTZCMLPLD_CheckOutRefresh( zVIEW vSubtask, zVIEW vTZCMLPLO )
{
   ...
   if ( zwfnTZCMLPLD_ActivateREFO( vSubtask, vTZCMLPLO, &vTZCMREFO, szName,
                                   zRefreshLPLR ) < 0 )
   {
      return( -1 );
   }
   ...
   GetViewByName( &vTZCMRPTO, "TZCMRPTO", vSubtask, zLEVEL_TASK );
   nRC = SetEntityCursor( vTZCMRPTO, "Z_MetaDef", 0,
   ...
   if ( nRC >= zCURSOR_SET )
   {
      ...
      {
         DropObjectInstance( vTZCMREFO );
         SetWindowActionBehavior( vSubtask, zWAB_StartModalWindow |
                                            zWAB_ProcessImmediateAction,
                                  "TZCMRPTD", "RefreshList" );
         return( -1 );
      }
   }
                                ...
                                zVIEW vTZCMCPRO, zLONG Type )
{
   ...
   {
      zstrcpy( szMsg, "Checking Out the Data Model will Check Out all\n" );
      zstrcat( szMsg, "Subject Areas and TE with it!" );
   }

   if ( Type == (zLONG) zSOURCE_SA_META )
   {
      zstrcpy( szMsg, "Checking Out a Subject Area will force\na Check Out of " );
      zstrcat( szMsg, "the ER Model and all Subject Areas\n"
                      "and TE with it!" );
   }

   if ( Type == (zLONG) zSOURCE_DTE_META )
   {
      zstrcpy( szMsg, "Checking Out the TE will Check Out all\n" );
      zstrcat( szMsg, "Subject Area and Data Model with it!" );
   }
                                ...
                                zLONG lType )
{
   ...
   if ( SetCursorFirstEntityByInteger( vTZCMCPRO, "Z_MetaDef", "Type",
                                       lType, "" ) == zCURSOR_SET )
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMCPRO, "Z_MetaDef", "ZKey" );

      if ( zwfnTZCMLPLD_CheckOutMeta( vSubtask, vTZCMREPO, vTemp, vTZCMCPRO,
                                      vTZCMLPLO, lType, ulZKey ) != 0 )
      {
         zwTZCMLPLD_fnCheckOutSourceAbort( vSubtask, vTZCMCOML, vTZCMCPRO );
         oTZCMCPRO_CPLR_Enqueue( vTZCMCPRO, vSubtask, zCPLR_DEQUEUE );
   ...
   SetAttributeFromAttribute( vTemp, "W_MetaDef", "CheckOutDate",
                              vTZCMCPRO, "UserMetaStatus", "CheckOutDate" );
                                ...
                                zVIEW vTZCMCPRO, zVIEW vTZCMCOML, zVIEW vTZCMLPLO )
{
   ...
   // ERD
   if ( zwfnTZCMLPLD_CheckOutERD_or_TE( vSubtask, vTZCMREPO, vTemp, vTZCMCPRO,
                                        vTZCMCOML, vTZCMLPLO,
                                        (zLONG) zSOURCE_ERD_META ) < 0 )
   {
      return( -1 );
   }

   // Subject Area
   nRC = SetCursorFirstEntityByInteger( vTZCMCPRO, "Z_MetaDef", "Type",
                                        (zLONG) zSOURCE_SA_META, "" );
   while ( nRC == zCURSOR_SET )
   {
      GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMCPRO,
                               "Z_MetaDef", "ZKey" );

      if ( zwfnTZCMLPLD_CheckOutMeta( vSubtask, vTZCMREPO, vTemp, vTZCMCPRO,
                                      vTZCMLPLO,(zLONG) zSOURCE_SA_META,
                                      ulZKey ) != 0 )
      {
         zwTZCMLPLD_fnCheckOutSourceAbort( vSubtask, vTZCMCOML, vTZCMCPRO );
         oTZCMCPRO_CPLR_Enqueue( vTZCMCPRO, vSubtask, zCPLR_DEQUEUE );
      ...
      nRC = SetCursorNextEntityByInteger( vTZCMCPRO, "Z_MetaDef", "Type",
                                          (zLONG) zSOURCE_SA_META, "" );
   }
                              ...
                              zVIEW vTZCMCOML, zVIEW vTZCMCPRO )
{
   ...
   zstrcpy( szMsg, "Checking out Component: " );
   GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ],
                           vTZCMCPRO, "Z_MetaDef", "Name" );
   ...
   if ( ( Type == (zLONG) zSOURCE_ERD_META ) ||
        ...
        ( Type == (zLONG) zSOURCE_SA_META ) )
   {
      if ( zwfnTZCMLPLD_CheckOutAskForERD( vSubtask, vTZCMCOML,
                                           vTZCMCPRO, Type ) < 0 )
      {
         return( -1 );
      }
   ...
   if ( ( Type == (zLONG) zSOURCE_ERD_META ) ||
        ...
        ( Type == (zLONG) zSOURCE_SA_META ) )
   {
      if ( zwfnTZCMLPLD_CheckOutERD_SA_TE( vSubtask, vTZCMREPO, vTemp, vTZCMCPRO,
                                           vTZCMCOML, vTZCMLPLO ) < 0 )
      {
         DropView( vTemp );
         return( -1 );
      }
   ...
   {
      if ( zwfnTZCMLPLD_CheckOutOtherMeta( vSubtask, vTZCMREPO, vTemp, vTZCMCPRO,
                                           vTZCMCOML, vTZCMLPLO, Type,
                                           ulZKey ) < 0 )
      {
         DropView( vTemp );
         return( -1 );
   ...
   GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMCOML, "Z_MetaDef", "ZKey" );
   SetCursorFirstEntityByInteger( vTemp, "W_MetaDef", "CPLR_ZKey",
                                  ulZKey, "LPLR" );
   SetAttributeFromAttribute( vTemp,     "W_MetaDef", "CheckOutDate",
                              vTZCMCPRO, "UserMetaStatus", "CheckOutDate" );
   DropView( vTemp );
...
///////////////////////////////////////////////////////////////////////
   ...
   if ( zwfnTZCMLPLD_LoadRepo_CPRO( vSubtask, &vTZCMREPO, &vTZCMCPRO,
                                    vTZCMCOML ) < 0 )
   {
      return( -1 );
   }

   // Loop through each selected MetaDef and check in the corresponding
   // UserMetaStatus entity in the CPLR.
   // Then commit the CPLR.
   nRC = SetCursorFirstSelectedEntity( vTZCMCOML, "Z_MetaDef", 0 );
   while ( nRC >= zCURSOR_SET )
   {
      if ( zwfnTZCMLPLD_CheckOutProcess( vSubtask, vTZCMLPLO,
                                         vTZCMCOML, vTZCMCPRO ) < 0 )
      {
         return( -1 );
      }
                                  ...
                                  zVIEW vTZCMWKSO, zULONG ulZKey2 )
{
   ...
   zstrcpy( szMsg, "Your RepositoryClient was not found on the\n " );
   zstrcat( szMsg, "Installation. This is an ERROR situation.\n" );
   zstrcat( szMsg, "However, I'm willing to add your RepoClient.\n" );
   zstrcat( szMsg, "to the Installation if you choose correctly." );
   ...
   // if the RepositoryClient not in Entity UserRepositoryClient, then this
   // RepositoryClient does not exists in database, create this
   nRC = SetCursorFirstEntityByInteger( vTZCMREPO, "UserRepositoryClient",
                                        "ZKey",(zLONG) ulZKey2, "Installation" );
   if ( nRC != zCURSOR_SET )
   {
      // check the workstation-ID ( NextWKS_Id is greater than WKS_ID )
      if ( CompareAttributeToAttribute( vTZCMREPO, "Installation", "NextWKS_Id",
                                        vTZCMWKSO, "RepositoryClient", "WKS_Id" )
                                        == 1 )
      {
         CreateEntity( vTZCMREPO, "RepositoryClient", zPOS_AFTER );
         SetMatchingAttributesByName( vTZCMREPO, "RepositoryClient",
                                      vTZCMWKSO, "RepositoryClient", zSET_ALL );
      }
      else
      {
          zstrcpy( szMsg, "Your RepositoryClient cannot be added to \n" );
          zstrcat( szMsg, "the Repository for the following reasons:\n\n" );
          zstrcat( szMsg, "Your Workstation was not set up with the current\n" );
          zstrcat( szMsg, "Repository, so that adding the workstation might\n" );
          zstrcat( szMsg, "destroy the Repository by generating invalid zKeys.\n" );
          zstrcat( szMsg, "Sorry, you have to delete your POR-File and setup \n" );
          zstrcat( szMsg, "a new workstation.\n\n" );
          zstrcat( szMsg, "Check Out aborted!" );
   ...
   {
      zstrcpy( szMsg, "This is an error situation. \n\n" );
      zstrcat( szMsg, "Your RepositoryClient cannot be added to the Repository\n" );
      zstrcat( szMsg, "because a workstation with the current ID already exists,\n" );
      zstrcat( szMsg, "but not for the current installation. \n\n" );
      zstrcat( szMsg, "Sorry, you have to delete your POR-File and setup a new\n" );
      zstrcat( szMsg, "workstation.\n\n" );
      zstrcat( szMsg, "Check Out aborted!" );
                                ...
                                zVIEW vTZCMWKSO, zVIEW vTZCMWIPO )
{
   ...
   SetAttributeFromAttribute( vTZCMCPRO, "UserMetaStatus", "Comments",
                              vTZCMWIPO, "ROOT", "CheckOutComments" );

   SysGetDateTime( szTimestamp );
   SetAttributeFromString( vTZCMCPRO, "UserMetaStatus", "CheckOutDate",
                           szTimestamp );


   IncludeSubobjectFromSubobject( vTZCMCPRO, "User_1",
                                  vTZCMWKSO, "User", zPOS_AFTER );
   IncludeSubobjectFromSubobject( vTZCMCPRO, "RepositoryClient",
                                  vTZCMWKSO, "RepositoryClient", zPOS_AFTER );

   GetIntegerFromAttribute( (zPLONG)&ulZKey2, vTZCMWKSO, "LPLR", "ZKey" );
   if ( SetCursorFirstEntityByInteger( vTZCMCPRO, "LPLR", "ZKey",(zLONG) ulZKey2,
                                       "" ) < zCURSOR_SET )
   {
      // This is an error condition that should never occur, so if it does,
      // we are going to put the LPLR entry back into the CPLR.
      IncludeSubobjectFromSubobject( vTZCMCPRO, "LPLR",
                                     vTZCMWKSO, "LPLR", zPOS_AFTER );
   }

   IncludeSubobjectFromSubobject( vTZCMCPRO, "LPLR_CheckOut",
                                  vTZCMCPRO, "LPLR", zPOS_AFTER );

   if ( GetViewByName( &vTZCMCOML, "TZCMCOML", vSubtask, zLEVEL_TASK ) > 0 )
   {
      CreateMetaEntity( vSubtask, vTZCMCOML, "UserMetaStatus", zPOS_AFTER );

      SetMatchingAttributesByName( vTZCMCOML, "UserMetaStatus",
                                   vTZCMCPRO, "UserMetaStatus", zSET_ALL );
      IncludeSubobjectFromSubobject( vTZCMCOML, "User",
                                     vTZCMWKSO, "User", zPOS_AFTER );
   }
                           ...
                           zULONG ulZKey )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &ulZKey2, vTZCMWKSO,
                            "RepositoryClient", "ZKey" );
   nRC = SetCursorFirstEntityByInteger( vTZCMREPO, "RepositoryClient",
                                        "ZKey",(zLONG) ulZKey2, "" );
   if ( nRC != zCURSOR_SET )
   {
      if ( zwfnTZCMLPLD_SetRepositoryClient( vSubtask, vTZCMREPO, vTZCMWKSO,
                                             ulZKey2 ) < 0 )
         return( -1 );
   }
   ...
   if ( SetCursorFirstEntityByInteger( vTemp, "W_MetaDef", "CPLR_ZKey",
                                       (zLONG) ulZKey, "" ) == zCURSOR_SET )
   {
      // We used to give an error if a meta looked like it was already
      // checked out. But it no longer seems necessary and actually gets
      // in the way if an ER was checked in but its DTE or SA was left
      // hanging. Thus we will now exclude an existing W_MetaDef entry if
      // it is found.
   ...
   IncludeSubobjectFromSubobject( vTemp, "W_MetaDef",
                                  vTZCMLPLO, "W_MetaDef", zPOS_AFTER );
                                ...
                                zLONG Type, zULONG ulZKey )
{
   ...
   GetStringFromAttribute( szLPLR_FileSpec, vTZCMLPLO, "LPLR", "MetaSrcDir" );
   ofnTZCMWKSO_AppendSlash( szLPLR_FileSpec );

   if ( Type == zSOURCE_VOR_META )
      zltoxa( ulZKey, szName );
   else
      GetStringFromAttribute( szName, vTZCMCPRO, "Z_MetaDef", "Name" );

   zstrcat( szLPLR_FileSpec, szName );
   zstrcat( szLPLR_FileSpec, SRC_CMOD[ Type ].szOD_EXT );

   nRC = oTZCMLPLO_CopyFileFromServerFile( vSubtask, szLPLR_FileSpec,
                                           vTZCMCPRO, szName,
                                           SRC_CMOD[ Type ].szOD_EXT + 1 );
   if ( nRC >= 0 )
   {
      nRC = ActivateOI_FromFile( &vMetaOI, SRC_CMOD[ Type ].szOD,
                                 vTZCMCPRO, szLPLR_FileSpec,
...
zwfnTZCMLPLD_ActivateCOMPL( zVIEW vSubtask, zPVIEW vTZCMCOML, zVIEW vTZCMLPLO )
{
   ...
   nRC = ActivateObjectInstance( vTZCMCOML, "TZCMCOML", vSubtask,
                                 vKZDBHQUA, zSINGLE | zLEVEL_APPLICATION );
   ...
   {
      GetStringFromAttribute( szName, vTZCMLPLO, "CorrespondingCPLR", "Name" );
      zstrcpy( szMsg, "Unable to activate Corresponding CPLR ' " );
      zstrcat( szMsg, szName );
      zstrcat( szMsg, "' from database! " );
                           ...
                           zLONG lType )
{
   ...
   SetMatchingAttributesByName( vTZCMRPTO, "Z_MetaDef",
                                vTZCMLPLW, "W_MetaDef", zSET_ALL );
   SetAttributeFromAttribute( vTZCMRPTO, "Z_MetaDef", "ZKey",
                              vTZCMLPLW, "W_MetaDef", "CPLR_ZKey" );
   CreateEntity( vTZCMRPTO, "ReportLine", zREPOS_AFTER );
   SetAttributeFromString( vTZCMRPTO, "ReportLine", "ReportLine",
                           "Remove Check Out State" );
   ...
   if ( CheckExistenceOfEntity( vTZCMCOML, "UserMetaStatus" ) >= zCURSOR_SET )
   {
      ...
      if ( CheckExistenceOfEntity( vTZCMCOML, "LPLR_CheckOut" ) >= zCURSOR_SET &&
           CompareAttributeToInteger( vTZCMCOML, "LPLR_CheckOut", "ZKey",
                                      lLPLR_ZKey ) == 0 )
      {
         // Remove meta entries
   ...
   if ( lType == zSOURCE_DOMAINGRP_META || lType == zSOURCE_GOPGRP_META )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         if ( CompareAttributeToAttribute( vTZCMLPLO2, "W_MetaDef", "GroupName",
                                           vTZCMLPLO,  "W_MetaDef", "Name" ) == 0 )
         {
                               ...
                               zVIEW vTZCMLPLO )
{
   ...
   if ( CheckExistenceOfEntity( vTZCMRPTO, "Z_MetaDef" ) >= zCURSOR_SET )
   {
      ...
      if ( zwTZCMLPLD_OptionRefreshFromRepo( vSubtask, &vTZCMLPLO, szName,
                                             zRefreshUserDefined ) < 0 )
      {
         return( -1 );
      }
   }
...
zwfnTZCMLPLD_AskForRemoveERD_TE_SA( zVIEW vTZCMLPLW, zLONG lType )
{
   ...
   {
      zstrcpy( szMsg, "Removing Check Out State from Data Model will remove\n"
                      "Check Out State for all Subject Areas and TE!" );
   }
   else
   if ( lType == (zLONG) zSOURCE_DTE_META )
   {
      zstrcpy( szMsg, "Removing Check Out State from TE will remove\n"
                      "Check Out State for Data Model and all Subject Areas!" );
   }
   else
   if ( lType == (zLONG) zSOURCE_SA_META )
   {
      zstrcpy( szMsg, "Removing Check Out State from a Subject Area will remove\n"
...
zwfnTZCMLPLD_SelectERD_TE_SA( zVIEW vTZCMLPLW )
{
   if ( SetCursorFirstEntityByInteger( vTZCMLPLW, "W_MetaType", "Type",
                                       (zLONG) zSOURCE_ERD_META,
                                       "" ) >= zCURSOR_SET )
   {
...
zwfnTZCMLPLD_CheckSelectSate( zVIEW vTZCMLPLW, zLONG lSourceType, zLONG lType )
{
   ...
   if ( lType == (zLONG) zSOURCE_ERD_META ||
        lType == (zLONG) zSOURCE_DTE_META )
   {
      if ( SetCursorFirstEntityByInteger( vTZCMLPLW_Sel, "W_MetaType", "Type",
                                          lType, "" ) >= zCURSOR_SET )
      {
...
zwfnTZCMLPLD_AskForRemove( zVIEW vSubtask, zVIEW vTZCMLPLW )
{
   ...
   if ( SetCursorFirstSelectedEntity( vTZCMLPLW, "W_MetaDef",
                                      "LPLR" ) < zCURSOR_SET )
   {
      MessageSend( vSubtask, "CM00241", "Configuration Management",
...
zwTZCMLPLD_ExcludeActive( zVIEW vSubtask )
{
   ...
   {
      GetIntegerFromAttribute( &lType, vTZCMLPLW, "W_MetaType", "Type" );
      nRC = SetCursorFirstEntityByInteger( vTZCMLPLO, "W_MetaType",
                                           "Type", lType, "" );
      ...
      if ( nRC < zCURSOR_SET )
      {
         zstrcpy( szMsg, "Component '" );
         GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ],
                                 vTZCMLPLW, "W_MetaDef", "Name" );
         zstrcat( szMsg, "' not found in Project." );
         MessageSend( vSubtask, "CM00256", "Configuration Management",
                      szMsg, zMSGQ_OBJECT_CONSTRAINT_WARNING, zBEEP );
         continue;
      }

      nRC = SetCursorFirstEntityByInteger( vTZCMCOML, "Z_MetaDef", "ZKey",
                                           lZKey, "" );
      if ( nRC >= zCURSOR_SET )
         zwfnTZCMLPLD_RemoveStatus( vSubtask, vTZCMCOML, vTZCMWKSO,
                                    vTZCMLPLO, vTZCMLPLW, vTZCMRPTO, lType );
      else
         MessageSend( vSubtask, "CM00256", "Configuration Management",
                           ...
                           zPCHAR szName, zSHORT nRefreshLPLR )
{
   ...
   {
      nRC = ActivateObjectInstance( vTZCMREFO, "TZCMREFL", vSubtask,
                                    vKZDBHQUA, zSINGLE | zLEVEL_APPLICATION );
   }
   else
   {
      nRC = ActivateObjectInstance( vTZCMREFO, "TZCMREFO", vSubtask,
                                    vKZDBHQUA, zSINGLE | zLEVEL_APPLICATION );
   }
                              ...
                              zVIEW vTZCMCPRO )
{
   ...
   SysGetDateTime( szTimestamp );
   GetStringFromAttribute( szPrevLPLR_LastRefreshDate, vTZCMLPLO,
                           "LPLR", "LastRefreshDate" );

   // TZCMRPTO is used to build a Refresh Report */
   ActivateEmptyObjectInstance( &vTZCMRPTO, "TZCMRPTO",
                                 vTZCMLPLO, zMULTIPLE );
   ...
   {
      if ( zwTZCMLPLO_RefreshReusableCPLRs( vSubtask, vTZCMLPLO, vTZCMCPRO,
                                            szTimestamp,
   ...
   if ( zwTZCMLPLO_LoadRefreshListForCPLR( vSubtask, vTZCMLPLO, vTZCMCPRO,
                                           szPrevLPLR_LastRefreshDate ) < 0 )
      return( -1 );
...
zwTZCMLPLD_RefreshUserDefined( zVIEW vSubtask )
{
   ...
   else
   {
      ...
      SetEntityCursor( vTZCMRPTO, "Z_MetaDef", 0,
                       zQUAL_SCOPE_OI | zPOS_FIRST, 0, 0, 0, 0, 0, 0 );
   }
...
zwTZCMLPLD_OptionRefresh( zVIEW vSubtask )
{
        ...
        zQUAL_SCOPE_OI | zPOS_FIRST, 0, 0, 0, 0, 0, 0 ) >= zCURSOR_SET )
   {
                          ...
                          zRESPONSE_YES,  0 ) == zRESPONSE_YES )
      {
         ...
         SetWindowActionBehavior( vSubtask,
                                  zWAB_StartModalWindow | zWAB_ProcessImmediateAction,
                                  "TZCMRPTD", "RefreshList" );
      }
      else
         DropObjectInstance( vTZCMRPTO );
...
zwTZCMLPLD_ViewRefreshReport( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileSpec, vTZCMLPLO, "LPLR", "PgmSrcDir" );
   ofnTZCMWKSO_AppendSlash( szFileSpec );
   zstrcat( szFileSpec, "ZREFRESH.RPT" );

   if ( ActivateOI_FromFile( &vTZCMRPTO, "TZCMRPTO", vSubtask,
                             szFileSpec, zMULTIPLE | zNOI_OKAY ) < 0 )
   {
...
zwTZCMLPLD_TaskCountCheck( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK ) <= 0 )
      return( -1 );
   GetIntegerFromAttribute( (zPLONG) &ulSelectedZKey, vTZCMLPLO,
                            "LPLR", "ZKey" );
   ...
   if ( TaskCount > 1 )
   {
      zstrcpy( szMsg, "Other Zeidon tools are currently using Project\n" );
      GetStringFromAttribute( &szMsg[ zstrlen( szMsg ) ], vTZCMLPLO,
                              "LPLR", "Name" );
      zstrcat( szMsg, "!\nExecuting the requested function may corrupt\n" );
      zstrcat( szMsg, "any active metas. To continue end any Zeidon\n" );
      zstrcat( szMsg, "tools currently using this Project!" );
...
// zCHAR  szMsg[  zSHORT_MESSAGE_LTH + 1  ];
   ...
   // PETTIT - In the future I will check the return code and continue with
   //          the switch based upon it.
   SetAttributeFromInteger( vTZCMWKSO, "RepositoryClient",
                              "DefaultLPLR_ZKey", ulZKey );
...
zwfnTZCMLPLD_BuildAllActive( zVIEW vTZCMLPLW, zVIEW vTZCMLPLO )
{
   ...
   while ( TRUE )
   {
      ...
      // DGC - DOMGRP
      // We don't want to show domains and global operations, so just
      // continue loop if the current type is a domain or gop.
      if ( lType == zSOURCE_DOMAIN_META || lType == zSOURCE_GO_META )
      {
         if ( SetCursorNextEntity( vTZCMLPLO,
                                   "W_MetaDef", "LPLR" ) < zCURSOR_SET )
         {
            break;
         }
      ...
      SetCursorFirstEntityByInteger( vTZCMLPLW, "W_MetaType",
                                     "Type", lType, "" );

      CreateEntity( vTZCMLPLW, "W_MetaDef", zPOS_AFTER );
      SetMatchingAttributesByName( vTZCMLPLW, "W_MetaDef",
                                   vTZCMLPLO, "W_MetaDef", zSET_ALL );
...
zwfnTZCMLPLD_BuildActiveUpdated( zVIEW vTZCMLPLW, zVIEW vTZCMLPLO, zVIEW vTZCMCPRO )
{
   ...
   {
      // DGC - DOMGRP
      // We don't want to show domains and global operations, so just continue
      // loop if the current type is a domain or gop.
      if ( lType != zSOURCE_DOMAIN_META && lType != zSOURCE_GO_META )
      {
...
zwfnTZCMLPLD_BuildRefreshAll( zVIEW vTZCMLPLW, zVIEW vTZCMLPLO, zVIEW vTZCMCPRO )
{
   ...
   if ( CompareAttributeToAttribute( vTZCMCPRO, "CPLR",
                                     "LastCheckInDate",
                                     vTZCMLPLO, "LPLR",
                                     "LastRefreshDate" ) < 1 )
      return( 0 );
      ...
      if ( lType != zSOURCE_DOMAIN_META && lType != zSOURCE_GO_META )
      {
         if ( nRC >= zCURSOR_SET )
         {
            if ( CompareAttributeToInteger( vTZCMLPLO, "W_MetaDef",
                                            "UpdateInd", 0 )          < 1 )
            {
               if ( CompareAttributeToAttribute( vTZCMCPRO, "Z_MetaDef",
                                                 "LastUpdateDate",
                                                 vTZCMLPLO, "LPLR",
                                                 "LastRefreshDate" ) == 1 ||
                    CompareAttributeToAttribute( vTZCMCPRO, "Z_MetaDef",
                                                 "LastCheckInDate",
                                                 vTZCMLPLO, "LPLR",
                                                 "LastRefreshDate" ) == 1 )
               {
         ...
         else
         {
            SetCursorFirstEntityByInteger( vTZCMLPLW, "W_MetaType",
                                           ...
                                           "" );
            CreateEntity( vTZCMLPLW, "W_MetaDef", zPOS_AFTER );
            SetAttributeFromAttribute( vTZCMLPLW, "W_MetaDef", "CPLR_ZKey",
                                       vTZCMCPRO, "Z_MetaDef", "ZKey" );
            SetAttributeFromAttribute( vTZCMLPLW, "W_MetaDef", "Name",
                                       vTZCMCPRO, "Z_MetaDef", "Name" );
            SetAttributeFromAttribute( vTZCMLPLW, "W_MetaDef", "Desc",
                                       vTZCMCPRO, "Z_MetaDef", "Desc" );
            SetAttributeFromAttribute( vTZCMLPLW, "W_MetaDef", "LastUpdateDate",
...
zwfnTZCMLPLD_BuildRefreshUpdate( zVIEW vTZCMLPLW, zVIEW vTZCMLPLO, zVIEW vTZCMCPRO )
{
   ...
   if ( CompareAttributeToAttribute( vTZCMCPRO, "CPLR", "LastCheckInDate",
                                     vTZCMLPLO, "LPLR", "LastRefreshDate" ) < 1 )
      return( 0 );
                                    ...
                                    zPCHAR pchUserName )
{
   ...
   nRC = SetCursorFirstEntityByInteger( vTZCMCOML, "LPLR_CheckOut",
                                        "ZKey", lLPLR_ZKey, "CPLR" );
   while ( nRC >= zCURSOR_SET )
   {
      nRC = SetCursorFirstEntityByInteger( vTZCMCOML, "User",
                                           "ZKey", lUserZKey, "" );
      if ( nRC >= zCURSOR_SET )
      {
         nRC = SetCursorFirstEntityByInteger( vTZCMCOML, "RepositoryClient",
                                              "ZKey", lWksZKey, "" );
         if ( nRC >= zCURSOR_SET )
         {
      ...
      nRC = SetCursorNextEntityByInteger( vTZCMCOML, "LPLR_CheckOut",
                                          "ZKey", lLPLR_ZKey, "CPLR" );
   }
...
zwTZCMLPLD_BuildTZCMLPLW( zVIEW vSubtask, zPVIEW vTZCMLPLW )
{
   ...
   if ( ActivateEmptyObjectInstance( vTZCMLPLW, "TZCMLPLO", vSubtask,
                                      zSINGLE ) >= 0 )
   {
                                     ...
                                     zVIEW vTZCMLPLW, zVIEW vCompL )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      if ( SetCursorFirstEntityByAttr( vTZCMLPLW_Copy, "W_MetaDef", "CPLR_ZKey",
                                       vTZCMLPLO_Copy, "W_MetaDef", "CPLR_ZKey",
                                       "LPLR" ) != zCURSOR_SET &&
           CompareAttributeToInteger( vTZCMLPLO_Copy, "W_MetaDef", "Status",
                                      1 ) == 0 )
      {
         if ( SetCursorFirstEntityByAttr( vCompL, "Z_MetaDef", "ZKey",
                                          vTZCMLPLO_Copy, "W_MetaDef", "CPLR_ZKey",
                                          "" ) >= zCURSOR_SET &&
              CheckExistenceOfEntity( vCompL, "UserMetaStatus" ) != zCURSOR_SET )
         {
            if ( SetCursorFirstEntityByAttr( vTZCMLPLW, "W_MetaType", "Type",
                                             vTZCMLPLO_Copy, "W_MetaType", "Type",
                                             "" ) >= zCURSOR_SET )
            {
               CreateEntity( vTZCMLPLW, "W_MetaDef", zPOS_AFTER );
               SetMatchingAttributesByName( vTZCMLPLW, "W_MetaDef",
                                            vTZCMLPLO_Copy, "W_MetaDef", zSET_ALL );
               SetAttributeFromString( vTZCMLPLW, "W_MetaDef",
                                       "SysadmRemoveCheckOutState",
                                       "Check Out Removed by SYSADM" );
...
zwTZCMLPLD_BuildSelectedCompList( zVIEW vSubtask )
{
   ...
   switch ( Option )
   {
      ...
      case 8:           // Checked Out By Signed On User
         GetStringFromAttribute( szUserName, vTZCMWIPO, "ROOT", "UserName" );
         zwfnTZCMLPLD_BuildCheckedOutByUser( vSubtask, vTZCMLPLW, vTemp, vCompL,
                                             szUserName );
         zstrcpy( szMsg, "Checked Out By - " );
      ...
      case 9:     // Checked Out By User and Remove Check Out State in SYSADM
         GetStringFromAttribute( szUserName, vTZCMWIPO, "ROOT", "UserName" );
         zwfnTZCMLPLD_BuildCheckedOutByUser( vSubtask, vTZCMLPLW, vTemp,
                                             vCompL, szUserName );
         zwfnTZCMLPLD_BuildRemoveCheckedOuts( vSubtask, vTZCMLPLO,
                                              vTZCMLPLW, vCompL );
         zstrcpy( szMsg, "Checked Out By - " );
         zstrcat( szMsg, szUserName );
         SetWindowCaptionTitle( vSubtask, 0, szMsg );
   ...
   // We want to temporarily eliminate UI_Specs from the list.  To do this
   // and make it easy to remove later, we will just delete the MetaType
   // zSOURCE_UIS_META, if it exists.
   if ( SetCursorFirstEntityByInteger( vTZCMLPLW, "W_MetaType", "Type",
                                       zSOURCE_UIS_META, 0 ) >= zCURSOR_SET )
...
zwTZCMLPLD_PreBldCompDetail( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCMWIPO, "TZCMWIPO", vSubtask, zLEVEL_TASK ) < 0 )
      return( -1 );
   if ( CompareAttributeToInteger( vTZCMWIPO, "ROOT",
                                   "ComponentListType", 6 ) > 0 )
   {
      if ( GetViewByName( &vTZCMCPRO, "TZCMCPRO", vSubtask, zLEVEL_TASK ) < 0 )
      {
         return( -1 );
      ...
      SetCursorFirstEntityByInteger( vTZCMCPRO, "Z_MetaDef",
                                     "ZKey", ZKey, "" );
   }
...
zwTZCMLPLD_CommitMessageTextOI( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileName, vMsgText, "Task", "TemporaryWorkText" );
   ...
   if ( GetViewByName( &vTZCMLPLO, "TZCMLPLO", vSubtask, zLEVEL_TASK ) >= 0 )
   {
      ...
      if ( zwTZCMLPLD_GetZeidonApp( vSubtask, &vKZAPPLOO, szZeidonDirectory ) >= 0 )
      {
         if ( SetCursorFirstEntityByAttr( vKZAPPLOO, "APPLICATION", "APP_NAME",
                                          ...
                                          "" ) >= zCURSOR_SET )
         {
            GetStringFromAttribute( szZeidonDirectory, vKZAPPLOO, "APPLICATION",
                                    "APP_LOCAL" );
            SetCtrlText( vSubtask, "edWorkDirectory", szZeidonDirectory );
         }
                                 ...
                                 zVIEW vTZCMREPO )
{
   ...
   GetStringFromAttribute( szShadowDirectory, vTZCMREPO, "CPLR", "ExecDir" );
   bRC = SysValidDirOrFile( szShadowDirectory, TRUE, FALSE,
                            (sizeof(szShadowDirectory)+1) );
   if ( bRC == 0 )
   {
...
zwTZCMLPLD_BuildDialogExecs( zVIEW vSubtask )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      if ( nRC >= 0 )
      {
         GetStringFromAttribute( szDialogName, vDialog, "Dialog", "Tag" );
         zstrcpy( szMsg, "Building Dialog: " );
         zstrcat( szMsg, szDialogName );
         MB_SetMessage( vSubtask, 1, szMsg );
...
zwTZCMLPLD_BuildServerExecs( zVIEW vSubtask )
{
   ...
   if ( nRC >= 0 )
   {
      nRC = SetCursorFirstEntityByString( vMeta, "APPLICATION", "APP_NAME",
                                          szAppName, 0 );
      if ( nRC >= zCURSOR_SET )
         DropObjectInstance( vMeta );
      else
      {
         ...
         SetAttributeFromString( vMeta, "APPLICATION", "APP_NAME",
                                 szAppName );
   ...
   nRC = ActivateOI_FromFile( &vMeta, "TZDMXGPO", vSubtask, szFileName,
                              zSINGLE );
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      zstrcat( szFileName, szMetaName );
      zstrcpy( szMerkeFileName, szFileName);
      zstrcat( szFileName, ".XOD" );
      TraceLineS( "*** Activating client file: ", szFileName );
      nRC = ActivateOI_FromFile( &vMeta, "TZZOXODO", vSubtask, szFileName,
                                 zSINGLE );
      ...
      if ( nRC >= 0 )
      {
         ...
         GetStringFromAttribute( szDBType, vMeta, "OBJECT", "SironDB_Type" );
         if ( *szDBType == 'F' )
         {

            // Now we know, that the XOD is for Siron-FlatFiles.
            // Sending the XSK-Files.
            zstrcat( szMerkeFileName, ".XSK" );
            TraceLineS( "*** Activating client file: ", szMerkeFileName);
            nRC = ActivateOI_FromFile( &vSiron, "KZSIXSKO", vSubtask,
                                       szMerkeFileName, zSINGLE );
            ...
            TraceLineS( "*** Committing server file: ", szFileName );
            zstrcpy( szMsg, "Sending XSK: " );
            zstrcat( szMsg, szMetaName );
            MB_SetMessage( vSubtask, 1, szMsg );

            // Send OI to server.
            nRC = NetCommitOI_ToFile( 0, szServer, vSiron,
                                      szFileName, zASCII );
            if ( nRC )
            {
               TraceLineI( "NetCommitOI_ToFile nRC: ", nRC );
...
zwTZCMLPLD_CopyDirs( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( DirName, vTZCMLPLO, "LPLR", "ExecDir" );
...
zwTZCMLPLD_DefaultDir( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCMLPLO, "TZCMLPLO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      // Set Work Directory
      szZeidonDirectory[ 0 ] = 0;
      if ( zwTZCMLPLD_GetZeidonApp( vSubtask, &vKZAPPLOO,
                                    szZeidonDirectory ) >= 0 )
      {
         zwTZCMLPLD_GetWorkDir( vKZAPPLOO, szZeidonDirectory );
         SetCtrlText( vSubtask, "edWorkDirectory", szZeidonDirectory );
...
zwTZCMLPLD_ImportTextFile( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCMAUDO, "TZCMAUDO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      if ( OperatorPromptForFile( vSubtask, szFileName, sizeof( szFileName ),
                                  "Text Files (*.TXT)|*.txt|", "TXT", TRUE ) == 1 )
      {
         lfFile = SysOpenFile( vSubtask, szFileName, COREFILE_READ );

         // If the File exists, then we need to copy prototypes from the old
         ...
         if ( lfFile >= 0 )
         {
            ...
            // Get the buffer.
            hGlobalHandle = SysAllocMemory( (zCOREMEM) &szBuffer,
                                            lLth + 10, 0,
                                            zCOREMEM_ALLOC, 0 );

            lRC = SysReadFile( vSubtask, lfFile,
                               szBuffer, (zUSHORT) (lLth + 5) );
...
//                SetAttributeFromString( vTZCMAUDO, "AuditTrail", "Desc", "" );
            ...
            szBuffer[ lLth ] = 0;
            SetAttributeFromString( vTZCMAUDO, "AuditTrail",
                                    "Desc", szBuffer );
            SysFreeMemory( hGlobalHandle );
...
zwTZCMLPLD_CopyText( zVIEW vSubtask )
{
   ...
   if ( CompareAttributeToString( vTZCMAUDO,
                                  "AuditTrail", "Desc", "" ) != 0 &&
   ...
   if ( CheckExistenceOfEntity( vTZCMLPLO,
                                "CorrespondingCPLR" ) >= zCURSOR_SET )
   {
      GetIntegerFromAttribute( &lZKey, vTZCMLPLO, "CorrespondingCPLR", "ZKey" );
      if ( GetViewByName( &vTZCMCPRO, "TZCMCPRO", vSubtask, zLEVEL_TASK ) < 0 ||
           CompareAttributeToInteger( vTZCMCPRO, "CPLR", "ZKey", lZKey ) != 0 )
      {
         MB_SetMessage( vSubtask, 1, "Activating CPLR" );
      ...
      if ( SetCursorFirstEntityByAttr( vTZCMCPRO, "Z_MetaDef", "ZKey",
                                       vTZCM_ChkInList2, "W_MetaDef",
                                       "CPLR_ZKey", "CPLR" ) >= zCURSOR_SET &&
           CheckExistenceOfEntity( vTZCMCPRO, "UserMetaStatus" ) >= zCURSOR_SET )
      {
         SetAttributeFromAttribute( vTZCMAUDO, "AuditTrail", "Desc",
                                    vTZCMCPRO, "UserMetaStatus", "Comments" );
         RefreshCtrl( vSubtask, "DescEdit" );
      }
   }
...
zwfnTZCMLPLD_ActivateUSER( zVIEW vSubtask, zPVIEW vTZCMUSER, zVIEW vTZCMWIPO )
{
   ...
   GetStringFromAttribute( szName, vTZCMWIPO, "ROOT", "UserName" );
   if ( ActivateEmptyObjectInstance( &vKZDBHQUA, "KZDBHQUA", vSubtask,
                                     zSINGLE ) < 0 )
   {
      return( -1 );
   ...
   nRC = ActivateObjectInstance( vTZCMUSER, "TZCMUSER", vSubtask,
                                 vKZDBHQUA, zMULTIPLE | zLEVEL_APPLICATION );
...
zwTZCMLPLD_InitMenu( zVIEW vSubtask )
{
   ...
   {
      // Gray the CheckIn and CheckOut options if there is no
      // CorrespondingCPLR or Repository User with ReadOnly Authority
      if ( CheckExistenceOfEntity( vTZCMLPLO, "CorrespondingCPLR" ) >= zCURSOR_SET )
      {
         zwfnTZCMLPLD_ActivateUSER( vSubtask, &vTZCMUSER, vTZCMWIPO );

         if ( GetViewByName( &vTZCMUSER, "TZCMUSER", vSubtask, zLEVEL_TASK ) > 0 &&
              SetCursorFirstEntityByAttr( vTZCMUSER, "CPLR", "ZKey",
                                          vTZCMLPLO, "CorrespondingCPLR", "ZKey",
                                          "User" ) >= zCURSOR_SET )
         {
            if ( CompareAttributeToInteger( vTZCMUSER, "AccessAuthority",
                                            "Authority", 0 ) != 0 )
            {
               nEnable = 1;
            }
                                  ...
                                  zPCHAR szReportPathName )
{
        ...
        nRC1 = SetCursorNextEntity( vTempTZCMLPLO, "W_MetaDef", 0 ) )
   {
      ...
      // and *.rep
      GetStringFromAttribute( szReportName, vTempTZCMLPLO,
                              "W_MetaDef", "Name" );
      ...
      if ( hReport >= 0 )
      {
         nRC = ActivateOI_FromFile( &ReturnedReport, "TZRPSRCO",
                                    vTempTZCMLPLO, szFileName,
                                    zIGNORE_ATTRIB_ERRORS );
         if ( nRC >= 0 )
         {
            GetStringFromAttribute( szFileName, vTempTZCMLPLO,
                                    "LPLR", "PgmSrcDir" );
            ...
            {
               // The *.REP file exists
               oTZRPSRCO_ChangeRepFileContents( ReturnedReport,
                                                vTempTZCMLPLO,
                                                szPathNameREP, szPathNameXSQ,
                                                szReportName, "REP",
                                                zCHANGE_REP_MIGRATE_EXTERNAL,
                                                vSubtask );
            }
            ...
            {
               // The *.XSQ file exists
               oTZRPSRCO_ChangeRepFileContents( ReturnedReport, vTempTZCMLPLO,
                                                szPathNameREP, szPathNameXSQ,
                                                szReportName, "XSQ",
                                                zCHANGE_REP_MIGRATE_EXTERNAL,
                                                vSubtask );
                               ...
                               zPCHAR szRootEntityName )
{
   ...
   for ( nRC = SetCursorFirstEntity( vTZCMLPLO, "W_MetaDef", "" );
         ...
         nRC = SetCursorNextEntity( vTZCMLPLO, "W_MetaDef", "" ) )
   {
      GetStringFromAttribute( szMetaName, vTZCMLPLO, "W_MetaDef", "Name" );
      zstrcpy( szMsg, "Create Prototype for " );
      zstrcat( szMsg, szMsgText );
      zstrcat( szMsg, ": '" );
      zstrcat( szMsg, szMetaName );
      zstrcat( szMsg, "'." );
      MB_SetMessage( vSubtask, 1, szMsg );

      if ( ActivateMetaOI( vSubtask, &vMeta, vTZCMLPLO, nType,
                           zSINGLE | zLEVEL_TASK | zIGNORE_ATTRIB_ERRORS ) != 1 )
      {
         zstrcpy( szMsg, "Could not load " );
         zstrcat( szMsg, szMsgText );
         zstrcat( szMsg, "'" );
         zstrcat( szMsg, szMetaName );
         zstrcat( szMsg, "'." );
...
zwTZCMLPLD_GenerateZeidonOp_h( zVIEW vSubtask )
{
   ...
   // Send a Warning
   zstrcpy( szMsg, "This function creates a new ZeidonOp.h for all \nGlobal and LOD " );
   zstrcat( szMsg, "Operations. \n\n Do you want to continue ?" );
...
zwTZCMLPLD_LoadCompListPopup( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vTZCMUSER, "TZCMUSER", vSubtask, zLEVEL_TASK ) > 0 &&
        GetViewByName( &vTZCMLPLO, "TZCMLPLO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      if ( SetCursorFirstEntityByAttr( vTZCMUSER, "CPLR", "ZKey",
                                       vTZCMLPLO, "CorrespondingCPLR", "ZKey",
                                       "User" ) >= zCURSOR_SET &&
           CompareAttributeToInteger( vTZCMUSER, "AccessAuthority",
                                      "Authority", 0 ) != 0 )
      {
         CreateTrackingPopupMenu( vSubtask, "RemovePopup", -1, -1,
                                  FALSE, FALSE );
      }
   }
...
zwfnTZCMLPLD_Fill_TZCMRUWK( zVIEW vSubtask, zVIEW vTZCMRUWK, zVIEW vTZCMREPO )
{
   ...
   if ( GetViewByName( &vTZCMWKSO, "TZCMWKSO", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMWKSO,
                                  "User", "ZKey" );
         nRC = SetCursorFirstEntityByInteger( vTZCMRUWK, "User",
                                              "ZKey", ulZKey, "" );
         if ( nRC >= zCURSOR_SET )
         {
...
zwTZCMLPLD_BuildUserWkList( zVIEW vSubtask )
{
   ...
   CreateEntity( vTZCMRUWK, "Installation", zPOS_AFTER );
   SetNameForView( vTZCMRUWK, "TZCMRUWK", vSubtask, zLEVEL_TASK );

   zwfnTZCMLPLD_Fill_TZCMRUWK( vSubtask, vTZCMRUWK, vTZCMREPO );

   if ( CheckExistenceOfEntity( vTZCMRUWK, "User" ) >= zCURSOR_SET )
      OrderEntityForView( vTZCMRUWK, "User", "Name A " );

   SetCursorFirstEntityByString( vTZCMRUWK, "User", "CPLR_User", "Y", "" );
...
zwfnTZCMLPLD_ExcludeUser( zVIEW vTZCMRUWK, zVIEW vTZCMWKSO, zVIEW vTZCMREPO )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &ulZKey, vTZCMWKSO, "User", "ZKey" );
   GetIntegerFromAttribute( (zPLONG) &ulWKS_ZKey, vTZCMWKSO,
                            "RepositoryClient", "ZKey" );
        ...
        CompareAttributeToString( vTZCMRUWK, "User", "CPLR_User", "N" ) == 0 )
   {
      ...
      if ( SetCursorFirstEntityByInteger( vTZCMREPO, "User", "ZKey",
                                          ulZKey, "" ) >= zCURSOR_SET &&
           SetCursorFirstEntityByInteger( vTZCMREPO, "UserRepositoryClient",
                                          "ZKey", (zLONG) ulWKS_ZKey,
                                          "" ) >= zCURSOR_SET )
      {
...
zwfnTZCMLPLD_IncludeUser( zVIEW vTZCMRUWK, zVIEW vTempWKSO, zVIEW vTZCMREPO )
{
   ...
   nRC = SetCursorFirstEntityByInteger( vTempWKSO, "User", "ZKey",
                                        ulZKey, "" );
   if ( nRC < zCURSOR_SET )
   {
      if ( SetCursorFirstEntityByInteger( vTZCMREPO, "User", "ZKey", ulZKey,
                                          "" ) >= zCURSOR_SET )
      {
         IncludeSubobjectFromSubobject( vTempWKSO, "User",
                                        vTZCMREPO, "User", zPOS_AFTER );
...
zwTZCMLPLD_CheckUser( zVIEW vSubtask )
{
        ...
        CheckExistenceOfEntity( vTZCMRUWK, "User" ) >= zCURSOR_SET )
   {
      ...
      if ( lColumn == 0 )
      {
         if ( CompareAttributeToString( vTZCMRUWK, "User", "CPLR_User", "N" )
                                        == 0 )
         {
            if ( CompareAttributeToAttribute( vTZCMWIPO, "ROOT", "UserName",
                                              vTZCMRUWK, "User", "Name" ) == 0 )
            {
