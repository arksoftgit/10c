...
#define ADDEXPRESSIONENTRYTOSTRING( p1, p2, p3, p4, p5, p6 ) \
        AddExpressionEntryToString( pchLocalCode,\
           plLocalCode, pchLocalText, plLocalText,\
           p1, p2, p3, p4, p5, p6 )
...
//
//    1: zstrcpy( szTemp, B );  A is "XXX" B is "YYY" szTemp is "YYY"
//    2: zstrcat( szTemp, A );  A is "XXX" B is "YYY" szTemp is "YYYXXX"
   ...
   switch( lAnalyzeResult )
   {
      ...
      case qCANNOTOPTIMIZENEEDTEMP:
         ...
         else
         {
            ...
            {
               lLth = GetVariableLength( szGlobalTempNumber, sGExprStruct.lOffset );
            ...
            if ( sGExprStruct.lElementClass == qATTRIBUTEQUALIFIER )
            {
               ...
               lGlobalTempID = GenerateTempVariable( vSubtask, qTSTRING, lLth );
               lLth = GetVariableLength( szGlobalTempNumber, lGlobalTempID );
            }
        ...
        sGExprStruct.lElementClass == qATTRIBUTEQUALIFIER )  // 2001
   {
      ...
      if ( sGExprStruct.lElementClass == qATTRIBUTEQUALIFIER )
      {
         *lpTargetDataClass = GetClassBasedOnType( sGExprStruct.lDataType );
         zstrcpy( szTargetString, sGExprStruct.szText );
      }
      ...
      while( lRC == zCURSOR_SET )
      {
         ...
         else // is operator
         {
            if ( sGExprStruct.lLevel < 10 )
            {
               ...
               {
                  // 10/6/96 - DonC
                  // I am elimating the nSimpleFlag setting.  It used to be
                  // used to force a temporary value when the target string
                  // variable was indexed as in: S1[3:5] = S2[1:4] + S3.
                  // Instead we are copying directly into the string in this
                  // case.
                             ...
                             zLONG  lStartLevel )
{
   ...
   do
   {
      ...
      switch ( sGExprStruct.lElementClass )
      {
         case qOPERATOR:
            ...
            case qRPAREN:
            {
               ...
               if ( sExprStruct.lElementClass != qOPERATOR )
               {
                  ...
                  //   RETURN ( 5 )
                  // copy the expression node, with extraction of a
                  //  temp variable, if needed.
               ...
               if ( sExprStruct.lOffset == qCLOSEOPER )
               {
                  ...
                  else
                  {
                     ...
                     SysMessageBox( vTgt, "unexpected error during VML parse",
                                    "Operation call not found", zBEEP );
                     return( -1 );
                  }
               }
            ...
            default:
               ...
               // for the  current operator, determine
               //  "right" and "left" side.
               // this was already done under fnAnalyzeComplexExpression.
               // The results were stored in a table, which we want
               //  to access in order to get "right" and "left" side.
               // To look up the table, we use the relative position
               //  in the source view.
               ...
               if ( iFound == 0 )
               {
                  ...
                  //  should not occur
                  SysMessageBox( vTgt, "unexpected error during VML parse",
                                 "Operator not found in Table", zBEEP );
                  return( -1 );
               }
               ...
               lRC = fnHandleOperatorTab( vSubtask, vSrc, vTgt, vLeftView, vRightView,
                                          vLeftTargetView, pOTab, lStartLevel, lMaxLevel );
         ...
         // if the entry is an operation call we extract it out into its
         // own expression and put back a temp variable that was generated
         ...
         case qATTRIBUTEQUALIFIER:
            // if the "thing" was not an operation call or a string variable
            // just push it onto the stack. Pushing onto the stack is setting
            // the "right" view to the right most position. and putting the
            // "left" view where the right was. Eventually we have the left
            // pointing at an operand, the right pointing at an operand and the
            // operator pointing at an operator
            // was PUSH( lExprEntry );
            break;
         default:
            break;
      }
   } while ( SetEntityCursor( vSrc, "ExprNode", "Level",
                              zPOS_NEXT | zQUAL_INTEGER | zQUAL_LESS_THAN,
                              &lMaxLevel, "", "", 0, "", 0 ) >= zCURSOR_SET );
...
fnReArrangeLogOper( zVIEW vExpr )
{
   ...
   // if we find, on  the same logical level, an OR preceding an AND,
   //  we will move it behind the last AND of that logical level.
   zVIEW  vExprAND;
   zVIEW  vExprOR;
   sQExprStruct sExprStruct = { 0 };
   zSHORT nState = 0;
   ...
   {
      // if we find an OR followed by one ore more AND's,
      // then we will move the OR behind the last AND of that chain.
                    ...
                    T_OPER_TABLE *pOTab )
{
   ...
   {
      // the operand is not an intermediate result,
      //   so copy it to the target view
                     ...
                     zLONG lStartLevel, zLONG lMaxLevel )
{
   ...
   if ( (pOTab->lRightPos <= 0) && (pOTab->lLeftPos > 0) )
   {
      ...
      if ( sGExpr.lDataType == qTUNKNOWN )
      {
         // as data type is unknown,
         // take the oper table entry of the other side
      ...
      case qATTRIBUTEQUALIFIER:
         ...
         switch ( sLeftStruct.lDataType )
         {
            case qTDECIMAL:
               ...
               zstrcpy( szOperName, "GetDecimalFromAttribute" );
               break;

            case qTINTEGER:
               lTempID = GenerateTempVariable( vSubtask, qTINTEGER, 4 );
               zstrcpy( szOperName, "GetIntegerFromAttribute" );
               break;
            ...
            case qTSTRING: // not yet tested
               ...
               zstrcpy( szOperName, "GetStringFromAttribute" );
               break;
              ...
              zVIEW vTgt, zVIEW vOperatorView )
{
   ...
   if ( sOperatorStruct.lOffset == qEQUAL )
   {
      ...
      if ( sLeftStruct.lDataType == qTSTRING && sLeftStruct.lElementClass == qVARIABLE )
      {
         ...
         zstrcpy( szOperName, "GetVariableFromAttribute" );
   ...
   if ( IsCondOperator( sOperatorStruct.lOffset ) == 0 )
   {
      ...
      if ( sLeftStruct.lDataType == qTDECIMAL )
         zstrcpy( szOperName, "CompareAttributeToDecimal" );
      else
         zstrcpy( szOperName, "CompareAttributeToInteger" );
              ...
              zPCHAR szEName, zPCHAR szAName )
{
   ...
   if ( sOperatorStruct.lOffset == qEQUAL && vRightView == 0 )
   {
      ...
      if ( sLeftStruct.lDataType == qTDECIMAL )
         zstrcpy( szOperName, "SetAttributeFromDecimal" );
      else
         zstrcpy( szOperName, "SetAttributeFromInteger" );
   ...
   if ( sOperatorStruct.lOffset == qEQUAL )
   {
      ...
      if ( sRightStruct.lDataType == qTSTRING )
         zstrcpy( szOperName, "SetAttributeFromString" );
      else
         zstrcpy( szOperName, "SetAttributeFromInteger" );
               ...
               zPCHAR szVName, zPCHAR szEName, zPCHAR szAName )
{
        ...
        IsCondOperator( sOperatorStruct.lOffset ) == 0 )
   {
      if ( sOperatorStruct.lOffset == qEQUAL )
      {
         zstrcpy( szOperName, "SetAttributeFromAttribute" );
      ...
      {
         zstrcpy( szOperName, "CompareAttributeToAttribute" );
         AddCompareToView( vSubtask, vTgt, szOperName, &sLeftStruct, sLeftStruct.szText,
                           &sRightStruct, sRightStruct.szText, 0 );
         InsertExprNodeValuesToEntity( vTgt, qCONSTANT, qNUMERIC, qTINTEGER, 0, 0, "0", 0, zPOS_AFTER );
         fnCopyExprNode( vTgt, vOperatorView, zPOS_AFTER, FALSE );
                              ...
                              zLONG  lAnalyzeResult, zLONG  lStartLevel )
{
   ...
   // go to the next expr value by level as it may not be the
   // immediate next level this will skip the [] combo as they have
   // weighted priorities.
   lMaxLevel = lStartLevel + 10;
   while ( SetEntityCursor( vSrc, "ExprNode", "Level",
                            zPOS_NEXT | zQUAL_INTEGER | zQUAL_LESS_THAN,
   ...
   // if we are optimizing by concatenation then we need to skip past this
   // expression entry as it is the same as the left (i.e. a = a + b)
        ...
        (lTargetElementClass == qATTRIBUTEQUALIFIER && lRightIndexed == 1) )
   {
      if ( lTargetElementClass == qVARIABLE )
         lLth = GetVariableLength( szGlobalTempNumber, lTargetID );
      else
         lLth = -1;
               ...
               zLONG  lStartLevel )
{
   ...
   {
      // If the left view was zero, then we are sitting on an operator.
      // In this case what is going on is that we are going to "operate" into
      ...
      else
      {
         ...
         zstrcpy( szTempNumber, szGlobalTempNumber );
      }
   }
   ...
   {
      // if the operator is equal and the left and right are the same
      // variable, then we are done so delete the entities
      // we are done because the left hasx been set by a series of concats
      // if a temp is used then the oper would not be equal
      if ( sOperatorStruct.lOffset == qEQUAL &&
           sLeftStruct.lOffset == lGlobalTempID // && // was
         ) // lGlobalRightID == -1 )
      {
         return( 1 );
      }
      ...
      if ( (sOperatorStruct.lOffset == qEQUAL && sLeftStruct.lElementClass == qVARIABLE) ||
           (sOperatorStruct.lOffset == qEQUAL && sLeftStruct.lElementClass != qVARIABLE &&
            lGlobalTempID != -1) )
      {
         sRightStruct.lElementClass = qVARIABLE;
         sRightStruct.lOffset = lGlobalTempID;
         ...
         switch( sRightStruct.lElementClass )
         {
            ...
            case qATTRIBUTEQUALIFIER:
               ...
               if ( sOperatorStruct.lOffset == qADDITION )
               {
                  ...
                  lTempID = GenerateTempVariable( vSubtask, qTSTRING, lLth );
                  lLth = GetVariableLength( szTempNumber, lTempID );
                  if ( g_szGenLang[ 0 ] == 'J' )
                     lReturnLthID = GenerateTempVariable( vSubtask, qTINTEGER, 4 );
                  else
                     lReturnLthID = 0;
                  ...
                  // add the get string. If it was left indexed, then lTempID
                  // will hold the temp generated to hold the left. If it was
                  // concatenate then lTempID will hold the temp for the
                  // GetfromAQ. either way we want to get into lTempid
                  AddGetVariableFromAttribute( vSubtask, szOperName, lViewID, lReturnLthID, 0,
                                               sRightStruct.lDataType, lTempID,
                                               szEName, lEntityID, szAName, lAttributeID, 0, "", 0 );
                  // break omitted here
               }
               else
            ...
            case qDEFINE:
               ...
               if ( sOperatorStruct.lOffset == qADDITION )  // Generated ZeidonStringCopy/ZeidonStringConcat
               {
                  ...
                  if ( lGlobalTempID != sLeftStruct.lOffset )
                  {
                     ...
                     CreateTargetZeidonStrOpSWithID( vSubtask, lGlobalTempID, szLocal7Code,
                                                     &lLocal7Code, szLocal7Text,
                                                     &lLocal7Text, "ZeidonStringCopy" );
                     ...
                     if ( vLeftView )
                     {
                        ...
                        CreateSourceZeidonStrOpS( vSubtask, vLeftView, szLocal7Code,
                                                  &lLocal7Code, szLocal7Text,
                                                  &lLocal7Text, szGlobalTempNumber );
                     }
                     else
                     {
                        CreateSourceZeidonStrOpSWithID( vSubtask, sLeftStruct.lOffset,
                                                        szLocal7Code, &lLocal7Code, szLocal7Text,
                                                        &lLocal7Text, szGlobalTempNumber );
                     }

                     // Finally add the statement from the stack.
                     AddOperatorToView( vSubtask, 0, -1 );
                     AddStatementEntityWithStringToPI( vSubtask, qEXPRESSION, qqlineno, szLocal7Code,
                                                       &lLocal7Code, szLocal7Text, &lLocal7Text, zPOS_AFTER );
                  }
                  ...
                  GetVariableLength( szGlobalTempNumber, lGlobalTempID );

                  // concatenate the right element (the c in b c +)
                  if ( sRightStruct.lElementClass == qATTRIBUTEQUALIFIER )
                  {
                     CreateSourceZeidonStrOpWithID( vSubtask, lTempID, vTgt, szGlobalTempNumber );
                  }
                  else
                  {
                     if ( vRightView != 0 )
                        CreateSourceZeidonStrOp( vSubtask, vRightView, vTgt, szGlobalTempNumber );
                     else
                        CreateSourceZeidonStrOpWithID( vSubtask, sRightStruct.lOffset,
                                                       vTgt, szGlobalTempNumber );
                  }
               }
               ...
               else
               {
                  if ( sLeftStruct.lElementClass == qVARIABLE )
                     lLth = GetVariableLength( szTempNumber, sLeftStruct.lOffset );
                  else
                  {
                     lLth = zstrlen( sLeftStruct.szText );
                     zltoa( lLth, szTempNumber );
                  }
                  ...
                  if ( sRightStruct.lElementClass != qATTRIBUTEQUALIFIER )
                  {
                     ...
                     if ( sOperatorStruct.lOffset == qEQUAL )
                        zstrcpy( szOperName, "ZeidonStringCopy" );
                     else // is cond operator
                        zstrcpy( szOperName, "ZeidonStringCompare" );
                     ...
                     if ( sRightStruct.lDataType == qTSTRING || sRightStruct.lDataType == qTCHARACTER )
                     {
                        ...
                        if ( vRightView != 0 )
                        {
                           CreateSourceZeidonStrOp( vSubtask, vRightView, vTgt, szTempNumber );
                        }
                     ...
                     if ( sOperatorStruct.lOffset != qEQUAL )
                     {
                        ...
                        AddExprNodeEntityToView( vSubtask, vTgt, qOPERATOR, qNUMERIC, qTINTEGER,
                                                 "0", sOperatorStruct.lOffset, 0 );
                  ...
                  else
                  {
                     ...
                     {
                        zstrcpy( szOperName, "CompareAttributeToString" );
                        if ( sLeftStruct.lElementClass == qVARIABLE )
                        {
                           ...
                           if ( sRightStruct.lDataType == 0 )
                              CreateOperationCallUsingAQ( vSubtask, vTgt, szOperName, lViewID, 1, qTSTRING,
                                                          sLeftStruct.lOffset, szEName, lEntityID, szAName,
                                                          lAttributeID, 0, "", 0, 0 );
                           else
                              CreateOperationCallUsingAQ( vSubtask, vTgt, szOperName, lViewID, 1,
                                                          sRightStruct.lDataType, sLeftStruct.lOffset,
...
/////////// end test
            ...
            AddGetOrSetAttribute( vSubtask, "GetStringFromAttribute", lViewID, 0,
                                  sLeftStruct.lDataType, lTempID, szEName, lEntityID,
         ...
         switch( sRightStruct.lElementClass )
         {
            ...
            case qVARIABLE:
               ...
               if ( sOperatorStruct.lOffset == qADDITION )
               {
                  CreateTargetZeidonStrOpSWithID( vSubtask, lTempID, szLocal7Code, &lLocal7Code,
                                                  szLocal7Text, &lLocal7Text, "ZeidonStringConcat" );
                  GetVariableLength( szTempNumber, lTempID );
                  CreateSourceZeidonStrOpS( vSubtask, vRightView, szLocal7Code, &lLocal7Code,
                                            szLocal7Text, &lLocal7Text, szTempNumber );
               ...
               {
                  if ( sRightStruct.lElementClass == qCONSTANT ||
                       sRightStruct.lElementClass == qDEFINE )
                  {
                     if ( sRightStruct.lDataType == qTINTEGER )
                        AddGetOrSetAttribute( vSubtask, "SetAttributeFromInteger", lViewID, -1,
                                              qTINTEGER, lTempID, szEName, lEntityID, szAName,
                                              lAttributeID, sRightStruct.lDataType, sRightStruct.szText,
                                              sRightStruct.lOffset, sRightStruct.lElementClass );
                     else
                     if ( sRightStruct.lDataType == qTDECIMAL )
                        AddGetOrSetAttribute( vSubtask, "SetAttributeFromDecimal", lViewID, -1,
                                              qTDECIMAL, lTempID, szEName, lEntityID, szAName,
                                              lAttributeID, sRightStruct.lDataType,
                                              sRightStruct.szText, sRightStruct.lOffset, sRightStruct.lElementClass );
                     else
                        AddGetOrSetAttribute( vSubtask, "SetAttributeFromString", lViewID, -1,
                                              qTSTRING, lTempID, szEName, lEntityID, szAName,
                                              lAttributeID, sRightStruct.lDataType,
                                              sRightStruct.szText, sRightStruct.lOffset, sRightStruct.lElementClass );
                  }
                  else
                  {
                     // It is possible, that the right side of the assignment is NOT a string.
                     // We want to handle that. **HH**
                     if ( sRightStruct.lDataType == qTINTEGER )
                     {
                        AddGetOrSetAttribute( vSubtask, "SetAttributeFromInteger", lViewID, 1,
                                              qTINTEGER, lTempID, szEName, lEntityID,
                                              szAName, lAttributeID, 0, "", 0, 0 );
                     }
                     else
                     if ( sRightStruct.lDataType == qTDECIMAL )
                     {
                        AddGetOrSetAttribute( vSubtask, "SetAttributeFromDecimal", lViewID, 1,
                                              qTDECIMAL, lTempID, szEName, lEntityID,
                                              szAName, lAttributeID, 0, "", 0, 0 );
                     }
                     else
                     {
                        AddGetOrSetAttribute( vSubtask, "SetAttributeFromString", lViewID, 1,
                                              qTSTRING, lTempID, szEName, lEntityID,
                                              szAName, lAttributeID, 0, "", 0, 0 );
                     }
                  }
               }
            ...
            case qATTRIBUTEQUALIFIER:
               if ( sOperatorStruct.lOffset == qADDITION )
               {
                  ...
                  AtomizeAQ( sRightStruct.szText, szViewNameR, szEntityNameR, szAttributeNameR,
                             &lViewIDR, &lEntityIDR, &lAttributeIDR, sRightStruct.lDataClass );
...
/*  DonC change 1999.06.11   Old code used lTempIDR for length, which
    ...
    for target variable.
                  lLth = GetVariableLength( szTempNumber, lTempIDR );
*/
                  lLth = GetVariableLength( szTempNumber, lTempID );

                  // add a get then a string concat
                  AddGetOrSetAttribute( vSubtask, "GetStringFromAttribute", lViewIDR, 0,
                                        sRightStruct.lDataType, lTempIDR, szEntityNameR, lEntityIDR,
                                        szAttributeNameR, lAttributeIDR, 0, "", 0, 0 );
                  CreateTargetZeidonStrOpSWithID( vSubtask, lTempID, szLocal7Code,
                                                  &lLocal7Code, szLocal7Text, &lLocal7Text,
                                                  "ZeidonStringConcat" );
                  CreateSourceZeidonStrOpSWithID( vSubtask, lTempIDR, szLocal7Code, &lLocal7Code,
                                                  szLocal7Text, &lLocal7Text, szTempNumber );
               ...
               else
               {
                       ...
                       IsCondOperator( sOperatorStruct.lOffset ) == 0 )
                  {
                     ...
                     {
                        zstrcpy( szOperName, "SetAttributeFromAttribute" );
                        AddCompareToView( vSubtask, vTgt, szOperName, &sLeftStruct,
                                          sLeftStruct.szText, &sRightStruct, sRightStruct.szText, 0 );
                     }
                     else
                     {
                        zstrcpy( szOperName, "CompareAttributeToAttribute" );
           ...
           zLONG   lDataClass )
{
   ...
   *plAttributeID = 0;
   ...
   if ( lDataClass == qENTITYISUNKNOWN ||
        lDataClass == qENTITYANDATTRIBUTEAREUNKNOWN ||
        lDataClass == 0 )
   {
                         ...
                         zPCHAR szTempNumber )
{
   ...
   // Add the Max Target String Length parm.  We will add 1 to the length
   // because the length passed must be the C length, not the VML length.
   lWork = zatol( szTempNumber );
   lWork++;
   zltoa( lWork, szTempNumber );
   AddExprNodeEntityToView( vSubtask, vTgt, qCONSTANT, qNUMERIC, qTINTEGER, szTempNumber, 0, 0 );
                          ...
                          zPCHAR szTempNumber )
{
   ...
   ADDEXPRESSIONENTRYTOSTRING( sExprStruct.lElementClass, sExprStruct.lDataClass,
                               sExprStruct.lDataType, sExprStruct.szText, sExprStruct.lOffset, 0 );
   ...
   lWork++;
   zltoa( lWork, szTempNumber );
   ADDEXPRESSIONENTRYTOSTRING( qCONSTANT, qNUMERIC, qTINTEGER, szTempNumber, 0, 0 );
   ...
   if ( CheckExistenceOfEntity( vSrc, "ExprNode" ) == zCURSOR_SET )
   {
      ...
      while ( lOffset != qCLOSEOPER && lNextEntity == zCURSOR_SET )
      {
         ...
         else
         {
            ...
            GetStringFromAttribute( szText, vSrc, "ExprNode", "Text" );
            ...
            if ( lDataType == qTSTRING )
            {
               ...
               zstrcpy( szOperationName, "GetStringFromAttribute" );
            }
            else
            if ( lDataType == qTDECIMAL )
            {
               lVariableID = GenerateTempVariable( vSubtask, qTDECIMAL, 4 );
               zstrcpy( szOperationName, "GetDecimalFromAttribute" );
            }
            else
            ...
            else
            {
               ...
               zstrcpy( szOperationName, "GetIntegerFromAttribute" );
            }
...
//          so that addition of 2 codes gives you a unique result
...
//  the low part of the stack value normally contains the position
//  in the expression view.
// If it is <= 0, then we have a computed expression and we set
//  the OTab position to the negative data type instead
zLONG
   ...
   for ( ; ; )
   {
      ...
      if ( lElementClass == qOPERATOR && lOffset == qLPAREN )
      {
         ...
         *plResult |= DATATYPE_LPAREN_BIT;
         ...
         lDataType = flDataType( vExpr, plDataTypeStack, plStackPointer, plResult,
                               pOperTable, plOperTablePos, 1 ); // Step Over First
         if ( lDataType == DATATYPE_ERROR )
            return( DATATYPE_ERROR );

         // if we come back from a paren expression,
         //  the returned data type does normally not have a high part.
         // this is, because we have a "computed" result of the
         //  term inside the parens.
         // however, if there is a constant inside the parens, like (-1),
         //  then there is a high part in lDataType, which is pointing
         //  to the constant in the source view. The parser later
         //  does not like that, because parens are always "copied"
         //  to the target view. A subseqent Operator must relate
      ...
      if ( lElementClass == qOPERATOR )
      {
         ...
         //  if we have a "computed" expression as the result of an
         //  evaluation, the high parts are zero. Then, there is no
         //  view argument to match that expression and the
         //  table position is set to -1.
         pOTab = pOperTable + (*plOperTablePos)++;
...
CaseExpression( zVIEW vSubtask, zVIEW vTgt, zVIEW vSrc )
{
   ...
   if ( lElementClass == qATTRIBUTEQUALIFIER )
   {
      ...
      AtomizeAQ( pchText, szViewName, szEntityName, szAttributeName,
                 &lViewID, &lEntityID, &lAttributeID, lDataClass );
