...
*/
    ...
    if (lExecState == 1L)
    {
      ...
      bFree = TRUE;
/*
      SysGetDateTime ( szCurrentTime );
      GetStringFromAttribute( szTimeout, vSirPara, "SirPara", "TimeoutTime");
      ...
      {
    TraceLineS( "(kzsirrep) Report discarded, cause it was not started before Timeout: ",
                     szCurrentTime);
         bFree = TRUE;
      }
*/
...
*/
  ...
  GetStringFromAttribute( szCurrentTime, vSirPara, "SirPara", "TimeoutTime" );
              ...
              zPCHAR pszPassword )
{
  ...
  // Set the values
  zstrcpy( szUsername, pszUsername );
  zstrcpy( szPassword, pszPassword );
