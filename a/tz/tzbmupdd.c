...
zwTZBMUPDD_SetNewBitmapFileName( zVIEW vSubtask )
{
   ...
   zstrcpy( szZeidonPath, szFileName );
   SysAppendcDirSep( szZeidonPath );
   zstrcat( szZeidonPath, "Bitmap.bmp" );
...
// zltoa( lTaskID, szTaskID );
   lTaskID = SysGetTaskFromView( vSubtask );
   zsprintf( szTaskID, "%08x", lTaskID );
   zstrcpy( szMetaOI_Name, "__CM." );
   zstrcpy( szMetaOI_Name + 5, szTaskID );
...
static zBOOL
zwfnSearch_Replace( zVIEW vDialog, zPCHAR szOrigPath,
                    zPCHAR szSearchPath, zPCHAR szReplacePath )
{
   ...
   else
   {
      ...
      else
      {
         ...
         {
            zstrcpy( szMsg, "Path replacement for Window '" );
            GetStringFromAttribute( szTemp, vDialog, "Window", "Tag" );
            zstrcat( szMsg, szTemp );
            zstrcat( szMsg, "', Control '" );
            GetStringFromAttribute( szTemp, vDialog, "Control", "Tag" );
            zstrcat( szMsg, szTemp );
            zstrcat( szMsg, "' failed. New Path longer than 256 characters. " );
            zwTZERRORD_WriteErrorMsg( vDialog, vCM_List, "W_MetaDef", "Name",
                                      szMsg, 1 );
            return( FALSE );
         }
      }
                              ...
                              zPCHAR szNewPath )
{
   ...
   zstrcpy( szMsg, "Replace Bitmap Path in Dialog: '" );
...
zwTZBMUPDD_ReplaceSelectedDialog( zVIEW vSubtask )
{
   ...
   zstrcpy( szMsg, "All Bitmap Paths for selected Dialogs will be changed." );
...
zwTZBMUPDD_ReplaceAllDialog( zVIEW vSubtask )
{
   ...
   // send Message Box for continue
   zstrcpy( szMsg, "All Bitmap Paths for all Dialogs will be changed." );
   zstrcat( szMsg, "\n\nDo you want to continue?" );
   if ( MessagePrompt( vSubtask, "BM00003", "Bitmap Maintenance",
                       szMsg, zBEEP, zBUTTONS_YESNO, zRESPONSE_NO,
                       zICON_QUESTION ) == zRESPONSE_NO )
...
zwTZBMUPDD_SetCountError( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vTZBMPATH, "BitmapPath" ) >= zCURSOR_SET )
   {
      ...
      if ( lCountPath > 0 )
      {
         zltoa( lCountPath, szCount );
         zstrcat( szControlText, szCount );
         zstrcat( szControlText, " Bitmap Path(s) " );
      }
      else
      {
         zstrcat( szControlText, "  No Bitmap Paths " );
      }
   }
   else
   {
      zstrcpy( szControlText, "  No Bitmap Paths " );
   }
...
zwTZBMUPDD_CreateNewBitmapRoot( zVIEW vSubtask, zSHORT nForAllDialogs )
{
   ...
   szTitle[ 0 ] = 0;
   zstrncat( szTitle, szCurrentDateTime + 6, 2 );
   zstrcat(  szTitle, "." );
   zstrncat( szTitle, szCurrentDateTime + 4, 2 );
   zstrcat(  szTitle, "." );
   zstrncat( szTitle, szCurrentDateTime, 4 );
   zstrcat(  szTitle, "  " );
   zstrncat( szTitle, szCurrentDateTime + 8, 2 );
   zstrcat(  szTitle, ":" );
   zstrncat( szTitle, szCurrentDateTime + 10, 2 );
   zstrcat(  szTitle, ":" );
   zstrncat( szTitle, szCurrentDateTime + 12, 2 );

   if ( nForAllDialogs )
      zstrcat( szTitle, "  for all Dialogs" );
   else
      zstrcat( szTitle, "  for selected Dialogs" );
                              ...
                              zCTRLBOI_BMP BMP, zULONG ulControlDefKey )
{
   ...
   GetStringFromAttribute( szControl, vDialog, "Control", "Tag" );
                              ...
                              zSHORT nError )
{
   ...
   {
      zstrcpy( szMsg, "*** Error:  " );
      zstrcat( szMsg, szBitmapPath );
      zstrcat( szMsg, " ***" );
   }
   else
   {
      zstrcpy( szMsg, szBitmapPath );
   }
...
zwTZBMUPDD_SearchBitmaps( zVIEW vSubtask, zVIEW vCM_List, zPCHAR szMetaSrcDir )
{
   ...
   GetStringFromAttribute( szDialogName, vCM_List, "W_MetaDef", "Name" );
   zstrcpy( szMsg, "Read Bitmap Path in Dialog: '" );
   zstrcat( szMsg, szDialogName );
   zstrcat( szMsg, "'." );
   MB_SetMessage( vSubtask, 1, szMsg );

   zstrcpy( szFullName, szMetaSrcDir );
   zstrcat( szFullName, szDialogName );
   zstrcat( szFullName, ".pwd" );
         ...
         nRC = SetCursorNextEntity( vDialog, "Window", 0 ) )
   {
      if ( SetCursorFirstEntity( vDialog, "Control", 0 ) >= zCURSOR_SET )
      {
         GetStringFromAttribute( szWindowName, vDialog, "Window", "Tag" );
         zwfnSearchPathRecursive( vDialog, szDialogName, szWindowName );
      }
   }
...
zwfnTZERRORD_WriteErrorListToFile( zVIEW vTZBMPATH, zLONG  FileHandle )
{
   ...
   zstrcpy( szMsg, "Zeidon:  Bitmap List from " );
   GetStringFromAttribute( szTemp, vTZBMPATH, "PathList", "DateTimeAndDialogs" );
   zstrcat( szMsg, szTemp );
   ...
   if ( GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK ) > 0 )
   {
      GetStringFromAttribute( szTemp, vTaskLPLR, "LPLR", "Name" );
      if ( szTemp[ 0 ] )
      {
   ...
   zstrcpy( szMsg, "Dialog:    Window:                           Control: " );
   zstrcat( szMsg, "                         Bitmap:" );
   SysWriteLine( vTZBMPATH, FileHandle, szMsg );
   ...
   for ( nRC = SetCursorFirstEntity( vTZBMPATH, "BitmapPath", "" );
         ...
         nRC = SetCursorNextEntity( vTZBMPATH, "BitmapPath", "" ) )
   {
      GetStringFromAttribute( szTemp, vTZBMPATH, "BitmapPath", "Dialog" );
      zstrcpy( szMsg, szTemp );
      for ( i = zstrlen( szTemp ); i < 11; i++ )
         zstrcat( szMsg, " " );

      GetStringFromAttribute( szTemp, vTZBMPATH, "BitmapPath", "Window" );
      zstrcat( szMsg, szTemp );
      for ( i = zstrlen( szTemp ); i < 34; i++ )
         zstrcat( szMsg, " " );

      GetStringFromAttribute( szTemp, vTZBMPATH, "BitmapPath", "Control" );
      zstrcat( szMsg, szTemp );
      for ( i = zstrlen( szTemp ); i < 34; i++ )
         zstrcat( szMsg, " " );

      GetStringFromAttribute( szTemp, vTZBMPATH, "BitmapPath", "BitmapPath" );
      zstrcat( szMsg, szTemp );
      SysWriteLine( FileHandle, szMsg );
   }

   SysWriteLine( FileHandle, "" );
   SysWriteLine( FileHandle, "End of Bitmap List" );
}
...
zwTZBMUPDD_CopyToField( zVIEW vSubtask )
{
   ...
   if ( vParentWindow )
   {
      GetStringFromAttribute( szBitmap, vTZBMPATH, "BitmapPath", "BitmapPath" );
