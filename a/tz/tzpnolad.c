...
} zOL_COLOR_FONT, zFAR *zLPOL_COLOR_FONT;
...
OL_DetailPreBuild( zVIEW vSubtask )
{
   ...
   // Get view to LOD lod.
   GetStringFromAttribute( szTempString, vCtlDialog, "CtrlMapView", "Name" );

   if ( SetCursorFirstEntityByString( vDialogW, "ViewObjRef", "Name",
                                      szTempString, 0 ) >= 0 )
   {
      GetStringFromAttribute( szTempString, vDialogW, "LOD", "Name" );
      ...
      if ( zstrcmp( OL_Color_Font->szEntityName, szEntityName ) == 0 )
      {
         ...
         while ( bFoundDefault == FALSE )
         {
                          ...
                          OL_Item->szEntityName ) == 0 )
            {
               ...
               zstrcpy( OL_Default_Color_Font->szDefaultBitmapFile,
                        OL_Color_Font->szDefaultBitmapFile );
               zstrcpy( OL_Default_Color_Font->szSelectedBitmapFile,
                        OL_Color_Font->szSelectedBitmapFile );
                            ...
                            zSHORT     nCountEntities )
{
   ...
   while ( bSibling )
   {
      ...
      zstrcpy( szTempString, OL_Item->szEntityName );
//    TraceLineS( "Loaded EntityName: ", szTempString );
      ...
      zstrcpy( szTempString, pchStart + OL_Item->lTagAttributeOffset );
                                   ...
                                   zPCHAR           *ppchColor )
{
   ...
   // Set Entity Name
   zstrcpy( OL_Color_Font->szEntityName, szEntityName );
                       ...
                       zSHORT           nCountEntities1 )
{
   ...
   while ( bSibling )
   {
      ...
      GetStringFromAttribute( szTemp, vLOD_Ent, "LOD_EntityParent", "Name" );
      ...
      if ( CompareAttributeToString( vLOD_Ent, "LOD_EntityParent",
                                     "Recursive", "Y" ) == 0 )
      {
      ...
      if ( SetCursorFirstEntityByString( vLOD_Attr, "LOD_Entity", "Name",
                                         szTemp, 0 ) == zCURSOR_SET )
      {
         ...
         while ( nRC == zCURSOR_SET )
         {
            if ( nRC = CheckExistenceOfEntity( vLOD_Attr, "ER_Attribute" ) == zCURSOR_SET )
            {
               ...
               SetAttributeFromAttribute( vCtlWork,  "Attribute",    "Name",
                                          vLOD_Attr, "ER_Attribute", "Name" );
...
OL_GetProperties( zVIEW vCtlWorkTop )
{
   ...
   if ( CompareAttributeToString( vCtlWorkTop, "Outliner",
                                  "MultiSelect", "1" ) == 0 )
   {
      lWork |= zOLS_MULTISEL;
   }
...
OL_SetTextColor( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szColor, vCtlWork, "Entity", "TextColor" );
...
OL_SetTextFont( zVIEW vSubtask )
{
   ...
   while ( bShowWindowAgain &&
           OperatorPromptForFont( vSubtask, &TextFont, &lSize,
                                  (zPLONG) &clr, TRUE ) == 1 )
   {
      if ( lSize > 16 )
      {
...
OL_SetNewBackgrdColor( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szColor, vCtlWork, "Outliner", "BackgroundColor" );
...
OL_DetailSave( zVIEW vSubtask )
{
   ...
   if ( CompareAttributeToString( vCtlWorkTop, "Object", "Tag",
                                  "__Bootstrap" ) == 0  )
   {
   ...
   SetAttributeFromAttribute( vCtlDialog,  "Control", "Text",
                              vCtlWorkTop, "Object",  "Text" );
   SetAttributeFromAttribute( vCtlDialog,  "Control", "Tag",
                              vCtlWorkTop, "Object",  "Tag" );
   ...
   GetStringFromAttribute( szColor, vCtlWorkTop, "Outliner", "BackgroundColor" );
   if ( zstrcmp( szColor, "" ) == 0 )
      lTemp = -1;
   else
      lTemp = zatol( szColor );
   ...
   *((zLONG *) pchTempColor) = lTemp;
   ...
   GetStringFromAttribute( pchTemp, vCtlDialog, "CtrlMapView", "Name" );
   pchTemp += zstrlen( pchTemp ) + 1;
   ...
   *((zLONG *) pchTemp) = lTemp;
   ...
   // Load a null terminator. MapOutlinerFromOI (in ZdCtlMst) will
   // look for an operation name and dll name but will skip them
   ...
   *pchTemp = 0;
   ...
   // the data for lack of a better thing until we learn different.
   GetStringFromAttribute( pchTemp, vCtlDialog, "CtrlMapView", "Name" );
   pchTemp += zstrlen( pchTemp ) + 1;
   ...
   fnWriteSiblingsToBuffer( vCtlWork, OL_CurrentItem, pchStart,
                            &pchTemp, OL_Color_Font, &pchTempColor );

   // Write buffer to the CtrlBOI.
   SetAttributeFromBlob( vCtlDialog, "Control", "CtrlBOI",
                         pchBuffer, (zULONG) (pchTemp - pchBuffer) );

   // Write the Background and Text Color Properties.
   SetAttributeFromBlob( vCtlDialog, "Control", "Properties",
                         pchColorBlob, (zULONG) (pchTempColor - pchColorBlob) );
                                ...
                                zPCHAR           *ppchColor )
{
   ...
   GetStringFromAttribute( szTemp, vCtlWork, "Entity", "DefaultBitmapFile" );
   zstrcpy( OL_Color_Font->szDefaultBitmapFile, szTemp );
   GetStringFromAttribute( szTemp, vCtlWork, "Entity", "SelectedBitmapFile" );
   zstrcpy( OL_Color_Font->szSelectedBitmapFile, szTemp );
                       ...
                       zPCHAR szScopeEntity )
{
   ...
   while ( bFound == FALSE && ResetViewFromSubobject( vCtlWork_Copy ) == 0 )
   {
      ...
      {
         bFound = TRUE;
         GetStringFromAttribute( szScope, vCtlWork_Copy, "Entity", "Name" );
      }
   }

   // if Parent not found, set Root Entity Name as Scope
   if ( bFound == FALSE )
      zstrcpy( szScope, "zSCOPE_OI" );

   zstrcpy( szScopeEntity, szScope );
                         ...
                         zPCHAR           *ppchColor )
{
   ...
   while ( bSibling )
   {
                                     ...
                                     "RecursiveSelected", "1" ) == 0 )
      {
         ...
         GetStringFromAttribute( szTemp, vCtlWork, "Entity", "Name" );
         ...
         if ( OL_ParentItem )
         {
            zstrcpy( OL_ParentItem->szRecursiveEntity, szTemp );
      ...
      else
      {
         ...
         *ppchTemp += sizeof( zOL_ITEM );
         ...
         // Put in the name.
         GetStringFromAttribute( szTemp, vCtlWork, "Entity", "Name" );
         zstrcpy( OL_CurrentItem->szEntityName, szTemp );

         // Set Scope Entity Name.
         OL_GetScopeEntityName( vCtlWork, szScopeEntity );
         zstrcpy( OL_CurrentItem->szScopeEntity, szScopeEntity );

         // Create room in the buffer.
         OL_Color_Font = (zLPOL_COLOR_FONT) *ppchColor;

         // Set Version Number, Text Color, Text Font and Bitmap
         fnWriteColorFontBitmapToBuffer( vCtlWork, szTemp,
                                         OL_Color_Font, ppchColor );
         ...
         *ppchTemp += zstrlen( *ppchTemp ) + 1;
         ...
         // ??? - NOTE: currently we are only loading one attribute
         // per entity.
...
fnSpecifyOutlinerOutliner( zVIEW vSubtask )
{
   ...
   *((zLONG *) pchTemp) = 1;
   ...
   zstrcpy( OL_Item->szEntityName, "Object" );
   OL_Item->szScopeEntity[ 0 ] = 0;
   OL_Item->szRecursiveEntity[ 0 ] = 0;

   // Put in tag attribute.
   zstrcpy( pchTemp, "Name" );
   ...
   *pchTemp = 0;
   ...
   // Put in the name and null scope entity.
   zstrcpy( OL_Item->szEntityName, "Entity" );
   OL_Item->szScopeEntity[ 0 ] = 0;

   OL_Item->lFlags = MAKELONG( zOL_RECURSIVE, sizeof( zOL_ITEM ) );
   zstrcpy( OL_Item->szRecursiveEntity, "ChildEntity" );
                ...
                zPCHAR szButtonTagName, zSHORT lBitmapID )
{
        ...
        TAB_GetActiveTab( vSubtask, "TreeTab" ) == 4 )
   {
      ...
      {
         GetStringFromAttribute( szFileName, vCtlWork, "Entity", szEntityName2 );
...
OL_LoadBitmapWithResourcePath( zVIEW vSubtask, zPCHAR szFileName )
{
   ...
   if ( szZeidonPath[ 0 ] )
   {
      ...
      zstrcat( szZeidonPath, szFileName );
...
OL_SetDflBitmap( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileName, vCtlWork, "Entity", "DefaultBitmapFile" );
...
OL_SetSelBitmap( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szFileName, vCtlWork, "Entity", "SelectedBitmapFile" );
