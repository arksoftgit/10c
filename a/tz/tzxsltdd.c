...
//    lFlags = 0x00000002;
   ...
   {
      DropNameForView( vTZXSLTSO, "__SysApplicationXSLT",
                       vSubtask, zLEVEL_SYSTEM );
...
AskForSave( zVIEW vSubtask )
{
   ...
   if ( MiGetUpdateForView( vXSLT ) && ObjectInstanceUpdatedFromFile( vXSLT ) )
   {
      ...
      {
         bSaveAs = TRUE;
         GetStringFromAttribute( szXSLTName, vXSLT, "XSLT", "Tag" );
         zstrcpy( szMessageText, "XSLT is not checked out, but XSLT '" );
         zstrcat( szMessageText, szXSLTName );
...
fnRecurseChangeDlgTags( zVIEW vXSLT )
{
   ...
   {
      GetStringFromAttribute( szTag, vXSLT, "Control", "Tag" );
      if ( szTag[ 0 ] == 0   ||
          (szTag[ 0 ] == 'T' &&
           szTag[ 1 ] == 'e' &&
           szTag[ 2 ] == 'x' &&
           szTag[ 3 ] == 't') )
      {
         GetStringFromAttribute( szText, vXSLT, "Control", "Text" );
         if ( szText[ 0 ] )
         {
            szText[ 32 ] = 0;
         ...
         else
         {
            ...
            {
               GetStringFromAttribute( szText, vXSLT,
         ...
         if ( szText[ 0 ] )
         {
            ...
            GetStringFromAttribute( szTag, vXSLT, "ControlDef", "Tag" );
...
TZXSLTDD_GenerateXSLT( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szXSLTName, vMetaList, "W_MetaDef", "Name" );
      zstrcpy( szMsg, "Generate XSLT for: '" );
      zstrcat( szMsg, szXSLTName );
      zstrcat( szMsg, "'." );
      ...
      {
         zstrcpy( szMsg, "Could not load XSLT '" );
         zstrcat( szMsg, szXSLTName );
         zstrcat( szMsg, "'." );
...
NewPanel( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTag, vXSLT, "XSLT", "Tag" );
   }
...
CleanupActions( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vXSLT, "TZOPENXSLT", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      zstrcpy( szTempString, "Action unreferenced XSLT.Window.Action: " );
...
CleanupOperations( zVIEW vSubtask )
{
   ...
   if ( GetViewByName( &vXSLT, "TZOPENXSLT", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      zstrcpy( szTempString, "Operation unreferenced in XSLT - " );
                            ...
                            zPCHAR  szXSLTName )
{
   ...
   GetStringFromAttribute( szXSLTName, vPainter, "Palette", "NewFile" );
   if ( szXSLTName[ 0 ] == 0 )
   {
...
SortWindows( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szSelected, vXSLT, "Window", "Tag" );
...
SortActionsForWindow( zVIEW vSubtask )
{
   ...
   GetViewByName( &vXSLT, "TZXSLT", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szSelected, vXSLT, "Action", "Tag" );
                         ...
                         zSHORT nLevel, zSHORT nID )
{
   ...
   if ( SetCursorFirstEntity( vXSLT, "Option", 0 ) == zCURSOR_SET )
   {
      do
      {
         ...
         GetStringFromAttribute( szText, vXSLT, "Option", "Text" );
...
LIST_MENU_Prebuild( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szTempString, vXSLT, "Window", "Tag");
   ...
   if ( GetViewByName( &vXSLT, "TZXSLT", vSubtask, zLEVEL_TASK ) > 0 )
   {
            ...
            nRC = SetCursorNextEntity( vXSLT, "Menu", 0 ) )
      {
         if ( CheckExistenceOfEntity( vXSLT, "Option" ) < zCURSOR_SET )
         {
            ...
            GetStringFromAttribute( szMenuName, vXSLT, "Menu", "Tag" );
...
UPD_MENU_OK( zVIEW vSubtask )
{
   ...
   CreateViewFromViewForTask( &vXSLTCopy, vXSLT, 0 );
   GetStringFromAttribute( szTag, vXSLT, "Menu", "Tag" );
...
fnWndUpdate( zVIEW vSubtask, zBOOL bStayOnWindow )
{
   ...
   GetStringFromAttribute( szNewTag, vXSLT, "Window", "Tag" );
   if ( szNewTag[ 0 ] == 0 ||
...
WND_UPD_OperationEdit( zVIEW vSubtask )
{
        ...
        CompareAttributeToString( vWork, "EventAct", "Tag", "" ) != 0 )
   {
      ...
      {
         GetStringFromAttribute( sTag, vXSLTW, "ActOper", "Name" );
         SetCursorFirstEntityByString( vXSLTW, "Operation", "Name",
                                       sTag, "XSLT" );
...
OPT_UPD_OK( zVIEW vSubtask )
{
   ...
   GetViewByName( &vXSLTO, "TZXSLTOPT", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szTag, vXSLTO, "Option", "NLS_Text" );

   // Make sure that the option tag is unique
   GetStringFromAttribute( szTag, vXSLTO, "Option", "Tag" );
...
OPT_UPD_OperationEdit( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( sTag, vXSLTC, "EventAct", "Tag" );
      SetCursorFirstEntityByString( vXSLTW, "Action", "Tag", sTag, "" );

      if ( CheckExistenceOfEntity( vXSLTW, "ActOper" ) == zCURSOR_SET )
      {
         SetCursorFirstEntityByEntityCsr( vXSLTW, "Operation",
                                          vXSLTW, "ActOper", "XSLT" );
         TransferToEditor( vSubtask, vXSLTW, "PN", 0 );
      }
      else
...
OptContextMappingInit( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vXSLT,
...
SEL_OPT_Init( zVIEW vSubtask )
{
   ...
   if ( CheckExistenceOfEntity( vXSLTO, "OptMapView" ) == zCURSOR_SET )
   {
      ...
      SetCursorFirstEntityByInteger( vXSLTW, "ViewObjRef",
                                     "ZKey", lZKey, "" );
      ...
      if ( CheckExistenceOfEntity( vXSLTO,
                                   "OptMapLOD_Attribute" ) == zCURSOR_SET )
      {
         GetIntegerFromAttribute( &lZKey, vXSLTO,
   ...
   else
   {
      ...
      if ( SetCursorFirstEntityByInteger( vXSLTW, "ViewObjRef",
                                          "ZKey", lZKey, "" ) == zCURSOR_SET )
      {
         if ( CheckExistenceOfEntity( vXSLTO, "OptMapView" ) == zCURSOR_SET )
            ExcludeEntity( vXSLTO, "OptMapView", zREPOS_PREV );

         IncludeSubobjectFromSubobject( vXSLTO, "OptMapView",
                                        vXSLTW, "ViewObjRef", zPOS_AFTER );
         GetIntegerFromAttribute( &lZKey, vXSLTW, "LOD", "ZKey" );
         SetCursorFirstEntityByInteger( vCM_List, "W_MetaDef",
                                        "CPLR_ZKey", lZKey, "" );
         ActivateMetaOI( vSubtask, &vLOD, vCM_List, zREFER_LOD_META, 0 );
      ...
      if ( lKey == zGROUPBOX_CONTROL )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
                   ...
                   ControlDef.PSDLG_Y + ControlDef.SZDLG_Y )
            {
               ...
               SetAttributeFromInteger( vCtrlXSLT, "Control",
                                        "PSDLG_X", ControlDef.PSDLG_X );
               SetAttributeFromInteger( vCtrlXSLT, "Control",
                                        "PSDLG_Y", ControlDef.PSDLG_Y );
               nRC = MoveSubobject( vXSLT, "CtrlCtrl",
                                    vCtrlXSLT, "Control",
...
TZXSLTDD_DeleteAllMenus( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szTag, vXSLT, "Window", "Tag" );
                                  ...
                                  zSHORT  nIndex )
{
   ...
   do
   {
      ...
      nRC = -1;
      if ( vXSLTData )
         nRC = SetCursorFirstEntityByString( vXSLTData, "SourceFile",
                                             "Name", szNewName, "" );
                              ...
                              zPCHAR   szOutName )
{
   ...
   if ( zstrcmp( szOutName, "" ) == 0 )
   {
      zstrcpy( szMsg, "XSLT Name is required." );
      MessageSend( vSubtask, "ZO00137", "XSLT Maintenance",
                   szMsg, zMSGQ_OBJECT_CONSTRAINT_ERROR, zBEEP );
      SetWindowActionBehavior( vSubtask, zWAB_StayOnWindow, 0, 0 );
      SetFocusToCtrl( vSubtask, "edXSLTName" );
...
TZXSLTDD_SaveAsXSLT( zVIEW vSubtask )
{
   ...
   if ( SetCursorFirstEntityByString( vTZXSLTS, "W_MetaDef", "Name",
                                      szOutName, 0 ) > zCURSOR_UNCHANGED )
   {
      zstrcpy( szMsg, "XSLT already exists. Replace existing XSLT?" );
      nRC = MessagePrompt( vSubtask, "ZO00138", "XSLT Maintenance",
                           szMsg, zBEEP, zBUTTONS_YESNO,
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         while ( nRC > zCURSOR_UNCHANGED )
         {
            ...
            if ( CompareAttributeToString( vOldXSLT, "Parameter", "ShortDesc",
                 "" ) == 0 )
            {
                 ...
                 zPCHAR  szXSLTName)
{
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      GetStringFromAttribute( szViewObjRefName, vOldXSLT, "ViewObjRef", "Name" );
   ...
   // Copy Windows
   GetStringFromAttribute( szOldXSLTName, vOldXSLT, "XSLT", "Tag" );
   ...
   {
      GetStringFromAttribute( szDfltWnd, vOldXSLT, "DfltWnd", "Tag" );
                   ...
                   zLONG lData )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      GetStringFromAttribute( szCtrlTag, vXSLT, "Control", "Tag" );
      ...
      if ( lCheckFlags & (zCHECK_REPORT_EVENTS | zCHECK_CHANGE_WAB) )
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
                                  ...
                                  0, 0, 0, 0 ) >= zCURSOR_SET )
            {
               ...
               GetStringFromAttribute( szDlgTag, vAct, "Action", "XSLTName" );
      ...
      {
         GetIntegerFromAttribute( &lKey, vXSLT, "ControlDef", "Key" );
         if ( CompareAttributeToInteger( vXSLT, "ControlDef",
                                         "Key", lKey ) != 0 )
         {
              ...
              (lKey == zTAB_CONTROL) )
         {
            ...
            while ( nRC >= zCURSOR_SET )
            {
               ...
               GetStringFromAttribute( szCtrlTag, vXSLT, "CtrlCtrl", "Tag" );
...
// }
                    ...
                    "XSLT Migration", MB_YESNO ) == IDYES )
   {
      ...
      zsprintf( szMsg, "Are you sure you want to change mapping for XSLT: %s", g_szDlgTag );
   ...
   while ( nRC == zCURSOR_SET )
   {
                           ...
                           zSINGLE | zLEVEL_TASK ) == 1 )
      {
              ...
              (zCHECK_INCREMENT_POS_SIZE | zCHECK_DECREMENT_POS_SIZE) )
         {
            if ( !ComponentIsCheckedOut( vSubtask, vXSLT, zSOURCE_XSLT_META ) )
            {
               zstrcpy( szMsg, "Unable to save XSLT '" );
         ...
         if ( (lCheckFlags & zCHECK_CHANGE_MAPPING) && zstrcmp( g_szDlgTag, pchDlgTag ) == 0 )
         {
            ...
            {
               zsprintf( szMsg, "Checking XSLT: %s for Migration", pchDlgTag );
               nRC = SetCursorFirstEntity( vXSLT, "Window", 0 );
               while ( nRC == 0 )
               {
                  zVIEW  vOldViewName;
         ...
         if ( bSaveAll )
         {
            ...
            {
               zsprintf( szMsg, "Saving XSLT: %s", pchDlgTag );
               MB_SetMessage( vSubtask, 1, szMsg );
         ...
         else
         {
            ...
            else
            {
               ...
               zsprintf( szMsg, "Checking XSLT: %s for synchronization", pchDlgTag );
               TraceLineS( szMsg, "" );
               MB_SetMessage( vSubtask, 1, szMsg );
      ...
      {
         TraceLineI( "MigrateXSLTs unexpected failure to open dialog in list ",
                     k );
      }
...
// GetViewByName( &vXSLT, "TZXSLT", vSubtask, zLEVEL_TASK );
   ...
   if ( vXSLT )
   {
      ...
      GetAddrForAttribute( &pchDlg, vXSLT, "XSLT", "Tag" );
      zsprintf( szMsg, "Reset Control Pos/Size/Class for ALL Windows in XSLT: %s?", pchDlg );
...
fnChangeMappingViewChildren( zVIEW vSubtask, zBOOL bAtLevel )
{
   ...
   GetViewByName( &vXSLT, "TZXSLT", vSubtask, zLEVEL_TASK );
   GetViewByName( &vTZXSLTL, "TZXSLTL", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szCtrlTag, vTZXSLTL, "Control", "Tag" );
...
GenerateAllXSLT( zVIEW vSubtask )
{
   ...
   zsprintf( szMsg, "Generate XSLT for ALL Panels in: %s?", pchDlg );
...
FindCtrl( zVIEW vSubtask )
{
   ...
   if ( szText[ 0 ] )
   {
      ...
      // if ( OL_SetCursorByEntityNumber( vTZXSLT, "Control", lRC ) == 0 )
         ...
         GetStringFromAttribute( szText, vTZXSLT, "Control", "Tag" );
   ...
   if ( GetViewByName( &vXSLT, "TZOPENXSLT", vSubtask, zLEVEL_TASK ) > 0 )
   {
      ...
      while ( nRC == zCURSOR_SET )
      {
         ...
         while ( nRC == zCURSOR_SET )
         {
            ...
            GetStringFromAttribute( szDlgTag, vXSLT, "Action", "XSLTName" );
            GetStringFromAttribute( szWndTag, vXSLT, "Action", "WindowName" );
            if ( szWndTag[ 0 ] )
            {
            ...
            if ( CheckExistenceOfEntity( vXSLT, "ActEvent" ) == 0 )
            {
               ...
               while ( nRC == zCURSOR_SET )
               {
                  if ( CheckExistenceOfEntity( vXSLT, "ActCtrl" ) == 0 )
                  {
                     ...
                     GetStringFromAttribute( szText, vXSLT, "ActCtrl", "Tag" );
                     nLth = zstrlen( szText );
                     szText[ nLth++ ] = ' ';
                     szText[ nLth++ ] = '-';
                     szText[ nLth++ ] = ' ';
                     GetStringFromAttribute( szText + nLth, vXSLT, "ActCtrl", "Text" );
