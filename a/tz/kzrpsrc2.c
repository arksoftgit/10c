...
#define IDM_GETPRINTDIALOG   207
   ...
   while ( nRC >= zCURSOR_SET )  // calculate extent on the fly!
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         GetStringFromAttribute( szType, vReportDef, "GroupSet", "Type" );
   ...
   {
      zCHAR szMsg[ 1024 ];
      zstrcpy( szMsg, "Rows to print report: Report = " );
      zstrcat( szMsg, cpcReportDefName );
      zstrcat( szMsg, " Entity = ");
      zstrcat( szMsg, cpcSubobjectEntityName );
      TraceLineS( szMsg,  "" );
   }
   else
   {
                              ...
                              zLONG  lMaxReturnedStringLength )
{
   ...
   MaxLength = lMaxReturnedStringLength - 6;    // Leave room for the max special character length.
   SourcePtr = pchSourceString;
   TargetPtr = pchReturnedString;
   CurrentTargetLength = 0;
   while ( *SourcePtr != 0 && CurrentTargetLength < MaxLength )
   {
      if ( *SourcePtr == '"' )
      {
         zstrcpy( TargetPtr, "&quot;" );
      ...
      else
      {
         *TargetPtr = *SourcePtr;
         TargetPtr++;
         zstrcpy( szTrace, "*** Source Character: " );
