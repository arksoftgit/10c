...
void
fnFindDefine( zPCHAR pchName, zLONG lType )
{
   zSHORT k = 0;
   ...
   zstrcpy( pchName, g_qDefines[ k ].pchName );
}
...
// zCHAR   szMsg[ 1024 ];
   ...
   fnFindDefine( szElementClass, psExpr->lElementClass );
   fnFindDefine( szDataClass, psExpr->lDataClass );
   fnFindDefine( szDataType, psExpr->lDataType );
   fnFindDefine( szOffset, psExpr->lOffset );
   fnFindDefine( szLevel, psExpr->lLevel );
...
void
fnAddView( zVIEW vSubtask, zPCHAR pchViewName, sQExprStruct *psExpr, zPCHAR pchPFlag, zPCHAR pchExprText )
{
   zCHAR  szTemp[ 64 ];
   zSHORT k;

   szTemp[ sizeof( szTemp ) - 1 ] = 0;
   k = AddExprEntryToString( vSubtask, psExpr, szTemp, pchPFlag, sizeof( szTemp ) - 2, pchExprText, 0 ); // get view variable

   pchViewName[ 0 ] = '(';
   pchViewName[ 1 ] = ' ';
   zstrcpy( pchViewName + 2, szTemp + k + 1 );
   zstrcat( pchViewName, " );\n" );
}
...
GenerateNewExpression( zVIEW vSubtask, zPCHAR pchOutputString )
{
   ...
   // Make the index into the output string start at the end since we will be building the
   // statement in reverse ... from end to beginning.
   ...
   nExprIdx = (zSHORT) ulCodeIdx;    // e.g. 288 or 240 or 216 (some multiple of 24)
   ...
   // It appears that the structures in the blob are are just the portion of
   // sQExprStruct without the "zCHAR szText[ MAXSTRING ];" element. If this
   // turns out not to be the case, will rewrite this comment ... dks 2009.08.23
   // Therefore we are initializing nExprIdx to point to the beginning of
   // the last ExprStruct in the blob.

#ifdef DEBUG_PARSEGEN  // enable just to help figure out what's going on
   {
      zSHORT  nWork;

      nWork = sizeof( sQExprStruct );     // 1048
      nWork -= MAXSTRING;                 // 1048 - 1024 = 24
      ...
      nExprIdx -= nWork;  // e.g. 288 - 24 = 264  or  240 - 24 = 216  or  216 - 24 = 192
   ...
   // This is code to iterate through the Expression list and sort things out in preparation
   // for Java generation.  We want to modify the flags in the blob, so we use a pointer to
   // get around the Expression list.
   if ( g_szGenLang[ 0 ] == 'J' )
   {
#ifdef DEBUG_PARSEGEN
...
//#endif
         ...
         if ( psExpr->lElementClass == qOPERATIONCALL )
         {
             ...
             if ( psExpr->lOffset > MAXSTRING )
               GetOperationNameFromZKey( vSubtask, szOperationName, szHeaderName, szThrowsException, szThrowsExceptionCatchCode, szOperationGroup, psExpr->lDataClass, psExpr->lOffset );
            else
               zstrcpy( szOperationName, szExprText + psExpr->lOffset );
      ...
      if ( bOperation == FALSE && nEqual > 0 )
      {
         ...
         if ( psExpr1 && psExpr2 )
         {
            ...
            k = AddExprEntryToString( vSubtask, psExpr2, szStringBuilder2, szPFlag2, sizeof( szStringBuilder2 ) - 2, szExprText, 0 ); // get variable
            zstrcpy( szStringBuilder2, szStringBuilder2 + k + 1 );

            szStringBuilder1[ sizeof( szStringBuilder1 ) - 1 ] = 0;
            k = AddExprEntryToString( vSubtask, psExpr1, szStringBuilder1, szPFlag1, sizeof( szStringBuilder1 ) - 2, szExprText, 0 ); // get variable
            zstrcpy( szStringBuilder1, szStringBuilder1 + k + 1 );
            ...
            if ( nCnt == 5 )
            {
               if (lOperator > 0 && nConstantIdx >= 0 )
               {
                  ...
                  if ( psExpr->lElementClass == qCONSTANT && zstrcmp( szExprText + psExpr->lOffset, "1" ) == 0 &&
                       zstrcmp( szStringBuilder1, szStringBuilder2 ) == 0 )
                  {
                          ...
                          szPFlag1[ 0 ] == 'Y' )
                     {
                        if ( lOperator == qADDITION || lOperator == qMINUS )
                        {
                           ...
                           zstrcpy( pchOutputString + nIndent, szStringBuilder1 );
                           if ( lOperator == qADDITION )
                              zstrcat( pchOutputString, ".increment( )" );
                           else
                              zstrcat( pchOutputString, ".decrement( )" );
               ...
               if ( nParmCnt == 3 )
               {
                  ...
                  k = AddExprEntryToString( vSubtask, psExpr, szVarName, szPFlag, sizeof( szVarName ) - 2, szExprText, 0 ); // get variable
                  zstrcpy( szVarName, szVarName + k + 1 );
            ...
            if ( nCnt == 3 )
            {
               ...
               {
                  zmemset( pchOutputString, ' ', nIndent );
                  zstrcpy( pchOutputString + nIndent, "SetViewFromView( " );
                  zstrcat( pchOutputString, szStringBuilder1 );
                  zstrcat( pchOutputString, ", " );
                  zstrcat( pchOutputString, szStringBuilder2 );
                  zstrcat( pchOutputString, " )" );
                  goto EXPRESSION_EXIT_label;
               }
               else
               if ( nDecimalCnt > 0 )
               {
                  if ( szPFlag1[ 0 ] == 'Y' && psExpr1->lDataType == qTDECIMAL )
                  {
                     zmemset( pchOutputString, ' ', nIndent );
                     zstrcpy( pchOutputString + nIndent, szStringBuilder1 );
                     zstrcat( pchOutputString, ".setValue( " );
                     zstrcat( pchOutputString, szStringBuilder2 );
                     zstrcat( pchOutputString, " )" );
                     goto EXPRESSION_EXIT_label;
                  }
                  ...
                  if ( szPFlag2[ 0 ] == 'Y' && psExpr2->lDataType == qTDECIMAL )
                  {
                     ...
                     {
                        zmemset( pchOutputString, ' ', nIndent );
                        zstrcpy( pchOutputString + nIndent, szStringBuilder2 );
                        zstrcat( pchOutputString, ".setValue( " );
                        zstrcat( pchOutputString, szStringBuilder1 );
                        zstrcat( pchOutputString, " )" );
                        goto EXPRESSION_EXIT_label;
      ...
      // First, process the list looking for (non-string) return arguments so we can determine
      // if there is a return value that needs to replace the return code.  At the same time,
      // determine if there are return string argument(s) that need to be declared as mutable.
      while ( nExprIdx >= 0 )
      {
         psExpr = (sQExprStruct *) ((zPCHAR) szExprCode + nExprIdx);
         if ( nExprIdx == 0 && psExpr->lElementClass == qCONSTANT &&
              psExpr->lDataClass == qNUMERIC && psExpr->lDataType == qTINTEGER &&
              zstrcmp( szExprText + psExpr->lOffset, "while " ) == 0 )
         {
            bDirectOutput = TRUE;
         }
         ...
         if ( psExpr->lElementClass == qVARIABLE )
         {
            ...
            // if ( psExpr->lDataClass == qNUMERIC )
               if ( psExpr->lDataClass == qNUMERIC || psExpr->lDataClass == qVARIABLE )
               {
                  ...
                  {
                     // Take the view closest to the start of the parameter list as the qualifying
                     // view unless it is a return view.
      ...
      if ( bOperation )
      {
         if ( szThrowsException[ 0 ] == 'Y' )
         {
            ...
            zstrcpy( szStringBuilder1 + nIndent + 3, "throw ZeidonException.wrapException( e );\n" );
            nLth = zstrlen( szStringBuilder1 );
            nOutputIdx -= nLth;
            ...
            {
               nLth = zstrlen( szThrowsExceptionCatchCode );
               nOutputIdx -= nLth;
            ...
            zstrcpy( szStringBuilder1 + nIndent, "catch ( Exception e )\n" );
            nLth = zstrlen( szStringBuilder1 );
            nOutputIdx -= nLth;
            zmemcpy( g_pchOutputExprStr + nOutputIdx + 1, szStringBuilder1, nLth );
            g_pchOutputExprStr[ nOutputIdx-- ] = '\n';
         ...
         // zmemset( g_pchOutputExprStr + nOutputIdx + 1, ' ', nIndent );
            ...
            zstrcpy( szStringBuilder1 + nIndent + 8, szOperationGroup );
            zstrcat( szStringBuilder1, " = null;  // permit gc  (unnecessary)\n" );
            nLth = zstrlen( szStringBuilder1 );
         ...
         // zmemset( szNewOperationClassStatement, ' ', nIndent );
            ...
            zstrcpy( szNewOperationClassStatement + nIndent + 3, szOperationGroup );
            zstrcat( szNewOperationClassStatement, " m_" );
            zstrcat( szNewOperationClassStatement, szOperationGroup );
            zstrcat( szNewOperationClassStatement, " = new " );
            zstrcat( szNewOperationClassStatement, szOperationGroup );

            if ( nFirstViewIdx >= 0 )
            {
               fnAddView( vSubtask, szNewOperationClassStatement + zstrlen( szNewOperationClassStatement ),
                          (sQExprStruct *) ((zPCHAR) szExprCode + nFirstViewIdx), szPFlag, szExprText );
            }
            else
            {
               zCHAR  szName[ 33 ];
               ...
               while ( nRC >= zCURSOR_SET )
               {
                  ...
                  if ( lDataType == qTVIEW )
                  {
                     ...
                     {
                        GetStringFromAttribute( szName, g_lpPIView, "Variable", "Name" );
                        break;
                     }
                  }
               ...
               if ( szName[ 0 ] )
               {
                  zstrcat( szNewOperationClassStatement, "( " );
                  zstrcat( szNewOperationClassStatement, szName );
                  zstrcat( szNewOperationClassStatement, " );\n" );
               }
               else
                  zstrcat( szNewOperationClassStatement, "( task );\n" );  // punt
            }
         ...
         if ( (/* nReturnCodeIdx >= 0 && */ nReturnVarIdx >= 0 && nReturnCodeIdx != nReturnVarIdx) || nReturnParmCnt > 0 )
         {
            if ( nReturnParmCnt > 0 )  // we at least found one return variable
            {
               ...
               // Extra return variables are either of type String or Int or Double.
               // We will turn them into StringBuilder or MutableInt or MutableDouble.
               while ( nIdx < nReturnParmCnt )
               {
                  nExprIdx = nReturnParmIdx[ nIdx ];
                  psExpr = (sQExprStruct *) ((zPCHAR) szExprCode + nExprIdx);
                  szVarName[ sizeof( szVarName ) - 1 ] = 0;
                  k = AddExprEntryToString( vSubtask, psExpr, szVarName, szPFlag, sizeof( szVarName ) - 2, szExprText, 0 ); // get return variable
                  while ( TRUE )  // purist's goto
                  {
                     ...
                     {
                        zstrcat( szStringBuilder1, "StringBuilder sb_" );
                        zstrcat( szStringBuilder1, szVarName + k );
                        zstrcat( szStringBuilder1, ";\n" );
                        nLth = zstrlen( szStringBuilder1 );
                        zmemset( szStringBuilder1 + nLth, ' ', nIndent );
                        nLth += nIndent;
                        szStringBuilder1[ nLth ] = 0;
                        zstrcat( szStringBuilder1, "if ( " );
                        zstrcat( szStringBuilder1, szVarName + k );
                        zstrcat( szStringBuilder1, " == null )\n" );
                        nLth = zstrlen( szStringBuilder1 );
                        zmemset( szStringBuilder1 + nLth, ' ', nIndent + 3 );
                        nLth += nIndent + 3;
                        szStringBuilder1[ nLth ] = 0;
                        zstrcat( szStringBuilder1, "sb_" );
                        zstrcat( szStringBuilder1, szVarName + k );
                        zstrcat( szStringBuilder1, " = new StringBuilder( 32 );\n" );
                        nLth = zstrlen( szStringBuilder1 );
                        zmemset( szStringBuilder1 + nLth, ' ', nIndent );
                        nLth += nIndent;
                        szStringBuilder1[ nLth ] = 0;
                        zstrcat( szStringBuilder1, "else\n" );
                        nLth = zstrlen( szStringBuilder1 );
                        zmemset( szStringBuilder1 + nLth, ' ', nIndent + 3 );
                        nLth += nIndent + 3;
                        szStringBuilder1[ nLth ] = 0;
                        zstrcat( szStringBuilder1, "sb_" );
                        zstrcat( szStringBuilder1, szVarName + k );
                        zstrcat( szStringBuilder1, " = new StringBuilder( " );
                     }
                     else
                     if ( lReturnParmType[ nIdx ] == qTDECIMAL )
                     {
                        zstrcat( szStringBuilder1, "MutableDouble md_" );
                        zstrcat( szStringBuilder1, szVarName + k );
                        zstrcat( szStringBuilder1, " = new MutableDouble( " );
                     }
                     else
                     {
                        zstrcat( szStringBuilder1, "MutableInt mi_" );
                        zstrcat( szStringBuilder1, szVarName + k );
                        zstrcat( szStringBuilder1, " = new MutableInt( " );
                     }
                     ...
                     nLth = zstrlen( szStringBuilder2 );
                     zmemset( szStringBuilder2 + nLth, ' ', nIndent );
                     zstrcpy( szStringBuilder2 + nLth + nIndent, szVarName + k );
                     if ( lReturnParmType[ nIdx ] == qTSTRING )
                        zstrcat( szStringBuilder2, " = sb_" );
                     else
                     if ( lReturnParmType[ nIdx ] == qTDECIMAL )
                        zstrcat( szStringBuilder2, " = md_" );
                     else
                        zstrcat( szStringBuilder2, " = mi_" );

                     zstrcat( szStringBuilder2, szVarName + k );
                     if ( lReturnParmType[ nIdx ] == qTSTRING )
                        zstrcat( szStringBuilder2, ".toString( )" );
                     else
                     if ( lReturnParmType[ nIdx ] == qTDECIMAL )
                        zstrcat( szStringBuilder2, ".doubleValue( )" );
                     else
                        zstrcat( szStringBuilder2, ".intValue( )" );
            ...
            nOutputIdx = AddExprEntryToString( vSubtask, psExpr, g_pchOutputExprStr, szPFlag,
                                               nOutputIdx, szExprText, lFlag );
   ...
   while ( nExprIdx >= 0 )
   {
      ...
      if ( (sCurrExpr.lFlags & SKIP_BIT) == 0 )
      {
         ...
         {
            // If the WorkExpr contains an operator, add it to the infix (push the "index
            // into expression" onto the values stack).
            if ( sCurrExpr.lElementClass == qOPERATOR ) // 1034
            {
                 ...
                 // sCurrExpr.lOffset == qOR )
               {
                  ...
                  nOutputIdx = AddExprEntryToString( vSubtask, &sCurrExpr, g_pchOutputExprStr, szPFlag,
                                                     nOutputIdx, szExprText, lFlag );
                  ...
                  switch ( sCurrExpr.lOffset )
                  {
                     ...
                     case qCOMMA:
                        ...
                        // DM - Currently we are using the presence of a comma to indicate the
                        // presence of a function call in an expression.  Although C allows for
                        // commas inside for loop statements, I am assuming that no such for
                     ...
                     case qOPENOPER:
                        ...
                        // Turn off the flag to indicate that we are no longer "in" an operation
                        // call (processing operation arguments).
                     ...
                     case qCLOSEOPER:
                        ...
                        // Set flag indicating that we are "in" an operation call (i.e. current
                        // expressions are operation arguments).
                        lFlag |= zOPERATION_ARGUMENTS;
               ...
               else
               {
                  ...
                  {
                     // Note that for example, qEQUAL is 3001, qADDITION is 3003, qAND is 1051, qOR is 1052 ...
                     // so their indices get pushed here.
                     PUSH( nExprIdx );
                  }
               }
            }
            ...
            {
               // The Expression structure retrieved is a variable or constant (not an operator), so
               // we are copying it to the output string (backwards, starting at the end).
                    ...
                    sCurrExpr.lElementClass == qOPERATIONCALL )        // 2008
               {
                  ...
                  // If we are generating a return statement containing a decimal constant,
                  // a (ZDecimal) cast must be generated.
                  if ( bReturn &&
                       sCurrExpr.lElementClass == qCONSTANT && sCurrExpr.lDataType == qTDECIMAL )
                  {
                     if ( g_szGenLang[ 0 ] == 'J' )
                     {
                        nOutputIdx -= 9;
                  ...
                  while ( bUnaryContinue && (nTopOfStack = POP) > 0 )
                  {
                          ...
                          (nReturnVarIdx >= 0 && nReturnParmCnt > 0 && nReturnCodeIdx >= 0) )
                     {
                        ...
                        if ( g_szGenLang[ 0 ] == 'J' && nExprIdx - (sizeof( sQExprStruct ) - MAXSTRING) == 0 && bOperation == FALSE )
                        {
                           ...
                           szStringBuilder2[ sizeof( szStringBuilder2 ) - 1 ] = 0;
                           k = AddExprEntryToString( vSubtask, psExpr, szStringBuilder2, szPFlag, sizeof( szStringBuilder2 ) - 2, szExprText, 0 ); // get return variable
                           if ( (psExpr->lDataType == qTINTEGER || psExpr->lDataType == qTSHORT) && szPFlag[ 0 ] == 'Y' )  // 1040 || 1041
                           {
                              zstrcpy( szStringBuilder2, szStringBuilder2 + k + 1 );
                              k = zstrlen( g_pchOutputExprStr + nOutputIdx + 1 );
                              if ( k < sizeof( szStringBuilder2 ) - 36 )
                              {
                                 zstrcat( szStringBuilder2, ".setValue(" );
                                 zstrcat( szStringBuilder2, g_pchOutputExprStr + nOutputIdx + 1 );
                                 zstrcat( szStringBuilder2, " )" );
                                 nOutputIdx -= zstrlen( szStringBuilder2 ) - k; // for the setValue and parens
                                 zstrcpy( g_pchOutputExprStr + nOutputIdx + 1, szStringBuilder2 );
                                 nExprIdx = 0;
                                 g_pchOutputExprStr[ nOutputIdx-- ] = ' ';
                                 break;
                              }
   ...
   {
      zstrcpy( szStringBuilder1, pchOutputString + nIndent );
      zstrcpy( pchOutputString + nIndent, "if(8==8)" );
      zstrcat( pchOutputString, szStringBuilder1 );
   ...
   zstrcat( pchOutputString, g_pchOutputExprStr + nOutputIdx + 1 );

   if ( g_szGenLang[ 0 ] == 'J' )
   {
      if ( (pch = zstrstr( pchOutputString, " > null" )) != 0 ||
           (pch = zstrstr( pchOutputString, " < null" )) != 0 )
      {
         if ( zstrlen( pch + 2 ) < sizeof( szStringBuilder1 ) )
         {
            zstrcpy( szStringBuilder1, pch + 2 );
            pch++;
            *pch = '!';
            pch++;
            *pch = '=';
            zstrcpy( pch + 1, szStringBuilder1 );
         }
      }
...
EXPRESSION_EXIT_label:
   ...
   if ( (pch = zstrstr( pchOutputString, ".setNextContinue( \"" )) != 0 )
   {
      ...
      if ( pch2 )
         zstrcpy( pch + 18, pch2 );  // eliminate any and all parms on setNextContinue
   }
...
// pch1 = strchr( pch + nMargin, '(' );  // was this, but not great since it forces the next line to have a big margin  dks 2009.10.29
   ...
   for( ;; )
   {
      ...
      // Now look for a position where a line break can be inserted.  First, change everything
      // inside quotes or apostrophes to 'xff' so we don't find our special break characters
      // within quotes or apostrophes.
      zstrcpy( szTemp, pch ); // create temporary copy for changing everything inside "" and ''.
      ...
      if ( pch1 <= szTemp + nMargin )
      {
         ...
         else
         {
            ...
            // We could not find a good insert position so we do it with a backslash (no indentation
            // on following line).
            // Count the xff's following szTemp + 253.  If we are in a very long string, we continue
            // backslash processing until the whole quoted string is done.

#if 1
            while ( szTemp[ 253 + n255 ] == '\xff' )
               n255++;

            zstrcpy( szTemp, pch + 253 );
            *(pch + 253) = '\\';
            *(pch + 254) = '\n';
                 ...
                 zBOOL  bQuotes )
{
   ...
   zstrcpy( pchSource, pchOutputString );

   // Count the blanks, starting from insert position.
   i = strspn( pchInsertPosition, " " );
   ...
   *pchInsertPosition++ = '\n';
   ...
   // Copy rest of line.
   zstrcpy( pchInsertPosition, pchSource + lInsertPosition );
                      ...
                      zLONG          lFlag )
{
   ...
   // If the expr struct contains information about a variable, then get the offset since
   // this is the ID number of the variable and must be retrieved.
   switch ( psExpr->lElementClass )
   {
      ...
      // if ( psExpr->lOffset < 0 )  // preserve the structure setting
         ...
         {
            // DGC 3/19/98
            // If we get here it appears that the current variable is an argument to an
            // operation and the argument is an address to something.
            bPointer = TRUE;
         // psExpr->lOffset *= -1;
            lOffset *= -1;
            ...
            if ( g_szGenLang[ 0 ] != 'J' )  // don't do any of this if generating Java
            {
               ...
               // Check to see if variable an argument to a operation and if the arguments changes.
               // This means that we're passing the argument as a pointer.
         ...
         GetStringFromAttribute( szVarName + nVarIndex, g_lpPIView, "Variable", "Name" );
         GetStringFromAttribute( pchPFlag, g_lpPIView, "Variable", "PFlag" );
      // if ( 0 )
      // {
      //    SetNameForView( g_lpPIView, "g_lpPIView", vSubtask, zLEVEL_TASK );
      ...
      // }
         ...
         if ( g_szGenLang[ 0 ] == 'J' )  // if generating Java
         {
            if ( lIsParm == 2 && pchPFlag[ 0 ] == 'Y' && psExpr->lOffset >= 0 )
            {
               ...
               {
                  zstrcat( szVarName + nVarIndex, ".toString( )" );
               }
               else
               if ( psExpr->lDataType == qTINTEGER || psExpr->lDataType == qTSHORT )
               {
                  zstrcat( szVarName + nVarIndex, ".intValue( )" );
               }
               else
               if ( psExpr->lDataType == qTDECIMAL )
               {
                  zstrcat( szVarName + nVarIndex, ".doubleValue( )" );
               }
            }
         }
         else
         {
            ...
            if ( psExpr->lDataType == qTINTEGER && (lFlag & zOPERATION_ARGUMENTS) &&
                 bPointer && (psExpr->lFlags & UNSIGNED_BIT) )
            {
               // We are generating a pointer to a long in an operation call.  As the definition
               // is "unsigned" we have to add a "(zPULONG)" cast.  HH,16.7.1998
               zstrcpy( szVarSave, szVarName );
               zstrcpy( szVarName, "(zPULONG) ");
               zstrcat( szVarName, szVarSave );
            }

            if ( psExpr->lDataType == qTSHORT &&
                 (lFlag & zOPERATION_ARGUMENTS) &&
                 bPointer &&
                 (psExpr->lFlags & UNSIGNED_BIT) )
            {
               // We are generating a pointer to a short in an operation call.  As the definition
               // is "unsigned" we have to add a "(zPUSHORT)" cast.  RG,03.2.1999
               zstrcpy( szVarSave, szVarName );
               zstrcpy( szVarName, "(zPUSHORT) ");
               zstrcat( szVarName, szVarSave );
            }
         }
         ...
         GetIntegerFromAttribute( &lDataType, g_lpPIView, "Variable", "DataType" );
         if ( (psExpr->lDataType != qTINTEGER || lDataType != qTINTEGER) &&
              (lFlag & zOPERATION_ARGUMENTS) &&
              (psExpr->lFlags & NPARM_TYPE_MASK) == NPARM_TYPE_LONG )
         {
            ...
            if ( (psExpr->lDataType != qTDECIMAL || lDataType != qTDECIMAL) &&
                 (lFlag & zOPERATION_ARGUMENTS) &&
                 ...
                 bPointer == FALSE )
            {
               ...
               zstrcpy( szVarSave, szVarName );
               if ( g_szGenLang[ 0 ] == 'J' )
                  zstrcpy( szVarName, "(double) ");
               else
                  zstrcpy( szVarName, "(ZDecimal) ");
         ...
         // we must prefix the "something" string with a (ZDecimal) cast.  Otherwise we get a failure
         // if the right side is a numeric constant.  HH,17.6.1998
         if ( psExpr->lDataClass == qNUMERIC &&
              psExpr->lDataType == qTDECIMAL &&
              szVarName[ 0 ] == '*' )
         {
            ...
            if ( pchOutputExprStr[ nOutputIdx ] == '=' )
            {
               ...
               if ( g_szGenLang[ 0 ] == 'J' )
                  zstrcat( szVarName, " = (double)");
               else
                  zstrcat( szVarName, " = (ZDecimal)" );
            }
            else
      ...
      case qDEFINE:  // 1060
         GetDefineExternalValue( szVarName, psExpr->lDataClass, psExpr->lOffset );
         break;

      case qOPERATIONCALL:  // 2008
         if ( psExpr->lOffset > MAXSTRING )
         {
            GetOperationNameFromZKey( vSubtask, szVarName, szVarSave, szThrowsException, szThrowsExceptionCatchCode, szOperationGroup, psExpr->lDataClass, psExpr->lOffset );
         }
         else
         {
            zstrcpy( szVarName, szExprText + psExpr->lOffset );
         }
      ...
      case qCONSTANT:  // 1010
         ...
         //      psExpr->lDataType != qTCHARACTER )
            ...
            {
               zstrcpy( szVarName, "\"" );
               StrConcatAndMassage( szVarName, szExprText + psExpr->lOffset );
               zstrcat( szVarName, "\"" );
            }
            else
            {
         ...
         else
         {
            ...
            if ( psExpr->lDataType == qTDECIMAL &&
                 (lFlag & zOPERATION_ARGUMENTS) &&
                 strchr( szExprText + psExpr->lOffset, '.' ) != 0 )
            {
               // We are generating a decimal constant in an operation call.  The function
               // prototype normally has a zDECIMAL at the corresponding place. We need a
               // cast to ZDecimal to get working C code.  HH,16.6.1998
               if ( g_szGenLang[ 0 ] == 'J' )
                  zstrcat( szVarName, "(double) ");
               else
                  zstrcat( szVarName, "(ZDecimal) ");
                 ...
                 (psExpr->lFlags & NPARM_TYPE_MASK) == NPARM_TYPE_DECIMAL )
            {
               if ( g_szGenLang[ 0 ] != 'J' )  // don't do any of this if generating Java
               {
                  // We are generating a non-decimal constant in an operation call, where the
                  // target argument is decimal.  Generate a cast to ZDecimal.
                  if ( g_szGenLang[ 0 ] == 'J' )
                     zstrcat( szVarName, "(double) ");
                  else
                     zstrcat( szVarName, "(ZDecimal) ");
            ...
            zstrcat( szVarName, szExprText + psExpr->lOffset );
         }
   ...
   nVarNameLength = zstrlen( szVarName );
...
// g_lpPIView is the TZVSP000 object created by parse.
   ...
   switch ( lOperationClass )
   {
      case qLOCALOPERATION:
         // Use source meta (LOD or Dialog) to find out whether a C operation is called from
         // VML ... not for "GLOBAL Operations"
         ...
         lRC = SetCursorFirstEntityByInteger( vLookupView, "Operation",
                                              "ZKey", lZKey, szParentEntityName );

         if ( lRC >= zCURSOR_SET )
         {
            // If it is a C operation, just return the name.  Otherwise, go to the XPG object
            // and get the "C generated" name.
         // szType[ 0 ] = 0;
            GetStringFromAttribute( szType, vLookupView, "SourceFile", "LanguageType" );

            if ( szType[ 0 ] == 'C' )
            {
               // Source type is C.
               GetStringFromAttribute( pchReturnName, vLookupView, "Operation", "Name" );
            ...
            else
            {
               ...
               {
                  GetStringFromAttribute( pchReturnName, vLookupView,
                                          "Operation", "C_GeneratedOperationName" );
                  DropView( vLookupView );
                  return( 0 );
               }
            }
      ...
      case qOBJECTOPERATION:
         ...
         if ( lRC >= zCURSOR_SET )
         {
            ...
            ActivateMetaOI_ByZKey( vSubtask, &vLookupView, 0, zREFER_LOD_META,
                                   zSINGLE, lSourceZKey, zCURRENT_OI );
            ...
            if ( lRC > zCURSOR_UNCHANGED )    // found
            {
               ...
               if ( g_szGenLang[ 0 ] == 'J' )
               {
                  ...
                  zstrcat( pchOperationGroup, "_Object" );
               ...
               GetStringFromAttribute( pchReturnName + nLth, vLookupView,
                                       "Operation", "C_GeneratedOperationName" );
      ...
      case qGLOBALOPERATION:
      {
         ...
         lRC = ActivateMetaOI( vSubtask, &vLookupView, vTempList,
   ...
   if ( lRC > zCURSOR_UNCHANGED )    // found
   {
      ...
      else
      {
         ...
         if ( g_szGenLang[ 0 ] == 'J' )
         {
            ...
            {
               GetStringFromAttribute( pchReturnHeaderName, vLookupView, "HeaderFile_Oper", "Name" );
            }
            else
            {
               pchReturnName[ 0 ] = 'm';
               pchReturnName[ 1 ] = '_';
               if ( GetStringFromAttribute( pchOperationGroup, vLookupView, "GlobalOperationGroup", "Name" ) != 0 )
               {
               ...
               zstrcat( pchOperationGroup, "_Operation" );
               zstrcpy( pchReturnName + 2, pchOperationGroup );
               nLth = zstrlen( pchReturnName );
               pchReturnName[ nLth++ ] = '.';
               pchReturnName[ nLth ] = 0;
         ...
         GetStringFromAttribute( pchReturnName + nLth, vLookupView, "Operation", "Name" );
      }
   ...
   zstrcpy( pchReturnName, "OperZKey_" );
   zltoa( lZKey, pchReturnName + 9 );
   zstrcat( pchReturnName, "_NotFound" );
   MiGetParentEntityNameForView( szParentEntityName, vLookupView, "Operation" );
   TraceLine( "Unable to find ZKey: %d in LookupView 0x%08x under parent: %s ... See OI Display",
              lZKey, vLookupView, szParentEntityName );
                          ...
                          zLONG   lZKey )
{
   ...
   if ( lRC > zCURSOR_UNCHANGED )    // found
   {
      GetStringFromAttribute( pchReturnName, lpZOListView,
                              "Operation", "Name" );
                        ...
                        zLONG  lDefineZKey )
{
   ...
   while ( lRC == zCURSOR_SET )
   {
      ...
      if ( lRC == zCURSOR_SET )
      {
         GetStringFromAttribute( pchExternalValue, vLookupView, "DefinedItem", "ExternalValue" );
         DropView( vLookupView );
         return( 0 );
      }
