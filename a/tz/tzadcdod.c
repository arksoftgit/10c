...
zwTZADCDOD_DeleteMaintAttrib( zVIEW vSubtask )
{
   ...
   {
      zstrcpy( szMsg, "OK to delete selected Attributes?" );
...
zwTZADCDOD_DeleteTabAttrib( zVIEW vSubtask )
{
   ...
   {
      // Ask the operator if it is ok to delete.
      GetStringFromAttribute( szName, vUserSpec, "MT_ER_Attribute", "Name" );
      zstrcpy( szMsg, "OK to delete Attribute '" );
...
zwTZADCDOD_LoadFileList( zVIEW vSubtask )
{
        ...
        CheckExistenceOfEntity( vUserSpec, "UI_Spec" ) == 0 )
   {
      ...
      {
         GetStringFromAttribute( szTempString_0, vUserSpec,
                                 "UI_Spec", "Name" );
...
zwTZADCDOD_SaveSaveAs( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szNewObjectName, vNewObj, "UI_Spec", "Name" );

   nRC = SetCursorFirstEntityByString( vFileSpec, "W_MetaDef",
                                       "Name", szNewObjectName, "" );
   if ( nRC > zCURSOR_UNCHANGED )
   {
      zstrcpy( szWorkString, szNewObjectName );
      zstrcat( szWorkString, " - Style object already exists, Overwrite?" );
   ...
   else
   {
      ...
      if ( nRC == -1 )
      {
         zstrcpy( szWorkString, "Check-out failed on newly created Object - " );
         zstrcat( szWorkString, szNewObjectName );
...
zwTZADCDOD_OpenStyleFile( zVIEW vSubtask )
{
                                   ...
                                   "InitializeOpen", 1 ) == 0 )
   {
      ...
      GetStringFromAttribute( szWorkString, vWDWorkObj, "Palette", "UIS_Name" );
      ...
      GetStringFromAttribute( szWorkString, vFileObj, "W_MetaDef", "Name" );
   ...
   GetStringFromAttribute( szObjectName, vUserSpec, "UIS_LOD", "Name" );
   ...
   if ( lRC == -1 )
   {
      zstrcpy( szWorkString, "Object, " );
      zstrcat( szWorkString, szObjectName );
      zstrcat( szWorkString, " Not Found" );
...
zwTZADCDOD_DeleteStyleFile( zVIEW vSubtask )
{
   ...
   zstrcpy( WorkString, "Delete Dialog Spec '" );
   GetStringFromAttribute( szTempString_0, vFileObj, "W_MetaDef", "Name" );
   zstrcat( WorkString, szTempString_0 );
   zstrcat( WorkString, "' ?" );
...
zwTZADCDOD_CreateNewStyle( zVIEW vSubtask )
{
   ...
   GetStringFromAttribute( szStyleName, vSDWorkObj, "ROOT", "STYLE_NAME" );
   ...
   zstrcpy( szWorkString, szStyleName );
   ...
   {
      zstrcat( szWorkString, " - Dialog Spec Exists, Open existing Dialog Spec?" );
   ...
   lRC = ActivateMetaOI( vSubtask, &vNewObj, vFileObj, zREFER_LOD_META,
                         zSINGLE | zLEVEL_APPLICATION );
   GetStringFromAttribute( szObjectName, vNewObj, "LOD", "Name" );
   ...
   zstrcat( szObjectName, ".LOD" );
   if ( lRC < 0 )
   {
      zstrcpy( szWorkString, "The dialog spec object definition - " );
      zstrcat( szWorkString, szObjectName );
                                 ...
                                 zVIEW    vUIS_List )
{
   ...
   GetStringFromAttribute( szRootIndicator,
                           vUserSpec, "UIS_Entity", "RootIndicator" );
   GetStringFromAttribute( szParentRootIndicator,
                           vUS_Parent, "UIS_Entity", "RootIndicator" );
   if ( CheckExistenceOfEntity( vLOD, "ER_RelLink" ) >= zCURSOR_SET )
      GetStringFromAttribute( szCardMax, vLOD, "ER_RelLink", "CardMax" );
   if ( CheckExistenceOfEntity( vLOD, "ER_Entity" ) >= zCURSOR_SET )
      GetStringFromAttribute( szPurpose, vLOD, "ER_Entity", "Purpose" );
   if ( CheckExistenceOfEntity( vLOD_Parent, "ER_Entity" ) >= zCURSOR_SET )
      GetStringFromAttribute( szParentPurpose, vLOD_Parent, "ER_Entity", "Purpose" );

   GetStringFromAttribute( szParentInclude, vLOD_Parent, "LOD_Entity", "Include" );
   GetStringFromAttribute( szInclude, vLOD, "LOD_Entity", "Include" );
   GetStringFromAttribute( szCreate, vLOD, "LOD_Entity", "Create" );
   GetStringFromAttribute( szUpdate, vLOD, "LOD_Entity", "Update" );
   GetStringFromAttribute( szDelete, vLOD, "LOD_Entity", "Delete" );
...
zwTZADCDOD_RefreshDialogStyle( zVIEW  vSubtask )
{
   ...
   GetStringFromAttribute( szSpecName, vUserSpec, "UI_Spec", "Name" );
...
zwTZADCDOD_AcceptCaseX( zVIEW vSubtask )
{
   ...
   {
      GetStringFromAttribute( szViewName, vUserSpec, "ListViewObjRef", "Name" );
   ...
   {
      GetStringFromAttribute( szViewName, vUserSpec, "IncludeViewObjRef", "Name" );
      if ( SetCursorFirstEntityByString( vUserSpecRoot, "UIS_ViewObjRef", "Name",
                                         szViewName, 0 ) < zCURSOR_SET )
         IncludeSubobjectFromSubobject( vUserSpecRoot, "UIS_ViewObjRef",
...
zwTZADCDOD_Delete_I_LOD_Attrib( zVIEW vSubtask )
{
   ...
   {
      // Ask the operator if it is ok to delete.
      zstrcpy( szMsg, "OK to delete selected Attribute?" );
      if ( MessagePrompt( vSubtask,                 "CM00111",
...
// CPoint pt( GetMessagePos( ) );
   ...
   GetViewByName( &vUserSpec, "User_Spec", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szType, vUserSpec, "UI_Spec", "Type" );
...
zwTZADCDOD_GoToCaseDetail( zVIEW    vSubtask )
{
   ...
   GetStringFromAttribute( szCaseType, vUserSpec,
...
zwTZADCDOD_SelectCase( zVIEW    vSubtask )
{
   ...
   GetStringFromAttribute( szOrigCaseType,
                           vUserSpec, "UIS_Entity", "AutodesignCaseType" );
   GetStringFromAttribute( szNewCaseType,
...
zwTZADCDOD_IncludeListVOR( zVIEW vSubtask )
{
   ...
   else
   {
      ...
      GetStringFromAttribute( szVOR_Name, vListVOR, "W_MetaDef", "Name" );
