         ...
         CFormView( IDD_INPUTFORM )  // instead of CScrollView to get TABBING
{
   ...
   if ( m_pZSubtask )
   {
      if ( m_pZSubtask && (m_pZSubtask->m_pZTask->m_nTraceAction & 0x74) == 0x74 )
      {
         ...
         zsprintf( szMsg, "ZDrView::ctor (0x%08x): %d  Subtask: (%s) %d  View: %d",
                   this, m_ulViewIdNbr, *(m_pZSubtask->m_pzsWndTag),
                   pZSubtask->m_ulSubtaskIdNbr, pZSubtask->m_pZView ?
                                          pZSubtask->m_pZView->m_ulViewIdNbr : 0 );
         if ( pZSubtask->m_pZView )
         {
            zstrcat( szMsg, " ##### Original view: " );
...
// m_bInitialized = FALSE;
   ...
   SysReadZeidonIni( -1, "[ActiveX]",
                     "TraceAmbientPropertyRequest", pszStorageFile );
...
// m_apList.Add( pProp );
   ...
   if ( m_pZSubtask && (m_pZSubtask->m_pZTask->m_nTraceAction & 0x74) == 0x74 )
   {
      ...
      zsprintf( szMsg, "ZDrView::InitAmbientProperties (0x%08x) PropertyCnt: ", this );
...
//#ifndef QUINSOFT
   ...
   SysReadZeidonIni( -1, "[Debug]", "UseReportFixZooming", szEnableZooming );
