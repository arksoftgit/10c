   ...
   *pZSubtaskReturn = 0;
   ...
   if ( cpcCtrlTag )
   {
      ...
      if ( (ulSubtaskState & zSUBTASK_STATE_DELETED) != zSUBTASK_STATE_DELETED )
      {
         if ( (*pZSubtaskReturn)->m_pZMIXCtrl == 0 )
         {
            if ( (*pZSubtaskReturn)->m_pZTask->m_nTraceAction > 1 )
            {
               ...
               zsprintf( szMsg,
                         ...
                         *(*pZSubtaskReturn)->m_pzsDlgTag,
                         *(*pZSubtaskReturn)->m_pzsWndTag, cpcCtrlTag );
         ...
         *pzmaReturn = (ZMapAct *)
                        (*pZSubtaskReturn)->m_pZMIXCtrl->SeekNode( &csSeek );
         if ( *pzmaReturn )
         {
                            ...
                            zCPCHAR cpcCtrlTag )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      zPCHAR pch = new char[ zstrlen( cpcCtrlTag ) + 1 ];
      zstrcpy( pch, cpcCtrlTag );
...
GetLastDlgWnd( zVIEW vSubtask, zPCHAR pchDlgTag, zPCHAR pchWndTag )
{
   ...
   *pchDlgTag = *pchWndTag = 0;
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      {
         zstrcpy( pchDlgTag, *(pZSubtask->m_pZTask->m_pzsLastDlgTag) );
         zstrcpy( pchWndTag, *(pZSubtask->m_pZTask->m_pzsLastWndTag) );
         return( 0 );
      }
   }
            ...
            zLONG   lFlag )
{
   ...
   // zCHAR szMsg[ 256 ];
   //
   // zsprintf( szMsg, "SplitFrame: %s ZSubtask: 0x%08x  ZView: 0x%08x",
                   ...
                   zSHORT  nMaxFileNameLth )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      if ( pZSubtask->m_pZTask && pZSubtask->m_pZFWnd )
      {
         ...
         {
            zstrncpy( pchReturnFileName, cpcFileName, nMaxFileNameLth );
         }
         else
         {
            ...
            if ( szZeidonPath[ 0 ] == 0  )
               zstrcpy( szZeidonPath, "c:\\temp\\" );

            SysAppendcDirSep( szZeidonPath );
            SysConvertEnvironmentString( szWorkString, szZeidonPath );
            GenerateUniqueFileName( pchReturnFileName,
                                    nMaxFileNameLth,
                                    szWorkString, "Twain", "bmp" );
         }
                  ...
                  zCPCHAR cpcWndTag )
{
      ...
      for ( zLONG lPos = 0; lPos < lItemCnt; lPos++ )
      {
         if ( g_pSubtaskList[ lPos ].pZSubtask &&
              ...
              (zstricmp( g_pSubtaskList[ lPos ].DlgN, cpcDlgTag ) == 0 ||
               zstricmp( g_pSubtaskList[ lPos ].
                                      pZSubtask->m_pZTask->m_szInitialDlgName,
                         cpcDlgTag ) == 0) )
         {
            return( (zVIEW)
   ...
   if ( vSubtask == 0 )
   {
      ...
      fd.pvDialog = 0;
      zstrcpy( fd.DlgN, cpcDlgTag );
      if ( cpcWndTag )
         zstrcpy( fd.WndN, cpcWndTag );
      else
         fd.WndN[ 0 ] = 0;
                    ...
                    zPCHAR pchCtrlName )
{
   ...
   *pchDlgName = *pchWndName = *pchCtrlName = 0;
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      zstrcpy( pchDlgName, *(pZSubtask->m_pzsDlgTag) );
      zstrcpy( pchWndName, *(pZSubtask->m_pzsWndTag) );
      if ( pZSubtask->m_pzmaWithFocus )
         zstrcpy( pchCtrlName, *(pZSubtask->m_pzmaWithFocus->m_pzsTag) );
...
GetMDI_Client( zVIEW vSubtask )
{
        ...
        pZSubtask->m_pMDI_Client )
   {
      ...
      {
         TraceLineI( "Invalid GetMDI_Client ",
                     (zLONG) pZSubtask->m_pMDI_Client->m_hWnd );
      }
      else
      {
         TraceLineI( "Returning valid GetMDI_Client ",
                     (zLONG) pZSubtask->m_pMDI_Client->m_hWnd );
      }
                          ...
                          zSHORT   nReturnStringLth )
{
   ...
   pchReturnString[ 0 ] = 0;
   GetApplDirectoryFromView( szAppName, vSubtask,
                             zAPPL_NAME, sizeof( szAppName ) );
   zsprintf( szAppSection, "[App.%s]", szAppName );

   SysReadZeidonIni( -1, szAppSection, (zPCHAR) cpcKey, szValue );
   if ( szValue[ 0 ] == 0 )
   {
      SysReadZeidonIni( -1, "[Workstation]", (zPCHAR) cpcKey, szValue );
   }

   if ( szValue[ 0 ] )
   {
      zstrncat( pchReturnString, szValue, nReturnStringLth );
      return( TRUE );
   }
                        ...
                        zSHORT   nReturnStringLth )
{
   ...
   if ( SfGetApplicationForSubtask( &pApp, vSubtask ) == 0 && pApp )
      zstrcpy( szZeidonRT_Path, pApp->szLocalDir );

   if ( szZeidonRT_Path[ 0 ] )
   {
      SysAppendcDirSep( szZeidonRT_Path );
      zstrcat( szZeidonRT_Path, "ZeidonRT.ini" );
      GetPrivateProfileString( cpcSection, cpcKey, "", pchReturnString,
                               nReturnStringLth, szZeidonRT_Path );
   }

   if ( pchReturnString[ 0 ] == 0 )
   {
      SysReadZeidonIni( -1, "[Workstation]", "LocalDir", szZeidonRT_Path );
      if ( szZeidonRT_Path[ 0 ] )
      {
         SysAppendcDirSep( szZeidonRT_Path );
         zstrcat( szZeidonRT_Path, "ZeidonRT.ini" );
      ...
      {
         zCHAR  szValue[ 16000 ];
         zSHORT nLth = zstrlen( cpcSection ) + 1;
         zPCHAR pch = new char[ nLth + 2 ];

         zstrcpy( pch + 1, cpcSection );
         pch[ 0 ] = '['; pch[ nLth ] = ']'; pch[ nLth + 1 ] = 0;
         SysReadZeidonIni( -1, pch, (zPCHAR) cpcKey, szValue );
         zstrncat( pchReturnString, szValue, nReturnStringLth );
         delete [] pch;
      }
   }
                                 ...
                                 zPLONG   plVal10 )
{
   ...
   pchReturnString[ 0 ] = 0;
   zstrncat( pchReturnString, pch, nReturnStringLth );
   pch = pchComma;
