         ...
         int       nCmdShow )
{
   ...
   zstrcpy( szBody, "This is a plain Text EMAIL" );
   ...
   zstrcpy( szBody, "This is aNOTHER plain Text EMAIL with Attachment");
...
{
   zPCHAR pchAtFileName = new char[ zstrlen( cpcFileName ) + 2 ];
   if ( *cpcFileName == '@' )
      zstrcpy( pchAtFileName, cpcFileName );
   else
   {
      pchAtFileName[ 0 ] = '@';
      zstrcpy( pchAtFileName + 1, cpcFileName );
   }
...
{
   zPCHAR pchAngleRecipient = new char[ zstrlen( cpcRecipient ) + 3 ];

   if ( (zstrchr( cpcRecipient, '<' ) == 0) || (zstrchr( cpcRecipient, '>' ) == 0) )
   {
      // Surround with angle brackets.
      pchAngleRecipient[ 0 ] = '<';
      zstrcpy( pchAngleRecipient + 1, cpcRecipient );
      zstrcat( pchAngleRecipient, ">" );
   }
   else
   {
      zstrcpy( pchAngleRecipient, cpcRecipient );
   }
