...
zSHORT
fnGetDIL_Msg( zVIEW vDialog, zPCHAR pchReturnDIL_Msg )
{
   GetVariableFromAttribute( pchReturnDIL_Msg, 0, zTYPE_STRING,
                             256, vDialog,
                             "Control", "DIL_Text", 0, 0 );
   ...
   else
   {
      if ( CheckExistenceOfEntity( vDialog,
                                   "CtrlMapER_Domain" ) >= zCURSOR_SET )
      {
         // DIL_Inherited.
         GetStringFromAttribute( pchReturnDIL_Msg, vDialog,
                                 "Control", "DIL_Inherited" );
      }
      else
      {
   ...
   return( zstrlen( pchReturnDIL_Msg ) );
}
                    ...
                    zBOOL   bDelete )
{
   ...
   zstrcpy( szWork, cpcMsg );
   GetStringFromAttribute( szWork2, vSrcDialog, szlControl, szlTag );
   zstrcat( szWork, szWork2 );
   zstrcat( szWork, " ... on window: " );
   CreateViewFromViewForTask( &vWindow, vSrcDialog, vSubtask );
   while ( ResetViewFromSubobject( vWindow ) == 0 )
   {
   }

   GetStringFromAttribute( szWork2, vWindow, szlWindow, szlTag );
   DropView( vWindow );
   zstrcat( szWork, szWork2 );
   if ( bDelete )
                    ...
                    zCPCHAR cpcBMPName )   // filename (with or without path)
{
   ...
   if ( cpcBMPName )
   {
      ...
      if ( pchPos )
      {
         ...
         {
            TraceLine( "fnBuildRC Skipping Filename: %s  BMPName: %s",
   ...
   *pchRCFileName++ = '\"';
   ...
   if ( bResourcePath )
   {
      ...
      szTempPath[ 0 ] = 0;
      SysReadZeidonIni( -1, "[Workstation]", "ResourcePath", szTempPath );
      SysConvertEnvironmentString( szResourcePath, szTempPath );
      nLth = zstrlen( szResourcePath );
                      ...
                      zLONG   lIdNbr )
{
   if ( lIdNbr == 0 || cpcName[ 0 ] )
   {
      ...
      zSHORT nLth = zstrlen( cpcName ) + zMAX_FILENAME_LTH + 1;
      zPCHAR pchName = new char[ nLth ];

      zstrcpy( pchName, cpcName );
      GetStringFromAttribute( szImage, vApp, "App", "Image" );
      if ( szImage[ 0 ] )
      {
         zPCHAR pchBMP;
         ...
         {
            zstrcpy( pchName + (pchBMP - pch + 1), szImage );
            zstrcat( pchName, pchBMP + 4 );
         }

         delete [] pch;
      }

      GetStringFromAttribute( szImage, vApp, "App", "ImageURL" );
      if ( pchName[ 0 ] )
      {
         if ( szImage[ 0 ] )
         {
            ...
            zstrcpy( pch, szImage );
            SysConvertEnvironmentString( szImage, pch );
            ...
            if ( zstrncmp( pch, szImage, nLth ) == 0 )
               zstrcpy( pchName, pchName + nLth );

            zPCHAR pchColon = zstrchr( pchName, ':' );
            if ( pchColon )
               zstrcpy( pchName, pchColon + 1 );
      ...
      DropView( vApp );
      SetAttributeFromString( vTgtRemote, cpcEntityName,
                              cpcAttributeName, pchName );
      delete [] pchName;
   }
   else
   {
      SetAttributeFromInteger( vTgtRemote, cpcEntityName,
                               cpcAttributeName, lIdNbr );
   }
}
             ...
             zPSHORT nCtrlID )
{
   ...
   if ( ulLth >= 4 + 2 * sizeof( zLONG ) )
   {
      ...
      //    SetAttributeFromString( vTgtRemote, "Label", "Justify", "Left" );
         ...
         {
            if ( ld.szFontName[ 0 ] )
               SetAttributeFromString( vTgtRemote, "Label", "FaceName",
                                       ld.szFontName );

            if ( ld.lEscapement )
               SetAttributeFromInteger( vTgtRemote, "Label",
                                        "Escapement", ld.lEscapement );
         }
      }
   }
            ...
            zPSHORT nCtrlID )
{
   ...
   *((zPLONG) (szBlob + 4 * sizeof( zLONG ))) = 0;  // tab color
   GetBlobFromAttribute( szBlob, &ulLth, vSrcDialog,
...
typedef zVIEW (POPERATION zFARPROC_SETUPMAPLOD)( zVIEW, zVIEW,
                                                 zVIEW, zPCHAR );
...
fnSetUpMapLOD( zVIEW vSubtask, zVIEW vSrcDialog )
{
   ...
   lpfnDynRoutine = (zFARPROC_SETUPMAPLOD)
   ...
   if ( lpfnDynRoutine )
   {
      ...
      vLOD = (*lpfnDynRoutine)( vSubtask, vWindow,
                                vSrcDialog, "CtrlMapView" );
      DropView( vWindow );
   }
...
fnDropMetaOI( zVIEW vSubtask, zVIEW vLOD )
{
   ...
   lpfnDynRoutine = (zFARPROC_DROPMETAOI)
          GetOperationDynamicCallAddress( vSubtask,
                                          &lpLibrary,
                                          "tzlodopr", "DropMetaOI",
                                          "(tzctlcvt ListBox)" );
                ...
                zPSHORT nCtrlID )
{
   ...
   SetAttributeFromAttribute( vSrcDialog, szlControl, szlText,
                              vSrcDialog, szlControl, szlTag );
   SetAttributeFromAttribute( vTgtDialog, szlCtrl, szlText,
                              vTgtDialog, szlCtrl, szlTag );
   ...
   if ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      GetIntegerFromAttribute( &lSubtype, vSrcDialog,
      ...
      if ( SetCursorNextEntity( vSrcDialog, szlCtrlMap, 0 ) > zCURSOR_UNCHANGED )
      {
         ...
         // DisplayEntityInstance( vSrcDialog, szlCtrlMapLOD_Entity );
            SetAttributeFromAttribute( vTgtDialog, szlCtrlMap, "AN",
                                       vSrcDialog, szlCtrlMapLOD_Entity,
                                       szlName );
         }
      ...
      if ( SetCursorNextEntity( vSrcDialog, szlCtrlMap, 0 ) > zCURSOR_UNCHANGED )
      {
         ...
         {
            if ( SetCursorNextEntity( vTgtDialog, szlCtrlMap, 0 ) <=
                                                          zCURSOR_UNCHANGED )
            {
               CreateEntity( vTgtDialog, szlCtrlMap, zPOS_AFTER );
            }

            SetAttributeFromAttribute( vTgtDialog, szlCtrlMap, "VN",
                                       vSrcDialog, szlCtrlMapView, szlName );
         }
         else
         {
      ...
      if ( CheckExistenceOfEntity( vSrcDialog, szlCtrlMapLOD_Entity ) == 0 )
      {
         GetAddrForAttribute( (zCOREMEM) &pchEntityName,
                              vSrcDialog, szlCtrlMapLOD_Entity, szlName );
      }
      else
      {
         zCHAR  szTag[ 34 ];

         GetStringFromAttribute( szTag, vSrcDialog, szlControl, szlTag );
         ...
         else
         {
            fnDisplayCtrlError( vSubtask, vSrcDialog,
                                "ListBox MAPPING error ... "
                                "Control has no mapping entity.\nListBox: ",
                                TRUE );
            return( -16 );
         }
      }
      ...
      // Handle the sub-controls within the listbox ... setting up the
      ...
      if ( bButtonHeaders )
      {
         ...
         // Get the subtype of the first column to determine if it is
         // editable or not.
         GetIntegerFromAttribute( &lSubtype, vSrcDialog,
                                  szlControl, szlSubtype );
      }
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         else
         {
            ...
            GetIntegerFromAttribute( &lCharLth, vSrcDialog,
                                     szlControl, szlPSDLG_X );
      ...
      *((zPLONG) (pchBuffer + sizeof( zLONG ))) = k;  // set # of tabstops
      ...
      if ( bButtonHeaders )
      {
         ...
         while ( nRC > zCURSOR_UNCHANGED )
         {
            ...
            *((zPLONG) (pchBuffer + ulLth)) = k;  // set the tag length
            ...
            if ( nRC >= zCURSOR_SET )
            {
               ...
               while ( nRC >= zCURSOR_SET )
               {
                  ...
                  SetAttributeFromAttribute( vTgtWindow, "NLS_Text", "LI",
                                             vSrcDialog, "ControlNLS_Text",
                                             "LanguageIndex" );
                  SetAttributeFromAttribute( vTgtWindow, "NLS_Text", "Text",
                                             vSrcDialog, "ControlNLS_Text",
                                             "Text" );
            ...
            *((zPLONG) (pchBuffer + ulLth)) = k;
            ...
            zstrcpy( pchBuffer + ulLth, pchCtrlTag );
            if ( nDIL_Lth )
               zstrcat( pchBuffer + ulLth, szTempString );
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         if ( CheckExistenceOfEntity( vSrcDialog,
                                      szlCtrlMapRelatedEntity ) == 0 )
         {
            vLOD = fnSetUpMapLOD( vSubtask, vSrcDialog );
            GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName, vSrcDialog,
                                 szlCtrlMapRelatedEntity, szlName );
            if ( vLOD )
            {
               nLOD_RC = SetCursorFirstEntityByString( vLOD, "LOD_Entity",
                                                       "Name",
                                                       pchSubCtrlName, 0 );
            }
         }
         else
         {
            ...
            else
            {
               ResetViewFromSubobject( vSrcDialog );
               fnDisplayCtrlError( vSubtask, vSrcDialog,
                                   "ListBox MAPPING error ... ListCtrl has no "
                                   "mapping entity.\nListBox - ",
                                   TRUE );
               return( -16 );
            }
         ...
         GetAddrForAttribute( (zCOREMEM) &pchCtrlTag,
                              vSrcDialog, szlControl, szlText );
         SetAttributeFromString( vTgtRemote, "ListBoxColumn",
                                 "Text", pchCtrlTag );
         SetAttributeFromInteger( vTgtRemote, "ListBoxColumn",
                                  "Tag", lColumnCount );
         ...
         k = lCharLth;
         GetIntegerFromAttribute( &lCharLth, vSrcDialog,
                                  szlControl, szlSZDLG_X );
         SetAttributeFromInteger( vTgtRemote, "ListBoxColumn",
                                  "ColumnWidth", lCharLth );
         ...
         *((zPLONG) (pchBuffer + ulLth)) = lCharLth; // dialog units
         ...
         else
         {
            ...
            *((zPLONG) (pchBuffer + ulLth)) = k;
            ulLth += sizeof( zLONG );
            zstrcpy( pchBuffer + ulLth, pchSubCtrlName );
            ulLth += k + 1;
         }

         if ( CheckExistenceOfEntity( vSrcDialog,
                                      szlCtrlMapER_Attribute ) == 0 )
         {
            GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName, vSrcDialog,
                                 szlCtrlMapER_Attribute, szlName );
            if ( vLOD && nLOD_RC >= zCURSOR_SET )
            {
                                                  ...
                                                  "LOD_Entity" ) >= zCURSOR_SET )
               {
                  if ( CheckExistenceOfEntity( vLOD, "Domain" ) == 0 )
                  {
                     ...
                     GetStringFromAttribute( szDataType, vLOD,
                                             "Domain", "DataType" );
         ...
         *((zPLONG) (pchBuffer + ulLth)) = k;
         ...
         if ( CheckExistenceOfEntity( vSrcDialog,
                                      szlCtrlMapContext ) == 0 )
         {
            GetStringFromAttribute( pchBuffer + ulLth + sizeof( zLONG ),
                                    vSrcDialog, szlCtrlMapContext, szlName );
         // GetContextFromSrcCtrl( vSrcDialog,
         //                        pchBuffer + ulLth + sizeof( zLONG ) );

            // Set the length of the attribute.
            k = StringLthOn4_ByteBoundary( pchBuffer +
                                              ulLth + sizeof( zLONG ) );
            *((zPLONG) (pchBuffer + ulLth)) = k;
   ...
   else
   {
      ResetViewFromSubobject( vSrcDialog );
      fnDisplayCtrlError( vSubtask, vSrcDialog,
                          "ListBox error ... ListBox has no ListCtrls.\n"
                          "ListBox - ", TRUE );
      return( -16 );
   }
}
                 ...
                 zPSHORT nCtrlID )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &ulSubtype, vTgtDialog,
                            szlCtrl, szlSubtype );
   ...
   if ( (nRC = SetCursorFirstEntity( vSrcDialog, szlCtrlMap, 0 ))
                                                      > zCURSOR_UNCHANGED )
   {
      if ( DrAllocTaskMemory( (zCOREMEM) &pchBuffer, zMAX_BLOB ) != 0 )
      {
         IssueError( vSubtask, 0, 0,
                     "Memory alloc error - convert ComboBox" );
         return( 1 );
      }
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
              ...
              (ulSubtype & zCOMBOBOX_INCLUDE) )
         {
            ...
            {
               if ( CheckExistenceOfEntity( vSrcDialog,
                                            szlCtrlMapView ) == zCURSOR_SET )
               {
                  // Put the view name into the CtrlBOI.
                  GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName,
                                       vSrcDialog, szlCtrlMapView, szlName );
                  zstrcpy( pchBuffer + ulLth, pchSubCtrlName );
                  ...
                  GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName,
                                       vSrcDialog, szlCtrlMapRelatedEntity,
                                       szlName );
                  zstrcpy( pchBuffer + ulLth, pchSubCtrlName );
                  ulLth += zstrlen( pchSubCtrlName ) + 1;
                  ...
                  GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName,
                                       vSrcDialog, szlCtrlMapER_Attribute,
                                       szlName );
                  zstrcpy( pchBuffer + ulLth, pchSubCtrlName );
                  ulLth += zstrlen( pchSubCtrlName ) + 1;
                  ...
                  if ( CheckExistenceOfEntity( vSrcDialog,
                                               szlCtrlMapContext ) == 0 )
                  {
                     GetStringFromAttribute( pchBuffer + ulLth,
                                             vSrcDialog, szlCtrlMapContext,
                                             szlName );
            ...
            else
            {
               ...
               else
               {
                  ...
                  if ( k == 5 &&
                       CheckExistenceOfEntity( vSrcDialog, szlCtrlMapView )
                                                             == zCURSOR_SET )
                  {
                     GetStringFromAttribute( pchBuffer + ulLth,
                                             vSrcDialog, szlCtrlMapView,
                                             szlName );
                     ulLth += zstrlen( pchBuffer + ulLth ) + 1;
                  }
                  else
         ...
         if ( k == 4 )
         {
            if ( SetEntityCursor( vSrcDialog, szlCtrlMap, 0,
   ...
   if ( (ulSubtype & zCOMBOBOX_EXTERNAL_DATA) == 0 && lMappingError > 0 )
   {
      ...
      {
         fnDisplayCtrlError( vSubtask, vSrcDialog,
                             "ComboBox MAPPING error ... EditCtrl has no "
                             "mapping entity.\nComboBox - ", TRUE );
         return( -16 );
      }
      ...
      {
         fnDisplayCtrlError( vSubtask, vSrcDialog,
                             "ComboBox MAPPING error ... ListCtrl has no "
                             "mapping entity.\nComboBox - ", TRUE );
         return( -16 );
      }
   }
              ...
              zPSHORT nCtrlID )
{
   ...
   SetEntityAttributesFromBlob( vWork, "Graph",
                                vSrcDialog, szlControl, szlCtrlBOI );
   ...
   pchBuffer = (zPCHAR) DrLockTaskMemory( pchBuffer );
   GetAddrForAttribute( (zCOREMEM) &pchTitle,
                        vWork, "Graph", "GraphTitle" );
   GetIntegerFromAttribute( &lType, vWork, "Graph", "GraphType" );
   zULONG ulLth = 0;
   zstrcpy( pchBuffer, pchTitle );
                 ...
                 zPSHORT nCtrlID )
{
   ...
   GetIntegerFromAttribute( (zPLONG) &ulSubtype, vSrcDialog,
                            szlControl, szlSubtype );
...
// zCHAR  szURL[ zMAX_FILENAME_LTH ];
   ...
   fnResetResourceImage( vSubtask, vTgtRemote, "Bitmap", "URL",
...
// zCHAR  szURL[ zMAX_FILENAME_LTH ];
   ...
   GetAddrForAttribute( &pchCtrlBOI, vSrcDialog, "Control", "CtrlBOI" );
   SplitCtrlBOI_BMP( pchCtrlBOI, ulLth, &BMP );
   fnResetResourceImage( vSubtask, vTgtRemote, "BitmapBtn", "URL_Up",
                         BMP.szBMPUpOrIconName, BMP.lBMPUpOrIconID );
   fnResetResourceImage( vSubtask, vTgtRemote, "BitmapBtn", "URL_Down",
                         BMP.szBMPDownName, BMP.lBMPDownID );
   fnResetResourceImage( vSubtask, vTgtRemote, "BitmapBtn", "URL_Focus",
                         BMP.szBMPFocusName, BMP.lBMPFocusID );
   fnResetResourceImage( vSubtask, vTgtRemote, "BitmapBtn", "URL_Disabled",
...
// zCHAR  szURL[ zMAX_FILENAME_LTH ];
   ...
   GetAddrForAttribute( &pchCtrlBOI, vSrcDialog, "Control", "CtrlBOI" );
   SplitCtrlBOI_BMP( pchCtrlBOI, ulLth, &BMP );
   fnResetResourceImage( vSubtask, vTgtRemote, "Icon", "URL",
                         BMP.szBMPUpOrIconName, BMP.lBMPUpOrIconID );
   return( 0 );
}
                 ...
                 zPSHORT nCtrlID )
{
   ...
   if ( *pch )
   {
      ...
      GetIntegerFromAttribute( (zPLONG) &ulSubtype, vGroup,
                               szlCtrl, szlSubtype );
   ...
   if ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      if ( SetCursorNextEntity( vSrcDialog, szlCtrlMap, 0 ) > zCURSOR_UNCHANGED )
      {
         if ( CheckExistenceOfEntity( vSrcDialog, szlCtrlMapLOD_Entity ) == 0 )
         {
            ...
            GetAddrForAttribute( (zCOREMEM) &pchText,
                                 vTgtDialog, szlCtrlMap, "AN" );
            if ( *pchText == 0 )
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         // fnDisplayCtrlError( vSubtask, vSrcDialog,
         //                     "Grid MAPPING error ... Grid Ctrl has no "
         //                     "mapping entity.\nGrid - ", TRUE );
            GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName, vSrcDialog,
                                 szlControl, szlTag );
            TraceLine( "Grid MAPPING error ... Grid Ctrl (%s) has no mapping entity",
                       pchSubCtrlName );
            nRC = SetCursorNextEntity( vSrcDialog, szlControl, 0 );
            continue;
         }
         ...
         else
         {
            ...
            if ( vPE &&
                 SetCursorFirstEntityByInteger( vPE, szlControlDef,
                                                szlKey, zGRID_SUB_CONTROL, 0 ) == 0 )
            {
            ...
            // GetIntegerFromAttribute( &lKey, vSrcDialog, szlControlDef, szlKey );
               ...
               SetMatchingAttributesByName( vSrcDialog, szlControl,
                                            vTemp, szlControl, zSET_ALL );
               szTag[ 0 ] = '_'; // reset the tag to the tag preceded by an underscore
               GetStringFromAttribute( szTag + 1, vTemp, szlControl, szlTag );
               SetAttributeFromString( vTemp, szlControl, szlTag, szTag );
               IncludeSubobjectFromSubobject( vSrcDialog, szlControlDef,
                                              vPE, szlControlDef, zPOS_BEFORE );
               ...
               if ( SetCursorFirstEntity( vTemp, szlCtrlMap, 0 ) == zCURSOR_SET )
               {
                  ...
                  SetMatchingAttributesByName( vSrcDialog, szlCtrlMap,
                                               vTemp, szlCtrlMap,
                                               zSET_NULL | zSET_NOTNULL );
                  if ( CheckExistenceOfEntity( vTemp, szlCtrlMapLOD_Attribute ) == zCURSOR_SET )
                  {
                     IncludeSubobjectFromSubobject( vSrcDialog, szlCtrlMapLOD_Attribute,
                                                    vTemp, szlCtrlMapLOD_Attribute, zPOS_AFTER );
                  }

                  if ( CheckExistenceOfEntity( vTemp, szlCtrlMapLOD_Entity ) == zCURSOR_SET )
                  {
                     IncludeSubobjectFromSubobject( vSrcDialog, szlCtrlMapLOD_Entity,
                                                    vTemp, szlCtrlMapLOD_Entity, zPOS_AFTER );
                  }

                  if ( CheckExistenceOfEntity( vTemp, szlCtrlMapContext ) == zCURSOR_SET )
                  {
                     IncludeSubobjectFromSubobject( vSrcDialog, szlCtrlMapContext,
                                                    vTemp, szlCtrlMapContext, zPOS_AFTER );
                  }

                  if ( CheckExistenceOfEntity( vTemp, szlCtrlMapView ) == zCURSOR_SET )
                  {
                     IncludeSubobjectFromSubobject( vSrcDialog, szlCtrlMapView,
                                                    vTemp, szlCtrlMapView, zPOS_AFTER );
                  }
               }

               if ( MoveSubobject( vSrcDialog, szlCtrlCtrl,
                                   vTemp, szlControl,
                                   zPOS_AFTER, zREPOS_NEXT ) == zCALL_ERROR )
               {
                  fnDisplayCtrlError( vSubtask, vTemp,
                                      "Grid MAPPING error ... Grid SubCtrl unable "
                                      "to move.\nGrid - ", TRUE );
               }
         ...
         if ( lKey == zGRID_SUB_CONTROL )
         {
            ...
            GetBlobFromAttribute( szSubBlob, &ulSubLth, vSrcDialog,
                                  szlControl, "CtrlBOI" );
            ...
            if ( CheckExistenceOfEntity( vSrcDialog, szlCtrlCtrl ) == 0 )
            {
               GetStringFromAttribute( szSubBlob + (3 * sizeof( zLONG )),
                                       vSrcDialog, szlCtrlCtrl, szlTag );

               SetViewToSubobject( vSrcDialog, szlCtrlCtrl );
               if ( MoveSubobject( vTopSrcDialog, szlControl,
                                   vSrcDialog, szlControl,
                                   zPOS_AFTER, zREPOS_NEXT ) == zCALL_ERROR )
               {
                  fnDisplayCtrlError( vSubtask, vSrcDialog,
                                      "Grid MAPPING error ... Grid SubCtrl unable "
         ...
         GetAddrForAttribute( (zCOREMEM) &pchEntity,
                              vSrcDialog, szlCtrlMapRelatedEntity, szlName );
         ...
         else
         {
            ...
            *((zPLONG) (pchBuffer + ulLth)) = k;  // set length of entity
            ulLth += sizeof( zLONG );
            zstrcpy( pchBuffer + ulLth, pchEntity );
            ulLth += k;
         }
         ...
         {
            GetAddrForAttribute( (zCOREMEM) &pchAttribute,
                                 vSrcDialog, szlCtrlMapER_Attribute, szlName );
         }
         else
         {
         // if ( CompareAttributeToString( vSrcDialog, szlControlDef, szlTag, "PushBtn" ) != 0 )
            if ( lKey == zPUSHBUTTON_CONTROL )
            {
               GetAddrForAttribute( (zCOREMEM) &pchSubCtrlName, vSrcDialog,
                                    szlControl, szlTag );
               TraceLine( "Grid MAPPING error ... Grid Ctrl (%s) has no mapping attribute",
                          pchSubCtrlName );
            }
            else
            {
         ...
         *((zPLONG) (pchBuffer + ulLth)) = k;  // set length of attribute
         ulLth += sizeof( zLONG );
         zstrcpy( pchBuffer + ulLth, pchContext );
         ulLth += k;
      ...
      // }
         ...
         *((zPLONG) (pchBuffer + ulLth)) = ulSubtype;
         ...
         if ( lControlType == zGRID_CHECKBOX )
         {
            ...
            GetAddrForAttribute( (zCOREMEM) &pch, vSrcDialog,
                                 szlControl, szlText );
            lCharLth = zstrlen( pch ) + 1;               // Text
            *((zPLONG) (pchBuffer + ulLth)) = lCharLth;
            ulLth += sizeof( zLONG );
            zstrcpy( pchBuffer + ulLth, pch );
            ulLth += lCharLth;
      ...
      *((zPULONG) (pchBuffer + sizeof( zLONG ))) = ulLth;
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         *((zPLONG) (pchBuffer + ulLth)) = (zLONG) lCharLth;
         ...
         GetAddrForAttribute( (zCOREMEM) &pchText, vSrcDialog,
                              szlControl, szlText );
         ...
         *((zPLONG) (pchBuffer + ulLth)) = k;  // set the text length
         ...
         zstrcpy( pchBuffer + ulLth, pchText );
         if ( nDIL_Lth )
            zstrcat( pchBuffer + ulLth, szTempString );
      ...
      *((zPLONG) pchBuffer) = lCols;  // store the number of controls
      ...
      ResetViewFromSubobject( vSrcDialog );
      SetAttributeFromBlob( vTgtDialog, szlCtrl,
                            szlCtrlBOI, pchBuffer, (zUSHORT) ulLth );
      pchBuffer = (zPCHAR) DrUnlockTaskMemory( pchBuffer );
      DrFreeTaskMemory( pchBuffer );
      DropView( vTopSrcDialog );
      return( 0 );
            ...
            zPSHORT nCtrlID )
{
   ...
   if ( nRC >= zCURSOR_UNCHANGED )
   {
      ...
      GetIntegerFromAttribute( (zPLONG) &ulIdNbr, vSrcDialog,
                               szlControl, szlSubtype );
      ...
      *((zPLONG) (pchBuffer + sizeof( zLONG ))) = nItems;  // nItems
...
#endif
         GetStructFromEntityAttrs( (zPCHAR) CtrlDef.Tag, vSrcDialog,
                                   szlCtrlCtrl );
#ifdef DEBUG_LODSTRUCT
      ...
      //    TraceLineS( "tzctlcvt CtrlDef CheckByte error??", "" );
#endif
         ...
         zstrcpy( pchBuffer + ulLth, CtrlDef.Text );
   ...
   else
   {
      ...
      *(pchBuffer + ulLth) = 0;
      ...
      SetAttributeFromBlob( vTgtDialog, szlCtrl, szlCtrlBOI, pchBuffer,
                            ulLth + sizeof( zSHORT ) );
   }

   MoveSubobject( vTgtDialog, szlCtrl, vTgtDialog, szlCtrl,
                  zPOS_FIRST, zREPOS_PREV );
   pchBuffer = (zPCHAR) DrUnlockTaskMemory( pchBuffer );
   DrFreeTaskMemory( pchBuffer );
   return( 0 );
}
                ...
                zPSHORT nCtrlID )
{
   ...
   if ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      GetAddrForAttribute( &pchCtrlBOI, vSrcDialog, "Control", "CtrlBOI" );
      SplitCtrlBOI_BMP( pchCtrlBOI, ulLth, &BMP );
      fnResetResourceImage( vSubtask, vTgtRemote, "ToolBar", "URL",
                            BMP.szBMPUpOrIconName, BMP.lBMPUpOrIconID );

      // Starting blob with room for four longs.
      ulLth = zMAX_BLOB - (4 * sizeof( zLONG ));
      GetBlobFromAttribute( pchBuffer + (4 * sizeof( zLONG )),
                            (zPULONG) &ulLth, vSrcDialog,
                            szlControl, szlCtrlBOI );
      *((zPULONG) pchBuffer) = ulLth;
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         GetIntegerFromAttribute( &lType, vSrcDialog,
                                  szlControl, szlSubtype );
         GetStringFromAttribute( szTempString, vSrcDialog,
                                 szlControl, "Underline" );
         nRC = SetCursorNextEntity( vSrcDialog, szlControl, 0 );
         if ( nRC >= zCURSOR_SET )
         {
            GetIntegerFromAttribute( &lTypeNext, vSrcDialog,
                                     szlControl, szlSubtype );
         ...
         *((zPLONG) (pchBuffer + ulLth)) = lType;
         ulLth += sizeof( zPLONG );
         GetStringFromAttribute( pchBuffer + ulLth, vSrcDialog,
                                 szlControl, szlTag );
         ulLth += zstrlen( pchBuffer + ulLth ) + 1;
         GetStringFromAttribute( pchBuffer + ulLth, vSrcDialog,
                                 szlControl, szlText );
   ...
   else
   {
      ...
      fnDisplayCtrlError( vSubtask, vSrcDialog,
                          "ToolBar error ... ListBox has no ListCtrls.\n"
                          "ToolBar - ", TRUE );
      return( -16 );
   }
}
              ...
              zPSHORT nCtrlID )
{
   ...
   SetAttributeFromAttribute( vTgtRemote, "Timer", "Interval",
                              vTgtDialog, szlCtrl, "StyleX" );
...
//    zCHAR    szEntityName[ 33 ];  // Entity/attribute name
                 ...
                 zPSHORT nCtrlID )
{
   ...
   GetBlobFromAttribute( lpOldBuffer, (zPULONG) &lLth, vSrcDialog,
                         szlControl, szlCtrlBOI );

   // strip off the registered view name and 2 longs
   nStringLth = zstrlen( lpOldBuffer ) + 1;
...
//    lOffset++;
   ...
   SetAttributeFromBlob( vTgtDialog, szlCtrl, szlCtrlBOI,
                         lpBuffer, lLth );
   lpOldBuffer = (zPCHAR) DrUnlockTaskMemory( lpOldBuffer );
   DrFreeTaskMemory( lpOldBuffer );
                      ...
                      zLONG   lOffset )
{
   ...
   GetStringFromAttribute( OL_Item->szEntityName, vSrcDialog,
                           szlCtrlMapRelatedEntity, szlName );

   // get the scoping entity from the Controls CtrlBOI and set it into
   // the OL_Item data structure
   GetBlobFromAttribute( OL_Item->szScopeEntity, &ulLth, vSrcDialog,
                         szlControl, szlCtrlBOI );
...
//    TraceLineS( "Tree CtrlMap Tag ", szTempString );
      ...
      GetStringFromAttribute( lpBuffer + lOffset, vSrcDialog,
                              szlCtrlMapER_Attribute, szlName );
      if ( szTempString[ 0 ] == 'Y' ||
           lpBuffer[ OL_Item->lTagAttributeOffset ] == 0 )
      {
//       TraceLineS( "Tree tag ", lpBuffer + lOffset );
         zstrcpy( lpBuffer + OL_Item->lTagAttributeOffset,
                  lpBuffer + lOffset );
      }
...
//       TraceLineS( "Tree attribute ", lpBuffer + lOffset );
         ...
         zstrcpy( lpBuffer + lOffset, szTempString + 8 );  // context
         lOffset += zstrlen( lpBuffer + lOffset ) + 1;
      }
   ...
   if ( lSubtype & zTREE_RECURSIVE_ENTITY )
   {
      ...
      GetStringFromAttribute( OL_Item->szRecursiveEntity,
                              vSrcDialog, szlControl, szlText );
   }
   ...
   // If there are CtrlCtrls then set to the subobject and call this
                  ...
                  zLONG        lCtrlBOI_Size )
{
   ...
   zstrncpy( pchCtrlBOI, pBMP->szDLLName, lCtrlBOI_Size );
   ...
   *pch = 0;
   if ( pBMP->szBMPUpOrIconName[ 0 ] )
   {
      ...
      {
         zltoa( pBMP->lBMPUpOrIconID, szID );
         zstrncat( pch, szID, lCtrlBOI_Size - lLth );
         zstrcat( pch, ";" );
      }

      zstrncat( pch, pBMP->szBMPUpOrIconName, lCtrlBOI_Size - lLth );
   }
   ...
   *pch = 0;
   if ( pBMP->szBMPDownName[ 0 ] )
   {
      if ( pBMP->lBMPDownID )
      {
         zltoa( pBMP->lBMPDownID, szID );
         zstrncat( pch, szID, lCtrlBOI_Size - lLth );
         zstrcat( pch, ";" );
      }

      zstrncat( pch, pBMP->szBMPDownName, lCtrlBOI_Size - lLth );
   }
   ...
   *pch = 0;
   if ( pBMP->szBMPFocusName[ 0 ] )
   {
      if ( pBMP->lBMPFocusID )
      {
         zltoa( pBMP->lBMPFocusID, szID );
         zstrncat( pch, szID, lCtrlBOI_Size - lLth );
         zstrcat( pch, ";" );
      }

      zstrncat( pch, pBMP->szBMPFocusName, lCtrlBOI_Size - lLth );
   }
   ...
   *pch = 0;
   if ( pBMP->szBMPDisabledName[ 0 ] )
   {
      ...
      {
         zltoa( pBMP->lBMPDisabledID, szID );
         zstrncat( pch, szID, lCtrlBOI_Size - lLth );
         zstrcat( pch, ";" );
      }

      zstrncat( pch, pBMP->szBMPDisabledName, lCtrlBOI_Size - lLth );
   }
                    ...
                    zLONG  lMaxResID )
{
         ...
         nRC = SetCursorNextEntity( vDialog, cpcCtrlEntity, 0 ) )
   {
           ...
           lCtrlType == zTOOLBAR_CONTROL )
      {
         ...
         {
            // Get resources.
            GetAddrForAttribute( &pchCtrlBOI, vDialog,
                                 cpcCtrlEntity, "CtrlBOI" );
            SplitCtrlBOI_BMP( pchCtrlBOI, ulLth, &BMP );
            ...
            else
            {
               ...
               {
                  bResourceDLL = TRUE;
                  TraceLineS( "Dialog Conversion:  Resource DLL specified "
                                 "... no resources generated in .RC file: ",
                              BMP.szDLLName );
               }
            }
         }
...
{
   zPCHAR pch;
   zPLONG pl = (zPLONG) (pchRC_Bytes +
                             (lId - zMAX_RES_ID_USER - 1) * sizeof( zLONG ));
   if ( *pl )
   {
      pch = (zPCHAR) *pl;
   ...
   DrAllocTaskMemory( (zCOREMEM) &pv, zstrlen( cpcResName ) + 1 );
   pch = (zPCHAR) DrLockTaskMemory( pv );
   zstrcpy( pch, cpcResName );
   *pl = (zLONG) pch;
   return( 1 );
}
                     ...
                     zPLONG        plResID )
{
   ...
   if ( cpcResName[ 0 ] &&
        (*plResID == 0 || *plResID > zMAX_RES_ID_USER) )
   {
      if ( *plResID == 0 ||
           (*plResID > zMAX_RES_ID_USER &&
            fnIsRC_ByteSet( pchRC_Bytes, *plResID, cpcResName )) )
      {
      ...
      // Write resource line.
      if ( nResType == 0 )
         zsprintf( pchLine, "%-6ld ICON   ", *plResID );
      else
   // if ( nResType == 1 || nResType == 2 )  // bitmap to toolbar
         zsprintf( pchLine, "%-6ld BITMAP ", *plResID );

      if ( hFile )
      {
         if ( nRC > 0 )
         {
            zSHORT nLth = zstrlen( pchLine );
                          ...
                          zPCHAR        pchRC_Bytes )
{
         ...
         nRC = SetCursorNextEntity( vDialog, cpcCtrlEntity, 0 ) )
   {
      ...
      {
         SetViewToSubobject( vDialog, "CtrlCtrl" );
         nChangeLevel = fnWriteCtrlResourcesToRC( vSubtask, hFile, vDialog,
                                                  pcd, pchLine, plMaxResID,
                                                  pchRC_Bytes );
      ...
      GetIntegerFromAttribute( &lCtrlType, vDialog,
                               cpcCtrlDefEntity, "Key" );
      ...
      if ( nResType >= 0 )
      {
         ...
         if ( ulLth )
         {
            ...
            if ( BMP.szDLLName[ 0 ] == 0 )
            {
               ...
               // for BMP up / Icon
               nChangeLevel = fnWriteResourceLine( vSubtask, hFile, pcd,
                                                   nResType, pchLine,
                                                   plMaxResID, pchRC_Bytes,
               ...
               if ( bCtrlChanged )
               {
                  ...
                  SetAttributeFromBlob( vDialog, cpcCtrlEntity,
                                        "CtrlBOI", pchLine, ulLth );
               }
            } // if create RC entry
         }
      }
                    ...
                    zVIEW         vDialog )
{
         ...
         nRC = SetCursorNextEntity( vDialog, cpcWinEntity, 0 ) )
   {
      ...
      nRC = fnWriteCtrlResourcesToRC( vTaskLPLR, hFile, vDialog, pcd,
                                      pchLine, &lMaxResID, pchRC_Bytes );
      if ( nRC > 0 )
      {
         if ( nRC > nChangeLevel )
            nChangeLevel = nRC;
         ...
         if ( SetCursorFirstEntity( vMetaListPE,
                                    "W_MetaDef", "" ) < zCURSOR_SET )
         {
            // I assume this means we have an LPLR without Dialogs,
            // such as a reusable CPLR with only Global Operations.
                 ...
                 zPSHORT nCtrlID )
{
   ...
   if ( CheckExistenceOfEntity( vSrcDialog, szlCtrlCtrl ) == zCURSOR_SET )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         //SetAttributeFromAttribute( vTgtRemote, "GridColumn", "Tag",
         //                           vSrcDialog, szlControl, "Tag" );
         SetAttributeFromAttribute( vTgtRemote, "GridColumn", "Text",
                                    vSrcDialog, szlControl, "Text" );
         ...
         //            // *** so only do the default
            ...
            case CONTROL_TYPE_RADIOGRP:
               ...
               SetMatchingAttributesByName( vTgtRemote, "GridRadioGroup",
                                            vSrcDialog, szlControl,
                                            zSET_ALL );
               ...
               if ( CheckExistenceOfEntity( vSrcDialog, szlCtrlCtrl ) > zCURSOR_UNCHANGED )
               {
                  ...
                  while ( nRC > zCURSOR_UNCHANGED )
                  {
                     ...
                     SetMatchingAttributesByName( vTgtRemote, "GridRadioButton",
                                                  vSrcDialog, szlControl,
                                                  zSET_ALL );
         ...
         SetMatchingAttributesByName( vTgtDialog, szlCtrl,
                                      vSrcDialog, szlControl,
                                      zSET_ALL );
         ...
         while ( nRCSubControl > zCURSOR_UNCHANGED )
         {
            ...
            {
               GetStringFromAttribute( szViewName, vSrcDialog, szlCtrlMapView, szlName );
               GetStringFromAttribute( szEntityName, vSrcDialog, szlCtrlMapRelatedEntity, szlName );
               GetStringFromAttribute( szAttributeName, vSrcDialog, szlCtrlMapER_Attribute, szlName );
            }

            if ( CheckExistenceOfEntity( vSrcDialog, szlCtrlMapContext ) == zCURSOR_SET )
            {
               GetStringFromAttribute( szContextName, vSrcDialog, szlCtrlMapContext, szlName );
            }
                            ...
                            zCPCHAR cpcCtrlTag )
{
   ...
   zSHORT nRC = SetCursorFirstEntityByString( vWindow, "Window",
                                              "Tag", cpcWndTag, 0 );
   if ( nRC >= zCURSOR_SET )
      bDuplicate = fnFindTagAtLevel( vWindow, cpcCtrlTag );
                   ...
                   ZNameList *pNameList, zSHORT& nCnt )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      if ( pNameList->AddTail( pchTag ) == 0 )  // 0 ==> already in list
      {
         ...
         TraceLine( "Duplicate Control Tag: %s  found in Dialog.Window: %s.%s",
                    pchTag, pchDlgTag, pchWndTag );
         nCnt++;
      }
      ...
      if ( lFlags & 0x00000001 ) // check action tags as well
      {
         ...
         while ( nRC >= zCURSOR_SET )
         {
            ...
            if ( pNameList->AddTail( pchTag ) == 0 )  // 0 ==> already in list
            {
               ...
               GetAddrForAttribute( &pchDlgTag, vSrcDialog, "Dialog", "Tag" );
               GetAddrForAttribute( &pchWndTag, vSrcDialog, "Window", "Tag" );
               TraceLine( "Duplicate Action Tag: %s  found in Dialog.Window: %s.%s",
                          pchTag, pchDlgTag, pchWndTag );
               nCnt++;
            }
                      ...
                      ZNameList *pNameList, zSHORT& nCnt )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      if ( pNameList->AddTail( pchTag ) == 0 )
      {
         ...
         TraceLine( "Duplicate Control Tag: %s  found in GroupSet.Group: %s.%s",
                    pchTag, pchGrpSetTag, pchGrpTag );
         nCnt++;
      }
