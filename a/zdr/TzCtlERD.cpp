                      ...
                      "ER" )
{
   ...
   SysReadZeidonIni( -1, "[ZeidonTools]", "RefreshERD", szTemp );
   if ( ((*szTemp && *(szTemp + 1) == 0) && (*szTemp == 'N' || *szTemp == 'n')) )
      m_bRefreshERD = FALSE;

   lAlloc = ((zLONG) tzERDIAGRAMX * (zLONG) tzERDIAGRAMY);
   m_hGrid = SysAllocMemory( (zCOREMEM) &m_pchGrid, lAlloc, 0,
                             zCOREMEM_ALLOC, 0 );
   // Zero out the grid
   pchWork = m_pchGrid;
   while ( lAlloc > 16384 )
...
ZER_Diagram::ZoomDiagram( zSHORT nPercent, zSHORT nOptions )
{
   ...
   if ( nOptions == 3 )   // Fit to window
   {
      ...
      if ( m_lMaxPosX || m_lMaxPosY )
      {
         CalculateDisplaySize( lDisplaySizeX, lDisplaySizeY,
                               rect, lNewScale );
         while ( lDisplaySizeX > (m_lMaxPosX + tzERENTITYX ) &&
                 lDisplaySizeY > (m_lMaxPosY + tzERENTITYY ) )
         {
            lNewScale += 10;
            CalculateDisplaySize( lDisplaySizeX, lDisplaySizeY,
                                  rect, lNewScale );
         ...
         if ( lDisplaySizeX > (m_lMaxPosX + tzERENTITYX) &&
              lDisplaySizeY > (m_lMaxPosY + tzERENTITYY) )
         {
            while ( lDisplaySizeX > (m_lMaxPosX + tzERENTITYX) &&
                    lDisplaySizeY > (m_lMaxPosY + tzERENTITYY) )
   ...
   if ( m_lDisplayPos && nOptions != 4 )
   {
      ...
      while ( PosX( m_lDisplayPos ) &&
              (PosX( m_lDisplayPos ) + lDisplaySizeX) >= tzERDIAGRAMX )
      {
         m_lDisplayPos--;
      }

      while ( PosY( m_lDisplayPos ) &&
              (PosY( m_lDisplayPos ) + lDisplaySizeY) >= tzERDIAGRAMY )
      {
         m_lDisplayPos -= tzERDIAGRAMX;
      }
   }
...
//    ptPageSize.y  = pPrintDC->GetDeviceCaps( VERTRES );
      ...
      if ( pPrintDC )
      {
         ...
         if ( nOptions == 0 || (nOptions & (zPRINT_PAGE | zPRINT_FILLPAGE)) )
         {
            ...
            {
               ptDiagramSize.x = (m_lMaxPosX + m_lMinPosX + tzERENTITYX) *
                                                                tzERCELLSIZE;
               ptDiagramSize.y = (m_lMaxPosY + m_lMinPosY + tzERENTITYY) *
                                                                tzERCELLSIZE;
         ...
         zstrcpy( szMessage, "Print Analysis\r\n\r\n" );
         zstrcat( szMessage, "          Size: " );
         zltoa( m_lScale + 100, &szMessage[ 240 ] );
         zstrcat( szMessage, &szMessage[ 240 ] );
         zstrcat( szMessage, "%\r\n" );
         zstrcat( szMessage, "   Orientation: " );
         if ( nOptions & zPRINT_PORTRAIT )
            zstrcat( szMessage, "Portrait\r\n" );
         else
            zstrcat( szMessage, "Landscape\r\n" );

         zstrcat( szMessage, "         Pages: " );
         zltoa( lPages, &szMessage[ 240 ] );
         zstrcat( szMessage, &szMessage[ 240 ] );
         zstrcat( szMessage, "\r\n\r\n" );
         zstrcat( szMessage, "Continue with printing?" );
         ...
         if ( nRC == zRESPONSE_YES && pPrintDC->StartDoc( &DocInfo ) > 0 )
         {
            ...
            for ( ;; )
            {
               ...
               TraceLineI( "              Entities printed = ",
                           lEntitiesPrinted );
               TraceLineI( "          Entities not printed = ",
                           lEntitiesNotPrinted );
               TraceLineI( "         Relationships printed = ",
                           lRelationshipsPrinted );
               TraceLineI( "     Relationships not printed = ",
                           lRelationshipsNotPrinted );
               ...
               if ( lPrinted )
               {
                  ...
                  else
                  {
                     ...
                     if ( GetViewByName( &vDiagram, "TZEREMDO",
                                         m_pZSubtask->m_vDialog,
                                         zLEVEL_ANY ) > 0 )
                     {
                        GetAddrForAttribute( &pchPrintTitle, vDiagram,
                                             "EntpER_Model", "Name" );
                        GetAddrForAttribute( &pchPrintDesc, vDiagram,
                                             "EntpER_Model", "Desc" );
                     }
                     else
                     {
                     ...
                     {
                        zstrcpy( szMessage, pchPrintTitle );
                        lPos = zstrlen( szMessage );
                        szMessage[ lPos++ ] = ' ';
                        szMessage[ lPos++ ] = '[';
                     }
                     ...
                     SysGetDateTime( szMessage + 200 );
                     UfFormatDateTime( szMessage + lPos, szMessage + 200,
                                       "YYYY/MM/DD HH:MI:SS" );
...
ZER_Diagram::CountEntitiesAtIndex( zLONG lIdx )
{
   ...
   if ( lIdx <= tzERMAXGRID && (m_pchGrid[ lIdx ] & tziENTITY) )
   {
      ...
      {
         pEntity = m_EntityList[ nPos ];
         lEndPos = pEntity->m_lPosIdx +
                   (pEntity->m_lEntPortX * pEntity->m_lEntPortY);
                        ...
                        zBOOL      bCtrl )
{
   if ( vDiagram == 0 )
   {
      GetViewByName( &vDiagram, *m_pzsVName,
                     m_pZSubtask->m_vDialog, zLEVEL_ANY );
   }

   if ( vDiagram &&
        SetCursorFirstEntityByInteger( vDiagram, *m_pzsEName,
                                       szlZKey,
                                       pEntity->m_lPosZKey,
                                       0 ) >= zCURSOR_SET )
   {
      if ( bCtrl == FALSE )
         SetAllSelStatesForEntityForSet( vDiagram, *m_pzsEName,
                                         0, 1, 0 );
      else
      if ( bSelect && GetSelectStateOfEntityForSet( vDiagram,
                                                    *m_pzsEName,
                                                    1 ) )
      {
         bSelect = FALSE;
         SetSelectStateOfEntityForSet( vDiagram, *m_pzsEName,
                                       0, 1 );
      }
                              ...
                              zBOOL       bCtrl )
{
   ...
   if ( vDiagram &&
        SetCursorFirstEntityByInteger( vDiagram, *m_pzsAName,
                                       szlZKey,
                                       pRelationship->m_lPosZKey,
                                       0 ) >= zCURSOR_SET )
   {
      if ( bCtrl == FALSE )
         SetAllSelStatesForEntityForSet( vDiagram, *m_pzsAName,
                                         0, 1, 0 );
      else
      if ( bSelect && GetSelectStateOfEntityForSet( vDiagram,
                                                    *m_pzsAName,
      ...
      {
         bSelect = 0;
         SetSelectStateOfEntityForSet( vDiagram, *m_pzsAName,
                                       0, 1 );
      }
...
//       return( 1 );
   ...
   {
      if ( (lStartIdx < m_lUpdateDisplayPos &&
                                         lEndIdx < m_lUpdateDisplayPos) ||
           (lStartIdx > m_lUpdateDisplayPosMax &&
                                         lEndIdx > m_lUpdateDisplayPosMax) )
      {
         return( 0 );
      }
...
ZER_Diagram::AnalyzeGrid( )
{
   ...
   for ( lIdx = 0; lIdx <= tzERMAXGRID; lIdx++ )
   {
      if ( m_pchGrid[ lIdx ] )
      {
         ...
         {
            lLineCells++;
            if ( (m_pchGrid[ lIdx ] & (tziVERT | tziHORIZ)) ==
                                                (tziVERT | tziHORIZ) )
            {
               lLineCells++;
               lCrossedLines++;
...
ZER_Diagram::GetDeviceContext( )
{
   ...
   if ( m_lScale )
   {
      ...
      m_sizeOldVExt = m_hDC->SetViewportExt( CSize( 100 + m_lScale,
                                                  100 + m_lScale ) );
   }
                              ...
                              ZER_Relationship *pRelationship )
{
   ...
   if ( (m_pchGrid[ lIdx ] & tziENTITY) == 0 )
   {
      ...
      if ( m_pchGrid[ lIdx ] & (tziTEXTHORIZ | tziTEXTVERT) &&
           (m_pchGrid[ lIdx ] & tziTEXT) == 0 )
      {
         ...
         if ( nPos < m_nRelCnt )
         {
            ...
            {
               // Draw a circle to represent minimum cardinality of
               // 0
               if ( bDirection == tzLEFT )
               {
                  rect.left = pt1.x;
...
{
   if ( (PosY( m_lDisplayPosMax ) - PosY( m_lDisplayPos )) <
                                                        (tzERDIAGRAMY - 1) )
   {
      SetScrollRange( SB_VERT, 0,
                      tzERDIAGRAMY -
                      (((PosY( m_lDisplayPosMax ) - PosY( m_lDisplayPos )) + 1)),
                      FALSE );
   ...
   if ( (PosX( m_lDisplayPosMax ) - PosX( m_lDisplayPos )) < (tzERDIAGRAMX - 1) )
   {
      SetScrollRange( SB_HORZ, 0,
                      tzERDIAGRAMX -
                      (((PosX( m_lDisplayPosMax ) - PosX( m_lDisplayPos )) + 1)),
                      FALSE );
...
ZER_Diagram::MapFromOI( WPARAM wFlag )
{
   ...
   if ( vDiagram )
   {
      ...
      if ( nRC == zCURSOR_SET )
      {
         ...
         {
            bSelect = (zBOOL) GetSelectStateOfEntityForSet( vDiagram,
                                                            *m_pzsEName,
                                                            1 );
            ...
            GetIntegerFromAttribute( &lPosY, vDiagram,
                                     *m_pzsEName,
                                     szlER_DiagramPosY );
            GetIntegerFromAttribute( &lPortFactor, vDiagram,
                                     *m_pzsEName,
                                     szlER_PortFactor );
            if ( lPosX || lPosY )
            {
               pZER_Entity = new ZER_Entity( this, lPosX, lPosY,
                                             pchText, pchType[ 0 ],
                                             lPosZKey, lZKey,
                                             lPortFactor, bSelect );
         ...
         while ( nRC == zCURSOR_SET )
         {
            ...
            GetIntegerFromAttribute( &lPosZKey, vDiagram,
                                     *m_pzsAName, szlZKey );
            GetStringFromAttribute( szSrcText, vDiagram,
                                    szlER_RelLink_2, szlName );
            zstrcat( szSrcText, szlLPAREN );
            GetStringFromAttribute( szCard, vDiagram,
                                    szlER_RelLink_2, szlCardMin );
            zstrcat( szSrcText, szCard );
            if ( atoi( szCard ) == 1 )
               bSrcReq = 1;
            else
               bSrcReq = 0;

            zstrcat( szSrcText, szlCOMMA );
            GetStringFromAttribute( szCard, vDiagram,
            ...
            zstrcat( szSrcText, szlRPAREN );
            if ( CheckExistenceOfEntity( vDiagram,
                                         szlER_Entity_2 ) == zCURSOR_NULL )
            {
               bRelError = 1;
               lSrcZKey = 0;
            ...
            GetStringFromAttribute( szTgtText, vDiagram,
                                    szlER_RelLink_2, szlName );
            zstrcat( szTgtText, szlLPAREN );
            GetStringFromAttribute( szCard, vDiagram,
                                    szlER_RelLink_2, szlCardMin );
            zstrcat( szTgtText, szCard );
            if ( atoi( szCard ) == 1 )
               bTgtReq = 1;
            else
               bTgtReq = 0;

            zstrcat( szTgtText, szlCOMMA );
            GetStringFromAttribute( szCard, vDiagram,
                                    szlER_RelLink_2, szlCardMax );
            zstrcat( szTgtText, szCard );
            if ( atoi( szCard ) == 1 )
            ...
            // If the relationship is in error, offer to delete the
            ...
            if ( bRelError )
            {
                                    ...
                                    0 ) == zRESPONSE_YES )
               {
                  ...
                  {
                     OperatorPrompt( m_pZSubtask->m_vDialog,
                                     "ER Relationship Delete",
            ...
            if ( pSrcEntity && pTgtEntity )
            {
               ...
               if ( lPosX || lPosY )
               {
                  ...
                  GetIntegerFromAttribute( &lPosX, vDiagram, *m_pzsAName,
                                           szlER_DiagramE2PosX );
                  GetIntegerFromAttribute( &lPosY, vDiagram, *m_pzsAName,
                                           szlER_DiagramE2PosY );
                  lEndIdx = (lPosY * tzERDIAGRAMX) + lPosX;
                  GetIntegerFromAttribute( &lPosX, vDiagram, *m_pzsAName,
                                           szlER_DiagramBendPosX );
                  GetIntegerFromAttribute( &lPosY, vDiagram, *m_pzsAName,
                                           szlER_DiagramBendPosY );
                  lArcIdx = (lPosY * tzERDIAGRAMX) + lPosX;
               // HH, 2000.01.17
               // GetIntegerFromAttribute( &lFlags, vDiagram,
               //                          *m_pzsAName, "Flags" );
               }
               else
                  nCalcRels++;
...
// DisplayCells( "ZER_Diagram::MapToOI prior", 3, 9, 32, 55 );
        ...
        MiGetUpdateForView( vDiagram ) > 0 )
   {
      ...
      for ( nPos = 0; nPos < m_nEntCnt; nPos++ )
      {
         ...
         {
            if ( SetCursorFirstEntityByInteger( vDiagram,
                                                *m_pzsEName,
                                                szlZKey,
                                                pZER_Entity->m_lPosZKey, 0 )
                                                            == zCURSOR_SET )
            {
               SetAttributeFromInteger( vDiagram,
                                        *m_pzsEName,
                                        szlER_DiagramPosX,
                                        (zLONG) pZER_Entity->PosX( ) );
               SetAttributeFromInteger( vDiagram,
                                        *m_pzsEName,
                                        szlER_DiagramPosY,
                                        (zLONG) pZER_Entity->PosY( ) );
            }
      ...
      for ( nPos = 0; nPos < m_nRelCnt; nPos++ )
      {
         ...
         if ( pZER_Relationship->m_bUpdated )
         {
            ...
            {
               SetAttributeFromInteger( vDiagram,
                        *m_pzsAName, szlER_DiagramE1PosX,
                        (zLONG) PosX( pZER_Relationship->m_lStartIdx ) );
               SetAttributeFromInteger( vDiagram,
                        *m_pzsAName, szlER_DiagramE1PosY,
                        (zLONG) PosY( pZER_Relationship->m_lStartIdx ) );
               SetAttributeFromInteger( vDiagram,
                        *m_pzsAName, szlER_DiagramE2PosX,
                        (zLONG) PosX( pZER_Relationship->m_lEndIdx ) );
               SetAttributeFromInteger( vDiagram,
                        *m_pzsAName, szlER_DiagramE2PosY,
                        (zLONG) PosY( pZER_Relationship->m_lEndIdx ) );
               if ( pZER_Relationship->m_pSrcEntity ==
                                           pZER_Relationship->m_pTgtEntity )
               {
                  SetAttributeFromInteger( vDiagram,
                           *m_pzsAName, szlER_DiagramBendPosX,
                           (zLONG) PosX( pZER_Relationship->m_lArcIdx2 ) );
                  SetAttributeFromInteger( vDiagram,
                           *m_pzsAName, szlER_DiagramBendPosY,
                           (zLONG) PosY( pZER_Relationship->m_lArcIdx2 ) );
               }
               else
               {
                  SetAttributeFromInteger( vDiagram,
                           *m_pzsAName, szlER_DiagramBendPosX,
                           (zLONG) PosX( pZER_Relationship->m_lArcIdx1 ) );
                  SetAttributeFromInteger( vDiagram,
                           *m_pzsAName, szlER_DiagramBendPosY,
                           (zLONG) PosY( pZER_Relationship->m_lArcIdx1 ) );
               }
            }
