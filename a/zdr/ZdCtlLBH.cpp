...
#define SubItemHitTest( plvhti ) \
           (int) SNDMSG( m_hWnd, LVM_SUBITEMHITTEST, 0, \
...
// TraceLineI( "LBH AutoSort: ", m_bAutoSort );
   ...
   if ( m_pzsVName )
   {
      ...
      if ( m_ulListLth )
      {
         ...
         zULONG ulLth = (zUSHORT) (*((zPLONG) (m_pchListInfo +
                                                      2 * sizeof( zLONG ))));
         for ( zSHORT nCol = 1; nCol <= m_nCols; nCol++ )
         {
            ...
            *((zPLONG) (m_pchListInfo + ((nCol + nLists) * sizeof( zLONG ))));
            ...
            {
               k = (zSHORT) *((zPLONG) (m_pchListInfo + ulLth));
               ulLth += sizeof( zLONG );
               m_pCol[ nCol ].pchEntity = new char[ k + 1 ];
               zstrcpy( m_pCol[ nCol ].pchEntity, m_pchListInfo + ulLth );
            ...
            {
               ulLth += sizeof( zLONG );
               m_pCol[ nCol ].pchEntity = new char[ zstrlen( *m_pzsEName ) + 1 ];
               zstrcpy( m_pCol[ nCol ].pchEntity, *m_pzsEName );
            }
            ...
            zstrcpy( m_pCol[ nCol ].pchAttrib, m_pchListInfo + ulLth );
            ...
            else
            {
               ...
               zstrcpy( m_pCol[ nCol ].pchContext, m_pchListInfo + ulLth );
            ...
            k = (zSHORT) *((zPLONG) (m_pchListInfo + ulTagLth));
            ulTagLth += sizeof( zLONG );
            m_pCol[ nCol ].pchTag = new char[ k + 1 ];
            zstrcpy( m_pCol[ nCol ].pchTag, m_pchListInfo + ulTagLth );
            ...
            if ( pch )
            {
               ...
               *pch = 0;
                  ...
                  *pchTab = 0;
                  ...
                  if ( lNLS >= 0 )
                  {
                     ...
                     if ( *pchTab )
                     {
                        ...
                        m_pCol[ nCol ].pchText =
                                           new char[ zstrlen( pchTab ) + 1 ];
                        zstrcpy( m_pCol[ nCol ].pchText, pchTab );
               ...
               j = zstrlen( pch );
               m_pCol[ nCol ].pchMB_Text = new char[ j + 1 ];
               zstrcpy( m_pCol[ nCol ].pchMB_Text, pch );
            }
            else
            {
...
ZListCtrl::GetPreferencesFromIni( )
{
   ...
   zstrcpy( szBuffer, "Ctrl:" );
   zstrcpy( szBuffer + 5, *m_pzsTag );
   ...
   if ( nCol == m_nCols )
   {
      ...
      if ( m_bRememberSort )
      {
         ...
         zstrcpy( szBuffer, "Sort:" );
         zstrcpy( szBuffer + 5, *m_pzsTag );
         GetWindowPreferenceString( m_pZSubtask->m_vDialog,
                                    szBuffer, pchBuffer, nLth );
         zSHORT k = zstrlen( pchBuffer );
         if ( k )
         {
            ...
            // valid "check bytes".
            {
               ...
               while ( pch && *pch )
               {
                  ...
                  {
                     zPCHAR pchMsg = new char[ zstrlen( pchBuffer ) + 256 ];

                     // Invalid entry ... get out!
                     zsprintf( pchMsg, "ZListCtrl invalid sort order: %s=%s",
                               szBuffer, pchBuffer );
                     TraceLineS( pchMsg, "" );
                     pchBuffer[ 0 ] = 0;
                     m_nColLastSorted = -1;
...
ZListCtrl::DestroyWindow( )
{
   ...
   {
      CWnd *pButtonWnd = GetWindow( GW_CHILD );
      SetWindowLong( pButtonWnd->m_hWnd, GWL_WNDPROC,
                     (DWORD) m_lpfnHeaderButtonWndProc );
      m_lpfnHeaderButtonWndProc = 0;
   }
   ...
   zPCHAR pch = new char[ sizeof( "-2147483647," ) * (m_nCols + 1) ];
   zSHORT nLth = 0;
   zSHORT k;

   for ( k = 0; k < m_nCols; k++ )
   {
      zltoa( GetColumnWidth( k ), pch + nLth );
      nLth = zstrlen( pch );
      if ( k < m_nCols - 1 )
      {
         pch[ nLth ] = ',';
...
ZListCtrl::SetColorAttribute( zCPCHAR cpcAttributeName, zSHORT nCol )
{
   ...
   m_pCol[ nCol ].pchColorAttribute = new char[ zstrlen( cpcAttributeName ) + 1 ];
   zstrcpy( m_pCol[ nCol ].pchColorAttribute, cpcAttributeName );
}
...
ZListCtrl::SetDisplaySelSet( zSHORT nSelSet )
{
   ...
   {
      TraceLineI( "ZListCtrl::SetDisplaySelSet warning - "
                             ...
                             zCPCHAR cpcStringProperty )
{
   ...
   if ( lPropertyType == zCONTROL_PROPERTY_TAG )
   {
      if ( ulNumericProperty > 0 )
      {
         ...
         m_pCol[ nCol ].pchTag = new char[ zstrlen( cpcStringProperty ) + 1 ];
         zstrcpy( m_pCol[ nCol ].pchTag, cpcStringProperty );
         return( 0 );
      }
   }
                             ...
                             zPCHAR  pchStringProperty )
{
   ...
   if ( lPropertyType == zCONTROL_PROPERTY_TAG )
   {
      if ( *pulNumericProperty > 0 )
      {
         ...
         zstrcpy( pchStringProperty, m_pCol[ nCol ].pchTag );
         return( 0 );
      }
   }
                          ...
                          zULONG ulValue )
{
   ...
   if ( lStatusType == zCONTROL_STATUS_REPAINT )
   {
      if ( mIs_hWnd( m_hWnd ) )
      {
         ...
         if ( ulValue == 0 || m_vAppList == 0 || m_pchListInfo == 0 ||
              MiValidView( m_vAppList ) == 0 )
         {
            Invalidate( );
         }
         else
         ...
         // zCHAR  szMsg[ 128 ];
         // zLONG lTop = GetTopIndex( );
            zLONG lEntityNbr = GetRelativeEntityNumber( m_vApp, *m_pzsEName,
                                                        m_pzsScope ? *m_pzsScope : (zCPCHAR) 0,
                                                        m_ulFlag );
            GetItemRect( lEntityNbr, rect, LVIR_BOUNDS );
         // zsprintf( szMsg, "REPAINT top(%ld) item %ld:", lTop, lEntityNbr );
                  ...
                  zLPLB_COL pCol )
{
   ...
   if ( vApp )
   {
      ...
      zsprintf( szMsg, " App: %6d %s", ulKey, szText );
   ...
   if ( vAppList )
   {
      ...
      GetVariableFromAttribute( szText, 0, zTYPE_STRING,
                                ...
                                pCol[ 1 ].pchContext ?
                                    zACCEPT_NULL_ENTITY :
                                 zACCEPT_NULL_ENTITY | zUSE_DEFAULT_CONTEXT );
      zsprintf( szMsg, " AppList: %6d %s", ulKey, szText );
      TraceLineS( cpcText, szMsg );
   }
}
#else
                                 ...
                                 zSHORT nMaxLth )
{
   ...
   if ( m_lEntityCnt > 0 )
   {
      if ( MiValidView( m_vAppList ) == 0 ||
           CheckExistenceOfEntity( m_vAppList, *m_pzsEName ) != 0 ) // no data to display
      {
...
// DisplayCellData( "ZGrid::FormatTextForCell1" );
                       ...
                       m_pZSubtask->m_vDialog, zLEVEL_SUBTASK ) <= 0 )
   {
           ...
           (m_ulMapActFlag2 & zMAPACT_MAPPED_FROM_OI) == 0 )
      {
         ...
         TraceLineS( "ZListCtrl::FormatTextAtPosition Retry MapFromOI: ",
                     szMsg );
                         ...
                         m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, 0 ) < 0 ) // m_pzsAName
   {
           ...
           m_pZSubtask->m_pZTask->m_nTraceAction > 1 )
      {
         ...
         zsprintf( szMsg,
                   "Error in entity count for view: %s  entity: %s for %s.%s.%s",
                   *m_pzsVName, *m_pzsEName, *m_pZSubtask->m_pzsDlgTag,
                   *m_pZSubtask->m_pzsWndTag, *m_pzsTag );
         TraceLineS( "ZListCtrl::FormatTextAtPosition ", szMsg );
      }
   ...
   {
      SetCursorNextEntity( m_vAppList, *m_pzsEName,
                           m_pzsScope ? *m_pzsScope : (zCPCHAR) 0 );  // m_pzsAName
   ...
   if ( ulKey != ulEntityKey &&
        SetEntityCursor( m_vAppList, *m_pzsEName, 0,
                         m_ulFlag | zQUAL_ENTITYKEY | zPOS_FIRST,
                         (zCPVOID) &ulKey, 0, 0, 0,
                         m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, 0 ) < zCURSOR_SET ) // m_pzsAName
   {
      if ( m_nRetryMapFromOI < 2 &&
           m_pZSubtask->m_pZTask->m_nTraceAction > 1 )
      {
         ...
         zsprintf( szMsg,
                   "Entity Key Error: %s  Entity: %s  row: %d for %s.%s.%s",
                   *m_pzsVName, *m_pzsEName, lRow + 1, *m_pZSubtask->m_pzsDlgTag,
                   *m_pZSubtask->m_pzsWndTag, *m_pzsTag );
         TraceLineS( "ZListCtrl::FormatTextAtPosition ", szMsg );
      }
...
// if ( zstrcmp( *m_pzsTag, "DKS" ) == 0 )
   if ( m_bSortDisplay )
   {
      ...
      zsprintf( szMsg, "ZListCtrl::FormatTextAtPosition Tag: %s  "
        ...
        (m_pCol[ nCol ].pchColorAttribute || m_pCol[ 0 ].pchColorAttribute) )
   {
      ...
      if ( pPairedList->Find( nCol, 0, 0, 0, 1 ) == 0 )
      {
         ...
         GetStringFromAttribute( szValue, m_vAppList, *m_pzsEName,
                                 m_pCol[ nCol ].pchColorAttribute ?
                                   m_pCol[ nCol ].pchColorAttribute :
                                   m_pCol[ 0 ].pchColorAttribute );

         // Use the setting for the column (if it exists).  Otherwise
         // use the setting for the whole row if we can find it.
         zSHORT k;
   ...
   TranslateValue( pchText, TRUE, nCol - 1 );
                   ...
                   zPVOID pvSortData )
{
   ...
   if ( lKey1 != pListCtrl->m_lKey1 )
   {
      ...
      if ( SetEntityCursor( pListCtrl->m_vApp1, *(pListCtrl->m_pzsEName), 0,
                            ...
                            (zCPVOID) &lKey1,
                            0, 0, 0, *(pListCtrl->m_pzsScope), 0 ) < 0 )
      {
         pListCtrl->m_lKey1 = -1;
      }
   }
...
ZListCtrl::SortByDisplayData( )
{
   ...
   zPCHAR pchSortBuffer = new char[ m_csSortBuffer.GetLength( ) + 1 ];
   zPCHAR pchEntity;
   zPCHAR pchAttrib;
   zPCHAR pchContext;
   zPCHAR pchEnd;
   ...
   zstrcpy( pchSortBuffer, m_csSortBuffer );
   ...
   while ( *pchEntity )
   {
      ...
      {
         TraceLineS( "ZListCtrl::SortByDisplayData Unexpected Entity/Attribute condition: ",
                     pchEntity );
         break;
      }
      ...
      {
         TraceLineS( "ZListCtrl::SortByDisplayData Unexpected Ascend/Descend condition: ",
                     pchEnd );
         break;
      }
      ...
      if ( *pchEnd == '[' )
      {
         ...
         {
            TraceLineS( "ZListCtrl::SortByDisplayData Unexpected Scope condition: ",
                        pchEntity );
            pchContext = 0;
            break;
         }
      ...
      for ( k = 1; k <= m_nCols; k++ )
      {
         if ( zstrcmp( pchEntity, m_pCol[ k ].pchEntity ) == 0 &&
              ...
              (pchContext == 0 ||
              zstrcmp( pchContext, m_pCol[ k ].pchContext ) == 0) )
         {
            if ( bAscend )
      ...
      if ( k > m_nCols )
      {
         TraceLineS( "ZListCtrl::SortByDisplayData Unexpected Entity name: ",
                     pchEntity );
         break;
      }
   ...
   HeapSortArray( (CArray<void *, void *> *) &m_EntityKeyList,
                  (zFARPROC_COMPARE) fnCompareListData,
                  m_lEntityCnt, this );
...
//    TraceLineS( "ZListCtrl::MapFromOI m_vAppList null ", *m_pzsTag );
   ...
   if ( m_pzsVName == 0 || **m_pzsVName == 0 ||
        GetViewByName( &m_vApp, *m_pzsVName,
                       m_pZSubtask->m_vDialog, zLEVEL_ANY ) <= 0 )
   {
   ...
   //            *m_pzsTag, m_csSortBuffer, m_bRememberSort ? "True" : "False" );
      if ( m_bRememberSort && m_csSortBuffer.IsEmpty( ) == FALSE )
      {
         ...
         {
            m_SortedHeaderCtrl.SetSortImage( m_nColLastSorted - 1,
                                             m_chAscendDescend == 'A' ?
                                                              TRUE : FALSE );
         }
   ...
   while ( m_ulListLth )  // purist's goto
   {
      ...
      if ( CheckEntityInView( m_vApp, *m_pzsEName ) != 0 ||
           SetEntityCursor( m_vApp, *m_pzsEName, 0,
                            m_ulFlag | zPOS_FIRST | zTEST_CSR_RESULT,
                            0, 0, 0, 0,
                            m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, 0 ) < 0 )
      {
         break;  // goto end of while
      }
   ...
   // TraceLineX( "ZListCtrl::MapFromOI creating m_vAppList ",
   //             (zLONG) m_vAppList );

   // DisplayEntityKey( "ZListCtrl::MapFromOI",
         ...
         for ( zSHORT nCol = 1; nCol <= m_nCols; nCol++ )
         {
                 ...
                 m_pCol[ nCol ].pchAttrib[ 0 ] )
            {
               ...
               // if ( MiGetUpdateForView( m_vApp ) <= 0 )
                  ...
                  {
                     TraceLineS( "Listbox edit length restricted ",
                                 "to 255 characters" );
                     m_bLabelEdit = FALSE;
                     ModifyStyle( LVS_EDITLABELS, 0L );
                  }
   ...
   //    TraceLineS( "", "" );
      ...
      if ( m_chHilitePosition == 'Y' && m_vApp &&
      ...
      // DisplayEntityKey( "ZListCtrl::MapFromOI Relative",
      //                   m_vApp, m_vAppList, m_pCol );
         ...
         m_lEntityNbr = GetRelativeEntityNumber( m_vApp, *m_pzsEName,
                                                 m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, m_ulFlag );
      ...
      // if ( m_bSortDisplay )
         {
            ...
            if ( ulKey != GetEntityKey( m_vAppList, *m_pzsEName ) )
            {
               ...
               if ( ulKey != GetEntityKey( m_vAppList, *m_pzsEName ) )
               {
                  TraceLine( "ZListCtrl::MapFromOI ... Refresh of ListBox: %s required?",
                             *m_pzsTag );
...
// DisplayEntityKey( "ZListCtrl::MapFromOI term",
//                   m_vApp, m_vAppList, m_pCol );
...
ZListCtrl::AddCol( zCPCHAR cpcText, zLONG lColumn )
{
   ...
   while ( m_nCols < lColumn )
   {
      ...
      if ( m_nCols == lColumn )
      {
         m_pCol[ m_nCols ].pchText = new char[ zstrlen( cpcText ) + 1 ];
         zstrcpy( m_pCol[ m_nCols ].pchText, cpcText );
      }
      else
      {
      ...
      {
         TraceLineI( "ZListCtrl::SetRowColText InsertColumn failed: ",
                     m_nCols );
      }
   }
}
...
ZListCtrl::SetRowColText( zCPCHAR cpcText, zLONG lRow, zLONG lColumn )
{
   ...
   if ( lColumn <= m_nColsMax && lColumn > 0 )
   {
      if ( lRow == 0 )
      {
         ...
         zstrcpy( m_pCol[ lColumn ].pchText, cpcText );
      }
   ...
   // lvc.cchTextMax = zstrlen( m_pCol[ lColumn ].pchText ) + 1;
      ...
      if ( lColumn == m_nCols - 1 )
      {
                  ...
                  m_pCol[ lColumn ].pchAttrib[ 0 ] == 0) )
         {
            ...
            {
               TraceLineI( "ZListCtrl::SetRowColText DeleteColumn failed: ",
                           m_nCols );
            }
                          ...
                          zLONG lRow, zLONG lColumn )
{
   ...
   {
      if ( lRow == 0 )
         zstrncat( pchText, m_pCol[ lColumn ].pchText, uLth );
   }
                           ...
                           zLONG lColumn )
{
   ...
   else
   {
      ...
      if ( lColumn > 0 && lColumn <= m_nColsMax )
      {
         ...
         m_pCol[ lColumn ].pchEntity = new char[ zstrlen( cpcEntity ) + 1 ];
         zstrcpy( m_pCol[ lColumn ].pchEntity, cpcEntity );
         m_pCol[ lColumn ].pchAttrib = new char[ zstrlen( cpcAttrib ) + 1 ];
         zstrcpy( m_pCol[ lColumn ].pchAttrib, cpcAttrib );
         if ( cpcContext && *cpcContext )
         {
            m_pCol[ lColumn ].pchContext =
                                      new char[ zstrlen( cpcContext ) + 1 ];
            zstrcpy( m_pCol[ lColumn ].pchContext, cpcContext );
         }
      ...
      // if ( lColumn == m_nCols - 1 )
         if ( lColumn == m_nCols )  // - 1 ??? dks 2003.07.21
         {
                     ...
                     m_pCol[ lColumn ].pchAttrib[ 0 ] == 0) )
            {
               ...
               if ( DeleteColumn( m_nCols ) == FALSE )
               {
                  TraceLineI( "ZListCtrl::SetCtrlMapping DeleteColumn failed: ",
                              m_nCols );
               }
                           ...
                           zLONG lColumn )
{
   ...
   {
      if ( m_pzsVName )
         zstrcpy( pchViewName, *m_pzsVName );
      else
         pchViewName[ 0 ] = 0;
      ...
      if ( lColumn > 0 && lColumn <= m_nCols )
      {
         if ( m_pCol[ lColumn ].pchEntity )
            zstrcpy( pchEntity, m_pCol[ lColumn ].pchEntity );

         if ( m_pCol[ lColumn ].pchAttrib )
            zstrcpy( pchAttrib, m_pCol[ lColumn ].pchAttrib );

         if ( m_pCol[ lColumn ].pchContext )
            zstrcpy( pchContext, m_pCol[ lColumn ].pchContext );
      }
      else
      {
         pchEntity[ 0 ] = 0;
...
//x   zBOOL bLock = LockWindowUpdate( );  causes repaint of entire desktop
      ...
      do    // purist's goto
      {
         ...
         else
         {
            ...
            lOffset = GetRelativeEntityNumber( m_vApp, *m_pzsEName,
                                               m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, m_ulFlag );
         }
...
// TraceLine( "ZListCtrl::SelectAll (Multi=%c): %s", m_chMultiSelect, *m_pzsTag );
   if ( m_chMultiSelect == 'Y' )
   {
      ...
      if ( (m_ulFlag & zQUAL_SCOPE_OI) &&
           (m_pzsScope == 0 || **m_pzsScope == 0) )
      {
         SetAllSelectStatesForEntity( m_vApp, *m_pzsEName, TRUE,
                                      (zCPCHAR) zSCOPE_OI );
      }
      else
      {
...
// if ( zstrcmp( "AttrList", *m_pzsTag ) == 0 )
   ...
   zSHORT nRC = SetEntityCursor( vApp, *m_pzsEName, 0,
                                 zPOS_FIRST | ulFlag,
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      if ( GetSelectStateOfEntity( vApp, *m_pzsEName ) )
      {
         ...
         {
            lvi.iItem = GetRelativeEntityNumber( vApp, *m_pzsEName,
                                                 m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, ulFlag );
         }
      ...
      SetSelectSetForView( vApp, m_nSelectSelSet );
      nRC = SetEntityCursor( vApp, *m_pzsEName, 0,
                             zPOS_NEXT | ulFlag,
                             0, 0, 0, 0, m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, 0 );
   }
   ...
   if ( m_pchListInfo && m_pzsVName )
   {
      ...
      if ( vApp )
      {
         ...
         if ( SetEntityCursor( vApp, *m_pzsEName, 0,
                               ...
                               0, 0, 0, 0, m_pzsScope ? *m_pzsScope : (zCPCHAR) 0,
                               0 ) > zCURSOR_UNCHANGED )
         {
            ...
            if ( nSelCnt > 1 )
            {
               ...
               for ( iRelIdx = 0; iRelIdx < nSelCnt; iRelIdx++ )
               {
                  SelectHilitedEntity( vApp, *m_pzsEName,
                                       m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, pIntArray[ iRelIdx ] );
               }
...
ZListCtrl::OnSetFocus( CWnd *pWndLostFocus )
{
   ...
   if ( pWndLostFocus == 0 ||
        mIs_hWnd( pWndLostFocus->m_hWnd ) == 0 ||
        pWndLostFocus->GetParent( ) != this )
   {
...
#if 0
   if ( GetViewByName( &m_vApp, m_csViewName,
                       m_pZSubtask->m_vDialog, zLEVEL_SUBTASK ) <= 0 )
   {
      if ( MiValidView( m_vAppList ) )
      {
         DropNameForView( m_vAppList, m_csViewNameList,
                          m_pZSubtask->m_vDialog, zLEVEL_SUBTASK );
      }
...
//                    *((zLONG *) (m_pchDragDrop + sizeof( zLONG ))) ) == 0 )
         ...
         if ( m_bAutoDragSort &&
              (m_chMultiSelect == 0 ||
               (m_chMultiSelect &&
                (nSelSrc == -1 ||
                 GetNextItem( nSelSrc, LVNI_SELECTED ) == -1))) )
         {
            CRect rect;
...
//                                              m_pzsScope ? *m_pzsScope : (zCPCHAR) 0 );
                  ...
                  if ( nSelTgt < 0 )
                  {
                     ...
                     {
                        nRC = SetEntityCursor( vTgt, *m_pzsEName, 0,
                                               zPOS_LAST | m_ulFlag,
                                               0, 0, 0, 0,
                                               m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, 0 );
                     }
...
//BL Fix for Auto Drag Sort
                  SetEntityCursor( vSrc, *m_pzsEName, 0,
                                   zPOS_FIRST | zPOS_RELATIVE | m_ulFlag,
                                   0, 0, 0,
                                   nSelSrc > nSelTgt ? nSelTgt + 1 : nSelTgt,
                                   m_pzsScope ? *m_pzsScope : (zCPCHAR) 0, 0 );
         ...
         // if the drop was accepted or not ... it doesn't really matter
         // to the list box.
...
// TraceLineS( "ZListCtrl::OnMouseMove", " default" );
   ...
   if ( SubItemHitTest( &HitTestInfo ) >= 0 )
   {
      m_pZSubtask->InformDriverOfFlyBy( this,
                                 m_pCol[ HitTestInfo.iSubItem + 1 ].pchMB_Text );
      if ( m_wndListTip.GetSafeHwnd( ) && GetFocus( ) == this )
      {
         ...
         GetSubItemRect( HitTestInfo.iItem, HitTestInfo.iSubItem,
                         LVIR_BOUNDS, rectCell );
         if ( HitTestInfo.iSubItem == 0 )
         {
            CRect rectCell2;
            GetSubItemRect( HitTestInfo.iItem, 1,
                            LVIR_BOUNDS, rectCell2 );
            rectCell.right = rectCell2.left - 1;
         }

         FormatTextAtPosition( szText, HitTestInfo.iItem,
                               HitTestInfo.iSubItem, sizeof( szText ) );
...
ZListCtrl::SortByColumn( zSHORT nCol, zSHORT nAscending )
{
   ...
   if ( m_vApp && m_pCol[ nCol ].pchAttrib[ 0 ] )
   {
      ...
      zstrcpy( szBuffer, m_pCol[ nCol ].pchEntity );
      nLth = zstrlen( szBuffer );
      szBuffer[ nLth++ ] = '.';
      zstrcpy( szBuffer + nLth, m_pCol[ nCol ].pchAttrib );
      nLth = zstrlen( szBuffer );
      szBuffer[ nLth++ ] = ' ';
      szBuffer[ nLth ] = 0;
   ...
   // "Abteilung.InterneNummer A Abteilung.BezOrgText A" <== m_csSortBuffer
      ...
      if ( nFind >= 0 )
      {
         zPCHAR pch;
         zPCHAR p =
            m_csSortBuffer.GetBuffer( m_csSortBuffer.GetLength( ) + nLth );
         if ( *(p + nFind + nLth + 1) )
            zstrcpy( p + nFind, p + nFind + nLth + 2 );
         else
            p[ nFind ] = 0;
      ...
      if ( m_pCol[ nCol ].pchContext )
      {
         szBuffer[ nLth++ ] = ' ';
         szBuffer[ nLth++ ] = '[';
         zstrcpy( szBuffer + nLth, m_pCol[ nCol ].pchContext );
         nLth = zstrlen( szBuffer );
         szBuffer[ nLth++ ] = ']';
      }
...
fnTextState( UINT uState, zPCHAR pchTextState )
{
   ...
   pchTextState[ 0 ] = '(';
   zltoa( uState, pchTextState + 1 );
   nLth = zstrlen( pchTextState );
   pchTextState[ nLth++ ] = ')';
   if ( uState )
   {
      pchTextState[ nLth++ ] = ':';
   ...
   while ( pState->pchName[ 0 ] )
   {
      if ( pState->uState & uState )
      {
         ...
         j++;
         zstrcpy( pchTextState + nLth, pState->pchName );
         nLth += zstrlen( pchTextState + nLth );
      }
...
{
   zSHORT nLth = zstrlen( pchString );
   zSHORT nFormatLth = zstrlen( cpcFormat );
   zSHORT i, j, k;
   ...
   pchNewString[ k ] = 0;
   zstrcpy( pchString, pchNewString );
   delete [] pchNewString;
}
...
// rectItem.OffsetRect( 0, sizeNormal.cy );
   ...
   // if ( lPageNbr > 1 )
   {
      ...
      zltoa( lPageNbr, szPageNbr );
      k = zstrlen( szPageNbr );
      szPageNbr[ k ] = '/';
      zltoa( m_nMaxPage, szPageNbr + k + 1 );
      rectItem = rect;
   ...
   if ( nRC == 0 && lLast < m_EntityKeyList.GetSize( ) &&
        FormatTextAtPosition( szText, lLast, 0,
                              sizeof( szText ) ) == 0 )
   {
      nRC = 1;     // more pages
   }
...
// TraceRect( "ZListCtrl::DrawItem rectAllLabels", rectAllLabels );
   ...
   if ( cs.GetLength( ) == 0 )
      ...
      MakeShortString( pDC, szText, m_pCol[ 1 ].ulLth, cs,
                       rectItem.right - rectItem.left, 2 * OFFSET_FIRST );

   if ( zstrcmp( szText, cs ) == 0 && m_pCol[ 1 ].nJustify == LVCFMT_RIGHT )
      uFormat |= DT_RIGHT;
   else
      uFormat |= DT_LEFT;
   ...
   // TraceRect( "ZListCtrl::DrawItem rect", rectItem );
      ...
      MakeShortString( pDC, szText, m_pCol[ nCol + 1 ].ulLth, cs,
                       rectItem.right - rectItem.left, 2 * OFFSET_OTHER );
                 ...
                 zSHORT  nMaxReturnLth )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pListCtrl )
      {
         ...
         zstrncpy( pchReturnOrder, pListCtrl->m_csSortBuffer,
                   nMaxReturnLth - 1 );
         pchReturnOrder[ nMaxReturnLth ] = 0;
         return( 0 );
      }

      TraceLineS( "drvr - Invalid control type for LB_GetSortOrder ",
                  cpcCtrlTag );
   }
                 ...
                 zCPCHAR cpcNewSortOrder )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for LB_SetSortOrder ",
                  cpcCtrlTag );
   }
                        ...
                        zLONG   lDisplayPos )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for LB_SetSelectedPosition ",
                  cpcCtrlTag );
   }
                          ...
                          zLONG   lColumn )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for LB_SetColorAttributeName ",
                  cpcCtrlTag );
   }
                        ...
                        zLONG   lFlags )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for LB_SetColorAssociation ",
                  cpcCtrlTag );
   }
                            ...
                            zCPCHAR cpcCtrlTag )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for LB_RemoveColorAssociations ",
                  cpcCtrlTag );
   }
