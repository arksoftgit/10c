        ...
        pXRA->m_vXRA )
   {
      ...
      if ( PositionOnRemoteCtrl( this, vXRA, FALSE ) )
      {
         ...
         if ( szImageURL[ nLth ] != 'Y' )
         {
            ...
            GetStringFromAttribute( szBitmap, vXRA,
                                    "Bitmap", "URL" );
            ...
            zstrcpy( szImageURL +nLth, szBitmap );
                  ...
                  zPCHAR   pchBitmap )
{
   ...
   if ( pchSemiColon && *pchSemiColon == '@' ) // V.E.A.P.X reference
   {
      ...
      if ( *szFileName )
      {
         if ( m_pchVEAPX == 0 || zstrcmp( pchSemiColon, m_pchVEAPX ) != 0 )
         {
            ...
            m_pchVEAPX = new char[ zstrlen( pchSemiColon ) + 1 ];
            zstrcpy( m_pchVEAPX, pchSemiColon );
         }
   ...
   m_pchBitmapFileName = new char[ zstrlen( pchSemiColon ) + 1 ];
   zstrcpy( m_pchBitmapFileName, pchSemiColon );
   ...
   if ( pchSemiColon )
   {
           ...
           (pchSemiColon[ 0 ] != '\\' && pchSemiColon[ 1 ] != '\\') )
      {
         ...
         if ( szZeidonPath[ 0 ] )
         {
            ...
            if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 1 )
            {
               TraceLineS( "ZBmpCtl::LoadDib: Cannot access bitmap file "
                           "in ResourcePath ", szWorkString );
            }
         }
...
// TraceLineS( "ZBmpCtl::MapFromOI ", *m_pzsTag );
   ...
   if ( m_pchBitmapFileName && *m_pchBitmapFileName &&
        m_dib.m_bBitmapLoaded == FALSE )
   {
...
ZBmpCtl::SetBitmapFileName( zCPCHAR cpcBitmapFileName )
{
        ...
        zstrcmp( cpcBitmapFileName, m_pchBitmapFileName ) != 0 )
   {
      ...
      m_pchBitmapFileName = new char[ zstrlen( cpcBitmapFileName ) + 1 ];
      zstrcpy( m_pchBitmapFileName, cpcBitmapFileName );
...
// TraceLineS( "ZBmpCtl::OnPaint", "*************************" );
                    ...
                    FALSE, 0, TRUE ) == FALSE )
   {
      ...
      if ( m_pzsText && *m_pzsText )
         ...
         dc.DrawText( "Unknown BmpCtl", -1, rectTgt,
...
ZBmpCtl::PreTranslateMessage( MSG *pMsg )
{
   if ( pMsg->message == WM_SYSKEYDOWN || pMsg->message == WM_KEYDOWN )
   {
      ...
      if ( ad.uVK != VK_MENU && ad.fAlt &&
           (pzma = m_pZSubtask->FindCtrlWithQuickKey( this,
                                                      (char) ad.uVK )) != 0 &&
                       ...
                       zCPCHAR  cpcBitmapFileName )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      GetAppOrWorkstationValue( vSubtask, "ExecutableRemoteImage",
                                szImage, sizeof( szImage ) );
      zstrcpy( pchName, cpcBitmapFileName );
      if ( szImage[ 0 ] )
      {
         zPCHAR pchBMP;
      ...
      GetAppOrWorkstationValue( vSubtask, "ExecutableRemoteImageStrip",
                                szImage, sizeof( szImage ) );
      if ( szImage[ 0 ] )
      {
         zPCHAR pch = new char[ nLth ];
      ...
      TraceLineS( "drvr - Invalid control type for BMP_SetBitmapFileName ",
                  cpcCtrlTag );
   }
