...
// AutoGrayActions( );     this is too early
   ...
   {
//??? m_pMenu = new CMenu( (HINSTANCE) m_pZDrApp->m_hInstance,
//???                      TResId( MDI_MENU ) );
   }

   // Check if any menu entities exist.
   if ( cpcMenuName[ 0 ] &&
        SetCursorFirstEntityByString( m_vDialog, szlMenu, szlTag,
                                      cpcMenuName, 0 ) > zCURSOR_UNCHANGED &&
...
//    {
//       m_pTrackMenu->TrackPopupMenu( TPM_LEFTALIGN | TPM_RIGHTBUTTON,
//                                     pt.x, pt.y, 0, *m_pZFWnd );
//       return;
//    }
//    else
      {
   ...
   // TraceLineS( "CreateZeidonPopupMenu TrackPopupMenu - ", cpcMenuName );
      ...
      m_pTrackMenu->TrackPopupMenu( TPM_LEFTALIGN | TPM_RIGHTBUTTON,
                                    pt.x, pt.y, pZSubtaskMain->m_pZFWnd, 0 );
                               ...
                               zUSHORT uOptBaseID )
{
   ...
   zmemset( OptDef.Tag, 0xFE, sizeof( OptDef ) );
   zmemset( OptMapDef.Tag, 0xFE, sizeof( OptMapDef ) );
...
// OptMapDef.CheckByte = 0xFE;
   ...
   while ( nRC > zCURSOR_UNCHANGED )        // Loop for each Option entity
   {
      ...
      if ( OptDef.CheckByte != 0xFE )
         MessageBox( 0, "OptDef Structure Length Error",
                     "GetStructFromEntityAttrs", MB_OK );
   // zPCHAR pchCheckByte = (zPCHAR) &(OptDef.CheckByte);
   // pchCheckByte--;
   // if ( *pchCheckByte == (char) 0xFE )
   //    TraceLineS( "zdrmenu OptDef CheckByte error??", "" );
#endif

      GetAddrForAttribute( (zPCHAR *) &pch, vSubOption, szlOpt, szlD_Text );
      if ( *pch )
         zstrcpy( OptDef.Text, pch );
      ...
      if ( CheckExistenceOfEntity( vSubOption, szlOptAct ) > zCURSOR_UNCHANGED )
      {
         ...
         if ( pch[ 0 ] &&
              SetCursorFirstEntityByString( m_vDialog, szlAct, szlTag,
                                            pch, 0 ) > zCURSOR_UNCHANGED )
         {
            zULONG  ulIdx;

            // Get the sync index for this action.
            GetIntegerFromAttribute( (zPLONG) &ulIdx, m_vDialog,
                                     szlAct, "SyncIdx" );
      ...
      {
         GetStructFromEntityAttrs( (zPCHAR) OptMapDef.Tag,
                                   vSubOption, szlOptMap );
#ifdef DEBUG_LODSTRUCT
         if ( OptMapDef.CheckByte != 0xFE )
            MessageBox( 0, "OptMapDef Structure Length Error",
                        "GetStructFromEntityAttrs", MB_OK );
      // zPCHAR pchCheckByte = (zPCHAR) &(OptMapDef.CheckByte);
      // pchCheckByte--;
      // if ( *pchCheckByte == (char) 0xFE )
      //    TraceLineS( "zdrmenu OptMapDef CheckByte error??", "" );
#endif
      }
      else
      {
      ...
      zstrcpy( szOptionText, OptDef.Text );
      if ( CheckExistenceOfEntity( vSubOption,
                                   szlOptOpt ) > zCURSOR_UNCHANGED )
      {
         ...
         uOptID = fnRecurseSubOptions( vSubOption, pZBinOpt, pSubMenu,
                                       pZOption, uOptID, uOptID, uOptBaseID );
      ...
      else
      {
         ...
         if ( CheckExistenceOfEntity( vSubOption, szlOptAct ) == 0 )
         {
            ...
            SetEntityCursor( vAct, szlAct, 0, zPOS_FIRST | zQUAL_ENTITYCSR,
                             vSubOption, szlOptAct, 0, 0, 0, 0 );
            if ( SetEntityCursor( vAct, szlActHot, 0, zPOS_FIRST,
                                  0, 0, 0, 0, 0, 0 ) == 0 )
            {
//             SetEntityCursor( vAct, szlHotkey, 0,
//                              zPOS_FIRST | zQUAL_ENTITYCSR,
//                              vAct, szlActHot, 0, 0, 0, 0 );
               GetAddrForAttribute( (zCOREMEM) &pch, vAct,
                                    szlActHot, szlTitle );
               if ( *pch )
               {
                  zstrcat( szOptionText, "\t" );
                  zstrcat( szOptionText, pch );
               }
            }
...
ZOption::SetCheckForOption( )
{
                       ...
                       m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
   {
      ...
      {
         GetVariableFromAttribute( szText, 0, zTYPE_STRING, 64, vApp,
                                   *m_pzsEName, *m_pzsAName, *m_pzsContext,
                                   **m_pzsContext ? 0 : zUSE_DEFAULT_CONTEXT );
#ifdef DEBUG_ALL
...
#endif
         if ( zstrcmp( *m_pzsValueOn, szText ) == 0 )
            m_bChecked = TRUE;
      }
   }
                     ...
                     ZSubtask *pZSubtask )
{
   ...
   if ( m_bParent )
   {
      ...
      if ( pZOption )
         nRC = pMenu->InsertMenu( pZOption->m_uOptionID, uFlags,
                                  (UINT) pSubMenu->Detach( ), *m_pzsText );
      else
         nRC = pMenu->AppendMenu( uFlags, (UINT) pSubMenu->Detach( ),
                                  *m_pzsText );
