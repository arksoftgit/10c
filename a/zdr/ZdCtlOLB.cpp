...
ZOutlookBar::SetFolderText( const int nIdx, zCPCHAR cpcText )
{
   ...
   pbf->m_pchName = new char[ zstrlen( cpcText ) + 1 ];
   zstrcpy( pbf->m_pchName, cpcText );
}
...
ZOutlookBar::SetItemText( const int nIdx, zCPCHAR cpcText )
{
   ...
   pi->m_pchItem = new char[ zstrlen( cpcText ) + 1 ];
   zstrcpy( pi->m_pchItem, cpcText );
}
...
ZOutlookBar::ZBarFolder::ZBarFolder( zCPCHAR cpcName, DWORD exData )
{
   ...
   if ( cpcName )
   {
      m_pchName = new char[ zstrlen( cpcName ) + 1 ];
      ASSERT( m_pchName );
      zstrcpy( m_pchName, cpcName );
   }
                                 ...
                                 DWORD dwData )
{
   ...
   if ( cpcName )
   {
      m_pchItem = new char[ zstrlen( cpcName ) + 1 ];
      ASSERT( m_pchItem );
      zstrcpy( m_pchItem, cpcName );
   }
}
