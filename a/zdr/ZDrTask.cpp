...
ZSocketTask::ZSocketTask( SOCKET sock, ZTask *pZTask )
{
   ...
   zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] ZSocketTask::ctor: pCacheWndList: ",
             this,
             AfxGetThread( ),
             m_pWinThread,
             m_sock );
...
ZSocketTask::~ZSocketTask( )
{
   ...
   zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] ZSocketTask::dtor: "
                    "pCacheWndList: %ld   SysMutexLockCnt: ",
             this,
             AfxGetThread( ),
             m_pWinThread,
             m_sock,
             (zLONG) m_pCacheWndList );
   TraceLineI( szMsg, m_lSysMutexLockCnt );
#endif

   if ( m_lSysMutexLockCnt )
   {
      MessageSend( m_vSubtask, 0,
                   "ZSocketTask::dtor", "Mutex leak detected",
...
ZSocketTask::SendZDxFrameMessage( WPARAM wParam )
{
   ...
   if ( g_pSocketList )
   {
      ...
      if ( m_pZDxFrame == 0 ||
           mIs_hWnd( m_pZDxFrame->m_hWnd ) == FALSE ||
           ::SendMessageTimeout( m_pZDxFrame->m_hWnd, zZEIDON_REMOTE_SOCKET,
                                 lPos, (LPARAM) g_pSocketList,
                                 SMTO_ABORTIFHUNG | SMTO_BLOCK,
                                 10000, (zPULONG) &lResult ) == 0 )
      {
         TraceLineS( "ZSocketTask::SendZDxFrameMessage: ",
                     "SendMessageTimeout" );
      }
   }
...
ZSocketTask::LockMutex( zBOOL bLock, zCPCHAR cpcText )
{
   ...
   zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] %sMutex: %s LockCnt = ",
             this,
             AfxGetThread( ),
             m_pWinThread,
             m_sock,
             bLock ? "Lock" : "Unlock", cpcText );
#endif
...
ZSocketTask::ApplyState( zBOOL bLocateActionOnly )
{
   ...
   zstrcpy( szHoldRemoteWndTag, m_szRemoteWndTag );
   m_lSyncIdx = 0;
   m_szAppTag[ 0 ] = 0;
   m_szRemoteWndTag[ 0 ] = 0;
   m_szActTag[ 0 ] = 0;
...
// ::MessageBox( 0, "DKS", "ApplyState", MB_OK );
...
#if 0
   if ( SetCursorFirstEntityByString( vXRA, "Wnd",
                                      "Active", "Y", 0 ) == zCURSOR_SET )
   {
      GetStringFromAttribute( szRealRemoteWndTag, vXRA, "Wnd", "Tag" );
   }
   else
   {
      MessageBox( 0, "Error ... No Active Window specified",
                  "RemoteServer ApplyState", MB_OK );
      return;
   }
#endif
   ...
   // DisplayEntityInstance( vXRA, szEntityName );
      ...
      if ( m_lSyncIdx == 0 && zstrcmp( szEntityName, "Wnd" ) == 0 )
      {
         ...
         GetStringFromAttribute( m_szRemoteWndTag, vXRA,
                                 szEntityName, "Tag" );
         pXRA = m_pCacheWndList->Find( m_szRemoteWndTag, 0,
                                       "ZSocketTask::ApplyState" );
         if ( pXRA )
         {
      ...
      while ( lpViewAttrib )
      {
         ...
         {
            GetStringFromAttribute( szTag, vXRA, szEntityName, "Tag" );
         ...
         {
#ifdef DEBUG_ALL
            zsprintf( szMsg, "Entity: %s  Attribute: %s  updated",
                     szEntityName, lpViewAttrib->szName );
            TraceLineS( szMsg, "" );
#endif
            if ( zstrcmp( lpViewAttrib->szName, "SyncIdx" ) == 0 )
            {
               zstrcpy( szRealRemoteWndTag, m_szRemoteWndTag );
               GetIntegerFromAttribute( &m_lSyncIdx, vXRA,
                                        szEntityName, "SyncIdx" );
               ...
               switch ( m_chActType )
               {
                  ...
                  case 'O':
                  {
                     ...
                     if ( GetWindowAndOption( &pZSubtask, &pOpt,
                                              pXRA->m_pZSubtask->m_vDialog,
                                              szTag ) == 0 )
                     {
            ...
            {
               GetStringFromAttribute( m_szActTag, vXRA,
                                       szEntityName, "SelectedAction" );
            }
            else
            if ( zstrcmp( lpViewAttrib->szName, "SelectedIndex" ) == 0 )
            {
               ...
               if ( pXRA && pXRA->m_pZSubtask )
               {
                  ...
                  GetIntegerFromAttribute( &lSelectedIndex, vXRA,
                                           szEntityName, "SelectedIndex" );
                  ZSubtask *pZSubtask;
                  ZMapAct  *pzma;
                                         ...
                                         szTag ) == 0 )
                  {
                     ...
                     {
                        pzma->SetZCtrlState( zCONTROL_STATUS_SELECT_INDEX,
                                             -1 );
                        bResetSelected = TRUE;
                     }

                  // TraceLineI( "ZSocketTask::ApplyState SelectedIndex: ",
                  //             lSelectedIndex );
                  // TraceLineS( "                             for Ctrl: ",
                  //             *(pzma->m_pzsTag) );
            ...
            if ( pXRA && pXRA->m_pZSubtask &&
                 zstrcmp( szEntityName, "Wnd" ) == 0 &&
                 zstrcmp( lpViewAttrib->szName, "CtrlLastFocusTag" ) == 0 )
            {
               ZSubtask *pZSubtask;
               ZMapAct  *pzma;

               GetStringFromAttribute( szValue, vXRA, "Wnd",
                                       "CtrlLastFocusTag" );
               if ( szValue[ 0 ] &&
                    GetWindowAndCtrl( &pZSubtask, &pzma,
                                      pXRA->m_pZSubtask->m_vDialog,
                                      szValue ) == 0 )
               {
            ...
            {
               GetStringFromAttribute( szValue, vXRA, szEntityName,
                                       lpViewAttrib->szName );
               ...
               if ( zstrcmp( szEntityName, "Wnd" ) == 0 )
               {
                  SetRemoteWndAttribute( pXRA->m_pZSubtask->m_vDialog,
                                         lpViewAttrib->szName, szValue );
               }
               else
               {
                  SetRemoteCtrlAttribute( pXRA->m_pZSubtask->m_vDialog,
                                          szTag, szEntityName,
                                          lpViewAttrib->szName, szValue );
               }
            }
      ...
      // else
      // {
      //    sprintf( szMsg, "Entity: %s  Attribute: %s  NOT updated",
      //             szEntityName, lpViewAttrib->szName );
      //    TraceLineS( szMsg, "" );
      // }
      ...
      if ( nRC == zCURSOR_SET_RECURSIVECHILD )
      {
...
#else
         nRC = SetViewToSubobject( vXRA, szEntityName );
         lpViewEntity = (LPVIEWENTITY) MiGetViewEntityForView( vXRA,
                                                               szEntityName );
         ...
         zstrcpy( szEntityName, lpParent->szName );
#endif
      }
   }

   if ( szRealRemoteWndTag[ 0 ] )
      zstrcpy( m_szRemoteWndTag, szRealRemoteWndTag );
   else
   if ( bWnd == FALSE )
      zstrcpy( m_szRemoteWndTag, szHoldRemoteWndTag );

#ifdef DEBUG_ALL
   sprintf( szMsg,
            "ApplyState App: %s Wnd: %s Action: %s ActType: %c SyncIdx: %d",
...
//    MessageBox( 0, "Negative SyncIdx", "???", MB_OK );
   ...
   if ( pXRA )
   {
   // TraceLineS( "After ApplyState ... ",
   //             "DisplayObjectInstance m_vXRA_Delta" );
   ...
   {
      TraceLineS( "ApplyState unable to locate appropriate Delta: ",
                  m_szRemoteWndTag );
      ...
      if ( pXRA == 0 )
      {
         TraceLineS( "ApplyState remains unable to locate appropriate Delta: ",
                     m_szRemoteWndTag );
      }
...
// ZDxFrame    *pFrame = (ZDxFrame *) AfxGetMainWnd( );
   ...
   if ( GetViewByName( &vZDrSystem, "__ZDrSystem", 0, zLEVEL_SYSTEM ) > 0 )
   {
      ...
      SetAttributeFromInteger( vZDrSystem, "Task", "Handle",
                               pZSocketTask->m_pWinThread->m_nThreadID );
      DropView( vZDrSystem );
   }
   ...
   while ( lItemCnt > 0 )
   {
      ...
      {
         zsprintf( szMsg, "   [S%ld.T%ld.t%ld.s%ld] Dlg: %s  Wnd: %s  "
                          "vSubtask: %#010hx   Subtask: ",
                   g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZSocketTask,
                   AfxGetThread( ),
...
#ifdef DEBUG_ALL
   TraceLineI( "fnZDrConnectionThreadProc Initialization buffer size: ",
               lBytes );
#endif
   if ( lBytes == SOCKET_ERROR || lBytes == 0 ||
        (pch = zstrstr( szBuffer, "<SelectedAction>" )) == 0 ||
        zstrstr( pch, ">StartApplication<" ) == 0 )
   {
   ...
   {
      TraceLineS( "fnZDrConnectionThreadProc ",
                  "Initialization buffer overflow" );
   }
   ...
   {
      zsprintf( szMsg,
                "Received a connection ==> AppName '%s'  DialogName '%s'",
                szAppName + 2, szDlgName );
      TraceLineS( szMsg, "" );
   }

   RegisterZeidonAppForThread( &(pZSocketTask->m_vRegisteredSubtask),
                               "~~Zeidon System", 0, 0, 0 );
   bRegistered = TRUE;
   ...
   zstrcpy( pZSocketTask->m_szAppTag, szAppName + 2 );
   pZSocketTask->m_pzsAppName = new CString( szAppName + 2 );
   zstrcpy( pZSocketTask->m_pZTask->m_szInitialDlgName, szDlgName );

#ifdef DEBUG_ALL
   TraceLineS( "fnZDrConnectionThreadProc Calling init ", "Prev" );
#endif
   pZSubtask = pZSocketTask->m_pZTask->NewZWnd( 0, 0,
                                                pZSocketTask->m_vSubtask,
                                                0, FALSE, szDlgName,
                                                "", 0, pZSocketTask );
#ifdef DEBUG_ALL
   ...
   if ( pZSubtask == 0 )
   {
...
#ifdef DEBUG_ALL
      TraceLineS( "fnZDrConnectionThreadProc Calling init ",
                  "NULL Subtask" );
#endif
      SfActivateSysEmptyOI( &vXRA, "KZXRA",
                            pZSocketTask->m_vSubtask, zSINGLE );
      ...
      if ( szXRAFile[ 0 ] )
      {
         ...
         if ( nExtensionPos > -1 )
         {
            zstrcpy( szExtension, ((LPCSTR( strFileName )) + nExtensionPos ) );
            szXRAFile[ nExtensionPos ] = 0;
         }
         else
         {
            zstrcpy( szExtension, ".XML" );
         }
...
/////////////////////////////////////////////////////////////////////////////
   ...
   {
#ifdef DEBUG_ALL
      TraceLineX( "fnZDrConnectionThreadProc SendReceiveXML: ",
                  (zLONG) pZSubtask );
#endif
      ...
      // Protect this with a semaphore!!!
      pZSocketTask->LockMutex( pZSocketTask->m_pZTask->m_vSubtask,
                               TRUE, "SendReceive" );

      SetAttributeFromString( pZSocketTask->m_vXRA_State, "App",
                              "Tag", pZSocketTask->m_szAppTag );
      ...
      // DisplayObjectInstance( pZSocketTask->m_vXRA_State, 0, 0 );
         GetStringFromAttribute( szMsg, pZSocketTask->m_vXRA_State,
                                 "App", "SelectedAction" );
      }
      else
         TraceLineS( "Did NOT receive m_vXRA_State ", "..." );

      if ( pZSocketTask->m_vXRA_State == 0 ||
           zstrcmp( szMsg, "StopApplication" ) == 0 )
      {
         pZSocketTask->LockMutex( pZSocketTask->m_pZTask->m_vSubtask,
                                  FALSE, "SendReceive" );
         break;
   ...
   // pZSocketTask->m_pCacheWndList->ResetSubtasks( );
      ...
      pXRA = pZSocketTask->
                   m_pCacheWndList->Find( pZSocketTask->m_szRemoteWndTag, 0,
                                          "fnZDrConnectionThreadProc" );
      if ( pXRA == 0 )
      {
         TraceLineS( "RemoteServer error condition ... cannot find: ",
                     pZSocketTask->m_szRemoteWndTag );
         pZSocketTask->LockMutex( pZSocketTask->m_pZTask->m_vSubtask,
                                  FALSE, "SendReceive" );
         break;
      }
      ...
      {
         case 'C':
            zstrcpy( szMsg, "CtrlEvent" );
            break;

         case 'W':
            zstrcpy( szMsg, "WndEvent" );
            break;

         case 'O':
            zstrcpy( szMsg, "Option" );
            break;

         case 'H':
            zstrcpy( szMsg, "Hotkey" );
            break;

         default:
            zstrcpy( szMsg, "Unknown" );
            break;
      }
      ...
      if ( pZSocketTask->m_lSyncIdx > 0 )
      {
         ...
         if ( pXRA && pXRA->m_pZSubtask )
         {
            ...
            {
               SetEntityCursor( pXRA->m_pZSubtask->m_vDialog,
                                szlAct, szlSyncIdx,
                                zPOS_FIRST | zQUAL_INTEGER,
                                &(pZSocketTask->m_lSyncIdx), 0, 0, 0, 0, 0 );
               GetAddrForAttribute( &pch, pXRA->m_pZSubtask->m_vDialog,
                                    szlAct, szlTag );
            ...
            switch ( pZSocketTask->m_chActType )
            {
               ...
               default:
               {
                       ...
                       pXRA->m_pZSubtask->m_pZMIXCtrl )
                  {
                     ...
                     {
                        pzma = (ZMapAct *) pXRA->m_pZSubtask->m_pZMIXCtrl->
                                                         SeekNode( &csSeek );
                        if ( pzma )
      ...
      if ( pXRA )
      {
         zsprintf( szMsg, "pXRA: %ld  m_pZSubtask: %ld  found for %s",
      ...
      if ( pXRA &&
           (pXRA->m_pZSubtask == 0 ||
            ((ulSubtaskState =
               ZSubtask::GetSubtaskState( pXRA->m_pZSubtask )) &
                                           zSUBTASK_STATE_MARK_FOR_DELETE)) )
      {
         ...
         if ( CheckExistenceOfEntity( pXRA->m_vXRA_Delta,
                                      "Wnd" ) < zCURSOR_SET ||
              SetCursorFirstEntityByString( pXRA->m_vXRA_Delta, "Wnd", "Tag",
                                            pZSocketTask->m_szRemoteWndTag,
                                            0 ) < zCURSOR_SET )
         {
            CreateEntity( pXRA->m_vXRA_Delta, "Wnd", zPOS_AFTER );
            SetAttributeFromString( pXRA->m_vXRA_Delta, "Wnd",
                                    "Tag", pZSocketTask->m_szRemoteWndTag );
         }
...
// TraceLineI( "Done!!! Sending Termination String: ",
//             (zLONG) pZSocketTask->m_sock );
   ...
   while ( lItemCnt > 0 )
   {
      ...
      {
         zsprintf( szMsg,
                   "ZDRemote detected abnormal termination ... cleaning up: "
                      "[S%ld.T%ld.t%ld.s%ld] Dlg: %s  Wnd: %s  "
                      "vSubtask: %#010hx   Subtask: %ld  @",
                   g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZSocketTask,
                   AfxGetThread( ),
                   g_pSubtaskList[ lItemCnt ].lTID,
                   pZSocketTask->m_sock,
   ...
   if ( nRC >= 0 )
   {
      ...
      cr.sock = pZSocketTask->m_sock;
      cr.cMore = 0;
      fnSend( 0, "<zOI><SystemClose><Text>ApplicationTermination</Text></SystemClose></zOI>",
              73, 0, (zPVOID) &cr );
//    fnSend( 0, "X", 1, 0, (zPVOID) &cr );
   ...
   {
      CreateViewFromViewForTask( &vZDrSystem, vZDrSystem, m_vSubtask );
      if ( SetCursorFirstEntityByInteger( vZDrSystem, "Task", "Handle",
                                     pZSocketTask->m_pWinThread->m_nThreadID,
                                     0 ) == zCURSOR_SET )
      {
...
TrackConnectionThreadProc( zPVOID p )
{
   ...
   SysReadZeidonIni( -1, "[zWinSock]", "RemoteServerPort", szPortNbr );
   ...
   nRC = bind( listenSocket, (LPSOCKADDR) &saServer,
...
// UINT   dwThreadID;
   ...
   while ( TRUE )
   {
      ...
      CWinThread *pThread = AfxBeginThread( fnZDrConnectionThreadProc,
                                            (zPVOID) pZSocketTask,
              ...
              ZCommandLineInfo *pCmdInfo )
{
   ...
   m_nOS_Version = DetermineSystemVersion( 0, 0, 0 );
   m_pZDrApp = pDrApp;
   ...
   SfGetTaskInfo( m_szInitialDlgName, zTASK_DIALOG,
                  SfGetCurrentTask( vSubtask ) );
...
#define IDM_GETACTIONTRACE 204
   ...
   if ( (char) m_nTraceAction == '0' || (char) m_nTraceAction == 'N' ||
        (char) m_nTraceAction == 'n' )
   {
      m_nTraceAction = 0;
   }
   else
   if ( (char) m_nTraceAction == '4' || (char) m_nTraceAction == 'Y' ||
        (char) m_nTraceAction == 'y' )
   {
      m_nTraceAction = 4;
   }
   else
   ...
   SysReadZeidonIni( -1, "[Workstation]", "HoverDelay", szHoverDelay );
   ...
   ZSubtask *pZSubtask = NewZWnd( 0, 0, vSubtask, 0,
                                  FALSE, m_szInitialDlgName, "" );
...
ZTask::~ZTask( )
{
...
#ifdef zREMOTE_SERVER
   TraceLineI( "End Track Connection thread: ",
               (zLONG) m_pThread->m_nThreadID );
   ...
   if ( m_pZDrApp &&
        GetViewByName( &vZDrSystem, "__ZDrSystem",
                       m_pZDrApp->m_vSubtask, zLEVEL_SYSTEM ) > 0 )
   {
      CreateViewFromViewForTask( &vZDrSystem, vZDrSystem,
                                 m_pZDrApp->m_vSubtask );
      if ( SetCursorFirstEntityByInteger( vZDrSystem, "Task", "Handle",
                                          (zLONG) this, 0 ) == zCURSOR_SET )
      {
         DeleteEntity( vZDrSystem, "Task", zREPOS_FIRST );
      }
   ...
   if ( m_pZDrApp )
   {
      if ( g_MemHead )
      {
         TraceLineX( "ZTask::dtor: Memory leak detected: ",
                     (zLONG) g_MemHead );
      }
...
ZTask::InitTask( zVIEW vSubtask )
{
   ...
   if ( m_vPE == 0 )
   {
      ...
      if ( GetViewByName( &vTaskLPLR, "TaskLPLR", vSubtask, zLEVEL_TASK ) > 0 )
      {
         ...
         zstrcat( szFileName, szlZEIDON_XPE );
         if ( SfActivateSysOI_FromFile( &m_vPE, szlKZPEXVOO,
                                        vSubtask, szFileName,
      ...
      if ( bLPLR_XPE )
      {
         ...
         SfLockView( m_vPE );  // we lock this view because it is
      ...
      else
      {
         ...
         TraceLineS( "(drvr) Cannot load Presentation Environment from file - ",
                     szlZEIDON_XPE );
         MessageSend( vSubtask, 0, szlDrvr,
                      "Cannot load Presentation Environment",
                      zMSGQ_MODAL_ERROR, FALSE );
   ...
   if ( m_vHK == 0 )
   {
      ...
      // Activate using SfActivateSysOI_FromFile so that the kzhkeyxo.xod
      // does not have to be in the application LPLR.
      GetApplDirectoryFromView( szFileSpec, vSubtask,
                                zAPPL_DIR_OBJECT, sizeof( szFileSpec ) );
      zstrcat( szFileSpec, szlKZHKEYAA_POR );
      if ( SfActivateSysOI_FromFile( &m_vHK, szlKZHKEYXO,
                                     vSubtask, szFileSpec,
                                                           ...
                                                           zNOI_OKAY ) < 0 )
      {
         ...
         ulHotkeyLth = -1;
         SetAttributeFromBlob( m_vHK, szlApp, szlHotkey,
                               &ulHotkeyLth, ulGlobalHotkeyLth );
      }
...
// SysMutexLock( vSubtask, "ZTask", 0, zMUTEX_SAMETASK );
   ...
   if ( pZSubtaskReplace )
      ulSubtaskStateHold =
               ZSubtask::SetSubtaskState( pZSubtaskReplace, 0,
                                          zSUBTASK_STATE_PROVISION_REPLACE );
        ...
        zstrcmp( cpcWndTag, *(pZSubtaskReplace->m_pzsWndTag) ) == 0 )
   {
      ...
      pZSubtaskReplace = 0;
      ZSubtask::SetSubtaskState( pZSubtask,
                                 ~zSUBTASK_STATE_IN_CONSTRUCTION,
                                 zSUBTASK_STATE_IN_CONSTRUCTION |
                                   zSUBTASK_STATE_REPLACED_BY_SELF );
   }
   else
#endif
   {
           ...
           (pZParent->m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) )
      {
         ...
         if ( pZSubtask )
         {
            pZParent = pZSubtask;
            if ( pZOwner &&
                 (pZOwner->m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) )
            {
      ...
      // First create the subtask object which will localize all the
      // information garnered from the Zeidon Dialog object (.XWD).
#ifdef zREMOTE_SERVER
      if ( pZSocketTask == 0 )
         pZSocketTask = pZParent->m_pZSocketTask;

   // TraceLineS( "ZTask::NewZWnd: ", "new ZSubtask" );
      pZSubtask = new ZSubtask( this, pZParent, pZOwner, vSubtask,
                                vNewSubtask, bModal,
                                cpcDlgTag, cpcWndTag,
                                pZSocketTask );
   ...
   // TraceLineS( "ZTask::NewZWnd: ", "ZSubtask created" );
      ...
      zstrcpy( pZSocketTask->m_szRemoteWndTag,
               *(pZSubtask->m_pzsRemoteWndTag) );
      ZXRA_Item *pXRA =
        pZSocketTask->m_pCacheWndList->Find( *(pZSubtask->m_pzsRemoteWndTag),
                                             0, "ZTask::NewZWnd" );
      ...
      if ( pXRA == 0 )
      {
         ...
         if ( CreateRemoteWindowSubtask( &(pZSocketTask->m_vXRA),
                                         vSocketSubtask,
                                         *(pZSubtask->m_pzsDlgTag),
                                         *(pZSubtask->m_pzsRemoteWndTag) ) )
         {
            MessageSend( pZSocketTask->m_vSubtask, 0,
                         "Zeidon Application",
         ...
         else
         {
            if ( (m_nTraceAction & 0x71) == 0x71 )
            {
               ...
               cs += "__SocketSubtask";
               SetNameForView( pZSocketTask->m_vXRA, cs,
                               pZSocketTask->m_vSubtask, zLEVEL_TASK );
            }

            SetAttributeFromString( pZSocketTask->m_vXRA, "App",
                                    "Tag", pZSocketTask->m_szAppTag );

            pXRA = pZSocketTask->
                  m_pCacheWndList->Add( *(pZSubtask->m_pzsRemoteWndTag),
                                        pZSubtask,
                                        pZSocketTask->m_vXRA );
      ...
      else
      {
         ...
         ActivateOI_FromOI_ForTask( &(pXRA->m_vXRA), pXRA->m_vXRA_Def,
                                    0, zSINGLE );
         if ( (m_nTraceAction & 0x71) == 0x71 )
         {
            CString cs = *(pZSubtask->m_pzsDlgTag);
      ...
      // zCHAR szMsg[ 256 ];
      //
      // zsprintf( szMsg, "pXRA: %ld  m_pZSubtask: %ld  set for %s",
      //           pXRA, pXRA->m_pZSubtask, pZSocketTask->m_szRemoteWndTag );
      // TraceLineS( szMsg, "" );
      }
   // else
   ...
   if ( pZSubtaskReplace )
   {
      ZSubtask::SetSubtaskState( pZSubtaskReplace, 0xFFFFFFFF,
                                 ulSubtaskStateHold );
   }
   ...
   if ( ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE )
   {
      TraceLine( "ZTask::NewZWnd unable to create Subtask: %s.%s",
...
// }
   ...
   if ( pZSubtaskReplace )
   {
      ...
      ZSubtask::SetSubtaskState( pZSubtaskReplace, 0,
                                 zSUBTASK_STATE_REPLACED |
                                   zSUBTASK_STATE_DELETE );
   }
...
#endif
         {
            // If this was the main window and there are other windows left,
            // we must find another main window to take this one's place.
            ...
            while ( lItemCnt )
            {
               if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
                    (g_pSubtaskList[ lItemCnt ].ulSubtaskState &
                                       zSUBTASK_STATE_MARK_FOR_DELETE) == 0 )
               {
                  if ( pZSubtask->m_lTID == g_pSubtaskList[ lItemCnt ].lTID &&
                       pZSubtask == g_pSubtaskList[ lItemCnt ].pZSubtask )
                  {
                     MessageSend( pZSubtask->m_vDialog, 0,
                                  "ZTask::NewZWnd",
                                  "Unexpected subtask pointer",
                                  zMSGQ_MODAL_ERROR, FALSE );
                  }
                  else
                  if ( pZSubtask->m_lTID == g_pSubtaskList[ lItemCnt ].lTID &&
                       g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZParent == 0 )
                  {
#ifdef zREMOTE_SERVER
                     pZSocketTask->m_pZSubtaskTop =
                                   g_pSubtaskList[ lItemCnt ].pZSubtask;
#else
                     m_pZSubtaskTop = g_pSubtaskList[ lItemCnt ].pZSubtask;
                     m_pZDrApp->m_pMainWnd = m_pZSubtaskTop->m_pZFWnd;
                     ::SetWindowLong( m_pZDrApp->m_hWndCommOE, GWL_USERDATA,
                                      (zLONG) m_pZDrApp->m_pMainWnd->m_hWnd );
#endif
                     break;
                  }
               }
...
// k /= k;
   TraceLine( "ZTask::DeleteZWnd Task: 0x%08x  IdNbr: %d  Subtask: 0x%08x %s",
              this, pZSubtask->m_ulSubtaskIdNbr, pZSubtask,
   ...
   // Run through the list of Child windows to find any that have
   // this window as its parent ... destroy them if they exist.
   zULONG ulSubtaskState;
   zLONG  lPos = 1;
   zLONG  lItemCnt = (zLONG) g_pSubtaskList[ 0 ].pZSubtask;
   while ( lPos <= lItemCnt )
   ...
   if ( lItemCnt == 0 ||
        (lItemCnt == 1 && pZSubtask->m_pZParent &&
         ((ulSubtaskState = ZSubtask::GetSubtaskState( pZSubtask->m_pZParent )) &
                                                zSUBTASK_STATE_EXTERNAL_APP)) )
   {
      if ( m_pzsHelpFile )
      {
         ::WinHelp( pZSubtask->m_pZFWnd->m_hWnd,
                    *m_pzsHelpFile, HELP_QUIT, 0 );
         mDeleteInit( m_pzsHelpFile );
      }
...
ZTask::DeleteSubtask( ZSubtask *pZSubtask )
{
   ...
   if ( ((ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
         lTID == pZSubtask->m_pZSubtaskGlobal->m_lTID) ||
        ((ulSubtaskState1 & zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
         lTID == pZSubtask->m_pZSubtaskProxy->m_lTID) )
   {
...
// TraceLineI( "ZTask::DeleteSubtask Item Count: ", lItemCnt );
   ...
   while ( lItemCnt > 0 )
   {
      if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
           (g_pSubtaskList[ lItemCnt ].ulSubtaskState &
                       zSUBTASK_STATE_DELETED) != zSUBTASK_STATE_DELETED &&
...
// TraceLineS( "ZSubtask dtor Unlock Mutex ZDr", "" );
   ...
   if ( (m_nTraceAction & 0x71) == 0x71 )
   {
      ...
      zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] ZTask::DeleteSubtask: ",
...
#endif
            {
               if ( (pZSubtask->m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) )
               {
                  ...
                  ZSplitterWnd *pSplitterWnd =
                                   pZSubtask->m_pZView->m_pParentSplitterWnd;
                  if ( pZSubtaskMain == pZSubtask &&
                       pSplitterWnd->m_pZSubtask2 &&
                       (ZSubtask::GetSubtaskState( pSplitterWnd->m_pZSubtask2 ) &
                        zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
                       pSplitterWnd->m_pZSubtask2->m_pZFWnd )
                  {
                     ...
                     ZDrFrame *pZFWnd1 =
                        (ZDrFrame *) pSplitterWnd->m_pZSubtask1->m_pZFWnd;
                     ZDrFrame *pZFWnd2 =
                        (ZDrFrame *) pSplitterWnd->m_pZSubtask2->m_pZFWnd;
                     ...
                     if ( (m_nTraceAction & 0x71) == 0x71 )
                     {
                        TraceLine( "ZTask::DeleteTopSubtask IdNbr: %d SWAP Frame1: "
                                      "%d Subtask: %d SplitWnd: %s <=> "
                                      "Frame2: %d Subtask: %d SplitWnd: %s",
                                   pZSubtask->m_ulSubtaskIdNbr, pZFWnd1->m_ulFrameIdNbr,
                                   pZFWnd1->m_pZSubtask->m_ulSubtaskIdNbr,
                                   pZFWnd1->m_pSplitterWnd ?
                                     *(pZFWnd1->m_pSplitterWnd->m_pcsSplitTag) :
                                     "null",
                                   pZFWnd2->m_ulFrameIdNbr,
                                   pZFWnd2->m_pZSubtask->m_ulSubtaskIdNbr,
                                   pZFWnd2->m_pSplitterWnd ?
                                     *(pZFWnd2->m_pSplitterWnd->m_pcsSplitTag) :
                                     "null" );
                     }
                  }
               }
               ...
               while ( lItemCnt )
               {
                  if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
                       (g_pSubtaskList[ lItemCnt ].ulSubtaskState &
                                    zSUBTASK_STATE_MARK_FOR_DELETE) == 0 )
                  {
                     if ( lTID == g_pSubtaskList[ lItemCnt ].lTID &&
                          pZSubtask == g_pSubtaskList[ lItemCnt ].pZSubtask )
                     {
                        MessageSend( pZSubtask->m_vDialog, 0,
                                     "ZTask::DeleteSubtask",
   ...
   // TraceLineI( "ZTask::DeleteSubtask Item Count: ", lItemCnt );
      ...
      while ( lItemCnt > 0 )
      {
         if ( g_pSubtaskList[ lItemCnt ].pZSubtask == pZSubtaskOwner &&
              ...
              g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZTask == this )
         {
            if ( pZSubtaskOwner->m_pZFWnd &&
                 mIs_hWnd( pZSubtaskOwner->m_pZFWnd->m_hWnd ) )
            {
   ...
   if ( (m_nTraceAction & 0x71) == 0x71 )
   {
      ...
      while ( lItemCnt > 0 )
      {
              ...
              g_pSubtaskList[ lItemCnt ].lTID == lTID )
         {
            ...
            {
               TraceLine( "ZTask::DeleteSubtask POST: Subtask: [0x%08x]  IdNbr: %d   FWnd %d  View: %d  "
                             "Quad: %s  ParentSplitterView (0x%08x): %d",
                          pZSubtask, pZSubtask->m_ulSubtaskIdNbr,
                          ...
                          pZSubtask->m_pZView->m_pParentSplitterView ?
                            pZSubtask->m_pZView->m_pParentSplitterView : 0,
                          pZSubtask->m_pZView->m_pParentSplitterView ?
                            pZSubtask->m_pZView->m_pParentSplitterView->m_ulIdNbr : 0 );
            }
            else
            {
                                         ...
                                         zCPCHAR cpcWndTag )
{
   ...
   while ( pNext )
   {
      ...
      if ( zstrcmp( pZSubtask->m_csSplitTag, cpcSplitTag ) == 0 &&
           zstrcmp( *pZSubtask->m_pzsDlgTag, cpcDlgTag ) == 0 &&
           zstrcmp( *pZSubtask->m_pzsWndTag, cpcWndTag ) == 0 )
      {
...
// }
   ...
   if ( m_pZFWnd && ((ZDrFrame *) m_pZFWnd)->m_pZSubtask &&
        (((ZDrFrame *) m_pZFWnd)->m_pZSubtask->m_ulWndOper &
                           zWND_EVENT_ON_ACQUIRE) == zWND_EVENT_ON_ACQUIRE &&
        SetCursorFirstEntityByInteger( ((ZDrFrame *) m_pZFWnd)->
                                                      m_pZSubtask->m_vDialog,
                                       szlWndEvent, szlType,
                                       zWND_EVENT_ON_ACQUIRE,
                                       0 ) > zCURSOR_UNCHANGED )
   {
      if ( ProcessActionLoop( ((ZDrFrame *) m_pZFWnd)->m_pZSubtask,
                              szlWndAct, 0, 0,
                              zACTION_DIALOG_CODE_ONLY,
                              TRUE, "OnAcquire" ) > 0 )
      {
         return;
      }
...
ZTask::AddApplicationDLL( ZSubtask *pZSubtask, zCPCHAR pchDLL_Name )
{
   ...
   if ( m_DLL_InitList.AddTail( zsDLL ) )
   {
      ...
      zFARPROC_INIT lpfnInitProc = (zFARPROC_INIT)
        GetOperationDynamicCallAddress( pZSubtask->m_vDialog,
                                        (LPLPLIBRARY) &hCCtlLibrary,
                                        zsDLL, zs, "(drvr) Init" );
      if ( lpfnInitProc )
      {
         (*lpfnInitProc)( pZSubtask->m_vDialog,
                          g_lDlgUnitCharX, g_lDlgUnitCharY,
                          g_lMapDlgCharX, g_lMapDlgCharY );
         if ( m_nTraceAction > 2 )
...
fnShowError( zPCHAR pszFuncName )
{
   ...
   {
      zsprintf( szMsg, "(zwinsock) WinSock error Func = %s   %s %d",
                pszFuncName, g_ErrMsgs[ k ].pszErrorMessage, iErr );
   }
   else
   {
      zsprintf( szMsg, "(zwinsock) Unknown WinSock error Func = %s   %d",
                pszFuncName, iErr );
   }
...
fnTransferMessages( zVIEW vXRA, zVIEW vSubtask )
{
   ...
   if ( (nRC = SetCursorFirstEntity( vMsg, "QMsg", 0 )) >= zCURSOR_SET )
   {
      ...
      if ( nMsgCnt > 1 )  // || bAbort
      {
         ...
         OperatorPrompt( vSubtask, "Application Unavailable",
                         "Please contact support", FALSE,
                         zBUTTONS_OK, zRESPONSE_OK, zICON_EXCLAMATION );
      ...
      else
      {
         ...
         csTag += ':';
         zCHAR szTemp[ 16 ];
         zltoa( lMsgTag, szTemp );
         csTag += szTemp;
         SetAttributeFromString( vXRA, "MsgBox", "Tag", csTag );

         SetAttributeFromAttribute( vXRA, "MsgBox", "Text",
                                    vMsg, "QMsg", "Text" );
        ...
        zPVOID lpData )
{
   ...
   // Check to see if we need to flush the current buffer.  If pchText is 0
   // then user wants a flush.
   if ( pchText == 0 || lpConn->ulBufferLth + uLth > zWEB_COMMUNICATION_BUFFER_SIZE )
   {
      ...
      do
      {
         ...
         // TraceLineS( "Perfoming send()", "" );
            TraceBlob( "fnSend Sending buffer lth: ",
                       lpConn->pszBuffer, lpConn->ulBufferLth );
         }

         zsprintf( szMsg, "%05d/%c", (int) lpConn->ulBufferLth,
                   pchText ? 'C' : '0' );
      ...
      // TraceLineS( "==> ", szMsg );
         ...
         do
         {
            if ( lpConn->lFileHandle != -1 )
               SysWriteFile( lpConn->lFileHandle, szMsg + lBytesSent,
                             lBytesToSend - lBytesSent );

            nRC = send( lpConn->sock, szMsg + lBytesSent,
                        lBytesToSend - lBytesSent, 0 );
         ...
         {
            if ( lpConn->lFileHandle != -1 )
               SysWriteFile( lpConn->lFileHandle,
                             lpConn->pszBuffer + lBytesSent,
                             lBytesToSend - lBytesSent );

            lBytesSent += send( lpConn->sock, lpConn->pszBuffer + lBytesSent,
                                lBytesToSend - lBytesSent, 0 );
            if ( lBytesSent == SOCKET_ERROR )
            {
               TraceLineS( "Sockets Error: ", "Error in send()" );
        ...
        zPVOID  lpData )
{
   ...
   *ppchReturnBuffer = lpConn->pszBuffer;
   ...
   if ( lpConn->ulBlockLth == 0 && lpConn->cMore != '0' )
   {
      ...
      {
         zCHAR szMsg[ 256 ];
         zsprintf( szMsg, "fnRead Received block lth [%s] = %ld  "
                          "More: '%c' BytesRead: ",
   ...
   // If the block lth is *still* 0 then there's no more data for the
   // XML object.  Set appropriate flags and get out.
   ...
   {
      // The block size is bigger than the buffer so fill up the buffer and
      // set the bDone flag off because there is more data in the block.
      lBytesToRead = zWEB_COMMUNICATION_BUFFER_SIZE;
      bDone = FALSE;
   }
   else
   {
      // The block is smaller than the buffer.  Read the rest of the block
      // and then set bDone according to the value of cMore.
...
// TraceLineI( "fnRead BytesRead: ", lBytesRead );
   ...
   // If we didn't read all the bytes we thought we would read then we aren't
   // done so set flag.
                  ...
                  zPCHAR pchBuffer )
{
   ...
   if ( (m_nTraceAction & 0x71) == 0x71 )
   {
      csText.Format( TEXT( "[S%ld.T%ld.t%ld.s%ld]: " ),
                     (zLONG) pZSocketTask,
                     AfxGetThread( ),
                     (zLONG) pZSocketTask->m_pWinThread,
                     (zLONG) pZSocketTask->m_sock );
   }
...
// ZDxFrame *pFrame = (ZDxFrame *) AfxGetMainWnd( );
   ...
   if ( szXRAFile[ 0 ] )
   {
      ...
      if ( ::GetEnvironmentVariable( "TEMP", szTempDir, MAX_PATH ) == 0 )
      {
         strFileName.Format( "%s%s%d%s","c:\\", szXRAFile,
                             lXMLTraceCount, szExtension );
      }
      else
      {
         strFileName.Format( "%s%s%s%d%s",szTempDir, "\\",
                             szXRAFile, lXMLTraceCount, szExtension );
      }
...
// pFrame->SendMessage( zZEIDON_REMOTE_SOCKET, 2, (LPARAM) pZSocketTask );
   ...
   ZXRA_Item *pXRA = pZSocketTask->
                     m_pCacheWndList->Find( pZSocketTask->m_szRemoteWndTag,
                                            0, "fnSendReceiveXML" );
   ...
   if ( nViewCnt )
   {
      ...
      nRC = WriteMultipleOIs_ToXML( pvOI, nViewCnt,
                                    zXML_ZEIDONINFO, // | zXML_INCREFLAGS,
                                    fnSend, (zPVOID) &cr );
      delete [] pvOI;
   }
...
// pFrame->SendMessage( zZEIDON_REMOTE_SOCKET, 4, (LPARAM) pZSocketTask );
   ...
   cr.cMore = 0;
   pchBuffer[ 0 ] = 0;
   nRC = ActivateOI_FromXML( &(pZSocketTask->m_vXRA_State), "KZXRA",
                             pZSocketTask->m_vRegisteredSubtask, 0,
                             fnRead, &cr );
   if ( (m_nTraceAction & 0x71) == 0x71 &&
        nRC != zCALL_ERROR )
   {
      ...
      cs += "__XRA_State";
      SetNameForView( pZSocketTask->m_vXRA_State, cs,
                      pZSocketTask->m_vSubtask, zLEVEL_TASK );
   }
