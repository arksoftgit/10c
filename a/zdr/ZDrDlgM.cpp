   ...
   *pchReturnPopupTag = 0;
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      if ( pZSubtask->m_pzsPopupTag )
         zstrcpy( pchReturnPopupTag, *(pZSubtask->m_pzsPopupTag) );
   ...
   *pchReturnTag = 0;
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      if ( pZSubtask->m_pzsLastTag )
         zstrcpy( pchReturnTag, *(pZSubtask->m_pzsLastTag) );
...
#ifdef DEBUG_ALL
      zCHAR  szMsg[ 256 ];
      zsprintf( szMsg, "SetRemoteZOptAttribute Tag: %s  Entity: %s  "
                       "Attribute: %s  Value: %s",
