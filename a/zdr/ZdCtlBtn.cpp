   ...
   if ( m_pzsVName && m_pzsEName && m_pzsAName )
   {
                          ...
                          m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
      {
         if ( CheckExistenceOfEntity( vApp, *m_pzsEName ) == zCURSOR_SET )
         {
            ...
            Invalidate( );
            if ( m_pzsText == 0 || zstrcmp( szText, *m_pzsText ) != 0 )
            {
                                  ...
                                  CRect&        rectClient )
{
   ...
   zPCHAR pchText = new char[ (zstrlen( cpcText ) + 1) * 2 ];
   zstrcpy( pchText, cpcText );
   pch = pchText;
   while ( pch && *pch )
   {
   ...
   //    pDC->SetTextAlign( uAlign | TA_UPDATECP );
      ...
      for ( zLONG k = 0; k < nSize; k++ )
      {
         ...
         if ( (GetStyle( ) & WS_DISABLED) == WS_DISABLED )
         {
            ...
            rectTemp.right += 1;
            rectTemp.bottom += 1;
            pDC->DrawText( arrayText.GetAt( k ), -1, &rectTemp,
      ...
      pDC->DrawText( cpcBuffer, zstrlen( cpcBuffer ),
                     rectTemp, DT_CENTER | DT_VCENTER );
   // if ( (uAlign & TA_UPDATECP) == 0 )
...
ZPushButton::DrawFrame( CDC *pDC, CRect rect, UINT Inset )
{
   ...
   for ( k = 0; k < width; k += 1 )
   {
      ...
      DrawLine( pDC, rect.left, rect.top,
                rect.right, rect.top, clrTopLeft );      // Across top
      DrawLine( pDC, rect.left, rect.top,
                rect.left, rect.bottom, clrTopLeft ); // Down left

      if ( (Inset < 0) && (k == width - 1) && (width > 1) )
      {
         DrawLine( pDC, rect.left + 1, rect.bottom - 1,
                   rect.right, rect.bottom - 1, RGB( 1, 1, 1 ) ); // Across bottom
         DrawLine( pDC, rect.right - 1, rect.top + 1,
                   rect.right - 1, rect.bottom, RGB( 1, 1, 1 ) ); // Down right
      }
      else
      {
         DrawLine( pDC, rect.left + 1, rect.bottom - 1,
                   rect.right, rect.bottom - 1, clrBottomRight );     // Across bottom
         DrawLine( pDC, rect.right - 1, rect.top + 1,
                   rect.right - 1, rect.bottom, clrBottomRight );     // Down right
      }
              ...
              zLONG   lClrBackDark )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for BTN_SetColor ",
                  cpcCtrlTag );
   }
                      ...
                      "Font" )
{
   ...
   Attr.Style = CBS_DROPDOWN | /* CBS_OWNERDRAWFIXED | */ CBS_HASSTRINGS |
   ...
   if ( m_pzsVName && m_pzsEName && m_pzsAName )
   {
                          ...
                          m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
      {
         if ( CheckExistenceOfEntity( vApp, *m_pzsEName ) == zCURSOR_SET )
         {
            zCHAR  szFont[ 256 ];
            zSHORT nRC =
               GetVariableFromAttribute( szFont, 0, zTYPE_STRING,
                                         sizeof( szFont ),
                                         vApp, *m_pzsEName,
                                         *m_pzsAName, *m_pzsContext,
                                         m_pzsContext->IsEmpty( ) ?
...
ZFontPicker::DrawItem( LPDRAWITEMSTRUCT lpDIS )
{
   ...
   {
      m_ImageList.Draw( pDC, 1, CPoint( rect.left, rect.top ),
                        ILD_TRANSPARENT );
   }
...
// CString csDefault = "";   // set default font name
   ...
   // We set the timer because it's the only way we know when a selection
...
ZToolTip::ShowTips( CPoint pt, const CString& cs )
{
   ...
   if ( m_csFont != cs )
   {
      ...
      if ( m_bResetFont )
      {
         ...
         zstrcpy( lf.lfFaceName, m_csFont );
