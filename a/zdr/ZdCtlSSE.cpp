   ...
   // CtrlMapDef.CheckByte = 0xFE;
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         GetStructFromEntityAttrs( (zPCHAR) CtrlMapDef.Tag,
                                   m_pZSubtask->m_vDialog, szlCtrlMap );
#ifdef DEBUG_LODSTRUCT
...
//                                                0 : zUSE_DEFAULT_CONTEXT );
                  ...
                  if ( (ulSubtype & zCONTROL_DISABLED) == 0 )
                  {
                     ...
                     ulLth = 0;
                     nRC = GetFirstTableEntryForAttribute( pchBuffer,
                                                           m_vAppList,
                                                           cpcEntityName,
                                                           pchAttributeName,
                                                           cpcContextName,
                     ...
                     while ( nRC == 0 )
                     {
                        ...
                        *(pchBuffer + ulLth++) = '\t';
                        nRC = GetNextTableEntryForAttribute( pchBuffer + ulLth,
                                                             m_vAppList,
                                                             cpcEntityName,
                                                             pchAttributeName,
                                                             cpcContextName,
...
//       ShowActiveCell( SS_SHOW_CENTERLEFT );
      ...
      else
      {
         ...
         if ( m_pZSubtask->m_pZTask->m_nTraceAction > 2 )
         {
            ...
            zsprintf( szMsg, "Grid: %s - no %s entities",
                      *m_pzsTag, *m_pzsEName );
            TraceLineS( szMsg, "" );
         }
      }
...
//          TraceLineI( "ZSS::MapToOI col ", k );
            ...
            switch ( ulStyle & 0x0FFF0000L )
            {
               ...
               // zCHAR szMsg[ sizeof( szMap ) + 100 ];
               //
               // zsprintf( szMsg, "GetValue for Row: %d   Col: %d   %s",
                  ...
                  zPLONG   plActiveCol )
{
   ...
   *plActiveRow = *plActiveCol = 0;
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for SS_GetActiveCell ",
                  cpcCtrlTag );
   }
                ...
                zLONG    lColNbr )
{
   ...
   *pchReturnData = 0;
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for SS_GetCellData ",
                  cpcCtrlTag );
   }
   ...
   //                  if ( lpCell == 0 || lpCell->m_Data.bDataType == 0 )
         ...
         case SS_TYPE_COMBOBOX:
            if ( !EscapeInProgress )
            {
               ...
               if ( DrAllocTaskMemory( (zCOREMEM) &hText,
                                       nLth + 1, 1020 ) == 0 )
               {
...
//                        DrLockTaskMemory( CellType->Spec.ComboBox.hItems );
                  ...
                  if ( fChangeMade )
                  {
                                             ...
                                             1022 ) == 0 )
                     {
                        ...
                        zstrcpy( Data, lpText );
                        zstrcpy( &Data[ zstrlen( Data ) + 1 ], Buffer );

                        SSSetCellData( hWnd, lpSS->Col.ssCurrAt, lpSS->Row.ssCurrAt,
                                        Data, zstrlen( lpText ) + 1 +
                                          zstrlen( Buffer ) );
...
//                DrUnlockTaskMemory( lpItems );
      ...
      if ( !EscapeInProgress && lpSS->CalcAuto && lpSS->fFormulaMode == 0 )
         SS_CalcDependencies( hWnd, lpSS, lpSS->Col.ssCurrAt,
                             lpSS->Row.ssCurrAt );
...
#else
   {
      ...
      SS_SendMsg( hWnd, SSM_EDITMODEOFF, GetDlgCtrlID( hWnd ),
                  (zLONG)(zPCHAR) &EditModeOff );
   }
#endif
      ...
      if ( lpSS->fProcessingKillFocus &&
           lpSS->fSetFocusWhileMsgBeingSent && hWnd == GetFocus( ) )
      {
...
//    lpText = (zPCHAR) DrLockTaskMemory( *lphItemSrc );
      ...
      if ( (DrAllocTaskMemory( (zCOREMEM) lphItemDest,
                               zstrlen( lpText ) + 1, 1027 )) == 0 )
      {
         lpTextDest = (zPCHAR) DrLockTaskMemory( *lphItemDest );
         zstrcpy( lpTextDest, lpText );
...
//    lpText = (zPCHAR) DrLockTaskMemory( *pchItemSrc );
      ...
      if ( DrAllocTaskMemory( pchItemDest,
                              zstrlen( lpText ) + 1, 1028 ) == 0 )
      {
...
//    TraceLineS( "SS_EditPasteProc Msg: ", msgMap.GetMsgName( uMsg ) );
   ...
   if ( lpSS->fAutoClipboard )
   {
      ...
      if ( (hTextOld = SS_GetTextFromClipboard( hWnd )) != 0 )
      {
         ...
         if ( !SS_ValidatePasteData( GetParent( hWnd ), lpSS, lpszText,
                                     &CellUL, &CellLR, TRUE ) )
         {
         ...
         if ( (hText = GlobalAlloc( GHND, nLth + 1 )) != 0 )
         {
            ...
            for ( k = 0, nDest = 0; k < nLth; k++ )
            {
               ...
               if ( SS_EditIsValidKey( &CellType, chChar ) )
               {
                  if ( CellType.Spec.Edit.uCase == SS_CASE_UCASE )
                     chChar = (char)(zLONG)
                      zstrupr( (zPCHAR) MAKELONG( (WORD) chChar, 0 ) );
                  else
                  if ( CellType.Spec.Edit.uCase == SS_CASE_LCASE )
                     chChar = (char)(zLONG)
                      zstrupr( (zPCHAR) MAKELONG( (WORD) chChar, 0 ) );
            ...
            if ( OpenClipboard( hWnd ) )
            {
               ...
               lRet = SS_DefProc( lpfnProc, GetParent( hWnd ),
...
SS_GetTextFromClipboard( HWND   hWnd )
{
   ...
   if ( (hMem = GetClipboardData( CF_TEXT )) != 0 )
   {
      ...
      if ( zstrlen( lpszMem ) )
      {
         if ( (hText = GlobalAlloc( GHND, (DWORD) (zstrlen( lpszMem ) + 1) )) != 0 )
         {
            lpszText = (zPCHAR) GlobalLock( hText );
