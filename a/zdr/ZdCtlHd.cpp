...
ZHD_Diagram::DestroyWindow( )
{
   ...
   zPCHAR pch = new char[ sizeof( "-2147483647," ) * 7 ];
   zSHORT nLth = 0;

   zltoa( m_bOrientation, pch + nLth );
   nLth = zstrlen( pch );
   pch[ nLth ] = ',';
   nLth++;
   zltoa( m_bScrolledWindow, pch + nLth );
   nLth = zstrlen( pch );
   pch[ nLth ] = ',';
   nLth++;
   zltoa( m_lDisplayPos, pch + nLth );
   nLth = zstrlen( pch );
   pch[ nLth ] = ',';
   nLth++;
   zltoa( m_lDisplayPosMax, pch + nLth );
   nLth = zstrlen( pch );
   pch[ nLth ] = ',';
   nLth++;
   zltoa( m_lCellSize, pch + nLth );
   nLth = zstrlen( pch );
   pch[ nLth ] = ',';
   nLth++;
   zltoa( m_lScale, pch + nLth );
      ...
      if ( pPrintDC )
      {
         ...
         if ( nRC == zRESPONSE_YES && pPrintDC->StartDoc( &DocInfo ) > 0 )
         {
            ...
            for ( ;; )
            {
               ...
               TraceLineI( "                 Nodes printed = ",
                           lNodesPrinted );
               TraceLineI( "            Nodes not printed = ",
                           lNodesNotPrinted );
               ...
               if ( lPrinted )
               {
                  ...
                  if ( pchPrintBox && pchPrintBox[ 0 ] )
                  {
                     ...
                     CBrush *pBrushOld = (CBrush *)
                                    pPrintDC->SelectStockObject( NULL_BRUSH );
...
ZHD_Diagram::RecursDrawNodes( zVIEW vDiagram, ZHD_Node *pParentNode )
{
   ...
   do
   {
      ...
      GetAddrForAttribute( &pchOutsideText1, vDiagram,
                           szlHD_Node, szlOutsideText1 );
      GetAddrForAttribute( &pchOutsideText2, vDiagram,
                           szlHD_Node, szlOutsideText2 );
      GetAddrForAttribute( &pchOutsideText3, vDiagram,
                           szlHD_Node, szlOutsideText3 );
      GetAddrForAttribute( &pchOutsideText4, vDiagram,
                           szlHD_Node, szlOutsideText4 );
      GetAddrForAttribute( &pchOutsideText5, vDiagram,
                           szlHD_Node, szlOutsideText5 );
      GetAddrForAttribute( &pchType, vDiagram,
                           szlHD_Node, szlPurpose );
      ...
      nCollapsed = CompareAttributeToString( vDiagram,
                                             szlHD_Node, szlCollapsed, "Y" );
      nHidden = CompareAttributeToString( vDiagram,
                                          szlHD_Node, szlHidden, "Y" );
      ...
      else
      {
         ...
         if ( nCollapsed == 0 &&
              CheckExistenceOfEntity( vDiagram, szlHD_ChildNode ) >
                                                             zCURSOR_NULL )
         {
...
// zLONG    lOrientation;
   ...
   if ( GetViewByName( &vDiagram, *m_pzsVName,
                       m_pZSubtask->m_vDialog, zLEVEL_ANY ) < 1 )
   {
      return( 0 );
   }
...
ZHD_Diagram::PositionNodes( zVIEW  vDiagram )
{
   ...
   if ( CompareAttributeToString( vDiagram, szlHD_Node,
                                  szlHidden, "Y" ) == 0 )
   {
      nNodeWidth = 1;
   }
...
// nLeftEdgeStart -= nNodeWidth;
   ...
   if ( CompareAttributeToString( vDiagram, szlHD_Node,
                                  szlCollapsed, "Y" ) != 0 &&
        CompareAttributeToString( vDiagram, szlHD_Node,
                                  szlHidden, "Y" ) != 0 )
   {
   ...
   if ( m_bOrientation == FALSE )
   {
      ...
      SetAttributeFromInteger( vDiagram, szlHD_Node, szlPosY,
                               HALF_X_SPACING +
                               ((m_nLevel - 1) * (nNodeHeight + Y_SPACING)));
   }
   else
   {
...
ZHD_Diagram::SetScrollBars( )
{
   ...
   if ( (PosX( m_lDisplayPosMax ) - PosX( m_lDisplayPos )) <
                                                         (tzHDDIAGRAMX - 1) )
   {
...
ZHD_Diagram::SetDisplayPosMax( zLONG lX, zLONG lY )
{
   m_lDisplayPosMax = m_lDisplayPos +
                  ((lY / tzHDCELLSIZE) * tzHDDIAGRAMX) + (lX / tzHDCELLSIZE);
...
ZHD_Diagram::PositionOnDiagramNode( ZHD_Node *pZHD_Node, zVIEW vDiagram )
{
   ...
   while ( nParents )
   {
      ...
      SetCursorFirstEntityByInteger( vDiagram, szlHD_ChildNode, szlZKey,
                                     pWkNode->m_lZKey, 0 );
      SetViewToSubobject( vDiagram, szlHD_ChildNode );
      nParents--;
   }
   ...
   if ( (pZHD_Node = PointInNode( pt )) != 0 )
   {
           ...
           pt.x == pZHD_Node->PosX( ) && pt.y == pZHD_Node->PosY( ) )
      {
         ...
         if ( pZHD_Node->m_bCollapsed )
         {
            ...
            if ( SetCursorFirstEntity( vDiagram,
                                       szlHD_ChildNode, 0 ) == zCURSOR_SET )
            {
               do
               {
                  SetAttributeFromString( vDiagram, szlHD_ChildNode,
                                          szlCollapsed, "Y" );
                  SetAttributeFromString( vDiagram, szlHD_ChildNode,
                                          szlHidden, "N" );
               } while ( SetCursorNextEntity( vDiagram, szlHD_ChildNode, 0 )
                                                            == zCURSOR_SET );
            }
         }
         else
