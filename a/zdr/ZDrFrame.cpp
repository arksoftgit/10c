...
// ON_NOTIFY( TBN_DROPDOWN, 0xE822, OnToolbarDropDown )
   ON_NOTIFY_EX_RANGE( TBN_DROPDOWN, zZEIDON_FIRST_TOOLBAR,
                       zZEIDON_LAST_TOOLBAR, OnToolbarDropDown )
END_MESSAGE_MAP( )
...
FindMenuItem( CMenu *pMenu, zCPCHAR cpcMenuText )
{
   ...
   {
      if ( pMenu->GetMenuString( k, cs, MF_BYPOSITION ) &&
           (zstrstr( cs, cpcMenuText ) != 0) )
      {
         return( k );
      }
   }
...
//    k /= k;
   ...
   while ( lItemCnt > 0 )
   {
      if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
           ...
           m_pZSubtask == g_pSubtaskList[ lItemCnt ].pZSubtask )
      {
...
//                *(m_pZSubtask->m_pzsWndTag) );
   ...
   {
      m_pZSubtask->Attr.StyleEx |= WS_EX_APPWINDOW | WS_EX_CONTROLPARENT |
   ...
   if ( m_pZSubtask->m_hInstDLL )
   {
      ...
      HICON hIcon = m_pZSubtask->m_pZTask->m_pZDrApp->
                                   LoadIcon( *(m_pZSubtask->m_pzsDLL_Name) );
...
#if 0
   if ( m_wndToolBar.Create( this ) == 0 ||
        m_wndToolBar.LoadToolBar( IDR_MAINFRAME ) == 0 )
   {
...
// All predefined Control-menu items have ID numbers greater than 0xF000.
// If an application adds items to the Control menu, it should use ID
                   ...
                   zCPCHAR cpcDLL_Name, zCPCHAR cpcOperationName )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      {
         zBOOL bRC =
         ((ZDrFrame *) (pZSubtask->m_pZFWnd))->SetSystemMenuItem( cpcItemText,
                                                                  cpcAfterItem,
                                                                  uItemId,
                                                                  cpcDLL_Name,
                                                                  cpcOperationName );
...
AddHotkeysToGlobalHotkeys( zVIEW vSubtask )
{
   ...
   GetBlobFromAttribute( Accels + ulHotkeyLth,
                         &ulGlobalHotkeyLth, vHotkey, "App", "Hotkey" );

   // Reset the ID since it is used later to position on the Hotkey entity.
   // Also force the zACCEL_ACTION window keys to zACCEL_HOTKEY keys that
...
// while ( k >= 0 )
// {
//    TraceLine( "AddHotkeysToGlobalHotkeys: lpAT[%d].nID ==> %d for VK = 0x%x",
...
// }
   ...
   while ( kk >= 0 )
   {
      ...
      TraceLine( "AddHotkeysToGlobalHotkeys: reset lpAT[%d].nID ==> %d for VK = 0x%x",
            kk, lpAT[ kk ].nID, lpAT[ kk ].ac.uVK );
      kk--;
   }
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      SetAttributeFromBlob( vHotkey, "App", "Hotkey", Accels,
            ...
            nRC = SetCursorNextEntity( vSubtask, "Hotkey", 0 ) )
      {
         if ( CheckExistenceOfEntity( vSubtask, "HotAct" ) >= zCURSOR_SET )
         {
            ...
            SetAttributeFromAttribute( vHotkey, "Hotkey", "Tag",
                                       vSubtask, "Hotkey", "Tag" );
            SetAttributeFromAttribute( vHotkey, "Hotkey", "Title",
                                       vSubtask, "Hotkey", "Title" );
            SetAttributeFromAttribute( vHotkey, "Hotkey", "COP",
                                       vSubtask, "HotAct", "DO" );
            SetAttributeFromAttribute( vHotkey, "Hotkey", "DLL",
                                       vSubtask, "Dlg", "DLL" );
...
ZDrFrame::OnSetCursor( CWnd *pWnd, UINT uHitTest, UINT uMsg )
{
   ...
   if ( (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
        m_pZSubtask && m_pZSubtask->m_pZView )
   {
...
ZDrFrame::OnNcDestroy( )
{
   ...
   TraceLine( "ZDrFrame::OnNcDestroy: [0x%08x]  %s.%s  hWnd: 0x%08x", this,
              ...
              m_pZSubtask && m_pZSubtask->m_pzsWndTag ? *(m_pZSubtask->m_pzsWndTag) : "Wnd?",
              m_hWnd );
#endif
...
ZDrFrame::OnDestroy( )
{
   ...
   TraceLine( "ZDrFrame::OnDestroy  [0x%08x]  IdNbr: %d  %s.%s  IdNbr: %d  hWnd: 0x%08x  =======>>>>>>>>>>>",
              ...
              *(m_pZSubtask->m_pzsDlgTag), *(m_pZSubtask->m_pzsWndTag),
              m_pZSubtask->m_ulSubtaskIdNbr, m_hWnd );
...
ZDrFrame::DestroyWindow( )
{
   ...
   if ( (m_pZSubtask->m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) &&
        m_pZSubtask != m_pZSubtask->GetSplitterMainSubtask( ) )
   {
#if 0
      if ( m_pZView->m_pOrigParent == this )
      {
         if ( m_pZSubtask->m_pZParent &&
              m_pZSubtask->m_pZParent->m_pSplitterView )
         {
            m_pZSubtask->m_pZParent->
               m_pSplitterView->SendMessage( zZEIDON_POSTED_SPLITTER_CLOSE,
                                             0, (LPARAM) m_pZSubtask );
         }
      }
#endif
   }
   ...
   // TraceLineI( "ZdrFrame::OnDestroy uEdge", uEdge );
      ...
      WriteWindowPlacement( m_pZSubtask->m_vDialog,
                            "ZDR", *(m_pZSubtask->m_pzsDlgTag),
                    ...
                    AFX_CMDHANDLERINFO *pHandlerInfo )
{
   ...
   if ( CFrameWnd::OnCmdMsg( uID, nCode, pExtra, pHandlerInfo ) != 0 ||
        (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) != 0 ||
...
#if 1
   if ( uID >= AFX_IDW_CONTROLBAR_FIRST + 32 &&
        uID <= AFX_IDW_CONTROLBAR_LAST )
   {
      if ( nCode == CN_UPDATE_COMMAND_UI || nCode == CN_COMMAND )
      {
      // TraceLineI( "ZDrFrame::OnCmdMsg CN_UPDATE_COMMAND_UI nID: ",
...
ZDrFrame::OnCreateClient( LPCREATESTRUCT lpcs, CCreateContext *pContext )
{
   ...
   if ( m_pZView->Create( NULL, NULL, AFX_WS_DEFAULT_VIEW,
                          CRect( 0, 0, 0, 0 ), this,
      ...
      case SC_MINIMIZE:
      {
         ...
         while ( lPos > 0 )
         {
            if ( g_pSubtaskList[ lPos ].pZSubtask &&
                 ...
                 mIs_hWnd( g_pSubtaskList[ lPos ].hWnd ) )
            {
            // vSubtaskReturn = (zVIEW)
            ...
            // if ( vSubtaskReturn && MiValidView( vSubtaskReturn ) &&
               ...
               pZTask->m_pWndList->Add( (zLONG) g_pSubtaskList[ lPos ].hWnd,
                                        (zLONG) g_pSubtaskList[ lPos ].pZSubtask,
                                        lState, 0 );
...
// TraceLineX( "ZDrFrame::OnSysCommand = ", uCmdType );
   ...
   switch ( uCmdType & 0xFFF0 )
   {
      case SC_CLOSE:
         ...
         {
            CFrameWnd *pFrame = 0;
            if ( pZSubtask && pZSubtask->m_pZParent &&
                 pZSubtask->m_pZParent->m_pZFWnd &&
                 mIs_hWnd( pZSubtask->m_pZParent->m_pZFWnd->m_hWnd ) )
            {
            // TraceLineX( "ZDrFrame::OnSysCommand ShowWindow: ",
            //             (zLONG) pZSubtask->m_pZParent->m_pZFWnd->m_hWnd );
...
//       TraceLineS( "ZDrFrame::OnSysCommand SC_MINIMIZE ",
//                   *(pZSubtask->m_pzsWndTag) );
#if 0 //def QUINSOFT
         if ( IsZoomed( ) )
         {
            if ( pZSubtask->m_pZTask->m_nOS_Version >= 60 &&
                 pZSubtask->m_pZTask->m_nOS_Version < 70 )
            {
...
//       TraceLineS( "ZDrFrame::OnSysCommand SC_RESTORE ",
//                   *(pZSubtask->m_pzsWndTag) );
         lEvent = zWND_EVENT_RESTORE;
         break;
      ...
      // if ( m_pSystemMenu )
         {
            ...
            {
               bHandled = SetApplicationState( SC_RESTORE );
            // m_pSystemMenu->ModifyMenu( 0xEFF0,
...
//    m_pZSubtask->m_pEventParms = (zPVOID) wParam;
   ...
   return( ProcessActionLoop( m_pZSubtask, (zCPCHAR) lParam,
                              wParam, 0, 0, FALSE, "CtrlEvent" ) );
}
...
// TraceLine( "ZDrFrame::OnProcessHotkey Zeidon Hotkey: %d   DLL: %s   Operation: %s",
//            lParam, szDLL_Name, szOperName );
   lpfnDynRoutine = (zFARPROC_DRIVER)
   ...
   {
   // TraceLine( "ZDrFrame::OnProcessHotkey could not load operation %s from %s",
   //            szOperName, szDLL_Name );
   }
...
ZDrFrame::OnPostedPopupMenu( WPARAM wParam, LPARAM lParam )
{
   if ( m_pZSubtask && m_pZSubtask->m_pzsPopupTag )
   {
      ...
      zstrcpy( szPopupTag, *(m_pZSubtask->m_pzsPopupTag) );
...
ZDrFrame::OnPostedSplitterClose( WPARAM wParam, LPARAM lParam )
{
   ...
   if ( m_pZSubtask->m_pSplitterView )
   {
      ...
      if ( pSplitterWnd->m_pZView == pBaseView )
      {
         ...
         zsprintf( szMsg, "ZDrFrame::OnPostedSplitterClose SetParent of (SplitterData ZView) 0x%08x "
                          "to (pParentWnd) 0x%08x from 0x%08x",
                   pSplitterWnd->m_pZView, pParentWnd, p );
         TraceLineS( szMsg, "" );
...
// TraceLineX( "OnFindDialogWindow pfd->pvDialog: ", (zLONG) pfd->pvDialog );
   ...
   if ( pfd->pvDialog )
   {
      GetWindowAndCtrl( &(pfd->pZSubtask), 0,
                        (zVIEW) SysGetPointerFromHandle( pfd->pvDialog ), 0 );
   }
   else
   {
      pfd->pZSubtask = fnGetSubtask( m_pZSubtask, &vSubtask,
                                     pfd->DlgN, pfd->WndN, FALSE );
   }
   ...
   if ( pfd->pZSubtask )
   {
      ...
      zstrcpy( pfd->DlgN, *(pZSubtask->m_pzsDlgTag) );
      zstrcpy( pfd->WndN, *(pZSubtask->m_pzsWndTag) );
   }
...
// TraceLineI( "ZDrFrame::OnCommand uID = ", uID );
   ...
   // Note that uID's < zPOPUP_MENU_FIRST are standard menu id's default
   // routing for command messages (through closure table).
   if ( hWndCtrl == 0 )  // ==> this is a menu command
   {
      zVIEW vApp = 0;
      zCHAR szChkValue[ 65 ];

      ZSubtask *pZSubtask = m_pZSubtask->m_pZSubtaskGlobal ?
                      m_pZSubtask->m_pZSubtaskGlobal : m_pZSubtask;
...
//       uNotifyCode = 0;        // accelerators are not special
         if ( pOption )
         {
            ...
            TraceLineS( "Option command Tag - ",
                        *(pOption->m_pzsTag) );
            TraceLineS( "Option command ViewName - ",
                        *(pOption->m_pzsVName) );
            TraceLineS( "Option command Entity - ",
                        *(pOption->m_pzsEName) );
            TraceLineS( "Option command Attribute - ",
                        *(pOption->m_pzsAName) );
            TraceLineS( "Option command ACTION - ",
                        *(pOption->m_pzsActionTag) );
#endif

            if ( pOption->m_pzsVName )
            {
               if ( GetViewByName( &vApp, *(pOption->m_pzsVName),
                                   pOption->m_pZSubtask->m_vDialog,
                                   zLEVEL_ANY ) <= 0 )
               {
            ...
            // If it's off ... it's going to be turned on.  If there are
            // other options that have the same mapping, they must all
            ...
            if ( vApp && pOption->m_bChecked == FALSE )
            {
               ...
               for ( k = 1; k <= ulNodeCnt; k++ )
               {
                  ...
                  if ( pOptTemp->m_bChecked && pOptTemp->m_pzsVName &&
                       zstrcmp( *(pOptTemp->m_pzsAName),
                                *(pOption->m_pzsAName) ) == 0 &&
                       zstrcmp( *(pOptTemp->m_pzsEName),
                                *(pOption->m_pzsEName) ) == 0 &&
                       zstrcmp( *(pOptTemp->m_pzsVName),
                                *(pOption->m_pzsVName) ) == 0 )
                  {
                     pOptTemp->m_bChecked = FALSE;
//                   pOption->m_pZSubtask->m_pMenu->
//                           CheckMenuItem( pOptTemp->m_uOptionID,
//  done in MenuInit method!!!                 MF_BYCOMMAND | MF_UNCHECKED );
            ...
            if ( vApp &&
                 CheckExistenceOfEntity( vApp, *(pOption->m_pzsEName) )
                                                        > zCURSOR_UNCHANGED )
            {
               if ( pOption->m_bChecked )
               {
                  zstrcpy( szChkValue, *(pOption->m_pzsValueOff) );
                  pOption->m_bChecked = FALSE;
               }
               else
               {
                  zstrcpy( szChkValue, *(pOption->m_pzsValueOn) );
...
//             TraceLineS( " Set Value - ", szChkValue );
               ...
               SetAttributeFromVariable( vApp,
                                         *(pOption->m_pzsEName),
                                         *(pOption->m_pzsAName),
                                         szChkValue, zTYPE_STRING, 0,
                                         *(pOption->m_pzsContext),
                                         **(pOption->m_pzsContext) ?
                                                 0 : zUSE_DEFAULT_CONTEXT );
//             GetStringFromAttribute( szChkValue, vApp,
//                                     *(pOption->m_pzsEName),
//                                     *(pOption->m_pzsAName) );
//             TraceLineS( " Get Value Before - ", szChkValue );

//             pOption->m_pZSubtask->
//                m_pMenu->CheckMenuItem( pOption->m_uOptionID,
//                                         MF_BYCOMMAND |
//                                           (pOption->m_bChecked ?
//                                                           MF_CHECKED :
//  done in MenuInit method!!!                               MF_UNCHECKED) );
            }

            if ( pOption->m_pzsActionTag )
            {
               pOption->m_pZSubtask->m_pzsLastTag = pOption->m_pzsTag;
               ProcessActionLoop( pOption->m_pZSubtask,
                                  *(pOption->m_pzsActionTag),
                                  0, 0, 0, FALSE, "Option" );
            }
...
//          }
            ...
            return( TRUE );   // We have processed the menu ... if we do
                              // not return TRUE (==> we have processed the
                              // message), the CWnd class will attempt to
                              // forward the message to the default proc.
                              // If our processing has destroyed this frame
                              // window, the CWnd instance will have been
      ...
      if ( pZSubtask->m_pZBinPopup && uID >= zPOPUP_MENU_FIRST )
      {
         ...
         if ( pOption && pOption->m_pzsVName &&
              GetViewByName( &vApp, *(pOption->m_pzsVName),
                             pOption->m_pZSubtask->m_vDialog,
                             zLEVEL_ANY ) > 0 )
         {
            if ( pOption->m_bChecked )
...
//          TraceLineS( " Set Value - ", szChkValue );
         ...
         return( TRUE );   // We have processed the popup menu ... if we
                           // not return TRUE (==> we have processed the
                           // message), the CWnd class will attempt to
                           // forward the message to the default proc.
                           // If our processing has destroyed this frame
   ...
   {
      // The driver ignores all messages in this range (to permit
      // handling of special messages by the application)
...
//          TraceLineI( "(drvr) OnCommand: ", uID );
            ...
            {
               m_pZSubtask->m_pZView->SendMessage( WM_COMMAND,
...
//             TraceLineX( "ZDrFrame::OnCommand PREVIEW_CLOSE hWnd", (zLONG) hWndCtrl );
               ...
               {
                  if ( m_pZSubtask && m_pZSubtask->m_pZParent &&
                       m_pZSubtask->m_pZParent->m_pZFWnd &&
                       mIs_hWnd( m_pZSubtask->m_pZParent->m_pZFWnd->m_hWnd ) )
                  {
//                   TraceLineX( "ZDrFrame::OnCommand ShowWindow: ",
//                               (zLONG) m_pZSubtask->m_pZParent->m_pZFWnd->m_hWnd );
                     m_pZSubtask->m_pZParent->
                            m_pZFWnd->PostMessage( zZEIDON_POSTED_ACTIVATE );
                  }
               }
#endif
            }
...
ZDrFrame::OnBeginDrag( WPARAM wParam, LPARAM lParam )
{
   ...
   if ( (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
        pzmaSrc && pzmaSrc->m_pchDragDrop )
   {
      m_pZSubtask->m_pZTask->m_hcurSave =
               (HCURSOR) ::SetCursor( m_pZSubtask->m_pZTask->m_hcurDropNot );
      zstrcpy( g_DragDrop.szSrcDragKey,
               pzmaSrc->m_pchDragDrop + (2 * sizeof( zLONG )) );
      g_DragDrop.pzmaSrc = pzmaSrc;
      g_DragDrop.vSrcSubtask = pzmaSrc->m_pZSubtask->m_vDialog;
      zstrcpy( g_DragDrop.szSrcDialogName,
               *(pzmaSrc->m_pZSubtask->m_pzsDlgTag) );
      zstrcpy( g_DragDrop.szSrcWindowName,
               *(pzmaSrc->m_pZSubtask->m_pzsWndTag) );
      zstrcpy( g_DragDrop.szSrcCtrlName, *(pzmaSrc->m_pzsTag) );
      g_DragDrop.vData = 0;

      if ( pzmaSrc->m_pzsVName )
         zstrcpy( g_DragDrop.szViewName, *(pzmaSrc->m_pzsVName) );

      if ( pzmaSrc->m_pzsEName )
         zstrcpy( g_DragDrop.szEntityName, *(pzmaSrc->m_pzsEName) );

      if ( pzmaSrc->m_pzsAName )
         zstrcpy( g_DragDrop.szAttributeName, *(pzmaSrc->m_pzsAName) );
...
// TraceLineI( "Child  = ", (zLONG) hWndChild );
   ...
   {
      if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
           g_pSubtaskList[ lItemCnt ].lTID == m_pZSubtask->m_lTID &&
           ((g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZView &&
             g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZView->m_hWnd ==
                                                             pWnd->m_hWnd) ||
            g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZFWnd->m_hWnd ==
                                                             pWnd->m_hWnd) )
      {
...
// TraceLineI( "Subtask found = ", (zLONG) pZSubtask->m_pZFWnd->m_hWnd );
   ...
   while ( k )
   {
      ...
      if ( pzmaTgt && pzmaTgt->m_pCtrl && pWndChild &&
           pzmaTgt->m_pCtrl->m_hWnd == pWndChild->m_hWnd )
      {
         break;
      }
...
// TraceLineS( "Zeidon DoDrop", "" );
   ...
   if ( g_pzmaDragDrop )  // this guy will accept the drop
   {
      ...
      zstrcpy( g_DragDrop.szTgtDialogName,
               *(g_pzmaDragDrop->m_pZSubtask->m_pzsDlgTag) );
      zstrcpy( g_DragDrop.szTgtWindowName,
               *(g_pzmaDragDrop->m_pZSubtask->m_pzsWndTag) );
      zstrcpy( g_DragDrop.szTgtCtrlName,
               *(g_pzmaDragDrop->m_pzsTag) );

      // Need to check for the existence of a DoDrop event on this
      // control.  If the DoDrop event does not exist, the do drop
      // is denied.  Otherwise, the application code specified for
      // the event is called to process the drop.
...
// #define WA_CLICKACTIVE  2 - window is being activated by mouse-click
   ...
   zULONG ulSubtaskState = ZSubtask::SetSubtaskState( m_pZSubtask,
                                                      zSUBTASK_STATE_ON_RETURN,
                                                      0 );

   if ( fActive && pWndOther == this &&
        (ulSubtaskState & (zSUBTASK_STATE_IN_CONSTRUCTION |
                           zSUBTASK_STATE_ON_RETURN)) == 0 )
   {
      return;
   }

   if ( (m_ulFrameFlags & zSUBTASK_STATE_MARK_FOR_DELETE) ||
        (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) ||
        (ulSubtaskState & (zSUBTASK_STATE_POSTBUILD_PRECODE |
                           zSUBTASK_STATE_POSTBUILD_POSTCODE |
                           zSUBTASK_STATE_WELL_FORMED)) == 0 )
   {
      return;
   }

   CFrameWnd::OnActivate( fActive, pWndOther, bIsIconified );
   if ( fActive != WA_INACTIVE && bIsIconified == false &&
        (m_pZSubtask->m_ulSubtaskFlags & zSUBTASK_PROCESSING_ACTIVATE) == 0 &&
        m_pZSubtask->m_nInvisible >= 0 )
   {
      ...
      if ( m_pZSubtask->m_pZMIXCtrl &&
           (m_pZSubtask->m_ulWndOper & zWND_EVENT_NOTIFYCHILD_ACTIVATE) ==
                                            zWND_EVENT_NOTIFYCHILD_ACTIVATE )
      {
         m_pZSubtask->m_pZMIXCtrl->
             VisitInorder( (fnVISIT_INORDER) fnSendCtrlsParentActivated, 0 );
      }

      if ( (m_pZSubtask->m_ulWndOper & zWND_EVENT_ON_ACTIVATE) ==
                                                    zWND_EVENT_ON_ACTIVATE &&
           SetCursorFirstEntityByInteger( m_pZSubtask->m_vDialog, szlWndEvent,
                                          szlType, zWND_EVENT_ON_ACTIVATE, 0 )
                                                         > zCURSOR_UNCHANGED )
      {
         ProcessActionLoop( m_pZSubtask, szlWndAct, 0, 0,
                            zACTION_DIALOG_CODE_ONLY, TRUE,
                            "OnActivate" );   // what happens on error???
      }
...
ZDrFrame::OnKillFocus( CWnd *pWndGotFocus )
{
   ...
   if ( ((ulSubtaskState = ZSubtask::GetSubtaskState( m_pZSubtask )) &
                                    zSUBTASK_STATE_MARK_FOR_DELETE) == 0 )
   {
      Default( );
   }
...
ZDrFrame::OnSetFocus( CWnd *pWndLostFocus )
{
   ...
   TraceLine( "ZDrFrame OnSetFocus  %s.%s  hWnd: 0x%08x   LostFocus: 0x%08x",
              *(m_pZSubtask->m_pzsDlgTag), *(m_pZSubtask->m_pzsWndTag),
              m_hWnd, pWndLostFocus ? pWndLostFocus->m_hWnd : 0 );
#endif
...
//             CFrameWnd::OnSetFocus( hWndLostFocus );
         ...
         {
//          TraceLineS( "ZDrFrame m_pzmaWithFocus ",
//                      *(m_pZSubtask->m_pzmaWithFocus->m_pzsTag) );
            if ( m_pZSubtask->m_pzmaWithFocus->m_pCtrl &&
                 mIs_hWnd( m_pZSubtask->m_pzmaWithFocus->m_pCtrl->m_hWnd ) )
            {
...
ZDrFrame::OnSize( UINT uSizeType, int cx, int cy )
{
   ...
   if ( (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) ||
        (ulSubtaskState & 0x000000FF) < zSUBTASK_STATE_POSTBUILD_PRECODE )
   {
      return;
   }

   if ( (m_pZSubtask->m_ulWndOper & zWND_EVENT_ON_SIZE) == zWND_EVENT_ON_SIZE &&
        SetCursorFirstEntityByInteger( m_pZSubtask->m_vDialog, szlWndEvent, szlType,
                                       zWND_EVENT_ON_SIZE, 0 ) >
                                                         zCURSOR_UNCHANGED )
   {
...
ZDrFrame::OnMove( int x, int y )
{
   ...
   if ( (m_pZSubtask->m_ulWndOper & zWND_EVENT_ON_MOVE) ==
                                                 zWND_EVENT_ON_MOVE &&
   ...
   if ( m_pZSubtask->m_pZMIXCtrl )
   {
      CPoint pt( x, y );
      m_pZSubtask->m_pZMIXCtrl->
                VisitInorder( (fnVISIT_INORDER) fnSendCtrlsParentMove,
                              0, 0, (LPARAM) &pt );
   }
}
...
ZDrFrame::OnMenuSelect( UINT uMenuItemId, UINT uFlags, HMENU hMenu )
{
   ...
   if ( (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
        pZSubtask->m_pDIL && pZSubtask->m_nAutoDIL )
   {
      ZOption *pOption = 0;
      zPCHAR  pch;
      ...
      // TraceLineS( "ZDrFrame::OnMenuSelect ", "Do Nothing" );
         GetAddrForAttribute( (zPCHAR *) &pch, pZSubtask->m_vDialog,
                              szlDIL, szlD_Msg );
      }
      else
      {
      // TraceLineS( "ZDrFrame::OnMenuSelect ", "Do Something" );
         if ( m_pZSubtask->m_pZBinPopup && uMenuItemId > zPOPUP_MENU_FIRST )
            pOption = (ZOption *) m_pZSubtask->m_pZBinPopup->
                                 GetAt( uMenuItemId - zPOPUP_MENU_FIRST );
         else
         if ( m_pZSubtask->m_pZBinMenu )
            pOption = (ZOption *) m_pZSubtask->m_pZBinMenu->
                                                     GetAt( uMenuItemId );

         if ( pOption )
            SetCursorRelativeEntity( m_pZSubtask->m_vDialog,
                                     szlDIL, pOption->m_nDIL, 0 );
...
ZDrFrame::OnInitMenu( CMenu *pMenu )
{
   ...
   {
      if ( (m_pZSubtask->m_ulWndOper & zWND_EVENT_POPUP_PRE_DROPDOWN)
                                           == zWND_EVENT_POPUP_PRE_DROPDOWN )
      {
      ...
      for ( k = 1; k <= pZSubtask->m_pZBinPopup->GetCount( ); k++ )
      {
         ...
         pZSubtask->m_pTrackMenu->CheckMenuItem( pZOption->m_uOptionID,
                                                   MF_BYCOMMAND |
                                                    (bChecked ? MF_CHECKED :
                                                              MF_UNCHECKED) );
      }
   ...
   else
   {
      ...
      if ( (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) == 0 &&
           pZSubtask->m_pZBinMenu && pMenu == pZSubtask->m_pMenu )
      {
         pZSubtask->AutoGrayActions( );
         if ( (pZSubtask->m_ulWndOper & zWND_EVENT_MENU_PRE_DROPDOWN) ==
                                               zWND_EVENT_MENU_PRE_DROPDOWN )
         {
         ...
         for ( k = 1; k <= pZSubtask->m_pZBinMenu->GetCount( ); k++ )
         {
            ...
            bChecked = pZOption->SetCheckForOption( );
            pZSubtask->m_pMenu->CheckMenuItem( pZOption->m_uOptionID,
                                               MF_BYCOMMAND |
                                                 (bChecked ? MF_CHECKED :
                                                             MF_UNCHECKED) );
         // TraceLineS( "ZDrFrame::OnInitMenu EnableMenuItem", "" );
            pZSubtask->m_pMenu->EnableMenuItem( pZOption->m_uOptionID,
                                                MF_BYCOMMAND |
                                                  pZOption->m_bEnabled ?
   ...
   if ( lOper &&
        SetCursorFirstEntityByInteger( pZSubtask->m_vDialog, szlWndEvent,
                                       szlType, lOper, 0 ) > zCURSOR_UNCHANGED )
   {
...
//       TraceLineI( "OnInitMenuPopup SC_CLOSE: ", SC_CLOSE );
         ...
         // TraceLineI( "ZDrFrame::OnInitMenuPopup uID: ", uID );
            ...
            {
               if ( nState == 0 )
                  pPopupMenu->EnableMenuItem( k, MF_BYPOSITION |
...
ZDrFrame::OnPaletteChanged( CWnd *pWndPalChg )
{
   ...
   {
      if ( m_pZSubtask->m_pZMIXCtrl &&
           (m_pZSubtask->m_ulWndOper & zWND_EVENT_NOTIFYCHILD_PALETTE) ==
                                             zWND_EVENT_NOTIFYCHILD_PALETTE )
      {
         m_pZSubtask->m_pZMIXCtrl->
               VisitInorder( (fnVISIT_INORDER) fnSendCtrlsPaletteChanged,
                             0, 0, (LPARAM) pWndPalChg );
      }

      if ( (m_pZSubtask->m_ulWndOper & zWND_EVENT_ON_PALETTE_CHANGE) ==
                                             zWND_EVENT_ON_PALETTE_CHANGE &&
           SetCursorFirstEntityByInteger( m_pZSubtask->m_vDialog,
                                          szlWndEvent, szlType,
                                          zWND_EVENT_ON_PALETTE_CHANGE, 0 )
                                                      > zCURSOR_UNCHANGED )
      {
         if ( ProcessActionLoop( m_pZSubtask, szlWndAct, 0, 0,
                                 zACTION_DIALOG_CODE_ONLY, TRUE,
...
ZDrFrame::OnToolbarDropDown( UINT uIdNbr, NMHDR *pNMHDR, LRESULT *plResult )
{
   ...
   if ( uID )
   {
      ...
      if ( hWnd )
      {
         ::SendMessage( hWnd, TB_PRESSBUTTON, uID,
   ...
   VERIFY( pPopup->Display( &m_lstPopup, &m_btnListCtrl,
                            rectCaller, &m_lstPopup.m_xPopupCtrlEvent ) );

////////////////////////////////////////////////////////////////////////////
   CRect rectCaller;

   // Get rect of toolbar item
   m_wndToolBar.GetItemRect( m_wndToolBar.CommandToIndex( ID_EDIT_MYUNDO ),
                             rectCaller );
   ...
   //¨Popup the TreeCtrl
   VERIFY( pPopup->Display( &m_treePopup, this,
                            rectCaller, &m_treePopup.m_xPopupCtrlEvent ) );
...
////////////////////////////////////////////////////////////////////////////
   ...
   m_wndToolBar.SendMessage( TB_PRESSBUTTON, lpnmtb->iItem,
                             (LPARAM) MAKELONG( TRUE, 0 ) );
...
// TraceLineX( "ZDrFrame::OnToolTipText Sending nID: ", nID );
   ...
   {
//    TraceLineX( "ZDrFrame::OnToolTipText", (zLONG) (HWND) pTTTW->lParam );
      ::SendMessage( (HWND) hWnd, zZEIDON_GET_TOOLTIP_TEXT,
                     uID, (LPARAM) szTipText );
   }
...
ZDrFrame::OnUpdateKeyIndicator( CCmdUI *pCmdUI )
{
   ...
   switch ( pCmdUI->m_nID )
   {
      ...
      default:
         TraceLineI( "Warning: OnUpdateKeyIndicator - unknown indicator: ",
                     pCmdUI->m_nID );
         pCmdUI->ContinueRouting( );
         return; // not for us
   }
...
#if 0    // by handling OnUpdateCmdUI properly in the ControlBar, the
         // command handler is not needed ... except for standard commands
         // like CUT, PASTE, etc?
void
ZDrFrame::OnMenuSelection( )
{
   // This sample doesn't do anything with button presses but a command
   // handler needs to be present for MFC to enable the buttons
...
ZDrFrame::PreTranslateMessage( MSG *pMsg )
{
   ...
   if ( pMsg->message == WM_SYSKEYDOWN || pMsg->message == WM_KEYDOWN )
   {
   // TraceLine( "ZDrFrame::PreTranslateMessage %d, %d",
