                  ...
                  PARAFORMAT *pf )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for RT_GetParaFormat ",
                  cpcCtrlTag );
   }
               ...
               zLONG   lMaxLth )
{
        ...
        GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pRE )
      {
         ...
         {
            zstrcpy( pchBuffer, zs );
            return( zs.GetLength( ) );
         }
         else
         {
            zstrncpy( pchBuffer, zs, lMaxLth - 1 );
            pchBuffer[ lMaxLth - 1 ] = 0;
            return( -1 );
         }
      }

      TraceLineS( "drvr - Invalid control type for RT_GetSelText ",
                  cpcCtrlTag );
   }
