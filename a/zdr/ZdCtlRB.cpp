...
// mDeleteInit( m_pBrush );
   ...
   zmemset( pCtrlDef->Tag, 0xFE, sizeof( zKZWDLGXO_Ctrl_DEF ) );
// pCtrlDef->CheckByte = 0xFE;
   ...
   while ( nRC > zCURSOR_UNCHANGED )
   {
      ...
      GetStructFromEntityAttrs( (zPCHAR) pCtrlDef->Tag, vSubobject, szlCtrl );
#ifdef DEBUG_LODSTRUCT
         if ( pCtrlDef->CheckByte != 0xFE )
            ::MessageBox( 0, "CtrlDef Structure Length Error",
                          "GetStructFromEntityAttrs", MB_OK );
      // zPCHAR pchCheckByte = (zPCHAR) &(pCtrlDef->CheckByte);
      // pchCheckByte--;
      // if ( *pchCheckByte == (char) 0xFE )
      //    TraceLineS( "zdctlrb CtrlDef CheckByte error??", "" );
#endif

      new ZRadioBtn( pZSubtask, pWndParent, this, vSubobject,
                     nOffsetX, nOffsetY, pCtrlDef );

      nRC = SetEntityCursor( vSubobject, szlCtrl, 0, zPOS_NEXT,
                             0, 0, 0, 0, 0, 0 );
   }
#else
...
ZRadioBtnGroup::GetZCtrlState( zLONG lStatusType )
{
   if ( lStatusType == zCONTROL_STATUS_CHECKED )
   {
      ...
      {
         m_pchCurrRB_Value =
            new char[ zstrlen( *(m_pZCurrRB->m_pzsValueOn) ) + 1 ];
         zstrcpy( m_pchCurrRB_Value, *(m_pZCurrRB->m_pzsValueOn) );
