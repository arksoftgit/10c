...
//  97 - Scope - 3-byte char (2 unused) - Scope Entity OI
   ...
   zstrncat( pCtrlDef->Tag, "_MiniCal", 33 - nLth );
   ...
   zstrcpy( m_szScopeEntity, szBlob + 64 );
   m_chScope = szBlob[ 97 ];
...
ZDayTimer::MapToOI( zLONG lFlag )
{
        ...
        (vApp = IsVisibleForMapToOI( *m_pzsVName )) != 0 )
   {
            ...
            m_pMiniCal->m_SelectedDays.GetNext( pos ) )
      {
         ...
         SetZeidonDateFromOleDateTime( szDate, pSel->m_date );
         if ( SetCursorFirstEntityByString( vApp, *m_pzsEName, *m_pzsAName,
                                            szDate, 0 ) != zCURSOR_SET )
         {
            CreateEntity( vApp, *m_pzsEName, zPOS_AFTER );
            SetZeidonDateAttributeFromOleDateTime( pSel->m_date, vApp,
                                                   *m_pzsEName, *m_pzsAName );
         }
...
ZDayTimer::UpdateAppointment( ZAppointment *pA )
{
        ...
        *m_csAlarmAttribute && *m_csFullDayAttribute )
   {
      ...
      SetZeidonDateFromOleDateTime( szDate, pA->m_date );
      if ( SetCursorFirstEntityByString( vApp, *m_pzsEName, *m_pzsAName,
                                         szDate, 0 ) != zCURSOR_SET )
      {
         CreateEntity( vApp, *m_pzsEName, zPOS_AFTER );
...
ZAgenda::OnMouseMove( UINT uFlags, CPoint pt )
{
   ...
   if ( m_bTrackingSelected )
   {
      ...
      {
         if ( m_timeFirstSelected.GetMinute( ) == 0 )
            m_timeLastSelected.SetTime( m_timeFirstSelected.GetHour( ),
                                        30, 0 );
         else
            m_timeLastSelected.SetTime( m_timeFirstSelected.GetHour( ) + 1,
                                        0, 0 );
      }
                          ...
                          COleDateTime& dateTracked )
{
   ...
   if ( m_pDayTimer )
   {
      ...
      if ( uKey && m_pDayTimer->m_pzsVName &&
           (vApp = m_pDayTimer->
              IsVisibleForMapToOI( *(m_pDayTimer->m_pzsVName) )) != 0 &&
           ...
           *(m_pDayTimer->m_csFullDayAttribute) )
      {
         ...
         SetZeidonDateFromOleDateTime( szDate, date );
                               ...
                               0 ) >= zCURSOR_SET )
         {
            ...
            CreateViewFromViewForTask( &vTgt, vApp,
                                       m_pDayTimer->m_pZSubtask->m_vDialog );
            SetZeidonDateFromOleDateTime( szDate, dateTracked );
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pDayTimer )
      {
         ...
         if ( pA )
         {
            ...
            zstrncat( pchMsg, csMsg, lMsgMaxLth - 1 );
         }

         return( 0 );
      }

      TraceLineS( "drvr - Invalid control type for DT_DisplayCurrentAppointment ",
                  cpcCtrlTag );
   }
