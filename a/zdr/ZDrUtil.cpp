...
//    directory to LOCAL work directory specified in Zeidon.app. (TB 336)
//
// 1998.08.13    DKS
...
void
fnGetLocalPath( zVIEW vSubtask, zPCHAR pchLocalPath, zCPCHAR cpcIniID )
{
   LPAPP pApp;
   static int nTrace = 0;

   if ( vSubtask &&
        SfGetApplicationForSubtask( &pApp, vSubtask ) == 0 && pApp )
   {
      zstrcpy( pchLocalPath, pApp->szLocalDir );
   }
   else
                     ...
                     LPWINDOWPLACEMENT pwp )
{
   ...
   fnGetLocalPath( vSubtask, szZeidonPath, cpcIniID );
...
#ifdef DEBUG_ALL
   zCHAR szMsg[ 512 ];
   zsprintf( szMsg, "%s %s: %s", szSection,
                      ...
                      CControlBarInfo *pCB_Info )
{
   ...
   zstrcpy( szCtrlInfo, "CtrlInfo:" );
   zstrcpy( szCtrlInfo + 9, cpcBarID );
               ...
               zLONG    lBufferLth )
{
   ...
   {
      zPCHAR pchBuffer = new char[ zstrlen( pchReturnBuffer ) + 1 ];
      zstrcpy( pchBuffer, pchReturnBuffer );
      SysConvertEnvironmentString( pchReturnBuffer, pchBuffer );
      delete [] pchBuffer;
   }
}
                        ...
                        zPCHAR pchVersion )
{
   ...
   switch ( osvi.dwPlatformId )
   {
      ...
      case VER_PLATFORM_WIN32_NT:
      {
         ...
         {
            zstrcat( szVersion, "Microsoft Windows NT " );
            nRC = 35;
         }
         ...
         {
            zstrcat( szVersion, "Microsoft Windows 2000 " );
            nRC = 50;
         }
         ...
         if ( bOsVersionInfoEx )  // use information from GetVersionEx
         {
            ...
            if ( osvi.wProductType == VER_NT_WORKSTATION )
            {
               ...
               {
                  zstrcat( szVersion, "Microsoft Windows XP " );
                  if ( osvi.wSuiteMask & VER_SUITE_PERSONAL )
                  {
                     zstrcat( szVersion, "Home Edition " );
                     nRC = 60;
                  }
                  else
                  {
                     zstrcat( szVersion, "Professional " );
                     nRC = 65;
                  }
               }
            }
            ...
            if ( osvi.wProductType == VER_NT_SERVER )
            {
               if ( osvi.dwMajorVersion == 5 && osvi.dwMinorVersion == 1 )
               {
                  zstrcat( szVersion, "Microsoft Windows .NET " );
                  nRC = 180;
               }

               if ( osvi.wSuiteMask & VER_SUITE_DATACENTER )
               {
                  zstrcat( szVersion, "DataCenter Server " );
                  nRC = 120;
               }
               else
               if ( osvi.wSuiteMask & VER_SUITE_ENTERPRISE )
               {
                  if ( osvi.dwMajorVersion == 4 )
                  {
                     zstrcat( szVersion, "Advanced Server " );
                     nRC = 130;
                  }
                  else
                  {
                     zstrcat( szVersion, "Enterprise Server " );
                     nRC = 140;
                  }
               }
               else
         ...
         if ( osvi.dwMajorVersion <= 4 )
         {
            zsprintf( szVersion + zstrlen( szVersion ),
      ...
      case VER_PLATFORM_WIN32_WINDOWS:
      {
         ...
         {
             zstrcpy( szVersion, "Microsoft Windows 95 " );
             if ( osvi.szCSDVersion[ 1 ] == 'C' ||
                  osvi.szCSDVersion[ 1 ] == 'B' )
             {
                nRC = 15;
                zstrcat( szVersion, "OSR2 " );
             }
             else
             {
                nRC = 10;
         ...
         {
             zstrcpy( szVersion, "Microsoft Windows 98 " );
             if ( osvi.szCSDVersion[ 1 ] == 'A' )
             {
                zstrcat( szVersion, "SE " );
                nRC = 25;
             }
             else
             {
         ...
         {
             zstrcpy( szVersion, "Microsoft Windows Millennium Edition " );
             nRC = 30;
         }
...
CopyStringToClipboard( zCPCHAR cpcSource )
{
   ...
   zPCHAR  pchBuffer;
   EmptyClipboard( );
   hClipBuffer = GlobalAlloc( GMEM_DDESHARE, zstrlen( cpcSource ) + 1 );
   pchBuffer = (zPCHAR) GlobalLock( hClipBuffer );
   zstrcpy( pchBuffer, cpcSource );
                  ...
                  zLONG   lFlags )          // reserved
{
   ...
   while ( TRUE )
   {
      zstrcpy( pchCommand, "mailto:" );
      zstrcat( pchCommand, cpcEmailAddress );
      if ( bRetry )
      {
      ...
      zstrcat( pchCommand, "?subject=" );
      lLth = zstrlen( pchCommand );
      if ( cpcSubject && *cpcSubject )
         TranslateEmail( pchCommand + lLth, cpcSubject );
      else
         TranslateEmail( pchCommand + lLth, " " );
      ...
      if ( cpcCopyTo && *cpcCopyTo )
      {
         ...
         {
            zstrcat( pchCommand, "&cc=" );
            zstrcat( pchCommand, cpcCopyTo );
         }
      }
      ...
      if ( cpcBlindCopy && *cpcBlindCopy )
      {
         ...
         {
            zstrcat( pchCommand, "&bcc=" );
            zstrcat( pchCommand, cpcBlindCopy );
         }
      }

      if ( cpcBody && *cpcBody )
      {
         zstrcat( pchCommand, "&body=" );
         lLth = zstrlen( pchCommand );
         TranslateEmail( pchCommand + lLth, cpcBody );
      // zstrcat( pchCommand, cpcBody );
      }
   ...
   // WriteFile( (HANDLE) hFileTo, pchCommand, lLth, &ulRC, 0 );
   // SysCloseFile( g_vRegister, hFileTo, 0 );

      if ( cpcAttachment && *cpcAttachment )
      {
         zstrcat( pchCommand, "&attach=" );
         zstrcat( pchCommand, cpcAttachment );
      }

      // 0,"Open","mailto:user@email.com?subject=hello there&body=any text",
      // "","",SW_SHOWNORMAL);
      ulRC = (zULONG) ShellExecute( 0, "Open", pchCommand,
                                    "", "", SW_SHOWNORMAL );
      if ( ulRC > 32 )
      {
