...
void
HyphenateHD_Text( zCPCHAR cpcText, zPCHAR pchReturn, zUSHORT uIdx )
{
   zBOOL bGoodBreak = FALSE;
   while ( uIdx > 5 && uIdx > (zstrlen( cpcText ) / 2) )  // purist's goto
   {
      zPCHAR  pchTemp;
      zPCHAR  pch;
      zUSHORT k;

      zstrcpy( pchReturn, cpcText );
//    TraceLineS( "HyphenateCheck ", cpcText );
      ...
      {
         if ( pchReturn[ k ] == '_' ||
              (isupper( pchReturn[ k ] ) && islower( pchReturn[ k - 1 ] )) )
         {
            if ( k <= uIdx )
            {
               bGoodBreak = TRUE;
      ...
      {
         if ( pch < pchReturn + uIdx )
         {
            k = pchReturn + uIdx - pch;
            uIdx -= k;
         }
      }

      break;  // get out of purist's goto
   }

   if ( bGoodBreak )
   {
      uIdx++;
      zstrcpy( pchReturn + 1, cpcText );
      zstrncpy( pchReturn, pchReturn + 1, uIdx );
   }
   else
   {
      zstrcpy( pchReturn + 2, cpcText );
      zstrncpy( pchReturn, pchReturn + 2, uIdx );
      pchReturn[ uIdx++ ] = '-';
   }
...
ZHD_Node::DrawNode( )
{
   ...
   if ( !m_pHD_Diagram->PositionVisible( m_lPosIdx,
        m_lPosIdx + (tzHDNODEX - 1) + ((tzHDNODEY - 1) * tzHDDIAGRAMX) ) )
   {
      return( 0 );
   }
   ...
   if ( pFont->CreateFont( (tzHDFONTHEIGHT * 5) / 4, 0,
                           ...
                           PROOF_QUALITY, tzHDFONTFF, tzHDFONTNAME ) )
   {
   ...
   if ( m_bHidden )
   {
      ...
      m_pHD_Diagram->m_hDC->Ellipse( rect.left, rect.top,
                                     rect.right, rect.bottom );
      if ( bSpecialSelect )
         m_pHD_Diagram->m_hDC->Ellipse( rect.left + 2, rect.top + 2,
   ...
   else
   {
      ...
      m_pHD_Diagram->m_hDC->RoundRect( rect.left, rect.top,
                                       rect.right, rect.bottom,
                                       tzHDARCSIZE, tzHDARCSIZE );
      if ( bSpecialSelect )
         m_pHD_Diagram->m_hDC->RoundRect( rect.left + 2, rect.top + 2,
                                          rect.right - 2, rect.bottom - 2,
                                          tzHDARCSIZE, tzHDARCSIZE );
      // Now that the rectangle is drawn, see if we need to draw the
      // attributive or associative lines in the entity
      if ( m_chType == 'A' )
      {
...
//       rect.bottom -= tzHDCELLSIZE;
         ...
         if ( Size.cx > (rect.right - rect.left) )
         {
            ...
            if ( nIdx > 0 )
            {
               HyphenateHD_Text( m_csText, szText, nIdx );
            }

            m_pHD_Diagram->m_hDC->DrawText( szText, -1, rect,
   ...
   //                             DT_SINGLELINE | DT_LEFT | DT_VCENTER );
         ...
         {
   //       rect.top += ((tzHDCELLSIZE * 3) / 4);
            m_pHD_Diagram->m_hDC->DrawText( m_csText, -1, rect,
           ...
           m_csOutsideText5.IsEmpty( ) == FALSE )
      {
         ...
         mDeleteInit( pFont );
         pFont = new CFont( );
         if ( pFont->CreateFont( tzHDCELLSIZE - 1, 0,
                                 0, 0, 0, 0, 0, 0, ANSI_CHARSET,
                                 OUT_TT_ONLY_PRECIS, CLIP_DEFAULT_PRECIS,
                                 PROOF_QUALITY, tzHDFONTFF, tzHDFONTNAME ) )
         {
         ...
         if ( m_csTitle.IsEmpty( ) == FALSE )
         {
            ...
            rect.top    = (PosY( ) - PosY( tzHDDISPLAYPOS )) * tzHDCELLSIZE;
            rect.bottom = rect.top  + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csTitle, -1, rect,
                                    DT_SINGLELINE | DT_CENTER | DT_VCENTER );
         }
         ...
         if ( m_csDIL.IsEmpty( ) == FALSE )
         {
            ...
            rect.bottom = rect.top + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csDIL, -1, rect,
                                    DT_SINGLELINE | DT_CENTER | DT_VCENTER );
         }
         ...
         if ( m_csOutsideText1.IsEmpty( ) == FALSE )
         {
            ...
            rect.top    = (PosY( ) - PosY( tzHDDISPLAYPOS )) * tzHDCELLSIZE;
            rect.bottom = rect.top  + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csOutsideText1,
                                            -1, rect,
         ...
         if ( m_csOutsideText2.IsEmpty( ) == FALSE )
         {
            ...
            rect.top    += tzHDCELLSIZE;
            rect.bottom = rect.top  + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csOutsideText2,
                                            -1, rect,
                                            DT_SINGLELINE | DT_VCENTER );
         }
         ...
         if ( m_csOutsideText3.IsEmpty( ) == FALSE )
         {
            ...
            rect.bottom = rect.top  + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csOutsideText3,
                                            -1, rect,
                                            DT_SINGLELINE | DT_VCENTER );
         }
         ...
         if ( m_csOutsideText4.IsEmpty( ) == FALSE )
         {
            ...
            rect.bottom = rect.top  + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csOutsideText4,
                                            -1, rect,
                                            DT_SINGLELINE | DT_VCENTER );
         }
         ...
         if ( m_csOutsideText5.IsEmpty( ) == FALSE )
         {
            ...
            rect.bottom = rect.top  + tzHDCELLSIZE;
            m_pHD_Diagram->m_hDC->DrawText( m_csOutsideText5, -1, rect,
                                            DT_SINGLELINE | DT_VCENTER );
         }
      }
      ...
      if ( m_pHD_Diagram->m_bCollapsable )
      {
         ...
         if ( m_bCollapsed )
         {
            ...
            m_pHD_Diagram->m_hDC->DrawText( "+", -1, rect,
                                   DT_SINGLELINE | DT_CENTER | DT_VCENTER );
         }
//       else
//       {
...
//       }
   ...
   // Now that the node has been drawn, draw the line to the parent
   // node if the node exists
   DrawNodeLine( );
...
ZHD_Node::DrawNodeLine( zBOOL bErase )
{
   ...
   if ( m_pParentNode )
   {
      ...
      rect.left = (m_pParentNode->PosX( ) -
                      m_pParentNode->PosX( tzHDDISPLAYPOS )) * tzHDCELLSIZE;
      rect.top  = (m_pParentNode->PosY( ) -
                      m_pParentNode->PosY( tzHDDISPLAYPOS )) * tzHDCELLSIZE;
      rect.right  = rect.left + (tzHDNODEX * tzHDCELLSIZE) - 1;
      rect.bottom = rect.top  + (tzHDNODEY * tzHDCELLSIZE) - 1;
      if ( m_pHD_Diagram->m_bOrientation == 0 )
      {
...
ZHD_Node::CenterNode( )
{
   ...
   m_pHD_Diagram->CalculateDisplaySize( lDisplaySizeX, lDisplaySizeY,
                                        rect, m_pHD_Diagram->m_lScale );
