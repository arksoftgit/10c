   ...
   if ( m_bMultiCol )
   {
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( CheckExistenceOfEntity( vDialog, szlCtrlMap ) > zCURSOR_UNCHANGED )
         {
            ...
            zmemset( CtrlMapDef.Tag, 0xFE, sizeof( CtrlMapDef ) );
         // CtrlMapDef.CheckByte = 0xFE;
#endif
            GetStructFromEntityAttrs( /*(zPCHAR)*/ CtrlMapDef.Tag,
                                      vDialog, szlCtrlMap );
#ifdef DEBUG_LODSTRUCT
            if ( CtrlMapDef.CheckByte != 0xFE )
               ::MessageBox( 0, "CtrlMapDef Structure Length Error",
                             "GetStructFromEntityAttrs", MB_OK );
         // zPCHAR pchCheckByte = (zPCHAR) &(CtrlMapDef.CheckByte);
         // pchCheckByte--;
         // if ( *pchCheckByte == (char) 0xFE )
         //    TraceLineS( "zdrmapac CtrlMapDef CheckByte error??", "" );
#endif
            ...
            if ( (m_ulSubtype & zCOMBOBOX_POSITION_ONLY) == 0 &&
                 zstrcmp( *m_pzsListMapVName, CtrlMapDef.VN ) != 0 )
            {
               bError = TRUE;
               break;
            }
            ...
            if ( zstrcmp( CtrlMapDef.AN, *m_pzsAName ) == 0 &&
                 zstrcmp( CtrlMapDef.EN, *m_pzsEName ) == 0 )
            {
...
ZComboBox::SetColumnWidth( zSHORT nColumnWidth0, ... )
{
   ...
   if ( (GetWindowLong( m_hWnd, GWL_STYLE ) & 0x3) == CBS_SIMPLE )
   {
      ...
      SetWindowPos( 0, 0, 0, lTotalColWidth,
      ...
      else
      {
         ...
         else
         {
            m_pcsColItemList[ nIdx ].
                InsertBefore( m_pcsColItemList[ nIdx ].FindIndex( lItemIdx ),
                              cpcString );
         }
      }
   }
...
// m_pcsColItemList = new CStringList[ m_nTotalCols ];
   ...
   {
      zSHORT nComboBoxWidth = GetDroppedWidth( ) -
                       LEFTMARGIN_OF_DROPDOWNLIST - SPACE_BETWEEN_COLUMN - 1;

      m_nTotalColWidth = LEFTMARGIN_OF_DROPDOWNLIST +
                                                 RIGHTMARGIN_OF_DROPDOWNLIST;
   ...
   {
      m_pMCCEdit = new ZMCComboEdit( );
      m_pMCCEdit->SubclassWindow( (GetWindow( GW_CHILD )->
                                         GetWindow( GW_HWNDNEXT ))->m_hWnd );
   }
...
// SetRedraw( FALSE );  // tried to do this ... repainting occurred improperly
   ...
   if ( (m_ulSubtype & zCOMBOBOX_POSITION_ONLY) || m_bMultiCol )
   {
      ...
      if ( m_pzsListMapVName &&
           GetViewByName( &vApp, *m_pzsListMapVName,
                          m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
      {
         ...
         if ( m_pzsListMapContext == 0 || m_pzsListMapContext->IsEmpty( ) )
         {
            ...
            FindDefaultContext( &cpcContext, vApp,
                                *pzsEntity, *m_pzsListMapAName );
            if ( cpcContext )
            {
               mDeleteInit( m_pzsListMapContext );
         ...
         else
         {
            ...
            // happen!
            sprintf( szMap, "ZComboBox::BuildListFromOI unable to locate "
                            "attribute %s for entity ",
                     *m_pzsListMapAName );
              ...
              (m_ulSubtype & zCOMBOBOX_SORT) )
         {
            ...
            zstrcpy( szAttribute, *m_pzsListMapAName );
            zstrcat( szAttribute, " A" );
      ...
      )
   {
      ...
      // if ( m_pzsContext->IsEmpty( ) )  always do it to determine "required"
         {
            ...
            bRequired = FindDefaultContext( &cpcContext, vApp,
      ...
      // if ( nMinCardinality == 0 )
         ...
         {
            nRC = GetFirstTableEntryForAttribute( szMap, vApp,
                                                  *m_pzsEName, *m_pzsAName,
                                                  *m_pzsContext, &pvCursor );
            bFirstValue = TRUE;
         }
         ...
         while ( nRC == 0 )
         {
            ...
            if ( bFirstValue )
            {
               nRC = GetNextTableEntryForAttribute( szMap, vApp,
                                                    *m_pzsEName, *m_pzsAName,
                                                    *m_pzsContext, &pvCursor );
            }
            else
...
ZComboBox::SetData( zCPCHAR cpcSelectItem, zCPCHAR cpcListData )
{
   m_ulSubtype |= zCOMBOBOX_EXTERNAL_DATA;
   mDeleteInitA( m_pchSelectItem );
   m_pchSelectItem = new char[ zstrlen( cpcSelectItem ) + 1 ];
   zstrcpy( m_pchSelectItem, cpcSelectItem );
   m_pchListData = new char[ zstrlen( cpcListData ) + 1 ];
   zstrcpy( m_pchListData, cpcListData );
   mDeleteInit( m_pzsVName );
   mDeleteInit( m_pzsEName );
   mDeleteInit( m_pzsAName );
...
// zVIEW   vApp = 0;
   ...
   CreateViewFromViewForTask( &vAppList, vAppBrowseOI,
                              m_pZSubtask->m_vDialog );

   if ( cpcBrowseContext == 0 || cpcBrowseContext[ 0 ] == 0 )
   {
      FindDefaultContext( &cpcBrowseContext, vAppList,
                          cpcBrowseEName, cpcBrowseAName );
      // TraceLineS( "ZComboBox default context ", cpcContext );
   }
   ...
   else
   {
      // Borrow szMap to display the message ... this should not
      // happen!
      sprintf( szMap, "ZComboBox::SetDataFromOI unable to locate "
                      "attribute %s for entity ",
               cpcBrowseAName );
      ...
      // if ( m_pzsContext->IsEmpty( ) )  always do it to determine "required"
         {
            ...
            bRequired = FindDefaultContext( &cpcDfltContext, vAppOI,
                                            cpcEName, cpcAName );
         ...
         {
            nRC = GetFirstTableEntryForAttribute( szMap, vAppOI,
                                                  cpcEName, cpcAName,
                                                  cpcContext, &pvCursor );
            bFirstValue = TRUE;
         }
         ...
         while ( nRC == 0 )
         {
            ...
            if ( bFirstValue )
            {
               nRC = GetNextTableEntryForAttribute( szMap, vAppOI,
                                                    cpcEName, cpcAName,
                                                    cpcContext, &pvCursor );
            }
            else
...
zSHORT
ZComboBox::GetSelectedString( zPCHAR pchSelectedString )
{
   ...
   {
      if ( m_pchSelectItem )
         zstrcpy( pchSelectedString, m_pchSelectItem );
      else
         *pchSelectedString = 0;
   ...
   if ( m_pzsVName && m_pzsEName && m_pzsAName )
   {
      ...
      if ( m_ulSubtype & zCOMBOBOX_POSITION_ONLY )
      {
         ...
         if ( m_nDisplaySelSet )
         {
            ...
            if ( GetViewByName( &vApp, *m_pzsListMapVName,
                               m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
            {
         ...
         {
         // uMapState = GetMappingAutoGray( &vApp,
         //                                 *m_pzsListMapVName,
         //                                 *m_pzsListMapEName, 0 );
#if 1
            uMapState = 0;
            ...
            if ( m_pzsListMapVName &&
                 ...
                 CheckExistenceOfEntity( vApp,
                                         *m_pzsListMapEName ) == zCURSOR_SET &&
                 GetViewByName( &vApp, *m_pzsVName,
                                m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
            {
               if ( MiGetUpdateForViewEntityAttr( vApp, *m_pzsEName,
                                                  *m_pzsAName, 0 ) > 0 )
               {
            ...
            {
               uMapState = GetMappingAutoGray( &vApp, *m_pzsVName,
                                               *m_pzsEName, *m_pzsAName );
            }
#endif
         }
         else
         {
            ...
            {
               TraceLineS( "zdctlcmb - Combo List view name is null ",
                           *m_pzsTag );
               vApp = 0;
            }
            else
            ...
            //                              *m_pzsListMapEName ) != zCURSOR_NULL )   //??? SCOPE
               ...
               if ( GetViewByName( &vApp, *m_pzsVName,
                                   ...
                                   zLEVEL_ANY ) > 0 )
               {
                  if ( MiGetUpdateForViewEntityAttr( vApp, *m_pzsEName, 0,
                                                     zUPD_INCLUDE ) > 0 )
                  {
...
ZComboBox::MapToOI( zLONG lFlag )
{
   ...
   if ( m_ulSubtype & zCOMBOBOX_POSITION_ONLY )
   {
      ...
      if ( m_pzsListMapVName &&
           GetViewByName( &vMap, *m_pzsListMapVName,
                          m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
      {
         CString *pzsEntity;
         zSHORT  nPrevSelSet;
         ...
         if ( szMap[ 0 ] == 0 ||
              SetEntityCursor( vMap, *pzsEntity,
                               *m_pzsListMapAName,
                               ...
                               szMap, 0, 0, 0,
                               cpcBrowseScope, cpcMapContext ) >= 0 ||
               ...
               (m_ulSubtype & zCOMBOBOX_INCLUDE) == 0) )
         {
            if ( m_ulSubtype & zCOMBOBOX_SELECT )
            {
               ...
               SetAllSelectStatesForEntity( vMap, *m_pzsListMapEName,
                                            FALSE, cpcBrowseScope );
               if ( szMap[ 0 ] )
            ...
            if ( vApp )
            {
               if ( m_ulSubtype & zCOMBOBOX_INCLUDE )
               {
                  if ( m_bWasSelected )
                  {
                     ...
                     if ( szMap[ 0 ] )
                     {
                        nRC = IncludeSubobjectFromSubobject( vApp, *m_pzsEName,
                                                             vMap, *m_pzsListMapEName,
                                                             zPOS_AFTER );
                     }
                           ...
                           zLONG lColumn )
{
   ...
   {
      AddColumn( (zSHORT) lColumn, -1, 0, cpcViewName, cpcEntity,
                 cpcAttrib, cpcContext );
   }
                           ...
                           zLONG lColumn )
{
   ...
   {
      if ( m_pzsListMapVName )
         zstrcpy( pchViewName, *m_pzsListMapVName );
      else
         pchViewName[ 0 ] = 0;

      if ( m_pzsListMapEName )
         zstrcpy( pchEntity, *m_pzsListMapEName );
      else
         pchEntity[ 0 ] = 0;

      if ( m_pzsListMapAName )
         zstrcpy( pchAttrib, *m_pzsListMapAName );
      else
         pchAttrib[ 0 ] = 0;

      if ( m_pzsListMapContext )
         zstrcpy( pchContext, *m_pzsListMapContext );
      else
         pchContext[ 0 ] = 0;
   }
   else
   if ( lColumn == -2 )  // remap the browse part of the combobox
   {
      ...
      if ( m_pzsListBrowseEName )
         zstrcpy( pchEntity, *m_pzsListBrowseEName );
      else
         pchEntity[ 0 ] = 0;

      if ( m_pzsListBrowseScopeEName )
         zstrcpy( pchAttrib, *m_pzsListBrowseScopeEName );
      else
         pchAttrib[ 0 ] = 0;
   ...
   {
   // GetColumn( (zSHORT) lColumn, -1, 0, pchViewName, pchEntity,
   //            pchAttrib, pchContext );
   }
...
ZComboBox::SelectStringByKeyAttributes( zVIEW vXRA )
{
   ...
   if ( m_pzsVName == 0 || m_pzsListMapVName == 0 ||
        m_pzsEName == 0 || m_pzsListMapEName == 0 ||
        GetViewByName( &vApp, *m_pzsVName, m_pZSubtask->m_vDialog,
                       zLEVEL_ANY ) < 0 ||
        GetViewByName( &vListApp, *m_pzsListMapVName, m_pZSubtask->m_vDialog,
                       zLEVEL_ANY ) < 0 ||
        CheckExistenceOfEntity( vApp, *m_pzsEName ) != zCURSOR_SET ||
   ...
   zVIEW  vListSearch;
   CreateViewFromViewForTask( &vListSearch, vListApp,
                              m_pZSubtask->m_vDialog );
   ...
   if ( bFound )
   {
      ...
      {
         SetZCtrlText( this, szMap );
         SetAttributeFromInteger( vXRA, "ComboBox",
                                  "SelectedIndex", lNewSelIdx );
         nRC = 0;
      }
   }
...
// Disable Delta Data implementation.
   ...
   // zULONG   ulState;
      ...
      while ( pos )
      {
         ...
         {
            SetAttributeFromInteger( vXRA, cpcCMBDataRowEntity,
                                     "Tag", pDataRow->GetKeyOrig( ) );
            SetAttributeFromInteger( vXRA, cpcCMBDataRowEntity,
                                     "NewTag", pDataRow->GetKey( ) );
         }
         else
            SetAttributeFromInteger( vXRA, cpcCMBDataRowEntity,
                                     "Tag", pDataRow->GetKey( ) );
         ...
         {
            SetAttributeFromString( vXRA, cpcCMBDataRowEntity,
                                    "Deleted", "Y" );
         }
         else
         {
      ...
      GetIntegerFromAttribute( &lIndex, vXRA, "ComboBox",
                               "SelectedIndex" );
      if ( lIndex < 0 ||
           CompareAttributeToString( vXRA, "ComboBox",
                                     "SelectedIndex", "" ) == 0 )
      {
         if ( lSelIdx >= 0 )
         {
        ...
        (pXRA->m_vXRA || pXRA->m_vXRA_Delta) )
   {
      ...
      while ( TRUE )
      {
         ...
         if ( PositionOnRemoteCtrl( this, vXRA, FALSE ) )
         {
            ...
            if ( GetIntegerFromAttribute( &lIndex, vXRA, "ComboBox",
                                          "SelectedIndex" ) >= 0 )
            {
                              ...
                              zSHORT nMaxLth, zSHORT nCol )
{
   ...
   if ( (lIdx = CComboBox::GetCurSel( )) != CB_ERR &&
        nCol < m_nTotalCols )
   {
                                 ...
                                 zSHORT nMaxLth )
{
   ...
   if ( nCol == 0 )
   {
      GetViewByName( &v, pCol->m_pchViewName,
                     m_pZSubtask->m_vDialog, zLEVEL_ANY );
      if ( v == 0 )
         v = m_vAppList;
   }
   ...
   TranslateValue( pchText, TRUE, nCol );
...
ZComboBox::OnPaint( )
{
   ...
   if ( m_hWndChild && GetDroppedState( ) == FALSE )
   {
      ...
      if ( dw != CB_ERR )
      {
         ...
         {
            TraceLine( "ZComboBox::OnPaint Tag: %s  Start: %d   End: %d",
                       *m_pzsTag, wStart, wEnd );
         }
...
ZComboBox::DrawItem( LPDRAWITEMSTRUCT pdis )
{
   ...
   zBOOL    bHighLightText = (pdis->itemAction & ODA_FOCUS) ||
   ...
   pDC->SetTextColor( m_ulCtrlErr ? zRGB( 255, 0, 0 ) :
                  ...
                  (bHighLightText ? COLOR_HIGHLIGHTTEXT : COLOR_WINDOWTEXT) :
                                                          COLOR_WINDOWTEXT) );
   pDC->SetBkColor( GetSysColor( mIs_hWndEnabled( m_hWnd ) ?
                          (bHighLightText ? COLOR_HIGHLIGHT : COLOR_WINDOW) :
                                                             COLOR_3DFACE ) );
   ...
   // if ( pWndDropDown && pWndDropDown != this ) // draw items in drop down list
      if ( pWndDropDown && pdis->itemState < 0x1000 )
      {
              ...
              (GetWindowLong( m_hWnd, GWL_STYLE ) & 0x3) != CBS_SIMPLE )
         {
            ...
            {
               rectWnd.OffsetRect( GetSystemMetrics( SM_CXSCREEN ) -
                                                          rectWnd.right, 0 );
         ...
         for ( zSHORT nColIdx = 1; nColIdx <= m_nTotalCols; nColIdx++ )
         {
            ...
            if ( pCol->m_nColWidth )
            {
                    ...
                    (nColIdx == 2 && FindColumn( 1 )->m_nColWidth != 0) )
               {
                  ...
                  if ( bHighLightText == FALSE )
                  {
                     ...
                     pDC->MoveTo( rectText.left - SPACE_BETWEEN_COLUMN / 2,
                                  rectText.top );
                     pDC->LineTo( rectText.left - SPACE_BETWEEN_COLUMN / 2,
                                  rectText.bottom );
                  }
               }
               ...
               if ( pWndDropDown &&
                    ...
                    (GetWindowLong( pWndDropDown->m_hWnd,
            ...
            //    TraceLineS( "", "" );
               ...
               FormatTextAtPosition( szText, pdis->itemID,
                                     nColIdx, sizeof( szText ) );
            // zsprintf( szText, "%s.%d", "BOX", pdis->itemID );
            // pdis->itemID /= k;

               pDC->DrawText( szText, -1, rectText,
                              pCol->m_lAlignStyle | DT_SINGLELINE |
      ...
      else              // draw item in edit box control/static text control
      {
         ...
         {
            FormatTextAtPosition( szText, pdis->itemID,
                                  m_nBoundCol, sizeof( szText ) );
         // zsprintf( szText, "%s.%d", "EDIT BOX", pdis->itemID );
         // if ( pdis->itemID == 3 )
         //    pdis->itemID /= k;
...
//                           DT_NOPREFIX | DT_NOCLIP | DT_VCENTER );
            FormatTextAtPosition( szText, pdis->itemID,
                                  m_nShowColumn, sizeof( szText ) );
         // zsprintf( szText, "%s.%d", "EDIT", pdis->itemID );
         // if ( pdis->itemID == 3 )
         //    pdis->itemID /= k;
                      ...
                      zCPCHAR cpcAttribute, zCPCHAR cpcContext )
{
   ...
   {
      pCol = new ZComboColumn( nCol, nColWidth, lAlignStyle,
                               cpcViewName, cpcEntity,
                               cpcAttribute, cpcContext );
      if ( m_pHeadCol )
      {
                            ...
                            zCPCHAR cpcContext )
{
   ...
   m_pchViewName = new char[ zstrlen( cpcViewName ) + 1 ];
   zstrcpy( m_pchViewName, cpcViewName );
   m_pchEntity = new char[ zstrlen( cpcEntity ) + 1 ];
   zstrcpy( m_pchEntity, cpcEntity );
   m_pchAttribute = new char[ zstrlen( cpcAttribute ) + 1 ];
   zstrcpy( m_pchAttribute, cpcAttribute );
   if ( cpcContext && *cpcContext )
   {
      m_pchContext = new char[ zstrlen( cpcContext ) + 1 ];
      zstrcpy( m_pchContext, cpcContext );
   }
}
                        ...
                        zSHORT  nSelectSet )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for CB_SetDisplaySelectSet ",
                  cpcCtrlTag );
   }
                       ...
                       zSHORT  nSelectSet )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for CB_SetSelectSelectSet ",
                  cpcCtrlTag );
   }
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pCombo )
      {
         ...
         if ( lFlag & 1 )
         {
            ...
            zstrcpy( pchTableData, cpcListData );  // borrow pchTableData
                 ...
                 (pchDomain = zstrchr( pchAttribute + 1, ';' )) != 0 )
            {
               ...
               *pchDomain = 0;
               ...
               if ( pchContext )
               {
                  *pchContext = 0;
                  pchContext++;
                  zstrcpy( szContext, pchContext );
               }
               else
                  szContext[ 0 ] = 0;

               zstrcpy( szEntity, pchEntity );
               zstrcpy( szAttribute, pchAttribute );
               zstrcpy( szDomain, pchDomain );
               ...
               if ( lpDomain )
               {
                  ...
                  zstrcpy( ve.szName, szEntity );
                  zstrcpy( va.szName, szAttribute );
      ...
      TraceLineS( "drvr - Invalid control type for CB_SetData ",
                  cpcCtrlTag );
   }
                      ...
                      zPCHAR  pchSelectedString )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pCombo )
      {
         return( pCombo->GetSelectedString( pchSelectedString ) );
      }

      TraceLineS( "drvr - Invalid control type for CB_GetSelectedString ",
                  cpcCtrlTag );
   }
                 ...
                 zLONG   lFlags )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for CB_SelectString ",
                  cpcCtrlTag );
   }
...
ZMCComboEdit::WindowProc( UINT uMsg, WPARAM wParam, LPARAM lParam )
{
   switch ( uMsg )
   {
      case WM_GETTEXTLENGTH:
      {
         ...
         if ( GetParent( )->SendMessage( WM_APP + WM_GETTEXT,
                                         255, (LPARAM) szString ) != 0 )
         {
      ...
      case WM_GETTEXT:
         ...
         return( GetParent( )->SendMessage( WM_APP + WM_GETTEXT,
                                            wParam, lParam ) ); // replace it
         break;

      case WM_SETTEXT:     // replace text, then set text
         return( CEdit::WindowProc( uMsg, wParam,
                     (LPARAM) GetParent( )->SendMessage( WM_APP + WM_SETTEXT,
                                                         wParam, lParam ) ) );
         break;
...
ZMCComboEdit::OnPaint( )
{
   ...
   dc.DrawText( szString, &(dc.m_ps.rcPaint),
                DT_SINGLELINE | DT_NOPREFIX |
                  DT_NOCLIP | DT_VCENTER | DT_BOTTOM );
