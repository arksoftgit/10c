                                    ...
                                    zLONG       lFlags )
{
   ...
   if ( pchForwardText && *pchForwardText )
   {
      ...
      if ( pchForwardText[ m_uLPOffsetF ] != '(' )
         m_uLPOffsetF = zstrlen( pchForwardText );
   }
   ...
   if ( pchReverseText )
   {
      ...
      m_uLPOffsetR = zstrlen( pchReverseText );
...
//          if ( lMinImpact < tzERLINEABUTSENTITY )      DKS 3/5/96
   ...
   //xzstrcpy( szMessage, m_pSrcEntity->m_csText );
   //xzstrcat( szMessage, "/" );
   //xzstrcat( szMessage, m_pTgtEntity->m_csText );
   //xzstrcat( szMessage, " - Error calculating relationship!" );
                               ...
                               zLONG   lPortY )
{
   ...
   zBOOL  bRecursive;
   zSHORT nDirection;
   zCHAR zHUGE *pchGrid = m_pER_Diagram->m_pchGrid;
...
#ifdef DEBUG_ALL
   zCHAR szMsg[ 256 ];
   zsprintf( szMsg, "(TzCtlERL) Line Analysis m_csTextF: %s"
                    "  SourcePort: %d"
                    "  TargetPort: %d"
                    "  Axis: %d"
                    "  Impact Points: %ld ",
