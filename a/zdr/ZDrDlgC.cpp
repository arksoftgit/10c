   ...
   *pchReturnTag = 0;
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      if ( pZSubtask->m_pzsLastCtrlTag )
         zstrcpy( pchReturnTag, *(pZSubtask->m_pzsLastCtrlTag) );
             ...
             zCPCHAR cpcCtrlTag )
{
        ...
        pZSubtask->m_pZMIXCtrl )
   {
      ...
      {
         zPCHAR pchTags = new char[ zstrlen( cpcCtrlTag ) + 1 ];
         zstrcpy( pchTags, cpcCtrlTag );
                            ...
                            zBOOL   bCreateEntity )
{
   ...
   {
      bRC = PositionOnRemoteCtrlParent( pzma->m_pzmaComposite,
                                        vXRA, bCreateEntity );
      if ( bRC )
   ...
   if ( bRC )
   {
      if ( CheckExistenceOfEntity( vXRA, "Ctrl" ) >= zCURSOR_SET &&
           SetCursorFirstEntityByString( vXRA, "Ctrl", "Tag",
                                         *(pzma->m_pzsTag), 0 ) == 0 )
      {
         if ( CheckExistenceOfEntity( vXRA,
                                      pzma->m_csERemote ) < zCURSOR_SET )
         {
                         ...
                         zCPCHAR cpcValue )
{
 ...
 TraceLineS( "In SetRemoteZCtrlAttribute ", *(pzma->m_pzsTag) );
   ...
   if ( pZSocketTask &&
        (pXRA = pZSocketTask->
            m_pCacheWndList->Find( *(pzma->m_pZSubtask->m_pzsRemoteWndTag),
                                    pzma->m_pZSubtask,
                                    "SetRemoteZCtrlAttribute" )) != 0 &&
        pXRA->m_vXRA )
   {
      zVIEW  vXRA;
      zSHORT nPass;
...
#ifdef DEBUG_ALL
      zCHAR  szMsg[ 256 ];
      zsprintf( szMsg, "SetRemoteZCtrlAttribute Tag: %s  Entity: %s  "
                       "Attribute: %s  Value: %s",
      ...
      while ( nPass <= 1 )
      {
         ...
         if ( CheckExistenceOfEntity( vXRA, "Wnd" ) < zCURSOR_SET ||
              SetCursorFirstEntityByString( vXRA, "Wnd", "Tag",
                                      *(pzma->m_pZSubtask->m_pzsRemoteWndTag),
                                      0 ) < zCURSOR_SET )
         {
         ...
         //                            cpcAttribute, *(pzma->m_pzsText) );
         // }

            SetAttributeFromString( vXRA, cpcEntity, cpcAttribute, cpcValue );

            // If the Control Text is null, we have to set the Update flag
            // for XML generation.
         // if ( cpcValue[ 0 ] == 0 )
...
// *pchValue = 0;  permit the caller to set the default return value!
   ...
   if ( pzma->m_pZSubtask->m_pZSocketTask &&
        (pXRA = pzma->m_pZSubtask->m_pZSocketTask->
            m_pCacheWndList->Find( *(pzma->m_pZSubtask->m_pzsRemoteWndTag),
                                   pzma->m_pZSubtask,
                                   "GetRemoteZCtrlAttribute" )) != 0 &&
                        ...
                        zCPCHAR cpcValue )
{
   ...
   {
      return( SetRemoteZCtrlAttribute( pzma, cpcEntity,
              ...
              zUSHORT uMaxTextLth )
{
   ...
   pchText[ 0 ] = 0;
#ifdef zREMOTE_SERVER
   if ( GetRemoteZCtrlAttribute( pzma, "Ctrl", "Text",
                                 pchText, uMaxTextLth ) >= 0 )
   {
      if ( pzma->m_pzsText == 0 ||
   ...
   if ( pzma->m_pCtrl && mIs_hWnd( pzma->m_pCtrl->m_hWnd ) )
   {
      ...
      zstrncat( pchText, pch, uMaxTextLth );       // controls force this
      delete [] pch;                               // gimmick.
      if ( pzma->m_pzsText == 0 ||
           zstrcmp( pchText, *(pzma->m_pzsText) ) != 0 )
      {
   ...
   if ( pzma->m_pzsText )
   {
      zstrncat( pchText, *(pzma->m_pzsText), uMaxTextLth );
      return( 1 );
   }
#endif
              ...
              zCPCHAR cpcText )
{
   ...
   if ( pzma->m_pzsText == 0 || zstrcmp( cpcText, *(pzma->m_pzsText) ) != 0 )
   {
      mDeleteInit( pzma->m_pzsText );
                    ...
                    zUSHORT uLth )
{
        ...
        GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      zstrcpy( szValue, pchValue );
      *pchValue = 0;
      pzma->TranslateValue( szValue, (zBOOL) bToDisplay, nColumn );
      zstrncat( pchValue, szValue, uLth );
      return( 0 );
   }
                 ...
                 zSHORT  bDlgUnits )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( cpcCtrlTag )
      {
         ...
         if ( mIs_hWnd( pzma->m_pCtrl->m_hWnd ) )
         {
            pzma->m_pCtrl->SetWindowPos( 0, lX, lY, 0, 0,
      ...
      else
      {
         ...
         zltoa( lX, szPos );
         SetRemoteWndAttribute( pZSubtask->m_vDialog,
                                "PSDLG_X", szPos, FALSE );
         zltoa( lY, szPos );
         SetRemoteWndAttribute( pZSubtask->m_vDialog,
                                "PSDLG_Y", szPos, FALSE );
         ...
         {
            pZSubtask->m_pZFWnd->SetWindowPos( 0, lX, lY, 0, 0,
                                               SWP_NOSIZE | SWP_NOZORDER );
         }
#endif
      }
             ...
             zSHORT  bDlgUnits )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( cpcCtrlTag )
      {
         ...
         if ( mIs_hWnd( pzma->m_pCtrl->m_hWnd ) )
         {
            pzma->m_pCtrl->SetWindowPos( 0, 0, 0, lCX, lCY,
      ...
      else
      {
         ...
         zltoa( lCX, szSize );
         SetRemoteWndAttribute( pZSubtask->m_vDialog,
                                "SZDLG_X", szSize, FALSE );
         zltoa( lCY, szSize );
         SetRemoteWndAttribute( pZSubtask->m_vDialog,
                                "SZDLG_Y", szSize, FALSE );
#else
         if ( mIs_hWnd( pZSubtask->m_pZFWnd->m_hWnd ) )
         {
            pZSubtask->m_pZFWnd->SetWindowPos( 0, 0, 0, lCX, lCY,
                                               SWP_NOMOVE | SWP_NOZORDER );
         }
#endif
      }
                 ...
                 zCPCHAR cpcStringProperty )
{
   ...
   if ( cpcCtrlTag &&
        GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      return( pzma->SetZCtrlProperty( lPropertyType, ulNumericProperty,
                                      cpcStringProperty ) );
   }
                 ...
                 zPCHAR  pchStringProperty )
{
   ...
   if ( cpcCtrlTag &&
        GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      return( pzma->GetZCtrlProperty( lPropertyType, pulNumericProperty,
                                      pchStringProperty ) );
   }
...
//                                (should allocate at least 33 bytes).
      ...
      if ( pZSubtask->m_pZMIXCtrl && pZSubtask->m_pzmaWithFocus )
      {
         ...
         while ( pzma && pzma != pZSubtask->m_pzmaWithFocus )
         {
                 ...
                 (pzma->m_ulMapActFlags & zMAPACT_FOCUS_ENABLED) )
            {
               ...
               if ( pzma->m_pzsTag )
               {
                  zstrcpy( pchReturnCtrlTag, *(pzma->m_pzsTag) );
               }
                  ...
                  zPCHAR pchReturnCtrlTag )
{
   ...
   *pchReturnCtrlTag = 0;
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      {
#ifdef zREMOTE_SERVER
         zstrcpy( pchReturnCtrlTag, *(pZSubtask->m_pzmaWithFocus->m_pzsTag) );
         return( 0 );
#else
         HWND hWndFocus = ::GetFocus( );

         zstrcpy( pchReturnCtrlTag, *(pZSubtask->m_pzmaWithFocus->m_pzsTag) );
         if ( pZSubtask->m_pzmaWithFocus->m_pCtrl &&
              mIs_hWnd( pZSubtask->m_pzmaWithFocus->m_pCtrl->m_hWnd ) &&
              (hWndFocus == pZSubtask->m_pzmaWithFocus->m_pCtrl->m_hWnd ||
