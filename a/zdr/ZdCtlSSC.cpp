...
typedef GENFIELD zFAR *LPGENFIELD;
...
} EDIT3DFORMAT, zFAR *LPEDIT3DFORMAT;


typedef struct tagEditField
{
   zCHAR        lpszString[ MAXFIELD + 1 ];   // The contents of the field
...
zBOOL
Validate( HWND hWnd, zPCHAR lpszString, zLONG lPos );
zLONG OPERATION
...
//
zPCHAR OPERATION
StrRemove( zPCHAR lpszString, zSHORT nStart, zSHORT nEnd )
{
   ...
   if ( lpszString )
   {
      ...
      zstrcpy( lpszString + nStart, lpszString + nEnd + 1 );
   }
...
AdjustLength( HWND hWnd )
{
   ...
   lpField->nChars = zstrlen( lpField->lpszString );
...
LeftOf( LPEDITFIELD lpField, zSHORT nChar )
{
   ...
   for ( nCharIdx = 0, nPos = nStart; nCharIdx < nChar; ++nCharIdx )
      nPos += GetSingleCharWidth( lpField, (BYTE) lpField->lpszString[ nCharIdx ] );
                 ...
                 LPARAM  lParam )
{
   ...
   if ( lParam )
      zmemcpy( &lpField->Edit3DFormat, (LPEDIT3DFORMAT) lParam,
               sizeof( EDIT3DFORMAT ) );

   GetClientRect( hWnd, &rectClient );
   TraceRect( "EditSet3DParams ClientRect", rectClient );
   SendMessage( hWnd, WM_SIZE, 0,
                MAKELONG( rectClient.right, rectClient.bottom ) );
             ...
             zSHORT      nEnd )
{
   ...
   if ( fIsEnabled || GetBkColor( hDC ) != GetSysColor( COLOR_GRAYTEXT ) )
   {
      ...
      DrawText( hDC, lpszText, nCnt, &RectTemp, wFormat );
      ...
      if ( GetFocus( ) == hWnd && lpField->bHilited )
      {
         ...
         DrawText( hDC, &lpszText[ nStart ], nEnd - nStart,
                   &RectTempHi, wFormat );
   ...
   else
   {
      ...
      fnGrayString( hDC, hBrushTemp, 0, (DWORD) lpszText, 0, x, y, 0, 0 );
...
GenInitializeField( HWND hWnd, LPARAM  lParam )
{
   ...
   if ( ((LPCREATESTRUCT) lParam)->lpszName )
      zstrncpy( szGen, (zPCHAR) ((LPCREATESTRUCT)lParam)->lpszName,
                sizeof( szGen ) );
   else
      szGen[ 0 ] = 0;
...
FieldLength( LPEDITFIELD lpField )
{
   ...
   for ( k = 0, nLth = 0; k < lpField->nChars; k++ )
      nLth += GetSingleCharWidth( lpField, (BYTE) lpField->lpszString[ k ] );
...
CaretPosOfCharacter( LPEDITFIELD lpField, zSHORT nChar )
{
   ...
   for ( k = 0, lPos = lStart; k < nChar; k++ )
      lPos += GetSingleCharWidth( lpField, (BYTE) lpField->lpszString[ k ] );

   if ( GetOverstrikeMode( LOCAL ) )
   {
      if ( nChar < lpField->nChars )
         chLast = lpField->lpszString[ nChar ];
      else
         chLast = ' ';
...
GetTextLength( HWND hWnd )
{
   ...
   LPEDITFIELD lpField = LockField( hWnd );
   lTextLth = zstrlen( lpField->lpszString );
// DrUnlockTaskMemory( lpField );
...
FindNearestCharacter( LPEDITFIELD lpField, zSHORT nRawPos, zBOOL bRoundUp )
{
   ...
   {
      nCharWidth = GetSingleCharWidth( lpField,
                                       (BYTE)lpField->lpszString[ nCharIdx ] );
...
EditPaintField( HWND hWnd, HDC hDC, LPEDITFIELD lpField )
{
   ...
   {
      for ( k = 0; k < zstrlen( lpField->lpszString ); k++ )
         szBuffer[ k ] = '*';

      szBuffer[ zstrlen( lpField->lpszString ) ] = 0;
   }
   else
      zstrcpy( szBuffer, lpField->lpszString );
...
Validate( HWND hWnd, zPCHAR lpszString, zLONG lPos )
{
   ...
   return( (zBOOL) SendMessage( hWnd, EM_VALIDATE, lPos, (zLONG) lpszString ) );
}
...
InsertCharacter( HWND hWnd, LPEDITFIELD lpField, zCHAR cChar )
{
   ...
   zCHAR lpszLocal[MAXFIELD];

   if ( !CheckFit( lpField, (BYTE) cChar ) )
      ::MessageBeep( MB_OK );              // No more room in field.
   else
   {
      zstrcpy( lpszLocal, lpField->lpszString );
      StrInsertChar( cChar, lpszLocal, lpField->nCurrentPos );
      lpField->bCharBeingInserted = TRUE;
      if ( Validate( hWnd, lpszLocal, lpField->nCurrentPos ) )
      {
         NotifyParent ( hWnd, EN_UPDATE );
         zstrcpy( lpField->lpszString, lpszLocal );
         lpField->nChars++;
...
ReplaceCharacter( HWND hWnd, LPEDITFIELD lpField, zCHAR cChar )
{
   ...
   zstrcpy( lpszLocal, lpField->lpszString );
   lpszLocal[ lpField->nCurrentPos ] = cChar;
   if ( Validate( hWnd, lpszLocal, lpField->nCurrentPos ) )
   {
      NotifyParent( hWnd, EN_UPDATE );
      zstrcpy( lpField->lpszString, lpszLocal );
...
ReplaceSelectedText( HWND hWnd, LPEDITFIELD lpField, zPCHAR lpNewText )
{
   ...
   if ( lpField->bHilited )
   {
      zstrcpy( lpszLocal, lpNewText );
      nTextLth = zstrlen( lpNewText );

      if ( !SendMessage( hWnd, EM_REPLACETEXT, 0, (zLONG) lpszLocal ) )
      {
         zstrncpy( lpszLocal, lpField->lpszString, lpField->nHiStart + 1 );
         lpszLocal[ lpField->nHiStart + 1 ] = 0;  // force null terminator
         zstrcat( lpszLocal, lpNewText );
         zstrcat( lpszLocal, &lpField->lpszString[ lpField->nHiEnd ] );
      ...
      if ( Validate( hWnd, lpszLocal, nCurrentPos ) )
      {
         NotifyParent( hWnd, EN_UPDATE );
         zstrcpy( lpField->lpszString, lpszLocal );
         lpField->nChars = zstrlen( lpField->lpszString );
...
GetTextFromClipboard( HWND hWnd )
{
   ...
   if ( (hMem = GetClipboardData( CF_TEXT )) != 0 )
   {
      ...
      if ( zstrlen( lpszMem ) )
      {
         if ( (hText = GlobalAlloc( GHND,
                                    (DWORD)( zstrlen( lpszMem ) + 1 ) ) ) != 0 )
         {
            lpszText = (zPCHAR) GlobalLock( hText );
            zstrcpy( lpszText, lpszMem );
            GlobalUnlock( hText );
         }
      }
   ...
   else
   {
      switch ( wKey )
      {
         ...
         case VK_BACK:
            ...
            {
               zstrcpy( lpszLocal, lpField->lpszString );
               zstrcpy( lpszLocal + lpField->nCurrentPos - 1, // delete 1 char
                        lpszLocal + lpField->nCurrentPos );
               if ( Validate( hWnd, lpszLocal, lpField->nCurrentPos ) )
               {
                  NotifyParent( hWnd, EN_UPDATE );
                  zstrcpy( lpField->lpszString, lpszLocal );
         ...
         case VK_DELETE:
            ...
            else
            {
               ...
               {
                  zstrcpy( lpszLocal, lpField->lpszString );
                  zstrcpy( lpszLocal + lpField->nCurrentPos, // delete 1 char
                           lpszLocal + lpField->nCurrentPos + 1 );
                  if ( Validate( hWnd, lpszLocal, lpField->nCurrentPos ) )
                  {
                     NotifyParent( hWnd, EN_UPDATE );
                     zstrcpy( lpField->lpszString, lpszLocal );
...
InsertText( HWND hWnd, LPEDITFIELD lpField, zPCHAR lpNewText )
{
   ...
   zmemset( lpszLocal, 0, sizeof( lpszLocal ) );

   lTextLth = zstrlen( lpNewText );
   lTextLth = mMin( lTextLth, MAXFIELD - 1 - zstrlen( lpField->lpszString ) );

   zmemcpy( lpszLocal, lpField->lpszString, lpField->nCurrentPos );
   zmemcpy( &lpszLocal[ zstrlen( lpszLocal ) ], lpNewText, lTextLth );
   zstrcat( &lpszLocal[ zstrlen( lpszLocal ) ],
            &lpField->lpszString[ lpField->nCurrentPos ] );
...
DeleteSelectedText( HWND hWnd, LPEDITFIELD lpField )
{
   ...
   {
      zstrcpy( lpszLocal, lpField->lpszString );
      StrRemove( lpszLocal, lpField->nHiStart,
                 mMin( zstrlen( lpszLocal ),
                       (zUSHORT) lpField->nHiEnd ) - 1 );
      if ( Validate( hWnd, lpszLocal, lpField->nCurrentPos ) )
      {
         NotifyParent( hWnd, EN_UPDATE );
         zstrcpy( lpField->lpszString, lpszLocal );
...
ClipboardCopy( HWND hWnd )
{
   ...
   if ( lpField->bHilited )
      SendTextToClipboard( hWnd, &lpField->lpszString[lpField->nHiStart],
...
ClipboardCut( HWND hWnd )
{
   ...
   if ( lpField->bHilited )
   {
      if ( SendTextToClipboard( hWnd, &lpField->lpszString[ lpField->nHiStart ],
                                lpField->nHiEnd - lpField->nHiStart ) )
         DeleteSelectedText( hWnd, lpField );
   }
...
SetFieldText( HWND hWnd, LPARAM  lParam )
{
   ...
   zstrncpy( lpField->lpszString, lpszText, MAXFIELD );

   if ( lpField->nMaxChars != -1 )
      lpField->lpszString[lpField->nMaxChars] = '\0';

   if ( lpField->nCase == UPPERCASE )       // Force case?
      zstrupr( lpField->lpszString );
   else
   if ( lpField->nCase == LOWERCASE )
      zstrlwr( lpField->lpszString );
...
GetFieldText( HWND hWnd, WPARAM  wParam, LPARAM  lParam )
{
   ...
   {
      LPEDITFIELD lpField = LockField( hWnd );
      zPCHAR      lpszText = (zPCHAR) lParam;
      zSHORT      nLth = zstrlen( lpField->lpszString );
      zSHORT      nLth2 = nLth + 1;
      ...
      zmemcpy( lpszText, lpField->lpszString, nLth2 );
//    DrUnlockTaskMemory( lpField );
   }
...
//       DrUnlockTaskMemory( lpSS );
      ...
      case CB_SHOWDROPDOWN:
      {
         ...
         if ( GetFocus( ) != hWnd &&
              GetFocus( ) != (HWND) GetWindowLong( hWnd, GWL_HWNDEDIT ) )
         {
            return( 0 );
         }
...
//          TraceLineI( "SHOWDROPDOWN lCurSel: ", lCurSel );
         ...
         // TraceLineS( "kzSSComboBoxWndProc: ", "SetWindowPos" );
            SetWindowPos( hWndCtrl, 0, 0, 0, 0, 0, SWP_NOACTIVATE | SWP_NOMOVE |
...
//                hWndEdit = (HWND) GetWindowLong( hWnd, GWL_HWNDEDIT );
      ...
      case WM_COMMAND:
         if ( HIWORD( lParam ) == LBN_SELCHANGE )
         {
            ...
            hWndCtrl = (HWND) GetWindowLong( hWnd, GWL_LISTBOX );
            if ( hWndCtrl &&
                 (lSel = (zLONG) SendMessage( hWndCtrl, LB_GETCURSEL,
                                              0, 0L )) != LB_ERR )
            {
...
CheckBoxInit( )
{
   ...
   if ( DrAllocTaskMemory( (zCOREMEM) &lpCheckBox,
                           (zLONG) sizeof( CHECKBOX ), 1006 ) != 0 )
   {
      return( 0 );
   }
...
// zPCHAR         hCheckBox;
   ...
   kzCheckBoxGetPict( (HINSTANCE) GetWindowLong( hWnd, GWL_HINSTANCE ),
                      &lpCheckBox->Picts );
   ...
   CheckBoxSetText( hWnd, (zPCHAR) lpcsData->lpszName );
   GetClientRect( hWnd, &Rect );

   // Create Bitmap to select to the memory device context.
   hDC = GetDC( hWnd );
   lpCheckBox->hBitmapDC = kzCreateCompatibleBitmap( hDC, Rect.right - Rect.left,
                                                     Rect.bottom - Rect.top );
   ReleaseDC( hWnd, hDC );
                 ...
                 zPCHAR Text )
{
   ...
   if ( DrAllocTaskMemory( &hTextTemp, (zLONG) zstrlen( Text ) + 1L,
                           1007 ) != 0 )
   {
      return( FALSE );
   }
                 ...
                 LPARAM          lParam )
{
   ...
   lpPict = CheckBoxGetPicturePtr( lpPicts, wParam &
                                   (BT_PICT_UP | BT_PICT_DOWN |
   ...
   if ( lpPicts->Pict.chPictType )
      lpPicts->lMaxPictWidth = mMax( lpPicts->lMaxPictWidth,
                                     lpPicts->Pict.lPictWidth );

   if ( lpPicts->PictBtnDown.chPictType )
      lpPicts->lMaxPictWidth = mMax( lpPicts->lMaxPictWidth,
                                     lpPicts->PictBtnDown.lPictWidth );

   if ( lpPicts->PictFocus.chPictType )
      lpPicts->lMaxPictWidth = mMax( lpPicts->lMaxPictWidth,
                                     lpPicts->PictFocus.lPictWidth );

   if ( lpPicts->PictFocusDown.chPictType )
      lpPicts->lMaxPictWidth = mMax( lpPicts->lMaxPictWidth,
                                     lpPicts->PictFocusDown.lPictWidth );

   if ( lpPicts->PictDisabled.chPictType )
      lpPicts->lMaxPictWidth = mMax( lpPicts->lMaxPictWidth,
                                     lpPicts->PictDisabled.lPictWidth );

   if ( lpPicts->PictDisabledDown.chPictType )
      lpPicts->lMaxPictWidth = mMax( lpPicts->lMaxPictWidth,
                                     lpPicts->PictDisabledDown.lPictWidth );
}
...
#else
   HBRUSH  hBrush = (HBRUSH) SendMessage( hWndParent, WM_CTLCOLOR,
                 (WPARAM) hDC, MAKELONG( hWnd, lpCheckBox->nCtlColorCode ) );
#endif
...
// DrUnlockTaskMemory( lpSS );
   ...
   CheckBoxPaintPict( hWnd, lpCheckBox, &rectTemp,
                      hDC, hDC, hBrush, FALSE, fUseDib, &rectShrink );
//xCheckBoxPaintText( hWnd, lpCheckBox, lpRect, &rectTemp,
//x                   hDC, hDC, hBrush, FALSE, fUseDib, &rectShrink );
...
//       DrUnlockTaskMemory( lpText );
   ...
   if ( Pict.chPictType )
   {
      ...
      if ( fUseDib )
      {
         ...
         if ( Pict.hPict )
            switch ( Pict.chPictType )
            {
               case SUPERBTN_PICT_ICON:
                  ...
                  hPict = (HBITMAP) kzCreateStretchedIcon( hDC, &rectTemp,
                                                           GetBkColor( hDC ),
                                                           (HICON) Pict.hPict );
                  fDeletePict = TRUE;
                  break;
         ...
         lPictWidthOrig = mMin( Pict.lPictWidth,
                                (zSHORT)((rectPict.right - x) / dxScaleX) );
         lPictHeightOrig = mMin( Pict.lPictHeight,
                                 (zSHORT)((rectPict.bottom - y) / dxScaleY) );

         nOrigX = mMax( (Pict.lPictWidth - (zSHORT)((rectPict.right - x) /
                        dxScaleX)) / 2, 0 );
         nOrigY = mMax( (Pict.lPictHeight - (zSHORT)((rectPict.bottom - y) /
                        dxScaleY)) / 2, 0 );

         StretchDibBlt( hDC, x, y, mMin( rectPict.right - x, lPictWidth ),
                        mMin( rectPict.bottom - y, lPictHeight ), hDIB,
                        nOrigX, nOrigY,
      ...
      else
      {
         switch ( Pict.chPictType )
         {
            case SUPERBTN_PICT_ICON:
               if ( Pict.hPict )
                  kzDrawIcon( hDC, &rectPict, GetBkColor( hDC ), x, y,
                              (HICON) Pict.hPict );
...
//    Text = (zPCHAR) DrLockTaskMemory( lpCheckBox->hText );
   ...
   if ( nLth )
   {
      ...
      {
         if ( DrAllocTaskMemory( &hTextTemp,
                                 zstrlen( lpCheckBox->hText ) + 1L,
                                 1008 ) == 0 )
         {
            TextTemp = (zPCHAR) DrLockTaskMemory( hTextTemp );
            ...
            hBrushTemp = kzCreateSolidBrush( GetTextColor( hDC ) );
            fnGrayString( hDC, hBrushTemp, 0,
                          (DWORD) TextTemp, 0, x, y, 0, 0 );
            DeleteBrush( hBrushTemp );
...
// TraceLineI( "SS_ComboPaint Row: ", lpSS->Row.ssCurrAt );
   ...
   SS_GetClrTblItem( GetParent( hWnd ), lpSS,
                       lpSS->Col.ssCurrAt, lpSS->Row.ssCurrAt,
                       &BackClrTblItem, 0 );
   ...
   SS_DrawComboBtn( hWnd, lpSS, hDC, lpRect,
                    lpSS->nComboButtonBitmapWidth,
                    (zBOOL) GetWindowLong( hWnd, GWL_BTNDOWN ) );
   ...
   if ( SS_CB_ISDROPDOWN( CellType.lStyle ) == 0 )
   {
      ...
      if ( DrAllocTaskMemory( &hText, nLth + 1, 1009 ) == 0 )
      {
         ...
         Cell = SS_LockCellItem( lpSS, lpSS->Col.ssCurrAt,
                                 lpSS->Row.ssCurrAt );
         Font = SS_RetrieveFont( lpSS, &FontTemp, Cell, lpSS->Col.ssCurrAt,
                                 lpSS->Row.ssCurrAt );
         SS_UnlockCellItem( lpSS, lpSS->Col.ssCurrAt, lpSS->Row.ssCurrAt );
         ...
         COLORREF clrTextOld =
                     SetTxColor( hDC, GetSysColor( COLOR_HIGHLIGHTTEXT ) );
                    ...
                    LPBTNPICT   lpPict )
{
   ...
   if ( lpCheckBox->fDisabled &&
        (((lpCheckBox->lStyle & 0x0F) == BS_3STATE) ||
        ((lpCheckBox->lStyle & 0x0F) == BS_AUTO3STATE)) )
   {
                   ...
                   zLONG lPict )
{
   ...
   return( kzxCheckBoxLoadDef( &g_hCheckBoxDefPicts,
                               &g_nCheckBoxDefPictCnt,
                               hInst, wButtonType, lPict ) );
}
...
kzCheckBoxUnloadDefs( HINSTANCE  hInst )
{
   ...
   return( kzxCheckBoxUnloadDefs( &g_hCheckBoxDefPicts,
                                  &g_nCheckBoxDefPictCnt,
                                  hInst ) );
}
                   ...
                   LPCHECKBOXPICTS lpDefPict )
{
   ...
   return( kzxCheckBoxGetPict( &g_hCheckBoxDefPicts,
                               &g_nCheckBoxDefPictCnt,
                               hInst, lpDefPict ) );
}
...
//    lpDefPicts = (LPCHECKBOXPICTS) DrLockTaskMemory( *lphDefPicts );
      ...
      if ( nFound == -1 )
      {
         *lphDefPicts = (zPCHAR) DrUnlockTaskMemory( lpDefPicts );
         if ( DrReAllocTaskMemory( lphDefPicts,
                                   (zLONG)(sizeof( CHECKBOXPICTS ) *
                                          (*pnDefPictCnt + 1)) ) != 0 )
         {
...
//       lpDefPicts = (LPCHECKBOXPICTS) *lphDefPicts;
   ...
   {
      if ( DrAllocTaskMemory( lphDefPicts, sizeof( CHECKBOXPICTS ),
                              1011 ) != 0 )
      {
         return( FALSE );
      }
...
//    lpDefPicts = (LPCHECKBOXPICTS) DrLockTaskMemory( *lphDefPicts );
      ...
      if ( nFound != -1 )
      {
         ...
         for ( k = nFound; k < *pnDefPictCnt - 1; k++ )
            zmemcpy( &lpDefPicts[ nFound ], &lpDefPicts[ nFound + 1 ],
...
SS_ComboSubclassDesktop( HWND hWnd )
{
   ...
   g_lpfnSSDeskTopProc = (WNDPROC) MakeProcInstance( (FARPROC) SS_DeskTopWndProc,
                                                     g_hInstanceDLL );
   ...
   switch ( uMsg )
   {
      case WM_CTLCOLOR:
         ...
         if ( (hWndSpread = (HWND) GetProp( hWnd, "HWNDSPREAD" )) != 0 )
         {
            ...
            SS_GetClrTblItem( hWndSpread, lpSS, lpSS->Col.ssCurrAt,
                                lpSS->Row.ssCurrAt, &BackClrTblItem,
                                &ForeClrTblItem );
...
//          DrUnlockTaskMemory( lpSS );
   ...
   return( CallWindowProc( (WNDPROC) g_lpfnDeskTopProc,
                           hWnd, uMsg, wParam, lParam ) );
}
