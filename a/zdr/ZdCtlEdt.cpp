...
ZEdit::CWndSetWindowText( zCPCHAR cpcText )
{
   ...
   if ( zstrcmp( cpcText, cs ) != 0 )
      CWnd::SetWindowText( cpcText );
}

void
ZEdit::SetText( zPCHAR pchText )
{
...
{
   zPCHAR pchFormatText = new char[ zstrlen( cpcText ) +
                                    m_lFormatPicLth + m_lSpecialCnt + 1 ];
   zstrcpy( pchFormatText, cpcText );
   SetText( pchFormatText );
...
{
//#ifdef DEBUG_ALL
   if ( zstrcmp( "DIL_Text", *m_pzsTag ) == 0 )
      TraceLineS( "ZEdit::ENSetFocus: ", *m_pzsTag );
//#endif
...
// ProcessEvent( this, 19 );
   ...
   if ( m_lFormatPicLth > 0 )
   {
      ...
      if ( zstrcmp( csText, m_csSaveText ) == 0 )
         return;
...
CGCStringListField::GetEntryPos( const CString& Str )
{
   ...
   for ( k = 0; (k < m_List.size( )) && (Result < 0); k++ )
   {
