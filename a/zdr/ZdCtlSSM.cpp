               ...
               LPRECT  lpRect )
{
   ...
   if ( nLth < sizeof( szBuffer ) )
      zstrcpy( szBuffer, pchMessage );
   else
      nLth = 0;

   zstrcpy( szBuffer + nLth, " X=" );
   zltoa( lpRect->left, szBuffer + nLth + 3 );
   nLth = zstrlen( szBuffer );
   zstrcpy( szBuffer + nLth, " Y=" );
   zltoa( lpRect->top, szBuffer + nLth + 3 );
   nLth = zstrlen( szBuffer );
   zstrcpy( szBuffer + nLth, " W=" );
   zltoa( lpRect->right - lpRect->left, szBuffer + nLth + 3 );
   nLth = zstrlen( szBuffer );
   zstrcpy( szBuffer + nLth, " H=" );
// zltoa( lpRect->bottom - lpRect->top, szBuffer + nLth + 3 );
                   ...
                   zPCHAR        lpItems )
{
   ...
   if ( lpItems )
   {
      if ( DrAllocTaskMemory( (zCOREMEM) &hItems,
                              zstrlen( lpItems ) + 1, 1034 ) != 0 )
      {
         return( 0 );
      }

      lpItemsTemp = (zPCHAR) DrLockTaskMemory( hItems );
      zstrcpy( lpItemsTemp, lpItems );
...
//    lpItems = (zPCHAR) DrLockTaskMemory( lpCellType->Spec.ComboBox.hItems );
      ...
      {
         if ( (fValue && k == lIndex) ||
              (!fValue && zstricmp( pchText, lpItems ) == 0) )
         {
            zstrcpy( pchTextFmt, lpItems );
...
//    DrUnlockTaskMemory( lpItems );
   ...
   if ( !fFound && (lpCellType->lStyle & SS_CB_DROPDOWN) && pchText &&
        zstrlen( pchText ) )
   {
      zstrcpy( pchTextFmt, pchText );
      nLth = zstrlen( pchTextFmt ) + 1;
   ...
   if ( bRC )
   {
      ...
      else
      {
         if ( DrAllocTaskMemory( (zCOREMEM) &hText,
                                 zstrlen( pchTextFmt ) + 1, 1036 ) != 0 )
         {
            return( FALSE );
         }

         pchTextTemp = (zPCHAR) DrLockTaskMemory( hText );
         zstrcpy( pchTextTemp, pchTextFmt );
         SSx_FreeData( lpData );
         lpData->m_DataType = SS_TYPE_EDIT;
...
//       DrUnlockTaskMemory( pchTextTemp );
   ...
   if ( bRC )
   {
      ...
      pchTextTemp = (zPCHAR) DrLockTaskMemory( hText );
      zstrncpy( pchTextTemp, pchTextValue, SS_MAXDATALTH );
//    DrUnlockTaskMemory( pchTextTemp );
