...
PrintActiveWindow( zVIEW vSubtask )
{
   ...
   if ( vSubtask && GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
           ...
           pZSubtask->m_pzmaWithFocus->PrintZCtrl( TRUE ) )
      {
         ...
         zsprintf( szCaption, "Print Window or %s",
                   pZSubtask->m_pzmaWithFocus->m_csERemote );
         zsprintf( szMessage, "OK to print %s control:\n"
                              "for Dialog: %s.%s.%s\n"
                              "for Window: %s?",
                   pZSubtask->m_pzmaWithFocus->m_csERemote,
                   *(pZSubtask->m_pzsDlgTag), *(pZSubtask->m_pzsWndTag),
                   *(pZSubtask->m_pzmaWithFocus->m_pzsTag), szTitle );
...
#endif
         zsprintf( szMessage, "OK to print Window: %s (%s.%s)?",
                   szTitle, *(pZSubtask->m_pzsDlgTag),
                   *(pZSubtask->m_pzsWndTag) );
         nRC = OperatorPrompt( vSubtask, "Print Window",
                               szMessage, 0, nButtons,
                               zRESPONSE_YES, 0 );
...
SaveActiveCtrlToCSV( zVIEW vSubtask )
{
   ...
   if ( vSubtask && GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
           ...
           pZSubtask->m_pzmaWithFocus->PrintZCtrl( TRUE ) )
      {
         ...
         zsprintf( szCaption, "Create CSV" );
         zsprintf( szMessage, "OK to create CSV file for %s control:\n"
                              "for Dialog: %s.%s.%s\n"
                              "for Window: %s?",
                   pZSubtask->m_pzmaWithFocus->m_csERemote,
                   *(pZSubtask->m_pzsDlgTag), *(pZSubtask->m_pzsWndTag),
         ...
         zstrcat( szFileName, *(pZSubtask->m_pzmaWithFocus->m_pzsTag) );
...
//x   }
   ...
   DocInfo.lpszDocName = (LPTSTR) cpcMsg;
      ...
      if ( pZSubtask->m_pZFWnd && mIs_hWnd( pZSubtask->m_pZFWnd->m_hWnd ) )
      {
         ...
         if ( printDC.Attach( dlg.GetPrinterDC( ) ) )
         {
            ...
            if ( hDib )
            {
               ...
               rectDest.bottom = (int)(((double) cyDIB * cxPage * cyInch)
                                                / ((double) cxDIB * cxInch));
               ...
               if ( InitPrinting( printDC, cpcTitle ) )
               {
                  ...
                  zSHORT nLth = zstrlen( cpcTitle );
                  CSize size = printDC.GetTextExtent( cpcTitle, nLth );
                  printDC.TextOut( 16, rectDest.bottom + size.cy + 16,
                                   cpcTitle, nLth );
