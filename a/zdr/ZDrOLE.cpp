...
{
   zVIEW  vApp = 0;
   zPCHAR pchVEA = new char[ zstrlen( cpcVEAPX ) + 1 ];
   zPCHAR pchView;
   zPCHAR pchEntity;
   zPCHAR pchAttribute;
   zPCHAR pchImagePath;
         ...
         *pchAttribute = 0;
         ...
         if ( pchImagePath )
         {
            *pchImagePath = 0;
            ...
            GetAppOrWorkstationValue( vSubtask, pchImagePath,
                                      pchFileName, nMaxLth );
         }
         ...
         if ( vApp )
         {
            zSHORT nLth = zstrlen( pchFileName );

            GetStringFromAttribute( pchFileName + nLth, vApp,
                                    pchEntity, pchAttribute );
         // zstrcpy( pchFileName, "c:\\10e\\rrkk.jpg" );
            if ( pchFileName[ nLth ] )
            {
               if ( pchExt && *pchExt )
               {
                  ...
                  pchFileName[ nLth++ ] = '.';
                  zstrcpy( pchFileName + nLth, pchExt );
               }
            }
         }
      }
...
// zCHAR buf[ 250 ];
   ...
   *height = cinfo.output_height;
   ...
   while ( cinfo.output_scanline < cinfo.output_height )
   {
      ...
      {
         j_putRGBScanline( buffer[ 0 ], *width, dataBuf,
      ...
      if ( cinfo.out_color_components == 1 )
      {
         ...
         j_putGrayScanlineToRGB( buffer[ 0 ], *width, dataBuf,
                                 cinfo.output_scanline - 1 );
      }
   }
                                ...
                                UINT height )
{
   ...
   {
      memcpy( (tmp + uRow * widthPix * 3), (inBuf + uRow * widthBytes),
              widthPix * 3 );
   }
