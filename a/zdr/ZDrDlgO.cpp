...
// #define zCONTROL_RESERVED_2           0x02000000L // 33554432
                                ...
                                zCPCHAR     cpcErrorMessagePrefix )
{
   ...
   if ( cpcErrorMessagePrefix && *cpcErrorMessagePrefix )
   {
      ...
      zstrcpy( pch, cpcErrorMessagePrefix );
      if ( *lpLibrary == 0 )
      {
         zstrcat( pch, " Unable to load\n application DLL - " );
         zstrcat( pch, cpcDLL_Name );
         zstrcat( pch, "\n for operation - " );
         zstrcat( pch, cpcOperationName );
      }
      else
      {
         zstrcat( pch, " " );
         zstrcat( pch, cpcOperationName );
         zstrcat( pch, " Operation Code not found\n in DLL - " );
         zstrcat( pch, cpcDLL_Name );
      }
...
// TraceLineI( "ResumeFromDebugger =============> ", (zLONG) vAppSubtask );
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vDebuggerSubtask, 0 ) == 0 )
   {
      ...
      if ( cpcDebuggerAction )
         ProcessActionLoop( pZSubtask, cpcDebuggerAction,
                            0, 0, 0, FALSE, "Debugger" );
      ...
      //                               TRUE );
         ...
         else
         {
            ...
            zstrcpy( szActionName, *(pZSubtask->m_pzsResumeAction) );
            mDeleteInit( pZSubtask->m_pzsResumeAction );

            // The first time thru ... resume flag is ON.
            while ( ProcessAction( pZSubtask, szActionName,
                                   vAppSubtask, 0, "Resume" ) > 0 )
            {
               vAppSubtask = 0;
            }
         }
...
HotkeyToPainter( zVIEW vSubtask )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      if ( GetViewByName( &vIdlePainter, "__SysIdlePainter",
...
//          szFileSpec[ nLth++ ] = '\\';
         ...
         zstrcpy( szFileSpec + nLth, "zdr ~~Zeidon_Tools~tzpntrad" );
...
fnResumeFromPainter( ZSubtask *pZSubtask )
{
   ...
   zstrcpy( szViewName + 2, *(pZSubtask->m_pzsDlgTag) );
              ...
              zSHORT   bBeep )
{
   ...
   if ( PeekMessage( &msg, 0, g_uMsgPostedClose, g_uMsgPostedClose,
                        ...
                        zUSHORT  uBufferLth )
{
   ...
   // pZSubtask->m_ulSubtaskFlags |= zSUBTASK_APPLICATION_OPERATION;
      ...
      if ( pXRA )
      {
         ...
         SetAttributeFromString( pXRA->m_vXRA_Delta, "MsgBox",
                                 "Value", pchReturnBuffer );
         SetAttributeFromInteger( pXRA->m_vXRA_Delta, "MsgBox",
                                  "MaxLength", uBufferLth );
         SetAttributeFromString( pXRA->m_vXRA_Delta, "MsgBox",
                                 "OK", "Y" );
         SetAttributeFromString( pXRA->m_vXRA_Delta, "MsgBox",
                                 "Cancel", "Y" );
         ...
         GetStringFromAttribute( szYN, pXRA->m_vXRA_Delta, "MsgBox", "OK" );
         if ( szYN[ 0 ] )
         {
            nRC = zRESPONSE_OK;
            GetStringFromAttribute( pchReturnBuffer, pXRA->m_vXRA_Delta,
                                    "MsgBox", "Value" );
         }
         else
                ...
                zSHORT   nIcon )
{
   ...
   //       case zBUTTONS_OK:
         ...
         GetStringFromAttribute( szYN, vXRA_State, "MsgBox", "OK" );
         if ( szYN[ 0 ] )
            nRC = zRESPONSE_OK;
         else
         {
            GetStringFromAttribute( szYN, vXRA_State, "MsgBox", "Cancel" );
            if ( szYN[ 0 ] )
               nRC = zRESPONSE_CANCEL;
            else
            {
               GetStringFromAttribute( szYN, vXRA_State, "MsgBox", "Yes" );
               if ( szYN[ 0 ] )
                  nRC = zRESPONSE_YES;
               else
               {
                  GetStringFromAttribute( szYN, vXRA_State, "MsgBox", "No" );
                  if ( szYN[ 0 ] )
                     nRC = zRESPONSE_NO;
                  else
                  {
                     GetStringFromAttribute( szYN, vXRA_State,
                                             "MsgBox", "Abort" );
                     if ( szYN[ 0 ] )
                        nRC = zRESPONSE_ABORT;
                     else
...
//                              a memory area at least 33 bytes long).  If
                       ...
                       zUSHORT  uHelpType )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( cpcHelpFile && cpcHelpFile[ 0 ] )
         zstrcpy( szHelpFile, cpcHelpFile );
      else
      {
         GetStringFromAttribute( szHelpFile, pZSubtask->m_vDialog,
                                 szlDlg, szlDLL );
         zstrcat( szHelpFile, ".hlp" );
      }
      ...
      if ( cpcHelpContext && cpcHelpContext[ 0 ] )
         zstrcpy( szHelpContext, cpcHelpContext );
      else
      {
         zstrcpy( szHelpContext, *(pZSubtask->m_pzsDlgTag) );
         usSize = zstrlen( szHelpContext );
         if ( usSize )
            szHelpContext[ usSize++ ] = '.';

         zstrcpy( szHelpContext + usSize, *(pZSubtask->m_pzsWndTag) );
         usSize += zstrlen( szHelpContext + usSize );
         szHelpContext[ usSize++ ] = '.';

         if ( cpcCtrlTag && cpcCtrlTag[ 0 ] )
            zstrcpy( szHelpContext + usSize, cpcCtrlTag );
         else
         if ( pZSubtask->m_pzmaWithFocus )
            zstrcpy( szHelpContext + usSize,
      ...
      switch ( uHelpType )
      {
         ...
         case HELP_HELPONHELP:
            WinHelp( *(pZSubtask->m_pZFWnd),
                     szHelpFile, uHelpType, (DWORD) 0 );
            break;

         case HELP_CONTEXT:
         case HELP_SETINDEX:
            WinHelp( *(pZSubtask->m_pZFWnd),
                     szHelpFile, uHelpType, (DWORD) ulHelpContextNbr );
            break;

         case HELP_KEY:
            WinHelp( *(pZSubtask->m_pZFWnd),
                     szHelpFile, uHelpType, (DWORD) szHelpContext );
            break;

         case HELP_MULTIKEY:
            usSize = sizeof( MULTIKEYHELP ) + zstrlen( szHelpContext );
            ...
            mkh->mkSize = usSize;
            mkh->mkKeylist = 'Z';
            zstrcpy( (zPCHAR) mkh->szKeyphrase, (zPCHAR) szHelpContext );
            WinHelp( *(pZSubtask->m_pZFWnd),
                     szHelpFile, HELP_MULTIKEY, (DWORD) mkh );
                           ...
                           zPCHAR pchErrorPrefix )
{
   ...
   zstrcpy( szOperation, "o" );
   zstrcat( szOperation, pchObjectName );
   zstrcat( szOperation, "_" );
