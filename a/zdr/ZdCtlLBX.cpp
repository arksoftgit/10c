            ...
            if ( m_bShowHeaderButtons )
            {
               ...
               if ( pch )
               {
                  ...
                  *pch = 0;
                  ...
                  zstrcpy( m_pCol[ nCol ].pchMB_Text, pch + 1 );
               }
               else
               {
...
ZListBox::FormatTextAtPosition( zLONG lPos )
{
   ...
   *m_pchMap = 0; // initialize to null string in case there are problems
   ...
   for ( nCol = 1; nCol <= m_nCols; nCol++ )
   {
      ...
      zSHORT nRC =
      GetVariableFromAttribute( m_pchMap + nOffset, 0,
                                zTYPE_STRING,
   ...
   if ( zstrchr( m_pchMap, '&' ) )
   {
      ...
      while ( pchAmpersand )
      {
         zstrcpy( pchBuffer + ulLth, pchPos );
...
#ifdef DEBUG_ALL
   {
      ...
      zsprintf( szMsg, "%ld %s", lPos, m_pchMap );
...
//                (zLONG) m_vAppList );
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         if ( m_pchMap[ 0 ] &&
              DrAllocTaskMemory( (zCOREMEM) &pchMem,
                                 zstrlen( m_pchMap ) + 1 ) == 0 )
         {
            pchMem = (zPCHAR) DrLockTaskMemory( pchMem );
            zstrcpy( pchMem, m_pchMap );
         }
         else
         {
            pchMem = "";
   ...
   if ( m_nDisplaySelSet )
   {
                            ...
                            0, 0, 0, 0, cpcScope, 0 ) >= 0 )
      {
         while ( TRUE )
         {
            ...
            {
               lOffset = GetRelativeEntityNumber( vApp, cpcEName, cpcScope,
                                                  m_ulFlag | zQUAL_SELECTED );
               break;
            }
                          ...
                          zLONG lColumn )
{
   ...
   if ( lColumn > 0 && lColumn <= m_nCols )
   {
      ...
      m_pCol[ lColumn ].pchContext = new char[ zstrlen( cpcContext ) + 1 ];
...
// Disable Delta Data implementation.
      ...
      while ( pos )
      {
         ...
         {
            SetAttributeFromInteger( vXRA, cpcLBDataRowEntity,
         ...
         {
            SetAttributeFromString( vXRA, cpcLBDataRowEntity,
                                    "Deleted", "Y" );
         }
         else
         {
         ...
         if ( pDataRow->GetSel( ) == TRUE )
         {
            ...
            SetAttributeFromInteger( vXRA, cpcLBSelItemEntity,
                                     "Tag", lRelNbr );
         }
   ...
   if ( m_pZSubtask->m_pZSocketTask &&
        (pXRA = m_pZSubtask->m_pZSocketTask->
           m_pCacheWndList->Find( *(m_pZSubtask->m_pzsRemoteWndTag),
                                  m_pZSubtask,
                                  "ZListBox::MapFromRemoteDataset" )) != 0 &&
        (pXRA->m_vXRA || pXRA->m_vXRA_Delta) )
   {
      ...
      if ( PositionOnRemoteCtrl( this, vXRA, TRUE ) )
      {
              ...
              SetCursorFirstEntity( vXRA, cpcLBDataRowEntity, 0 ) >= zCURSOR_SET )
         {
            ...
            GetIntegerFromAttribute( &lRelNbr, vXRA,
                                     cpcLBDataRowEntity, "Tag" );
            pDataRow = FindDataRowByIndex( lRelNbr );
            if ( pDataRow )
            {
               GetAttributeLength( &lLength, vXRA,
                                   cpcLBDataRowEntity, "Text" );
               pchRowText = csRowText.GetBufferSetLength( lLength );

               GetStringFromAttribute( pchRowText, vXRA,
                                       cpcLBDataRowEntity, "Text" );
            ...
            while ( SetCursorNextEntity( vXRA, cpcLBDataRowEntity,
                                         0 ) >= zCURSOR_SET )
            {
               GetIntegerFromAttribute( &lRelNbr, vXRA,
                                        cpcLBDataRowEntity, "Tag" );
              ...
              SetCursorFirstEntity( vXRA, cpcLBSelItemEntity, 0 ) >= zCURSOR_SET )
         {
            ...
            while ( SetCursorNextEntity( vXRA, cpcLBSelItemEntity,
                                         0 ) >= zCURSOR_SET )
            {
               GetIntegerFromAttribute( &lRelNbr, vXRA,
                                        cpcLBSelItemEntity, "Tag" );
...
ZListBox::SetMultiSelection()
{
   ...
   if ( GetViewByName( &vXRA, "__XRA_Select",
                       m_pZSubtask->m_vDialog, zLEVEL_TASK ) < 0 )
   {
      return( -1 );
   }
   ...
   if ( (vXRA = SetupRemoteObject( )) != 0 )
   {
      ...
      if ( lRow == 0 )
      {
         ...
         if ( CheckExistenceOfEntity( vXRA, cpcLBColumnEntity ) == 0 &&
              SetCursorFirstEntityByInteger( vXRA, cpcLBColumnEntity, "Tag",
                                             lColumn, "" ) >= zCURSOR_SET )
         {
            bFoundColumn = TRUE;
         }
         ...
         if ( bFoundColumn == FALSE )
         {
            ...
            SetAttributeFromInteger( vXRA, cpcLBColumnEntity,
                                     "Tag", lColumn );
         }
         ...
         zKZWDLGXO_Ctrl_DEF *pCtrlDef )
{
        ...
        pCtrlDef->Tag[ 1 ] == '_' )
   {
      if ( pCtrlDef->Tag[ 2 ] == 'P' &&
           ...
           pCtrlDef->Tag[ 5 ] == 'p' )
      {
         return( new ZPropertyList( pZSubtask, pWndParent,
   ...
   if ( (pCtrlDef->Subtype & zLISTBOX_COLUMNHEADERS) ||
        (pCtrlDef->Subtype & zLISTBOX_SORTEDHEADERS) )
   {
      return( ListCtrl( pZSubtask, pWndParent, pzmaComposite,
                        vDialog, nOffsetX, nOffsetY, pCtrlDef ) );
   }
#endif

   return( new ZListBox( pZSubtask, pWndParent,
                         pzmaComposite, vDialog,
                         nOffsetX, nOffsetY, pCtrlDef ) );
}
                        ...
                        zSHORT  nSelectSet )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for LB_SetDisplaySelectSet ",
                  cpcCtrlTag );
   }
