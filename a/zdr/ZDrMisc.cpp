      ...
      for ( k = 0; k < pZSubtask->m_pZMIXCtrl->GetCount( ); k++ )
      {
         ...
         if ( pzma->m_pzmaComposite == pzmaParent )
         {
            ...
            if ( nLineLth > 3 )
            {
               ...
               if ( nLineLth > nTgtLth )
               {
                  zstrcpy( pchCtrlList + nCurrLth, "\n   " );
                  nCurrLth += 4;
                  nLineLth = 3;
               }
            }

            zstrcpy( pchCtrlList + nCurrLth, *(pzma->m_pzsTag) );
            nLth = zstrlen( *(pzma->m_pzsTag) );
            nCurrLth += nLth;
            nLineLth += nLth;
...
zkCtrlInfo( zVIEW vSubtask )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      if ( pzma )
      {
         ...
         if ( sizeof( szMsg ) - lLth > 50 )
         {
            if ( pzma->m_pzsVName && *(pzma->m_pzsVName) )
            {
               zstrcat( szMsg, "  View: " );
               lLth = zstrlen( szMsg ) + 1;
               zstrncat( szMsg, *(pzma->m_pzsVName), sizeof( szMsg ) - lLth );
            }
         }
         ...
         if ( sizeof( szMsg ) - zstrlen( szMsg ) > 50 )
         {
            ...
            {
               zstrcat( szMsg, "  Entity: " );
               lLth = zstrlen( szMsg ) + 1;
               zstrncat( szMsg, *(pzma->m_pzsEName), sizeof( szMsg ) - lLth );
            }
         }
         ...
         if ( sizeof( szMsg ) - zstrlen( szMsg ) > 50 )
         {
            ...
            {
               zstrcat( szMsg, "  Attribute: " );
               lLth = zstrlen( szMsg ) + 1;
               zstrncat( szMsg, *(pzma->m_pzsAName), sizeof( szMsg ) - lLth );
            }
         }
         ...
         {
            zstrcat( szMsg, "  Text: " );
            zstrncat( szMsg, *(pzma->m_pzsText), sizeof( szMsg ) - lLth - 1000 );
         }
      ...
      // else
            OperatorSend( vSubtask, "Control with focus Information",
                          szMsg, FALSE );
      }
   }
...
GetRealEntityName( zVIEW v, zPCHAR pchEntityName )
{
   ...
   if ( lpViewEntity->bRecursiveSt )
   {
      ...
      if ( lpParent )
      {
         zstrcpy( pchEntityName, lpParent->szName );
      }
      else
         return( -1 );
   }
         ...
         if ( mIs_hWnd( g_pSubtaskList[ lItemCnt ].hWnd ) )
         {
            if ( mIs_hWndVisible( g_pSubtaskList[ lItemCnt ].hWnd ) )
            {
               if ( mIs_hWndEnabled( g_pSubtaskList[ lItemCnt ].hWnd ) )
                  zstrcpy( szListLine, "Enabled" );
               else
                  zstrcpy( szListLine, "Disabled" );
            }
            else
               zstrcpy( szListLine, "Invisible" );
         }
         else
            zstrcpy( szListLine, "Invalid" );
...
// }
   ...
   zsprintf( szMsg, "EnableZWindowsForApplication bEnable: %s   ulInterp: ",
             bEnable ? "TRUE" : "FALSE" );
   ...
   while ( lItemCnt )
   {
      if ( g_pSubtaskList[ lItemCnt ].pZSubtask )
      {
              ...
              //                                                   ulInterp )
         {
            ...
            zsprintf( szMsg, "EnableZWindowsForApplication Dlg: %s  Wnd: %s  nInvisible: ",
                      *(g_pSubtaskList[ lItemCnt ].pZSubtask->m_pzsDlgTag),
...
// pListCtrl->SetItemCount( lItemCnt );
   for ( lPos = 1; lPos <= lItemCnt; lPos++ )
   {
      ...
      // lvItem.cchTextMax = zstrlen( g_pSubtaskList[ lPos ].DlgN ) + 1;
         ...
         zsprintf( szListLine, "0x%08x", g_pSubtaskList[ lPos ].pvDialog );
      // zltoa( (zLONG) g_pSubtaskList[ lPos ].pvDialog, szListLine );
         lvItem.mask = LVIF_TEXT;
         lvItem.iItem = lPos - 1;
         lvItem.iSubItem = 2;
         ...
         if ( g_pSubtaskList[ lPos ].pZSubtask )
            zltoa( (zLONG) g_pSubtaskList[ lPos ].pZSubtask->m_ulSubtaskIdNbr,
                   szListLine );
         else
            zstrcpy( szListLine, "0" );
         ...
         zsprintf( szListLine, "0x%08x", g_pSubtaskList[ lPos ].lTID );
      // zltoa( (zLONG) g_pSubtaskList[ lPos ].lTID, szListLine );
         lvItem.mask = LVIF_TEXT;
         lvItem.iItem = lPos - 1;
         lvItem.iSubItem = 4;
         lvItem.pszText = szListLine;
