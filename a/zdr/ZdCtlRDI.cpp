...
ZRmtDataRow::GetRowDataText( zBOOL bUseSeparator ) const
{
   ...
   zstrcpy( pchHelper, csHelper );
   zstrcpy( pchBuffer, csHelper );
   ...
   while ( pchDelim )
   {
      ...
      if ( *(pchDelim + 1) == zREMOTE_DATA_DELIMITER )
      {
         ...
         pchDelim++;
         pchPrev = pchDelim;
         zstrcpy( pchBuffer + lLth, pchDelim );
      }
      else
         pchDelim++;
...
ZRmtDataSet::GetLBText( zLONG lIdx, LPTSTR lpszText )
{
   ...
   if ( lIdx >= 0 )
   {
      ...
      if ( pRow )
      {
         zstrcpy( lpszText, pRow->GetString( ));
                  ...
                  LPARAM  lParam )
{
   ...
   if ( lParam )
   {
      ...
      if ( pos )
      {
         ...
         zsprintf( szMsg, "InitRmtDataSet: %s %ld %ld %-32s  State: 0x%08lx",
                     ...
                     LPARAM  lParam )
{
   ...
   zsprintf( szMsg, "RmtDataSet Row: %s %ld %ld - ",
             pdr->GetRowDataText( ),
