   ...
   if ( pchTagReturn[ 0 ] == 0 )
   {
      if ( lType &&
           ...
           SetCursorFirstEntityByInteger( vTZPESRCO, szlControlDef, szlKey,
                                          lType, 0 ) == zCURSOR_SET )
      {
         GetStringFromAttribute( pchTagReturn, vTZPESRCO,
                                 szlControlDef, szlTag );
      }

      if ( pchTagReturn[ 0 ] == 0 )
      {
         zstrcpy( pchTagReturn,
           m_pBar->
           m_PaletteBarItems[ m_pBar->m_uSelectedPaletteItem ].szTag );
      }
   }
...
//          if ( pCtrl->m_csTag == csTag )
            if ( zstrcmp( pCtrl->m_csTag, csTag ) == 0 )
            {
               bTagFound = TRUE;
               break;
            }
                        ...
                        CMenu *pMenu )
{
   ...
   {
      GetStringFromAttribute( szSeparator, vSubMeta,
                              szlOption, szlSeparator );
      if ( szSeparator[ 0 ] == 'Y' )
         pMenu->AppendMenu( MF_SEPARATOR );

      GetStringFromAttribute( szOptionText, vSubMeta, szlOption, szlText );
...
// TraceLineS( "TZPainterWindow::PaintCtrl", "" );
   ...
   if ( vCtrl && lType == 0 )
   {
           ...
           zstrcmp( pchTag, "FinancialAcctTransaction" ) == 0 )
      {
         ...
         zsprintf( szMsg, " PX %ld PY %ld SX %ld SY %ld",
      ...
      zstrcpy( szBuffer, "TZPainterWindow::PaintCtrl " );
      zstrcat( szBuffer, pchTag );
...
// TraceRect( "CTRLMSG_PAINT Rect:", rectPaint );
   ...
   GetStringFromAttribute( szPlaceHolder, vTZPESRCO, szlControlDef, "PlaceHolder" );
...
// zCHAR szBuffer[ 64 ];
// zstrcpy( szBuffer, m_csWndTag );
// zstrcat( szBuffer, " lSizeY: " );
...
// TraceLineI( szBuffer, lSizeY );
   ...
   if ( m_pBar->m_fState & (zTZCB_REPORT | zTZCB_XSLT) )
   {
      ...
      zstrcpy( szCaption, "Report - " );
      GetStringFromAttribute( szCaption + 9, m_vDialog, pchWindowPage, szlTag );
      SetWindowText( szCaption );
      GetIntegerFromAttribute( &m_lReportTextSize, m_vDialog,
   ...
   else
   {
      ...
      GetStringFromAttribute( szCaption, m_vDialog, pchWindowPage, szlTag );
      zstrcat( szCaption, " - " );
      GetStringFromAttribute( szCaption + 64, m_vDialog,
                              pchWindowPage, szlCaption );
      zstrcat( szCaption, szCaption + 64 );
      SetWindowText( szCaption );

      // Now that sizing and positioning are out of the way, paint the
      // action bar for the window.
      PaintActionBar( );
   }
...
//       if ( pCtrl->m_chEntity == 'S' )
      ...
      zCHAR szBuffer[ 256 ];
      CRect rect( lPosX, lPosY, lPosX + lSizeX, lPosY + lSizeY );
      zstrcpy( szBuffer, "TZPainterWindow::UpdateZeidonWindow " );
      zstrcat( szBuffer, pCtrl->m_csTag );
      TraceRect( szBuffer, rect );
...
//    if ( pCtrl->m_chEntity == 'S' )
      ...
      if ( bPosOnly == FALSE )
      {
         zstrcpy( szDilMsg, pchEntity );
         zstrcat( szDilMsg, ": " );
         zstrcat( szDilMsg, pCtrl->m_csTag );
         ...
         if ( vDialog )
         {
            ...
            if ( pCtrl->m_chEntity == 'C' )
            {
               if ( CheckExistenceOfEntity( vDialog, "EventAct" ) == 0 )
               {
                  ...
                  nLth = zstrlen( szDilMsg );
                  szDilMsg[ nLth++ ] = ' ';
                  szDilMsg[ nLth++ ] = '[';
                  GetStringFromAttribute( szDilMsg + nLth, vDialog, "EventAct", "Tag" );
                  nLth = zstrlen( szDilMsg );
                  szDilMsg[ nLth++ ] = ']';
                  szDilMsg[ nLth++ ] = ' ';
                  zstrcpy( szDilMsg + nLth, GetActionString( lWork ) );
                  ...
                  {
                     nLth = zstrlen( szDilMsg );
                     zsprintf( szDilMsg + nLth, "  ==> %s.%s", pchAttr, pchEntity );
                  }
               }
            }
         }
   ...
   // TraceRect( "TZPainterWindow::SetDIL_Text rect", pCtrl->m_rectCtrl );
      ...
      else
      {
         ...
         zstrcat( szDilMsg, "), Size(" );
         if ( m_pBar->m_bUseMapDlgUnits )
            lWork = mConvertPixelToMapDlgX( (pCtrl->m_rectCtrl.right -
                                             pCtrl->m_rectCtrl.left) );
         else
            lWork = mConvertPixelToDlgUnitX( (pCtrl->m_rectCtrl.right -
                                              pCtrl->m_rectCtrl.left) );
         ...
         zstrcat( szDilMsg, "," );
         if ( m_pBar->m_bUseMapDlgUnits )
            lWork = mConvertPixelToMapDlgY( (pCtrl->m_rectCtrl.bottom -
                                             pCtrl->m_rectCtrl.top) );
         else
            lWork = mConvertPixelToDlgUnitY( (pCtrl->m_rectCtrl.bottom -
                                              pCtrl->m_rectCtrl.top) );
   ...
   // TraceLineS( "TZPainterWindow::SetDIL_Text msg: ", szDilMsg );
      ...
      // if ( pCtrl->m_lZKey )
         if ( vDialog )
         {
            ...
            {
               zstrcat( szDilMsg, "Null,Null,Null" );
            }
            ...
            (pCtrl = pCtrl->IsDeletable( )) != 0)) )
      {
         ...
         for ( k = 0; pCtrl && k < m_nCtrlCnt; k++ )
         {
            if ( pCtrl == m_CtrlList[ k ] )
            {
               ...
               nTotalDeleteCnt++;
               nDeleteCnt++;
               zsprintf( szLabel, "&xxdo Delete %d item%s\tCtrl+x",
                         nTotalDeleteCnt,
                         nTotalDeleteCnt == 1 ? "" : "s" );
...
//       if ( pCtrl->m_csTag == pCtrl2->csTag )
         if ( zstrcmp( pCtrl->m_csTag, pCtrl2->m_csTag ) == 0 )
         {
            zstrcpy( szMessage, "Duplicate tag '" );
            zstrcat( szMessage, pCtrl->m_csTag );
            zstrcat( szMessage, "' found!" );
            SysMessageBox( m_pBar->m_pZSubtask->m_vDialog,
                           "Error", szMessage, 1 );
         }
      }
   }
   ...
   if ( pMsg->message == WM_SYSKEYDOWN || pMsg->message == WM_KEYDOWN )
   {
           ...
           (GetKeyState( VK_SHIFT ) >= 0) )
      {
         ...
         // TraceLineS( "tzctlptr Copy/Paste", "" );
            ...
            PainterCall( nMessage, 0, m_pBar, 0, 0 );
            return( TRUE ); // the message was translated and should not be dispatched
         }
      }
   }
