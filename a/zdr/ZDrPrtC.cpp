                            ...
                            zCPCHAR cpcViewNameList )
{
   ...
   szGroup[ 0 ] = '[';
   SysReadZeidonIni( -1, "[Network]", "DefaultNetwork", szGroup + 1 );
   zstrcat( szGroup + 1, "]" );
   SysReadZeidonIni( -1, szGroup, "ApplServer", szApplServer );
   NetGetLocalHostAddress( vSubtask, 0, szLocalAddress );
   if ( zstrcmp( szLocalAddress, szApplServer ) == 0 )
   {
      lRC = PrintReportToPDF( vSubtask, vSourceOI,
                              cpcSubobjectEntityName,
                              cpcReportDefName,
                              pchReportName, lFlags );
   }
   else
   {
      ...
      GetApplDirectoryFromView( szAppName, vSubtask, zAPPL_NAME,
                                sizeof( szAppName ) );
            ...
            zPCHAR  pchReportInfo )
{
   ...
   ZPrintout *pPrint = new ZPrintout( pZSubtask, cpcDLL_Name,
                                      cpcPrintPageFunction,
...
CloseReport( zVIEW vSubtask, zLONG lFlags )
{
        ...
        GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      if ( pZSubtask->m_pZPrintout )
      {
         if ( lFlags & 0x00000001 )  // close report file only
         {
            if ( pZSubtask->m_pZPrintout->m_lFileCSV != -1 )
            {
               ...
               {
                  SysWriteFile( pZSubtask->m_vDialog,
                                pZSubtask->m_pZPrintout->m_lFileCSV,
                                "<br /></p>\r\n", 12 );
                  SysWriteFile( pZSubtask->m_vDialog,
                                pZSubtask->m_pZPrintout->m_lFileCSV,
                                "</html>\r\n", 9 );
                  TraceLineS( "ZPrintout Closing HTML file: ",
                              pZSubtask->m_pZPrintout->m_csReportFileName );
               }
               else
                  TraceLineS( "ZPrintout Closing CSV file: ",
                              pZSubtask->m_pZPrintout->m_csReportFileName );

               SysCloseFile( pZSubtask->m_vDialog,
                             pZSubtask->m_pZPrintout->m_lFileCSV, 0 );
         ...
         {
            TraceLineS( "ZPrintout drawing filename to report: ",
                        pZSubtask->m_pZPrintout->m_csReportFileName );
            CreateFontForReport( pZSubtask->m_vDialog, "bold", 18 * 12,
                                 TRUE, FALSE, FALSE, FALSE, 0, 0, 0, "Arial" );
            SelectFontForReport( pZSubtask->m_vDialog, "bold" );
            DrawTextToReport( pZSubtask->m_vDialog, 0, "Report created in file:",
                              100, 100, 100000, 2000, 0, 0, 0, 0, 0 );
            DrawTextToReport( pZSubtask->m_vDialog, 0,
                              pZSubtask->m_pZPrintout->m_csReportFileName,
                              1000, 2500, 100000, 10000, 0, 0, 0, 0, 0 );
         }
      }
      ...
      if ( (lFlags & 0x00000001) == 0 )
      {
         ...
         pZSubtask->m_pZTask->m_uAppState &= ~zTASK_STATE_PRINT;
         if ( (pZSubtask->m_pZPrintout->m_bPrintToCSV == FALSE &&
               pZSubtask->m_pZPrintout->m_bPrintToHTML == FALSE) )
         {
            mDeleteInit( pZSubtask->m_pZPrintout );
         }
...
//    TraceLineS( "DrawTextToReport: ", cpcText );
        ...
        pZSubtask->m_pZPrintout->m_pDC->m_hDC )
   {
      ...
      {
         pFontOld = pZSubtask->m_pZPrintout->m_pDC->
                        SelectObject( pZSubtask->m_pZPrintout->m_pFontCurr );

         if ( (pZSubtask->m_pZPrintout->m_pFontCurr->m_clrText & 0xff000000) == 0 )
         {
            clrOldText = (COLORREF)
            pZSubtask->m_pZPrintout->m_pDC->
                         SetTextColor( pZSubtask->m_pZPrintout->
                                                  m_pFontCurr->m_clrText );
         }
         ...
         if ( (pZSubtask->m_pZPrintout->m_pFontCurr->m_clrTextBk & 0xff000000) == 0 )
         {
            ...
            pZSubtask->m_pZPrintout->m_pDC->
                         SetBkColor( pZSubtask->m_pZPrintout->
                                                  m_pFontCurr->m_clrTextBk );
         }
         ...
         if ( pZSubtask->m_pZPrintout->m_pFontCurr->m_bSet == FALSE )
         {
            ...
            if ( pZSubtask->m_pZPrintout->
                  m_pDC->GetOutlineTextMetrics( sizeof( OUTLINETEXTMETRIC ),
                                                &OTM ) )
            {
               pZSubtask->m_pZPrintout->m_pFontCurr->m_Panose =
                                                      OTM.otmPanoseNumber;
            }
            ...
            {
               pZSubtask->m_pZPrintout->m_pFontCurr->m_byCharset =
                                                                lf.lfCharSet;
               pZSubtask->m_pZPrintout->m_pFontCurr->m_byPitchFamily =
                                                         lf.lfPitchAndFamily;
            }
         }
      }
      ...
      zLONG lLeft =
            (lLeft256ths * pZSubtask->m_pZPrintout->m_nPixelsPerInchX) / 256;
      zLONG lTop =
             (lTop256ths * pZSubtask->m_pZPrintout->m_nPixelsPerInchY) / 256;
      zLONG lRight =
           (lRight256ths * pZSubtask->m_pZPrintout->m_nPixelsPerInchX) / 256;
      zLONG lBottom =
          (lBottom256ths * pZSubtask->m_pZPrintout->m_nPixelsPerInchY) / 256;
      ...
      if ( lJustify & 4 )
      {
         ...
         {
            sizeExtent = pZSubtask->m_pZPrintout->
                                    m_pDC->GetTextExtent( " ", 1 );
   ...
   // TraceLineI( "Escapement: ", lEscapement );
      ...
      // uFormat = DT_LEFT | DT_SINGLELINE | DT_NOCLIP;
         uFormat = DT_CENTER | DT_VCENTER | DT_NOCLIP |
                   DT_SINGLELINE | DT_NOPREFIX; // force single line printing

         sizeExtent = pZSubtask->m_pZPrintout->
                   m_pDC->GetTextExtent( szWorkString, // in logical units
      ...
      if ( pZSubtask->m_pZPrintout->m_bPhantom == FALSE )
      {
         ...
         pZSubtask->m_pZPrintout->m_pDC->DrawText( szWorkString, lCopyLength,
                                                   rect, uFormat );
              ...
              zLONG  lMaxReturnLth, zSHORT nTabSpaces )
{
   ...
   zstrcpy( pchReturnString, cpcString );
   ...
   if ( pchTab )
   {
      ...
      while ( pchTab )
      {
         ...
         pchTemp++;
         zstrcpy( pchReturnString + lOffset, pchTemp );
                           ...
                           zPLONG  plNextWord )
{
   ...
   // size.cx = 0; size.cy += tm.tmDescent;
      ...
      zLONG lLine256ths = ((lLineHeight * 256) /
                                 pZSubtask->m_pZPrintout->m_nPixelsPerInchX);
   // zSHORT nLineHeight = (zSHORT) ((size.cy * 256) /
   //                            pZSubtask->m_pZPrintout->m_nPixelsPerInchX);
      lTop256ths = -32000;
      ...
      if ( cpcText[ 0 ] == 0 )
      {
         ...
         *plNewBottom256ths += (lLineHeight * 256) /
   ...
   //    lTabStopExtent = -1;
      ...
      if ( lMaxTextLth >= sizeof( szWork ) - 1 )
      {
         DrAllocTaskMemory( &pchText, lMaxTextLth + 1 );
         zstrncpy( pchText, cpcText, lMaxTextLth );
         cpcText = pchText;
      }
      else
      {
         zstrncpy( szWork, cpcText, lMaxTextLth );
            ...
            //    bTab = TRUE;
               ...
               if ( k == j && cpcText[ k ] != '\t' && cpcText[ k ] != ' ' )
               {
                  ...
                  if ( cpcText[ k ] == 0 )
                  {
                     ...
                     *plNewBottom256ths = ((zLONG) lCurrentY * 256) /
                                  pZSubtask->m_pZPrintout->m_nPixelsPerInchY;
                     *plNextWord = 0;
...
//BL why?            szPrint[ 0 ] = cpcText[ k ]; // used as a flag later
                     ...
                     // k != j or tab or blank
               {
                  ...
                  {
                     TraceLineI( "Cannot handle extremely long multi-line "
                                    "text length: ",
                                 k - lStart );
                     break;
                  }
                  ...
                  zstrncpy( szPrint, cpcText + lStart, k - lStart );
                  szPrint[ k - lStart ] = 0;
                  GetTextExtentPoint32( pZSubtask->m_pZPrintout->m_pDC->m_hDC,
                                        szPrint, k - lStart, &size );
                  if ( cpcText[ k ] == '\t' )
                  {
                     k--;
                     if ( k < lStart )
               ...
               // pZSubtask->m_pZPrintout->m_pDC->DPtoLP( &size );
                  if ( size.cx > rect.right - rect.left )
                  {
                     ...
                     else  // the line has more than one word in it
                     {
                        ...
                        zstrncpy( szPrint, cpcText + lStart, j - lStart + 1 );
                        ...
                        if ( lStart != j )
                        {
                           ...
                           zstrncpy( szPrint, cpcText + lStart, j - lStart );
                           szPrint[ j - lStart ] = 0;
                           lStart = j;
                           k = j;
                        }
            ...
            if ( szPrint[ 0 ] != '\n' && szPrint[ 0 ] != '\r' )
            {
               ...
               if ( pZSubtask->m_pZPrintout->m_bPhantom == FALSE )
               {
                  ...
                  if ( szPrint[ 0 ] )
                     pZSubtask->m_pZPrintout->m_pDC->DrawText( szPrint,
                                                               zstrlen( szPrint ),
                                                               rect, uFormat );
#ifdef DEBUG_ALL
                  delete [] pchTemp;
               ...
               zstrcpy( szPrint2 + lLth2, szPrint );
         ...
         *plNewBottom256ths = ((zLONG) (lCurrentY + lLineHeight) * 256) /
                                  pZSubtask->m_pZPrintout->m_nPixelsPerInchY;
         if ( bDone )
         {
         ...
         for ( k = 0; k < 4; k++ )
         {
            ...
            // if ( k == 4 )  // right
               ...
               if ( pZSubtask->m_pZPrintout->m_bPhantom == FALSE )
               {
                  ...
                  CPen *pPen = new CPen( nPenType, (zSHORT) size.cy,
                                         lBorderColor );
                  CPen *pPenOld =
                        pZSubtask->m_pZPrintout->m_pDC->SelectObject( pPen );
                  CBrush *pBrushOld = (CBrush *) pZSubtask->m_pZPrintout->
                                    m_pDC->SelectStockObject( HOLLOW_BRUSH );
                  pZSubtask->m_pZPrintout->m_pDC->MoveTo( lLinePosX,
                                                          lLinePosY );
                  pZSubtask->m_pZPrintout->m_pDC->LineTo( lLineEndX,
                                                          lLineEndY );
                       ...
                       zLONG   lBackgroundColor )
{
   ...
   //       TraceLineI( "Drawing a DIAMOND: ", lRounded );
            pZSubtask->m_pZPrintout->m_pDC->MoveTo( (rect.left + rect.right) / 2,
                                                    rect.top );
            pZSubtask->m_pZPrintout->m_pDC->LineTo( rect.right,
                                                    (rect.top + rect.bottom) / 2 );
            pZSubtask->m_pZPrintout->m_pDC->LineTo( (rect.left + rect.right) / 2,
                                                    rect.bottom );
            pZSubtask->m_pZPrintout->m_pDC->LineTo( rect.left,
                                                    (rect.top + rect.bottom) / 2 );
            pZSubtask->m_pZPrintout->m_pDC->LineTo( (rect.left + rect.right) / 2,
                                                    rect.top );
         }
         ...
         {
            pZSubtask->m_pZPrintout->m_pDC->RoundRect( rect,
                                                       CPoint( lRounded,
                                                               lRounded ) );
         }
                    ...
                    zCPCHAR cpcBitmapCtrlBOI )
{
   ...
   // TraceRect( "DrawBitmapToReport: ", rect );
      ...
      {
         GetVEAPX_Reference( vSubtask, vAppOI, pchBitmapName + 1,
         ...
         if ( *szFileName )
         {
            ...
            if ( pchExt &&
                 (zstricmp( pchExt + 1, "jpg" ) == 0 ||
                  zstricmp( pchExt + 1, "jpeg" ) == 0) )
            {
               UINT nWidth;
               UINT nHeight;

               pBmp = LoadBitmapFromJPG( szFileName,
                                         &nWidth, &nHeight );
            // TraceRect( "DrawJPeg: ", rect );
               ZJpegFile::DrawIt( *(pZSubtask->m_pZPrintout->m_pDC),
                                  rect, pBmp, nWidth, nHeight, TRUE );
               delete [] pBmp;
               nRC = 0;
            }
         }
      ...
      if ( pchBitmapName && *pchBitmapName )   // DLL_Name
      {
         ...
         {
            hLibrary = SysLoadLibraryWithErrFlag( vSubtask, pchBitmapName,
                                                  zLOADLIB_RESOURCES );
      ...
      do    // purist's goto
      {
         if ( hInstDLL )
         {
                 ...
                 (uResId = (zUSHORT) zatol( pchBitmapName )) != 0 )
            {
               ...
               {
                  TraceLineI( "DrawBitmapToReport: Cannot access bitmap "
                              "res id ", uResId );
               }
            }
            ...
            if ( pZSubtask == 0 || pZSubtask->m_pZTask->m_nTraceAction > 2 )
            {
               TraceLineS( "DrawBitmapToReport: Cannot access bitmap "
                           "resource ", pchSemiColon );
            }
         }
         ...
         if ( pchSemiColon )
         {
                 ...
                 pchSemiColon[ 0 ] && pchSemiColon[ 1 ] != ':' )
            {
               ...
               if ( SfGetApplicationForSubtask( &pApp, vSubtask ) == 0 &&
                    pApp )
               {
                  zstrcpy( szZeidonPath, pApp->szLocalDir );

                  if ( szZeidonPath[ 0 ] )
                  {
                     SysAppendcDirSep( szZeidonPath );
                     zstrcat( szZeidonPath, pchSemiColon );
                     ...
                     {
                        TraceLineS( "DrawBitmapToReport: Cannot access bitmap "
                                    "file in Application Dir ", szZeidonPath );
                     }
                  }
               }

               SysReadZeidonIni( -1, "[Workstation]",
                                 "ResourcePath", szZeidonPath );
               if ( szZeidonPath[ 0 ] )
               {
                  SysAppendcDirSep( szZeidonPath );
                  zstrcat( szZeidonPath, pchSemiColon );
                  ...
                  {
                     TraceLineS( "DrawBitmapToReport: Cannot access bitmap "
                                 "file in ResourcePath ", szZeidonPath );
                  }
               }
      ...
      if ( dib.m_bBitmapLoaded )
      {
         if ( pZSubtask->m_pZPrintout->m_bPhantom == FALSE )
         {
            if ( dib.Draw( *(pZSubtask->m_pZPrintout->m_pDC), &rect, 0,
                           FALSE, 0, TRUE ) == TRUE )
            {
               return( 0 );
            }
...
//    TraceLineX( "m_pDC from SelectFontForReport = ",
//                (zLONG) pZSubtask->m_pZPrintout->m_pDC );
      ...
      if ( cpcFontTag )
      {
         pZSubtask->m_pZPrintout->m_pFontCurr =
                  pZSubtask->m_pZPrintout->LocateFont( cpcFontTag );
         if ( pZSubtask->m_pZPrintout->m_pFontCurr )
            return( 0 );
         else
            return( -3 );
              ...
              zCPCHAR cpcText )
{
        ...
        pZSubtask->m_pZPrintout->m_pDC->m_hDC )
   {
      ...
      GetTextExtentPoint32( pZSubtask->m_pZPrintout->m_pDC->m_hDC, cpcText,
                            zstrlen( cpcText ), &sizeExtent );
                      ...
                      zVIEW  vSourceOI )
{
   ...
   if ( *cpcCodeName )
   {
      ...
      GetAddrForAttribute( (zPCHAR *) &pchDLL_Name,
                           vReportDef, "Report", "wDLL_Name" );
      if ( *pchDLL_Name )
      {
         zSHORT nIdx;

         GetAddrForAttribute( (zPCHAR *) &pchGrpSetName,
                              vReportDef, "GroupSet", "Tag" );
         GetAddrForAttribute( (zPCHAR *) &pchGroupName,
                              vReportDef, "Group", "Tag" );
         ...
         lpfnDynRoutine = (zFARPROC_REPORT)
              GetOperationDynamicCallAddress( vSubtask,
                                              (LPLPLIBRARY) &(hLibrary),
                                              pchDLL_Name, cpcCodeName, 0 );
         ...
         if ( lpfnDynRoutine )
         {
            ...
            {
               if ( pZSubtask == 0 ||
                    (pZSubtask->m_pZTask->m_nTraceAction & 0x70) == 0x70 )
               {
                  bTrace = TRUE;
               }
            }
            ...
            nRC = (*lpfnDynRoutine)( vSubtask, vSourceOI,
                                     pchGrpSetName, pchGroupName,
                                     nEvent, nPageNbr );
            if ( bTrace )
            {
               TraceLine( "<==rptExit [DLL:%s]%s returned: %d",
                          pchDLL_Name, cpcCodeName, nRC );
            }
         }
         else
         {
                    ...
                    zPCHAR   szControl )
{
   ...
   if ( SetCursorFirstEntityByString( vReport, "GroupSet", "Tag",
                                      szGroupSet, "Report" ) >= zCURSOR_SET )
   {
      if ( SetCursorFirstEntityByString( vReport, "Group", "Tag",
                                         szGroup, "" ) >= zCURSOR_SET )
      {
         if ( szControl[ 0 ] != '*' )
         {
            if ( SetCursorFirstEntityByString( vReport, "Control", "Tag",
                                               szControl, "" ) >= zCURSOR_SET )
            {
               GetIntegerFromAttribute( &lRC, vReport,
                                        "Control", "wPrintStatus" );
            }
         }
         else
         {
            ...
            {
               GetStringFromAttributeByContext( szCtrl, vReport, "Control",
                                                "Tag", "", 256 );
               GetIntegerFromAttribute( &lStatus, vReport,
                                        "Control", "wPrintStatus" );
               GetStringFromAttributeByContext( szText, vReport, "Control",
                                                "wLastPrintText", "", 256 );
               GetIntegerFromAttribute( &lPageNbr, vReport,
                                        "Control", "wLastPrintPage" );
               GetIntegerFromAttribute( &lPageX, vReport,
                                        "Control", "wLastPrintPosX" );
               GetIntegerFromAttribute( &lPageY, vReport,
                                        "Control", "wLastPrintPosY" );
               zsprintf( szMsg, "RPT CTL Dump: %-32s - Status=%d; Page=%d; "
                                "X=%d; Y=%d; Text=%s",
                                      ...
                                      szGroupSet, "Report" ) >= zCURSOR_SET )
   {
                                         ...
                                         szGroup, "" ) >= zCURSOR_SET )
      {
         if ( SetCursorFirstEntityByString( vReport, "Control", "Tag",
                                            szControl, "" ) >= zCURSOR_SET )
         {
            GetIntegerFromAttribute( plPageNbr, vReport,
                                     "Control", "wLastPrintPage" );
            GetIntegerFromAttribute( plPageX, vReport,
                                     "Control", "wLastPrintPosX" );
            GetIntegerFromAttribute( plPageY, vReport,
                                     "Control", "wLastPrintPosY" );
            lRC = 0;
         }
      }
                       ...
                       zPCHAR  pchEntityName )
{
   ...
   *pchEntityName = 0;
   if ( GetViewByName( &vReportDef, "TZ_ReportDef", vSubtask, zLEVEL_TASK ) > 0 )
   {
      GetStringFromAttribute( pchEntityName, vReportDef,
                              "Report", "CurrentEntityName" );
   }
...
GetContinuedGroupFlag( zVIEW  vSubtask )
{
   ...
   if ( GetViewByName( &vReportDef, "TZ_ReportDef", vSubtask, zLEVEL_TASK ) > 0 )
   {
      GetIntegerFromAttribute( &lContinuedGroupFlag, vReportDef,
                               "Report", "ContinuedGroupFlag" );
   }
