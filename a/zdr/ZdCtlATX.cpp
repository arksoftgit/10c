                        ...
                        zPCHAR pchValue, zSHORT nMaxLth )
{
   ...
   if ( pEventItem )
   {
      ...
      {
         if ( nMaxLth > 0 )
            zstrncat( pchValue, pParmItem->m_csValue, nMaxLth - 1 );

         return( zstrlen( pchValue ) );
      }
...
// TraceLineI( "ZActiveX::OnCmdMsg Code: ", nCode );
   ...
   {
      zCHAR szMsg[ 256 ];
      zsprintf( szMsg, "nCode %d is no Event", nCode );
      TraceLineS( szMsg, "" );
   }
...
// }
   ...
   while ( nPos-- )
   {
      ...
      if ( zstrcmp( pMapping->m_csTag, cpcTag ) == 0 )
         return( pMapping );
   }
                          ...
                          zLEVEL_ANY ) > 0 )
      {
         ...
         while ( nRC == zCURSOR_SET )
         {
            if ( lMappingFlag == zGET_DATA_FROM_CONTROL )  // Get the Data
            {
               ...
               rowData = pDispRow->GetValue( ); // These Line retrieve the
                                                // Data from the ActiveX-Control
            }
         ...
         // CtrlMapDef.CheckByte = 0xFE;
            ...
            nRC = SetCursorFirstEntity( pzActiveX->m_pZSubtask->m_vDialog,
                                        szlCtrlMap, 0 );
            while ( nRC == zCURSOR_SET &&
                    nActiveXColCounter < pDispColSet->GetCount( ) )
            {
               GetStructFromEntityAttrs( (zPCHAR) CtrlMapDef.Tag,
                                         pzActiveX->m_pZSubtask->m_vDialog,
                                         szlCtrlMap );
