           ...
           TZPainterCtrl *pCtrl )
{
   ...
   zstrcpy( szMessage, pchText );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " Tag:" );
   nLth += 5;
   zstrcpy( szMessage + nLth, pCtrl->m_csTag );
   if ( pCtrl->m_pCtrlParent )
   {
      zstrcat( szMessage + nLth, " Parent:" );
      zstrcat( szMessage + nLth, pCtrl->m_pCtrlParent->m_csTag );
   }

   zstrcat( szMessage + nLth, " Type:" );
   nLth = zstrlen( szMessage );
   zltoa( pCtrl->m_lType, szMessage + nLth );

   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, "  X:" );
   zltoa( pCtrl->m_rectPainter.left, szMessage + nLth + 4 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " Y:" );
   zltoa( pCtrl->m_rectPainter.top, szMessage + nLth + 3 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " W:" );
   zltoa( pCtrl->m_rectPainter.right - pCtrl->m_rectPainter.left,
          szMessage + nLth + 3 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " H:" );
   zltoa( pCtrl->m_rectPainter.bottom - pCtrl->m_rectPainter.top,
          szMessage + nLth + 3 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, "  Ctrl X:" );
   zltoa( pCtrl->m_rectCtrl.left, szMessage + nLth + 9 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " Y:" );
   zltoa( pCtrl->m_rectCtrl.top, szMessage + nLth + 3 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " W:" );
   zltoa( pCtrl->m_rectCtrl.right - pCtrl->m_rectCtrl.left,
          szMessage + nLth + 3 );
   nLth = zstrlen( szMessage );
   zstrcpy( szMessage + nLth, " H:" );
// zltoa( pCtrl->m_rectCtrl.bottom - pCtrl->m_rectCtrl.top,
//        szMessage + nLth + 3 );
...
//
// There are several states that need to be addressed during button down
// processing:
//  - If a ctrl type has been selected for painting, let the PainterClient
//    handle the message.
//  - If the Shift Key is pressed, the user is drawing a selection rect.
//    all selected ctrls are cloned.
...
//  - Otherwise, if the Ctrl Key is pressed, this ctrl's selection state
//    is toggled.
...
TZCtrlCover::OnPaint( )
{
   ...
   if ( m_pCtrl )
   {
      ...
      zsprintf( szMsg, "TZCtrlCover::OnPaint: %s ====>", m_pCtrl->m_csTag );
...
//    dc.SetBkMode( nOldBkMode );
   ...
   if ( m_pPainterWindow->m_pBar->m_fState & zTZCB_SHOWTABS )
   {
      ...
      if ( m_pCtrl->m_nTabStopNbr > 0 && m_pCtrl->m_nTabStopNbr < 9999 )
         zltoa( m_pCtrl->m_nTabStopNbr, szTab );
      else
         szTab[ 0 ] = '0', szTab[ 1 ] = 0;
...
//    TraceLineS( "TZCtrlCover::MoveCtrl ParentReal: ", pNewParent->m_csTag );
   ...
   if ( nIdNbr )
   {
      ...
      {
         if ( cpcLabel )
            zstrcpy( szLabel, cpcLabel );
         else
            zsprintf( szLabel, "&xxdo Size %d item%s\tCtrl+x",
      ...
      else
      {
         if ( cpcLabel )
            ...
            zsprintf( szLabel, "&xxdo Move %d item%s\tCtrl+x",
                      nSelectCnt, nSelectCnt == 1 ? "" : "s" );
                              ...
                              zLONG lMultY, zLONG lDivY )
{
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      GetStringFromAttribute( szTag, vDialog, szlControl, "Tag" );
         ...
         for ( k = 0; k < lCnt; k++ )
         {
            if ( *(pPainterBar->m_pZMIXWebCtrl->m_pvNode + k) )
            {
               ...
               if ( pWebCtrl->m_lType == zGRID_CONTROL )
               {
                  ...
                  {
                     zstrcpy( pchListCtrlTag, pWebCtrl->m_csTag );
                     *plHeight = pWebCtrl->m_rect.Height( );
                     *plWidth = pWebCtrl->m_rect.Width( );
                     return( 1 );
                  }

                  if ( zstrcmp( pchListCtrlTag, pWebCtrl->m_csTag ) == 0 )
                     pchListCtrlTag[ 0 ] = 0;
               }
            }
         }
      ...
      if ( pPainterBar && pPainterBar->m_pZMIXWebCtrl )
      {
         ...
         for ( k = 0; k < lCnt; k++ )
         {
            if ( *(pPainterBar->m_pZMIXWebCtrl->m_pvNode + k) )
            {
               ...
               {
                  if ( zstrcmp( pchFloatCtrlTag, pWebCtrl->m_csTag ) == 0 )
                     pchFloatCtrlTag[ 0 ] = 0;
               }
               else
               {
...
#if 1
                  if ( m_pName && zstrcmp( cpcListCtrlTag, m_pName->m_csName ) == 0 )
                  {
                     zstrcpy( pchFloatCtrlTag, pWebCtrl->m_csTag );
                     if ( bVertical )
