...
public:
   zLONG       m_lLevel;
   zLONG       m_lFooterPos;
   zCHAR       m_szEntityName[ 33 ];
   zCHAR       m_szFooterEntityName[ 33 ];
   ZLevelItem  *m_pNext;
};
...
public:
   ...
   zCHAR       m_szEntityName[ 33 ];
   zCHAR       m_szRptGroupSetTag[ 33 ];
   zCHAR       m_szRptGroupTag[ 33 ];
   ZPageItem   *m_pNext;
...
// SetBanding( m_bBanding );
   ...
   if ( m_bPrintToCSV || m_bPrintToHTML )
   {
      ...
      {
         SysMakeWebFileName( szFileName, m_vSubtask, 2 );
      }
      else
      {
         if ( lPrintFlags & zREPORT_USEFOLDER )
            zstrcpy( szFileName, pchReportInfo );
         ...
         if ( lPrintFlags & zREPORT_CREATENAME )
         {
            ...
            zsprintf( szName, "0x%08x", pZSubtask->m_vDialog );
            zstrcat( szFileName, szName );
         }
         else
            zstrcat( szFileName, pchReportInfo );
      }
      ...
      if ( zstrchr( szFileName, '/' ) == 0 && zstrchr( szFileName, '\\' ) == 0 )
      {
         ...
         if ( lpApp )
         {
            ...
            zstrcpy( szTempName, szFileName );
            zstrcpy( szFileName, lpApp->szLocalDir );
      ...
      if ( m_bPrintToCSV )
      {
         zstrcat( szFileName, ".csv" );
         TraceLineS( "Creating CSV File: ", szFileName );
      }
      else
      {
         zstrcat( szFileName, ".html" );
...
// m_nMapMode = MM_ANISOTROPIC;  // Make printout fill the page
   ...
   m_pchDLL_Name = new char[ zstrlen( cpcDLL_Name ) + 1 ];
   zstrcpy( m_pchDLL_Name, cpcDLL_Name );
   m_pchPrintPageFunction = new char[ zstrlen( cpcPrintPageFunction ) + 1 ];
   zstrcpy( m_pchPrintPageFunction, cpcPrintPageFunction );
                       ...
                       zCPCHAR cpcFaceName )
{
   ...
   while ( pItem )
   {
      ...
      if ( pFont->m_nPointSize == nPtSize &&
           ...
           zstrcmp( pFont->m_csFaceName, cpcFaceName ) == 0 )
      {
         return( pFont );
      }
...
// {\f28\fnil\fcharset0\fprq2{\*\panose 00000000000000000000}PT Cozy Outline;}
   ...
   if ( m_nFontCnt > 0 )
   {
      ...
      zstrcpy( pchFontList + nLth, "{\\fonttbl" );
      nLth += 9;
      ...
      while ( pItem && nLth < lMaxLth - 1024 )
      {
         ...
         // zSHORT j = nLth;
            nFontCnt++;
            zstrcpy( pchFontList + nLth, "\r\n{\\f" );
            nLth += 5;
            zltoa( pFont->m_nIdNbr, pchFontList + nLth );
            nLth += zstrlen( pchFontList + nLth );
            zstrcpy( pchFontList + nLth, "\\f" );
            nLth += 2;
            switch ( pFont->m_byPitchFamily >> 4 )        // family
            {
               case FF_DECORATIVE:
                  zstrcpy( pchFontList + nLth, "decorative" );
                  nLth += sizeof( "decorative" );
                  break;

               case FF_MODERN:
                  zstrcpy( pchFontList + nLth, "modern" );
                  nLth += sizeof( "modern" );
                  break;

               case FF_ROMAN:
                  zstrcpy( pchFontList + nLth, "roman" );
                  nLth += sizeof( "roman" );
                  break;

               case FF_SCRIPT:
                  zstrcpy( pchFontList + nLth, "script" );
                  nLth += sizeof( "script" );
                  break;

               case FF_SWISS:
                  zstrcpy( pchFontList + nLth, "swiss" );
                  nLth += sizeof( "swiss" );
                  break;

               case FF_DONTCARE:
               default:
                  zstrcpy( pchFontList + nLth, "nil" );
                  nLth += sizeof( "nil" );
                  break;
            }

            zstrcpy( pchFontList + nLth - 1, "\\fcharset" ); // charset
            nLth += 8;
            zltoa( pFont->m_byCharset, pchFontList + nLth );
            nLth += zstrlen( pchFontList + nLth );
            zstrcpy( pchFontList + nLth, "\\fprq" );     // pitch
            nLth += 5;
            zltoa( (pFont->m_byPitchFamily & 0x0F), pchFontList + nLth );
            nLth += zstrlen( pchFontList + nLth );
            zstrcpy( pchFontList + nLth, "{\\*\\panose " );
            nLth += 11;
#if 0
            zsprintf( pchFontList + nLth,
            ...
            nLth++;
#endif
            zstrcpy( pchFontList + nLth, pFont->m_csFaceName );
            nLth += zstrlen( pchFontList + nLth );
            zstrcpy( pchFontList + nLth, ";}" );
            nLth += 2;
      ...
      while ( pItem && nLth < lMaxLth - 100 )
      {
         ...
         cr = pItem->m_lNumber;
         zstrcpy( pchFontList + nLth, "\\red" );
         nLth += 4;
         lColor = RGB_GETRED( cr );
         zltoa( lColor, pchFontList + nLth );
         nLth += zstrlen( pchFontList + nLth );

         zstrcpy( pchFontList + nLth, "\\green" );
         nLth += 6;
         lColor = RGB_GETGREEN( cr );
         zltoa( lColor, pchFontList + nLth );
         nLth += zstrlen( pchFontList + nLth );

         zstrcpy( pchFontList + nLth, "\\blue" );
         nLth += 5;
   ...
   if ( lPageNbr && (lFlags & 0x40000000) )  // set up "PageNumber"
   {
      if ( *plTotalPages >= 0 )
      {
         ...
         SysReadZeidonIni( -1, "[Workstation]",
                           "TotalPageSeparator", szTotalPageSeparator );
         if ( szTotalPageSeparator[ 0 ] == 0 )
            zstrcpy( szTotalPageSeparator, "of" );
         ...
         while ( pResetPagingItem )
         {
            if ( pResetPagingItem->m_lValue2 == lPageNbr )
            {
               ...
               *plTotalPages = lPageNbr;
               while ( pResetPagingItem->m_pNextItem &&
                       pResetPagingItem->m_lValue1 ==
                                      pResetPagingItem->m_pNextItem->m_lValue1 )
               {
         ...
         zsprintf( pchPageNumber, "%d %s %d", lPageNbr,
                   szTotalPageSeparator, *plTotalPages );
      }
      else
      {
         zltoa( lPageNbr, pchPageNumber );
      }
                            ...
                            zCPCHAR cpcRptGroupTag )
{
                                     ...
                                     "FooterAtLevel", 0 )) >= zCURSOR_SET )
   {
      while ( nRC >= zCURSOR_SET )
      {
         ...
         GetIntegerFromAttribute( &lCurrLevel, vReportDef,
                                 "FooterAtLevel", "Level" );
         GetStringFromAttribute( szEntityName, vReportDef,
   ...
   if ( CheckExistenceOfEntity( vReportDef, "FooterAtLevel" ) &&
        (nRC = SetCursorFirstEntity( vReportDef,
                                     "LastEntityAtLevel", 0 )) >= zCURSOR_SET )
   {
      while ( nRC >= zCURSOR_SET )
      {
         GetIntegerFromAttribute( &lCurrLevel, vReportDef,
                                 "LastEntityAtLevel", "Level" );
         GetStringFromAttribute( szEntityName, vReportDef,
                                 "LastEntityAtLevel", "EntityName" );
         pLevelItem = new ZLevelItem( lCurrLevel, szEntityName );
   ...
   while ( pLevelItem )
   {
      ...
      SetAttributeFromInteger( vReportDef, "FooterAtLevel",
                               "Level", pLevelItem->m_lLevel );
      SetAttributeFromString( vReportDef, "FooterAtLevel",
                              "EntityName", pLevelItem->m_szEntityName );
   ...
   while ( pLevelItem )
   {
      ...
      SetAttributeFromInteger( vReportDef, "LastEntityAtLevel",
                               "Level", pLevelItem->m_lLevel );
      SetAttributeFromString( vReportDef, "LastEntityAtLevel",
                              "EntityName", pLevelItem->m_szEntityName );
      pLevelItem = pLevelItem->m_pNext;
   }
#endif
...
// TraceLineI( "GetPagePosition looking for page: ", lPageNbr );
   ...
   if ( pPage )
   {
      ...
      *plContinuedGroupFlag = pPage->m_lContinuedGroupFlag;
      zstrcpy( pchEntityName, pPage->m_szEntityName );
      ...
      while ( pLevelItem )
      {
         ...
         {
            CreateViewFromViewForTask( &vFooterHierOI, vSourceHierOI,
                                       m_pZSubtask->m_vDialog );
            SetCursorAbsolutePosition( pLevelItem->m_szFooterEntityName,
                                       vFooterHierOI,
                                       pLevelItem->m_lFooterPos );
            SetAttributeFromInteger( vReportDef, "FooterAtLevel",
                                     "vSourceOI_Hier", (zLONG) vFooterHierOI );
         }
...
{
   zstrcpy( m_szCurrentEntityName, cpcCurrentEntityName );
                        ...
                        zPLONG  plViewPortRight )
{
   ...
   if ( m_pViewPortHead )
   {
      ...
      zstrcpy( pchCurrentEntityName, pViewPort->m_szCurrentEntityName );
                      ...
                      zCPCHAR cpcRptGroupSetTag, zCPCHAR cpcRptGroupTag )
{
   ...
   zstrcpy( m_szEntityName, cpcEntityName );
   zstrcpy( m_szRptGroupSetTag, cpcRptGroupSetTag );
   zstrcpy( m_szRptGroupTag, cpcRptGroupTag );
   m_pNext = 0;
   m_pFooterHead = 0;
   m_pLastEntityHead = 0;
}
                        ...
                        zLONG lFooterPos, zCPCHAR cpcFooterEntityName )
{
   ...
   if ( cpcFooterEntityName && *cpcFooterEntityName )
      zstrcpy( m_szFooterEntityName, m_szFooterEntityName );
   else
                   ...
                   zCPCHAR cpcReportDefName )
{
   ...
   if ( GetViewByName( &vAnimator, "__TZVSXCAD", vSubtask, zLEVEL_TASK ) < 0 )
   {
      ...
      if ( lpApp )
         zstrcpy( szFileName, lpApp->szObjectDir );
   }
   else
   {
   ...
   zstrcat( szFileName, cpcReportDefName );
   zstrcat( szFileName, ".XRP" );
   zstrlower( szFileName );
...
// CWaitCursor wait;
   ...
   if ( szDeviceFileName[ 0 ] )
   {
      zsprintf( szReportFileName, szDeviceFileName, m_csReportInfo );
   ...
   */
   ...
   // Set up document title.
   // Get the reportname.
   zsprintf( szReportFileName, m_csReportInfo );
                 ...
                 zLONG   lType )
{
        ...
        pZSubtask->m_pZPrintout->m_lFileCSV != -1 )
   {
      ...
      {
         zLONG lTextLth = zstrlen( cpcText );
         zLONG lLth;
      ...
      // POSITION:absolute;LEFT:8px;TOP:20px;">Church</span>
         ...
         zsprintf( pch, "position:absolute;left:%dpx;top:%dpx;", lPixelsLeft, lPixelsTop );
         CString csPosition( pch );

         zsprintf( pch, "width:%dpx; height:%dpx;", lPixelsWidth, lPixelsHeight );
         CString csSize( pch );
         ...
         if ( pZSubtask->m_pZPrintout->m_pFontCurr )
         {
                 ...
                 pZSubtask->m_pZPrintout->m_pFontCurr->m_clrText != 0 )
            {
               ...
               zsprintf( pch, "color:#%.2x%.2x%.2x;", byRed, byGreen, byBlue );
               csFont += pch;
            }
            ...
            {
               zsprintf( pch, "font-size:%dpt;", pZSubtask->m_pZPrintout->m_pFontCurr->m_nPointSize / 16 );
               csFont += pch;
            }
   ...
   // <hr style="width:190px;height:3px;POSITION:absolute;LEFT:25px;TOP:101px;" />
           ...
           pZSubtask->m_pZPrintout->m_pFontCurr->m_clrText != 0 )
      {
         ...
         zsprintf( szText,
                   ...
                   csPenType, byRed, byGreen, byBlue,
      ...
      else
      {
         zsprintf( szText,
                   "<hr %s style=\"width:%dpx;height:%dpx;"
                     "POSITION:absolute;LEFT:%dpx;TOP:%dpx;\" />",
                   csPenType, lPixelsWidth, nPenWidth, lPixelsLeft, lPixelsTop );
      }

      SysWriteFile( vSubtask, pZSubtask->m_pZPrintout->m_lFileCSV,
                    szText, zstrlen( szText ) );
      return( 0 );
   }
                   ...
                   zCPCHAR cpcBitmapCtrlBOI )
{
        ...
        pZSubtask->m_pZPrintout->m_lFileCSV != -1 )
   {
      ...
      SysReadZeidonIni( -1, "[Workstation]",
                        "ResourcePath", szBitmapFileName );
      if ( szBitmapFileName[ 0 ] )
         SysAppendcDirSep( szBitmapFileName );

      zstrcat( szBitmapFileName, pchBitmapName );
      TraceLineS( "PrintBitmapToHTML File Name: ", szBitmapFileName );
      zsprintf( szText,
                "<span style=\"width:%dpx; height:%dpx;"
                  "POSITION:absolute;LEFT:%dpx;TOP:%dpx;\"><img border=0 hspace=6 width=%dpx height=%dpx src=\"%s\"></span>",
                lPixelsWidth, lPixelsHeight, lPixelsLeft, lPixelsTop,
                lPixelsWidth, lPixelsHeight, szBitmapFileName );
      SysWriteFile( vSubtask, pZSubtask->m_pZPrintout->m_lFileCSV,
                    szText, zstrlen( szText ) );
      return( 0 );
   }
                        ...
                        zCPCHAR cpcExtension )
{
   GUID my_guid;
   zCHAR pszGuid[ 40 ];
   zSHORT nLth = zstrlen( cpcPathName );

   ::CoCreateGuid( &my_guid );

   zsprintf( pszGuid, "%08lx-%04x-%04x-%02x%02x-%02x%02x%02x%02x%02x%02x",
   ...
   nMaxNameLth -= (40 + zstrlen( cpcExtension ));

   if ( nLth + (zSHORT) zstrlen( cpcFileName ) < nMaxNameLth )
   {
      zstrcpy( pchReturnFileName, cpcPathName );
      ...
      zstrcpy( pchReturnFileName + nLth, cpcFileName );
      zstrcat( pchReturnFileName, pszGuid );
      if ( cpcExtension && *cpcExtension )
      {
         zstrcat( pchReturnFileName, "." );
         zstrcat( pchReturnFileName, cpcExtension );
      }
                              ...
                              zCPCHAR cpcExtension )
{
   ...
   GetAppOrWorkstationValue( vSubtask, "ExecutableRemoteDocPath",
                             szDocPath, sizeof( szDocPath ) );
   nLth = zstrlen( szDocPath );
              ...
              const int  iTextMode )
{
   ...
   hInternetSession = InternetOpenA( "ZeidonWeb",
                           ...
                           zULONG  ulControlFlag )
{
   ...
   if ( ((ulControlFlag & 0x00000010) && (cpcURL == 0 || *cpcURL == 0)) ||
        ((ulControlFlag & 0x00000010) == 0 &&
         (cpcFileName == 0 || *cpcFileName == 0)) )
   {
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
                                    ...
                                    "StartDocumentOnClient" )) != 0 )
      {
         ...
         {
            // Get Doc URL name.
            GetAppOrWorkstationValue( vSubtask, "FtpHostName",
                                      szFtpHostName, sizeof( szFtpHostName ) );
            GetAppOrWorkstationValue( vSubtask, "FtpUserName",
                                      szFtpUserName, sizeof( szFtpUserName ) );
            GetAppOrWorkstationValue( vSubtask, "FtpPassword",
                                      szFtpPassword, sizeof( szFtpPassword ) );
            GetAppOrWorkstationValue( vSubtask, "FtpPort",
                                      szFtpPort, sizeof( szFtpPort ) );
            GetAppOrWorkstationValue( vSubtask, "ExecutableRemoteDocURL",
                                      szDocURL, sizeof( szDocURL ) );
            nLth = zstrlen( szDocURL );
            ...
            zstrcpy( pchFileName, cpcFileName );
            cs = cpcFileName;
            cs.MakeLower( );
            ...
            if ( pchReturnFileName && nMaxNameLth > 0 )
               zstrncat( pchReturnFileName, szReportFileName, nMaxNameLth );
         }
         else
         {
            cpcTag = cpcURL;
            zstrcpy( szDocURL, cpcURL );
         }
         ...
         CreateViewFromViewForTask( &vXRA, pXRA->m_vXRA_Delta,
                                    m_pZSubtask->m_vDialog );
...
GetDefaultPrinterName( zPCHAR pchPrinterName, zPLONG plBufferSize )
{
   ...
   if ( osv.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS )
   {
      ...
      bFlag = EnumPrinters( PRINTER_ENUM_DEFAULT, 0, 2,
      ...
      zstrcpy( pchPrinterName, ppi2->pPrinterName );
   ...
   if ( osv.dwPlatformId == VER_PLATFORM_WIN32_NT )
   {
...
#endif
      {
         ...
         // Copy printer name into passed-in buffer...
         zstrcpy( pchPrinterName, cBuffer );
...
SetDefaultPrinterName( zPCHAR pchPrinterName )
{
   ...
   if ( osv.dwPlatformId == VER_PLATFORM_WIN32_NT )
   {
...
#endif
      {
         ...
         // String will be in form "printername,drivername,portname"...
         pBuffer = (zPCHAR) GlobalAlloc( GPTR,
                                         zstrlen( pchPrinterName ) +
                                         zstrlen( ppi2->pDriverName ) +
                                         zstrlen( ppi2->pPortName ) + 3 );
         if ( pBuffer == 0 )
         {
            ClosePrinter( hPrinter );
            GlobalFree( ppi2 );
         ...
         zstrcpy( pBuffer, pchPrinterName );
         zstrcat( pBuffer, "," );
         zstrcat( pBuffer, ppi2->pDriverName );
         zstrcat( pBuffer, "," );
         zstrcat( pBuffer, ppi2->pPortName );
                  ...
                  HGLOBAL *phDevMode )
{
   ...
   *phDevNames = 0;
   ...
   if ( GetPrinter( hPrinter, 2, (LPBYTE) p2,
                    dwBytesNeeded,
                    &dwBytesReturned ) == 0 )
   {
      GlobalFree( p2 );
   ...
   zmemcpy( pDevMode, p2->pDevMode,
            sizeof( *(p2->pDevMode) ) + p2->pDevMode->dmDriverExtra );
   GlobalUnlock( hDevMode );

   // Compute size of DEVNAMES structure from PRINTER_INFO_2's data.
   zUSHORT uDriverNameLth = zstrlen( p2->pDriverName ) + 1;     // driver name
   zUSHORT uPrinterNameLth = zstrlen( p2->pPrinterName ) + 1;   // printer name
   zUSHORT uPortNameLth = zstrlen( p2->pPortName ) + 1;         // port name

   // Allocate a global handle big enough to hold DEVNAMES.
   HGLOBAL hDevNames = GlobalAlloc( GHND,
                                    sizeof( DEVNAMES ) +
                                      (uDriverNameLth + uPrinterNameLth +
                                       uPortNameLth) * sizeof( TCHAR ) );
   ASSERT( hDevNames );
   ...
   zmemcpy( (zPCHAR) pDevNames + uOffset,
            p2->pDriverName,
            uDriverNameLth * sizeof( TCHAR ) );
   uOffset += uDriverNameLth;

   pDevNames->wDeviceOffset = uOffset;
   zmemcpy( (zPCHAR) pDevNames + uOffset,
            p2->pPrinterName,
            uPrinterNameLth * sizeof( TCHAR ) );
   uOffset += uPrinterNameLth;

   pDevNames->wOutputOffset = uOffset;
   zmemcpy( (LPTSTR) pDevNames + uOffset,
            p2->pPortName,
            uPortNameLth*sizeof( TCHAR ) );
