   ...
   if ( vCtrl )
   {
      ...
      if ( ulLth )
      {
         ...
         if ( ulLth >= 8 + (6 * sizeof( zLONG )) )
         {
            ...
            zstrcpy( m_szFaceName, ld.szFontName );
...
TZText::OnPaint( )
{
   ...
   if ( zstrcmp( m_csTag, "Line" ) == 0 )
   {
      ...
      zsprintf( szMsg, "TZText::OnPaint: %s ====>", m_csTag );
...
//             rect.left = (zLONG) (dSin * sizeExtent.cy);
   ...
   //    TraceRect( "  Line Rect", rect );
      ...
      if ( m_chMultiLine )
      {
         zPCHAR pchText = new char[ zstrlen( m_csText ) + 1 ];
         zPCHAR pch = pchText;

         zstrcpy( pchText, m_csText );
...
TZDayTimer::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZDayTimer::OnPaint: %s ====>", m_csTag );
   TraceRect( szMsg, m_rectPainter );
#endif
                       ...
                       lType, bPlaceHolder, lZKey, lTabStopNbr, lCtrlID )
{
   ...
   if ( vCtrl )
   {
      ...
      if ( ulLth > 0 )
      {
         ...
         m_pchMsgText = new char[ zstrlen( pchText ) + 1 ];
         zstrcpy( m_pchMsgText, pchText );
      }
   }
   ...
   if ( pch && *pch )
   {
      ...
      if ( m_hIcon == 0 || m_hIcon == (HICON) 1 )
      {
              ...
              pch[ 0 ] && pch[ 1 ] != ':' )
         {
            ...
            {
               SysConvertEnvironmentString( szZeidonPath, pApp->szLocalDir );
            }
            else
            {
               zPCHAR pchExecDir;
               ...
               SysConvertEnvironmentString( szZeidonPath, pchExecDir );
            }

            if ( szZeidonPath[ 0 ] )
            {
               SysAppendcDirSep( szZeidonPath );
               zstrcat( szZeidonPath, pch );
...
TZIcon::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZIcon::OnPaint: %s ====>", m_csTag );
   TraceRect( szMsg, m_rectPainter );
#endif
...
/////////////////////////////////////////////////////////////////////////////
   ...
   if ( pch && *pch )   // DLL_Name
   {
      ...
      GetStringFromAttribute( szDir, vTaskLPLR, "LPLR", "ExecDir" );
      zULONG  ulLth = zstrlen( szDir );
      if ( ulLth && szDir[ ulLth ] != '\\' )
         szDir[ ulLth++ ] = '\\';

      zstrcpy( szDir + ulLth, pch );
      hLibrary = SysLoadLibraryWithErrFlag( m_vCtrl, szDir,
                                            zLOADLIB_RESOURCES );
      if ( hLibrary == 0 )
      {
         // Try to load the DLL via normal path.
         hLibrary = SysLoadLibraryWithErrFlag( m_vCtrl, pch,
                                               zLOADLIB_RESOURCES );
      }
                   ...
                   zPCHAR    pchBitmap )
{
   ...
   if ( hInstDLL )
   {
           ...
           (uResId = (zUSHORT) zatol( pchBitmap )) != 0 )
      {
         ...
         {
            TraceLineI( "TZBitmap::LoadDib: Cannot access bitmap "
                        "res id ", uResId );
         }
      }
      ...
      {
        TraceLineS( "TZBitmap::LoadDib: Cannot access bitmap "
   ...
   if ( pchSemiColon )
   {
           ...
           pchSemiColon[ 0 ] && pchSemiColon[ 1 ] != ':' )
      {
         ...
         if ( szZeidonPath[ 0 ] )
         {
            ...
            TraceLineS( "TZBitmap::LoadDib: Cannot access bitmap "
                        "file in ResourcePath ", szWorkString );
         }
...
TZComboBox::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZComboBox::OnPaint: %s ====>", m_csTag );
   TraceRect( szMsg, m_rectPainter );
#endif
...
TZGroupBox::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZGroupBox::OnPaint: %s ====>", m_csTag );
   TraceRect( szMsg, m_rectPainter );
#endif
                              ...
                              lType, bPlaceHolder, lZKey, lTabStopNbr, lCtrlID, 'S' )
{
   ...
   if ( vCtrl )
   {
      ...
      GetStringFromAttribute( szTemp, vCtrl, szlGroupSet, szlType );
      if ( szTemp[ 0 ] )
...
TZGroupSetRpt::OnCreate( LPCREATESTRUCT lpCreateStruct )
{
   ...
   MakeShortString( &dc, szTag, sizeof( szTag ) - 1, m_csTag,
                    m_pPainterWindow->m_lLeftMargin - 8, 0 );
   SetWindowText( szTag );
...
TZGroupSetRpt::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZGroupSetRpt::OnPaint: %s ====>", m_csTag );
   TraceRect( szMsg, m_rectPainter );
#endif
                           ...
                           lType, bPlaceHolder, lZKey, lTabStopNbr, lCtrlID, 'G' )
{
   ...
   if ( vCtrl )
   {
      ...
      GetStringFromAttribute( szType, vCtrl, szlGroup, szlType );
...
TZGroupRpt::OnPaint( )
{
   ...
   if ( m_csTag == "PageHeaderOverlay" )
   {
      ...
      zsprintf( szMsg, "TZGroupRpt::OnPaint: %s ====>", m_csTag );
      TraceRect( szMsg, m_rectPainter );
   }
#endif
...
TZShape::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZShape::OnPaint: %s ====>", m_csTag );
   TraceRect( szMsg, m_rectPainter );
#endif
