...
//       pAction = pAction->m_pNextAction;
      ...
      zstrcpy( pAction->m_szAction, cpcNewAction );
      return( 0 );
   }
              ...
              zCPCHAR cpcActionTag )
{
   ...
   if ( cpcActionTag && cpcActionTag[ 0 ] &&
        GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
//    ZAction *pAction = &(pZSubtask->m_Action);
//    while ( pAction->m_pNextAction )
...
//    pAction = pAction->m_pNextAction;
      ...
      zCHAR szAction[ 33 ];
      zstrcpy( szAction, cpcActionTag );  // localize action
      return( (zSHORT) ProcessAction( pZSubtask, szAction,
                                      0, 0, "InvokeAction" ) );
   }
...
// DisplayObjectInstance( vKZXMLPGO, 0, 0 );
   if ( vKZXMLPGO )
   {
      GetStringFromAttribute( pchDlgTag, vKZXMLPGO, "NextDialogWindow", "DialogName" );
      GetStringFromAttribute( pchWndTag, vKZXMLPGO, "NextDialogWindow", "WindowName" );
      GetStringFromAttribute( szFunctionCall, vKZXMLPGO, "NextDialogWindow", "FunctionCall" );

      DeleteEntity( vKZXMLPGO, "NextDialogWindow", zREPOS_LAST );

      if ( zstrcmp( szFunctionCall, "StartSubwindow" ) == 0 )
      {
         zstrcpy( szDlgWnd, pchDlgTag );
         zstrcat( szDlgWnd, pchWndTag );
...
//                 zWAB_StartModelessSubwindow           33
   ...
   if ( vSubtask )
   {
      ...
      if ( lpTask && lpTask->vWebSubtask )
      {
         ...
         if ( cpcDlgTag && cpcDlgTag[ 0 ] )
            zstrcpy( szDlgTag, cpcDlgTag );
         else
            szDlgTag[ 0 ] = 0;

         if ( cpcWndTag && cpcWndTag[ 0 ] )
            zstrcpy( szWndTag, cpcWndTag );
         else
            szWndTag[ 0 ] = 0;
#endif
      ...
      if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
      {
         ...
         ASSERT( ((pAction->m_lActType & ~zWAB_ProcessImmediateReturn) >= 0 &&
                 ...
                 (pAction->m_lActType & ~zWAB_ProcessImmediateReturn) ==
                                                 zWAB_TerminateActionForError );
         if ( cpcDlgTag )
            zstrcpy( pAction->m_szActDlg, cpcDlgTag );
         else
            pAction->m_szActDlg[ 0 ] = 0;

         if ( cpcWndTag )
            zstrcpy( pAction->m_szActWnd, cpcWndTag );
         else
            pAction->m_szActWnd[ 0 ] = 0;

         if ( (lWindowBehavior & zWAB_ProcessImmediateAction) ==
      ...
      else
      {
         ...
         if ( FindGlobalDialogWindow( &fd ) && mIs_hWnd( fd.hWnd ) )
         {
            ...
            if ( SfAllocTaskMemory( vSubtask, (zCOREMEM) &pv,
                                    sizeof( ZFindDialog ) ) == 0 )
            {
               ZFindDialog *pfd = (ZFindDialog *) SfLockTaskMemory( pv );

               if ( cpcDlgTag && *cpcDlgTag )
                  zstrcpy( pfd->DlgN, cpcDlgTag );
               else
                  pfd->DlgN[ 0 ] = 0;

               if ( cpcWndTag && *cpcWndTag )
                  zstrcpy( pfd->WndN, cpcWndTag );
               else
                  pfd->WndN[ 0 ] = 0;
               ...
               zSHORT nRC = (zSHORT) ::SendMessage( fd.hWnd,
                                                    zZEIDON_SET_WAB,
                                                    0, (LPARAM) pv );
              ...
              zSHORT  nEnable )
{
   ...
   // k /= k;
      zsprintf( szMsg, "EnableAction: %s  Enable=%s", cpcActionName,
...
//   zSTART_SHOWNORMAL, zSTART_SHOWMAXIMIZED, ...
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      ...
      zstrcpy( szTempString, cpcCmdLine );
      ...
      if ( *pch && *pch == *(pch + 1) )
      {
         ...
         if ( pchDlg )
         {
            ...
            *pchDlg = 0;
            ...
            szStartTask[ nLth++ ] = '-';
            szStartTask[ nLth++ ] = 'a';
            zstrcpy( szStartTask + nLth, pchApp );
            ...
            zstrcat( szNameExt, pchDlg );
            zstrcat( szNameExt, ".xra" );
                     ...
                     zCPCHAR cpcAppName )
{
   ...
   zstrcpy( szViewName + 2, cpcDlgTag );
                       ...
                       vSubtask, zLEVEL_TASK ) != zLEVEL_TASK )
   {
      ...
      GetApplDirectoryFromView( szNameExt, vNewSubtask,
                                zAPPL_DIR_OBJECT, sizeof( szNameExt ) );
      zstrcat( szNameExt, cpcDlgTag );
      zstrcat( szNameExt, ".xwd" );

      // Activate using SfActivateSysOI_FromFile so that the kzwdlgxo.xod
      // does not have to be in the application LPLR.
      if ( (nRC = SfActivateSysOI_FromFile( &vDialogNew,
                                            szlKZWDLGXO, vSubtask,
                                            szNameExt, zSINGLE )) == 0 )
      {
...
fnResetCtrls( zVIEW vDialogNew, zLONG lX_Factor, zLONG lY_Factor )
{
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      if ( lType == zLISTBOX_CONTROL )
      {
         ...
         while ( nRC == zCURSOR_SET )
         {
            GetIntegerFromAttribute( &lValue, vDialogNew,
                                     "ListBoxColumn", "ColumnWidth" );
            lValue = lValue * g_lDlgUnitCharX / 2;
            SetAttributeFromInteger( vDialogNew, "ListBoxColumn",
                                     "ColumnWidth", lValue );
                           ...
                           zCPCHAR cpcWndTag )
{
                       ...
                       vSubtask, zLEVEL_TASK ) != zLEVEL_TASK )
   {
      ...
      // This original Dialog view will never be used ... it is just here by
      // name so that new (usable) views can be declared/set from this one.
      GetApplDirectoryFromView( szNameExt, vSubtask,
                                zAPPL_DIR_OBJECT, sizeof( szNameExt ) );
      zstrcat( szNameExt, cpcDlgTag );
      zstrcat( szNameExt, ".xra" );
               ...
               zPCHAR pchActionName )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, 0, vSubtask, 0 ) == 0 )
   {
      zstrcpy( pchActionName, pZSubtask->m_szActionName );
      return( 0 );
   }
