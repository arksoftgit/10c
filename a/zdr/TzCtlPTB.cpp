      ...
      case ID_ACTION_BAR_UPDATE:
      {
         if ( (m_fState & (zTZCB_REPORT | zTZCB_XSLT)) == 0 )
         {
            ...
            if ( lpfnDynRoutine )
            {
               ...
               PainterCall( zMSG_UPDATEALLZEIDONWINDOWS,
                            vTZPNTROO, this, m_vSubtask, 0 );
...
TZPainterBar::OnGetToolTipText( WPARAM wParam, LPARAM lParam )
{
   ...
   {
      case ID_ACTION_BAR_UPDATE:
         zstrncpy( pchText, "Build Dialog", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_ALIGN_LEFT:
         zstrncpy( pchText, "Align Left", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_ALIGN_TOP:
         zstrncpy( pchText, "Align Top", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_ALIGN_RIGHT:
         zstrncpy( pchText, "Align Right", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_ALIGN_BOTTOM:
         zstrncpy( pchText, "Align Bottom", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_SIZE_HEIGHT:
         zstrncpy( pchText, "Equal Height", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_SIZE_WIDTH:
         zstrncpy( pchText, "Equal Width", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_SIZE_WIDTH_HEIGHT:
         zstrncpy( pchText, "Equal Width/Height", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_EQUAL_SPACE_VERTICAL:
         zstrncpy( pchText, "Equi-Space Vertical", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_EQUAL_SPACE_HORIZONTAL:
         zstrncpy( pchText, "Equi-Space Horizontal", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_ABUT_HORIZONTAL:
         zstrncpy( pchText, "Abut Horizontal", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_ABUT_VERTICAL:
         zstrncpy( pchText, "Abut Vertical", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_DELETE_SELECTED:
         zstrncpy( pchText, "Delete Selected", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_EQUAL_SIZE_POS_CLASS:
         zstrncpy( pchText, "Equi-Size/Pos/Class Selected Across Windows", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_GROUP_SELECTED:
         zstrncpy( pchText, "Create GroupBox around selected controls", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_UNGROUP_SELECTED:
         zstrncpy( pchText, "Remove GroupBox (leaving controls in place)", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_TOOLBAR_WNDLIST:
         zstrncpy( pchText, "Dialog Window List", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_TOOLBAR_ACTLIST:
         zstrncpy( pchText, "Window Action List", zZEIDON_TOOLTIP_TEXTLTH );
         break;

      case ID_TOOLBAR_OPERLIST:
         zstrncpy( pchText, "Dialog Operation List", zZEIDON_TOOLTIP_TEXTLTH );
         break;
                        ...
                        AFX_CMDHANDLERINFO *pHandlerInfo )
{
   ...
   if ( uId == ID_TOOLBAR_WNDLIST )
   {
      ...
      // TraceLineS( "PainterBar WndList EndOK", "" );
         ...
         {
            GetViewByName( &vTZPNTROO, "TZPNTROO", m_vSubtask, zLEVEL_TASK );
            PainterCall( zMSG_UPDATEWINDOWBYNAME, vTZPNTROO, this,
                         vDialogList, szTag );
         }
...
TZPainterBar::EnablePainterWindows( zBOOL bEnable )
{
   ...
   if ( bEnable && m_pActivePainterWindow )
   {
      ...
      for ( nPos = 0; nPos < m_pActivePainterWindow->m_nCtrlCnt; nPos++ )
      {
         ...
         {
            zsprintf( szMsg, " %3d.%s%-32s %-32s%s0x%04x:", nPos + 1,
                      pCtrl->IsSelected( ) ? "*" : " ",
                      pCtrl->m_pCtrlParent ?
...
// ZActiveX_CtrlList *pActiveX_CtrlList = 0; now a data item of TZPainterBar
   ...
   {
      GetStringFromAttribute( szBitmap, vTZPESRCO,
                              szlControlDef, szlPainterBitmap );
      GetIntegerFromAttribute( &lVisible, vTZPESRCO,
                               szlControlDef, "Invisible" );
      ...
      if ( (((lVisible == 0 || lVisible == -1 || lVisible == -3 || lVisible == -5) &&
             ...
             SetEntityCursor( vTZ__PRFO, szlControlDef, szlTag,
                              zPOS_FIRST | zQUAL_ENTITYATTR,
                              vTZPESRCO, szlControlDef, szlTag,
                              0, 0, 0 ) >= zCURSOR_SET )) ||
             ...
             (m_fState & zTZCB_XSLT)) )                          // Ctrl visible for XSLT
      {
         GetAddrForAttribute( &pch, vTZPESRCO,
                              szlControlDef, "PainterDLL_Name" );
         if ( zstrcmp( pch, szDLL_Name ) != 0 )
         {
            hInst = 0;
            zstrcpy( szDLL_Name, pch );
         ...
         if ( hInst )
         {
            GetIntegerFromAttribute( &lKey, vTZPESRCO,
                                     szlControlDef, szlKey );
            GetIntegerFromAttribute( &lId, vTZPESRCO,
                                     szlControlDef, "Id" );
                 ...
                 (j = rPaletteCtrl.AddBitmap( 1, (UINT) lId )) >= 0 )
            {
               ...
               GetAddrForAttribute( &pchTag, vTZPESRCO,
                                    szlControlDef, szlTag );
               zstrcpy( m_PaletteBarItems[ uButtonCnt ].szTag, pchTag );
            }
         }
      }

   } while ( SetCursorNextEntity( vTZPESRCO,
                                  szlControlDef, 0 ) >= zCURSOR_SET );
      ...
      while ( pos )
      {
         ...
         if ( pAX_Ctrl )
         {
                 ...
                 (j = rPaletteCtrl.AddBitmap( 1, pbm )) >= 0 )
            {
               ...
               m_PaletteBarItems[ uButtonCnt ].uCmdId =
                                               m_pZSubtask->m_ulNextBarTipCmd;
               zstrcpy( m_PaletteBarItems[ uButtonCnt ].szTag,
   ...
   m_pZSubtask->AddBarTip( m_wndPaletteBar.m_hWnd,
                           lFirstId, lFirstId + uButtonCnt );
...
//          rectPaint.bottom -= rectParent.top;
   ...
   else
   {
      ...
      if ( pCtrl )
      {
         ...
         while ( nPos > 0 )
         {
            ...
            if ( pCtrl == pPainterWindow->m_CtrlList[ nPos ] )
            {
               ...
               zsprintf( szLabel, "&xxdo Create %s control\tCtrl+x", szTag );
      ...
      if ( pCtrl )
      {
         ...
         SetAttributeFromInteger( vCtrlDef, szlControl,
                                  szlPSDLG_X, pCtrl->Attr.X );
         SetAttributeFromInteger( vCtrlDef, szlControl,
                                  szlPSDLG_Y, pCtrl->Attr.Y );
         SetAttributeFromInteger( vCtrlDef, szlControl,
                                  szlSZDLG_X, pCtrl->Attr.W );
         SetAttributeFromInteger( vCtrlDef, szlControl,
                                  szlSZDLG_Y, pCtrl->Attr.H );
...
// TraceLineS( "TZPainterBar::CreateAppView", "" );
   ...
   if ( vTaskLPLR )
   {
      ...
      if ( zstrcmp( pchName, m_csLPLR_Name ) == 0 )
      {
         if ( m_vTaskLPLR == 0  )
         {
...
//       TraceLineS( "Palette selected: ",
//                   m_pBar->m_PaletteBarItems[ k ].szTag );
         MB_SetMessage( m_pBar->m_vSubtask, 3,
                        m_pBar->m_PaletteBarItems[ k ].szTag );

         if ( m_pBar->m_pActivePainterWindow &&
              m_pBar->m_pActivePainterWindow->m_pPainterClient )
         {
            if ( m_pBar->m_uSelectedPaletteItem )
            {
               ::SetClassLong( m_pBar->m_pActivePainterWindow->
                                                   m_pPainterClient->m_hWnd,
                               GCL_HCURSOR,
                               (zLONG) ::LoadCursor( 0, IDC_CROSS ) );
...
TZPalette::OnGetToolTipText( WPARAM wParam, LPARAM lParam )
{
   ...
   for ( UINT k = 0; k < m_pBar->m_uPaletteItemCnt; k++ )
   {
      ...
      {
         zstrncpy( pchText, m_pBar->m_PaletteBarItems[ k ].szTag,
                   zZEIDON_TOOLTIP_TEXTLTH );
         break;
      }
   }
                     ...
                     AFX_CMDHANDLERINFO *pHandlerInfo )
{
   ...
   TraceLineI( "         OnCmdMsg nCode ", nCode );
   TraceLineI( "   ZSubtask's lMsgCmdId ",
               m_pBar->m_pZSubtask->m_lMsgCmdId );
#endif
   ...
   if ( uId == 0 && nCode == 5176814 ) //??? what is 5176814's define???
   {
      ...
      {
         if ( m_pBar->m_PaletteBarItems[ k ].chState &&
              m_pBar->m_PaletteBarItems[ k ].uCmdId ==
                            (UINT) m_pBar->m_pZSubtask->m_lMsgCmdId )
         {
            MB_SetMessage( m_pBar->m_vSubtask, 3,
                           m_pBar->m_PaletteBarItems[ k ].szTag );
            break;
...
// CControlBarInfo CB_Info;
//
// ReadToolBarPlacement( m_pBar->m_pZSubtask,
//                       "ZDR", "Palette", &CB_Info );
//
