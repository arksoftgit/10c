...
//
// 1998.08.26    RG
//    Use __WIN16__ instead of WIN32
//
...
/////////////////////////////////////////////////////////////////////////////
// This implementation is needed for backward compatibility of
// C++ Zeidon Sources ... to create a 16-Bit runtime-environment
// out of 32 Bit C++ Code.
/////////////////////////////////////////////////////////////////////////////
// Microsofts CCommandLineInfo implementation
...
void
CCommandLineInfo::ParseParam( LPCSTR pszParam, BOOL bFlag, BOOL bLast )
{
   if ( bFlag )
...
void
CCommandLineInfo::ParseParamFlag( LPCSTR pParam )
{
   ...
   if ( pszParam.Compare( "pt" ) == 0 )
      m_nShellCommand = FilePrintTo;
   else
   if ( pszParam.Compare( "p" ) == 0 )
      m_nShellCommand = FilePrint;
   else
   if ( pszParam.CompareNoCase( "Unregister" ) == 0 ||
        pszParam.CompareNoCase( "Unregserver" ) == 0 )
   {
      m_nShellCommand = AppUnregister;
   }
   else
   if ( pszParam.Compare( "dde" ) == 0 )
   {
      //AfxOleSetUserCtrl(FALSE);
      m_nShellCommand = FileDDE;
   }
   else
   if (pszParam.CompareNoCase("Embedding") == 0)
   {
      //AfxOleSetUserCtrl( FALSE );
      m_bRunEmbedded = TRUE;
      m_bShowSplash = FALSE;
   ...
   if ( pszParam.CompareNoCase( "Automation" ) == 0 )
   {
      //AfxOleSetUserCtrl( FALSE );
...
void
CCommandLineInfo::ParseParamNotFlag( LPCSTR pszParam )
{
   if ( m_csFileName.IsEmpty() )
      m_csFileName = pszParam;
   else
   if ( m_nShellCommand == FilePrintTo && m_csPrinterName.IsEmpty() )
      m_csPrinterName = pszParam;
   else
   if ( m_nShellCommand == FilePrintTo && m_csDriverName.IsEmpty() )
      m_csDriverName = pszParam;
   else
   if ( m_nShellCommand == FilePrintTo && m_csPortName.IsEmpty() )
      m_csPortName = pszParam;
}

void
CCommandLineInfo::ParseLast(BOOL bLast)
{
   if ( bLast )
   {
      if ( m_nShellCommand == FileNew && !m_csFileName.IsEmpty() )
...
//    }
   ...
   SetAttributeFromInteger( m_pZSubtask->m_vDialog, szlSubtask,
                            szlCPP, (zLONG) m_pZSubtask );
   ...
   if ( m_pZSubtask->m_pWndDef == 0 )
   {
      ...
      zmemset( m_pZSubtask->m_pWndDef->Tag, 0xFE, sizeof( zKZWDLGXO_Wnd_DEF ) );
   // m_pZSubtask->m_pWndDef->CheckByte = 0xFE;
#endif
      GetStructFromEntityAttrs( (zPCHAR) m_pZSubtask->m_pWndDef->Tag,
                                m_pZSubtask->m_vDialog, szlWnd );
#ifdef DEBUG_LODSTRUCT
      if ( m_pZSubtask->m_pWndDef->CheckByte != 0xFE )
         MessageBox( "WndDef Structure Length Error",
                     "GetStructFromEntityAttrs", MB_OK );
   // zPCHAR pchCheckByte = (zPCHAR) &(m_pZSubtask->m_pWndDef->CheckByte);
   // pchCheckByte--;
   // if ( *pchCheckByte == (char) 0xFE )
   //    TraceLineS( "zdrcmdln WndDef CheckByte error??", "" );
#endif
   }
   ...
   GetAddrForAttribute( (zPCHAR *) &pch, m_pZSubtask->m_vDialog,
                        szlWnd, szlD_Caption );
   if ( *pch )
      zstrcpy( m_pZSubtask->m_pWndDef->Caption, pch );
   ...
   {
      m_pZSubtask->m_vInterpSubtask =
                     m_pZSubtask->m_pZParent->m_vInterpSubtask;
   }
   else
   {
   ...
   m_pZSubtask->m_pchEnabledAction =
    new char[ (zSHORT) (m_pZSubtask->m_pWndDef->ActCnt + sizeof( zSHORT )) ];
   ...
   *pn = (zSHORT) m_pZSubtask->m_pWndDef->ActCnt;
...
#ifdef DEBUG_ACTION
   TraceLineI( "zdrcmdln init lth: ",
               m_pZSubtask->m_pWndDef->ActCnt + sizeof( zSHORT ) );
   ...
   if ( m_pZSubtask->m_vInterpSubtask == 0 )   // not done if interpreting
   {
      ...
      //                             LOAD_LIBRARY_AS_DATAFILE );
         ...
         m_pZSubtask->m_hLibrary =
                 SysLoadLibraryWithErrFlag( m_pZSubtask->m_vDialog, pch, 0 );
         m_pZSubtask->m_hInstDLL = (HINSTANCE) SysGetInstance( m_pZSubtask->m_hLibrary );
         m_pZSubtask->m_hLibRes =
                    SysLoadLibraryWithErrFlag( m_pZSubtask->m_vDialog, pch,
                                               zLOADLIB_RESOURCES );
...
ZModalDlg::OnOK( )
{
   ...
   // TraceLineS( "ZModalDlg::OnOK Event: ", *(m_pZSubtask->m_pDfltBtn->m_pzsTag) );
      zstrcpy( m_pchReturnCtrlTag, *(m_pZSubtask->m_pDfltBtn->m_pzsTag) );
...
ZModalDlg::OnPostedClose( WPARAM wParam, LPARAM lParam )
{
         ...
         (wParam == -1 && lParam == (zLONG) m_hWnd)) )
   {
      ...
      if ( m_pZSubtask->m_szActionName[ 0 ] )
         zstrcpy( m_pchReturnCtrlTag, m_pZSubtask->m_szActionName );
      else
      if ( m_pZSubtask->m_pzsLastTag )
         zstrcpy( m_pchReturnCtrlTag, *(m_pZSubtask->m_pzsLastTag) );
      else
         m_pchReturnCtrlTag[ 0 ] = 0;
