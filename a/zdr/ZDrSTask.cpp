...
#endif
{
   ...
   {
      TraceLine( "ZSubtask::ZSubtask::ctor (0x%08x): %d  %s",
                 this, m_ulSubtaskIdNbr, cpcWndTag );
   }
...
// m_clrBk = RGB( 0, 255, 255 ); teal test
   ...
   // loaded this definition so that the test for success/failure below
   // is moot.
   zVIEW vMsgQ;

   if ( GetViewByName( &vMsgQ, szl__MSGQ,
                       m_vDialog, zLEVEL_TASK ) != zLEVEL_TASK )
   {
      if ( ActivateEmptyObjectInstance( &vMsgQ, szlKZMSGQOO, m_vDialog,
                                        zMULTIPLE ) >= 0 )
      {
      ...
      else
      {
         ...
         MessageSend( m_vDialog, 0, szlDrvr,
                      "Cannot load Message Object",
                      zMSGQ_MODAL_ERROR, FALSE );
...
// TraceLineI( "Create MsgQ view for Task: ", m_lTID );
   if ( SetEntityCursor( m_vMsgQ, szlTask, szlId,
                         ...
                         &m_lTID, 0, 0, 0, 0, 0 ) < 0 )
   {
      ...
      SetAttributeFromVariable( m_vMsgQ, szlTask, szlId,
                                &m_lTID, zTYPE_INTEGER, 0, 0, 0 );
      SetAttributeFromVariable( m_vMsgQ, szlTask, szlClient,
...
// DisplayObjectInstance( m_vDialog, 0, 0 );
   ...
   if ( cpcWndTag && cpcWndTag[ 0 ] &&
        SetCursorFirstEntityByString( m_vDialog, szlWnd, szlTag,
   ...
   if ( m_pzsWndTag == 0 || **m_pzsWndTag == 0 )
   {
      ...
      zstrcpy( szMessage, "(drvr) Window name not found in Dialog: " );
      if ( cpcDlgTag )
         zstrcat( szMessage, cpcDlgTag );
      else
         zstrcat( szMessage, "<nul>" );

      if ( cpcWndTag && cpcWndTag[ 0 ] )
         zstrcpy( szWindowName, cpcWndTag );
      else
         GetStringFromAttribute( szWindowName, m_vDialog, szlDlg, szlDfltWnd );
      ...
      {
         if ( ConstructZeidonPrompt( pParent->m_vDialog,
                                     "Valid Window", szMessage,
                                     szWindowName, sizeof( szWindowName ),
                                     &lFlags, "Window:", 0, 0 ) == IDOK )
         {
            if ( SetCursorFirstEntityByString( m_vDialog, szlWnd,
                                               szlTag, szWindowName,
                                               0 ) > zCURSOR_UNCHANGED )
            {
               mDeleteInit( m_pzsWndTag );
         ...
         else
         {
            ...
            ZSubtask::InitializeSubtaskState( this );
            ZSubtask::SetSubtaskState( this, 0,
   ...
   if ( ulHotkeyLth % sizeof( zACCEL_TABLE ) )
   {
      ...
      MessageSend( m_vDialog, 0, *m_pzsDlgTag,
...
// if ( ulHotkeyLth || ulGlobalHotkeyLth )
   {
      ...
      GetBlobFromAttribute( m_pAccels, &ulHotkeyLth,
                            m_vDialog, szlWnd, szlHotkey );
      GetBlobFromAttribute( m_pAccels + ulHotkeyLth,
                            &ulGlobalHotkeyLth,
                            vHotkey, szlApp, szlHotkey );
...
//    }
   ...
   zmemset( m_pWndDef->Tag, 0xFE, sizeof( zKZWDLGXO_Wnd_DEF ) );
// m_pWndDef->CheckByte = 0xFE;
#endif
   GetStructFromEntityAttrs( (zPCHAR) m_pWndDef->Tag, m_vDialog, szlWnd );
#ifdef DEBUG_LODSTRUCT
   if ( m_pWndDef->CheckByte != 0xFE )
      MessageBox( 0, "WndDef Structure Length Error",
                  "GetStructFromEntityAttrs", MB_OK );
// zPCHAR pchCheckByte = (zPCHAR) &(m_pWndDef->CheckByte);
// pchCheckByte--;
// if ( *pchCheckByte == (char) 0xFE )
//    TraceLineS( "zdrstask WndDef CheckByte error??", "" );
#endif
   GetAddrForAttribute( (zPCHAR *) &pch, m_vDialog, szlWnd, szlD_Caption );
   if ( *pch )
      zstrcpy( m_pWndDef->Caption, pch );
...
// if ( m_pWndDef->Subtype & zWNDTYPE_3D )      // this needs to be accounted
   ...
   m_pchEnabledAction =
                 new char[ (zSHORT) (m_pWndDef->ActCnt + sizeof( zSHORT )) ];
   ...
   zmemset( m_pchEnabledAction, zAUTO_ENABLE,
            (zSHORT) (m_pWndDef->ActCnt + sizeof( zSHORT )) );
   zSHORT *pn = (zSHORT *) m_pchEnabledAction;
   *pn = (zSHORT) m_pWndDef->ActCnt;

#ifdef DEBUG_ACTION
   TraceLineI( "zdrstask init lth: ", m_pWndDef->ActCnt + sizeof( zSHORT ) );
   TraceBlob( "zdrstask init:",
              m_pchEnabledAction + sizeof( zSHORT ),
              *((zSHORT *) m_pchEnabledAction) );
#endif
   ...
   if ( m_vInterpSubtask == 0 )   // not done if interpreting
   {
      ...
      {
      // m_hInstDll = LoadLibraryEx( szFileName, 0,
      //                             LOAD_LIBRARY_AS_DATAFILE );
         ...
         m_hLibRes = SysLoadLibraryWithErrFlag( m_vDialog, pch,
                                                zLOADLIB_RESOURCES );
...
//          break;
   ...
   if ( m_pWndDef->ProfileView[ 0 ] &&
        GetViewByName( &vProfile, m_pWndDef->ProfileView,
                       m_vDialog, zLEVEL_ANY ) > 0 )
   {
                    ...
                    CWnd   *pCWnd )    // Parent window (from existing app)
{
   ...
   if ( pZTask && (pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      TraceLine( "ZSubtask::ZSubtask ALTERNATE ctor (0x%08x): %d",
                 this, m_ulSubtaskIdNbr );
   }
...
fnDeleteCtrls( ZMultiIdxSet *pZMIXCtrl, ZMapAct *pzmaComposite )
{
   ...
   if ( pzmaComposite == 0 )
   {
      ...
      for ( k = pZMIXCtrl->uNodeCnt - 1; k >= 0; k-- )
      {
         ...
         {
            zstrcpy( szBuffer, "Delete ctrl " );
            zstrcat( szBuffer, *(pzma->m_pzsTag) );
            zstrcat( szBuffer, " with composite " );
   ...
   for ( k = pZMIXCtrl->GetCount( ) - 1; k >= 0; k-- )
   {
      if ( *(pZMIXCtrl->m_pvNode + k) )
      {
         ...
         if ( pzmaComposite == 0 ||
              (pzmaComposite && pzmaComposite == pzma->m_pzmaComposite) )
         {
...
ZSubtask::~ZSubtask( )
{
   zULONG ulSubtaskState =
                ZSubtask::SetSubtaskState( this, 0, zSUBTASK_STATE_DELETED );

#ifdef zREMOTE_SERVER
   zCHAR szMsg[ 128 ];
   if ( m_pZTask && (m_pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] ZSubtask::dtor #$# "
                       "state: %#06hx   Subtask: ",
                m_pZSocketTask,
                AfxGetThread( ),
   ...
   if ( m_pZTask && (m_pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      if ( m_pzsDlgTag && m_pzsWndTag )
         TraceLine( "ZSubtask::dtor [0x%08x] IdNbr: %d  %s.%s",
                    this, m_ulSubtaskIdNbr, *m_pzsDlgTag, *m_pzsWndTag );
      else
...
// TraceLineI( "ZSubtask dtor: ", (zLONG) this );
   ...
   if ( m_pZSocketTask && m_pZSocketTask->m_pCacheWndList )
   {
      ...
      zsprintf( szMsg, "ZSubtask::dtor %s", *m_pzsRemoteWndTag );
      ZXRA_Item *pXRA =
         m_pZSocketTask->m_pCacheWndList->Find( *m_pzsRemoteWndTag,
                                                this, szMsg );
      if ( pXRA )
      {
         pXRA->m_pZSubtask = 0;
        ...
        GetViewByName( &vSubtask, "__ZDrSubtask", m_vDialog, zLEVEL_TASK ) > 0 )
   {
      ...
      TraceLineS( "(drvr) Waiting for Application Operation to complete ",
                  "for ZSubtask destructor" );
...
// ListWindowsAndControls( this, "ZSubtask::dtor2", 0 );
   if ( pZView && pZView->m_pParentSplitterWnd &&
        (m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) )
   {
      ...
      if ( pZView->m_pParentSplitterView )
      {
         ...
         if ( m_pZTask && (m_pZTask->m_nTraceAction & 0x72) == 0x72 )
         {
            TraceLine( "ZSubtask::dtor ParentSplitterWnd: 0x%08x for Pane( %d, %d ): "
                           "0x%08x Pane( %d, %d ): 0x%08x",
                         pParentSplitterWnd, nRow, nCol, pZView,
                         nRow2, nCol2, pPane );
         }

         pPane->SetParent( pParentSplitterWnd );
         pPane->SetDlgCtrlID( pParentSplitterWnd->
                                       IdFromRowCol( pParentData->m_nRow,
                                                     pParentData->m_nCol ) );
         ...
         if ( pSplitterWnd->m_bSplitterView[ nRow2 ][ nCol2 ] ) // SplitterView
         {
            ...
            pParentData->m_pParentSplitterWnd->
                     m_bSplitterView[ pParentData->m_nRow ][ pParentData->m_nCol ] = TRUE;
            ...
            pOtherView->m_pParentData->m_pParentSplitterView =
                                          pParentData->m_pParentSplitterView;
            pOtherView->m_pParentData->m_pParentSplitterWnd =
                                           pParentData->m_pParentSplitterWnd;
         ...
         else
         {
            ...
            if ( pSplitterWnd->m_pZSubtask1 == this &&
                 pParentData->m_pParentSplitterWnd->m_pZSubtask2 == this )
            {
               pParentData->m_pParentSplitterWnd->m_pZSubtask2 =
                                                  pSplitterWnd->m_pZSubtask2;
            }

            pOtherPaneView->m_pParentSplitterView =
                                           pParentData->m_pParentSplitterView;
      ...
      else  // of:  if ( pZView->m_pParentSplitterView )
      {
         ...
         if ( pSplitterWnd->m_pZSubtask1 == this )
         {
            if ( (ZSubtask::GetSubtaskState( pSplitterWnd->m_pZSubtask2 ) &
      ...
      // ASSERT( m_ulSubtaskFlags & zSUBTASK_SPLITTER_MAIN_SUBTASK ); can't!
         ...
         else
         {
            ...
            pOtherFWnd = 0;
            bPostQuit = TRUE;
            TraceLineS( "ZSubtask destructor: ",
                        "No subtask set for splitter pane" );
         }
         ...
         // ASSERT( pParentSplitterWnd == pParentData->m_pParentSplitterWnd );
            ...
            // Reset the pane's parent so we can delete the splitter and
            // its view.
         ...
         m_ulSubtaskFlags &= ~zSUBTASK_SUBWND_SPLITTER_PANE;
         ASSERT( pZFWnd->m_pSplitterWnd == 0 ||
                 pZFWnd->m_pSplitterWnd == pSplitterWnd );
      ...
      if ( pParentSplitterView )
      {
         ...
         while ( lItem )
         {
            if ( g_pSubtaskList[ lItem ].pZSubtask &&
                 g_pSubtaskList[ lItem ].lTID == m_lTID &&
                 (g_pSubtaskList[ lItem ].ulSubtaskState &
            ...
            //      pZSubtask->m_pZView->m_pParentSplitterWnd->GetParent( ) : 0;
               ...
               if ( pZSubtask->m_pZView &&
                    ...
                    pZSubtask->m_pZView->m_pParentSplitterView->m_pParentData->m_pParentSplitterView ==
                                                                                 pParentSplitterView )
               {
   ...
   if ( m_pZTask && (m_pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] ZSubtask::dtor Termination: ",
                m_pZSocketTask,
                AfxGetThread( ),
                m_pZSocketTask->m_pWinThread,
                m_pZSocketTask->m_sock );
...
// SysMutexLock( m_vDialog, "ZDw", 0, 0 );
   ...
   if ( m_pZTask && (m_pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      ...
      zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] ZSubtask::RestartBuildWindow "
                       "state: %#06hx   Subtask: ",
                m_pZSocketTask,
                AfxGetThread( ),
                m_pZSocketTask->m_pWinThread,
...
// TraceLine( "ZSubtask::RestartBuildWindow %s.%s *********** State: 0x%08x",
//            *m_pzsDlgTag, *m_pzsWndTag, ulSubtaskState );

   if ( (ulSubtaskState & zSUBTASK_STATE_IN_CONSTRUCTION) == 0 ||
   ...
   if ( (m_ulWndOper & zWND_EVENT_PRE_BUILD) &&
        SetCursorFirstEntityByInteger( m_vDialog, szlWndEvent, szlType,
                                       zWND_EVENT_PRE_BUILD, 0 ) > zCURSOR_UNCHANGED )
   {
...
#ifdef DEBUG_TASK
      TraceLine( "======================> [drvr] RestartBuildWindow "
                   "Task: 0x%08x  Dlg.Wnd: %s.%s   vDialog: 0x%08x  hWnd: 0x%08x",
...
POSTBUILD_PRECODE_label:
   ...
   if ( zstrcmp( *m_pzsWndTag, "account" ) == 0 && m_pZMIXCtrl )
   {
      ...
      // #define WS_DISABLED         0x08000000L
         ...
         {
            ::MessageBox( 0, "GridListAccount Disabled",
                          "ZSubtask::Before Subtask PostBuild", MB_OK );
         }
      }
   }
#endif
   ...
   if ( m_lRelease > g_lRel &&
        (m_ulWndOper & zWND_EVENT_POST_BUILD) &&
        SetCursorFirstEntityByInteger( m_vDialog, szlWndEvent, szlType,
                                       zWND_EVENT_POST_BUILD, 0 )
                                                       > zCURSOR_UNCHANGED )
   {
...
POSTBUILD_POSTCODE_label:
   ...
   if ( (m_ulWndOper & lEvent) == (zULONG) lEvent &&
        SetCursorFirstEntityByInteger( m_vDialog, szlWndEvent,
                                       szlType, lEvent, 0 )
                                                     > zCURSOR_UNCHANGED )
   {
      lRC = ProcessActionLoop( this, szlWndAct, 0, vAppSubtask,
   ...
   if ( m_pzmaWithFocus &&
        m_pzmaWithFocus->m_pCtrl &&
        mIs_hWnd( m_pzmaWithFocus->m_pCtrl->m_hWnd ) )
   {
...
// }
   ...
   if ( ReadWindowPlacement( m_vDialog, "ZDR",
                             *m_pzsDlgTag, *m_pzsWndTag, &wp ) &&
        CheckWindowPos( &wp ) )
   {
...
//       TraceLineS( "Setting ACTIVE window ==============> ", *m_pzsWndTag );
   ...
   //    TraceLineI( "Invalid window placement ", wp->length );
      ...
      else
      {
         ...
         if ( uEdge == ABE_TOP )
            m_pZFWnd->MoveWindow( Attr.X, Attr.Y + abd.rc.bottom,
                                  Attr.W, Attr.H, TRUE );
         else
         if ( uEdge == ABE_LEFT )
            m_pZFWnd->MoveWindow( Attr.X + abd.rc.right, Attr.Y,
                                  Attr.W, Attr.H, TRUE );
         else
         {
            ...
            m_pZFWnd->MoveWindow( Attr.X, Attr.Y,
                                  Attr.W, Attr.H, TRUE );
         }
   ...
   if ( Attr.W != (zSHORT) m_pWndDef->SZDLG_X ||
        Attr.H != (zSHORT) m_pWndDef->SZDLG_Y )
   {
   ...
   zCHAR szAppName[ zZEIDON_NAME_LTH + 1 ];
   GetApplDirectoryFromView( szAppName, m_vDialog,
                             zAPPL_NAME, sizeof( szAppName ) );
   ...
   if ( ((ZDrFrame *) m_pZFWnd)->VerifyBarState( csProfile ) )
   {
      ...
      LPLIBRARY hLibrary = SysLoadLibraryWithErrFlag( m_vDialog,
                                                      "zdctl", 0 );
      if ( hLibrary )
      {
         lpfnDynRoutine = (zFARPROC_CBAR) SysGetProc( hLibrary,
                                                      "CBarLoadSaveState" );
         if ( lpfnDynRoutine )
         {
         ...
         // SetDockState( state );
            ...
            for (i = 0; i < state.m_arrBarInfo.GetSize(); i++)
            {
               ...
               if (pInfo->m_bFloating)
               {
                  ...
                  CMiniDockFrameWnd* pDockFrame = m_pZFWnd->CreateFloatingFrame(
...
// if ( zstrcmp( "TZPNTRAD", *m_pzsWndTag ) == 0 )
   {
      if ( bEnable )
         TraceLine( "[drvr] ================> ENabling window %s.%s   nDisabled = %d",
                    *m_pzsDlgTag, *m_pzsWndTag, m_nDisabled );
      else
         TraceLine( "[drvr] ================> DISabling window %s.%s  nDisabled = %d",
                    *m_pzsDlgTag, *m_pzsWndTag, m_nDisabled );
   }
#endif

   if ( (bEnable == FALSE && m_nDisabled < 0) ||
        (bEnable == TRUE && m_nDisabled == 0 && m_nInvisible == 0) )
   {
#ifdef zREMOTE_SERVER
      if ( m_pZSocketTask )
...
ZSubtask::AutoGrayActions( )
{
...
#ifdef DEBUG_LODSTRUCT
   zmemset( ActDef.Tag, 0xFE, sizeof( ActDef ) );
   ...
   while ( nRC >= zCURSOR_SET )
   {
      ...
      GetStructFromEntityAttrs( (zPCHAR) ActDef.Tag, m_vDialog, szlAct );
#ifdef DEBUG_LODSTRUCT
      if ( ActDef.CheckByte != 0xFE )
         MessageBox( 0, "ActDef Structure Length Error",
                     "GetStructFromEntityAttrs", MB_OK );
     // zPCHAR pchCheckByte = (zPCHAR) &(ActDef.CheckByte);
     // pchCheckByte--;
     // if ( *pchCheckByte == (char) 0xFE )
     //    TraceLineS( "zdrstask ActDef CheckByte error??", "" );
#endif
      while ( ActDef.AutoGray )  // purist's goto
      {
      // if ( zstrcmp( ActDef.VN, "TZERATLO" ) == 0 &&
      //      zstrcmp( ActDef.EN, "ER_Attribute" ) == 0 )
      ...
      // TraceLineS( "ZSubtask::AutoGrayActions ViewName ", ActDef.VN );
         ...
         if ( (ActDef.AutoGray & zAUTOGRAY_ACTION_NULL) ||
              (ActDef.AutoGray & zAUTOGRAY_ACTION_NOT_NULL) )
         {
            if ( vApp )
            {
               if ( CheckEntityInView( vApp, ActDef.EN ) == 0 )
               {
                  nRC = SetEntityCursor( vApp, ActDef.EN, 0,
                                         zPOS_FIRST | zTEST_CSR_RESULT,
                                         0, 0, 0, 0, ActDef.Scope, 0 );
               }
               else
               {
                  nRC = zCALL_ERROR;
         ...
         {
            if ( vApp &&
                 SetEntityCursor( vApp, ActDef.EN, 0,
                                  zPOS_PREV | zTEST_CSR_RESULT,
                                  0, 0, 0, 0, ActDef.Scope, 0 ) >= 0 )
            {
               EnableAction( m_vDialog, ActDef.Tag, zAUTO_ENABLE );
            }
            else
         ...
         {
            if ( vApp &&
                 MiGetUpdateForViewEntityAttr( vApp, ActDef.EN,
                                               0, 0 ) > 0 ) // ==> updateable
            {
               EnableAction( m_vDialog, ActDef.Tag, zAUTO_ENABLE );
            }
            ...
            {
            // TraceLineS( "ZSubtask::AutoGrayActions "BROWSE_ONLY",
            //             ActDef.Tag );
               EnableAction( m_vDialog, ActDef.Tag, zAUTO_DISABLE );
            }
         }
...
// TraceLineS( "ZSubtask::ResetFocus ", *(pzma->m_pzsTag) );
   ...
   if ( m_pZMIXCtrl == 0 ||
        (lIdx = m_pZMIXCtrl->GetNodeIndex( pzma )) == 0 )
   {
      return;
   }
...
// CWaitRedraw wd;
   ...
   if ( (m_ulWndOper & zWND_EVENT_ON_REFRESH) == zWND_EVENT_ON_REFRESH &&
        SetCursorFirstEntityByInteger( m_vDialog, szlWndEvent, szlType,
                                       zWND_EVENT_ON_REFRESH, 0 )
                                                       > zCURSOR_UNCHANGED )
   {
      if ( ProcessActionLoop( this, szlWndAct, 0, 0,
...
// TraceLineS( "ZSubtask::MapFromOI Locked: ", bLock ? "TRUE" : "FALSE" );
   ...
   if ( m_pZMIXCtrl )
   {
      m_pZMIXCtrl->VisitInorder( (fnVISIT_INORDER) fnMapCtrlsFromOI,
                                 0, wFlag, (LPARAM) cpcCtrlTag );
   }
   ...
   if ( m_pZMIXCtrl )
   {
      m_pZMIXCtrl->VisitInorder( (fnVISIT_INORDER) fnPostMapCtrlsFromOI,
                                 0, wFlag, (LPARAM) cpcCtrlTag );
   }
   ...
   // m_pZView->UpdateWindow( );
   // m_pZView->RedrawWindow( 0, 0, RDW_ALLCHILDREN | RDW_ERASENOW |
   //                               /* RDW_UPDATENOW  |*/ RDW_INTERNALPAINT );
      m_pZView->Repaint( );
   }
   else
   {
...
ZSubtask::MapToOI( )
{
   ...
   if ( m_pZMIXCtrl )
   {
      ...
      if ( m_pZMIXCtrl->m_pvNodeErr )
      {
         ...
         if ( ((ZMapAct *) (m_pZMIXCtrl->m_pvNodeErr))->m_pCtrl &&
              mIs_hWnd( ((ZMapAct *) (m_pZMIXCtrl->m_pvNodeErr))->
                                                        m_pCtrl->m_hWnd ) )
         {
...
ZSubtask::InformDriverOfSetFocus( ZMapAct *pzmaReceivingFocus,
                                  zBOOL   bPushButton )
{
// TraceLineS( "InformDriverOfSetFocus = ",
//             *(pzmaReceivingFocus->m_pzsTag) );

   if ( pzmaReceivingFocus->m_pzmaComposite )
   {
      if ( pzmaReceivingFocus->m_pzmaComposite->
                   InformCompositeOfSetFocus( pzmaReceivingFocus ) == FALSE )
      {
         return( FALSE );  // this control did not want focus and has
                           // set focus to whichever control it wanted
                           // to receive focus
      }
   }
   ...
   if ( m_pZView )
   {
      if ( bPushButton )
      {
      // TraceLineS( "InformDriverOfSetFocus PushButton ",
      //             *(pzmaReceivingFocus->m_pzsTag) );
         m_pCurrDfltBtn = pzmaReceivingFocus;
      // m_pZView->SetDefID( pzmaReceivingFocus->m_nIdNbr );
         ::SendMessage( m_pZView->m_hWnd, DM_SETDEFID,
                        pzmaReceivingFocus->m_nIdNbr, 0 );
#if 1
      ...
      if ( m_pDfltBtn )
      {
      // TraceLineS( "InformDriverOfSetFocus DfltPushButton ",
      //             *(m_pDfltBtn->m_pzsTag) );
         m_pCurrDfltBtn = m_pDfltBtn;
      // m_pZView->SetDefID( m_pDfltBtn->m_nIdNbr );
         ::SendMessage( m_pZView->m_hWnd, DM_SETDEFID,
                        m_pDfltBtn->m_nIdNbr, 0 );
#if 1
         if ( m_pCurrDfltBtn->m_pCtrl &&
              mIs_hWnd( m_pCurrDfltBtn->m_pCtrl->m_hWnd ) )
         {
      ...
      {
#if 1
         if ( m_pCurrDfltBtn && m_pCurrDfltBtn->m_pCtrl &&
              mIs_hWnd( m_pCurrDfltBtn->m_pCtrl->m_hWnd ) )
         {
         // TraceLineS( "InformDriverOfSetFocus CurrDfltPushButton (OFF): ",
         //             *(m_pCurrDfltBtn->m_pzsTag) );
            UINT uStyle = ((CButton *) m_pCurrDfltBtn->m_pCtrl)->GetButtonStyle( );
            if ( (uStyle & BS_OWNERDRAW) != BS_OWNERDRAW )
            {
...
//       m_pZView->SetDefID( -1 );
   ...
   if ( m_pDIL && m_nAutoDIL )
   {
      ...
      // This processing assumes that the first message in the list of
      // DIL messages in the .XWD is "".
      if ( SetEntityCursor( m_vDialog, szlDIL, 0,
                            zPOS_FIRST | zPOS_RELATIVE,
                            0, 0, 0, pzmaReceivingFocus->m_nDIL,
                            0, 0 ) >= zCURSOR_SET )
      {
   ...
   if ( (m_ulWndOper & zWND_EVENT_CHILD_RECEIVE_FOCUS) &&
        SetCursorFirstEntityByInteger( m_vDialog, szlWndEvent, szlType,
                                       zWND_EVENT_CHILD_RECEIVE_FOCUS, 0 ) >
                                                         zCURSOR_UNCHANGED )
   {
      ProcessActionLoop( this, szlWndAct, 0, 0,
                         zACTION_DIALOG_CODE_ONLY, TRUE,
                         "ChildFocus" );
   }
...
{
// TraceLineS( "InformDriverOfKillFocus pzma = ",
//             *(pzmaLosingFocus->m_pzsTag) );

   if ( pzmaLosingFocus->m_pzmaComposite )
   {
      if ( pzmaLosingFocus->m_pzmaComposite->
                   InformCompositeOfKillFocus( pzmaLosingFocus ) == FALSE )
      {
...
#if 0
   if ( pzma )
      TraceLine( "ZSubtask::InformDriverOfFlyBy Subtask: %s  Ctrl: %s",
                 *m_pzsWndTag, *(pzma->m_pzsTag) );

   if ( pzma &&
        ((pzma->m_ulMapActFlags & zMAPACT_VISIBLE) == 0 ||
         (pzma->m_ulMapActFlags & zMAPACT_ENABLED) == 0) )
   {
      return;
   }
#endif
...
//    pzma = m_pzmaWithFocus;   to the last control with focus
   ...
   if ( m_ulFlyByTickCnt &&
        (SysGetTickCount( ) - m_ulFlyByTickCnt) < (zTICKS_PER_SECOND * 2) )
   {
      return;
   }
   ...
   if ( m_pDIL && m_nAutoDIL )
   {
      if ( pzma )
      {
         ...
         else
         {
            ...
            // This processing assumes that the first message in the list of
            // DIL messages in the .XWD is "".
            SetEntityCursor( m_vDialog, szlDIL, 0,
                             zPOS_FIRST | zPOS_RELATIVE,
                             0, 0, 0, pzma->m_nDIL, 0, 0 );
...
// TraceLineS( "ZSubtask::GetFocusCtrlWithinComposite ", "" );
   ...
   if ( pZMIXCtrl == 0 ||
        (pzmaCompositeIn == 0 && pzma == 0) ||
        (pzmaCompositeIn && (lIdxComposite =
                      pZMIXCtrl->GetNodeIndex( pzmaCompositeIn )) == 0) ||
   ...
   while ( k != lIdx )  // if we've gone thru the list, we gotta stop
   {
...
#if 0
         if ( zstrcmp( "LBPurchaseOrder", *(pzma->m_pzsTag) ) == 0 &&
              pzma->m_pCtrl && mIs_hWnd( pzma->m_pCtrl->m_hWnd ) )
         {
            TraceLineS( "GetFocusCtrlWithinComposite testing pzma = ",
                        *(pzma->m_pzsTag) );
         }
#endif
      ...
      //    TraceLineS( "", "" );
         ...
         // This will skip ctrls that have been auto-disabled unless they are
         ...
         {
         // TraceLineS( "GetFocusCtrlWithinComposite pzma = ",
         //             *(pzma->m_pzsTag) );
            return( pzma );
         }
      ...
      else
      {
         if ( bNext )
         {
            if ( k >= pZMIXCtrl->GetCount( ) )
            {
               ...
               if ( (pZSubtask->m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) &&
                    (pZSubtask->m_pZView && pZSubtask->m_pZView->m_pNextView) )
               {
         ...
         else
         {
            ...
            else
            {
               // Do something special for splitters.
               if ( (pZSubtask->m_ulSubtaskFlags & zSUBTASK_SUBWND_SPLITTER_PANE) &&
                    (pZSubtask->m_pZView && pZSubtask->m_pZView->m_pPrevView) )
               {
...
ZSubtask::RecalcLayout( HWND hWnd )
{
        ...
        mIs_hWnd( pZSubtaskMain->m_pZFWnd->m_hWnd ) )
   {
           ...
           mIs_hWnd( pZSubtaskMain->m_pZView->m_pParentSplitterWnd->m_hWnd ) )
      {
         ...
         while ( pZView && pZView->m_pZSubtask &&
                 pZView->m_pZSubtask != pZSubtaskMain )
         {
      ...
      if ( pZSubtaskMain->m_pZView &&
           mIs_hWnd( pZSubtaskMain->m_pZView->m_hWnd ) )
      {
      // pZSubtaskMain->m_pZFWnd->ShowWindow( SW_SHOW );
         pZSubtaskMain->m_pZView->ShowWindow( SW_SHOW );
      }

      //xpZSubtaskMain->m_pZFWnd->
      //x                SetWindowPos( 0, 0, 0, 0, 0,
      //x                              SWP_NOSIZE | SWP_NOMOVE | SWP_NOZORDER |
...
ZSubtask::SendParentResize( ZMapAct *pzma, CSize *pSize )
{
   if ( m_pZMIXCtrl )
   {
      m_pZMIXCtrl->VisitInorder( (fnVISIT_INORDER) fnSendCtrlsParentResize,
                                 0, (WPARAM) pzma, (LPARAM) pSize );
   }
}
...
ZSubtask::SendCtrlSelectAll( )
{
   if ( m_pzmaWithFocus )
   {
   // TraceLineS( "ZSubtask::SendCtrlSelectAll for Ctrl: ",
   //             *(m_pzmaWithFocus->m_pzsTag) );
...
// TraceLine( "ZSubtask::SetStatusBarText: %s  for Item: %d",
//            cpcMessage, nItem );
...
ZSubtask::PostClose( WPARAM wParam )
{
   ...
   if ( m_pZTask && (m_pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      TraceLine( "ZSubtask::PostClose: [0x%08x] %s.%s  IdNbr: %d",
                 this, *m_pzsDlgTag, *m_pzsWndTag, m_ulSubtaskIdNbr );
   }
   ...
   while ( pZSubtask )
   {
      ...
      //xm_pzsWndTag = new CString( szWndTag );
         ...
         //?pZSubtask->m_pZFWnd->UpdateWindow( );
            return( pZSubtask->m_pZFWnd->PostMessage( g_uMsgPostedClose,
                                                      0, (zLONG) pZSubtask ) );
         }
      }
      else
...
//    if ( (ulSubtaskState & zSUBTASK_STATE_MARK_FOR_DELETE) )
   ...
   {
      if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
           g_pSubtaskList[ lItemCnt ].lTID == m_lTID &&
           ...
           g_pSubtaskList[ lItemCnt ].pZSubtask->m_pZParent == m_pZParent )
      {
         ...
         {
            TraceLineI( "ZSubtask::EnableParentWindows invalid window at: ",
                        lItemCnt );
            ASSERT( FALSE );
         }
#endif
...
ZSubtask::CreateZ( )
{
   ...
   if ( m_ulSubtaskFlags & zSUBTASK_SUBWND_MDI_FRAME )
   {
      ...
      if ( m_pZFWnd->LoadFrame( IDR_MAINFRAME ) == 0 ||
           m_pZFWnd->m_hWnd == 0 )
      {
         m_pZTask->m_uAppState |= zAPP_STATE_TERMINATED;
      }
...
//            m_pZTask->m_pZDrApp->m_pMainWnd->m_hWnd );
   ...
   if ( (m_pZTask->m_uAppState & zAPP_STATE_TERMINATED) == 0 &&
        m_pZTask->m_pZDrApp->m_pMainWnd == 0 )
   {
...
ZSubtask::HandleEvent( zLONG lEvent )
{
...
#endif
   {
      if ( ProcessActionLoop( this, szlWndAct, 0, 0, 0,
                              TRUE, "SysEvent" ) > 0 ||
           lEvent == zWND_EVENT_CLOSE )
      {
   ...
   if ( lEvent == zWND_EVENT_CLOSE )   // NO close action specified
   {
         ...
         while ( lpAT->nID != -1 )
         {
...
#endif
            {
               ...
               // m_pZFWnd->PostMessage( zZEIDON_PROCESS_ACTION, 0, lpAT->nID );
                  if ( SetEntityCursor( m_vDialog, szlAct, 0,
                                        zPOS_FIRST | zPOS_RELATIVE, 0, 0, 0,
                                        lpAT->nID, 0, 0 ) > zCURSOR_UNCHANGED )
                  {
                     zPCHAR pch;
               ...
               // m_pZFWnd->PostMessage( zZEIDON_PROCESS_HOTKEY, 0, lpAT->nID );
                  ...
                  SetEntityCursor( vHotkey, szlHotkey, 0,
                                   zPOS_FIRST | zPOS_RELATIVE, 0, 0, 0,
                                  lpAT->nID, 0, 0 );
                  GetStringFromAttribute( szDLL_Name, vHotkey, szlHotkey, szlDLL );
                  GetStringFromAttribute( szOperName, vHotkey, szlHotkey, szlCOP );

                  lpfnDynRoutine = (zFARPROC_DRIVER)
                       GetOperationDynamicCallAddress( m_vDialog,
      ...
      if ( OperatorPrompt( m_vDialog,
                           "No close action specified",
                           "Permit close?", TRUE, zBUTTONS_YESNO,
...
ZSubtask::CheckMouseMove( UINT uModKeys, CPoint pt )
{
   ...
   if ( (m_uMouseStatus & zMOUSE_CAPTURE) == zMOUSE_CAPTURE )
   {
      ...
      {
         if ( (m_ulWndOper & zWND_EVENT_LBUTTON_DRAG) ==
                                                  zWND_EVENT_LBUTTON_DRAG &&
              SetCursorFirstEntityByInteger( m_vDialog,
                                             szlWndEvent, szlType,
                                             zWND_EVENT_LBUTTON_DRAG, 0 )
                                                         > zCURSOR_UNCHANGED )
         {
            if ( ProcessActionLoop( this, szlWndAct, 0, 0,
                                    zACTION_DIALOG_CODE_ONLY, TRUE,
   ...
   if ( (m_ulWndOper & zWND_EVENT_MOUSE_MOVE) == zWND_EVENT_MOUSE_MOVE &&
        SetCursorFirstEntityByInteger( m_vDialog, szlWndEvent,
                                       szlType, zWND_EVENT_MOUSE_MOVE, 0 )
                                                       > zCURSOR_UNCHANGED )
   {
      if ( ProcessActionLoop( this, szlWndAct, 0, 0,
                              zACTION_DIALOG_CODE_ONLY, TRUE,
...
ZSubtask::CreateDefaultFont( )
{
   ...
   GetWorkstationApplicationValues( m_vDialog, "Font", szFaceName,
                                    sizeof( szFaceName ), &lFontSize,
                                    0, 0, 0, 0, 0, 0, 0, 0, 0 );
   if ( szFaceName[ 0 ] == 'Z' &&
        (zstrcmp( szFaceName, "ZeidonOld" ) == 0 ||
         zstrcmp( szFaceName, "ZeidonSize" ) == 0) )
   {
   ...
   if ( szFaceName[ 0 ] && lFontSize )
   {
      ...
      zstrcpy( logFont.lfFaceName, szFaceName );
      HDC hDC = ::GetDC( NULL );
   ...
   GetWorkstationApplicationValues( m_vDialog, "FontCtrl", szFaceName,
                                    sizeof( szFaceName ), &lFontSize,
                                    0, 0, 0, 0, 0, 0, 0, 0, 0 );
   mDeleteInit( m_pFontCtrl );
...
fnStartBrowseOfSubtaskList( zLONG lType )
{
   ...
   // If we get here everything's OK and the current task can now browse the
   // subtask list.
   return( 0 );
}
...
fnEndBrowseOfSubtaskList( zLONG lType )
{
   ...
   if ( lType > 1 )
   {
      // See NOTE in fnStartBrowseOfSubtaskList( ) for an explanation of why
   ...
   else
   {
      ...
      // Decrement the counter that keeps track of all the subtasks that are
      // reading the subtask list.
      g_pSubtaskList[ 0 ].lTID--;
   ...
   if ( pZSubtask && g_pSubtaskList )
   {
      ...
      while ( lItemCnt > 0 )
      {
         if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
              ...
              pZSubtask->m_lTID == g_pSubtaskList[ lItemCnt ].lTID )
         {
            return( lItemCnt );
         }
...
ZSubtask::GetSubtaskCount( zLONG lId )
{
   ...
   {
      if ( g_pSubtaskList[ lItemCnt ].pZSubtask &&
           (lId == 0 || lId == g_pSubtaskList[ lItemCnt ].lTID) )
      {
         lCnt++;
      }
...
// TraceLineS( "ZSubtask::InitializeSubtaskState Lock Mutex ZDrSS", "" );
        ...
        (pZSubtask->m_pZTask->m_nTraceAction & 0x72) == 0x72 )
   {
      ...
      zsprintf( szMsg, "ZSubtask::ctor %s.%s ItemCnt before attach: ",
                *(pZSubtask->m_pzsDlgTag), *(pZSubtask->m_pzsWndTag) );
   ...
   if ( lPos > 0 && hWnd == 0 )
   {
      TraceLineS( "ZSubtask::InitializeSubtaskState: ",
                  "Subtask unexpectedly located!!!" );
   }
   else
   if ( lPos <= 0 )
   {
...
// g_pSubtaskList[ lPos ].lTID = (zLONG) AfxGetThread( )->m_nThreadID;
   ...
   zstrcpy( g_pSubtaskList[ lPos ].DlgN, *(pZSubtask->m_pzsDlgTag) );
   zstrcpy( g_pSubtaskList[ lPos ].WndN, *(pZSubtask->m_pzsWndTag) );

// TraceLineS( "ZSubtask::ctor Lock Mutex ZDrSS", "" );

   if ( GetViewByName( &vZDrSystem, "__ZDrSystem",
                       pZSubtask->m_pZTask->m_pZDrApp->m_vSubtask,
                       zLEVEL_SYSTEM ) > 0 )
   {
      CreateViewFromViewForTask( &vZDrSystem, vZDrSystem,
                                 pZSubtask->m_vDialog );
      SetCursorFirstEntityByInteger( vZDrSystem, "Task", "Handle",
                                     (zLONG) pZSubtask->m_pZTask, 0 );
      CreateEntity( vZDrSystem, "Subtask", zPOS_FIRST );
      SetAttributeFromString( vZDrSystem, "Subtask", "DlgTag",
                              *(pZSubtask->m_pzsDlgTag) );
      SetAttributeFromString( vZDrSystem, "Subtask", "WndTag",
                              *(pZSubtask->m_pzsWndTag) );
      SetAttributeFromInteger( vZDrSystem, "Subtask", "Handle",
                               (zLONG) pvDialog );
   // TraceLine( "ZSubtask::InitializeSubtaskState for Dialog: 0x%08x",
   //            pvDialog );
      DropView( vZDrSystem );
   }
   ...
   {
      zCHAR  szMsg[ 256 ];
      zsprintf( szMsg, "ZSubtask::ctor %s.%s ItemCnt: after attach: ",
                *(pZSubtask->m_pzsDlgTag), *(pZSubtask->m_pzsWndTag) );
      TraceLineI( szMsg, ZSubtask::GetSubtaskCount( 0 ) );
      lItemCnt = (zLONG) g_pSubtaskList[ 0 ].pZSubtask;
      while ( lItemCnt > 0 )
                           ...
                           zULONG   ulStateAdd )
{
   ...
   // TraceLineS( "SetSubtaskState Unlock Mutex ZDrSS", "" );
      ...
      if ( (ulStateAdd & zSUBTASK_STATE_MARK_FOR_DELETE) && lItemCnt > 0 &&
           mIs_hWnd( g_pSubtaskList[ lItemCnt ].hWnd ) )
      {
         ::SendMessage( g_pSubtaskList[ lItemCnt ].hWnd,
                        zZEIDON_POSTED_MARK_DELETE, 1, 0 );
      }
   }
   ...
   if ( lPos > 0 )
   {
           ...
           (pZSubtask->m_pZTask->m_nTraceAction & 0x72) == 0x72 )
      {
         ...
         while ( lTemp > 0 )
         {
            if ( g_pSubtaskList[ lTemp ].pZSubtask )
            {
               zsprintf( szMsg, "   [S%ld.T%ld.t%ld.s%ld] Dlg: %s  Wnd: %s  "
                                "vSubtask: %#010hx   Subtask: ",
                         g_pSubtaskList[ lTemp ].pZSubtask->m_pZSocketTask,
                         AfxGetThread( ),
                          ...
                          zLEVEL_SYSTEM ) > 0 )
      {
         ...
         if ( SetCursorFirstEntityByInteger( vZDrSystem, "Task", "Handle",
                          (zLONG) pZSubtask->m_pZTask, 0 ) == zCURSOR_SET &&
              SetCursorFirstEntityByInteger( vZDrSystem, "Subtask", "Handle",
                                       (zLONG) pvDialog, 0 ) == zCURSOR_SET )
         {
         // TraceLine( "ZSubtask::TerminateSubtaskState for Dialog: 0x%08x",
         //            pvDialog );
...
#ifdef zREMOTE_SERVER
         zCHAR  szMsg[ 256 ];
         TraceLineI( "ZSubtask::TerminateSubtaskState Item Count: after detach: ",
                     ZSubtask::GetSubtaskCount( 0 ) );
         zLONG  lTemp = (zLONG) g_pSubtaskList[ 0 ].pZSubtask;
         while ( lTemp > 0 )
         {
...
ZSubtask::GetSplitterMainSubtask( )
{
   ...
   {
      if ( pZView->m_pZSubtask &&
           (pZView->m_pZSubtask->m_ulSubtaskFlags & zSUBTASK_SPLITTER_MAIN_SUBTASK) )
      {
...
{
   if ( m_pZMIXCtrl == 0 || pzma == 0 ||
        m_pZMIXCtrl->GetNodeIndex( pzma ) == 0 )
   {
      return( FALSE );
   }
