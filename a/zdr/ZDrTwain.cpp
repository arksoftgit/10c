...
ZTwain::InitTwain( CFrameWnd *pZFWnd )
{
   ...
   zstrcpy( szTwainLibName, "TWAIN_32.DLL" );
...
ZTwain::GetIdentity( )
{
   ...
   zstrcpy( m_AppId.Version.Info, "3.5" );
   m_AppId.ProtocolMajor = TWON_PROTOCOLMAJOR;
   m_AppId.ProtocolMinor = TWON_PROTOCOLMINOR;
   m_AppId.SupportedGroups = DG_IMAGE | DG_CONTROL;
   zstrcpy( m_AppId.Manufacturer, "MICSS" );
   zstrcpy( m_AppId.ProductFamily, "Generic" );
   zstrcpy( m_AppId.ProductName, "Zeidon" );
}
...
ZTwain::SelectSource( )
{
   ...
   if ( CallTwainProc( &m_AppId, 0, DG_CONTROL, DAT_IDENTITY,
                       MSG_USERSELECT, &m_Source ) )
   {
