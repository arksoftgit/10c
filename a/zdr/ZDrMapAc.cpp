         ...
         m_csERemote( cpcType )
{
   ...
   if ( (m_lIdx = pZSubtask->m_pZMIXCtrl->InsertNode( this )) == 0 )
   {
      ...
      TraceLineS( "(drvr) Control Error (DUPLICATE Ctrl tag) ===> ",
                  pCtrlDef->Tag );
      ...
      } while ( (m_lIdx = pZSubtask->m_pZMIXCtrl->InsertNode( this )) == 0 &&
                ch != '0' ); // don't wrap forever.

      if ( m_lIdx == 0 )
      {
         TraceLineS( "(drvr) Could not generate unique Ctrl tag ===> ",
                     pCtrlDef->Tag );
         m_lIdx = 0;
         return;
      }
      else
      {
         ...
         zstrcat( szMessage, "] Generated unique Ctrl tag ===> " );
...
#ifdef DEBUG_ALL
      zCHAR szBuffer[ 64 ];
      zstrcpy( szBuffer, "ZMapAct ctor for " );
      zstrcat( szBuffer, *m_pzsTag );
      zstrcat( szBuffer, ": " );
   ...
   if ( ulLth )
   {
      ...
      GetBlobFromAttribute( m_pchDragDrop, &ulLth,
                            vDialog, szlCtrl, szlDD_BOI );
   ...
   else
   {
      ...
      {
         Attr.X = pCtrlDef->PSDLG_X =
                  mConvertMapDlgToPixelX( pCtrlDef->PSDLG_X ) + nOffsetX;
         Attr.Y = pCtrlDef->PSDLG_Y =
                  mConvertMapDlgToPixelY( pCtrlDef->PSDLG_Y ) + nOffsetY;
      }
      else
      {
         Attr.X = pCtrlDef->PSDLG_X =
                  mConvertDlgUnitToPixelX( pCtrlDef->PSDLG_X ) + nOffsetX;
         Attr.Y = pCtrlDef->PSDLG_Y =
                  mConvertDlgUnitToPixelY( pCtrlDef->PSDLG_Y ) + nOffsetY;
      }
   }
   ...
   else
   {
      ...
      {
         Attr.W = pCtrlDef->SZDLG_X =
                             mConvertMapDlgToPixelX( pCtrlDef->SZDLG_X );
         Attr.H = pCtrlDef->SZDLG_Y =
                             mConvertMapDlgToPixelY( pCtrlDef->SZDLG_Y );
      }
      else
      {
         Attr.W = pCtrlDef->SZDLG_X =
                             mConvertDlgUnitToPixelX( pCtrlDef->SZDLG_X );
         Attr.H = pCtrlDef->SZDLG_Y =
                             mConvertDlgUnitToPixelY( pCtrlDef->SZDLG_Y );
      }
   }

#ifdef DEBUG_RESIZE
   zCHAR szTag[ 33 ];
   zstrcpy( szTag, *m_pzsTag );
...
//#endif
   ...
   if ( m_pZSubtask->m_vPE )
   {
      ...
      // If a CtrlEvent Entity exists we have to register it for driver
      // default handling.
                             ...
                             BOOL bSrc )
{
   ...
   if ( m_pzsVName )
   {
      ...
      if ( vTgt )
      {
         ...
         if ( SetEntityCursor( vTgt, cpcEName, 0,
                               ...
                               0, 0, 0, 0, 0 ) != 0 )
         {
            ...
            zsprintf( szMsg,
                      "Entity Key Error: Control: %s  Entity: %s",
                      *m_pzsTag, cpcEName );
                             ...
                             zCPCHAR cpcAName )
{
   ...
   if ( GetViewByName( pvApp, cpcVName,
                       m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
   {
      if ( MiGetUpdateForViewEntityAttr( *pvApp, cpcEName,
                                         cpcAName, 0 ) > 0 )
      {
      ...
      if ( nRC >= zCURSOR_UNDEFINED )
      {
         if ( nRC == zCURSOR_UNDEFINED )
         {
            ...
            zsprintf( szMsg,
                      "Resetting (undefined) application cursor for "
                        "View.Entity: %s.%s\n"
   ...
   else
   {
      *pvApp = 0;
      ...
      {
         if ( zstrcmp( m_pZSubtask->m_csVName, cpcVName ) )
         {
   ...
   // Auto Graying, if the view does not permit update for the mapping
   // entity or the entity instance does not exist, disable control
...
ZMapAct::MapFromOI( WPARAM wFlag )
{
   ...
   // TraceLineX( "ZMapAct::MapFromOI vApp: ", (zLONG) vApp );
      ...
      TranslateValue( pch, TRUE, 1 );
      if ( pch[ 0 ] == 0 && (m_ulMapActFlag2 & zMAPACT_PREFILL) )
         zstrcpy( pch, *m_pzsText );
...
ZMapAct::IsVisibleForMapToOI( zCPCHAR cpcViewName )
{
   ...
   while ( pzmaParent )
   {
      if ( (pzmaParent->m_ulMapActFlags & zMAPACT_ENABLED) == 0 ||
           (pzmaParent->m_ulMapActFlags & zMAPACT_VISIBLE) == 0 )
      {
         return( 0 );
      }
...
//#endif
   ...
   if ( (m_ulMapActFlags & zMAPACT_IGNORE_RESIZE) ||
     // (m_lRelPsX == 0 && m_lRelPsY == 0 && m_lRelSzX == 0 && m_lRelSzY == 0) ||
        ...
        (pSize->cx < m_pZSubtask->m_lRelLeft + m_pZSubtask->m_lRelRight &&
         pSize->cy < m_pZSubtask->m_lRelTop + m_pZSubtask->m_lRelBottom) )
   {
      break;
   }
...
// if ( zstrcmp( "OK", *m_pzsTag ) == 0 || zstrcmp( "Cancel", *m_pzsTag ) == 0 )
   ...
   else
   {
      ...
      if ( lRemainY > 0 )
      {
         ...
         {
            aw.Y = ((lRemainY * m_lRelPsY) / m_pZSubtask->m_lOrigRelSizeY) +
                                                      m_pZSubtask->m_lRelTop;
         ...
         if ( m_lRelSzY )
         {
            aw.H = ((Attr.Y + m_lRelSzY - m_pZSubtask->m_lRelTop) *
                                  lRemainY / m_pZSubtask->m_lOrigRelSizeY) -
                                           (Attr.Y - m_pZSubtask->m_lRelTop);
         }
      }
      else
      if ( m_lRelSzY )
   ...
   else
   {
      ...
      if ( lRemainX > 0 )
      {
         ...
         {
            aw.X = ((lRemainX * m_lRelPsX) / m_pZSubtask->m_lOrigRelSizeX) +
                                                     m_pZSubtask->m_lRelLeft;
         ...
         if ( m_lRelSzX )
         {
            aw.W = ((Attr.X + m_lRelSzX - m_pZSubtask->m_lRelLeft) *
                                  lRemainX / m_pZSubtask->m_lOrigRelSizeX) -
                                          (Attr.X - m_pZSubtask->m_lRelLeft);
         }
      }
      else
      if ( m_lRelSzX )
                        ...
                        zULONG ulValue )
{
   ...
   switch ( lStatusType )
   {
         ...
         else
         {
            ...
            if ( m_ulMapActFlags & zMAPACT_ENABLED )
            {
               ...
               if ( m_pchEventAction )
               {
                  ...
                  {
                  // l = *((zPLONG) (m_pchEventAction +
                  //                                (k * 2 * sizeof( zLONG ))));
                     l = *((zPLONG) (m_pchEventAction +
                               ((k * 2 * sizeof( zLONG )) + sizeof( zLONG ))));
                     if ( m_pZSubtask->m_pchEnabledAction[ (zSHORT) (l +
                                                     sizeof( zSHORT )) ] > 0 )
                     {
                        nCnt++;
                        if ( nCnt > 1 )
      ...
      {
#ifdef zREMOTE_SERVER
         SetRemoteZCtrlAttribute( this, "Ctrl", "Visible",
                                  ulValue ? "Y" : "N" );
#endif
         if ( ulValue )
         {
      ...
      case zCONTROL_STATUS_ERROR:
      {
...
#ifdef zREMOTE_SERVER
         if ( ulValue )
            SetRemoteCtrlAttribute( m_pZSubtask->m_vDialog, *m_pzsTag,
                                    "Ctrl", "Error", "Y" );
#else
         if ( mIs_hWnd( m_pCtrl->m_hWnd ) )
         ...
         if ( ulValue && lStatusType == zCONTROL_STATUS_MAP_ERROR &&
      ...
      {
         if ( mIs_hWnd( m_pCtrl->m_hWnd ) )
            m_pCtrl->SetWindowPos( &CWnd::wndTop, 0, 0, 0, 0,
      ...
      {
         if ( mIs_hWnd( m_pCtrl->m_hWnd ) )
            m_pCtrl->SetWindowPos( &CWnd::wndBottom, 0, 0, 0, 0,
                                   SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE );
   ...
   switch ( lPropertyType )
   {
      case zCONTROL_PROPERTY_INTEGER_DATA:
         if ( mIs_hWnd( m_pCtrl->m_hWnd ) )
         {
            ...
            zltoa( ulNumericProperty, szNumeric );
      ...
      case zCONTROL_PROPERTY_FONT:
      {
              ...
              *cpcStringProperty )
         {
            ...
            if ( pFont->CreatePointFont( ulNumericProperty,
                                         cpcStringProperty ) == 0 )
            {
               mDeleteInit( pFont );
            }
            else
            {
               ...
               if ( mIs_hWnd( m_pCtrl->m_hWnd ) )
               {
                  m_pCtrl->SendMessage( WM_SETFONT,
                                        (WPARAM) (HFONT) *m_pFont );
                  m_pCtrl->Invalidate( );
               }
                           ...
                           zPCHAR  pchStringProperty )
{
   ...
   switch ( lPropertyType )
   {
      ...
      case zCONTROL_PROPERTY_STRING_DATA:
         if ( mIs_hWnd( m_pCtrl->m_hWnd ) )
         {
            GetWindowText( m_pCtrl->m_hWnd, pchStringProperty,
                           *pulNumericProperty );
            lRC = 0;
         }
         else
      ...
      case zCONTROL_PROPERTY_TAG:
         zstrcpy( pchStringProperty, *m_pzsTag );
         lRC = 0;
         break;
...
#ifdef DEBUG_ALL
   TraceLineX( "ZMapAct::GetZCtrlProperty Term MapActFlags: ",
               m_ulMapActFlags );
#endif
                         ...
                         zLONG lColumn )
{
   ...
   if ( m_pzsEName )
      zstrcpy( pchEntity, *m_pzsEName );
   else
      pchEntity[ 0 ] = 0;

   if ( m_pzsAName )
      zstrcpy( pchAttrib, *m_pzsAName );
   else
      pchAttrib[ 0 ] = 0;

   if ( m_pzsContext )
      zstrcpy( pchContext, *m_pzsContext );
   else
      pchContext[ 0 ] = 0;
        ...
        (pXRA->m_vXRA || pXRA->m_vXRA_Delta) )
   {
                                         ...
                                         0 ) < zCURSOR_SET )
      {
         ...
         SetAttributeFromString( vXRA, "Wnd",
                                 "Tag", *(m_pZSubtask->m_pzsRemoteWndTag) );
      }

      // Determine the current ctrl's entity in KZXRA ...
      // if the entity is not there, create it (if bCreate is TRUE).
...
ZMapAct::SetFontOverride( )
{
   ...
   // m_pCtrl->SetFont( m_pZSubtask->m_pFontCtrl );
      m_pCtrl->SendMessage( WM_SETFONT,
                            (WPARAM)(HFONT) *(m_pZSubtask->m_pFontCtrl) );
      return( TRUE );
   }
   else
   ...
   {
      m_pCtrl->SendMessage( WM_SETFONT,
                            (WPARAM)(HFONT) *(m_pZSubtask->m_pFontWS) );
      return( TRUE );
   }
...
zBOOL
ZMapAct::TranslateValue( zPCHAR pchValue, zBOOL bToDisplay, zSHORT nCol )
{
