                   ...
                   "ToolBar" )
{
   ...
   if ( ulButtonInfoLth )
   {
      ...
      k += zstrlen( m_pchButtonInfo + k ) + 1;
      nBtnCnt =
            (zSHORT) (*((zPLONG) (m_pchButtonInfo + (2 * sizeof( zLONG )))));
...
ZEditBar::DestroyWindow( )
{
   ...
   zPCHAR pch = new char[ sizeof( "-2147483647," ) * 4 ];
...
{
   zstrncpy( pchText, m_csLastSelect, lMaxTextLth );
...
ZEditBar::OnGetToolTipText( WPARAM wParam, LPARAM lParam )
{
   ...
   {
      zstrncpy( pchText, pBarItem->m_pchTipText, zZEIDON_TOOLTIP_TEXTLTH - 1 );
...
ZEditBar::OnToolBarSetSelect( WPARAM wParam, LPARAM lParam )
{
   ...
   if ( m_lLastSelect >= 0 && pBarItem )
   {
      if ( pBarItem->m_cpcViewName )
      {
         ...
         if ( v )
         {
            SetCursorFirstEntityByString( v, pBarItem->m_cpcEntity,
                                          pBarItem->m_cpcAttribute,
                                          m_csLastSelect, 0 );
         }
      }
      ...
      {
         ProcessActionLoop( m_pZSubtask, pBarItem->m_cpcAction,
                            0, 0, 0, FALSE, "ToolBar" );
      }
   }
...
ZEditBar::OnToolBarDropDown( WPARAM wParam, LPARAM lParam )
{
   ...
   // Popup the ListCtrl.
   VERIFY( pPopup->Display( &m_lstPopup, this,
...
ZEditBar::SetRowColText( zCPCHAR cpcText, zLONG lRow, zLONG lColumn )
{
   if ( lColumn <= m_nBtnCnt && lColumn > 0 )
   {
      ...
      if ( lRow == 0 )
      {
         ...
         if ( cpcText )
         {
            pBarItem->m_pchText = new char[ zstrlen( cpcText ) + 1 ];
            zstrcpy( pBarItem->m_pchText, cpcText );
         }
      ...
      if ( lRow == 1 )
      {
         ...
         if ( cpcText )
         {
            pBarItem->m_pchTipText = new char[ zstrlen( cpcText ) + 1 ];
            zstrcpy( pBarItem->m_pchTipText, cpcText );
         }
         return( 0 );
      }
   }
...
ZToolBar::LoadToolBar( UINT uResourceId )       // toolbar resource id
{
   ...
   for ( nIndex = bar.GetButtonCount( ); nIndex >= 0; nIndex-- )
   {
      ...
      {
         pszButtonText = _tcstok( (LPTSTR)(LPCTSTR) csText, seps );
         while ( pszButtonText )
         {
                    ...
                    zCPCHAR cpcAttribute )
{
   ...
   if ( cpcText )
   {
      m_pchText = new char[ zstrlen( cpcText ) + 1 ];
      zstrcpy( m_pchText, cpcText );
   }
   else
      m_pchText = 0;
   ...
   if ( cpcTipText )
   {
      m_pchTipText = new char[ zstrlen( cpcTipText ) + 1 ];
      zstrcpy( m_pchTipText, cpcTipText );
   }
   else
      m_pchTipText = 0;
                      ...
                      zCPCHAR cpcAttribute, zCPCHAR cpcScope )
{
   ...
   while ( nPos )
   {
      ...
      if ( pBarItem->m_ulId == ulId )
      {
         ...
         if ( cpcTipText )
         {
            pBarItem->m_pchTipText = new char[ zstrlen( cpcTipText ) + 1 ];
            zstrcpy( pBarItem->m_pchTipText, cpcTipText );
         }
   ...
   if ( pBarItem )
   {
      if ( pBarItem->m_pBarCombo )
      {
         ...
         if ( pWnd )
         {
            ...
            zstrncat( pchText, cs, lMaxTextLth );
         }
      ...
      // pBarItem->m_pBarCombo->GetLBText( lIdx, cs );
         ...
         zstrcpy( pchText, pBarItem->m_pchText );
                     ...
                     zLONG   lMaxTextLth )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for TB_GetLastSelection ",
                  cpcCtrlTag );
   }
                ...
                zLONG   lMaxTextLth )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for TB_GetItemText ",
                  cpcCtrlTag );
   }
                ...
                zCPCHAR cpcText )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for TB_SetItemText ",
                  cpcCtrlTag );
   }
                ...
                zCPCHAR cpcCtrlTag )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for TB_GetTextMode ",
                  cpcCtrlTag );
   }
                ...
                zSHORT  bTextMode )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for TB_SetTextMode ",
                  cpcCtrlTag );
   }
         ...
         if ( m_pEvents )
         {
            ...
            if ( pEvents )
            {
               nCode = pEvents->OnLButtonDown( pMsg->wParam,
                                 CPoint( (zSHORT) LOWORD( pMsg->lParam ),
...
ZTB_List::XPopupCtrlEvent::OnLButtonDown( UINT, CPoint pt )
{
   ...
   {
      pThis->SetItemState( hItem, LVIS_SELECTED | LVIS_FOCUSED,
...
ZTB_Combo::CBNSelchange( )
{
   ...
   {
      pBar->m_uLastDropDownId = m_ulId - 0x0100;
      pBar->PostMessage( zZEIDON_TOOLBAR_SET_SELECT,
                         (WPARAM) lSelectedItem,
                         (LPARAM) &m_csSendText );
   }
}
...
ZControlBarG::OnSize( UINT uType, int cx, int cy )
{
   if ( m_pZSubtask->m_pZMIXCtrl && (m_ulMapActFlag2 & zMAPACT_CREATED) )
   {
      ...
      {
         ::SetWindowPos( m_hWndFullClient, HWND_TOP,
                         0, 0, cx, cy, SWP_SHOWWINDOW );
      }
      else
      {
            ...
            zKZWDLGXO_Ctrl_DEF *pCtrlDef )
{
   if ( pCtrlDef->Subtype & zCONTROLBAR_GRIPPERS )
      return( new ZControlBarG( pZSubtask, pWndParent,
                                pzmaComposite, vDialog,
                                nOffsetX, nOffsetY, pCtrlDef ) );
   else
      return( new ZControlBarC( pZSubtask, pWndParent,
                                pzmaComposite, vDialog,
                                nOffsetX, nOffsetY, pCtrlDef ) );
}
                ...
                zSHORT  nVisible )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      CControlBar *pControlBar = DYNAMIC_DOWNCAST( CControlBar,
                                                   pzma->m_pCtrl );
      if ( pControlBar && pControlBar->m_pDockSite &&
           mIs_hWnd( pControlBar->m_hWnd ) )
      {
         zBOOL bVisible;
         bVisible = (pControlBar->GetStyle( ) & WS_VISIBLE) ? TRUE : FALSE;
         pControlBar->m_pDockSite->ShowControlBar( pControlBar,
                                                   nVisible, FALSE );
         return( bVisible );
      }

      TraceLineS( "drvr - Invalid control type for CBarShowOrHide ",
                  cpcCtrlTag );
   }
