   ...
   if ( m_pzsVName && m_pzsEName && m_pzsAName )
   {
                          ...
                          m_pZSubtask->m_vDialog, zLEVEL_ANY ) > 0 )
      {
         if ( CheckExistenceOfEntity( vApp, *m_pzsEName ) == zCURSOR_SET )
         {
            ...
            if ( zstrchr( pch, '&' ) )
            {
               zPCHAR  pchBuffer = new char[ (zstrlen( pch ) + 1) * 2 ];
   ...
   if ( m_lType > 1 )
   {
      if ( m_bForceNoOwnerDraw )
      {
         if ( CTLCOLOR_STATIC == uCtlColor )
         {
            ...
            pDC->SetTextColor( m_bUseDefaultTextColor ?
                  ::GetSysColor( COLOR_WINDOWTEXT ) : m_clrTextForeColor );
...
//             rect.left = (zLONG) (dSin * sizeExtent.cy);
   ...
   else
   {
      ...
      TextRotator.SetBackgroundColor( !m_bUseDefaultTextBackColor ?
                        m_clrTextBackColor : m_bUseDefaultBackColor ?
                            ::GetSysColor( COLOR_3DFACE ) : m_clrBackColor );
      ...
      {
         COLORREF clr = m_bUseDefaultBackColor ? m_clrBackColor / 2 :
      ...
      if ( m_hBackImage )
      {
         ...
         else
         {
            ...
            // The following lines are used if it is desirable to keep the
            // top-left edge of the bitmap fixed.  Otherwise, the center of
            // the bitmap remains "relatively fixed".
            if ( m_bFixTopLeft )
            {
               if ( X < 0 )
                  X = 0;
...
ZText::GetWindowText( zPCHAR pchStringBuf, int nMaxCount )
{
   ...
   GetWindowText( cs );
   pchStringBuf[ 0 ] = 0;
   zstrncat( pchStringBuf, cs, nMaxCount );
...
{
   zstrcpy( m_lf.lfFaceName, cpcFaceName );
   ReconstructFont( );
   return( *this );
}
...
// m_ulAngle = ulAngle;
   ...
   zPCHAR pchText = new char[ m_csText.GetLength( ) + 1 ];
   if ( pchText == 0 )
      return( 0 );
...
//       if ( pLD->chTopLeft & 1 )
      ...
      // if ( pLD->chTopLeft & 2 )
         ...
         {
            zstrcpy( pText->m_szFaceName, pLD->szFontName );
   ...
   if ( ulLth )
   {
      ...
      *pbMultiLine = ld.chMultiLine;
      ...
      if ( pchFaceName && (ulLth > 7 + (2 * sizeof( zLONG ))) )
      {
         ...
         *plType = ld.lType;
         zstrcpy( pchFaceName, ld.szFontName );
      }
   }
   else
   {
                      ...
                      zCPCHAR cpcFaceName )
{
        ...
        (cpcFaceName && *cpcFaceName) )
   {
      ...
      if ( cpcFaceName && *cpcFaceName )
         zstrcpy( ld.szFontName, cpcFaceName );
      else
         ld.szFontName[ 0 ] = 0;
   }
   else
