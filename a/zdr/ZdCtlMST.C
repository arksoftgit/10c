...
void
TreeCursorListener( zVIEW pView, zCPCHAR cpcEntityName,
                        zPVOID pvInstance )
{
...
ZMSTree::CursorListener( zVIEW pView, zCPCHAR cpcEntityName )
{
   if ( cpcEntityName && pView == m_vApp && IsMappedEntity( cpcEntityName ) )
      zstrncpy( m_szLastAccEntity, cpcEntityName, 33 );
}
...
ZMSTree::CreateZ( )
{
...
#endif
   m_dwStyle |= WS_BORDER | TVS_SHOWSELALWAYS | TVS_NOTOOLTIPS;
   ...
   if ( m_bNoImage == FALSE )
   {
      ...
      if ( IsDesignTime( ) )
      {
         if ( m_Images.Create( m_lBitmapWidth, m_lBitmapHeight, ILC_MASK, 3, 3 ) )
         {
            m_Images.Add( AfxGetApp( )->LoadIcon( IDI_FOLDERCLOSE ) );
            m_Images.Add( AfxGetApp( )->LoadIcon( IDI_FOLDEROPEN ) );
            m_Images.Add( AfxGetApp( )->LoadIcon( IDI_FOLDERSELECT ) );
         }
      ...
      {
         if ( m_Images.Create( m_lBitmapWidth, m_lBitmapHeight,
                               ILC_MASK, 2, 2 ) )
         {
            m_Images.Add( AfxGetApp( )->LoadIcon( IDI_FOLDERCLOSE ) );
            m_Images.Add( AfxGetApp( )->LoadIcon( IDI_FOLDEROPEN ) );
...
// TraceLineS( "ZMSTree::IsDesignTime", *m_pzsTag );
                          ...
                          zTREE_Tag_ForDesignTime ) == 0 )
   {
      if ( pzma && pzma->m_pCtrl && mIs_hWnd( pzma->m_pCtrl->m_hWnd ) &&
           zstrcmp( *(pzma->m_pzsTag), zTREE_Tag_ForDesignTime ) == 0 )
      {
         bDesignTime = TRUE;
      }
   }
   ...
   else
   {
      ...
      if ( bMultiSelect == FALSE )
      {
         ...
         if ( m_csCurrentEntity.IsEmpty( ) == FALSE )
         {
            ...
            if ( CheckEntityExistence( m_vApp, m_csCurrentEntity ) >= zCURSOR_SET )
            {
...
ZMSTree::SelectCurrentEntity( zBOOL bReset )
{
   ...
   if ( m_szLastAccEntity[ 0 ] )
   {
      if ( CheckEntityExistence( m_vApp, m_szLastAccEntity ) >= zCURSOR_SET )
      {
         zULONG ulEntityKey;
...
#endif
   if ( m_ItemColorText.GetSize( ) > 0 )
   {
      ItemColorText cfi;
      for ( int k = 0; k < m_ItemColorText.GetSize( ); k++ )
      {
         cfi = m_ItemColorText.GetAt( k );
         SetTxtItemColorText( cfi.csItemText, cfi.clrTextColor, FALSE );
      }
   }
}
...
//            *m_pzsTag, nRecurse );
   ...
   while ( bSibling )
   {
      ...
      {
         nRC = CheckEntityExistence( vApp, OL_Item->szEntityName );
      ...
      //              GetEntityInstanceHandle( vApp, OL_Item->szEntityName );
         ...
         if ( IsDesignTime( ) == FALSE )
         {
            ...
            zstrcpy( szEntityName, OL_Item->szEntityName );
         ...
         if ( (OL_Item->lFlags & zTREE_RECURSIVE) &&
              CheckEntityExistence( vApp, OL_Item->szRecursiveEntity ) == zCURSOR_SET )
         {
...
ZMSTree::SetBitmapFileToImageList( CString csBitmapFile )
{
   ...
   if ( hIcon == 0 || hIcon == (HICON) 1 )
   {
      ...
      SysReadZeidonIni( -1, "[Workstation]", "ResourcePath", szZeidonPath );

      if ( szZeidonPath[ 0 ] )
      {
         SysAppendcDirSep( szZeidonPath );
         zstrcat( szZeidonPath, csBitmapFile );
         SysConvertEnvironmentString( szWorkString, szZeidonPath );
      ...
      {
         hIcon = 0;
         TraceLineS( "Invalid bitmap file ", csBitmapFile );
      }
      else
      if ( hIcon == 0 )
      {
         TraceLineS( "Cannot access bitmap file ", csBitmapFile );
      }
   }
   ...
   {
      m_Images.Add( hIcon );
      lImage = m_Images.GetImageCount( ) - 1;
   }
...
{
   zLONG      lImage = -1;
   ItemColorFontImage cf_current;
   ItemColorFontImage cfi;
   zBOOL      bFound = FALSE;

   // Set default values for Item.
   if ( m_mapItemColorFontImage.Lookup( hCurrentItem, cf_current ) == 0 )
   {
      cf_current.color = CLR_INVALID;
      cf_current.logfont.lfFaceName[ 0 ] = 0;
      cf_current.csBitmapFile.Empty( );
      cf_current.lBitmapID = -1;
   }
         ...
         hItem = GetNextItemPhysical( hItem ) )
   {
      if ( m_mapItemColorFontImage.Lookup( hItem, cfi ) &&
           cfi.csBitmapFile == csBitmapFile && cfi.lBitmapID != -1 )
      {
         lImage = cfi.lBitmapID;
         bFound = TRUE;
         break;
      }
   }
   ...
   if ( bFound == FALSE )
      lImage = SetBitmapFileToImageList( csBitmapFile );

#ifdef DEBUG_ALL
   TraceLine( "ZMSTree::GetBitmapIdFromImageList %s       Image: %d   Found: %s   File: %s",
              *m_pzsTag, lImage, bFound ? "TRUE" : "FALSE", csBitmapFile );
#endif

   cf_current.lBitmapID = lImage;
   cf_current.csBitmapFile = csBitmapFile;

   m_mapItemColorFontImage[ hCurrentItem ] = cf_current;
...
// TraceLineS( "ZMSTree::SetItemBitmap ", *m_pzsTag );
   ...
   {
      lImage = zTREE_STATE_CLOSE_IMG;
   }
   else
   {
      lImage = GetBitmapIdFromImageList( hItem, pNode->m_csDefaultBitmapFile );
      if ( lImage < 0 )
         lImage = zTREE_STATE_CLOSE_IMG;
   }

   lSelectedImage = lImage;
   ...
   if ( IsDesignTime( ) == FALSE )
   {
      ...
      {
         lSelectedImage = GetBitmapIdFromImageList( hItem, pNode->m_csSelectedBitmapFile );
         if ( lSelectedImage < 0 )
            lImage = zTREE_STATE_OPEN_IMG;
      }
   }

   if ( lImage >= 0 )
      SetItemImage( hItem, lImage, lSelectedImage );
                                     ...
                                     zCPCHAR cpcBitmapFileName )
{
   ...
   {
      SysAppendcDirSep( szZeidonPath );
      zstrcat( szZeidonPath, cpcBitmapFileName );
      SysConvertEnvironmentString( szWorkString, szZeidonPath );
      lImage = GetBitmapIdFromImageList( hItem, szWorkString );
   }
                            ...
                            zULONG  ulMaxReturnBitmapNameLength )
{
   ...
   while ( lRC && lSize < m_lFontColorBitmapBlobSize - 1 )
   {
      ...
      if ( zstrcmp( OL_ItemColorFontImage->szEntityName, csEntityName ) == 0 )
      {
         lRC = 0;
         if ( OL_ItemColorFontImage->lUseSelectedBitmap != 1 )
         {
            zstrncat( pchBitmapFileName, OL_ItemColorFontImage->szSelectedBitmapFile,
                           ...
                           zULONG  ulMaxReturnBitmapNameLength )
{
   ...
   // If cpcEntityName = 0, set Entity Name from current Entity.
   if ( cpcEntityName == 0 || cpcEntityName[ 0 ] == 0 )
      csEntityName = GetCurrentEntityName( );
   else
      csEntityName = cpcEntityName;
   ...
   while ( lRC && lSize < m_lFontColorBitmapBlobSize - 1 )
   {
      ...
      if ( zstrcmp( OL_ItemColorFontImage->szEntityName, csEntityName ) == 0 )
      {
         ...
         {
            zstrncat( pchBitmapFileName, OL_ItemColorFontImage->szDefaultBitmapFile,
                      ulMaxReturnBitmapNameLength );
         }
      }
                           ...
                           zCPCHAR cpcBitmapFileName )
{
         ...
         hItem = GetNextItemPhysical( hItem ) )
   {
      ...
      {
         if ( lImage != zTREE_STATE_CLOSE_IMG )
         {
            if ( lImage == -1 )
            {
               lImage = GetBitmapIdFromImageList( hItem, cpcBitmapFileName );
               if ( lImage < 0 )
               {
                  lImage = LoadBitmapWithResourcePath( hItem,
                                                       cpcBitmapFileName );
                  if ( lImage < 0 )
                     break;
               }
         ...
         SetItemImage( hItem, lImage, lSelImageIdx );
         pNodeFound = pNode;
      } // endif ( pNode && pNode->m_csEntityName == csEntityName )
   } // endfor ( hItem = GetRootItem( )
         ...
         hItem = GetNextItemPhysical( hItem ) )
   {
      ...
      if ( pNode && pNode->m_csEntityName == csEntityName )
      {
         if ( lSelImageIdx != zTREE_STATE_OPEN_IMG )
         {
            if ( lSelImageIdx == -1 )
            {
               lSelImageIdx = GetBitmapIdFromImageList( hItem, cpcBitmapFileName );
               if ( lSelImageIdx < 0 )
               {
                  lSelImageIdx = LoadBitmapWithResourcePath( hItem, cpcBitmapFileName );
                  if ( lSelImageIdx < 0 )
                     break;
               }
            }
         ...
         if ( lImage == -1 )
         {
            ...
            {
               lImage = GetBitmapIdFromImageList( hItem,
                                                  pNode->m_csDefaultBitmapFile );
            }
         }
   ...
   if ( mIs_hWnd( m_hWnd ) == 0 ||
        ...
        ((cpcEntityName && *cpcEntityName) &&
         (IsMappedEntity( cpcEntityName ) == FALSE ||
          CheckEntityExistence( m_vApp, cpcEntityName ) < zCURSOR_SET)) )
   {
      return( -1 );
   }
                                  ...
                                  zBOOL   bEnsureVisible )
{
   ...
   if ( m_ItemColorText.GetSize( ) > 0 )
   {
      ...
      {
         cfi = m_ItemColorText.GetAt( k );
         if ( cfi.csItemText == cpcItemText )
         {
            bTextFound = TRUE;
            cfi.clrTextColor = clrTextColor;
            m_ItemColorText.SetAt( k, cfi );
         }
      }
   }
   ...
   if ( bTextFound == FALSE )
   {
      ...
      cfi.csItemText = cpcItemText;
      cfi.clrTextColor = clrTextColor;
      m_ItemColorText.Add( cfi );
   }
...
ZMSTree::SetNewFontColorBitmapBlob( ZOL_Node *pNode, CString csEntityName )
{
   ...
   while ( lSize < m_lFontColorBitmapBlobSize -1 )
   {
      ...
      if ( zstrcmp( csText, csEntityName ) == 0 )
      {
         ...
         zstrcpy( OL_ItemColorFontImage->szDefaultBitmapFile, pNode->m_csDefaultBitmapFile );
         zstrcpy( OL_ItemColorFontImage->szSelectedBitmapFile, pNode->m_csSelectedBitmapFile );
      }
...
//            pNode1->m_hItem, csItem1, pNode2->m_hItem, csItem2 );
   return( zstrcmp( csItem2, csItem1 ) );
}
...
ZMSTree::GetExpandState( zCPCHAR cpcEntityName )
{
   ...
   if ( CheckEntityExistence( m_vApp, cpcEntityName ) >= zCURSOR_SET )
   {
                               ...
                               zPCHAR  pcFaceName )
{
   ...
   if ( pcFaceName )
      zstrcpy( fntTextFont.lfFaceName, pcFaceName );
                               ...
                               zPCHAR   pcFaceName )
{
   ...
   if ( bFound )
   {
      ...
      *plEscapement = fntTextFont.lfEscapement;
      ...
      zstrcpy( pcFaceName, fntTextFont.lfFaceName );
...
// End BLZ
   ...
   // HTREEITEM hRootItem = GetRootItem( );
      ...
      while ( hItem && k-- )
      {
         ...
         bColorFont = m_mapItemColorFontImage.Lookup( hItem, cfi ) ? TRUE : FALSE;

         if ( bColorFont && cfi.logfont.lfFaceName[ 0 ] )
         {
            logfont = cfi.logfont;
            bLogFont = TRUE;
         }
         ...
         if ( (bColorFont && logfont.lfWeight >= FW_BOLD) ||
              GetItemBold( hItem ) )
         {
            bBold = TRUE;
            logfont.lfWeight = FW_BOLD;
   ...
   if ( pNode )
   {
                          ...
                          m_vSubtask, zLEVEL_SUBTASK ) >= 0 )
      {
         ...
         if ( nRC >= zCURSOR_SET )
         {
            ...
            zstrcpy( szEntityName, pNode->m_csEntityName );
   ...
   if ( pEdit )
   {
      ...
      if ( (m_lStyle & zTREES_NOSHOW_ENTITY) == 0 )
      {
         ...
         else
         {
            ...
            // Set result to 0 to do not accept the user changes
            *pResult = 0;
         }
      }
...
{
   zstrcpy( pchEntityName, m_csLastExpandedEntity );
}

void
ZMSTree::GetLastCollapsedEntityName( zPCHAR pchEntityName )
{
   zstrcpy( pchEntityName, m_csLastCollapsedEntity );
}

void
ZMSTree::GetExpandingEntityName( zPCHAR pchEntityName )
{
   zstrcpy( pchEntityName, m_csExpandingEntity );
}

void
ZMSTree::GetCollapsingEntityName( zPCHAR pchEntityName )
{
   zstrcpy( pchEntityName, m_csCollapsingEntity );
}
   ...
   if ( pNodeNew )
   {
                          ...
                          m_vSubtask, zLEVEL_SUBTASK ) >= 0 )
      {
         ...
         if ( nRC < zCURSOR_SET )
         {
            ...
            zstrcpy( szEntityName, pNodeNew->m_csEntityName );
            GetRealEntityName( m_vApp, szEntityName );
            if ( zstrcmp( szEntityName, pNodeNew->m_csEntityName ) != 0 )
            {
...
// TraceLineS( "ZMSOutLiner::DeselectAllEntities ", *m_pzsTag );
   SelectItem( 0 );
   m_hItemFirstSel = 0;
...
ZMSTree::HandleRemoteState( )
{
   ...
   if ( vXRA )
   {
      nRC = CheckEntityExistence( vXRA, m_csTreeSelectionEntity );
      if ( nRC < zCURSOR_SET )
      {
...
// if ( lPageNbr > 1 )
   {
      ...
      k = zstrlen( szPageNbr );
      szPageNbr[ k ] = '/';
      zltoa( m_lMaxPage, szPageNbr + k + 1 );
      rectItem = m_rectDraw;
...
ZMSTree::PrintHeadFoot( CDC *pDC, zLONG lPage )
{
   ...
   pDC->DrawText( csTemp, rectHeader,
   ...
   pDC->DrawText( csTemp, rectFooter,
...
//              pcFaceName     - Return FaceName (font face name 0 = default)
//
//  RETURNS:   -1 - Error locating control
//             Otherwise 0
//
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
           ...
           (pOL = DYNAMIC_DOWNCAST( ZMSTree, pzma->m_pCtrl )) != 0 )
      {
         zLONG nRC = pOL->GetTextFontForEntity( cpcEntityName, plSize, plBold,
                                                ...
                                                plTextColor, plEscapement, pcFaceName );
         return( nRC );
      }
#endif
   }
...
//              pcFaceName     - FaceName (font face name 0 = default)
//
//  RETURNS:   -1 - Error locating control
//             Otherwise old Color
//
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
           ...
           (pOL = DYNAMIC_DOWNCAST( ZMSTree, pzma->m_pCtrl )) != 0 )
      {
         zLONG nRC = pOL->SetTextFontForEntity( cpcEntityName, lSize, lBold,
                                                ...
                                                lTextColor, lEscapement, pcFaceName );
         return( nRC );
      }
#endif
   }
...
//
//
//  PURPOSE:    This operation selects an tree item for a specified
                          ...
                          zCPCHAR  cpcItemText )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      {
      // zCHAR szMsg[ 256 ];
      // zsprintf( szMsg, "OL_SetTextForCurrentItem Tag: %s  %s", cpcCtrlTag, cpcItemText );
                                  ...
                                  zCPCHAR  cpcCtrlTag )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pzma->m_ulIdNbr != zTREE_CONTROL )
      {
         TraceLineS( "drvr - Invalid control type for OL_ResetAllTextColorsForItemText ",
                     cpcCtrlTag );
         return( -1 );
      }
#endif
                            ...
                            zBOOL    bEnsureVisible )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pzma->m_ulIdNbr != zTREE_CONTROL )
      {
         TraceLineS( "drvr - Invalid control type for OL_SetTextColorForItemText ",
                     cpcCtrlTag );
         return( -1 );
      }
#endif
      ...
      if ( pzma && pzma->m_pCtrl &&
           ...
           (pOL = DYNAMIC_DOWNCAST( ZMSTree, pzma->m_pCtrl )) != 0 )
      {
         zLONG lRC = pOL->SetTextItemColorText( cpcItemText, clrTextColor,
                                                   bEnsureVisible );
         return( lRC );
      }
#endif
                       ...
                       zLONG    lColor )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pzma->m_ulIdNbr != zTREE_CONTROL )
      {
         TraceLineS( "drvr - Invalid control type for OL_SetBackgroundColor ",
                     cpcCtrlTag );
         return( -1 );
      }
#endif
                            ...
                            zPCHAR  lpReturnEntityName )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pzma->m_ulIdNbr != zTREE_CONTROL )
      {
         TraceLineS( "drvr - Invalid control type for OL_GetCollapsingEntityName ",
                     cpcCtrlTag );
         return( -1 );
      }
#endif
                               ...
                               zPCHAR  lpReturnEntityName )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pzma->m_ulIdNbr != zTREE_CONTROL )
      {
         TraceLineS( "drvr - Invalid control type for OL_GetLastCollapsedEntityName ",
                     cpcCtrlTag );
         return( -1 );
      }
#endif
                           ...
                           zPCHAR  lpReturnEntityName )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pzma->m_ulIdNbr != zTREE_CONTROL )
      {
         TraceLineS( "drvr - Invalid control type for OL_GetExpandingEntityName ",
                     cpcCtrlTag );
         return( -1 );
      }
#endif
                         ...
                         zPCHAR   lpReturnEntityName )
{
   ...
   // TraceLineI( "OL_GetCurrentEntityName EntityKey: ", ulEntityKey );
      if ( bContinue && ulEntityKey )
      {
         ...
         if ( pNode )
         {
            OL_SetCursorByEntityNumber( vApp, pNode->m_csEntityName,
                                        ulEntityKey );
            zstrcpy( lpReturnEntityName, pNode->m_csEntityName );
      ...
      {
         // *HH*
         OL_SetCursorByEntityNumber( vApp,
                            pOL->m_pTree->OL_EntryCurr->m_szEntityName,
                            ulEntityKey );
         if ( lpReturnView )
            *lpReturnView = vApp;

         zstrcpy( lpReturnEntityName,
                  pOL->m_pTree->OL_EntryCurr->m_szEntityName );
                            ...
                            zULONG  ulEntityNbr )
{
   ...
   if ( ulEntityNbr )
   {
      ...
      zstrcpy( szEntityName, cpcEntityName );
      GetRealEntityName( v, szEntityName );

      if ( SetEntityCursor( v, szEntityName, 0,
                            zQUAL_ENTITYKEY | zPOS_FIRST | zRECURS,
                            (zCPVOID) &ulEntityNbr,
                            0, 0, 0, zSCOPE_OI, 0 ) >= zCURSOR_SET )
      {
   ...
   for ( ;
         ...
         lpSearchEntityInstance = (LPENTITYINSTANCE)
                               zGETPTR( lpSearchEntityInstance->hNextHier ) )
   {
...
//    TraceLineS( "", "" );
//
// TraceLine( "OL_GetCurrentEntityNumber: %s (%s) Key:0x%08x ================",
//            cpcEntityName, szEntityName, *plEntityNbr );
// DisplayEntityInstance( v, cpcEntityName );
   ...
   *pulEntityNbr = -1;
   ...
   // We need to count entities matching the name of cpcEntityName.
   lpSearchEntityInstance = (LPENTITYINSTANCE)
                                zGETPTR( lpSearchEntityInstance->hPrevHier );
   for ( ;
         lpSearchEntityInstance;
         lpSearchEntityInstance = (LPENTITYINSTANCE)
                               zGETPTR( lpSearchEntityInstance->hPrevHier ) )
