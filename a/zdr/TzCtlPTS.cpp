                 ...
                 CFormView( IDD_INPUTFORM )
{
   ...
   SysReadZeidonIni( -1, "[ActiveX]", "TraceAmbientPropertyRequest", pszStorageFile );
                       ...
                       zPVOID pvSortData )
{
         ...
         (nRC == 1 || nRC == -1) )
   {
      ...
      zsprintf( szMsg, "Ctrl1: %s @Y:%ld   Ctrl2: %s @Y:%ld  RC = %ld",
...
// #define zDEBUG_ADJUST
   ...
   //               0123456789012345678
   zstrcpy( szMsg, "Prior to Adjust - x " );
   for ( nPos = 0; nPos < m_pPainterWindow->m_nCtrlCnt; nPos++ )
   {
      pCtrl = m_pPainterWindow->m_CtrlList[ nPos ];
      if ( pCtrl->m_chType )
      ...
      zstrcpy( szMsg + 20, pCtrl->m_csTag );
   ...
   for ( nPos = 0; nPos < m_pPainterWindow->m_nCtrlCnt; nPos++ )
   {
           ...
           zstrcmp( pCtrl->m_csTag, "FinancialAcctTransaction" ) == 0 )
      {
         ...
         TraceLineS( "TZPainterClient::AdjustGroupSets: FinancialAcctTransaction",
                     szMsg );
      }
   }
#endif
   ...
   for ( nPos = 0; nPos < m_pPainterWindow->m_nCtrlCnt; nPos++ )
   {
      ...
      pCtrl = m_pPainterWindow->m_CtrlList[ nPos ];
      zsprintf( szMsg, " %s PX %ld PY %ld SX %ld SY %ld", pCtrl->m_csTag,
                pCtrl->m_rectCtrl.left, pCtrl->m_rectCtrl.top,
                pCtrl->m_rectCtrl.right - pCtrl->m_rectCtrl.left,
                pCtrl->m_rectCtrl.bottom - pCtrl->m_rectCtrl.top );
...
//                pLastGroupSet->m_rectCtrl.bottom );
   ...
   if ( m_nRecurse == 0 )
   {
      ...
      for ( nPos = 0; nPos < m_pPainterWindow->m_nCtrlCnt; nPos++ )
      {
         ...
         pCtrl = m_pPainterWindow->m_CtrlList[ nPos ];
         zsprintf( szMsg, " %s PX %ld PY %ld SX %ld SY %ld  RX %ld  RY %ld",
                   pCtrl->m_csTag,
                   pCtrl->m_rectCtrl.left, pCtrl->m_rectCtrl.top,
                   pCtrl->m_rectCtrl.right - pCtrl->m_rectCtrl.left,
                   pCtrl->m_rectCtrl.bottom - pCtrl->m_rectCtrl.top,
                                    ...
                                    zSHORT        nIdNbr )
{
   ...
   if ( bCtrlKey && nIdNbr )
   {
      zsprintf( szLabel, "&xxdo Clone %d item%s\tCtrl+x",
