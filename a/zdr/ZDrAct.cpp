...
ProcessEvent( const ZMapAct *pzma,
              zLONG lEvent )
{
   if ( pzma->m_pZSubtask->m_bStopEvents == FALSE &&
        pzma->m_pchEventAction &&
        (pzma->m_pZSubtask->m_pZTask->m_pZSubtaskConversational == 0 ||
         pzma->m_pZSubtask->m_pZTask->m_pZSubtaskConversational ==
                                                    pzma->m_pZSubtask) )
   {
      zSHORT k;
      zLONG  l;
      ...
      for ( k = 0; k < pzma->m_nEventActionCnt; k++ )
      {
         ...
         if ( lEvent == l )
         {
            ...
            l = *((zPLONG) (pzma->m_pchEventAction +
                           ((k * 2 * sizeof( zLONG )) + sizeof( zLONG ))));
            SetEntityCursor( pzma->m_pZSubtask->m_vDialog,
                             szlAct, szlSyncIdx,
                             zPOS_FIRST | zQUAL_INTEGER,
                             &l, 0, 0, 0, 0, 0 );
            GetAddrForAttribute( (zPCHAR *) &pch,
                                 pzma->m_pZSubtask->m_vDialog,
                                 szlAct, szlTag );
//          TraceLineS( "Processing EventAct - ", pch );
//          TraceLineI( "           SyncIdx = ",
//                      *((zPLONG) (pzma->m_pchEventAction + k)) );
         // pzma->m_pZSubtask->m_pEventParms = 0; can't do this!
            ...
            ProcessActionLoop( pzma->m_pZSubtask, pch,
                               GetMessagePos( ), 0, 0, FALSE, "CtrlEvent" );
#else
            if ( pzma->m_pZSubtask->m_pZFWnd &&
                 mIs_hWnd( pzma->m_pZSubtask->m_pZFWnd->m_hWnd ) )
            {
               pzma->m_pZSubtask->m_pZFWnd->
                       PostMessage( zZEIDON_PROCESS_EVENT,
                                    GetMessagePos( ), (zLONG) pch );
            }
            else
            {
               ProcessActionLoop( pzma->m_pZSubtask, pch,
...
//          TraceLineS( "Processing EventAct - ", pch );
            ...
            pzma->m_pZSubtask->m_pzsLastCtrlTag = pzma->m_pzsTag;
#ifdef zREMOTE_SERVER
            return( ProcessActionLoop( pzma->m_pZSubtask, pch,
                                       GetMessagePos( ), 0, 0,
                                       FALSE, "CtrlEvent" ) );
#else
            if ( pzma->m_pZSubtask->m_pZFWnd &&
                 mIs_hWnd( pzma->m_pZSubtask->m_pZFWnd->m_hWnd ) )
            {
               return( pzma->m_pZSubtask->m_pZFWnd->
                       SendMessage( zZEIDON_PROCESS_EVENT,
                                    GetMessagePos( ), (zLONG) pch ) );
            }
            else
            {
...
//
// Process the requested action ... note that if vResumeSubtask is non-zero,
// pZSubtask has been set to the ZSubtask corresponding to the resume subtask.
// If pchActionName is null ... if the action has been set for this ZSubtask,
// process the "dynamic" action, otherwise return.  I am aware that this
// function violates all rules of structured programming.  If you would like
// to analyze the function and re-write it to conform to all these rules,
// be my guest.  My guess is it may conform by using purist's goto's and
               ...
               zCPCHAR  cpcEvent )
{
   ...
   {
      zsprintf( szMsg, "ProcessAction Name: %s   Subtask: %ld   Type: %ld",
   ...
   if ( pZSubtask->m_ulSubtaskFlags & zSUBTASK_MAPPING_FROM_OI )
   {
      ...
      TraceLineS( "(drvr) Waiting for MapFromOI to complete for Action: ",
                  cpcActionName ? cpcActionName : cpcEvent ? cpcEvent : "" );
   ...
   // Implement a stack of Actions to prevent one action from overlaying
   // the processing of another.  The stack should probably never go deeper
   // than two or three, so the definition of 16 should be way more than
   // sufficient ... we will check the depth, but will essentially punt if
   ...
   do    // to implement purist's goto
   {
      ...
      {
      // zsprintf( szMessage, "(drvr[%ld", pZSubtask->m_ulIdNbr );
         zsprintf( szMessage, "(drvr[0x%08x.%ld",
                   v ? v->hTask : 0, pZSubtask->m_ulSubtaskIdNbr );
      // zstrcpy( szMessage, "(drvr[" );
      // zltoa( (zLONG) pZSubtask, szMessage + 6 );

         if ( pAction && pAction->m_szActDlg[ 0 ] )
         {
            zstrcat( szMessage, "." );
            zstrcat( szMessage, pAction->m_szActDlg );
            zstrcat( szMessage, "." );
            zstrcat( szMessage, pAction->m_szActWnd );
         }
         else
         if ( pZSubtask )
         {
            zstrcat( szMessage, "." );
            zstrcat( szMessage, *(pZSubtask->m_pzsDlgTag) );
            zstrcat( szMessage, "." );
            zstrcat( szMessage, *(pZSubtask->m_pzsWndTag) );
         }

         zstrcat( szMessage, "]) Action=>" );

         // Localize Action name so it can be modified.
         if ( cpcActionName )
         {
            zstrcpy( szActionName, cpcActionName );
            zstrcat( szMessage, szActionName );
         }
         else
         {
            szActionName[ 0 ] = 0;
            zstrcat( szMessage, "(code)" );
         }
      }
      else
      {
      ...
      if ( szActionName[ 0 ] == 0 )
      {
         ...
         ASSERT( ((ActDef.Type & ~zWAB_ProcessImmediateReturn) >= 0 &&
                  (ActDef.Type & ~zWAB_ProcessImmediateReturn) < 100) ||
                 (ActDef.Type & ~zWAB_ProcessImmediateReturn) ==
                                              zWAB_TerminateActionForError ||
                 ActDef.Type == -1 );
         pAction->m_lActType = 0;
         zstrcpy( ActDef.DlgN, pAction->m_szActDlg );
         zstrcpy( ActDef.WndN, pAction->m_szActWnd );
      }
      else
      if ( SetCursorFirstEntityByString( pZSubtask->m_vDialog, szlAct,
                                         szlTag, szActionName,
                                         0 ) > zCURSOR_UNCHANGED )
      {
...
#endif
         GetStructFromEntityAttrs( (zPCHAR) ActDef.Tag, pZSubtask->m_vDialog, szlAct );
#ifdef DEBUG_LODSTRUCT
      ...
      //    TraceLineS( "zdract ActDef CheckByte error??", "" );
#endif
...
#ifdef DEBUG_ACTION
         TraceBlob( "ProcessAction:",
                    pZSubtask->m_pchEnabledAction + sizeof( zSHORT ),
                    *((zSHORT *) pZSubtask->m_pchEnabledAction) );
#endif
         zSHORT nEnabled =
                 pZSubtask->m_pchEnabledAction[ (zSHORT) (ActDef.SyncIdx +
                                                         sizeof( zSHORT )) ];
         if ( nEnabled <= 0 )
         {
            if ( pZTask->m_nTraceAction > 1 )
            {
            // zstrcat( szMessage, ActDef.Tag );
               if ( nEnabled < 0 )
                  zstrcat( szMessage, " (auto)" );

               zstrcat( szMessage, " disabled at index: " );
      ...
      zstrcpy( pZSubtask->m_szActionName, szActionName );  // dks conversational fix
      if ( pZTask->m_nTraceAction )
      {
         nLth = zstrlen( szMessage );
         zstrcpy( szMessage + nLth, "  Event=>" );
         zstrcpy( szMessage + nLth + 9, cpcEvent );
         if ( ActDef.DO[ 0 ] )
         {
            zstrcat( szMessage, "  Operation=>" );
            zstrcat( szMessage, ActDef.DO );
         }
         else
         {
            if ( ActDef.Type == zWAB_SwitchToAction )
            {
               zstrcat( szMessage, "  (no code)  WAB=>" );
               TraceLineS( szMessage, "SwitchToAction" );
            }
            else
            {
               nLth = zstrlen( szMessage );
               sprintf( szMessage + nLth, "  (no code)  WAB=>%s %s.%s",
                        g_Actions[ ActDef.Type ], ActDef.DlgN, ActDef.WndN );
      ...
      {
         if ( ActDef.OperCtrl & zACTION_DIALOG_CODE_ONLY ||
              lOperCtrl & zACTION_DIALOG_CODE_ONLY )
         {
            // If Dialog Operation code exists for this action, execute it.
            if ( pZTask->m_nTraceAction )
            {
               zstrcat( szMessage, " (code only)" );
            ...
            // zstrcpy( pZSubtask->m_szActionName, szActionName );  // dks conversational fix
               ...
               if ( lRC == zNO_APPLICATION_EVENT ||
                    (ulSubtaskState & zSUBTASK_STATE_DELETED) ==
            ...
            {
               zstrcpy( szActionName, pAction->m_szAction );
               cpcActionName = szActionName;

               if ( nLth && pZTask->m_nTraceAction )
               {
                  double dTime = ((double)
                          (SysGetTickCount( ) - ulTime)) / zTICKS_PER_SECOND;
               ...
               {
                  // Get the structure of attributes for this action.
                  GetStructFromEntityAttrs( (zPCHAR) ActDef.Tag,
                                            pZSubtask->m_vDialog, szlAct );
                  pAction->m_lActType = ActDef.Type;
               }

               // This is against all structured programming rules and as
               // such is clearly indefensible, but here GOes anyway ...
            ...
            if ( ActDef.Type != zWAB_StartPopupMenu )
            {
               // The only other "code only" option is to stay on the window
               // (no refresh).  If refresh must occur, call it from code.
         ...
         if ( vApp && ActDef.SB )
         {
            ...
            {
               nTemporalState =
                (MiGetTemporalStateOfEntity( vApp,
                 ActDef.EN ) == zTES_SUBOBJECT_ROOT ) ? 1 : -1;
            }
            ...
            if ( ActDef.SB & zTEMPORAL_SUBOBJECT_VERIFY )
            {
               ...
               if ( ActDef.Prompt[ 0 ] == 0 )
               {
                  if ( ActDef.SB & zTEMPORAL_SUBOBJECT_DELETE )
                     zstrcpy( szBehavior, "Delete " );
                  else
                  if ( ActDef.SB & zTEMPORAL_SUBOBJECT_EXCLUDE )
                     zstrcpy( szBehavior, "Exclude " );
                  else
                  if ( ActDef.SB & zTEMPORAL_SUBOBJECT_ACCEPT )
                     zstrcpy( szBehavior, "Accept " );
                  else
                  if ( ActDef.SB & zTEMPORAL_SUBOBJECT_CANCEL )
                     zstrcpy( szBehavior, "Cancel " );

                  zstrcpy( szTitle, szBehavior );
                  zstrcat( szTitle, "Confirmation " );

                  zstrcpy( szText, "OK to continue with " );
                  zstrcat( szText, szBehavior );

                  zstrcat( szText, "of " );
                  zstrcat( szText, ActDef.EN );
                  zstrcat( szText, "?" );
               }
               else
               {
                  zstrcpy( szTitle, "Confirmation " );
                  zstrcpy( szText, ActDef.Prompt );
               }

               if ( MessagePrompt( vApp, 0, szTitle, szText,
                      FALSE, zBUTTONS_YESNO,
                      ActDef.SB == zTEMPORAL_SUBOBJECT_ACCEPT ?
                                                zRESPONSE_YES : zRESPONSE_NO,
            ...
            else
            {
               if ( ActDef.SB & zTEMPORAL_SUBOBJECT_CANCEL )
               {
                  if ( (lRC = CancelSubobject( vApp, ActDef.EN ))
                                                        < zCURSOR_UNDEFINED )
                  {
               ...
               if ( ActDef.SB & zTEMPORAL_SUBOBJECT_DELETE )
               {
                  if ( (lRC = DeleteEntity( vApp, ActDef.EN,
                  ...
                  // TraceLineS( "Auto DeleteEntity", "" );
                     ...
                     // Skip this CreateTemporalSubobjectVersion because if
                     // you don't stay on the same window, you don't want
                     // the entity to be versioned.
               ...
               if ( ActDef.SB & zTEMPORAL_SUBOBJECT_EXCLUDE )
               {
                  if ( (lRC = ExcludeEntity( vApp, ActDef.EN,
                                             zREPOS_NONE )) < zCURSOR_UNDEFINED )
                  {
               ...
               {
//                TraceLineS( "(drvr) Resetting undefined cursor after "
//                            "automatic subobject execution for entity ",
//                            ActDef[ ulAD ].EN );
                  lRC = SetCursorPrevEntity( vApp, ActDef.EN, 0 );

                  // Even if SetCursorPrevEntity fails according to the
                  // return code, it will still set cursor to a defined
                  // position so that the code below would set cursor to
                  // the second entity if permitted to execute.
               // if ( lRC < zCURSOR_SET )
                 ...
                 ActDef.SB & zTEMPORAL_SUBOBJECT_CREATE_ENTITY )
            {
               if ( ActDef.SB & zTEMPORAL_SUBOBJECT_CREATE_VERSION )
               {
                  if ( CreateTemporalSubobjectVersion( vApp,
                                                       ActDef.EN ) < 0 )
                  {
               ...
               {
                  if ( CreateTemporalEntity( vApp, ActDef.EN,
                                             zPOS_AFTER ) < 0 )
                  {
               ...
               {
                  if ( CreateEntity( vApp, ActDef.EN,
                                     zPOS_AFTER ) < 0 )
                  {
         ...
         if ( szActionName[ 0 ] && ActDef.DO[ 0 ] )
         {
            // This next if should be removed at an appropriate point
            // in the future ... when we are sure that operation code
            // and subobject behaviors are no longer present in legacy
            // code. // DKS 12/9/96
         // if ( vApp && ActDef[ ulAD ].SB )
         // {
         //    ::MessageBox( 0, "Dialog operation code and Automatic\n"
         //                  "Subobject Behavior occur simultaneously\n"
         //                  "Please correct", "Zeidon runtime error",
         //                  MB_OK );
         // }
      ...
      if ( pAction->m_lActType )
      {
         ...
         if ( ActDef.Type == zWAB_SwitchToAction )
         {
            ...
            // This is against all structured programming rules and as such
            // is clearly indefensible, but here goes anyway ...
            continue;  // go back up (in code) and restart
                       // processing this new action
         }
      }
...
CLEANUP_ACT_label:
   ...
   // We just want to skip the action switch below.  There appears to be
   // a bug in MSVC that thinks CLEANUP_ACT_label is in the exception
   // handing code if it is placed after the switch.  So the label is
   // placed here and we do nothing in the switch statement.
...
SKIP_TO_ACTION_label:
   ...
   if ( nLth && (pZTask == 0 || pZTask->m_nTraceAction) )
   {
      if ( nLth > 0 )
      {
         if ( ActDef.DO[ 0 ] )
         {
            zstrcpy( szMessage + nLth, "  Operation=>" );
            nLth += 13;
            zstrcpy( szMessage + nLth, ActDef.DO );
            nLth += zstrlen( szMessage + nLth );
         }

         zstrcpy( szMessage + nLth, "  WAB=>" );
         nLth += 7;
         if ( ActDef.Type < 100 )
            zstrcpy( szMessage + nLth, g_Actions[ ActDef.Type ] );
         else
            zltoa( ActDef.Type, szMessage + nLth );
      }

      double dTime =
            ((double) (SysGetTickCount( ) - ulTime)) / zTICKS_PER_SECOND;

      nLth = zstrlen( szMessage );
      sprintf( szMessage + nLth, " [%1.3f sec] %s.%s",
               dTime, ActDef.DlgN, ActDef.WndN );
      TraceLineS( szMessage, "" );
   }
...
//       TraceLineI( "nMultiRefresh = ", nMultiRefresh );
      ...
      case zWAB_StartOrFocusModelessSubwindow:
      {
         ...
         if ( fnGetSubtask( pZSubtask, &vSubtaskReturn,
                            ActDef.DlgN, ActDef.WndN, TRUE ) )
         {
            pZTask->m_vNewAppSubtask = 0;
      ...
      // TraceLineS( "ProcessAction ModelessApp Lock Mutex ZDr", "" );
         ...
         while ( lPos > 1 )
         {
            if ( g_pSubtaskList[ lPos ].pZSubtask &&
                 ...
                 zstricmp( g_pSubtaskList[ lPos ].DlgN, ActDef.DlgN ) == 0 &&
                 (ActDef.WndN[ 0 ] == 0 ||
                  zstricmp( g_pSubtaskList[ lPos ].WndN, ActDef.WndN ) == 0) )
            {
               ...
               if ( vSubtaskReturn && MiValidView( vSubtaskReturn ) &&
                    ...
                    mIs_hWndEnabled( g_pSubtaskList[ lPos ].hWnd ) )
               {
                  ...
                  ::SetWindowPos( g_pSubtaskList[ lPos ].hWnd,
                                  HWND_TOP, 0, 0, 0, 0,
                                  SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE );
                  ::FlashWindow( g_pSubtaskList[ lPos ].hWnd, FALSE );
               // ::SetFocus( g_pSubtaskList[ lPos ].hWnd );

                  lPos = -1;  // break (out of for) and set flag to cause
                              // break (out of case)
               }
            }
      ...
      case zWAB_StartNewApp:
      {
         ...
         if ( SfGetApplicationForSubtask( &pApp, vSubtask ) == 0 && pApp )
         {
            ...
            zstrcpy( szCmdLine, "zdr.exe ~~" );
            zstrcat( szCmdLine, pApp->szName );
            zstrcat( szCmdLine, "~" );
            zstrcat( szCmdLine, ActDef.DlgN );
            StartTask( pZSubtask->m_vDialog, szCmdLine, SW_SHOW );

            // Try to make this subtask the top subtask of the newly
            // created subtask.
            if ( (ActDef.Type & ~zWAB_ProcessImmediateReturn) ==
                                              zWAB_StartOrFocusModelessApp )
            {
               for ( zSHORT k = 0; k < 4; k++ )
               {
                  if ( (pZSubtaskParent = fnGetSubtask( pZSubtask, &vSubtask,
                                                        ActDef.DlgN, ActDef.WndN,
                                                        TRUE )) != 0 )
                  {
#ifdef zREMOTE_SERVER
        ...
        pZTask && pZTask->m_nTraceAction )
   {
      ...
      {
         TraceLine( "DGC: ProcessAction termination error: "
                       "OperIdx Original: %d   Current: %d",
                    nOperIdx, lpTask->nOperIdx );
                 ...
                 zKZWDLGXO_Act_DEF ActDef )
{
   ...
   zBOOL bConversational =
      (pZSubtask->m_ulSubtaskFlags & zSUBTASK_SUBWND_CONVERSATIONAL) ? TRUE : FALSE;
   if ( lBehavior == zWAB_ExitDialogTask ) // terminate application
   {
      ...
      while ( pZSubtask->m_pZParent &&
              ((ulSubtaskState =
                  ZSubtask::GetSubtaskState( pZSubtask->m_pZParent )) &
                         zSUBTASK_STATE_DELETED) != zSUBTASK_STATE_DELETED )
      {
...
#ifdef DEBUG_ALL
      zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] fnProcessReturn (to Top) pZSubtask: ",
                pZSubtask->m_pZSocketTask,
                AfxGetThread( ),
                pZSubtask->m_pZSocketTask->m_pWinThread,
                pZSubtask->m_pZSocketTask->m_sock );
   ...
   {
      if ( ActDef.DlgN[ 0 ] && ActDef.WndN[ 0 ] &&
           zstricmp( *(pZSubtask->m_pzsDlgTag), ActDef.DlgN ) == 0 &&
           zstricmp( *(pZSubtask->m_pzsWndTag), ActDef.WndN ) == 0 )
      {
         ActDef.DlgN[ 0 ] = 0;
      }

      // Delete this window from subtask list.  If the Dialog Name and
      // the Window Name are specified for return to parent, keep deleting
      ...
      while ( pZSubtask )
      {
...
#ifdef zREMOTE_SERVER
#ifdef DEBUG_ALL
         zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] fnProcessReturn pZSubtask: ",
                   pZSubtask->m_pZSocketTask,
                   AfxGetThread( ),
                   pZSubtask->m_pZSocketTask->m_pWinThread,
                   pZSubtask->m_pZSocketTask->m_sock );
         ...
         if ( ActDef.Type & zWAB_ProcessImmediateReturn )
         {
//          TraceLineS( "ProcessAction ImmediateReturn: ",
//                      *(pZSubtask->m_pzsWndTag) );
         ...
         {
//          TraceLineS( "ProcessAction PostClose: ",
//                      *(pZSubtask->m_pzsWndTag) );
            pZSubtask->PostClose( 1 );
            pZSubtask = 0;
         }
   ...
   if ( pZSubtask )
   {
   // TraceLineS( "ProcessAction Returning to: ",
   //             *(pZSubtask->m_pzsWndTag) );
#ifdef zREMOTE_SERVER
      SetRemoteWndAttribute( pZSubtask->m_vDialog, "Active", "Y" );
      if ( pZSubtask->m_pZSocketTask )
         zstrcpy( pZSubtask->m_pZSocketTask->m_szRemoteWndTag,
      ...
      // Even if pZSubtask is 0, it is possible that we should still set
      // the active window ... but this is very difficult to determine!!!
   // TraceLine( "ProcessAction Returning to: %s.%s   IdNbr: %d   Return Count: %d",
   //            *(pZSubtask->m_pzsDlgTag), *(pZSubtask->m_pzsWndTag),
   //            pZSubtask->m_ulSubtaskIdNbr, nReturn );
      while ( nReturn > 0 )
      {
         nReturn--;
      ...
      if ( pZSubtask->m_pZFWnd &&
           pZSubtask->m_pSplitterView == 0 &&
           mIs_hWnd( pZSubtask->m_pZFWnd->m_hWnd ) )
      {
         pZSubtask->m_pZFWnd->SetWindowPos( &CWnd::wndTop, 0, 0, 0, 0,
      ...
      if ( bConversational == FALSE &&
           (pZSubtask->m_ulWndOper & zWND_EVENT_ON_RETURN) &&
           SetCursorFirstEntityByInteger( pZSubtask->m_vDialog,
                                          szlWndEvent, szlType,
                                          zWND_EVENT_ON_RETURN, 0 ) >
                                                     zCURSOR_UNCHANGED )
      {
//       MSG    msg;
//       if ( PeekMessage( &msg, 0, g_uMsgPostedClose,
//                         g_uMsgPostedClose,
...
//
         ...
         if ( ProcessActionLoop( pZSubtask, szlWndAct, 0, 0, 0,
                                 TRUE, "OnReturn" ) > 0 )
         {
      ...
      if ( lBehavior == zWAB_ReturnToParentWithRefresh ||
           lBehavior == zWAB_ReturnToTopWindowWithRefresh )
      {
         ulSubtaskState = ZSubtask::GetSubtaskState( pZSubtask );
              ...
              zBOOL    bMatchTask )
{
   ...
   do
   {
      for ( zLONG lPos = 1; lPos <= lItemCnt; lPos++ )
      {
         if ( g_pSubtaskList[ lPos ].pZSubtask &&
              (bMatch == FALSE ||
               (bMatch && g_pSubtaskList[ lPos ].lTID == pZSubtask->m_lTID)) &&
              (g_pSubtaskList[ lPos ].ulSubtaskState &
                 zSUBTASK_STATE_WELL_FORMED) == zSUBTASK_STATE_WELL_FORMED &&
              zstricmp( g_pSubtaskList[ lPos ].DlgN, cpcDlgTag ) == 0 )
         {
            if ( (*cpcWndTag &&
                  zstricmp( g_pSubtaskList[ lPos ].WndN, cpcWndTag ) == 0) ||
                 *cpcWndTag == 0 )
            {
//             TraceLineS( "Subtask FOUND Dialog ",
//                         *(pZSubtaskCurr->m_pzsDlgTag) );
//             TraceLineS( " !!!!!!!!!!!! Window ",
//                         *(pZSubtaskCurr->m_pzsWndTag) );
...
// TraceLineS( "fnProcessDialogOperationCode ZSubtask: ",
//             *(pZSubtask->m_pzsWndTag) );
...
// SetWindowState( pZSubtask->m_vDialog, zWINDOW_STATUS_ASYNC, TRUE );
   ...
   if ( pZSubtask->m_vInterpSubtask )
   {
      ...
      lRC = (*(pZSubtask->m_pZTask->m_lpfnInterp))( pZSubtask->m_vInterpSubtask,
                                                    pZSubtask->m_vDialog,
                                                    pchDLL_Name, cpcCodeName,
                                                    &lRetVal );
      if ( zHIUSHORT( lRC ) == 2 )
...
//    TraceLineS( "Processing Dialog Operation ", cpcCodeName );
      ...
      catch( CException *e )
      {
         ...
         e->GetErrorMessage( szMsg, sizeof( szMsg ) );
         zsprintf( szOperMsg, "(drvr) Operation [%s] Exception ====> ",
                   cpcCodeName );
      ...
      if ( (ZSubtask::GetSubtaskState( pZSubtask ) & zSUBTASK_STATE_DELETED) !=
                                                       zSUBTASK_STATE_DELETED )
      {
