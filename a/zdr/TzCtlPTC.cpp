...
TZSelectCover::OnPaint( )
{
   ...
   zsprintf( szMsg, "TZSelectCover::OnPaint: %s ====>", m_pCtrl->m_csTag );
...
TZRuler::OnLButtonUp( UINT uModKeys, CPoint pt )
{
   ...
   if ( m_bTrack )
   {
      ...
      if ( lLastX != -64000 )
      {
         ...
         // zsprintf( szMsg1, "Old Margin: %ld", m_pPainterWindow->m_lLeftMargin );
            m_pPainterWindow->m_lLeftMargin = lLastX;
            m_pPainterWindow->PaintZeidonWindow( FALSE );

         // zsprintf( szMsg2, "New Margin: %ld", lLastX );
