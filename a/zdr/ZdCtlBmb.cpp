                        ...
                        "BitmapBtn" )
{
   ...
   m_pchMsgText = new char[ zstrlen( pCtrlDef->Text ) + 1 ];
   zstrcpy( m_pchMsgText, pCtrlDef->Text );
...
ZBitmapButton::LoadBitmaps( )
{
        ...
        pXRA->m_vXRA )
   {
      ...
      {
         GetStringFromAttribute( szImageUp, vXRA, "Ctrl", "Fix" );
         if ( szImageUp[ 0 ] != 'Y' )
         {
            SetRemoteZCtrlAttribute( this, "Ctrl", "Fix", "Y" );

            // handle UP URL, this is the default
            GetStringFromAttribute( szImageUp, vXRA, "BitmapBtn", "URL_Up" );
            if ( bHTTP )
               zstrcpy( szImageURL + nLth, StripDir( szImage ) );
            else
               zstrcpy( szImageURL + nLth, szImageUp );

            SysTranslateString( szImageURL, 'L' );
            SetRemoteZCtrlAttribute( this, "BitmapBtn",
                                     "URL_Up", szImageURL );
            zstrcpy( szImageUp, szImageURL ); // save to be used as default

            // handle DOWN URL
            GetStringFromAttribute( szImage, vXRA,
                                    "BitmapBtn", "URL_Down" );
            if ( szImage[ 0 ] == 0 )
               zstrcpy( szImageURL, szImageUp );
            else
            if ( bHTTP )
               zstrcpy( szImageURL + nLth, StripDir( szImage ) );
            else
               zstrcpy( szImageURL + nLth, szImage );
            ...
            SysTranslateString( szImageURL, 'L' );
            SetRemoteZCtrlAttribute( this, "BitmapBtn",
                                     "URL_Focus", szImageURL );

            // handle Disabled URL
            GetStringFromAttribute( szImage, vXRA,
                                    "BitmapBtn", "URL_Disabled" );
            if ( szImage[ 0 ] == 0 )
               zstrcpy( szImageURL, szImageUp );
            else
            if ( bHTTP )
                        ...
                        zPCHAR    pchBitmap )
{
   ...
   if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 1 )
   {
      if ( nFound == 0 )
      {
         ...
         {
            TraceLine( "ZBitmapButton::LoadDib: Cannot access bitmap "
      ...
      if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 2 )
      {
         if ( nFound == 1 )
         {
            TraceLineI( "ZBitmapButton::LoadDib: Bitmap loaded, "
                        "res id ", uResId );
         }
         else
      // if ( nFound == 2 )
   ...
   if ( pchSemiColon )
   {
           ...
           pchSemiColon[ 0 ] && pchSemiColon[ 1 ] != ':' )
      {
              ...
              pApp )
         {
            ...
            if ( szZeidonPath[ 0 ] )
            {
               ...
               if ( m_pDib->Load( szWorkString ) )
               {
                  if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 2 )
                  {
                     TraceLineS( "ZBitmapButton::LoadDib: Bitmap loaded, "
               ...
               if ( hBitmap )
               {
                  ...
                  if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 2 )
                  {
                     TraceLineS( "ZBitmapButton::LoadDib: Bitmap loaded from "
                                 "file in Application Dir ", szWorkString );
                  }
               ...
               if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 1 )
               {
                  TraceLineS( "ZBitmapButton::LoadDib: Cannot access bitmap "
                              "file in Application Dir ", szWorkString );
               }
            }
         }
         ...
         if ( pApp )
         {
            zstrcpy( szZeidonPath, pApp->szObjectDir );
            if ( szZeidonPath[ 0 ] )
            {
               SysAppendcDirSep( szZeidonPath );
               zstrcat( szZeidonPath, pchSemiColon );
               ...
               if ( m_pDib->Load( szWorkString ) )
               {
                  ...
                  {
                     TraceLineS( "ZBitmapButton::LoadDib: Bitmap loaded, "
                                 "file in Object Dir ", szWorkString );
                  }
         ...
         if ( m_pZSubtask == 0 || m_pZSubtask->m_pZTask->m_nTraceAction > 1 )
         {
            TraceLineS( "ZBitmapButton::LoadDib: Cannot access bitmap file ",
                        pchSemiColon );
         }
      }
   }
