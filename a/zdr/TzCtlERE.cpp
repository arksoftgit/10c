...
void
HyphenateER_Text( zCPCHAR cpcText, zPCHAR pchReturn, zUSHORT uIdx )
{
...
//      m_lPosIdx + (tzERENTITYX - 1) + ((tzERENTITYY - 1) * tzERDIAGRAMX)) )
   ...
   // NOTE, Orientation 1 used instead of 0 because of the way
   // Truetype handles scaling of non-standard font sizes
   pFont = new CFont( );
   pFont->CreateFont( (tzERFONTHEIGHT * 5) / 4, 0,
                      1, 1, 0, 0, 0, 0, ANSI_CHARSET,
...
// }
   ...
   m_pER_Diagram->m_hDC->RoundRect( rect.left, rect.top,
                                    rect.right, rect.bottom,
                                    tzERARCSIZE * m_lPortFactor,
                                    tzERARCSIZE * m_lPortFactor );
   if ( bErase == FALSE )
   {
      ...
      if ( Size.cx > (rect.right - rect.left) )
      {
         ...
         if ( Size.cx > (rect.right - rect.left) )
         {
            ...
            if ( nIdx > 0 )
            {
               HyphenateER_Text( m_csText, szText, nIdx );
            }

            m_pER_Diagram->m_hDC->DrawText( szText, -1, rect,
   ...
   //                             DT_SINGLELINE | DT_LEFT | DT_VCENTER );
         ...
         {
   //       rect.top += ((tzERCELLSIZE * 3) / 4);
            m_pER_Diagram->m_hDC->DrawText( m_csText, -1, rect,
...
// m_pER_Diagram->m_hDC->RoundRect( rect.left - 1, rect.top - 1,
//                                  rect.right + 1, rect.bottom + 1,
//                                  tzERARCSIZE, tzERARCSIZE );
...
//            (pchGrid[ lIdx ] & (tziENTITY | tziTEXT)) &&
   ...
   if ( PosY( lNewPosIdx ) != PosY( lNewPosIdx + (tzERENTITYX - 1) ) ||
        (lNewPosIdx + (tzERENTITYX - 1) +
             ((zLONG) (tzERENTITYY - 1) * tzERDIAGRAMX )) > tzERMAXGRID )
   {
      return;
   }
   ...
   for ( nItemPos = 0; nItemPos < m_pER_Diagram->m_nRelCnt; nItemPos++ )
   {
      ...
      if ( pRelationship->m_pSrcEntity == this ||
           pRelationship->m_pTgtEntity == this )
      {
         pRelationship->Erase( );
      }
   }
   ...
   // If the diagram is Scaled, then invalidate the diagram since
   // erasing a scaled diagram has problems
   if ( m_pER_Diagram->m_lScale && m_pER_Diagram->m_bScrolledWindow )
   {
                             ...
                             zBOOL  bClear )
{
   ...
   if ( bClear )
   {
      if ( vDiagram )
      {
         SetAllSelStatesForEntityForSet( vDiagram,
                                         *(m_pER_Diagram->m_pzsEName),
                                         0, 1, 0 );
         if ( nSelectRels )
         {
            SetAllSelStatesForEntityForSet( vDiagram,
                                            *(m_pER_Diagram->m_pzsAName),
                                            0, 1, 0 );
         }
      }
   ...
   // If deselect and clear were both specified, then there is nothing
   // else to do but deselect ourself and return.
   if ( !bSelect && bClear )
   {
      Deselect( );
      if ( bRelease )
   ...
   for ( nItemPos = 0; nItemPos < m_pER_Diagram->m_nRelCnt; nItemPos++ )
   {
      ...
      if ( pRelationship->m_pSrcEntity == this )
      {
         if ( nSelectRels != 2 )
         {
            ...
            {
               m_pER_Diagram->
                        PositionOnZeidonEntity( pRelationship->m_pTgtEntity,
                                                vDiagram, 1, 1 );
         ...
         if ( nSelectRels )
         {
            ...
            if ( vDiagram && (bSelect || !bClear) )
            {
               m_pER_Diagram->PositionOnZeidonRelationship( pRelationship,
                                                            vDiagram, 1, 1 );
               SetSelectStateOfEntity( vDiagram,
                                       *(m_pER_Diagram->m_pzsAName),
                                       bSelect );
      ...
      if ( pRelationship->m_pTgtEntity == this )
      {
         if ( nSelectRels != 2 )
         {
            ...
            {
               m_pER_Diagram->
                        PositionOnZeidonEntity( pRelationship->m_pSrcEntity,
                                                vDiagram, 1, 1 );
               SetSelectStateOfEntity( vDiagram,
                                       *(m_pER_Diagram->m_pzsEName),
                                       bSelect );
...
ZER_Entity::CenterEntity( )
{
   ...
   m_pER_Diagram->CalculateDisplaySize( lDisplaySizeX, lDisplaySizeY,
                                        rect, m_pER_Diagram->m_lScale );
