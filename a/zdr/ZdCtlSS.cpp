   ...
   if ( m_pzsVName )
   {
      ...
      // zCHAR  szSubBlob[ (3 * sizeof( zLONG )) + 34 ]; // flags + 2 reserved longs + tag
         ...
         for ( k = m_nFixedCols; k < nCols; k++ )
         {
            ...
            zstrcpy( m_pCol[ k ].pchText, pch );

            if ( pchComma )
            {
               *pchComma = '\t';
               m_pCol[ k ].pchMB_Text = new char[ zstrlen( pchComma + 1 ) + 1 ];
               zstrcpy( m_pCol[ k ].pchMB_Text, pchComma + 1 );
            }
            else
...
//          TraceLineI( "ZSS::ctor col ", k );
            if ( *pch )
            {
               ...
               zstrcpy( m_pCol[ k ].pchEntity, pch );
            ...
            else
            {
               ...
               zstrcpy( m_pCol[ k ].pchEntity, *m_pzsEName );
            }

            lChar = *((zPLONG) pch);
            pch += sizeof( zLONG );
            m_pCol[ k ].pchAttrib = new char[ lChar ];
            zstrcpy( m_pCol[ k ].pchAttrib, pch );
            ...
            zstrcpy( m_pCol[ k ].pchContext, pch );
   ...
   if ( zstrcmp( *m_pzsTag, "SSQueryAttributes" ) == 0 )
   {
      ...
      for ( lRow = 0; lRow < m_lRows; lRow++ )
      {
         ...
         // lCellData = GetItemData( lRow, nCol );
            TraceLine( "GetItemData1 Row: %d  Col: %d  CellData: %s",
                       lRow, nCol, pCell->m_csText );
         }
      }
   }
}
...
ZGrid::FormatTextForCell( CString& csText, zLONG lRow, zSHORT nCol )
{
                       ...
                       m_pZSubtask->m_vDialog, zLEVEL_SUBTASK ) <= 0 )
   {
           ...
           (m_ulMapActFlag2 & zMAPACT_MAPPED_FROM_OI) )
      {
         ...
         TraceLineS( "ZGrid::FormatTextForCell Retry MapFromOI: ",
                     szMsg );
...
// if ( zstrcmp( *m_pzsTag, "lbAttribute" ) == 0 )
   ...
   TranslateValue( szMap, TRUE, nCol );
   csText = szMap;
...
ZGrid::TransferDataFromCellToOI( zCPCHAR cpcText, zLONG lRow, zSHORT nCol )
{
   ...
   {
      szMap[ 0 ] = 0;
      zstrncat( szMap, cpcText, sizeof( szMap ) - 1 );
      zBOOL bTranslated = TranslateValue( szMap, FALSE, nCol );
      zSHORT nRC;
...
//             TraceLineI( "Grid editbox ", ulStyle );
            ...
            //
            // zsprintf( szMsg, "SetValue for Row: %d   Col: %d   %s",
...
// CPen penShadow( PS_SOLID, 1, ::GetSysColor( COLOR_GRAYTEXT ) );
   ...
   PrintTextToReport( m_pZSubtask->m_vDialog, cs, 0, 0, lPageWidth,
                      m_nTextHeight, 96, 0x00000F00, 0, 0, 0 );
   ...
   for ( nCol = 0; nCol < m_nCols; nCol++ )
   {
      ...
      MakeShortString( pZSubtask->m_pZPrintout->m_pDC, szText,
                       sizeof( szText ) - 1, m_pCol[ nCol ].pchText,
                       size.cx, 0 );
      PrintTextToReport( m_pZSubtask->m_vDialog, szText,
                         lLeft256ths, lTop256ths,
                         lRight256ths, lBottom256ths,
                         34, 0x00000F00, 0, 0, 0 );
   ...
   while ( lRow++ < m_nLinesPerPage && nRC == 0 )
   {
      ...
      // if ( m_pCol[ nCol ].nJustify == LVCFMT_LEFT )
         ...
         PrintTextToReport( m_pZSubtask->m_vDialog, szText,
                            lLeft256ths, lTop256ths,
                            lRight256ths, lBottom256ths,
                            lJustify + 64 + 32, 0x00000F00, 0, 0, 0 );
                   ...
                   zSHORT  nMaxReturnLth )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pGrid )
      {
         ...
         zstrncpy( pchReturnOrder, pGrid->m_csSortBuffer,
                   nMaxReturnLth - 1 );
         pchReturnOrder[ nMaxReturnLth ] = 0;
         return( 0 );
      }

      TraceLineS( "drvr - Invalid control type for GRID_GetSortOrder ",
                  cpcCtrlTag );
   }
                   ...
                   zCPCHAR cpcNewSortOrder )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_SetSortOrder ",
                  cpcCtrlTag );
   }
                          ...
                          zLONG   lDisplayPos )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_SetSelectedPosition ",
                  cpcCtrlTag );
   }
                              ...
                              zCPCHAR cpcCtrlTag )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_RemoveColorAssociations ",
                  cpcCtrlTag );
   }
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      if ( pGrid )
      {
                                                      ...
                                                      zGRID_CHECKBOX )
         {
            ...
            {
               pGrid->m_pCol[ lCol ].pchEntityX =
                                 new char[ zstrlen( cpcTextEntity ) + 1 ];
               zstrcpy( pGrid->m_pCol[ lCol ].pchEntityX, cpcTextEntity );
            }

            if ( cpcTextAttribute && *cpcTextAttribute )
            {
               pGrid->m_pCol[ lCol ].pchAttribX =
                                 new char[ zstrlen( cpcTextAttribute ) + 1 ];
               zstrcpy( pGrid->m_pCol[ lCol ].pchAttribX, cpcTextAttribute );
            }

            if ( cpcTextContext && *cpcTextContext )
            {
               pGrid->m_pCol[ lCol ].pchContextX =
                                  new char[ zstrlen( cpcTextContext ) + 1 ];
               zstrcpy( pGrid->m_pCol[ lCol ].pchContextX, cpcTextContext );
            }
      ...
      TraceLineS( "drvr - Invalid control type for GRID_SetCheckAttribute ",
                  cpcCtrlTag );
   }
                         ...
                         zLONG   clrBack )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_SetRowColumnColors ",
                  cpcCtrlTag );
   }
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
           ...
           GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcSubCtrlTag ) == 0 )
      {
         if ( lColumn > 0 && lColumn <= pGrid->GetColumnCount( ) + pGrid->GetFixedColumnCount( ) )
         {
            ...
            pGrid->m_pCol[ nCol ].pchData =
                                 new char[ zstrlen( cpcSubCtrlTag ) + 1 ];
            zstrcpy( pGrid->m_pCol[ nCol ].pchData, cpcSubCtrlTag );
                 ...
                 zSHORT   bLock )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_LockColumn ",
                  cpcCtrlTag );
   }
                    ...
                    zPLONG   plActiveCol )
{
   ...
   *plActiveRow = *plActiveCol = 0;
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_GetActiveCell ",
                  cpcCtrlTag );
   }
                          ...
                          zSHORT   nMaxTextLth )
{
   ...
   pchText[ 0 ] = 0; // initialize here for zstrncat
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ZGrid *pGrid = DYNAMIC_DOWNCAST( ZGrid, pzma->m_pCtrl );
      if ( pGrid )
      {
         ...
         *plRowNbr = pCell->lRow;
         *plColNbr = pCell->nCol;
         zstrncat( pchText, pCell->pchText, nMaxTextLth );
         return( 0 );
      }

      TraceLineS( "drvr - Invalid control type for GRID_GetPasteCellAndText ",
                  cpcCtrlTag );
   }
                  ...
                  zLONG    lColNbr )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      TraceLineS( "drvr - Invalid control type for GRID_GetCellData ",
                  cpcCtrlTag );
   }
