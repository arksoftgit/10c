...
//
// 1998.08.13    DKS
//    Use __WIN16__ instead of __WIN32__.  Use a #define for ZCommandLineInfo
//    rather than a derived class in __WIN32__.
//
// 1998.07.23    TMV
...
#include "resource.h"      // main symbols
   ...
   // 1998.07.23  TMV it's not defined in VC++ 1.5
...
#else
#define ZCommandLineInfo CCommandLineInfo
#endif
...
typedef struct ZDragDrop_tag
{
   ...
   zCHAR    szSrcDialogName[ 33 ];     // source dialog name
   zCHAR    szSrcWindowName[ 33 ];     // source window name
   zCHAR    szSrcCtrlName[ 33 ];       // source control name
   zVIEW    vData;                     // source view
   zCHAR    szViewName[ 33 ];          // source view name
   zCHAR    szEntityName[ 33 ];        // source entity name
   zCHAR    szAttributeName[ 33 ];     // source attribute name
   ...
   zCHAR    szTgtDialogName[ 33 ];     // target dialog name
   zCHAR    szTgtWindowName[ 33 ];     // target window name
   zCHAR    szTgtCtrlName[ 33 ];       // target control name
...
public:
   ...
   zLONG    m_lSocketState;         // Current state of socket thread ...
                                    ...
                                    //   -1 - termination
   zLONG    m_lSyncIdx;
   zCHAR    m_szAppTag[ 33 ];       // Application/LPLR name
...
public:
   ...
   zCHAR  m_szAction[ 33 ]; // data for requested action (e.g. Y, 1, 2 ...)
   zCHAR  m_szActDlg[ 33 ]; // name of dialog for the action type
   zCHAR  m_szActWnd[ 33 ]; // name of window for the action type
...
public:
   ...
   zCHAR    m_szActionName[ 33 ];// current action during Application operation
...
public:
   ...
   zBOOL  TranslateValue( zPCHAR pchValue, zBOOL bToDisplay,
                          zSHORT nCol = 0 );
...
public:
   ...
   zCHAR   m_szCurrentEntityName[ 33 ];
...
//
#ifndef __WIN16__   // not needed at runtime
#if 0
class ZDRAPP_CLASS ZTraceFn
{
private:
   static int  m_nIndent; // current indent level
...
// NOTE: You cannot use TRACEFN in a one-line statement!
// This will fail:
//
// if ( foo )
//    TRACEFN( ... )
//
// Instead, you must enclose TRACEFN in curly braces.
//
// if ( foo )
// {
//    TRACEFN( ... )
// }
//
#define TRACEFN ZTraceFn __fooble; TRACE
#endif //0
#endif // __WIN16__
...
#ifdef __WIN16__
// Class definition for ZCommandlineInfo.
...
// Of course this is MS stuff ... see VC 5.0 Documentation for details.

class ZDRAPP_CLASS CCommandLineInfo : public CObject
{
public:
   // Sets default values
   CCommandLineInfo( );
...
//#ifdef _UNICODE
// virtual void ParseParam( zCPCHAR pszParam, BOOL bFlag, BOOL bLast );
//#endif
...
// Implementation
protected:
   void ParseParamFlag( LPCSTR pszParam );
//   void ParseParamNotFlag( const TCHAR *pszParam );
//#ifdef _UNICODE
   void ParseParamNotFlag( LPCSTR pszParam );
//#endif
...
GetVEAPX_Reference( zVIEW vSubtask, zVIEW vAppOI, zCPCHAR cpcVEAPX,
                    zPCHAR pchFileName, zSHORT nMaxLth );
