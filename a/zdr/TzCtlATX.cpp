                          ...
                          lType, bPlaceHolder, lZKey, lTabStopNbr, lCtrlID )
{
   ...
   SysReadZeidonIni( -1, "[ActiveX]", "TraceBlob", pszStorageFile );
   ...
   SysReadZeidonIni( -1, "[ActiveX]", "TraceDisplayEntityInstance", pszStorageFile );
   strHelp.ReleaseBuffer();
   if ( strHelp.CompareNoCase( "y" ) == 0 )
   {
         ...
         while ( posAX_Oper )
         {
            ...
            if ( pchOperation )
            {
               if ( zstrcmp( pchOperation, cs ) == 0 )
               {
                  ...
                  cs += " )";
                  pchOperation[ 0 ] = 0;
                  zstrncat( pchOperation, cs, nMaxOperLth - 1 );
