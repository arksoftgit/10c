...
ZeidonCommOE_ThreadProc( zPVOID pParam )
{
   ...
   pDrApp->m_hWndCommOE = CreateWindowEx( WS_EX_TRANSPARENT,
                                          "ZDR_Communication_OE",
                                          "Zeidon DR Message Thread",
                                          0, 0, 0, 0, 0, 0, 0,
                                          (HINSTANCE) hInst, 0 );
   ...
   if ( !RegisterClass( &wc ) )
   {
      SysMessageBox( "wc.lpszClassName", "Error in register", 0 );
      return( 0 );
   }

   pDrApp->m_hWndCommOE = CreateWindow( "ZDR_Communication_OE",
                                        "Zeidon DR Message Thread",
                                        0, 0, 0, 0, 0, 0, 0,
                                        (HINSTANCE) hInst, 0 );
#endif
...
#if 0
   m_pzsZeidonWnd =
      new CString( AfxRegisterWndClass( CS_GLOBALCLASS | CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW,
                                        IDC_ARROW ) );
   m_pzsZeidonFrameOrView =
      new CString( AfxRegisterWndClass( CS_GLOBALCLASS | CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW,
                                        IDC_ARROW, COLOR_WINDOW,
                                        AFX_IDI_STD_FRAME ) );
   m_pzsZeidonMDI_Frame =
      new CString( AfxRegisterWndClass( CS_GLOBALCLASS | CS_DBLCLKS,
                                        IDC_ARROW, 0,
...
#ifdef __WIN16__
   m_hWndCommOE = ::CreateWindow( "ZDR_Communication_OE",
                                  pCmdInfo->m_csFileName,
                                  WS_OVERLAPPEDWINDOW,
...
// UINT uThreadID;
   ...
   CWinThread *pThread = AfxBeginThread( ZeidonCommOE_ThreadProc,
                                         (zPVOID) this,
   ...
   if ( RegisterZeidonApplication( &m_vSubtask, (zLONG) hInst,
                                   (zLONG) m_hWndCommOE,
                                   zZEIDON_COMMUNICATE_OE,
                                   pCmdInfo->m_strFileName, 0, 0 ) != 0 )
   {
      zCHAR szMsg[ 256 ];
      zstrcpy( szMsg, "Unable to Register Object Engine for: " );
      zstrcat( szMsg, pCmdInfo->m_strFileName );
   ...
   // MessageSend( 0, 0, "Zeidon Application", szMsg,
...
// TraceLineS( "RegisterZeidonApplication successful for: ",
//             pCmdInfo->m_strFileName );

   zCHAR szUseHack[ 100 ];
   SysReadZeidonIni( -1, "[Debug]", "UseDefaultTaskHack", szUseHack );
...
//                                      zSINGLE | zLEVEL_SYSTEM ) != 0 ||
      if ( SfActivateSysEmptyOI( &vZDrSystem, "KZZDR", m_vSubtask,
                                 zSINGLE | zLEVEL_SYSTEM ) != 0 ||
           vZDrSystem == 0 ||
           CreateEntity( vZDrSystem, "System", zPOS_AFTER ) != 0 ||
#ifdef zREMOTE_SERVER
           SetAttributeFromBlob( vZDrSystem, "System", "Subtasks",
                                 pchSubtasks, uLth ) != 0 ||
           SetAttributeFromBlob( vZDrSystem, "System", "Sockets",
                                 pchSubtasks, uLth ) != 0 )
#else
           SetAttributeFromBlob( vZDrSystem, "System", "Subtasks",
                                 pchSubtasks, uLth ) != 0 )
      ...
      {
         zCHAR szMsg[ 256 ];
         zstrcpy( szMsg, "Unable to Activate KZZDR object" );
         MessageSend( m_vSubtask, 0, "ZeidonApplication", szMsg,
                      zMSGQ_MODAL_ERROR, FALSE );
         ::PostMessage( m_hWndCommOE, WM_CLOSE, 0, 0L );
...
// CWinApp::InitInstance( ); // call parent class method
   ...
   ZeidonInitZDRAPP( m_vSubtask, g_lDlgUnitCharX, g_lDlgUnitCharY,
   ...
   // ZTask is a container class for all ZSubtask's within an application.
   // Multiple instances of the Zeidon driver may be active at once.  Each
   // instance has exactly one main window (it can change from one window
   // to another depending upon processing flow).  When the main window
   // is destroyed (and has not been replaced), the instance terminates.
   //
   // ZSubtask is a base class containing Zeidon information for each
   // "frame" type of window.  ZDrFrame is derived from the MFC
   // CFrameWnd class.  ZSubtask contains pointers to both the
   // ZDrFrame and ZTask to permit information to be shared among
   // all windows within the task.

   try   // we always have to re-enable the window
   {  // begin of try block

      // Construct a one-to-one class with this class ... to minimize the
      // interface between this .exe and the ZDrApp.dll
      m_pZTask = new ZTask( this, m_vSubtask, pCmdInfo );
...
ZDrApp::SetOrientation( zBOOL bLandscape )
{
   ...
   if ( GetPrinterDeviceDefaults( &pd ) )
   {
      ...
      pDevMode->dmOrientation = bLandscape ? DMORIENT_LANDSCAPE :
                                             DMORIENT_PORTRAIT;
...
#ifndef AFX_IDP_UNREG_DONE
   #define AFX_IDP_UNREG_DONE     _T("")
#endif

#ifndef AFX_IDP_UNREG_FAILURE
   #define AFX_IDP_UNREG_FAILURE  _T("")
#endif

void
ZDrApp::ParseCommandLine( ZCommandLineInfo& rCmdInfo )
{
   if ( m_lpCmdLine[ 0 ] == '-' || m_lpCmdLine[ 0 ] == '/' )
   {
      rCmdInfo.ParseParam( (LPCSTR) m_lpCmdLine + 1, TRUE, TRUE );
   }
}

BOOL
ZDrApp::ProcessShellCommand( ZCommandLineInfo& rCmdInfo )
{
   BOOL bResult = TRUE;
   switch ( rCmdInfo.m_nShellCommand )
   {
      ...
      case CCommandLineInfo::FileOpen:
         if ( !OpenDocumentFile( rCmdInfo.m_csFileName ) )
            bResult = FALSE;
         break;
...
/*
      ...
      case CCommandLineInfo::FilePrint:
         ...
         OpenDocumentFile( rCmdInfo.m_csFileName );
         m_pCmdInfo = &rCmdInfo;
      ...
      case CCommandLineInfo::AppUnregister:
      {
         ...
         if ( m_pCmdInfo == NULL )
         {
            m_pCmdInfo = new ZCommandLineInfo;
...
ZDxFrame::OnCreate( LPCREATESTRUCT lpCreateStruct )
{
   ...
   {
      SetAttributeFromInteger( vZDrSystem, "System",
                               "hWndRemote", (zLONG) m_hWnd );
   }
...
#endif
                                  CBRS_TOOLTIPS |
                                  CBRS_FLYBY | CBRS_SIZE_DYNAMIC ) == 0 ||
   ...
   if ( m_wndStatusBar.Create( this ) == 0 ||
        m_wndStatusBar.SetIndicators( indicators,
                     sizeof( indicators ) / sizeof( UINT ) ) == 0 )
   {
   ...
   if ( ReadWindowPlacement( vZDrSystem, "ZDRemote",
                             "RemoteServer", "Main", &wp ) &&
        CheckWindowPos( &wp ) )
   {
      if ( uEdge == ABE_TOP )
...
ZDxFrame::OnDestroy( )
{
   ...
   if ( GetWindowPlacement( &wp ) )
   {
      ...
      if ( mIs_hWnd( m_wndListCtrl.m_hWnd ) )
      {
         ...
         for ( k = 0; k < 3; k++ )
         {
            zltoa( m_wndListCtrl.GetColumnWidth( k ), pch + nLth );
            nLth = zstrlen( pch );
            if ( k < 3 - 1 )
            {
               pch[ nLth ] = ',';
         ...
         SetProfileStr( 0, "ZDRemote", "RemoteServer", "TaskList",
                        "ColumnWidth", pch );
         mDeleteInitA( pch );
      }
   }
      ...
      case SC_CLOSE:
      {
         ...
         if ( pZDrApp->m_pZTask->GetSocketTaskCount( ) > 0 )
         {
            if ( ::MessageBox( m_hWnd,
                               "Sockets are still active, do you want to exit?",
                               "Zeidon Remote Server", MB_OKCANCEL ) != IDOK )
            {
               return;
            }
      ...
      for ( lPos = 1; lPos <= lCnt; lPos++ )
      {
              ...
              pSocketList[ lPos ].ulSocketState != -1 )
         {
            ...
            csText.Format( TEXT( "[S%ld.T%ld.t%ld.s%ld]" ),
                           pZSocketTask,
                           AfxGetThread( ),
                           pZSocketTask->m_pWinThread,
                           pZSocketTask->m_sock );

            m_wndListCtrl.InsertItem( LVIF_TEXT | LVIF_STATE | LVIF_PARAM,
                                      lItem, csText, LVIS_SELECTED, 0, 0,
                                      (zLONG) lPos );
            m_wndListCtrl.SetItemText( lItem, 1,
...
ZDxFrame::OnRemotePrintReport( WPARAM wParam, LPARAM lParam )
{
   ...
   lRC = PrintReportToPDF( vSubtask, pvViewList[ 0 ],
                           pchSubobjectEntityName,
...
ZDxView::OnCreate( LPCREATESTRUCT lpCreateStruct )
{
   ...
   if ( m_pZDxFrame->m_wndListCtrl.Create( WS_CHILD | WS_VISIBLE | WS_BORDER |
...
ZDrApp::CheckAndCleanProcesses( )
{
   ...
   for ( dw = 0; dw < numTasks && !bOERunning; dw++ )
   {
      ...
      tname[ 0 ] = 0;
      zstrcpy( tname, (LPCSTR) tlist[ dw ].ProcessName );
      p = zstrchr( tname, '.' );
      if ( p )
      {
         p[ 0 ] = 0;
               ...
               DWORD      dwNumTasks )
{
   ...
   if ( pProcess32First( hProcessSnap, &pe32 ) )
   {
      do
      {
         ...
         for ( pCurChar = (pe32.szExeFile + zstrlen( pe32.szExeFile ));
               *pCurChar != '\\' && pCurChar != pe32.szExeFile;
               pCurChar-- )
         {
            zstrcpy( (LPSTR) pTask->ProcessName, pCurChar );
         }
      ...
      while ( dwTaskCount < dwNumTasks &&
              pProcess32Next( hProcessSnap, &pe32 ) );
   }
   else
   {
...
ZeidonFormatMessage( zCPCHAR cpcMsgTitle, zBOOL bMessageBox )
{
   ...
   ::FormatMessage( FORMAT_MESSAGE_FROM_SYSTEM |
                    ...
                    (zPCHAR) &pchMsg, 0, 0 );
   zsprintf( szMsgBuffer, "%s: (%d) %s", cpcMsgTitle, dw, pchMsg );
   TraceLineS( szMsgBuffer, "" );
   TRACE0( szMsgBuffer );
   if ( bMessageBox )
   {
      ::MessageBox( 0, (zCPCHAR) pchMsg, cpcMsgTitle,
               ...
               DWORD       dwNumTasks )
{
   ...
   // Need to initialize this because of goto logic.  Note that this code
   // can fail if privileges are not set correctly.  It can kill processes
   // that are not owned if privileges are set to "admin", since "all"
   // process are returned.  If privileges are
   dwNumTasks = 0;
   ...
   zsprintf( szSubKey, "%s\\%03x", REGKEY_PERF, lid );
   ...
   for ( dw = 0; dw < dwNumTasks; dw++ )
   {
      ...
      if ( rc == 0 )
      {
         ...
         zstrcpy( (LPSTR) pTask->ProcessName, UNKNOWN_TASK );
      }

      if ( zstrlen( szProcessName ) + 4 <= sizeof( pTask->ProcessName ) )
      {
         zstrcpy( (LPSTR) pTask->ProcessName, szProcessName );
         zstrcat( (LPSTR) pTask->ProcessName, ".exe" );
      }
   ...
   for ( dw = 0; dw < numTasks; dw++ )
   {
      if ( tlist[ dw ].dwProcessId == pid )
      {
         ...
         if ( GetWindowText( tlist[ dw ].hwnd, buf, sizeof( buf ) - 1 ) )
         {
            ...
            zstrcpy( (LPSTR) tlist[ dw ].WindowTitle, buf );
         }
