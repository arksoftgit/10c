...
ZWebGridCell::SetValueFromXRA( zVIEW vXRA )
{
   ...
   GetStringFromAttribute( szCellChanged,  vXRA, pszGridCell, pszChangeNode );
   GetStringFromAttribute( szCellInserted, vXRA, pszGridCell, pszInsertNode );
   GetStringFromAttribute( szCellDeleted,  vXRA, pszGridCell, pszDeleteNode );
   ...
   if ( szCellChanged[ 0 ] == 'Y' || szCellChanged[ 0 ] == 'y' )
   {
      ...
      {
         zCHAR szChecked[ 32 ];
         GetStringFromAttribute( szChecked, vXRA, pszGridCell, pszChecked );
      ...
      if ( csListContext.IsEmpty( ) )
      {
         ...
         if ( lpViewEntity )
         {
            for ( lpViewAttrib = (LPVIEWATTRIB) zGETPTR( lpViewEntity->hFirstOD_Attrib );
                  ...
                  lpViewAttrib = (LPVIEWATTRIB) zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
            {
               if ( zstrcmp( lpViewAttrib->szName, csListAttr ) == 0 )
               {
...
ZWebGrid::LoadMapping( zVIEW vDialog )
{
                                ...
                                pszCtrlCtrl ) > zCURSOR_UNCHANGED )
   {
      ...
      while ( nRC > zCURSOR_UNCHANGED )
      {
         ...
         GetStructFromEntityAttrs( (zPCHAR) CtrlDef.Tag, vDialog, pszCtrl );
         CtrlDef.IdNbr = k;
         k++;
            ...
            case CONTROL_TYPE_RADIOGRP :
               ...
               if ( pCol )
               {
                                               ...
                                               pszCtrlCtrl ) > zCURSOR_UNCHANGED )
                  {
                     ...
                     while ( nRC > zCURSOR_UNCHANGED )
                     {
                        GetStructFromEntityAttrs( (zPCHAR) CtrlDefRB.Tag, vDialog, pszCtrl );
            ...
            if ( CheckExistenceOfEntity( vXRA, pszGrid ) >= zCURSOR_SET )
            {
               ...
               while ( nRC >= zCURSOR_SET && !bFinished )
               {
                  ...
                  if ( zstrcmp( szEntityName, pszGridRow ) == 0 )
                  {
                     ...
                     GetStringFromAttribute( szChanged, vXRA,
                                             pszGridRow, pszChangeNode );
                     GetStringFromAttribute( szInserted, vXRA,
                                             pszGridRow, pszInsertNode );
                     GetStringFromAttribute( szDeleteded, vXRA,
...
zSHORT
ZWebGrid::GetCellData( zPCHAR pchReturnData, zLONG lRowNbr, zLONG lColNbr )
{
   zSHORT nRC = 0;
   ...
   *pchReturnData = '\0';
   // after calling SetCurrentCell the members m_lCurrentRow and m_lCurrentCell
   // should be in good shape so we can call GetValue using those menbers
   CString csValue = GetValue( m_lCurrentRow, m_lCurrentCell );
   zstrcpy ( pchReturnData, csValue );
...
{
   return( zstrcmp( m_csValue, comp.m_csValue ) > 0 );
}
                     ...
                     zBOOL   bLock )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      {
         TraceLineS( "drvr - Invalid control type for ZWEBGRID_LockColumn ",
                     cpcCtrlTag );
         nRC = -1;
      }
   }
                      ...
                      zLONG   lColNbr )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      {
         TraceLineS( "drvr - Invalid control type for TG_SetCellData ",
                     cpcCtrlTag );
         nRC = -1;
      }
   }
...
//  PARAMETERS: zVIEW vSubtask      - View to the Dialog window
//              zCPCHAR  cpcCtrlTag - the controls tag
//              zPCHAR  pchData     - pointer to a buffer getting the
//                                    current value
//              zLONG lRowNbr       - the row index of the cell
//                                    if -1 the current row
//              zLONG lColNbr       - the column index of the cell
                      ...
                      zLONG   lColNbr )
{
        ...
        GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      *pchData = 0;
      ...
      if ( pGrid )
      {
         nRC = pGrid->GetCellData( pchData, lRowNbr, lColNbr );
      }
      else
      {
         TraceLineS( "drvr - Invalid control type for GetCellData ",
                     cpcCtrlTag );
         nRC = -1;
      }
   }
                        ...
                        zPLONG  lColNbr )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      {
         TraceLineS( "drvr - Invalid control type for TG_GetActiveCell ",
                     cpcCtrlTag );
         nRC = -1;
      }
   }
                        ...
                        zLONG   lColNbr )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      {
         TraceLineS( "drvr - Invalid control type for TG_SetActiveCell ",
                     cpcCtrlTag );
         nRC = -1;
      }
   }
                                 ...
                                 zCPCHAR cpcFALSE )
{
   ...
   if ( GetWindowAndCtrl( &pZSubtask, &pzma, vSubtask, cpcCtrlTag ) == 0 )
   {
      ...
      {
         TraceLineS( "drvr - Invalid control type for "
                       "TG_SetCheckBoxTranslation ",
                     cpcCtrlTag );
         nRC = -1;
      }
   }
