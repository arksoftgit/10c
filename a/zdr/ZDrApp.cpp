...
ZNameList::Find( zCPCHAR cpcName )
{
   ...
   while ( pItem )
   {
      if ( zstrcmp( cpcName, pItem->m_csName ) == 0 )
      {
         return( pItem );
      }
...
{
   if ( m_pHeadName && zstrcmp( cpcName, m_pHeadName->m_csName ) == 0 )
      return( m_pHeadName );
...
ZPairedList::Display( zCPCHAR cpcText )
{
   ...
   while ( pItem )
   {
      zsprintf( szMsg, "   0x%8x  0x%8x  0x%8x  0x%8x",
...
ZCorrelatedList::Find( zCPCHAR cpcValue, zBOOL bKey )
{
   ...
   while ( pItem )
   {
      if ( (bKey == FALSE && zstrcmp( cpcValue, pItem->m_csValue ) == 0) ||
           (bKey && zstrcmp( cpcValue, *(pItem->m_pcsKey) ) == 0) )
      {
         return( pItem );
      }
...
ZDualCorrelatedList::Find( zLONG lKey, zLONG lValue, zCPCHAR cpcKey, zCPCHAR cpcValue, zLONG lFlag )
{
   ...
   while ( m_pCurrItem )
   {
      if ( (((lFlag & 1) == 0) || (lKey == m_pCurrItem->m_lKey)) &&
           ...
           (((lFlag & 4) == 0) || (m_pCurrItem->m_pcsKey && zstrcmp( cpcKey, *(m_pCurrItem->m_pcsKey)) == 0)) &&
           (((lFlag & 8) == 0) || (m_pCurrItem->m_pcsValue && zstrcmp( cpcValue, *(m_pCurrItem->m_pcsValue)) == 0)) )
      {
         break;
      }
...
ZDualCorrelatedList::FindNext( zLONG lKey, zLONG lValue, zCPCHAR cpcKey, zCPCHAR cpcValue, zLONG lFlag )
{
   ...
   while ( m_pCurrItem )
   {
      if ( (((lFlag & 1) == 0) || (lKey == m_pCurrItem->m_lKey)) &&
           (((lFlag & 2) == 0) || (lValue == m_pCurrItem->m_lValue)) &&
           (((lFlag & 4) == 0) || (zstrcmp( cpcKey, *(m_pCurrItem->m_pcsKey)) == 0)) &&
           (((lFlag & 8) == 0) || (zstrcmp( cpcValue, *(m_pCurrItem->m_pcsValue)) == 0)) )
      {
         break;
      }
                 ...
                 zLONG   lClrFlags )
{
   ...
   while ( pColorItem )
   {
      if ( pColorItem->m_lColumn == lColumn &&
           zstrcmp( pColorItem->m_csValue, cpcValue ) == 0 &&
                    ...
                    zCPCHAR cpcValue )
{
   ...
   while ( pColorItem )
   {
      if ( lColumn == pColorItem->m_lColumn &&
           zstrcmp( cpcValue, pColorItem->m_csValue ) == 0 )
      {
         if ( pItemPrev )
...
ZXRA_List::ZXRA_List( ZSocketTask *pZSocketTask )
{
   ...
   zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] XRA_List ctor",
             m_pZSocketTask,
             AfxGetThread( ),
...
ZXRA_List::~ZXRA_List( )
{
   ...
   zsprintf( szMsg, "[S%ld.T%ld.t%ld.s%ld] XRA_List dtor",
             m_pZSocketTask,
             AfxGetThread( ),
             m_pZSocketTask->m_pWinThread,
             m_pZSocketTask->m_sock );
...
ZXRA_List::Find( zCPCHAR cpcName, ZSubtask *pZSubtask, zCPCHAR cpcLabel )
{
   ...
   pXRA = m_pHead;
   zsprintf( szMsg, "ZXRA_List find: %s   Label: %s", cpcName, cpcLabel );
   TraceLineS( szMsg, "" );
   ...
   if ( pZSubtask )
   {
      ...
      while ( pXRA )
      {
         ...
         {
#ifdef DEBUG_ALL
            zsprintf( szMsg, "=> XRA: %ld   pZSubtask: %ld   Def: %ld   Name: %s",
                      pXRA, pXRA->m_pZSubtask, pXRA->m_vXRA_Def, pXRA->m_csName );
            TraceLineS( szMsg, "" );
#endif
            break;
   ...
   while ( pXRA )
   {
      ...
      if ( pXRA->m_vXRA_Delta )
      {
         if ( pvOI )
         {
            ...
            GetStringFromAttribute( szY, pXRA->m_vXRA_Delta,
                                    "Wnd", "Closed" );
            if ( szY[ 0 ] == 0 )
               GetStringFromAttribute( szY, pXRA->m_vXRA_Delta,
                                       "Wnd", "Deleted" );
...
DrAllocTaskMemory( zCOREMEM Memory, zLONG lBytes, zLONG lGuard )
{
   ...
   if ( pchMem )
   {
      ...
      *Memory = pchMem;
      ...
      {
         zCHAR szMsg[ 128 ];
         zsprintf( szMsg, "DrAllocTaskMemory: 0x%lx   %d",
                   (zLONG) pchMem, lGuard );
         TraceLineS( szMsg, "" );
      }
#endif
...
NEXT_CHECK_label:
   ...
   else
   {
      ...
      {
         zCHAR szMsg[ 128 ];
         zsprintf( szMsg, "DrFreeTaskMemory: 0x%lx   %d",
