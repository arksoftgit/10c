          ...
          zVIEW  vSubtask )
{
   ...
   zsprintf( szLine, "database %s [2048] {", pchName );

   SysWriteLine( vSubtask, f, szLine );
   SysWriteLine( vSubtask, f, "/*" );

   GetAddrForAttribute( &pch, vDTE, "TE_DBMS_Source", "Desc" );
   zsprintf( szLine, "   %s", pch );
   SysWriteLine( vSubtask, f, szLine );
   SysWriteLine( vSubtask, f, "*/" );
   SysWriteLine( vSubtask, f, "" );

   zsprintf( szLine, "   data file \"%s.d01\" contains", pchName );
   SysWriteLine( vSubtask, f, szLine );
   ...
   SysWriteLine( vSubtask, f, "" );
   zsprintf( szLine, "   key  file \"%s.k01\" contains", pchName );
   SysWriteLine( vSubtask, f, szLine );
   ...
   if ( lOverflowAttrLth )
   {
      ...
      zsprintf( szLine, "%*s db_addr ZOF_DBADDR;",
                (zSHORT) COLUMN_INDENT, " " );
      SysWriteLine( vSubtask, f, szLine );

      zsprintf( szLine, "%*s char   ZOF_Data[ %ld ];",
         ...
         RESULT = SetCursorNextEntity( vDTE, "TE_TablRec", 0 ) )
   {
      ...
      zsprintf( szLine, "record %s %s For Entity %s %s {", pch,
                                                  ...
                                                  "D", 0 ) )
      {
         ...
         switch ( *pch )
         {
            ...
            case zTYPE_DATETIME:
               zstrcat( szDataType, "int" );
               zsprintf( szLine, "%*s %*s %s[ 3 ];",
                         (zSHORT) COLUMN_INDENT, " ",
                         MAX_DATATYPE_LTH, szDataType, pchFieldName );
               break;

            case zTYPE_INTEGER:
               zstrcat( szDataType, "long" );
               zsprintf( szLine, "%*s %*s %s;",
                         (zSHORT) COLUMN_INDENT, " ",
                         MAX_DATATYPE_LTH, szDataType, pchFieldName );
               break;
            ...
            case zTYPE_DECIMAL:
               zstrcat( szDataType, "long double" );
               zsprintf( szLine, "%*s %*s %s;",
                         (zSHORT) COLUMN_INDENT, " ",
                         MAX_DATATYPE_LTH, szDataType, pchFieldName );
               break;
                                               ...
                                               "TE_DBMS_Source" ) )
   {
      ...
      zsprintf( szLine, "%*s order  last;", (zSHORT) COLUMN_INDENT, " " );
      if ( SysWriteLine( vSubtask, f, szLine ) < 0 )
      {
         nRC = zCALL_ERROR;
         goto EndOfFunction;
      ...
      zsprintf( szLine, "%*s owner  %s;", (zSHORT) COLUMN_INDENT, " ", pch );
      if ( SysWriteLine( vSubtask, f, szLine ) < 0 )
      {
         nRC = zCALL_ERROR;
         goto EndOfFunction;
      ...
      GetAddrForAttribute( &pch, vDTE, "TE_TablRec", "Name" );
      zsprintf( szLine, "%*s member %s;", (zSHORT) COLUMN_INDENT, " ", pch );
      if ( SysWriteLine( vSubtask, f, szLine ) < 0 )
      {
         nRC = zCALL_ERROR;
         goto EndOfFunction;
   ...
   zsprintf( szLine, "}  %s database %s %s",
...
ImportDB_Data( zVIEW vTE_View, zVIEW vEMD )
{
   ...
   zstrcpy( szDictionaryFile, pchDatabaseName );
   zstrcat( szDictionaryFile, ".DBD" );
   ...
   zstrcat( szPath, szDictionaryFile );

   // Open db-vista database dictionary.
   hFile = SysOpenFile( vTE_View, szPath, COREFILE_READ );
   if ( hFile == -1 )
   {
      zstrcpy( szWork, "DBVista Dictionary not found for database " );
      zstrcat( szWork, pchDatabaseName );
      zstrcat( szWork, ".\n  \'" );
      zstrcat( szWork, szPath );
      zstrcat( szWork, "\' cannot be opened." );
   ...
   for ( k = 0; k < Header.size_rt; k++ )
   {
      ...
      if ( nb > 0 && zstrcmp( szName, "ZOF_REC0" ) != 0 )
      {
         ...
         if ( nRC < zCURSOR_SET )
         {
            zstrcpy( szWork, "Record " );
            zstrcat( szWork, szName );
            zstrcat( szWork, " has no corresponding TE_TablRec.\n" );
            zstrcat( szWork, "Import aborted." );
   ...
   for ( k = 0; k < Header.size_fd; k++ )
   {
           ...
           zstrcmp( szName, "ZOF_DATA" ) != 0 )
      {
         ...
         if ( nRC < zCURSOR_SET )
         {
            zsprintf( szWork, "Field \"%s.%s\" has no corresponding "
         ...
         else
         {
            ...
            {
               zstrcpy( szWork, "Field \"" );
               zstrcat( szWork, szRecordName );
               zstrcat( szWork, "." );
               zstrcat( szWork, szName );
               zstrcat( szWork, "\" has a different length from it's "
   ...
   for ( k = 0; k < Header.size_st; k++ )
   {
      ...
      if ( nb > 0 )
      {
         ...
         if ( nRC < zCURSOR_SET )
         {
            zstrcpy( szWork, "Set " );
            zstrcat( szWork, szName );
            zstrcat( szWork, " has no corresponding TE_FieldDataRel.\n" );
            zstrcat( szWork, "Import aborted." );
