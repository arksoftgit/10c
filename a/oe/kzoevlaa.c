...
fnAddEntityToDebugChange( LPENTITYINSTANCE lpEntityInstance )
{
   ...
   //    SysMessageBox( 0, "fnAddEntityToDebugChange", "UNSET_CSR", -1 );
      ...
      zsprintf( szMsg, "%s.%s.%s", lpViewOD->szName,
...
fnCompareDebugChange( zBOOL bOperationCall, LPTASK lpTask )
{
         ...
         lpDbgChg = zGETPTR( lpDbgChg->hNextDebugChange ) )
   {
      ...
      if ( bDifference )
      {
         ...
         zsprintf( szMsg, "Operation%s: Core detected a change in the attribute "
...
fnValidViewEntity( LPVIEWENTITYCSR zFAR *pvReturnViewEntityCsr,
                   ...
                   zLONG         lControl )
{
   ...
   else
   {
      ...
      zsprintf( szLOD_Entity_Attribute, "[View: 0x%08x] %s.%s.%s",
                lpView, lpViewOD->szName, lpViewEntity->szName,
...
TrueName( zCPCHAR cpcName, zSHORT nControl )
{
   ...
   if ( NameForZKey( szWorkString, lZKey ) && nControl & zSHOW_ZKEY )
   {
      ...
      zltoa( lZKey, szWorkString + zstrlen( szWorkString ) );
      zstrcat( szWorkString, ")" );
   }
