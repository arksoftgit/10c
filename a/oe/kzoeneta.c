               ...
               zBOOL  bShowError )
{
   ...
   if ( lpNetwork == 0 && bShowError )
   {
      ...
      zsprintf( szMsg, "Cannot find network '%s'", pchNetworkName );
...
fnRetrieveAppName( zVIEW vSubtask, zPCHAR pchAppName )
{
   ...
   if ( lpApp )
      zstrcpy( pchAppName, lpApp->szName );
   else
      pchAppName[ 0 ] = 0;
}
...
// zVIEW     vTask = zGETPTR( lpSystemTask->hFirstView );
   ...
   if ( hLibrary )
   {
      ...
      lpNetwork->lpfnServDirInit = (zLPFN_SERVDIRINIT)
                                SysGetProc( hLibrary, "zServerDirInit" );
      lpNetwork->lpfnServDirClose = (zLPFN_SERVDIRCLOSE)
                                SysGetProc( hLibrary, "zServerDirClose" );
      lpNetwork->lpfnServDirGetAddr = (zLPFN_SERVDIRADDR)
                                SysGetProc( hLibrary, "zServerDirGetAddr" );

      if ( lpNetwork->lpfnServDirInit    == 0 ||
           lpNetwork->lpfnServDirClose   == 0 ||
           lpNetwork->lpfnServDirGetAddr == 0 )
      {
         zCHAR szMsg[ 200 ];

         zsprintf( szMsg, "Can't find Server Directory functions in '%s'. "
                  ...
                  zPCHAR    pchUserAddress )
{
   ...
   if ( lpNetwork->lpfnServDirGetAddr )
   {
      return( (*lpNetwork->lpfnServDirGetAddr)( lpTask,
                                                pchReturnAddress,
   ...
   {
      zstrcpy( pchReturnAddress, pchUserAddress );
      return( 0 );
   }

   // The default is to read the server address from the INI file
   // using the network name.
   zsprintf( szGroup, "[%s]", lpNetwork->szNetworkName );
   SysReadZeidonIni( -1, szGroup, "ApplServer", pchReturnAddress );
                          ...
                          zVIEW     vSubtask )
{
   ...
   if ( bCallOpen )
   {
      ...
      if ( lpNetwork->nTraceLevel > 0 )
         TraceLineI( "(kzoeneta) ** Opening a connection. Current connections = ",
                     lpNetwork->nConnCount );

      // Try to open connections and keep trying until we get an open conn or
      // an error.
      do
      {
         // Get address of data server.
         nRC = fnGetApplAddress( lpTask, lpNetwork, lRequest, pvRequestInfo,
                                 szServer, pchServerAddress );
         if ( nRC < 0 )
            return( zNETWORK_ERROR );
...
fnFlushBuffer( LPNETWORK lpNetwork, zPVOID *ppvConn )
{
   if ( lpNetwork->lpfnBufferEnd )
      ...
      return(*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, ppvConn,
                                     0, 0, zTYPE_BLOB );
} // fnFlushBuffer
                  ...
                  zLONG     lPacketData )
{
   ...
   else
   {
      ...
      if ( lPacketData < 0 )
      {
         ...
         zltox( lPacketData, &Packet.szPacketData[ 1 ] );
      }
      else
         zltox( lPacketData, Packet.szPacketData );
                    ...
                    zPCHAR    szAppName )
{
   ...
   zstrcpy( &szHeaderStr[ 1 ], szAppName );
   nLth = zstrlen( szHeaderStr ) + 1;

   if ( fnSendDataPacket( lpNetwork, ppvConn, zPACKET_HEADER, nLth ) != 0 )
      return( zCALL_ERROR );

   // Send server the header string.
   if ( (*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, ppvConn,
                                szHeaderStr, nLth, zTYPE_STRING ) != 0 )
   {
      return( zCALL_ERROR );
   }
...
fnCreateMsgObj( zVIEW vSubtask )
{
   ...
   if ( vMsgQ == 0 )
   {
      ...
      if ( ActivateEmptyObjectInstance( &vMsgQ, "KZMSGQOO", vSubtask,
                                        zMULTIPLE ) < 0 )
      {
         zCHAR szMsg[ 512 ];

         zstrcpy( szMsg, "Cannot load Message Object: " );
         zstrcat( szMsg, "KZMSGQOO" );
                ...
                zVIEW     vSubtask )
{
   ...
   if ( vTrace )
   {
      ...
      SysGetDateTime( szDateTime );  // DateTime yyyymmddhhmmssttt
   ...
   if ( pchFlag[ 0 ] == szlTrue[ 0 ] )
   {
           ...
           CreateViewFromViewForTask( &vCopy, vLocalMsg, 0 ) >= 0 )
      {
         ...
         while ( nc >= zCURSOR_SET )
         {
            ...
            SetMatchingAttributesByName( vCopy, "DBH_Error",
                                         vMessage, "DBH_Error", zSET_ALL );
                    ...
                    zVIEW     vSubtask )
{
   ...
   if ( pchFlag[ 0 ] == szlTrue[ 0 ] )
   {
      ...
      {
         zCHAR cDTime[ 32 ];
         GetStringFromAttribute( cDTime, vTrace,"Trace", "TraceStart" );
         TraceLineS( "(kzoeneta) ** Start Server Trace: ", cDTime );
         while ( nRC == zCURSOR_SET )
         {
           zCHAR cLine[ 255 ];
           cLine[ 0 ] = 0;
           GetStringFromAttribute( cLine, vTrace, "Line", "Line" );
           if ( cLine[ 0 ] )
              TraceLineS( "(kzoeneta) ** Line: ", cLine );

           nRC = SetCursorNextEntity( vTrace, "Line", "Trace" );
         } // while

         GetStringFromAttribute( cDTime, vTrace,"Trace", "TraceEnd" );
         TraceLineS( "(kzoeneta) ** End Server Trace: ", cDTime );
      }
                     ...
                     LPTASK    lpTask )
{
   ...
   zsprintf( szTraceSwitch, "%2.2hd", lpNetwork->nServerCoreTraceLevel );
// zsprintf( szTraceSwitch + 2, "%2.2hd", lpTask->nDBHandlerTraceLevel );
   zsprintf( szTraceSwitch + 2, "%2.2hd", lpNetwork->nServerDBHTraceLevel );
   zsprintf( szTraceSwitch + 4, "%2.2hd", lpNetwork->nServerNetTraceLevel );

   if ( fnSendDataPacket( lpNetwork, ppvConn, zPACKET_TRACESWITCH,
                          L_TRACE_SWITCH_MSG  ) != 0 )
   {
      return( zCALL_ERROR );
   }
   ...
   if ( (*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, ppvConn,
                                szTraceSwitch, L_TRACE_SWITCH_MSG,
                                zTYPE_STRING ) != 0 )
   {
      return( zCALL_ERROR );
   }
                 ...
                 LPTASK    lpTask )
{
   ...
   if ( fnSendDataPacket( lpNetwork, ppvConn, zPACKET_TIMESTAMP,
                          L_TIMESTAMP_MSG  ) != 0 )
   {
      return( zCALL_ERROR );
   }
   ...
   if ( (*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, ppvConn,
                                szTimestamp, L_TRACE_SWITCH_MSG,
                                zTYPE_STRING ) != 0 )
   {
      return( zCALL_ERROR );
   }
                 ...
                 zLONG     lControl )
{
   ...
   *pvReturnView = 0;
   ...
   zstrcpy( ActPacket.szVersion, zCURRENT_PROTOCOL_VERSION );
   zltox( lControl, ActPacket.szControl );
   zstrcpy( ActPacket.szObjectDef, pchObjName );
   if ( vQual )
      ActPacket.cUseQualification = 'Y';
   else
      ActPacket.cUseQualification = 'N';

   zstrcpy( ActPacket.szAppName, szAppName );
   SysGetUserID( vSubtask, ActPacket.szUserName, ActPacket.szPassword );

   if ( lpNetwork->nTraceLevel > 1 )
   {
      TraceLine( "(kzoeneta) ** Sending Activate Packet ... ApplName: %s  "
                   "Object Def: %s  User Name: %s",
                 ActPacket.szAppName, ActPacket.szObjectDef,
                 ActPacket.szUserName );
   }

   // Send server activate info.
   if ( (*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, &pvConn,
                                &ActPacket, sizeof( zActivatePacket ),
                                zTYPE_STRING ) != 0 )
   {
      goto EndOfFunction;
   }
   ...
   switch ( cPacketType )
   {
      case zPACKET_ACK:
         ...
         nRC = fnReceiveOI( &pvConn, lpNetwork, pvReturnView,
                            vSubtask, pchObjName, lControl );
         break;
      ...
      case zPACKET_EMPTYOI:
         ...
         // the server (and wasting time on the network), we'll just activate
         // it here.
         ActivateEmptyObjectInstance( pvReturnView, pchObjName,
                                      vSubtask, lControl );
         break;
      ...
      case zPACKET_ERROR:
         ...
         {
            fnSysMessageBox( lpTask, "Application not defined on Server",
                             ActPacket.szAppName, 0 );
         }
         else
         if ( lPacketData == zNETERR_OUTOFMEMORY )
         {
            fnSysMessageBox( lpTask, "Server Transaction Crash ",
                             "*Out Of Memory*", 1 );
         }
         else
      ...
      case zPACKET_RC:
         ...
         // the OI on the server.  Get the return code and go to the end of
         // the function.
         if ( lpNetwork->nTraceLevel > 0 )
...
// zBOOL            bOldTrace;
   ...
   {
      SysReadZeidonIni( -1, "[Zeidon]", "DefaultNetwork", szDefaultNetwork );
      ...
      SysReadZeidonIni( -1, "[Zeidon]", "DefaultNetworkExec", szExecutableName );
      if ( szExecutableName[ 0 ] == 0 )
      {
         // If no exec name give, use network name for executable too.
         zstrcpy( szExecutableName, szDefaultNetwork);
      }
   ...
   if ( zstrchr( pchNetworkName, cDirSep ) == 0 )
   {
      ...
      zstrcat( szFileName, pchExecutable );
   }
   else
      zstrcpy( szFileName, pchExecutable );
   ...
   if ( hLibrary == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find library '%s'", pchExecutable );
   ...
   // ===
   // === Make sure that the Network Handler contains all the necessary
   // === operations.
   // ===
   ...
   if ( lpfnInit == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetStart in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
   ...
   if ( lpfnClose == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetClose in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   if ( lpfnListen == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetListen in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   if ( lpfnStopListen == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetStopListen in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   lpfnOpenConn = (zLPFN_NETOPCONN) SysGetProc( hLibrary,
                                                "zNetOpenConnection" );
   if ( lpfnOpenConn == 0 )
   {
      zCHAR szMsg[ 200 ];
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetOpenConnection in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   lpfnCloseConn = (zLPFN_NETCLCONN) SysGetProc( hLibrary,
                                                 "zNetCloseConnection" );
   if ( lpfnCloseConn == 0 )
   {
      zCHAR szMsg[ 200 ];
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetCloseConnection in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   if ( lpfnSend == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Can't find operation zNetSend in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   if ( lpfnReceive == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Cannot find operation zNetReceive in library '%s'",
                pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
   ...
   if ( lpfnGetAddress == 0 )
   {
      ...
      zUNLOCK_MUTEX( zMUTEX_NETWORKCHAIN );
      zsprintf( szMsg, "Cannot find operation zNetGetLocalHostAddress in "
                "library '%s'", pchExecutable );
      SysMessageBox( lpTaskView, "Zeidon Network Error", szMsg, 1 );
      SysFreeLibrary( vSystemView, hLibrary );
      fnOperationReturn( iNetStartup, lpTask );
      return( zCALL_ERROR );
   ...
   hNetwork = (LPNETWORK)
          fnAllocDataspace( (LPDATAHEADER) lpSystemTask->hFirstDataHeader,
...
// lpTask->bTrace = bOldTrace;
   ...
   zstrcpy( lpNetwork->szNetworkName, pchNetworkName );
   zstrcpy( lpNetwork->szExecutable, pchExecutable );
   ...
   zsprintf( szGroup, "[%s]", pchNetworkName );
   SysReadZeidonIni( -1, szGroup, "TraceLevel", szTraceLevel );
   lpNetwork->nTraceLevel = (zSHORT) zatol( szTraceLevel );
   SysReadZeidonIni( -1, szGroup, "ServerCoreTraceLevel", szTraceLevel );
   lpNetwork->nServerCoreTraceLevel = (zSHORT) zatol( szTraceLevel );
   SysReadZeidonIni( -1, szGroup, "ServerNetTraceLevel", szTraceLevel );
   lpNetwork->nServerNetTraceLevel = (zSHORT) zatol( szTraceLevel );
   SysReadZeidonIni( -1, szGroup, "ServerDBHTraceLevel", szTraceLevel );
   ...
   lpNetwork->lpfnBufferEnd  = (zLPFN_NETCLCONN)
                                 SysGetProc( hLibrary, "zNetBufferEnd" );
   lpNetwork->lpfnSendPacket = (zLPFN_NETSENDPKT)
                                 SysGetProc( hLibrary, "zNetSendPacket" );
   lpNetwork->lpfnRecvPacket = (zLPFN_NETRECVPKT)
                                 SysGetProc( hLibrary, "zNetReceivePacket" );
   lpNetwork->lpfnSetMode    = (zLPFN_NETSETM)
                                 SysGetProc( hLibrary, "zNetSetMode" );
   ...
   SysReadZeidonIni( -1, szGroup, "ServerDirApp", szServerDirApp );
   if ( szServerDirApp[ 0 ] == 0 )
      strcpy( szServerDirApp, "zSimpDir" );

   if ( zstrchr( szServerDirApp, cDirSep ) == 0 )
   {
      SysGetLocalDirectory( lpNetwork->szFileName );
      zstrcat( lpNetwork->szFileName, szServerDirApp );
   }
   else
      zstrcpy( lpNetwork->szFileName, szServerDirApp );

   // Try loading the library.
   if ( LoadNetworkOperations( (LPTASK) zGETPTR( lpTaskView->hTask ),
                               lpNetwork ) == 0 )
   {
      (*lpNetwork->lpfnServDirInit)( lpTask, pchNetworkName,
                                     &lpNetwork->pvServDirData, 0 );
   }
   else
   {
...
NetClose( zVIEW lpTaskView, zPCHAR pchNetworkName )
{
   ...
   while ( lpNetwork->hFirstNetConn )
   {
      ...
      fnRemoveConnectionFromNetwork( lpNetwork,
                                     lpNetConn->pvConn,
                                     TRUE );
   }
...
// }
   ...
   if ( lpNetwork->lpfnServDirClose &&
        ...
        (*lpNetwork->lpfnServDirClose)( lpTask,
                                        &lpNetwork->pvServDirData, 0 ) != 0 )
   {
      nRC = zCALL_ERROR;
   }
...
NetListen( zVIEW lpTaskView, zPCHAR pchNetworkName )
{
   ...
   if ( lpNetwork->nTraceLevel > 0 )
   {
      TraceLineS( "(kzoeneta) ** Starting NetListen for network ",
                  lpNetwork->szNetworkName );
   }

   if ( lpNetwork->bListen )
   {
      TraceLineS( "(kzoeneta) ** Network already listening!",
                  lpNetwork->szNetworkName );
      fnOperationReturn( iNetListen, lpTask );
      return( 0 );
   }
   ...
   if ( lpNetwork->nTraceLevel > 0 )
   {
      TraceLineS( "(kzoeneta) ** Starting listen for network ",
                  lpNetwork->szNetworkName );
   }

   if ( (*lpNetwork->lpfnListen)( &lpNetwork->pNetworkHandle,
                                  0, lpTaskView ) != 0 )
   {
      fnOperationReturn( iNetListen, lpTask );
      return( zCALL_ERROR );
...
NetStopListen( zVIEW lpTaskView, zPCHAR pchNetworkName )
{
   ...
   if ( lpNetwork->nTraceLevel > 0 )
   {
      TraceLineS( "(kzoeneta) ** Stopping listen for network ",
                  lpNetwork->szNetworkName );
   }

   if ( lpNetwork->bListen == FALSE )
   {
      TraceLineS( "(kzoeneta) ** Network not listening!",
                  lpNetwork->szNetworkName );
      fnOperationReturn( iNetStopListen, lpTask );
      return( 0 );
   }

   if ( (*lpNetwork->lpfnStopListen)( lpTaskView,
                                      &lpNetwork->pNetworkHandle ) != 0 )
   {
      fnOperationReturn( iNetStopListen, lpTask );
      return( zCALL_ERROR );
   }
...
NetProcessMessage( zVIEW lpTaskView, zPCHAR pchNetworkName, zPVOID *ppvConn )
{
   ...
   fnAddConnectionToNetwork( lpTask, lpNetwork, 0, ppvConn,
                             FALSE, 0, 0, lpTaskView );

// if ( lpNetwork->nTraceLevel > 0 )
      TraceLineS( "(kzoeneta) ** Received message for network ",
                  lpNetwork->szNetworkName );
   ...
   while ( TRUE )
   {
      if ( fnReceiveDataPacket( lpNetwork, ppvConn, &cPacketType,
                                &lPacketData ) != 0 )
      {
   ...
   if ( nRC == 0 )
   {
      ...
      SysGetDateTime( szDateTime );  // DateTime yyyymmddhhmmssttt
      SetAttributeFromVariable( vTrace, "Trace", "TraceStart",
                                szDateTime, zTYPE_STRING, 17, "DateTime", 0 );
   }
   ...
   fnOperationReturn( iNetProcessMessage, lpTask );
   lpTask->bServer = FALSE;
   TraceLine( "(kzoeneta) ** Leaving NetProcessMessage( ) network: %s  RC: %d",
              pchNetworkName, nRC );
...
fnConvertIS_Clause( zPVIEW plpQualView )
{
   ...
   *plpQualView = lpNewQual;
   ...
   lpQualViewEntity    = fnValidViewEntity( &lpQualViewEntityCsr, lpNewQual,
                                            "QualAttrib", 0 );
   lpSubQualViewEntity = fnValidViewEntity( &lpSubQualViewEntityCsr, lpNewQual,
                                            "SubQualAttrib", 0 );

   // Loop through each of the entities in the and look for QualAttribs and
   // SubQualAttribs.
         ...
         lpEntityInstance = zGETPTR( lpEntityInstance->hNextHier ) )
   {
      ...
      if ( CompareAttributeToString( lpNewQual, pchEntityName,
                                     "Oper", "IS" ) != 0 )
      {
         continue;
      }
      ...
      GetStringFromAttribute( szSrcViewName, lpNewQual, pchEntityName,
                              "SourceViewName" );
      if ( szSrcViewName && szSrcViewName[ 0 ] )
         GetViewByName( &lpSrcView, szSrcViewName, lpOrigQual, zLEVEL_TASK );
      else
         GetIntegerFromAttribute( (zPLONG) &lpSrcView, lpNewQual,
                                  pchEntityName, "SourceViewID" );
      GetStringFromAttribute( szSourceEntity, lpNewQual, pchEntityName,
                              "SourceEntityName" );
      GetStringFromAttribute( szTargetEntity, lpNewQual, pchEntityName,
                              "EntityName" );
      ...
      lpSrcViewEntity = fnValidViewEntity( &lpTempViewEntityCsr,
                                           lpSrcView, szSourceEntity, 0 );
      bFirstTime = TRUE;
            ...
            lpViewAttrib = zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
      {
         ...
         SetAttributeFromString( lpNewQual, pchEntityName, "EntityName",
                                 szTargetEntity );
         SetAttributeFromString( lpNewQual, pchEntityName, "AttributeName",
                                 lpViewAttrib->szName );
         SetAttributeFromString( lpNewQual, pchEntityName, "Oper", "=" );
         SetAttributeFromAttribute( lpNewQual, pchEntityName, "Value",
                                    lpSrcView, szSourceEntity,
                                    lpViewAttrib->szName );
      } // for ( lpViewAttrib )...
...
// DGC Not supported at the moment (security risk).
   ...
   SysGetUserID( vSubtask, LAD_Info.szUserName, LAD_Info.szPassword );
   ...
   // Open connection.
   if ( fnAddConnectionToNetwork( &LAD_Info, &pvConn, TRUE, vSubtask ) < 0 )
   {
      fnOperationReturn( iNetActivateOI_FromFile, lpTask );
      return( zNETWORK_ERROR );
   }
   ...
   zstrcpy( ActPacket.szVersion, zCURRENT_PROTOCOL_VERSION );
   zltox( lControl, ActPacket.szControl );
   zstrcpy( ActPacket.szObjectDef, pchViewOD_Name );
   ActPacket.cUseQualification = 'N';
   zltox( (zLONG) zstrlen( pchFileName ) + 1, ActPacket.szFileNameLth );

   zstrcpy( ActPacket.szAppName, LAD_Info.szAppName );
   SysGetUserID( vSubtask, ActPacket.szUserName, ActPacket.szPassword );
                                 ...
                                 zLONG            lHierCount )
{
   ...
   while ( lpEntityInstance )
   {
      ...
      while ( lpNextTwin )
      {
         if ( lpNextTwin->lHierCount <= lHierCount &&
              !fnEntityInstanceIsDead( lpNextTwin ) )
         {
            lpEntityInstance = lpNextTwin;
         }
...
/*
   ...
   Creates a buffer with a list of all inter-OI entity links.  The info in
                 ...
                 zPLONG  plLinkBufferLth )
{
   ...
   *plLinkBufferLth = 0;
   ...
   // If there is no interlinking between the OIs then there's nothing more
   // to do.
            ...
            lpEntityInstance = zGETPTR( lpEntityInstance->hNextHier ) )
      {
               ...
               lpLinked = zGETPTR( lpLinked->hNextLinked ) )
         {
            ...
            for ( j = 0; j < nViewCount; j++ )
            {
               ...
               // If lpLinked is not part of the OI specified by
               // lpViewArray[ j ] then skip it.
               lpTempViewCsr = zGETPTR( lpViewArray[ j ]->hViewCsr );
               if ( lpTempViewCsr->hViewOI != lpLinked->hViewOI )
                  continue;

               // At this point 'k' is the index pointing to the lpView for
               // lpEntityInstance and 'j' is the index pointing to the lpView
               // that lpLinked is part of.
               if ( j < k )
               {
               ...
               // Check to see if we've already found a linked EI in this
               // view.  We only need to find 1 since all the linked EIs in
               // the OI are already linked together.
               ...
               // If this is the first EI that we've found add a paren to
               // start things and then add lpEntityInstance.
               if ( nLinkInfo == 0 )
               {
                  *pchBufferEnd++ = '(';
                  zsprintf( pchBufferEnd, "%x:%lx", k,
               ...
               *pchBufferEnd++ = '/';
               ...
               // Set flag to indicate that we've found interlinking and
               // that we're adding it to the buffer.
               nLinkInfo = 1;

               //
               zsprintf( pchBufferEnd, "%x:%lx", j, lpLinked->lHierCount );
               pchBufferEnd += zstrlen( pchBufferEnd );
         ...
         // If nLinkInfo is 2 then we don't need to add information to the
         // link buffer for the current entity so reset pchBufferEnd.
         if ( nLinkInfo == 2 )
...
/*
   ...
   We can't just drop the old view because that will
   invalidate all other views to the OI referenced by vOld.  In addition,
   all the names for vOld will be dropped.  Lastly, it will not "update"
   any linked instances.

   If we merge the changes from vNew to vOld then all the views to
...
fnMergeViews( zVIEW vNew, zVIEW vOld )
{
         ...
         lpOld = (bGetNextOld ? zGETPTR( lpOld->hNextHier ) : lpOld) )
   {
      ...
      if ( (zLONG) lpOld != lpNew->lTag )
      {
         ...
         // We'll worry about handling server commit operations that create/
         // delete entities later.  For now we'll just assume that lpOld
         // wasn't sent to the server...so skip it.

         // NOTE: We can determine if the lpNew entity was created on the
         // server by checking the value of lpNew->lTag.  If it is 0 then
           ...
           lpViewEntity->bHasDB_Oper == FALSE ) // added Phil/Doug 2005.10.17
      {
               ...
               lpViewAttrib = zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
         {
            ...
            fnGetAttrAddrFromEntityInstance( (zCOREMEM) &lpValue, &uLth,
                                             lpNew, lpViewAttrib );
            fnStoreValueInEntityInstance( lpOld, lpViewEntity, lpViewAttrib,
                                          lpValue, uLth );
             ...
             zLONG         lControl )
{
   ...
   // Create a buffer to store object names.  We'll arbitrarily allocate
   // 32000 bytes.
   pchObjectNameBuffer = (zPCHAR) fnAllocDataspace( lpTask->hFirstDataHeader,
                                                    32000, TRUE, 0, 0 );
   ...
   for ( k = 0; k < nViewCount; k++ )
   {
      ...
      pch = zstrcpyp( pch, lpViewOD->szName );
   ...
   // Open connection.
   if ( fnAddConnectionToNetwork( lpTask, lpNetwork, szServer,
                                  &pvConn, TRUE, zAPPLREQ_COMMIT,
                                  &CommitInfo, lpTaskView ) < 0 )
   {
      nRC = zNETWORK_ERROR;
      goto EndOfFunction;
   }
   ...
   // First send the header for the server driver and then the regular
   // packet for the function structure.
   if ( fnSendHeaderPacket( lpNetwork, &pvConn, zPACKET_COMMITOI,
                            szAppName ) != 0 )
   {
      goto EndOfFunction;
   }
   ...
   zstrcpy( CommitPacket.szVersion, zCURRENT_PROTOCOL_VERSION );
   zstrcpy( CommitPacket.szAppName, szAppName );
   SysGetUserID( lpTaskView, CommitPacket.szUserName, CommitPacket.szPassword );
   zltox( nViewCount, CommitPacket.szViewCount );
   zltox( lControl, CommitPacket.szControl );

   nObjectNameLth = (zSHORT) (pch - pchObjectNameBuffer);
   zltox( (zLONG) nObjectNameLth, CommitPacket.szObjectNameLth );

   // Set up the link buffer.  This buffer contains a list of all entity
   // instances that are linked with each other in lpViewArray.
   fnSetLinkBuffer( lpTask, lpViewArray, nViewCount,
                    &pchLinkBuffer, &lLinkBufferLth );

   zltox( lLinkBufferLth, CommitPacket.szLinkBufferLth );

   if ( lpNetwork->nTraceLevel > 0 )
   {
      TraceLine( "(kzoeneta) ** Appl Name: %s  User Name: %s ",
                 CommitPacket.szAppName, CommitPacket.szUserName );
      if ( nViewCount > 1 )
      {
         TraceLine( "(kzoeneta) ** View Count: %s  OD Name Lth: %d",
                    CommitPacket.szViewCount, nObjectNameLth );
         TraceBuffer( "(kzoeneta) ** OD Names: ",
                      pchObjectNameBuffer, nObjectNameLth );
      }
   ...
   for ( k = 0; k < nViewCount; k++ )
   {
      ...
      zltox( lpViewCluster[ k ].lControl, &szControl[ 2 ] );
   ...
   for ( k = 0; k < nViewCount; k++ )
   {
      ...
      // Get the next packet.
      if ( fnReceiveDataPacket( lpNetwork, &pvConn, &cPacketType,
                                &lPacketData ) != 0 )
      {
         TraceLineS( "(kzoeneta) ** Error receiving OI ACK!", "" );
         goto EndOfFunction;
...
EndOfFunction:
   ...
   if ( bConnectionMade &&
        fnRemoveConnectionFromNetwork( lpNetwork, &pvConn,
                                       TRUE ) == zCALL_ERROR )
   {
      nRC = zCALL_ERROR;
   }
...
// DGC Not supported at the moment (security risk).
   ...
   SysGetUserID( lpView, LAD_Info.szUserName, LAD_Info.szPassword );
   ...
   // Open connection.
   if ( fnAddConnectionToNetwork( &LAD_Info, &pvConn,
                                  TRUE, lpView ) == zCALL_ERROR )
   {
      fnOperationReturn( iNetCommitOI_ToFile, lpTask );
      return( zCALL_ERROR );
   }
   ...
   zmemset( &CommitPacket, 0, sizeof( zCommitPacket ) );
   zstrcpy( CommitPacket.szVersion, zCURRENT_PROTOCOL_VERSION );
   zstrcpy( CommitPacket.szObjectDef, lpViewOD->szName );
   zltox( lControl, CommitPacket.szControl );
   zltox( (zLONG) zstrlen( pchFileName ) + 1, CommitPacket.szFileNameLth );
   zstrcpy( CommitPacket.szAppName, LAD_Info.szAppName );
   SysGetUserID( lpView, CommitPacket.szUserName, CommitPacket.szPassword );
                  ...
                  zPCHAR   pchArgList, ... )
{
      ...
      switch ( *pchArgList++ )
      {
         ...
         case 'N':      // Short integer
            if ( bPointer )
            {
               ...
               if ( ps )
                  SetAttributeFromInteger( vArgs, szlArgument, "Value",
                                           (zLONG) *ps );
            }
            else
            {
         ...
         case 'V':      // View
            if ( bPointer )
            {
               ...
               else
               {
                  ...
                  SetAttributeFromString( vArgs, szlArgument, "ObjectName",
                                          lpViewOD->szName );
                  SetAttributeFromString( vArgs, szlArgument, "SendArgument", "Y" );
               }
            }
            else
            {
               ...
               if ( v->hSubtask )
               {
                  ...
                  SetAttributeFromString( vArgs, szlArgument,
                                          "SubtaskView", "Y" );
                  SetAttributeFromString( vArgs, szlArgument,
                                          "ApplicationName", lpApp->szName );
                  SetAttributeFromString( vArgs, szlArgument,
                                          "SendArgument", "N" );
               }
               else
   ...
   for ( nRC = SetCursorFirstEntityByString( vArgs, szlArgument, "DataType",
                                             "V", 0 );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityByString( vArgs, szlArgument, "DataType",
                                            "V", 0 ) )
   {
      zVIEW  vSendView;
                                     ...
                                     "ReturnArgument", "Y" ) == 0 )
      {
         ...
         GetBlobFromAttribute( &vSendView, 0, vArgs,
                               szlArgument, "PointerValue" );

      // We need to send the view to the server...
      if ( fnSendOI( &pvConn, lpNetwork, vSendView,
   ...
   {
      case zTYPE_INTEGER:
         GetIntegerFromAttribute( (zPLONG) pvReturn,
                                  vArgs, "Operation", "ReturnValue" );
         break;
      ...
      case zTYPE_STRING:
         GetStringFromAttribute( (zPCHAR) pvReturn,
                                 vArgs, "Operation", "ReturnValue" );
         break;

      case 'R':
      case zTYPE_DECIMAL:
         GetDecimalFromAttribute( (zPDECIMAL) pvReturn,
                                  vArgs, "Operation", "ReturnValue" );
         break;
   }
...
EndOfFunction:
   if ( pvConn && fnRemoveConnectionFromNetwork( lpNetwork, &pvConn,
                                                 TRUE ) == zCALL_ERROR )
   {
...
// DGC Not supported at the moment (security risk).
   ...
   if ( bBinaryFile )
   {
      pchLine = (zPCHAR) fnAllocDataspace( lpTask->hFirstDataHeader,
                                           zSENDFILE_BUFFER, TRUE, 0, 0 );
   ...
   // First send the header for the server driver and then the regular
   // packet for the function structure.
   if ( fnSendHeaderPacket( lpNetwork, &pvConn, zPACKET_SENDFILE,
                            sf.szAppName ) != 0 )
   {
      goto EndOfFunction;
   }
   ...
   pchBuffer = (zPCHAR) fnAllocDataspace( lpTask->hFirstDataHeader,
                                          nFileNameLth, TRUE, 0, 0 );
   pchBuffer = (zPCHAR) zGETPTR( pchBuffer );
   zstrcpy( pchBuffer, pchServerDirectory );
   zstrcpy( &pchBuffer[ zstrlen( pchBuffer ) + 1 ], pchServerFileName );
   if ( (*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, &pvConn,
                                pchBuffer, nFileNameLth,
                                zTYPE_STRING ) != 0 )
   {
      goto EndOfFunction;
   }
   ...
   while ( TRUE )
   {
      if ( bBinaryFile )
      {
         usBytesRead = SysReadFile( vSubtask, hFile,
                                    pchLine, zSENDFILE_BUFFER );
         if ( usBytesRead == 0 )
            break;
      }
      else
      ...
      if ( fnSendDataPacket( lpNetwork, &pvConn, cPacketType,
                             usBytesRead ) != 0 )
         goto EndOfFunction;

      // Send the line.
      if ( (*lpNetwork->lpfnSend)( &lpNetwork->pNetworkHandle, &pvConn,
                                   pchLine, usBytesRead, cDataType ) != 0 )
      {
      ...
      // If we're reading a binary file then check to see if we are at the
      // end of the file.
      if ( bBinaryFile )
      {
...
EndOfFunction:
   if ( nRC == zCALL_ERROR )
   {
      ...
      zsprintf( szMsg, "Error sending file '%s' to server.", pchLocalFileName );
      SysMessageBox( szlNetworkError, szMsg, 1 );
   }

   if ( bConnOpen && fnRemoveConnectionFromNetwork( lpNetwork, &pvConn,
                                                    TRUE ) == zCALL_ERROR )
   {
      nRC = zCALL_ERROR;
   }
...
NetStatus( zVIEW lpTaskView, zPCHAR pchNetworkName )
{
   ...
   if ( (lpNetwork = fnFindNetwork( lpTask, pchNetworkName, FALSE )) == 0 )
   {
      ...
      TraceLineS( "(kzoeneta) ** Network status: no network running for ",
                  pchNetworkName );
   ...
   if ( lpNetwork->nTraceLevel > 0 )
   {
      TraceLineS( "(kzoeneta) ** Network status for network: ",
                  lpNetwork->szNetworkName );
   ...
   if ( lpNetwork->nConnCount > 0 )
   {
      ...
      if ( lpNetwork->nTraceLevel > 0 )
      {
         TraceLineI( "(kzoeneta)         ** Connections running: ",
                     lpNetwork->nConnCount );
      }
   }
                   ...
                   zSHORT  nErrorNum )
{
   ...
   if ( nErrorNum )
   {
      ...
      switch ( nErrorNum )
      {
         case 74:
            ...
            zsprintf( szMsg, "Invalid Entity name = %s", pchExtraData );
            break;

         case 75:
            // "KZOEE075 - Invalid Entity level on line "
            zsprintf( szMsg, "Invalid Entity level = %s", pchExtraData );
            break;

         case 90:
            // "KZOEE090 - Maximum number of entites in portable file exceeded"
            zstrcpy( szMsg, "Maximum number of entities in stream exceeded" );
            break;

         case 104:
            // "KZOEE104 - Invalid Attribute name for Entity"
            zsprintf( szMsg, "Invalid Attr name for Entity = %s",
                      pchExtraData );
            break;
   ...
   if ( (*lpNetwork->lpfnReceive)( &lpNetwork->pNetworkHandle,
                                   ppvConn, ppchReturnBuffer,
                                   lPacketData, cDataType ) != 0 )
   {
             ...
             zLONG       lControl )
{
   ...
   nRC = SfActivateOI_FromStream( pvOI, pchObjectDef, vSubtask,
                                  lControl, fnReadDataFromNet, &ar );
   if ( nRC != zCALL_ERROR )
   {
      if ( lpNetwork->nTraceLevel > 0 )
      {
          ...
          zLONG       lControl )
{
   ...
   if ( g_chNetSendCompressed == 0 )
   {
      ...
      SysReadZeidonIni( -1, "[Zeidon]", "NetSendCompressed", szCompressed );
   ...
   nRC = SfWriteOI_ToStream( vOI, "NETACT", lControl,
                             fnSendOI_Data, (zPVOID) &ar );
   ...
   if ( nRC != zCALL_ERROR )
   {
      if ( fnSendDataPacket( lpNetwork, ppvConn, zPACKET_END,
                             ar.lLineCount ) != 0 )
      {
         TraceLineS( "(kzoeneta) ** Error sending zPACKET_END", "" );
         return( zCALL_ERROR );
...
fnProcessActivateOI( zVIEW lpTaskView, LPNETWORK lpNetwork, zPVOID *ppvConn )
{
   ...
   nRC = ActivateObjectInstance( &vOI, ActPacket.szObjectDef, vSubtask, vQual,
                                 lControl );
   ...
   if ( vOI )
   {
      if ( nRC < 0 )
      {
         ...
         // client activate an empty OI on it's side.
             ...
             zPCHAR        pchLinkBuffer )
{
   ...
   if ( *pch++ != '(' )
   {
      fnSysMessageBox( lpTask, szlNetworkError,
                       "Link buffer doesn't start with a '('", 1 );
      goto EndOfFunction;
   }
   ...
   while ( *pch )
   {
      ...
      {
         fnSysMessageBox( lpTask, szlNetworkError,
                          "Link buffer: expecting hex digit.", 1 );
         goto EndOfFunction;
      }
      ...
      {
         fnSysMessageBox( lpTask, szlNetworkError,
                          "Link buffer: expecting ':' after OI tag.", 1 );
         goto EndOfFunction;
      }
      ...
      {
         fnSysMessageBox( lpTask, szlNetworkError,
                          "Link buffer: Could find OI matching Tag.", 1 );
         goto EndOfFunction;
      }
      ...
      {
         fnSysMessageBox( lpTask, szlNetworkError,
                          "Link buffer: expecting ')/' after Hier Num.", 1 );
         goto EndOfFunction;
      }

      // Find the entity instance that matches lHierCount.
      lpEntityInstance = zGETPTR( plpViewOI[ nOI_Idx ]->hRootEntityInstance );
      lpEntityInstance = fnFindEntityInstanceByHierCount( lpEntityInstance,
                                                          lHierCount );
      if ( lpEntityInstance == 0 )
      {
         fnSysMessageBox( lpTask, szlNetworkError,
                          "Link buffer: Could find EI matching Hier Count.", 1 );
         goto EndOfFunction;
      }
      ...
      if ( lpLastEI )
      {
         ...
         {
            fnSysMessageBox( lpTask, szlNetworkError,
                             "Link buffer: Link EIs don't have matching tokens.", 1 );
            goto EndOfFunction;
         }
...
fnProcessCommitOI( zVIEW lpTaskView, LPNETWORK lpNetwork, zPVOID *ppvConn )
{
   ...
   pchOD_Names = fnAllocDataspace( lpTask->hFirstDataHeader,
                                   nOD_NameLth, TRUE, 0, 0 );
   if ( pchOD_Names == 0 )
      goto EndOfFunction;
   ...
   if ( lLinkBufferLth > 0 )
   {
      pchLinkBuffer = fnAllocDataspace( lpTask->hFirstDataHeader,
                                        lLinkBufferLth, TRUE, 0, 0 );
      if ( pchLinkBuffer == 0 )
         goto EndOfFunction;
   ...
   if ( lpNetwork->nTraceLevel > 0 )
   {
      ...
      {
         TraceLine( "(kzoeneta) ** View Count: %s  OD Name Lth: %d",
                    CommitPacket.szViewCount, nOD_NameLth );
         TraceBuffer( "(kzoeneta) ** OD Names: ",
                      pchOD_Names, nOD_NameLth );
      }
   }
   ...
   if ( lpNetwork->nTraceLevel > 0 )
   {
      ...
      zstrcat( szTempFileName, "commitoi.por" );
   ...
   nRC = CommitMultipleOIs( lpViewCluster, nViewCount,
                            &nViewErrorIdx, lCommitControl );
   ...
   if ( nRC == 0 )
   {
      for ( k = 0; k < nViewCount; k++ )
      {
         ...
         else
         {
            ...
            if ( fnSendOI( ppvConn, lpNetwork, lpViewCluster[ k ].vOI,
                           zINCREMENTAL ) == zCALL_ERROR )
            {
               nRC = zCALL_ERROR;
               goto EndOfFunction;
            }
...
fnProcessCommitOI_ToFile( LPNETWORK lpNetwork, zPVOID *ppvConn )
{
   ...
   */
   zstrcpy( szFileName, vSubtask->hSubtask->hApp->hObjectDir );
   SysAppendcDirSep( szFileName );
   zstrcat( szFileName, pch );
#endif
...
fnProcessSendFile( zVIEW lpTaskView, LPNETWORK lpNetwork, zPVOID *ppvConn )
{
   ...
   zstrcpy( szFileName, pch );
   SysAppendcDirSep( szFileName );
   zstrcat( szFileName, &pch[ zstrlen( pch ) + 1 ] );

   if ( lpNetwork->nTraceLevel > 0 )
   {
      TraceLine( "(kzoeneta) ** App Name: %s  File Name: %s",
                 sf.szAppName, szFileName );
   }
   ...
   while ( TRUE )
   {
      ...
      if ( sf.bBinaryFile == 'Y' )
      {
         if ( SysWriteFile( vSubtask, f, (zCPCHAR) pchLine,
                            lPacketData ) < lPacketData )
         {
...
fnProcessOperation( zVIEW lpTaskView, LPNETWORK lpNetwork, zPVOID *ppvConn )
{
   ...
   bTransformation = (CompareAttributeToString( vArgs, "Operation",
                                                "OperationType",
                                                zTYPE_TRAN ) == 0);
         ...
         nRC = SetCursorNextEntity( vArgs, szlArgument, 0 ) )
   {
                                     ...
                                     "DataType", "l" ) == 0 )
      {
         ...
         if ( (*lpNetwork->lpfnGetAddress)( vSubtask,
                                            &lpNetwork->pNetworkHandle,
                                            ppvConn, 'R',
                                            szClientAddress ) != 0 )
         {
            goto EndOfFunction;
      ...
      if ( CompareAttributeToString( vArgs, szlArgument, "SendArgument",
                                     "N" ) == 0 )
      {
         continue;
      }
   ...
   pvViewList[ k ] = 0;
   zstrcpy( szName, lpApp->szLibraryDir );
   ...
   if ( bTransformation )
   {
      ...
      zstrcat( szName, lpViewOD->szOperLibname );
   }
   else
   {
   ...
   zsprintf( szName, "_zCall_%s", pchOperName );
         ...
         nRC = SetCursorNextEntity( vArgs, szlArgument, 0 ) )
   {
      ...
      nRC = CompareAttributeToString( vArgs, szlArgument,
                                      "ReturnArgument", "Y" );
      if ( nRC )
         continue;
         ...
         nRC = SetCursorNextEntity( vArgs, szlArgument, 0 ) )
   {
      ...
      if ( lpNetwork->nTraceLevel > 0 )
      {
         ...
         SysGetEnvVar( szTempFileName, "TEMP", zMAX_FILENAME_LTH + 1 );
         SysAppendcDirSep( szTempFileName );
         zstrcat( szTempFileName, "tranoi.por" );
      ...
      // Send the vArgs view back to the client.
      if ( fnSendOI( ppvConn, lpNetwork, vViewList[ k ],
                     zINCREMENTAL | zSAVE_CURSORS ) == zCALL_ERROR )
      {
...
SfStartNetworks( zVIEW lpView, zLONG lControl )
{
   ...
   for ( k = 1; TRUE; k++ )
   {
      zsprintf( szNetName, "Network%d", k );
   // TraceLineS( "(kzoeneta) ** Looking for Net = ", szNetName );

      szValue[ 0 ] = 0;
      SysReadZeidonIni( -1, "[Network]", szNetName, szValue );
      if ( szValue[ 0 ] == 0 )
         break;               // No network# item so break loop.

      zstrcpy( szNetName, szValue );
      zsprintf( szGroupName, "[%s]", szValue );
      SysReadZeidonIni( -1, szGroupName, "Startup", szStartup );
      if ( szValue[ 0 ] == 0 )
         continue;
      ...
      SysReadZeidonIni( -1, szGroupName, "ExecName", szExecName );
      if ( szExecName[ 0 ] == 0 )
         zstrcpy( szExecName, szNetName );

      // If network is started see if we need to do a listen.
      if ( NetStartup( lpView, szNetName, szExecName ) != zCALL_ERROR )
      {
         SysReadZeidonIni( -1, szGroupName, "TraceLevel", szValue );
         if ( szValue[ 0 ] )
