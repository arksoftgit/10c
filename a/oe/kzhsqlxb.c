                  ...
                  zLONG  f )
{
   zCHAR szLine[ 500 ];
      ...
      zsprintf( szLine, "DROP INDEX %s%s.%s %s",
                pchOwner, pchTableName, pchIndexName, LINE_TERMINATOR );
   #else
      zsprintf( szLine, "DROP INDEX %s ON %s%s %s",
...
zSHORT LOCALOPER
fnBuildColumn( zVIEW  vDTE, zLONG  f, zPCHAR pchLine )
{
   ...
   if ( vDBH_Data )
   {
      ...
      bTimestampAsString = ( CompareAttributeToString( vDBH_Data, "ODBC",
                                                       "TimestampAsString",
                                                       "Y" ) == 0 );
   ...
   if ( vTZDBHODO )
   {
      bTimestampAsString = ( CompareAttributeToString( vTZDBHODO, "ODBC",
                                                       "TimestampAsString",
                                                       "Y" ) == 0 );
   }
   else
   ...
   zsprintf( pchEnd, "%-*s", nMaxColumnNameLth, szColName );
   pchEnd = pchEnd + zstrlen( pchEnd );
   ...
   switch ( pchDataType[ 0 ] )
   {
      ...
      case zTYPE_FIXEDCHAR:
         GetIntegerFromAttribute( &nLth, vDTE, "TE_FieldDataRel",
                                  "Length" );
         zsprintf( pchEnd, " TEXT( %ld )", nLth );

         break;

      case zTYPE_BLOB:
         zsprintf( pchEnd, " LONGBINARY" );
         break;
      ...
      case zTYPE_DECIMAL:
         zsprintf( pchEnd, " DOUBLE" );
         break;

      case zTYPE_DATETIME:
      case zTYPE_TIME:
         if ( bTimestampAsString )
            zsprintf( pchEnd, " TEXT( 25 )" );
         else
         {
            switch ( pchDataType[ 0 ] )
            {
               case zTYPE_DATETIME:
                  zsprintf( pchEnd, " DATETIME" );
                  break;

               case zTYPE_TIME:
                  zsprintf( pchEnd, " TIME" );
                  break;
            }
         }
      ...
      // Long text fields.
      case 'V':
         zsprintf( pchEnd, " MEMO" );
         break;

      // TimeStampEx
      case 'X':
         zsprintf( pchEnd, " TEXT( 30 )" );
         break;
      ...
      default:
      {
         ...
         GetStringFromAttribute( szTableName, vDTE, "TE_TablRec", "Name" );
         zsprintf( szMsg, "Invalid DataType '%s' for attribute %s.%s",
   ...
   switch ( pchDataType[ 0 ] )
   {
      ...
      case zTYPE_DECIMAL:
      {
         ...
         GetIntegerFromAttribute( &nScale, vDTE, "TE_FieldDataRel",
                                  "SQL_SCALE" );

         zsprintf( pchEnd, " DECIMAL( %ld,%ld )", nLth, nScale );
         break;
      }
...
fnBuildFK_Index( zVIEW  vDTE, zLONG  f )
{
   ...
   GetStringFromAttribute( szOwner, vDTE, "TE_TablRec", "SQL_TableOwner" );
   if ( szOwner[ 0 ] == 0 && pchDefaultOwner )
      zstrcpy( szOwner, pchDefaultOwner );
   ...
   if ( pch && pch[ 0 ] )
   {
      zsprintf( szLine, "%s Index for Relationship - '%s' %s",
   ...
   GetStringFromAttribute( szKeyName, vDTE, "TE_FieldDataRel", "Name" );
   RemoveBrackets( szKeyName );

   // The base name for the index is a concatenation of the table and
   // key name.  To ensure that the index name is unique, we also use the
   // zkey value.
   // NOTE: Becaues of a bug in UfCompressName, we tack on a extra '0' to the
   // name so that the zkey does not get truncated.
   zsprintf( szWorkIdxName, "%s_%s_%ld0", szTableName, szKeyName, lZKey );

   // Make sure that the index name is a valid length.
   UfCompressName( szWorkIdxName, szWorkIdxName, nMaxTableNameLth,
                   "", "B", "", "B_AEIOUYBCDFGHJKLMNPQRSTVWXZ", 0 );

   zsprintf( szLine, "CREATE INDEX %s%s %s",
             szOwner, szWorkIdxName, CONTINUATION_STR );
   if ( fnWriteLine( vDTE, f, szLine ) < 0 )
      return( -1 );

   zsprintf( szLine, "       ON %s%s ( %s",
             szOwner, szTableName, CONTINUATION_STR );
   if ( fnWriteLine( vDTE, f, szLine ) < 0 )
      return( -1 );

   // Write Key-Name.
   zsprintf( szLine, "%*s %s ) %s",
...
fnBuildIndexFromTablRecKey( zVIEW vDTE, zBOOL bUnique, zLONG f )
{
   ...
   if ( CheckExistenceOfEntity( vDTE, "ER_Entity" ) >= zCURSOR_SET )
   {
      ...
      zsprintf( szLine, "%s Main key for Entity - %s %s",
                COMMENT_START, pch, COMMENT_END );
   ...
   // us the Name value.
   GetStringFromAttribute( szName, vDTE, "TE_TablRecKey", "IndexName" );
   if ( szName[ 0 ] == 0 )
   {
      zCHAR  szEntityName[ MAX_NAME_LTH + 1 ];
      zLONG  lZKey;

      GetStringFromAttribute( szEntityName, vDTE, "TE_TablRec", "Name" );
      RemoveBrackets( szEntityName );

      GetStringFromAttribute( szName, vDTE, "TE_TablRecKey", "Name" );
      ...
      if ( bUnique )
         zsprintf( szWorkIdxName, "U%s_%s_%ld0", szEntityName, szName, lZKey );
      else
         zsprintf( szWorkIdxName, "%s_%s_%ld0", szEntityName, szName, lZKey );
   }
   else
   {
      RemoveBrackets( szName );
      zsprintf( szWorkIdxName, "%s", szName );
   }
   ...
   if ( bUnique )
   {
      zsprintf( szLine, "CREATE UNIQUE INDEX %s%s %s",
                szOwner, szWorkIdxName, CONTINUATION_STR );
   }
   else
      zsprintf( szLine, "CREATE INDEX %s%s %s",
   ...
   {
      GetStringFromAttribute (szName, vDTE, "TE_FieldDataRelKey", "Name" );
      RemoveBrackets( szName );
...
fnBuildCreateTable( zVIEW  vDTE, zLONG  f )
{
   ...
   zsprintf( szLine, "CREATE TABLE %s%s ( %s",
             szOwner, szTableName, CONTINUATION_STR );
   ...
   while ( nLoop >= zCURSOR_SET )
   {
      zsprintf( szLine, "%*s ", (zSHORT) COLUMN_INDENT, " " );
      ...
      if ( nLoop >= zCURSOR_SET )
         ...
         zsprintf( pchEnd, ", %s", CONTINUATION_STR );
      else
         // No more columns, so terminate line.
         zsprintf( pchEnd, " ) %s", LINE_TERMINATOR );
          ...
          zVIEW  vSubtask )
{
   ...
   GetStringFromAttribute( pchFileName + nLth,
                           vDTE, "TE_DBMS_Source", "Name" );

   // Make sure the filename isn't too long.
   if ( zstrlen( pchFileName + nLth ) > zBASE_FILENAME_LTH )
      pchFileName[ nLth + zBASE_FILENAME_LTH ] = 0;

   zstrcat( pchFileName, ".ddl" );
...
#ifdef CREATE_DB
   zsprintf( szLine, "%s CREATE DATABASE %s %s %s", COMMENT_START,
   ...
   zsprintf( szLine, "CONNECT TO %s %s", pchDatabaseName, LINE_TERMINATOR );
   if ( fnWriteLine( vDTE, f, szLine ) < 0 )
      goto EndOfFunction;
   ...
   if ( pchGenDropIdxs[ 0 ] != 'N' )
   {
            ...
            nRCTable = SetCursorNextEntity( vDTE, "TE_TablRec", 0 ) )
      {
         ...
         {
            GetAddrForAttribute( &pch, vDTE, "ER_Entity", "Name" );
            zsprintf( szLine, "%s Indexes for Entity - %s %s",
                      COMMENT_START, pch, COMMENT_END );
               ...
               nLoop = SetCursorNextEntity( vDTE, "TE_TablRecKey", 0 ) )
         {
            ...
            if ( szName[ 0 ] == 0 )
            {
               ...
               zsprintf( szWorkIdxName, "%s_%s", szEntityName, szName );
            }
            else
            {
               RemoveBrackets( szName );
...

         {
            ...
            GetStringFromAttribute( szKeyName, vDTE, "TE_FieldDataRel", "Name" );
            RemoveBrackets( szKeyName );
            zsprintf( szWorkIdxName, "%s_%s", pchTableName, szKeyName );
            ...
            nLth = zstrlen( szWorkIdxName );
            zsprintf( &szWorkIdxName[ nLth ], "%02d", usForeignKeyCntForTable );
         ...
         nLoop = SetCursorNextEntity( vDTE, "TE_TablRec", 0 ) )
   {
      ...
      pchTableName = szTableName;
      RemoveBrackets( pchTableName );
      zsprintf( szLine, "GRANT ALL ON %s%s TO PUBLIC %s",
...
EndOfFunction:
   ...
   {
      zstrcpy( szMsg, "File " );
      zstrcat( szMsg, pchFileName );
      zstrcat( szMsg, " is created successfully!" );
     MB_SetMessage( vSubtask, 0, szMsg );
   }
   else
   {
       zstrcpy( szMsg, "Couldn't create file " );
       zstrcat( szMsg, pchFileName );
              ...
              zVIEW  vSubtask )
{
   ...
   zsprintf( szLine, "%s Alter script for %s -- %s %s",
   ...
   for ( k = 0; k < 2; k++ )
   {
            ...
            nRC = SetCursorNextEntity( vBoth[ k ], "TE_TablRec", 0 ) )
      {
         ...
         {
            SetAttributeFromString( vBoth[ k ], "TE_FieldDataRel",
                                    "MatchFound", "N" );
         }
      }
   }
         ...
         nRC = SetCursorNextEntity( vDTE, "TE_TablRec", 0 ) )
   {
      ...
      if ( SetCursorFirstEntityByAttr( vDB, "TE_TablRec", "Name",
                                       vDTE, "TE_TablRec",
                                       "Name", 0 ) < zCURSOR_SET )
      {
         continue;
            ...
            nRC = SetCursorNextEntity( vDTE, "TE_FieldDataRel", 0 ) )
      {
         ...
         GetStringFromAttribute( szDB_Type,  vDB, "TE_FieldDataRel", "DataType" );
         GetStringFromAttribute( szDTE_Type, vDTE, "TE_FieldDataRel", "DataType" );
         ...
         if ( lDB_Lth != lDTE_Lth || zstrcmp( szDB_Type, szDTE_Type ) != 0 )
         {
               ...
               zsprintf( sz, "%s.%s  TE Datatype = %s, Length = %d",
                         pchTableName, pchColName, szDTE_Type, lDTE_Lth );
               TraceLineS( sz, "" );
               zsprintf( sz, "%s.%s  DB Datatype = %s, Length = %d",
         ...
         nRC = SetCursorNextEntity( vDTE, "TE_TablRec", 0 ) )
   {
            ...
            nRC = SetCursorNextEntity( vDTE, "TE_FieldDataRel", 0 ) )
      {
         ...
         if ( pchMatchFound[ 0 ] == 'N' )
         {
            ...
            zsprintf( szLine, "ALTER TABLE %s " ADD_COLUMN_STMT " ", szTableName );
            fnBuildColumn( vDTE, f, szLine );
            zstrcat( szLine, LINE_TERMINATOR );
            if ( fnWriteLine( vDTE, f, szLine ) < 0 )
               goto EndOfFunction;
   ...
   {
      GetStringFromAttribute( szTableName, vDB, "TE_TablRec", "Name" );
      RemoveBrackets( szTableName );

      if ( CompareAttributeToString( vDB, "TE_TablRec", "MatchFound", "Y" ) != 0 )
      {
         zsprintf( szLine, "DROP TABLE %s %s", szTableName,
                   LINE_TERMINATOR );
         ...
         nRC = SetCursorNextEntity( vDB, "TE_TablRec", 0 ) )
   {
            ...
            nRC = SetCursorNextEntity( vDB, "TE_FieldDataRel", 0 ) )
      {
         ...
         GetStringFromAttribute( szColumnName, vDB, "TE_FieldDataRel", "Name" );
                                                  ...
                                                  "TE_TablRec" ) )
         {
            ...
            GetStringFromAttribute( szIndex, vDB, "TE_TablRecKey", "IndexName" );
         ...
         {
            // Drop the column.
            zsprintf( szLine, "ALTER TABLE %s%s " DROP_COLUMN_STMT " %s %s",
                                        ...
                                        "MatchFound", "D" ) == 0 )
         {
            ...
            zsprintf( szLine, "ALTER TABLE %s%-*s ALTER COLUMN ",
            ...
            if ( nIndexDroppedCount )
            {
               ...
               zsprintf( szMsg, "WARNING: The data type for the column %s.%s "
               ...
               zsprintf( szMsg, "%s WARNING--Following index may not have the "
...
zVOID LOCALOPER
fnChangeReservedName( zPCHAR pchName )
{
   zSHORT   k, j;
   ...
   for ( k = 0; szReservedName[ k ][ 0 ]; k++ )
   {
      ...
      if ( j == 0 )
      {
         ...
         zstrcat( szTemp, pchName );
         zstrcpy( pchName, szTemp );
         return;
      }
              ...
              zCHAR  cMetaType )
{
   ...
   switch ( cMetaType )
   {
      ...
      case 'X':      // pchName is a duplicate, generate a new one.
      {
         ...
         // number.  For example, "ColName" becomes "ColName02", "ColName02"
         // becomes "ColName03", etc.
         ...
         zsprintf( pchName + nLth, "%02d", nNameCount );
...
fnCreateStartFunctions( zVIEW lpTaskView, zLONG f, LPVIEWOD lpViewOD )
{
   ...
   zsprintf( szLine, "%s_OpenConnection( zPCHAR pchUser, zPCHAR pchPassword, "
   ...
   SysWriteLine( lpTaskView, f, "   EXEC SQL END DECLARE SECTION;" );
   SysWriteLine( lpTaskView, f, " " );
   SysWriteLine( lpTaskView, f, "   zstrcpy( szLocalUser, pchUser );" );
   SysWriteLine( lpTaskView, f, "   zstrcpy( szLocalPassword, pchPassword );" );

   zsprintf( szLine, "   EXEC SQL CONNECT TO %s USER :szLocalUser "
   ...
   zsprintf( szLine, "      TraceLineS( \"   EXEC SQL CONNECT TO %s USER :szLocalUser "
                     "USING :szLocalPassword;\", \"\" );", lpViewOD->szDfltDBName );
   SysWriteLine( lpTaskView, f, szLine );
   ...
   SysWriteLine( lpTaskView, f, "zSHORT OPERATION" );
   zsprintf( szLine, "%s_CloseConnection( zSHORT nTraceLevel )",
             lpViewOD->szName );
   SysWriteLine( lpTaskView, f, szLine );
   ...
   SysWriteLine( lpTaskView, f, "zSHORT OPERATION" );
   zsprintf( szLine, "%s_Commit( zSHORT nTraceLevel )", lpViewOD->szName );
   SysWriteLine( lpTaskView, f, szLine );
   ...
   SysWriteLine( lpTaskView, f, "zSHORT OPERATION" );
   zsprintf( szLine, "%s_Rollback( zSHORT nTraceLevel )", lpViewOD->szName );
   SysWriteLine( lpTaskView, f, szLine );
...
fnGenerateVarName( zPCHAR       pchTarget,
                   ...
                   zSHORT       nGen )
{
   zsprintf( pchTarget, "%s%d_%ld_%ld", (nGen & GEN_UNIQUE) ? "u" : "v",
             lpViewEntity->nHierNbr,
             lpViewAttrib->lERAttTok, lpBoundViewEntity->lEREntTok );

   if ( nGen & GEN_QUAL )
      zstrcat( pchTarget, "_Qual" );

   if ( nGen & GEN_NULL )
      zstrcat( pchTarget, "_Null" );
}
                ...
                zPCHAR      pchTarget )
{
   ...
   switch ( nCommand )
   {
      ...
      case zSQLCALLBACK_GETCOLUMNNAME:
      {
         ...
         zstrcat( pchTarget, lpDataField->szFldName );
         zsprintf( szTemp, "CAST( %s AS CHAR( 40 ) )", pchTarget );
         zstrcpy( pchTarget, szTemp );
         return( TRUE );
      }
      ...
      case zSQLCALLBACK_GETINTONAME:
         ...
         if ( lpViewAttrib->bRequired == FALSE )
         {
            ...
            zsprintf( pchTarget + zstrlen( pchTarget ),
...
fnWriteSql( zVIEW lpTaskView, zPCHAR pchSQL, zPVOID pInfo )
{
   ...
   while ( zstrlen( pchSQL ) > 80 )
   {
      ...
      zsprintf( szLine, "%*s%s", nIndent, " ", pchSQL );
                          ...
                          zPCHAR       szLine )
{
   ...
   // --> {
   // -->    long lIndex = 0;
   zsprintf( szLine, "%*sif ( %s >= 0 )", g_nIndent, " ", szVar );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*s{", g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   g_nIndent += 3;
   zsprintf( szLine, "%*slong lIndex = 0;", g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   SysWriteLine( lpTaskView, f, " " );
   ...
   // -->   EXEC SQL VALUES ( LENGTH( :_locator_ ) ) INTO g_lColumnLth;
   zsprintf( szLine, "%*sEXEC SQL VALUES ( LENGTH( :%s ) ) INTO :g_lColumnLth;",
             g_nIndent, " ", szVar );
   SysWriteLine( lpTaskView, f, szLine );

   // Alloc the buffer.
   // -->   SDB2_AllocBuffer( &g_pvBuffer, &g_lBufferLth, g_lColumnLth );
   zsprintf( szLine, "%*sSDB2_AllocBuffer( &g_pvBuffer, "
   ...
   // -->    while ( lLth < g_lColumnLth )
   // -->    {
   zsprintf( szLine, "%*swhile ( lIndex < g_lColumnLth )",
             g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*s{", g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   ...
   // -->      lIndex += 10000;
   zsprintf( szLine, "%*sEXEC SQL VALUES ( :%s ) INTO :g_%sData;",
             g_nIndent, " ", szVar, pchType );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*szmemcpy( &g_pvBuffer[ lIndex ], "
             "&g_%sData.data, g_%sData.length );", g_nIndent, " ",
             pchType, pchType );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*slIndex += g_%sData.length;",
             g_nIndent, " ", pchType );
   SysWriteLine( lpTaskView, f, szLine );
   ...
   // --> }
   g_nIndent -= 3;
   zsprintf( szLine, "%*s} // while...", g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );

   // Close 'if' statement.
   // --> }
   g_nIndent -= 3;
   zsprintf( szLine, "%*s} // if...", g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
                       ...
                       zBOOL        bUniqueQual )
{
   ...
   //
   SysWriteLine( lpTaskView, f, "zSHORT OPERATION" );
   zsprintf( szLine, "SQL_%s_%s_Select%s( zVIEW        lpView,",
   ...
   zsprintf( szLine, "%*sLPVIEWOD     lpViewOD,", nTempIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*sLPVIEWENTITY lpViewEntity,", nTempIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   if ( bUniqueQual )
   {
      zsprintf( szLine, "%*sLPQUALENTITY lpQualEntity,", nTempIndent, " " );
      SysWriteLine( lpTaskView, f, szLine );
   }

   zsprintf( szLine, "%*sint          nIndicators,", nTempIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*szSHORT       nTraceLevel )", nTempIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
         ...
         lpBoundAttr = lpBoundAttr->lpNextBoundAttr )
   {
      ...
      switch ( lpViewAttrib->cType )
      {
         ...
         case zTYPE_BLOB:
            zstrcat( szLine, "sql type is blob_locator" );
            SysWriteLine( lpTaskView, f, szLine );
            zstrcpy( szLine, "              " );
            break;
         ...
         case zTYPE_STRING:
            ...
            {
               zstrcat( szLine, "sql type is clob_locator" );
               SysWriteLine( lpTaskView, f, szLine );
               zstrcpy( szLine, "              " );
            }
            else
      ...
      switch ( lpViewAttrib->cType )
      {
         ...
         case zTYPE_BLOB:
            ...
            if ( szFreeLocatorsCmd[ 0 ] == 0 )
               zstrcpy( szFreeLocatorsCmd, "EXEC SQL FREE LOCATOR " );
            else
               zstrcat( szFreeLocatorsCmd, ", " );
      ...
      if ( lpBoundViewEntity != lpViewEntity )
      {
         ...
         zsprintf( szTemp, "// %s.%s", lpBoundViewEntity->szName,
                   lpViewAttrib->szName );
         zstrcat( szLine, szTemp );
      }
      else
      {
         zCHAR szTemp[ 100 ];

         zsprintf( szTemp, "// %s.%s", lpViewEntity->szName,
                   lpViewAttrib->szName );
         zstrcat( szLine, szTemp );
      }
      ...
      if ( lpViewAttrib->bRequired == FALSE && lpBoundAttr->pszValue == 0 )
      {
         ...
         zsprintf( szLine, "      short   %s;", szNullIndicator );
         SysWriteLine( lpTaskView, f, szLine );
      }
         ...
         lpBoundAttr = lpBoundAttr->lpNextBoundAttr )
   {
      ...
      if ( lpBoundViewEntity == lpViewEntity )
      {
         ...
         {
            case zTYPE_INTEGER:
               zstrcat( szLine, "SDB2_GetIntegerFromQualification( &" );
               break;

            default:
               zstrcat( szLine, "SDB2_GetStringFromQualification( " );
               break;
         }
         ...
         zsprintf( szLine, "%*slpViewEntity, \"%s\", lpQualEntity );",
      ...
      else
      {
         ...
         {
            case zTYPE_INTEGER:
               zstrcat( szLine, "SDB2_GetIntegerFromAttribute( &" );
               break;

            default:
               zstrcat( szLine, "SDB2_GetStringFromAttribute( " );
               break;
         }
         ...
         zsprintf( szLine, "%*slpView, lpViewOD, \"%s\", \"%s\" );",
                   nIndent, " ",
   ...
   // If the result of the SELECT statement can have more than 1 row we
   // need to declare a cursor.  If we have unique qualification then we
   // know we are only loading 1 entity.
   ...
   // lpBoundList = 0;
      ...
      SqlInitBoundAttrList( &lpBoundList, 0, fnDBH_Callback, 0xFFFFFFFF,
                            &Callback );
      nRC = SqlBuildSelect( lpTaskView, lpViewOD, lpViewEntity, lpQualEntity,
                            szSqlCmd, lpBoundList );

      // Now generate the cursor for the SQL statement.
      zsprintf( szLine, "   EXEC SQL DECLARE c%d CURSOR FOR",
                lpViewEntity->nHierNbr );
      SysWriteLine( lpTaskView, f, szLine );
      zstrcat( szSqlCmd, ";" );
      g_nIndent = 6;
   ...
   // lpBoundList = 0;
      ...
      SysWriteLine( lpTaskView, f, " " );
      zsprintf( szLine, "   EXEC SQL OPEN c%d;", lpViewEntity->nHierNbr );
      SysWriteLine( lpTaskView, f, szLine );
      ...
      // =================== Generate 'INTO' ========================
      //
      zsprintf( szSqlCmd, "   EXEC SQL FETCH c%d INTO",
                lpViewEntity->nHierNbr );
      pch = szSqlCmd;
      bFirstAttr = TRUE;
      for ( lpBoundAttr = lpBoundList->lpFirstBoundAttr;
            ...
            lpBoundAttr = lpBoundAttr->lpNextBoundAttr )
      {
         ...
         if ( lpViewAttrib->bRequired == FALSE )
         {
            ...
            pch = pch + zstrlen( pch );
            zsprintf( pch, " INDICATOR :%s", szNullIndicator );
         }
      }
   ...
   zsprintf( szLine, "%*snRowCount++;", g_nIndent, " " );
   SysWriteLine( lpTaskView, f, szLine );
   zsprintf( szLine, "%*sLoadEntity( lpView, \"%s\", zPOS_AFTER, 0 );",
         ...
         lpBoundAttr = lpBoundAttr->lpNextBoundAttr )
   {
      ...
      zsprintf( szLine, "%*s// %s.%s ", g_nIndent, " ",
      ...
      {
         case zTYPE_INTEGER:
            zsprintf( szLine, "%*sSDB2_SetAttributeFromInteger( lpView, "
                      "lpViewOD, lpViewEntity, \"%s\",", g_nIndent, " ",
                      lpViewAttrib->szName );
            fnGenerateVarName( szVar, lpViewEntity,
                               lpBoundViewEntity, lpViewAttrib, nGen );
         ...
         case zTYPE_BLOB:
         {
            ...
            zsprintf( szLine, "%*sSDB2_SetAttributeFromBlob( lpView, "
                      "lpViewOD, lpViewEntity, \"%s\", ",
                      g_nIndent, " ", lpViewAttrib->szName );

            // szVar is the name of the variable we are sending to
            // SDB2_SetAttributeFromBlob( ).
            zsprintf( szVar, "g_pvBuffer", lpViewOD->szName );
            break;
         }
         ...
         default:
            ...
            {
               zsprintf( szLine, "%*sSDB2_SetAttributeFromString(  lpView, "
                         "lpViewOD, lpViewEntity, \"%s\",", g_nIndent, " ",
                         lpViewAttrib->szName );
      ...
      zsprintf( szLine, "%*s%s, ", 30 +  g_nIndent, " ", szVar );
      if ( lpViewAttrib->bRequired == FALSE )
         fnGenerateVarName( &szLine[ zstrlen( szLine ) ], lpViewEntity,
                            lpBoundViewEntity, lpViewAttrib,
                            (zSHORT) (nGen | GEN_NULL) );
   ...
   {
      SysWriteLine( lpTaskView, f, " " );
      zsprintf( szLine, "   %s;", szFreeLocatorsCmd );
        ...
        (lpViewEntity->hParent == 0 || lpViewEntity->uCardMax > 1) )
   {
      ...
      SysWriteLine( lpTaskView, f, "   } // while ( TRUE )..." );
      SysWriteLine( lpTaskView, f, " " );
      zsprintf( szLine, "   EXEC SQL CLOSE c%d;", lpViewEntity->nHierNbr );
      SysWriteLine( lpTaskView, f, szLine );
   }
                       ...
                       LPVIEWENTITY lpViewEntity )
{
   ...
   //
   SysWriteLine( lpTaskView, f, "zSHORT OPERATION" );
   zsprintf( szLine, "SQL_%s_%s_Insert( zVIEW        lpView,",
             lpViewOD->szName, lpViewEntity->szName );
   SysWriteLine( lpTaskView, f, szLine );
         ...
         lpBoundAttr = lpBoundAttr->lpNextBoundAttr )
   {
      ...
      zsprintf( &szLine[ zstrlen( szLine ) ], "// %s.%s",
                lpViewEntity->szName, lpViewAttrib->szName );
         ...
         lpBoundAttr = lpBoundAttr->lpNextBoundAttr )
   {
      ...
      {
         case zTYPE_INTEGER:
            zstrcat( szLine, "fnGetIntegerFromAttribute( &" );
            break;

         default:
            zstrcat( szLine, "fnGetStringFromAttribute( " );
            break;
      }
                      ...
                      zVIEW  vTZZOXODO )
{
   ...
   zstrcpy( pchReturnDirectory, szFileName );
   SysAppendcDirSep( szFileName );
   GetStringFromAttribute( szLOD_Name, vTZZOLODO, "LOD", "Name" );
   zstrcpy( pchReturnFileName, szLOD_Name );
   zstrcat( szFileName, szLOD_Name );
   zstrcpy( szTempFileName, szFileName );  // Save temp file name for later.
   zstrcat( pchReturnFileName, ".sqc" );
   zstrcat( szFileName, ".sqc" );
                          ...
                          zPCHAR pchLine )
{
   ...
   zsprintf( pchLine, "%s\\%s.MAK", pchMakefileDir, g_szSqlDLLName );
   hMakefile = SysOpenFile( vTaskLPLR, pchLine, COREFILE_APPEND );
   if ( hMakefile == -1 )
   {
      zsprintf( pchLine, "Could not re-open file\n  '%s'.\nCheck Compiler "
   ...
   zsprintf( pchLine, "!include \"%s\\zcompile.hnm\"", pchMakefileDir );
   ...
   zsprintf( pchLine, "%s\\%s.BAT", pchMakefileDir, g_szSqlDLLName );
                ...
                LPDB2_OBJECTDATA lpDB2_Data )
{
   ...
   GetStringFromAttribute( lpDB2_Data->szStaticSQL_DLL,
                           TZDBHDBO, "ApplA", "StaticSQL_DLL_Name" );
   ...
   zsprintf( szFileName, "%s\\%s.MAK", pchMakefileDir, pchTargetName );
   ...
   if ( zstrcmpi( pchTargetName, g_szSqlDLLName ) != 0 )
   {
      ...
      if ( hMakefile == -1 )
      {
         zsprintf( szLine, "Could not create file\n  '%s'\n.Check Compiler "
      ...
      zsprintf( szTemp, "%s.MAK", pchTargetName );
      ...
      zsprintf( szLine, "#  %s.mak", pchTargetName );
      SysWriteLine( vTZZOLODO, hMakefile, szLine );
      SysWriteLine( vTZZOLODO, hMakefile, "#  Makefile for compiling DB2 static SQL" );
      SysGetDateTime( szTime1 );
      UfFormatDateTime( szTime2, szTime1, "DD-MM-YYYY HH:MI:SS" );
      zsprintf( szLine, "#  Generated on %s", szTime2 );
      SysWriteLine( vTZZOLODO, hMakefile, szLine );
      SysWriteLine( vTZZOLODO, hMakefile, "#" );
      SysWriteLine( vTZZOLODO, hMakefile, " " );

      SysWriteLine( vTZZOLODO, hMakefile, "# Include makefile to set environment variables." );
      zsprintf( szLine, "!include \"%s\\zenv.hnm\"", pchMakefileDir );
      SysWriteLine( vTZZOLODO, hMakefile, szLine );
      SysWriteLine( vTZZOLODO, hMakefile, " " );

      zsprintf( szLine, "MODULE     = %s", pchTargetName );
      SysWriteLine( vTZZOLODO, hMakefile, szLine );
      GetAddrForAttribute( &pch,
                           TZDBHDBO, "ApplA", "StaticSQL_C_FileDir" );
      zsprintf( szLine, "SRC_DIR    = %s", pch );
      SysWriteLine( vTZZOLODO, hMakefile, szLine );
      zsprintf( szLine, "OBJ_DIR    = %s", pch );
      SysWriteLine( vTZZOLODO, hMakefile, szLine );
      SysWriteLine( vTZZOLODO, hMakefile, " " );
      ...
      zstrcpy( g_szSqlDLLName, pchTargetName );

      // Add the batch file to the list of files we must copy to the server.
      CreateEntity( vApSvDisO, "DS_TempFile", zPOS_FIRST );
      zsprintf( szFileName, "%s.BAT", pchTargetName );
      SetAttributeFromString( vApSvDisO, "DS_TempFile", "LocalDirectory", pchMakefileDir );
      SetAttributeFromString( vApSvDisO, "DS_TempFile", "Name", szFileName );

      // Open the batch file.
      zsprintf( szFileName, "%s\\%s.BAT", pchMakefileDir, pchTargetName );
      ...
      zsprintf( szLine, "rem  Usage: %1.bat <db_name> <userid> <password>", pchTargetName );
   ...
   else
   {
      ...
      if ( hMakefile == -1 )
      {
         zsprintf( szLine, "Could not re-open file\n  '%s'.\nCheck Compiler "
                  "Specification for valid Makefile Directory", szFileName );
         MessageSend( vTaskLPLR, "LO00107", "Zeidon Tools",
                      szLine, zMSGQ_SYSTEM_ERROR, TRUE );
         goto EndOfFunction;
   ...
   zsprintf( szLine, "    $(OBJ_DIR)\\%s.obj \\", pch );
   SysWriteLine( vTZZOLODO, hMakefile, szLine );

   // Add the the LOD to the precompile/bind batch file.
   GetAddrForAttribute( &pchOutDir, TZDBHDBO, "ApplA", "StaticSQL_C_FileDir" );
   zsprintf( szLine, "db2 prep %s.SQC bindfile disconnect explicit output %s\\%s.c",
             pch, pchOutDir, pch );
   SysWriteLine( vTZZOLODO, hBatFile, szLine );
   zsprintf( szLine, "db2 bind %s.bnd", pch );
   SysWriteLine( vTZZOLODO, hBatFile, szLine );
   SysWriteLine( vTZZOLODO, hBatFile, " " );
...
fnSortDataFields( zVIEW vTZZOXODO )
{
   ...
   // Initialize DATARECORD into szEntityName so that we reference the first
   // ENTITY.
   zstrcpy( szEntityName, "DATARECORD" );

   for ( nRC = DefineHierarchicalCursor( vTZZOXODO, "DATARECORD" );
         nRC >= zCURSOR_SET;
         nRC = SetCursorNextEntityHierarchical( &usReturnLevel,
                                                szEntityName, vTZZOXODO ) )
   {
      zLONG lPos;
      ...
      // to the beginning. We'll skip the very last entity since it's already
      // last.
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         if ( CompareAttributeToString( vXOD2, "DATAFIELD", "FLDTYPE",
                                        "V" ) == 0 ||
              CompareAttributeToString( vXOD2, "DATAFIELD", "FLDTYPE",
                                        "B" ) == 0 )
         {
            // Move current DATAFIELD to the end.
            MoveSubobject( vXOD2, "DATAFIELD", vXOD2, "DATAFIELD",
                           zPOS_LAST, zREPOS_PREV );
         }
      }

      // Move Keys fields to the beginning.  We'll start at the end and go back
      // to the beginning. We'll skip the very first entity since it's already
      // first.
      SetViewFromView( vXOD2, vXOD );
      ...
      while ( nRC >= zCURSOR_SET )
      {
         ...
         SetCursorFirstEntityByAttr( vXOD, "ATTRIB", "XVAATT_TOK",
                                     vXOD, "DATAFIELD", "XVAATT_TOK", 0 );
...
fnSetEntityInformation( zVIEW vTZZOLODO, zVIEW vTZZOXODO, zLONG lDBMS_ZKey )
{
                                       ...
                                       "LOD_EntityParent" ) == zCURSOR_SET )
   {
      ...
      if ( CompareAttributeToString( vTZZOLODO, "POD_Entity",
                                     "SQL_JoinWithParent", "Y" ) == 0 )
      {
   ...
   if ( CheckExistenceOfEntity( vTZZOLODO, "ER_RelLinkRec" ) == zCURSOR_SET &&
        CompareAttributeToInteger( vTZZOLODO, "ER_RelLinkRec",
                                   "CardMax", 1 ) == 0 )
   {
      // Join is defaulted to Y if the entity has a x-to-1 relationship
      // with it's parent.
      SetAttributeFromString( vTZZOXODO, "DATARECORD", "JOIN", "Y" );
   }
         ...
         nRC = SetCursorNextEntity( vTZZOXODO, "CHILDENTITY", 0 ) )
   {
      ...
      GetIntegerFromAttribute( &lER_EntTok, vTZZOXODO, "CHILDENTITY",
                               "ERENT_TOK" );
      GetIntegerFromAttribute( &lER_RelTok, vTZZOXODO, "CHILDENTITY",
                               "ERREL_TOK" );
            ...
            nRC = SetCursorNextEntity( vTZZOLODO, "LOD_EntityChild", 0 ) )
      {
         ...
         if ( CheckExistenceOfEntity( vTZZOLODO,
                                      "ER_EntityRec" ) == zCURSOR_SET &&
              CompareAttributeToInteger( vTZZOLODO, "ER_EntityRec", "ZKey",
                                         lER_EntTok ) == 0 &&
              CheckExistenceOfEntity( vTZZOLODO,
                                      "ER_RelLinkRec" ) == zCURSOR_SET &&
              CompareAttributeToInteger( vTZZOLODO, "ER_RelLinkRec", "ZKey",
                                         lER_EntTok ) == 0 )
         {
            // We found the child entity that matches the CHILDENTITY
            // in the XOD.  Stop 'for' loop.
            break;
         }
