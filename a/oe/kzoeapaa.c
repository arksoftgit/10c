                          ...
                          zSHORT nMaxLth )
{
   ...
   if ( (lpCurrentTask = fnOperationCall( iGetApplDirectoryFromView,
   ...
   if ( pchDirectory )
   {
      if ( (zSHORT) zstrlen( pchDirectory ) > (nMaxLth - 1) )
         ...
         zstrcpy( pchReturnDirectory, pchDirectory );
   }
...
// DGC 9/6/96  Created following local operation from SfGetApplicationForSubtask
// to be used internally by Core.
zSHORT LOCALOPER
                            ...
                            zVIEW lpView )
{
   *lpApp = 0;
   ...
   // If this is a subtask view, which it should be, pass the pointer
   // to the application.
   if ( lpView->hSubtask )
   {
      LPSUBTASK lpSubtask = zGETPTR( lpView->hSubtask );
...
// Change by DGC 5/27/96 -- Instead of just a subtask view we will get the
// application from any kind of view.
...
// Change by DGC 10/15/96 -- Look at lpView->hViewOD before lpView->hTask.
   ...
   else
   {
      ...
      // We don't have a subtask view, zero the returned application
      // pointer, issue an error message, and give an error return code.
      *lpApp = 0;
...
//./ END + 3
zSHORT OPERATION
SfGetApplicationForSubtask( LPAPP zFAR *lpApp,
                            zVIEW lpView )
{
   LPTASK lpCurrentTask;
   zSHORT nRC;
                  ...
                  zCPCHAR cpcAppString )
{
   ...
   zstrcpy( szWorkAppString, cpcAppString );
...
//    pchWdodPtr = pchParsePtr;
   ...
   if ( pchApplPtr )
      zstrcpy( pchReturnApp, pchApplPtr );

   if ( pchWdodPtr )
      zstrcpy( pchReturnDialog, pchWdodPtr );

   if ( pchUserPtr )
      zstrcpy( pchReturnUserID, pchUserPtr );

   if ( pchPassPtr )
      zstrcpy( pchReturnPassword, pchPassPtr );

   if ( pchCmdPtr )
      zstrcpy( pchReturnCommand, pchCmdPtr );
...
//    chMainTask = 0;
   ...
   if ( cpcAppName ) // if not a call to load system app
   {
      ...
      zstrcpy( szWorkString, AnchorBlock->szZeidonLocal );
      zstrcat( szWorkString, szlAppFile );
      ...
      while ( nEOF > 0 )
      {
         ...
         {
            while ( (nEOF = fnSysReadLine( lpTask, &pchLine,
                                           hFile, &pvFile )) > 0 )
            {
   ...
   if ( cpcAppName )   // look for the user application
   {
      ...
      while ( nEOF > 0 )
      {
         ...
         if ( pchLine[ 0 ] == 'a' )
         {
            ...
            if ( bLName )
            {
               if ( lpLName )
               {
                  ...
                  if ( zstrcmp( szWorkString, szlLNAME ) == 0 )
                     zstrcpy( lpLName->szLName, pchLine );
                  else
                  if ( zstrcmp( szWorkString, szlPNAME ) == 0 )
                     zstrcpy( lpLName->szPName, pchLine );
               }
            }
            else
            ...
            {
               zstrcpy( lpApp->szDefaultDialog, pchLine );
            }
            else
                 ...
                 lpApp->szLibraryDir[ 0 ] == 0 )
            {
               ...
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szLibraryDir, szWorkString2 );
            }
            else
                 ...
                 lpApp->szObjectDir[ 0 ] == 0 )
            {
               SysAppendcDirSep( pchLine );
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szObjectDir, szWorkString2 );
            }
            else
                 ...
                 lpApp->szLocalDir[ 0 ] == 0 )
            {
               SysAppendcDirSep( pchLine );
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szLocalDir, szWorkString2 );
            }
            else
                 ...
                 lpApp->szSharedDir[ 0 ] == 0 )
            {
               SysAppendcDirSep( pchLine );
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szSharedDir, szWorkString2 );
            }
            else
                 ...
                 lpApp->szQLPLR_Dir[ 0 ] == 0 )
            {
               SysAppendcDirSep( pchLine );
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szQLPLR_Dir, szWorkString2 );
            }
            else
                 ...
                 lpApp->szQXODS_Dir[ 0 ] == 0 )
            {
               SysAppendcDirSep( pchLine );
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szQXODS_Dir, szWorkString2 );
            }
            else
                 ...
                 lpApp->szSourceDir[ 0 ] == 0 )
            {
               SysAppendcDirSep( pchLine );
               fnConvertEnvironmentString( szWorkString2, pchLine );
               zstrcpy( lpApp->szSourceDir, szWorkString2 );
            }
         }
   ...
   else       // set up the system application
   {
      ...
      zstrcpy( lpApp->szName,       "Zeidon System" );
      zstrcpy( lpApp->szObjectDir,  AnchorBlock->szZeidonSys );
      zstrcpy( lpApp->szLibraryDir, AnchorBlock->szZeidonBin );
      zstrcpy( lpApp->szLocalDir,   AnchorBlock->szZeidonLoc );
      zstrcpy( lpApp->szSharedDir,  AnchorBlock->szZeidonShr );
   }
   ...
   // for the application
   zstrcpy( szWorkString, "[App." );
   zstrcpy( szWorkString + 5, lpApp->szName );
   zstrcat( szWorkString, "]" );
   szWorkString2[ 0 ] = 0;

   // Check to see if we should keep the app running at all times.
   SysReadZeidonIni( -1, szWorkString, "KeepAlive", szWorkString2 );
   ...
   SysReadZeidonIni( -1, szWorkString, "Domains", szWorkString2 );
   ...
   if ( szWorkString2[ 0 ] )
   {
      ...
      {
         zstrcpy( szWorkString, lpApp->szObjectDir );
         SysAppendcDirSep( szWorkString );
         zstrcat( szWorkString, szWorkString2 );
      }
   }
   else
   {
      ...
      zstrcpy( szWorkString, lpApp->szObjectDir );
      SysAppendcDirSep( szWorkString );
      zstrcat( szWorkString, szlDomainFile );
#if 0 // DGC Removed 09/05/2002
      if ( cpcAppName )
         zstrcat( szWorkString, szlDomainFile );
      else
         zstrcat( szWorkString, szlSystemDomainFile );
#endif
   }
   ...
   while ( nEOF > 0 )
   {
      ...
      if ( pchLine[ 0 ] == 'e' )
      {
         SysParseLine( szEntityName, &pchLine, pchLine + 1 );
            ...
            switch ( nCurrEntity )
            {
                  ...
                  switch ( szAttrName[ 0 ] )
                  {
                     ...
                     case 'D':
                        ...
                        if ( zstrcmpi( szAttrName, szlDLL_NAME ) == 0 )
                           zstrcpy( lpDomain->szLibraryName, pchLine );
                     ...
                     case 'N':
                        if ( zstrcmpi( szAttrName, szlNAME ) == 0 )
                           zstrcpy( lpDomain->szName, pchLine );

                        break;

                     case 'O':
                        if ( zstrcmpi( szAttrName, "OPERNAME" ) == 0 )
                           zstrcpy( lpDomain->szDomainOper, pchLine );
                        else
               ...
               case 2:
                  ...
                  switch ( szAttrName[ 0 ] )
                  {
                     ...
                     case 'N':
                        if ( zstrcmpi( szAttrName, szlNAME ) == 0 )
                             zstrcpy( lpContext->szName, pchLine );
