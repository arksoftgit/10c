                   ...
                   zPVOID  pvInfoStruct )
{
   ...
   if ( lpBaseInfo->pszUserSuppliedAddress &&
        lpBaseInfo->pszUserSuppliedAddress[ 0 ] )
   {
      zstrcpy( pchReturnAddress, lpBaseInfo->pszUserSuppliedAddress );
      return( 0 );
   }
   ...
   if ( SetCursorFirstEntityByString( vOI, "Group", "Name",
                                      "ApplServ", 0 ) < zCURSOR_SET )
   {
      // No...create it.
   ...
   if ( SetCursorFirstEntityByString( vOI, "Key", "Name",
                                      "ServerAddress", 0 ) < zCURSOR_SET )
   {
      zCHAR szGroup[ 4096 ];
      ...
      SetAttributeFromString( vOI, "Key", "Desc",
                              "Current address of the appl server" );

      // Read the address from the zeidon ini.
      zsprintf( szGroup, "[%s]", lpBaseInfo->pszNetworkName );
      SysReadZeidonIni( -1, szGroup, "ApplServer", pchReturnAddress );
      SetAttributeFromString( vOI, "Key", "Value", pchReturnAddress );
   }
   else
      GetStringFromAttribute( pchReturnAddress, vOI, "Key", "Value" );
