...
// lpTask->lTaskID          = (zLONG) hTask; // SysGetProcessID( );
   ...
   zsprintf( szTaskName, "App Task 0x%08x", hTask );
   ...
   zstrcpy( szMessage, "(tm) "); // prefix message
   if ( AnchorBlock->hMainTask == 0 ||
        AnchorBlock->hMainTask == hTask )
   {
      zstrcat( szMessage, "Main " );
   }

   zstrcat( szMessage, szTaskName );
   zstrncpy( lpTask->szTaskTitle, cpcTaskTitle, 127 );
...
//./ END + 2
zSHORT  OPERATION
SetTaskDBHandlerTraceLevel( zVIEW  lpView, zSHORT nTraceLevel )
{
   LPTASK lpCurrentTask;
   LPTASK lpTask;
   ...
   lpTask = zGETPTR( lpView->hTask );
   lpTask->nDBHandlerTraceLevel = nTraceLevel;
...
// }
   ...
   // zltoa( (zLONG) hTask, szMsg );
      sprintf( szMsg, "0x%08x", hTask );
...
SfGetTaskInfo( zPCHAR pchReturnString, zSHORT nInfoRequest, LPTASK lpTask )
{
   ...
   switch ( nInfoRequest )
   {
      case zTASK_DIALOG:
         ...
         {
            zstrcpy( pchReturnString, lpTask->szDialog );
      ...
      case zTASK_COMMAND:
         if ( lpTask->hCommand )
            zstrcpy( pchReturnString, zGETPTR( lpTask->hCommand ) );
         else
            pchReturnString[ 0 ] = 0;
                 ...
                 zCPCHAR cpcAppName )
{
   ...
   if ( lpView == 0 )
   {
      TraceLine( "SfCreateSubtask now REQUIRES a non-zero qualification view"
                   "for Application: %s.", cpcAppName );
      SysMessageBox( 0, "Zeidon OE", "SfCreateSubtask now REQUIRES a "
   ...
   // If task not active or disabled, or view invalid, return zCALL_ERROR.
   if ( (lpCurrentTask = fnOperationCall( iSfCreateSubtask, lpView,
                                          zVALID_VIEW )) == 0 )
   {
      return( zCALL_ERROR );
   }
...
fnCreateSubtask( LPTASK lpTask, LPAPP lpApp )
{
   ...
   // Allocate an application task record.
   hView = fnAllocDataspace( (LPDATAHEADER) g_hAnchorBlock,
                             sizeof( ViewRecord ), 1, 0, iView );
   lpView = zGETPTR( hView );
   if ( lpView )
   {
      hSubtask = fnAllocDataspace( (LPDATAHEADER) g_hAnchorBlock,
                                   sizeof( SubtaskRecord ), 1, 0, iSubtask );
      lpSubtask = zGETPTR( hSubtask );
      ...
      // TraceLine( "CreateSubtask from task: 0x%08x   app: 0x%08x %s",
      //            lpView->hTask, lpSubtask->hApp, lpApp ? lpApp->szName : "" );
      }
      else
      {
...
SfDropSubtask( zVIEW vSubtask, zSHORT nCleanup )
{
   ...
   if ( (lpCurrentTask = fnOperationCall( iSfDropSubtask, vSubtask,
                     ...
                     zLONG  lpDropApp )
{
   ...
   while ( hTask )
   {
      ...
      if ( lpTask->bTransientTask && hTask != lpTaskView->hTask )
      {
         ...
         {
            fnEndBrowseOfTaskList( FALSE );
            TraceLineX( "GarbageCollectViews Unregistering Task: ",
                        (zLONG) hTask );
...
DisplayOpenFiles( zCPCHAR cpcTitle, zPVOID hTask )
{
   ...
   while ( lpTask )
   {
      zsprintf( szMsg, "   Task (0x%08x) %s   Process: %d",
                zGETHNDL( lpTask ), lpTask->szTaskTitle, lProcessID );
...
//*********************************************************************************
      ...
      while ( lpOpenFile )
      {
         zsprintf( szMsg, "      Open file: '%s'", lpOpenFile->szFileName );
         SysMessageList( szMsg );
   ...
   while ( lpApp )
   {
      zsprintf( szMsg, "   App (0x%08x) %s   %s",
                zGETHNDL( lpApp ), lpApp->szName, lpApp->bKeepAlive ? "(KeepAlive)" : "" );
      SysMessageList( szMsg );
      lpViewOD = zGETPTR( lpApp->hFirstViewOD );
      while ( lpViewOD )
      {
         zsprintf( szMsg, "      View OD (0x%08x): '%s'",
                   zGETHNDL( lpViewOD), lpViewOD->szName );
         SysMessageList( szMsg );
