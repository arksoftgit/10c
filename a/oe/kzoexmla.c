                    ...
                    LPVIEWENTITY lpViewEntity )
{
         ...
         lpViewAttrib = zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
   {
      ...
      {
         zstrcpy( szAttribIdent, "KEY WAS BLOB - NOT SUPPORTED" );
      }
      else
         ...
         else
         {
            ...
            if ( (lCharCount = fnContainsSpecialChar( pchAttribValue ) ) )
            {
                  ...
                  switch ( *pchAttribValue )
                  {
                     ...
                     default:
                        if ( *pchAttribValue < 32 || *pchAttribValue > 127 )
                        {
                           ...
                           *pch++ = '&';
                           *pch++ = '#';
                           zltoa( (zLONG) (zUCHAR) *pchAttribValue, pch );
               ...
               *pch++ = 0;
               ...
               zstrcpy( szAttribIdent, pchTempValueBuffer );

            } // if contains special chars
            else
            {
               zstrcpy( szAttribIdent, pchAttribValue );
      ...
      zsprintf( &lpWriteInfo->szBuffer[ zstrlen( lpWriteInfo->szBuffer ) ],
                ...
                zLONG        lFlags )
{
         ...
         lpViewAttrib = zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
   {
      ...
      if ( (lFlags & 1) == 0 )  // not doing "simple attributes"
      {
         if ( lpViewAttrib->szXML_SimpleName[ 0 ] == 0 )
         {
            if ( lpViewAttrib->szXML_ExternalName[ 0 ] == 0 )
               zsprintf( szAttribTag, "<%s", lpViewAttrib->szName );
            else
               zsprintf( szAttribTag, "<%s", lpViewAttrib->szXML_ExternalName );
            ...
            if ( lpWriteInfo->lControl & zXML_INCREFLAGS )
            {
               ...
               {
                  zsprintf( szAttribTag + zstrlen( szAttribTag ),
      ...
      if ( lFlags & 1 )  // doing "simple attributes"
      {
         ...
         {
            zsprintf( szAttribTag, " %s=", lpViewAttrib->szXML_SimpleName );
            if ( pchAttribValue[ 0 ] )
            {
               zsprintf( szAttribTag, " %s=\"%s\"", lpViewAttrib->szXML_SimpleName, pchAttribValue );
               zstrcat( lpWriteInfo->szBuffer, szAttribTag );
            }
         }
      }
      else
                       ...
                       LPENTITYINSTANCE lpEntityInstance )
{
   ...
   // Build Entity tag.
   zsprintf( lpWriteInfo->szBuffer, "<%s", pchRealEntityName );
                    ...
                    LPVIEWATTRIB lpViewAttribValue )
{
   ...
   zsprintf( szAttribTag, "<%s>", pchAttribName );

   // Treat the attribute lpViewAttribValue as the regular attribute and
   // use its value.
                       ...
                       LPENTITYINSTANCE lpEntityInstance )
{
   ...
   zsprintf( szWriteLine, "<ListEntryValue Name=\"%s\">", pchAttribName );
                     ...
                     LPENTITYINSTANCE lpEntityInstance )
{
   ...
   zsprintf( szWriteLine, "<Control Name=\"%s\" Type=\"%s\">", pchAttribName, pchAttribType );
   if ( (*lpWriteInfo->lpfnWrite)( lpTaskView, zXML_STARTENTITY, szWriteLine,
                                   0, lpEntityInstance->nLevel,
                                   lpWriteInfo->lpPtr ) == zCALL_ERROR )
   {
        ...
        zstrcmp( pchEntityName, "ListEntry" ) == 0 )
   {
            ...
            lpListEntryInstance = zGETPTR( lpListEntryInstance->hNextTwin ) )
      {
         ...
         if ( nFlag == 0 )
            zstrcpy( szWriteLine, "<ListEntry>" );
         else
            zstrcpy( szWriteLine, "<ListEntry zSelect1=\"Y\">" );
        ...
        zstrcmp( pchEntityName, "ComboValue" ) == 0 )
   {
            ...
            lpComboInstance = zGETPTR( lpComboInstance->hNextTwin ) )
      {
         ...
         if ( nFlag == 0 )
            zstrcpy( szWriteLine, "<ComboValue>" );
         else
            zstrcpy( szWriteLine, "<ComboValue zSelect1=\"Y\">" );
                 ...
                 LPENTITYINSTANCE lpEntityInstance )
{
   ...
   {
      zsprintf( lpWriteInfo->szBuffer, "</%s>",
                     ...
                     zPVOID        lpPtr )
{
   ...
   if ( lControl & zXML_ZEIDONINFO )
   {
      ...
      if ( lControl & zXML_INCREFLAGS )
         zsprintf( szBuffer, "<zOI zObjectName=\"%s\" zAppName=\"%s\" "
                   ...
                   const zCHAR **ppAtts )
{
   ...
   for ( k = 0; ppAtts[ k ] != 0; k += 2 )
   {
      ...
      zsprintf( szMsg, "(xml) Attrib %s = %s", ppAtts[ k ], ppAtts[ k + 1 ] );
      TraceLineS( szMsg, "" );
   }
}
                        ...
                        const zCHAR **ppAtts )
{
        ...
        zstrcmp( lpParent->szName, lpInfo->lpViewEntity->szName ) != 0 )
   {
      ...
      zsprintf( szMsg, "(xml) Parent mismatch (%s != %s) for entity: %s",
                        ...
                        const zCHAR **ppAtts )
{
   ...
   if ( lpViewAttrib == 0 )
   {
      ...
      zsprintf( szMsg, "(xml) Could not find Entity.Attribute: %s.%s",
                     ...
                     zLONG   nFlags )
{
   ...
   if ( zstrchr( pchFileName, cDirSep ) == 0 )
   {
      ...
      if ( lpApp )
         zstrcpy( szOpenFileName, lpApp->szObjectDir );
      else
      {
   ...
   zstrcat( szOpenFileName, pchFileName );
