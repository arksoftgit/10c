...
fnAddOD( LPVIEWOD lpViewOD, LPCONNECTION lpConnection )
{
   ...
   lpOD->hMem     = hMem;
   zstrcpy( lpOD->szOD_Name, lpViewOD->szName );
...
InitDatabase( zVIEW lpView )
{
   ...
   if ( nRC != zCURSOR_SET )
   {
      ...
      // Set up information shared by all ODBC tasks.
      {
         ...
         SysReadZeidonIni( -1, "[kzhsqloa]", "CommitLatency", szValue );
   ...
   if ( g_bHoldStmtHandles == 'X' )
   {
      ...
      SysReadZeidonIni( -1, "[kzhsqloa]", "HoldStmtHandles", szValue );
...
#if defined( DB2 )
   {
            ...
            lpOD = lpOD->lpNextOD )
      {
         if ( lpOD->bOpenConn )
         {
            ...
            zsprintf( szFuncName, "%s_CloseConnection", lpOD->szOD_Name );
                  ...
                  LPCONNECTION lpConnection )
{
   ...
   if ( g_ucDB2_Comma == 0 )
   {
      ...
      szValue[ 0 ] = 0;
      SysReadZeidonIni( -1, "[Zeidon]", "DB2COMMA", szValue );
...

{
         ...
         lpOD = lpOD->lpNextOD )
   {
      ...
      if ( lpOD->pfnCommit == 0 )
      {
         ...
         zsprintf( szProcName, "%s_Commit", lpOD->lpViewOD->szName );
...

{
         ...
         lpOD = lpOD->lpNextOD )
   {
      ...
      if ( lpOD->pfnRollback == 0 )
      {
         ...
         zsprintf( szProcName, "%s_Rollback", lpOD->lpViewOD->szName );
      ...
      int          Indicators )
{
   ...
   if ( lpViewAttrib->bKey == FALSE )
   {
      ...
      zsprintf( szMsg, "XOD '%s' needs to be saved with new JOIN code. "
...
#if defined( DB2 )
{
   ...
   if ( lpDB2_Data && lpDB2_Data->bUseStaticSQL )
   {
      ...
      if ( lpQualEntity == 0 )
      {
         ...
         zsprintf( szFuncName, "SQL_%s_%s_Select", lpOD->lpViewOD->szName,
                   lpViewEntity->szName );
      ...
      if ( lpQualEntity->bUniqueQualification )
      {
         ...
         // We have static binding so call the static function.
         zsprintf( szFuncName, "SQL_%s_%s_SelectUnique", lpOD->lpViewOD->szName,
                   lpViewEntity->szName );
         pfnFunc = SysGetProc( lpOD->hStaticLibrary, szFuncName );
   ...
   if ( g_bHoldStmtHandles )
   {
      ...
      if ( lpConnection->lpHandles[ k ] == 0 )
      {
         ...
         nLth = sizeof( EntityHandlesRecord ) * (lpViewOD->uEntities + 1);

         // We didn't find one so create space to store the handles.
         lpConnection->lpHandles[ k ]->lpViewOD = lpViewOD;
         lpConnection->lpHandles[ k ]->lpStmtHandles = malloc( nLth );
         zmemset( lpConnection->lpHandles[ k ]->lpStmtHandles, 0, nLth );
      }
         ...
         lpDataField = fnGetNextDataField( &LoopInfo ) )
   {
      ...
      // lpDataPtr points to the area that is to hold the data returned from
      // the database.
      ...
      *(SQLINTEGER *) lpPtr = 0;
      ...
      switch ( lpViewAttrib->cType )
      {
         ...
         case zTYPE_DATE:
            fSqlType = SQL_C_CHAR;
            ulLth = CONVERSION_AREA_LTH; // We'll store the date as a string
                                         // and then convert it.
            break;
         ...
         case zTYPE_DECIMAL:
            fSqlType = SQL_C_CHAR;
            ulLth = CONVERSION_AREA_LTH; // We'll retrieve the decimal as a
                                         // string and then convert it.
            break;
      ...
      nRC = SQLBindCol( lpConnection->hstmt, nColumnCnt, fSqlType,
                        (SQLPOINTER) lpDataPtr,
                        ulLth, (SQLINTEGER *) lpPtr );
   ...
   if ( g_bHoldStmtHandles )
   {
      zSHORT nLth = zstrlen( lpConnection->pchConnSqlCmd );

      lpStmtHandle->pchSqlCmd = malloc( nLth + 1 );
      zstrcpy( lpStmtHandle->pchSqlCmd, lpConnection->pchConnSqlCmd );
...
StartOfFetches:
   ...
   if ( nRC == zFETCH_OK )
   {
      ...
      do
      {
               ...
               lpDataField = fnGetNextDataField( &LoopInfo ) )
         {
            ...
            else
            {
                    ...
                    lpDataField->cFldType == zTYPE_TIME )
               {
                  ...
                  if ( lpDataField->cFldType == zTYPE_TIME )
                     ...
                     zstrcpy( szDatetime, "1900-01-01 " );
#endif

                  zstrcat( szDatetime, lpPtr );

#ifdef DB2
                  if ( lpDataField->cFldType == zTYPE_DATE )
                     // lpPtr points to "YYYY-MM-DD".  Postfix dummy time
                     zstrcat( szDatetime, " 00:00:00" );
#endif

                  // Convert datetime to YYYYMMDDHHmmSS.
                  if ( UfEditFormatDateTime( szDatetime,
                                             "YYYY-MM-DD HH:MI:SS" ) == 0 )
                     ; // Do nothing...szDatetime converted OK.
                  else
                  if ( UfEditFormatDateTime( szDatetime,
                                             "M/D/YYYY H:MI:SS AM" ) == 0 )
                     ; // Do nothing...szDatetime converted OK.

                  // For date and time formats we need to change unused part
                  // to 0's.
                  switch ( lpDataField->cFldType )
                  {
                     case zTYPE_DATE:
                        zstrcpy( &szDatetime[ 8 ], "000000000" );
                        break;
                  ...
                  zstrcpy( (zPCHAR) lpPtr, szDatetime );
               }
#ifdef DB2
               else
               if ( lpDataField->cFldType == zTYPE_DECIMAL )
               {
                  if ( g_ucDB2_Comma == 2)
                  {
                     /* If the Database uses decimal comma
                      *  instead of point ..., but the functions
                      * in decari use only '.', therefore we have
                      * to change the ',' into '.'
                      */
               ...
               if ( lpViewAttrib->bKey )
               {
                  // Store the key value for now.  When we reach the end
                  // of the keys we will use them to determine if the entity
               ...
               // At this point lpPtr points to the datbase value. Copy the value
               // into the attribute.
               switch ( lpDataField->cFldType )
               {
                  case zTYPE_DATETIME:
            ...
            switch ( lpViewAttrib->cType )
            {
               ...
               case zTYPE_STRING:
               {
                  ...
                  zsprintf( szTemp, "TempString%ld",
                            lpConnection->lStringCount++ );
                  StoreValueInRecord( lpView, lpLoadEntity, lpViewAttrib,
                                      (zPVOID) szTemp, 0 );
                  break;
               }
               ...
               case zTYPE_DECIMAL:
               {
                  ...
                  SysConvertDoubleToDecimal(
                           (double) (lpConnection->lDecimalCount++ * 1.1),
                           &dTemp );
                  StoreValueInRecord( lpView, lpLoadEntity, lpViewAttrib,
                                      (zPVOID) &dTemp, 0 );
                  break;
         ...
         // If CardMax of the entity is 1 then we'll pretend we didn't find
         // any more entities.
...
#endif
         // If we didn't fetch a row this time and if we only fetched 1 row
         // then set the return code to 0 and break loop.
   ...
   if ( nReturnCode == 0 && bRootLimitReached )
   {
      if ( lRootLimit > 1 )
         ...
         nReturnCode = 1; // Set RC to indicate we only got one root but that
...
fnRC_Text( zSHORT nRC )
{
   ...
   {
      case SQL_SUCCESS:
         zsprintf( szReturn, "%d (SQL_SUCCESS)", nRC );
         break;

      case SQL_SUCCESS_WITH_INFO:
         zsprintf( szReturn, "%d (SQL_SUCCESS_WITH_INFO)", nRC );
         break;

      case SQL_ERROR:
         zsprintf( szReturn, "%d (SQL_ERROR)", nRC );
         break;

      case SQL_INVALID_HANDLE:
         zsprintf( szReturn, "%d (SQL_INVALID_HANDLE)", nRC );
         break;

      case SQL_NEED_DATA:
         zsprintf( szReturn, "%d (SQL_NEED_DATA)", nRC );
         break;

#ifdef DB2
      case SQL_NO_DATA_FOUND:
         zsprintf( szReturn, "%d (SQL_NO_DATA_FOUND)", nRC );
         break;
#else
      #if (ODBCVER >= 0x0300)
         case SQL_NO_DATA:
            zsprintf( szReturn, "%d (SQL_NO_DATA)", nRC );
            break;
#endif

      case SQL_STILL_EXECUTING:
         zsprintf( szReturn, "%d (SQL_STILL_EXECUTING)", nRC );
         break;
#endif

      default:
         zsprintf( szReturn, "%d (unknown)", nRC );
         break;
   }
                ...
                LPCONNECTION lpConnection )
{
   ...
   if ( nRC != SQL_SUCCESS )
   {
      ...
      for ( k = 1; k <= sdNumRecs && k < 5; k++ )
      {
         ...
         zsprintf( szText2, "[ODBC] SQLState = %s, Err=", szSQLState );
                          ...
                          LPCONNECTION lpConnection )
{
        ...
        lpConnection->DBConnection[ k ].bValidConnection == FALSE )
   {
      ...
      if ( k >= lpConnection->nOpenDatabaseCount )
      {
         ...
         zstrcpy( lpConnection->DBConnection[ k ].szDatabaseName,
                  lpDBName );
      }

      SysGetDB_UserID( lpView, szUserID, szPassword );
      ...
      if ( !RC_SUCCESSFUL( nRC ) )
      {
         ...
         zsprintf( szText, "Error connecting to DB %s", lpDBName );
         MessageSend( lpConnection->lpAppView, "KZH0102", "ODBC DBH Error",
                      szText, zMSGQ_SYSTEM_ERROR, 0 );
                   ...
                   LPCONNECTION *lppConnection )
{
   ...
   // zltoa( (zLONG) hTask, szLogicalName );
      zsprintf( szLogicalName, "0x%08x:%08x", hTask, lProcessID );

   // sprintf( szLogicalName, "0x%08x", SysGetTaskFromView( lpView ) );
...
#ifdef DEBUG
         zstrcpy( ConnRec.szDebug1, "Conn BEGIN" );
         zstrcpy( ConnRec.szDebug2, "Conn END" );
#endif
      ...
      if ( nTraceLevel == 0 )
      {
         ...
         SysReadZeidonIni( -1, "[" DBHANDLER_NAME "]", "TraceLevel", szStr );
         if ( szStr[ 0 ] )
...
#if defined ( DB2 )
{
   ...
   nRC = fnOpenDatabaseConnection( lpView, lpViewOD,
                                   lpViewEntity, lpConnection );
   return( nRC );
   ...
   switch ( nCommand )
   {
      ...
      case zSQLCALLBACK_CONVERTSTRING:
      {
         ...
         if ( lpViewAttrib->cType == zTYPE_DATETIME )
         {
            ...
            // yyyymmddhhmmsss.
            zstrcpy( szDateTime1, (zPCHAR) pInfo );
            ...
            {
               case zTYPE_TIME:
                  UfFormatDateTime( szDateTime2, szDateTime1, "HH.MI.SS" );
                  break;

               case zTYPE_DATE:
                  UfFormatDateTime( szDateTime2, szDateTime1, "YYYY-mm-DD" );
         ...
         if ( lpViewAttrib->cType == zTYPE_DATETIME )
         {
            ...
            switch ( lpDataField->cFldType )
            {
               case zTYPE_TIME:
                  if ( lpObjectData->bStoreTimestampAsString )
                     UfFormatDateTime( szDateTime2, szDateTime1, "HHMISS" );
                  else
                  {
                     bAddEscapeSequence = TRUE;
                     zstrcpy( szEscapeCode, "t" );
                     UfFormatDateTime( szDateTime2, szDateTime1, "HH:MI:SS" );
                  }
               ...
               case zTYPE_DATE:
                  ...
                  if ( lpObjectData->bStoreTimestampAsString )
                     UfFormatDateTime( szDateTime2, szDateTime1, "YYYYmmDD" );
                  else
#endif
                  {
                     bAddEscapeSequence = TRUE;
                     zstrcpy( szEscapeCode, "d" );
                     UfFormatDateTime( szDateTime2, szDateTime1, "YYYY-mm-DD" );
                  }
               ...
               case zTYPE_DATETIME:
                  ...
                  else
                  {
                     ...
                     UfFormatDateTime( szDateTime2, szDateTime1, pchFormat );
            ...
            if ( bAddEscapeSequence )
            {
               ...
               *pchTarget++ = '{';
               ...
               zstrcpy( pchTarget, szEscapeCode );
...
// TraceLineS( "Password: ", szPassword );
   ...
   if ( !RC_SUCCESSFUL( nRC ) )
   {
      ...
      zsprintf( szText, "Error connecting to DB %s", pchDBName );
      MessageSend( lpConnection->lpAppView, "KZH0102", "ODBC DBH Error",
                   szText, zMSGQ_SYSTEM_ERROR, 0 );
