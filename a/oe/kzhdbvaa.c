...
typedef struct KeyStruct zFAR *LPKEY;
...
typedef struct EntityStruct zFAR *LPENTITY;
...
typedef struct Set_Struct zFAR *LPSET;
...
typedef struct PathStruct zFAR *LPPATH;
typedef struct PathStruct PathTable[ 5 ];
typedef PathTable zFAR *LPPATHTABLE;

typedef struct OverflowDeleteStruct OverflowDeleteRecord;
typedef struct OverflowDeleteStruct zFAR * LPOVERFLOW;
...
#define MAX_LOCKS 400
   ...
   LOCK_REQUEST zFAR  *lpLockRequest;
   LPVIEWENTITY       lpViewEntity;
   ...
   *((zPVOID zFAR *) lpWorkPtr) = 0;
   ...
   if ( zstrcmp( szDatabaseName, "ZMM" ) == 0 )
   {
      ...
      zstrcpy( szFullDBN, szPath );
      zstrcat( szFullDBN, "\\ZMMBK.LOK" );
   ...
   // Retrieve user id.
   szUserId[ 0 ] = 0;
   SysGetDB_UserID( szUserId, szPassword );
   if ( szUserId[ 0 ] == 0 )
   {
...
//    zstrcpy( szTemp, "Your Raima Database User ID specified in ZEIDON.INI "
//             "is greater than 7 characters.\n\nDatabase User ID = '" );
//    zstrcat( szTemp, szUserId );
...
//                          0, zBUTTONS_YESNO, 1, 0 );
   ...
   // SetNameForView( vDbhWork, "DG Temp View raimadm", 0, zLEVEL_SYSTEM );
      ...
      nIdx = 0;
      zstrncpy( szUniqueDatabaseID, szUserId, 7 );
      szUniqueDatabaseID[ 7 ] = 0;
      nLastChar = zstrlen( szUniqueDatabaseID );
      zstrcat( szUniqueDatabaseID, "1" );
      ...
      zstrcpy( szFullDBN, szPath );
      zstrcat( szFullDBN, "\\" );
      zstrcat( szFullDBN, szDatabaseName );

      // Now open database using fully qualified database name.
#if MultiUser
      zstrcpy( szOpenType, "s" );
#else
      zstrcpy( szOpenType, "o" );
#endif
...
//                  "Database name = ", szFullDBN );
   ...
   if ( lpConnection->nTraceLevel > 1 )
   {
      ...
      for ( k = 0; k < nLockRequestCount; k++ )
      {
         zstrcpy( szWork, "  Record/Set Name = " );
         if ( lpLockName[ k ] )
            zstrcat( szWork, lpLockName[ k ] );
         else
            zstrcat( szWork, "Overflow record" );
         zstrcat( szWork, "  ID = " );
...
//    OperatorSend( 0, "dgc", "Locking all sets for delete", 0 );
      ...
      for ( k = 20000; k < 20234 ; k++ )
      {
         ...
         zstrcat( sz, cLockType );
         zstrcat( sz, ".  Lock type = " );
           ...
           LPCONNECTION lpConnection )
{
   ...
   if ( nRC > 0 )
   {
      ...
      zstrcpy( szMsg, "Attempting to exclude an entity that has already "
               "been excluded." );
      zstrcat( szMsg, "\n\n   Parent entity name = " );
      zstrcat( szMsg, lpViewEntity->hParent->szName );
      zstrcat( szMsg, "\n   Child entity name = " );
      zstrcat( szMsg, lpViewEntity->szName );
      zstrcat( szMsg, ", Record name = " );
      zstrcat( szMsg, lpDataRecord->szRecordName );
      if ( lpRelRecord->cOwnerMember == zDBH_MANY_TO_MANY )
      {
         zstrcat( szMsg, "\n   Set name1 = " );
         zstrcat( szMsg, lpRelRecord->szSetName1 );
         zstrcat( szMsg, "\n   Intersection record = " );
         zstrcat( szMsg, lpRelRecord->szRecordName );
         zstrcat( szMsg, "\n   Set name2 = " );
         zstrcat( szMsg, lpRelRecord->szSetName2 );
      }
      else
      {
         zstrcat( szMsg, "\n   Set name = " );
                        ...
                        zPCHAR szWhereClause )
{
   ...
   zstrcpy( szWhereClause, " WHERE ( " );
   nRC = SetCursorFirstEntity( lpView, "DB_Key", "DB_Entity" );

   if ( lpRelRecord )
   {
      zstrcpy( szWhereClause, " ( db_addr( " );
      copy parent record name.
      zstrcpy( szWhereClause, " ) = " );
      copy parent record dba.
      zstrcpy( szWhereClause, " ) AND " );
   }  // if ( lpRelRecord )...
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      zstrcat( szWhereClause, szRecordName );
      zstrcat( szWhereClause, "." );
      zstrcat( szWhereClause, lpDataField->hFldName );
      zstrcat( szWhereClause, " = " );
      ...
      switch ( lpViewAttrib->cType )
      {
         ...
         case zTYPE_STRING:
            zstrcat( szWhereClause, "\"" );
            zstrcat( szWhereClause, bKeyValue );
            zstrcat( szWhereClause, "\"" );
            break;
      }  // switch (...)...
                       ...
                       zPULONG  pulLth )
{
   ...
   zstrcpy( szWork, "Internal error--converting illegal types. From-type = " );
   zstrncat( szWork, (zPCHAR) &cInType, 1 );
   zstrcat( szWork, "; To-type = " );
                               ...
                               LPCONNECTION lpConnection )
{
   ...
   while ( lpDataField )
   {
      ...
      {
         OverflowIndicatorRecord zFAR *Overflow;

         // Get length of overflow data.
         Overflow = (OverflowIndicatorRecord zFAR *) pDataPtr;
         uLth = Overflow->ulLth;
                      ...
                      LPCONNECTION lpConnection )
{
   ...
   long    hOverflowMem;
   OverflowIndicatorRecord zFAR *OverflowInd;
   OverflowRecord zFAR *OverflowRec;
   zULONG  lth;
   zSHORT  nRC;
...
DeleteOverflowRecords( LPCONNECTION lpConnection, zBOOL bDelete )
{
   ...
   OverflowRecord zFAR *lpOverflowRec;
                   ...
                   LPCONNECTION lpConnection )
{
   ...
   if ( lpRelRecord == 0 )
   {
      ...
      zsprintf( szSelect, "SELECT 1 FROM %s %s;", pchRecordName,
               szWhereClause );
   ...
   if ( lpRelRecord->cOwnerMember == zDBH_PARENT_IS_OWNER )
   {
      ...
      zsprintf( szSelect, "SELECT 1 FROM %s", lpRelRecord->szSetName1 );
      DBA_ToStr( &ParentRecordDBA, szWork2 );
      zsprintf( szWork1, " ( db_addr( %s ) = %s ) AND ", pchParentRecordName,
               szWork2 );
      hey!!! there ain't no vDbhWork
      BuildWhereClause( vDbhWork, szWhereClause, pchRecordName, szWork1 );
      zstrcat( szSelect, szWhereClause );
      zstrcat( szSelect, ";" );
                            ...
                            LPCONNECTION lpConnection )
{
   ...
   OverflowInd = (OverflowIndicatorRecord zFAR *) lpSrc;
