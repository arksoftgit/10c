...
typedef SqlStatementRecord zFAR *LPSQLSTATEMENT;
...
#define fnSqlAddStringToSql( a, b )   (a)->pchSqlCmdEnd = zstrcatp( (a)->pchSqlCmdEnd, (b) )
#define fnSqlAddStringToWhere( a, b ) (a)->pchWhereEnd = zstrcatp( (a)->pchWhereEnd, (b) )
#define fnSqlAddStringToSuffix( a, b ) (a)->pchSuffixEnd = zstrcatp( (a)->pchSuffixEnd, (b) )
...
fnSqlAssembleSqlStatement( LPSQLSTATEMENT lpSql )
{
   if ( lpSql->pchColList2[ 0 ] )
      fnSqlAddStringToSql( lpSql, lpSql->pchColList2 );

   if ( lpSql->pchFrom[ 0 ] )
   {
      fnSqlAddStringToSql( lpSql, " FROM " );
      fnSqlAddStringToSql( lpSql, lpSql->pchFrom );
   }
   ...
   if ( lpSql->pchWhere[ 0 ] )
   {
      ...
      {
         fnSqlAddStringToSql( lpSql, " WHERE ( " );
         fnSqlAddStringToSql( lpSql, lpSql->pchWhere );
         fnSqlAddStringToSql( lpSql, " )" );
      }
      else
      {
         fnSqlAddStringToSql( lpSql, " WHERE " );
   ...
   if ( lpSql->pchSuffix[ 0 ] )
      fnSqlAddStringToSql( lpSql, lpSql->pchSuffix );
...
fnAddAliasName( LPSQLSTATEMENT lpSql, int nTableNbr )
{
   ...
   // If we are using outer joins then we need to add "AS" to the join
   // statement.
   if ( lpSql->bUseLeftJoin )
      lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, " AS" );
   ...
   zltoa( (zLONG) nTableNbr, &szAlias[ 2 ] );

   lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, szAlias );

   // Copy alias name to table info (skipping the leading space ).
   zstrcpy( lpSql->TableNameInfo[ nTableNbr ].szAliasName, &szAlias[ 1 ] );
}
                  ...
                  zLONG          lControl )
{
        ...
        zstrcmp( lpRelRecord->szRecordName, pchTableName ) != 0 )
   {
      ...
      {
         if ( lpSql->bUseLeftJoin )
            lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, " LEFT JOIN " );
         else
            lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, ", " );
      }
           ...
           lpViewEntity != lpSql->lpLoadViewEntity )
      {
         ...
         lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, " ON " );
               ...
               lpRelField = zGETPTR( lpRelField->hNextRelField ) )
         {
            ...
            if ( bFirstTime )
               ...
               lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, " AND " );
   ...
   lpSql->pchFromEnd = zstrcatp( lpSql->pchFromEnd, pchTableName );
...
fnSqlUnformatDateTimeString( zPCHAR pchTarget, zPCHAR pchSource )
{
   ...
   if ( UfEditFormatDateTime( szStr, "YYYYMMDD" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "YYYY-M-D H:MI:SSAM" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "D-Mmm-YYYY H:MI:SSAM" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "D-Mmm-YYYY" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "YYYY-M-D H:MI:SS" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "YYYY-M-D H:MIAM" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "YYYY-M-D H:MI" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "YYYY-M-D" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "H:MI:SSAM" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "H:MI:SS" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "H:MIAM" ) == 0 )
      nRC = 1;
   else
   if ( UfEditFormatDateTime( szStr, "H:MI" ) == 0 )
      nRC = 1;
                               ...
                               LPBOUNDLIST  lpBoundList )
{
   ...
   switch ( lpViewAttrib->cType )
   {
      ...
      case zTYPE_DATETIME:
         ...
         {
            case zTYPE_TIME:
               UfFormatDateTime( szDateTime2, szDateTime1,
                                 "HH.MI.SS.999" );
               break;
         ...
         zsprintf( pchTgt, "'%s'", szDateTime2 );
         break;

      default:
         // For all other datatypes, just copy it to pchTgt.
         zstrcpy( pchTgt, pchSrc );
         break;
                      ...
                      zSHORT         nControl )
{
         ...
         lpDataField = zGETPTR( lpDataField->hNextDataField ) )
   {
      ...
      if ( lpSql->nCommandType & zSELECT_CMD )
      {
         ...
         {
            case 'A':
               zstrcat( szColumnName, "AVG( " );
               break;

            case 'C':
               zstrcat( szColumnName, "COUNT( " );
               break;

            case 'N':
               zstrcat( szColumnName, "MIN( " );
               break;

            case 'S':
               zstrcat( szColumnName, "SUM( " );
               break;

            case 'X':
               zstrcat( szColumnName, "MAX( " );
               break;
         }
      }
      ...
      else
      {
         ...
         zstrcat( szColumnName, lpDataField->szFldName );
      }
                          ...
                          (lpSql->nCommandType & zINSERT_CMD), 0 ) )
      {
         ...
         lpSql->pchColList2End = zstrcatp( lpSql->pchColList2End, ", " );
         lpSql->pchColList2End = zstrcatp( lpSql->pchColList2End, szColumnName );
      }
      else
      {
         ...
         fnSqlAddStringToSql( lpSql, szColumnName );
      }
                    ...
                    LPSQLSTATEMENT lpSql )
{
   ...
   fnSqlAddStringToSql( lpSql, " INTO " );
         ...
         lpDataField = zGETPTR( lpDataField->hNextDataField ) )
   {
      ...
      else
      {
         ...
         zstrcat( szColumnName, lpViewAttrib->szName );
      }
                 ...
                 zSHORT         nQual )
{
   ...
   if ( nQual & zFOREIGN_KEY )
   {
      ...
      fnSqlAddStringToWhere( lpSql, " ( " );
      fnSqlAddStringToWhere( lpSql, lpRelDataField->szFldName );
      if ( SqlAttributeIsNull( lpView, lpSrcDataField, lpBoundList ) )
      {
         ...
         fnSqlAddStringToWhere( lpSql, " IS " );
         fnSqlGetNullString( pchValue, lpSrcDataField, lpBoundList );
         fnSqlAddStringToWhere( lpSql, pchValue );
      }
      else
      {
         fnSqlAddStringToWhere( lpSql, " = " );
         ...
         lpDataField = zGETPTR( lpDataField->hNextDataField ) )
   {
      ...
      if ( fnSqlWhereIsEmpty( lpSql ) )
         ...
         fnSqlAddStringToWhere( lpSql, " AND " );

      if ( nQual == zFULLY_QUALIFIED )
      {
         fnSqlAddStringToWhere( lpSql, fnTableName( lpSql,
                                                    lpDataRecord->szRecordName,
                                                    lpViewEntity ) );
         fnSqlAddStringToWhere( lpSql, "." );
      }

      fnSqlAddStringToWhere( lpSql, lpDataField->szFldName );
      if ( SqlAttributeIsNull( lpView, lpDataField, lpBoundList ) )
      {
         // If the attribute is null, then write 'null'-value into the
         // statement 29.10.1997, RG
   ...
   if ( bKeyFound == FALSE )
   {
      ...
      zstrcpy( szMsg, "SQL Error creating WHERE clause -- no key attribute "
               "found in object definition.\nObject name = " );
      zstrcat( szMsg, lpViewOD->szName );
      zstrcat( szMsg, "\nEntity name = " );
      zstrcat( szMsg, lpViewEntity->szName );
                ...
                LPBOUNDLIST  lpBoundList )
{
   ...
   fnSqlAddStringToSql( lpSql, "INSERT INTO " );
   fnSqlAddStringToSql( lpSql, lpDataRecord->szRecordName );
   fnSqlAddStringToSql( lpSql, " ( " );
   ...
   fnSqlAddStringToSql( lpSql, " ) VALUES ( " );
                         ...
                         LPBOUNDLIST  lpBoundList )
{
   ...
   fnSqlAddStringToSql( lpSql, "INSERT INTO " );
   fnSqlAddStringToSql( lpSql, lpRelRecord->szRecordName );
   fnSqlAddStringToSql( lpSql, " ( " );
   ...
   while ( lpRelField )
   {
      fnSqlAddStringToSql( lpSql, lpRelField->szFldName );
   ...
   if ( lpViewEntity->bAutoSeq )
   {
      ...
      while ( lpDataField )
      {
         ...
         {
            fnSqlAddStringToSql( lpSql, ", " );
            fnSqlAddStringToSql( lpSql, lpDataField->szFldName );
                ...
                LPBOUNDLIST  lpBoundList )
{
   ...
   fnSqlAddStringToSql( lpSql, "UPDATE " );
   fnSqlAddStringToSql( lpSql, lpDataRecord->szRecordName );
                ...
                LPBOUNDLIST  lpBoundList )
{
   ...
   fnSqlAddStringToSql( lpSql, "DELETE FROM " );
   fnSqlAddStringToSql( lpSql, lpDataRecord->szRecordName );
   fnSqlAddStringToSql( lpSql, " " );
                         ...
                         LPBOUNDLIST  lpBoundList )
{
   ...
   while ( lpRelField )
   {
      ...
      fnSqlAddStringToWhere( lpSql, lpRelField->szFldName );
      fnSqlAddStringToWhere( lpSql, " = " );
         ...
         else
         {
            ...
            fnSqlAddStringToWhere( lpSql,
                                   fnTableName( lpSql,
                                                ...
                                                lpSrcViewEntity ) );
            fnSqlAddStringToWhere( lpSql, "." );
            fnSqlAddStringToWhere( lpSql, lpSrcDataField->szFldName );
         }
      } // if ( lpRelRecord->cOwnerMember == zDBH_MANY_TO_MANY )
      else
                     ...
                     LPSQLSTATEMENT lpParentSql )
{
   ...
   fnSqlAddStringToSql( lpSql, lpQualAttrib->szOper );
   fnSqlAddStringToSql( lpSql, " ( SELECT " );
   ...
   {
      fnSqlAddStringToWhere( lpParentSql, lpSql->pchSqlCmd );
      fnSqlAddStringToWhere( lpParentSql, " FROM " );
      fnSqlAddStringToWhere( lpParentSql, lpSql->pchFrom );
      fnSqlAddStringToWhere( lpParentSql, " WHERE " );
      if ( lpSql->nComponentCount > 1 )
         fnSqlAddStringToWhere( lpParentSql, " ( " );
      fnSqlAddStringToWhere( lpParentSql, lpSql->pchWhere );
      if ( lpSql->nComponentCount > 1 )
         fnSqlAddStringToWhere( lpParentSql, " ) " );
      fnSqlAddStringToWhere( lpParentSql, " )" );
   }
                     ...
                     LPSQLSTATEMENT lpSql )
{
         ...
         lpQualAttrib = lpQualAttrib->lpNextQualAttrib )
   {
      if ( lpQualAttrib->lpViewEntity == 0 )
      {
         ...
         fnSqlAddStringToWhere( lpSql, " " );
         fnSqlAddStringToWhere( lpSql, lpQualAttrib->szOper );
         fnSqlAddStringToWhere( lpSql, " " );
      ...
      if ( lpQualAttrib->lpKeyList )
      {
         ...
         fnSqlAddStringToWhere( lpSql,
                                fnTableName( lpSql,
                                    ...
                                    lpQualAttrib->lpViewEntity ) );
         fnSqlAddStringToWhere( lpSql, "." );
         fnSqlAddStringToWhere( lpSql, lpQualAttrib->lpDataField->szFldName );


         zsprintf( szCmd, " IN ( SELECT INTVALUE FROM ZEIDONKEYLIST"
                          " WHERE TASKID = %ld ) ", SysGetTaskFromView( lpView ) );
         fnSqlAddStringToWhere( lpSql, szCmd );
      }
      else
      {
      // zSHORT bNotEqual;
      // zSHORT nLoop;
         ...
         if ( zstrncmpi( lpQualAttrib->szOper, "UPPER", 5 ) == 0 )
         {
            ...
            fnSqlAddStringToWhere( lpSql, szUpper );
            fnSqlAddStringToWhere( lpSql, "( " );
         }
         else
         {
...
#endif
         {
            ...
            {
               fnSqlAddStringToWhere( lpSql, " IS NULL OR " );
                 ...
                 pchQualOper[ 0 ] != '>' )
            {
               ...
               if ( bNull )
                  fnSqlAddStringToWhere( lpSql, " IS NOT " );
               else
                  fnSqlAddStringToWhere( lpSql, "<>" );
            }
            else
            {
               if ( bNull )
                  ...
                  fnSqlAddStringToWhere( lpSql, pchQualOper );
            }

            if ( bNull )
            {
               fnSqlGetNullString( szStr, lpQualAttrib->lpDataField, lpBoundList );
               fnSqlAddStringToWhere( lpSql, szStr );
            }
            else
            if ( lpQualAttrib->szValue )
            {
               ...
               if ( lpQualAttrib->lpDataField->cFldType == zTYPE_FIXEDCHAR &&
                    lpQualAttrib->bLike )
               {
                  zSHORT nLth = zstrlen( szStr ) - 1;

                  while ( nLth >= 0 && szStr[ nLth ] == ' ' )
                     szStr[ nLth-- ] = 0;
               }
                   ...
                   LPBOUNDLIST   lpBoundList )
{
   ...
   while ( *pchSrc )
   {
      ...
      nRC = GetStringFromAttribute( szTempStr, lpView, szEntityName,
                                    szAttributeName );
      if ( nRC < -2 )
         return( zCALL_ERROR );
      else
      if ( nRC == -2 )
      {
         zsprintf( szTempStr, "Cursor is NULL for attribute '@%s.%s'",
      ...
      fnSqlConvertStringToSqlString ( lpDataField, szTempStr, pchTgt,
                                      lpBoundList );
                ...
                LPBOUNDLIST  lpBoundList )
{
   ...
   // We have to add an extra paren for each child that will be joined with
   // the root.
   ...
   nRC = fnSqlBuildColumnList( lpViewEntity, lpSql,
                               zFULLY_QUALIFIED | zNO_HIDDEN );
                                 ...
                                 LPBOUNDLIST  lpBoundList )
{
   ...
   for (; lpDataField; lpDataField = zGETPTR( lpDataField->hNextDataField ) )
   {
      ...
      {
         uLth    = zstrlen( lpDataField->szFldName );
         szToken = pchColumnList;
         while ( szToken[ 0 ] &&
                 zstrncmpi( szToken,
                            lpDataField->szFldName, uLth ) != 0 )
         {
   ...
   do
   {
      ...
      if ( lpOrderViewEntity->bAttrOrder )
      {
         ...
         if ( chMaxAttrNbr > 0 )
         {
            ...
            if ( nOrderBy == 0 )
               fnSqlAddStringToSuffix( lpSql, " ORDER BY " );
      ...
      for ( k = 0; k < nKey; k++ )
      {
         ...
         fnSqlAddStringToSuffix( lpSql,
                                 fnTableName( lpSql,
                                    ...
                                    lpOrderViewEntity ) );
         fnSqlAddStringToSuffix( lpSql, "." );
         fnSqlAddStringToSuffix( lpSql, apDataField[ k ]->szFldName );
                        ...
                        zSHORT       nIndent )
{
   ...
   while ( lpQualAttrib )
   {
      ...
      {
         zstrcat( szStr, "Source Entity name: " );
      ...
      {
         LPVIEWATTRIB lpViewAttrib =
                     zGETPTR( lpQualAttrib->lpSourceDataField->hViewAttrib );

         zstrcat( szStr, "Source Attrib name: " );
                     ...
                     zPCHAR szQualEntity )
{
   ...
   lpViewOD = (LPVIEWOD) MiGetViewEntityForView( lpView, 0 );
   zsprintf( szText, "%s loading qualification object while activating LOD "
             "%s: %s", bWarning ? "Warning" : "Error", lpViewOD->szName,
             pchMsg );

   MessageSend( lpView, "KZH0201", bWarning ? "Qualification Object Warning" :
                                              "Qualification Object Error",
             ...
             zPCHAR       pchOper )
{
   ...
   if ( lpQualAttrib->szOper )
      zstrcpy( lpQualAttrib->szOper, pchOper );
}
                    ...
                    zPCHAR       pchValue )
{
   ...
   *lpFirstQualAttrib = lpQualAttrib;
   ...
   if ( pchValue && *pchValue )
   {
      ...
      zstrcpy( lpQualAttrib->szValue, pchValue );
   }
         ...
         nRC = SetCursorPrevEntity( lpQualView, szlQualAttrib, "" ) )
   {
      ...
      *lpFirstQualAttrib = lpQualAttrib;
      ...
      if ( *pchAttEntityName )
      {
         ...
         if ( lpViewEntity == 0 )
         {
            zsprintf( szStr, "Entity '%s' was specified in the Qualification "
                      "object but does not exist in LOD '%s'",
                      pchAttEntityName, lpViewOD->szName );
            fnSqlDisplayQualMsg( lpView, lpQualView, szStr,
                                 zERROR, szlQualAttrib );
            return( zCALL_ERROR );
         }
              ...
              lpViewEntity->hFirstDataRecord == 0 )
         {
            ...
            zsprintf( szMsg, "Entity '%s' in Object '%s' is a work or derived entity "
         ...
         // This entity had better be a child of the entity we are
         // qualifying.
               ...
               lpSearchEntity = zGETPTR( lpSearchEntity->hParent ) )
         {
            if ( lpSearchEntity->hParent == 0 )
            {
               ...
               // we haven't found a match with lpQualEntity->lpViewEntity.
               // Illegal qual.
               zCHAR szMsg[ 500 ];

               zsprintf( szMsg, "You cannot qualify entity '%s' using entity "
      ...
      if ( CheckExistenceOfEntity( lpQualView, "KeyList" ) >= zCURSOR_SET )
      {
         ...
         {
            GetIntegerFromAttribute( &pl[ k++ ], lpQualView, "KeyList",
                                     "IntegerValue" );
         }
      ...
      GetAddrForAttribute( &pchSrcViewName, lpQualView, szlQualAttrib,
                           "SourceViewName" );
      if ( *pchSrcViewName )
      {
         GetViewByName( &lpSourceView, pchSrcViewName, lpQualView,
                        zLEVEL_TASK );
         if ( lpSourceView )
         {
      ...
      if ( *pchSrcEntityName )
      {
         ...
         if ( lpQualAttrib->lpSourceView )
            lpSourceViewEntity =
                        MiGetViewEntityForView( lpQualAttrib->lpSourceView,
                                                pchSrcEntityName );
         else
            lpSourceViewEntity =
                        MiGetViewEntityForView( lpView, pchSrcEntityName );
         ...
         if ( lpSourceViewEntity->nHierNbr >
                                       lpQualEntity->lpViewEntity->nHierNbr )
         {
            fnSqlDisplayQualMsg( lpView, lpQualView,
         ...
         nRC = SetCursorNextEntity( lpQualView, szlEntitySpec, 0 ) )
   {
      ...
      *lpFirstQualEntity = lpQualEntity;
      ...
      if ( lpViewEntity == 0 )
      {
         ...
         zsprintf( szMsg, "Entity '%s' does not exist in Object '%s'",
      ...
      if ( lpQualEntity->pszOpenSQL )
      {
         ...
         while ( *pch )
         {
            ...
            if ( lpDataField == 0 )
            {
               ...
               zsprintf( szMsg, "Attribute name '%s' specified in "
                ...
                zPVOID  pInfo )
{
   ...
   zPCHAR pchPtr;
   zPCHAR pchIndentStr;
   void (POPERATION pfnWrite)( zPCHAR, zPVOID ) =
                               (void (POPERATION )( zPCHAR, zPVOID )) pfn;

   // Terminate ppchLine string.
   **ppchLine = 0;

   pchIndentStr = SysMalloc( zstrlen( pchBuffer ) + 1000 );

   // If the first character is a right paren ( ')' ) then un-indent two
   // spaces for each right paren.
   pchPtr = pchBuffer;
   nParenIndent = 0;
   while ( *pchPtr )
   {
   ...
   *pchPtr = 0;
   ...
   zstrcat( pchIndentStr, pchBuffer );
   (*pfnWrite)( pchIndentStr, pInfo );

   if ( *pnErrorIdx )
   {
      zstrcpy( pchIndentStr, "ERR>  " );
      pchPtr = pchIndentStr;
...
/////////////////////////////////////////////////////////////////////////////
static void
fnSqlDisplayCommand( zPCHAR zFAR *pchSqlCmdStr,
                     zPCHAR pchErrorChar,
                     zSHORT pnIndentLth,
                     ...
                     zPVOID pInfo )
{
   ...
   if ( pchInitString )
   {
      zstrcpy( pchBuffer, pchInitString );
      mAdvanceToNull( pchLine );
      pnIndentIncrement = zstrlen( pchInitString ) + 1;
   }
   else
      pnIndentIncrement = 0;
   ...
   if ( pchLine > pchBuffer )
      fnSqlPrintLine( pchBuffer, &pchLine, &pnIndentLth,
                      &pnIndentIncrement, &pnErrorIdx, pfnWrite, pInfo );
                     ...
                     zPVOID pInfo )
{
   ...
   hMemBuffer = SysAllocMemory( (zCOREMEM) &pchBuffer,
                                zstrlen( pchSqlCmd ) + 1,
                                0, zCOREMEM_ALLOC, 0 );
   if ( pchBuffer == 0 )
      return;

   zstrcpy( pchBuffer, pchSqlCmd );
