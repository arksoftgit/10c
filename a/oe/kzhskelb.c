...
fnSqlAssembleSqlStatement( LPSQLSTATEMENT lpSql )
{
   if ( lpSql->szFrom[ 0 ] )
   {
      ...
      fnSqlAddStringToSql( lpSql, lpSql->szFrom );
   }
   ...
   if ( lpSql->szWhere[ 0 ] )
   {
      if ( lpSql->nComponentCount > 1 )
      {
         ...
         fnSqlAddStringToSql( lpSql, lpSql->szWhere );
                             ...
                             zPCHAR       pchTgt )
{
   ...
   switch ( lpDataField->cFldType )
   {
      ...
      case zTYPE_DATETIME:
         ...
         // Convert DateTime string to a formatted string that proprietary
         // SQL understands.
         switch ( cDBType )
         {
            case zSQL_SQLBASE:
                    ...
                    LPDATAFIELD    lpDataField )
{
   ...
   switch ( lpDataField->cFldType )
   {
      ...
      case zTYPE_STRING:
         ...
         GetValueFromRecord( lpView, lpViewEntity, lpViewAttrib, &pchStr,
                             &chType, &uLth );
   ...
   switch ( cDBType )
   {
      ...
      case zSQL_SQLSERVER:
         if ( lpDataField->cFldType == zTYPE_STRING )
            zstrcpy( pchValue, "''" );       // two single quotes.
         else
            zstrcpy( pchValue, "NULL" );
         break;
   }
   ...
   for ( ; lpDataField; lpDataField = lpDataField->lpNextDataField )
   {
      ...
      if ( nControl & zFULLY_QUALIFIED )
      {
              ...
              lpRelRecord->cOwnerMember == zDBH_MANY_TO_MANY )
         {
            ...
            zstrcpy( szColumnName, lpRelRecord->lpRecordName );
         }
         else
            zstrcpy( szColumnName, lpDataRecord->lpRecordName );
      ...
      if ( lpDataField->cFldType == zTYPE_DATETIME )
      {
         switch ( cDBType )
         {
            ...
            case zSQL_SQLBASE:
               zstrcat( szColumnName, lpDataField->lpFldName );
               break;
            ...
            case zSQL_SQLSERVER:
               ...
               else
               {
                  ...
                  zstrcpy( sz, "CONVERT( VARCHAR, " );
                  if ( szColumnName[ 0 ] )
                     zstrcat( sz, szColumnName );

                  zstrcat( sz, lpDataField->lpFldName );
                  zstrcat( sz, ", 109 )" );
                  zstrcpy( szColumnName, sz );
               }
               ...
               zSHORT         nQual )
{
   ...
   while ( lpDataField )
   {
      ...
      if ( lpViewAttrib->bKey )
      {
         ...
         {
            fnSqlAddStringToWhere( lpSql, lpDataRecord->lpRecordName );
            fnSqlAddStringToWhere( lpSql, "." );
         }

         fnSqlAddStringToWhere( lpSql, lpDataField->lpFldName );
         if ( SqlAttributeIsNull( cDBType, lpView, lpDataField ) )
   ...
   else
   {
      ...
      zstrcat( szMsg, lpViewOD->lpName );
      zstrcat( szMsg, "\nEntity name = " );
      zstrcat( szMsg, lpViewEntity->lpName );
                ...
                zPCHAR       pchSqlCmd )
{
   ...
   fnSqlAddStringToSql( lpSql, lpDataRecord->lpRecordName );
   fnSqlAddStringToSql( lpSql, " ( " );
                         ...
                         zPCHAR       pchSqlCmd )
{
   ...
   fnSqlAddStringToSql( lpSql, "INSERT INTO " );
   fnSqlAddStringToSql( lpSql, lpRelRecord->lpRecordName );
   fnSqlAddStringToSql( lpSql, " ( " );
   ...
   while ( lpRelField )
   {
      fnSqlAddStringToSql( lpSql, lpRelField->lpFldName );
   ...
   if ( lpViewEntity->bAutoSeq )
   {
      ...
      while ( lpDataField )
      {
         ...
         {
            fnSqlAddStringToSql( lpSql, ", " );
            fnSqlAddStringToSql( lpSql, lpDataField->lpFldName );
   ...
   while ( lpRelField )
   {
      SqlGetAttributeValue( cDBType, lpView, lpRelField->lpSrcDataField,
                            lpSql );
                ...
                zPCHAR       pchSqlCmd )
{
      ...
      if ( lpViewAttrib->bPersist && lpViewAttrib->bKey == FALSE )
      {
         ...
         if ( lpViewAttrib->cType == zTYPE_BLOB || lpViewAttrib->ulLth > 254 )
         {
            ...
            switch ( cDBType )
            {
               ...
               case zSQL_SQLSERVER:
                  fnSqlAddStringToSql( lpSql, "NULL" );
                  break;
   ...
   fnSqlAddStringToSql( lpSql, " " );
   nRC = SqlBuildWhere( cDBType, lpView, lpViewOD, lpViewEntity, lpSql,
                        zFULLY_QUALIFIED );
   if ( nRC == zCALL_ERROR )
      return( zCALL_ERROR );
                         ...
                         zPCHAR       pchSqlCmd )
{
   ...
   while ( lpRelField )
   {
      ...
      fnSqlAddStringToWhere( lpSql, lpRelField->lpFldName );
      fnSqlAddStringToWhere( lpSql, " = " );
...
/////////////////////////////////////////////////////////////////////////////
 ...
 ??This routine does not appear to be used.
   ...
   RETURNS: TRUE  - if lpDataField is part of an entity that is in the
                    parent path of lpViewEntity.
                     ...
                     zSHORT         nControl )
{
   ...
   while ( lpRelField )
   {
      ...
      {
         fnSqlAddStringToWhere( lpSql, lpRelRecord->lpRecordName );
         fnSqlAddStringToWhere( lpSql, "." );
         fnSqlAddStringToWhere( lpSql, lpRelField->lpFldName );
         fnSqlAddStringToWhere( lpSql, " = " );
         ...
         if ( lpRelField->lpSrcDataField->lpViewAttrib->lpViewEntity ==
              lpViewEntity )
         {
            fnSqlAddStringToWhere( lpSql, lpDataRecord->lpRecordName );
            fnSqlAddStringToWhere( lpSql, "." );
            fnSqlAddStringToWhere( lpSql,
                                   lpRelField->lpSrcDataField->lpFldName );
         }
         else
         if ( nControl == zROOT_ENTITY )
         {
            ...
            nRC = SqlGetAttributeValue( cDBType, lpView,
                                        lpRelField->lpSrcDataField, lpSql );
         ...
         else
         {
            ...
            // ???
            // If we get here, then there is a sub-qualification on the
            // entity that we are loading.

            lpTempRecord = lpRelField->lpSrcDataField->lpViewAttrib->
                            lpViewEntity->lpFirstDataRecord;

            fnSqlAddStringToWhere( lpSql, lpTempRecord->lpRecordName );
            fnSqlAddStringToWhere( lpSql, "." );
            fnSqlAddStringToWhere( lpSql,
                                   lpRelField->lpSrcDataField->lpFldName );
         }
      ...
      if ( lpRelRecord->cOwnerMember == zDBH_PARENT_IS_SOURCE )
      {
         ...
         fnSqlAddStringToWhere( lpSql, lpRelDataRecord->lpRecordName );
         fnSqlAddStringToWhere( lpSql, "." );
         fnSqlAddStringToWhere( lpSql,
                                lpRelField->lpRelDataField->lpFldName );
         ...
         if ( nControl == zSUB_ENTITY )  // What is subentity????
         {
            ...
            // If we get here, then there is a sub-qualification on the
            // entity that we are loading.
            lpForeignDataRecord = lpRelField->lpSrcDataField->lpViewAttrib->
                                   lpViewEntity->lpFirstDataRecord;
            fnSqlAddStringToWhere( lpSql, lpForeignDataRecord->lpRecordName );
            fnSqlAddStringToWhere( lpSql, "." );
            fnSqlAddStringToWhere( lpSql,
                                   lpRelField->lpSrcDataField->lpFldName );
      ...
      if ( lpRelRecord->cOwnerMember == zDBH_CHILD_IS_SOURCE )
      {
         ...
         lpSrcViewEntity = lpRelField->lpSrcDataField->lpViewAttrib->
                              lpViewEntity;
         lpSrcDataRecord = lpSrcViewEntity->lpFirstDataRecord;

         fnSqlAddStringToWhere( lpSql, lpSrcDataRecord->lpRecordName );
         fnSqlAddStringToWhere( lpSql, "." );
         fnSqlAddStringToWhere( lpSql,
                                lpRelField->lpSrcDataField->lpFldName );
         ...
         if ( nControl == zSUB_ENTITY )
         {
            ...
            lpForeignDataRecord = lpRelField->lpRelDataField->lpViewAttrib->
                                   lpViewEntity->lpFirstDataRecord;
            fnSqlAddStringToWhere( lpSql,
                                   lpForeignDataRecord->lpRecordName );
            fnSqlAddStringToWhere( lpSql, "." );
            fnSqlAddStringToWhere( lpSql,
                                   lpRelField->lpRelDataField->lpFldName );
         }
         ...
         {
            nRC = SqlGetAttributeValue( cDBType, lpView,
                                        lpRelField->lpRelDataField, lpSql );
                     ...
                     LPSQLSTATEMENT lpSql )
{
   ...
   SqlBuildForeignKeys( cDBType, lpView, lpViewEntity, lpSql,
                        zSUB_ENTITY );

   // Now make sure that the parent of lpViewEntity is included in the
                   ...
                   LPSQLSTATEMENT lpParentSql )
{
   ...
   // Add the QualEntity table to szFromTable so that SqlAddTableToSelect
   // doesn't try to add it later.
   fnSqlAddTableToFrom( lpSql, lpQualEntity->lpViewEntity->
                                          lpFirstDataRecord->lpRecordName );
   ...
   {
      SqlAddQualToWhere ( cDBType, lpView, lpQualEntity,
                          lpQualAttrib->lpFirstSubQualAttrib, lpSql );
   }

   if ( fnSqlWhereIsNotEmpty( lpSql ) )
   {
      fnSqlAddStringToWhere( lpParentSql, lpSql->szSqlCmd );
      fnSqlAddStringToWhere( lpParentSql, " FROM " );
      fnSqlAddStringToWhere( lpParentSql, lpSql->szFrom );
      fnSqlAddStringToWhere( lpParentSql, " WHERE " );
      if ( lpSql->nComponentCount > 1 )
         fnSqlAddStringToWhere( lpParentSql, " ( " );

      fnSqlAddStringToWhere( lpParentSql, lpSql->szWhere );
      if ( lpSql->nComponentCount > 1 )
         fnSqlAddStringToWhere( lpParentSql, " ) " );
...
/////////////////////////////////////////////////////////////////////////////
   ...
   keys of another.  Go through all the data fields (looking for keys) and
   compare them in the where clause.
                   ...
                   LPSQLSTATEMENT lpSql )
{
   ...
   while ( lpQualAttrib )
   {
      ...
      if ( lpQualAttrib->lpViewEntity && lpQualAttrib->lpViewAttrib )
      {
         ...
         if ( SqlFindTableName( lpQualAttrib->lpDataRecord->lpRecordName,
                                lpSql ) == FALSE )
         {
            if ( fnSqlWhereIsNotEmpty( lpSql ) )
               fnSqlAddStringToWhere( lpSql, " AND " );
            ...
            SqlAddTableToSelect( cDBType, lpView, lpQualAttrib->lpViewEntity,
                                 lpSql );
         }
   ...
   while ( lpQualAttrib )
   {
      ...
      else
      {
         ...
         {
            nRC = SqlBuildSubselect( cDBType, lpView, lpQualEntity,
                                     lpQualAttrib, lpSql );
            fnSqlAddTableToFrom( lpSql,
                                 lpQualAttrib->lpDataRecord->lpRecordName );
            if ( nRC < 0 )
               return( zCALL_ERROR );
         }
         else
         ...
         {
            nRC = SqlBuildIS_Clause( cDBType, lpQualEntity,
                                     lpQualAttrib, lpSql );
            if ( nRC < 0 )
               return( zCALL_ERROR );
         }
         else
         {
            ...
            fnSqlAddStringToWhere( lpSql, "." );
            fnSqlAddStringToWhere( lpSql,
                             lpQualAttrib->lpDataField->lpFldName );
            fnSqlAddStringToWhere( lpSql, " " );
            fnSqlAddStringToWhere( lpSql, lpQualAttrib->szOper );
            ...
            {
               fnSqlAddStringToWhere( lpSql, " " );
     ????      SqlConvertStringToSqlString( cDBType, lpQualAttrib->lpDataField,
            ...
            {
               fnSqlAddStringToWhere( lpSql, " " );
               SqlGetAttributeValue( cDBType, lpView,
                ...
                zPCHAR       pchSqlCmd )
{
   ...
   // Build the list of all columns that are to be accessed.
   // Does Siron return the whole record for a userview, or do we
   ...
   if ( lpViewEntity->lpParent )
   {
      ...
      nNull = SqlBuildForeignKeys( cDBType, lpView, lpViewEntity,
                                   lpSql, zROOT_ENTITY );
   } // if ( lpViewEntity->lpParent )...
                      ...
                      zSHORT       nIndent )
{
   ...
   while ( lpQualAttrib )
   {
      ...
      if ( lpQualAttrib->lpSourceDataField )
      {
         ...
         TraceLineS( szStr,
                     lpQualAttrib->lpSourceDataField->lpViewAttrib->lpName );
         szStr[ n ] = 0;
      }
...
/////////////////////////////////////////////////////////////////////////////
void
SqlFreeQualEntity( LPQUALENTITY zFAR *lpFirstQualEntity )
{
   LPQUALENTITY lpQualEntity;
                   ...
                   zPCHAR pchQualEntity )
{
   ...
   if ( nErrorOrWarning == zWARNING )
   {
      ...
      zstrcpy( szText, "Warning loading qualification object for Activate:\n" );
   }
   else
   {
      ...
      bBeep   = TRUE;
      nIcon   = zICON_ERROR;
      zstrcpy( szText, "Error loading qualification object for Activate:\n\n" );
   }
                     ...
                     zBOOL        bDBNameIsDefault )
{
         ...
         lpQualAttrib = lpQualAttrib->lpNextQualAttrib )
   {
      ...
      if ( lpQualAttrib->lpFirstSubQualAttrib &&
          !fnSqlVerifyDataBase( cDBType, lpQualAttrib->lpFirstSubQualAttrib,
...
SqlRetrieveQualAttrib( zCHAR        cDBType,
                       ...
                       zVIEW        lpQualView,
                       LPQUALENTITY lpQualEntity,
                       LPQUALATTRIB zFAR *lpFirstQualAttrib )
{
   zSHORT       nRC;
   zSHORT       nParenCount;
   zUSHORT      uLth;
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      *lpFirstQualAttrib = lpQualAttrib;
      ...
      hMem = SysAllocMemory( (zPVOID) &lpQualAttrib->szOper,
      ...
      zstrcpy( lpQualAttrib->szOper, szOper );

      // ===
      // ===  QualAttrib.EntityName
      // ===

      GetAddrForAttribute( &szAttEntityName, lpQualView, szlQualAttrib,
                           "EntityName" );
      if ( szAttEntityName[ 0 ] )
      {
         ...
         // child entities.
         if ( zstrcmp( lpQualEntity->lpViewEntity->lpName,
                       szAttEntityName ) != 0 )
         ...
         {
            SqlDisplayQualMsg( lpQualView,
                               "Attribute AttributeName for entity "
                               "QualAttrib not found in target object "
                               "definition.",
                               zERROR, szlQualAttrib );
            return( zCALL_ERROR );
         }
      ...
      GetAddrForAttribute( &szAttributeName, lpQualView, szlQualAttrib,
                           "AttributeName" );
      ...
      // The value, which was stored in the Qual Object is retrieved
      // here.
               ...
               lpQualAttrib->lpSourceDataField == 0) )
         {
            ...
            if ( strstr( "<= =< >= => !=", lpQualAttrib->szOper ) )
            {
               SqlDisplayQualMsg( lpQualView,
                                  ...
                                  "which are needed for a comparison "
                                  "opererator ( '=', '<', '>=', etc.)",
                                  zERROR, szlQualAttrib );
               return( zCALL_ERROR );
            }
...
fnSqlRetrieveQualObject( zCHAR        cDBType,
                         ...
                         zVIEW        lpQualView,
                         LPVIEWOD     lpViewOD,
                         LPQUALENTITY zFAR *lpFirstQualEntity )
{
   LPQUALENTITY lpQualEntity;
   LPVIEWENTITY lpViewEntity;
   ...
   *lpFirstQualEntity = 0;
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      *lpFirstQualEntity = lpQualEntity;
      ...
      {
         SqlDisplayQualMsg( lpQualView,
                            "Attribute EntityName in entity EntitySpec not "
                            "found in target object definition.",
                       ...
                       LPQUALENTITY zFAR *lpFirstQualEntity )
{
   ...
   nRC = fnSqlRetrieveQualObject( cDBType, lpView, lpQualView, lpViewOD,
                                  lpFirstQualEntity );
   if ( nRC < 0 )
   {
...
/////////////////////////////////////////////////////////////////////////////
      ...
      Copy pchToken to the buffer pointed to by pchBuffer.  *ppchLine points to
      the position in pchBuffer where pchToken is to be copied and will be
...
/////////////////////////////////////////////////////////////////////////////
void
fnSqlDisplayCommand( zPCHAR zFAR *ppchSqlCmdStr,
                     zPCHAR pchErrorChar,
                     zSHORT nIndentLth,
                     ...
                     zBOOL  bFirstCall )
{
   ...
   if ( pchInitString )
   {
      zstrcpy( szBuffer, pchInitString );
   ...
   // Loop through, copying each character from pchSqlCmd to szBuffer for
   // printing.
