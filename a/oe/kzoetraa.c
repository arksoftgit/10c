            ...
            *lplCount2 = zatol( szMessage + 8 ) + 100000000;
            ...
            for ( nIdx = 1 ; nIdx < 450 ; nIdx++ )
            {
               ...
               if ( *lplCount1 || *lplCount2 )
               {
                  ...
                  lInternal += *lplCount2;
                  szMessage[ 3 ] = 0;
                  SysGetOperationMessage( &szMessage[ 3 ],
                                  nIdx, 40 );
                  if ( szMessage[ 3 ] == 0 )
                  {
                     szMessage[ 3 ] = '(';
                     zltoa( nIdx, &szMessage[ 4 ] );
                     zstrcat( szMessage, ")" );
                  }

                  zstrcat( szMessage, ", External calls (" );
                  zltoa( *lplCount1, &szMessage[ 200 ] );
                  zstrcat( szMessage, &szMessage[ 200 ] );
                  zstrcat( szMessage, "), Internal calls (" );
                  zltoa( *lplCount2, &szMessage[ 200 ] );
                  zstrcat( szMessage, &szMessage[ 200 ] );
                  zstrcat( szMessage, ")" );
            ...
            lplCount1 = (zPLONG) zGETPTR( lpTask->hCountBuf );
            lplCount2 = lplCount1 + 1;
            zltoa( *lplCount1, &szMessage[ 199 ] );
            zstrncpy( szMessage, &szMessage[ 200 ], 2 );
            zstrcpy( &szMessage[ 2 ], ":" );
            zstrncpy( &szMessage[ 3 ], &szMessage[ 202 ], 2 );
            zstrcpy( &szMessage[ 5 ], ":" );
            zstrncpy( &szMessage[ 6 ], &szMessage[ 204 ], 2 );
            zstrcpy( &szMessage[ 8 ], "." );
            zstrcpy( &szMessage[ 9 ], &szMessage[ 206 ] );
            TraceLineS( "   Start time: ", szMessage );
            zltoa( *lplCount2, &szMessage[ 199 ] );
            zstrncpy( szMessage, &szMessage[ 200 ], 2 );
            zstrcpy( &szMessage[ 2 ], ":" );
            zstrncpy( &szMessage[ 3 ], &szMessage[ 202 ], 2 );
            zstrcpy( &szMessage[ 5 ], ":" );
...
TraceLineS( zCPCHAR Title, zCPCHAR String )
{
   ...
   if ( Title )
   {
      ...
      // The common case is that everything fits nicely ... so take care of
      // it right away.
      if ( nTitleLth + nStringLth < zMAX_MSG_LTH )
      {
         zstrcpy( szMessage, Title );
         if ( String )
            zstrcpy( szMessage + nTitleLth, String );
         else
            zstrcpy( szMessage + nTitleLth, "(null)" );
      }
      else
      {
         ...
         while ( nLth < nTitleLth )
         {
            ...
            {
               zstrcpy( szMessage + nMsgOffset, Title + nLth );
            ...
            else
            {
               ...
               zstrncpy( szMessage + nMsgOffset, Title + nLth, nVacant );
         ...
         while ( nLth < nStringLth )
         {
            ...
            {
               zstrcpy( szMessage + nMsgOffset, String + nLth );
            ...
            else
            {
               // fit as much of title as possible into szMessage using
               // commas and spaces to break the message if possible
               zstrncpy( szMessage + nMsgOffset, String + nLth, nVacant );
               szMessage[ zMAX_MSG_LTH ] = 0;
   ...
   {
      zstrcpy( szMessage, "(oetr) Invalid \"null\" TraceLine title" );
   }
...
TraceLine( zCPCHAR cpcFormat, ... )
{
   ...
   nBufCnt = _vsnprintf( szBuffer, _countof( szBuffer ), cpcFormat, args );
   ...
   for ( ulPos = 0; ulPos < ulLth; ulPos++ )
   {
      ...
      if ( ulPos % 16 == 0 )
      {
         if ( szHex[ 0 ] )
         {
            ...
            if ( uLongestStr < zstrlen( szHex ) )
               uLongestStr = zstrlen( szHex );
         }
         ...
         {
            n = ulBlankLines * 16;
            zsprintf( szHex, "%s  %d Repeat Lines (%4d thru %4d) ",
         ...
         zsprintf( szHex, "%s(%4d) ", cpcPrefix, ulPos );
         zstrcpy( szAscii, "    " );
      }
      else
      if ( ulPos % 8 == 0 )
      ...
      zltox( (zLONG) cpcBuffer[ ulPos ], sz );
      n = zstrlen( sz );
      if ( n < 2 )
      {
         zstrcat( szHex, "0" );
                         ...
                         LPVIEWENTITYCSR  lpViewEntityCsrPosition )
{
   ...
   SysReadZeidonIni( -1, "[Debug]", "TraceAllAttributes", szTraceAll );
   ...
   else
   {
      ...
      zstrcpy( szWorkString, ">>>EntityInstance  " );
      if ( lpSelectedInstance && lpSelectedInstance->nSelectSet )
      {
         ...
         for ( nWk3 = 1; nWk3 < 17; nWk3++ )
         {
            if ( lpSelectedInstance->nSelectSet & nWk2 )
            {
               ...
               zltoa( (zLONG) nWk3, &szWorkString[ 250 ] );
               zstrcat( szWorkString, &szWorkString[ 250 ] );
               nWk1++;
            }
      ...
      while ( lpViewAttrib )
      {
         zstrcpy( szWorkString + 3, lpViewAttrib->szName );
         ...
         if ( lpViewAttrib->cType == 'B' )
         {
            ...
            if ( ulLth )
            {
               zstrcpy( &szWorkString[ k ], "Blob Lth=" );
               zltoa( (zLONG) ulLth, &szWorkString[ k + 9 ] );
               k = zstrlen( szWorkString );
               szWorkString[ k++ ] = ' ';
               szWorkString[ k ] = 0;
            }
