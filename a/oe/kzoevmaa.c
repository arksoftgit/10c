                           ...
                           zVIEW  vTask )
{
   ...
   *pvReturnView = 0;  // init in case of error
   ...
   // If task not active or disabled, or view invalid, return zCALL_ERROR.
   if ( (lpCurrentTask = fnOperationCall( iCreateViewFromView,
                                          vSrc, zVALID_VIEW )) == 0 )
   {
      return( zCALL_ERROR );
   }
   ...
   if ( vSrc->bApplicationView &&
        (nRC = SfTransferView( vTemp, vSrc, zLEVEL_APPLICATION )) != 0 )
   {
      fnOperationReturn( iCreateViewFromView, lpCurrentTask );
      return( nRC );
   }
   ...
   if ( nRC == 0 )
   {
      ...
      {
         TraceLine( "CreateViewFromView: 0x%08x   LOD: %s",
                    vTemp, lpViewOD->szName );
         if ( k >= zMAX_VIEW_TEST )
                ...
                zSHORT   nLevel )
{
   ...
   if ( nRC > 0 )
      zstrcpy( pchReturnName, lpViewName->szName );
                ...
                zSHORT   nLevel )
{
   ...
   // If task not active or disabled, or view invalid, return zCALL_ERROR.
   if ( (lpCurrentTask = fnOperationCall( iSetNameForView, lpView,
                                          zVALID_VIEW )) == 0 )
   {
      return( zCALL_ERROR );
   }
...
// if ( zstrcmp( cpcName, "vERD_LPLR" ) == 0 )
   ...
   {
      // If level is application but lpSubtaskView is not specified, then
      // use the lpView.
      if ( lpSubtaskView == 0 )
         lpSubtaskView = lpView;

      // Since we are using the passed view in SetNameForView, we should
      // probably use the task of the passed view as the task on which to
      // hang the name.
      lpTask = zGETPTR( lpSubtaskView->hTask );
      if ( (nLevel & zLEVEL_APPLICATION) &&
      ...
      // zCHAR szDebug[ 2 ];
         ...
         TraceLineS( "(vm) Naming non-application level view at the application level: ",
                     cpcName );

      // SysReadZeidonIni( -1, "[Debug]", "BombZDr", szDebug );
      // if ( szDebug[ 0 ] == 'Y' )
      // {
      //    k /= k;
      //    k++;
   ...
   if ( cpcName && cpcName[ 0 ] )
   {
      if ( nLevel == zLEVEL_SUBTASK )
      {
         ...
         else
         {
            ...
            if ( lpViewName )
            {
               zstrcpy( lpViewName->szName, cpcName );
               lpViewName->hView = zGETHNDL( lpView );
               lpView->nViewNames++;
...
SfSetUserIdForTask( zVIEW lpTaskView, zCPCHAR cpcUserId )
{
   ...
   lpTask->szUserId[ 0 ] = 0;
   zstrncat( lpTask->szUserId, cpcUserId, 33 );
...
SfGetUserIdForTask( zVIEW lpTaskView, zPCHAR pchUserId )
{
   ...
   zstrcpy( pchUserId, lpTask->szUserId );
...
fnDropView( zVIEW  lpView )
{
        ...
        (lpView->bFlagDropView || lpView->bTraceDropView) )
   {
      ...
      zsprintf( szMsg, "View about to be dropped (0x%08x).  Object Name = %s",
                lpView, pch );
      TraceLineS( "(vm) ", szMsg );
      if ( lpView->bFlagDropView )
