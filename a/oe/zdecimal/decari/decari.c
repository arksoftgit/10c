 ...
 */
void
decari_to_string( char *szReturn, const tDec *pd1, short nPrecision, char cEF )
{
      ...
      if ( iExp < (short) strlen( cy ) )
      {
         ...
         *(pc++) = '.';
         ...
         else
         {
            /* remove trailing zeros
             */
         // for ( j = strlen( cy + iExp + 1 ) -1; j > 0; j--)
            for ( j = strlen( cy + iExp + 1 ) -1; j > 0 && j >= nPrecision; j-- )  // dks 2009.11.20
            {
               if ( cy[ iExp + 1 + j ] != '0' )
                  break;
               cy[ iExp + 1 + j ] = 0;
