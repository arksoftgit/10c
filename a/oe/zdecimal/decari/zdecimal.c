   ...
   switch ( lCommand )
   {
      ...
      case zDEC_CMD_DECTOSTRING:
      {
         ...
         */
         ...
         decari_to_string( (char*) pvReturn, &tempDecimal,
                                nPrecision, 'F' );
         return( 0 );
      }
      ...
      case zDEC_CMD_DECTOLONG:
         ...
         decari_to_string( cTemp, (tDec*) pvDecimal, 0, 'F' );
         {
            char *pc = strchr( cTemp, '.' );
            if( pc )
               *pc = '\0' ;
         }
         * (long *) pvReturn = zatol(cTemp);
         return( 0 );

      case zDEC_CMD_LONGTODEC:
         // Convert long to decimal.
         sprintf (cTemp, "%ld", * (long *) pvInfo);
         decari_from_string( (tDec*) pvReturn, cTemp) ;
         return( 0 );

      case zDEC_CMD_DECTODOUBLE:
         // Convert decimal to double.
       decari_to_string( cTemp, (tDec*) pvDecimal, -1, 'E' );
         *(double *) pvReturn = atof( cTemp );
         return( 0 );

      case zDEC_CMD_DOUBLETODEC:
         // Convert double to decimal.
         sprintf (cTemp, "%.15lE", * (double *) pvInfo);
         if ( (pc = strchr (cTemp, ',')) != NULL )
            *pc = '.';
         decari_from_string( (tDec*) pvReturn, cTemp) ;
         return( 0 );
      ...
      case zDEC_CMD_COMPAREDOUBLE:
      {
         zDECIMAL dTemp;
         sprintf (cTemp, "%.15lE", * (double *) pvInfo);
         decari_from_string( (tDec*) &dTemp, cTemp) ;
