                             ...
                             LPTASK           lpCurrentTask )
{
   ...
   // Compare the Search to the Src in the context of the Src's Domain
   {
      ...
      if ( nRC == 0 )
      {
         ...
         if ( lpDomain )
            zstrcpy( szContext, lpDomain->szName );
         else
            szContext[ 0 ] = 0;
                              ...
                              zCPCHAR  cpcScopingEntityName )
{
   ...
   if ( cpcStringValue && *cpcStringValue )
   {
      ...
      if ( lpViewAttrib->cType == zTYPE_DATETIME )
      {
                                  ...
                                  (LPDATETIME) &stInternalDateTime ) != 0 )
         {
            ...
            zstrcpy( sz, "(S->DT), " );
            zstrcat( sz, lpViewEntity->szName );
        ...
        lpDomain->cDomainType == zDM_TYPE_TABLE )
   {
      ...
      if ( nRC2 >= zCURSOR_SET )
      {
         ...
         zstrcpy( sz, "Failure during standard call, but would have been "
                      "successful if the default context was used.\n"
                      "Probable User Error." );
         zstrcat( sz, "\nObject Def: " );
         zstrcat( sz, lpViewOD->szName );
         zstrcat( sz, "\nEntity: " );
         zstrcat( sz, lpViewEntity->szName );
         zstrcat( sz, "\nAttribute: " );
         zstrcat( sz, lpViewAttrib->szName );
   ...
   *pchReturnEntityName = 0;
        ...
        lpSearchEntityInstance && lpSearchEntityInstance->u.nInd.bHidden == FALSE )
   {
      ...
      //    SysMessageBox( 0, "SetCursorAbsolutePosition", "UNSET_CSR", -1 );
         ...
         zstrcpy( pchReturnEntityName, lpViewEntity->szName );
         nRC = zCURSOR_SET;
      }
      else
      {
              ...
              SetViewToSubobject( lpView, lpViewEntity->szName ) == 0 )
         {
            ...
            zstrcpy( pchReturnEntityName, lpSearchViewEntity->szName );
                   ...
                   LPTASK           lpCurrentTask )
{
              ...
              lpSrcViewEntityAttrib->hDomain != lpViewEntityAttrib->hDomain )
         {
            ...
            if ( nRC == 0 || nRC == zVAR_NULL )
            {
               ...
               if ( cpcContextName && *cpcContextName )
                  zstrcpy( szContext, cpcContextName );
               else
   ...
   if ( cpcContextName && *cpcContextName )
   {
      ...
      switch ( lQualRequest )
      {
         ...
         case zQUAL_STRING:
            zstrcpy( unTemp.szString, (zPCHAR) pQualValue );
               ...
               case -1:
                       ...
                       (lControl & zIGNORE_ERROR_358) == 0 )
                  {
                     ...
                     switch ( lQualRequest )
                     {
                        ...
                        case zQUAL_INTEGER:
                           zltoa( *((zPLONG) pQualValue), unTemp.szString );
                           break;
                     ...
                     zstrcat( unTemp.szString,
                              lpDomain->szName );
                     zstrcat( unTemp.szString, ", " );
                     zstrcat( unTemp.szString, lpViewEntity->szName );
                     // "KZOEE358 - Value not in Context for Domain "
                     fnIssueCoreError( lpCurrentTask, lpView, 0, 358, 0,
                                       unTemp.szString,
                                       lpViewEntityAttrib->szName );
