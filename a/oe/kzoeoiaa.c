...
typedef FileDataRecord zFAR *LPFILEDATA;
...
typedef EntityCache zFAR *LPENTITYCACHE;
...
typedef CacheInfo zFAR *LPCACHEINFO;
         ...
         lpViewAttrib = zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
   {
      ...
      if ( zstrcmp( lpViewAttrib->szName, "UserName" ) == 0 )
      {
         ...
         if ( bSetUserName )
         {
            ...
            SysGetUserID( vLock, szUserName, szPassword );
            if ( fnSetAttributeFromVariable( vLock, lpLockEntityCsr,
                                             lpViewAttrib,
                                             (zPVOID) szUserName, 'S',
                                             0, 0, lpCurrentTask, 0 ) != 0 )
                              ...
                              zVIEW    vSubtask )
{
   ...
   for ( k = 0; k < nViewCount; k++ )
   {
      ...
      if ( lpViewOD->nEntityLock )
      {
         ...
         zstrcpy( szLockName, "#Global Entity Lock" );
      }
      else
         ...
         zsprintf( szLockName, "#%s", lpViewOD->szName );
   ...
   if ( lpViewOD->szNetwork[ 0 ] == 0 || lControl & zACTIVATE_LOCAL )
   {
      ...
      while ( TRUE )
      {
         ...
         if ( nRC == zDB_DEADLOCK )
         {
            ...
            zstrcpy( szMsg, "(oi) Database Deadlock - Object=" );
            zstrcat( szMsg, cpcViewOD_Name );
            zstrcat( szMsg, " Activate Retry Count=" );
...
EndOfFunction:
   ...
   if ( AnchorBlock->TraceFlags.bOI_Times )
   {
      ...
      zsprintf( sz, "%lf seconds for object %s ",
...
// for the application. I repeated this change in CommitOI_ToFile.
   ...
   {
      if ( lpApp )
         zstrcpy( szWork, lpApp->szObjectDir );
      else
      {
         LPAPP lpSystemApp = zGETPTR( AnchorBlock->hSystemApp );

         zstrcpy( szWork, lpSystemApp->szObjectDir );
      }
   ...
   zstrcat( szWork, cpcFileName ); // w\bin\sys\tzrpsrco.lod
                ...
                zCPCHAR cpcDBHandlerName )
{
   ...
   // }
      ...
      zstrcpy( lpDBHandler->szName, cpcDBHandlerName );
   ...
   if ( lpDBHandler->hLibrary == 0 )
   {
      ...
      if ( zstrncmpi( lpDBHandler->szName, "KZ", 2 ) == 0 )
      {
         ...
         zstrcpy( szLibName, AnchorBlock->szZeidonBin );
         zstrcat( szLibName, lpDBHandler->szName );
      ...
      if ( hLibrary )
      {
         ...
         if ( lpDBHandler->pfDBHandlerOper == 0 )
         {
            ...
            zstrcpy( szFuncName, lpDBHandler->szName );
            zstrcat( szFuncName, "_DBH_MsgProc" );
...
// lpDBHandler = fnGetDBHandler( hTask, lpViewOD->szDBHandler );
   ...
   else
   {
      ...
      {
         // Couldn't find the default name.  Try the old-style name.
         zstrcpy( (zPCHAR) szFuncName, lpDBHandler->szName );
         zstrcat( (zPCHAR) szFuncName, "_DBH_MsgProc" );
                    ...
                    zlpPGKHOPER pOperReturn )
{
   ...
   else
   {
      ...
      if ( zstrncmp( lpGKHandler->szName, "KZ", 2 ) == 0 )
      {
         ...
         zstrcpy( szLibName, AnchorBlock->szZeidonBin );
         zstrcat( szLibName, lpGKHandler->szName );
      ...
      if ( pGKOper == 0 )
      {
         ...
         zstrcpy( (zPCHAR) szFuncName, lpGKHandler->szName );
         zstrcat( (zPCHAR) szFuncName, "_GKH_MsgProc" );
      ...
      // Call gkhandler routine to issue start of transaction.  Bomb out if
      // it doesn't work.
                ...
                zCPCHAR cpcGKHandlerName )
{
   ...
   // }
      ...
      zstrcpy( lpGKHandler->szName, cpcGKHandlerName );
   ...
   if ( lpGKHandler->hLibrary == 0 )
   {
      ...
      if ( hLibrary )
      {
         ...
         {
            // Couldn't find the default name.  Try the old-style name.
            zstrcpy( szFuncName, lpGKHandler->szName );
            zstrcat( szFuncName, "_GKH_MsgProc" );
...
*/
                   )
               {
                  if ( (lControl & zCOMMIT_DROPLOCKSONLY) == 0 )
                  {
                     ...
                     zsprintf( szMsg, "%s  %s   Object Definition: %s   "
                          ...
                          zBOOL            bCheckCreate )
{
   ...
   if ( bShowMsg )
   {
      ...
      TraceLineS( "----------------- Warning ----------------", "" );
      zsprintf( szMsg, "Warning: Trying to commit an entity that "
      ...
      for ( k = 0; k < nViewCount; k++ )
      {
         ...
         zsprintf( szMsg, "   View ID = %lx, Object name = %s",
                   lpViewArray[ k ], lpViewOD_Array[ k ]->szName );
         TraceLineS( szMsg, "" );
      }


      zsprintf( szMsg, "%s.%s", lpViewOD->szName, lpViewEntity->szName );
      if ( bCheckCreate )
      ...
      {
         zsprintf( szMsg, "%s.%s", lpCandidateViewOD->szName,
      ...
      if ( lpViewOD_Array[ k ]->bGenkey )
      {
         if ( hGKHandler == 0 )
         {
            ...
            if ( hGKHandler == 0 )
            {
               ...
               zsprintf( szMsg, "No GenKey handler set for LOD: %s",
                      ...
                      LPVIEWOI         lpViewOI )
{
   ...
   if ( bLOD_Lock )
   {
      ...
      // the LOD.
      zstrcpy( szLockName, lpViewOD->szName );
   }
   else
   {
      ...
      zsprintf( szLockName, "@%s", lpViewEntity->szName );
   }
                 ...
                 zCHAR   cDataType )
{
   ...
   if ( ulLth )
      return( SysWriteLineLth( lpTaskView, lpFileData->hFile, pchBuffer, ulLth, TRUE ) );
   else
      return( SysWriteLine( lpTaskView, lpFileData->hFile, pchBuffer ) );
}
                        ...
                        zLONG          lControl )
{
   ...
   if ( lControl & zENTITY_TAGS )
   {
      if ( lControl & zCOMPRESSED )
         zsprintf( szWorkString, "mOITAG %lx", (zLONG) lpViewOI );
      else
         zsprintf( szWorkString, "mOITAG      %lx", (zLONG) lpViewOI );
   ...
   if ( lpViewOI->lTag )
   {
      ...
      if ( lControl & zCOMPRESSED )
         zsprintf( szWorkString, "mOITAG %lx", lpViewOI->lTag );
      else
         zsprintf( szWorkString, "mOITAG      %lx", lpViewOI->lTag );
   ...
   if ( lControl & zINCREMENTAL )
   {
      ...
      {
         if ( lControl & zCOMPRESSED )
            zsprintf( szWorkString, "mOIFLAGS %lx", uFlags );
         else
            zsprintf( szWorkString, "mOIFLAGS    %lx", uFlags );
         ...
         lpEntityInstance = zGETPTR( lpEntityInstance->hNextHier ) )
   {
      ...
      if ( lControl & zCOMPRESSED )
      {
         ...
         zsprintf( szWorkString + 1, "%x %x", lpViewEntity->nHierNbr,
                   lpEntityInstance->nLevel );
         k = zstrlen( szWorkString );
      }
      else
      {
         zstrcpy( szWorkString + 1, lpViewEntity->szName );
         k = zstrlen( szWorkString );
         ...
         zltoa( lpEntityInstance->nLevel, &szWorkString[ k ] );
         k = zstrlen( szWorkString );
      }
      ...
      {
         szWorkString[ k++ ] = ',';
         zbtoa( lpEntityInstance->u.nIndicators, &szWorkString[ k ] );
      ...
      {
         if ( lControl & zCOMPRESSED )
            zsprintf( szWorkString, "mETAG %lx", (zLONG) lpEntityInstance );
         else
            zsprintf( szWorkString, "mETAG      %lx", (zLONG) lpEntityInstance );
      ...
      if ( lpEntityInstance->lTag )
      {
         ...
         // write it.
         if ( lControl & zCOMPRESSED )
            zsprintf( szWorkString, "mETAG %lx", lpEntityInstance->lTag );
         else
            zsprintf( szWorkString, "mETAG      %lx", lpEntityInstance->lTag );
      ...
      {
         if ( lControl & zCOMPRESSED )
            zsprintf( szWorkString, "mEKEY %lx", lpEntityInstance->ulKey );
         else
            zsprintf( szWorkString, "mEKEY      %lx", lpEntityInstance->ulKey );
      ...
      {
         // pchBufferEnd points to the first unused byte in the compression
         // buffer.
            ...
            lpViewAttrib = zGETPTR( lpViewAttrib->hNextOD_Attrib ) )
      {
         ...
         if ( bEntityIsCompressed && lpViewAttrib->cType != zTYPE_BLOB )
         {
            ...
            if ( lpViewAttrib->cType == zTYPE_INTEGER )
            {
               ...
               if ( fnGetIntegerFromAttribute( &l, lpView, &ViewEntityCsr,
                                               lpViewAttrib ) != 0 )
               {
               ...
               if ( l >= 0 )
                  zltox( l, pchBufferEnd );
               else
               {
                  *pchBufferEnd = '-';
                  zltox( l * -1, pchBufferEnd + 1 );
               }
            }
            else
            ...
            if ( lpAttribFlags )
            {
               zltox( (zLONG) lpAttribFlags->u.uFlags, pchBufferEnd );
               pchBufferEnd += zstrlen( pchBufferEnd ) + 1;
            }
         ...
         else
         {
            ...
            if ( lpAttribFlags )
               zsprintf( szAttribFlags, ",%lx", lpAttribFlags->u.uFlags );
            else
               szAttribFlags[ 0 ] = 0;
            ...
            if ( lControl & zCOMPRESSED )
               zsprintf( szWorkString, "a%lx%s ", lpViewAttrib->lXVAAttTok,
                         szAttribFlags );
            else
            {
               zCHAR szAttribName[ 50 ];

               zsprintf( szAttribName, "%s%s", lpViewAttrib->szName,
                         szAttribFlags );
               zsprintf( szWorkString, "a%-9s ", szAttribName );
            }

            k = zstrlen( szWorkString );

            fnGetAddrForAttribute( &pchAttr, lpView, &ViewEntityCsr, 0,
                                   lpViewAttrib );
            ...
            else
            {
                    ...
                    lpViewAttrib->cType == zTYPE_INTEGER )
               {
                  ...
                  // We are writing a compressed file so convert the value to
                  // a hex number.
                  if ( fnGetIntegerFromAttribute( &l, lpView, &ViewEntityCsr,
                                                  lpViewAttrib ) == -1 )
                  {
                     ...
                     // Write the attribute value as a '*' to indicate that the
                     // integer is null.
                     szWorkString[ k++ ] = '*';
                     szWorkString[ k++ ] = 0;
                  }
      ...
      if ( bEntityIsCompressed && bValuesStored )
      {
         ...
         zsprintf( szLth, "a %x", ulLth );
         if ( (*lpfnStreamFunc)( lpView, lpvData, szLth, zstrlen( szLth ),
                                 zTYPE_STRING ) )
         {
            goto EndOfFunction;
         }
   ...
   if ( lLastLinkedInstance > -1 )
   {
            ...
            lpEntityInstance = zGETPTR( lpEntityInstance->hNextHier ) )
      {
         ...
         {
            szWorkString[ 0 ] = 'i';
            zltoa( lpEntityInstance->lHierCount, &szWorkString[ 1 ] );
            k = zstrlen( szWorkString );
            do
            {
               szWorkString[ k++ ] = ' ';
            ...
            zltoa( lpSourceEntityInstance->lHierCount, &szWorkString[ k ] );
         }
   ...
   if ( lControl & zSAVE_CURSORS )
   {
            ...
            lpViewEntityCsr = zGETPTR( lpViewEntityCsr->hNextHier ) )
      {
         ...
         if ( lpViewEntityCsr->hEntityInstance == UNSET_CSR )
         {
            zsprintf( szWorkString, "c%x 0", lpViewEntityCsr->nHierNbr );
         ...
         if ( lpCurrentEI->u.nInd.bWritten == FALSE )
         {
            ...
            // none of it's twins were written to the stream then the cursor
            // is considered NULL.
         ...
         else
         {
            ...
            zsprintf( szWorkString, "c%x %lx", lpViewEntityCsr->nHierNbr,
                      lInstanceCount );
         }
                    ...
                    zPVOID         lpvData )
{
   ...
   // Get rid of all deadwood instances (e.g. entities that have been created
   // AND deleted).
   fnReclaimHiddenInstances( lpViewOI );
   ...
   zstrcpy( FileHeader.szZeidon, szlmZeidon );
   ...
   zstrcpy( FileHeader.szFileName, pchFileNmp );
   ...
   zstrcpy( FileHeader.szObjectType, lpViewOD->szName );
   SysGetDateTime( szDateTime );
   fnDateTimeFormat( szDateTime, FileHeader.szDate, FileHeader.szTime );
   ...
   zstrcpy( FileHeader.szRelease, lpViewOI->szRelease );  // dks 3/5/96
   ...
   if ( (*lpfnStreamFunc)( lpView, lpvData, (zPCHAR) &FileHeader,
   ...
   if ( bIncremental &&
        (*lpfnStreamFunc)( lpView, lpvData, lpViewOD->szER_Date,
                           0, zTYPE_STRING ) == zCALL_ERROR )
   {
      return( zCALL_ERROR );
   }
   ...
   {
      zsprintf( szTemp, "mCONTROL %lx", lpViewOI->lActivateControl );
      if ( (*lpfnStreamFunc)( lpView, lpvData, szTemp, 0,
                              zTYPE_STRING ) == zCALL_ERROR )
      {
         return( zCALL_ERROR );
      }
   }

   nRC = fnWriteOI_ToTextStream( lpView, lpfnStreamFunc, lpvData,
                                 bIncremental, lControl );
                               ...
                               zLONG     lControl )
{
   ...
   zstrcpy( lpViewOI->szRelease, szlReleaseCompatible );  // initialize release for compatiblity
...
/*
   ...
   instance and set it's key value to match the foreign key of the parent.
                   ...
                   zSHORT           nKeyCnt )
{
   ...
   // Because the delimiter can change, the first char will be the delimiter
   // for the string.
   pch = pchKeyString;
      ...
      *pch++ = chDelim;
      ...
      switch ( lpKeyViewAttrib[ k ]->cType )
      {
         case zTYPE_STRING:
            ...
            zstrcpy( pch, (zPCHAR) lpPtr );
            break;

         case zTYPE_INTEGER:
            zltoa( *((zPLONG) lpPtr), pch );
            break;

         case zTYPE_DECIMAL:
            ConvertDecimalToString( pch, lpKeyViewAttrib[ k ],
                                    *((zPDECIMAL) lpPtr), 0 );
            break;
      ...
      {
         // The current delimiter exists in the key value so we must get a
         // new one.  fnFindKeyDelim will change all the old delims to match
         // the new delim.
         ...
         lpEntityInstance = zGETPTR( lpEntityInstance->hNextTwin ) )
   {
      ...
      if ( k >= 0 )
      {
         ...
         if ( *pch != 'Y' || bAllowReadOnly == FALSE )
         {
            ...
            if ( bLOD_Lock )
               zsprintf( szWhat, "LOD '%s' is locked", lpViewOD->szName );
            else
               zsprintf( szWhat, "Entity '%s' is locked", lpViewEntity->szName );

            GetAddrForAttribute( &pch, vLock, "ZeidonLock", "UserName" );
            GetStringFromAttribute( szMsg, vLock, "ZeidonLock", "Timestamp" );
            UfFormatDateTime( szDate, szMsg, "YYYY-M-D HH:MI:SS" );
            TraceLineS( "(oi) Object locked by ", pch );
            TraceLineS( "(oi) Object locked on ", szDate );
            TraceLineS( "(oi) Concatenated key = ", szKeyString );
         // zsprintf( szMsg, "By '%s' on %s", pch, szDate );
                    ...
                    LPCACHEINFO      lpCacheInfo )
{
         ...
         uIdx++ )
   {
      ...
      {
         TraceLineS( "", "" );
         zsprintf( szMsg, "Entity %s loaded from cache.", lpViewEntity->szName );
         TraceLineS( szMsg, "" );
         TraceLineS( "", "" );
      }
                             ...
                             zPLONG         plControl )
{
                                      ...
                                      &pchLine, 0, 0 )) == 1 )
   {
      ...
      if ( nErrorEntityLvl )
      {
         if ( pchLine[ 0 ] == 'e' )
         {
            SysParseLine( szWorkString, &pchLine, pchLine + 1 );
      ...
      if ( pchLine[ 0 ] == 'a' && lpViewEntity )  // Attrib and entity exists
      {
         ...
         if ( lpViewAttrib == 0 )
         {
            if ( bIgnoreAttribErrors == FALSE )
            {
               ...
               zstrcpy( sz, lpViewEntity->szName );
               zstrcat( sz, "." );
               zstrcat( sz, szWorkString );
      ...
      if ( pchLine[ 0 ] == 'i' )
      {
         ...
         lpSrcInstance = (LPENTITYINSTANCE) *(pRelinkBufferTable[ k ] +
                                                     lLinkSrc % 160000);  // moved up from 16000 dks/don 2007.01.04
         k = lLinkTgt / 160000;  // moved up from 16000 dks/don 2007.01.04
         lpTgtInstance = (LPENTITYINSTANCE) *(pRelinkBufferTable[ k ] +
                                                     lLinkTgt % 160000);  // moved up from 16000 dks/don 2007.01.04
         ...
         if ( lpTgtInstance && lpSrcInstance )
         {
            ...
            {
               TraceLine( "(oi) Error (OD: %s) - Tgt View Entity: %s Token: %d  View Entity: %s Token = %d",
               ...
               if ( bMsgBox )
               {
                  bMsgBox = FALSE;
                  SysMessageBox( lpView, szlOE_SystemError,
                                 "ER Tokens don't match for linked entities!", 1 );
               }
            }
            else
            {
      ...
      if ( pchLine[ 0 ] == 'c' )
      {
         ...
         // First make sure that the last created entity instance has it's
         // indicators set.
         if ( lpEntityInstance )
         {
            if ( *plControl & zINCREMENTAL )
...
z1000-Zeidon    ACCOUNT  TZWDLGSO 04/18/07   09:18:42 1.0a2
   ...
   else
   {
      ...
      {
         zstrncpy( lpViewOI->szRelease, pchLine + 49, 8 );
         lpViewOI->szRelease[ 8 ] = 0;
      }
   ...
   nRC = fnActivateOI_FromTextStream( lpView, lpfnStreamFunc,
                                      lpvData, &lControl );
   if ( nRC < -1 )
      goto EndOfFunction;
...
//./ END + 2
void
fnDateTimeFormat( zPCHAR pchDateTime, zPCHAR pchDate, zPCHAR pchTime )
{
   zstrncpy( pchDate, pchDateTime + 4, 2 );      // copy month
   pchDate[ 2 ] = '/';                           // separator
   zstrncpy( pchDate + 3, pchDateTime + 6, 2 );  // copy day
   pchDate[ 5 ] = '/';                           // separator
   zstrncpy( pchDate + 6, pchDateTime + 2, 2 );  // copy year
   pchDate[ 8 ] = 0;                             // null terminator

   zstrncpy( pchTime, pchDateTime + 8, 2 );      // copy hours
   pchTime[ 2 ] = ':';                           // separator
   zstrncpy( pchTime + 3, pchDateTime + 10, 2 ); // copy minutes
   pchTime[ 5 ] = ':';                           // separator
   zstrncpy( pchTime + 6, pchDateTime + 12, 2 ); // copy seconds
   pchDate[ 8 ] = 0;                             // null terminator
}
