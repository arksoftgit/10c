...
ARACrypt::ARACrypt( ) :
   ...
   //
          ...
          m_Rot1_C( 0xF0000000 )
{
   strcpy( m_szKey, "Don2Cam9Pas1Gig3Dks5Dgc8Kat2Mik3Jab1Mat2" );
}
...
ARACrypt::SetKey( zCPCHAR cpcKey )
{
   ...
   strcpy( m_szKey, cpcKey );

   if ( m_szKey[ 0 ] == 0 )
   {
      // Put some really outrageous characters in seed just in case.
      strcpy( szSeed, "\x43\xC8\x21\xD3\xF4\xB3\x10\x27\x09\xAA\x18\x56" );
...
void
ARACrypt::GetKey( zPCHAR  pchKey )
{
   strcpy( pchKey, m_szKey );
}
...
// zsprintf( szMsg, "UfEncryptStringUL Length: %d   In: \"%s\"", uLth, pchIn );
// TraceLineS( szMsg, "" );
   ...
   if ( pchKey && *pchKey )
   {
      ...
      zstrncpy( szNewKey, pchKey, sizeof( szNewKey ) - 1 );
      szNewKey[ sizeof( szNewKey ) - 1 ] = 0;  // force null terminator
      nInLth = zstrlen( szNewKey );

      crypt.GetKey( szKey );
      nKeyLth = zstrlen( szKey );
      if ( nKeyLth + nInLth > sizeof( szKey ) - 1 )
         nKeyLth = sizeof( szKey ) - 1 - nInLth;

      if ( nKeyLth > 0 )
      {
         zstrncpy( szNewKey + nInLth, pchKey, nKeyLth );
   ...
   pchOut[ uLth ] = 0;
   nInLth = zstrlen( pchIn );
   if ( uLth > zMAXEL || nInLth >= uLth )
   {
...
// srand( (unsigned) time( NULL ) );
   ...
   if ( nInLth > sizeof( unsigned ) )
      k = nInLth - sizeof( unsigned );
   else
   {
      k = nInLth + 1;
   ...
   for ( k = nInLth; k < uLth; k++ )  // set "empty" cells to random values
   {
      do
      {
         pch[ k ] = (zCHAR) rand( );
   ...
   while ( (zUSHORT) nInLth > uLth || nInLth > zMAXEL )
      nInLth -= uLth;

   pchOut[ EncryptB[ nOrderIdx ] ] = EncryptA[ nInLth ] + 'a';
// zsprintf( szMsg, "UfEncryptStringUL Idx: %d   EncryptIdx: %d   Value: %2x",
...
// zsprintf( szMsg, "UfEncryptStringUL Out: \"%s\"", pchOut );
// TraceLineS( szMsg, "" );
...
// TraceLineS( szMsg, "" );
   ...
   if ( uLth > zMAXEL || nInLth >= uLth || nInLth < 0 )
   {
      zsprintf( szMsg, "Length Error Decrypting String: (%d:%d)  \"%s\"", uLth, nInLth, pchIn );
      SysMessageBox( 0, "Zeidon Internal Error", szMsg, 0 );
      return( zCALL_ERROR );
   }

   nReturnLth = 0;
   while ( EncryptA[ nReturnLth ] != (zUCHAR) nInLth )
      nReturnLth++;

   nInLth = (zCHAR) nReturnLth;
   nOrderIdx++;
   while ( nInLth )
   {
      while ( EncryptB[ nOrderIdx ] > (uLth - 1) )
   ...
   pchOut[ uChar ] = 0;
// zsprintf( szMsg, "UfDecryptStringUL Out: \"%s\"", pchOut );
// TraceLineS( szMsg, "" );
   return( nReturnLth );
}
...
// pfn = fnDecimalOperation;   // in kzoesyaa.c
   ...
   else
   {
      ...
      if ( lpLibrary == 0 )
      {
         SysReadZeidonIni( -1, "[Zeidon]", "DefaultDecimalOK", szValue );
         zstrupper( szValue );
