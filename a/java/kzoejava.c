...
// (*jEnv)->CallBooleanMethod( jEnv, oVector, Vector_add_ID, line1 );
   ...
   if ( SetTransientTaskProcess( cpcSessionId, vView, TRUE, "GetTableListForAttribute" ) == 0 )
   {
      ...
      for ( nRC = GetFirstTableEntryForAttribute( szTableEntry, vView, cpcEntityName,
                                                  cpcAttribName, cpcContextName, &pv );
            nRC >= 0;
            nRC = GetNextTableEntryForAttribute( szTableEntry, vView, cpcEntityName,
                                                 cpcAttribName, cpcContextName, &pv ) )
      {
              ...
              jint     jiNbr )
{
   ...
   if ( SetTransientTaskProcess( cpcSessionId, vSubtask /*vSrcView*/, TRUE, "SetOI_FromBlob" ) == 0 )
   {
      ...
      // Look at kzoeataa.c.
      zstrcpy( szTgtLodName, cpcTgtLodName );
              ...
              jstring  jstrAttribName )
{
   ...
   {
      nRC = GetStringFromAttribute( szTemp, vView,
                                    cpcEntityName, cpcAttributeName );
              ...
              jint     jiMaxStringLth )
{
   ...
   {
   // zstrcpy( szTemp, "GetStringFromAttributeByContext Entity: " );
   // zstrcat( szTemp, cpcEntityName );
   // zstrcat( szTemp, "  Attribute: " );
   // zstrcat( szTemp, cpcAttributeName );
   // zstrcat( szTemp, "  Context: " );
   // zstrcat( szTemp, cpcContextName );
   // zstrcat( szTemp, "  reallocating ... buffer size (1024) exceeded by maximum size of attribute: " );
              ...
              jint     jiFlags )
{
   ...
   {
      nRC = SysMakeWebFileName( szTemp, vSubtask, jiFlags );
              ...
              jstring  jstrSessionId )
{
   ...
   if ( SetTransientTaskProcess( cpcSessionId, vSubtask, TRUE, "GetWebRedirection" ) == 0 )
   {
      ...
      {
         zstrcpy( szNextJSP, szDlgTag );
         zstrcat( szNextJSP, szWndTag );
         zstrcat( szNextJSP, ".jsp" );
      }
   }
              ...
              jstring  jstrWndTag )
{
   ...
   if ( SetTransientTaskProcess( cpcSessionId, vSubtask, TRUE, "SetWebRedirection" ) == 0 )
   {
      ...
      {
         GetStringFromAttribute( szDlgTag, vKZXMLPGO, "NextDialogWindow", "DialogName" );
         GetStringFromAttribute( szWndTag, vKZXMLPGO, "NextDialogWindow", "WindowName" );
      }
              ...
              jstring  jstrWindow )
{
   ...
   {
      GetStringFromAttribute( szFocusCtrl, vKZXMLPGO, "Window", "FocusCtrl" );
   }
   else
   {
      szFocusCtrl[ 0 ] = 0;
              ...
              jstring  jstrSessionId )
{
   ...
   szPDFfile[ 0 ] = 0;
   GetViewByName( &vKZXMLPGO, "KZXMLPGO", vSubtask, zLEVEL_TASK );
   GetStringFromAttribute( szPDFfile, vKZXMLPGO, "Session", "PrintFileName" );
   GetStringFromAttribute( szFileExt, vKZXMLPGO, "Session", "PrintFileType" );
   if ( szPDFfile[ 0 ] != 0 )
   {
      strcat( szPDFfile, "." );
              ...
              jstring  jstrFeedback )
{
   ...
   {
      GetStringFromAttribute( szUserId, v, "Feedback", "UserId" );
      if ( szUserId[ 0 ] == 0 )
      {
