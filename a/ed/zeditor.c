...
typedef BufferSettings zFAR *LPBUFFERSETTINGS;
                      ...
                      zPCHAR pszFileName )
{
   ...
   GetStringFromAttribute( szFontString, vProfile, szlED, "FontString" );
...
InitSession( zVIEW  vSubtask )
{
   ...
   if ( zstrcmp( pszInvokingTool, "GO" ) == 0 )
   {
      ...
      GetStringFromAttribute( szMetaName,
                              vSource, "GlobalOperationGroup", "Name" );
   }
   else
   if ( zstrcmp( pszInvokingTool, "DM" ) == 0 )
   {
   ...
   nRC = SetCursorFirstEntityByInteger( vTempLPLR, "W_MetaType", "Type",
                                        lMetaType, 0 );
   if ( nRC >= zCURSOR_SET )
   {
   ...
   if ( bFileExists == FALSE )
   {
                                     ...
                                     szlLanguageType, szlC_File ) == 0 )
      {
         ...
         GetAddrForAttribute( &pszName, vSource,
                              lpszSourceFileEntityName, szlName );
   ...
   zstrcpy( szOperSrch, "OPERATION[ \\t\\n]+" );
   zstrcat( szOperSrch, lpszOperName );
   zstrcat( szOperSrch, "[ \\t\\n\\(]+" );
   MovTopBuf( );
   MovHome( );
...
int fnShowSubwindows( zVIEW  vSubtask )
{
         ...
         nRC = SetCursorNextEntity( vProfileXFER, szlSubwindow, 0 ) )
   {
      ...
      SetWindowActionBehavior( vSubtask, zWAB_StartOrFocusModelessSubwindow |
                               zWAB_ProcessImmediateAction, szlTZEDCWZD,
                               pszWindowName );
   }
...
SetupEditor( zVIEW vSubtask )
{
   ...
   if ( SetCursorFirstEntityByString( vProfileXFER, szlSubwindow,
                                      szlName, "Toolbox", 0 ) == zCURSOR_SET )
   {
      if ( CompareAttributeToInteger( vProfileXFER, szlSubwindow,
                                      szlHeight, 219 ) > 0 )
      {
   ...
   SetAttributeFromInteger( vEdWrk, szlEditor, szlParentToolSubtaskID,
                            (zLONG) vTemp );
...
void fnSystemClose( vSubtask )
{
   ...
   SetAttributeFromInteger( vProfileXFER, szlED, "SearchFlags",
                            (zLONG) SrchQFlags() );
   ...
   if ( BufQModified () )
   {
      if ( BufWrite () )
      {
         ...
         if ( *sz == 'y' || *sz == 'Y' )
         {
            ...
            zstrcpy( szNewFileName, psz );
            zstrcat( szNewFileName, "._zz" );
   ...
   if ( GetViewByName( &vMetaLPLR, "TZDIALOGS", 0, zLEVEL_TASK ) < 0 )
      if ( GetViewByName( &vMetaLPLR, "TZZOLFLO", 0, zLEVEL_TASK ) < 0 )
         if ( GetViewByName( &vMetaLPLR, "CM_ListGroup", 0, zLEVEL_TASK ) < 0 )
            // Use default.
            GetViewByName( &vMetaLPLR, "TaskLPLR", 0, zLEVEL_TASK );
...
SaveAndParse( zVIEW vSubtask )
{
   ...
   zstrcpy( szErrorFile, szFilename );
...
fnSetCurrentBuffer( zPVIEW pvEdWrk, zPCHAR szRoutine )
{
   ...
   if ( *pvEdWrk == 0 )
   {
      ...
      if ( !*pvEdWrk )
      {
         ...
         zstrcpy( sz, "TZEDWRKO not found in fnSetCurrentBuffer" );
         zstrcat( sz, szRoutine );
         fnEditorMsg( sz, 1, 0, 0, zICON_EXCLAMATION );
         return( zCALL_ERROR );
      }
   }
...
_TZ_BufferDelete( DWORD eventnum, HBUFFER hBuffer )
{
   ...
   if ( nRC == 0 )
   {
      ...
      zstrcpy( sz, "TZEDWRKO current buffer not found for Delete!." );
      fnEditorMsg( sz, 1, 0, 0, zICON_EXCLAMATION );
      if ( hCurrentBuffer )
         BufSwitchBuffer( hCurrentBuffer );
      return( 0 );
...
_TZ_BufferSaved( DWORD eventnum )
{
                                  ...
                                  szlVML_File ) == 0 )
   {
      ...
      szFileName = BufQFilename ();
      zstrcpy( sz, "File " );
      zstrcat( sz, szFileName );
      zstrcat( sz, " has been saved.  Parse file?" );
   ...
   while ( nRC == zCURSOR_SET )
   {
      ...
      if ( *szStr )
      {
         if ( bVML_File )
         {
            switch ( *szStr )
            {
            case 'A': zstrcat( szTemp, "\"ATTRIBUTE_NAME\"" );
                        break;
            case 'E': zstrcat( szTemp, "\"ENTITY_NAME\"" );
                        break;
            case 'L': zstrcat( szTemp, "INTEGER" );
                        break;
            case 'M': zstrcat( szTemp, "DOUBLE" );
                        break;
            case 'N': zstrcat( szTemp, "INTEGER" );
                        break;
            case 'O': zstrcat( szTemp, "VOID" );
                        break;
            case 'S': zstrcat( szTemp, "STRING" );
                        break;
            case 'V': zstrcat( szTemp, "VIEW" );
                        break;
            case 'Y': zstrcat( szTemp, "STRING" );
                        break;
            default : zstrcat( szTemp, szStr );
                        break;
            } // switch
         }
         else
         {
            ...
            case 'E': zstrcat( szTemp, "\"ENTITY_NAME\"" );
                        break;
            case 'L': zstrcat( szTemp, "zLONG" );
                        break;
            case 'M': zstrcat( szTemp, "DOUBLE" );
                        break;
            case 'N': zstrcat( szTemp, "zSHORT" );
                        break;
            case 'O': zstrcat( szTemp, "void" );
                        break;
            case 'S': zstrcat( szTemp, "zCHAR" );
                        break;
            case 'V': zstrcat( szTemp, "zVIEW" );
                        break;
            case 'Y': zstrcat( szTemp, "zCHAR" );
                        break;
            default : zstrcat( szTemp, szStr );
                        break;
            } // switch
...
void OPERATION tbNextError( BOOL bShowErrorFile )
{
   ...
   zstrcpy( szErrorFile, pszFilename );
   zstrcat( szErrorFile, ".ERR" );
   StrFree( pszFilename );
