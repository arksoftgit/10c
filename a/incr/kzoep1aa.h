...
typedef ProcessRecord zFAR *LPPROCESS;

typedef struct TaskStruct TaskRecord;
typedef TaskRecord zFAR *LPTASK;

typedef struct MutexStruct MutexRecord;
typedef MutexRecord zFAR *LPMUTEX;
...
typedef NetConnectionRecord zFAR *LPNETCONN;

typedef struct NetworkStruct NetworkRecord;
typedef NetworkRecord zFAR *LPNETWORK;
...
#ifndef zCOREMEM
#define zCOREMEM char zFAR * zFAR *
#endif
...
void OPERATION
SysGetDateTime( zPCHAR pchReturnDateTime );
...
void OPERATION
SysGetFileDateTime( zLONG hFile, zPCHAR pchDateTime );
...
void  OPERATION
SysParseLine( zPCHAR PortName, zCOREMEM PortValue, zPCHAR InputString );
...
zSHORT OPERATION
SfGetApplicationForSubtask( LPAPP zFAR *lpApp, zVIEW vAnyView );
...
zSHORT OPERATION
SfGetTaskInfo( zPCHAR pchReturnString, zSHORT nInfoRequest, LPTASK lpTask );
