...
#if 0
   The following are the states set by the driver.  The default message
   processing does use them to differentiate processing.
...
#define zMSGQ_DialogOperation                  4L
   ...
   The following are the states set by the whomever.  They may be relevant
   to some applications, but the default message processing does not take
   advantage of them.
                        ...
                        zSHORT   bBeep )
{
   ...
   UfFormatDateTime( szDateTime, szTimeStamp, "YYYY-mm-DD-HH.MI.SS.999" );

   // Setup timestamped message buffer.
   zstrcpy( szTSMessage, szDateTime );
   zstrcat( szTSMessage, "\n" );
   ...
   zstrcat( szTSMessage, cpcMsgText );
                  ...
                  zSHORT   bBeep )
{
   ...
   // 1. A message object named LAND.MSG is activated, if it exists, for
   //    overriding messages with German equivalents.  If the object is
   //    found and there is a match on Id, the new message attributes are used.
   // 2. OperatorPrompt is used to send the message, since we can control the
   //    icon on the message box with that operation.
   ...
   {
      GetStringFromAttribute( pchMsgText,  vMsg, "OverrideMsg", "Text" );
      // If the original message contains '\n', then we want to add
      // everything after the \n.
      // This is done, because special information is added (Domain Messages),
      // separated by \n. We do not want to loose this information.
      AddSpecialInfo( pchMsgText, cpcMsgText, sizeof( pchMsgText ) );
      GetStringFromAttribute( szMsgTitle, vMsg, "OverrideMsg", "Title" );
   }
   else                       // No override message was found.
   {
      zstrncpy( pchMsgText,  cpcMsgText, sizeof( pchMsgText ) );
      zstrncpy( szMsgTitle, cpcTitle, sizeof( szMsgTitle ) );
   }

   zstrcat( szTSMessage, pchMsgText );
   TraceLineS( "", szTSMessage );

#ifdef NO_DRIVER
   SysMessageBox( szMsgTitle, pchMsgText, 1 );
   return( zRESPONSE_OK );
#else
   switch( lMsgType )
   {
      case zMSGQ_DOMAIN_ERROR:
         return( OperatorPrompt( vSubtask, "Domain Validation Error",
                                 pchMsgText, FALSE, 0, 0, zICON_STOP ) );

      case zMSGQ_OBJECT_CONSTRAINT_ERROR:
         return( OperatorPrompt( vSubtask, szMsgTitle,
                                 pchMsgText, FALSE, 0, 0, zICON_STOP  ) );

      case zMSGQ_MODAL_ERROR:
         return( OperatorPrompt( vSubtask, szMsgTitle,
                                 pchMsgText, FALSE, 0, 0, zICON_ERROR ) );

      case zMSGQ_OBJECT_CONSTRAINT_WARNING:
         return( OperatorPrompt( vSubtask, szMsgTitle,
                                 pchMsgText, FALSE, 0, 0, zICON_EXCLAMATION ) );

      case zMSGQ_OBJECT_CONSTRAINT_INFORMATION:
         return( OperatorPrompt( vSubtask, szMsgTitle,
                                 pchMsgText, FALSE, 0, 0, zICON_INFORMATION ) );
      default:
                ...
                zSHORT   bBeep )
{
   ...
   if ( SfCurrentTaskIsServer( vSubtask ) )
      nRC = fnMQ_MessageSendServer( vMsgQ, vSubtask, cpcMsgId, cpcTitle,
                                    cpcMsgText, lMsgType, bBeep );
   else
      nRC = fnMQ_MessageSend( vMsgQ, vSubtask, cpcMsgId, cpcTitle,
                              cpcMsgText, lMsgType, bBeep );
                  ...
                  zSHORT   nIcon )
{
   ...
   TraceLine( "MessagePrompt: %s %s %s", cpcMsgId, szMsgTitle, pchMsgText );

#ifdef NO_DRIVER
   SysMessageBox( "Zeidon Message Error", "Can't get prompt without driver", 1 );
   nRC = zCALL_ERROR;
#else
   nRC = OperatorPrompt( vSubtask, szMsgTitle, pchMsgText, bBeep,
                          ...
                          zUSHORT  usBufferLth )
{
   ...
   return( zCALL_ERROR );
#else
   return( OperatorPromptForInput( vSubtask, cpcTitle, cpcMsgText, bBeep,
                                   lpchReturnBuffer, usBufferLth ) );
#endif
}
                   ...
                   zLONG    lEventType )
{
        ...
        nRC = DeleteEntity( vMsgQ, "QMsg", zREPOS_NEXT ) )
   {
      ...
      // We'll just use MessageSend to display the message.  NOTE that at
      // the moment we don't too anything with lState.
      MQ_MessageSend( vMsgQ, vSubtask, cpcId, cpcTitle, cpcMsgText,
                      lMsgType, TRUE );
   }
...
// if the original message contains '\n', then we assume that everything
// following was added as special information, which must not be lost
// when a message is overwritten from land.msg.
static zVOID
AddSpecialInfo( zPCHAR pchMsgText, zCPCHAR szOrigText, zULONG nLth )
{
   zPCHAR pch;
   if ( (pch = strchr( szOrigText, '\n' )) != 0 )
   {
      zstrncat( pchMsgText, pch, nLth - zstrlen( pchMsgText ) );
   }
}
